/**
 * NeurologyConsultationsDocument.jsx
 * March 2026 — Canonical one-pass polish, inline editing, blue glow theme
 * Collection: neurology_consultations
 *
 * 7 Sections:
 *   1. clinical-info: consultDate (BlueDatePicker), referralReason, neurologicComplaint,
 *                     symptomOnset, symptomProgression
 *   2. neurologic-history: neurologicHistory (label == section title → suppressed)
 *   3. neuro-exams: mentalStatusExam, cranialNerveExam, motorExam, sensoryExam
 *   4. exam-findings: reflexes, coordination, gait
 *   5. diagnostic-studies: imagingReviews, eegFindings, emgFindings, lpFindings, strokeScale
 *   6. assessment-plan: neurologicDiagnosis, treatmentPlan, medicationsAdjusted,
 *                       therapyRecommendations, followUpPlan
 *   7. recommendations: recommendations — array of {recommendation, date}; each object decomposes
 *                       into a nested-subtitle (recommendation text) + a formatted date row (BlueDatePicker)
 *
 * NOTE: consultDate is a real date FIELD (rendered via BlueDatePicker inside clinical-info). The
 *       record has ONLY createdAt/updatedAt ingestion timestamps → NO record-header date pill (title only).
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NeurologyConsultationsDocumentPDFTemplate from '../pdf-templates/NeurologyConsultationsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './NeurologyConsultationsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'neurology_consultationsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = [
  'referralReason', 'neurologicComplaint', 'symptomOnset', 'symptomProgression',
  'neurologicHistory', 'mentalStatusExam', 'cranialNerveExam', 'motorExam', 'sensoryExam',
  'reflexes', 'coordination', 'gait',
  'imagingReviews', 'eegFindings', 'emgFindings', 'lpFindings', 'strokeScale',
  'neurologicDiagnosis', 'treatmentPlan', 'medicationsAdjusted', 'therapyRecommendations', 'followUpPlan',
];
...
/* ═══════ COMPONENT ═══════ */
const NeurologyConsultationsDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.document?.neurology_consultations) return Array.isArray(r.document.neurology_consultations) ? r.document.neurology_consultations : [r.document.neurology_consultations];
      if (r?.neurology_consultations) return Array.isArray(r.neurology_consultations) ? r.neurology_consultations : [r.neurology_consultations];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.neurology_consultations) return Array.isArray(dd.neurology_consultations) ? dd.neurology_consultations : [dd.neurology_consultations]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Neurology Consultation ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/neurology_consultations/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/neurology_consultations/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NEUROLOGY CONSULTATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Neurology Consultation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="neurology-consultations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Neurology Consultations</h2></div>
        <div className="empty-state">No neurology consultation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="neurology-consultations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Neurology Consultations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NeurologyConsultationsDocumentPDFTemplate document={pdfData} />} fileName="Neurology_Consultations.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search neurology consultations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Neurology Consultation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'clinical-info')}
            {renderSection(record, idx, 'neurologic-history')}
            {renderSection(record, idx, 'neuro-exams')}
...
export default NeurologyConsultationsDocument;
