/**
 * MedicationTherapyManagementDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: medication_therapy_management
 *
 * 8 Sections:
 *   1. session-overview: medicationTherapyManagementSessionType (string),
 *          comprehensiveMedicationReviewDate (date), totalMedicationCount (num),
 *          medicationAdherenceRate (num), polypharmacyRiskCategory (string)
 *   2. drug-therapy-problems: drugTherapyProblemsIdentified (array)
 *   3. drug-safety: potentialDrugInteractions (array), beersCriteriaViolations (array),
 *          stompCriteriaAssessment (array), anticholinergicBurdenScore (num),
 *          therapeuticDuplicationIdentified (array), highRiskMedicationsPresent (array)
 *   4. action-plan: medicationRelatedActionPlan (array)
 *   5. dose-adjustments: renalDoseAdjustmentRequired (bool), hepaticDoseAdjustmentRequired (bool)
 *   6. interventions-quality: targetedInterventionCategories (array),
 *          starRatingMeasuresAddressed (array), immunizationGapsIdentified (array),
 *          prescriberNotificationsSent (num)
 *   7. cost-savings: costSavingsOpportunities (array), estimatedAnnualMedicationCost (num)
 *   8. outcome-followup: mtmOutcomeDocumentation (string), nextMtmReviewDueDate (date)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MedicationTherapyManagementDocumentPDFTemplate from '../pdf-templates/MedicationTherapyManagementDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './MedicationTherapyManagementDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (value can be a string or a whole array) */
const DRAFT_KEY = 'medication_therapy_managementPendingEdits';
const readDrafts = () => {
...
const NUMBER_FIELDS = [
  'totalMedicationCount', 'medicationAdherenceRate',
  'anticholinergicBurdenScore',
  'prescriberNotificationsSent',
  'estimatedAnnualMedicationCost',
];

const BOOLEAN_FIELDS = [
  'renalDoseAdjustmentRequired', 'hepaticDoseAdjustmentRequired',
];

const STRING_FIELDS = [
  'medicationTherapyManagementSessionType', 'polypharmacyRiskCategory',
  'mtmOutcomeDocumentation',
];
...
/* ======= COMPONENT ======= */
const MedicationTherapyManagementDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.medication_therapy_management) return Array.isArray(r.medication_therapy_management) ? r.medication_therapy_management : [r.medication_therapy_management];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.medication_therapy_management) return Array.isArray(dd.medication_therapy_management) ? dd.medication_therapy_management : [dd.medication_therapy_management]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Medication Therapy Management ${idx + 1}`.toLowerCase();
  ...
  const pdfData = useMemo(() => {
    return filteredRecords.map((record, idx) => {
      ...
      // Array fields: only merge the staged edit when it is NOT a pending draft
      ARRAY_FIELDS.forEach(f => {
        if (pendingEdits[`${f}-${idx}`]) { merged[f] = Array.isArray(record[f]) ? record[f] : []; return; }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // pending drafts for THIS record + THIS section (keys are "field-idx"; value may be string or array)
    ...
    try {
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/medication_therapy_management/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/medication_therapy_management/${id}/approve`, { sectionId: sid, approved: true });
      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this record's committed drafts from localStorage
      const store = readDrafts();
      if (store[id]) { toCommit.forEach(k => { const fp = k.slice(0, -suffix.length); const bf = fp.includes('.') ? fp.split('.')[0] : fp; delete store[id][bf]; }); if (Object.keys(store[id]).length === 0) delete store[id]; writeDrafts(store); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MEDICATION THERAPY MANAGEMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Medication Therapy Management ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; stageDraft(record, fn, idx, sid, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="medication-therapy-management-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Medication Therapy Management</h2></div>
        <div className="empty-state">No medication therapy management records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="medication-therapy-management-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Medication Therapy Management</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MedicationTherapyManagementDocumentPDFTemplate document={pdfData} />} fileName="Medication_Therapy_Management.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search medication therapy management..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Medication Therapy Management ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-overview')}
            {renderSection(record, idx, 'drug-therapy-problems')}
            {renderSection(record, idx, 'drug-safety')}
            {renderSection(record, idx, 'action-plan')}
            {renderSection(record, idx, 'dose-adjustments')}
            {renderSection(record, idx, 'interventions-quality')}
            {renderSection(record, idx, 'cost-savings')}
            {renderSection(record, idx, 'outcome-followup')}
          </div>
        ))}
      </div>
    </div>
...
export default MedicationTherapyManagementDocument;
