...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'pulmonary-function-tests-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'testDate', 'provider', 'technician', 'qualityGrade',
  'preBronchodilator', 'preBronchodilator.fev1', 'preBronchodilator.fev1.percentPredicted',
  'preBronchodilator.fvc', 'preBronchodilator.fvc.percentPredicted',
  'preBronchodilator.fev1FvcRatio',
  'postBronchodilator', 'postBronchodilator.fev1', 'postBronchodilator.fev1.percentPredicted',
  'postBronchodilator.fev1.percentChange',
  'interpretation', 'reversibility',
  'lungVolumes', 'comprehensiveLungVolumes',
  'dlco', 'dlcoComprehensive',
  'bronchodilatorResponse',
  'sixMinuteWalkTest', 'cardiopulmonaryExerciseTest',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('pulmonary_function_tests', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[pulmonary_function_tests] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('pulmonary_function_tests', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[pulmonary_function_tests] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
