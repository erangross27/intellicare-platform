/**
 * PostDialysisAssessmentDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: post_dialysis_assessment
 *
 * 8 Sections:
 *   1. provider-details: createdAt
 *   2. session-parameters: dialysisSessionDuration, dialyzerType, bloodFlowRate, dialysateFlowRate, anticoagulationProtocol
 *   3. vital-signs: preDialysisSystolicBP, postDialysisSystolicBP, preDialysisDiastolicBP, postDialysisDiastolicBP, preDialysisWeight, postDialysisWeight
 *   4. fluid-management: fluidRemovalVolume, ultrafiltrationRate, dryWeightAssessment
 *   5. dialysis-adequacy: ktVRatio, ureaReductionRatio
 *   6. vascular-access: accessType, accessFunctionStatus
 *   7. complications-symptoms: intradialyticHypotensionOccurred, muscleCrampsReported, disequilibriumSyndromeSymptoms, sessionComplications
 *   8. recovery-followup: electrolyteLevelsPostDialysis, residualRenalFunction, postDialysisRecoveryTime
 */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" — arrays store full array value) */
const DRAFT_KEY = 'post_dialysis_assessmentPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'complications-symptoms': 'Complications & Symptoms',
  'recovery-followup': 'Recovery & Follow-up',
};
...
const SECTION_FIELDS = {
  ...
  'dialysis-adequacy': ['ktVRatio', 'ureaReductionRatio'],
  'vascular-access': ['accessType', 'accessFunctionStatus'],
  'complications-symptoms': ['intradialyticHypotensionOccurred', 'muscleCrampsReported', 'disequilibriumSyndromeSymptoms', 'sessionComplications'],
  'recovery-followup': ['electrolyteLevelsPostDialysis', 'residualRenalFunction', 'postDialysisRecoveryTime'],
};
...
const PostDialysisAssessmentDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Post Dialysis Assessment ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Staged drafts for this section live under localEdits["<field>-<idx>"] (full value; arrays stored whole)
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/post_dialysis_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/post_dialysis_assessment/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== POST DIALYSIS ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Post Dialysis Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="post-dialysis-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Post Dialysis Assessment</h2></div>
        <div className="empty-state">No post dialysis assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="post-dialysis-assessment-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search post dialysis assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'dialysis-adequacy')}
            {renderSection(record, idx, 'vascular-access')}
            {renderSection(record, idx, 'complications-symptoms')}
            {renderSection(record, idx, 'recovery-followup')}
          </div>
        ))}
      </div>
    </div>
...
export default PostDialysisAssessmentDocument;
