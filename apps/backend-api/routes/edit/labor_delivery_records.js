...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'labor-delivery-records-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'admissionDateTime', 'deliveryDateTime', 'gestationalAge', 'laborType', 'deliveryType', 'presentations',
  'anesthesia', 'episiotomy', 'lacerations', 'bloodLoss',
  'complications',
  'placentaDelivery.mechanism', 'placentaDelivery.deliveredAt', 'placentaDelivery.durationThirdStage', 'placentaDelivery.examination', 'placentaDelivery.cordClamping',
  'newborn.sex', 'newborn.birthWeight', 'newborn.length', 'newborn.headCircumference', 'newborn.apgar1Minute', 'newborn.apgar5Minutes',
  'newborn.cordBloodGases',
];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('labor_delivery_records', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[labor_delivery_records] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('labor_delivery_records', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[labor_delivery_records] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
