/**
 * ApgarScoresDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: apgar_scores
 *
 * 5 Sections:
 *   1. birth-info: birthDate, birthTime, assessor
 *   2. apgar-scores: apgar1Minute, apgar5Minutes, apgar10Minutes
 *   3. component-details: appearance, pulse, grimace, activity, respiration
 *   4. interventions: interventions
 *   5. recommendations: recommendations (array of objects with recommendation+date)
 */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = localEdits key without "-idx" suffix) */
const DRAFT_KEY = 'apgar_scoresPendingEdits';
const readDrafts = () => {
...
/* COMPONENT_FIELDS: APGAR component scores whose value is semicolon-separated timepoints
   ("1 minute: X (...); 5 minutes: Y (...)"). Rendered as nested-subtitle (component) +
   one nested-mini-card per timepoint (sub-label "1 minute" + value). */
...
/* splitFirstColon: split a timepoint "1 minute: 1 (...)" into { tlabel:'1 minute', value:'1 (...)' }.
   Splits on the FIRST colon only — handles digit-leading labels that parseLabel rejects, and keeps
   inner colons (e.g. "Below 100 bpm: ~80 bpm") inside the value. */
...
const ApgarScoresDocument = ({ document: docProp, data, templateData }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `APGAR Score ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const toCommit = Object.keys(localEdits).filter(k => {
      ...
      const fieldPart = k.slice(0, -suffix.length); // "field" (no numeric arrayIndex suffix in this template)
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/apgar_scores/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/apgar_scores/${id}/approve`, { sectionId: sid, approved: true });
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); const id3 = safeId(record); if (!id3) return; setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullText })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); clearApprovedForField(fn, idx); stageDraft(id3, fn, fullText); setEditingField(null); setEditValue(''); setSaveError(null); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  const renderRecommendationsField = (record, idx, sid) => {
    ...
    const recs = Array.isArray(val) ? val.filter(r => r && (typeof r === 'object' ? r.recommendation : r)) : [];
    ...
    return (
      <div key="recommendations" className="rec-mini-card">
        ...
        {recs.map((rec, ri) => {
          ...
          const editKey = `recommendations.${ri}-${idx}`;
          const isEditing = editingField === editKey;
          const isModified = editedFields[editKey];
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(recText); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentArr = [...(Array.isArray(getFieldValue(record, 'recommendations', idx)) ? getFieldValue(record, 'recommendations', idx) : [])]; const updatedRec = typeof currentArr[ri] === 'object' ? { ...currentArr[ri], recommendation: editValue } : editValue; currentArr[ri] = updatedRec; setLocalEdits(prev => ({ ...prev, [`recommendations-${idx}`]: currentArr })); setPendingEdits(prev => ({ ...prev, [`recommendations-${idx}`]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); clearApprovedForField('recommendations', idx); stageDraft(id, 'recommendations', currentArr); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <>
                    <div className="row-content">
                      ...
                      {recDate && <span style={{ color: '#60a5fa', marginLeft: 8, fontSize: 14 }}>({formatDate(recDate)})</span>}
                      <span className="edit-indicator">&#9998;</span>
                    </div>
                    <button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${recText}${recDate ? ` (${formatDate(recDate)})` : ''}`, editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button>
                  </>
                )}
              </div>
              {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
  ...
  const renderComponentField = (record, fn, idx, sid) => {
    ...
    const items = splitBySemicolon(strVal).map(splitFirstColon); // [{tlabel:'1 minute', value:'1 (...)'}, ...]
    const phraseMatch = !searchTerm.trim() || sectionTitleMatches(sid) || record._showAllSections;
    const phrase = searchTerm.toLowerCase().trim();
    const labelMatch = searchTerm.trim() && label.toLowerCase().includes(phrase);

    const visible = items.map((it, i) => ({ it, i })).filter(({ it }) =>
      phraseMatch || labelMatch || `${it.tlabel} ${it.value}`.toLowerCase().includes(phrase));
    ...
    return (
      <div key={fn} className="rec-mini-card">
        <div className="nested-subtitle">{highlightText(label)}</div>
        {visible.map(({ it, i }) => {
          const tpKey = `${fn}-${idx}-t${i}`;
          const isEditing = editingField === tpKey;
          const badge = editedSentences[tpKey];
          const saveTp = () => {
            const id = safeId(record); if (!id) return;
            const cur = splitBySemicolon(String(getFieldValue(record, fn, idx) || ''));
            const newVal = editValue.trim();
            cur[i] = it.tlabel ? `${it.tlabel}: ${newVal}` : newVal;
            const fullText = cur.join('; ');
            setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullText }));
            setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true }));
            setEditedSentences(prev => ({ ...prev, [tpKey]: 'edited' }));
            clearApprovedForField(fn, idx);
            stageDraft(id, fn, fullText);
            setEditingField(null); setEditValue(''); setSaveError(null);
          };
              ...
              <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(tpKey); setEditValue(it.value); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveTp(); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <>
                    <div className="row-content"><span className="content-value">{highlightText(it.value)}</span><span className="edit-indicator">&#9998;</span></div>
                    <button className={`copy-btn ${copiedItems[tpKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(it.tlabel ? `${it.tlabel}: ${it.value}` : it.value, tpKey); }}>{copiedItems[tpKey] ? 'Copied!' : 'Copy'}</button>
                  </>
                )}
              </div>
              {badge && <span className="modified-badge">edited - click Pending Approve to save</span>}
  ...
  /* ═══════ RENDER: APGAR SCORE SUMMARY BAR CHART ═══════ */
  const renderApgarChart = (record, idx) => {
    const s1 = getFieldValue(record, 'apgar1Minute', idx);
    const s5 = getFieldValue(record, 'apgar5Minutes', idx);
    const s10 = getFieldValue(record, 'apgar10Minutes', idx);
    const scores = [
      { label: '1 Minute', value: s1 },
      { label: '5 Minutes', value: s5 },
      { label: '10 Minutes', value: s10 },
    ].filter(s => hasVal(s.value));
    if (scores.length === 0) return null;

    if (searchTerm.trim() && !record._showAllSections) {
      const phrase = searchTerm.toLowerCase().trim();
      const searchable = ['apgar score summary', 'apgar', 'score', 'summary', ...scores.map(s => `${s.label}: ${s.value}`)].join(' ').toLowerCase();
      if (!searchable.includes(phrase) && !phrase.split(' ').every(w => searchable.includes(w))) return null;
    }

    const getScoreColor = (score) => {
      const n = parseInt(score, 10);
      if (isNaN(n)) return '#60a5fa';
    ...
    return (
      <div className="section">
        <div className="mini-cards-container">
          <div className="section-header">
            <h4 className="section-title">{highlightText('APGAR Score Summary')}</h4>
            <div className="header-right-actions">
              <button className={`copy-btn ${copiedSection === `chart-${idx}` ? 'copied' : ''}`} onClick={() => copySection(scores.map(s => `${s.label}: ${s.value}/10`).join('\n'), `chart-${idx}`)}>{copiedSection === `chart-${idx}` ? 'Copied!' : 'Copy Section'}</button>
            </div>
          </div>
          <div className="chart-container">
            {scores.map((s, si) => {
              ...
              return (
                <div key={si} className="bar-chart-item" style={{ marginBottom: si < scores.length - 1 ? 12 : 0 }}>
                  <div className="bar-label">{highlightText(s.label)}</div>
                  <div className="bar-wrapper">
                    <div className="bar-background">
                      <div className="bar-fill" style={{ width: `${percentage}%`, backgroundColor: getScoreColor(s.value) }} />
                    </div>
                    <span className="bar-value">{highlightText(`${s.value}/10`)}</span>
                  </div>
                </div>
              );
            })}
  ...
  if (!records || records.length === 0) {
    return (
      <div className="apgar-scores-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">APGAR Scores</h2></div>
        <div className="empty-state">No APGAR score records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="apgar-scores-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search APGAR scores..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'component-details')}
...
export default ApgarScoresDocument;
