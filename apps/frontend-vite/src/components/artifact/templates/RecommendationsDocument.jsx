/**
 * RecommendationsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: recommendations
 *
 * 4 Sections:
 *   1. recommendation-details: __simpleType, medication, dosage, route, frequency, duration
 *   2. indication-priority: indication, priority
 *   3. provider-date: provider, date
 *   4. processing: source, aiProcessed
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import RecommendationsDocumentPDFTemplate from '../pdf-templates/RecommendationsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './RecommendationsDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'recommendation-details': 'Recommendation Details',
  'indication-priority': 'Indication & Priority',
  'provider-date': 'Provider & Date',
  'processing': 'Processing',
};

const FIELD_LABELS = {
  __simpleType: 'Recommendation',
...
const SECTION_FIELDS = {
  'recommendation-details': ['__simpleType', 'medication', 'dosage', 'route', 'frequency', 'duration'],
  'indication-priority': ['indication', 'priority'],
  'provider-date': ['provider', 'date'],
  'processing': ['source', 'aiProcessed'],
};

const BOOLEAN_FIELDS = ['aiProcessed'];
const DATE_FIELDS = ['date'];
const NUMBER_FIELDS = [];
const STRING_FIELDS = ['__simpleType', 'medication', 'dosage', 'route', 'frequency', 'duration', 'indication', 'priority', 'provider', 'source'];
...
const DRAFT_KEY = 'recommendationsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const RecommendationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.recommendations) return Array.isArray(r.recommendations) ? r.recommendations : [r.recommendations];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.recommendations) return Array.isArray(dd.recommendations) ? dd.recommendations : [dd.recommendations]; return [dd]; }
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Recommendation ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Staged drafts for THIS section's fields (editKey = "<fieldPart>-<idx>", fieldPart = "field" or "field.arrayIndex")
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/recommendations/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/recommendations/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (store[id]) {
        toCommit.forEach(editKey => {
          ...
          const lastDot = fieldPart.lastIndexOf('.');
          const baseField = (lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1))) ? fieldPart.slice(0, lastDot) : fieldPart;
          if (store[id][baseField] !== undefined) delete store[id][baseField];
        });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="recommendations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Recommendations</h2></div>
        <div className="empty-state">No recommendations records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="recommendations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Recommendations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<RecommendationsDocumentPDFTemplate document={pdfData} />} fileName={`recommendations-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search recommendations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(record.medication || record.__simpleType?.substring(0, 60) || `Recommendation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'recommendation-details')}
            {renderSection(record, idx, 'indication-priority')}
            {renderSection(record, idx, 'provider-date')}
...
export default RecommendationsDocument;
