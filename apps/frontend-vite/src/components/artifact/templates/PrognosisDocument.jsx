 ...
 *   2. short-term: shortTerm
 *   3. long-term: longTerm
 *   4. risk-factors: riskFactors (array)
 *   5. protective-factors: protectiveFactors (array)
 *   6. motivation: motivationFactors
 *   7. prev-treatment: previousTreatmentResponse
 *   8. insight: insightLevel
 *   9. assessment: assessment, findings, plan
 *  10. outcomes: mortality, functionalStatus
 *  11. recommendations: recommendations (array of {recommendation,date} or strings)
 *  12. results: results (dynamic-key object)
 *  13. notes: notes
 */
...
const FIELD_LABELS = {
  ...
  facility: 'Facility',
  status: 'Status',
  shortTerm: 'Short-Term Prognosis',
  longTerm: 'Long-Term Prognosis',
...
/* humanizeKey: camelCase / snake_case object key → readable label */
const humanizeKey = (key) => {
  if (!key && key !== 0) return '';
  return String(key).replace(/_/g, ' ').replace(/([a-z\d])([A-Z])/g, '$1 $2').replace(/([A-Z]+)([A-Z][a-z])/g, '$1 $2').replace(/\b\w/g, c => c.toUpperCase()).trim();
};

/* flattenItem: flatten an array item ({recommendation,date} object or string) into one readable line */
...
/* ═══════ COMPONENT ═══════ */
const PrognosisDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Prognosis ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Collect this record's pending edits for this section's fields (localEdits keys = "field-idx")
      ...
      for (const key of toCommit) {
        const m = key.match(/^(.+)-(\d+)$/);
        const fieldPart = m[1]; // "field" or "field.arrayIndex"
        ...
        const resp = await secureApiClient.put(`/api/edit/prognosis/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/prognosis/${id}/approve`, { sectionId: sid, approved: true });
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; const draftKey = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [draftKey]: currentArr })); setPendingEdits(prev => ({ ...prev, [draftKey]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const store = readDrafts(); if (!store[id]) store[id] = {}; store[id][fn] = currentArr; writeDrafts(store); setSaveError(null); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  /* ═══════ RENDER: OBJECT FIELD (dynamic-key object, e.g. results — stacked key sub-label + value) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="prognosis-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Prognosis</h2></div>
  ...
  return (
    <div className="prognosis-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Prognosis</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PrognosisDocumentPDFTemplate document={pdfData} />} fileName="Prognosis.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search prognosis..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Prognosis ${idx + 1}`)}</h3>
            </div>
