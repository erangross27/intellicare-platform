/**
 * LiverFunctionAssessments Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/liver_function_assessments/:id/edit    — Edit a single field
 * PUT /api/edit/liver_function_assessments/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'liver-function-assessments-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'totalBilirubin',
  'directConjugatedBilirubin',
  'indirectUnconjugatedBilirubin',
  'albumin',
  'prothrombinTimeInr',
  'childPughScore',
  'meldScore',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('liver_function_assessments', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[LiverFunctionAssessments] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('liver_function_assessments', { _id: objectId }, {
      $set: {
    ...
    console.error('[LiverFunctionAssessments] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
