/**
 * HospitalAdmissionNotesDocument.jsx
 * March 2026 — Canonical one-pass polish, inline editing, blue glow theme
 * Collection: hospital_admission_notes
 *
 * 5 Sections:
 *   1. admission-info: admissionDate (BlueDatePicker), admissionTime, admissionSource, admissionUnit,
 *                      admittingProvider, attendingPhysician
 *   2. diagnosis-info: admittingDiagnosis, secondaryDiagnoses, chiefComplaint (first-person quote)
 *   3. clinical-details: historyPresentIllness (long), admissionVitalSigns, admissionCondition, codeStatus
 *   4. orders-plan: admissionOrders (long)
 *   5. recommendations: recommendations — array of {recommendation, date}; each object decomposes into a
 *                       nested-subtitle (recommendation text) + a formatted date row (BlueDatePicker)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HospitalAdmissionNotesDocumentPDFTemplate from '../pdf-templates/HospitalAdmissionNotesDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './HospitalAdmissionNotesDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'hospital_admission_notesPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'diagnosis-info': 'Diagnosis & Chief Complaint',
  'clinical-details': 'Clinical Details',
  'orders-plan': 'Admission Orders & Plan',
...
const FIELD_LABELS = {
  ...
  admissionCondition: 'Admission Condition',
...
const SECTION_FIELDS = {
  'admission-info': ['admissionDate', 'admissionTime', 'admissionSource', 'admissionUnit', 'admittingProvider', 'attendingPhysician'],
  'diagnosis-info': ['admittingDiagnosis', 'secondaryDiagnoses', 'chiefComplaint'],
  'clinical-details': ['historyPresentIllness', 'admissionVitalSigns', 'admissionCondition', 'codeStatus'],
  'orders-plan': ['admissionOrders'],
  'recommendations': ['recommendations'],
};
...
const STRING_FIELDS = [
  'admissionTime', 'admissionSource', 'admissionUnit', 'admittingProvider', 'attendingPhysician',
  'admittingDiagnosis', 'secondaryDiagnoses', 'chiefComplaint',
  'historyPresentIllness', 'admissionVitalSigns', 'admissionCondition', 'codeStatus', 'admissionOrders',
];
...
/* ═══════ COMPONENT ═══════ */
const HospitalAdmissionNotesDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.hospital_admission_notes) return Array.isArray(r.hospital_admission_notes) ? r.hospital_admission_notes : [r.hospital_admission_notes];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.hospital_admission_notes) return Array.isArray(dd.hospital_admission_notes) ? dd.hospital_admission_notes : [dd.hospital_admission_notes]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Hospital Admission Notes ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/hospital_admission_notes/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/hospital_admission_notes/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HOSPITAL ADMISSION NOTES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Hospital Admission Notes ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="hospital-admission-notes-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Hospital Admission Notes</h2></div>
        <div className="empty-state">No hospital admission notes records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="hospital-admission-notes-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Hospital Admission Notes</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HospitalAdmissionNotesDocumentPDFTemplate document={pdfData} />} fileName="Hospital_Admission_Notes.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search hospital admission notes..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Hospital Admission Notes ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'admission-info')}
            {renderSection(record, idx, 'diagnosis-info')}
            {renderSection(record, idx, 'clinical-details')}
            {renderSection(record, idx, 'orders-plan')}
...
export default HospitalAdmissionNotesDocument;
