/**
 * Advance Directives Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/advance_directives/:id/edit    — Edit a single field
 * PUT /api/edit/advance_directives/:id/approve — Approve a section
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'advance-directives-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('advance_directives', { _id: objectId }, {
      $set: {
        [field]: value,
        updatedAt: new Date(),
    ...
    console.error('[AdvanceDirectives] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    console.error('[AdvanceDirectives] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
