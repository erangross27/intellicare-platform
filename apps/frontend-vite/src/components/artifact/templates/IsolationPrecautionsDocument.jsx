/**
 * IsolationPrecautionsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: isolation_precautions
 *
 * 7 Sections:
 *   1. order-info: date, orderingProviderId
 *   2. precaution-details: precautionType, initiationDateTime, discontinuationDateTime, isolationIndication, infectiousOrganism
 *   3. room-environment: roomType, roomNumber, airExchangeRate, doorSignageRequired, dedicatedEquipmentRequired, environmentalCleaningFrequency
 *   4. protective-measures: personalProtectiveEquipment, handHygieneProtocol
 *   5. patient-management: visitationRestrictions, transportationRequirements, cohortingAllowed, discontinuationCriteria
 *   6. notification-compliance: infectionPreventionistNotified, reviewDate, contactTracingRequired, publicHealthReportable, adherenceMonitoring, laboratorySpecimenHandling
 *   7. education: educationProvidedTo
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import IsolationPrecautionsDocumentPDFTemplate from '../pdf-templates/IsolationPrecautionsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './IsolationPrecautionsDocument.css';
...
const STRING_FIELDS = ['orderingProviderId', 'precautionType', 'isolationIndication', 'infectiousOrganism', 'roomType', 'roomNumber', 'environmentalCleaningFrequency', 'handHygieneProtocol', 'visitationRestrictions', 'transportationRequirements', 'discontinuationCriteria', 'adherenceMonitoring', 'laboratorySpecimenHandling'];
...
const DRAFT_KEY = 'isolation_precautionsPendingEdits';
const readDrafts = () => {
...
/* ======= COMPONENT ======= */
const IsolationPrecautionsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.isolation_precautions) return Array.isArray(r.isolation_precautions) ? r.isolation_precautions : [r.isolation_precautions];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.isolation_precautions) return Array.isArray(dd.isolation_precautions) ? dd.isolation_precautions : [dd.isolation_precautions]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Isolation Precautions ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/isolation_precautions/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/isolation_precautions/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== ISOLATION PRECAUTIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Isolation Precautions ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ======= RENDER: NUMBER FIELD — num-stepper (NEVER type=number); hides 0 ======= */
  const renderNumberField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx); if (!hasVal(val) || numHidden(fn, val)) return null;
    const editKey = `${fn}-${idx}`;
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      if (numHidden(f, val)) return false;
      return hasVal(val);
    });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="isolation-precautions-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Isolation Precautions</h2></div>
        <div className="empty-state">No isolation precautions records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="isolation-precautions-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Isolation Precautions</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<IsolationPrecautionsDocumentPDFTemplate document={pdfData} />} fileName="Isolation_Precautions.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search isolation precautions records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Isolation Precautions ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'order-info')}
            {renderSection(record, idx, 'precaution-details')}
            {renderSection(record, idx, 'room-environment')}
            {renderSection(record, idx, 'protective-measures')}
            {renderSection(record, idx, 'patient-management')}
            {renderSection(record, idx, 'notification-compliance')}
...
export default IsolationPrecautionsDocument;
