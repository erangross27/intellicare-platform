/**
 * PrepAndDrapeDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: prep_and_drape
 *
 * 6 Sections:
 *   1. procedure: procedureName
 *   2. prep-information: prepArea, prepSolution, prepMethod, prepTime, hairRemoval
 *   3. draping: drapeType, drapingMethod, inciseDrape
 *   4. verification: timeoutPerformed, sterilityVerified
 *   5. prep-team: prepTeam
 *   6. details: date, facility, notes, additionalData
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PrepAndDrapeDocumentPDFTemplate from '../pdf-templates/PrepAndDrapeDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './PrepAndDrapeDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; value is string/array/object) */
const DRAFT_KEY = 'prep_and_drapePendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['procedureName', 'prepArea', 'prepSolution', 'prepMethod', 'prepTime', 'hairRemoval', 'drapeType', 'drapingMethod', 'inciseDrape', 'timeoutPerformed', 'sterilityVerified', 'facility', 'notes'];

/* sameAsTitle: hide a field label that duplicates its section title */
const sameAsTitle = (label, sid) => (label || '').trim().toLowerCase() === (SECTION_TITLES[sid] || '').trim().toLowerCase();

/* stepFor: decimal-aware step size for the num-stepper */
...
/* ═══════ COMPONENT ═══════ */
const PrepAndDrapeDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Prep and Drape ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/prep_and_drape/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/prep_and_drape/${id}/approve`, { sectionId: sid, approved: true });
      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this record's committed drafts from localStorage (only the fields just committed)
      const store = readDrafts();
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PREP AND DRAPE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Prep and Drape ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT FIELD (additionalData — dynamic keys) ═══════ */
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(valueStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentObj = { ...(getFieldValue(record, fn, idx) || {}) }; currentObj[key] = editValue.trim(); setSaveError(null); const lk = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [lk]: currentObj })); setPendingEdits(prev => ({ ...prev, [lk]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const store = readDrafts(); if (!store[id]) store[id] = {}; store[id][fn] = currentObj; writeDrafts(store); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const id = safeId(record); if (!id) return; const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); setSaveError(null); const lk = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [lk]: fullText })); setPendingEdits(prev => ({ ...prev, [lk]: true })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const store = readDrafts(); if (!store[id]) store[id] = {}; store[id][fn] = fullText; writeDrafts(store); setEditingField(null); setEditValue(''); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="prep-and-drape-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Prep and Drape</h2></div>
        <div className="empty-state">No prep and drape records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="prep-and-drape-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Prep and Drape</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PrepAndDrapeDocumentPDFTemplate document={pdfData} />} fileName="Prep_And_Drape.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search prep and drape records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Prep and Drape ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'procedure')}
            {renderSection(record, idx, 'prep-information')}
            {renderSection(record, idx, 'draping')}
            {renderSection(record, idx, 'verification')}
            {renderSection(record, idx, 'prep-team')}
            {renderSection(record, idx, 'details')}
          </div>
        ))}
      </div>
    </div>
...
export default PrepAndDrapeDocument;
