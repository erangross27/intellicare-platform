/**
 * Dermatology Consultations Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/dermatology_consultations/:id/edit    — Edit a single field
 * PUT /api/edit/dermatology_consultations/:id/approve — Approve all pending edits
 */
...
// Allowed editable fields for dermatology_consultations
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    if (!objectId) {
      return res.status(400).json({ success: false, error: 'Invalid dermatology consultation ID' });
    }
    ...
    await sda.update('dermatology_consultations', { _id: objectId }, {
      $set: setFields,
    }, context);

    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) {
    console.error('[DermatologyConsultations] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    console.error('[DermatologyConsultations] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
