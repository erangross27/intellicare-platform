...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'pneumoperitoneum-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['procedureName', 'date', 'facility', 'accessMethod', 'accessLocation', 'initialPressure', 'targetPressure', 'maximumPressure', 'gasType', 'totalGasVolume', 'flowRate', 'insufflationTime', 'insufflationEquipment', 'desufflationMethod', 'visualInspection', 'adhesions', 'complications', 'verificationTests', 'specialConsiderations', 'notes'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('pneumoperitoneum', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[pneumoperitoneum] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('pneumoperitoneum', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[pneumoperitoneum] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
