/**
 * LymphNodeCytomorphologyDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: lymph_node_cytomorphology
 *
 * 4 Sections:
 *   1. specimen-info: specimenSite, aspirationTechnique, cellularityAssessment
 *   2. cell-morphology: lymphocyteMorphology, lymphoblastPercentage, reedSternbergCells,
 *                       follicularDendriticCells, tingibleBodyMacrophages, mitosisCount, apoptosisRate
 *   3. immunology-markers: immunophenotype, bcl2Expression, ki67Index, monoclonality, chromosomalAberrations
 *   4. pathology-findings: necrosisExtent, epithelioidCells, granulomatousInflammation, centralNecrosisPattern,
 *                          metastaticCells, primaryTumorOrigin, cytoplasmicInclusions, vascularInvasion
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import LymphNodeCytomorphologyDocumentPDFTemplate from '../pdf-templates/LymphNodeCytomorphologyDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './LymphNodeCytomorphologyDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field") */
const DRAFT_KEY = 'lymph_node_cytomorphologyPendingEdits';
const readDrafts = () => {
...
/* 3 number fields — ALL use input[type=number step=any] + parseFloat+isNaN→block save */
const NUMBER_FIELDS = ['lymphoblastPercentage', 'mitosisCount', 'ki67Index'];

/* 6 boolean fields — ALL use select Yes/No dropdown, convert to boolean on save */
...
/* 2 array fields — comma-separated display */
const ARRAY_FIELDS = ['immunophenotype', 'chromosomalAberrations'];

const STRING_FIELDS = ['specimenSite', 'aspirationTechnique', 'cellularityAssessment', 'lymphocyteMorphology', 'follicularDendriticCells', 'apoptosisRate', 'bcl2Expression', 'monoclonality', 'necrosisExtent', 'centralNecrosisPattern', 'primaryTumorOrigin', 'cytoplasmicInclusions'];
...
/* ═══════ COMPONENT ═══════ */
const LymphNodeCytomorphologyDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.lymph_node_cytomorphology) return Array.isArray(dd.lymph_node_cytomorphology) ? dd.lymph_node_cytomorphology : [dd.lymph_node_cytomorphology]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Lymph Node Cytomorphology ${idx + 1}`.toLowerCase();
  ...
  // refresh). NOT written to MongoDB and NOT shown in the PDF until the user clicks Approve (which
  // commits). Mirrors the gold MonitoringPlansDocument defer-save-until-approve pattern.
  const stageDraft = useCallback((record, fn, idx, sid, value, marks) => {
    const id = safeId(record); if (!id) return;
    const editKey = `${fn}-${idx}`;
    setLocalEdits(prev => ({ ...prev, [editKey]: value }));
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
    if (marks) setEditedSentences(prev => ({ ...prev, ...marks }));
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    stageDraft(record, fn, idx, sid, saveVal, { [trackKey]: 'edited' });
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // Collect this record's staged edits for this section (editKey = "field-idx"; whole-field replace).
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k]) return false;
        const m = k.match(/^(.+)-(\d+)$/);
        if (!m || parseInt(m[2], 10) !== idx) return false;
        const fieldPart = m[1];
        // Dot-suffix → arrayIndex ONLY when the segment after the LAST dot is purely numeric.
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/lymph_node_cytomorphology/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/lymph_node_cytomorphology/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (NUMBER_FIELDS.includes(f) && Number(val) === 0) return; // hide-zero numbers
      if (ARRAY_FIELDS.includes(f)) {
        const arr = Array.isArray(val) ? val : splitByComma(fmtVal(val));
        text += `${label}\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== LYMPH NODE CYTOMORPHOLOGY ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Lymph Node Cytomorphology ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD — −/+ stepper (type=text inputMode=decimal) + parseFloat+isNaN→block save; hide-zero ═══════ */
  const renderNumberField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx); if (!hasVal(val)) return null;
    if (Number(val) === 0) return null; // hide-zero numbers
    const editKey = `${fn}-${idx}`;
  ...
  /* ═══════ RENDER: ARRAY FIELD — comma-separated items, each editable ═══════ */
  const renderArrayField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx); if (!hasVal(val)) return null;
    const label = FIELD_LABELS[fn] || fn;
    const items = Array.isArray(val) ? val : splitByComma(fmtVal(val));
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, ciIdx) => {
          ...
          const ciMatches = phraseMatch || labelMatch || !searchTerm.trim() || String(item).toLowerCase().includes(searchTerm.toLowerCase().trim());
          if (!ciMatches && searchTerm.trim()) return null;
          return (
            <div key={ciIdx}>
              <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(String(item)); setSaveError(null); } }}>
                {ciEditing ? (
                  <div className="edit-field-container">
                    <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const currentArr = Array.isArray(getFieldValue(record, fn, idx)) ? [...getFieldValue(record, fn, idx)] : splitByComma(fmtVal(getFieldValue(record, fn, idx))); const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { currentArr.splice(ciIdx, 1, ...subParts); } else { currentArr[ciIdx] = trimmed.replace(/[;.]+$/, '').trim(); } setSaveError(null); const marks = { [commaKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-c${ciIdx + ei}`] = 'added'; stageDraft(record, fn, idx, sid, currentArr, marks); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <>
                    ...
                    <button className={`copy-btn ${copiedItems[commaKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(String(item), commaKey); }}>{copiedItems[commaKey] ? 'Copied!' : 'Copy'}</button>
                  </>
                )}
              </div>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="lymph-node-cytomorphology-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Lymph Node Cytomorphology</h2></div>
        <div className="empty-state">No lymph node cytomorphology records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="lymph-node-cytomorphology-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Lymph Node Cytomorphology</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<LymphNodeCytomorphologyDocumentPDFTemplate document={pdfData} />} fileName="Lymph_Node_Cytomorphology.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search lymph node cytomorphology..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Lymph Node Cytomorphology ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'specimen-info')}
            {renderSection(record, idx, 'cell-morphology')}
            {renderSection(record, idx, 'immunology-markers')}
...
export default LymphNodeCytomorphologyDocument;
