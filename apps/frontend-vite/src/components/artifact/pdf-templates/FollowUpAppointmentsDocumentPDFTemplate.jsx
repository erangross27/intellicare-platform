/**
 * FollowUpAppointmentsDocumentPDFTemplate.jsx
 * Box-free B&W LETTER (canonical, memory 6a2d6af6) — mirrors the JSX: real record.date (never createdAt),
 * status/type enum canonical, booleans Yes/No, values numbered ('1.' even singles), single-name label gate,
 * narratives split on [.;]. Multi-record: each appointment breaks to a new page. Rule #74: each field is ONE
...
const FollowUpAppointmentsDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].follow_up_appointments && Array.isArray(templateData[0].follow_up_appointments)) records = templateData[0].follow_up_appointments;
    else records = templateData;
  } else if (templateData && templateData.follow_up_appointments) {
    records = Array.isArray(templateData.follow_up_appointments) ? templateData.follow_up_appointments : [templateData.follow_up_appointments];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.follow_up_appointments ? (Array.isArray(dd.follow_up_appointments) ? dd.follow_up_appointments : [dd.follow_up_appointments]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Follow Up Appointments</Text></View>
        <Text style={styles.emptyState}>No follow up appointments data available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Follow Up Appointment ${idx + 1}`}</Text>
...
export default FollowUpAppointmentsDocumentPDFTemplate;
