/**
 * OrthopedicFollowupNotesDocument.jsx
 * March 2026 — Canonical one-pass polish: inline editing, blue glow theme, BlueDatePicker.
 * Collection: orthopedic_followup_notes (displayed as "Orthopedic Follow-Up Notes")
 *
 * Record `date` → editable record-header date pill (BlueDatePicker).
 * previousSurgeryDate / nextFollowupDate → in-section date FIELDS (BlueDatePicker).
 * All remaining fields are narrative strings.
 *
 * 7 Sections:
 *   1. record-info: date (header), provider, facility
 *   2. complaint-surgery: chiefComplaint, surgeryType, previousSurgeryDate
 *   3. exam-findings: painLevel, rangeOfMotion, strengthAssessment, gaitPattern, weightBearingStatus
 *   4. rehabilitation: rehabilitationProgress, assistiveDevices
 *   5. imaging: imagingResults
 *   6. treatment: medicationAdjustments, woundStatus, complications
 *   7. goals-disposition: functionalGoals, returnToWork, clinicalImprovement, nextFollowupDate
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import OrthopedicFollowupNotesDocumentPDFTemplate from '../pdf-templates/OrthopedicFollowupNotesDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './OrthopedicFollowupNotesDocument.css';
...
/* `date` renders as the editable record-header pill (NOT in the section body / Copy All).
   previousSurgeryDate + nextFollowupDate render as in-section BlueDatePicker date FIELDS. */
...
const DRAFT_KEY = 'orthopedic_followup_notesPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const OrthopedicFollowupNotesDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.orthopedic_followup_notes) return Array.isArray(r.orthopedic_followup_notes) ? r.orthopedic_followup_notes : [r.orthopedic_followup_notes];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.orthopedic_followup_notes) return Array.isArray(dd.orthopedic_followup_notes) ? dd.orthopedic_followup_notes : [dd.orthopedic_followup_notes]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Orthopedic Follow-Up Notes ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const field = m[1];
        await secureApiClient.put(`/api/edit/orthopedic_followup_notes/${id}/edit`, { field, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/orthopedic_followup_notes/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== ORTHOPEDIC FOLLOW-UP NOTES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Orthopedic Follow-Up Notes ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="orthopedic-followup-notes-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Orthopedic Follow-Up Notes</h2></div>
        <div className="empty-state">No orthopedic follow-up notes available</div>
      </div>
    );
  }
  ...
  return (
    <div className="orthopedic-followup-notes-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Orthopedic Follow-Up Notes</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OrthopedicFollowupNotesDocumentPDFTemplate document={pdfData} />} fileName="Orthopedic_Followup_Notes.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search orthopedic follow-up notes..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Orthopedic Follow-Up Notes ${idx + 1}`)}</h3>
            </div>
...
export default OrthopedicFollowupNotesDocument;
