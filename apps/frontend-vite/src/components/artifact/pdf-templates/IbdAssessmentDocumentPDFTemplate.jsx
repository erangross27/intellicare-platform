...
/**
 * IbdAssessmentDocumentPDFTemplate - box-free canonical (LETTER)
 * Config-driven from the JSX SECTION_ORDER/SECTION_TITLES/FIELD_LABELS/SECTION_FIELDS.
 * Renders EVERY populated field the JSX renders for JSX/PDF field parity.
 * No boxes: underline rules only (documentTitle 2 / recordTitle+sectionTitle 1 / fieldLabel 0.5).
 * Nested object (currentFlare / results) + arrays render via a generic recursive renderer;
...
const SECTION_ORDER = ['session-info', 'disease-characteristics', 'current-flare', 'medical-history', 'clinical-findings', 'assessment-plan', 'results-section', 'recommendations-section', 'notes-section'];
...
/* resolvePath: read a dotted SECTION_FIELDS key (e.g. currentFlare.duration) off a record */
...
/* recommendationRows: array of {recommendation, date}, date-grouped (subtitle date -> numbered items) */
const recommendationRows = (arr, kp) => {
  const recs = arr.filter(r => !isEmptyDeep(r));
  const groups = [];
  ...
  groups.forEach((group, gi) => {
    if (group.date) rows.push(<Text key={`${kp}-${gi}-d`} style={styles.subLabel}>{safeString(group.date)}</Text>);
    group.items.forEach((r, i) => rows.push(<Text key={`${kp}-${gi}-${i}`} style={styles.value}>{`${i + 1}. ${strip((r && r.recommendation) || '')}`}</Text>));
  });
  return rows;
};
...
const fieldBody = (record, f) => {
  ...
  if (OBJECT_ARRAY_FIELDS.includes(f) && Array.isArray(v)) return recommendationRows(v, f);
...
const IbdAssessmentDocumentPDFTemplate = ({ document: docProp, data = docProp }) => {
  let records = [];
  if (Array.isArray(data)) records = data;
  else if (data && typeof data === 'object') {
    if (data.ibd_assessment) records = Array.isArray(data.ibd_assessment) ? data.ibd_assessment : [data.ibd_assessment];
    else records = [data];
  }
    ...
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <Text style={styles.documentTitle}>IBD Assessment</Text>
          <Text style={styles.noData}>No IBD assessment records available.</Text>
        </Page>
      </Document>
    );
  }
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, rIdx) => (
          <View key={rIdx}>
            <Text style={styles.recordTitle} break={rIdx > 0}>{safeString(`IBD Assessment ${rIdx + 1}`)}</Text>
...
export default IbdAssessmentDocumentPDFTemplate;
