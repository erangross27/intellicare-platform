...
/**
 * HomeMonitoringDocumentPDFTemplate - box-free canonical (LETTER)
 * Config-driven from the JSX SECTION_ORDER/SECTION_TITLES/FIELD_LABELS/SECTION_FIELDS.
 * Renders EVERY populated field the JSX renders for JSX/PDF field parity (empty {}/[] skipped).
 * No boxes: underline rules only (documentTitle 2 / recordTitle+sectionTitle 1 / fieldLabel 0.5).
 * Nested objects (bloodPressure/glucose/weight/oxygenSaturation/peakFlow/results) + arrays render
 * via a generic recursive renderer; inline "Key: value" is allowed in the PDF only.
...
const SECTION_ORDER = ['session-info', 'blood-pressure', 'monitoring-data', 'symptoms-assessment'];
...
const HomeMonitoringDocumentPDFTemplate = ({ document: docProp, data = docProp }) => {
  let records = [];
  if (Array.isArray(data)) records = data;
  else if (data && typeof data === 'object') {
    if (data.home_monitoring) records = Array.isArray(data.home_monitoring) ? data.home_monitoring : [data.home_monitoring];
    else records = [data];
  }
    ...
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <Text style={styles.documentTitle}>Home Monitoring</Text>
          <Text style={styles.noData}>No home monitoring records available.</Text>
        </Page>
      </Document>
    );
  }
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, rIdx) => (
          <View key={rIdx}>
            <Text style={styles.recordTitle} break={rIdx > 0}>{safeString(`Home Monitoring ${rIdx + 1}`)}</Text>
...
export default HomeMonitoringDocumentPDFTemplate;
