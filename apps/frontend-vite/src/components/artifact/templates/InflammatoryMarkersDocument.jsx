/**
 * InflammatoryMarkersDocument.jsx
 * Canonical one-pass polish — blue glow editing theme
 * Collection: inflammatory_markers
 *
 * 5 Sections: Session Information · Marker Results · Complement & Immunoglobulins ·
 *             Clinical Notes · Additional Notes
 *
 * esr/crp/ferritin are UNIT STRINGS ("68 mm/hr (elevated)") — kept as editable string fields
 * (NOT numbers). date is an editable BlueDatePicker. complement (c3/c4/ch50) and the (empty)
 * immunoglobulins/results objects are decomposed read-only (key subtitle ABOVE its value row —
 * never a side-by-side "Key: value"). Empty objects/strings/arrays are gated out.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import InflammatoryMarkersDocumentPDFTemplate from '../pdf-templates/InflammatoryMarkersDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './InflammatoryMarkersDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'inflammatory_markersPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'complement-immunoglobulins': 'Complement & Immunoglobulins',
...
const SECTION_FIELDS = {
  ...
  'marker-results': ['esr', 'crp', 'ferritin'],
  'complement-immunoglobulins': ['complement', 'immunoglobulins'],
  'clinical-notes': ['findings', 'assessment', 'plan', 'results', 'recommendations'],
  'additional-notes': ['notes'],
};

/* Editable scalar strings (all backend-ALLOWED) — inline text editing.
   esr/crp/ferritin are UNIT STRINGS, kept as string fields (NOT numbers). */
const STRING_FIELDS = ['provider', 'facility', 'status', 'esr', 'crp', 'ferritin', 'findings', 'assessment', 'plan', 'notes'];
/* Editable date — BlueDatePicker */
const DATE_FIELDS = ['date'];
/* Read-only, decomposed nested objects */
const OBJECT_FIELDS = ['complement', 'immunoglobulins', 'results'];
/* Read-only, decomposed arrays */
const ARRAY_FIELDS = ['recommendations'];
const NUMBER_FIELDS = [];
/* Read-only scalars (none for this collection) */
const DISPLAY_FIELDS = [];

/* ═══════ OBJECT-FIELD HELPERS ═══════ */
const KEY_OVERRIDES = {
  c3: 'C3', c4: 'C4', ch50: 'CH50', esr: 'ESR', crp: 'CRP', ana: 'ANA',
  igg: 'IgG', iga: 'IgA', igm: 'IgM', ige: 'IgE', igd: 'IgD',
  rf: 'RF', dsdna: 'dsDNA', ena: 'ENA', ccp: 'CCP', sle: 'SLE',
};
...
/* object → copy lines: KEY on its own line, then its value(s) as numbered rows below
   (decomposed; never a side-by-side "Key: value"). Numbering lives ONLY in Copy/PDF. */
const objectCopyBlock = (value, lines, counter) => {
  Object.entries(value).filter(([, v]) => !isEmptyDeep(v)).forEach(([k, v]) => {
    lines.push(humanizeKey(k));
    if (isScalar(v)) { lines.push(`${counter.n++}. ${fmtScalar(v)}`); }
    else if (Array.isArray(v)) { v.filter(x => !isEmptyDeep(x)).forEach((it) => lines.push(`${counter.n++}. ${fmtScalar(it)}`)); }
    else { objectCopyBlock(v, lines, counter); }
  });
};
...
/* ═══════ COMPONENT ═══════ */
const InflammatoryMarkersDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.inflammatory_markers) return Array.isArray(r.inflammatory_markers) ? r.inflammatory_markers : [r.inflammatory_markers];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.inflammatory_markers) return Array.isArray(dd.inflammatory_markers) ? dd.inflammatory_markers : [dd.inflammatory_markers]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Inflammatory Markers ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/inflammatory_markers/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/inflammatory_markers/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_FIELDS.includes(f)) {
        ...
        const lines = []; objectCopyBlock(val, lines, { n: 1 }); lines.forEach(l => { text += `${l}\n`; });
        text += '\n';
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== INFLAMMATORY MARKERS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Inflammatory Markers ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT FIELD (complement / immunoglobulins / results — read-only) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="inflammatory-markers-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Inflammatory Markers</h2></div>
        <div className="empty-state">No inflammatory markers records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="inflammatory-markers-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Inflammatory Markers</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<InflammatoryMarkersDocumentPDFTemplate document={pdfData} />} fileName="Inflammatory_Markers.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search inflammatory markers..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Inflammatory Markers ${idx + 1}`)}</h3>
            </div>
...
export default InflammatoryMarkersDocument;
