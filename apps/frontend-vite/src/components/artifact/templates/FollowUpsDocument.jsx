/**
 * FollowUpsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: follow_ups
 *
 * 4 Sections:
 *   1. report-info: reportDate (date picker), reportType (string), urgency (string)
 *   2. clinical-info: clinicalIndication (sentence), findings (sentence)
 *   3. recommendations-section: recommendations (sentence — NOT array, STRING with splitBySentence)
 *   4. follow-up-section: followUp (sentence)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import FollowUpsDocumentPDFTemplate from '../pdf-templates/FollowUpsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './FollowUpsDocument.css';
...
const SECTION_FIELDS = {
  'report-info': ['reportDate', 'reportType', 'urgency'],
  'clinical-info': ['clinicalIndication', 'findings'],
  'recommendations-section': ['recommendations'],
  'follow-up-section': ['followUp'],
};

const NUMBER_FIELDS = [];
const STRING_FIELDS = ['reportType', 'urgency', 'clinicalIndication', 'findings', 'recommendations', 'followUp'];
/* All narrative fields split by SENTENCE ([.;]) then labeled sentences split by comma — canonical.
   (findings/recommendations were legacy COMMA_FIELDS: that split their embedded ';' clauses incorrectly.) */
const COMMA_FIELDS = [];
...
/* splitByComma: parenthesis-aware + digit-guarded comma split
   (does NOT split inside "(...)" and NOT between digits like "1,200" / "October 10, 2025") */
const splitByComma = (text) => {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (ch === '(') { depth++; current += ch; }
      ...
      const prev = current.replace(/\s+$/, '').slice(-1);
      let j = i + 1; while (j < text.length && /\s/.test(text[j])) j++;
      const next = text[j] || '';
      if (/\d/.test(prev) && /\d/.test(next)) { current += ch; continue; } // digit-guard
...
const DRAFT_KEY = 'follow_upsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const FollowUpsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.follow_ups) return Array.isArray(r.follow_ups) ? r.follow_ups : [r.follow_ups];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.follow_ups) return Array.isArray(dd.follow_ups) ? dd.follow_ups : [dd.follow_ups]; return [dd]; }
      return [r];
    });
  ...
  /* ═══════ REHYDRATE PENDING DRAFTS ═══════
     ...
     Maps record _id (handling _id.$oid) to the current render index. */
  useEffect(() => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Follow-Up ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/follow_ups/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/follow_ups/${id}/approve`, { sectionId: sid, approved: true });
  ...
  /* ═══════ SAVE: comma-list item (splice-safe, preserves ", " delimiters) ═══════ */
  function saveCommaItem(record, fn, idx, ciIdx) {
    const id = safeId(record); if (!id) return;
    const currentVal = String(getFieldValue(record, fn, idx) || '');
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      rebuilt += currentVal.slice(cursor, pos) + (i === ciIdx ? edited : it);
      cursor = pos + it.length;
    }
    let fullText = ok ? (rebuilt + currentVal.slice(cursor)) : items.map((it, i) => (i === ciIdx ? edited : it)).filter(Boolean).join(', ');
    fullText = fullText.replace(/,\s*,/g, ', ').replace(/^[\s,]+/, '').replace(/[\s,]+$/, '').trim();
    const extra = Math.max(0, splitByComma(fullText).length - items.length);
    // Stage as a DRAFT (no DB write). Approve commits it. sid derived from SECTION_FIELDS so the approve button resets.
    const sid = Object.keys(SECTION_FIELDS).find(s => SECTION_FIELDS[s].includes(fn));
    stageDraft(id, fn, idx, sid, fullText);
    const marks = { [`${fn}-${idx}-c${ciIdx}`]: 'edited' };
    for (let ei = 1; ei <= extra; ei++) marks[`${fn}-${idx}-c${ciIdx + ei}`] = 'added';
    setEditedSentences(prev => ({ ...prev, ...marks }));
    setEditingField(null); setEditValue('');
  }

  /* ═══════ RENDER: COMMA-LIST FIELD (e.g. findings) — split by comma into items ═══════ */
  const renderCommaField = (record, fn, idx, sid) => {
    ...
    return (
      <div key={fn} className="rec-mini-card">
        {label !== (SECTION_TITLES[sid] || '') && <div className="nested-subtitle">{highlightText(label)}</div>}
        {items.map((item, ciIdx) => {
          const itemKey = `${fn}-${idx}-c${ciIdx}`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="follow-ups-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Follow-Ups</h2></div>
        <div className="empty-state">No follow-ups available</div>
      </div>
    );
  }
  ...
  return (
    <div className="follow-ups-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Follow-Ups</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FollowUpsDocumentPDFTemplate document={pdfData} />} fileName="Follow_Ups.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search follow-ups (recommendations, urgency, findings...)" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Follow-Up ${idx + 1}`)}</h3>
            </div>
            ...
            {renderSection(record, idx, 'follow-up-section')}
          </div>
        ))}
      </div>
    </div>
...
export default FollowUpsDocument;
