/**
 * Webhook Subscription Management Service - DDD Integration Feature
...
class WebhookSubscriptionService {
  ...
  async initialize() {
    ...
    try {
      ...
      throw new Error(`Failed to initialize WebhookSubscriptionService: ${error.message}`);
    }
  }
  ...
  async updateSubscription(subscriptionId, updates, options = {}) {
    ...
    try {
      const subscription = await this.getSubscription(subscriptionId, options);
      if (!subscription) {
      ...
      await AuditLog.create({
        ...
        userId: options.userId || 'system',
        practiceId: options.practiceId || subscription.practiceId,
        details: {
          subscriptionId,
  ...
  async deleteSubscription(subscriptionId, options = {}) {
    ...
    try {
      ...
      // Soft delete - mark as inactive using SecureDataAccess
      await SecureDataAccess.update(
        'webhook_subscriptions',
        { id: subscriptionId },
        {
  ...
  async getSubscription(subscriptionId, options = {}) {
    ...
    try {
      ...
      // Query database using SecureDataAccess
   ...
   */
  async getDeliveryAnalytics(subscriptionId, timeRange = '24h', options = {}) {
    await this.initialize();
    ...
    try {
      ...
      // Query delivery history using SecureDataAccess
   ...
   */
  async testWebhook(subscriptionId, options = {}) {
    await this.initialize();
   ...
   */
  async logDelivery(deliveryLog, practiceId) {
    try {
      await SecureDataAccess.create(
        'webhook_deliveries', 
        deliveryLog, 
  ...
  async scheduleRetry(subscription, event, error) {
    ...
    try {
      await AuditLog.create({
        ...
        userId: 'system',
        practiceId: subscription.practiceId || 'global',
        details: {
          subscriptionId: subscription.id,
          eventId: event.eventId,
          error: error.message,
      ...
      console.error('Failed to log retry scheduling:', logError);
    }
  }
  ...
  startDeliveryTracking() {
    ...
    setInterval(async () => {
      ...
      try {
        ...
        console.log(`🧹 Cleaned up webhook delivery logs older than ${cutoffDate.toISOString()}`);
      } catch (error) {
