/**
 * Canonical box-free PDF for symptom_progression.
 * Mirrors SymptomProgressionDocument JSX field order, grouping, and numbering.
 */
...
const SECTION_CONFIGS = [
  ...
  { id: 'progression', title: 'Progression Timeline', fields: ['timeline', 'week1', 'week2', 'week3', 'current'] },
...
const unwrapRecords = (data) => {
  ...
  return input.flatMap(record => {
    if (record?.symptom_progression) return Array.isArray(record.symptom_progression) ? record.symptom_progression : [record.symptom_progression];
    if (record?.documentData) {
      const nested = record.documentData;
      if (Array.isArray(nested)) return nested;
      if (nested?.symptom_progression) return Array.isArray(nested.symptom_progression) ? nested.symptom_progression : [nested.symptom_progression];
      return [nested];
    }
    return [record];
      ...
      {records.length ? records.map((record, index) => (
          ...
          <View style={styles.pageBody}>
            ...
            <View style={styles.recordContainer}>
              <View style={styles.recordHeader} wrap={false}>
                <Text style={styles.recordTitle}>Symptom Progression {index + 1}</Text>
              </View>
        ...
        <Page size="A4" style={styles.page}>
          <View style={styles.pageBody}>
            ...
            <Text style={styles.noDataText}>No symptom progression records available.</Text>
          </View>
        </Page>
      )}
    </Document>
