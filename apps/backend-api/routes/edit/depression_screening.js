/**
 * DepressionScreening Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/depression_screening/:id/edit    — Edit a single field
 * PUT /api/edit/depression_screening/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'depression-screening-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'appetiteChanges',
  'psychomotorChanges',
  'anxietySymptomSeverity',
  'sleepDisturbanceType',
  'medicalComorbidities',
  'currentAntidepressantMedications',
  // clinical booleans (Yes/No dropdown) + numeric — made editable July 2026
  'majorDepressiveEpisodeCriteria',
  'psychoticFeaturesPresent',
  'suicidalIdeationPresent',
  'concentrationDifficulties',
  'worthlessnessGuilt',
  'substanceUseComorbidity',
  'priorDepressionEpisodes',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('depression_screening', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[DepressionScreening] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('depression_screening', { _id: objectId }, {
      $set: {
    ...
    console.error('[DepressionScreening] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
