/**
 * GrowthUltrasoundScheduleDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: growth_ultrasound_schedule
 *
 * 4 Sections:
 *   1. header-info: date, provider, facility, status
 *   2. schedule-info: frequency, startingWeek, purpose, specificWeeks
 *   3. clinical-assessment: findings, assessment, results (OBJECT)
 *   4. treatment-plan: plan, recommendations (OBJECT-ARRAY), notes
 *
 * Donor: PointOfCareUltrasoundHeartRateDocument — OBJECT `results` recursive renderer
 *   (renderObjectField / renderObjectNode / renderObjectLeaf, humanizeKey + nested-mini-card)
 *   and `recommendations` array-of-{recommendation,date} date-grouped renderer.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import GrowthUltrasoundScheduleDocumentPDFTemplate from '../pdf-templates/GrowthUltrasoundScheduleDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './GrowthUltrasoundScheduleDocument.css';
...
const DRAFT_KEY = 'growth_ultrasound_schedulePendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  ...
  'schedule-info': ['frequency', 'startingWeek', 'purpose', 'specificWeeks'],
  'clinical-assessment': ['findings', 'assessment', 'results'],
  'treatment-plan': ['plan', 'recommendations', 'notes'],
};

const BOOLEAN_FIELDS = [];
const DATE_FIELDS = ['date'];
const ARRAY_FIELDS = ['specificWeeks'];
const OBJECT_ARRAY_FIELDS = ['recommendations'];
const OBJECT_FIELDS = ['results'];
const NUMBER_FIELDS = [];
const STRING_FIELDS = ['provider', 'facility', 'status', 'frequency', 'startingWeek', 'purpose', 'findings', 'assessment', 'plan', 'notes'];

/* ═══════ OBJECT HELPERS (donor: PointOfCareUltrasoundHeartRateDocument) ═══════ */
const KEY_OVERRIDES = {};
...
/* parseLabel: detect "Label: value" patterns (CLAUSE_OPENER guard: conditional clause ≠ Label:Value) */
...
/* ═══════ COMPONENT ═══════ */
const GrowthUltrasoundScheduleDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.growth_ultrasound_schedule) return Array.isArray(r.growth_ultrasound_schedule) ? r.growth_ultrasound_schedule : [r.growth_ultrasound_schedule];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.growth_ultrasound_schedule) return Array.isArray(dd.growth_ultrasound_schedule) ? dd.growth_ultrasound_schedule : [dd.growth_ultrasound_schedule]; return [dd]; }
      return [r];
    });
  ...
  /* flatten OBJECT/OBJECT-ARRAY values into searchable text */
  ...
  const searchableVal = useCallback((fn, v) => {
    if (OBJECT_FIELDS.includes(fn) || OBJECT_ARRAY_FIELDS.includes(fn)) return flattenSearchable(v);
    return null;
  }, [flattenSearchable]);
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        const sv = searchableVal(f, val);
        if (sv !== null) { if (sv.toLowerCase().includes(phrase)) return true; }
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) {
      const sv = searchableVal(fn, val);
      if (sv !== null) return sv.toLowerCase().includes(phrase);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Growth Ultrasound Schedule ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (!val) continue;
          const sv = searchableVal(f, val);
          if (sv !== null) { if (sv.toLowerCase().includes(phrase)) return true; continue; }
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Stage one field's full computed value as a DRAFT (localEdits + pendingEdits + localStorage).
  // NOT written to MongoDB and NOT shown in the PDF until the user clicks Approve. fieldPart is the
  // localEdits key minus the "-<idx>" suffix (always the field name here; whole arrays/objects ride
  // along as the value so Approve replays a single { field, value } PUT). Re-edit after approval drops
  // the section's approved flag so the button returns to yellow Pending Approve.
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.gestationalAge) — value stays a STRING */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    // Build the merged full object clone, then stage it as a DRAFT (no DB write). Approve replays a
    // single { field: rootField, value: <full object> } PUT.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/growth_ultrasound_schedule/${id}/edit`, payload);
      }
      // Flag the record approved (audit trail).
      await secureApiClient.put(`/api/edit/growth_ultrasound_schedule/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = 'Growth Ultrasound Schedule\n' + '='.repeat(40) + '\n\n';
    pdfData.forEach((r, idx) => {
      text += `Growth Ultrasound Schedule ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: SPECIFIC WEEKS (array of strings) ═══════ */
  const renderSpecificWeeksField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    const items = Array.isArray(val) ? val.filter(item => item && String(item).trim()) : [];
                  ...
                  <div className="edit-field-container">
                    ...
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => {
                        ...
                        setSaveError(null);
                        // Stage the full updated array as a DRAFT (no DB write). Approve commits it.
        ...
        <div key={fn}>
          <div className="rec-mini-card">
            ...
            {sentences.map((sentence, sIdx) => {
                                ...
                                <div className="edit-field-container">
                                  ...
                                  <div className="edit-actions">
                                    <button className="save-btn" disabled={saving} onClick={e => {
                                      ...
                                      setSaveError(null);
                                      // Stage as a DRAFT (no DB write). Approve commits it.
                                      stageDraft(record, fn, idx, sid, fullText2);
              ...
              return (
                <div key={sIdx} className={parsed.isLabeled ? 'mini-card' : ''} style={parsed.isLabeled ? { marginTop: 8 } : undefined}>
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (f === 'specificWeeks') return renderSpecificWeeksField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="growth-ultrasound-schedule-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Growth Ultrasound Schedule</h2></div>
        <div className="empty-state">No growth ultrasound schedule records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="growth-ultrasound-schedule-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Growth Ultrasound Schedule</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<GrowthUltrasoundScheduleDocumentPDFTemplate document={pdfData} />} fileName="Growth_Ultrasound_Schedule.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search growth ultrasound schedule..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Growth Ultrasound Schedule ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'header-info')}
            {renderSection(record, idx, 'schedule-info')}
...
export default GrowthUltrasoundScheduleDocument;
