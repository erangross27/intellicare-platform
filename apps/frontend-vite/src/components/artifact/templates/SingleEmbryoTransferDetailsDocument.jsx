 ...
 * July 2026 — recursive, dot-path editable single embryo transfer details
...
const SECTION_TITLES = {
  embryoAssessment: 'Embryo Assessment',
  endometrialPreparation: 'Endometrial Preparation',
  transferProcedure: 'Transfer Procedure',
  embryoLoading: 'Embryo Loading',
  contaminationRetention: 'Contamination & Retention',
  hormonalLevels: 'Hormonal Levels',
  medicationsCryopreservation: 'Medications & Cryopreservation',
};
const FIELD_LABELS = {
...
const SECTION_FIELDS = {
  embryoAssessment: ['embryoQualityGrade', 'embryoDevelopmentalStage', 'innerCellMassGrade', 'trophectodermGrade'],
  endometrialPreparation: ['endometrialThickness', 'endometrialPattern'],
  transferProcedure: ['transferCatheterType', 'cervicalDilatationRequired', 'transferDifficultyScore', 'ultrasoundGuidance', 'transferDateTime'],
  embryoLoading: ['embryoLoadingVolume', 'uterineFundalDistance', 'catheterTipPosition'],
  contaminationRetention: ['bloodContamination', 'mucusContamination', 'embryoRetentionConfirmed'],
  hormonalLevels: ['progesteroneLevel', 'estradiolLevel', 'lutealPhaseSupport'],
  medicationsCryopreservation: ['premedication', 'embryoCryopreservationMethod', 'postTransferRestDuration'],
};
const DATE_FIELDS = ['transferDateTime'];
const DATETIME_FIELDS = ['transferDateTime'];
const ENUM_FIELDS = ['embryoDevelopmentalStage', 'innerCellMassGrade', 'trophectodermGrade', 'endometrialPattern', 'embryoCryopreservationMethod'];
  ...
  const options = field === 'embryoDevelopmentalStage' ? ['Day 3 cleavage embryo', 'Day 5 blastocyst', 'Day 6 blastocyst']
    : field === 'innerCellMassGrade' || field === 'trophectodermGrade' ? ['A', 'B', 'C']
      : field === 'endometrialPattern' ? ['Trilaminar', 'Homogeneous', 'Heterogeneous']
        : field === 'embryoCryopreservationMethod' ? ['Vitrification', 'Slow freezing'] : [];
...
const SingleEmbryoTransferDetailsDocument = ({ document: docProp }) => {
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/single_embryo_transfer_details/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/single_embryo_transfer_details/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SingleEmbryoTransferDetails] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Single Embryo Transfer Details Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="single-embryo-transfer-details-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Single Embryo Transfer Details</h2></div><div className="empty-state">No single embryo transfer details available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Single Embryo Transfer Details Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="single-embryo-transfer-details-document" ref={containerRef}>
    <div className="document-header">
      ...
      <div className="header-actions">
        ...
        <PDFDownloadLink document={<SingleEmbryoTransferDetailsDocumentPDFTemplate document={pdfData} />} fileName="Single_Embryo_Transfer_Details.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search single embryo transfer details..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Single Embryo Transfer Details Record ' + (recordIndex + 1))}</h3></div>
