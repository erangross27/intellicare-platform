/**
 * PointOfCareUltrasoundHeartRateDocument.jsx
 * June 2026 — Collection: point_of_care_ultrasound_heart_rate (NEW template)
 *
 * Donors cloned:
 *  - StemCellTransplantAssessmentDocument: per-sentence renderStringField, 4-level search,
 *    per-section approve, Copy row/Section/All, pdfData memo, blue-glow theme.
 *  - MaternalWeightMonitoringDocument: DATE_FIELDS + renderDateField (record-header date),
 *    date+provider+facility header.
 *  - MyositisAssessmentDocument: OBJECT field `results` recursive renderer (renderObjectField /
 *    renderObjectNode / renderObjectLeaf, humanizeKey + nested-mini-card, hide-empty at every level)
 *    AND the array-of-{recommendation,date} renderRecommendationsField (date-grouped, numbered list).
 *
 * FIELDS (14 → 100% schema coverage):
 *   DATE:   date
 *   STRING: heartRate (schema STRING, rendered as text), unit, method, type, provider, facility,
...
import PointOfCareUltrasoundHeartRateDocumentPDFTemplate from '../pdf-templates/PointOfCareUltrasoundHeartRateDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './PointOfCareUltrasoundHeartRateDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; value may be string/array/object) */
const DRAFT_KEY = 'point_of_care_ultrasound_heart_ratePendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  'measurement': ['heartRate', 'unit', 'method', 'type'],
...
const SECTION_ORDER = ['measurement', 'clinical', 'results-section', 'recommendations-section', 'notes-status'];

const DATE_FIELDS = ['date'];
/* heartRate is type STRING in schema — rendered as text */
const STRING_FIELDS = ['heartRate', 'unit', 'method', 'type', 'provider', 'facility', 'findings', 'assessment', 'plan', 'notes', 'status'];
...
const KEY_OVERRIDES = {
  ef: 'EF', lvef: 'LVEF', ivc: 'IVC', bpm: 'BPM', hr: 'HR', poc: 'POC', pocus: 'POCUS',
};
...
/* ═══════ COMPONENT ═══════ */
const PointOfCareUltrasoundHeartRateDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.point_of_care_ultrasound_heart_rate) return Array.isArray(r.point_of_care_ultrasound_heart_rate) ? r.point_of_care_ultrasound_heart_rate : [r.point_of_care_ultrasound_heart_rate];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.point_of_care_ultrasound_heart_rate) return Array.isArray(dd.point_of_care_ultrasound_heart_rate) ? dd.point_of_care_ultrasound_heart_rate : [dd.point_of_care_ultrasound_heart_rate]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((r, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        // Mark the field edited so the section's Pending Approve button reappears.
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `point-of-care ultrasound — heart rate ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Stage a DRAFT for the whole field locally + persist to the pending-drafts localStorage store
  // (survives refresh). NOT written to MongoDB and NOT shown in the PDF until the user clicks Approve.
  // Re-editing an approved section drops its approved flag so the button returns to yellow Pending.
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.ef) — value stays a STRING.
     Stage as a DRAFT: merge into the whole `rootField` object and store it locally + in localStorage.
     NO DB write here — Approve commits the whole object via { field: rootField, value: object }. */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    setLocalEdits(prev => {
      ...
      // Persist the merged whole-object draft (survives refresh; stays OUT of DB/PDF until Approve).
      const store = readDrafts();
      if (!store[id]) store[id] = {};
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // localEdits keys are `${fieldName}-${idx}` (whole-field value; objects/arrays stored intact).
      const suffix = `-${idx}`;
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length); // field name (no numeric dot suffix in this template)
        return fields.includes(fieldPart);
      });
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length);
        // arrayIndex ONLY when the segment after the LAST dot is purely numeric (mirrors save fieldPart build)
        ...
        const resp = await secureApiClient.put(`/api/edit/point_of_care_ultrasound_heart_rate/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/point_of_care_ultrasound_heart_rate/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== POINT-OF-CARE ULTRASOUND — HEART RATE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Point-of-Care Ultrasound — Heart Rate ${idx + 1}\n${'='.repeat(40)}\n\n`;
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              setSaveError(null);
                              // Stage the whole recommendations array as a DRAFT (no DB write). Approve commits it.
  ...
  if (!records || records.length === 0) {
    return (
      <div className="point-of-care-ultrasound-heart-rate-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Point-of-Care Ultrasound — Heart Rate</h2></div>
        <div className="empty-state">No point-of-care ultrasound heart rate records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="point-of-care-ultrasound-heart-rate-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Point-of-Care Ultrasound — Heart Rate</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PointOfCareUltrasoundHeartRateDocumentPDFTemplate document={pdfData} />} fileName={`point-of-care-ultrasound-heart-rate-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search point-of-care ultrasound heart rate records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Point-of-Care Ultrasound — Heart Rate ${idx + 1}`)}</h3>
            </div>
...
export default PointOfCareUltrasoundHeartRateDocument;
