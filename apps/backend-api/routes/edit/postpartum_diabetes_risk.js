...
function buildContext(req, operation = 'read') { return { serviceId: 'postpartum-diabetes-risk-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['gestationalDiabetesHistory', 'oralGlucoseToleranceTest', 'fastingPlasmaGlucose', 'hemoglobinA1c', 'prepregnancyBmi', 'gestationalWeightGain', 'macrosomiaHistory', 'familyHistoryDiabetes', 'polycysticOvarySyndrome', 'breastfeedingStatus', 'ethnicRiskGroup', 'maternalAge', 'previousStillbirth', 'hypertensiveDisorders', 'insulinTherapyDuringPregnancy', 'postpartumScreeningDate', 'currentBmi', 'triglycerideLevels', 'hdlCholesterol', 'metabolicSyndrome', 'thyroidDisorders', 'corticosteroidUse', 'contraceptiveMethod'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('postpartum_diabetes_risk', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[PostpartumDiabetesRisk] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('postpartum_diabetes_risk', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[PostpartumDiabetesRisk] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
