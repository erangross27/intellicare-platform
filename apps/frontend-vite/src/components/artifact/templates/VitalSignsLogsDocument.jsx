/**
 * VitalSignsLogsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: vital_signs_logs
 *
 * 7 Sections:
 *   1. provider-info: provider, facility, date
 *   2. blood-pressure: systolicBloodPressure, diastolicBloodPressure, meanArterialPressure, bloodPressureCuffSize, patientPosition
 *   3. cardiac: heartRate, pulseCharacter
 *   4. respiratory: respiratoryRate, respiratoryEffort, oxygenSaturation, supplementalOxygenFlow, oxygenDeliveryMethod
 *   5. temperature: temperatureCelsius, temperatureFahrenheit, temperatureSite
 *   6. pain-glucose: painScore, bloodGlucose
 *   7. anthropometrics: weightKilograms, heightCentimeters, bodyMassIndex, measurementMethod
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import VitalSignsLogsDocumentPDFTemplate from '../pdf-templates/VitalSignsLogsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './VitalSignsLogsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field") */
const DRAFT_KEY = 'vital_signs_logsPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  bloodPressureCuffSize: 'Blood Pressure Cuff Size',
  patientPosition: 'Patient Position',
  heartRate: 'Heart Rate',
  pulseCharacter: 'Pulse Character',
  respiratoryRate: 'Respiratory Rate',
  respiratoryEffort: 'Respiratory Effort',
  oxygenSaturation: 'Oxygen Saturation',
  supplementalOxygenFlow: 'Supplemental Oxygen Flow',
  ...
  temperatureSite: 'Temperature Site',
...
const SECTION_FIELDS = {
  ...
  'blood-pressure': ['systolicBloodPressure', 'diastolicBloodPressure', 'meanArterialPressure', 'bloodPressureCuffSize', 'patientPosition'],
  'cardiac': ['heartRate', 'pulseCharacter'],
  'respiratory': ['respiratoryRate', 'respiratoryEffort', 'oxygenSaturation', 'supplementalOxygenFlow', 'oxygenDeliveryMethod'],
  'temperature': ['temperatureCelsius', 'temperatureFahrenheit', 'temperatureSite'],
  'pain-glucose': ['painScore', 'bloodGlucose'],
  'anthropometrics': ['weightKilograms', 'heightCentimeters', 'bodyMassIndex', 'measurementMethod'],
};

const BOOLEAN_FIELDS = [];
const DATE_FIELDS = ['date'];
const NUMBER_FIELDS = ['systolicBloodPressure', 'diastolicBloodPressure', 'heartRate', 'respiratoryRate', 'temperatureCelsius', 'temperatureFahrenheit', 'oxygenSaturation', 'supplementalOxygenFlow', 'painScore', 'bloodGlucose', 'meanArterialPressure', 'bodyMassIndex', 'weightKilograms', 'heightCentimeters'];
const STRING_FIELDS = ['provider', 'facility', 'temperatureSite', 'oxygenDeliveryMethod', 'patientPosition', 'bloodPressureCuffSize', 'pulseCharacter', 'respiratoryEffort', 'measurementMethod'];
...
/* ======= COMPONENT ======= */
const VitalSignsLogsDocument = ({ document: docProp }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      const recId = (!record?._id) ? null : (typeof record._id === 'string' ? record._id : (record._id.$oid || String(record._id)));
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Vital Signs Log ${idx + 1}`.toLowerCase();
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    if (sid) setApprovedSections(prev => {
      ...
      const next = { ...prev }; delete next[sKey]; return next;
    });
    const store = readDrafts();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // Collect this section's staged (pending) edits. editKey convention: "field-idx".
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/vital_signs_logs/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/vital_signs_logs/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== VITAL SIGNS LOGS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Vital Signs Log ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="vital-signs-logs-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Vital Signs Logs</h2></div>
        <div className="empty-state">No vital signs log records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="vital-signs-logs-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Vital Signs Logs</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<VitalSignsLogsDocumentPDFTemplate document={pdfData} />} fileName={`vital-signs-logs-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search vital signs logs..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Vital Signs Log ${idx + 1}`)}</h3>
            </div>
            ...
            {renderSection(record, idx, 'pain-glucose')}
...
export default VitalSignsLogsDocument;
