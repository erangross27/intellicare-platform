...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'prenatal-screening-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['date', 'type', 'provider', 'facility', 'firstTrimesterScreen', 'cellFreeDna', 'quadScreen', 'amniocentesis', 'cvs', 'results', 'ntScanResult', 'cellFreeDNAResult', 'firstTrimesterScreenResult', 'anatomyScanResult', 'cervicalLengthMeasurement', 'fetalEchoResult', 'perinatalMentalHealthReferral', 'findings', 'assessment', 'plan', 'recommendations', 'notes', 'status'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('prenatal_screening', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[prenatal_screening] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('prenatal_screening', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[prenatal_screening] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
