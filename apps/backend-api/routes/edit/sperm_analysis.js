...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'sperm-analysis-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'abstinencePeriodDays',
  'semenVolumeMl',
  'liquefactionTimeMinutes',
  'semenViscosity',
  'semenPh',
  'semenAppearance',
  'spermConcentrationMillionPerMl',
  'totalSpermCountMillion',
  'roundCellConcentrationMillionPerMl',
  'leukocyteConcentrationMillionPerMl',
  'totalMotilityPercent',
  'progressiveMotilityPercent',
  'nonProgressiveMotilityPercent',
  'immotileSpermPercent',
  'spermMotilityGrade',
  'normalMorphologyPercent',
  'headDefectsPercent',
  'midpieceDefectsPercent',
  'tailDefectsPercent',
  'teratozoospermiaIndex',
  'spermVitalityPercent',
  'dnaFragmentationIndexPercent',
  'spermAgglutinationGrade',
  'antispermAntibodiesPresent',
  'marTestPercent',
  'fructoseLevelMgPerDl',
  'zincConcentrationUmolPerL',
  'semenCultureResult',
  'whoSemenAnalysisDiagnosis',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('sperm_analysis', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[sperm_analysis] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('sperm_analysis', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[sperm_analysis] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
