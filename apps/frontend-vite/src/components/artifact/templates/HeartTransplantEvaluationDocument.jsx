/**
 * HeartTransplantEvaluationDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: heart_transplant_evaluation
 *
 * 8 Sections:
 *   1. session-info: date (date picker), provider (string), facility (string)
 *   2. listing-immunologic: listingStatus, unosStatusJustification, calculatedPanelReactiveAntibody,
 *                           hlaTyping, unacceptableAntigens, donorSpecificAntibodies
 *   3. functional-cardiac: nyhaFunctionalClass, intermacsMilestoneProfile,
 *                          leftVentricularEjectionFraction, rightVentricularFunction, cardiacIndex
 *   4. hemodynamics: pulmonaryCapillaryWedgePressure, pulmonaryArteryPressure,
 *                    pulmonaryVascularResistance, transpulmonaryGradient, pvrReversibilityTest
 *   5. exercise-biomarkers: peakVO2, ventilatorEquivalentCO2Slope, heartFailureSurvivalScore,
 *                           bnpLevel, ntProBnpLevel
 *   6. mechanical-support: mechanicalCirculatorySupport, lvadParameters, inotropeDependence
 *   7. etiology-surgical: cardiomyopathyEtiology, previousCardiacSurgeries
 *   8. fitness-screening: estimatedGlomerularFiltrationRate, frailtyIndex,
 *                         psychosocialEvaluation, infectiousScreening, malignancyHistory
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HeartTransplantEvaluationDocumentPDFTemplate from '../pdf-templates/HeartTransplantEvaluationDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './HeartTransplantEvaluationDocument.css';
...
/* 12 number fields — ALL use input[type=number step=any] + parseFloat+isNaN→block save */
const NUMBER_FIELDS = [
  'calculatedPanelReactiveAntibody', 'intermacsMilestoneProfile',
  'leftVentricularEjectionFraction', 'cardiacIndex',
  'pulmonaryCapillaryWedgePressure', 'pulmonaryVascularResistance',
  'transpulmonaryGradient', 'peakVO2', 'ventilatorEquivalentCO2Slope',
  'bnpLevel', 'ntProBnpLevel', 'estimatedGlomerularFiltrationRate',
];

const STRING_FIELDS = [
  'provider', 'facility', 'listingStatus', 'unosStatusJustification',
  'hlaTyping', 'nyhaFunctionalClass', 'rightVentricularFunction',
  'pulmonaryArteryPressure', 'pvrReversibilityTest',
  'heartFailureSurvivalScore', 'mechanicalCirculatorySupport',
  'lvadParameters', 'inotropeDependence', 'cardiomyopathyEtiology',
  'frailtyIndex', 'psychosocialEvaluation', 'infectiousScreening',
  'malignancyHistory',
];

const ARRAY_FIELDS = ['unacceptableAntigens', 'donorSpecificAntibodies', 'previousCardiacSurgeries'];
const DATE_FIELDS = ['date'];

/* parseLabel: detect "Label: value" patterns (CLAUSE_OPENER guard rejects sentence clauses; strips leading list-marker) */
...
const DRAFT_KEY = 'heart_transplant_evaluationPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const HeartTransplantEvaluationDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.heart_transplant_evaluation) return Array.isArray(r.heart_transplant_evaluation) ? r.heart_transplant_evaluation : [r.heart_transplant_evaluation];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.heart_transplant_evaluation) return Array.isArray(dd.heart_transplant_evaluation) ? dd.heart_transplant_evaluation : [dd.heart_transplant_evaluation]; return [dd]; }
      return [r];
    });
  ...
  const fmtVal = useCallback((v) => { if (typeof v === 'boolean') return v ? 'Yes' : 'No'; if (typeof v === 'number') return String(v); if (Array.isArray(v)) return v.filter(Boolean).join(', '); return String(v || ''); }, []);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Heart Transplant Evaluation ${idx + 1}`.toLowerCase();
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    // Stage the draft in localStorage (key by record id + field name, no arrayIndex for this template)
    const store = readDrafts();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/heart_transplant_evaluation/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/heart_transplant_evaluation/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        const arrVal = Array.isArray(val) ? val.filter(Boolean).join(', ') : fmtVal(val);
        text += `${label}\n${arrVal}\n\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HEART TRANSPLANT EVALUATION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Heart Transplant Evaluation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderArrayField = (record, fn, idx, sid) => {
    ...
    const isEditing = editingField === editKey;
    const label = FIELD_LABELS[fn] || fn;
    const displayVal = Array.isArray(val) ? val.filter(Boolean).join(', ') : fmtVal(val);
    const isModified = editedFields[editKey];
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayVal); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              ...
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const arrVal = editValue.split(',').map(s => s.trim()).filter(Boolean); handleSaveField(record, fn, idx, sid, null, arrVal); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="heart-transplant-evaluation-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Heart Transplant Evaluation</h2></div>
        <div className="empty-state">No heart transplant evaluation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="heart-transplant-evaluation-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Heart Transplant Evaluation</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HeartTransplantEvaluationDocumentPDFTemplate document={pdfData} />} fileName="Heart_Transplant_Evaluation.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search heart transplant evaluation..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Heart Transplant Evaluation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'listing-immunologic')}
            {renderSection(record, idx, 'functional-cardiac')}
            {renderSection(record, idx, 'hemodynamics')}
            {renderSection(record, idx, 'exercise-biomarkers')}
            {renderSection(record, idx, 'mechanical-support')}
            {renderSection(record, idx, 'etiology-surgical')}
            {renderSection(record, idx, 'fitness-screening')}
          </div>
        ))}
      </div>
    </div>
...
export default HeartTransplantEvaluationDocument;
