/**
 * MovementDisorderAssessmentDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: movement_disorder_assessment
 *
 * 6 Sections:
 *   1. assessment-info: date, provider, facility, status
 *   2. diagnosis-section: diagnosis
 *   3. disease-characteristics: hoehnYahrStage, diseaseOnset, diseaseDuration, motorSubtype, laterality, progressionRate
 *   4. updrs-scores: updrsScores.partIII, updrsScores.total
 *   5. clinical-findings: findings, assessment
...
import MovementDisorderAssessmentDocumentPDFTemplate from '../pdf-templates/MovementDisorderAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './MovementDisorderAssessmentDocument.css';
...
const STRING_FIELDS = ['provider', 'facility', 'status', 'diagnosis', 'hoehnYahrStage', 'diseaseOnset', 'diseaseDuration', 'motorSubtype', 'laterality', 'progressionRate', 'updrsScores.partIII', 'updrsScores.total', 'findings', 'assessment', 'plan', 'notes'];
...
// Hoehn & Yahr Stage severity mapping (1-5 scale)
const HOEHN_YAHR_STAGES = {
  '1': { level: 1, label: 'Stage 1', interpretation: 'Unilateral involvement only', color: '#22c55e' },
  '1.5': { level: 1.5, label: 'Stage 1.5', interpretation: 'Unilateral and axial involvement', color: '#22c55e' },
  '2': { level: 2, label: 'Stage 2', interpretation: 'Bilateral involvement, no balance impairment', color: '#3b82f6' },
  '2.5': { level: 2.5, label: 'Stage 2.5', interpretation: 'Mild bilateral disease with recovery on pull test', color: '#3b82f6' },
  '3': { level: 3, label: 'Stage 3', interpretation: 'Mild to moderate bilateral, physically independent', color: '#f59e0b' },
  '4': { level: 4, label: 'Stage 4', interpretation: 'Severe disability, still able to walk or stand', color: '#f97316' },
  '5': { level: 5, label: 'Stage 5', interpretation: 'Wheelchair bound or bedridden', color: '#ef4444' },
};

const parseHoehnYahrStage = (stageText) => {
  if (!stageText) return null;
  const match = String(stageText).match(/(\d+\.?\d*)/);
  if (match) return HOEHN_YAHR_STAGES[match[1]] || null;
  return null;
};

const parseUPDRSScore = (scoreText) => {
  ...
  if (match) {
    ...
    if (percentage <= 20) { color = '#22c55e'; interpretation = 'Mild motor impairment'; }
    else if (percentage <= 40) { color = '#3b82f6'; interpretation = 'Moderate motor impairment'; }
    else if (percentage <= 60) { color = '#f59e0b'; interpretation = 'Moderately severe motor impairment'; }
    else if (percentage <= 80) { color = '#f97316'; interpretation = 'Severe motor impairment'; }
    else { color = '#ef4444'; interpretation = 'Very severe motor impairment'; }
    return { score, maxScore, percentage, color, interpretation, rawValue: `${score}/${maxScore}` };
  }
  return null;
};
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   or a dotted nested path "updrsScores.partIII"; value = the full reconstructed field value) */
const DRAFT_KEY = 'movement_disorder_assessmentPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const MovementDisorderAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.movement_disorder_assessment) return Array.isArray(r.movement_disorder_assessment) ? r.movement_disorder_assessment : [r.movement_disorder_assessment];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.movement_disorder_assessment) return Array.isArray(dd.movement_disorder_assessment) ? dd.movement_disorder_assessment : [dd.movement_disorder_assessment]; return [dd]; }
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Maps each record's _id (handling _id.$oid) to its render index, then repopulates localEdits +
     pendingEdits + the edited markers so the row badge + Pending Approve button reappear. */
  useEffect(() => {
  ...
  const getFieldValue = useCallback((record, fn, idx) => {
    ...
    // Handle nested fields like updrsScores.partIII
    if (fn.includes('.')) {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Movement Disorder Assessment ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* Stage a DRAFT locally (localStorage) instead of writing to MongoDB. The draft survives refresh
     and shows in the JSX, but stays OUT of the PDF/DB until the user clicks Pending Approve.
     fieldPart = the field name (plain "findings" or dotted nested path "updrsScores.partIII").
     Also clears the section's approved flag so a re-edit returns the button to yellow. */
  ...
  /* save a nested object leaf — stage the whole updated object as a DRAFT under rootField (no DB write) */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Staged edits for THIS record+section: localEdits key = "<fieldPart>-<idx>", fieldPart ∈ fields
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length);
        return fields.includes(fieldPart);
      });
      // Persist each staged field to the DB now. Treat a trailing ".N" (N purely numeric) as arrayIndex.
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "field" or dotted "a.b" or "a.2"
        ...
        const resp = await secureApiClient.put(`/api/edit/movement_disorder_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the record/section approved (audit trail)
      await secureApiClient.put(`/api/edit/movement_disorder_assessment/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this record's committed drafts (only the fields we just persisted) from localStorage
      const store = readDrafts();
    ...
    } catch (err) { console.error('[MovementDisorderAssessment] Approve error:', err); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MOVEMENT DISORDER ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Movement Disorder Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    if (sentences.length > 1) {
      ...
      return (
        <div key={fn}>
          <div className={`rec-mini-card ${fn === 'diagnosis' ? 'diagnosis-card' : ''}`}>
                ...
                <div key={sIdx} className={parsed.isLabeled ? 'rec-mini-card' : ''} style={parsed.isLabeled ? { marginTop: 8 } : undefined}>
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      ...
                      <>
                        <div className="row-content"><span className={`content-value ${fn === 'diagnosis' ? 'diagnosis-value' : ''}`}>{highlightText(parsed.isLabeled ? parsed.value : sentence)}</span><span className="edit-indicator">&#9998;</span></div>
    ...
    return (
      <div key={fn} className={`rec-mini-card ${fn === 'diagnosis' ? 'diagnosis-card' : ''}`}>
        {showLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(strVal); setSaveError(null); } }}>
          {isEditing ? (
            ...
            <>
              <div className="row-content"><span className={`content-value ${fn === 'diagnosis' ? 'diagnosis-value' : ''}`}>{highlightText(strVal)}</span><span className="edit-indicator">&#9998;</span></div>
  ...
  const renderBarChart = (record, idx) => {
    const hoehnYahrData = parseHoehnYahrStage(getFieldValue(record, 'hoehnYahrStage', idx));
    const updrsPartIII = getFieldValue(record, 'updrsScores.partIII', idx);
    const updrsData = parseUPDRSScore(updrsPartIII);
    if (!hoehnYahrData && !updrsData) return null;

    const chartData = [];
    if (hoehnYahrData) {
      chartData.push({ id: 'hoehnYahr', label: 'Hoehn & Yahr Stage', rawValue: hoehnYahrData.label, percentage: (hoehnYahrData.level / 5) * 100, color: hoehnYahrData.color, interpretation: hoehnYahrData.interpretation });
    }
    if (updrsData) {
      chartData.push({ id: 'updrs', label: 'UPDRS Part III (Motor)', rawValue: updrsData.rawValue, percentage: updrsData.percentage, color: updrsData.color, interpretation: updrsData.interpretation });
    }

    // Filter chart data based on search
    const filteredChartData = (() => {
      if (!searchTerm.trim() || record._showAllSections) return chartData;
      const phrase = searchTerm.toLowerCase().trim();
      const sectionTitle = 'severity indicators';
      if (sectionTitle.includes(phrase) || phrase.includes(sectionTitle)) return chartData;
      return chartData.filter(chart => {
        const combined = `${chart.label} ${chart.rawValue} ${chart.interpretation}`.toLowerCase();
      ...
      <div className="section">
        <div className="chart-container">
          <div className="section-header" style={{ borderBottom: '1px solid rgba(96, 165, 250, 0.4)', paddingBottom: 10, marginBottom: 12 }}>
            <h4 className="section-title">{highlightText('Severity Indicators')}</h4>
          </div>
          <div className="chart-legend">
            <div className="legend-item"><span className="legend-color" style={{ backgroundColor: '#22c55e' }}></span><span>Mild</span></div>
            <div className="legend-item"><span className="legend-color" style={{ backgroundColor: '#3b82f6' }}></span><span>Moderate</span></div>
            <div className="legend-item"><span className="legend-color" style={{ backgroundColor: '#f59e0b' }}></span><span>Mod. Severe</span></div>
            <div className="legend-item"><span className="legend-color" style={{ backgroundColor: '#f97316' }}></span><span>Severe</span></div>
            <div className="legend-item"><span className="legend-color" style={{ backgroundColor: '#ef4444' }}></span><span>Very Severe</span></div>
          </div>
          {filteredChartData.map((chart) => (
            <div key={chart.id} className="bar-chart-row">
              ...
              {chart.id === 'hoehnYahr' && (
                <div className="bar-scale">
                  <span className="scale-item" style={{ left: '0%', color: '#22c55e' }}>1</span>
                  <span className="scale-item" style={{ left: '20%', color: '#22c55e' }}>1.5</span>
                  <span className="scale-item" style={{ left: '40%', color: '#3b82f6' }}>2</span>
                  <span className="scale-item" style={{ left: '50%', color: '#3b82f6' }}>2.5</span>
                  <span className="scale-item" style={{ left: '60%', color: '#f59e0b' }}>3</span>
                  <span className="scale-item" style={{ left: '80%', color: '#f97316' }}>4</span>
                  <span className="scale-item" style={{ left: '100%', color: '#ef4444' }}>5</span>
                </div>
              )}
              {chart.id === 'updrs' && (
                <div className="bar-scale">
                  <span className="scale-item" style={{ left: '0%' }}>0</span>
                  <span className="scale-item" style={{ left: '25%' }}>33</span>
                  <span className="scale-item" style={{ left: '50%' }}>66</span>
                  <span className="scale-item" style={{ left: '75%' }}>99</span>
                  <span className="scale-item" style={{ left: '100%' }}>132</span>
                </div>
              )}
              <div className="bar-interpretation" style={{ color: chart.color }}>{highlightText(chart.interpretation)}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="movement-disorder-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Movement Disorder Assessment</h2></div>
        <div className="empty-state">No movement disorder assessment data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="movement-disorder-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Movement Disorder Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MovementDisorderAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Movement_Disorder_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search assessments (diagnosis, findings, UPDRS, Hoehn-Yahr...)" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Movement Disorder Assessment ${idx + 1}`)}</h3>
            </div>
            {renderBarChart(record, idx)}
            ...
            {renderSection(record, idx, 'updrs-scores')}
...
export default MovementDisorderAssessmentDocument;
