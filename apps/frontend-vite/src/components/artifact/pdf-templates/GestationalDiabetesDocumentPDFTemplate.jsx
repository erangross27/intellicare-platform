/**
 * GestationalDiabetesDocumentPDFTemplate.jsx
 ...
 * status/screeningMethod/gdmClass/managementType enum canonical, DOTTED-PATH fields (ogttResults.*,
 * glucoseMonitoring.*, glucoseLogReview.*, glucoseMonitoringGoals.*) resolved via getVal, results object
 * stacked, recommendations date-grouped, narratives split on [.;] with a single-letter genus guard +
 ...
 * Static PHI footer. Collection: gestational_diabetes.
 */
...
const SECTION_ORDER = ['session-info', 'screening-diagnosis', 'glucose-monitoring', 'glucose-log-review', 'lab-goals', 'education-risk', 'assessment-plan', 'recommendations-section', 'results-section'];
const SECTION_TITLES = {
  'session-info': 'Session Information',
  'screening-diagnosis': 'Screening and Diagnosis',
  'glucose-monitoring': 'Glucose Monitoring',
  'glucose-log-review': 'Glucose Log Review',
  'lab-goals': 'Lab and Monitoring Goals',
  'education-risk': 'Education and Risk Factors',
...
const FIELD_LABELS = {
  ...
  screeningMethod: 'Screening Method', glucoseChallengeTest: 'Glucose Challenge Test',
  'ogttResults.fasting': 'OGTT - Fasting', 'ogttResults.oneHour': 'OGTT - One Hour',
  'ogttResults.twoHour': 'OGTT - Two Hour', 'ogttResults.threeHour': 'OGTT - Three Hour',
  gdmClass: 'GDM Class', managementType: 'Management Type',
  'glucoseMonitoring.fastingGoal': 'Fasting Goal', 'glucoseMonitoring.postprandialGoal': 'Postprandial Goal',
  'glucoseMonitoring.averageValues.fasting': 'Average - Fasting', 'glucoseMonitoring.averageValues.postBreakfast': 'Average - Post Breakfast',
  'glucoseMonitoring.averageValues.postLunch': 'Average - Post Lunch', 'glucoseMonitoring.averageValues.postDinner': 'Average - Post Dinner',
  'glucoseLogReview.fasting': 'Log - Fasting', 'glucoseLogReview.postBreakfast': 'Log - Post Breakfast',
  'glucoseLogReview.postLunch': 'Log - Post Lunch', 'glucoseLogReview.postDinner': 'Log - Post Dinner',
  'glucoseLogReview.daysCovered': 'Days Covered', hba1c: 'HbA1c',
  'glucoseMonitoringGoals.fasting': 'Goal - Fasting', 'glucoseMonitoringGoals.oneHourPostprandial': 'Goal - 1-Hour Postprandial',
  'glucoseMonitoringGoals.twoHourPostprandial': 'Goal - 2-Hour Postprandial',
  insulinStorageInstructions: 'Insulin Storage Instructions', macrosomiaThreshold: 'Macrosomia Threshold',
  cesareanThreshold: 'Cesarean Threshold', postpartumDiabetesRisk: 'Postpartum Diabetes Risk',
  gdmRecurrenceRisk: 'GDM Recurrence Risk',
...
const SECTION_FIELDS = {
  ...
  'screening-diagnosis': ['screeningMethod', 'glucoseChallengeTest', 'ogttResults.fasting', 'ogttResults.oneHour', 'ogttResults.twoHour', 'ogttResults.threeHour', 'gdmClass', 'managementType'],
  'glucose-monitoring': ['glucoseMonitoring.fastingGoal', 'glucoseMonitoring.postprandialGoal', 'glucoseMonitoring.averageValues.fasting', 'glucoseMonitoring.averageValues.postBreakfast', 'glucoseMonitoring.averageValues.postLunch', 'glucoseMonitoring.averageValues.postDinner'],
  'glucose-log-review': ['glucoseLogReview.fasting', 'glucoseLogReview.postBreakfast', 'glucoseLogReview.postLunch', 'glucoseLogReview.postDinner', 'glucoseLogReview.daysCovered'],
  'lab-goals': ['hba1c', 'glucoseMonitoringGoals.fasting', 'glucoseMonitoringGoals.oneHourPostprandial', 'glucoseMonitoringGoals.twoHourPostprandial'],
  'education-risk': ['insulinStorageInstructions', 'macrosomiaThreshold', 'cesareanThreshold', 'postpartumDiabetesRisk', 'gdmRecurrenceRisk'],
  'assessment-plan': ['findings', 'assessment', 'plan', 'notes'],
  'recommendations-section': ['recommendations'],
  'results-section': ['results'],
};
const DATE_FIELDS = ['date'];
const ENUM_FIELDS = ['status', 'screeningMethod', 'gdmClass', 'managementType'];
const ENUM_OPTIONS = {
  status: ['Active', 'Completed', 'Not Active'],
  screeningMethod: ['One-step', 'Two-step'],
  gdmClass: ['A1', 'A2', 'B', 'C', 'D', 'F', 'R', 'T', 'H'],
  managementType: ['Diet', 'Metformin', 'Insulin', 'Glyburide'],
};
...
const GestationalDiabetesDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].gestational_diabetes && Array.isArray(templateData[0].gestational_diabetes)) records = templateData[0].gestational_diabetes;
    else records = templateData;
  } else if (templateData && templateData.gestational_diabetes) {
    records = Array.isArray(templateData.gestational_diabetes) ? templateData.gestational_diabetes : [templateData.gestational_diabetes];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.gestational_diabetes ? (Array.isArray(dd.gestational_diabetes) ? dd.gestational_diabetes : [dd.gestational_diabetes]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Gestational Diabetes</Text></View>
        <Text style={styles.emptyState}>No gestational diabetes records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Gestational Diabetes ${idx + 1}`}</Text>
...
export default GestationalDiabetesDocumentPDFTemplate;
