...
/**
 * NeurologyProgressNotesDocumentPDFTemplate - box-free canonical (LETTER)
 ...
 * Record date: consultDate is a rendered FIELD (not a header date) - NEVER createdAt/updatedAt.
 */
...
const SECTION_ORDER = ['clinical-info', 'exam-findings', 'diagnosis', 'treatment-plan', 'follow-up-plan', 'recommendations'];
...
const NeurologyProgressNotesDocumentPDFTemplate = ({ document: docProp, data = docProp }) => {
  let records = [];
  if (Array.isArray(data)) {
    if (data.length === 1 && data[0]?.neurology_progress_notes) records = Array.isArray(data[0].neurology_progress_notes) ? data[0].neurology_progress_notes : [data[0].neurology_progress_notes];
    else records = data;
  } else if (data?.neurology_progress_notes) records = Array.isArray(data.neurology_progress_notes) ? data.neurology_progress_notes : [data.neurology_progress_notes];
    ...
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <Text style={styles.documentTitle}>Neurology Progress Notes</Text>
          <Text style={styles.noData}>No neurology progress notes records available.</Text>
        </Page>
      </Document>
    );
  }
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, rIdx) => (
          <View key={rIdx}>
            <Text style={styles.recordTitle} break={rIdx > 0}>{safeString(`Neurology Progress Notes ${rIdx + 1}`)}</Text>
...
export default NeurologyProgressNotesDocumentPDFTemplate;
