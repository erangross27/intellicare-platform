 ...
 *
 * 7 Sections:
 *   1. provider-details: date, provider, facility
 *   2. assessment: assessment
 *   3. hydration: hydration
 *   4. nephrotoxin-avoidance: nephrotoxinAvoidance
...
const DRAFT_KEY = 'renal_protection_planPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  assessment: 'Assessment',
  hydration: 'Hydration',
  nephrotoxinAvoidance: 'Nephrotoxin Avoidance',
  monitoring: 'Monitoring',
  results: 'Results',
  plan: 'Plan',
  recommendations: 'Recommendations',
...
const SECTION_ORDER = ['provider-details', 'findings', 'assessment', 'hydration', 'nephrotoxin-avoidance', 'monitoring', 'results', 'plan', 'recommendations', 'consultations', 'notes-status'];

const BOOLEAN_FIELDS = [];
const DATE_FIELDS = ['date'];
const NUMBER_FIELDS = [];
const ARRAY_FIELDS = ['nephrotoxinAvoidance', 'monitoring', 'recommendations'];
const OBJECT_FIELDS = ['results'];
const SENTENCE_FIELDS = ['hydration', 'consultations', 'findings', 'assessment', 'plan', 'notes'];
const STRING_FIELDS = ['provider', 'facility', 'findings', 'assessment', 'hydration', 'plan', 'consultations', 'notes', 'status'];
...
const RenalProtectionPlanDocument = ({ document: docProp }) => {
  ...
  /* Stage a DRAFT locally (NO DB write). localStorage keeps it across refresh; Approve commits it.
     localEdits/pendingEdits keyed by `${fn}-${idx}`; the draft store is keyed by recordId then fn. */
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    setSaveError(null);
    // Stage locally only — NO DB write until Approve.
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    // Build the updated object clone, then stage it locally only (NO DB write until Approve).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // Collect this record's staged edits belonging to this section.
      // editKey = `${fieldPart}-${idx}`; arrayIndex applies ONLY when the trailing dot-segment is numeric.
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/renal_protection_plan/${id}/edit`, payload);
  ...
  const renderStringField = (record, fn, idx, sid, title) => {
    ...
    /* Multi-sentence: per-sentence editing only for designated narrative fields (SENTENCE_FIELDS).
       Short fields (provider, facility, status) stay simple even if they contain a period. */
    if (sentences.length > 1 && SENTENCE_FIELDS.includes(fn)) {
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; number+unit -> number input, ratio stays text, text -> textarea) ═══════ */
    ...
    <div className="renal-protection-plan-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<RenalProtectionPlanDocumentPDFTemplate document={pdfData} />} fileName={`renal-protection-plan-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search renal protection plan..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
