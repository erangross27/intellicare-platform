...
const SECTIONS = [
  { id: 'programDetails', title: 'Program Details', fields: ['date', 'type', 'provider', 'facility'] },
  ...
  { id: 'resultsStatus', title: 'Results & Status', fields: ['results', 'status'] },
...
export default function SmokingCessationProgramDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Smoking Cessation Program</Text></View>
    {!records.length && <Text style={styles.noDataText}>No smoking cessation program data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Smoking Cessation Program Record {recordIndex + 1}</Text></View>
