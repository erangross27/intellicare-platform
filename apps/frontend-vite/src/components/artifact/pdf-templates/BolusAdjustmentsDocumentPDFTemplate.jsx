...
const COLLECTION = 'bolus_adjustments';
const COMMA_SPLIT_FIELDS = [];
const ARRAY_FIELDS = new Set(['recommendations']);
const OBJECT_FIELDS = new Set(['results', 'additionalData']);
const NARRATIVE_FIELDS = new Set(['reason', 'findings', 'assessment', 'plan', 'notes']);
...
const LABELS = {
  ...
  mealTime: 'Meal Time', oldRatio: 'Old Ratio', newRatio: 'New Ratio', reason: 'Reason',
...
const SECTIONS = [
  ...
  { title: 'Bolus Adjustment', fields: ['mealTime', 'oldRatio', 'newRatio', 'reason'] },
...
const BolusAdjustmentsDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Bolus Adjustments</Text><Text style={styles.noData}>No bolus adjustment data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Bolus Adjustments</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Bolus Adjustment {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
export default BolusAdjustmentsDocumentPDFTemplate;
