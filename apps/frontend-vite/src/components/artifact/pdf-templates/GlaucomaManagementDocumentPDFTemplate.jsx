/**
 * GlaucomaManagementDocumentPDFTemplate.jsx
 * Box-free B&W LETTER (canonical, memory 6a2d6af6) — mirrors the JSX: real record.date (never createdAt),
 * status enum canonical, medicalTherapy object (firstLine/secondLine/precautions[]/iopTarget) STACKED,
 * results recursive object, recommendations date-grouped, narratives split on [.;] with abbrev/single-initial/
 * genus guard + labeled OD:/OS: comma-split into subtitle groups, values numbered ('1.' even singles),
 * single-name label gate. Rule #74: each field is ONE wrap={false} atomic View with the sectionTitle riding
 * INSIDE the first present field's View. Static PHI footer. Collection: glaucoma_management.
 */
...
const SECTION_ORDER = ['session-info', 'clinical-notes', 'treatment-plan', 'procedures', 'follow-up'];
...
const FIELD_LABELS = {
  ...
  findings: 'Findings', assessment: 'Assessment', results: 'Results', notes: 'Notes',
  plan: 'Plan', medicalTherapy: 'Medical Therapy',
  laserTherapy: 'Laser Therapy', surgicalConsideration: 'Surgical Consideration',
  monitoringPlan: 'Monitoring Plan', recommendations: 'Recommendations',
};
const SECTION_FIELDS = {
  'session-info': ['date', 'provider', 'facility', 'status'],
  'clinical-notes': ['findings', 'assessment', 'results', 'notes'],
  'treatment-plan': ['plan', 'medicalTherapy'],
  'procedures': ['laserTherapy', 'surgicalConsideration'],
  'follow-up': ['monitoringPlan', 'recommendations'],
};
...
const OBJECT_FIELDS = ['medicalTherapy', 'results'];
const OBJECT_ARRAY_FIELDS = ['recommendations'];
const SUB_LABEL_OVERRIDES = { firstLine: 'First Line', secondLine: 'Second Line', precautions: 'Precautions', iopTarget: 'IOP Target' };
...
const humanizeKey = (key) => {
  ...
  if (SUB_LABEL_OVERRIDES[key]) return SUB_LABEL_OVERRIDES[key];
...
const renderObjectNode = (label, value, keyPath, depth) => {
  ...
  if (isScalar(value)) {
    ...
    return (
      <View key={keyPath}>
        {label ? <Text style={LabelTag}>{label}</Text> : null}
        <Text style={styles.value}>1. {s}</Text>
      </View>
    );
  }
...
const GlaucomaManagementDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].glaucoma_management && Array.isArray(templateData[0].glaucoma_management)) records = templateData[0].glaucoma_management;
    else records = templateData;
  } else if (templateData && templateData.glaucoma_management) {
    records = Array.isArray(templateData.glaucoma_management) ? templateData.glaucoma_management : [templateData.glaucoma_management];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.glaucoma_management ? (Array.isArray(dd.glaucoma_management) ? dd.glaucoma_management : [dd.glaucoma_management]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Glaucoma Management</Text></View>
        <Text style={styles.emptyState}>No glaucoma management records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Glaucoma Management ${idx + 1}`}</Text>
...
export default GlaucomaManagementDocumentPDFTemplate;
