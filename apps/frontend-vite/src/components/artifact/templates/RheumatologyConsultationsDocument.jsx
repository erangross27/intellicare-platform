/**
 * RheumatologyConsultationsDocument.jsx
 * July 2026 — recursive, dot-path editable rheumatology consultations
...
import RheumatologyConsultationsDocumentPDFTemplate from '../pdf-templates/RheumatologyConsultationsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './RheumatologyConsultationsDocument.css';

const DRAFT_KEY = 'rheumatology_consultationsPendingEdits';
const SECTION_TITLES = {
  presentingConcern: 'Presenting Concern',
  jointExamination: 'Joint Examination',
  diseaseActivity: 'Disease Activity',
  autoimmuneTesting: 'Autoimmune Testing',
  inflammatoryMarkers: 'Inflammatory Markers',
  synovialAssessment: 'Synovial Assessment',
  currentTreatment: 'Current Treatment',
};
const FIELD_LABELS = {
  chiefComplaint: 'Chief Complaint', jointPainDuration: 'Joint Pain Duration', morningStiffnessDuration: 'Morning Stiffness Duration', functionalStatus: 'Functional Status',
  affectedJoints: 'Affected Joints', swollenJointCount: 'Swollen Joint Count', tenderJointCount: 'Tender Joint Count',
  das28Score: 'DAS28 Score', cdaiScore: 'CDAI Score', patientGlobalAssessment: 'Patient Global Assessment',
  rheumatoidFactor: 'Rheumatoid Factor', antiCcpAntibodies: 'Anti-CCP Antibodies', anaPattern: 'ANA Pattern', anaTiter: 'ANA Titer', hlaB27Status: 'HLA-B27 Status', complementC3Level: 'Complement C3 Level', complementC4Level: 'Complement C4 Level',
  crpLevel: 'CRP Level', esrValue: 'ESR Value', uricAcidLevel: 'Uric Acid Level', synovialFluidAnalysis: 'Synovial Fluid Analysis', crystalType: 'Crystal Type',
  currentDmards: 'Current DMARDs', biologicTherapy: 'Biologic Therapy', corticosteroidUse: 'Corticosteroid Use',
};
const SECTION_FIELDS = {
  presentingConcern: ['chiefComplaint', 'jointPainDuration', 'morningStiffnessDuration', 'functionalStatus'],
  jointExamination: ['affectedJoints', 'swollenJointCount', 'tenderJointCount'],
  diseaseActivity: ['das28Score', 'cdaiScore', 'patientGlobalAssessment'],
  autoimmuneTesting: ['rheumatoidFactor', 'antiCcpAntibodies', 'anaPattern', 'anaTiter', 'hlaB27Status', 'complementC3Level', 'complementC4Level'],
  inflammatoryMarkers: ['crpLevel', 'esrValue', 'uricAcidLevel'],
  synovialAssessment: ['synovialFluidAnalysis', 'crystalType'],
  currentTreatment: ['currentDmards', 'biologicTherapy', 'corticosteroidUse'],
};
const DATE_FIELDS = ['date'];
const ENUM_FIELDS = ['status'];
const NUMBER_FIELDS = ['morningStiffnessDuration', 'swollenJointCount', 'tenderJointCount', 'das28Score', 'cdaiScore', 'patientGlobalAssessment', 'complementC3Level', 'complementC4Level', 'crpLevel', 'esrValue', 'uricAcidLevel'];
const NUMBER_STRING_FIELDS = [];
const OBJECT_FIELDS = [];
const NARRATIVE_PATHS = [];
const PARENTHETICAL_LABEL_FIELDS = [];
const COMMA_FIELDS = ['chiefComplaint', 'functionalStatus', 'affectedJoints', 'currentDmards', 'synovialFluidAnalysis'];
const COMMA_ARRAY_FIELDS = ['affectedJoints', 'currentDmards'];
const ARRAY_FIELDS = ['affectedJoints', 'currentDmards'];
const SEMICOLON_FIELDS = ['chiefComplaint', 'functionalStatus', 'synovialFluidAnalysis'];
...
const KEY_LABELS = {
  crpLevel: 'CRP Level', esrValue: 'ESR Value', antiCcpAntibodies: 'Anti-CCP Antibodies', hlaB27Status: 'HLA-B27 Status',
};
...
const RheumatologyConsultationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.rheumatology_consultations) return Array.isArray(record.rheumatology_consultations) ? record.rheumatology_consultations : [record.rheumatology_consultations];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.rheumatology_consultations) return Array.isArray(nested.rheumatology_consultations) ? nested.rheumatology_consultations : [nested.rheumatology_consultations];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/rheumatology_consultations/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/rheumatology_consultations/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[RheumatologyConsultations] Approve error:', error);
      ...
      <div className="rec-mini-card nested-mini-card regular-row-group">
        {rows.map(({ itemIndex, partIndex, part }) => {
          ...
          props.onSave = () => {
            ...
            parts[partIndex] = editValue.trim();
  ...
  const copyAll = useCallback(async () => {
    let text = '=== RHEUMATOLOGY CONSULTATIONS ===\n\n';
    records.forEach((record, recordIndex) => {
      text += 'Rheumatology Consultations ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="rheumatology-consultations-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Rheumatology Consultations</h2></div><div className="empty-state">No rheumatology consultations records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Rheumatology Consultations ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="rheumatology-consultations-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Rheumatology Consultations</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<RheumatologyConsultationsDocumentPDFTemplate document={pdfData} />} fileName="Rheumatology_Consultations.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search rheumatology consultationss..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Rheumatology Consultations ' + (recordIndex + 1))}</h3></div>
...
export default RheumatologyConsultationsDocument;
