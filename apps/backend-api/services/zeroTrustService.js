...
class ZeroTrustService {
  ...
  async initializeCertificateAuthority() {
    const caDir = path.join(__dirname, '../config/ca');
  ...
  async createSecureSession(userId, userInfo, clientInfo, practiceDb = null) {
    ...
    if (practiceDb) {
      try {
        ...
        await SecureDataAccess.update('zerotrustsessions', { _id: sessionDoc._id }, sessionDoc, {
          serviceId: 'zero-trust-service',
          apiKey: this.serviceToken?.apiKey || this.serviceToken,
          practiceId: practiceDb.name
        });
  ...
  async validateAndRefreshSession(sessionId, clientInfo, practiceDb = null) {
    ...
    try {
      ...
      let sessionDoc = await SessionModel.findActiveBySessionId(sessionId);
      ...
      if (!sessionDoc) {
        ...
        const anySession = await SecureDataAccess.query('sessionmodels', { sessionId }, { limit: 1 }, {
          serviceId: 'zero-trust-service',
          apiKey: this.serviceToken?.apiKey || this.serviceToken,
          practiceId: 'global'
        ...
        if (anySession) {
          ...
          const now = new Date();
          const timeSinceActivity = now.getTime() - anySession.lastActivity.getTime();

          if (timeSinceActivity <= this.sessionTimeout && anySession.expiresAt > now) {
            console.log('🔄 ZERO TRUST: Reactivating valid but inactive session');
            anySession.isActive = true;
            anySession.lastActivity = now;
            await SecureDataAccess.update('zerotrustsessions', { _id: anySession._id }, anySession, {
              serviceId: 'zero-trust-service',
              apiKey: this.serviceToken?.apiKey || this.serviceToken,
              practiceId: 'global'
            });
      ...
      // Activity update handled via SecureDataAccess
      ...
      if (Date.now() - lastRefresh.getTime() > this.tokenRefreshInterval) {
        ...
        await sessionDoc.refreshToken(newToken);
      ...
      // Risk score update handled via SecureDataAccess
  ...
  async loadPermissionMatrix() {
    const permissionsFile = path.join(__dirname, '../config/permissions-matrix.json');
  ...
  async getUserPermissions(userId) {
    ...
    try {
      // Fetch actual user permissions from database
      ...
      const practices = await SecureDataAccess.query('practices', {}, {}, context);

      for (const practice of practices) {
        try {
          const practiceDb = await /* SECURITY: Direct database access removed - use SecureDataAccess */null;
          const User = practiceDb.model('User', require('../models/User').schema);
          const user = await SecureDataAccess.query('users', { _id: new ObjectId(userId) }, { limit: 1 }, context).then(results => results[0]);
    ...
    try {
      ...
      for (const { subdomain, db } of practices) {
        try {
          ...
          // FIX: Use the actual database name, not db.name which can be undefined or corrupted
          const dbName = subdomain === 'global' ? 'intellicare_practice_global' : `intellicare_practice_${subdomain}`;
