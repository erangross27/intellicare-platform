/**
 * SuicideRiskAssessmentDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: suicide_risk_assessment
 *
 * 11 Sections:
 *   1. record-info: date, provider, facility, riskLevel
 *   2. ideation: ideation.current, ideation.passive, ideation.active, ideation.frequency, ideation.duration
 *   3. plan-details: plan.hasPlan, plan.method, plan.means, plan.timeline
 *   4. intent-section: intent
 *   5. history: previousAttempts, psychiatricHospitalizations
 *   6. risk-factors: riskFactors
 *   7. protective-factors: protectiveFactors
 *   8. interventions-section: interventions
 *   9. columbia-scale: columbiaScale
 *  10. findings-section: findings
 *  11. assessment-section: assessment
 *  12. recommendations-section: recommendations (array of {recommendation, date})
 *  13. results-section: results (recursive object)
 *  14. notes-section: notes
 *  15. status-section: status
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import SuicideRiskAssessmentDocumentPDFTemplate from '../pdf-templates/SuicideRiskAssessmentDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SuicideRiskAssessmentDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the field name, e.g. "intent" or
   "ideation.current" or "recommendations" — the full field value is stored, never a per-index slice) */
const DRAFT_KEY = 'suicide_risk_assessmentPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'columbia-scale': 'Columbia Scale (C-SSRS)',
...
const FIELD_LABELS = {
  ...
  'ideation.current': 'Current',
  'ideation.passive': 'Passive',
  'ideation.active': 'Active',
  'ideation.frequency': 'Frequency',
  'ideation.duration': 'Duration',
  'plan.hasPlan': 'Has Plan',
  'plan.method': 'Method',
  'plan.means': 'Means',
  'plan.timeline': 'Timeline',
  intent: 'Intent',
  previousAttempts: 'Previous Attempts',
  psychiatricHospitalizations: 'Psychiatric Hospitalizations',
  riskFactors: 'Risk Factors',
  protectiveFactors: 'Protective Factors',
  interventions: 'Interventions',
  columbiaScale: 'Columbia Scale',
  findings: 'Findings',
...
const SECTION_FIELDS = {
  'record-info': ['date', 'provider', 'facility', 'riskLevel'],
  'ideation': ['ideation.current', 'ideation.passive', 'ideation.active', 'ideation.frequency', 'ideation.duration'],
  'plan-details': ['plan.hasPlan', 'plan.method', 'plan.means', 'plan.timeline'],
  'intent-section': ['intent'],
  'history': ['previousAttempts', 'psychiatricHospitalizations'],
  'risk-factors': ['riskFactors'],
  'protective-factors': ['protectiveFactors'],
  'interventions-section': ['interventions'],
  'columbia-scale': ['columbiaScale'],
  'findings-section': ['findings'],
  'assessment-section': ['assessment'],
  'recommendations-section': ['recommendations'],
  'results-section': ['results'],
...
const BOOLEAN_FIELDS = ['ideation.passive', 'ideation.active', 'plan.hasPlan', 'previousAttempts'];
const DATE_FIELDS = ['date'];
const ARRAY_FIELDS = ['riskFactors', 'protectiveFactors', 'interventions'];
/* Array-of-objects {recommendation, date} (date-grouped, editable) */
const OBJECT_ARRAY_FIELDS = ['recommendations'];
/* Generic recursive OBJECT fields */
const OBJECT_FIELDS = ['results'];
/* Short coded single-value strings → simple editable (NOT per-sentence) */
const SIMPLE_STRING_FIELDS = ['provider', 'facility', 'riskLevel', 'status', 'columbiaScale'];
const STRING_FIELDS = ['provider', 'facility', 'riskLevel', 'ideation.current', 'ideation.frequency', 'ideation.duration', 'plan.method', 'plan.means', 'plan.timeline', 'intent', 'psychiatricHospitalizations', 'columbiaScale', 'findings', 'assessment', 'notes', 'status'];

/* ═══════ OBJECT HELPERS (donor: PointOfCareUltrasoundHeartRate) ═══════ */
const KEY_OVERRIDES = {};
...
const getRiskLevelClass = (level) => {
  ...
  if (l.includes('acute')) return 'acute';
...
/* ═══════ COMPONENT ═══════ */
const SuicideRiskAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.suicide_risk_assessment) return Array.isArray(r.suicide_risk_assessment) ? r.suicide_risk_assessment : [r.suicide_risk_assessment];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.suicide_risk_assessment) return Array.isArray(dd.suicide_risk_assessment) ? dd.suicide_risk_assessment : [dd.suicide_risk_assessment]; return [dd]; }
      return [r];
    });
  ...
  /* getFieldValue: supports dot-path (e.g. "ideation.current") */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Suicide Risk Assessment ${idx + 1}`.toLowerCase();
  ...
  const pdfData = useMemo(() => {
    return filteredRecords.map((record, idx) => {
      const merged = { ...record };
      if (merged.ideation) merged.ideation = { ...merged.ideation };
      if (merged.plan) merged.plan = { ...merged.plan };
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* Stage a DRAFT for one field locally + write it to the pending-drafts localStorage store (survives
     refresh). NOT written to MongoDB and NOT shown in the PDF until the user clicks Approve.
     `fn` is the field name (may be dotted e.g. "ideation.current"); `value` is the FULL field value
     (string/array/object). Records clear any prior approved state so re-edits return to Pending Approve. */
  ...
  // Save = stage a DRAFT locally (no DB write). handleApproveSection commits it later.
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.summary) — value stays a STRING/boolean */
  /* save a nested OBJECT leaf by dot-path (e.g. results.summary) — staged as a DRAFT (no DB write).
     The FULL root object (with the leaf updated) is stored at `${rootField}-${idx}`; Approve commits it. */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Collect this record's pending edits for fields in this section (key = "${fn}-${idx}").
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k]) return false;
        const m = k.match(/^(.+)-(\d+)$/);
        if (!m || parseInt(m[2]) !== idx) return false;
        return fields.includes(m[1]);
      });
      // Persist each staged field to the DB now. The trailing dot-segment is an arrayIndex ONLY when
      // it is purely numeric (e.g. "foo.2"); dotted field NAMES like "ideation.current" stay whole.
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, editKey.lastIndexOf('-')); // the field name
        ...
        const resp = await secureApiClient.put(`/api/edit/suicide_risk_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the record/section approved (audit trail)
      await secureApiClient.put(`/api/edit/suicide_risk_assessment/${id}/approve`, { sectionId: sid, approved: true });
      ...
      const store = readDrafts();
      if (store[id]) {
        toCommit.forEach(k => { const fp = k.slice(0, k.lastIndexOf('-')); delete store[id][fp]; });
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        recs.forEach(rec => {
          ...
          if (!recText && !recDate) return;
          text += `${n++}. ${recText}${recDate ? ` (${recDate})` : ''}\n`;
        });
        text += '\n';
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== SUICIDE RISK ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Suicide Risk Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
      ...
      <div key={fn} className="rec-mini-card">
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(val ? 'Yes' : 'No'); setSaveError(null); } }}>
          {isEditing ? (
            ...
            <>
              <div className="row-content"><span className={`boolean-badge ${val ? 'yes' : 'no'}`}>{highlightText(displayVal)}</span><span className="edit-indicator">&#9998;</span></div>
  ...
  /* ═══════ RENDER: ARRAY FIELD ═══════ */
  const renderArrayField = (record, fn, idx, sid, extraRowClass) => {
    const val = getFieldValue(record, fn, idx);
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          return (
            <div key={itemIdx}>
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row ${extraRowClass || ''}`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; number+unit -> number input, ratio stays text, bool -> select) ═══════ */
  ...
  const renderSection = (record, idx, sid, extraContainerClass, extraRowClass) => {
    const title = SECTION_TITLES[sid];
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      return hasVal(val) || (BOOLEAN_FIELDS.includes(f) && val !== null && val !== undefined);
    });
    ...
    return (
      <div key={sid} className="section">
        <div className={`mini-cards-container ${extraContainerClass || ''}`}>
          ...
          {fields.map(f => {
            ...
            if (ARRAY_FIELDS.includes(f)) return renderArrayField(record, f, idx, sid, extraRowClass);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="suicide-risk-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Suicide Risk Assessment</h2></div>
        <div className="empty-state">No suicide risk assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="suicide-risk-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Suicide Risk Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<SuicideRiskAssessmentDocumentPDFTemplate document={pdfData} />} fileName={`suicide-risk-assessment-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search suicide risk assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          <div key={idx} className="record-card">
            <div className="record-header">
              <div className="record-meta-row">
                {hasVal(record.date || getFieldValue(record, 'date', idx)) && (
                  <span className="record-date">{formatDate(getFieldValue(record, 'date', idx) || record.date)}</span>
                )}
                {hasVal(record.riskLevel || getFieldValue(record, 'riskLevel', idx)) && (
                  <span className={`risk-level-badge ${getRiskLevelClass(getFieldValue(record, 'riskLevel', idx) || record.riskLevel)}`}>
                    {highlightText(getFieldValue(record, 'riskLevel', idx) || record.riskLevel)}
                  </span>
                )}
              </div>
              <h3 className="record-name">{highlightText(`Suicide Risk Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'record-info')}
            {renderSection(record, idx, 'ideation')}
            {renderSection(record, idx, 'plan-details')}
            {renderSection(record, idx, 'intent-section')}
            {renderSection(record, idx, 'history')}
            {renderSection(record, idx, 'risk-factors', 'risk-factors-container', 'risk-factor-row')}
            {renderSection(record, idx, 'protective-factors', 'protective-factors-container', 'protective-factor-row')}
            {renderSection(record, idx, 'interventions-section')}
            {renderSection(record, idx, 'columbia-scale')}
            ...
            {renderSection(record, idx, 'status-section')}
          </div>
        ))}
      </div>
    </div>
...
export default SuicideRiskAssessmentDocument;
