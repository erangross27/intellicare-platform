/**
 * FrailtyAssessmentDocumentPDFTemplate.jsx
 * Box-free B&W LETTER (canonical, memory 6a2d6af6) — mirrors the JSX: real record.date (never createdAt),
 * status enum canonical, sarcopenia boolean Yes/No, values numbered ('1.' even singles), single-name label gate,
 * OBJECT `results` leaves stacked, recommendations date-grouped, narratives split on [.;] (labeled → sub-label +
 * numbered comma items). safeString scrubs non-Latin-1 glyphs. Rule #74: each field is ONE wrap={false} atomic
 * View with the sectionTitle riding INSIDE the first present field's View. Static PHI footer. Collection: frailty_assessment.
 */
...
const SECTION_ORDER = ['record-info', 'frailty-phenotype', 'scales', 'clinical', 'results-section', 'recommendations-section', 'notes-status'];
const SECTION_TITLES = {
  'record-info': 'Record Information',
  'frailty-phenotype': 'Frailty Phenotype',
  'scales': 'Scales',
  'clinical': 'Clinical',
...
const FIELD_LABELS = {
  ...
  sarcopenia: 'Sarcopenia', frailtyIndex: 'Frailty Index', gripStrength: 'Grip Strength',
  walkingSpeed: 'Walking Speed', exhaustion: 'Exhaustion', physicalActivity: 'Physical Activity',
  unintentionalWeightLoss: 'Unintentional Weight Loss', clinicalFrailtyScale: 'Clinical Frailty Scale',
  type: 'Type', findings: 'Findings', assessment: 'Assessment', plan: 'Plan', notes: 'Notes', status: 'Status',
  results: 'Results', recommendations: 'Recommendations',
};
const SECTION_FIELDS = {
  'record-info': ['date', 'provider', 'facility'],
  'frailty-phenotype': ['frailtyIndex', 'gripStrength', 'walkingSpeed', 'exhaustion', 'physicalActivity', 'unintentionalWeightLoss'],
  'scales': ['clinicalFrailtyScale', 'sarcopenia'],
...
const BOOLEAN_FIELDS = ['sarcopenia'];
...
const FrailtyAssessmentDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].frailty_assessment && Array.isArray(templateData[0].frailty_assessment)) records = templateData[0].frailty_assessment;
    else records = templateData;
  } else if (templateData && templateData.frailty_assessment) {
    records = Array.isArray(templateData.frailty_assessment) ? templateData.frailty_assessment : [templateData.frailty_assessment];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.frailty_assessment ? (Array.isArray(dd.frailty_assessment) ? dd.frailty_assessment : [dd.frailty_assessment]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Frailty Assessment</Text></View>
        <Text style={styles.emptyState}>No frailty assessment records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Frailty Assessment ${idx + 1}`}</Text>
...
export default FrailtyAssessmentDocumentPDFTemplate;
