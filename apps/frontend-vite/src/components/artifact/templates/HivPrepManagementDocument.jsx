/**
 * HivPrepManagementDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: hiv_prep_management
 *
 * 5 Sections:
 *   1. prep-indication:   prepIndicationCategory, hivRiskAssessmentScore, baselineHivAgAbTestResult, baselineHivRnaVlResult, prepRegimen
 *   2. baseline-labs:     baselineCreatinineClearance, baselineSerumCreatinine, hepatitisBSurfaceAntigen, hepatitisBSurfaceAntibody, hepatitisCAbResult, baselineBoneDensityTScore, stiScreeningGonorrhea, stiScreeningChlamydia, syphilisSerologyResult
 *   3. adherence:         prepAdherenceSelfReport, tenofovirdpUrineDbs
 *   4. monitoring:        quarterlyHivAgAbTestDate, quarterlyHivTestResult, renalFunctionMonitoringDate, currentCreatinineClearance, cabotegravirLAInjectionDate, nextCabLAInjectionDue
 *   5. clinical-notes:    prepDiscontinuationReason, pregnancyStatus, concomitantNephrotoxicMedications
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HivPrepManagementDocumentPDFTemplate from '../pdf-templates/HivPrepManagementDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './HivPrepManagementDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'hiv_prep_managementPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['prepIndicationCategory', 'baselineHivAgAbTestResult', 'baselineHivRnaVlResult', 'prepRegimen', 'hepatitisBSurfaceAntigen', 'hepatitisBSurfaceAntibody', 'hepatitisCAbResult', 'stiScreeningGonorrhea', 'stiScreeningChlamydia', 'syphilisSerologyResult', 'tenofovirdpUrineDbs', 'quarterlyHivTestResult', 'prepDiscontinuationReason', 'pregnancyStatus'];
...
/* ═══════ COMPONENT ═══════ */
const HivPrepManagementDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.hiv_prep_management) return Array.isArray(r.hiv_prep_management) ? r.hiv_prep_management : [r.hiv_prep_management];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.hiv_prep_management) return Array.isArray(dd.hiv_prep_management) ? dd.hiv_prep_management : [dd.hiv_prep_management]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        // localEdits key convention: array fields store the whole array under `${field}-${idx}`,
        // scalar/string fields under `${field}-${idx}`.
        const editKey = `${baseField}-${idx}`;
        nLocal[editKey] = value;
        nPending[editKey] = true;
        if (isArrayElem) {
          nFields[`${baseField}.${fieldPart.slice(lastDot + 1)}-${idx}`] = 'edited';
        } else {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `HIV PrEP Management ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Persist each staged field for THIS section/record to the DB now.
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/hiv_prep_management/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/hiv_prep_management/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HIV PrEP MANAGEMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `HIV PrEP Management ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: ARRAY FIELD (array of strings) ═══════ */
                  ...
                  <div className="edit-field-container">
                    ...
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => {
                        ...
                        setEditedFields(prev => ({ ...prev, [editKey]: ciBadge === 'added' ? 'added' : 'edited' }));
                        if (sid) setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
                        const store = readDrafts();
  ...
  const renderStringField = (record, fn, idx, sid) => {
                                ...
                                <div className="edit-field-container">
                                  ...
                                  <div className="edit-actions">
                                    <button className="save-btn" disabled={saving} onClick={e => {
                                      ...
                                      const p2 = parseLabel(s2); if (!p2.isLabeled) return;
    ...
    /* Single-value string: saveSentence for single-value, showLabelSingle */
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="hiv-prep-management-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">HIV PrEP Management</h2></div>
        <div className="empty-state">No HIV PrEP management records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="hiv-prep-management-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">HIV PrEP Management</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HivPrepManagementDocumentPDFTemplate document={pdfData} />} fileName="Hiv_Prep_Management.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search HIV PrEP management..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`HIV PrEP Management ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'prep-indication')}
            {renderSection(record, idx, 'baseline-labs')}
            {renderSection(record, idx, 'adherence')}
...
export default HivPrepManagementDocument;
