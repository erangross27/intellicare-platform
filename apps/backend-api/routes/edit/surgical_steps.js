...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'surgical-steps-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['date', 'procedureName', 'status', 'facility', 'surgeon', 'assistants', 'technique', 'approach', 'anatomicalLocation', 'stepNumber', 'stepDescription', 'equipmentUsed', 'instrumentsUsed', 'duration', 'bloodLoss', 'tissuesInvolved', 'findings', 'specimens', 'complications', 'safetyChecks', 'anesthesiaEvents', 'notes', 'operativeReport', 'images'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('surgical_steps', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[surgical_steps] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('surgical_steps', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[surgical_steps] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
