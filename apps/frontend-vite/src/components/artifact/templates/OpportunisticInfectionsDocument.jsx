/**
 * OpportunisticInfectionsDocument.jsx
 * March 2026 — Canonical inline-editing rewrite, blue glow theme
 * Collection: opportunistic_infections
 *
 * Widgets: BlueDatePicker (dates) · num-stepper text input (numbers) · BlueSelect Yes/No (booleans).
 * Real top-level `date` (≠ createdAt) rendered via BlueDatePicker. Units live in the field labels.
 *
 * 8 Sections:
 *   1. clinical-info: date, symptomOnsetDate, severityGrade
 *   2. infection-details: infectionType, pathogenIdentified, infectionSite, underlyingImmunodeficiency
 *   3. immune-status: cd4Count, cd4Percentage, viralLoadHiv
 *   4. art-info: artStatus, artRegimen, irisPresent
 *   5. diagnosis: diagnosisMethod, radiologicFeatures, cultureResults, histopathologyResults, susceptibilityTesting
 *   6. treatment: antimicrobialTherapy, treatmentDuration, treatmentResponse, adverseDrugReactions
 *   7. prophylaxis: prophylaxisHistory, prophylaxisIndicated
 *   8. clinical-status: hospitalAdmissionRequired, coinfections
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import OpportunisticInfectionsDocumentPDFTemplate from '../pdf-templates/OpportunisticInfectionsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './OpportunisticInfectionsDocument.css';
...
const DRAFT_KEY = 'opportunistic_infectionsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['severityGrade', 'infectionType', 'pathogenIdentified', 'underlyingImmunodeficiency', 'artStatus', 'artRegimen', 'radiologicFeatures', 'cultureResults', 'histopathologyResults', 'susceptibilityTesting', 'treatmentDuration', 'treatmentResponse', 'prophylaxisHistory'];
/* Only narrative fields get sentence-splitting; short identifiers stay whole */
...
/* ═══════ COMPONENT ═══════ */
const OpportunisticInfectionsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.opportunistic_infections) return Array.isArray(r.opportunistic_infections) ? r.opportunistic_infections : [r.opportunistic_infections];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.opportunistic_infections) return Array.isArray(dd.opportunistic_infections) ? dd.opportunistic_infections : [dd.opportunistic_infections]; return [dd]; }
      return [r];
    });
  ...
  /* searchable string for a field value — flattens arrays */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Opportunistic Infection ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/opportunistic_infections/${id}/edit`, { field: fn, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/opportunistic_infections/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[OpportunisticInfections] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== OPPORTUNISTIC INFECTIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Opportunistic Infection ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: BOOLEAN FIELD (BlueSelect Yes/No — renders both true AND false) ═══════ */
  ...
  /* ═══════ RENDER: STRING FIELD with splitBySentence (LONG_TEXT only) ═══════ */
  const renderStringField = (record, fn, idx, sid, title) => {
    ...
    /* Multi-sentence OR a single "Label: value" sentence: decompose */
    if (sentences.length > 1 || (sentences.length === 1 && parseLabel(sentences[0]).isLabeled)) {
  ...
  if (!records || records.length === 0) {
    return (
      <div className="opportunistic-infections-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Opportunistic Infections</h2></div>
        <div className="empty-state">No opportunistic infections records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="opportunistic-infections-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Opportunistic Infections</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OpportunisticInfectionsDocumentPDFTemplate document={pdfData} />} fileName="Opportunistic_Infections.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search opportunistic infections..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        {searchTerm && <span className="search-results">Showing {filteredRecords.length} of {records.length} records</span>}
      </div>
      <div className="records-container">
        {filteredRecords.map((record, idx) => {
          const recDate = getFieldValue(record, 'date', idx);
          return (
            <div key={idx} className="record-card">
              <div className="record-header">
                {hasVal(recDate) && (
                  <div className="record-meta-row">
                    <span className="record-date">{formatDate(recDate)}</span>
                  </div>
                )}
                <h3 className="record-name">{highlightText(`Opportunistic Infection ${idx + 1}`)}</h3>
              </div>
...
export default OpportunisticInfectionsDocument;
