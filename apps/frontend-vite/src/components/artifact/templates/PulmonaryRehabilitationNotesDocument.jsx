/**
 * PulmonaryRehabilitationNotesDocument.jsx
 * March 2026 — Canonical one-pass, inline editing, blue glow theme
 * Collection: pulmonary_rehabilitation_notes
 *
 * 11 Sections:
 *   1. session-overview: sessionDurationMinutes, exerciseIntensityPercentage, mmrcDyspneaGrade
 *   2. vital-signs: preExerciseOxygenSaturation, postExerciseOxygenSaturation, restingBloodPressure, postExerciseBloodPressure, peakHeartRate, targetHeartRate
 *   3. exercise-performance: sixMinuteWalkDistance, exerciseToleranceMinutes, exerciseInducedDesaturation, oxygenFlowRateExercise
 *   4. assessment-scores: borgDyspneaScale, borgExertionScale, copyCatScore, chronicRespiratoryQuestionnaire
 *   5. exercise-modalities: exerciseModalitiesPerformed
 *   6. breathing-techniques: breathingTechniques
 *   7. energy-conservation: energyConservationTechniques
 *   8. nutritional-counseling: nutritionalCounselingTopics
 *   9. psychosocial-support: psychosocialSupport
 *  10. medication-adherence: medicationAdherence
 *  11. progress-goals: progressTowardGoals
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PulmonaryRehabilitationNotesDocumentPDFTemplate from '../pdf-templates/PulmonaryRehabilitationNotesDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PulmonaryRehabilitationNotesDocument.css';
...
const DATE_FIELDS = [];
/* MEANINGFUL_ZERO_FIELDS: numeric fields where 0 is a real clinical value (show it). Borg 0 = no
   dyspnea/exertion, mMRC 0 = a valid grade. All other numerics treat 0 as an extractor "not
   measured" sentinel and hide it (targetHeartRate/chronicRespiratoryQuestionnaire/exerciseIntensityPercentage). */
...
/* flattenItem: array item → readable string (all pulmonary arrays are plain strings) */
...
const DRAFT_KEY = 'pulmonary_rehabilitation_notesPendingEdits';
const readDrafts = () => {
...
const PulmonaryRehabilitationNotesDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.pulmonary_rehabilitation_notes) return Array.isArray(r.pulmonary_rehabilitation_notes) ? r.pulmonary_rehabilitation_notes : [r.pulmonary_rehabilitation_notes];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.pulmonary_rehabilitation_notes) return Array.isArray(dd.pulmonary_rehabilitation_notes) ? dd.pulmonary_rehabilitation_notes : [dd.pulmonary_rehabilitation_notes]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pulmonary Rehabilitation Note ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const key of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/pulmonary_rehabilitation_notes/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/pulmonary_rehabilitation_notes/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PULMONARY REHABILITATION NOTES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Pulmonary Rehabilitation Note ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pulmonary-rehabilitation-notes-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pulmonary Rehabilitation Notes</h2></div>
        <div className="empty-state">No pulmonary rehabilitation notes data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pulmonary-rehabilitation-notes-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Pulmonary Rehabilitation Notes</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PulmonaryRehabilitationNotesDocumentPDFTemplate document={pdfData} />} fileName="Pulmonary_Rehabilitation_Notes.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pulmonary rehabilitation notes..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Pulmonary Rehabilitation Note ${idx + 1}`)}</h3>
            </div>
...
export default PulmonaryRehabilitationNotesDocument;
