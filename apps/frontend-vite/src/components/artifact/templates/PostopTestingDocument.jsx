/**
 * PostOpTestingDocument.jsx
 * June 2026 — Collection: post_op_testing (NEW template)
 *
 * Donor cloned: SupportGroupReferralDocument
 *  - date header renderDateField; OBJECT recursive render; ARRAY date-grouped recommendations;
 *    STRING per-sentence; BOOLEAN Yes/No <select> saving real boolean; 4-level search;
 *    per-section approve; Copy row/Section/All; pdfData memo; blue-glow theme.
 *
 * FIELDS (19 → 100% schema coverage):
 *   DATE:    date
 *   BOOLEAN: impingement
 *   STRING:  lachmanTest, pivotShift, anteriorDrawer, posteriorDrawer, type, provider,
 *            facility, findings, assessment, plan, notes, status
 *   ARRAY:   recommendations (array of {recommendation, date})
 *   OBJECT:  varusValgusStress, rangeOfMotion, postOperativeDay1Assessment, results (recursive key-value)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PostOpTestingDocumentPDFTemplate from '../pdf-templates/PostOpTestingDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './PostOpTestingDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = top-level field name) */
const DRAFT_KEY = 'post_op_testingPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'ligament-exam': 'Ligament Exam',
  'stress-rom': 'Stress & ROM',
  'pod1-assessment': 'POD1 Assessment',
  'clinical': 'Clinical',
...
const SECTION_FIELDS = {
  'ligament-exam': ['lachmanTest', 'pivotShift', 'anteriorDrawer', 'posteriorDrawer', 'impingement'],
  'stress-rom': ['varusValgusStress', 'rangeOfMotion'],
  'pod1-assessment': ['postOperativeDay1Assessment'],
...
const SECTION_ORDER = ['ligament-exam', 'stress-rom', 'pod1-assessment', 'clinical', 'results-section', 'recommendations-section', 'notes-status'];

const DATE_FIELDS = ['date'];
const BOOLEAN_FIELDS = ['impingement'];
const STRING_FIELDS = ['lachmanTest', 'pivotShift', 'anteriorDrawer', 'posteriorDrawer', 'type', 'provider', 'facility', 'findings', 'assessment', 'plan', 'notes', 'status'];
/* DISPLAY/EDIT recursive object fields */
const OBJECT_FIELDS = ['varusValgusStress', 'rangeOfMotion', 'postOperativeDay1Assessment', 'results'];
...
/* ═══════ COMPONENT ═══════ */
const PostOpTestingDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.post_op_testing) return Array.isArray(r.post_op_testing) ? r.post_op_testing : [r.post_op_testing];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.post_op_testing) return Array.isArray(dd.post_op_testing) ? dd.post_op_testing : [dd.post_op_testing]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `post op testing ${idx + 1}`.toLowerCase();
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.outcome) — value stays a STRING.
     DEFER-SAVE: stage the merged root object locally (NO DB write); Approve commits the whole field. */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    const store = readDrafts();
    if (!store[id]) store[id] = {};
    store[id][rootField] = cloneForDraft;
    writeDrafts(store);
    setEditingField(null); setEditValue('');
  }, [safeId]);

  // DEFER-SAVE: splice the sentence into the full text and stage it locally + in the draft store.
  // NO DB write; Approve commits. Survives refresh.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const suffix = `-${idx}`;
      // Only this section's fields that are staged pending drafts
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/post_op_testing/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/post_op_testing/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[PostOpTesting] Approve error:', err); } finally { setSaving(false); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== POST-OPERATIVE TESTING (ORTHOPEDIC) ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Post-Operative Testing ${idx + 1}\n${'='.repeat(40)}\n\n`;
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              const editKey = `${fn}-${idx}`;
                              // DEFER-SAVE: stage locally + draft store, NO DB write; Approve commits.
                              setSaveError(null);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="post-op-testing-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Post-Operative Testing (Orthopedic)</h2></div>
        <div className="empty-state">No post-operative testing records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="post-op-testing-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Post-Operative Testing (Orthopedic)</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PostOpTestingDocumentPDFTemplate document={pdfData} />} fileName={`post-op-testing-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search post-operative testing records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Post-Operative Testing ${idx + 1}`)}</h3>
            </div>
...
export default PostOpTestingDocument;
