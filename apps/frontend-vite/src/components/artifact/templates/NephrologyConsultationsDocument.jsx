/**
 * NephrologyConsultationsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: nephrology_consultations
 *
 * 5 Sections:
 *   1. renal-function: estimatedGFR, serumCreatinine, creatinineClearance, bloodUreaNitrogen, proteinuria, albuminCreatinineRatio, ckdStage, akiStaging
 *   2. electrolytes-acid-base: serumElectrolytes, acidBaseStatus
 *   3. imaging-pathology: renalUltrasoundFindings, renalBiopsyResults, urinalysisFindings, glomerularDisease
 *   4. management: hypertensionControl, mineralBoneDisorder, anemiaManagement, dialysisType, dialysisAccess
 *   5. transplant-immunosuppression: transplantEvaluation, immunosuppressionRegimen
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NephrologyConsultationsDocumentPDFTemplate from '../pdf-templates/NephrologyConsultationsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './NephrologyConsultationsDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'renal-function': 'Renal Function',
  'electrolytes-acid-base': 'Electrolytes & Acid-Base',
  'imaging-pathology': 'Imaging & Pathology',
  'management': 'Management',
  'transplant-immunosuppression': 'Transplant & Immunosuppression',
};
...
const FIELD_LABELS = {
  ...
  albuminCreatinineRatio: 'Albumin-Creatinine Ratio',
  ckdStage: 'CKD Stage',
  akiStaging: 'AKI Staging',
  ...
  urinalysisFindings: 'Urinalysis Findings',
  glomerularDisease: 'Glomerular Disease',
  hypertensionControl: 'Hypertension Control',
  mineralBoneDisorder: 'Mineral and Bone Disorder',
  anemiaManagement: 'Anemia Management',
  dialysisType: 'Dialysis Type',
  dialysisAccess: 'Dialysis Access',
  transplantEvaluation: 'Transplant Evaluation',
  immunosuppressionRegimen: 'Immunosuppression Regimen',
};

const SECTION_FIELDS = {
  'renal-function': ['estimatedGFR', 'serumCreatinine', 'creatinineClearance', 'bloodUreaNitrogen', 'proteinuria', 'albuminCreatinineRatio', 'ckdStage', 'akiStaging'],
  'electrolytes-acid-base': ['serumElectrolytes', 'acidBaseStatus'],
  'imaging-pathology': ['renalUltrasoundFindings', 'renalBiopsyResults', 'urinalysisFindings', 'glomerularDisease'],
  'management': ['hypertensionControl', 'mineralBoneDisorder', 'anemiaManagement', 'dialysisType', 'dialysisAccess'],
  'transplant-immunosuppression': ['transplantEvaluation', 'immunosuppressionRegimen'],
};

const NUMBER_FIELDS = ['estimatedGFR', 'serumCreatinine', 'creatinineClearance', 'bloodUreaNitrogen', 'proteinuria', 'albuminCreatinineRatio'];
const ARRAY_FIELDS = ['serumElectrolytes', 'immunosuppressionRegimen'];
const STRING_FIELDS = ['ckdStage', 'akiStaging', 'acidBaseStatus', 'renalUltrasoundFindings', 'renalBiopsyResults', 'urinalysisFindings', 'glomerularDisease', 'hypertensionControl', 'mineralBoneDisorder', 'anemiaManagement', 'dialysisType', 'dialysisAccess', 'transplantEvaluation'];
...
const DRAFT_KEY = 'nephrology_consultationsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const NephrologyConsultationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.nephrology_consultations) return Array.isArray(r.nephrology_consultations) ? r.nephrology_consultations : [r.nephrology_consultations];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.nephrology_consultations) return Array.isArray(dd.nephrology_consultations) ? dd.nephrology_consultations : [dd.nephrology_consultations]; return [dd]; }
      return [r];
    });
  ...
  /* ═══════ DRAFT REHYDRATION ═══════
     ...
     Draft fieldPart is "field" (string) or "field.arrayIndex" (array element). For array elements we
     rebuild the full array (record value + draft overrides) to match the in-memory localEdits shape. */
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      if (!recDrafts) return;
      // Group draft entries: plain field vs array-element (field.arrayIndex)
      const arrayOverrides = {}; // field -> { arrayIndex: value }
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (dotIdx !== -1 && /^\d+$/.test(tail)) {
          ...
          if (!arrayOverrides[baseField]) arrayOverrides[baseField] = {};
          arrayOverrides[baseField][arrIndex] = value;
      ...
      Object.entries(arrayOverrides).forEach(([baseField, overrides]) => {
        const base = Array.isArray(record[baseField]) ? [...record[baseField]] : [];
        Object.entries(overrides).forEach(([ai, v]) => { base[parseInt(ai, 10)] = v; });
        nLocal[`${baseField}-${idx}`] = base;
      });
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Nephrology Consultation ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Persist each staged field/element for THIS section to the DB now.
      const store = readDrafts();
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/nephrology_consultations/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedFieldParts.push(fieldPart);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/nephrology_consultations/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[NephrologyConsultations] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NEPHROLOGY CONSULTATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Nephrology Consultation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="nephrology-consultations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Nephrology Consultations</h2></div>
        <div className="empty-state">No nephrology consultation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="nephrology-consultations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Nephrology Consultations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NephrologyConsultationsDocumentPDFTemplate document={pdfData} />} fileName="Nephrology_Consultations.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search nephrology consultations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Nephrology Consultation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'renal-function')}
            {renderSection(record, idx, 'electrolytes-acid-base')}
            {renderSection(record, idx, 'imaging-pathology')}
            {renderSection(record, idx, 'management')}
            {renderSection(record, idx, 'transplant-immunosuppression')}
          </div>
        ))}
      </div>
    </div>
...
export default NephrologyConsultationsDocument;
