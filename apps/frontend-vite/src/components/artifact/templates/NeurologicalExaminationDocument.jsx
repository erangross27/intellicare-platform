/**
 * NeurologicalExaminationDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: neurological_examination
 *
 * 8 Sections:
 *   1. scores: glasgowComaScale (num), glasgowComaScaleEye (num), glasgowComaScaleVerbal (num),
 *              glasgowComaScaleMotor (num), nihStrokeScale (num), modifiedRankinScale (num),
 *              miniMentalStateExam (num), montrealCognitiveAssessment (num)
 *   2. cranial-nerves: cranialNerveExamination (array), pupillaryResponse (string),
 *                      extraocularMovements (string)
 *   3. motor: motorStrengthRightUpper (string), motorStrengthLeftUpper (string),
 *             motorStrengthRightLower (string), motorStrengthLeftLower (string)
 *   4. reflexes: deepTendonReflexes (string), pathologicalReflexes (array)
 *   5. sensory: sensoryExaminationFindings (string)
 *   6. coordination-gait: coordinationTesting (string), gaitAnalysis (string)
 *   7. speech: speechLanguageAssessment (string)
 *   8. clinical-signs: meningealSigns (bool), cerebellarDysfunction (bool), seizureActivity (bool)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NeurologicalExaminationDocumentPDFTemplate from '../pdf-templates/NeurologicalExaminationDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './NeurologicalExaminationDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the field name; arrays stored whole) */
const DRAFT_KEY = 'neurological_examinationPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = [
  'pupillaryResponse', 'extraocularMovements',
  'motorStrengthRightUpper', 'motorStrengthLeftUpper', 'motorStrengthRightLower', 'motorStrengthLeftLower',
  'deepTendonReflexes', 'sensoryExaminationFindings',
  'coordinationTesting', 'gaitAnalysis', 'speechLanguageAssessment',
];
...
/* splitByComma: parenthesis-aware comma split (thousands guard: only split before whitespace) */
...
/* ======= COMPONENT ======= */
const NeurologicalExaminationDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.neurological_examination) return Array.isArray(r.neurological_examination) ? r.neurological_examination : [r.neurological_examination];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.neurological_examination) return Array.isArray(dd.neurological_examination) ? dd.neurological_examination : [dd.neurological_examination]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Neurological Examination ${idx + 1}`.toLowerCase();
  ...
  const pdfData = useMemo(() => {
    return filteredRecords.map((record, idx) => {
      ...
      ARRAY_FIELDS.forEach(f => {
        const k = `${f}-${idx}`;
        if (pendingEdits[k]) return; // keep pending array drafts out of the PDF
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Collect this section's pending edits (keys are "<field>-<idx>"; value = whole field value).
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k]) return false;
        const m = k.match(/^(.+)-(\d+)$/);
        if (!m || parseInt(m[2], 10) !== idx) return false;
        const fieldPart = m[1];
        // fieldPart is "field" or "field.arrayIndex" — arrayIndex ONLY when trailing segment is numeric
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/neurological_examination/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/neurological_examination/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NEUROLOGICAL EXAMINATION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Neurological Examination ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="neurological-examination-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Neurological Examination</h2></div>
        <div className="empty-state">No neurological examination records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="neurological-examination-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Neurological Examination</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NeurologicalExaminationDocumentPDFTemplate document={pdfData} />} fileName="Neurological_Examination.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search neurological examination..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Neurological Examination ${idx + 1}`)}</h3>
            </div>
            ...
            {renderSection(record, idx, 'clinical-signs')}
          </div>
        ))}
      </div>
    </div>
...
export default NeurologicalExaminationDocument;
