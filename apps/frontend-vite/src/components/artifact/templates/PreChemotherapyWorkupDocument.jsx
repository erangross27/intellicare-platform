/**
 * PreChemotherapyWorkupDocument.jsx
 ...
 *   2. infectious-screening: infectiousScreening (nested: hepatitisB, hepatitisC, hiv)
 *   3. cardiac-assessment: cardiacAssessment (nested: ekg, echo, indication)
 *   4. consultations: fertilityConsultation, dentalClearance
 *   5. eligibility: findings, assessment
 *   6. treatment-plan: plan, notes
 */
...
const DRAFT_KEY = 'pre_chemotherapy_workupPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'infectious-screening': 'Infectious Screening',
...
const FIELD_LABELS = {
  ...
  facility: 'Facility',
  status: 'Status',
  'infectiousScreening.hepatitisB': 'Hepatitis B',
  'infectiousScreening.hepatitisC': 'Hepatitis C',
  'infectiousScreening.hiv': 'HIV',
  'cardiacAssessment.ekg': 'EKG',
  'cardiacAssessment.echo': 'Echo',
  'cardiacAssessment.indication': 'Indication',
  fertilityConsultation: 'Fertility Consultation',
  dentalClearance: 'Dental Clearance',
  findings: 'Findings',
...
const SECTION_FIELDS = {
  ...
  'infectious-screening': ['infectiousScreening.hepatitisB', 'infectiousScreening.hepatitisC', 'infectiousScreening.hiv'],
  'cardiac-assessment': ['cardiacAssessment.ekg', 'cardiacAssessment.echo', 'cardiacAssessment.indication'],
  'consultations': ['fertilityConsultation', 'dentalClearance'],
  'eligibility': ['findings', 'assessment'],
  'treatment-plan': ['plan', 'notes'],
  'results-section': ['results'],
};
...
const humanizeKey = (key) => {
  ...
  const base = String(key).includes('.') ? String(key).split('.').pop() : String(key);
...
/* ═══════ COMPONENT ═══════ */
const PreChemotherapyWorkupDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.pre_chemotherapy_workup) return Array.isArray(r.pre_chemotherapy_workup) ? r.pre_chemotherapy_workup : [r.pre_chemotherapy_workup];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.pre_chemotherapy_workup) return Array.isArray(dd.pre_chemotherapy_workup) ? dd.pre_chemotherapy_workup : [dd.pre_chemotherapy_workup]; return [dd]; }
      return [r];
    });
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Draft store shape: { [recordId]: { [fieldPart]: { value, track } } } where fieldPart is the
     localEdits key minus the "-<idx>" suffix (e.g. "findings", "infectiousScreening.hiv",
     "results" for object roots, "recommendations" for arrays) and track is the edited marker payload. */
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      if (!recDrafts) return;
      Object.entries(recDrafts).forEach(([fieldPart, entry]) => {
        const value = entry && typeof entry === 'object' && 'value' in entry ? entry.value : entry;
        const track = entry && typeof entry === 'object' && entry.track ? entry.track : null;
        const editKey = `${fieldPart}-${idx}`;
        nLocal[editKey] = value;
        nPending[editKey] = true;
        if (track && track.kind === 'sentence') nSentences[track.key] = track.marker || 'edited';
        else nFields[(track && track.key) || editKey] = (track && track.marker) || 'edited';
      });
    });
  ...
  const fmtVal = useCallback((v) => { if (typeof v === 'boolean') return v ? 'Yes' : 'No'; if (typeof v === 'number') return String(v); if (Array.isArray(v) || (v && typeof v === 'object')) return flattenSearchable(v); return String(v || ''); }, []);
  /* searchable text for any field (objects flattened) */
  const searchTextFor = useCallback((fn, v) => {
    if (OBJECT_FIELDS.includes(fn)) return flattenSearchable(v);
    return fmtVal(v);
  }, [fmtVal]);
  ...
  /* getFieldValue: supports dot-path for nested objects like infectiousScreening.hepatitisB */
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      const label = (FIELD_LABELS[f] || prettifyKey(f)).toLowerCase();
      if (label.includes(phrase) || phrase.includes(label)) return true;
      const val = getFieldValue(record, f, 0);
      if (val !== null && val !== undefined) {
        if (searchTextFor(f, val).toLowerCase().includes(phrase)) return true;
      }
    }
    return false;
  }, [searchTerm, getFieldValue, searchTextFor]);

  const fieldMatches = useCallback((record, fn, idx) => {
    if (!searchTerm.trim() || record._showAllSections) return true;
    const phrase = searchTerm.toLowerCase().trim();
    const label = (FIELD_LABELS[fn] || prettifyKey(fn)).toLowerCase();
    if (label.includes(phrase) || phrase.includes(label)) return true;
    const val = getFieldValue(record, fn, idx);
    if (val !== null && val !== undefined) {
      return searchTextFor(fn, val).toLowerCase().includes(phrase);
    }
    return false;
  }, [searchTerm, getFieldValue, searchTextFor]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pre-Chemotherapy Workup ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (hasVal(val) && searchTextFor(f, val).toLowerCase().includes(phrase)) return true;
        }
      }
      return false;
    });
  }, [records, searchTerm, getFieldValue, hasVal, searchTextFor]);
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* stageDraft: stage a DRAFT locally + persist to the pending-drafts localStorage store (survives
     refresh). NOT written to MongoDB and NOT shown in the PDF until the user clicks Approve.
     fieldPart is the localEdits key minus the "-<idx>" suffix; track records the edited marker so
     a refresh can re-show the yellow badge. Clears any prior approved flag for the section. */
  const stageDraft = useCallback((record, idx, sid, fieldPart, value, track) => {
    const id = safeId(record); if (!id) return;
    const editKey = `${fieldPart}-${idx}`;
    setLocalEdits(prev => ({ ...prev, [editKey]: value }));
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
    if (track && track.kind === 'sentence') setEditedSentences(prev => ({ ...prev, ...track.marks }));
    else setEditedFields(prev => ({ ...prev, [(track && track.key) || editKey]: (track && track.marker) || 'edited' }));
    ...
    store[id][fieldPart] = { value, track: track || { kind: 'field', key: editKey, marker: 'edited' } };
    writeDrafts(store);
  }, [safeId]);
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    stageDraft(record, idx, sid, fn, saveVal, { kind: 'field', key: trackKey, marker: 'edited' });
    setEditingField(null); setEditValue(''); setSaveError(null);
  }, [editValue, safeId, stageDraft]);

  /* save a nested OBJECT leaf by dot-path (e.g. results.cbc) */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    // Build the merged object value, then stage as a DRAFT (no DB write; Approve commits it).
    const cur = localEdits[`${rootField}-${idx}`] !== undefined ? localEdits[`${rootField}-${idx}`] : record[rootField];
    const clone = JSON.parse(JSON.stringify(cur ?? {}));
    let node = clone;
    for (let i = 0; i < path.length - 1; i++) node = node[path[i]];
    node[path[path.length - 1]] = newVal;
    stageDraft(record, idx, sid, rootField, clone, { kind: 'field', key: leafKeyTrack, marker: 'edited' });
    setEditingField(null); setEditValue(''); setSaveError(null);
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageDraft(record, idx, sid, fn, fullText, { kind: 'sentence', marks });
      setEditingField(null); setEditValue(''); setSaveError(null);
      return;
    }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      const toCommit = Object.keys(localEdits).filter(k => {
        ...
        // Treat a trailing ".<n>" (purely numeric) as an array element → base field is the head.
        const dot = fieldPart.lastIndexOf('.');
        const baseField = (dot !== -1 && /^\d+$/.test(fieldPart.slice(dot + 1))) ? fieldPart.slice(0, dot) : fieldPart;
        return fields.includes(baseField);
      });
      // Persist each staged field to the DB now (field, or field+arrayIndex for array elements).
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "field" / "a.b" / "field.<n>"
        const dot = fieldPart.lastIndexOf('.');
        const isArrayElem = dot !== -1 && /^\d+$/.test(fieldPart.slice(dot + 1));
        const payload = { field: isArrayElem ? fieldPart.slice(0, dot) : fieldPart, value: localEdits[editKey] };
        if (isArrayElem) payload.arrayIndex = parseInt(fieldPart.slice(dot + 1), 10);
        const resp = await secureApiClient.put(`/api/edit/pre_chemotherapy_workup/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/pre_chemotherapy_workup/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    fields.forEach(f => {
      const label = FIELD_LABELS[f] || prettifyKey(f);
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PRE-CHEMOTHERAPY WORKUP ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Pre-Chemotherapy Workup ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderSentenceField = (record, fn, idx, sid, title) => {
    const val = getFieldValue(record, fn, idx); if (!hasVal(val)) return null;
    const strVal = fn === 'date' ? formatDate(val) : fmtVal(val);
    const sentences = splitBySentence(strVal);
    const label = FIELD_LABELS[fn] || prettifyKey(fn);
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; stageDraft(record, idx, sid, fn, fullText, { kind: 'sentence', marks }); setEditingField(null); setEditValue(''); setSaveError(null); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { currentArr.splice(itemIdx, 1, ...subParts); } else { currentArr[itemIdx] = trimmed; } stageDraft(record, idx, sid, fn, currentArr, { kind: 'field', key: editKey, marker: 'edited' }); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pre-chemotherapy-workup-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pre-Chemotherapy Workup</h2></div>
        <div className="empty-state">No pre-chemotherapy workup records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pre-chemotherapy-workup-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pre-chemotherapy workup records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'infectious-screening')}
            {renderSection(record, idx, 'cardiac-assessment')}
            {renderSection(record, idx, 'consultations')}
            {renderSection(record, idx, 'eligibility')}
...
export default PreChemotherapyWorkupDocument;
