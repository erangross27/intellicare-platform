/**
 * PrenatalScreeningDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: prenatal_screening
 *
 * 7 Sections:
 *   1. clinical-info: date, provider, facility
 *   2. screening-results: ntScanResult, cellFreeDNAResult, firstTrimesterScreenResult, anatomyScanResult, cervicalLengthMeasurement, fetalEchoResult, perinatalMentalHealthReferral
 *   3. findings: findings
...
import PrenatalScreeningDocumentPDFTemplate from '../pdf-templates/PrenatalScreeningDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PrenatalScreeningDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'prenatalScreeningPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['type', 'provider', 'facility', 'ntScanResult', 'cellFreeDNAResult', 'firstTrimesterScreenResult', 'anatomyScanResult', 'cervicalLengthMeasurement', 'fetalEchoResult', 'perinatalMentalHealthReferral', 'findings', 'assessment', 'plan', 'notes', 'status'];
...
/* ═══════ OBJECT-FIELD HELPERS (recursive renderers; donor: PointOfCareUltrasoundHeartRateDocument) ═══════ */
...
/* ═══════ COMPONENT ═══════ */
const PrenatalScreeningDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.prenatal_screening) return Array.isArray(r.prenatal_screening) ? r.prenatal_screening : [r.prenatal_screening];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.prenatal_screening) return Array.isArray(dd.prenatal_screening) ? dd.prenatal_screening : [dd.prenatal_screening]; return [dd]; }
  ...
  // Stage an edit as a DRAFT: update in-memory localEdits + persist to the localStorage draft store
  // (survives refresh, shown in JSX). NO DB write here — the DB write happens only on Approve.
  // localValue is the full field value kept under `${fn}-${idx}`; draftValue is what is persisted
  // under the record draft (defaults to localValue). Returns the editKey.
  const stageDraft = useCallback((record, fn, idx, localValue, draftValue) => {
    ...
    if (id) {
      ...
      store[id][fn] = draftValue !== undefined ? draftValue : localValue;
      writeDrafts(store);
    }
    return editKey;
  }, [safeId]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Prenatal Screening ${idx + 1}`.toLowerCase();
  ...
  // Save one object leaf = stage a DRAFT (no DB write). The whole rootField object is kept under
  // `${rootField}-${idx}` (with the leaf updated) and persisted to localStorage; committed on Approve.
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    setLocalEdits(prev => {
      ...
      // Persist the full updated object as the draft for this rootField.
      const store = readDrafts();
      if (!store[id]) store[id] = {};
  ...
  // Save one sentence = stage a DRAFT (no DB write). Splices the edited/removed sentence back into
  // the full text, stages it under `${fn}-${idx}` + localStorage; committed on Approve.
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    const id = safeId(record); if (!id) return;
    const currentVal = String(getFieldValue(record, fn, idx) || '');
    const sentences = splitBySentence(currentVal);
    const editedVal = editValue.trim();
    const clearApprove = () => setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // Collect this record's staged (pending) edits for this section's fields, using the "-<idx>" suffix.
      const suffix = `-${idx}`;
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length); // "field" or (per GOTCHA) "field.arrayIndex"
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/prenatal_screening/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the record approved (audit trail)
      await secureApiClient.put(`/api/edit/prenatal_screening/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this record's committed drafts from localStorage (keyed by the field part)
      const store = readDrafts();
      if (store[id]) {
    ...
    } catch (err) { console.error('[PrenatalScreening] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_FIELDS.includes(f)) {
        ...
        text += `${labelLine}${items.map((item, i) => `${i + 1}. ${typeof item === 'string' ? item : item?.recommendation || ''}`).join('\n')}\n\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PRENATAL SCREENING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Prenatal Screening ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; ratio/number+unit/bool/text) ═══════ */
  ...
  const renderStringField = (record, fn, idx, sid, title) => {
    ...
    /* Multi-sentence OR single labeled sentence: render with splitBySentence (label above value-only rows) */
    if (sentences.length > 1 || parsedWhole.isLabeled) {
  ...
  if (!records || records.length === 0) {
    return (
      <div className="prenatal-screening-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Prenatal Screening</h2></div>
        <div className="empty-state">No prenatal screening records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="prenatal-screening-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Prenatal Screening</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PrenatalScreeningDocumentPDFTemplate document={pdfData} />} fileName="Prenatal_Screening.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search prenatal screening..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Prenatal Screening ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'clinical-info')}
            {renderSection(record, idx, 'screening-studies')}
...
export default PrenatalScreeningDocument;
