/**
 * HormoneTherapyRecordsDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: hormone_therapy_records
 *
 * Header: therapyStartDate (record-header date)
 * 4 Sections:
 *   1. indication-regimen: indicationForTherapy, estradiolFormulation, progesteroneFormulation, hormoneDosage, administrationRoute, patientAge
 *   2. baseline-hormones: baselineEstradiolLevel, baselineTestosteroneLevel, follicleStimulatingHormone, luteinizingHormone, thyroidStimulatingHormone, prolactinLevel
 *   3. monitoring: vasomotorSymptoms, menopauseGreeneScale, liverFunctionTests, lipidProfile, mammographyResults, boneDensityTScore, endometrialThickness
 *   4. risk-safety: thromboembolismHistory, cardiovascularRiskFactors, contraindications, adverseEffectsReported, complianceAssessment
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HormoneTherapyRecordsDocumentPDFTemplate from '../pdf-templates/HormoneTherapyRecordsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './HormoneTherapyRecordsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'hormone_therapy_recordsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'indication-regimen': 'Indication & Regimen',
  'baseline-hormones': 'Baseline Hormones',
...
const FIELD_LABELS = {
  ...
  estradiolFormulation: 'Estradiol Formulation',
  progesteroneFormulation: 'Progesterone Formulation',
  hormoneDosage: 'Hormone Dosage',
  administrationRoute: 'Administration Route',
  patientAge: 'Patient Age (years)',
  baselineEstradiolLevel: 'Baseline Estradiol (pg/mL)',
  baselineTestosteroneLevel: 'Baseline Testosterone (ng/dL)',
  follicleStimulatingHormone: 'FSH (mIU/mL)',
  luteinizingHormone: 'LH (mIU/mL)',
  thyroidStimulatingHormone: 'TSH (mIU/L)',
  prolactinLevel: 'Prolactin (ng/mL)',
  vasomotorSymptoms: 'Vasomotor Symptoms',
  menopauseGreeneScale: 'Menopause Greene Scale',
  liverFunctionTests: 'Liver Function Tests',
  lipidProfile: 'Lipid Profile',
  mammographyResults: 'Mammography Results',
  boneDensityTScore: 'Bone Density T-Score',
  endometrialThickness: 'Endometrial Thickness (mm)',
  therapyStartDate: 'Therapy Start Date',
  thromboembolismHistory: 'Thromboembolism History',
...
const SECTION_FIELDS = {
  'indication-regimen': ['indicationForTherapy', 'estradiolFormulation', 'progesteroneFormulation', 'hormoneDosage', 'administrationRoute', 'patientAge'],
  'baseline-hormones': ['baselineEstradiolLevel', 'baselineTestosteroneLevel', 'follicleStimulatingHormone', 'luteinizingHormone', 'thyroidStimulatingHormone', 'prolactinLevel'],
  'monitoring': ['vasomotorSymptoms', 'menopauseGreeneScale', 'liverFunctionTests', 'lipidProfile', 'mammographyResults', 'boneDensityTScore', 'endometrialThickness'],
  'risk-safety': ['thromboembolismHistory', 'cardiovascularRiskFactors', 'contraindications', 'adverseEffectsReported', 'complianceAssessment'],
};

const NUMBER_FIELDS = ['patientAge', 'baselineEstradiolLevel', 'baselineTestosteroneLevel', 'follicleStimulatingHormone', 'luteinizingHormone', 'thyroidStimulatingHormone', 'prolactinLevel', 'menopauseGreeneScale', 'boneDensityTScore', 'endometrialThickness'];
const DATE_FIELDS = ['therapyStartDate'];
const ARRAY_FIELDS = ['cardiovascularRiskFactors', 'adverseEffectsReported', 'contraindications'];
const BOOLEAN_FIELDS = ['thromboembolismHistory'];
const STRING_FIELDS = ['indicationForTherapy', 'estradiolFormulation', 'progesteroneFormulation', 'hormoneDosage', 'administrationRoute', 'vasomotorSymptoms', 'liverFunctionTests', 'lipidProfile', 'mammographyResults', 'complianceAssessment'];
...
/* ═══════ COMPONENT ═══════ */
const HormoneTherapyRecordsDocument = ({ document: docProp }) => {
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Array-element drafts (fieldPart "field.n") are merged back into the full-array localEdits value. */
  useEffect(() => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Hormone Therapy Records ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      // fieldParts belonging to this section's fields ("field" or "field.arrayIndex")
      const toCommit = Object.keys(recDrafts).filter(fp => {
        const dotIdx = fp.lastIndexOf('.');
        const base = (dotIdx !== -1 && /^\d+$/.test(fp.slice(dotIdx + 1))) ? fp.slice(0, dotIdx) : fp;
      ...
      for (const fieldPart of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/hormone_therapy_records/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/hormone_therapy_records/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF (localEdits keyed `${field}-${idx}`)
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HORMONE THERAPY RECORDS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Hormone Therapy Records ${idx + 1}\n${'='.repeat(40)}\n\n`;
      if (hasVal(r.therapyStartDate)) text += `${FIELD_LABELS.therapyStartDate}\n${formatDate(r.therapyStartDate)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="hormone-therapy-records-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Hormone Therapy Records</h2></div>
        <div className="empty-state">No hormone therapy records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="hormone-therapy-records-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Hormone Therapy Records</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HormoneTherapyRecordsDocumentPDFTemplate document={pdfData} />} fileName={`hormone-therapy-records-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search hormone therapy records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            ...
            <div className="record-header">
              {hasVal(record.therapyStartDate) && (
                <div className="record-meta-row">
                  <span className="record-date">{formatDate(record.therapyStartDate)}</span>
                </div>
              )}
              <h3 className="record-name">{highlightText(`Hormone Therapy Records ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'indication-regimen')}
            {renderSection(record, idx, 'baseline-hormones')}
...
export default HormoneTherapyRecordsDocument;
