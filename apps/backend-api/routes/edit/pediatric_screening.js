...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'pediatric-screening-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'visionScreening.result', 'visionScreening.acuity', 'visionScreening.method',
  'hearingScreening.result', 'hearingScreening.method',
  'behavioralScreening.tool', 'behavioralScreening.score', 'behavioralScreening.result', 'behavioralScreening.teacherRating',
  'leadLevel.riskAssessment',
  'developmentalScreening', 'cholesterolScreening', 'results',
  'tuberculosisRisk', 'dentalScreening',
  'assessment', 'plan', 'findings', 'notes', 'recommendations',
];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('pediatric_screening', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[pediatric_screening] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('pediatric_screening', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[pediatric_screening] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
