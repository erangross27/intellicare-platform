/**
 * Cardiology Followup Reports Edit Route
 */
...
function buildContext(req, operation = 'read') { return { serviceId: 'cardiology-followup-reports-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }

const ALLOWED_FIELDS = [
  'date',
  'primaryDiagnosis', 'provider', 'facility',
  'bloodPressure', 'heartRate', 'heartRhythm', 'bodyWeight',
  'chestPainCharacteristics', 'dyspneaSeverity', 'nyhaClass', 'ccsFunctionalClass', 'edemaLocation',
  'ejectionFraction', 'bnpLevel', 'troponinLevel', 'lipidPanel', 'inrValue',
  'currentMedications', 'medicationAdherence', 'anticoagulationTherapy',
  'deviceType', 'ecgInterpretation', 'deviceInterrogation', 'nextFollowupInterval',
];
...
function isAllowedField(field) {
  ...
  return OBJECT_ROOT_FIELDS.includes(field.slice(0, dot));
}
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('cardiology_followup_reports', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[CardiologyFollowup] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('cardiology_followup_reports', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[CardiologyFollowup] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
