...
function buildContext(req, operation = 'read') { return { serviceId: 'emergency-discharge-summaries-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['chiefComplaint', 'arrivalDateTime', 'dischargeDateTime', 'triageCategory', 'modeOfArrival', 'vitalSignsOnArrival', 'vitalSignsAtDischarge', 'emergencyDiagnoses', 'proceduresPerformed', 'imagingStudies', 'laboratoryTests', 'medicationsAdministered', 'dischargeMedications', 'dischargeDisposition', 'dischargeCondition', 'ivAccessRemoved', 'workRestrictions', 'followUpInstructions', 'returnPrecautions', 'patientEducationProvided', 'attendingPhysician', 'consultingServices', 'allergiesDocumented'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('emergency_discharge_summaries', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[EmergencyDischargeSummaries] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('emergency_discharge_summaries', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[EmergencyDischargeSummaries] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
