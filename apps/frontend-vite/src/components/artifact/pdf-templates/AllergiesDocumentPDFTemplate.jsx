...
const SECTIONS = [
  { title: 'Allergy Details', fields: [['allergen', 'Allergen', 'sentence'], ['reaction', 'Reaction', 'sentence'], ['severity', 'Severity', 'sentence'], ['type', 'Type'], ['status', 'Status'], ['dateIdentified', 'Date Identified', 'date']] },
  { title: 'Management & Notes', fields: [['management', 'Management', 'sentenceComma'], ['compliance', 'Compliance', 'sentence'], ['notes', 'Notes', 'sentence']] },
];
...
const unwrap = source => {
  ...
  rows = rows.flatMap(row => {
    ...
    if (row?.documentData) { const nested = row.documentData; if (Array.isArray(nested)) return nested; if (nested?.allergies) return Array.isArray(nested.allergies) ? nested.allergies : [nested.allergies]; return [nested]; }
    return [row];
  });
  return rows.filter(row => row && typeof row === 'object');
};

const AllergiesDocumentPDFTemplate = ({ document: documentProp, data: dataProp, templateData }) => {
  const records = React.useMemo(() => unwrap(documentProp ?? dataProp ?? templateData), [documentProp, dataProp, templateData]);
  if (!records.length) return <Document><Page size="LETTER" style={styles.page}><View style={styles.documentHeader}><Text style={styles.documentTitle}>Allergies</Text></View><Text style={styles.noData}>No airway management records available</Text></Page></Document>;
...
export default AllergiesDocumentPDFTemplate;
