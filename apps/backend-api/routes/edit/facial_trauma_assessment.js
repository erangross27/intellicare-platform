...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'facial-trauma-assessment-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'traumaMechanismCategory',
  'glasgowComaScaleScore',
  'facialInjurySeverityScore',
  'timeFromInjuryToPresentation',
  'lefortFractureClassification',
  'mandibularFractureLocations',
  'orbitalFloorFractureSize',
  'zygomaticomaxillaryComplexDisplacement',
  'nasalBoneDeviationAngle',
  'enophthalmosPresent',
  'interocularDistanceChange',
  'globeRuptureIndicators',
  'retrobulbarHematomaPresent',
  'intraocularPressure',
  'extraocularMovementRestriction',
  'infraorbitalNerveParesthesia',
  'facialNerveBranchesAffected',
  'houseBrackmannGrade',
  'parotidDuctInjury',
  'softTissueLacerationDepth',
  'occlusalDerangementType',
  'maximalInterincisalOpening',
  'dentoalveolarInjuryClassification',
  'septalHematomaPresent',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('facial_trauma_assessment', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[FacialTraumaAssessment] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('facial_trauma_assessment', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[FacialTraumaAssessment] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
