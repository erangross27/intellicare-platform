/**
 * Medication Recommendations Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/medication_recommendations/:id/edit    — Edit a single field
 * PUT /api/edit/medication_recommendations/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'medication-recommendations-edit-service',
    userId: req.user?.id,
    operation,
...
// Allowed editable fields for medication_recommendations
const ALLOWED_FIELDS = [
  'date',
  'provider', 'facility',
  'medicationName', 'pharmaceuticalClass', 'indication', 'prescriptionType', 'therapeuticGoal',
  'dosage', 'route', 'frequency', 'duration', 'strengthValue', 'strengthUnit', 'administrationInstructions',
  'quantityPrescribed', 'refillsAuthorized', 'substitutionAllowed', 'dea', 'priorAuthorizationRequired', 'tapering',
  'contraindications', 'drugInteractions', 'adverseReactions', 'monitoringParameters',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    if (!objectId) {
      return res.status(400).json({ success: false, error: 'Invalid medication recommendation ID' });
    }
    ...
    await sda.update('medication_recommendations', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[MedicationRecommendations] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    console.error('[MedicationRecommendations] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
