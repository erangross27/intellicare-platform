/**
 * FluidIntakeDocumentPDFTemplate.jsx
 ...
 * safeString scrubs non-Latin-1 glyphs Helvetica can't draw (incl. the × multiplication sign → x). splitByComma
 * splits only on comma+whitespace so thousands ('3,951') stay whole. Rule #74: each field is ONE wrap={false}
...
const SECTION_TITLES = {
  ...
  'fluid-details': 'Fluid Intake Details',
...
const FIELD_LABELS = {
  ...
  iv: 'IV Fluids', enteral: 'Enteral', oral: 'Oral Intake', bloodProducts: 'Blood Products', other: 'Other', total: 'Total Intake',
...
const SECTION_FIELDS = {
  ...
  'fluid-details': ['iv', 'enteral', 'oral', 'bloodProducts', 'other', 'total'],
  'assessment-section': ['assessment'],
  'plan-section': ['plan'],
  'findings-section': ['findings'],
  'results-section': ['results'],
...
const FluidIntakeDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].fluid_intake && Array.isArray(templateData[0].fluid_intake)) records = templateData[0].fluid_intake;
    else records = templateData;
  } else if (templateData && templateData.fluid_intake) {
    records = Array.isArray(templateData.fluid_intake) ? templateData.fluid_intake : [templateData.fluid_intake];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.fluid_intake ? (Array.isArray(dd.fluid_intake) ? dd.fluid_intake : [dd.fluid_intake]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Fluid Intake</Text></View>
        <Text style={styles.emptyState}>No fluid intake data available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Fluid Intake ${idx + 1}`}</Text>
...
export default FluidIntakeDocumentPDFTemplate;
