/**
 * OccupationalMedicineEvaluationsDocument.jsx
 * Canonical one-pass polish — inline editing, blue glow theme, box-free.
 * Collection: occupational_medicine_evaluations
 *
 * Sections:
 *   1. employment-info: occupation, employer, evaluationType, evaluationDate (date picker)
 *   2. work-injury: workInjury (recursive object)
 *   3. occupational-exposure: occupationalExposure (recursive object; booleans → Yes/No select)
 *   4. functional-capacity: functionalCapacityEvaluation (recursive object)
 *   5. work-restrictions: workRestrictions (array of {restriction, duration})
 *   6. accommodations: accommodationsNeeded (array of strings)
 *   7. follow-up: followUpPlan (recursive object)
 *   8. workers-comp: workersCompClaim (Yes/No select), returnToWorkStatus, impairmRating
 *
 * evaluationDate is a REAL record field → rendered as a BlueDatePicker field; header is title-only.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import OccupationalMedicineEvaluationsDocumentPDFTemplate from '../pdf-templates/OccupationalMedicineEvaluationsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './OccupationalMedicineEvaluationsDocument.css';
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   string, boolean, array, or nested object) */
const DRAFT_KEY = 'occupational_medicine_evaluationsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['occupation', 'employer', 'evaluationType', 'returnToWorkStatus', 'impairmRating'];
/* OBJECT fields (recursive editable leaves; boolean leaves → Yes/No select) */
const OBJECT_FIELDS = ['workInjury', 'occupationalExposure', 'functionalCapacityEvaluation', 'followUpPlan'];
/* Array-of-objects {restriction, duration} */
const OBJECT_ARRAY_FIELDS = ['workRestrictions'];
/* Array of scalar strings */
const ARRAY_FIELDS = ['accommodationsNeeded'];
/* Long free-text fields decomposed by splitBySentence (none short-form here) */
...
/* ═══════ COMPONENT ═══════ */
const OccupationalMedicineEvaluationsDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.occupational_medicine_evaluations) return Array.isArray(r.occupational_medicine_evaluations) ? r.occupational_medicine_evaluations : [r.occupational_medicine_evaluations];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.occupational_medicine_evaluations) return Array.isArray(dd.occupational_medicine_evaluations) ? dd.occupational_medicine_evaluations : [dd.occupational_medicine_evaluations]; return [dd]; }
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Occupational Medicine Evaluation ${idx + 1}`.toLowerCase();
      ...
      if (record.evaluationDate) { const dateStr = formatDate(record.evaluationDate).toLowerCase(); if (dateStr.includes(phrase)) return true; }
      return false;
    });
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. occupationalExposure.ppeUsed) — DRAFT only. */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/occupational_medicine_evaluations/${id}/edit`, { field: fn, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/occupational_medicine_evaluations/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[OccupationalMedicineEvaluations] Approve error:', err); setSaveError('Approve failed.'); }
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_ARRAY_FIELDS.includes(f)) {
        ...
        arr.forEach((item, i) => {
          ...
          body += `${i + 1}. ${first ? fmtScalar(first[1]) : ''}\n`;
          entries.slice(1).forEach(([k, v]) => { body += `   ${humanizeKey(k)}:\n     1. ${fmtScalar(v)}\n`; });
        });
        body += '\n';
        return;
      }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== OCCUPATIONAL MEDICINE EVALUATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Occupational Medicine Evaluation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderBooleanField = (record, fn, idx, sid) => {
    ...
    const isEditing = editingField === editKey;
    const label = FIELD_LABELS[fn] || fn;
    const boolVal = typeof val === 'boolean' ? val : (String(val).toLowerCase() === 'yes' || String(val).toLowerCase() === 'true');
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value.trim() : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveTextSentence(); }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  /* ═══════ RENDER: ARRAY FIELD (scalar strings) ═══════ */
  ...
  /* ═══════ RENDER: OBJECT-ARRAY FIELD (array of {restriction, duration}) ═══════ */
  const renderObjectArrayField = (record, fn, idx, sid) => {
    ...
    const showSubLabel = label.trim().toLowerCase() !== (SECTION_TITLES[sid] || '').trim().toLowerCase();
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
    const saveItemField = (aIdx, key, newVal, tKey) => {
      const id = safeId(record); if (!id) return;
      const cur = Array.isArray(getFieldValue(record, fn, idx)) ? JSON.parse(JSON.stringify(getFieldValue(record, fn, idx))) : [];
      cur[aIdx] = { ...cur[aIdx], [key]: newVal };
      setSaveError(null);
      stageDraft(record, idx, fn, cur);
      setEditedFields(prev => ({ ...prev, [tKey]: 'edited' }));
      setEditingField(null); setEditValue('');
    };
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, aIdx) => (
          <div key={aIdx} className="nested-mini-card">
            {Object.entries(item).filter(([, v]) => !isEmptyDeep(v)).map(([k, v]) => {
              const leafKey = `${fn}-${idx}-a${aIdx}.${k}`;
              const isEditing = editingField === leafKey;
              const isModified = editedFields[leafKey];
              const isBool = typeof v === 'boolean';
              const disp = fmtScalar(v);
              const startVal = isBool ? (v ? 'Yes' : 'No') : disp;
              return (
                <div key={k} className="nested-mini-card">
                  <div className="nested-subtitle sub-label">{highlightText(humanizeKey(k))}</div>
                  <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(leafKey); setEditValue(startVal); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        {isBool ? (
                          <BlueSelect value={editValue} options={['Yes', 'No']} onChange={vv => setEditValue(vv)} />
                        ) : (
                          <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                        )}
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveItemField(aIdx, k, isBool ? (editValue === 'Yes') : editValue.trim(), leafKey); }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
                      <>
                        <div className="row-content"><span className="content-value">{highlightText(disp)}</span><span className="edit-indicator">&#9998;</span></div>
                        <button className={`copy-btn ${copiedItems[leafKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${humanizeKey(k)}\n${disp}`, leafKey); }}>{copiedItems[leafKey] ? 'Copied!' : 'Copy'}</button>
                      </>
                    )}
                  </div>
                  {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
  ...
  if (!records || records.length === 0) {
    return (
      <div className="occupational-medicine-evaluations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Occupational Medicine Evaluations</h2></div>
        <div className="empty-state">No occupational medicine evaluation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="occupational-medicine-evaluations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Occupational Medicine Evaluations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OccupationalMedicineEvaluationsDocumentPDFTemplate document={pdfData} />} fileName="Occupational_Medicine_Evaluations.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search occupational medicine evaluations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Occupational Medicine Evaluation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'employment-info')}
            {renderSection(record, idx, 'work-injury')}
            {renderSection(record, idx, 'occupational-exposure')}
            ...
            {renderSection(record, idx, 'workers-comp')}
          </div>
        ))}
      </div>
    </div>
...
export default OccupationalMedicineEvaluationsDocument;
