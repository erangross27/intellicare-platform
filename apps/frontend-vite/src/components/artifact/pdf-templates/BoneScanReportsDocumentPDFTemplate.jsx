...
const COLLECTION = 'bone_scan_reports';
const COMMA_SPLIT_FIELDS = ['followUpRecommendations'];
const ARRAY_FIELDS = new Set(['hotSpotLocations', 'coldSpotLocations', 'fractureSites', 'arthriticChanges', 'prostheticDevices', 'photonDeficiency', 'imagingArtifacts']);
const OBJECT_FIELDS = new Set([]);
const NARRATIVE_FIELDS = new Set(['skeletalUptakePattern', 'followUpRecommendations', 'clinicalCorrelation']);
const DATE_FIELDS = new Set([]);
const ZERO_SENTINEL_FIELDS = new Set([]);
const KEY_OVERRIDES = {};
const LABELS = {
  radiopharmaceuticalAgent: 'Radiopharmaceutical Agent', injectedDose: 'Injected Dose', uptakeDelay: 'Uptake Delay',
  bloodPoolImaging: 'Blood Pool Imaging', skeletalUptakePattern: 'Skeletal Uptake Pattern',
  hotSpotLocations: 'Hot Spot Locations', coldSpotLocations: 'Cold Spot Locations',
  metastaticDisease: 'Metastatic Disease', fractureSites: 'Fracture Sites', arthriticChanges: 'Arthritic Changes',
  pageticChanges: 'Pagetic Changes', osteoporosisEvidence: 'Osteoporosis Evidence', superScanPattern: 'Super Scan Pattern',
  renalClearance: 'Renal Clearance', softtissueUptake: 'Soft Tissue Uptake', prostheticDevices: 'Prosthetic Devices',
  spinalAlignment: 'Spinal Alignment', photonDeficiency: 'Photon Deficiency', osteosarcomaFindings: 'Osteosarcoma Findings',
  osteomyelitisEvidence: 'Osteomyelitis Evidence', imagingArtifacts: 'Imaging Artifacts',
  followUpRecommendations: 'Follow-Up Recommendations', clinicalCorrelation: 'Clinical Correlation',
};
const SECTIONS = [
  { title: 'Imaging Technique', fields: ['radiopharmaceuticalAgent', 'injectedDose', 'uptakeDelay', 'bloodPoolImaging'] },
  { title: 'Uptake Pattern', fields: ['skeletalUptakePattern', 'superScanPattern'] },
  { title: 'Hot Spot Locations', fields: ['hotSpotLocations'] },
  { title: 'Cold Spot Locations', fields: ['coldSpotLocations'] },
  { title: 'Metastatic Disease', fields: ['metastaticDisease'] },
  { title: 'Fracture Sites', fields: ['fractureSites'] },
  { title: 'Arthritic Changes', fields: ['arthriticChanges'] },
  { title: 'Bone Disease Findings', fields: ['pageticChanges', 'osteoporosisEvidence', 'osteosarcomaFindings', 'osteomyelitisEvidence'] },
  { title: 'Other Findings', fields: ['renalClearance', 'softtissueUptake', 'spinalAlignment'] },
  { title: 'Prosthetic Devices', fields: ['prostheticDevices'] },
  { title: 'Photon Deficiency', fields: ['photonDeficiency'] },
  { title: 'Imaging Artifacts', fields: ['imagingArtifacts'] },
...
const BoneScanReportsDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Bone Scan Reports</Text><Text style={styles.noData}>No bone scan report data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Bone Scan Reports</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Bone Scan Report {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
