/**
 * OrthopedicAssessmentDocument.jsx
 * Canonical one-pass polish — blue glow editing theme
 * Collection: orthopedic_assessment
 *
 * Sections: Assessment Information · Neurovascular Status · Skin Integrity · Fractures ·
 *           Dislocations · Compartment Syndrome · Findings · Assessment · Plan ·
 *           Recommendations · Results · Notes
 *
 * date → editable BlueDatePicker (record date, NEVER createdAt/updatedAt).
 * Scalar string fields (provider/type/status/skinIntegrity/findings/…) are inline-editable
 * (canonical renderStringField — splitBySentence → parseLabel → splitByComma decomposition,
 * label ABOVE value, never a side-by-side "Label: value" span).
 * Nested objects (neurovascularStatus/compartmentSyndrome/results) and arrays
 * (fractures/dislocations/recommendations) render read-only via a generic recursive renderer.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import OrthopedicAssessmentDocumentPDFTemplate from '../pdf-templates/OrthopedicAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './OrthopedicAssessmentDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'orthopedic_assessmentPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'neurovascular-status': 'Neurovascular Status',
...
const SECTION_FIELDS = {
  'assessment-info': ['date', 'provider', 'facility', 'status', 'type'],
  'neurovascular-status': ['neurovascularStatus'],
...
const STRING_FIELDS = ['provider', 'facility', 'status', 'type', 'skinIntegrity', 'findings', 'assessment', 'plan', 'notes'];
/* Read-only, decomposed arrays + objects (generic recursive renderer) */
const ARRAY_FIELDS = ['fractures', 'dislocations', 'recommendations'];
const OBJECT_FIELDS = ['neurovascularStatus', 'compartmentSyndrome', 'results'];
...
const KEY_OVERRIDES = {
  rom: 'ROM', ct: 'CT', mri: 'MRI', xray: 'X-Ray', ecg: 'ECG', bp: 'BP', hr: 'HR',
};
...
/* ═══════ COMPONENT ═══════ */
const OrthopedicAssessmentDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.orthopedic_assessment) return Array.isArray(r.orthopedic_assessment) ? r.orthopedic_assessment : [r.orthopedic_assessment];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.orthopedic_assessment) return Array.isArray(dd.orthopedic_assessment) ? dd.orthopedic_assessment : [dd.orthopedic_assessment]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Orthopedic Assessment ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/orthopedic_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/orthopedic_assessment/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== ORTHOPEDIC ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Orthopedic Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT FIELD (neurovascularStatus / compartmentSyndrome / results — read-only) ═══════ */
  ...
  /* ═══════ RENDER: NUMBER FIELD (canonical — num-stepper, never type=number) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="orthopedic-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Orthopedic Assessment</h2></div>
        <div className="empty-state">No orthopedic assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="orthopedic-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Orthopedic Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OrthopedicAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Orthopedic_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search orthopedic assessments..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          <div key={idx} className="record-card">
            <div className="record-header">
              <div className="record-meta-row">
                ...
                {hasVal(record.status) && <span className="record-status">{fmtVal(record.status)}</span>}
              </div>
              <h3 className="record-name">{highlightText(`Orthopedic Assessment ${idx + 1}`)}</h3>
              {hasVal(record.type) && <div className="record-type">{highlightText(fmtVal(record.type))}</div>}
            </div>
...
export default OrthopedicAssessmentDocument;
