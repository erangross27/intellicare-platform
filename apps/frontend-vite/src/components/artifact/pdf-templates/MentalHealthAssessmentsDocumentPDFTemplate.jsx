...
/**
 * MentalHealthAssessmentsDocumentPDFTemplate - box-free canonical (LETTER)
 ...
 * Record has NO real clinical date field (only createdAt/updatedAt) -> TITLE ONLY, no record date.
 * assessmentDate is a FIELD in assessment-info. phq9Score/gad7Score are STRINGS, not numbers.
 */
...
const SECTION_ORDER = ['assessment-info', 'presenting-problems', 'mental-status-exam', 'screening-scores', 'risk-assessment', 'functioning-level', 'provisional-diagnosis', 'treatment-recommendations', 'safety-plan', 'disposition', 'recommendations'];
...
const MentalHealthAssessmentsDocumentPDFTemplate = ({ document: docProp, data = docProp }) => {
  let records = [];
  if (Array.isArray(data)) {
    if (data.length === 1 && data[0]?.mental_health_assessments) records = Array.isArray(data[0].mental_health_assessments) ? data[0].mental_health_assessments : [data[0].mental_health_assessments];
    else records = data;
  } else if (data?.mental_health_assessments) records = Array.isArray(data.mental_health_assessments) ? data.mental_health_assessments : [data.mental_health_assessments];
    ...
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <Text style={styles.documentTitle}>Mental Health Assessments</Text>
          <Text style={styles.noData}>No mental health assessment records available.</Text>
        </Page>
      </Document>
    );
  }
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, rIdx) => (
          <View key={rIdx}>
            <Text style={styles.recordTitle} break={rIdx > 0}>{safeString(`Mental Health Assessment ${rIdx + 1}`)}</Text>
...
export default MentalHealthAssessmentsDocumentPDFTemplate;
