/**
 * Hormone Therapy Records Edit Route
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'hormone-therapy-records-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'estradiolFormulation',
  'progesteroneFormulation',
  'hormoneDosage',
  'administrationRoute',
  'patientAge',
  'baselineEstradiolLevel',
  'baselineTestosteroneLevel',
  'follicleStimulatingHormone',
  'luteinizingHormone',
  'thyroidStimulatingHormone',
  'prolactinLevel',
  'vasomotorSymptoms',
  'menopauseGreeneScale',
  'liverFunctionTests',
  'lipidProfile',
  'mammographyResults',
  'boneDensityTScore',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('hormone_therapy_records', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[hormone_therapy_records] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('hormone_therapy_records', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[hormone_therapy_records] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
