/**
 * GeriatricNutritionalAssessmentDocument.jsx
 * June 2026 — Collection: geriatric_nutritional_assessment (NEW template)
 *
 * Donor cloned: SupportGroupReferralDocument (date header renderDateField; OBJECT recursive render;
 *   ARRAY date-grouped recommendations; STRING per-sentence; BOOLEAN Yes/No dropdown saving real boolean;
 *   4-level search; per-section approve; Copy row/Section/All; pdfData memo; blue-glow theme).
 *   Added: renderStringArrayField (plain string arrays as editable tags — dietaryRestrictions, supplementation).
 *
 * FIELDS (20 → 100% schema coverage):
 *   DATE:    date
 *   BOOLEAN: dysphagia
 *   STRING:  mnaScore, bmi, weightChange, albumin, prealbumin, appetiteChanges, type, provider, facility,
 *            findings, assessment, plan, notes, status
 *   ARRAY:   dietaryRestrictions (string[]), supplementation (string[]),
 *            recommendations (array of {recommendation, date})
 *   OBJECT:  results (recursive key-value)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import GeriatricNutritionalAssessmentDocumentPDFTemplate from '../pdf-templates/GeriatricNutritionalAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './GeriatricNutritionalAssessmentDocument.css';
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits field part, e.g. "findings") */
const DRAFT_KEY = 'geriatric_nutritional_assessmentPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'labs': 'Labs',
  'diet-supplementation': 'Diet & Supplementation',
  'clinical': 'Clinical',
...
const STRING_FIELDS = ['mnaScore', 'bmi', 'weightChange', 'albumin', 'prealbumin', 'appetiteChanges', 'type', 'provider', 'facility', 'findings', 'assessment', 'plan', 'notes', 'status'];
/* Plain string arrays (editable tags) */
const STRING_ARRAY_FIELDS = ['dietaryRestrictions', 'supplementation'];
...
/* splitByComma: parenthesis-aware, skips thousands separators (comma not followed by whitespace) */
...
/* ═══════ COMPONENT ═══════ */
const GeriatricNutritionalAssessmentDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.geriatric_nutritional_assessment) return Array.isArray(r.geriatric_nutritional_assessment) ? r.geriatric_nutritional_assessment : [r.geriatric_nutritional_assessment];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.geriatric_nutritional_assessment) return Array.isArray(dd.geriatric_nutritional_assessment) ? dd.geriatric_nutritional_assessment : [dd.geriatric_nutritional_assessment]; return [dd]; }
      return [r];
    });
  ...
  const fieldSearchText = useCallback((f, val) => {
    ...
    if (STRING_ARRAY_FIELDS.includes(f)) return Array.isArray(val) ? val.join(' ') : fmtVal(val);
    return fmtVal(val);
  }, [recsToText, fmtVal]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `geriatric nutritional assessment ${idx + 1}`.toLowerCase();
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.outcome) — staged as a DRAFT (no DB write).
     The whole merged object is staged under the root field; Approve commits it (field=rootField, value=object). */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Collect this record's pending edits that belong to a field in this section.
      const suffix = `-${idx}`;
      ...
      // Persist each staged field to the DB now (field, or field+arrayIndex for array-index suffixes).
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/geriatric_nutritional_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/geriatric_nutritional_assessment/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[GeriatricNutritionalAssessment] Approve error:', err); }
  ...
  const fmtSentenceLines = useCallback((strVal) => {
    const lines = []; let n = 1;
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (STRING_ARRAY_FIELDS.includes(f)) {
        ...
        body += head;
        items.forEach((it, i) => { body += `${i + 1}. ${String(it)}\n`; });
        body += '\n';
        ...
        if (sentences.length > 1 || (sentences.length === 1 && parseLabel(sentences[0]).isLabeled)) {
          body += head;
          fmtSentenceLines(strVal).forEach(l => { body += `${l}\n`; });
          body += '\n';
        } else {
          body += `${head}1. ${strVal}\n\n`;
        }
    ...
    if (!body.trim()) return '';
    return `${title}\n${'='.repeat(40)}\n\n${body}`;
  }, [getFieldValue, hasVal, fmtVal, splitBySentence, objectCopyLines, fmtSentenceLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== GERIATRIC NUTRITIONAL ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Geriatric Nutritional Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: STRING ARRAY FIELD (editable tags, comma-separated) ═══════ */
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(items.join(', ')); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              ...
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const arrVal = editValue.split(',').map(s => s.trim()).filter(s => s); handleSaveField(record, fn, idx, sid, arrVal); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="geriatric-nutritional-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Geriatric Nutritional Assessment</h2></div>
        <div className="empty-state">No geriatric nutritional assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="geriatric-nutritional-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Geriatric Nutritional Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<GeriatricNutritionalAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Geriatric_Nutritional_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search geriatric nutritional assessment records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Geriatric Nutritional Assessment ${idx + 1}`)}</h3>
            </div>
...
export default GeriatricNutritionalAssessmentDocument;
