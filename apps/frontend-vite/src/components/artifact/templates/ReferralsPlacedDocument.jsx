/**
 * ReferralsPlacedDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: referrals_placed
 *
 * 3 Sections:
 *   1. referral-details: specialty, reason, urgency
 *   2. provider-info: referringProvider, provider
 *   3. notes: notes
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ReferralsPlacedDocumentPDFTemplate from '../pdf-templates/ReferralsPlacedDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './ReferralsPlacedDocument.css';
...
const DRAFT_KEY = 'referralsPlacedPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  'referral-details': ['specialty', 'reason', 'urgency'],
  'provider-info': ['referringProvider', 'provider'],
  'notes': ['notes'],
};
...
const NUMBER_FIELDS = [];
const ARRAY_FIELDS = [];
const STRING_FIELDS = ['specialty', 'reason', 'urgency', 'referringProvider', 'provider', 'notes', 'status'];
...
/* ═══════ COMPONENT ═══════ */
const ReferralsPlacedDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.referrals_placed) return Array.isArray(r.referrals_placed) ? r.referrals_placed : [r.referrals_placed];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.referrals_placed) return Array.isArray(dd.referrals_placed) ? dd.referrals_placed : [dd.referrals_placed]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Referral Placed ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      // editKey = "fieldPart-idx"; treat trailing ".<digits>" as an arrayIndex (reverse of handleSaveField).
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/referrals_placed/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/referrals_placed/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== REFERRALS PLACED ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Referral Placed ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="referrals-placed-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Referrals Placed</h2></div>
        <div className="empty-state">No referrals placed records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="referrals-placed-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Referrals Placed</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ReferralsPlacedDocumentPDFTemplate document={pdfData} />} fileName={`referrals-placed-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search referrals..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          <div key={idx} className="record-card">
            <div className="record-header">
              <div className="record-meta-row">
                {hasVal(record.date) && <span className="record-date">{formatDate(getFieldValue(record, 'date', idx))}</span>}
                {hasVal(record.status) && (
                  <span className={`status-badge ${record.status === 'pending' ? 'pending' : record.status === 'completed' ? 'completed' : ''}`}>
                    {highlightText(record.status)}
                  </span>
                )}
              </div>
              <h3 className="record-name">{highlightText(`Referral Placed ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'referral-details')}
...
export default ReferralsPlacedDocument;
