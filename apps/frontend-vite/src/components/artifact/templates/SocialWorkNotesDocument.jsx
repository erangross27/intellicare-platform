/**
 * SocialWorkNotesDocument.jsx
 * July 2026 — recursive, dot-path editable social work notes
...
import SocialWorkNotesDocumentPDFTemplate from '../pdf-templates/SocialWorkNotesDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SocialWorkNotesDocument.css';

const DRAFT_KEY = 'social_work_notesPendingEdits';
const SECTION_TITLES = {
  assessment: 'Assessment and Functional Capacity',
  support: 'Social Support and Caregiver Burden',
  resources: 'Financial, Housing, and Community Resources',
  health: 'Mental Health, Substance Use, and Medication Support',
  planning: 'Advance Care and Discharge Planning',
  risk: 'Safety and Readmission Risks',
  additional: 'Additional Data',
};
const FIELD_LABELS = {
  psychosocialAssessmentScore: 'Psychosocial Assessment Score', adlFunctionalCapacity: 'ADL Functional Capacity', iadlAssessmentResults: 'IADL Assessment Results', cognitiveCapacityAssessment: 'Cognitive Capacity Assessment',
  socialSupportNetwork: 'Social Support Network', caregiverBurdenAssessment: 'Caregiver Burden Assessment', financialResourceAssessment: 'Financial Resource Assessment', insuranceCoverageStatus: 'Insurance Coverage Status',
  housingStabilityStatus: 'Housing Stability Status', transportationBarriers: 'Transportation Barriers', communityResourceReferrals: 'Community Resource Referrals', mentalHealthDiagnoses: 'Mental Health Diagnoses',
  substanceUseHistory: 'Substance Use History', medicationAdherence: 'Medication Adherence', advanceDirectiveStatus: 'Advance Directive Status', elderAbuseScreening: 'Elder Abuse Screening',
  culturalConsiderations: 'Cultural Considerations', dischargeDisposition: 'Discharge Disposition', fallRiskFactors: 'Fall Risk Factors', readmissionRiskFactors: 'Readmission Risk Factors', additionalData: 'Additional Data',
};
const SECTION_FIELDS = {
  assessment: ['psychosocialAssessmentScore', 'adlFunctionalCapacity', 'iadlAssessmentResults', 'cognitiveCapacityAssessment'],
  support: ['socialSupportNetwork', 'caregiverBurdenAssessment'],
  resources: ['financialResourceAssessment', 'insuranceCoverageStatus', 'housingStabilityStatus', 'transportationBarriers', 'communityResourceReferrals'],
  health: ['mentalHealthDiagnoses', 'substanceUseHistory', 'medicationAdherence'],
  planning: ['advanceDirectiveStatus', 'elderAbuseScreening', 'culturalConsiderations', 'dischargeDisposition'],
  risk: ['fallRiskFactors', 'readmissionRiskFactors'],
  additional: ['additionalData'],
};
const DATE_FIELDS = [];
const DATETIME_FIELDS = [];
const ENUM_FIELDS = [];
const NUMBER_FIELDS = ['psychosocialAssessmentScore'];
...
const COMMA_FIELDS = [];
const COMMA_ARRAY_SPLIT_FIELDS = ['culturalConsiderations'];
const ARRAY_FIELDS = ['socialSupportNetwork', 'transportationBarriers', 'mentalHealthDiagnoses', 'communityResourceReferrals', 'fallRiskFactors', 'culturalConsiderations', 'readmissionRiskFactors'];
const SEMICOLON_FIELDS = ['adlFunctionalCapacity', 'iadlAssessmentResults', 'caregiverBurdenAssessment', 'housingStabilityStatus', 'substanceUseHistory', 'advanceDirectiveStatus', 'financialResourceAssessment', 'cognitiveCapacityAssessment', 'transportationBarriers'];
...
const SocialWorkNotesDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.social_work_notes) return Array.isArray(record.social_work_notes) ? record.social_work_notes : [record.social_work_notes];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.social_work_notes) return Array.isArray(nested.social_work_notes) ? nested.social_work_notes : [nested.social_work_notes];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/social_work_notes/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/social_work_notes/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SocialWorkNotes] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    let text = '=== SOCIAL WORK NOTES ===\n\n';
    records.forEach((record, recordIndex) => {
      text += 'Social Work Notes Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="social-work-notes-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Social Work Notes</h2></div><div className="empty-state">No social work notes available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Social Work Notes Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="social-work-notes-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Social Work Notes</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<SocialWorkNotesDocumentPDFTemplate document={pdfData} />} fileName="Social_Work_Notes.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search social work notes..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Social Work Notes Record ' + (recordIndex + 1))}</h3></div>
...
export default SocialWorkNotesDocument;
