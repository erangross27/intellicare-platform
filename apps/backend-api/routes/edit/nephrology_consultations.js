...
function buildContext(req, operation = 'read') { return { serviceId: 'nephrology-consultations-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['estimatedGFR', 'serumCreatinine', 'creatinineClearance', 'bloodUreaNitrogen', 'proteinuria', 'albuminCreatinineRatio', 'ckdStage', 'akiStaging', 'serumElectrolytes', 'acidBaseStatus', 'renalUltrasoundFindings', 'renalBiopsyResults', 'urinalysisFindings', 'glomerularDisease', 'hypertensionControl', 'mineralBoneDisorder', 'anemiaManagement', 'dialysisType', 'dialysisAccess', 'transplantEvaluation', 'immunosuppressionRegimen'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('nephrology_consultations', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[NephrologyConsultations] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('nephrology_consultations', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[NephrologyConsultations] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
