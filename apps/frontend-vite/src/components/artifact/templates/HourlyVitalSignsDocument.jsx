/**
 * HourlyVitalSignsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: hourly_vital_signs
 *
 * 5 Sections:
 *   1. provider-info: date (date), provider (string), facility (string)
 *   2. hemodynamics: systolicBloodPressure, diastolicBloodPressure, meanArterialPressure,
 *                    heartRate, pulseRhythm, rhythmRegularity
 *   3. respiratory: respiratoryRate, oxygenSaturation, supplementalOxygenLiters, oxygenDeliveryMethod
 *   4. general-vitals: bodyTemperature, temperatureUnit, temperatureCelsius, temperatureFahrenheit,
 *                      temperatureRoute, painScore, painScaleType, consciousnessLevel, bloodGlucose,
 *                      weightKilograms
 *   5. monitoring: centralVenousPressure, urineOutput, capillaryRefillTime, pupilResponse,
 *                  recordingMethod, monitoringFrequency
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HourlyVitalSignsDocumentPDFTemplate from '../pdf-templates/HourlyVitalSignsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './HourlyVitalSignsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'hourly_vital_signsPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  supplementalOxygenLiters: 'Supplemental Oxygen (L/min)',
...
const SECTION_FIELDS = {
  ...
  'hemodynamics': ['systolicBloodPressure', 'diastolicBloodPressure', 'meanArterialPressure', 'heartRate', 'pulseRhythm', 'rhythmRegularity'],
  'respiratory': ['respiratoryRate', 'oxygenSaturation', 'supplementalOxygenLiters', 'oxygenDeliveryMethod'],
  'general-vitals': ['bodyTemperature', 'temperatureUnit', 'temperatureCelsius', 'temperatureFahrenheit', 'temperatureRoute', 'painScore', 'painScaleType', 'consciousnessLevel', 'bloodGlucose', 'weightKilograms'],
  'monitoring': ['centralVenousPressure', 'urineOutput', 'capillaryRefillTime', 'pupilResponse', 'recordingMethod', 'monitoringFrequency'],
};

/* 16 number fields — ALL use −/+ stepper + parseFloat+isNaN→block save. Zero-valued vitals still render. */
const NUMBER_FIELDS = [
  'systolicBloodPressure', 'diastolicBloodPressure', 'meanArterialPressure', 'heartRate',
  'respiratoryRate', 'bodyTemperature', 'temperatureCelsius', 'temperatureFahrenheit',
  'oxygenSaturation', 'supplementalOxygenLiters', 'painScore', 'bloodGlucose',
  'weightKilograms', 'centralVenousPressure', 'urineOutput', 'capillaryRefillTime',
];

const STRING_FIELDS = ['provider', 'facility', 'pulseRhythm', 'rhythmRegularity', 'oxygenDeliveryMethod', 'temperatureUnit', 'temperatureRoute', 'painScaleType', 'consciousnessLevel', 'pupilResponse', 'recordingMethod', 'monitoringFrequency'];
...
/* ═══════ COMPONENT ═══════ */
const HourlyVitalSignsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.hourly_vital_signs) return Array.isArray(r.hourly_vital_signs) ? r.hourly_vital_signs : [r.hourly_vital_signs];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.hourly_vital_signs) return Array.isArray(dd.hourly_vital_signs) ? dd.hourly_vital_signs : [dd.hourly_vital_signs]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Hourly Vital Signs ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/hourly_vital_signs/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/hourly_vital_signs/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HOURLY VITAL SIGNS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Hourly Vital Signs ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD — −/+ stepper + parseFloat+isNaN→block save (zero renders) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="hourly-vital-signs-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Hourly Vital Signs</h2></div>
        <div className="empty-state">No hourly vital signs records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="hourly-vital-signs-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Hourly Vital Signs</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HourlyVitalSignsDocumentPDFTemplate document={pdfData} />} fileName="Hourly_Vital_Signs.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search hourly vital signs records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
...
export default HourlyVitalSignsDocument;
