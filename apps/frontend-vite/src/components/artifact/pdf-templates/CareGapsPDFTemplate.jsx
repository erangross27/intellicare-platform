 ...
 * ARRAY field `screenings` (array of {screeningType, category, status, dueDate, actionRequired, priority}).
 * ARRAY field `recommendations` (array of {recommendation, date}) date-grouped numbered list.
 *
 * FIELDS (12 extractable):
 *   DATE:   date
 *   STRING: type, provider, facility, findings, assessment, plan, notes, status
 *   ARRAY:  screenings, recommendations
 *   OBJECT: results
 */
...
const styles = StyleSheet.create({
  ...
  itemTitle: { fontSize: 12, fontFamily: 'Helvetica-Bold', color: '#000000', marginTop: 6, marginBottom: 2 },
...
const FIELD_LABELS = {
  ...
  screenings: 'Screenings', findings: 'Findings', assessment: 'Assessment', plan: 'Plan',
...
const SCREENING_SUBLABELS = {
  screeningType: 'Screening Type', category: 'Category', status: 'Status',
  priority: 'Priority', dueDate: 'Due Date', actionRequired: 'Action Required',
};
    ...
    return items.map((item, i) => {
      ...
      return (
        <View key={`${field}-${i}`} style={styles.fieldGroup} wrap={rows > 8 ? undefined : false}>
          ...
          <Text style={styles.itemTitle}>{`${i + 1}. ${name}`}</Text>
          {subs.map(k => (
            <View key={k}>
              <Text style={styles.subLabel}>{SCREENING_SUBLABELS[k] || humanizeKey(k)}</Text>
              <Text style={styles.value}>{SCREENING_DATE_SUBFIELDS.includes(k) ? formatDate(item[k]) : fmtScalar(item[k])}</Text>
            </View>
          ))}
          {extras.map(k => (
            <View key={k}>
              <Text style={styles.subLabel}>{humanizeKey(k)}</Text>
              <Text style={styles.value}>{fmtScalar(item[k])}</Text>
            </View>
          ))}
        </View>
      );
...
const CareGapsPDFTemplate = ({ document: data }) => {
  let records = [];
  if (Array.isArray(data)) {
    if (data.length === 1 && data[0]?.care_gaps) records = Array.isArray(data[0].care_gaps) ? data[0].care_gaps : [data[0].care_gaps];
    else records = data;
  } else if (data?.care_gaps) records = Array.isArray(data.care_gaps) ? data.care_gaps : [data.care_gaps];
  else if (data?.documentData) { const dd = data.documentData; if (Array.isArray(dd)) records = dd; else if (dd?.care_gaps) records = Array.isArray(dd.care_gaps) ? dd.care_gaps : [dd.care_gaps]; else if (dd && typeof dd === 'object') records = [dd]; }
  else if (data && typeof data === 'object') records = [data];
  records = (records || []).filter(r => r && typeof r === 'object');

  if (records.length === 0) {
    return (<Document><Page size="A4" style={styles.page}><View style={styles.documentHeader}><Text style={styles.title}>Care Gaps</Text></View><Text style={styles.emptyState}>No records available</Text></Page></Document>);
  }

  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.title}>Care Gaps</Text></View>
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer}>
            <View style={styles.recordHeader} wrap={false}>
              <Text style={styles.recordTitle}>{record.type ? `Care Gap — ${fmtVal(record.type)}` : `Care Gap ${String(record._recordNumber || idx + 1)}`}</Text>
...
export default CareGapsPDFTemplate;
