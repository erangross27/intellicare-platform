/**
 * TropicalDiseaseAssessmentDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: tropical_disease_assessment
 *
 * 9 Sections:
 *   1. travel-history: travelHistoryCountries, travelStartDate, travelReturnDate, exposureHistory
 *   2. vector-exposure: vectorExposure, prophylaxisCompliance, vaccinationStatus
 *   3. fever-presentation: feverOnsetDate, feverPattern, peakTemperature, suspectedPathogen
 *   4. parasitology: parasitologyResults, malariaSpecies, parasitemia, rapidDiagnosticTest
 *   5. serology: serologyResults
 *   6. clinical-findings: hepatosplenomegaly, rashCharacteristics, neurologicSymptoms, hemorrhagicManifestations
 *   7. treatment: antimicrobialTherapy, treatmentResponse
 *   8. complications: complicationsDeveloped
 *   9. public-health: isolationRequired, publicHealthNotification
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import TropicalDiseaseAssessmentDocumentPDFTemplate from '../pdf-templates/TropicalDiseaseAssessmentDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './TropicalDiseaseAssessmentDocument.css';

/* ======= CONSTANTS ======= */
const SECTION_TITLES = {
  'travel-history': 'Travel History',
  'vector-exposure': 'Vector & Prophylaxis',
...
const SECTION_FIELDS = {
  'travel-history': ['travelHistoryCountries', 'travelStartDate', 'travelReturnDate', 'exposureHistory'],
...
const NUMBER_FIELDS = ['peakTemperature', 'parasitemia'];
/* Fields where 0 is MEANINGFUL (real value) vs a sentinel/placeholder.
 * parasitemia 0% = no parasites detected => show.
 * peakTemperature 0 (deg C/F non-physiological) = no fever recorded => hide. */
const MEANINGFUL_ZERO_FIELDS = ['parasitemia'];
const isSentinelZero = (fn, v) => typeof v === 'number' && v === 0 && !MEANINGFUL_ZERO_FIELDS.includes(fn);
const BOOLEAN_FIELDS = ['hepatosplenomegaly', 'hemorrhagicManifestations', 'isolationRequired', 'publicHealthNotification'];
const ARRAY_FIELDS = ['travelHistoryCountries', 'vectorExposure', 'neurologicSymptoms', 'complicationsDeveloped'];
const STRING_FIELDS = ['exposureHistory', 'prophylaxisCompliance', 'vaccinationStatus', 'feverPattern', 'suspectedPathogen', 'parasitologyResults', 'malariaSpecies', 'rapidDiagnosticTest', 'serologyResults', 'rashCharacteristics', 'antimicrobialTherapy', 'treatmentResponse'];
const DATE_FIELDS = ['date', 'travelStartDate', 'travelReturnDate', 'feverOnsetDate'];
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldName]: value } }  (value may be a string or an array — fields are
   always stored whole, keyed by plain field name; this template has no dotted field keys.) */
const DRAFT_KEY = 'tropical_disease_assessmentPendingEdits';
const readDrafts = () => {
...
/* ======= COMPONENT ======= */
const TropicalDiseaseAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.tropical_disease_assessment) return Array.isArray(r.tropical_disease_assessment) ? r.tropical_disease_assessment : [r.tropical_disease_assessment];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.tropical_disease_assessment) return Array.isArray(dd.tropical_disease_assessment) ? dd.tropical_disease_assessment : [dd.tropical_disease_assessment]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Tropical Disease Assessment ${idx + 1}`.toLowerCase();
  ...
  /* ======= EDIT HANDLERS ======= */
  // stageDraft = persist one field DRAFT locally (localEdits + pendingEdits) and to the localStorage
  // draft store (survives refresh). NO DB write — Approve (handleApproveSection) commits later.
  // Drafts are keyed by record _id -> plain field name (fields are always stored whole).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // localEdits keys are `${fieldName}-${idx}` (fields stored whole — no dotted arrayIndex keys here).
      ...
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // plain field name
        // GOTCHA: only treat a dot-suffix as arrayIndex when the segment after the LAST dot is purely numeric.
        ...
        const resp = await secureApiClient.put(`/api/edit/tropical_disease_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/tropical_disease_assessment/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const next = { ...prev }; toCommit.forEach(k => delete next[k]); return next; });
      // Drop this record's committed drafts from localStorage
      const store = readDrafts();
      if (store[id]) { toCommit.forEach(k => { const f = k.slice(0, -suffix.length); delete store[id][f]; }); if (Object.keys(store[id]).length === 0) delete store[id]; writeDrafts(store); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (isSentinelZero(f, val)) return;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== TROPICAL DISEASE ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Tropical Disease Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderDateField = (record, fn, idx, sid) => {
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(String(val || '').split('T')[0]); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              <input type="date" className="edit-date" value={editValue} onChange={e => setEditValue(e.target.value)} ref={el => { if (el) { el.focus(); try { el.showPicker(); } catch {} } }} onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter') { e.stopPropagation(); handleSaveField(record, fn, idx, sid, null, editValue); } }} />
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx); if (!hasVal(val)) return null;
    if (isSentinelZero(fn, val)) return null;
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
    const label = FIELD_LABELS[fn] || fn;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveArrayItem(record, fn, idx, itemIdx, editValue, sid); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="tropical-disease-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Tropical Disease Assessment</h2></div>
        <div className="empty-state">No tropical disease assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="tropical-disease-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Tropical Disease Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<TropicalDiseaseAssessmentDocumentPDFTemplate document={pdfData} />} fileName={`tropical-disease-assessment-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search tropical disease assessment records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Tropical Disease Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'travel-history')}
            {renderSection(record, idx, 'vector-exposure')}
            {renderSection(record, idx, 'fever-presentation')}
            {renderSection(record, idx, 'parasitology')}
            ...
            {renderSection(record, idx, 'public-health')}
          </div>
        ))}
      </div>
    </div>
...
export default TropicalDiseaseAssessmentDocument;
