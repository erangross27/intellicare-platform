...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'rheumatology-consultations-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['chiefComplaint', 'jointPainDuration', 'morningStiffnessDuration', 'affectedJoints', 'swollenJointCount', 'tenderJointCount', 'das28Score', 'cdaiScore', 'patientGlobalAssessment', 'functionalStatus', 'rheumatoidFactor', 'antiCcpAntibodies', 'anaPattern', 'anaTiter', 'crpLevel', 'esrValue', 'uricAcidLevel', 'hlaB27Status', 'complementC3Level', 'complementC4Level', 'synovialFluidAnalysis', 'crystalType', 'currentDmards', 'biologicTherapy', 'corticosteroidUse'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('rheumatology_consultations', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[rheumatology_consultations] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('rheumatology_consultations', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[rheumatology_consultations] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
