/**
 * MriReportsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: mri_reports
 *
 * 8 Sections:
 *   1. scan-info: mriType (string), anatomicalRegion (string)
 *   2. clinical-indication: indication (string, sentence-split)
 *   3. technical-parameters: contrast (string)
 *   4. sequences: sequences (array)
 *   5. findings: findings (string, sentence-split)
 *   6. measurements: measurements (string, sentence-split)
 *   7. impression-comparison: impression (string, sentence-split), comparison (string, sentence-split),
 *                             recommendations (string, sentence-split)
 *   8. report-info: date (date picker), facility (string), radiologist (string), notes (string, sentence-split)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MriReportsDocumentPDFTemplate from '../pdf-templates/MriReportsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './MriReportsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'mri_reportsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'impression-comparison': 'Impression & Recommendations',
...
const SECTION_FIELDS = {
  'scan-info': ['mriType', 'anatomicalRegion'],
  'clinical-indication': ['indication'],
  'technical-parameters': ['contrast'],
  'sequences': ['sequences'],
  'findings': ['findings'],
  'measurements': ['measurements'],
  'impression-comparison': ['impression', 'comparison', 'recommendations'],
  'report-info': ['facility', 'radiologist', 'notes'],
};
...
const STRING_FIELDS = [
  'mriType', 'anatomicalRegion', 'indication', 'contrast',
  'findings', 'measurements', 'impression', 'comparison',
  'recommendations', 'facility', 'radiologist', 'notes',
];

const DATE_FIELDS = ['date'];
const ARRAY_FIELDS = ['sequences'];
...
/* ======= COMPONENT ======= */
const MriReportsDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.mri_reports) return Array.isArray(r.mri_reports) ? r.mri_reports : [r.mri_reports];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.mri_reports) return Array.isArray(dd.mri_reports) ? dd.mri_reports : [dd.mri_reports]; return [dd]; }
      return [r];
    });
  ...
  /* ======= REHYDRATE PENDING DRAFTS =======
     ...
     fieldPart = "field" (whole-field/sentence edits) or "field.arrayIndex" (array element edits). */
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayElem) {
          // array element: localEdits stores the WHOLE array under "field-idx"; mark "field.N-idx"
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `MRI Report ${idx + 1}`.toLowerCase();
  ...
  const pdfData = useMemo(() => {
    return filteredRecords.map((record, idx) => {
      ...
      // Array fields: only use the staged array if it is NOT a pending draft (else keep original)
      ARRAY_FIELDS.forEach(f => {
        const baseKey = `${f}-${idx}`;
        merged[f] = pendingEdits[baseKey]
          ? (Array.isArray(record[f]) ? record[f] : [])
          : getEffectiveArray(record, f, idx);
      });
      return merged;
    });
  ...
  // Approve (handleApproveSection) is the only path that commits this to MongoDB.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      const committedKeys = []; // localEdits/pendingEdits keys to clear after commit
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/mri_reports/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedKeys.push(`${baseField}-${idx}`);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/mri_reports/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (store[id]) {
        for (const fieldPart of Object.keys(store[id])) {
          const dotIdx = fieldPart.lastIndexOf('.');
          const isArrayElem = dotIdx !== -1 && /^\d+$/.test(fieldPart.slice(dotIdx + 1));
          const baseField = isArrayElem ? fieldPart.slice(0, dotIdx) : fieldPart;
    ...
    } catch (err) { console.error('[MriReports] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MRI REPORTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `MRI Report ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="mri-reports-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">MRI Reports</h2></div>
        <div className="empty-state">No MRI reports available</div>
      </div>
    );
  }
  ...
  return (
    <div className="mri-reports-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">MRI Reports</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MriReportsDocumentPDFTemplate document={pdfData} />} fileName="MRI_Reports.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search MRI reports..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`MRI Report ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'scan-info')}
            {renderSection(record, idx, 'clinical-indication')}
            {renderSection(record, idx, 'technical-parameters')}
            {renderSection(record, idx, 'sequences')}
            {renderSection(record, idx, 'findings')}
            {renderSection(record, idx, 'measurements')}
            {renderSection(record, idx, 'impression-comparison')}
...
export default MriReportsDocument;
