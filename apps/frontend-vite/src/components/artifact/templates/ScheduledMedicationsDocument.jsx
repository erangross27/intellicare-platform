 ...
 * July 2026 — recursive, dot-path editable scheduled medications
...
const SECTION_TITLES = {
  ...
  prescriberDetails: 'Prescriber Details',
  duration: 'Duration',
  safety: 'Safety',
};
const FIELD_LABELS = {
  name: 'Name', genericName: 'Generic Name', dosage: 'Dosage', frequency: 'Frequency', route: 'Route',
  prescriber: 'Prescriber', indication: 'Indication', instructions: 'Instructions',
  startDate: 'Start Date', endDate: 'End Date', duration: 'Duration', durationDays: 'Duration Days', durationUnit: 'Duration Unit', active: 'Active', refills: 'Refills',
  safetyWarning: 'Safety Warning', sideEffects: 'Side Effects', drugInteractions: 'Drug Interactions',
};
const SECTION_FIELDS = {
  medicationInformation: ['name', 'genericName', 'dosage', 'frequency', 'route'],
  prescriberDetails: ['prescriber', 'indication', 'instructions'],
...
const ENUM_FIELDS = ['route', 'durationUnit'];
const NUMBER_FIELDS = ['durationDays', 'refills'];
const NUMBER_STRING_FIELDS = [];
const OBJECT_FIELDS = [];
const NARRATIVE_PATHS = [];
const PARENTHETICAL_LABEL_FIELDS = [];
const COMMA_FIELDS = ['safetyWarning'];
const COMMA_ARRAY_FIELDS = ['sideEffects'];
const ARRAY_FIELDS = ['sideEffects', 'drugInteractions'];
const SEMICOLON_FIELDS = ['instructions', 'safetyWarning', 'sideEffects', 'drugInteractions'];
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'route' ? ['oral', 'sublingual', 'topical', 'inhaled', 'intramuscular', 'intravenous', 'subcutaneous', 'rectal', 'vaginal', 'other']
    : field === 'durationUnit' ? ['day', 'days', 'week', 'weeks', 'month', 'months', 'ongoing'] : [];
...
const ScheduledMedicationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.scheduled_medications) return Array.isArray(record.scheduled_medications) ? record.scheduled_medications : [record.scheduled_medications];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.scheduled_medications) return Array.isArray(nested.scheduled_medications) ? nested.scheduled_medications : [nested.scheduled_medications];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/scheduled_medications/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/scheduled_medications/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[ScheduledMedications] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Scheduled Medication ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="scheduled-medications-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Scheduled Medications</h2></div><div className="empty-state">No scheduled medications records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Scheduled Medication ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="scheduled-medications-document" ref={containerRef}>
    <div className="document-header">
      ...
      <div className="header-actions">
        ...
        <PDFDownloadLink document={<ScheduledMedicationsDocumentPDFTemplate document={pdfData} />} fileName="Scheduled_Medications.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search scheduled medications..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Scheduled Medication ' + (recordIndex + 1))}</h3></div>
