/**
 * Continuous Glucose Monitor Discussion Edit Route
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'continuous-glucose-monitor-discussion-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'diabetesType',
  'currentA1c',
  'totalDailyInsulinDose',
  'insulinDeliveryMethod',
  'frequencyOfHypoglycemia',
  'hypoglycemiaUnawareness',
  'currentMonitoringMethod',
  'fingerstickFrequency',
  'timeInRange',
  'targetGlucoseRange',
  'cgmDeviceRecommended',
  'sensorWearDuration',
  'calibrationRequired',
  'smartphoneCompatibility',
  'cgmIndicationCriteria',
  'dataSharingDiscussed',
  'alarmSettingsReviewed',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('continuous_glucose_monitor_discussion', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[continuous_glucose_monitor_discussion] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('continuous_glucose_monitor_discussion', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[continuous_glucose_monitor_discussion] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
