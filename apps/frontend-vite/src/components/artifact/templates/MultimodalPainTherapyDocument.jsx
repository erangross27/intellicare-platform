/**
 * MultimodalPainTherapyDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: multimodal_pain_therapy
 *
 * 10 Sections:
 *   1. pain-profile: painDuration, primaryPainLocation, secondaryPainLocations, painCharacteristics, painFrequency
 *   2. pain-intensity: baselinePainIntensity, currentPainIntensity
 *   3. functional-assessment: functionalImpairmentScore, activitiesOfDailyLiving, workStatusChange
 *   4. therapy-modalities: therapyModalitiesUsed
 *   5. pharmacological-management: pharmacologicalInterventions, opioidDailyDose
 *   6. interventional-procedures: interventionalProcedures
 *   7. session-counts: physicalTherapySessions, psychologicalTherapySessions
 *   8. psychological-screening: catastrophizingScore, depressionScreeningScore, anxietyScreeningScore, sleepDisturbanceSeverity
 *   9. treatment-plan: treatmentDuration, programComplianceRate, treatmentGoals
 *  10. outcomes: outcomeAchievement, dischargePainScore
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MultimodalPainTherapyDocumentPDFTemplate from '../pdf-templates/MultimodalPainTherapyDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './MultimodalPainTherapyDocument.css';
...
const STRING_FIELDS = ['painDuration', 'primaryPainLocation', 'painFrequency', 'opioidDailyDose', 'activitiesOfDailyLiving', 'workStatusChange', 'sleepDisturbanceSeverity', 'treatmentDuration', 'outcomeAchievement'];
...
const DRAFT_KEY = 'multimodalPainTherapyPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const MultimodalPainTherapyDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.multimodal_pain_therapy) return Array.isArray(r.multimodal_pain_therapy) ? r.multimodal_pain_therapy : [r.multimodal_pain_therapy];
      ...
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.multimodal_pain_therapy) return Array.isArray(dd.multimodal_pain_therapy) ? dd.multimodal_pain_therapy : [dd.multimodal_pain_therapy]; return [dd]; }
      return [r];
    });
  ...
  /* ═══════ UTILS ═══════ */
  /* hide-zero: a numeric 0 (score not recorded) is treated as empty — consistent with PDF + Copy All */
  const hasVal = useCallback((v) => { if (v === null || v === undefined || v === '') return false; if (typeof v === 'boolean') return true; if (typeof v === 'number') return v !== 0; if (typeof v === 'string') return v.trim() !== ''; if (Array.isArray(v)) return v.filter(x => x !== null && x !== undefined && x !== '').length > 0; if (typeof v === 'object') return Object.keys(v).length > 0; return true; }, []);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Multimodal Pain Therapy ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Staged drafts for THIS section's fields (localEdits key = `${field}-${idx}`, value = full field value)
    const toCommit = Object.keys(localEdits).filter(k => {
      if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
      const fieldPart = k.slice(0, -suffix.length); // field name (no numeric arrayIndex — values are full fields)
      return fields.includes(fieldPart);
    });
    try {
      // Persist each staged field to the DB now (full field value; no arrayIndex needed — arrays are stored whole)
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length);
        const resp = await secureApiClient.put(`/api/edit/multimodal_pain_therapy/${id}/edit`, { field: fieldPart, value: localEdits[editKey] });
        if (!resp || !resp.success) throw new Error((resp && resp.error) || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/multimodal_pain_therapy/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        if (f === 'functionalImpairmentScore' || f === 'programComplianceRate') display = `${val}%`;
        else if (f === 'baselinePainIntensity' || f === 'currentPainIntensity' || f === 'dischargePainScore') display = `${val}/10`;
        text += `${label}\n${display}\n\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MULTIMODAL PAIN THERAPY ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Multimodal Pain Therapy ${idx + 1}\n${'='.repeat(40)}\n\n`;
      Object.keys(SECTION_FIELDS).forEach(sid => {
        const fields = SECTION_FIELDS[sid] || [];
        if (!fields.some(f => hasVal(r[f]))) return; // skip a fully-empty section (all fields blank/0)
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    ...
    if (fn === 'functionalImpairmentScore' || fn === 'programComplianceRate') displayVal = `${val}%`;
    else if (fn === 'baselinePainIntensity' || fn === 'currentPainIntensity' || fn === 'dischargePainScore') displayVal = `${val}/10`;
    const isModified = editedFields[editKey];
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
  ...
  const renderArrayField = (record, fn, idx, sid) => {
    ...
    const label = FIELD_LABELS[fn] || fn;
    // single-name gate: suppress the field subtitle when it equals its section title (e.g. Interventional Procedures)
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          // "Epidural: 0.0625% ..." → label becomes a nested-subtitle, value fills the row (never side-by-side)
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; stageFieldDraft(record, fn, idx, sid, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  /* ═══════ RENDER: PAIN INTENSITY SECTION (special visual) ═══════ */
  const renderPainIntensitySection = (record, idx) => {
    const sid = 'pain-intensity';
    if (!shouldShowSection(record, sid)) return null;
    const baseline = getFieldValue(record, 'baselinePainIntensity', idx);
    const current = getFieldValue(record, 'currentPainIntensity', idx);
    if (!hasVal(baseline) && !hasVal(current)) return null;

    const improvement = (hasVal(baseline) && hasVal(current)) ? baseline - current : 0;
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          <div className="section-header">
            <h4 className="section-title">{highlightText('Pain Intensity')}</h4>
            <div className="header-right-actions">
              <button className={`copy-btn ${copiedSection === copyId ? 'copied' : ''}`} onClick={() => {
                const lines = ['Pain Intensity', '='.repeat(40), ''];
                if (hasVal(baseline)) lines.push(`Baseline Pain Intensity: ${baseline}/10`);
                if (hasVal(current)) lines.push(`Current Pain Intensity: ${current}/10`);
                if (improvement > 0) lines.push(`Improvement: ${improvement} points`);
                copySection(lines.join('\n'), copyId);
              }}>{copiedSection === copyId ? 'Copied!' : 'Copy Section'}</button>
              {renderApproveButton(record, sid, idx)}
            </div>
          </div>
          {/* Pain Score Summary Card */}
          {(hasVal(baseline) || hasVal(current)) && (fieldMatches(record, 'baselinePainIntensity', idx) || fieldMatches(record, 'currentPainIntensity', idx) || sectionTitleMatches(sid)) && (
            <div className="rec-mini-card">
              <div className="nested-subtitle">{highlightText('Pain Score Change')}</div>
              <div className="numbered-row">
                <div className="row-content">
                  <span className="pain-score-badge">
                    {hasVal(baseline) && <span className="score-value pre">{highlightText(`${baseline}/10`)}</span>}
                    {hasVal(baseline) && hasVal(current) && <span className="score-arrow">&rarr;</span>}
                    {hasVal(current) && <span className="score-value post">{highlightText(`${current}/10`)}</span>}
                    {improvement > 0 && <span className="score-improvement">({improvement} pt improvement)</span>}
                  </span>
                </div>
                <button className={`copy-btn ${copiedItems[`scores-${idx}`] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`Baseline: ${baseline}/10 -> Current: ${current}/10${improvement > 0 ? ` (${improvement} pt improvement)` : ''}`, `scores-${idx}`); }}>{copiedItems[`scores-${idx}`] ? 'Copied!' : 'Copy'}</button>
              </div>
            </div>
          )}
          {/* Individual editable fields */}
          {renderNumberField(record, 'baselinePainIntensity', idx, sid)}
          {renderNumberField(record, 'currentPainIntensity', idx, sid)}
        </div>
      </div>
    );
  };
  ...
  const renderSection = (record, idx, sid) => {
    if (sid === 'pain-intensity') return renderPainIntensitySection(record, idx);
    const title = SECTION_TITLES[sid];
    if (!shouldShowSection(record, sid)) return null;
    const fields = SECTION_FIELDS[sid] || [];
  ...
  if (!records || records.length === 0) {
    return (
      <div className="multimodal-pain-therapy-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Multimodal Pain Therapy</h2></div>
        <div className="empty-state">No multimodal pain therapy records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="multimodal-pain-therapy-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Multimodal Pain Therapy</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MultimodalPainTherapyDocumentPDFTemplate document={pdfData} />} fileName="Multimodal_Pain_Therapy.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pain therapy records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Multimodal Pain Therapy ${idx + 1}`)}</h3>
              {hasVal(record.primaryPainLocation) && (
                <div className="record-subtitle">{highlightText(record.primaryPainLocation)}</div>
              )}
            </div>
            {renderSection(record, idx, 'visit-info')}
            {renderSection(record, idx, 'pain-profile')}
            {renderSection(record, idx, 'pain-intensity')}
            {renderSection(record, idx, 'functional-assessment')}
            {renderSection(record, idx, 'therapy-modalities')}
            {renderSection(record, idx, 'pharmacological-management')}
            {renderSection(record, idx, 'interventional-procedures')}
            {renderSection(record, idx, 'session-counts')}
            {renderSection(record, idx, 'psychological-screening')}
...
export default MultimodalPainTherapyDocument;
