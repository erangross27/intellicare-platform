/**
 * PodiatryConsultationsDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: podiatry_consultations
 *
 * 7 Sections:
 *   1. chief-complaint: chiefPodiatricComplaint
 *   2. vascular-assessment: ankleBrachialIndex, toeBrachialIndex, pedalPulsesAssessment, transcutaneousOxygenPressure
 *   3. classification-scores: wagnerUlcerClassification, universityOfTexasWoundClassification, diabeticFootInfectionSeverity, charcotArthropathyStage, footPostureIndex, manchesterOxfordFootQuestionnaire
 *   4. neuropathy-assessment: semmeswWeinsteinMonofilamentScore, vibrationPerceptionThreshold
 *   5. biomechanical-assessment: halluxValgusAngle, intermetatarsalAngle, calcanealPitchAngle, talusFirstMetatarsalAngle, firstMtpjRangeOfMotion, achillesTendonThompsonTest, anteriorDrawerTestAnkle
 *   6. nail-toe-deformities: nailDystrophyClassification, lesserToeDeformities
 *   7. treatment-plan: plantarFasciitisChronicity, customOrthoticPrescription, offloadingDevicePrescribed
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PodiatryConsultationsDocumentPDFTemplate from '../pdf-templates/PodiatryConsultationsDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PodiatryConsultationsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldName]: value } } */
const DRAFT_KEY = 'podiatry_consultationsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'chief-complaint': 'Chief Podiatric Complaint',
  ...
  'nail-toe-deformities': 'Nail & Toe Deformities',
...
const SECTION_FIELDS = {
  'chief-complaint': ['chiefPodiatricComplaint'],
  'vascular-assessment': ['ankleBrachialIndex', 'toeBrachialIndex', 'pedalPulsesAssessment', 'transcutaneousOxygenPressure'],
  'classification-scores': ['wagnerUlcerClassification', 'universityOfTexasWoundClassification', 'diabeticFootInfectionSeverity', 'charcotArthropathyStage', 'footPostureIndex', 'manchesterOxfordFootQuestionnaire'],
  'neuropathy-assessment': ['semmeswWeinsteinMonofilamentScore', 'vibrationPerceptionThreshold'],
  'biomechanical-assessment': ['halluxValgusAngle', 'intermetatarsalAngle', 'calcanealPitchAngle', 'talusFirstMetatarsalAngle', 'firstMtpjRangeOfMotion', 'achillesTendonThompsonTest', 'anteriorDrawerTestAnkle'],
  'nail-toe-deformities': ['nailDystrophyClassification', 'lesserToeDeformities'],
  'treatment-plan': ['plantarFasciitisChronicity', 'customOrthoticPrescription', 'offloadingDevicePrescribed'],
};

const NUMBER_FIELDS = ['ankleBrachialIndex', 'toeBrachialIndex', 'vibrationPerceptionThreshold', 'halluxValgusAngle', 'intermetatarsalAngle', 'calcanealPitchAngle', 'talusFirstMetatarsalAngle', 'footPostureIndex', 'manchesterOxfordFootQuestionnaire', 'transcutaneousOxygenPressure', 'wagnerUlcerClassification'];
const BOOLEAN_FIELDS = [];
const ARRAY_FIELDS = ['lesserToeDeformities'];
const STRING_FIELDS = ['chiefPodiatricComplaint', 'pedalPulsesAssessment', 'universityOfTexasWoundClassification', 'diabeticFootInfectionSeverity', 'charcotArthropathyStage', 'semmeswWeinsteinMonofilamentScore', 'firstMtpjRangeOfMotion', 'achillesTendonThompsonTest', 'anteriorDrawerTestAnkle', 'nailDystrophyClassification', 'plantarFasciitisChronicity', 'customOrthoticPrescription', 'offloadingDevicePrescribed'];

/* Number fields where 0 is a sentinel (not-measured / not-applicable), NOT a meaningful zero.
   ABI/TBI, angles, indices, and questionnaire scores of 0 mean "not obtained" for this batch data.
   Suppress these so "0" never leaks to UI/Copy/PDF. */
const SENTINEL_ZERO_FIELDS = ['ankleBrachialIndex', 'toeBrachialIndex', 'vibrationPerceptionThreshold', 'halluxValgusAngle', 'intermetatarsalAngle', 'calcanealPitchAngle', 'talusFirstMetatarsalAngle', 'footPostureIndex', 'manchesterOxfordFootQuestionnaire', 'transcutaneousOxygenPressure', 'wagnerUlcerClassification'];
...
/* ═══════ COMPONENT ═══════ */
const PodiatryConsultationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.podiatry_consultations) return Array.isArray(r.podiatry_consultations) ? r.podiatry_consultations : [r.podiatry_consultations];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.podiatry_consultations) return Array.isArray(dd.podiatry_consultations) ? dd.podiatry_consultations : [dd.podiatry_consultations]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Podiatry Consultation ${idx + 1}`.toLowerCase();
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    // Stage to localStorage (drafts keyed by the field name only; value is the full field value)
    const store = readDrafts();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Staged edits for this record live in localEdits keyed "<fieldPart><suffix>" and flagged pending.
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/podiatry_consultations/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/podiatry_consultations/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[PodiatryConsultations] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
    ...
    fields.forEach(f => {
      ...
      if (BOOLEAN_FIELDS.includes(f)) {
        ...
        if (sentences.length > 1) {
          ...
          const parsedS = parseLabel(cleaned);
          const commaParts = !parsedS.isLabeled ? splitByComma(cleaned) : null;
          if (commaParts && commaParts.length >= 3) {
            text += `${label}\n`;
            commaParts.forEach((item, i) => { text += `  ${i + 1}. ${item}\n`; });
            text += '\n';
          } else {
            text += `${label}\n${strVal}\n\n`;
          }
  ...
  const copyAllText = useCallback(async () => {
    let text = `Podiatry Consultations\n${'='.repeat(40)}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Podiatry Consultation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderStringField = (record, fn, idx, sid, title) => {
    ...
    /* Unlabeled clinical list (>=3 comma items) → split into value-only rows (user prefers aggressive splitting) */
    const singleClean = strVal.replace(/[;.]+$/, '').trim();
    const parsedSingle = parseLabel(singleClean);
    const commaPartsSingle = !parsedSingle.isLabeled ? splitByComma(singleClean) : null;
    if (commaPartsSingle && commaPartsSingle.length >= 3) {
      return (
        <div key={fn} className="rec-mini-card">
          {showLabelSingle && <div className="nested-subtitle">{highlightText(label)}</div>}
          {commaPartsSingle.map((ci, ciIdx) => {
            const commaKey = `${fn}-${idx}-c${ciIdx}`;
            const ciEditing = editingField === commaKey;
            const ciBadge = editedSentences[commaKey];
            if (searchTerm.trim() && !(sectionTitleMatches(sid) || label.toLowerCase().includes(searchTerm.toLowerCase().trim()) || ci.toLowerCase().includes(searchTerm.toLowerCase().trim()))) return null;
            return (
              <div key={ciIdx}>
                <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                  {ciEditing ? (
                    <div className="edit-field-container">
                      ...
                      {saveError && <div className="save-error">{saveError}</div>}
                      <div className="edit-actions">
                        <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const currentVal2 = String(getFieldValue(record, fn, idx) || '').replace(/[;.]+$/, '').trim(); const items2 = splitByComma(currentVal2); const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { items2.splice(ciIdx, 1, ...subParts); } else { items2[ciIdx] = trimmed.replace(/[;.]+$/, '').trim(); } const rebuilt = items2.join(', '); setSaveError(null); stageFieldDraft(record, fn, idx, sid, rebuilt); const marks = { [commaKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-c${ciIdx + ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                        <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                      </div>
                    </div>
                  ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="podiatry-consultations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Podiatry Consultations</h2></div>
        <div className="empty-state">No podiatry consultation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="podiatry-consultations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Podiatry Consultations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PodiatryConsultationsDocumentPDFTemplate document={pdfData} />} fileName="Podiatry_Consultations.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search podiatry consultation records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Podiatry Consultation ${idx + 1}`)}</h3>
            </div>
            ...
            {renderSection(record, idx, 'biomechanical-assessment')}
            {renderSection(record, idx, 'nail-toe-deformities')}
...
export default PodiatryConsultationsDocument;
