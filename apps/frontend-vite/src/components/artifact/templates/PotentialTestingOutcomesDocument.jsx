/**
 * PotentialTestingOutcomesDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: potential_testing_outcomes
 *
 * 5 Sections:
 *   1. diagnostic-imaging: imagingFindings, boneDensitometry, mammmographyResults
 *   2. cardiac-testing: electrocardiogramResults, echocardiogramResults, stressTestResults, holterMonitorResults, cardiacEnzymes, ankleBrachialIndex
 *   3. laboratory-panels: thyroidFunctionTests, hemoglobinA1c, liverFunctionPanel, glomerularFiltrationRate, urinalysisResults, arterialBloodGas, coagulationStudies
 *   4. specialized-testing: pulmonaryFunctionTests, colonoscopyFindings, pathologyReport, microbiolgyResults
 *   5. additional-studies: sleepStudyResults, cerebrospinalFluidAnalysis
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PotentialTestingOutcomesDocumentPDFTemplate from '../pdf-templates/PotentialTestingOutcomesDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './PotentialTestingOutcomesDocument.css';
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldName]: value } }  (value may be string, number, or array) */
const DRAFT_KEY = 'potential_testing_outcomesPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const PotentialTestingOutcomesDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Potential Testing Outcomes ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // localEdits keys are "<field>-<idx>"; commit only this section's pending fields for this record.
      ...
      for (const editKey of toCommit) {
        ...
        if (Array.isArray(value)) {
          // Array field: persist each element by arrayIndex (mirrors the inline array save payload).
          for (let ai = 0; ai < value.length; ai++) {
            const resp = await secureApiClient.put(`/api/edit/potential_testing_outcomes/${id}/edit`, { field, value: value[ai], arrayIndex: ai });
            if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
          }
        } else {
          const resp = await secureApiClient.put(`/api/edit/potential_testing_outcomes/${id}/edit`, { field, value });
          if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        }
      }
      await secureApiClient.put(`/api/edit/potential_testing_outcomes/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[PotentialTestingOutcomes] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== POTENTIAL TESTING OUTCOMES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Potential Testing Outcomes ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; const draftKey = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [draftKey]: currentArr })); setPendingEdits(prev => ({ ...prev, [draftKey]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); const store = readDrafts(); if (!store[id]) store[id] = {}; store[id][fn] = currentArr; writeDrafts(store); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="potential-testing-outcomes-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Potential Testing Outcomes</h2></div>
        <div className="empty-state">No potential testing outcomes records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="potential-testing-outcomes-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search potential testing outcomes..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Potential Testing Outcomes ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'diagnostic-imaging')}
            {renderSection(record, idx, 'cardiac-testing')}
            {renderSection(record, idx, 'laboratory-panels')}
            {renderSection(record, idx, 'specialized-testing')}
...
export default PotentialTestingOutcomesDocument;
