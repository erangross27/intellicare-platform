/**
 * VentilatorWeaningProtocolDocument.jsx
 * June 2026 — ICU critical-care weaning protocol template, blue glow theme, inline editing
 * Collection: ventilator_weaning_protocol
 *
 * 7 Sections:
 *   1. weaning-readiness: weaningReadinessScore, rapidShallowBreathingIndex, maximalInspiratoryPressure, airwayOcclusionPressure, integratedWeaningIndex
 *   2. spontaneous-breathing-trial: spontaneousBreathingTrialMethod, spontaneousBreathingTrialDuration, weaningProtocolType, dailySedationInterruption
 *   3. respiratory-mechanics: peepLevelPreWeaning, fio2PreWeaning, paoToFio2Ratio, staticCompliance, diaphragmUltrasoundThickening
 *   4. airway-secretions: cuffLeakTestResult, cuffLeakVolume, coughStrengthAssessment, secretionBurden
 *   5. neurological-sedation: glasgowComaScaleScore, richmondAgitationSedationScale
 *   6. hemodynamics-severity: vasopressorRequirement, apacheIiScoreAtWeaningStart, ventilatorDaysPreWeaning
 *   7. extubation-outcome: extubationOutcome, postExtubationRespiratorySupport
 */
...
const SECTION_TITLES = {
  'weaning-readiness': 'Weaning Readiness',
  'spontaneous-breathing-trial': 'Spontaneous Breathing Trial',
  'respiratory-mechanics': 'Respiratory Mechanics & Oxygenation',
  'airway-secretions': 'Airway & Secretions',
  'neurological-sedation': 'Neurological & Sedation',
  'hemodynamics-severity': 'Hemodynamics & Severity',
  'extubation-outcome': 'Extubation Outcome',
};

const FIELD_LABELS = {
  weaningReadinessScore: 'Weaning Readiness Score',
  rapidShallowBreathingIndex: 'Rapid Shallow Breathing Index',
  maximalInspiratoryPressure: 'Maximal Inspiratory Pressure',
  airwayOcclusionPressure: 'Airway Occlusion Pressure',
  integratedWeaningIndex: 'Integrated Weaning Index',
  spontaneousBreathingTrialMethod: 'Spontaneous Breathing Trial Method',
  spontaneousBreathingTrialDuration: 'Spontaneous Breathing Trial Duration (min)',
  weaningProtocolType: 'Weaning Protocol Type',
  dailySedationInterruption: 'Daily Sedation Interruption',
  peepLevelPreWeaning: 'PEEP Level Pre-Weaning',
  fio2PreWeaning: 'FiO2 Pre-Weaning',
  paoToFio2Ratio: 'PaO2/FiO2 Ratio',
  staticCompliance: 'Static Compliance',
  diaphragmUltrasoundThickening: 'Diaphragm Ultrasound Thickening',
  cuffLeakTestResult: 'Cuff Leak Test Result',
  cuffLeakVolume: 'Cuff Leak Volume',
  ...
  richmondAgitationSedationScale: 'Richmond Agitation-Sedation Scale (RASS)',
  vasopressorRequirement: 'Vasopressor Requirement',
  apacheIiScoreAtWeaningStart: 'APACHE II Score at Weaning Start',
  ventilatorDaysPreWeaning: 'Ventilator Days Pre-Weaning',
  extubationOutcome: 'Extubation Outcome',
  postExtubationRespiratorySupport: 'Post-Extubation Respiratory Support',
};

const SECTION_FIELDS = {
  'weaning-readiness': ['weaningReadinessScore', 'rapidShallowBreathingIndex', 'maximalInspiratoryPressure', 'airwayOcclusionPressure', 'integratedWeaningIndex'],
  'spontaneous-breathing-trial': ['spontaneousBreathingTrialMethod', 'spontaneousBreathingTrialDuration', 'weaningProtocolType', 'dailySedationInterruption'],
  'respiratory-mechanics': ['peepLevelPreWeaning', 'fio2PreWeaning', 'paoToFio2Ratio', 'staticCompliance', 'diaphragmUltrasoundThickening'],
  'airway-secretions': ['cuffLeakTestResult', 'cuffLeakVolume', 'coughStrengthAssessment', 'secretionBurden'],
  'neurological-sedation': ['glasgowComaScaleScore', 'richmondAgitationSedationScale'],
  'hemodynamics-severity': ['vasopressorRequirement', 'apacheIiScoreAtWeaningStart', 'ventilatorDaysPreWeaning'],
  'extubation-outcome': ['extubationOutcome', 'postExtubationRespiratorySupport'],
};
...
const DRAFT_KEY = 'ventilator_weaning_protocolPendingEdits';
const readDrafts = () => {
...
const VentilatorWeaningProtocolDocument = ({ document: docProp, data, templateData }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Ventilator Weaning Protocol ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Collect staged pending edits belonging to this section's fields for this record.
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/ventilator_weaning_protocol/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/ventilator_weaning_protocol/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    ...
    pdfData.forEach((r, idx) => {
      text += `Ventilator Weaning Protocol ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="ventilator-weaning-protocol-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Ventilator Weaning Protocol</h2></div>
        <div className="empty-state">No ventilator weaning protocol data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="ventilator-weaning-protocol-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search ventilator weaning protocol..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Ventilator Weaning Protocol ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'weaning-readiness')}
            {renderSection(record, idx, 'spontaneous-breathing-trial')}
            {renderSection(record, idx, 'respiratory-mechanics')}
            {renderSection(record, idx, 'airway-secretions')}
            {renderSection(record, idx, 'neurological-sedation')}
            {renderSection(record, idx, 'hemodynamics-severity')}
            {renderSection(record, idx, 'extubation-outcome')}
          </div>
        ))}
      </div>
    </div>
...
export default VentilatorWeaningProtocolDocument;
