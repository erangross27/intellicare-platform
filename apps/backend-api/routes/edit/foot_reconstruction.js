/**
 * Foot Reconstruction Edit Route
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'foot-reconstruction-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'footReconstructionIndication',
  'woundWagnerClassification',
  'texasWoundClassificationStage',
  'ankleBrachialIndex',
  'toeBrachialIndex',
  'transcutaneousOxygenPressure',
  'semmeswWeinsteinMonofilamentScore',
  'michiganNeuropathyScore',
  'preoperativeHemoglobinA1c',
  'flapTypeUtilized',
  'recipientVesselSelection',
  'angiosomeTerritory',
  'charcotEichenholtzStage',
  'brodskySandersClassification',
  'osteomyelitisCiernyMaderClassification',
  'boneDefectLength',
  'defectSizeDimensions',
  'externalFixatorType',
  'skinGraftMeshRatio',
  'negativePressureWoundTherapyDuration',
  'microsurgicalIschemiaTime',
  'flapPerfusionMonitoringMethod',
  'postoperativeAnticoagulationProtocol',
  'weightBearingRestrictionDuration',
  'aofasAnkleHindfootScore',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('foot_reconstruction', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[foot_reconstruction] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('foot_reconstruction', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[foot_reconstruction] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
