/**
 * KidneyFunctionReportsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: kidney_function_reports
 *
 * 4 Sections:
 *   1. kidney-function: serumCreatinine, estimatedGFR, bloodUreaNitrogen, creatinineClearance,
 *                       urinalysisProtein, albuminCreatinineRatio, ckdStage
 *   2. electrolytes: sodiumLevel, potassiumLevel, phosphorusLevel, calciumLevel,
 *                    parathyroidHormone, bicarbonateLevel
 *   3. hematology: hemoglobinLevel, hematocritLevel
 *   4. specialized: proteinuria24Hour, urineSpecificGravity, renalUltrasoundFindings,
 *                   dialysisType, ktVRatio, cystatinC, microalbuminuria,
 *                   fractionalExcretionSodium
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import KidneyFunctionReportsDocumentPDFTemplate from '../pdf-templates/KidneyFunctionReportsDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './KidneyFunctionReportsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name) */
const DRAFT_KEY = 'kidney_function_reportsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['urinalysisProtein', 'ckdStage', 'renalUltrasoundFindings', 'dialysisType'];
...
/* ═══════ COMPONENT ═══════ */
const KidneyFunctionReportsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.kidney_function_reports) return Array.isArray(r.kidney_function_reports) ? r.kidney_function_reports : [r.kidney_function_reports];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.kidney_function_reports) return Array.isArray(dd.kidney_function_reports) ? dd.kidney_function_reports : [dd.kidney_function_reports]; return [dd]; }
      return [r];
    });
  ...
  // Re-populate staged drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
  useEffect(() => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Kidney Function Report ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      // Persist each staged field to the DB now (field, or field+arrayIndex for purely-numeric array elements)
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/kidney_function_reports/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/kidney_function_reports/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== KIDNEY FUNCTION REPORTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Kidney Function Report ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="kidney-function-reports-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Kidney Function Reports</h2></div>
        <div className="empty-state">No kidney function reports records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="kidney-function-reports-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Kidney Function Reports</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<KidneyFunctionReportsDocumentPDFTemplate document={pdfData} />} fileName="Kidney_Function_Reports.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search kidney function reports..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Kidney Function Report ${idx + 1}`)}</h3>
            </div>
...
export default KidneyFunctionReportsDocument;
