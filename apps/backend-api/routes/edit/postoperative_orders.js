...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'postoperative-orders-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['date', 'provider', 'facility', 'type', 'status', 'diet', 'activity', 'painManagement', 'antibiotics', 'prophylaxis', 'monitoring', 'specialInstructions', 'dischargeHome', 'findings', 'assessment', 'plan', 'notes', 'recommendations', 'results'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('postoperative_orders', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[postoperative_orders] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('postoperative_orders', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[postoperative_orders] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
