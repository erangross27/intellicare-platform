/**
 * TherapyProgressNotes Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/therapy_progress_notes/:id/edit    — Edit a single field
 * PUT /api/edit/therapy_progress_notes/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'therapy-progress-notes-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'patientSubjectiveReport',
  'functionalOutcomeMeasures',
  'painScaleAssessment',
  'rangeOfMotionMeasurements',
  'muscleStrengthGrading',
  'balanceAssessmentScores',
  'gaitAnalysisFindings',
  'activitiesOfDailyLivingStatus',
  'cognitiveFunctionAssessment',
  'speechLanguageEvaluation',
  'respiratoryFunctionMetrics',
  'cardiovascularResponse',
  'therapeuticInterventionsProvided',
  'sessionDurationMinutes',
  'patientComplianceLevel',
  'homeExerciseProgramStatus',
  'assistiveDeviceRecommendations',
  'dischargeReadinessIndicators',
  'adverseEventsDocumentation',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('therapy_progress_notes', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[TherapyProgressNotes] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('therapy_progress_notes', { _id: objectId }, {
      $set: {
    ...
    console.error('[TherapyProgressNotes] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
