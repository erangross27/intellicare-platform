...
/**
 * HospitalAdmissionNotesDocumentPDFTemplate - box-free canonical (LETTER)
 ...
 * Record date is admissionDate — NEVER createdAt/updatedAt.
 */
...
const SECTION_ORDER = ['admission-info', 'diagnosis-info', 'clinical-details', 'orders-plan', 'recommendations'];
...
const HospitalAdmissionNotesDocumentPDFTemplate = ({ document: docProp, data = docProp }) => {
  let records = [];
  if (Array.isArray(data)) {
    if (data.length === 1 && data[0]?.hospital_admission_notes) records = Array.isArray(data[0].hospital_admission_notes) ? data[0].hospital_admission_notes : [data[0].hospital_admission_notes];
    else records = data;
  } else if (data?.hospital_admission_notes) records = Array.isArray(data.hospital_admission_notes) ? data.hospital_admission_notes : [data.hospital_admission_notes];
    ...
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <Text style={styles.documentTitle}>Hospital Admission Notes</Text>
          <Text style={styles.noData}>No hospital admission notes records available.</Text>
        </Page>
      </Document>
    );
  }
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, rIdx) => (
          <View key={rIdx}>
            <Text style={styles.recordTitle} break={rIdx > 0}>{safeString(`Hospital Admission Notes ${rIdx + 1}`)}</Text>
...
export default HospitalAdmissionNotesDocumentPDFTemplate;
