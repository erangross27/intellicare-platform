/**
 * SocialDeterminantsOfHealthDocument.jsx
 * July 2026 — recursive, dot-path editable social determinants of health
...
const SECTION_TITLES = {
  ...
  financialBarriers: 'Financial Barriers',
  transportation: 'Transportation',
  insurance: 'Insurance',
  socialSupport: 'Social Support',
  healthLiteracy: 'Health Literacy',
  substanceUse: 'Substance Use Barriers',
  legalBarriers: 'Legal Barriers',
...
const FIELD_LABELS = {
  assessmentDate: 'Assessment Date', overallRiskAssessment: 'Overall Risk Assessment', housingStatus: 'Housing', foodSecurity: 'Food Security', financialBarriers: 'Financial Barriers', transportation: 'Transportation', insurance: 'Insurance', socialSupport: 'Social Support', healthLiteracy: 'Health Literacy', substanceUseBarriers: 'Substance Use Barriers', legalBarriers: 'Legal Barriers', referralsMade: 'Referrals Made', dischargeBarriers: 'Discharge Barriers', utilities: 'Utilities', language: 'Language', employment: 'Employment', educationLevel: 'Education Level', interventions: 'Interventions',
};
const SECTION_FIELDS = {
  assessment: ['assessmentDate', 'overallRiskAssessment'],
  housing: ['housingStatus'],
  foodSecurity: ['foodSecurity'],
  financialBarriers: ['financialBarriers'],
  transportation: ['transportation'],
  insurance: ['insurance'],
  socialSupport: ['socialSupport'],
  healthLiteracy: ['healthLiteracy'],
  substanceUse: ['substanceUseBarriers'],
  legalBarriers: ['legalBarriers'],
  referrals: ['referralsMade'],
  dischargePlanning: ['dischargeBarriers'],
  background: ['utilities', 'language', 'employment', 'educationLevel'],
  interventions: ['interventions'],
};
const DATE_FIELDS = ['assessmentDate'];
const DATETIME_FIELDS = [];
const ENUM_FIELDS = ['overallRiskAssessment'];
const NUMBER_FIELDS = [];
const NUMBER_STRING_FIELDS = [];
const NUMBER_UNITS = {};
const OBJECT_FIELDS = ['housingStatus', 'foodSecurity', 'financialBarriers', 'transportation', 'insurance', 'socialSupport', 'healthLiteracy', 'substanceUseBarriers', 'legalBarriers', 'utilities', 'language', 'employment'];
const MIXED_OBJECT_ARRAY_FIELDS = ['referralsMade'];
const OBJECT_ITEM_LABELS = { referralsMade: 'Referral' };
const NARRATIVE_PATHS = ['foodSecurity.status', 'socialSupport.livingSituation', 'healthLiteracy.level', 'healthLiteracy.digitalLiteracy', 'substanceUseBarriers.treatmentEngagement', 'referralsMade[].status'];
const PARENTHETICAL_LABEL_FIELDS = [];
const PARENTHETICAL_SEMICOLON_FIELDS = ['healthLiteracy.digitalLiteracy'];
const COMMA_FIELDS = ['foodSecurity.status', 'socialSupport.livingSituation', 'healthLiteracy.level'];
const COMMA_ARRAY_SPLIT_FIELDS = ['dischargeBarriers'];
const ARRAY_FIELDS = ['dischargeBarriers', 'interventions'];
const SEMICOLON_FIELDS = ['substanceUseBarriers.treatmentEngagement', 'referralsMade[].status'];
...
const KEY_LABELS = { snapBenefits: 'SNAP Benefits', copayBurden: 'Copay Burden', primaryLanguage: 'Primary Language', preferredLanguage: 'Preferred Language', digitalLiteracy: 'Digital Literacy', activeSubstanceUse: 'Active Substance Use', missedAppointments: 'Missed Appointments', hasReliableTransportation: 'Has Reliable Transportation', medicationCostRationing: 'Medication Cost Rationing' };
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'overallRiskAssessment' ? ['Low risk', 'Moderate risk', 'High risk', 'Very high risk'] : [];
...
const SocialDeterminantsOfHealthDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.social_determinants_of_health) return Array.isArray(record.social_determinants_of_health) ? record.social_determinants_of_health : [record.social_determinants_of_health];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.social_determinants_of_health) return Array.isArray(nested.social_determinants_of_health) ? nested.social_determinants_of_health : [nested.social_determinants_of_health];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/social_determinants_of_health/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/social_determinants_of_health/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SocialDeterminantsOfHealth] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Social Determinants of Health Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="social-determinants-of-health-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Social Determinants of Health</h2></div><div className="empty-state">No social determinants of health available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Social Determinants of Health Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="social-determinants-of-health-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Social Determinants of Health</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<SocialDeterminantsOfHealthDocumentPDFTemplate document={pdfData} />} fileName="Social_Determinants_of_Health.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search social determinants of health..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Social Determinants of Health Record ' + (recordIndex + 1))}</h3></div>
