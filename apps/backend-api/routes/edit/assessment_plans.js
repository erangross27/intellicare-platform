/**
 * AssessmentPlans Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/assessment_plans/:id/edit    — Edit a single field
 * PUT /api/edit/assessment_plans/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'assessment-plans-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('assessment_plans', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[AssessmentPlans] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('assessment_plans', { _id: objectId }, {
      $set: {
    ...
    console.error('[AssessmentPlans] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
