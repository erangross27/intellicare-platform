/**
 * LifestyleRiskAssessmentDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: lifestyle_risk_assessment
 *
 * 8 Sections:
 *   1. session-info: date (date picker)
 *   2. tobacco: smokingStatus (string), cigarettesPerDay (num), smokingPackYears (num)
 *   3. alcohol: alcoholConsumptionFrequency (string), alcoholDrinksPerWeek (num),
 *              bingeeDrinkingFrequency (string)
 *   4. physical-activity: physicalActivityLevel (string), moderateExerciseMinutesPerWeek (num),
 *                         vigorousExerciseMinutesPerWeek (num), strengthTrainingDaysPerWeek (num),
 *                         sedentaryHoursPerDay (num), occupationalPhysicalDemand (string),
 *                         screenTimeHoursPerDay (num)
 *   5. body-metrics: bodyMassIndex (num), waistCircumference (num)
 *   6. diet: dietaryPattern (string), dailyFruitServings (num), dailyVegetableServings (num),
 *            processedFoodFrequency (string)
 *   7. sleep-stress: sleepHoursPerNight (num), sleepQualityRating (string), stressLevel (string),
 *                    substanceUseHistory (array)
 *   8. risk-scores: cardiovascularRiskScore (num), diabetesRiskScore (num)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import LifestyleRiskAssessmentDocumentPDFTemplate from '../pdf-templates/LifestyleRiskAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './LifestyleRiskAssessmentDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'lifestyle_risk_assessmentPendingEdits';
const readDrafts = () => {
...
/* 15 number fields — ALL use input[type=number step=any] + parseFloat+isNaN->block save */
const NUMBER_FIELDS = [
  'cigarettesPerDay', 'smokingPackYears',
  'alcoholDrinksPerWeek',
  'moderateExerciseMinutesPerWeek', 'vigorousExerciseMinutesPerWeek', 'strengthTrainingDaysPerWeek',
  'sedentaryHoursPerDay', 'screenTimeHoursPerDay',
  'bodyMassIndex', 'waistCircumference',
  'dailyFruitServings', 'dailyVegetableServings',
  'sleepHoursPerNight',
...
const STRING_FIELDS = [
  'smokingStatus', 'alcoholConsumptionFrequency', 'bingeeDrinkingFrequency',
  'physicalActivityLevel', 'occupationalPhysicalDemand',
  'dietaryPattern', 'processedFoodFrequency',
  'sleepQualityRating', 'stressLevel',
];
...
/* splitByComma: parenthesis-aware comma split (thousands guard: only split at ", " boundaries) */
...
/* ======= COMPONENT ======= */
const LifestyleRiskAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.lifestyle_risk_assessment) return Array.isArray(r.lifestyle_risk_assessment) ? r.lifestyle_risk_assessment : [r.lifestyle_risk_assessment];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.lifestyle_risk_assessment) return Array.isArray(dd.lifestyle_risk_assessment) ? dd.lifestyle_risk_assessment : [dd.lifestyle_risk_assessment]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // Trailing numeric dot-segment ⇒ array element; else scalar field.
        ...
        if (isArrayElem) {
          ...
          const arrKey = `${baseField}-${idx}`;
          const baseArr = Array.isArray(nLocal[arrKey]) ? nLocal[arrKey] : (Array.isArray(record[baseField]) ? [...record[baseField]] : []);
          baseArr[arrayIndex] = value;
          nLocal[arrKey] = baseArr;
          nPending[arrKey] = true;
          nFields[`${baseField}.${arrayIndex}-${idx}`] = 'edited';
        } else {
  ...
  // hide-zero: a numeric metric of 0 (never smoked, 0 servings, 0 risk) is treated as "not recorded" and skipped.
  const fieldHasVal = useCallback((f, v) => { if (!hasVal(v)) return false; if (NUMBER_FIELDS.includes(f) && Number(v) === 0) return false; return true; }, [hasVal]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Lifestyle Risk Assessment ${idx + 1}`.toLowerCase();
  ...
  const pdfData = useMemo(() => {
    return filteredRecords.map((record, idx) => {
      ...
      // Array fields: only merge the staged array when it is NOT a pending draft.
      ARRAY_FIELDS.forEach(f => { if (!pendingEdits[`${f}-${idx}`]) merged[f] = getEffectiveArray(record, f, idx); });
      return merged;
    });
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stage(fullText, n => { n[`${fn}-${idx}-s${sentenceIdx}`] = 'edited'; return n; });
      return;
    }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Persist each staged draft field for this record that belongs to this section (field, or field+arrayIndex).
      const store = readDrafts();
      ...
      for (const fieldPart of Object.keys(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/lifestyle_risk_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedFieldParts.push(fieldPart);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/lifestyle_risk_assessment/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF (localEdits keys are "field-idx").
      setPendingEdits(prev => { const n = { ...prev }; fields.forEach(f => { delete n[`${f}-${idx}`]; }); return n; });
      // Drop this section's committed drafts from localStorage; remove record entry if now empty.
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== LIFESTYLE RISK ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Lifestyle Risk Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ======= RENDER: NUMBER FIELD — num-stepper (−/+ + text/decimal input) + parseFloat+isNaN->block save ======= */
  ...
  /* ======= RENDER: ARRAY FIELD (substanceUseHistory) ======= */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="lifestyle-risk-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Lifestyle Risk Assessment</h2></div>
        <div className="empty-state">No lifestyle risk assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="lifestyle-risk-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Lifestyle Risk Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<LifestyleRiskAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Lifestyle_Risk_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search lifestyle risk assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Lifestyle Risk Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'tobacco')}
            {renderSection(record, idx, 'alcohol')}
            {renderSection(record, idx, 'physical-activity')}
            {renderSection(record, idx, 'body-metrics')}
            {renderSection(record, idx, 'diet')}
            {renderSection(record, idx, 'sleep-stress')}
...
export default LifestyleRiskAssessmentDocument;
