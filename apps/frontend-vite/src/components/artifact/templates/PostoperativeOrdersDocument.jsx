/**
 * PostoperativeOrdersDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: postoperative_orders
 *
 * 7 Sections:
 *   1. general-info: date, provider, facility, type, status
 *   2. diet-activity: diet, activity
 *   3. medications: painManagement, antibiotics, prophylaxis
 *   4. monitoring-instructions: monitoring, specialInstructions
 *   5. discharge: dischargeHome
 *   6. clinical-notes: findings, assessment, plan, notes
 *   7. recommendations-results: recommendations
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PostoperativeOrdersDocumentPDFTemplate from '../pdf-templates/PostoperativeOrdersDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './PostoperativeOrdersDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" — whole-field value) */
const DRAFT_KEY = 'postoperative_ordersPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  'general-info': ['date', 'provider', 'facility', 'type', 'status'],
  'diet-activity': ['diet', 'activity'],
  'medications': ['painManagement', 'antibiotics', 'prophylaxis'],
  'monitoring-instructions': ['monitoring', 'specialInstructions'],
...
const ARRAY_FIELDS = ['painManagement', 'antibiotics', 'prophylaxis', 'monitoring', 'specialInstructions', 'recommendations'];
const OBJECT_FIELDS = ['results'];
const STRING_FIELDS = ['provider', 'facility', 'type', 'status', 'diet', 'activity', 'dischargeHome', 'findings', 'assessment', 'plan', 'notes'];
...
/* STRING fields whose unlabeled >=3-item comma value should split into value-only rows (aggressive-split pref);
   scoped so provider/facility credential+name strings are NOT shattered. */
...
/* ═══════ COMPONENT ═══════ */
const PostoperativeOrdersDocument = ({ document: docProp }) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (OBJECT_FIELDS.includes(f)) {
          ...
          if (val.some(item => {
            if (typeof item === 'object' && item !== null) return JSON.stringify(item).toLowerCase().includes(phrase);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Postoperative Orders ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (val && (OBJECT_FIELDS.includes(f) ? flattenSearchable(val).toLowerCase().includes(phrase) : Array.isArray(val) ? val.some(item => {
            if (typeof item === 'object' && item !== null) return JSON.stringify(item).toLowerCase().includes(phrase);
            return String(item).toLowerCase().includes(phrase);
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* sectionIdForField — find which section a field belongs to (to clear its approved flag on re-edit) */
  ...
  /* Stage a DRAFT locally + persist to the pending-drafts localStorage store (survives refresh).
     localEdits always holds the WHOLE field value under "fn-idx". NOT written to MongoDB and NOT
     shown in the PDF until Approve commits it. Re-editing after approval re-arms the Pending button. */
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    setSaveError(null);
    // Stage a DRAFT only — DB write is deferred to Approve.
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.cd4) */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    // Build the whole updated object and stage it as a DRAFT (no DB write — deferred to Approve).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // localEdits keys are "field-idx" holding the WHOLE field value; commit each pending one in this section.
      const toCommit = fields
        .map(f => `${f}-${idx}`)
        .filter(k => pendingEdits[k] && localEdits[k] !== undefined);
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -(`-${idx}`).length); // "field" (no numeric dot-suffix → no arrayIndex)
        ...
        const resp = await secureApiClient.put(`/api/edit/postoperative_orders/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/postoperative_orders/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        text += `${label}\n${items.map((item, i) => {
          if (typeof item === 'object' && item !== null) {
            ...
            return `${i + 1}. ${item.date ? formatDate(item.date) + ' - ' : ''}${recText}`;
          }
          return `${i + 1}. ${item}`;
        }).join('\n')}\n\n`;
        ...
        const unlabeledList = sentences.length === 1 && !parsedWhole.isLabeled && COMMA_SPLIT_FIELDS.includes(f) && splitByComma(strVal).length >= 3;
        if (sentences.length > 1) {
          text += `${label}\n`;
          formatSentenceFieldLines(strVal).forEach(l => { text += `${l}\n`; });
          text += '\n';
        } else if (unlabeledList) {
          text += `${label}\n`;
          splitByComma(strVal).forEach((p, i) => { text += `${i + 1}. ${p}\n`; });
          text += '\n';
        } else {
          text += `${label}\n${strVal}\n\n`;
        }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== POSTOPERATIVE ORDERS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Postoperative Orders ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderArrayField = (record, fn, idx, sid) => {
    ...
    /* Special handling for recommendations (array of objects) */
      ...
      return (
        <div key={fn} className="rec-mini-card">
          ...
          {items.map((item, itemIdx) => {
            ...
            const itemStr = isObj ? (item.recommendation || item.text || JSON.stringify(item)) : String(item);
            ...
            if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
              ...
              const fullItemStr = isObj ? JSON.stringify(item).toLowerCase() : itemStr.toLowerCase();
              if (!labelLower.includes(phrase) && !phrase.includes(labelLower) && !fullItemStr.includes(phrase)) return null;
            }
                ...
                <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                  {isEditing ? (
                    <div className="edit-field-container">
                      ...
                      {saveError && <div className="save-error">{saveError}</div>}
                      <div className="edit-actions">
                        <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const newVal = isObj ? { ...item, recommendation: editValue } : editValue; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = newVal; stageDraft(record, fn, idx, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                        <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                      </div>
                    </div>
                  ) : (
                    <>
                      <div className="row-content">
                        <span className="content-value">
                          {isObj && item.date && <span className="rec-date-top">{highlightText(formatDate(item.date))}{' - '}</span>}
                          {highlightText(itemStr)}
                        </span>
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          return (
            <div key={itemIdx} className={parsed.isLabeled ? 'rec-mini-card' : ''} style={parsed.isLabeled ? { marginTop: 8 } : undefined}>
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayVal); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = parsed.isLabeled ? `${parsed.label}: ${editValue.trim()}` : editValue; stageDraft(record, fn, idx, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderStringField = (record, fn, idx, sid, title) => {
    ...
    /* Unlabeled >=3-item comma list (scoped fields, e.g. activity) → value-only editable rows (aggressive-split) */
    const commaParts = splitByComma(strVal);
    if (COMMA_SPLIT_FIELDS.includes(fn) && !parseLabel(strVal).isLabeled && commaParts.length >= 3) {
      return (
        <div key={fn} className="rec-mini-card">
          <div className="nested-subtitle">{highlightText(label)}</div>
          {commaParts.map((part, pIdx) => {
            const partKey = `${fn}-${idx}-c${pIdx}`;
            const isEditingP = editingField === partKey;
            const badge = editedSentences[partKey];
            if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid) && !part.toLowerCase().includes(searchTerm.toLowerCase().trim())) return null;
            return (
              <div key={pIdx}>
                <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditingP) { setEditingField(partKey); setEditValue(part); setSaveError(null); } }}>
                  {isEditingP ? (
                    <div className="edit-field-container">
                      <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                      {saveError && <div className="save-error">{saveError}</div>}
                      <div className="edit-actions">
                        <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const cur = String(getFieldValue(record, fn, idx) || ''); const parts2 = splitByComma(cur); parts2[pIdx] = editValue.trim(); const rejoined = parts2.filter(p => p !== '').join(', '); setSaveError(null); stageDraft(record, fn, idx, rejoined); setEditedSentences(prev => ({ ...prev, [partKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                        <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                      </div>
                    </div>
                  ) : (
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; number+unit → number input, boolean → Yes/No, text → textarea) ═══════ */
              ...
              {isBool ? (
                ...
                <div className="number-edit-row">
                  <div className="num-stepper-row">
                    <button type="button" className="num-step" onClick={() => { const cur = parseFloat(editValue) || 0; const nv = Math.max(0, cur - stepFor(editValue)); setEditValue(String(Number(nv.toFixed(6)))); }}>−</button>
                    ...
                    <button type="button" className="num-step" onClick={() => { const cur = parseFloat(editValue) || 0; const nv = cur + stepFor(editValue); setEditValue(String(Number(nv.toFixed(6)))); }}>+</button>
                  </div>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="postoperative-orders-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Postoperative Orders</h2></div>
        <div className="empty-state">No postoperative orders records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="postoperative-orders-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Postoperative Orders</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PostoperativeOrdersDocumentPDFTemplate document={pdfData} />} fileName="Postoperative_Orders.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search postoperative orders..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Postoperative Orders ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'general-info')}
            {renderSection(record, idx, 'diet-activity')}
            {renderSection(record, idx, 'medications')}
            {renderSection(record, idx, 'monitoring-instructions')}
...
export default PostoperativeOrdersDocument;
