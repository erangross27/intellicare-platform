...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'pain-medication-agreements-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'witnessName',
  'primaryOpioidMedication',
  'opioidDosageAmount',
  'morphineEquivalentDose',
  'primaryPainDiagnosis',
  'additionalPrescribedMedications',
  'urineScreeningFrequency',
  'lastUrineScreenDate',
  'designatedPharmacy',
  'earlyRefillProhibition',
  'multipleProviderProhibition',
  'pdmpReviewAcknowledged',
  'safeMedicationStorageAcknowledged',
  'drivingImpairmentWarningAcknowledged',
  'pregnancyRiskAcknowledged',
  'naloxonePrescribed',
  'emergencyContactName',
  'emergencyContactPhone',
  'agreementViolationConsequences',
  'nonPharmacologicTherapiesDiscussed',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('pain_medication_agreements', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[pain_medication_agreements] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('pain_medication_agreements', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[pain_medication_agreements] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
