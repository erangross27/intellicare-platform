/**
 * PostoperativePainManagementDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: postoperative_pain_management
 *
 * 8 Sections:
 *   1. provider-details: date, surgeryDate
 *   2. surgery-info: surgeryPerformed, anesthesiaType
 *   3. pain-assessment: baselinePainScore, currentPainScore, painScaleUsed, painCharacteristics, painLocation
 *   4. medications: opioidMedications, nonOpioidAnalgesics, multimodalAnalgesiaProtocol
 *   5. regional-anesthesia: regionalAnesthesiaTechnique, epiduralCatheterPresent, pcaDeviceActive, pcaSettings
 *   6. monitoring: totalOpioidConsumption, sedationScore, respiratoryRate, oxygenSaturation, naloxoneAdministered, adverseEffects
 *   7. non-pharm: nonPharmacologicalInterventions
 *   8. recovery-plan: mobilizationStatus, painManagementGoal, transitionPlan
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PostoperativePainManagementDocumentPDFTemplate from '../pdf-templates/PostoperativePainManagementDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './PostoperativePainManagementDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'postoperative_pain_managementPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'non-pharm': 'Non-Pharmacological Interventions',
...
const FIELD_LABELS = {
  ...
  surgeryPerformed: 'Surgery Performed',
  anesthesiaType: 'Anesthesia Type',
  baselinePainScore: 'Baseline Pain Score',
  currentPainScore: 'Current Pain Score',
  painScaleUsed: 'Pain Scale Used',
  painCharacteristics: 'Pain Characteristics',
  painLocation: 'Pain Location',
  opioidMedications: 'Opioid Medications',
  nonOpioidAnalgesics: 'Non-Opioid Analgesics',
  multimodalAnalgesiaProtocol: 'Multimodal Analgesia Protocol',
  regionalAnesthesiaTechnique: 'Regional Anesthesia Technique',
  epiduralCatheterPresent: 'Epidural Catheter Present',
  pcaDeviceActive: 'PCA Device Active',
  pcaSettings: 'PCA Settings',
  totalOpioidConsumption: 'Total Opioid Consumption',
  sedationScore: 'Sedation Score',
  respiratoryRate: 'Respiratory Rate',
  oxygenSaturation: 'Oxygen Saturation',
  naloxoneAdministered: 'Naloxone Administered',
  ...
  painManagementGoal: 'Pain Management Goal',
  transitionPlan: 'Transition Plan',
};

const SECTION_FIELDS = {
  'provider-details': ['date', 'surgeryDate'],
  'surgery-info': ['surgeryPerformed', 'anesthesiaType'],
  'pain-assessment': ['baselinePainScore', 'currentPainScore', 'painScaleUsed', 'painCharacteristics', 'painLocation'],
  'medications': ['opioidMedications', 'nonOpioidAnalgesics', 'multimodalAnalgesiaProtocol'],
  'regional-anesthesia': ['regionalAnesthesiaTechnique', 'epiduralCatheterPresent', 'pcaDeviceActive', 'pcaSettings'],
  'monitoring': ['totalOpioidConsumption', 'sedationScore', 'respiratoryRate', 'oxygenSaturation', 'naloxoneAdministered', 'adverseEffects'],
  'non-pharm': ['nonPharmacologicalInterventions'],
  'recovery-plan': ['mobilizationStatus', 'painManagementGoal', 'transitionPlan'],
};

const BOOLEAN_FIELDS = ['epiduralCatheterPresent', 'pcaDeviceActive', 'naloxoneAdministered'];
const DATE_FIELDS = ['date', 'surgeryDate'];
const NUMBER_FIELDS = ['baselinePainScore', 'currentPainScore', 'totalOpioidConsumption', 'sedationScore', 'respiratoryRate', 'oxygenSaturation'];
const ARRAY_FIELDS = ['painLocation', 'opioidMedications', 'nonOpioidAnalgesics', 'adverseEffects', 'nonPharmacologicalInterventions'];
const STRING_FIELDS = ['surgeryPerformed', 'anesthesiaType', 'painScaleUsed', 'painCharacteristics', 'multimodalAnalgesiaProtocol', 'regionalAnesthesiaTechnique', 'pcaSettings', 'mobilizationStatus', 'painManagementGoal', 'transitionPlan'];
...
/* splitByComma: parenthesis-aware comma split (thousands + year guard: only split on comma+space that is not "1,000"/",1999") */
...
/* Sentinel-zero: extractor defaults where 0 means "not recorded" (RR/SpO2 of 0 impossible; pain scores 0 with no
   scale, sedation/opioid-consumption 0 all unrecorded). Hide these when 0 in JSX + Copy + PDF + section visibility. */
...
/* ═══════ COMPONENT ═══════ */
const PostoperativePainManagementDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.postoperative_pain_management) return Array.isArray(r.postoperative_pain_management) ? r.postoperative_pain_management : [r.postoperative_pain_management];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.postoperative_pain_management) return Array.isArray(dd.postoperative_pain_management) ? dd.postoperative_pain_management : [dd.postoperative_pain_management]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      const recDrafts = idOf(record) ? store[idOf(record)] : null;
      if (!recDrafts) return;
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayItem) {
          ...
          const baseArr = Array.isArray(nLocal[`${fn}-${idx}`])
            ? nLocal[`${fn}-${idx}`]
          ...
          // Mark either as a simple field edit or a first-sentence edit so the modified badge + approve button show.
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Postoperative Pain Management Record ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ DRAFT STAGING ═══════
     ...
     fieldPart = "field" for scalar/string fields, "field.arrayIndex" for array items. */
  const stageDraft = useCallback((record, fn, idx, fieldPart, localValue, draftValue) => {
    ...
    store[id][fieldPart] = draftValue !== undefined ? draftValue : localValue;
    writeDrafts(store);
  }, [safeId]);
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageDraft(record, fn, idx, fn, fullText);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      // Only commit fieldParts whose base field belongs to this section.
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/postoperative_pain_management/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedParts.push(fieldPart);
        committedKeys.push(`${baseField}-${idx}`);
      }
      // Flag the record/section approved (audit trail)
      await secureApiClient.put(`/api/edit/postoperative_pain_management/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (committedParts.length > 0) {
        committedParts.forEach(p => delete recDrafts[p]);
      ...
      console.error('[PostoperativePainManagement] Approve error:', err);
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (NUMBER_FIELDS.includes(f) && isMeaninglessZero(f, val)) return;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== POSTOPERATIVE PAIN MANAGEMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Postoperative Pain Management Record ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; stageDraft(record, fn, idx, `${fn}.${itemIdx}`, currentArr, editValue); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { const k = `${sid}-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; }); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderStringField = (record, fn, idx, sid, title) => {
    ...
    /* A single sentence that is itself a labeled comma-list must NOT render whole (side-by-side trap) —
       route it through the multi-sentence renderer, which stacks label above comma-split value rows. */
    const parsedWhole = parseLabel(strVal);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="postoperative-pain-management-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Postoperative Pain Management</h2></div>
        <div className="empty-state">No postoperative pain management records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="postoperative-pain-management-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Postoperative Pain Management</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PostoperativePainManagementDocumentPDFTemplate document={pdfData} />} fileName="Postoperative_Pain_Management.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search postoperative pain management..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Postoperative Pain Management Record ${idx + 1}`)}</h3>
            </div>
            ...
            {renderSection(record, idx, 'regional-anesthesia')}
            {renderSection(record, idx, 'monitoring')}
            {renderSection(record, idx, 'non-pharm')}
            {renderSection(record, idx, 'recovery-plan')}
          </div>
        ))}
      </div>
    </div>
...
export default PostoperativePainManagementDocument;
