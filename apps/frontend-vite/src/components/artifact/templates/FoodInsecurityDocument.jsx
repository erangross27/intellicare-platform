/**
 * FoodInsecurityDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: food_insecurity
 *
 * 6 Sections:
 *   1. assessment-info: assessmentDate (date picker), programType (string)
 *   2. findings-section: findings (long string with parseLabel + splitBySentence/splitByComma)
 *   3. recommendations-section: recommendations (long string with parseLabel + splitBySentence/splitByComma)
 *   4. goals-section: goals (string)
 *   5. progress-section: progress (string)
 *   6. followup-section: followUp (string)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import FoodInsecurityDocumentPDFTemplate from '../pdf-templates/FoodInsecurityDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './FoodInsecurityDocument.css';
...
/* no number fields for food_insecurity */
const NUMBER_FIELDS = [];

const STRING_FIELDS = ['programType', 'findings', 'recommendations', 'goals', 'progress', 'followUp'];
...
const DRAFT_KEY = 'food_insecurityPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const FoodInsecurityDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.food_insecurity) return Array.isArray(r.food_insecurity) ? r.food_insecurity : [r.food_insecurity];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.food_insecurity) return Array.isArray(dd.food_insecurity) ? dd.food_insecurity : [dd.food_insecurity]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Food Insecurity Assessment ${idx + 1}`.toLowerCase();
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    // Stage the draft into localStorage (survives refresh; committed to DB on Approve)
    const store = readDrafts();
  ...
  // Stage one edited sentence as a DRAFT locally (no DB write). localStorage keeps it across refresh; Approve commits it.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/food_insecurity/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the record/section approved (audit trail)
      await secureApiClient.put(`/api/edit/food_insecurity/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = `Food Insecurity Assessments\n${COPY_LINE_EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Food Insecurity Assessment ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="food-insecurity-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Food Insecurity Assessments</h2></div>
        <div className="empty-state">No food insecurity assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="food-insecurity-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Food Insecurity Assessments</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FoodInsecurityDocumentPDFTemplate document={pdfData} />} fileName="Food_Insecurity.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search food insecurity assessments..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Food Insecurity Assessment ${idx + 1}`)}</h3>
            </div>
            ...
            {renderSection(record, idx, 'progress-section')}
            {renderSection(record, idx, 'followup-section')}
          </div>
        ))}
      </div>
    </div>
...
export default FoodInsecurityDocument;
