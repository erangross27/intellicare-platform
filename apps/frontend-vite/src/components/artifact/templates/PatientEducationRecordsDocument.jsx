/**
 * PatientEducationRecordsDocument.jsx
 * July 2026 — Canonical one-pass polish: inline editing, blue glow theme, box-free PDF
 * Collection: patient_education_records
 *
 * Sections:
 *   1. session-info: date (BlueDatePicker), provider, facility, status (enum), interpreter, educatorRole, duration
 *   2. education-overview: educationTopic (sentence), educationType, teachingMethod, educationContent (sentence),
 *                          materialsProvided[] (array), learningGoals[] (array)
 *   3. comprehension: comprehensionLevel, teachBackPerformed (bool), teachBackResults (sentence),
 *                     barriersToLearning[] (array), patientMotivation (sentence)
 *   4. follow-up: followUpPlanned (bool), followUpDetails (sentence), familyInvolved (bool), familyMembers[] (array)
 *   5. clinical-findings: findings (sentence), assessment (sentence), plan (sentence)
 *   6. results-section: results (recursive OBJECT)
 *   7. recommendations-section: recommendations[] (array of {recommendation, date}, date-grouped)
 *   8. additional-notes: notes (sentence)
 *
 * Booleans (teachBackPerformed, followUpPlanned, familyInvolved) render BOTH true ("Yes") and false ("No") —
 * false is NOT hidden; edited via BlueSelect Yes/No and saved as editValue === 'Yes'.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PatientEducationRecordsDocumentPDFTemplate from '../pdf-templates/PatientEducationRecordsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './PatientEducationRecordsDocument.css';
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   value may be a string, object, or array depending on the field). */
const DRAFT_KEY = 'patient_education_recordsPendingEdits';
const readDrafts = () => {
...
/* humanizeKey: prettify nested object keys for the recursive results renderer */
const KEY_OVERRIDES = {
  bp: 'BP', hr: 'HR', rr: 'RR', spo2: 'SpO2', ecg: 'ECG', ekg: 'EKG', ct: 'CT', mri: 'MRI', wbc: 'WBC', npo: 'NPO', iv: 'IV', ed: 'ED',
};
...
const SENTENCE_FIELDS = ['educationTopic', 'educationContent', 'teachBackResults', 'patientMotivation', 'followUpDetails', 'findings', 'assessment', 'plan', 'notes'];
...
/* parseLabel: detect "Label: value" patterns (skip subordinate-clause openers, strip leading list markers) */
...
/* ═══════ COMPONENT ═══════ */
const PatientEducationRecordsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.patient_education_records) return Array.isArray(r.patient_education_records) ? r.patient_education_records : [r.patient_education_records];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.patient_education_records) return Array.isArray(dd.patient_education_records) ? dd.patient_education_records : [dd.patient_education_records]; return [dd]; }
      return [r];
    });
  ...
  /* flattenSearchable: recursively collect leaf strings (keys + values) for object/array search */
  ...
  /* expandLeafPieces: split a narrative object-leaf value by SENTENCE then by COMMA (>=3-item lists only).
     Returns [{ text, sIdx, ci }] — ci = -1 when the sentence was NOT comma-split (kept whole). Used by the
     JSX object-leaf rows; the copy + PDF mirror the same expansion. */
  const expandLeafPieces = useCallback((text) => {
  ...
  /* getFieldValue: supports dot-notation for nested objects like dischargeInstructions.destination */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Patient Education Record ${idx + 1}`.toLowerCase();
  ...
  const pdfData = useMemo(() => {
    return filteredRecords.map((record, idx) => {
      const merged = { ...record };
      if (record.results && typeof record.results === 'object') merged.results = { ...record.results };
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* stageDraft: persist a single field's pending value to the localStorage draft store (survives refresh).
     fieldPart = the localEdits key MINUS its trailing "-idx" (e.g. "provider", "followUpRequired.2",
     "results", "dischargeInstructions.destination"). Value may be string/object/array. */
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.lab) — staged locally; the WHOLE root object
     is kept under "rootField-idx" so the merged value renders, and the draft stores the full object. */
  const saveLeaf = useCallback((record, rootField, path, idx, leafKeyTrack, newVal) => {
    ...
    if (clonedRoot !== undefined) stageDraft(record, rootField, clonedRoot);
    setEditingField(null); setEditValue(''); setSaveError(null);
  }, [safeId, stageDraft]);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Pending edit keys belonging to this section (fieldPart === f, or fieldPart starts with "f.").
    ...
    try {
      for (const editKey of toCommit) {
        ...
        if (lastDot !== -1 && /^\d+$/.test(trailing)) {
          // array element edit: field="parent", arrayIndex=N
          payload.field = fieldPart.slice(0, lastDot);
          payload.arrayIndex = parseInt(trailing, 10);
        } else {
          // plain or dotted field name (e.g. "dischargeInstructions.destination", "results")
          payload.field = fieldPart;
        }
        const resp = await secureApiClient.put(`/api/edit/patient_education_records/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/patient_education_records/${id}/approve`, { sectionId: sid, approved: true });
      ...
      console.error('[EdDisposition] Approve error:', err);
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        text += `${label}\n`;
        val.forEach((rec, i) => { const rt = (rec?.recommendation || '').trim(); const rd = (rec?.date || '').trim(); text += `${i + 1}. ${rt}${rd ? ` (${rd})` : ''}\n`; });
        text += '\n';
      } else if (OBJECT_FIELDS.includes(f) && val && typeof val === 'object') {
        // Single-name gate: hide the field label when it equals the section title.
        if (label.toLowerCase() !== (title || '').toLowerCase()) text += `${label}\n${'-'.repeat(40)}\n`;
        const walk = (obj) => {
          Object.entries(obj).forEach(([k, v]) => {
            if (isEmptyDeep(v)) return;
            text += `\n${humanizeKey(k)}\n${'-'.repeat(40)}\n`;
            if (isScalar(v)) expandLeafPieces(fmtScalar(v)).forEach((pc, i) => { text += `${i + 1}. ${pc.text}\n`; });
            else walk(v);
          });
        };
        walk(val);
    ...
    return text;
  }, [getFieldValue, hasVal, fmtVal, formatSentenceFieldLines, expandLeafPieces]);

  const copyAllText = useCallback(async () => {
    let text = '=== PATIENT EDUCATION RECORDS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Patient Education Record ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: BOOLEAN FIELD (BlueSelect Yes/No) — renders BOTH true and false (false → "No", never hidden) ═══════ */
    ...
    return (
      <div key={fn} className={sl ? 'rec-mini-card' : ''}>
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayVal); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              <BlueSelect value={editValue || displayVal} options={['Yes', 'No']} onChange={v => setEditValue(v)} />
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveField(record, fn, idx, sid, null, (editValue || displayVal) === 'Yes'); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
  ...
  /* ═══════ RENDER: ARRAY FIELD (followUpRequired) ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {arr.map((item, arrIdx) => {
          ...
          const localVal = localEdits[itemEditKey] !== undefined ? localEdits[itemEditKey] : item;
          const isEditing = editingField === itemEditKey;
          const badge = editedFields[itemEditKey];
          const itemStr = String(localVal);

          if (searchTerm.trim() && !phraseMatch && !itemStr.toLowerCase().includes(searchTerm.toLowerCase().trim())) return null;

          return (
            <div key={arrIdx}>
              <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemEditKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                    {saveError && <div className="save-error">{saveError}</div>}
                  ...
                  <>
                    ...
                    <button className={`copy-btn ${copiedItems[itemEditKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(itemStr, itemEditKey); }}>{copiedItems[itemEditKey] ? 'Copied!' : 'Copy'}</button>
                  </>
                )}
              </div>
                ...
                <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                  {isEditing ? (
                    <div className="edit-field-container">
                      ...
                      {saveError && <div className="save-error">{saveError}</div>}
                      <div className="edit-actions">
                        <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullText })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); if (sid) setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); stageDraft(record, fn, fullText); setEditingField(null); setEditValue(''); setSaveError(null); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                        <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                      </div>
                    </div>
                  ) : (
  ...
  const renderObjectLeaf = (record, rootField, path, idx, value) => {
    ...
    // NARRATIVE text leaf → split by sentence THEN by comma (>=3-item lists) into per-piece editable rows.
    // (bool / ratio / number+unit leaves stay a single-row widget below.)
    if (!isBool && !ratio && !nu) {
      const pieces = expandLeafPieces(leafValueString);
      const cur = leafValueString; // current (possibly-staged) leaf value for save reconstruction
      return (
        <div key={path[path.length - 1]} className="nested-mini-card">
          <div className="nested-subtitle sub-label">{highlightText(label)}</div>
          {pieces.map((pc, pIdx) => {
            const pieceKey = `${leafKey}-s${pc.sIdx}-c${pc.ci}`;
            const isEditing = editingField === pieceKey;
            const pMod = editedSentences[pieceKey];
            const saveEdited = () => {
              const edited = editValue.trim();
              const sentences2 = splitBySentence(cur);
              if (pc.ci === -1) {
                if (!edited) sentences2.splice(pc.sIdx, 1); else sentences2[pc.sIdx] = edited;
              } else {
                const items2 = splitByComma(sentences2[pc.sIdx] || '');
                if (!edited) items2.splice(pc.ci, 1); else items2[pc.ci] = edited;
                sentences2[pc.sIdx] = items2.join(', ');
              }
              const full = reconstructFullText(sentences2.filter(s => s && s.trim()));
              setEditedSentences(prev => ({ ...prev, [pieceKey]: 'edited' }));
              saveLeaf(record, rootField, path, idx, leafKey, full);
            };
            return (
              <div key={pIdx}>
    ...
    const editStartValue = isBool ? (value ? 'Yes' : 'No') : ratio ? ratio.num : nu.num;
    return (
      <div key={path[path.length - 1]} className="nested-mini-card">
        <div className="nested-subtitle sub-label">{highlightText(label)}</div>
            ...
            <div className="edit-field-container">
              ...
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => {
                  ...
                  else { const n = parseFloat(editValue); if (isNaN(n)) { setSaveError('Please enter a valid number'); return; } newVal = nu.unit ? `${n}${nu.sep || ' '}${nu.unit}` : String(n); }
                  saveLeaf(record, rootField, path, idx, leafKey, newVal);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="patient-education-records-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Patient Education Records</h2></div>
        <div className="empty-state">No patient education records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="patient-education-records-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Patient Education Records</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PatientEducationRecordsDocumentPDFTemplate document={pdfData} />} fileName="Patient_Education_Records.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search patient education records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Patient Education Record ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'education-overview')}
...
export default PatientEducationRecordsDocument;