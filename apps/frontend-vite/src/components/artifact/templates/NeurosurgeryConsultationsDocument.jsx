/**
 * NeurosurgeryConsultationsDocument.jsx
 * March 2026 — Canonical one-pass polish, inline editing, blue glow theme
 * Collection: neurosurgery_consultations
 *
 * Record date rule: the record has ONLY createdAt/updatedAt (ingestion timestamps) and a separate
 * real `consultDate` field — there is NO `date` field, so NO record date is shown in the title header
 * (title only). consultDate renders as a labeled FIELD row (via BlueDatePicker) in Clinical Information.
 *
 * 6 Sections:
 *   1. clinical-info: consultDate (BlueDatePicker), referralReason, chiefComplaint, symptomOnset,
 *                     symptomDuration, symptomProgression, priorTreatments
 *   2. examination-findings: examFindings, neurologicExam, imagingReview
 *   3. diagnosis: diagnosis
 *   4. assessment-plan: treatmentPlan, surgicalIndication, surgicalPlan, riskDiscussion,
 *                       patientConsent, alternativeTreatments
 *   5. follow-up: followUpPlan
 *   6. recommendations: recommendations — array of {recommendation, date}; each object decomposes into a
 *                       nested-subtitle (recommendation text) + a formatted date row (BlueDatePicker)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NeurosurgeryConsultationsDocumentPDFTemplate from '../pdf-templates/NeurosurgeryConsultationsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './NeurosurgeryConsultationsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'neurosurgery_consultationsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = [
  'referralReason', 'chiefComplaint', 'symptomOnset', 'symptomDuration', 'symptomProgression', 'priorTreatments',
  'examFindings', 'neurologicExam', 'imagingReview',
  'diagnosis',
  'treatmentPlan', 'surgicalIndication', 'surgicalPlan', 'riskDiscussion', 'patientConsent', 'alternativeTreatments',
  'followUpPlan',
];
...
/* ═══════ COMPONENT ═══════ */
const NeurosurgeryConsultationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.neurosurgery_consultations) return Array.isArray(r.neurosurgery_consultations) ? r.neurosurgery_consultations : [r.neurosurgery_consultations];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.neurosurgery_consultations) return Array.isArray(dd.neurosurgery_consultations) ? dd.neurosurgery_consultations : [dd.neurosurgery_consultations]; return [dd]; }
      if (r?.document?.neurosurgery_consultations) return Array.isArray(r.document.neurosurgery_consultations) ? r.document.neurosurgery_consultations : [r.document.neurosurgery_consultations];
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Neurosurgery Consultation ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/neurosurgery_consultations/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/neurosurgery_consultations/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (OBJECT_ARRAY_FIELDS.includes(f)) {
        if (Array.isArray(val) && val.length > 0) {
          ...
          val.forEach((rec, i) => {
            ...
            if (!rt) return;
            text += `${showLabel ? '  ' : ''}${i + 1}. ${rt}${rd ? ` (${formatDate(rd)})` : ''}\n`;
          });
          text += '\n';
        }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NEUROSURGERY CONSULTATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Neurosurgery Consultation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="neurosurgery-consultations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Neurosurgery Consultations</h2></div>
        <div className="empty-state">No neurosurgery consultations records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="neurosurgery-consultations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Neurosurgery Consultations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NeurosurgeryConsultationsDocumentPDFTemplate document={pdfData} />} fileName="Neurosurgery_Consultations.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search neurosurgery consultations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Neurosurgery Consultation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'clinical-info')}
            {renderSection(record, idx, 'examination-findings')}
...
export default NeurosurgeryConsultationsDocument;
