...
const SECTIONS = [
  { id: 'presentingConcern', title: 'Presenting Concern', fields: ['chiefComplaint', 'jointPainDuration', 'morningStiffnessDuration', 'functionalStatus'] },
  { id: 'jointExamination', title: 'Joint Examination', fields: ['affectedJoints', 'swollenJointCount', 'tenderJointCount'] },
  { id: 'diseaseActivity', title: 'Disease Activity', fields: ['das28Score', 'cdaiScore', 'patientGlobalAssessment'] },
  { id: 'autoimmuneTesting', title: 'Autoimmune Testing', fields: ['rheumatoidFactor', 'antiCcpAntibodies', 'anaPattern', 'anaTiter', 'hlaB27Status', 'complementC3Level', 'complementC4Level'] },
  { id: 'inflammatoryMarkers', title: 'Inflammatory Markers', fields: ['crpLevel', 'esrValue', 'uricAcidLevel'] },
  { id: 'synovialAssessment', title: 'Synovial Assessment', fields: ['synovialFluidAnalysis', 'crystalType'] },
  { id: 'currentTreatment', title: 'Current Treatment', fields: ['currentDmards', 'biologicTherapy', 'corticosteroidUse'] },
];
const FIELD_LABELS = {
...
export default function RheumatologyConsultationsDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Rheumatology Consultations</Text></View>
    {!records.length && <Text style={styles.noDataText}>No rheumatology consultations data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Rheumatology Consultations {recordIndex + 1}</Text></View>
