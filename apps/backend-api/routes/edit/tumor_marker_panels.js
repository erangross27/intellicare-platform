...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'tumor-marker-panels-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['panelIndication', 'specimenType', 'collectionDateTime', 'clinicalContext', 'afpLevel', 'ceaLevel', 'ca125Level', 'ca199Level', 'ca153Level', 'psaTotal', 'psaFree', 'betaHcgLevel', 'ldhLevel', 'calcitoninLevel', 'thyroglobulinLevel', 'chromograninALevel', 'neuronSpecificEnolase', 'squamousCellCarcinomaAntigen', 'cyfra211Level', 'ca242Level', 'her2NeuLevel', 'abnormalMarkers', 'previousMarkerComparison'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('tumor_marker_panels', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[tumor_marker_panels] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('tumor_marker_panels', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[tumor_marker_panels] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
