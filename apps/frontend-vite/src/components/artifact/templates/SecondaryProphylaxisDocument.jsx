 ...
 * July 2026 — recursive, dot-path editable secondary prophylaxis
...
const DRAFT_KEY = 'secondary_prophylaxisPendingEdits';
const SECTION_TITLES = {
  prophylaxisInfo: 'Prophylaxis Information',
...
const FIELD_LABELS = {
  infection: 'Infection', medication: 'Medication', discontinuationCriteria: 'Discontinuation Criteria', threshold: 'Threshold', type: 'Type', date: 'Date', provider: 'Provider', facility: 'Facility', findings: 'Findings', assessment: 'Assessment', plan: 'Plan', results: 'Results', recommendations: 'Recommendations', notes: 'Notes', status: 'Status',
};
const SECTION_FIELDS = {
  prophylaxisInfo: ['infection', 'medication', 'discontinuationCriteria', 'threshold', 'type'],
  providerDate: ['date', 'provider', 'facility'],
...
const ARRAY_FIELDS = [];
const SEMICOLON_FIELDS = ['discontinuationCriteria', 'findings', 'assessment', 'plan', 'notes'];
...
const KEY_LABELS = { cd4Count: 'CD4 Count', viralLoad: 'Viral Load', eligibleForDiscontinuation: 'Eligible for Discontinuation', missedDoses: 'Missed Doses' };
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'status' ? ['Planned', 'Active', 'Completed', 'Discontinued'] : [];
...
const SecondaryProphylaxisDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.secondary_prophylaxis) return Array.isArray(record.secondary_prophylaxis) ? record.secondary_prophylaxis : [record.secondary_prophylaxis];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.secondary_prophylaxis) return Array.isArray(nested.secondary_prophylaxis) ? nested.secondary_prophylaxis : [nested.secondary_prophylaxis];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/secondary_prophylaxis/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/secondary_prophylaxis/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SecondaryProphylaxis] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Secondary Prophylaxis ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="secondary-prophylaxis-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Secondary Prophylaxis</h2></div><div className="empty-state">No secondary prophylaxis records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Secondary Prophylaxis ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="secondary-prophylaxis-document" ref={containerRef}>
    <div className="document-header">
      ...
      <div className="header-actions">
        ...
        <PDFDownloadLink document={<SecondaryProphylaxisDocumentPDFTemplate document={pdfData} />} fileName="Secondary_Prophylaxis.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search secondary prophylaxis..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Secondary Prophylaxis ' + (recordIndex + 1))}</h3></div>
