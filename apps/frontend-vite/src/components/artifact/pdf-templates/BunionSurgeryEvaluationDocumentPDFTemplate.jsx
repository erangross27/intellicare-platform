...
const COLLECTION = 'bunion_surgery_evaluation';
const COMMA_SPLIT_FIELDS = ['firstMtpRangeOfMotion', 'medialEminenceSize', 'recommendedSurgicalProcedure', 'peripheralVascularStatus'];
const ARRAY_FIELDS = new Set([]);
const OBJECT_FIELDS = new Set([]);
const NARRATIVE_FIELDS = new Set(['firstMtpRangeOfMotion', 'medialEminenceSize', 'recommendedSurgicalProcedure', 'peripheralVascularStatus']);
const DATE_FIELDS = new Set([]);
const ZERO_SENTINEL_FIELDS = new Set(['halluximeterMeasurement', 'manchesterOxfordFootQuestionnaire', 'aofasHalluxScore', 'metatarsusAdductusAngle', 'mearyAngle', 'calcanealpitchAngle', 'halluxInterphalangealAngle', 'boneMineralDensityTScore']);
const KEY_OVERRIDES = {};
const LABELS = {
  halluxValgusAngle: 'Hallux Valgus Angle', intermetatarsalAngle: 'Intermetatarsal Angle', distalMetatarsalArticularAngle: 'Distal Metatarsal Articular Angle',
  sesamoidSubluxationGrade: 'Sesamoid Subluxation Grade', firstMtpJointCongruency: 'First MTP Joint Congruency',
  firstMtpRangeOfMotion: 'First MTP Range of Motion', halluximeterMeasurement: 'Halluximeter Measurement',
  manchesterOxfordFootQuestionnaire: 'Manchester-Oxford Foot Questionnaire', aofasHalluxScore: 'AOFAS Hallux Score',
  firstMtpJointDegenerativeChanges: 'First MTP Joint Degenerative Changes', metatarsusAdductusAngle: 'Metatarsus Adductus Angle',
  firstMetatarsalLength: 'First Metatarsal Length', firstTmtJointHypermobility: 'First TMT Joint Hypermobility',
  mearyAngle: 'Meary Angle', calcanealpitchAngle: 'Calcaneal Pitch Angle', medialEminenceSize: 'Medial Eminence Size',
  halluxInterphalangealAngle: 'Hallux Interphalangeal Angle', bursitisOverMedialEminence: 'Bursitis Over Medial Eminence',
  recommendedSurgicalProcedure: 'Recommended Surgical Procedure', peripheralVascularStatus: 'Peripheral Vascular Status',
  diabeticFootRiskCategory: 'Diabetic Foot Risk Category', boneMineralDensityTScore: 'Bone Mineral Density T-Score', nicotineUseStatus: 'Nicotine Use Status',
};
const SECTIONS = [
  { title: 'Radiographic Angles', fields: ['halluxValgusAngle', 'intermetatarsalAngle', 'distalMetatarsalArticularAngle', 'metatarsusAdductusAngle', 'halluxInterphalangealAngle', 'mearyAngle', 'calcanealpitchAngle'] },
  { title: 'First MTP Joint', fields: ['firstMtpJointCongruency', 'firstMtpRangeOfMotion', 'firstMtpJointDegenerativeChanges', 'firstMetatarsalLength', 'firstTmtJointHypermobility'] },
  { title: 'Sesamoid and Eminence', fields: ['sesamoidSubluxationGrade', 'medialEminenceSize', 'bursitisOverMedialEminence'] },
  { title: 'Outcome Scores', fields: ['halluximeterMeasurement', 'manchesterOxfordFootQuestionnaire', 'aofasHalluxScore', 'boneMineralDensityTScore'] },
  { title: 'Recommended Surgical Procedure', fields: ['recommendedSurgicalProcedure'] },
  { title: 'Peripheral Vascular Status', fields: ['peripheralVascularStatus'] },
  { title: 'Risk Factors', fields: ['diabeticFootRiskCategory', 'nicotineUseStatus'] },
];
...
const BunionSurgeryEvaluationDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Bunion Surgery Evaluation</Text><Text style={styles.noData}>No bunion surgery evaluation data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Bunion Surgery Evaluation</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Bunion Surgery Evaluation {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
