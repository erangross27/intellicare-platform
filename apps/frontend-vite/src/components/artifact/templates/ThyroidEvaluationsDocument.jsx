/**
 * ThyroidEvaluationsDocument.jsx
 * March 2026 -- Complete rewrite with inline editing, blue glow theme
 * Collection: thyroid_evaluations
 *
 * 7 Sections:
 *   1. thyroid-labs: tshLevel, freeT4Level, freeT3Level, totalT4Level, totalT3Level, thyroglobulinLevel
 *   2. antibodies: antiTPOAntibodies, antiThyroglobulinAntibodies, tsiLevel, traAntibodies
 *   3. clinical-status: clinicalThyroidFunction, thyroidSymptomScore, wayneThyrotoxicosisIndex, goiterClassification
 *   4. palpation-exam: thyroidGlandSize, thyroidPalpationFindings
 *   5. imaging: ultrasoundFindings, tiRadsScore, noduleCharacteristics, thyroidScintigraphyPattern, radioiodineUptakeTest
 *   6. biopsy: fnaBiopsyResults, bethesdaCategory
 *   7. medications: thyroidMedications
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ThyroidEvaluationsDocumentPDFTemplate from '../pdf-templates/ThyroidEvaluationsDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './ThyroidEvaluationsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field"; arrays store the whole array value) */
const DRAFT_KEY = 'thyroid_evaluationsPendingEdits';
const readDrafts = () => {
...
// For these numeric fields, a stored 0 is a "not measured / not assessed" sentinel, NOT a
// real assay result or score. Thyroid assays (TSH, T4, T3, antibodies, thyroglobulin) never
// read exactly 0 when measured; TI-RADS (1-5), Bethesda (I-VI), Wayne index and symptom score
// likewise have no meaningful 0. Hide these so blank records don't show "TSH Level: 0".
...
const ENUM_OPTIONS = {
  thyroidGlandSize: ['normal', 'enlarged'],
};
...
/* ======= COMPONENT ======= */
const ThyroidEvaluationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.thyroid_evaluations) return Array.isArray(r.thyroid_evaluations) ? r.thyroid_evaluations : [r.thyroid_evaluations];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.thyroid_evaluations) return Array.isArray(dd.thyroid_evaluations) ? dd.thyroid_evaluations : [dd.thyroid_evaluations]; return [dd]; }
      return [r];
    });
  ...
  /* ======= UTILS ======= */
  const hasVal = useCallback((v, fn) => { if (v === null || v === undefined || v === '') return false; if (typeof v === 'boolean') return true; if (typeof v === 'number') return !(fn && ZERO_SENTINEL_FIELDS.includes(fn) && v === 0); if (typeof v === 'string') return v.trim() !== ''; if (Array.isArray(v)) return v.length > 0; if (typeof v === 'object') return Object.keys(v).length > 0; return true; }, []);
  ...
  const shouldCommaSplit = useCallback((fn, value) => {
    return COMMA_SPLIT_FIELDS.includes(fn) && splitBySentence(value).length <= 1 && !parseLabel(value).isLabeled && splitByComma(value).length >= 2;
  }, [splitBySentence]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Thyroid Evaluation ${idx + 1}`.toLowerCase();
  ...
  // Stage a field DRAFT locally + persist to the pending-drafts localStorage store (survives refresh).
  // NO DB write — Approve commits it. sid is needed to reset the section's approved flag on re-edit.
  ...
  function saveCommaPart(record, fn, idx, sid, partIdx, partKey) {
    ...
    const replacements = editValue.split(',').map(part => part.trim()).filter(Boolean);
    if (replacements.length === 0) parts.splice(partIdx, 1);
    else parts.splice(partIdx, 1, ...replacements);
    const fullText = parts.join(', ');
    stageDraft(record, fn, idx, sid, fullText);
    const marks = { [partKey]: 'edited' };
    for (let offset = 1; offset < replacements.length; offset += 1) marks[`${fn}-${idx}-c${partIdx + offset}`] = 'added';
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/thyroid_evaluations/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/thyroid_evaluations/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this record's committed drafts from localStorage (only the fields just approved)
      const store = readDrafts();
      if (store[id]) {
    ...
    } catch (err) { console.error('[ThyroidEvaluations] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        items.filter(Boolean).forEach(item => {
          ...
          } else lines.push(`${lines.filter(line => /^\d+\./.test(line)).length + 1}. ${item}`);
        });
        ...
        if (shouldCommaSplit(f, strVal)) {
          text += `${labelLine}${splitByComma(strVal).map((part, partIdx) => `${partIdx + 1}. ${part}`).join('\n')}\n\n`;
        } else if (splitBySentence(strVal).length > 1 || parseLabel(strVal).isLabeled) {
          text += labelLine;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== THYROID EVALUATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Thyroid Evaluation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    ...
    const parsedEdit = parseFloat(editValue);
    const safeEdit = Number.isFinite(parsedEdit) ? parsedEdit : 0;
    const precision = String(step).includes('.') ? String(step).split('.')[1].length : 0;
    const adjusted = delta => Number((safeEdit + delta).toFixed(precision));
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                <button className="num-step" onClick={e => { e.stopPropagation(); setEditValue(String(adjusted(-step))); }}>&minus;</button>
                ...
                <button className="num-step" onClick={e => { e.stopPropagation(); setEditValue(String(adjusted(step))); }}>+</button>
              </div>
  ...
  /* ======= RENDER: ENUM FIELD — custom LTR dropdown ======= */
  const renderEnumField = (record, fn, idx, sid) => {
    ...
    const displayVal = String(val);
    const isModified = editedFields[editKey];
    const options = Array.from(new Set([...(ENUM_OPTIONS[fn] || []), displayVal].filter(Boolean)));
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(parsed.isLabeled ? parsed.value : itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = parsed.isLabeled ? `${parsed.label}: ${editValue}` : editValue; stageDraft(record, fn, idx, sid, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  /* ======= RENDER: DECLARED COMMA-LIST FIELD ======= */
              ...
              <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(partKey); setEditValue(part); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveCommaPart(record, fn, idx, sid, partIdx, partKey); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    if (shouldCommaSplit(fn, strVal)) return renderCommaField(record, fn, idx, sid);
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (ENUM_OPTIONS[f]) return renderEnumField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="thyroid-evaluations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Thyroid Evaluations</h2></div>
        <div className="empty-state">No thyroid evaluation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="thyroid-evaluations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Thyroid Evaluations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ThyroidEvaluationsDocumentPDFTemplate document={pdfData} />} fileName="Thyroid_Evaluations.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search thyroid evaluations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(record.clinicalThyroidFunction ? `Thyroid Evaluation - ${record.clinicalThyroidFunction}` : `Thyroid Evaluation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'thyroid-labs')}
            {renderSection(record, idx, 'antibodies')}
            {renderSection(record, idx, 'clinical-status')}
            {renderSection(record, idx, 'palpation-exam')}
            {renderSection(record, idx, 'imaging')}
            {renderSection(record, idx, 'biopsy')}
...
export default ThyroidEvaluationsDocument;
