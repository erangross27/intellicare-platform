/**
 * FacialTraumaAssessmentDocument.jsx
 * June 2026 — Inline editing, blue glow theme
 * Collection: facial_trauma_assessment
 *
 * 6 Sections:
 *   1. injury-overview: traumaMechanismCategory, glasgowComaScaleScore, facialInjurySeverityScore, timeFromInjuryToPresentation
 *   2. fractures: lefortFractureClassification, mandibularFractureLocations[], orbitalFloorFractureSize, zygomaticomaxillaryComplexDisplacement, nasalBoneDeviationAngle
 *   3. ocular-findings: enophthalmosPresent, interocularDistanceChange, globeRuptureIndicators[], retrobulbarHematomaPresent, intraocularPressure, extraocularMovementRestriction[]
 *   4. nerve-soft-tissue: infraorbitalNerveParesthesia, facialNerveBranchesAffected[], houseBrackmannGrade, parotidDuctInjury, softTissueLacerationDepth
 *   5. dental-occlusion: occlusalDerangementType, maximalInterincisalOpening, dentoalveolarInjuryClassification
 *   6. other-critical-findings: septalHematomaPresent, cerebrospinalFluidRhinorrhea
 *
 * NOTE: this schema has NO date field — record header shows ONLY the numbered record title.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import FacialTraumaAssessmentDocumentPDFTemplate from '../pdf-templates/FacialTraumaAssessmentDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueSelect from '../components/BlueSelect';
import './FacialTraumaAssessmentDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldName]: value } }  (value = scalar/string OR full array for array fields) */
const DRAFT_KEY = 'facial_trauma_assessmentPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'nerve-soft-tissue': 'Nerve & Soft Tissue',
  'dental-occlusion': 'Dental & Occlusion',
  'other-critical-findings': 'Other Critical Findings',
};

const FIELD_LABELS = {
  traumaMechanismCategory: 'Trauma Mechanism',
  glasgowComaScaleScore: 'Glasgow Coma Scale Score',
  facialInjurySeverityScore: 'Facial Injury Severity Score',
  timeFromInjuryToPresentation: 'Time From Injury to Presentation (hours)',
  lefortFractureClassification: 'Le Fort Fracture Classification',
  mandibularFractureLocations: 'Mandibular Fracture Locations',
  orbitalFloorFractureSize: 'Orbital Floor Fracture Size (mm)',
  zygomaticomaxillaryComplexDisplacement: 'ZMC Displacement (mm)',
  nasalBoneDeviationAngle: 'Nasal Bone Deviation (degrees)',
  enophthalmosPresent: 'Enophthalmos Present',
  interocularDistanceChange: 'Interocular Distance Change (mm)',
  globeRuptureIndicators: 'Globe Rupture Indicators',
  retrobulbarHematomaPresent: 'Retrobulbar Hematoma Present',
  intraocularPressure: 'Intraocular Pressure (mmHg)',
  extraocularMovementRestriction: 'Extraocular Movement Restriction',
  infraorbitalNerveParesthesia: 'Infraorbital Nerve Paresthesia',
  facialNerveBranchesAffected: 'Facial Nerve Branches Affected',
  houseBrackmannGrade: 'House-Brackmann Grade',
  parotidDuctInjury: 'Parotid Duct Injury',
  softTissueLacerationDepth: 'Soft Tissue Laceration Depth',
  occlusalDerangementType: 'Occlusal Derangement Type',
  maximalInterincisalOpening: 'Maximal Interincisal Opening (mm)',
  dentoalveolarInjuryClassification: 'Dentoalveolar Injury Classification',
  septalHematomaPresent: 'Septal Hematoma Present',
  cerebrospinalFluidRhinorrhea: 'CSF Rhinorrhea',
};

const SECTION_FIELDS = {
  'injury-overview': ['traumaMechanismCategory', 'glasgowComaScaleScore', 'facialInjurySeverityScore', 'timeFromInjuryToPresentation'],
  'fractures': ['lefortFractureClassification', 'mandibularFractureLocations', 'orbitalFloorFractureSize', 'zygomaticomaxillaryComplexDisplacement', 'nasalBoneDeviationAngle'],
  'ocular-findings': ['enophthalmosPresent', 'interocularDistanceChange', 'globeRuptureIndicators', 'retrobulbarHematomaPresent', 'intraocularPressure', 'extraocularMovementRestriction'],
  'nerve-soft-tissue': ['infraorbitalNerveParesthesia', 'facialNerveBranchesAffected', 'houseBrackmannGrade', 'parotidDuctInjury', 'softTissueLacerationDepth'],
  'dental-occlusion': ['occlusalDerangementType', 'maximalInterincisalOpening', 'dentoalveolarInjuryClassification'],
  'other-critical-findings': ['septalHematomaPresent', 'cerebrospinalFluidRhinorrhea'],
};

const SENTENCE_FIELDS = ['softTissueLacerationDepth', 'dentoalveolarInjuryClassification'];
const SIMPLE_FIELDS = ['traumaMechanismCategory', 'glasgowComaScaleScore', 'facialInjurySeverityScore', 'timeFromInjuryToPresentation', 'lefortFractureClassification', 'orbitalFloorFractureSize', 'zygomaticomaxillaryComplexDisplacement', 'nasalBoneDeviationAngle', 'interocularDistanceChange', 'intraocularPressure', 'houseBrackmannGrade', 'occlusalDerangementType', 'maximalInterincisalOpening'];
const BOOLEAN_FIELDS = ['enophthalmosPresent', 'retrobulbarHematomaPresent', 'infraorbitalNerveParesthesia', 'parotidDuctInjury', 'septalHematomaPresent', 'cerebrospinalFluidRhinorrhea'];
const ARRAY_FIELDS = ['mandibularFractureLocations', 'globeRuptureIndicators', 'extraocularMovementRestriction', 'facialNerveBranchesAffected'];
/* NUMERIC_FIELDS — edited via the −/+ num-stepper (parseFloat + isNaN guard), NOT free-text textarea,
   so the DB keeps real number types. These are exactly the 10 numerics (all also in HIDE_ZERO_FIELDS).
   timeFromInjuryToPresentation accepts decimals (stepFor is decimal-aware: 0.47 → step 0.01). */
const NUMERIC_FIELDS = ['glasgowComaScaleScore', 'facialInjurySeverityScore', 'timeFromInjuryToPresentation', 'orbitalFloorFractureSize', 'zygomaticomaxillaryComplexDisplacement', 'nasalBoneDeviationAngle', 'interocularDistanceChange', 'intraocularPressure', 'houseBrackmannGrade', 'maximalInterincisalOpening'];
/* Numeric fields where 0 means "not measured / not scored" — hidden when exactly 0.
   ALL 10 numerics: scores have clinical minimums (GCS min 3, House-Brackmann min I),
   measurements of 0 mm / 0 mmHg / 0 degrees / 0 hours mean "not assessed".
   Real decimals (e.g. timeFromInjuryToPresentation 0.47) still display. */
const HIDE_ZERO_FIELDS = ['glasgowComaScaleScore', 'facialInjurySeverityScore', 'timeFromInjuryToPresentation', 'orbitalFloorFractureSize', 'zygomaticomaxillaryComplexDisplacement', 'nasalBoneDeviationAngle', 'interocularDistanceChange', 'intraocularPressure', 'houseBrackmannGrade', 'maximalInterincisalOpening'];

/* ENUM fields → BlueSelect with the FULL clinical scale (act-like-medical-professional rule, memory
   6a4908b2). LeFort is THE canonical maxillary fracture classification (I/II/III); trauma mechanism is
   the standard ED category set. Off-scale stored values are kept as an extra option by enumOptionsWith. */
const ENUM_OPTIONS = {
...
/* stepFor: decimal-aware step for the −/+ number stepper (9 → 1, 0.47 → 0.01). */
...
/* splitByComma: parenthesis-aware comma split — items like "Grade II luxation (teeth #7, #8)" stay
   intact. Keeps a comma JOINED when it is a decimal grouping ("5,000"), a continuation conjunction
   ("…, and/or/then …"), or follows a trailing and/or. */
...
/* ═══════ COMPONENT ═══════ */
const FacialTraumaAssessmentDocument = ({ document: docProp, data, templateData }) => {
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Maps record _id (handling _id.$oid) → render index (_originalIdx). */
  useEffect(() => {
    ...
    records.forEach((r) => {
      ...
      let id = null;
      if (r && r._id) { id = typeof r._id === 'string' ? r._id : (r._id.$oid || String(r._id)); }
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      const docTitle = 'facial trauma assessment';
      const rt = `facial trauma assessment ${oi + 1}`;
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* stageDraft: the ONE place all save handlers funnel through. Stages a DRAFT locally (localEdits +
     pendingEdits) and writes it to the pending-drafts localStorage store (survives refresh). NO DB write —
     handleApproveSection is the ONLY path that persists to MongoDB. fieldName is the localEdits/draft key
     base (`${fn}`); value may be a scalar/string OR a full array (array fields store the whole array). */
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    setSaveError(null);
    // Stage a DRAFT only — committed to the DB later by handleApproveSection.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // localEdits keys for this template are always `${field}-${idx}` (array fields store the WHOLE
      // array under that key), so the editKey never contains a "." → field=fieldPart, NO arrayIndex.
      const toCommit = fields
        .map(f => `${f}-${idx}`)
        .filter(k => pendingEdits[k] && localEdits[k] !== undefined);
      for (const editKey of toCommit) {
        const fieldName = editKey.slice(0, -`-${idx}`.length);
        const resp = await secureApiClient.put(`/api/edit/facial_trauma_assessment/${id}/edit`, { field: fieldName, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/facial_trauma_assessment/${id}/approve`, { sectionId: sid, approved: true });
  ...
  // Sentence field → numbered lines: labeled sentence → "Label:" sub-heading + comma-split rows;
  // unlabeled sentence → comma-split rows (or the whole sentence when it is not a list). Numbering is
  // continuous across the whole field (numbering lives ONLY in Copy/PDF; JSX rows stay unnumbered).
  ...
  }, [getFieldValue, hasFieldVal, fmtVal, formatSentenceFieldLines, arrItemText]);

  const copyAllText = useCallback(async () => {
    let text = '=== FACIAL TRAUMA ASSESSMENT ===\n\n';
    pdfData.forEach((r) => {
      const oi = r._originalIdx ?? 0;
      text += `Facial Trauma Assessment ${oi + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: SIMPLE FIELD (no splitting — mechanisms, scores, classifications, measurements) ═══════ */
  const renderSimpleField = (record, fn, idx, sid) => {
    ...
    /* saveNumeric: parse free-text to a real number; reject non-numeric input (keeps DB type integrity) */
    const saveNumeric = () => {
      const parsed = parseFloat(editValue);
      if (editValue.trim() === '' || isNaN(parsed)) { setSaveError('Enter a valid number.'); return; }
            ...
            <div className="edit-field-container">
              {isNumeric ? (
                <div className="num-stepper-row">
                  ...
                  <input type="text" inputMode="decimal" className="edit-number" value={editValue} autoFocus onClick={e => e.stopPropagation()} onChange={e => setEditValue(e.target.value)} onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter') { e.preventDefault(); saveNumeric(); } }} />
                  <button type="button" className="num-step" disabled={saving} onClick={e => { e.stopPropagation(); const st = parseFloat(stepFor(editValue)) || 1; const dec = (String(st).split('.')[1] || '').length; const cur = parseFloat(editValue); setEditValue(Math.max(0, (isNaN(cur) ? 0 : cur) + st).toFixed(dec)); }}>+</button>
                </div>
              ) : (
              ...
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (isNumeric) { saveNumeric(); } else { handleSaveField(record, fn, idx, sid, null, editValue); } }}>{saving ? 'Saving...' : 'Save'}</button>
  ...
  /* ═══════ RENDER: ENUM FIELD (BlueSelect, full clinical scale — LeFort I/II/III, trauma mechanism) ═══════ */
  ...
  /* ═══════ RENDER: BOOLEAN FIELD (BlueSelect Yes/No — saves REAL booleans, false still renders "No") ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          /* saveArrayItem — named closure shared by Save button AND Ctrl+Enter. Stages a DRAFT: splices the
             edited item into the full array, stages the WHOLE array (Approve PUTs it as one field value). */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="facial-trauma-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Facial Trauma Assessment</h2></div>
        <div className="empty-state">No facial trauma assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="facial-trauma-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Facial Trauma Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FacialTraumaAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Facial_Trauma_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search facial trauma assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          return (
            <div key={safeId(record) || idx} className="record-card">
              ...
              <div className="record-header">
                <h3 className="record-name">{highlightText(`Facial Trauma Assessment ${origIdx + 1}`)}</h3>
              </div>
              {renderSection(record, origIdx, 'injury-overview')}
              {renderSection(record, origIdx, 'fractures')}
              {renderSection(record, origIdx, 'ocular-findings')}
              {renderSection(record, origIdx, 'nerve-soft-tissue')}
              {renderSection(record, origIdx, 'dental-occlusion')}
              {renderSection(record, origIdx, 'other-critical-findings')}
            </div>
          );
        })}
      </div>
...
export default FacialTraumaAssessmentDocument;
