 ...
 *
 * 6 Sections:
 *   1. carrier: gestationalCarrierAge, gravidityParityHistory, previousUncomplictedLiveBirths, bodyMassIndexKgM2, cesareanSectionCount
 *   2. uterine: uterineAnatomyAssessment, endometrialThicknessMm, cervicalCompetenceHistory
 *   3. serology: bloodTypeAndRhFactor, irregularAntibodyScreen, infectiousDiseasePanel, cytomegalovirusSerostatus, rubellaTiterImmunity, varicellaTiterImmunity
 *   4. thrombophilia: thrombophiliaScreening, antiphospholipidAntibodyPanel, thyroidStimulatingHormoneMuL, hemoglobinA1cPercent
 *   5. psych: psychologicalClearanceStatus, minnesotaMultiphasicPersonalityInventoryResults, nicotineCotinineScreenResult, urineDrugScreenPanel
 *   6. ob-history: preexistingHypertensionHistory, gestationalDiabetesHistory, preeclampsiaEclampsiaHistory
 */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; localEdits always
   stores the full reconstructed field value under `${fn}-${idx}`, so fieldPart is just the field). */
const DRAFT_KEY = 'surrogacy_evaluationPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'ob-history': 'Obstetric History',
};

const FIELD_LABELS = {
  gestationalCarrierAge: 'Gestational Carrier Age',
  gravidityParityHistory: 'Gravidity & Parity History',
  ...
  bloodTypeAndRhFactor: 'Blood Type & Rh Factor',
...
const SECTION_FIELDS = {
  ...
  'ob-history': ['preexistingHypertensionHistory', 'gestationalDiabetesHistory', 'preeclampsiaEclampsiaHistory'],
};

const NUMBER_FIELDS = ['gestationalCarrierAge', 'previousUncomplictedLiveBirths', 'bodyMassIndexKgM2', 'cesareanSectionCount', 'endometrialThicknessMm', 'thyroidStimulatingHormoneMuL', 'hemoglobinA1cPercent'];
const ARRAY_FIELDS = [];
const BOOLEAN_FIELDS = ['cervicalCompetenceHistory', 'rubellaTiterImmunity', 'varicellaTiterImmunity', 'preexistingHypertensionHistory', 'gestationalDiabetesHistory'];
const STRING_FIELDS = ['gravidityParityHistory', 'uterineAnatomyAssessment', 'bloodTypeAndRhFactor', 'irregularAntibodyScreen', 'infectiousDiseasePanel', 'cytomegalovirusSerostatus', 'thrombophiliaScreening', 'antiphospholipidAntibodyPanel', 'psychologicalClearanceStatus', 'minnesotaMultiphasicPersonalityInventoryResults', 'nicotineCotinineScreenResult', 'urineDrugScreenPanel', 'preeclampsiaEclampsiaHistory'];
// Unlabeled comma-list fields: split parenthesis-aware (commas inside (...) are NOT split points)
const COMMA_LIST_FIELDS = ['uterineAnatomyAssessment', 'bloodTypeAndRhFactor', 'irregularAntibodyScreen', 'infectiousDiseasePanel', 'cytomegalovirusSerostatus', 'thrombophiliaScreening', 'antiphospholipidAntibodyPanel'];
...
const SurrogacyEvaluationDocument = ({ document: docProp }) => {
  ...
  /* helper: safe record id (string | {$oid}) — declared early so the rehydrate effect can use it */
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     localEdits always stores the full reconstructed field value under `${fn}-${idx}`, so the draft
     fieldPart is the plain field name. */
  useEffect(() => {
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    setSaving(true);
    try {
      // localEdits keys are `${fieldName}-${idx}` (the full reconstructed field value). Commit each
      // pending edit belonging to a field in this section. fieldPart = key minus the `-${idx}` suffix.
      const suffix = `-${idx}`;
      const toCommit = Object.keys(localEdits).filter(k => pendingEdits[k] && k.endsWith(suffix));
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "field" (field names contain no dots here)
        ...
        if (!fields.includes(field)) continue; // only commit this section's fields
      ...
      setPendingEdits(prev => {
        ...
        Object.keys(next).forEach(k => {
          ...
          const fp = k.slice(0, -suffix.length);
          const ld = fp.lastIndexOf('.');
          const f = (ld !== -1 && /^\d+$/.test(fp.slice(ld + 1))) ? fp.slice(0, ld) : fp;
          if (fields.includes(f)) { committedFieldParts.push(fp); delete next[k]; }
        });
        return next;
      });
      // Drop this section's committed field drafts from localStorage
      const store = readDrafts();
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (COMMA_LIST_FIELDS.includes(f)) {
        const items = splitByComma(fmtVal(val)).map(s => s.trim()).filter(Boolean);
  ...
  /* ═══════ RENDER: COMMA-LIST FIELD (parenthesis-aware split; commas inside (...) kept) ═══════ */
  const renderCommaListField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    const raw = String(val == null ? '' : val);
    if (!raw.trim()) return null;
    const items = splitByComma(raw).map(s => s.trim()).filter(Boolean);
    const label = FIELD_LABELS[fn] || fn;
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(item); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const cur = splitByComma(String(getFieldValue(record, fn, idx) || '')).map(s => s.trim()).filter(Boolean); cur[ciIdx] = editValue.trim(); const fullText = cur.join(', '); stageFieldDraft(record, fn, idx, sid, fullText, { [editKey]: 'edited' }); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (COMMA_LIST_FIELDS.includes(f)) return renderCommaListField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="surrogacy-evaluation-document" ref={containerRef}>
        ...
        <div className="empty-state">No surrogacy evaluation data available</div>
      </div>
    );
  }
    ...
    <div className="surrogacy-evaluation-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<SurrogacyEvaluationDocumentPDFTemplate document={pdfData} />} fileName={`surrogacy-evaluation-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'ob-history')}
          </div>
        ))}
      </div>
    </div>
