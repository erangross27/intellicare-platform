/**
 * InsulinAdjustmentProtocolDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: insulin_adjustment_protocol
 *
 * 6 Sections:
 *   1. session-info: date (date picker), provider (string), facility (string)
 *   2. dosing-protocol: fastingAdjustment (long string), mealTimeAdjustment (long string)
 *   3. thresholds-contact: thresholds (array of strings), contactInstructions (long string)
 *   4. clinical-notes: findings (string), assessment (string), plan (string), notes (string), status (string)
 *   5. results-section: results (OBJECT — recursive renderObjectField / renderObjectNode / renderObjectLeaf)
 *   6. recommendations-section: recommendations (ARRAY of {recommendation, date} — date-grouped, editable)
 *
 * Object/recommendations renderers donated from PointOfCareUltrasoundHeartRateDocument.jsx.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import InsulinAdjustmentProtocolDocumentPDFTemplate from '../pdf-templates/InsulinAdjustmentProtocolDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './InsulinAdjustmentProtocolDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits field name; value is the
   full merged field value — string / array / object — exactly as stored in localEdits[`${fn}-${idx}`]). */
const DRAFT_KEY = 'insulin_adjustment_protocolPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['provider', 'facility', 'fastingAdjustment', 'mealTimeAdjustment', 'contactInstructions', 'findings', 'assessment', 'plan', 'notes', 'status'];
const DATE_FIELDS = ['date'];
const ARRAY_FIELDS = ['thresholds'];
...
/* Long string fields use splitBySentence + parseLabel */
const LONG_STRING_FIELDS = ['fastingAdjustment', 'mealTimeAdjustment', 'contactInstructions', 'findings', 'assessment', 'plan', 'notes'];

/* ═══════ OBJECT/SCALAR HELPERS (donor: PointOfCareUltrasoundHeartRateDocument) ═══════ */
const KEY_OVERRIDES = {};
...
/* ═══════ COMPONENT ═══════ */
const InsulinAdjustmentProtocolDocument = ({ document: docProp }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [copiedSection, setCopiedSection] = useState(null);
  const [copiedItems, setCopiedItems] = useState({});
  const [showCopied, setShowCopied] = useState(false);
  const [localEdits, setLocalEdits] = useState({});
  // localEdits keys (`${fn}-${idx}`) that are staged drafts (saved locally, NOT yet committed to DB/PDF).
  // Cleared on Approve. Their values are kept OUT of pdfData until approved.
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.insulin_adjustment_protocol) return Array.isArray(r.insulin_adjustment_protocol) ? r.insulin_adjustment_protocol : [r.insulin_adjustment_protocol];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.insulin_adjustment_protocol) return Array.isArray(dd.insulin_adjustment_protocol) ? dd.insulin_adjustment_protocol : [dd.insulin_adjustment_protocol]; return [dd]; }
      return [r];
    });
  ...
  const fieldSearchText = useCallback((f, val) => {
    ...
    if (Array.isArray(val)) return val.map(it => isScalar(it) ? String(it) : flattenSearchable(it)).join(' ');
    return fmtVal(val);
  }, [recsToText, fmtVal]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Insulin Adjustment Protocol ${idx + 1}`.toLowerCase();
  ...
  /* stageDraft = stage a DRAFT locally + write it to the pending-drafts localStorage store (survives
     ...
     (handleApproveSection commits). localEdits is ALWAYS keyed `${fn}-${idx}` and holds the FULL
     merged field value (string/array/object); the store mirrors it under store[recordId][fn]. */
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    // Build the full merged object value (no DB write — staged as a DRAFT keyed `${rootField}-${idx}`).
  ...
  const handleSaveArrayItem = useCallback((record, fn, idx, arrayIndex) => {
    ...
    // Stage the full array as a DRAFT (no DB write). Approve commits the whole field value.
    stageDraft(record, fn, idx, null, currentArr);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Persist each staged field for this section now. localEdits is keyed `${fn}-${idx}` and holds the
      // FULL merged field value, so a single { field, value } write replaces the whole field (arrays,
      // objects and strings alike) — no arrayIndex / dotted-path needed.
      const toCommit = fields
        .map(f => `${f}-${idx}`)
        .filter(k => pendingEdits[k] && localEdits[k] !== undefined);
      for (const key of toCommit) {
        const fn = key.slice(0, -`-${idx}`.length);
        const resp = await secureApiClient.put(`/api/edit/insulin_adjustment_protocol/${id}/edit`, { field: fn, value: localEdits[key] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/insulin_adjustment_protocol/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== INSULIN ADJUSTMENT PROTOCOL ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Insulin Adjustment Protocol ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderStringField = (record, fn, idx, sid, title) => {
    ...
    /* Multi-sentence: render with splitBySentence + parseLabel */
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              // Stage the full recommendations array as a DRAFT (no DB write). Approve commits it.
  ...
  if (!records || records.length === 0) {
    return (
      <div className="insulin-adjustment-protocol-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Insulin Adjustment Protocol</h2></div>
        <div className="empty-state">No insulin adjustment protocol records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="insulin-adjustment-protocol-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Insulin Adjustment Protocol</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<InsulinAdjustmentProtocolDocumentPDFTemplate document={pdfData} />} fileName="Insulin_Adjustment_Protocol.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search insulin adjustment protocol..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Insulin Adjustment Protocol ${idx + 1}`)}</h3>
            </div>
...
export default InsulinAdjustmentProtocolDocument;
