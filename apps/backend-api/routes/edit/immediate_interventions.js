...
function buildContext(req, operation = 'read') { return { serviceId: 'immediate-interventions-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['interventionType', 'timeToIntervention', 'glasgowComaScale', 'apacheIIScore', 'sofaScore', 'oxygenationStatus', 'systolicBloodPressure', 'diastolicBloodPressure', 'meanArterialPressure', 'heartRate', 'respiratoryRate', 'oxygenSaturation', 'lactateLevel', 'centralVenousPressure', 'inotropicSupport', 'vasopressorDose', 'fluidResuscitationVolume', 'bloodProductsAdministered', 'hemoglobinLevel', 'plateletCount', 'internationalNormalizedRatio', 'activatedPartialThromboplastinTime', 'creatinineLevel', 'estimatedGlomerularFiltrationRate', 'urineOutput'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('immediate_interventions', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[ImmediateInterventions] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('immediate_interventions', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[ImmediateInterventions] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
