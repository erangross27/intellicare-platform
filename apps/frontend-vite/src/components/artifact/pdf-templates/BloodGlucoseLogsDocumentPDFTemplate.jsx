...
const COLLECTION = 'blood_glucose_logs';
const COMMA_SPLIT_FIELDS = ['notes'];
const ARRAY_FIELDS = new Set([]);
const OBJECT_FIELDS = new Set([]);
const NARRATIVE_FIELDS = new Set(['exercise', 'symptoms', 'notes']);
const DATE_FIELDS = new Set(['date']);
const ZERO_SENTINEL_FIELDS = new Set([]);
const KEY_OVERRIDES = {};
const LABELS = {
  date: 'Date', readingTime: 'Reading Time', glucoseValue: 'Glucose Value', mealTiming: 'Meal Timing',
  insulinDose: 'Insulin Dose', carbohydrates: 'Carbohydrates', correctionFactor: 'Correction Factor',
  carbRatio: 'Carb Ratio', exercise: 'Exercise', symptoms: 'Symptoms', notes: 'Notes',
};
const SECTIONS = [
  { title: 'Date', fields: ['date'] },
  { title: 'Glucose Reading', fields: ['readingTime', 'glucoseValue', 'mealTiming'] },
  { title: 'Insulin and Carbohydrates', fields: ['insulinDose', 'carbohydrates', 'correctionFactor', 'carbRatio'] },
  { title: 'Exercise and Symptoms', fields: ['exercise', 'symptoms'] },
...
const BloodGlucoseLogsDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Blood Glucose Logs</Text><Text style={styles.noData}>No blood glucose log data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Blood Glucose Logs</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Blood Glucose Log {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
export default BloodGlucoseLogsDocumentPDFTemplate;
