/**
 * PatientProviderDocument.jsx
 * Collection: patient_provider — canonical one-pass template.
 *
 * Field map (schema coverage):
 *   DATE:    date (record-header date-picker, BlueDatePicker)
 *   NUMBER:  visitDuration (num-stepper, hide-zero)
 *   ARRAY:   chronicConditionsAddressed, proceduresPerformed, diagnosticOrdersPlaced,
 *            medicationsReviewed, prescriptionsWritten, referralsIssued,
 *            patientEducationProvided, immunizationsAdministered, billingCodes (string-array rows)
 *   OBJECT:  vitalSignsRecorded (recursive renderObjectNode / renderObjectLeaf)
 *   SENTENCE:chiefComplaint (per-sentence editable)
 *   STRING:  provider, providerSpecialty, facility, providerNpi, providerRole, providerTaxonomy,
 *            supervisingProvider, referringProvider, encounterType, encounterLocation,
 *            followUpInterval, providerSignature (simple)
 *
 * Donor: ChemotherapyRecordsDocument (typed renderers, 4-level search, recursive objects, per-sentence)
 *   + HematologyConsultationsDocument (canonical num-stepper: text/inputMode=decimal, no type=number).
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import SearchBar from '../components/SearchBar';
import PatientProviderDocumentPDFTemplate from '../pdf-templates/PatientProviderDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './PatientProviderDocument.css';
...
const DRAFT_KEY = 'patient_providerPendingEdits';
const readDrafts = () => {
...
/* COMPONENT */
const PatientProviderDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.patient_provider) return Array.isArray(r.patient_provider) ? r.patient_provider : [r.patient_provider];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.patient_provider) return Array.isArray(dd.patient_provider) ? dd.patient_provider : [dd.patient_provider]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `patient provider ${idx + 1}`.toLowerCase();
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. vitalSignsRecorded.bloodPressure) — stage a DRAFT only */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/patient_provider/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/patient_provider/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[PatientProvider] Approve error:', err); }
  ...
  const copyAllText = useCallback(async () => {
    let text = `PATIENT PROVIDER\n${COPY_LINE_EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Patient Provider ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  /* RENDER: NUMBER FIELD (num-stepper: text/inputMode=decimal, parseFloat+isNaN→block, hide-zero) */
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                ...
                <input type="text" inputMode="decimal" className="edit-number" value={editValue} autoFocus onChange={e => setEditValue(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); const n = parseFloat(editValue); if (isNaN(n) || editValue.trim() === '') { setSaveError('Please enter a valid number'); return; } handleSaveField(record, fn, idx, sid, n); } if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
              ...
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const n = parseFloat(editValue); if (isNaN(n) || editValue.trim() === '') { setSaveError('Please enter a valid number'); return; } handleSaveField(record, fn, idx, sid, n); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
  ...
  /* RENDER: SIMPLE STRING FIELD (single value, no sentence split) */
  ...
  /* RENDER: SENTENCE FIELD (per-sentence editable) */
  ...
  /* RENDER: OBJECT LEAF (editable; boolean -> BlueSelect, number/unit/ratio -> num-stepper, text -> textarea) */
  ...
  if (!records || records.length === 0) {
    return (
      <article className="patient-provider-document" ref={containerRef}>
        <header className="document-header"><h1 className="document-title">Patient Provider</h1></header>
  ...
  return (
    <article className="patient-provider-document" ref={containerRef}>
      <header className="document-header">
        <h1 className="document-title">Patient Provider</h1>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PatientProviderDocumentPDFTemplate document={pdfData} />} fileName="Patient_Provider.pdf">
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <div className="record-title-row"><h3 className="record-name">{highlightText(`Patient Provider ${idx + 1}`)}</h3></div>
            </div>
...
export default PatientProviderDocument;
