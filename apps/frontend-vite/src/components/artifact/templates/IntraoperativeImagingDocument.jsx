/**
 * IntraoperativeImagingDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: intraoperative_imaging
 *
 * 4 Sections:
 *   1. session-info: date (date picker), provider (string), facility (string), status (string)
 *   2. cholangiography: cholangiography.performed (bool), cholangiography.normal (bool),
 *                       cholangiography.findings (string), cholangiography.contrastInjectionSite (string)
 *   3. imaging-modalities: fluoroscopy (object, empty), ultrasound (object, empty)
 *   4. clinical-notes: findings (string long), assessment (string), plan (string), notes (string)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import IntraoperativeImagingDocumentPDFTemplate from '../pdf-templates/IntraoperativeImagingDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './IntraoperativeImagingDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the field path, e.g. "provider" or "cholangiography.findings") */
const DRAFT_KEY = 'intraoperative_imagingPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['provider', 'facility', 'status', 'cholangiography.findings', 'cholangiography.contrastInjectionSite', 'findings', 'assessment', 'plan', 'notes'];
...
/* ═══════ COMPONENT ═══════ */
const IntraoperativeImagingDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.intraoperative_imaging) return Array.isArray(r.intraoperative_imaging) ? r.intraoperative_imaging : [r.intraoperative_imaging];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.intraoperative_imaging) return Array.isArray(dd.intraoperative_imaging) ? dd.intraoperative_imaging : [dd.intraoperative_imaging]; return [dd]; }
      return [r];
    });
  ...
  /* getNestedValue: access nested fields like "cholangiography.performed" */
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (typeof val === 'object' && !Array.isArray(val)) {
          const objStr = Object.entries(val).map(([k, v]) => `${k} ${fmtVal(v)}`).join(' ').toLowerCase();
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Intraoperative Imaging ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* stageDraft: write a single field's pending value into the localStorage draft store for this record
     (keyed by recordId -> fieldPart). Drafts survive refresh + show in JSX, but are NOT committed to
     MongoDB and NOT shown in the PDF until the user clicks Pending Approve (handleApproveSection commits). */
  ...
  // NO DB write here. handleApproveSection commits to MongoDB.
  ...
  /* saveLeaf: stage a single leaf within an object field as a DRAFT (no DB write). The whole rebuilt
     root object is stored in localEdits + the draft store; handleApproveSection commits it. */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    // Collect this record's pending edits for this section's fields (localEdits key = "<fieldPart>-<idx>").
    const suffix = `-${idx}`;
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/intraoperative_imaging/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/intraoperative_imaging/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const next = { ...prev }; toCommit.forEach(k => delete next[k]); return next; });
      // Drop this record's committed drafts (just this section's fields) from localStorage
      const store = readDrafts();
      if (store[id]) {
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_ARRAY_FIELDS.includes(f)) {
        ...
        if (typeof val === 'object' && val !== null && !isEmptyDeep(val)) {
          text += `${label}\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== INTRAOPERATIVE IMAGING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Intraoperative Imaging ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NESTED OBJECT FIELD (fluoroscopy, ultrasound) ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {entries.map(([subKey, subVal]) => {
          const nestedFn = `${fn}.${subKey}`;
          const editKey = `${nestedFn}-${idx}`;
          const isEditing = editingField === editKey;
          const isModified = editedFields[editKey];
          const subLabel = subKey.replace(/([A-Z])/g, ' $1').replace(/^./, s => s.toUpperCase()).trim();
          const isBool = typeof subVal === 'boolean';
          const displayVal = isBool ? (subVal ? 'Yes' : 'No') : fmtVal(subVal);

          return (
            <div key={subKey}>
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(isBool ? (subVal ? 'Yes' : 'No') : String(subVal || '')); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    {isBool ? (
                      <BlueSelect value={editValue} options={['Yes', 'No']} onChange={v => setEditValue(v)} />
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const sv = isBool ? editValue === 'Yes' : editValue; handleSaveField(record, nestedFn, idx, sid, null, sv, editKey); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <>
                    <div className="row-content">
                      <span className="nested-field-label">{highlightText(subLabel)}</span>
  ...
  const renderStringField = (record, fn, idx, sid, title) => {
    const val = getFieldValue(record, fn, idx); if (!hasVal(val)) return null;
    const strVal = fmtVal(val);
    const sentences = LONG_STRING_FIELDS.includes(fn) ? splitBySentence(strVal) : [];
    const label = FIELD_LABELS[fn] || fn;
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; number/ratio -> number input, bool -> select, text -> textarea) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="intraoperative-imaging-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Intraoperative Imaging</h2></div>
        <div className="empty-state">No intraoperative imaging records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="intraoperative-imaging-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Intraoperative Imaging</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<IntraoperativeImagingDocumentPDFTemplate document={pdfData} />} fileName="Intraoperative_Imaging.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search intraoperative imaging..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Intraoperative Imaging ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'cholangiography')}
            {renderSection(record, idx, 'imaging-modalities')}
...
export default IntraoperativeImagingDocument;
