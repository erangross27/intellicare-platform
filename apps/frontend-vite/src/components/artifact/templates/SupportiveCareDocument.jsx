...
const DRAFT_KEY = "supportive_carePendingEdits";
const SECTIONS = [
  {
    id: "performance-status",
    title: "Performance Status",
    fields: ["palliativeConsultDate", "painScaleScore", "edmontonSymptomAssessmentScore", "karnofskyPerformanceStatus", "ecogPerformanceStatus"],
  },
  {
    id: "symptom-management",
    title: "Symptom Management",
    fields: ["opioidMedicationName", "morphineEquivalentDose", "nauseaVomitingSeverity", "dyspneaSeverity", "constrainedBowelMovementDays", "symptomManagementMedications"],
  },
  {
    id: "nutrition-wellbeing",
    title: "Nutrition & Well-Being",
    fields: ["nutritionalSupportType", "albuminLevel", "prealbumin", "weightLossPercentage", "sleepDisturbanceScale", "anxietyLevel", "depressionScreeningScore"],
  },
  { id: "psychosocial-support", title: "Psychosocial Support", fields: ["socialWorkConsult", "chaplainServices", "familyMeetingDate"] },
  { id: "advance-care-planning", title: "Advance Care Planning", fields: ["advanceDirectiveStatus", "codeStatus", "hospiceReferralDate"] },
];
const LABELS = {
...
const ENUM_OPTIONS = {
  nauseaVomitingSeverity: ["none", "mild", "moderate", "severe"],
  dyspneaSeverity: ["none", "mild", "moderate", "severe"],
  nutritionalSupportType: ["oral", "enteral", "parenteral", "none"],
  anxietyLevel: ["none", "mild", "moderate", "severe"],
  advanceDirectiveStatus: ["completed", "pending", "declined", "unknown"],
  codeStatus: ["full code", "DNR", "DNI", "comfort care"],
};
const MULTI_NUMBER_FIELDS = ["opioidMedicationName", "symptomManagementMedications"];
const NUMBER_SUFFIXES = {
  painScaleScore: "/10",
  edmontonSymptomAssessmentScore: "/90",
  karnofskyPerformanceStatus: "/100",
  ecogPerformanceStatus: "/5",
  morphineEquivalentDose: " mg/day",
  constrainedBowelMovementDays: " days",
  albuminLevel: " g/dL",
  prealbumin: " mg/dL",
  weightLossPercentage: "%",
  sleepDisturbanceScale: "/10",
  depressionScreeningScore: "/27",
};
...
const SupportiveCareDocument = ({
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(source) ? source : [source])
      .flatMap((record) => {
        if (record?.supportive_care)
          return Array.isArray(record.supportive_care)
            ? record.supportive_care
            : [record.supportive_care];
        if (record?.documentData)
          return Array.isArray(record.documentData)
            ? record.documentData
            : record.documentData?.supportive_care
              ? Array.isArray(record.documentData.supportive_care)
                ? record.documentData.supportive_care
                : [record.documentData.supportive_care]
              : [record.documentData];
        if (record?.document)
    ...
    () =>
      records
        ...
        .filter(
          ({ record }) =>
            !searchTerm.trim() ||
            `supportive care ${flatten(SECTIONS.flatMap((section) => section.fields.map((field) => record[field])))}`
              .toLowerCase()
      ...
      try {
        for (const key of keys) {
          ...
          const response = await secureApiClient.put(
            `/api/edit/supportive_care/${id}/edit`,
        ...
        await secureApiClient.put(
          `/api/edit/supportive_care/${id}/approve`,
  ...
  const renderLeaf = (
    ...
    const configuredOptions = ENUM_OPTIONS[path] || null;
    const enumOptions = configuredOptions
      ? [...new Set([String(value), ...configuredOptions])]
      : null;
    const multiNumber =
      !bool &&
      !date &&
  ...
  return (
    <div className="supportive-care-document" ref={containerRef}>
      <div className="document-header">
        <h1 className="document-title">Supportive Care</h1>
        <div className="header-actions">
          ...
          <PDFDownloadLink
            document={
              <SupportiveCareDocumentPDFTemplate document={pdfData} />
            }
            fileName="Supportive_Care.pdf"
            className="copy-btn"
          >
      ...
      <div className="search-container">
        <input
          ...
          placeholder="Search supportive care..."
          value={searchTerm}
      ...
      {!records.length ? (
        <div className="empty-state">
          No supportive care records available
        </div>
      ) : (
          ...
          {filtered.map(({ record, idx }) => (
            <div className="record-card" key={idx}>
              <div className="record-header">
                <h3 className="record-name">{`Supportive Care ${idx + 1}`}</h3>
              </div>
