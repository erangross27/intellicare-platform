/**
 * ImmuneReconstitutionPlanningDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: immune_reconstitution_planning
 *
 * 4 Sections:
 *   1. session-info: date (date picker), provider (string), facility (string)
 *   2. monitoring: irisMonitoring (boolean), cd4Monitoring (string/long), prophylaxisDiscontinuation (string/long)
 *   3. clinical-findings: findings (string), assessment (string), plan (string)
 *   4. additional-notes: notes (string/long), status (string)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ImmuneReconstitutionPlanningDocumentPDFTemplate from '../pdf-templates/ImmuneReconstitutionPlanningDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './ImmuneReconstitutionPlanningDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'immune_reconstitution_planningPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES_RESULTS = 'Results';
...
const STRING_FIELDS = ['provider', 'facility', 'cd4Monitoring', 'prophylaxisDiscontinuation', 'findings', 'assessment', 'plan', 'notes', 'status'];
...
/* number+unit / ratio splitters for OBJECT leaves */
...
/* ═══════ COMPONENT ═══════ */
const ImmuneReconstitutionPlanningDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.immune_reconstitution_planning) return Array.isArray(r.immune_reconstitution_planning) ? r.immune_reconstitution_planning : [r.immune_reconstitution_planning];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.immune_reconstitution_planning) return Array.isArray(dd.immune_reconstitution_planning) ? dd.immune_reconstitution_planning : [dd.immune_reconstitution_planning]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        const root = dotIdx === -1 ? fieldPart : fieldPart.slice(0, dotIdx);
        // OBJECT-leaf dotted draft (e.g. results.cd4): rebuild the merged root object for display.
        if (dotIdx !== -1 && OBJECT_FIELDS.includes(root)) {
          const rootKey = `${root}-${idx}`;
          const cur = nLocal[rootKey] !== undefined ? nLocal[rootKey] : record[root];
          ...
          nFields[`${root}-${idx}-${path.join('.')}`] = 'edited';
          return;
        }
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Immune Reconstitution Planning ${idx + 1}`.toLowerCase();
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.cd4) — value stays a STRING.
     Stages a DRAFT only (no DB write). The merged object lives in localEdits for display; the
     authoritative dotted-leaf value is parked in the draft store and committed on Approve. */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    const dottedFieldPart = `${rootField}.${path.join('.')}`;
    setSaveError(null);
    ...
    // Mark the merged root field as a pending draft so it stays OUT of the PDF until approved.
    setPendingEdits(prev => ({ ...prev, [rootKey]: true }));
    setEditedFields(prev => ({ ...prev, [leafKeyTrack]: 'edited' }));
    setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
    const store = readDrafts();
    if (!store[id]) store[id] = {};
    store[id][dottedFieldPart] = newVal; // dotted object-leaf path → committed verbatim on Approve
    writeDrafts(store);
    setEditingField(null); setEditValue('');
  }, [safeId]);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        const dotIdx = fieldPart.indexOf('.');
        const root = dotIdx === -1 ? fieldPart : fieldPart.slice(0, dotIdx);
        if (!fields.includes(root)) continue;
        // arrayIndex ONLY when the LAST dot-segment is purely numeric (reverses handleSaveField's fieldPart)
        ...
        const resp = await secureApiClient.put(`/api/edit/immune_reconstitution_planning/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedKeys.push(`${root}-${idx}`);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/immune_reconstitution_planning/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (store2[id]) {
        Object.keys(store2[id]).forEach(fp => { const r = fp.indexOf('.') === -1 ? fp : fp.slice(0, fp.indexOf('.')); if (fields.includes(r)) delete store2[id][fp]; });
    ...
    } catch (err) { console.error('[ImmuneReconstitutionPlanning] Approve error:', err); }
  }, [safeId]);
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        if (Array.isArray(val) && val.length > 0) {
          text += `${label}\n`;
          val.forEach((rec, ri) => { const rt = (rec?.recommendation || '').trim(); const rd = (rec?.date || '').trim(); if (rt) text += `  ${ri + 1}. ${rt}${rd ? ` (${rd})` : ''}\n`; });
          text += '\n';
        }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== IMMUNE RECONSTITUTION PLANNING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Immune Reconstitution Planning ${idx + 1}\n${'='.repeat(40)}\n\n`;
      ...
      <div key={fn} className="rec-mini-card">
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(val ? 'Yes' : 'No'); setSaveError(null); } }}>
          {isEditing ? (
            ...
            <>
              <div className="row-content"><span className={`content-value ${val ? 'boolean-yes' : 'boolean-no'}`}>{highlightText(displayVal)}</span><span className="edit-indicator">&#9998;</span></div>
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; bool -> Yes/No, number+unit -> number input, ratio stays text) ═══════ */
  ...
  /* ═══════ RENDER: OBJECT NODE (recursive; humanizeKey + nested-mini-card; editable leaves; hide-empty) ═══════ */
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              setSaveError(null);
                              // Stage as a DRAFT (no DB write); committed on Approve.
                              const recKey = `${fn}-${idx}`;
                              setLocalEdits(prev => ({ ...prev, [recKey]: newArr }));
                              setPendingEdits(prev => ({ ...prev, [recKey]: true }));
  ...
  if (!records || records.length === 0) {
    return (
      <div className="immune-reconstitution-planning-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Immune Reconstitution Planning</h2></div>
        <div className="empty-state">No immune reconstitution planning records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="immune-reconstitution-planning-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Immune Reconstitution Planning</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ImmuneReconstitutionPlanningDocumentPDFTemplate document={pdfData} />} fileName="Immune_Reconstitution_Planning.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search immune reconstitution planning..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Immune Reconstitution Planning ${idx + 1}`)}</h3>
            </div>
...
export default ImmuneReconstitutionPlanningDocument;
