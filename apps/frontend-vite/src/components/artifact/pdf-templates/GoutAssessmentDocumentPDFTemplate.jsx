/**
 * GoutAssessmentDocumentPDFTemplate.jsx
 * Box-free B&W LETTER (canonical, memory 6a2d6af6) — mirrors the JSX: real record.date (never createdAt),
 * status enum canonical, ALL string fields sentence-split ([.;] with abbrev/single-initial guard + labeled
 * comma-split), tophiPresent boolean, tophiLocations/recommendations arrays, jointAspirate/results recursive
 * objects (keys via humanizeKey incl. WBC/MSU overrides), values numbered ('1.' even singles), single-name
 * label gate. Rule #74: each field is ONE wrap={false} atomic View with the sectionTitle riding INSIDE the
 * first present field's View. Static PHI footer. Collection: gout_assessment.
 */
...
const SECTION_ORDER = ['record-info', 'uric-acid', 'joint-aspirate', 'tophi', 'clinical-details', 'findings', 'assessment', 'plan', 'recommendations', 'results', 'notes'];
...
const FIELD_LABELS = {
  ...
  uricAcidLevel: 'Uric Acid Level', jointAspirate: 'Joint Aspirate',
  tophiPresent: 'Tophi Present', tophiLocations: 'Tophi Locations',
  flareFrequency: 'Flare Frequency', renalInvolvement: 'Renal Involvement', dualEnergyCtFindings: 'Dual Energy CT Findings',
      ...
      <View key={f} style={styles.fieldGroup} wrap={false}>
        ...
        {items.map((it, i) => {
          if (it && typeof it === 'object' && !Array.isArray(it)) {
            const subs = Object.entries(it).filter(([, v]) => !isEmptyDeep(v));
            return (
              <View key={i}>
                {subs.map(([k, v], si) => <Text key={si} style={styles.value}>{si === 0 ? `${i + 1}. ` : '   '}{humanizeKey(k)}: {safeString(fmtScalar(v))}</Text>)}
              </View>
            );
          }
...
const GoutAssessmentDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].gout_assessment && Array.isArray(templateData[0].gout_assessment)) records = templateData[0].gout_assessment;
    else records = templateData;
  } else if (templateData && templateData.gout_assessment) {
    records = Array.isArray(templateData.gout_assessment) ? templateData.gout_assessment : [templateData.gout_assessment];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.gout_assessment ? (Array.isArray(dd.gout_assessment) ? dd.gout_assessment : [dd.gout_assessment]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Gout Assessment</Text></View>
        <Text style={styles.emptyState}>No gout assessment records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Gout Assessment ${idx + 1}`}</Text>
...
export default GoutAssessmentDocumentPDFTemplate;
