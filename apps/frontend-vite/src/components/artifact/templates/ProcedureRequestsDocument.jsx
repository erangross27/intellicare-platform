/**
 * ProcedureRequestsDocument.jsx
 * June 2026 — Schema-only build with inline editing, blue glow theme
 * Collection: procedure_requests
 *
 * 5 Sections:
 *   1. request: requestIdentifier, procedureCode, procedureName, requestIntent, requestPriority, clinicalIndication, laterality
 *   2. requester-performer: requesterId, requesterName, requesterSpecialty, performerType, performerId, performerName
 *   3. scheduling-location: requestedDateTime (date), authoredOn (date), scheduledDateTime (date), expectedDuration (number), locationId, locationName
 *   4. clinical: bodySite (array), reasonCode (array), reasonReference (array)
 *   5. logistics: consentObtained (bool), insuranceAuthorization, preparationInstructions
 *
 * Field types: DATE (date header + requestedDateTime/authoredOn/scheduledDateTime), NUMBER (expectedDuration),
 *   BOOLEAN (consentObtained), ARRAY (bodySite/reasonCode/reasonReference), STRING (rest).
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ProcedureRequestsDocumentPDFTemplate from '../pdf-templates/ProcedureRequestsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './ProcedureRequestsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits key prefix, e.g. "procedureName") */
const DRAFT_KEY = 'procedure_requestsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'request': 'Request',
  'requester-performer': 'Requester & Performer',
  'scheduling-location': 'Scheduling & Location',
  'clinical': 'Clinical',
  'logistics': 'Logistics',
};
...
const SECTION_FIELDS = {
  'request': ['requestIdentifier', 'procedureCode', 'procedureName', 'requestIntent', 'requestPriority', 'clinicalIndication', 'laterality'],
  'requester-performer': ['requesterId', 'requesterName', 'requesterSpecialty', 'performerType', 'performerId', 'performerName'],
  'scheduling-location': ['requestedDateTime', 'authoredOn', 'scheduledDateTime', 'expectedDuration', 'locationId', 'locationName'],
  'clinical': ['bodySite', 'reasonCode', 'reasonReference'],
  'logistics': ['consentObtained', 'insuranceAuthorization', 'preparationInstructions'],
};

const DATE_FIELDS = ['date', 'requestedDateTime', 'authoredOn', 'scheduledDateTime'];
const NUMBER_FIELDS = ['expectedDuration'];
const BOOLEAN_FIELDS = ['consentObtained'];
const ARRAY_FIELDS = ['bodySite', 'reasonCode', 'reasonReference'];
const STRING_FIELDS = ['requestIdentifier', 'procedureCode', 'procedureName', 'requestIntent', 'requestPriority', 'requesterId', 'requesterName', 'requesterSpecialty', 'performerType', 'performerId', 'performerName', 'locationId', 'locationName', 'laterality', 'clinicalIndication', 'insuranceAuthorization', 'preparationInstructions'];
...
/* ═══════ COMPONENT ═══════ */
const ProcedureRequestsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.procedure_requests) return Array.isArray(r.procedure_requests) ? r.procedure_requests : [r.procedure_requests];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.procedure_requests) return Array.isArray(dd.procedure_requests) ? dd.procedure_requests : [dd.procedure_requests]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Procedure Request ${idx + 1}`.toLowerCase();
  ...
  // Save = stage a DRAFT locally (localStorage + state). NO DB write until Approve.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "field" (field names have no dot-index here)
        const lastDot = fieldPart.lastIndexOf('.');
        const payload = { field: lastDot === -1 ? fieldPart : fieldPart.slice(0, lastDot), value: localEdits[editKey] };
        ...
        const resp = await secureApiClient.put(`/api/edit/procedure_requests/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/procedure_requests/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PROCEDURE REQUESTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Procedure Request ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(parsed.isLabeled ? parsed.value : itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const saveVal = parsed.isLabeled ? `${parsed.label}: ${editValue}` : editValue; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = saveVal; stageDraft(id2, fn, idx, sid, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="procedure-requests-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Procedure Requests</h2></div>
        <div className="empty-state">No procedure requests data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="procedure-requests-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Procedure Requests</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ProcedureRequestsDocumentPDFTemplate document={pdfData} />} fileName={`procedure-requests-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search procedure requests..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Procedure Request ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'request')}
            {renderSection(record, idx, 'requester-performer')}
            {renderSection(record, idx, 'scheduling-location')}
...
export default ProcedureRequestsDocument;
