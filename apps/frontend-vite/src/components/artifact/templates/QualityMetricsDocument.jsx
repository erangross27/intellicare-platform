              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; stageDraft(record, idx, `${fn}-${idx}`, currentArr, { field: fn, value: editValue, arrayIndex: itemIdx }, { edited: [editKey] }, sid); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
    ...
    <div className="quality-metrics-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<QualityMetricsDocumentPDFTemplate document={pdfData} />} fileName={`quality-metrics-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            ...
            <div className="record-header">
              {hasVal(record.metricDate) && (
                <div className="record-meta-row">
                  <span className="record-date">{formatDate(record.metricDate)}</span>
                  <span className={`status-badge ${record.metricMet ? 'met' : 'not-met'}`}>
                    {record.metricMet ? 'MET' : 'NOT MET'}
                  </span>
                </div>
              )}
              <h3 className="record-name">{highlightText(record.metricName || `Quality Metric ${idx + 1}`)}</h3>
            </div>
