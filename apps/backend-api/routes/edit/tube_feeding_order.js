...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'tube-feeding-order-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'formulaName',
  'formulaCaloriesDensity',
  'proteinContentPerLiter',
  'feedingRouteType',
  'feedingTubeSize',
  'feedingMethodType',
  'initialInfusionRate',
  'goalInfusionRate',
  'rateAdvancementSchedule',
  'dailyCaloricGoal',
  'dailyProteinGoal',
  'freeWaterFlushVolume',
  'freeWaterFlushFrequency',
  'gastricResidualVolumeThreshold',
  'gastricResidualCheckFrequency',
  'headOfBedElevation',
  'prokineticsOrdered',
  'cyclicFeedingStartTime',
  'cyclicFeedingDurationHours',
  'bolusVolume',
  'bolusFrequency',
  'modularsSupplements',
  'tubePatencyFlushSolution',
  'refeedingSyndromeRisk',
  'glycemicControlProtocol',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('tube_feeding_order', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[tube_feeding_order] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('tube_feeding_order', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[tube_feeding_order] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
