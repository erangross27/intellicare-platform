/**
 * MyositisAssessmentDocument.jsx
 * June 2026 — Collection: myositis_assessment (NEW template)
 *
 * Combines two donor patterns:
 *  - PodiatryExaminations recursive generic OBJECT renderer (humanizeKey + nested-mini-card,
 *    hide-empty at every level) for the DISPLAY-ONLY object fields:
 *      muscleWeakness (nested: distribution string, mrcScale OBJECT of muscle→"4/5", functionalImpact string),
 *      muscleEnzymes (OBJECT of value+unit strings), skinManifestations (OBJECT of BOOLEANS → Yes/No),
 *      results (OBJECT key-value).
 ...
 *
 * Editable: narrative strings (type, emgFindings, muscleBiopsy, provider, facility, findings,
 *   assessment, plan, notes, status), date, myositisAntibodies (array of strings), recommendations.
 * Display-only (recursive renderer): muscleWeakness, muscleEnzymes, skinManifestations, results.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MyositisAssessmentDocumentPDFTemplate from '../pdf-templates/MyositisAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './MyositisAssessmentDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'myositis_assessmentPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  muscleEnzymes: 'Muscle Enzymes',
  emgFindings: 'EMG Findings',
  muscleBiopsy: 'Muscle Biopsy',
  skinManifestations: 'Skin Manifestations',
  myositisAntibodies: 'Myositis Antibodies',
  findings: 'Findings',
...
const STRING_FIELDS = ['type', 'provider', 'facility', 'status', 'emgFindings', 'muscleBiopsy', 'findings', 'assessment', 'plan', 'notes'];
/* DISPLAY-ONLY nested object fields (recursive renderer) */
const OBJECT_FIELDS = ['muscleWeakness', 'muscleEnzymes', 'skinManifestations', 'results'];
/* Array-of-strings (editable, numbered rows) */
const STRING_ARRAY_FIELDS = ['myositisAntibodies'];
...
/* parseLabel: detect "Label: value" patterns (skip subordinate-clause openers so a grammatical
   colon like "When X: do Y" is NOT mistaken for a Label:Value pair) */
...
/* splitByComma: parenthesis-aware comma split with a thousands guard — a comma is a separator ONLY
   when the NEXT char is whitespace, so "2,150 U/L" and "1,000 mg" stay intact (no space after comma). */
...
/* ═══════ COMPONENT ═══════ */
const MyositisAssessmentDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.myositis_assessment) return Array.isArray(r.myositis_assessment) ? r.myositis_assessment : [r.myositis_assessment];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.myositis_assessment) return Array.isArray(dd.myositis_assessment) ? dd.myositis_assessment : [dd.myositis_assessment]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // fieldPart is a plain field name (whole-field value); rebuild the localEdits "-<idx>" key.
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `myositis assessment ${idx + 1}`;
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. muscleEnzymes.ck) — value stays a STRING.
     Staged as a DRAFT only: the WHOLE root object is rebuilt in localEdits + saved to the draft store.
     Approve commits the whole root object to MongoDB (field=rootField). NO DB write here. */
  ...
  // Approve commits it (field=fn, value=fullText).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Only commit this section's pending edits (editKey = "fieldPart-idx", fieldPart's base field in section)
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const dotIdx = fieldPart.lastIndexOf('.');
        // arrayIndex ONLY when the trailing dot-segment is purely numeric (mirrors handleSaveField)
        ...
        await secureApiClient.put(`/api/edit/myositis_assessment/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/myositis_assessment/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[MyositisAssessment] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  /* narrative → numbered copy lines; a labeled "Label: a, b, c" sentence decomposes to a label line +
     comma-split numbered items (thousands-guarded), mirroring the on-screen decomposition. */
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MYOSITIS ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Myositis Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: STRING ARRAY (myositisAntibodies) — numbered editable rows ═══════ */
                  ...
                  <div className="edit-field-container">
                    ...
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => {
                        ...
                        // Stage as a DRAFT (no DB write). Approve commits the whole array (field=fn).
                        const editKey = `${fn}-${idx}`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="myositis-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Myositis Assessment</h2></div>
        <div className="empty-state">No myositis assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="myositis-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Myositis Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MyositisAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Myositis_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search myositis assessment records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Myositis Assessment ${idx + 1}`)}</h3>
            </div>
...
export default MyositisAssessmentDocument;
