...
const SECTIONS = [
  { id: 'referral', title: 'Referral and Appointment', fields: ['referred', 'name', 'contact', 'appointmentScheduled'] },
  { id: 'record', title: 'Nutrition Visit Details', fields: ['date', 'type', 'provider', 'facility', 'status'] },
  { id: 'clinical', title: 'Clinical Nutrition Assessment', fields: ['findings', 'assessment', 'plan', 'notes'] },
...
const FIELD_LABELS = {
  referred: 'Referred', name: 'Nutritionist Name', contact: 'Contact', appointmentScheduled: 'Appointment Scheduled', date: 'Date', type: 'Visit Type', provider: 'Provider', facility: 'Facility', status: 'Status', findings: 'Findings', assessment: 'Assessment', plan: 'Plan', notes: 'Notes', recommendations: 'Recommendations', results: 'Results', additionalData: 'Additional Data',
};
...
export default function SouthAsianNutritionistDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>South Asian Nutritionist</Text></View>
    {!records.length && <Text style={styles.noDataText}>No south asian nutritionist data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>South Asian Nutritionist Record {recordIndex + 1}</Text></View>
