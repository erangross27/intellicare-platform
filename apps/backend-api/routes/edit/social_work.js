...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'social-work-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['date', 'provider', 'facility', 'referralSource', 'primaryConcern', 'livingArrangement', 'primaryCaregiver', 'insuranceStatus', 'financialConcerns', 'mentalHealthHistory', 'substanceUseHistory', 'safetyRiskAssessment', 'supportSystem', 'advanceDirectives', 'communityResourcesProvided', 'barriersToDischarge', 'functionalStatus', 'transportationNeeds', 'caseManagementLevel', 'guardianshipStatus', 'culturalConsiderations', 'homeSafetyAssessment', 'dischargeDisposition', 'interventionsProvided', 'patientGoals', 'followUpRequired'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('social_work', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[social_work] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('social_work', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[social_work] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
