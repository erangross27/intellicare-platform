 ...
 *
 * Covers procedure details, pathology leaves, generic reconstruction/results objects,
 * complications, recommendations, narrative fields, and notes.
 */
...
const sectionFields = (record, sid) => {
  if (sid === 'reconstruction' || sid === 'results') return flattenLeafPaths(record?.[sid], sid);
  if (sid === 'complications') return Array.isArray(record?.complications)
...
const SurgicalOncologyDocument = ({ document: docProp }) => {
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/surgical_oncology/${id}/edit`, payload);
  ...
  const renderBooleanField = (record, fn, idx, sid) => {
    ...
    const isEditing = editingField === editKey;
    const label = fieldLabel(fn);
    const displayVal = val === true || val === 'true' || val === 'Yes' ? 'Yes' : 'No';
    const isModified = editedFields[editKey];
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayVal); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              ...
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={event => { event.stopPropagation(); handleSaveField(record, fn, idx, sid, null, editValue === 'Yes'); }}>{saving ? 'Saving...' : 'Save'}</button>
        ...
        <div key={fn}>
          <div className="rec-mini-card nested-mini-card">
            ...
            {sentences.map((sentence, sIdx) => {
              ...
              const parsed = parseLabel(sentence);
              if (parsed.isLabeled || COMMA_ARRAY_FIELDS.has(fn)) {
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (BOOLEAN_FIELDS.has(f)) return renderBooleanField(record, f, idx, sid);
    ...
    <div className="surgical-oncology-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<SurgicalOncologyDocumentPDFTemplate document={pdfData} />} fileName="Surgical_Oncology.pdf" className="copy-btn">
