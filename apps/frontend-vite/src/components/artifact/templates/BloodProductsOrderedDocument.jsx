/**
 * BloodProductsOrderedDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: blood_products_ordered
 *
 * 5 Sections:
 *   1. product-blood-group: bloodProductType, aboBloodGroup, rhFactor, unitsOrdered, leukocyteReduced, irradiatedProduct, cmvNegative
 *   2. indication-urgency: transfusionIndication, urgencyLevel, massiveTransfusionProtocol, estimatedBloodLoss
 *   3. compatibility: crossmatchCompatibility, antibodyScreen, directAntiglobulinTest
 *   4. hematology: hemoglobinLevel, hematocritLevel, plateletCount, targetHemoglobin, fibrinogenLevel, coagulationProfile
 *   5. administration: premedication, specialRequirements, patientLocation
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import BloodProductsOrderedDocumentPDFTemplate from '../pdf-templates/BloodProductsOrderedDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './BloodProductsOrderedDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'product-blood-group': 'Product & Blood Group',
  'indication-urgency': 'Indication & Urgency',
...
const FIELD_LABELS = {
  bloodProductType: 'Blood Product Type',
  aboBloodGroup: 'ABO Blood Group',
  rhFactor: 'Rh Factor',
  unitsOrdered: 'Units Ordered',
  leukocyteReduced: 'Leukocyte Reduced',
  irradiatedProduct: 'Irradiated Product',
  cmvNegative: 'CMV Negative',
  transfusionIndication: 'Transfusion Indication',
  urgencyLevel: 'Urgency Level',
  massiveTransfusionProtocol: 'Massive Transfusion Protocol',
  estimatedBloodLoss: 'Estimated Blood Loss (mL)',
  crossmatchCompatibility: 'Crossmatch Compatibility',
  antibodyScreen: 'Antibody Screen',
  directAntiglobulinTest: 'Direct Antiglobulin Test',
  hemoglobinLevel: 'Hemoglobin (g/dL)',
  hematocritLevel: 'Hematocrit (%)',
  plateletCount: 'Platelet Count (×10³/µL)',
  targetHemoglobin: 'Target Hemoglobin (g/dL)',
  fibrinogenLevel: 'Fibrinogen (mg/dL)',
...
const SECTION_FIELDS = {
  'product-blood-group': ['bloodProductType', 'aboBloodGroup', 'rhFactor', 'unitsOrdered', 'leukocyteReduced', 'irradiatedProduct', 'cmvNegative'],
  'indication-urgency': ['transfusionIndication', 'urgencyLevel', 'massiveTransfusionProtocol', 'estimatedBloodLoss'],
  'compatibility': ['crossmatchCompatibility', 'antibodyScreen', 'directAntiglobulinTest'],
  'hematology': ['hemoglobinLevel', 'hematocritLevel', 'plateletCount', 'targetHemoglobin', 'fibrinogenLevel', 'coagulationProfile'],
  'administration': ['premedication', 'specialRequirements', 'patientLocation'],
};

const NUMBER_FIELDS = ['unitsOrdered', 'hemoglobinLevel', 'hematocritLevel', 'plateletCount', 'targetHemoglobin', 'fibrinogenLevel', 'estimatedBloodLoss'];
const BOOLEAN_FIELDS = ['leukocyteReduced', 'irradiatedProduct', 'cmvNegative', 'massiveTransfusionProtocol'];
const ARRAY_FIELDS = ['premedication', 'specialRequirements'];
const STRING_FIELDS = ['bloodProductType', 'aboBloodGroup', 'rhFactor', 'transfusionIndication', 'crossmatchCompatibility', 'antibodyScreen', 'directAntiglobulinTest', 'urgencyLevel', 'coagulationProfile', 'patientLocation'];
...
const DRAFT_KEY = 'blood_products_orderedPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const BloodProductsOrderedDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.blood_products_ordered) return Array.isArray(r.blood_products_ordered) ? r.blood_products_ordered : [r.blood_products_ordered];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.blood_products_ordered) return Array.isArray(dd.blood_products_ordered) ? dd.blood_products_ordered : [dd.blood_products_ordered]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayElem) {
          // Array-element draft (e.g. "premedication.2") → rebuild the full array under "field-idx"
          const fn = fieldPart.slice(0, lastDot);
          const itemIdx = parseInt(fieldPart.slice(lastDot + 1), 10);
          const baseKey = `${fn}-${idx}`;
          const current = Array.isArray(nLocal[baseKey])
            ? [...nLocal[baseKey]]
            : [...(Array.isArray(record[fn]) ? record[fn] : [])];
          current[itemIdx] = value;
          nLocal[baseKey] = current;
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Blood Products Ordered ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    setSaving(true);
    try {
      // Persist each staged field belonging to this section to the DB now.
      // Read the precise fieldParts (incl. "field.arrayIndex") from the localStorage draft store.
      const store = readDrafts();
      const recDrafts = store[id] || {};
      const pendingKeysToClear = [];
      for (const fieldPart of Object.keys(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/blood_products_ordered/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        pendingKeysToClear.push(`${baseField}-${idx}`);
        delete recDrafts[fieldPart];
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/blood_products_ordered/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; pendingKeysToClear.forEach(k => delete n[k]); return n; });
      // Update the draft store: drop committed fieldParts; remove the record entry if now empty
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== BLOOD PRODUCTS ORDERED ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Blood Products Ordered ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="blood-products-ordered-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Blood Products Ordered</h2></div>
        <div className="empty-state">No blood products ordered data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="blood-products-ordered-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Blood Products Ordered</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<BloodProductsOrderedDocumentPDFTemplate document={pdfData} />} fileName={`blood-products-ordered-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search blood products ordered..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Blood Products Ordered ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'product-blood-group')}
            {renderSection(record, idx, 'indication-urgency')}
            {renderSection(record, idx, 'compatibility')}
            {renderSection(record, idx, 'hematology')}
            {renderSection(record, idx, 'administration')}
          </div>
        ))}
      </div>
    </div>
...
export default BloodProductsOrderedDocument;
