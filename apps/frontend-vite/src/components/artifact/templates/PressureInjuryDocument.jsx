/**
 * PressureInjuryDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: pressure_injury
 *
 * 5 Sections:
 *   1. provider-info: provider, facility, date, createdAt
 *   2. wound-details: location, stage, size, type, description
 *   3. clinical-findings: findings, assessment, notes, status
 *   4. treatment-plan: treatment, prevention, plan
 *   5. recommendations-results: recommendations, results
 */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; values may be string or array) */
const DRAFT_KEY = 'pressure_injuryPendingEdits';
const readDrafts = () => {
...
/* ======= COMPONENT ======= */
const PressureInjuryDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.pressure_injury) return Array.isArray(r.pressure_injury) ? r.pressure_injury : [r.pressure_injury];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.pressure_injury) return Array.isArray(dd.pressure_injury) ? dd.pressure_injury : [dd.pressure_injury]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      const rid = (() => { const r = record; if (!r?._id) return null; if (typeof r._id === 'string') return r._id; if (r._id.$oid) return r._id.$oid; return String(r._id); })();
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pressure Injury ${idx + 1}`.toLowerCase();
  ...
  // Save = stage a DRAFT (full updated array) locally + localStorage. No DB write until Approve.
  ...
  // Save one sentence = stage a DRAFT (full rebuilt text) locally + localStorage. No DB write until Approve.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // localEdits keys are always "<field>-<idx>" (string or full-array value) — commit each staged one.
      const toCommit = fields
        .map(f => `${f}-${idx}`)
        .filter(k => pendingEdits[k] && localEdits[k] !== undefined);
      for (const editKey of toCommit) {
        const fieldName = editKey.slice(0, -`-${idx}`.length);
        const resp = await secureApiClient.put(`/api/edit/pressure_injury/${id}/edit`, { field: fieldName, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/pressure_injury/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PRESSURE INJURY ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Pressure Injury ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ======= RENDER: DATE FIELD (display only) ======= */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pressure-injury-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pressure Injury</h2></div>
        <div className="empty-state">No pressure injury records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pressure-injury-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pressure injury records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'wound-details')}
...
export default PressureInjuryDocument;
