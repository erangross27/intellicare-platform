...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'wound-care-assessments-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'woundIdentification', 'woundIdentification.woundNumber', 'woundIdentification.anatomicLocation', 'woundIdentification.woundEtiology',
  'woundClassification',
  'woundMeasurements',
  ...
  'debridement',
  'dressingRegimen', 'dressingRegimen.dressingFrequency', 'dressingRegimen.primaryDressing', 'dressingRegimen.secondaryDressing', 'dressingRegimen.dressingChangeTechnique',
  'offloading',
  'adjunctiveTherapies',
  'healingProgress',
  'amputationRisk',
  'patientEducation', 'patientEducation.dressingChangeTechnique', 'patientEducation.signsOfInfection', 'patientEducation.activityModifications', 'patientEducation.nutritionalGuidance', 'patientEducation.followUpInstructions',
  'date',
];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('wound_care_assessments', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[wound_care_assessments] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('wound_care_assessments', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[wound_care_assessments] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
