/**
 * HomeHealthOrdersDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: home_health_orders
 *
 * 6 Sections:
 *   1. order-info: date, orderingPhysicianName, orderingPhysicianNpi, primaryDiagnosisCode,
 *                  homeHealthAgencyName
 *   2. secondary-diagnoses: secondaryDiagnosisCodes (array)
 *   3. skilled-services: skilledNursingOrdered, skilledNursingFrequency,
 ...
 *   4. additional-care: medicationManagementRequired, woundCareRequired
 *   5. limitations-safety: functionalLimitations (array), safetyMeasures (array),
 *                          dmeMedicalEquipmentOrdered (array)
 *   6. homebound-status: patientHomeboundStatus
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HomeHealthOrdersDocumentPDFTemplate from '../pdf-templates/HomeHealthOrdersDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './HomeHealthOrdersDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits key without "-<idx>") */
const DRAFT_KEY = 'home_health_ordersPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = [
  'orderingPhysicianName', 'orderingPhysicianNpi', 'primaryDiagnosisCode',
  'homeHealthAgencyName', 'skilledNursingFrequency', 'physicalTherapyFrequency',
  'occupationalTherapyFrequency', 'speechTherapyFrequency', 'homeHealthAideFrequency',
  'patientHomeboundStatus',
];
...
/* ═══════ COMPONENT ═══════ */
const HomeHealthOrdersDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.home_health_orders) return Array.isArray(r.home_health_orders) ? r.home_health_orders : [r.home_health_orders];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.home_health_orders) return Array.isArray(dd.home_health_orders) ? dd.home_health_orders : [dd.home_health_orders]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Home Health Orders ${idx + 1}`.toLowerCase();
  ...
  // Save array element = stage a DRAFT (full array) locally + localStorage. Approve commits to DB.
  const handleSaveArrayItem = useCallback((record, fn, idx, arrayIndex, newValue) => {
    const id = safeId(record); if (!id) return;
    const currentArr = [...(getFieldValue(record, fn, idx) || [])];
    currentArr[arrayIndex] = newValue;
    const editKey = `${fn}-${idx}`;
    setLocalEdits(prev => ({ ...prev, [editKey]: currentArr }));
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
    setEditedFields(prev => ({ ...prev, [`${fn}.${arrayIndex}-${idx}`]: 'edited' }));
    // Re-edit after approval → drop the approved flag of whichever section owns this field
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    // localEdits keys are `${field}-${idx}` holding the FULL field value (string or full array).
    ...
    try {
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/home_health_orders/${id}/edit`, { field: fn, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/home_health_orders/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HOME HEALTH ORDERS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Home Health Orders ${idx + 1}\n${'='.repeat(40)}\n\n`;
      Object.keys(SECTION_FIELDS).forEach(sid => {
  ...
  const renderArrayField = (record, fn, idx, sid) => {
    ...
    return (
      <div key={fn} className="rec-mini-card">
        {showSubLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
        {arr.map((item, ai) => {
          const itemKey = `${fn}.${ai}-${idx}`;
          const isEditing = editingField === itemKey;
          const isModified = editedFields[itemKey];
          if (searchTerm.trim() && !sectionTitleMatches(sid) && !(FIELD_LABELS[fn] || fn).toLowerCase().includes(phrase) && !String(item).toLowerCase().includes(phrase)) return null;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(String(item)); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveArrayItem(record, fn, idx, ai, editValue.trim()); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="home-health-orders-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Home Health Orders</h2></div>
        <div className="empty-state">No home health orders records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="home-health-orders-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Home Health Orders</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HomeHealthOrdersDocumentPDFTemplate document={pdfData} />} fileName="Home_Health_Orders.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search home health orders..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Home Health Orders ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'order-info')}
            {renderSection(record, idx, 'secondary-diagnoses')}
            {renderSection(record, idx, 'skilled-services')}
            {renderSection(record, idx, 'additional-care')}
            {renderSection(record, idx, 'limitations-safety')}
            {renderSection(record, idx, 'homebound-status')}
          </div>
        ))}
      </div>
    </div>
...
export default HomeHealthOrdersDocument;
