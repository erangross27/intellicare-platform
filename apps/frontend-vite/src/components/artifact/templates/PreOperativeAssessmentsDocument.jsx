/**
 * PreOperativeAssessmentsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: pre_operative_assessments
 *
 * Sections:
 *   1. assessment-info: assessmentDate, scheduledSurgeryDate, plannedProcedure, preOpDiagnosis
 *   2. medical-surgical: medicalHistory, surgicalHistory, currentMedications, allergies
 *   3. clinical-status: asaClass, functionalStatus, cardiovascularRisk, pulmonaryRisk, airwayAssessment
 *   4. organ-function: renalFunction, hepaticFunction, coagulationStatus
 *   5. anesthesia-plan: anesthesiaType, npoStatus
 *   6. testing-risk: preOpTesting, riskStratification
 *   7. clearance-special: clearance, specialConsiderations
 *   8. recommendations-section: recommendations
 */
...
const DRAFT_KEY = 'pre_operative_assessmentsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const PreOperativeAssessmentsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.pre_operative_assessments) return Array.isArray(r.pre_operative_assessments) ? r.pre_operative_assessments : [r.pre_operative_assessments];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.pre_operative_assessments) return Array.isArray(dd.pre_operative_assessments) ? dd.pre_operative_assessments : [dd.pre_operative_assessments]; return [dd]; }
      return [r];
    });
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayPart) {
          ...
          const baseArr = Array.isArray(nLocal[editKey]) ? nLocal[editKey] : [...(Array.isArray(record[baseField]) ? record[baseField] : [])];
          baseArr[itemIdx] = typeof baseArr[itemIdx] === 'object' && baseArr[itemIdx] !== null ? { ...baseArr[itemIdx], recommendation: value } : value;
          nLocal[editKey] = baseArr;
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pre-Operative Assessment ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    setSaving(true);
    try {
      // Commit each staged field-part from the draft store that belongs to this section's fields.
      const store = readDrafts();
      const recDrafts = (store && store[id]) || {};
      const committedKeys = []; // localEdits keys ("baseField-idx") to clear from pendingEdits
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/pre_operative_assessments/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedKeys.push(`${baseField}-${idx}`);
        delete store[id][fieldPart];
      }
      // Flag the record/section approved (audit trail)
      await secureApiClient.put(`/api/edit/pre_operative_assessments/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[PreOperativeAssessments] Approve error:', err); }
  ...
  const getRecText = useCallback((rec) => {
  ...
  const copyAllText = useCallback(async () => {
    ...
    pdfData.forEach((r, idx) => {
      text += `Pre-Operative Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: ARRAY FIELD (recommendations — objects with .recommendation) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pre-operative-assessments-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pre-Operative Assessments</h2></div>
        <div className="empty-state">No pre-operative assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pre-operative-assessments-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pre-operative assessments..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'medical-surgical')}
            {renderSection(record, idx, 'clinical-status')}
            {renderSection(record, idx, 'organ-function')}
            {renderSection(record, idx, 'anesthesia-plan')}
            {renderSection(record, idx, 'testing-risk')}
            {renderSection(record, idx, 'clearance-special')}
...
export default PreOperativeAssessmentsDocument;
