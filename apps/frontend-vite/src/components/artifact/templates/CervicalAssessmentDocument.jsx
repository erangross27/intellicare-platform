/**
 * CervicalAssessmentDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: cervical_assessment
 *
 * 6 Sections:
 *   1. header-info: date, provider, facility, status
 *   2. cervical-exam: cervicalLength, cervicalDilation, cervicalEffacement,
 *                     cervicalConsistency, cervicalPosition, bishopScore
 *   3. clinical-assessment: findings, assessment, plan, notes
 *   4. results-info: results (recursive object)
 *   5. recommendations-info: recommendations (array of {recommendation, date}, date-grouped)
 *   6. cerclage-info: cervicalCerclage (object)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import CervicalAssessmentDocumentPDFTemplate from '../pdf-templates/CervicalAssessmentDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import './CervicalAssessmentDocument.css';
...
const FIELD_LABELS = {
  ...
  cervicalEffacement: 'Cervical Effacement',
  cervicalConsistency: 'Cervical Consistency',
  ...
  cervicalCerclage: 'Cervical Cerclage',
};
...
const STRING_FIELDS = ['provider', 'facility', 'status', 'cervicalLength', 'cervicalDilation', 'cervicalEffacement', 'cervicalConsistency', 'cervicalPosition', 'bishopScore', 'findings', 'assessment', 'plan', 'notes'];

/* ═══════ OBJECT/RECURSION HELPERS (for results) ═══════ */
const KEY_OVERRIDES = {};
...
/* splitByComma: never inside parentheses; ", and …"/" , or …" stays connected (both sides);
...
/* Header fields that split into rows: provider by semicolon, facility by guarded comma */
const PARTS_FIELDS = {
  provider: { split: (s) => String(s).split(/;\s*/).map(t => t.trim()).filter(Boolean), join: '; ' },
  facility: { split: (s) => splitByComma(String(s)), join: ', ' },
};
...
const DRAFT_KEY = 'cervicalAssessmentPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const CervicalAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.cervical_assessment) return Array.isArray(r.cervical_assessment) ? r.cervical_assessment : [r.cervical_assessment];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.cervical_assessment) return Array.isArray(dd.cervical_assessment) ? dd.cervical_assessment : [dd.cervical_assessment]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        // Mark both a field-level and a sentence-level marker so the modified badge + approve button show.
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (typeof val === 'object' && !Array.isArray(val)) { if (Object.entries(val).some(([k, v]) => `${k} ${v}`.toLowerCase().includes(phrase))) return true; }
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) {
      if (typeof val === 'object' && !Array.isArray(val)) return Object.entries(val).some(([k, v]) => `${k} ${v}`.toLowerCase().includes(phrase));
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Cervical Assessment ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (val && (typeof val === 'object' && !Array.isArray(val) ? Object.entries(val).some(([k, v]) => `${k} ${v}`.toLowerCase().includes(phrase)) : fmtVal(val).toLowerCase().includes(phrase))) return true;
        }
      }
      return false;
    });
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Stage a DRAFT locally (NO DB write). localEdits[`${fn}-${idx}`] holds the full effective field
  // value (string / object / array). The draft is mirrored to localStorage (survives refresh) and
  // flagged pending so it stays OUT of the PDF/Copy All until Approve commits it.
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.ph) — value stays a STRING */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    // Stage as a DRAFT (no DB write): build the new full object value and store it under `${rootField}-${idx}`.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const suffix = `-${idx}`;
      // Pending drafts for THIS record AND THIS section (localEdits key = `${fn}-${idx}`).
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/cervical_assessment/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/cervical_assessment/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[CervicalAssessment] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  // labeled sentences restart numbering (own sub-label + dashes); unlabeled rows continue the count
  const formatSentenceFieldLines = useCallback((text) => {
    ...
    sentences.forEach(s => {
      ...
      if (parsed.isLabeled) {
        ...
        splitByComma(parsed.value).forEach(item => { lines.push(`${++n}. ${item.replace(/[.;]+$/, '').trim()}`); });
  ...
  const objectCopyLines = useCallback((label, value, indent) => {
    ...
    if (isScalar(value)) { if (label) { out.push(`${pad}${label}`); out.push(`${pad}${'-'.repeat(40)}`); } out.push(`${pad}1. ${fmtScalar(value)}`); return out; }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      const emitLabel = () => { if (!sameAsTitle) text += `${label}\n${DASH}\n`; };
      const val = getFieldValue(record, f, idx);
      if (!hasVal(val)) return;
      if (DATE_FIELDS.includes(f)) {
        emitLabel(); text += `1. ${formatDate(val)}\n\n`;
      } else if (BOOLEAN_FIELDS.includes(f)) {
        emitLabel(); text += `1. ${val ? 'Yes' : 'No'}\n\n`;
        ...
        if (recs.length > 0) {
          ...
          recs.forEach((r) => {
            ...
            // dated group restarts numbering; an undated run continues the count
        ...
        if (typeof val === 'object' && !Array.isArray(val)) {
          ...
          if (entries.length > 0) {
            emitLabel();
            entries.forEach(([k, v]) => { text += `${humanizeKey(k)}\n${DASH}\n1. ${fmtVal(v)}\n\n`; });
          }
        } else {
          emitLabel(); text += `1. ${fmtVal(val)}\n\n`;
        }
      } else if (STRING_FIELDS.includes(f)) {
        const strVal = fmtVal(val);
        const partsCfg = PARTS_FIELDS[f];
        const headerParts = partsCfg ? partsCfg.split(strVal) : null;
        if (partsCfg && headerParts.length > 1) {
          emitLabel();
          headerParts.forEach((p, i) => { text += `${i + 1}. ${p}\n`; });
          text += '\n';
        } else {
  ...
  const copyAllText = useCallback(async () => {
    let text = `CERVICAL ASSESSMENT\n${'='.repeat(40)}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Cervical Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT FIELD (cervicalCerclage) ═══════ */
  const renderObjectField = (record, fn, idx, sid) => {
    ...
    if (typeof val === 'object' && !Array.isArray(val)) {
      ...
      // single-name rule: label == section title → hide (the section header already says it)
      const showSubLabel = label.trim().toLowerCase() !== (SECTION_TITLES[sid] || '').trim().toLowerCase();
      return (
        <div key={fn} className="rec-mini-card">
          {showSubLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
          {entries.map(([rKey, rValue]) => {
            if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
              const phrase = searchTerm.toLowerCase().trim();
              if (!humanizeKey(rKey).toLowerCase().includes(phrase) && !String(rValue).toLowerCase().includes(phrase) && !label.toLowerCase().includes(phrase)) return null;
            }
            // nested-mini-card with humanized sub-label + value row beneath (never side by side)
            return renderObjectLeaf(record, fn, [rKey], idx, sid, rValue);
          })}
        </div>
      );
    }
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; number+unit -> number input, ratio -> ratio, text -> textarea) ═══════ */
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* provider/facility: split into per-part rows (semicolon / guarded comma); edits splice back */
    const partsCfg = PARTS_FIELDS[fn];
    const headerParts = partsCfg ? partsCfg.split(strVal) : null;
    if (partsCfg && headerParts.length > 1) {
      return (
        <div key={fn} className="rec-mini-card">
          <div className="nested-subtitle">{highlightText(label)}</div>
          {headerParts.map((part, pi) => {
            const partKey = `${fn}-${idx}-p${pi}`;
            const isEditingP = editingField === partKey;
            const badge = editedSentences[partKey];
            if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections && !part.toLowerCase().includes(searchTerm.toLowerCase().trim()) && !label.toLowerCase().includes(searchTerm.toLowerCase().trim())) return null;
            return (
              <div key={pi}>
                <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditingP) { setEditingField(partKey); setEditValue(part); setSaveError(null); } }}>
                  {isEditingP ? (
                    <div className="edit-field-container">
                      ...
                      {saveError && <div className="save-error">{saveError}</div>}
                      <div className="edit-actions">
                        <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const cur = partsCfg.split(String(getFieldValue(record, fn, idx) || '')); cur[pi] = editValue.trim(); const rebuilt = cur.filter(Boolean).join(partsCfg.join); setSaveError(null); stageDraft(record, fn, idx, rebuilt); setEditedSentences(prev => ({ ...prev, [partKey]: 'edited' })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                        <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                      </div>
                    </div>
                  ) : (
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (f === 'results') return renderRecursiveObjectField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="cervical-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Cervical Assessment</h2></div>
        <div className="empty-state">No cervical assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="cervical-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Cervical Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<CervicalAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Cervical_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search cervical assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              {/* date + provider render in the Header Information section — no duplicate pills here */}
              <h3 className="record-name">{highlightText(`Cervical Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'header-info')}
            {renderSection(record, idx, 'cervical-exam')}
            {renderSection(record, idx, 'clinical-assessment')}
            {renderSection(record, idx, 'results-info')}
            {renderSection(record, idx, 'recommendations-info')}
            {renderSection(record, idx, 'cerclage-info')}
          </div>
        ))}
      </div>
    </div>
...
export default CervicalAssessmentDocument;
