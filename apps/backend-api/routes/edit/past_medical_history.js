/**
 * PastMedicalHistory Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/past_medical_history/:id/edit    — Edit a single field
 * PUT /api/edit/past_medical_history/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'past-medical-history-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('past_medical_history', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[PastMedicalHistory] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('past_medical_history', { _id: objectId }, {
      $set: {
    ...
    console.error('[PastMedicalHistory] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
