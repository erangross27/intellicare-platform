/**
 * FrailtyAssessmentDocument.jsx
 * June 2026 — Collection: frailty_assessment (NEW template)
 *
 * Donors cloned:
 *  - SupportGroupReferralDocument: date header renderDateField; OBJECT recursive render;
 *    ARRAY date-grouped recommendations; STRING per-sentence; BOOLEAN Yes/No BlueSelect
 *    saving real boolean; 4-level search; per-section approve; Copy row/Section/All;
 *    pdfData memo; blue-glow theme.
 *
 * FIELDS (19 → 100% schema coverage):
 *   DATE:    date
 *   BOOLEAN: sarcopenia
 *   STRING:  frailtyIndex, gripStrength, walkingSpeed, exhaustion, physicalActivity,
 *            unintentionalWeightLoss, clinicalFrailtyScale, type, provider, facility,
...
import FrailtyAssessmentDocumentPDFTemplate from '../pdf-templates/FrailtyAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './FrailtyAssessmentDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }
     fieldPart = the DB field to PUT: "findings" | "results.outcome" | "recommendations"
     value     = the DB value (string for string/leaf fields, array for recommendations) */
const DRAFT_KEY = 'frailty_assessmentPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  clinicalFrailtyScale: 'Clinical Frailty Scale',
  type: 'Type',
  provider: 'Provider',
  facility: 'Facility',
  findings: 'Findings',
...
const STRING_FIELDS = ['frailtyIndex', 'gripStrength', 'walkingSpeed', 'exhaustion', 'physicalActivity', 'unintentionalWeightLoss', 'clinicalFrailtyScale', 'type', 'provider', 'facility', 'findings', 'assessment', 'plan', 'notes'];
...
/* splitByComma: parenthesis-aware, splits only on comma+whitespace (thousands stay whole) */
...
/* ═══════ COMPONENT ═══════ */
const FrailtyAssessmentDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.frailty_assessment) return Array.isArray(r.frailty_assessment) ? r.frailty_assessment : [r.frailty_assessment];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.frailty_assessment) return Array.isArray(dd.frailty_assessment) ? dd.frailty_assessment : [dd.frailty_assessment]; return [dd]; }
      return [r];
    });
  ...
  // The draft store keys are DB fieldParts ("findings" | "results.outcome" | "recommendations"); we
  // reconstruct each root field's render value (localEdits[`${root}-${idx}`]) by applying the dotted path.
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (dotIdx === -1) {
          // whole-field draft (string / boolean / recommendations array)
          nLocal[editKey] = value;
        } else {
          // nested object leaf draft → merge onto the root object
          const path = fieldPart.slice(dotIdx + 1).split('.');
          const base = nLocal[editKey] !== undefined ? nLocal[editKey]
            : (record[rootField] !== undefined ? record[rootField] : {});
        ...
        // mark the field edited so the section shows yellow Pending Approve (matches save markers)
        if (OBJECT_FIELDS.includes(rootField)) {
          nFields[`${rootField}-${idx}-${fieldPart.slice(dotIdx + 1)}`] = 'edited';
        } else if (OBJECT_ARRAY_FIELDS.includes(rootField)) {
          nSentences[`${rootField}-${idx}-r0`] = 'edited';
        } else {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `frailty assessment ${idx + 1}`.toLowerCase();
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.outcome) — value stays a STRING */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    const dottedField = `${rootField}.${path.join('.')}`;
    setSaveError(null);
    const editKey = `${rootField}-${idx}`;
    // Stage the merged root object as a DRAFT (no DB write). Approve commits the dotted field.
    setLocalEdits(prev => {
    ...
    setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
    // localStorage draft keyed by the DB dotted field so Approve can PUT it as-is (survives refresh).
    const store = readDrafts();
    if (!store[id]) store[id] = {};
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Collect this record's staged drafts whose ROOT field belongs to this section, and PUT each now.
      const store = readDrafts();
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/frailty_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedFieldParts.push(fieldPart);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/frailty_assessment/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending for this section's root fields → committed edits now flow into pdfData/PDF
      const committedRoots = new Set(committedFieldParts.map(fp => { const d = fp.indexOf('.'); return d === -1 ? fp : fp.slice(0, d); }));
      setPendingEdits(prev => { const n = { ...prev }; committedRoots.forEach(rf => delete n[`${rf}-${idx}`]); return n; });
    ...
    } catch (err) { console.error('[FrailtyAssessment] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  /* sentence field → parseLabel sub-label + comma-split numbered items (mirrors JSX renderSentence) */
  ...
  const copyAllText = useCallback(async () => {
    let text = `Frailty Assessment\n${COPY_LINE_EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Frailty Assessment ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  /* ═══════ RENDER: LABELED comma sub-card / plain sentence row (shared by string field) ═══════ */
  const renderSentence = (record, fn, idx, sid, sentence, sIdx, phraseMatch, labelMatch) => {
        ...
        return (
          <div key={sIdx} className="rec-mini-card" style={{ marginTop: 8 }}>
            ...
            {commaItems.map((ci, ciIdx) => {
              ...
              if (searchTerm.trim() && !(phraseMatch || labelMatch || parsed.label.toLowerCase().includes(phrase) || ci.toLowerCase().includes(phrase))) return null;
              return (
                <div key={ciIdx}>
                  <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                    {ciEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); const p2 = parseLabel(sentences2[sIdx] || ''); if (!p2.isLabeled) return; const items2 = splitByComma(p2.value); const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) items2.splice(ciIdx, 1, ...subParts); else items2[ciIdx] = trimmed.replace(/[;.]+$/, '').trim(); sentences2[sIdx] = `${p2.label}: ${items2.join(', ')}.`; const fullText2 = reconstructFullText(sentences2); setSaveError(null); const lk = `${fn}-${idx}`; const st = readDrafts(); if (!st[id2]) st[id2] = {}; st[id2][fn] = fullText2; writeDrafts(st); setLocalEdits(prev => ({ ...prev, [lk]: fullText2 })); setPendingEdits(prev => ({ ...prev, [lk]: true })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const marks = { [commaKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${sentenceKey}-c${ciIdx + ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
    ...
    /* labeled-single → sub-label + value row; unlabeled → plain row */
    const isEditing = editingField === sentenceKey;
    const badge = editedSentences[sentenceKey];
    const shown = parsed.isLabeled ? parsed.value : sentence;
    return (
        ...
        <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              ...
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const id3 = safeId(record); if (!id3) return; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = `${parsed.label}: ${editValue.trim().replace(/[;.]+$/, '')}`; const fullText = reconstructFullText(sentences2); const lk = `${fn}-${idx}`; const st = readDrafts(); if (!st[id3]) st[id3] = {}; st[id3][fn] = fullText; writeDrafts(st); setLocalEdits(prev => ({ ...prev, [lk]: fullText })); setPendingEdits(prev => ({ ...prev, [lk]: true })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); setEditedSentences(prev => ({ ...prev, [sentenceKey]: 'edited' })); setEditingField(null); setEditValue(''); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
            <>
              <div className="row-content"><span className="content-value">{highlightText(shown)}</span><span className="edit-indicator">&#9998;</span></div>
  ...
  /* ═══════ RENDER: STRING FIELD (per-sentence, parseLabel-aware) ═══════ */
  const renderStringField = (record, fn, idx, sid) => {
    ...
    const useLabeledPath = sentences.length > 1 || (sentences.length === 1 && parseLabel(sentences[0]).isLabeled);
    if (useLabeledPath) {
      ...
      return (
        <div key={fn} className="rec-mini-card">
          ...
          {sentences.map((sentence, sIdx) => {
            ...
            return renderSentence(record, fn, idx, sid, sentence, sIdx, phraseMatch, labelMatch);
          })}
        </div>
      );
    }

    /* Single-value unlabeled string */
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
    const isModified = editedFields[editKey];
    return (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="frailty-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Frailty Assessment</h2></div>
        <div className="empty-state">No frailty assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="frailty-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Frailty Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FrailtyAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Frailty_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search frailty assessment records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Frailty Assessment ${idx + 1}`)}</h3>
            </div>
...
export default FrailtyAssessmentDocument;
