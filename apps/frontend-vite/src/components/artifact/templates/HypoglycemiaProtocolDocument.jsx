/**
 * HypoglycemiaProtocolDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: hypoglycemia_protocol
 *
 * 5 Sections:
 *   1. presentation: bloodGlucoseLevel, hypoglycemiaSeverity, glasgowComaScale, symptomOnset, lastMealTime, precipitatingFactor
 *   2. symptoms: neurologicalSymptoms, autonomicSymptoms, hypoglycemiaUnawareness
 *   3. treatment: dextroseAdministered, dextroseDoseAmount, dextroseConcentration, glucagonAdministered, glucagonDose, oralCarbohydrateGiven, carbohydrateAmount, routeOfAdministration
 *   4. response: treatmentResponse, followUpGlucoseLevel, recoveryTimeMinutes
 *   5. context: diabetesType, continuousGlucoseMonitor, chronicKidneyDisease, estimatedGFR, currentMedications
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HypoglycemiaProtocolDocumentPDFTemplate from '../pdf-templates/HypoglycemiaProtocolDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './HypoglycemiaProtocolDocument.css';
...
const DRAFT_KEY = 'hypoglycemia_protocolPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  bloodGlucoseLevel: 'Blood Glucose (mg/dL)',
  hypoglycemiaSeverity: 'Hypoglycemia Severity',
  ...
  dextroseAdministered: 'Dextrose Administered',
  dextroseDoseAmount: 'Dextrose Dose (g)',
  dextroseConcentration: 'Dextrose Concentration',
  glucagonAdministered: 'Glucagon Administered',
  glucagonDose: 'Glucagon Dose (mg)',
  oralCarbohydrateGiven: 'Oral Carbohydrate Given',
  carbohydrateAmount: 'Carbohydrate Amount (g)',
  routeOfAdministration: 'Route of Administration',
  treatmentResponse: 'Treatment Response',
  followUpGlucoseLevel: 'Follow-Up Glucose (mg/dL)',
  recoveryTimeMinutes: 'Recovery Time (min)',
  diabetesType: 'Diabetes Type',
  continuousGlucoseMonitor: 'Continuous Glucose Monitor',
  chronicKidneyDisease: 'Chronic Kidney Disease',
  estimatedGFR: 'eGFR (mL/min/1.73m²)',
  currentMedications: 'Current Medications',
};

const SECTION_FIELDS = {
  'presentation': ['bloodGlucoseLevel', 'hypoglycemiaSeverity', 'glasgowComaScale', 'symptomOnset', 'lastMealTime', 'precipitatingFactor'],
  'symptoms': ['neurologicalSymptoms', 'autonomicSymptoms', 'hypoglycemiaUnawareness'],
  'treatment': ['dextroseAdministered', 'dextroseDoseAmount', 'dextroseConcentration', 'glucagonAdministered', 'glucagonDose', 'oralCarbohydrateGiven', 'carbohydrateAmount', 'routeOfAdministration'],
  'response': ['treatmentResponse', 'followUpGlucoseLevel', 'recoveryTimeMinutes'],
  'context': ['diabetesType', 'continuousGlucoseMonitor', 'chronicKidneyDisease', 'estimatedGFR', 'currentMedications'],
};

const NUMBER_FIELDS = ['bloodGlucoseLevel', 'glasgowComaScale', 'dextroseDoseAmount', 'glucagonDose', 'followUpGlucoseLevel', 'estimatedGFR', 'carbohydrateAmount', 'recoveryTimeMinutes'];
const ARRAY_FIELDS = ['neurologicalSymptoms', 'autonomicSymptoms', 'currentMedications'];
const BOOLEAN_FIELDS = ['dextroseAdministered', 'glucagonAdministered', 'hypoglycemiaUnawareness', 'chronicKidneyDisease', 'oralCarbohydrateGiven', 'continuousGlucoseMonitor'];
const DATE_FIELDS = ['symptomOnset', 'lastMealTime'];
const STRING_FIELDS = ['hypoglycemiaSeverity', 'dextroseConcentration', 'routeOfAdministration', 'treatmentResponse', 'precipitatingFactor', 'diabetesType'];
...
/* ═══════ COMPONENT ═══════ */
const HypoglycemiaProtocolDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.hypoglycemia_protocol) return Array.isArray(r.hypoglycemia_protocol) ? r.hypoglycemia_protocol : [r.hypoglycemia_protocol];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.hypoglycemia_protocol) return Array.isArray(dd.hypoglycemia_protocol) ? dd.hypoglycemia_protocol : [dd.hypoglycemia_protocol]; return [dd]; }
      return [r];
    });
  ...
  /* number presence: render ANY numeric value INCLUDING 0 (0 is a recorded reading) */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Hypoglycemia Protocol ${idx + 1}`.toLowerCase();
  ...
  // stageDraft: stage a full-field value locally + persist to the draft store (no DB write).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // localEdits keys are `${field}-${idx}` (value = full field value: string OR full array).
      ...
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "field" — no numeric dot-suffix in this template's localEdits keys
        ...
        const resp = await secureApiClient.put(`/api/edit/hypoglycemia_protocol/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/hypoglycemia_protocol/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HYPOGLYCEMIA PROTOCOL ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Hypoglycemia Protocol ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="hypoglycemia-protocol-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Hypoglycemia Protocol</h2></div>
        <div className="empty-state">No hypoglycemia protocol data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="hypoglycemia-protocol-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Hypoglycemia Protocol</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HypoglycemiaProtocolDocumentPDFTemplate document={pdfData} />} fileName="Hypoglycemia_Protocol.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search hypoglycemia protocol..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Hypoglycemia Protocol ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'presentation')}
            {renderSection(record, idx, 'symptoms')}
            {renderSection(record, idx, 'treatment')}
            {renderSection(record, idx, 'response')}
            {renderSection(record, idx, 'context')}
          </div>
        ))}
      </div>
    </div>
...
export default HypoglycemiaProtocolDocument;
