/**
 * DoctorsMedicationsRecommendationsOptimizationsDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: doctors_medications_recommendations_optimizations
 *
 * Header: date + provider + facility
 * 5 Sections:
 *   1. current-therapy: patientMedicalRecordNumber, currentMedicationName, currentDosage, currentRouteOfAdministration, indicationForTherapy, therapeuticClass, durationOfCurrentTherapy
 *   2. optimization: optimizationType, recommendedMedicationName, recommendedDosage, recommendedRouteOfAdministration, rationaleForOptimization, targetTherapeuticGoal
 *   3. safety: adverseDrugReactionHistory, drugInteractionsConcerns, contraindications, renalFunctionCreatinineClearance, hepaticFunction
 *   4. monitoring-plan: monitoringParametersRequired, patientAdherenceAssessment, titrationSchedule, followUpTimeframe
 *   5. evidence-status: costEffectivenessConsideration, evidenceGradeLevel, acceptanceStatus
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import DoctorsMedicationsRecommendationsOptimizationsDocumentPDFTemplate from '../pdf-templates/DoctorsMedicationsRecommendationsOptimizationsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './DoctorsMedicationsRecommendationsOptimizationsDocument.css';
...
const DRAFT_KEY = 'doctors_medications_recommendations_optimizationsPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  currentMedicationName: 'Current Medication Name',
  currentDosage: 'Current Dosage',
  currentRouteOfAdministration: 'Current Route of Administration',
  indicationForTherapy: 'Indication for Therapy',
  therapeuticClass: 'Therapeutic Class',
  durationOfCurrentTherapy: 'Duration of Current Therapy',
  optimizationType: 'Optimization Type',
  recommendedMedicationName: 'Recommended Medication Name',
  recommendedDosage: 'Recommended Dosage',
  recommendedRouteOfAdministration: 'Recommended Route of Administration',
  rationaleForOptimization: 'Rationale for Optimization',
  targetTherapeuticGoal: 'Target Therapeutic Goal',
  adverseDrugReactionHistory: 'Adverse Drug Reaction History',
  drugInteractionsConcerns: 'Drug Interactions & Concerns',
  contraindications: 'Contraindications',
  renalFunctionCreatinineClearance: 'Renal Function (CrCl)',
  hepaticFunction: 'Hepatic Function',
  monitoringParametersRequired: 'Monitoring Parameters Required',
  patientAdherenceAssessment: 'Patient Adherence Assessment',
  titrationSchedule: 'Titration Schedule',
  followUpTimeframe: 'Follow-Up Timeframe',
  costEffectivenessConsideration: 'Cost-Effectiveness Consideration',
  evidenceGradeLevel: 'Evidence Grade / Level',
  acceptanceStatus: 'Acceptance Status',
};

const SECTION_FIELDS = {
  'current-therapy': ['patientMedicalRecordNumber', 'currentMedicationName', 'currentDosage', 'currentRouteOfAdministration', 'indicationForTherapy', 'therapeuticClass', 'durationOfCurrentTherapy'],
  'optimization': ['optimizationType', 'recommendedMedicationName', 'recommendedDosage', 'recommendedRouteOfAdministration', 'rationaleForOptimization', 'targetTherapeuticGoal'],
  'safety': ['adverseDrugReactionHistory', 'drugInteractionsConcerns', 'contraindications', 'renalFunctionCreatinineClearance', 'hepaticFunction'],
  'monitoring-plan': ['monitoringParametersRequired', 'patientAdherenceAssessment', 'titrationSchedule', 'followUpTimeframe'],
  'evidence-status': ['costEffectivenessConsideration', 'evidenceGradeLevel', 'acceptanceStatus'],
};

const DATE_FIELDS = ['date'];
const NUMBER_FIELDS = ['renalFunctionCreatinineClearance'];
const ARRAY_FIELDS = ['adverseDrugReactionHistory', 'drugInteractionsConcerns', 'contraindications', 'monitoringParametersRequired'];
const STRING_FIELDS = ['provider', 'facility', 'patientMedicalRecordNumber', 'currentMedicationName', 'currentDosage', 'currentRouteOfAdministration', 'indicationForTherapy', 'therapeuticClass', 'durationOfCurrentTherapy', 'optimizationType', 'recommendedMedicationName', 'recommendedDosage', 'recommendedRouteOfAdministration', 'rationaleForOptimization', 'hepaticFunction', 'targetTherapeuticGoal', 'patientAdherenceAssessment', 'costEffectivenessConsideration', 'evidenceGradeLevel', 'titrationSchedule', 'followUpTimeframe', 'acceptanceStatus'];
...
/* ═══════ COMPONENT ═══════ */
const DoctorsMedicationsRecommendationsOptimizationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.doctors_medications_recommendations_optimizations) return Array.isArray(r.doctors_medications_recommendations_optimizations) ? r.doctors_medications_recommendations_optimizations : [r.doctors_medications_recommendations_optimizations];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.doctors_medications_recommendations_optimizations) return Array.isArray(dd.doctors_medications_recommendations_optimizations) ? dd.doctors_medications_recommendations_optimizations : [dd.doctors_medications_recommendations_optimizations]; return [dd]; }
      return [r];
    });
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1))) {
          // array element draft → mark array-item edited (matches `${fn}.${itemIdx}-${idx}`)
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Medication Recommendation ${idx + 1}`.toLowerCase();
      ...
      if (hasVal(record.provider) && String(record.provider).toLowerCase().includes(phrase)) return true;
      if (hasVal(record.facility) && String(record.facility).toLowerCase().includes(phrase)) return true;
    ...
    return filteredRecords.map((record, idx) => {
      ...
      Object.keys(localEdits).forEach(key => {
        ...
        if (m && parseInt(m[2]) === idx) {
          const fn = m[1];
          // pending drafts stay OUT of the PDF until approved. localEdits keys a whole
          // string/array under `${fn}-${idx}`; array-item drafts are tracked under the
          // dotted `${fn}.${itemIdx}-${idx}` pending key, so check both shapes.
          if (pendingEdits[key]) return;
          if (Object.keys(pendingEdits).some(pk => pendingEdits[pk] && pk.startsWith(`${fn}.`) && pk.endsWith(`-${idx}`))) return;
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const field = isArrayElem ? fieldPart.slice(0, lastDot) : fieldPart;
        if (isArrayElem) {
          // localEdits holds the whole edited array under `${field}-${idx}`; persist the element value.
          const arrIndex = parseInt(fieldPart.slice(lastDot + 1), 10);
          const arr = localEdits[`${field}${suffix}`];
          const elemVal = Array.isArray(arr) ? arr[arrIndex] : localEdits[editKey];
          await secureApiClient.put(`/api/edit/doctors_medications_recommendations_optimizations/${id}/edit`, { field, value: elemVal, arrayIndex: arrIndex });
        } else {
          await secureApiClient.put(`/api/edit/doctors_medications_recommendations_optimizations/${id}/edit`, { field, value: localEdits[editKey] });
        }
      }
      await secureApiClient.put(`/api/edit/doctors_medications_recommendations_optimizations/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MEDICATION RECOMMENDATIONS & OPTIMIZATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Medication Recommendation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="doctors-medications-recommendations-optimizations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Medication Recommendations &amp; Optimizations</h2></div>
        <div className="empty-state">No medication recommendations data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="doctors-medications-recommendations-optimizations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Medication Recommendations &amp; Optimizations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<DoctorsMedicationsRecommendationsOptimizationsDocumentPDFTemplate document={pdfData} />} fileName={`medication-recommendations-optimizations-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            ...
            <div className="record-header">
              {(hasVal(record.date) || hasVal(record.provider) || hasVal(record.facility)) && (
                <div className="record-meta-row">
                  ...
                  {hasVal(record.provider) && <span className="record-provider">{highlightText(fmtVal(record.provider))}</span>}
                  {hasVal(record.facility) && <span className="record-facility">{highlightText(fmtVal(record.facility))}</span>}
                </div>
              )}
              <h3 className="record-name">{highlightText(`Medication Recommendation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'current-therapy')}
            {renderSection(record, idx, 'optimization')}
            {renderSection(record, idx, 'safety')}
            {renderSection(record, idx, 'monitoring-plan')}
            {renderSection(record, idx, 'evidence-status')}
          </div>
        ))}
      </div>
    </div>
...
export default DoctorsMedicationsRecommendationsOptimizationsDocument;
