 ...
 * Tracks security events, analyzes patterns, and generates alerts.
 * 
 * SECURITY: Uses SecureDataAccess for all database operations.
 * COMPLIANCE: Maintains detailed audit trails for HIPAA requirements.
 */
...
class SecurityAuditService {
   ...
   */
  async storeSecurityEvent(auditEntry) {
    try {
      const context = {
        serviceId: 'security-audit-service',
        operation: 'store-security-event',
        practiceId: 'global'
      };

      await SecureDataAccess.create('security_audit_log', auditEntry, context);
    } catch (error) {
   ...
   */
  async checkAnomalies(auditEntry) {
  ...
  async triggerAlert(alert) {
    ...
    try {
      const context = {
        serviceId: 'security-audit-service',
        operation: 'store-security-alert',
        practiceId: 'global'
      };

      await SecureDataAccess.create('security_alerts', alertEntry, context);
    } catch (error) {
   ...
   */
  async generateSecurityReport(timeWindowMs = 24 * 60 * 60 * 1000) { // 24 hours
    try {
      const cutoff = new Date(Date.now() - timeWindowMs);
      
      const context = {
        serviceId: 'security-audit-service',
        operation: 'generate-security-report',
        practiceId: 'global'
      };

      const recentEvents = await SecureDataAccess.query('security_audit_log', 
  ...
  async saveAuditLog() {
    try {
      const logPath = path.join(__dirname, '../../../backend/logs/security-audit.log');
...
if (typeof module !== 'undefined' && module.exports) {
  const proxy = getServiceProxy();
  proxy.registerService('securityAuditService', () => securityAuditService);
}
