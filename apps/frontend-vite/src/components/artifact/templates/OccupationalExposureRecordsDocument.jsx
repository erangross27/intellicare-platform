/**
 * OccupationalExposureRecordsDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: occupational_exposure_records
 *
 * Header: date
 * 5 Sections:
 *   1. exposure-event: exposureType, exposureDateTime, exposureLocation, exposureRoute, exposureSeverity, exposureCircumstances
 *   2. source-agent: sourcePatientIdentifier, sourcePatientInfectionStatus, chemicalAgentName, sharpsDeviceType
 *   3. employee-ppe: employeeJobTitle, departmentOfExposure, personalProtectiveEquipmentUsed, immediateDecontaminationPerformed
 *   4. prophylaxis-serology: baselineSerologyOrdered, postExposureProphylaxisInitiated, pepMedicationRegimen, hepatitisBVaccinationStatus, hbigAdministered, followUpSchedule
 *   5. reporting-risk: oshaRecordableInjury, workersCompensationClaim, supervisorNotified, infectionControlNotified, riskAssessmentScore
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import OccupationalExposureRecordsDocumentPDFTemplate from '../pdf-templates/OccupationalExposureRecordsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './OccupationalExposureRecordsDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'exposure-event': 'Exposure Event',
  'source-agent': 'Source & Agent',
  'employee-ppe': 'Employee & PPE',
  'prophylaxis-serology': 'Prophylaxis & Serology',
...
const STRING_FIELDS = ['exposureType', 'exposureLocation', 'exposureRoute', 'sourcePatientIdentifier', 'sourcePatientInfectionStatus', 'chemicalAgentName', 'sharpsDeviceType', 'exposureSeverity', 'employeeJobTitle', 'departmentOfExposure', 'exposureCircumstances', 'pepMedicationRegimen', 'hepatitisBVaccinationStatus', 'workersCompensationClaim'];
...
/* datetime helpers — exposureDateTime is a true timestamp (time-of-day matters) */
...
/* Stored timestamp -> <input type="datetime-local"> value "YYYY-MM-DDTHH:mm" (local, lossless to the minute) */
...
/* datetime-local value -> storage shape "YYYY-MM-DDTHH:mm:ss" (local time, lossless) */
...
const DRAFT_KEY = 'occupational_exposure_recordsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const OccupationalExposureRecordsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.occupational_exposure_records) return Array.isArray(r.occupational_exposure_records) ? r.occupational_exposure_records : [r.occupational_exposure_records];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.occupational_exposure_records) return Array.isArray(dd.occupational_exposure_records) ? dd.occupational_exposure_records : [dd.occupational_exposure_records]; return [dd]; }
      return [r];
    });
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Map each record's draft (keyed by record _id, handling _id.$oid) onto its render index. */
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        const isArrayElem = dotIdx !== -1 && /^\d+$/.test(lastSeg);
        if (isArrayElem) {
          // Array element draft → editKey "field.arrayIndex-idx"; localEdits holds the merged array.
          const field = fieldPart.slice(0, dotIdx);
          const arrIndex = parseInt(lastSeg, 10);
          const editKey = `${fieldPart}-${idx}`;
          nPending[editKey] = true;
          nFields[editKey] = 'edited';
          const baseArr = Array.isArray(record[field]) ? [...record[field]] : [];
          baseArr[arrIndex] = value;
          nLocal[`${field}-${idx}`] = baseArr;
        } else {
          // Whole-field draft → editKey "field-idx".
          const editKey = `${fieldPart}-${idx}`;
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Occupational Exposure Record ${idx + 1}`.toLowerCase();
  ...
  // Stage a single-sentence edit as a DRAFT (no DB write). localStorage keeps it across refresh; Approve commits.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Collect this section's pending edits. localEdits is keyed "field-idx"; pendingEdits may be keyed
      // "field-idx" (whole field) or "field.arrayIndex-idx" (array element). Both map to localEdits["field-idx"].
      const suffix = `-${idx}`;
      const pendingKeys = Object.keys(pendingEdits).filter(k => pendingEdits[k] && k.endsWith(suffix));
      const toCommit = []; // { field, arrayIndex?, value, pendingKey }
      for (const pendingKey of pendingKeys) {
        const fieldPart = pendingKey.slice(0, -suffix.length); // "field" or "field.arrayIndex"
        ...
        if (isArrayElem) {
          // Array element edit: persist the WHOLE merged array element. localEdits holds the full array.
          const arrIndex = parseInt(lastSeg, 10);
          const localKey = `${baseField}${suffix}`;
          const arr = localEdits[localKey];
          const value = Array.isArray(arr) ? arr[arrIndex] : undefined;
          toCommit.push({ field: baseField, arrayIndex: arrIndex, value, pendingKey });
          seenLocalArray.add(localKey);
        } else {
          toCommit.push({ field: baseField, value: localEdits[pendingKey], pendingKey, localKey: pendingKey });
        }
      }
      // Persist each staged edit to the DB now
      for (const c of toCommit) {
        const payload = { field: c.field, value: c.value };
        if (typeof c.arrayIndex === 'number') payload.arrayIndex = c.arrayIndex;
        const resp = await secureApiClient.put(`/api/edit/occupational_exposure_records/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/occupational_exposure_records/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(c => delete n[c.pendingKey]); return n; });
      // Drop this section's staged drafts from localStorage (now committed)
      const store = readDrafts();
      if (store[id]) {
        for (const c of toCommit) {
          if (typeof c.arrayIndex === 'number') delete store[id][`${c.field}.${c.arrayIndex}`];
          else delete store[id][c.field];
        }
      ...
      console.error('[OccupationalExposureRecords] Approve error:', err);
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== OCCUPATIONAL EXPOSURE RECORD ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Occupational Exposure Record ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: DATETIME FIELD (datetime-local, lossless) ═══════ */
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(toDateTimeLocal(val)); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              ...
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (!editValue || isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date/time'); return; } handleSaveField(record, fn, idx, sid, null, fromDateTimeLocal(editValue)); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="occupational-exposure-records-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Occupational Exposure Record</h2></div>
        <div className="empty-state">No occupational exposure record data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="occupational-exposure-records-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Occupational Exposure Record</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OccupationalExposureRecordsDocumentPDFTemplate document={pdfData} />} fileName="Occupational_Exposure_Records.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search occupational exposure record..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Occupational Exposure Record ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'exposure-event')}
            {renderSection(record, idx, 'source-agent')}
            {renderSection(record, idx, 'employee-ppe')}
            {renderSection(record, idx, 'prophylaxis-serology')}
            {renderSection(record, idx, 'reporting-risk')}
          </div>
        ))}
      </div>
    </div>
...
export default OccupationalExposureRecordsDocument;
