/**
 * MonitoringPlansDocumentPDFTemplate.jsx
 * Box-free B&W LETTER (canonical, memory 6a2d6af6) - mirrors the JSX: date via formatDate, narrative
 * strings sentence-split ([.;] with abbrev/single-initial guard + labeled comma-split, values numbered),
 * array-of-strings numbered, single-name label gate. Rule #74: each field is ONE wrap={false} atomic View
...
const SECTION_ORDER = ['plan-overview', 'monitoring-plan'];
...
/* safeString: normalize common typographic glyphs to ASCII. \uXXXX escapes ONLY (no literal non-ASCII bytes). */
  ...
  if (DATE_FIELDS.includes(f)) {
    return [(
      <View key={f} style={styles.fieldGroup} wrap={false}>
        ...
        <Text style={styles.value}>1. {safeString(formatDate(val))}</Text>
      </View>
    )];
  }
...
const MonitoringPlansDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].monitoring_plans && Array.isArray(templateData[0].monitoring_plans)) records = templateData[0].monitoring_plans;
    else records = templateData;
  } else if (templateData && templateData.monitoring_plans) {
    records = Array.isArray(templateData.monitoring_plans) ? templateData.monitoring_plans : [templateData.monitoring_plans];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.monitoring_plans ? (Array.isArray(dd.monitoring_plans) ? dd.monitoring_plans : [dd.monitoring_plans]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Monitoring Plans</Text></View>
        <Text style={styles.emptyState}>No monitoring plans records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Monitoring Plan ${idx + 1}`}</Text>
...
export default MonitoringPlansDocumentPDFTemplate;
