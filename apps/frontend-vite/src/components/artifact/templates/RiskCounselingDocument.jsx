/**
 * RiskCounselingDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: risk_counseling
 *
 * 4 Sections:
 *   1. cardiovascular-metabolic: cardiovascularRiskScore, bleedingRiskScore, osteoporosisFractureRisk, hypertensionTargetOrganDamage, diabeticRetinopathyRisk, diabeticNephropathyRisk
 *   2. organ-systems: fallRiskAssessment, thromboembolismRisk, perioperativeRiskIndex, renalDiseaseProgression, obesityComplicationRisk
 *   3. screening-susceptibility: cancerScreeningRisk, strokeRiskFactors, infectiousDiseaseSusceptibility, polypharmacyRisk, cognitiveDeclineRisk, pregnancyComplicationRisk
 *   4. lifestyle-genetics: lifestyleModificationTargets, geneticPredispositionFactors, anticoagulationRiskBenefit
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import RiskCounselingDocumentPDFTemplate from '../pdf-templates/RiskCounselingDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './RiskCounselingDocument.css';
...
const DRAFT_KEY = 'risk_counselingPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['hypertensionTargetOrganDamage', 'diabeticRetinopathyRisk', 'diabeticNephropathyRisk', 'fallRiskAssessment', 'thromboembolismRisk', 'perioperativeRiskIndex', 'renalDiseaseProgression', 'obesityComplicationRisk', 'polypharmacyRisk', 'cognitiveDeclineRisk', 'pregnancyComplicationRisk', 'anticoagulationRiskBenefit'];
...
/* ═══════ COMPONENT ═══════ */
const RiskCounselingDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.risk_counseling) return Array.isArray(r.risk_counseling) ? r.risk_counseling : [r.risk_counseling];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.risk_counseling) return Array.isArray(dd.risk_counseling) ? dd.risk_counseling : [dd.risk_counseling]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Risk Counseling ${idx + 1}`.toLowerCase();
  ...
  const stageDraft = (record, fn, idx, sid, fullText, sentenceMarks) => {
    const id = safeId(record); if (!id) return;
    const editKey = `${fn}-${idx}`;
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/risk_counseling/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/risk_counseling/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== RISK COUNSELING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Risk Counseling ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="risk-counseling-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Risk Counseling</h2></div>
        <div className="empty-state">No risk counseling records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="risk-counseling-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Risk Counseling</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<RiskCounselingDocumentPDFTemplate document={pdfData} />} fileName={`risk-counseling-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search risk counseling..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Risk Counseling ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'cardiovascular-metabolic')}
            {renderSection(record, idx, 'organ-systems')}
            {renderSection(record, idx, 'screening-susceptibility')}
            {renderSection(record, idx, 'lifestyle-genetics')}
          </div>
        ))}
      </div>
    </div>
...
export default RiskCounselingDocument;
