/**
 * RenalAnemiaDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: renal_anemia
 *
 * 12 Sections:
 *   1. record-info: date, type, provider, facility, status
 *   2. hemoglobin-status: hemoglobin, hemoglobinTarget
 *   3. iron-studies: ironStudies.ferritin, ironStudies.tsat, ironStudies.tibc
 *   4. esa-therapy: esaTherapy.agent, esaTherapy.dose, esaTherapy.frequency, esaTherapy.response
 *   5. iron-therapy: ironTherapy.route, ironTherapy.agent, ironTherapy.dose
 *   6. transfusion-history: transfusionHistory (array)
 *   7. results-section: results (recursive object)
 *   8. findings-section: findings
 *   9. assessment-section: assessment
 *  10. plan-section: plan
 *  11. recommendations-section: recommendations (array of {recommendation, date})
 *  12. notes-section: notes
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import RenalAnemiaDocumentPDFTemplate from '../pdf-templates/RenalAnemiaDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './RenalAnemiaDocument.css';
...
const DRAFT_KEY = 'renalAnemiaPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'hemoglobin-status': 'Hemoglobin Status',
  'iron-studies': 'Iron Studies',
  'esa-therapy': 'ESA Therapy',
...
const FIELD_LABELS = {
  ...
  hemoglobinTarget: 'Hemoglobin Target',
  'ironStudies.ferritin': 'Ferritin',
  'ironStudies.tsat': 'TSAT',
  'ironStudies.tibc': 'TIBC',
  'esaTherapy.agent': 'Agent',
  'esaTherapy.dose': 'Dose',
  'esaTherapy.frequency': 'Frequency',
  'esaTherapy.response': 'Response',
  'ironTherapy.route': 'Route',
  'ironTherapy.agent': 'Agent',
  'ironTherapy.dose': 'Dose',
...
const SECTION_FIELDS = {
  ...
  'hemoglobin-status': ['hemoglobin', 'hemoglobinTarget'],
  'iron-studies': ['ironStudies.ferritin', 'ironStudies.tsat', 'ironStudies.tibc'],
  'esa-therapy': ['esaTherapy.agent', 'esaTherapy.dose', 'esaTherapy.frequency', 'esaTherapy.response'],
  'iron-therapy': ['ironTherapy.route', 'ironTherapy.agent', 'ironTherapy.dose'],
  'transfusion-history': ['transfusionHistory'],
  'results-section': ['results'],
  'findings-section': ['findings'],
  'assessment-section': ['assessment'],
  'plan-section': ['plan'],
...
const STRING_FIELDS = ['type', 'provider', 'facility', 'status', 'hemoglobin', 'hemoglobinTarget', 'ironStudies.ferritin', 'ironStudies.tsat', 'ironStudies.tibc', 'esaTherapy.agent', 'esaTherapy.dose', 'esaTherapy.frequency', 'esaTherapy.response', 'ironTherapy.route', 'ironTherapy.agent', 'ironTherapy.dose', 'findings', 'assessment', 'plan', 'notes'];
...
/* ═══════ COMPONENT ═══════ */
const RenalAnemiaDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.renal_anemia) return Array.isArray(r.renal_anemia) ? r.renal_anemia : [r.renal_anemia];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.renal_anemia) return Array.isArray(dd.renal_anemia) ? dd.renal_anemia : [dd.renal_anemia]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Renal Anemia ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of editKeysForSection) {
        ...
        for (const [dbField, dbValue] of Object.entries(payloads)) {
          const resp = await secureApiClient.put(`/api/edit/renal_anemia/${id}/edit`, { field: dbField, value: dbValue });
          if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        }
      }
      await secureApiClient.put(`/api/edit/renal_anemia/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[RenalAnemia] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const formatSentenceFieldLines = useCallback((text) => {
    ...
    sentences.forEach(sentence => {
      ...
      const parts = (parsed.isLabeled && commaParts.length >= 2) || (!parsed.isLabeled && commaParts.length >= 3) ? commaParts : [source];
      if (parsed.isLabeled) {
  ...
  const copyAllText = useCallback(async () => {
    let text = 'RENAL ANEMIA\n\n';
    pdfData.forEach((r, idx) => {
      text += `Renal Anemia ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    const savePart = (row, rowKey, replacement) => {
      ...
      const commaParts = splitByComma(source);
      const currentParts = (parsed.isLabeled && commaParts.length >= 2) || (!parsed.isLabeled && commaParts.length >= 3) ? commaParts : [source];
  ...
  /* ═══════ RENDER: TRANSFUSION HISTORY (array of objects) ═══════ */
  const renderTransfusionHistory = (record, idx, sid) => {
    const val = getFieldValue(record, 'transfusionHistory', idx);
    if (!Array.isArray(val) || val.length === 0) return null;
    if (searchTerm.trim() && !fieldMatches(record, 'transfusionHistory', idx) && !sectionTitleMatches(sid)) return null;

    return (
      <div key="transfusionHistory" className="rec-mini-card">
        {val.map((transfusion, tIdx) => {
          return (
            <div key={tIdx} className="nested-mini-card transfusion-group">
              <div className="nested-subtitle">{`Transfusion ${tIdx + 1}`}</div>
              {['date', 'units', 'type', 'reason'].map(field => {
                const fieldPath = `transfusionHistory.${tIdx}.${field}`;
                const editKey = `${fieldPath}-${idx}`;
                const sourceValue = localEdits[editKey] !== undefined ? localEdits[editKey] : transfusion?.[field];
                if (isEmptyDeep(sourceValue)) return null;
                const leafKey = `transfusion-${idx}-${tIdx}-${field}`;
                const isEditing = editingField === leafKey;
                const isModified = editedFields[editKey];
                const isDate = field === 'date';
                const isNumber = typeof sourceValue === 'number';
                const displayValue = isDate ? formatDate(sourceValue) : fmtScalar(sourceValue);
                return (
                  <div key={field} className="nested-mini-card" data-edit-field={fieldPath}>
                    <div className="nested-subtitle sub-label">{humanizeKey(field)}</div>
                    <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(leafKey); setEditValue(isDate ? toInputDate(sourceValue) : String(sourceValue)); setSaveError(null); } }}>
                      {isEditing ? (
                        <div className="edit-field-container">
                          {isDate ? (
                            <BlueDatePicker value={editValue} onSelect={value => { setEditValue(value || ''); setSaveError(null); }} />
                            ...
                            <div className="num-stepper-row">
                              <button type="button" className="num-step" onClick={event => { event.stopPropagation(); setEditValue(String((Number.parseFloat(editValue) || 0) - 1)); }}>−</button>
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={event => {
                              ...
                              if (isDate) {
                                if (isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date'); return; }
                                nextValue = String(editValue).includes('T') ? editValue : `${editValue}T00:00:00.000Z`;
                              } else if (isNumber) {
                                nextValue = Number.parseFloat(editValue);
                                if (!Number.isFinite(nextValue)) { setSaveError('Please enter a valid number'); return; }
                              }
                              setEditedFields(prev => ({ ...prev, [editKey]: 'edited' }));
                              stageDraft(record, idx, sid, fieldPath, nextValue, fieldPath, nextValue);
                            }}>{saving ? 'Saving...' : 'Save'}</button>
                            <button className="cancel-btn" onClick={event => { event.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                          </div>
                        </div>
                        ...
                        <>
                          ...
                          <button className={`copy-btn ${copiedItems[leafKey] ? 'copied' : ''}`} onClick={event => { event.stopPropagation(); copyItem(displayValue, leafKey); }}>{copiedItems[leafKey] ? 'Copied!' : 'Copy'}</button>
                        </>
                      )}
                    </div>
                    {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
    ...
    return (
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            if (f === 'transfusionHistory') return renderTransfusionHistory(record, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="renal-anemia-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Renal Anemia</h2></div>
        <div className="empty-state">No renal anemia records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="renal-anemia-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Renal Anemia</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<RenalAnemiaDocumentPDFTemplate document={pdfData} />} fileName="Renal_Anemia.pdf" className="copy-btn">
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Renal Anemia ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'record-info')}
            {renderSection(record, idx, 'hemoglobin-status')}
            {renderSection(record, idx, 'iron-studies')}
            {renderSection(record, idx, 'esa-therapy')}
            {renderSection(record, idx, 'iron-therapy')}
            {renderSection(record, idx, 'transfusion-history')}
...
export default RenalAnemiaDocument;
