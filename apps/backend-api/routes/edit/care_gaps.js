...
function buildContext(req, operation = 'read') {
  return {
    serviceId:   'care-gaps-edit-service',
    userId:      req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update(
      'care_gaps',
      { _id: objectId },
      { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } },
      context,
    );

    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[care_gaps] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    console.error(`[care_gaps] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
