/**
 * ImmediateRecommendationsDocument.jsx
 * June 2026 — Inline editing, blue glow theme
 * Collection: immediate_recommendations
 *
 * 6 Sections:
 *   1. urgency-overview: urgentReferralRequired, emergentInterventionNeeded, criticalValueAlert
 *   2. protocol-activations: sepsisBundleInitiation, strokeCodeActivation, stemiProtocolActivation, insulinDripInitiation
 *   3. airway-circulation-support: airwayManagementNeeded, mechanicalVentilationConsideration, vasopressorRequirement, transfusionRequirement, anticoagulationReversal, dialysisConsultation
 *   4. critical-values-vitals: glasgowComaScale, systolicBloodPressure, oxygenSaturation, troponinLevel, creatinineLevel, potassiumLevel
 *   5. medication-discontinuation: medicationDiscontinuation[]
 *   6. monitoring-plan: acuteSymptomMonitoring, cardiacMonitoringDuration, neurologicalChecksFrequency
 *
 * NOTE: this schema has NO date field — record header shows ONLY the numbered record title.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ImmediateRecommendationsDocumentPDFTemplate from '../pdf-templates/ImmediateRecommendationsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './ImmediateRecommendationsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'immediateRecommendationsPendingEdits';
const readDrafts = () => {
...
const SIMPLE_FIELDS = ['glasgowComaScale', 'systolicBloodPressure', 'oxygenSaturation', 'troponinLevel', 'creatinineLevel', 'potassiumLevel'];
...
/* Numeric fields where 0 means "not measured / not flagged" — hidden when exactly 0.
   ALL 6 numerics: GCS minimum is 3, potassium 0 mEq/L and oxygen saturation 0% are
   incompatible with measurement, SBP 0 / troponin 0 / creatinine 0 are extraction
   sentinels for "not part of this recommendation".
   Real decimals (e.g. creatinineLevel 0.78, troponin 0.04) still display. */
...
/* parseLabel: detect "Label: value" patterns (skip subordinate-clause openers so a grammatical
   colon in "If X: do Y" is never mistaken for a Label:Value pair) */
...
/* splitByComma: parenthesis-aware comma split with thousands guard
   — split ONLY when the comma is followed by whitespace, so "1,000" / "250,4" stay intact;
   items like "altered mental status (asterixis, confusion)" also stay whole (depth-aware) */
...
/* ═══════ COMPONENT ═══════ */
const ImmediateRecommendationsDocument = ({ document: docProp, data, templateData }) => {
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Draft store is keyed by record id; fieldPart is "field" (scalar/sentence) or "field.arrayIndex" (array item). */
  useEffect(() => {
    ...
    records.forEach((record) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayItem) {
          ...
          const editKey = `${fn}-${oi}`;
          const base = Array.isArray(nLocal[editKey]) ? nLocal[editKey] : (Array.isArray(record[fn]) ? [...record[fn]] : []);
          base[ai] = value;
          nLocal[editKey] = base;
          nPending[editKey] = true;
          nFields[`${fn}.${ai}-${oi}`] = 'edited';
        } else {
          const fn = fieldPart;
          const editKey = `${fn}-${oi}`;
          nLocal[editKey] = value;
          nPending[editKey] = true;
          nSentences[`${fn}-${oi}-s0`] = 'edited';
        }
      });
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      const docTitle = 'immediate recommendations';
      const rt = `immediate recommendations ${oi + 1}`;
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* stageDraft: write a single pending edit to the localStorage draft store (survives refresh).
     fieldPart = "field" (scalar/sentence) or "field.arrayIndex" (array item). NO DB write. */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Persist each staged field for THIS section to the DB now. Draft store fieldPart is
      // "field" (scalar/sentence) or "field.arrayIndex" (array item — reverses stageDraft).
      const store = readDrafts();
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/immediate_recommendations/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedFieldParts.push(fieldPart);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/immediate_recommendations/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== IMMEDIATE RECOMMENDATIONS ===\n\n';
    pdfData.forEach((r) => {
      const oi = r._originalIdx ?? 0;
      text += `Immediate Recommendations ${oi + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: SIMPLE FIELD (TYPED numeric — critical values and vital parameters save REAL numbers via parseFloat+isNaN) ═══════ */
  const renderSimpleField = (record, fn, idx, sid) => {
    ...
    /* saveNumber: validate + persist as a REAL number (not a string) — shared by Save button AND Ctrl+Enter */
    const saveNumber = () => { const n = parseFloat(editValue); if (isNaN(n)) { setSaveError('Please enter a valid number'); return; } handleSaveField(record, fn, idx, sid, null, n); };
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                ...
                <input type="text" inputMode="decimal" className="edit-number" value={editValue} autoFocus onChange={e => setEditValue(e.target.value)} onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter') { e.stopPropagation(); saveNumber(); } }} />
        ...
        <div key={fn}>
          <div className="rec-mini-card">
            ...
            {singleCommaItems.map((ci, ciIdx) => {
              ...
              const savePlainCommaItem = () => { const id = safeId(record); if (!id) return; const allItems = splitByComma(String(getFieldValue(record, fn, idx) || '')); allItems[ciIdx] = editValue.trim(); const fullText = allItems.join(', '); setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullText })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); setEditedSentences(prev => ({ ...prev, [commaKey]: 'edited' })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); stageDraft(record, fn, fullText); setEditingField(null); setEditValue(''); };
              return (
                <div key={ciIdx}>
                  <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                    {ciEditing ? (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="immediate-recommendations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Immediate Recommendations</h2></div>
        <div className="empty-state">No immediate recommendations records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="immediate-recommendations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Immediate Recommendations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ImmediateRecommendationsDocumentPDFTemplate document={pdfData} />} fileName="Immediate_Recommendations.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search immediate recommendations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          return (
            <div key={safeId(record) || idx} className="record-card">
              ...
              <div className="record-header">
                <h3 className="record-name">{highlightText(`Immediate Recommendations ${origIdx + 1}`)}</h3>
              </div>
              {renderSection(record, origIdx, 'urgency-overview')}
              {renderSection(record, origIdx, 'protocol-activations')}
              {renderSection(record, origIdx, 'airway-circulation-support')}
              {renderSection(record, origIdx, 'critical-values-vitals')}
              {renderSection(record, origIdx, 'medication-discontinuation')}
              {renderSection(record, origIdx, 'monitoring-plan')}
            </div>
          );
        })}
      </div>
...
export default ImmediateRecommendationsDocument;
