...
class BottleneckDetectorService {
  ...
  loadConfig() {
    ...
    this.minRepetitions = automationConfig?.minRepetitions || 10;
    this.minTimeSaving = automationConfig?.minTimeSaving || 300000; // 5 minutes
    ...
    try {
      ...
      for (const pattern of userPatterns) {
        ...
        if (bottleneck) {
          ...
          analysis.metrics.totalTimeLost += bottleneck.timeLost;
        }
      ...
      for (const bottleneck of analysis.bottlenecks) {
        await this.eventBus.emit(LEARNING_EVENTS.BOTTLENECK_DETECTED, bottleneck);
      }
  ...
  async getClinicUserPatterns(practiceId) {
    // Aggregate patterns from all users in practice
    const patterns = [];
    
    // Get all users for practice (simplified - would query database)
  ...
  async getClinicUsers(practiceId) {
    // Placeholder - would query database for practice users
