/**
 * Workers Comp Evaluations Edit Route
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'workers-comp-evaluations-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'injuryMechanism',
  'anatomicalSiteInjured',
  'occupationalDisease',
  'painScaleNumerical',
  'rangeOfMotionMeasurements',
  'manualMuscleTestGrades',
  'neurologicalDeficits',
  'phalemsTest',
  'tinelsSign',
  'quickDashScore',
  'oswestryDisabilityIndex',
  'maxMedicalImprovement',
  'permanentPartialImpairment',
  'workRestrictions',
  'returnToWorkStatus',
  'functionalCapacityEvaluation',
  'causationOpinion',
  'apportionmentAnalysis',
  'independentMedicalExam',
  'treatmentNecessity',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('workers_comp_evaluations', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[workers_comp_evaluations] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('workers_comp_evaluations', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[workers_comp_evaluations] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
