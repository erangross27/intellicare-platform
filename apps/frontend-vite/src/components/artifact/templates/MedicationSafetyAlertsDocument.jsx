/**
 * MedicationSafetyAlertsDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: medication_safety_alerts (displayed as "Medication Safety Alerts")
 *
 * 7 Sections:
 *   1. alert-information: alertId, alertSeverityLevel, adverseEventType, reportingSource
 *   2. medication-details: medicationName, ndcNumber, therapeuticClass, dosageFormulation, routeOfAdministration
 *   3. dosing-therapeutic-monitoring: prescribedDosage, dosageFrequency, therapeuticRange
 *   4. drug-interactions-contraindications: contraindicatedMedications, drugInteractionSeverity, allergyReactionType
 *   5. patient-factors: patientAgeGroup, renalFunction, creatinineClearance, hepaticFunction
 *   6. safety-warnings: blackBoxWarning, pregnancyCategory, naranjoScore
 *   7. lot-expiration: lotNumber, expirationDate
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MedicationSafetyAlertsDocumentPDFTemplate from '../pdf-templates/MedicationSafetyAlertsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './MedicationSafetyAlertsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits field name; value
   may be a string/number/boolean or a full array for array fields). */
const DRAFT_KEY = 'medication_safety_alertsPendingEdits';
const readDrafts = () => {
...
/* MEANINGFUL_ZERO_FIELDS: numeric fields where 0 is a valid clinical value → always shown even at 0.
   Empty for this template: both naranjoScore 0 and creatinineClearance 0 are "not recorded" sentinels
   (0 = no Naranjo assessment; 0 mL/min is physiologically impossible) → hidden when 0 unless doctor-edited. */
...
const STRING_FIELDS = ['alertId', 'alertSeverityLevel', 'adverseEventType', 'reportingSource', 'medicationName', 'ndcNumber', 'therapeuticClass', 'dosageFormulation', 'routeOfAdministration', 'prescribedDosage', 'dosageFrequency', 'therapeuticRange', 'drugInteractionSeverity', 'allergyReactionType', 'patientAgeGroup', 'renalFunction', 'hepaticFunction', 'pregnancyCategory', 'lotNumber'];
...
/* ═══════ COMPONENT ═══════ */
const MedicationSafetyAlertsDocument = ({ document: docProp }) => {
  ...
  /* numberShows: hide numeric "not recorded" (0) unless meaningful-zero field OR doctor-edited (local edit this session OR persisted doctorEdits) */
  const numberShows = useCallback((record, fn, idx, v) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Medication Safety Alert ${idx + 1}`.toLowerCase();
  ...
  function clearApproveForSection(sid, idx) {
    if (!sid) return;
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      clearApproveForSection(sid, idx);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Pending edits for THIS section's fields (localEdits key = "<field>-<idx>"; value may be a full array)
    ...
    try {
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // plain field name (no dotted arrayIndex convention here)
        const resp = await secureApiClient.put(`/api/edit/medication_safety_alerts/${id}/edit`, { field: fieldPart, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/medication_safety_alerts/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    const EQ = '='.repeat(40); const DASH = '-'.repeat(40);
    const fields = SECTION_FIELDS[sid] || [];
    let body = '';
    fields.forEach(f => {
      const label = FIELD_LABELS[f] || f;
      const val = getFieldValue(record, f, idx);
      if (NUMBER_FIELDS.includes(f)) { if (!numberShows(record, f, idx, val)) return; }
    ...
    if (!body.trim()) return '';
    return `${title}\n${EQ}\n\n${body}`;
  }, [getFieldValue, hasVal, fmtVal, splitBySentence, numberShows]);

  const copyAllText = useCallback(async () => {
    const EQ = '='.repeat(40);
    let text = `Medication Safety Alerts\n${EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Medication Safety Alert ${idx + 1}\n${EQ}\n\n`;
      Object.keys(SECTION_FIELDS).forEach(sid => { const sec = buildSectionCopyText(r, idx, sid); if (sec) text += sec; });
      text += '\n';
    });
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: currentArr })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); clearApproveForSection(sid, idx); stageDraft(id2, fn, currentArr); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* Single-value string: simple editable — uses saveSentence */
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
    const isModified = editedFields[editKey];
  ...
  if (!records || records.length === 0) {
    return (
      <div className="medication-safety-alerts-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Medication Safety Alerts</h2></div>
        <div className="empty-state">No medication safety alerts available</div>
      </div>
    );
  }
  ...
  return (
    <div className="medication-safety-alerts-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Medication Safety Alerts</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MedicationSafetyAlertsDocumentPDFTemplate document={pdfData} />} fileName="Medication_Safety_Alerts.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search medication safety alerts..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Medication Safety Alert ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'alert-information')}
            {renderSection(record, idx, 'medication-details')}
            {renderSection(record, idx, 'dosing-therapeutic-monitoring')}
            {renderSection(record, idx, 'drug-interactions-contraindications')}
            {renderSection(record, idx, 'patient-factors')}
            {renderSection(record, idx, 'safety-warnings')}
            {renderSection(record, idx, 'lot-expiration')}
          </div>
        ))}
      </div>
    </div>
...
export default MedicationSafetyAlertsDocument;
