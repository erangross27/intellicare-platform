/**
 * NeuroImagingDocument.jsx
 * Canonical one-pass polish — inline editing, blue glow theme, box-free PDF.
 * Collection: neuro_imaging
 *
 * 9 Sections:
 *   1. study-info: date, studyType, anatomicalRegion, radiologist, facility
...
import NeuroImagingDocumentPDFTemplate from '../pdf-templates/NeuroImagingDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './NeuroImagingDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'neuro_imagingPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  'study-info': ['date', 'studyType', 'anatomicalRegion', 'radiologist', 'facility'],
  'indication': ['indication'],
  'technique': ['technique'],
  'contrast': ['contrast'],
  'findings': ['findings'],
...
/* No numeric fields in this collection — num-stepper handler kept for parity. */
const NUMBER_FIELDS = [];
const STRING_FIELDS = ['studyType', 'anatomicalRegion', 'radiologist', 'facility', 'indication', 'technique', 'contrast', 'findings', 'impression', 'comparison', 'recommendations', 'notes'];
...
/* ═══════ COMPONENT ═══════ */
const NeuroImagingDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.neuro_imaging) return Array.isArray(r.neuro_imaging) ? r.neuro_imaging : [r.neuro_imaging];
      if (r?.document?.neuro_imaging) return Array.isArray(r.document.neuro_imaging) ? r.document.neuro_imaging : [r.document.neuro_imaging];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.neuro_imaging) return Array.isArray(dd.neuro_imaging) ? dd.neuro_imaging : [dd.neuro_imaging]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Neuro Imaging Study ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/neuro_imaging/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/neuro_imaging/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NEURO IMAGING STUDIES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Neuro Imaging Study ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    const sentences = splitBySentence(strVal);
    const label = FIELD_LABELS[fn] || fn;
    // Single-name gate: when the field label equals its section title, suppress the redundant nested-subtitle.
    const suppressLabel = (SECTION_TITLES[sid] || '').trim().toLowerCase() === label.trim().toLowerCase();
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
      ...
      return (
        <div key={fn}>
          <div className="rec-mini-card">
            {!suppressLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
  ...
  if (!records || records.length === 0) {
    return (
      <div className="neuro-imaging-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Neuro Imaging Studies</h2></div>
        <div className="empty-state">No neuro imaging records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="neuro-imaging-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Neuro Imaging Studies</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NeuroImagingDocumentPDFTemplate document={pdfData} />} fileName="Neuro_Imaging_Studies.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search neuro imaging studies..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Neuro Imaging Study ${idx + 1}`)}</h3>
            </div>
...
export default NeuroImagingDocument;
