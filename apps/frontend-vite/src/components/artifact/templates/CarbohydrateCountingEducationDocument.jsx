/**
 * CarbohydrateCountingEducationDocument.jsx
...
import CarbohydrateCountingEducationDocumentPDFTemplate from '../pdf-templates/CarbohydrateCountingEducationDocumentPDFTemplate';
import './CarbohydrateCountingEducationDocument.css';
...
const DRAFT_KEY = 'carbohydrate_counting_educationPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  overview: ['patientDiabetesType', 'baselineHbA1c', 'carbohydrateCountingCompetency', 'educationSessionDuration'],
  insulin: ['currentInsulinRegimen', 'rapidActingInsulinType', 'basalInsulinType', 'insulinToCarbohydrateRatio', 'insulinSensitivityFactor'],
  glucose: ['targetGlucoseRange', 'glucometerAccuracy', 'continuousGlucoseMonitor', 'timeInRange', 'glycemicVariability'],
  skills: ['nutritionLabelReadingSkills', 'portionSizeEstimation', 'glycemicIndexKnowledge'],
  medical: ['hypoglycemiaAwareness', 'severeHypoglycemiaHistory', 'diabeticKetoacidosisHistory', 'gastroparesisPresence'],
  mealTiming: ['mealTimingPatterns'],
  objectives: ['learningObjectivesAchieved'],
...
const ARRAY_FIELDS = ['mealTimingPatterns', 'learningObjectivesAchieved'];
const NUMBER_FIELDS = ['baselineHbA1c', 'educationSessionDuration', 'insulinSensitivityFactor', 'timeInRange', 'glycemicVariability', 'severeHypoglycemiaHistory', 'diabeticKetoacidosisHistory'];
const BOOLEAN_FIELDS = ['nutritionLabelReadingSkills', 'glycemicIndexKnowledge', 'gastroparesisPresence'];
...
const CarbohydrateCountingEducationDocument = ({ document: rawDoc }) => {
  ...
  // fieldPart "field" -> localEdits key `field-idx`; "field.arrayIndex" -> `field-idx-arrayIndex`.
  useEffect(() => {
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, valueOverride) => {
    const rid = getRecordId(record);
    if (!rid) { console.error('[CarbCounting] Cannot save — no record ID'); return; }
  ...
  // Save array element = stage a DRAFT locally only (localStorage survives refresh). Approve commits.
  ...
  const handleApproveSection = useCallback(async (record, idx, sid) => {
    ...
    try {
      ...
      const toCommit = Object.keys(localEdits).filter(k => {
        ...
        // localEdits key: `field-idx` (field/sentence) or `field-idx-arrayIndex` (array element)
        const parts = k.split('-');
        const last = parts[parts.length - 1];
        const prev = parts[parts.length - 2];
        let fn, recIdx;
        if (/^\d+$/.test(last) && /^\d+$/.test(prev)) { fn = parts.slice(0, -2).join('-'); recIdx = parseInt(prev, 10); }
        else if (/^\d+$/.test(last)) { fn = parts.slice(0, -1).join('-'); recIdx = parseInt(last, 10); }
        else return false;
        return recIdx === idx && sf.includes(fn);
      });
      for (const editKey of toCommit) {
        const parts = editKey.split('-');
        const last = parts[parts.length - 1];
        const prev = parts[parts.length - 2];
        const isArr = /^\d+$/.test(last) && /^\d+$/.test(prev);
        const fn = isArr ? parts.slice(0, -2).join('-') : parts.slice(0, -1).join('-');
        const payload = { field: fn, value: localEdits[editKey] };
        if (isArr) payload.arrayIndex = parseInt(last, 10);
        await sc.put(`/api/edit/carbohydrate_counting_education/${rid}/edit`, payload);
      }
      await sc.put(`/api/edit/carbohydrate_counting_education/${rid}/approve`, { sectionId: sid, approved: true });
      ...
      if (toCommit.length > 0) {
        ...
        if (store[rid]) {
          toCommit.forEach(editKey => {
            const parts = editKey.split('-');
            const last = parts[parts.length - 1];
            const prevp = parts[parts.length - 2];
            const isArr = /^\d+$/.test(last) && /^\d+$/.test(prevp);
            const fn = isArr ? parts.slice(0, -2).join('-') : parts.slice(0, -1).join('-');
            delete store[rid][isArr ? `${fn}.${last}` : fn];
          });
    ...
    } catch (err) { console.error('[CarbCounting] Approve failed:', err); }
  }, [localEdits, pendingEdits]);

  // Save one sentence = splice it back into the full text and stage as a DRAFT locally only
  // (localStorage survives refresh). NOT written to MongoDB until the user clicks Pending Approve.
  const saveSentence = useCallback((record, fn, idx, sid, sentenceIdx, valueOverride) => {
    const rid = getRecordId(record); if (!rid) { console.error('[CarbCounting] Cannot save — no record ID'); return; }
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      const title = `Carbohydrate Counting Education ${idx + 1}`;
      const allText = [title, ...Object.keys(FIELD_LABELS).map(f => fmtVal(record[f])), ...(Array.isArray(record.mealTimingPatterns) ? record.mealTimingPatterns : []), ...(Array.isArray(record.learningObjectivesAchieved) ? record.learningObjectivesAchieved : []), record.followUpRecommendations, ...Object.values(FIELD_LABELS), 'Diabetes Overview', 'Insulin Regimen', 'Glucose Targets', 'Skills Assessment', 'Medical History', 'Meal Timing', 'Learning Objectives', 'Follow-Up'].filter(Boolean).join(' ');
  ...
  const renderEditableField = (record, fn, idx, sid) => {
    ...
    if (ie) return (<div className="rec-mini-card"><div className="nested-subtitle">{highlightText(FIELD_LABELS[fn] || fn)}</div><div className="edit-field-container"><textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && e.ctrlKey) handleSaveField(record, fn, idx, sid); if (e.key === 'Escape') { setEditingField(null); setEditValue(''); } }} autoFocus rows={fn === 'followUpRecommendations' || fn === 'portionSizeEstimation' || fn === 'currentInsulinRegimen' ? 3 : 1} disabled={saving} /><div className="edit-actions"><button className="save-btn" onClick={() => handleSaveField(record, fn, idx, sid)} disabled={saving}>{saving ? 'Saving...' : 'Save'}</button><button className="cancel-btn" onClick={() => { setEditingField(null); setEditValue(''); }}>Cancel</button></div></div></div>);
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    const raw = getFieldValue(record, fn, idx); if (!hasVal(raw)) return null;
    const dv = fmtVal(raw); const ek = `${fn}-${idx}`; const ie = editingField === ek; const ed = editedFields[ek]; const cid = `row-${fn}-${idx}`;
    if (ie) return (<div className="rec-mini-card"><div className="nested-subtitle">{highlightText(FIELD_LABELS[fn] || fn)}</div><div className="edit-field-container"><input type="number" step="any" className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { const n = parseFloat(editValue); if (!isNaN(n)) handleSaveField(record, fn, idx, sid, n); } if (e.key === 'Escape') { setEditingField(null); setEditValue(''); } }} autoFocus disabled={saving} /><div className="edit-actions"><button className="save-btn" onClick={() => { const n = parseFloat(editValue); if (isNaN(n)) return; handleSaveField(record, fn, idx, sid, n); }} disabled={saving}>{saving ? 'Saving...' : 'Save'}</button><button className="cancel-btn" onClick={() => { setEditingField(null); setEditValue(''); }}>Cancel</button></div></div></div>);
    return (<div className="rec-mini-card"><div className="nested-subtitle">{highlightText(FIELD_LABELS[fn] || fn)}</div><div className={`numbered-row editable-row${ed ? ' modified' : ''}`} onClick={() => { setEditingField(ek); setEditValue(dv); }}><div className="row-content"><span className="content-value">{highlightText(dv)}</span>{!ed && <span className="edit-indicator">✎</span>}</div><button className={`copy-btn${copiedId === cid ? ' copied' : ''}`} onClick={(e) => { e.stopPropagation(); copyToClipboard(dv, cid); }}>{copiedId === cid ? 'Copied' : 'Copy'}</button></div>{ed && <div className="modified-badge">edited - click Pending Approve to save</div>}</div>);
  };
  ...
  const renderBooleanField = (record, fn, idx, sid) => {
    const raw = getFieldValue(record, fn, idx); if (!hasVal(raw)) return null;
    const dv = fmtVal(raw); const ek = `${fn}-${idx}`; const ie = editingField === ek; const ed = editedFields[ek]; const cid = `row-${fn}-${idx}`;
    if (ie) return (<div className="rec-mini-card"><div className="nested-subtitle">{highlightText(FIELD_LABELS[fn] || fn)}</div><div className="edit-field-container"><select className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); } }} autoFocus disabled={saving}><option value="Yes">Yes</option><option value="No">No</option></select><div className="edit-actions"><button className="save-btn" onClick={() => handleSaveField(record, fn, idx, sid, editValue === 'Yes')} disabled={saving}>{saving ? 'Saving...' : 'Save'}</button><button className="cancel-btn" onClick={() => { setEditingField(null); setEditValue(''); }}>Cancel</button></div></div></div>);
    return (<div className="rec-mini-card"><div className="nested-subtitle">{highlightText(FIELD_LABELS[fn] || fn)}</div><div className={`numbered-row editable-row${ed ? ' modified' : ''}`} onClick={() => { setEditingField(ek); setEditValue(raw ? 'Yes' : 'No'); }}><div className="row-content"><span className="content-value">{highlightText(dv)}</span>{!ed && <span className="edit-indicator">✎</span>}</div><button className={`copy-btn${copiedId === cid ? ' copied' : ''}`} onClick={(e) => { e.stopPropagation(); copyToClipboard(dv, cid); }}>{copiedId === cid ? 'Copied' : 'Copy'}</button></div>{ed && <div className="modified-badge">edited - click Pending Approve to save</div>}</div>);
  };
  ...
  const pdfData = useMemo(() => {
    ...
    return records.map((record, idx) => {
      ...
      // Array fields: overlay only committed (non-pending) element edits.
      ARRAY_FIELDS.forEach(field => {
        const arr = Array.isArray(record[field]) ? [...record[field]] : [];
        arr.forEach((_, ai) => {
          const ek = `${field}-${idx}-${ai}`;
          if (localEdits[ek] !== undefined && !pendingEdits[ek]) arr[ai] = localEdits[ek];
        });
        m[field] = arr;
      });
      return m;
  ...
  const SECTION_TITLES = { overview: 'DIABETES OVERVIEW', insulin: 'INSULIN REGIMEN', glucose: 'GLUCOSE TARGETS', skills: 'SKILLS ASSESSMENT', medical: 'MEDICAL HISTORY', mealTiming: 'MEAL TIMING', objectives: 'LEARNING OBJECTIVES', followUp: 'FOLLOW-UP RECOMMENDATIONS' };
  ...
  const copySectionText = (record, idx, sid) => {
    ...
    const addArr = (fn) => { (Array.isArray(pr[fn]) ? pr[fn] : []).forEach((item, i) => { text += `${i + 1}. ${item}\n`; }); };
    if (sid === 'mealTiming') { addArr('mealTimingPatterns'); }
    else if (sid === 'objectives') { addArr('learningObjectivesAchieved'); }
    else if (sid === 'followUp') { splitBySentence(fmtVal(pr.followUpRecommendations || '')).forEach((s, i) => { text += `${i + 1}. ${s}\n`; }); }
    else { (SECTION_FIELDS[sid] || []).forEach(addF); }
    copyToClipboard(text.trim(), `section-${sid}-${idx}`);
  };

  const copyAllContent = () => {
    let text = '=== CARBOHYDRATE COUNTING EDUCATION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Carbohydrate Counting Education ${idx + 1}\n`;
      const addF = (fn) => { if (hasVal(r[fn])) text += `${FIELD_LABELS[fn] || fn}: ${fmtVal(r[fn])}\n`; };
      Object.entries(SECTION_TITLES).forEach(([sid, title]) => {
        if (sid === 'mealTiming') { const items = Array.isArray(r.mealTimingPatterns) ? r.mealTimingPatterns : []; if (items.length) { text += `\n${title}\n`; items.forEach((it, i) => { text += `${i + 1}. ${it}\n`; }); } }
        else if (sid === 'objectives') { const items = Array.isArray(r.learningObjectivesAchieved) ? r.learningObjectivesAchieved : []; if (items.length) { text += `\n${title}\n`; items.forEach((it, i) => { text += `${i + 1}. ${it}\n`; }); } }
        else if (sid === 'followUp') { if (hasVal(r.followUpRecommendations)) { text += `\n${title}\n`; splitBySentence(fmtVal(r.followUpRecommendations)).forEach((s, i) => { text += `${i + 1}. ${s}\n`; }); } }
        else { const fields = SECTION_FIELDS[sid] || []; const vis = fields.filter(f => hasVal(r[f])); if (vis.length) { text += `\n${title}\n`; vis.forEach(addF); } }
      });
      text += '\n';
    });
  ...
  const renderMultiFieldSection = (record, idx, sid, title, fields) => {
    ...
    return renderSection(record, idx, sid, title, (() => { const stm = sectionTitleMatches(title); const sa = !searchTerm.trim() || record._showAllSections || stm; return <>{visibleFields.map(f => { if (!sa && !fieldMatches(record, f, idx)) return null; const renderer = NUMBER_FIELDS.includes(f) ? renderNumberField : BOOLEAN_FIELDS.includes(f) ? renderBooleanField : renderEditableField; return <React.Fragment key={f}>{renderer(record, f, idx, sid)}</React.Fragment>; })}</>; })());
  };
  ...
  if (!filteredRecords || filteredRecords.length === 0) return (<article className="carb-counting-document"><header className="document-header"><h1 className="document-title">Carbohydrate Counting Education</h1></header><SearchBar searchTerm={searchTerm} onSearchChange={setSearchTerm} /><div className="empty-state">No data available.</div></article>);

  return (
    <article className="carb-counting-document">
      <header className="document-header">
        <h1 className="document-title">Carbohydrate Counting Education</h1>
        <div className="header-actions">
          <button className={`copy-btn${copiedId === 'copy-all' ? ' copied' : ''}`} onClick={copyAllContent}>{copiedId === 'copy-all' ? 'Copied' : 'Copy All'}</button>
          <PDFDownloadLink document={<CarbohydrateCountingEducationDocumentPDFTemplate document={pdfData} />} fileName="Carbohydrate_Counting_Education.pdf">
          ...
          <div key={idx} className="record-card">
            <div className="record-header">
              <div className="record-meta-row">
                {record.patientDiabetesType && <span className="record-date">{highlightText(record.patientDiabetesType)}</span>}
              </div>
              <div className="record-title-row"><h3 className="record-name">{highlightText(`Carbohydrate Counting Education ${idx + 1}`)}</h3></div>
            </div>
            {renderMultiFieldSection(record, idx, 'overview', 'Diabetes Overview', SECTION_FIELDS.overview)}
            {renderMultiFieldSection(record, idx, 'insulin', 'Insulin Regimen', SECTION_FIELDS.insulin)}
            {renderMultiFieldSection(record, idx, 'glucose', 'Glucose Targets', SECTION_FIELDS.glucose)}
            {renderMultiFieldSection(record, idx, 'skills', 'Skills Assessment', SECTION_FIELDS.skills)}
            {renderMultiFieldSection(record, idx, 'medical', 'Medical History', SECTION_FIELDS.medical)}
            {renderArraySection(record, idx, 'mealTiming', 'Meal Timing', 'mealTimingPatterns')}
            {renderArraySection(record, idx, 'objectives', 'Learning Objectives', 'learningObjectivesAchieved')}
            {renderSentenceSplitSection(record, idx, 'followUp', 'Follow-Up Recommendations', 'followUpRecommendations')}
          </div>
        ))}
      </div>
    </article>
...
export default CarbohydrateCountingEducationDocument;
