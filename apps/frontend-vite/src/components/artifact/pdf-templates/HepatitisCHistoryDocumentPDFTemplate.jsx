...
/**
 * HepatitisCHistoryDocumentPDFTemplate - box-free canonical (LETTER)
 * Config-driven from the JSX SECTION_ORDER/SECTION_TITLES/FIELD_LABELS/SECTION_FIELDS.
 * Renders EVERY populated field the JSX renders for JSX/PDF field parity.
 * No boxes: underline rules only (documentTitle 2 / recordTitle+sectionTitle 1 / fieldLabel 0.5).
 * Nested objects (coinfections/results) + arrays render via a generic recursive renderer;
...
const SECTION_ORDER = ['record-info', 'hcv-details', 'coinfections', 'liver-assessment', 'treatment-history', 'findings', 'assessment', 'plan', 'results', 'notes'];
...
const HepatitisCHistoryDocumentPDFTemplate = ({ document: docProp, data = docProp }) => {
  let records = [];
  if (Array.isArray(data)) records = data;
  else if (data && typeof data === 'object') {
    if (data.hepatitis_c_history) records = Array.isArray(data.hepatitis_c_history) ? data.hepatitis_c_history : [data.hepatitis_c_history];
    else records = [data];
  }
    ...
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <Text style={styles.documentTitle}>Hepatitis C History</Text>
          <Text style={styles.noData}>No hepatitis C history records available.</Text>
        </Page>
      </Document>
    );
  }
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, rIdx) => (
          <View key={rIdx}>
            <Text style={styles.recordTitle} break={rIdx > 0}>{safeString(`Hepatitis C History ${rIdx + 1}`)}</Text>
...
export default HepatitisCHistoryDocumentPDFTemplate;
