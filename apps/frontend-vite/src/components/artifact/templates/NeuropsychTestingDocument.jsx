/**
 * NeuropsychTestingDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: neuropsych_testing
 *
 * Sections:
 *   0. record-header: provider, facility  (date rendered in record-header strip)
 *   1. cognitive-screening: cognitiveScreeningScore, mmseScore, mocaScore, clockDrawingTestScore, bostonNamingTestScore
 *   2. executive-attention: trailMakingTestA, trailMakingTestB, stroopTestPerformance, wisconsinCardSortTest, digitSpanPerformance, controlledOralWordAssociation
 *   3. memory-visuospatial: waisIvCompositeScores, wmsIvCompositeScores, reyAuditoryVerbalLearning, reyComplexFigureTest
 *   4. mood-function: depressionInventoryScore, anxietyInventoryScore, functionalAssessmentScale, globalDeteriorationScale, clinicalDementiaRating, neuropsychiatricInventory
 *   5. validity: validityIndicators
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NeuropsychTestingDocumentPDFTemplate from '../pdf-templates/NeuropsychTestingDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './NeuropsychTestingDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'neuropsych_testingPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'memory-visuospatial': 'Memory & Visuospatial',
  'mood-function': 'Mood & Function',
  'validity': 'Validity',
};
...
const FIELD_LABELS = {
  ...
  provider: 'Provider',
  facility: 'Facility',
  cognitiveScreeningScore: 'Cognitive Screening Score',
  mmseScore: 'MMSE Score',
  mocaScore: 'MoCA Score',
  clockDrawingTestScore: 'Clock Drawing Test Score',
  bostonNamingTestScore: 'Boston Naming Test Score',
  trailMakingTestA: 'Trail Making Test A (sec)',
  trailMakingTestB: 'Trail Making Test B (sec)',
  stroopTestPerformance: 'Stroop Test Performance',
  wisconsinCardSortTest: 'Wisconsin Card Sort Test',
  digitSpanPerformance: 'Digit Span Performance',
  controlledOralWordAssociation: 'Controlled Oral Word Association',
  waisIvCompositeScores: 'WAIS-IV Composite Scores',
  wmsIvCompositeScores: 'WMS-IV Composite Scores',
  reyAuditoryVerbalLearning: 'Rey Auditory Verbal Learning',
  reyComplexFigureTest: 'Rey Complex Figure Test',
  depressionInventoryScore: 'Depression Inventory Score',
  anxietyInventoryScore: 'Anxiety Inventory Score',
  functionalAssessmentScale: 'Functional Assessment Scale',
  globalDeteriorationScale: 'Global Deterioration Scale',
  clinicalDementiaRating: 'Clinical Dementia Rating',
  neuropsychiatricInventory: 'Neuropsychiatric Inventory',
  validityIndicators: 'Validity Indicators',
  cognitiveReserveIndicators: 'Cognitive Reserve Indicators',
  cognitiveProfileSummary: 'Cognitive Profile Summary',
  testingAccommodations: 'Testing Accommodations',
};

const SECTION_FIELDS = {
  'record-header': ['provider', 'facility'],
  'cognitive-screening': ['cognitiveScreeningScore', 'mmseScore', 'mocaScore', 'clockDrawingTestScore', 'bostonNamingTestScore'],
  'executive-attention': ['trailMakingTestA', 'trailMakingTestB', 'stroopTestPerformance', 'wisconsinCardSortTest', 'digitSpanPerformance', 'controlledOralWordAssociation'],
  'memory-visuospatial': ['waisIvCompositeScores', 'wmsIvCompositeScores', 'reyAuditoryVerbalLearning', 'reyComplexFigureTest'],
  'mood-function': ['depressionInventoryScore', 'anxietyInventoryScore', 'functionalAssessmentScale', 'globalDeteriorationScale', 'clinicalDementiaRating', 'neuropsychiatricInventory', 'cognitiveReserveIndicators', 'cognitiveProfileSummary'],
  'validity': ['validityIndicators', 'testingAccommodations'],
};

const DATE_FIELDS = [];
const NUMBER_FIELDS = ['cognitiveScreeningScore', 'mmseScore', 'mocaScore', 'trailMakingTestA', 'trailMakingTestB', 'bostonNamingTestScore', 'clockDrawingTestScore', 'depressionInventoryScore', 'anxietyInventoryScore', 'globalDeteriorationScale'];
const ARRAY_FIELDS = ['waisIvCompositeScores', 'wmsIvCompositeScores', 'validityIndicators', 'testingAccommodations'];
/* hide-zero: a numeric 0 is a "not recorded" sentinel for EVERY score in this collection
   (screening/inventory 0 = not administered, not a valid finding) → skip unless doctor-edited. */
const STRING_FIELDS = ['provider', 'facility', 'stroopTestPerformance', 'wisconsinCardSortTest', 'reyAuditoryVerbalLearning', 'controlledOralWordAssociation', 'reyComplexFigureTest', 'digitSpanPerformance', 'functionalAssessmentScale', 'clinicalDementiaRating', 'neuropsychiatricInventory', 'cognitiveReserveIndicators', 'cognitiveProfileSummary'];
...
/* ═══════ COMPONENT ═══════ */
const NeuropsychTestingDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.neuropsych_testing) return Array.isArray(r.neuropsych_testing) ? r.neuropsych_testing : [r.neuropsych_testing];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.neuropsych_testing) return Array.isArray(dd.neuropsych_testing) ? dd.neuropsych_testing : [dd.neuropsych_testing]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Neuropsychological Testing ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      // Persist each staged draft whose base field belongs to this section.
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/neuropsych_testing/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedLocalKeys.add(`${baseField}-${idx}`);
      }
      await secureApiClient.put(`/api/edit/neuropsych_testing/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NEUROPSYCHOLOGICAL TESTING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Neuropsychological Testing ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="neuropsych-testing-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Neuropsychological Testing</h2></div>
        <div className="empty-state">No neuropsychological testing data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="neuropsych-testing-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Neuropsychological Testing</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NeuropsychTestingDocumentPDFTemplate document={pdfData} />} fileName="Neuropsychological_Testing.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search neuropsychological testing..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Neuropsychological Testing ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'record-header')}
            {renderSection(record, idx, 'cognitive-screening')}
            {renderSection(record, idx, 'executive-attention')}
            {renderSection(record, idx, 'memory-visuospatial')}
            {renderSection(record, idx, 'mood-function')}
            {renderSection(record, idx, 'validity')}
          </div>
        ))}
      </div>
    </div>
...
export default NeuropsychTestingDocument;
