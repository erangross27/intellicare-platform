...
const styles = StyleSheet.create({
  ...
  fieldBox: { marginTop: 4, marginBottom: 8, paddingBottom: 8, minHeight: 64, flexDirection: 'column', width: 528 }, fieldHeader: { marginTop: 8, marginBottom: 6, width: 528 }, fieldLabel: { width: 528, fontSize: 13, fontFamily: 'Helvetica-Bold', paddingBottom: 3, marginBottom: 4, borderBottomWidth: 0.5, borderBottomColor: '#999999' },
...
const SECTIONS = [
  { title: 'Session, Diagnosis & Oxygenation', fields: [['therapySessionId', 'Therapy Session ID'], ['primaryDiagnosis', 'Primary Diagnosis', 'sentence'], ['baselineSpO2', 'Baseline SpO2 (%)', 'number'], ['postTherapySpO2', 'Post-Therapy SpO2 (%)', 'number'], ['supplementalOxygenFlowRate', 'Supplemental Oxygen Flow Rate (L/min)', 'number']] },
  { title: 'Chest Physiotherapy', fields: [['chestPhysiotherapyTechnique', 'Chest Physiotherapy Technique', 'sentence']] },
  { title: 'High-Frequency Chest Wall Oscillation', fields: [['highFrequencyChestWallOscillation', 'HFCWO Used', 'boolean'], ['hfcwoFrequencySetting', 'HFCWO Frequency Setting (Hz)', 'number'], ['hfcwoPressureSetting', 'HFCWO Pressure Setting', 'number']] },
  { title: 'Positive Expiratory Pressure', fields: [['positivExpiratoryPressureDevice', 'PEP Device'], ['pepResistanceSetting', 'PEP Resistance Setting (cm H2O)', 'number']] },
  { title: 'IPV & Mechanical Insufflation-Exsufflation', fields: [['intrapulmonaryPercussiveVentilation', 'IPV Used', 'boolean'], ['ipvOperatingPressure', 'IPV Operating Pressure (psi)', 'number'], ['mechanicalInsufflationExsufflation', 'MI-E Used', 'boolean'], ['mieInsufflationPressure', 'MI-E Insufflation Pressure (cm H2O)', 'number'], ['mieExsufflationPressure', 'MI-E Exsufflation Pressure (cm H2O)', 'number']] },
  { title: 'Sputum & Cough Flow', fields: [['sputumProductionVolume', 'Sputum Production Volume (mL)', 'number'], ['sputumCharacteristics', 'Sputum Characteristics', 'sentenceComma'], ['preTherapyPeakCoughFlow', 'Pre-Therapy Peak Cough Flow (L/min)', 'number'], ['postTherapyPeakCoughFlow', 'Post-Therapy Peak Cough Flow (L/min)', 'number']] },
  { title: 'Auscultation Findings', fields: [['auscultationFindingsPreTherapy', 'Pre-Therapy Auscultation', 'sentenceComma'], ['auscultationFindingsPostTherapy', 'Post-Therapy Auscultation', 'sentenceComma']] },
  { title: 'Therapy Duration & Tolerance', fields: [['therapyDurationMinutes', 'Therapy Duration (min)', 'number'], ['posturalDrainagePositions', 'Postural Drainage Positions', 'arrayKeep'], ['patientToleranceBorgScale', 'Borg Scale (0-10)', 'number'], ['adverseEventsObserved', 'Adverse Events', 'arrayKeep']] },
];
...
const HIDE_ZERO_FIELDS = new Set(['baselineSpO2', 'postTherapySpO2', 'hfcwoFrequencySetting', 'hfcwoPressureSetting', 'pepResistanceSetting', 'ipvOperatingPressure', 'mieInsufflationPressure', 'mieExsufflationPressure', 'preTherapyPeakCoughFlow', 'postTherapyPeakCoughFlow', 'therapyDurationMinutes']);
const FIELD_REQUIREMENTS = { hfcwoFrequencySetting: 'highFrequencyChestWallOscillation', hfcwoPressureSetting: 'highFrequencyChestWallOscillation', pepResistanceSetting: 'positivExpiratoryPressureDevice', ipvOperatingPressure: 'intrapulmonaryPercussiveVentilation', mieInsufflationPressure: 'mechanicalInsufflationExsufflation', mieExsufflationPressure: 'mechanicalInsufflationExsufflation' };
const BOOLEAN_REQUIREMENTS = new Set(['highFrequencyChestWallOscillation', 'intrapulmonaryPercussiveVentilation', 'mechanicalInsufflationExsufflation']);
const fieldHasVal = (record, field) => { const value = record?.[field]; if (!hasVal(value)) return false; const requirement = FIELD_REQUIREMENTS[field], controller = requirement ? record?.[requirement] : null; if (requirement && (BOOLEAN_REQUIREMENTS.has(requirement) ? controller !== true : !hasVal(controller))) return false; return !(HIDE_ZERO_FIELDS.has(field) && Number(value) === 0); };
...
const renderField = (config, value, key, sectionTitle) => {
  ...
  const renderRow = (row, index, priorSubtitle) => { const showSubtitle = row.subtitle && row.subtitle !== priorSubtitle; return <View key={`${key}-${index}`} style={styles.rowBlock} wrap={false}>{showSubtitle && <Text style={styles.nestedSubtitle}>{row.subtitle}</Text>}<Text style={styles.fieldValue}>{index + 1}. {row.value}</Text></View>; };
  const renderFirstRow = row => <View key={`${key}-0`} style={styles.rowBlock}>{row.subtitle && <Text style={styles.nestedSubtitle}>{row.subtitle}</Text>}<Text style={styles.fieldValue}>1. {row.value}</Text></View>;
  const first = rows[0];
  return <React.Fragment key={key}>{showLabel ? <View style={styles.fieldBox} wrap={false}><View style={styles.fieldHeader}><Text style={styles.fieldLabel}>{label}</Text></View>{renderFirstRow(first)}</View> : renderRow(first, 0, '')}{rows.slice(1).map((row, offset) => renderRow(row, offset + 1, rows[offset].subtitle || ''))}</React.Fragment>;
};

const AirwayClearanceTherapyDocumentPDFTemplate = ({ document: documentProp, data: dataProp, templateData }) => {
  const records = React.useMemo(() => { const source = documentProp ?? dataProp ?? templateData; if (!source) return []; let rows = Array.isArray(source) ? source : [source]; rows = rows.flatMap(row => { if (Array.isArray(row?.records)) return row.records; if (Array.isArray(row?._records)) return row._records; if (row?.airway_clearance_therapy) return Array.isArray(row.airway_clearance_therapy) ? row.airway_clearance_therapy : [row.airway_clearance_therapy]; if (row?.documentData) { const nested = row.documentData; if (Array.isArray(nested)) return nested; if (nested?.airway_clearance_therapy) return Array.isArray(nested.airway_clearance_therapy) ? nested.airway_clearance_therapy : [nested.airway_clearance_therapy]; return [nested]; } return [row]; }); return rows.filter(row => row && typeof row === 'object'); }, [documentProp, dataProp, templateData]);
  if (!records.length) return <Document><Page size="LETTER" style={styles.page}><View style={styles.documentHeader}><Text style={styles.documentTitle}>Airway Clearance Therapy</Text></View><Text style={styles.noData}>No airway clearance therapy records available</Text></Page></Document>;
  return <Document>{records.flatMap((record, recordIndex) => PAGE_GROUPS.map((indexes, pageIndex) => { const visible = indexes.map(index => ({ section: SECTIONS[index], index, fields: SECTIONS[index].fields.filter(([field]) => fieldHasVal(record, field)) })).filter(item => item.fields.length); if (!visible.length) return null; return <Page key={`${recordIndex}-${pageIndex}`} size="LETTER" style={styles.page}>{pageIndex === 0 && <View style={styles.documentHeader}><Text style={styles.documentTitle}>Airway Clearance Therapy</Text></View>}{pageIndex === 0 && <View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>{record.therapySessionId || `Airway Clearance Therapy Record ${recordIndex + 1}`}</Text></View>}{visible.map(({ section, index, fields }) => <View key={`${index}-${section.title}`} style={styles.section}><View wrap={false}><Text style={styles.sectionTitle}>{section.title}</Text></View>{fields.map((config, fieldIndex) => renderField(config, record[config[0]], `${index}-${fieldIndex}`, section.title))}</View>)}</Page>; }))}</Document>;
};
