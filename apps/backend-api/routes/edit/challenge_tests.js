...
function buildContext(req, operation = 'read') { return { serviceId: 'challenge-tests-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }

const ALLOWED_FIELDS = [
  'testDate', 'supervisedBy', 'threshold',
  'food.status', 'food.indication', 'drug.status', 'drug.indication',
  'aspirin.status', 'aspirin.indication', 'exercise.status', 'exercise.indication',
  'protocol', 'outcome', 'reactions',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('challenge_tests', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[ChallengeTests] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('challenge_tests', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[ChallengeTests] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
