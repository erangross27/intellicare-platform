...
const SECTIONS = [
  { id: 'medicationInformation', title: 'Medication Information', fields: ['name', 'genericName', 'dosage', 'frequency', 'route'] },
  { id: 'prescriberDetails', title: 'Prescriber Details', fields: ['prescriber', 'indication', 'instructions'] },
  { id: 'duration', title: 'Duration', fields: ['startDate', 'endDate', 'duration', 'durationDays', 'durationUnit', 'active', 'refills'] },
  { id: 'safety', title: 'Safety', fields: ['safetyWarning', 'sideEffects', 'drugInteractions'] },
];
const FIELD_LABELS = {
...
export default function ScheduledMedicationsDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Scheduled Medications</Text></View>
    {!records.length && <Text style={styles.noDataText}>No scheduled medications data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Scheduled Medication {recordIndex + 1}</Text></View>
