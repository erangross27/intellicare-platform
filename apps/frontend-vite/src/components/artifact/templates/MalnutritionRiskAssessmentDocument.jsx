/**
 * MalnutritionRiskAssessmentDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: malnutrition_risk_assessment (displayed as "Malnutrition Risk Assessment")
 *
 * 7 Sections:
 *   1. screening-info: screeningToolUsed, totalRiskScore, riskCategory
 *   2. anthropometric-data: bodyMassIndex, unintentionalWeightLoss, weightLossPercentage, weightLossTimeframe, midUpperArmCircumference
 *   3. dietary-intake: reducedDietaryIntake, appetiteLevel, dysphagia, gastrointestinalSymptoms
 *   4. disease-impact: acuteDiseaseEffect, chronicDiseaseImpact, functionalCapacity, sarcopenia, edemaPresent
 *   5. lab-markers: albuminLevel, prealbumin
 *   6. intervention-plan: nutritionInterventionRequired, recommendedInterventions, dietitianReferralMade, reassessmentInterval
 *   7. psychosocial-barriers: psychosocialBarriers, oralHealthIssues
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MalnutritionRiskAssessmentDocumentPDFTemplate from '../pdf-templates/MalnutritionRiskAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './MalnutritionRiskAssessmentDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'malnutrition_risk_assessmentPendingEdits';
const readDrafts = () => {
...
/* MEANINGFUL_ZERO_FIELDS: numbers where 0 is a valid clinical finding (weight-loss 0% = stable weight) → always show.
   All other numbers (totalRiskScore, bodyMassIndex, albuminLevel, prealbumin, midUpperArmCircumference) treat 0 as a
   sentinel "not measured / not scored" and hide it. */
...
const STRING_FIELDS = ['screeningToolUsed', 'riskCategory', 'weightLossTimeframe', 'appetiteLevel', 'chronicDiseaseImpact', 'functionalCapacity', 'reassessmentInterval'];
...
/* ═══════ COMPONENT ═══════ */
const MalnutritionRiskAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.malnutrition_risk_assessment) return Array.isArray(r.malnutrition_risk_assessment) ? r.malnutrition_risk_assessment : [r.malnutrition_risk_assessment];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.malnutrition_risk_assessment) return Array.isArray(dd.malnutrition_risk_assessment) ? dd.malnutrition_risk_assessment : [dd.malnutrition_risk_assessment]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        // localEdits is keyed by base field name; array fields store the whole array value.
        nLocal[`${baseField}-${idx}`] = value;
        if (isArrayElem) {
          const editKey = `${fieldPart}-${idx}`; // "field.arrayIndex-idx"
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Malnutrition Risk Assessment ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const suffix = `-${idx}`;
      // pendingEdits keys: "field-idx" (scalar/sentence) or "field.arrayIndex-idx" (array element).
      ...
      for (const editKey of toCommit) {
        ...
        if (isArrayElem) {
          ...
          const resp = await secureApiClient.put(`/api/edit/malnutrition_risk_assessment/${id}/edit`, { field, value, arrayIndex });
          if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        } else {
          const resp = await secureApiClient.put(`/api/edit/malnutrition_risk_assessment/${id}/edit`, { field: fieldPart, value: localEdits[editKey] });
          if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        }
      }
      await secureApiClient.put(`/api/edit/malnutrition_risk_assessment/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[MalnutritionRiskAssessment] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      const val = getFieldValue(record, f, idx);
      if (NUMBER_FIELDS.includes(f) ? !numberShows(f, val) : !hasVal(val)) return;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MALNUTRITION RISK ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Malnutrition Risk Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx); if (!numberShows(fn, val)) return null;
    const editKey = `${fn}-${idx}`;
  ...
  /* ═══════ RENDER: DATE FIELD (record-header date-picker via BlueDatePicker) ═══════ */
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      if (NUMBER_FIELDS.includes(f)) return numberShows(f, val);
      return hasVal(val);
    });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="malnutrition-risk-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Malnutrition Risk Assessment</h2></div>
        <div className="empty-state">No malnutrition risk assessment data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="malnutrition-risk-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Malnutrition Risk Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MalnutritionRiskAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Malnutrition_Risk_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search malnutrition risk assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Malnutrition Risk Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'screening-info')}
            {renderSection(record, idx, 'anthropometric-data')}
            {renderSection(record, idx, 'dietary-intake')}
            {renderSection(record, idx, 'disease-impact')}
            {renderSection(record, idx, 'lab-markers')}
            {renderSection(record, idx, 'intervention-plan')}
            {renderSection(record, idx, 'psychosocial-barriers')}
          </div>
        ))}
      </div>
    </div>
...
export default MalnutritionRiskAssessmentDocument;
