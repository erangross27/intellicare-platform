/**
 * HivPepProphylaxisDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: hiv_pep_prophylaxis
 *
 * 6 Sections:
 *   1. exposure-info:    exposureDateTime, exposureType, exposureSourceHivStatus, sourceViralLoadCopiesPerMl, hoursFromExposureToPepInitiation
 *   2. risk-assessment:  denverHivRiskScore, pepIndicationRiskCategory, bodyFluidInvolved, exposureSiteAnatomical, needlestickDepthAndGauge, visibleBloodOnDevice, hollowBoreNeedleExposure
 *   3. pep-regimen:      pepRegimen, tenofovirFormulation, integraseInhibitorSelected
 *   4. baseline-labs:    baselineHivAgAbComboTest, baselineHivRnaQualitative, baselineCreatinineMgDl, baselineEstimatedGfrMlMin, baselineHbsAgStatus, baselineHcvAntibodyStatus, stiScreeningPerformed
 *   5. pep-adherence:    pepAdherencePercentage, pepCompletedFullCourse, pepDiscontinuationReason
 *   6. follow-up:        followUpHivTestWeek4To6, followUpHivTestMonth3, prepCandidateAssessed, prepTransitionInitiated
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HivPepProphylaxisDocumentPDFTemplate from '../pdf-templates/HivPepProphylaxisDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './HivPepProphylaxisDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'hiv_pep_prophylaxisPendingEdits';
const readDrafts = () => {
...
const DATE_FIELDS     = ['exposureDateTime', 'prepCandidateAssessed']; // date fields → BlueDatePicker (prepCandidateAssessed may be an epoch sentinel)
...
const STRING_FIELDS   = [
  'exposureType', 'exposureSourceHivStatus', 'pepIndicationRiskCategory', 'bodyFluidInvolved',
  'exposureSiteAnatomical', 'needlestickDepthAndGauge', 'pepRegimen', 'tenofovirFormulation',
  'integraseInhibitorSelected', 'baselineHivAgAbComboTest', 'baselineHivRnaQualitative',
  'baselineHbsAgStatus', 'baselineHcvAntibodyStatus', 'pepDiscontinuationReason',
  'followUpHivTestWeek4To6', 'followUpHivTestMonth3',
];
...
// −/+ stepper granularity: integers step by 1, decimals step by the last decimal place (0.92 → 0.01)
const stepFor = (v) => { const d = String(v ?? '').split('.')[1]; return d ? parseFloat('0.' + '0'.repeat(Math.max(0, d.length - 1)) + '1') : 1; };

/* ═══════ COMPONENT ═══════ */
const HivPepProphylaxisDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.hiv_pep_prophylaxis) return Array.isArray(r.hiv_pep_prophylaxis) ? r.hiv_pep_prophylaxis : [r.hiv_pep_prophylaxis];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.hiv_pep_prophylaxis) return Array.isArray(dd.hiv_pep_prophylaxis) ? dd.hiv_pep_prophylaxis : [dd.hiv_pep_prophylaxis]; return [dd]; }
      return [r];
    });
  ...
  // A Save stages a DRAFT locally + in localStorage so it survives refresh (shown in JSX, NOT in DB/PDF).
  // Map each draft's recordId (handling _id.$oid) back to its render index in `records`.
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // fieldPart = "field" or "field.arrayIndex"; localEdits key convention here is always `${fn}-${idx}`
        const dotIdx = fieldPart.lastIndexOf('.');
        const isArrayPart = dotIdx !== -1 && /^\d+$/.test(fieldPart.slice(dotIdx + 1));
        const fieldName = isArrayPart ? fieldPart.slice(0, dotIdx) : fieldPart;
        const editKey = `${fieldName}-${idx}`;
        nLocal[editKey] = value;
        nPending[editKey] = true;
        if (isArrayPart) nFields[`${fieldName}.${fieldPart.slice(dotIdx + 1)}-${idx}`] = 'edited';
        else { nFields[editKey] = 'edited'; nSentences[`${fieldName}-${idx}-s0`] = 'edited'; }
      });
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `HIV PEP Prophylaxis ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Stage a DRAFT locally + in the pending-drafts localStorage store (survives refresh).
  // NOT written to MongoDB and NOT shown in the PDF until the user clicks Approve (handleApproveSection commits).
  // fieldPart = "field" (full value) or "field.arrayIndex" (single array element). localEdits key is always `${fn}-${idx}`.
  const stageDraft = useCallback((record, fn, idx, value, { fieldPart, sid } = {}) => {
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Commit each staged draft for this section's fields. localEdits key convention is `${fn}-${idx}`,
      // so the trailing dot-segment is never purely numeric here → field-only payloads (full value/array).
      const suffix = `-${idx}`;
      const toCommit = Object.keys(localEdits).filter(k =>
        pendingEdits[k] && k.endsWith(suffix) && fields.includes(k.slice(0, -suffix.length))
      );
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "field" (this file never stages "field.idx")
        const dotIdx = fieldPart.lastIndexOf('.');
        const isArrayPart = dotIdx !== -1 && /^\d+$/.test(fieldPart.slice(dotIdx + 1));
        ...
        const resp = await secureApiClient.put(`/api/edit/hiv_pep_prophylaxis/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/hiv_pep_prophylaxis/${id}/approve`, { sectionId: sid, approved: true });
      ...
      const store = readDrafts();
      if (store[id]) { fields.forEach(f => { delete store[id][f]; Object.keys(store[id]).forEach(k => { if (k.startsWith(`${f}.`)) delete store[id][k]; }); }); if (Object.keys(store[id]).length === 0) delete store[id]; writeDrafts(store); }

      setApprovedSections(prev => ({ ...prev, [`${sid}-${idx}`]: true }));
      setEditedFields(prev => { const n = { ...prev }; Object.keys(n).forEach(k => { fields.forEach(f => { if (k.startsWith(`${f}-${idx}`) || (k.startsWith(`${f}.`) && k.endsWith(`-${idx}`))) delete n[k]; }); }); return n; });
      setEditedSentences(prev => { const n = { ...prev }; Object.keys(n).forEach(k => { fields.forEach(f => { if (k.startsWith(`${f}-${idx}`)) delete n[k]; }); }); return n; });
    } catch (err) { console.error('[HivPepProphylaxis] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HIV PEP PROPHYLAXIS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `HIV PEP Prophylaxis ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderBooleanField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx); if (!hasVal(val)) return null;
    const boolVal    = val === true || val === 'Yes' || val === 'true';
    const editKey    = `${fn}-${idx}`;
    const isEditing  = editingField === editKey;
    const label      = FIELD_LABELS[fn] || fn;
  ...
  /* ═══════ RENDER: NUMBER FIELD — −/+ stepper (never type=number) ═══════ */
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                <button type="button" className="num-step" disabled={saving} onClick={e => { e.stopPropagation(); bump(-stepFor(editValue)); }}>&minus;</button>
                ...
                <button type="button" className="num-step" disabled={saving} onClick={e => { e.stopPropagation(); bump(stepFor(editValue)); }}>+</button>
              </div>
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
  ...
  /* ═══════ RENDER: ARRAY FIELD (stiScreeningPerformed) ═══════ */
      ...
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
            ...
            if (!labelLow.includes(phrase) && !phrase.includes(labelLow) && !itemStr.toLowerCase().includes(phrase)) return null;
          }
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); stageDraft(record, fn, idx, currentArr, { sid }); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="hiv-pep-prophylaxis-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">HIV PEP Prophylaxis</h2></div>
        <div className="empty-state">No HIV PEP prophylaxis records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="hiv-pep-prophylaxis-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">HIV PEP Prophylaxis</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HivPepProphylaxisDocumentPDFTemplate document={pdfData} />} fileName="Hiv_Pep_Prophylaxis.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search HIV PEP prophylaxis..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`HIV PEP Prophylaxis ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'exposure-info')}
            {renderSection(record, idx, 'risk-assessment')}
            {renderSection(record, idx, 'pep-regimen')}
            {renderSection(record, idx, 'baseline-labs')}
            {renderSection(record, idx, 'pep-adherence')}
...
export default HivPepProphylaxisDocument;
