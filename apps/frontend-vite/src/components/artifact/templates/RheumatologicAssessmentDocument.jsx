/**
 * RheumatologicAssessmentDocument.jsx
 * July 2026 — recursive, dot-path editable rheumatologic assessment
...
import RheumatologicAssessmentDocumentPDFTemplate from '../pdf-templates/RheumatologicAssessmentDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './RheumatologicAssessmentDocument.css';

const DRAFT_KEY = 'rheumatologic_assessmentPendingEdits';
const SECTION_TITLES = {
  recordInformation: 'Record Information',
  clinicalPresentation: 'Clinical Presentation',
  morningStiffness: 'Morning Stiffness',
  jointInvolvement: 'Joint Involvement',
...
const FIELD_LABELS = {
  ...
  chiefComplaint: 'Chief Complaint', symptomDuration: 'Symptom Duration', morningStiffness: 'Morning Stiffness', jointInvolvement: 'Joint Involvement', systemicSymptoms: 'Systemic Symptoms',
  findings: 'Findings',
  assessment: 'Assessment', plan: 'Plan', results: 'Results', recommendations: 'Recommendations', notes: 'Notes',
};
const SECTION_FIELDS = {
  recordInformation: ['date', 'type', 'status'],
  clinicalPresentation: ['chiefComplaint', 'symptomDuration'],
  morningStiffness: ['morningStiffness'],
  jointInvolvement: ['jointInvolvement'],
  systemicSymptoms: ['systemicSymptoms'],
  clinicalFindings: ['findings'],
  clinicalAssessment: ['assessment'],
  treatmentPlan: ['plan'],
  results: ['results'],
  recommendations: ['recommendations'],
  additionalNotes: ['notes'],
  providerInformation: ['provider', 'facility'],
};
const DATE_FIELDS = ['date'];
const ENUM_FIELDS = ['status'];
const NUMBER_STRING_FIELDS = [];
const OBJECT_FIELDS = ['morningStiffness', 'jointInvolvement', 'results'];
const NARRATIVE_PATHS = ['jointInvolvement.pattern', 'jointInvolvement.distribution'];
const COMMA_FIELDS = ['jointInvolvement.distribution', 'jointInvolvement.affectedJoints', 'jointInvolvement.synovitis', 'jointInvolvement.deformities', 'systemicSymptoms'];
const COMMA_ARRAY_FIELDS = ['jointInvolvement.affectedJoints', 'jointInvolvement.synovitis', 'jointInvolvement.deformities', 'systemicSymptoms'];
const SEMICOLON_FIELDS = ['symptomDuration', 'jointInvolvement.pattern', 'jointInvolvement.distribution', 'findings', 'assessment', 'plan', 'notes'];
...
const KEY_LABELS = {
  improvesWithActivity: 'Improves With Activity', affectedJoints: 'Affected Joints',
...
const RheumatologicAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.rheumatologic_assessment) return Array.isArray(record.rheumatologic_assessment) ? record.rheumatologic_assessment : [record.rheumatologic_assessment];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.rheumatologic_assessment) return Array.isArray(nested.rheumatologic_assessment) ? nested.rheumatologic_assessment : [nested.rheumatologic_assessment];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/rheumatologic_assessment/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/rheumatologic_assessment/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[RheumatologicAssessment] Approve error:', error);
      ...
      <div className="rec-mini-card nested-mini-card regular-row-group">
        {parts.map((part, partIndex) => {
          ...
          props.onSave = () => {
            ...
            nextParts[partIndex] = editValue.trim();
  ...
  const copyAll = useCallback(async () => {
    let text = '=== RHEUMATOLOGIC ASSESSMENT ===\n\n';
    records.forEach((record, recordIndex) => {
      text += 'Rheumatologic Assessment ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="rheumatologic-assessment-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Rheumatologic Assessment</h2></div><div className="empty-state">No rheumatologic assessment records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Rheumatologic Assessment ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="rheumatologic-assessment-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Rheumatologic Assessment</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<RheumatologicAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Rheumatologic_Assessment.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search rheumatologic assessments..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Rheumatologic Assessment ' + (recordIndex + 1))}</h3></div>
...
export default RheumatologicAssessmentDocument;
