/**
 * MentalStatusExams Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/mental_status_exams/:id/edit    — Edit a single field
 * PUT /api/edit/mental_status_exams/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'mental-status-exams-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('mental_status_exams', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[MentalStatusExams] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('mental_status_exams', { _id: objectId }, {
      $set: {
    ...
    console.error('[MentalStatusExams] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
