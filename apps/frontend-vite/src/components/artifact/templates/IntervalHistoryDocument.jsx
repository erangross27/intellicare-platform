/**
 * IntervalHistoryDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: interval_history (displayed as "Interval History")
 *
 * 5 Sections:
 *   1. record-info: reportType, clinicalIndication
 *   2. findings: findings
 *   3. therapy-lifestyle: physicalTherapy, exercise, sleep, mood
 *   4. recommendations: recommendations
 *   5. follow-up: followUp
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import IntervalHistoryDocumentPDFTemplate from '../pdf-templates/IntervalHistoryDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './IntervalHistoryDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name, full-text value) */
const DRAFT_KEY = 'intervalHistoryPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'findings': 'Findings',
  'therapy-lifestyle': 'Therapy & Lifestyle',
...
const SECTION_FIELDS = {
  'record-info': ['reportDate', 'reportType', 'clinicalIndication'],
  'findings': ['findings'],
  'therapy-lifestyle': ['physicalTherapy', 'exercise', 'sleep', 'mood'],
  'recommendations': ['recommendations'],
  'follow-up': ['followUp'],
};

const DATE_FIELDS = ['reportDate'];
const STRING_FIELDS = ['physicalTherapy', 'speechTherapy', 'exercise', 'sleep', 'mood', 'recommendations', 'reportType', 'clinicalIndication', 'findings', 'urgency', 'followUp'];
...
/* ═══════ COMPONENT ═══════ */
const IntervalHistoryDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.interval_history) return Array.isArray(r.interval_history) ? r.interval_history : [r.interval_history];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.interval_history) return Array.isArray(dd.interval_history) ? dd.interval_history : [dd.interval_history]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Interval History ${idx + 1}`.toLowerCase();
  ...
  /* Stage a DRAFT locally + persist it to the pending-drafts localStorage store (survives refresh).
     NOT written to MongoDB and NOT shown in the PDF until the user clicks Approve. The fieldPart key
     is the bare field name (full-text value); approve reverses it to { field, [arrayIndex] }. */
  const stageDraft = useCallback((record, fn, idx, fullText, sentenceKeys) => {
    const id = safeId(record); if (!id) return;
    setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullText }));
    setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true }));
    if (sentenceKeys && sentenceKeys.length) {
      setEditedSentences(prev => { const n = { ...prev }; sentenceKeys.forEach(([k, v]) => { n[k] = v; }); return n; });
    } else {
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    stageDraft(record, fn, idx, saveVal, editTrackingKey && editTrackingKey !== `${fn}-${idx}` ? [[editTrackingKey, 'edited']] : null);
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    const marks = [];
    if (changed) marks.push([`${fn}-${idx}-s${sentenceIdx}`, 'edited']);
    const extra = newSentences.length - 1;
    for (let ei = 0; ei < extra; ei++) marks.push([`${fn}-${idx}-s${sentenceIdx + 1 + ei}`, 'added']);
    stageDraft(record, fn, idx, fullText, marks.length ? marks : [[`${fn}-${idx}-s${sentenceIdx}`, 'edited']]);
  }
  ...
  function saveCommaItem(record, fn, idx, sid, sIdx, ciIdx) {
    ...
    for (let ei = 1; ei < subParts.length; ei++) marks.push([`${sentenceKey}-c${ciIdx + ei}`, 'added']);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Collect this record+section's staged drafts. localEdits key = "<fieldPart>-<idx>".
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        // Trailing dot-segment is an arrayIndex ONLY when purely numeric (GOTCHA).
        ...
        const resp = await secureApiClient.put(`/api/edit/interval_history/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/interval_history/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const formatSentenceFieldLines = useCallback((text) => {
    const periodItems = splitBySentence(text);
    const sentences = periodItems.length >= 2 ? periodItems : splitBySemicolon(text);
    const lines = []; let n = 1;
    sentences.forEach(s => {
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== INTERVAL HISTORY ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Interval History ${idx + 1}\n${'='.repeat(40)}\n\n`;
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const periodItems2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); const isSemicolon2 = periodItems2.length < 2; const sentences2 = isSemicolon2 ? splitBySemicolon(String(getFieldValue(record, fn, idx) || '')) : periodItems2; sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); const marks = [[sentenceKey, 'edited']]; for (let ei = 1; ei < subParts.length; ei++) marks.push([`${fn}-${idx}-s${sIdx}-c${ei}`, 'added']); stageDraft(record, fn, idx, fullText, marks); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
                  ...
                  <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                    {ciEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const allItems = splitByComma(String(getFieldValue(record, fn, idx) || '')); allItems[ciIdx] = editValue.trim(); const fullText = allItems.join(', '); stageDraft(record, fn, idx, fullText, [[commaKey, 'edited']]); }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="interval-history-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Interval History</h2></div>
        <div className="empty-state">No interval history records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="interval-history-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Interval History</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<IntervalHistoryDocumentPDFTemplate document={pdfData} />} fileName="Interval_History.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search interval history..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Interval History ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'record-info')}
            {renderSection(record, idx, 'findings')}
            {renderSection(record, idx, 'therapy-lifestyle')}
...
export default IntervalHistoryDocument;
