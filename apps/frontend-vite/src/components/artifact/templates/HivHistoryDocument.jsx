/**
 * HivHistoryDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: hiv_history
 *
 * 7 Sections:
 *   1. session-info: date (date picker), diagnosisDate (date picker), provider (string), facility (string)
 *   2. diagnosis-details: diagnosisContext (string), transmissionRoute (string), status (string)
 *   3. cd4-history: nadirCD4 (string), bestCD4 (string)
 *   4. genotype-resistance: genotype (array of strings)
 *   5. prior-ois: priorOIs (array of strings)
 *   6. art-history: artHistory (array of strings)
 *   7. clinical-notes: findings (string/long), assessment (string), plan (string), notes (string)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HivHistoryDocumentPDFTemplate from '../pdf-templates/HivHistoryDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './HivHistoryDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'hiv_historyPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['provider', 'facility', 'diagnosisContext', 'transmissionRoute', 'status', 'nadirCD4', 'bestCD4', 'findings', 'assessment', 'plan', 'notes'];
...
/* ═══════ COMPONENT ═══════ */
const HivHistoryDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.hiv_history) return Array.isArray(r.hiv_history) ? r.hiv_history : [r.hiv_history];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.hiv_history) return Array.isArray(dd.hiv_history) ? dd.hiv_history : [dd.hiv_history]; return [dd]; }
      return [r];
    });
  ...
  const getArrayFieldValue = useCallback((record, fn, arrayIndex, idx) => {
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      const val = ARRAY_FIELDS.includes(f) ? record[f] : getFieldValue(record, f, 0);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `HIV History ${idx + 1}`.toLowerCase();
      if (rt.includes(phrase) || phrase.includes(rt)) { record._showAllSections = true; return true; }
      for (const t of Object.values(SECTION_TITLES)) { if (t.toLowerCase().includes(phrase) || phrase.includes(t.toLowerCase())) return true; }
      for (const l of Object.values(FIELD_LABELS)) { if (l.toLowerCase().includes(phrase) || phrase.includes(l.toLowerCase())) return true; }
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          const val = ARRAY_FIELDS.includes(f) ? record[f] : getFieldValue(record, f, idx);
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* stageDraft — write one pending edit into the localStorage draft store (survives refresh). */
  const stageDraft = useCallback((recordId, fieldPart, value) => {
    const store = readDrafts();
  ...
  const handleSaveArrayItem = useCallback((record, fn, idx, sid, arrayIndex, valueOverride) => {
    ...
    setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
    stageDraft(id, `${fn}.${arrayIndex}`, saveVal);
    setEditingField(null); setEditValue('');
  }, [editValue, safeId, stageDraft]);
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
    // Stage the whole (merged) object under the root field — Approve PUTs it as field=rootField.
    stageDraft(id, rootField, cloneOut);
    setEditingField(null); setEditValue('');
  }, [safeId, stageDraft]);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const lastDot = fieldPart.lastIndexOf('.');
        // arrayIndex ONLY when the segment after the LAST dot is purely numeric (reverse of save's fieldPart)
        const isArrayElem = lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1));
        ...
        const resp = await secureApiClient.put(`/api/edit/hiv_history/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/hiv_history/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[HivHistory] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const objectCopyLines = useCallback((label, value, indent = 0) => {
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        if (Array.isArray(val) && val.length > 0) {
          ...
          val.forEach((rec, i) => {
            ...
            text += `  ${i + 1}. ${(rec?.recommendation || '').trim()}\n`;
          });
          text += '\n';
        }
        ...
        if (Array.isArray(val) && val.length > 0) {
          text += `${label}\n`;
          val.forEach((item, i) => {
            const displayItem = getArrayFieldValue(record, f, i, idx) || item;
            text += `  ${i + 1}. ${displayItem}\n`;
          });
          text += '\n';
        }
    ...
    return text;
  }, [getFieldValue, getArrayFieldValue, hasVal, fmtVal, splitBySentence, formatSentenceFieldLines, objectCopyLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== HIV HISTORY ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `HIV History ${idx + 1}\n${'='.repeat(40)}\n\n`;
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const id2 = safeId(record); if (!id2) return; const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); const editKey2 = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [editKey2]: fullText })); setPendingEdits(prev => ({ ...prev, [editKey2]: true })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); stageDraft(id2, fn, fullText); setEditingField(null); setEditValue(''); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  const renderArrayField = (record, fn, idx, sid) => {
    ...
    return arr.map((item, aiIdx) => {
      const displayVal = getArrayFieldValue(record, fn, aiIdx, idx) || item;
      if (!displayVal) return null;
      const strItem = String(displayVal);
      /* Decompose a labeled item ("Current: Biktarvy…") — label becomes a nested-subtitle, never side-by-side */
      const parsed = parseLabel(strItem);
      const contentVal = parsed.isLabeled ? parsed.value : strItem;
      const editKey = `${fn}.${aiIdx}-${idx}`;
      const isEditing = editingField === editKey;
      const isModified = editedFields[editKey];
      ...
      return (
        <React.Fragment key={aiIdx}>
          {parsed.isLabeled && <div className="nested-subtitle">{highlightText(parsed.label)}</div>}
          <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(parsed.isLabeled ? parsed.value : strItem); setSaveError(null); } }}>
            {isEditing ? (
              <div className="edit-field-container">
                <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                {saveError && <div className="save-error">{saveError}</div>}
                <div className="edit-actions">
                  <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveArrayItem(record, fn, idx, sid, aiIdx, parsed.isLabeled ? `${parsed.label}: ${editValue.trim()}` : undefined); }}>{saving ? 'Saving...' : 'Save'}</button>
                  <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                </div>
              </div>
            ) : (
              <>
                <div className="row-content"><span className="content-value">{highlightText(contentVal)}</span><span className="edit-indicator">&#9998;</span></div>
                <button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(strItem, editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button>
              </>
            )}
          </div>
          {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
  ...
  if (!records || records.length === 0) {
    return (
      <div className="hiv-history-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">HIV History</h2></div>
        <div className="empty-state">No HIV history records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="hiv-history-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">HIV History</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HivHistoryDocumentPDFTemplate document={pdfData} />} fileName="Hiv_History.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search HIV history..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`HIV History ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'diagnosis-details')}
            {renderSection(record, idx, 'cd4-history')}
            {renderSection(record, idx, 'genotype-resistance')}
            {renderSection(record, idx, 'prior-ois')}
            {renderSection(record, idx, 'art-history')}
...
export default HivHistoryDocument;
