/**
 * ThyroidManagementDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: thyroid_management
 *
 * 6 Sections:
 *   1. clinical-info: date, provider, facility, status
 *   2. medication-details: medication, dosage, frequency
 *   3. monitoring: monitoringSchedule, targetLevels
 *   4. findings-assessment: findings, assessment
 *   5. plan-recommendations: plan
 *   6. notes: notes
 */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" — the whole field value) */
const DRAFT_KEY = 'thyroid_managementPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const ThyroidManagementDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.thyroid_management) return Array.isArray(r.thyroid_management) ? r.thyroid_management : [r.thyroid_management];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.thyroid_management) return Array.isArray(dd.thyroid_management) ? dd.thyroid_management : [dd.thyroid_management]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Thyroid Management ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // Each localEdits key for this section is `${field}-${idx}` holding the full field value.
      const toCommit = fields
        .map(f => `${f}-${idx}`)
        .filter(k => pendingEdits[k] && localEdits[k] !== undefined);
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -`-${idx}`.length); // "field" — no arrayIndex (full value stored)
        ...
        else if (lastDot !== -1) payload.field = fieldPart; // dotted field name, not an array index
        const resp = await secureApiClient.put(`/api/edit/thyroid_management/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/thyroid_management/${id}/approve`, { sectionId: sid, approved: true });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="thyroid-management-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Thyroid Management</h2></div>
        <div className="empty-state">No thyroid management records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="thyroid-management-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search thyroid management records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
...
export default ThyroidManagementDocument;
