/**
 * SecondOpinionReportsDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: second_opinion_reports
 *
 * 4 Sections:
 *   1. diagnosis: primaryDiagnosisIcd10, differentialDiagnoses, nyhaClassification, stagingTnm, histopathologicalGrading, pathologySpecimenType
 *   2. clinical-measures: glasgowComaScale, performanceStatusEcog, estimatedGlomerularFiltrationRate, ankleBrachialIndex, ejectionFractionPercent, biomarkerLevels
 *   3. imaging-pathology: radiologicalModalityUsed, contrastAgentAdministered, lesionSizeMeasurements, immunohistochemistryResults, geneticMutationProfile
 *   4. recommendations: therapeuticRegimenProposed, contraindicationsIdentified, prognosticIndicators, consultingSpecialtyService, clinicalTrialEligibility, multidisciplinaryTeamInput, previousTreatmentResponse
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import SecondOpinionReportsDocumentPDFTemplate from '../pdf-templates/SecondOpinionReportsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SecondOpinionReportsDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'diagnosis': 'Diagnosis & Staging',
  'clinical-measures': 'Clinical Measures',
...
const FIELD_LABELS = {
  ...
  stagingTnm: 'TNM Staging',
  histopathologicalGrading: 'Histopathological Grading',
  pathologySpecimenType: 'Pathology Specimen Type',
  glasgowComaScale: 'Glasgow Coma Scale',
  performanceStatusEcog: 'ECOG Performance Status',
  estimatedGlomerularFiltrationRate: 'Estimated GFR (mL/min/1.73m²)',
  ankleBrachialIndex: 'Ankle-Brachial Index',
  ejectionFractionPercent: 'Ejection Fraction (%)',
  biomarkerLevels: 'Biomarker Levels',
  radiologicalModalityUsed: 'Radiological Modality',
  contrastAgentAdministered: 'Contrast Agent Administered',
  lesionSizeMeasurements: 'Lesion Size Measurements',
  immunohistochemistryResults: 'Immunohistochemistry Results',
  geneticMutationProfile: 'Genetic Mutation Profile',
  therapeuticRegimenProposed: 'Therapeutic Regimen Proposed',
  contraindicationsIdentified: 'Contraindications Identified',
  prognosticIndicators: 'Prognostic Indicators',
  consultingSpecialtyService: 'Consulting Specialty Service',
  clinicalTrialEligibility: 'Clinical Trial Eligibility',
  multidisciplinaryTeamInput: 'Multidisciplinary Team Input',
  previousTreatmentResponse: 'Previous Treatment Response',
};

const SECTION_FIELDS = {
  'diagnosis': ['primaryDiagnosisIcd10', 'differentialDiagnoses', 'nyhaClassification', 'stagingTnm', 'histopathologicalGrading', 'pathologySpecimenType'],
  'clinical-measures': ['glasgowComaScale', 'performanceStatusEcog', 'estimatedGlomerularFiltrationRate', 'ankleBrachialIndex', 'ejectionFractionPercent', 'biomarkerLevels'],
  'imaging-pathology': ['radiologicalModalityUsed', 'contrastAgentAdministered', 'lesionSizeMeasurements', 'immunohistochemistryResults', 'geneticMutationProfile'],
  'recommendations': ['therapeuticRegimenProposed', 'contraindicationsIdentified', 'prognosticIndicators', 'consultingSpecialtyService', 'clinicalTrialEligibility', 'multidisciplinaryTeamInput', 'previousTreatmentResponse'],
};

const NUMBER_FIELDS = ['glasgowComaScale', 'performanceStatusEcog', 'estimatedGlomerularFiltrationRate', 'ankleBrachialIndex', 'ejectionFractionPercent'];
const ARRAY_FIELDS = ['differentialDiagnoses', 'biomarkerLevels', 'immunohistochemistryResults', 'geneticMutationProfile', 'contraindicationsIdentified', 'prognosticIndicators'];
const BOOLEAN_FIELDS = ['contrastAgentAdministered', 'clinicalTrialEligibility'];
const STRING_FIELDS = ['primaryDiagnosisIcd10', 'nyhaClassification', 'stagingTnm', 'pathologySpecimenType', 'histopathologicalGrading', 'radiologicalModalityUsed', 'lesionSizeMeasurements', 'therapeuticRegimenProposed', 'consultingSpecialtyService', 'multidisciplinaryTeamInput', 'previousTreatmentResponse'];
...
const DRAFT_KEY = 'second_opinion_reportsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const SecondOpinionReportsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.second_opinion_reports) return Array.isArray(r.second_opinion_reports) ? r.second_opinion_reports : [r.second_opinion_reports];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.second_opinion_reports) return Array.isArray(dd.second_opinion_reports) ? dd.second_opinion_reports : [dd.second_opinion_reports]; return [dd]; }
      return [r];
    });
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Maps each record _id (handling _id.$oid) to its render index. */
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayIdx) {
          // Array element draft: localEdits stores the FULL array under "field-idx"
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Second Opinion Report ${idx + 1}`.toLowerCase();
  ...
  /* Stage a DRAFT locally + persist it to the pending-drafts localStorage store (survives refresh).
     NOT written to MongoDB and NOT shown in the PDF until the user clicks Approve. The store keys by
     record _id; fieldPart = "field" (scalar) or "field.arrayIndex" (array element). */
  const stageDraft = useCallback((record, idx, localKey, value, fieldPart, sid) => {
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    stageDraft(record, idx, `${fn}-${idx}`, saveVal, fn, sid);
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      setSaveError(null);
      // Stage as a DRAFT (no DB write). Approve commits it.
      stageDraft(record, idx, `${fn}-${idx}`, fullText, fn, sid);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Collect this section's staged edits from localEdits (keyed "field-idx"; full value per field).
      const suffix = `-${idx}`;
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length);
        // trailing dot-segment that is purely numeric → array element; base field before the dot
        const lastDot = fieldPart.lastIndexOf('.');
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/second_opinion_reports/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/second_opinion_reports/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (store[id]) {
        fields.forEach(f => {
          Object.keys(store[id]).forEach(fieldPart => {
            const lastDot = fieldPart.lastIndexOf('.');
            const baseField = (lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1))) ? fieldPart.slice(0, lastDot) : fieldPart;
            if (baseField === f) delete store[id][fieldPart];
          });
        });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== SECOND OPINION REPORTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Second Opinion Report ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(parsed.isLabeled ? parsed.value : itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; setSaveError(null); const saveVal = parsed.isLabeled ? `${parsed.label}: ${editValue}` : editValue; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = saveVal; /* Stage as a DRAFT (no DB write). Approve commits it. localEdits holds full array; draft store keys the element by "field.arrayIndex". */ stageDraft(record, idx, `${fn}-${idx}`, currentArr, `${fn}.${itemIdx}`, sid); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="second-opinion-reports-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Second Opinion Report</h2></div>
        <div className="empty-state">No second opinion report data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="second-opinion-reports-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Second Opinion Report</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<SecondOpinionReportsDocumentPDFTemplate document={pdfData} />} fileName={`second-opinion-report-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search second opinion report..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Second Opinion Report ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'diagnosis')}
            {renderSection(record, idx, 'clinical-measures')}
            {renderSection(record, idx, 'imaging-pathology')}
...
export default SecondOpinionReportsDocument;
