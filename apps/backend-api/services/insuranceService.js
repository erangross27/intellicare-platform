 ...
 *
 * NOTE: Currently uses hardcoded coverage rules. Future API integration planned:
 * - CMS Interoperability Rule (CMS-0057-F) mandates insurance APIs by January 1, 2027
 * - Impacted payers: Medicare Advantage, Medicaid, CHIP, QHP issuers
 * - Required APIs: Patient Access, Provider Access, Payer-to-Payer (HL7 FHIR standard)
 * - Free access for contracted providers (no API fees required)
 * - LIMITATION: Drug/medication coverage excluded from mandate, may need alternative approach
 * - See memory ID: 6905f1748d704545e81fc55e for full details
 */
...
class InsuranceService {
  ...
  isIsraeliProvider(provider) {
    // CRITICAL FIX: Add null check before toLowerCase()
  ...
  /**
   * Check medication coverage using CMS formulary API
   * Available starting January 1, 2027 (CMS-0057-F mandate)
   * @param {Object} params - Coverage check parameters
   * @param {string} params.insuranceCompany - Insurance company identifier (e.g., 'aetna', 'bluecross')
   * @param {string} params.medication - Medication name
   * @param {string} params.rxcui - Optional RxCUI (RXNORM identifier) for exact matching
   * @param {string} params.planId - Optional specific plan ID
   * @returns {Promise<Object>} Coverage information with tier, copay, prior auth requirements
   */
  async checkMedicationCoverageAPI(params) {
    const { insuranceCompany, medication, rxcui, planId } = params;

    // Validate insurance company is configured
    const configuredInsurers = formularyService.getConfiguredInsurers();
    const normalizedInsurer = insuranceCompany.toLowerCase();

    if (!configuredInsurers.includes(normalizedInsurer)) {
      return {
        covered: null,
        error: 'FORMULARY_NOT_CONFIGURED',
        message: `Formulary data not yet available for ${insuranceCompany}. Currently configured: ${configuredInsurers.join(', ')}`,
        fallback: 'Use checkCoverage() for hardcoded rules until 2027 mandate takes effect'
      };
    }
    ...
    try {
      ...
      // Prefer RxCUI lookup (more accurate)
      if (rxcui) {
        coverageResult = await formularyService.lookupMedicationCoverageByRxCUI(rxcui, normalizedInsurer, planId);
      } else if (medication) {
        // Fallback to name-based lookup (less reliable, may return multiple matches)
        coverageResult = await formularyService.lookupMedicationCoverageByName(medication, normalizedInsurer, planId);
      } else {
        throw new Error('Either medication name or rxcui is required');
      }

      // If coverage found, get cost-sharing details for first plan
      if (coverageResult.covered && coverageResult.plans && coverageResult.plans.length > 0) {
        const firstPlan = coverageResult.plans[0];

        try {
          const costSharing = await formularyService.getPlanCostSharing(
            normalizedInsurer,
            firstPlan.planId,
            firstPlan.tier
          );
          ...
          return {
            covered: true,
            source: 'CMS_FORMULARY_API',
            medication: coverageResult.drugName || medication,
            rxcui: coverageResult.rxcui,
            plans: coverageResult.plans.map(plan => ({
              planId: plan.planId,
              tier: plan.tier,
              tierDescription: this.getTierDescription(plan.tier),
              priorAuthorization: plan.priorAuthorization,
              stepTherapy: plan.stepTherapy,
              quantityLimit: plan.quantityLimit,
              costSharing: plan.planId === firstPlan.planId ? costSharing.costSharing : null
            }))
          };
        } catch (costErr) {
          // Cost-sharing fetch failed, but coverage found
          return {
            covered: true,
            source: 'CMS_FORMULARY_API',
            medication: coverageResult.drugName || medication,
            ...
            costSharingError: costErr.message
          };
        }
      } else if (coverageResult.results && coverageResult.results.length > 0) {
        // Multiple name matches found
        return {
          covered: null,
          source: 'CMS_FORMULARY_API',
          multipleMatches: true,
          searchTerm: medication,
          results: coverageResult.results,
          message: 'Multiple medications found. Please provide RxCUI for exact match or select from results.'
        };
      } else {
        // Not covered
        return {
          ...
          reason: coverageResult.reason || 'Medication not found in formulary'
        };
      }
    } catch (error) {
      return {
        covered: null,
        error: 'API_ERROR',
        message: `Failed to check formulary: ${error.message}`,
        fallback: 'Use checkCoverage() for hardcoded rules as fallback'
      };
    }
  }
   ...
   * @param {string} tier - Tier code (GENERIC, BRAND, SPECIALTY, etc.)
   * @returns {string} Description
   */
  getTierDescription(tier) {
    const descriptions = {
      'GENERIC': 'Generic medications - lowest cost',
      'PREFERRED_GENERIC': 'Preferred generic medications',
      'BRAND': 'Brand-name medications',
      'PREFERRED_BRAND': 'Preferred brand-name medications',
      'NON_PREFERRED_BRAND': 'Non-preferred brand-name medications',
      'SPECIALTY': 'Specialty medications - highest cost',
      'ZERO_COST_SHARE_PREVENTIVE': 'Preventive medications - no cost'
    };
