/**
 * GoalsOfCareDiscussionDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: goals_of_care_discussion
 *
 * 6 Sections:
 *   1. session-info: date, discussionDate
 *   2. decision-capacity: participantsPresent, decisionMakingCapacity, surrogateDecisionMaker, advanceDirectiveStatus
 *   3. code-preferences: codeStatus, resuscitationPreferences, intubationPreferences, artificialNutritionHydration,
 *                        hospitalAdmissionPreferences, intensiveCarePreferences, dialysisPreferences, antibioticPreferences
 *   4. goals-prognosis: primaryGoalsOfCare, prognosisDiscussed, prognosisUnderstanding
 *   5. quality-values: qualityOfLifeFactors, unacceptableOutcomes, spiritualCulturalFactors
 *   6. follow-up: palliativeCareConsult, hospiceDiscussion, trialPeriodAgreed, followUpDiscussionPlan,
 *                 familyConsensusDynamics, physicianOrdersEntered
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import GoalsOfCareDiscussionDocumentPDFTemplate from '../pdf-templates/GoalsOfCareDiscussionDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './GoalsOfCareDiscussionDocument.css';
...
const FIELD_LABELS = {
  ...
  surrogateDecisionMaker: 'Surrogate Decision Maker',
  advanceDirectiveStatus: 'Advance Directive Status',
  codeStatus: 'Code Status',
  resuscitationPreferences: 'Resuscitation Preferences',
  intubationPreferences: 'Intubation Preferences',
  artificialNutritionHydration: 'Artificial Nutrition / Hydration',
  hospitalAdmissionPreferences: 'Hospital Admission Preferences',
  intensiveCarePreferences: 'Intensive Care Preferences',
  dialysisPreferences: 'Dialysis Preferences',
  antibioticPreferences: 'Antibiotic Preferences',
  primaryGoalsOfCare: 'Primary Goals of Care',
  prognosisDiscussed: 'Prognosis Discussed',
  prognosisUnderstanding: 'Prognosis Understanding',
  qualityOfLifeFactors: 'Quality of Life Factors',
  unacceptableOutcomes: 'Unacceptable Outcomes',
  spiritualCulturalFactors: 'Spiritual / Cultural Factors',
  palliativeCareConsult: 'Palliative Care Consult',
  hospiceDiscussion: 'Hospice Discussion',
  trialPeriodAgreed: 'Trial Period Agreed',
  followUpDiscussionPlan: 'Follow-Up Discussion Plan',
  familyConsensusDynamics: 'Family Consensus / Dynamics',
  physicianOrdersEntered: 'Physician Orders Entered',
};
...
const parseLabel = (text) => { if (!text || typeof text !== 'string') return null; const m = text.match(/^([A-Za-z][A-Za-z0-9\s/&(),.#'"-]{2,60}?):\s+([\s\S]*)/); return (m && !CLAUSE_OPENER.test(m[1].trim())) ? { label: m[1].trim(), content: m[2].trim() } : null; };

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'goals_of_care_discussionPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const GoalsOfCareDiscussionDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.goals_of_care_discussion) return Array.isArray(r.goals_of_care_discussion) ? r.goals_of_care_discussion : [r.goals_of_care_discussion];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.goals_of_care_discussion) return Array.isArray(dd.goals_of_care_discussion) ? dd.goals_of_care_discussion : [dd.goals_of_care_discussion]; return [dd]; }
      return [r];
    });
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayPart) {
          ...
          const ai = parseInt(fieldPart.slice(dotIdx + 1), 10);
  ...
  const splitBySentence = useCallback((text) => { if (!text || typeof text !== 'string') return []; return text.split(/(?<!\b(?:Mr|Mrs|Ms|Dr|St|Jr|Sr|Prof|Rev|Gen|Col|Sgt|vs|etc|[A-Z]))[.;](?:\s+)/).map(s => s.trim()).filter(s => s && !/^[;.,!?]+$/.test(s)); }, []);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const recordTitle = `Goals of Care Discussion ${idx + 1}`.toLowerCase();
  ...
  // Drop the section's 'approved' flag so re-editing returns the button to yellow Pending Approve.
  const dropApprovedForField = useCallback((fn, idx) => {
  ...
  // Persist a scalar/sentence field draft (fieldPart = field name).
  const stageFieldDraft = useCallback((record, fn, idx, value) => {
    ...
    dropApprovedForField(fn, idx);
  }, [safeId, dropApprovedForField]);

  // Persist an array-element draft (fieldPart = "field.arrayIndex"); localEdits holds the whole array.
  const stageArrayDraft = useCallback((record, fn, idx, arrayIndex, value) => {
    const id = safeId(record); if (!id) return;
    const store = readDrafts();
    if (!store[id]) store[id] = {};
    store[id][`${fn}.${arrayIndex}`] = value;
    writeDrafts(store);
    setLocalEdits(prev => { const arr = [...(getEffectiveArray(record, fn, idx))]; arr[arrayIndex] = value; return { ...prev, [`${fn}-${idx}`]: arr }; });
    setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true }));
    dropApprovedForField(fn, idx);
  }, [safeId, getEffectiveArray, dropApprovedForField]);

  const handleSaveField = useCallback((record, fn, idx, _sid, _sIdx, overrideValue) => {
    const id = safeId(record); if (!id) return;
    const valueToSave = overrideValue !== undefined ? overrideValue : editValue;
    stageFieldDraft(record, fn, idx, valueToSave);
    setEditedFields(prev => ({ ...prev, [`${fn}-${idx}`]: 'edited' }));
    setEditingField(null); setEditValue('');
  }, [editValue, safeId, stageFieldDraft]);

  const handleSaveArrayItem = useCallback((record, fn, idx, arrayIndex) => {
    const id = safeId(record); if (!id) return;
    stageArrayDraft(record, fn, idx, arrayIndex, editValue);
    setEditedFields(prev => ({ ...prev, [`${fn}-${idx}-ai${arrayIndex}`]: 'edited' }));
    setEditingField(null); setEditValue('');
  }, [editValue, safeId, stageArrayDraft]);
  ...
  const handleSaveDateField = useCallback((record, fn, idx) => {
    ...
    stageFieldDraft(record, fn, idx, isoDate);
    setEditedFields(prev => ({ ...prev, [`${fn}-${idx}`]: 'edited' }));
    setEditingField(null); setEditValue('');
  }, [editValue, safeId, stageFieldDraft]);
  ...
  const handleSaveBooleanField = useCallback((record, fn, idx) => {
    ...
    stageFieldDraft(record, fn, idx, boolVal);
    setEditedFields(prev => ({ ...prev, [`${fn}-${idx}`]: 'edited' }));
    setEditingField(null); setEditValue('');
  }, [editValue, safeId, stageFieldDraft]);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Collect this record's staged drafts that belong to fields in THIS section.
      const store = readDrafts();
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        if (isArrayPart) payload.arrayIndex = parseInt(fieldPart.slice(dotIdx + 1), 10);
        const resp = await secureApiClient.put(`/api/edit/goals_of_care_discussion/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedLocalKeys.add(`${baseField}-${idx}`);
        delete recDrafts[fieldPart];
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/goals_of_care_discussion/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[GoalsOfCareDiscussion] Approve error:', err); }
  }, [safeId]);
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== GOALS OF CARE DISCUSSION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Goals of Care Discussion ${idx + 1}\n${'='.repeat(40)}\n\n`;
      if (r.date) text += `Date\n${formatDate(r.date)}\n\n`;
      Object.keys(SECTION_FIELDS).forEach(sid => {
        const fields = SECTION_FIELDS[sid]; const hasAny = fields.some(f => { if (ARRAY_FIELDS.includes(f)) { const raw = r[f]; return Array.isArray(raw) && raw.length > 0; } const val = r[f]; return hasVal(val); }); if (!hasAny) return;
      ...
      return (
        <div key={fn}>
          ...
          <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(toDateInputValue(val)); } }}>
            {isEditing ? (<div className="edit-field-container"><BlueDatePicker value={editValue} onSelect={iso => setEditValue(iso)} /><div className="edit-actions"><button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveDateField(record, fn, idx); }}>{saving ? 'Saving...' : 'Save'}</button><button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); }}>Cancel</button></div></div>
      ...
      return (
        <div key={fn}>
          ...
          <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayVal); } }}>
            {isEditing ? (<div className="edit-field-container"><BlueSelect value={editValue} options={['Yes', 'No']} onChange={v => setEditValue(v)} /><div className="edit-actions"><button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveBooleanField(record, fn, idx); }}>{saving ? 'Saving...' : 'Save'}</button><button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); }}>Cancel</button></div></div>
            ) : (<><div className="row-content"><span className="content-value">{highlightText(displayVal)}</span><span className="edit-indicator">&#9998;</span></div><button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${label}\n${displayVal}`, editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button></>)}
          </div>
          {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
        </div>
            ...
            if (parsed) {
              return (
                <div key={ai} className="rec-mini-card" style={{ marginTop: ai > 0 ? 8 : 0 }}>
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(parsed.content); } }}>
                    {isEditing ? (<div className="edit-field-container"><textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); } }} /><div className="edit-actions"><button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const fullVal = `${parsed.label}: ${editValue.trim()}`; const id = safeId(record); if (!id) return; stageArrayDraft(record, fn, idx, ai, fullVal); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button><button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); }}>Cancel</button></div></div>
            ...
            return (
              <div key={ai}>
                <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); } }}>
                  {isEditing ? (<div className="edit-field-container"><textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); } }} /><div className="edit-actions"><button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveArrayItem(record, fn, idx, ai); }}>{saving ? 'Saving...' : 'Save'}</button><button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); }}>Cancel</button></div></div>
  ...
  if (!records || records.length === 0) {
    return (<div className="goals-of-care-discussion-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Goals of Care Discussion</h2></div><div className="empty-state">No goals of care discussion records available</div></div>);
  }

  return (
    <div className="goals-of-care-discussion-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Goals of Care Discussion</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<GoalsOfCareDiscussionDocumentPDFTemplate document={pdfData} />} fileName="Goals_Of_Care_Discussion.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search goals of care discussion..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Goals of Care Discussion ${idx + 1}`)}</h3>
            </div>
            {renderMixedSection(record, idx, 'session-info')}
            {renderMixedSection(record, idx, 'decision-capacity')}
            {renderMixedSection(record, idx, 'code-preferences')}
            {renderMixedSection(record, idx, 'goals-prognosis')}
            {renderMixedSection(record, idx, 'quality-values')}
            {renderMixedSection(record, idx, 'follow-up')}
          </div>
        ))}
      </div>
    </div>
...
export default GoalsOfCareDiscussionDocument;
