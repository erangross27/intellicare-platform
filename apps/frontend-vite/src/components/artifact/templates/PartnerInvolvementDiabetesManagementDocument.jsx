/**
 * PartnerInvolvementDiabetesManagementDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: partner_involvement_diabetes_management
 *
 * 7 Sections:
 *   1. knowledge: partnerDiabetesKnowledgeLevel, partnerCarbohydrateCountingKnowledge, partnerHbA1cUnderstanding
 *   2. emergency: partnerHypoglycemiaRecognition, partnerGlucagonAdministration, partnerDiabeticKetoacidosisRecognition
 *   3. monitoring: partnerBloodGlucoseMonitoring, partnerContinuousGlucoseMonitoringSupport, partnerBloodPressureMonitoring
 *   4. medication: partnerInsulinAdministrationTraining, partnerInsulinStorageKnowledge, partnerMedicationComplianceMonitoring
 *   5. dailyCare: partnerDiabeticDietPreparation, partnerFootCareAssistance, partnerExerciseSupport, partnerStressManagementSupport, partnerDiabeticSupplyManagement
 *   6. followup: partnerEndocrinologyAppointmentSupport, partnerSickDayManagement
 *   7. complications: partnerDiabeticComplicationAwareness (array)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PartnerInvolvementDiabetesManagementDocumentPDFTemplate from '../pdf-templates/PartnerInvolvementDiabetesManagementDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PartnerInvolvementDiabetesManagementDocument.css';
...
const STRING_FIELDS = [
  'partnerDiabetesKnowledgeLevel', 'partnerCarbohydrateCountingKnowledge',
  'partnerHypoglycemiaRecognition', 'partnerDiabeticKetoacidosisRecognition',
  'partnerBloodGlucoseMonitoring', 'partnerBloodPressureMonitoring',
  'partnerFootCareAssistance', 'partnerSickDayManagement',
  'partnerDiabeticSupplyManagement',
];

/* Single-name gate: suppress a field's own subtitle when it equals the section title (never double-name) */
const showFieldLabel = (fn, sid) => (FIELD_LABELS[fn] || fn).trim().toLowerCase() !== (SECTION_TITLES[sid] || '').trim().toLowerCase();
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldName]: fullFieldValue } } */
const DRAFT_KEY = 'partner_involvement_diabetes_managementPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const PartnerInvolvementDiabetesManagementDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Partner Involvement Diabetes Management Record ${idx + 1}`.toLowerCase();
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    // Re-edit after approval → drop the section 'approved' flag so the button returns to yellow
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    // Staged pending edits for this record's section fields. localEdits key = "field-idx" (full field value).
    const toCommit = fields.map(f => `${f}-${idx}`).filter(k => pendingEdits[k] && localEdits[k] !== undefined);
    try {
      for (const editKey of toCommit) {
        const fn = editKey.slice(0, -(`-${idx}`).length); // strip trailing "-idx" → field name
        const resp = await secureApiClient.put(`/api/edit/partner_involvement_diabetes_management/${id}/edit`, { field: fn, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/partner_involvement_diabetes_management/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PARTNER INVOLVEMENT IN DIABETES MANAGEMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Partner Involvement Diabetes Management Record ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; stageFieldDraft(record, fn, idx, sid, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="partner-involvement-diabetes-management-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Partner Involvement in Diabetes Management</h2></div>
        <div className="empty-state">No partner involvement diabetes management data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="partner-involvement-diabetes-management-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Partner Involvement in Diabetes Management</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PartnerInvolvementDiabetesManagementDocumentPDFTemplate document={pdfData} />} fileName="Partner_Involvement_Diabetes_Management.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search partner involvement diabetes management..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Partner Involvement Diabetes Management Record ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'knowledge')}
            {renderSection(record, idx, 'emergency')}
            {renderSection(record, idx, 'monitoring')}
            {renderSection(record, idx, 'medication')}
            {renderSection(record, idx, 'dailyCare')}
            {renderSection(record, idx, 'followup')}
...
export default PartnerInvolvementDiabetesManagementDocument;
