...
function buildContext(req, operation = 'read') { return { serviceId: 'copd-assessments-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
...
const ALLOWED_FIELDS = [
  ...
  'goldStage', 'modifiedMedicalResearchCouncilScale', 'catScore', 'bodiScore', 'functionalStatus',
  'arterialBloodGasPh', 'arterialBloodGasPaco2', 'arterialBloodGasPao2', 'oxygenSaturation',
  'chestXrayFindings', 'ctEmphysemaScore', 'exacerbationFrequency', 'hospitalizationHistory',
  'smokingPackYears', 'alpha1AntitrypsinLevel',
  'currentMedications', 'comorbidities', 'inflammatoryMarkers',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('copd_assessments', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[CopdAssessments] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('copd_assessments', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[CopdAssessments] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
