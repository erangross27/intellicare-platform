/**
 * SleepApneaManagementDocument.jsx
 * July 2026 — recursive, dot-path editable sleep apnea management
...
import SleepApneaManagementDocumentPDFTemplate from '../pdf-templates/SleepApneaManagementDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SleepApneaManagementDocument.css';

const DRAFT_KEY = 'sleep_apnea_managementPendingEdits';
const SECTION_TITLES = {
  diagnosticMetrics: 'Diagnostic Metrics',
  cpapTherapy: 'CPAP Therapy',
  maskEquipment: 'Mask / Equipment',
...
const FIELD_LABELS = {
  date: 'Date', apneaHypopneaIndex: 'Apnea-Hypopnea Index (AHI)', oxygenDesaturationIndex: 'Oxygen Desaturation Index (ODI)', lowestOxygenSaturation: 'Lowest Oxygen Saturation', sleepApneaSeverity: 'Sleep Apnea Severity', epworthSleepinessScore: 'Epworth Sleepiness Score', cpapPrescribed: 'CPAP Prescribed', cpapPressureSetting: 'CPAP Pressure Setting', cpapComplianceRate: 'CPAP Compliance Rate', averageCpapUsageHours: 'Average CPAP Usage', residualAhiOnTherapy: 'Residual AHI on Therapy', maskType: 'Mask Type', maskLeakRate: 'Mask Leak Rate', remSleepPercentage: 'REM Sleep Percentage', arousalIndex: 'Arousal Index', centralApneaIndex: 'Central Apnea Index', obstructiveApneaIndex: 'Obstructive Apnea Index', neckCircumference: 'Neck Circumference', mallampatiScore: 'Mallampati Score', tonsilSize: 'Tonsil Size', positionalApnea: 'Positional Apnea', witnessedApneas: 'Witnessed Apneas', comorbidHypertension: 'Comorbid Hypertension', surgicalInterventions: 'Surgical Interventions', alternativeTherapyType: 'Alternative Therapy Type',
};
const SECTION_FIELDS = {
  diagnosticMetrics: ['date', 'apneaHypopneaIndex', 'oxygenDesaturationIndex', 'lowestOxygenSaturation', 'sleepApneaSeverity', 'epworthSleepinessScore'],
  cpapTherapy: ['cpapPrescribed', 'cpapPressureSetting', 'cpapComplianceRate', 'averageCpapUsageHours', 'residualAhiOnTherapy'],
  maskEquipment: ['maskType', 'maskLeakRate'],
  sleepStudyResults: ['remSleepPercentage', 'arousalIndex', 'centralApneaIndex', 'obstructiveApneaIndex'],
  physicalExam: ['neckCircumference', 'mallampatiScore', 'tonsilSize', 'positionalApnea'],
  symptoms: ['witnessedApneas', 'comorbidHypertension'],
  treatment: ['surgicalInterventions', 'alternativeTherapyType'],
};
const DATE_FIELDS = ['date'];
const DATETIME_FIELDS = [];
const ENUM_FIELDS = ['sleepApneaSeverity', 'maskType', 'mallampatiScore', 'tonsilSize'];
const NUMBER_FIELDS = ['apneaHypopneaIndex', 'oxygenDesaturationIndex', 'lowestOxygenSaturation', 'epworthSleepinessScore', 'cpapComplianceRate', 'averageCpapUsageHours', 'residualAhiOnTherapy', 'maskLeakRate', 'remSleepPercentage', 'arousalIndex', 'centralApneaIndex', 'obstructiveApneaIndex', 'neckCircumference'];
const NUMBER_STRING_FIELDS = [];
const NUMBER_UNITS = { apneaHypopneaIndex: 'events/hr', oxygenDesaturationIndex: 'events/hr', lowestOxygenSaturation: '%', cpapComplianceRate: '%', averageCpapUsageHours: 'hrs/night', residualAhiOnTherapy: 'events/hr', maskLeakRate: 'L/min', remSleepPercentage: '%', arousalIndex: 'events/hr', centralApneaIndex: 'events/hr', obstructiveApneaIndex: 'events/hr', neckCircumference: 'cm' };
const OBJECT_FIELDS = [];
...
const COMMA_FIELDS = ['alternativeTherapyType'];
const COMMA_ARRAY_SPLIT_FIELDS = ['surgicalInterventions'];
const ARRAY_FIELDS = ['surgicalInterventions'];
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'sleepApneaSeverity' ? ['None', 'Mild', 'Moderate', 'Severe']
    : field === 'maskType' ? ['Nasal Pillows', 'Nasal Mask', 'Full Face Mask', 'Oral Interface']
      : field === 'mallampatiScore' ? ['I', 'II', 'III', 'IV']
        : field === 'tonsilSize' ? ['0', '1+', '2+', '3+', '4+'] : [];
...
const SleepApneaManagementDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.sleep_apnea_management) return Array.isArray(record.sleep_apnea_management) ? record.sleep_apnea_management : [record.sleep_apnea_management];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.sleep_apnea_management) return Array.isArray(nested.sleep_apnea_management) ? nested.sleep_apnea_management : [nested.sleep_apnea_management];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/sleep_apnea_management/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/sleep_apnea_management/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SleepApneaManagement] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    let text = '=== SJOGRENS SYNDROME ASSESSMENT ===\n\n';
    records.forEach((record, recordIndex) => {
      text += 'Sleep Apnea Management Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="sleep-apnea-management-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Sleep Apnea Management</h2></div><div className="empty-state">No sleep apnea management available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Sleep Apnea Management Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="sleep-apnea-management-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Sleep Apnea Management</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<SleepApneaManagementDocumentPDFTemplate document={pdfData} />} fileName="Sleep_Apnea_Management.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search sleep apnea management..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Sleep Apnea Management Record ' + (recordIndex + 1))}</h3></div>
...
export default SleepApneaManagementDocument;
