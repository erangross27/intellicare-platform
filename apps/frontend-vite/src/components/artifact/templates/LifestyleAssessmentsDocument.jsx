/**
 * LifestyleAssessmentsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: lifestyle_assessments
 *
 * 6 Sections:
 *   1. session-info: assessmentDate (date), provider (string), facility (string)
 *   2. substance-use: smokingStatus, tobaccoUseFrequency, alcoholConsumption,
 *                     substanceUseHistory (array), caffeineIntake, waterIntake
 *   3. activity-nutrition: physicalActivityLevel (long), exerciseFrequency,
 *                          dietaryPatterns (long), nutritionalDeficiencies (array)
 *   4. sleep-wellbeing: sleepDuration (number), sleepQuality, stressLevel, socialSupport
 *   5. risk-lifestyle: occupationalHazards (array), screenTimeHours (number),
 *                      sexualActivityStatus, riskBehaviors (array), readinessToChange
 *   6. interventions-followup: lifestyleInterventions (array strings), followUpDate (date),
 *                              assessmentScore (number)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import LifestyleAssessmentsDocumentPDFTemplate from '../pdf-templates/LifestyleAssessmentsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './LifestyleAssessmentsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" — this template keys
   localEdits as "field-idx", so fieldPart is always the plain field name) */
const DRAFT_KEY = 'lifestyle_assessmentsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = [
  'provider', 'facility', 'smokingStatus', 'tobaccoUseFrequency', 'alcoholConsumption',
  'caffeineIntake', 'waterIntake', 'physicalActivityLevel', 'exerciseFrequency',
  'dietaryPatterns', 'sleepQuality', 'stressLevel', 'socialSupport',
  'sexualActivityStatus', 'readinessToChange',
];
...
/* ═══════ COMPONENT ═══════ */
const LifestyleAssessmentsDocument = ({ document: docProp }) => {
  ...
  /* ═══════ UTILS ═══════ */
  // hide-zero: numeric 0 counts as empty (sleepDuration/screenTimeHours/assessmentScore render nothing)
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Lifestyle Assessment ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      // localEdits keys are "field-idx" (fieldPart = plain field name), so collect by field membership.
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/lifestyle_assessments/${id}/edit`, payload);
      }
      // Flag the record/section approved (audit trail)
      await secureApiClient.put(`/api/edit/lifestyle_assessments/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        const arr = (Array.isArray(val) ? val : [val]).filter(hasVal);
        if (arr.length === 0) return;
        body += head;
        arr.forEach((item, i) => {
          const p = parseLabel(String(item));
          if (p.isLabeled) { body += `${p.label}:\n  ${i + 1}. ${p.value}\n`; }
          else { body += `${i + 1}. ${fmtVal(item)}\n`; }
        });
        body += '\n';
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== LIFESTYLE ASSESSMENTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Lifestyle Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="lifestyle-assessments-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Lifestyle Assessments</h2></div>
        <div className="empty-state">No lifestyle assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="lifestyle-assessments-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Lifestyle Assessments</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<LifestyleAssessmentsDocumentPDFTemplate document={pdfData} />} fileName="Lifestyle_Assessments.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search lifestyle assessments..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Lifestyle Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'substance-use')}
            {renderSection(record, idx, 'activity-nutrition')}
            {renderSection(record, idx, 'sleep-wellbeing')}
            {renderSection(record, idx, 'risk-lifestyle')}
            {renderSection(record, idx, 'interventions-followup')}
          </div>
        ))}
      </div>
    </div>
...
export default LifestyleAssessmentsDocument;
