/**
 * Box-free LETTER PDF for speech_therapy_assessments.
 */
...
const overviewUnits = (record) => {
  const units = [];
  if (hasVal(record.assessmentDate)) units.push(...fieldUnits(FIELD_LABELS.assessmentDate, [formatDate(record.assessmentDate)], 'assessmentDate'));
  if (hasVal(record.referralDiagnosis)) units.push(...fieldUnits(FIELD_LABELS.referralDiagnosis, partsForField('referralDiagnosis', record.referralDiagnosis), 'referralDiagnosis'));
  if (hasVal(record.therapist)) units.push(...fieldUnits(FIELD_LABELS.therapist, partsForField('therapist', record.therapist), 'therapist'));
  return units;
};
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, index) => (
          <View key={record._id?.$oid || record._id || index} style={styles.record} break={index > 0}>
            <Text style={styles.recordTitle}>Speech Therapy Assessment {index + 1}</Text>
            {renderSection(SECTION_TITLES.overview, overviewUnits(record), `overview-${index}`)}
            {renderSection(SECTION_TITLES.communication, objectUnits(record, 'communication'), `communication-${index}`)}
            {renderSection(SECTION_TITLES.swallowing, objectUnits(record, 'swallowing'), `swallowing-${index}`)}
            {renderSection(SECTION_TITLES.cognitive, objectUnits(record, 'cognitive'), `cognitive-${index}`)}
            {renderSection(SECTION_TITLES.voice, objectUnits(record, 'voice'), `voice-${index}`)}
            {renderSection(SECTION_TITLES.recommendations, recommendationUnits(record), `recommendations-${index}`)}
            {renderSection(SECTION_TITLES['treatment-plan'], objectUnits(record, 'treatment-plan'), `treatment-plan-${index}`)}
            {renderSection(SECTION_TITLES.goals, goalUnits(record), `goals-${index}`)}
          </View>
        ))}
      </Page>
    </Document>
