...
const HospitalAdmissionNotesTemplate = ({ document }) => {
  ...
  return (
    <View style={styles.card}>
      ...
      {/* Admission Time */}
      {document.admissionTime && (
        <View style={styles.row}>
          <Text style={styles.label}>Admission Time:</Text>
          <Text style={styles.value}>{document.admissionTime}</Text>
        </View>
      )}
      ...
      {document.reasonForAdmission && (
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Reason for Admission:</Text>
          <Text style={styles.paragraph}>{document.reasonForAdmission}</Text>
        </View>
      )}
      ...
      {/* Home Medications */}
      {document.homeMedications && (
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Home Medications:</Text>
          {Array.isArray(document.homeMedications) ? (
            document.homeMedications.map((med, i) => (
              <Text key={i} style={styles.listItem}>• {med}</Text>
            ))
          ) : (
            <Text style={styles.paragraph}>{document.homeMedications}</Text>
          )}
        </View>
      )}
      ...
      {document.assessmentAndPlan && (
        <View style={styles.section}>
          ...
          <Text style={styles.paragraph}>{document.assessmentAndPlan}</Text>
        </View>
      )}

      {/* Initial Orders */}
      {document.initialOrders && (
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Initial Orders:</Text>
          {Array.isArray(document.initialOrders) ? (
            document.initialOrders.map((order, i) => (
              <Text key={i} style={styles.listItem}>• {order}</Text>
            ))
          ) : (
            <Text style={styles.paragraph}>{document.initialOrders}</Text>
          )}
        </View>
      )}
      ...
      {document.codeStatus && (
        <View style={styles.row}>
          ...
          <Text style={styles.value}>{document.codeStatus}</Text>
        </View>
      )}
    </View>
  );
...
export default HospitalAdmissionNotesTemplate;
