/**
 * PregnancySymptomsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: pregnancy_symptoms
 *
 * 11 Sections:
 *   1. symptom-info: date, provider, facility
 *   2. gastrointestinal: nausea, vomiting, heartburn, constipation, hemorrhoids
 *   3. musculoskeletal: backPain, roundLigamentPain
 *   4. circulatory-skin: edema, varicoseVeins, skinChanges
 *   5. other-symptoms: sleepDisturbance, urinaryFrequency, vaginalDischarge
 *   6. findings: findings
 *   7. assessment: assessment
 *   8. results-section: results (OBJECT, recursive)
 *   9. recommendations-section: recommendations (ARRAY of {recommendation, date})
 *   10. plan-notes: plan, notes
 *   11. notes-status: status
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PregnancySymptomsDocumentPDFTemplate from '../pdf-templates/PregnancySymptomsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PregnancySymptomsDocument.css';
...
const DRAFT_KEY = 'pregnancy_symptomsPendingEdits';
const readDrafts = () => {
...
/* scoped aggressive-comma-split: unlabeled single-sentence >=2-comma symptom narratives → value-only rows */
...
/* deep value helpers (object/array aware) */
const isEmptyDeep = (v) => {
...
/* ═══════ COMPONENT ═══════ */
const PregnancySymptomsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.pregnancy_symptoms) return Array.isArray(r.pregnancy_symptoms) ? r.pregnancy_symptoms : [r.pregnancy_symptoms];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.pregnancy_symptoms) return Array.isArray(dd.pregnancy_symptoms) ? dd.pregnancy_symptoms : [dd.pregnancy_symptoms]; return [dd]; }
      return [r];
    });
  ...
  /* searchable text for any field type (object/recommendations aware) */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pregnancy Symptoms ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* stageDraft = stage a DRAFT locally + persist it to the pending-drafts localStorage store (survives
     refresh). NOT written to MongoDB and NOT shown in the PDF until the user clicks Approve. The whole
     field value is stored under editKey `${fn}-${idx}` (handleApproveSection commits it). */
  ...
  /* ═══════ SAVE: OBJECT LEAF (dotted-path; rebuilds nested clone in localEdits) ═══════ */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    // Rebuild the nested clone with the leaf edit, then stage the WHOLE object as a draft.
    setLocalEdits(prev => {
      ...
      const next = { ...prev, [`${rootField}-${idx}`]: clone };
      // Persist this record's draft for the whole rootField object.
      const store = readDrafts();
      if (!store[id]) store[id] = {};
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Commit each staged field for this section now. localEdits stores the WHOLE field value under
      // `${field}-${idx}`, so PUT { field, value } (no arrayIndex — full array/object is written).
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/pregnancy_symptoms/${id}/edit`, { field, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/pregnancy_symptoms/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[PregnancySymptoms] Approve error:', err); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PREGNANCY SYMPTOMS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Pregnancy Symptoms ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pregnancy-symptoms-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pregnancy Symptoms</h2></div>
        <div className="empty-state">No pregnancy symptoms records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pregnancy-symptoms-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Pregnancy Symptoms</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PregnancySymptomsDocumentPDFTemplate document={pdfData} />} fileName="Pregnancy_Symptoms.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pregnancy symptoms..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Pregnancy Symptoms ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'symptom-info')}
            {renderSection(record, idx, 'gastrointestinal')}
            {renderSection(record, idx, 'musculoskeletal')}
            {renderSection(record, idx, 'circulatory-skin')}
            {renderSection(record, idx, 'other-symptoms')}
...
export default PregnancySymptomsDocument;
