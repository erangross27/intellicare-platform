/**
 * InsuranceAuthorizations Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/insurance_authorizations/:id/edit    — Edit a single field
 * PUT /api/edit/insurance_authorizations/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'insurance-authorizations-edit-service',
    userId: req.user?.id,
    operation,
...
// Allowed editable fields — all 23 non-system schema fields
const ALLOWED_FIELDS = [
  ...
  'surgicalUrgencyLevel',
  'functionalCapacityEvaluation',
  'pharmacotherapyTrialDocumentation',
  'peerReviewOutcome',
  // Numbers
  'authorizedUnits',
  'estimatedCostAmount',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('insurance_authorizations', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[InsuranceAuthorizations] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('insurance_authorizations', { _id: objectId }, {
      $set: {
    ...
    console.error('[InsuranceAuthorizations] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
