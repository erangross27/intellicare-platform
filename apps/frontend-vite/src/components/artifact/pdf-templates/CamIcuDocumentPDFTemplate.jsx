...
const COLLECTION = 'cam_icu';
const COMMA_SPLIT_FIELDS = ['vitalSigns'];
const ARRAY_FIELDS = new Set(['recommendations']);
const OBJECT_FIELDS = new Set(['additionalData']);
const NARRATIVE_FIELDS = new Set(['vitalSigns', 'interventions', 'response', 'plan']);
...
const LABELS = {
  ...
  vitalSigns: 'Vital Signs', interventions: 'Interventions', response: 'CAM-ICU Features',
  plan: 'Plan', recommendations: 'Recommendations', additionalData: 'Additional Data',
};
const SECTIONS = [
  { title: 'Assessment', fields: ['assessmentDate', 'assessmentTime', 'clinicalStatus'] },
  { title: 'Vital Signs', fields: ['vitalSigns'] },
  { title: 'CAM-ICU Features', fields: ['response'] },
...
const CamIcuDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>CAM-ICU</Text><Text style={styles.noData}>No CAM-ICU data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>CAM-ICU</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>CAM-ICU Assessment {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
