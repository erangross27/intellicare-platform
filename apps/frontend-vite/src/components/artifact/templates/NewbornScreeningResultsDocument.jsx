/**
 * NewbornScreeningResultsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: newborn_screening_results
 *
 * 4 Sections:
 *   1. screening-info: assessmentDate, programType
 *   2. findings: findings
 *   3. goals-progress: goals, progress
 *   4. follow-up: followUp, recommendations
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NewbornScreeningResultsDocumentPDFTemplate from '../pdf-templates/NewbornScreeningResultsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './NewbornScreeningResultsDocument.css';
...
const SECTION_FIELDS = {
  'screening-info': ['assessmentDate', 'programType'],
  'findings': ['findings'],
  'goals-progress': ['goals', 'progress'],
  'follow-up': ['followUp', 'recommendations'],
};

const DATE_FIELDS = ['assessmentDate'];
const STRING_FIELDS = ['programType', 'findings', 'goals', 'progress', 'followUp', 'recommendations'];
...
const DRAFT_KEY = 'newborn_screening_resultsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const NewbornScreeningResultsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.newborn_screening_results) return Array.isArray(r.newborn_screening_results) ? r.newborn_screening_results : [r.newborn_screening_results];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.newborn_screening_results) return Array.isArray(dd.newborn_screening_results) ? dd.newborn_screening_results : [dd.newborn_screening_results]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Newborn Screening ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    setSaving(true);
    try {
      // Staged edit keys for THIS section's fields on THIS record (key = `${fn}-${idx}`)
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/newborn_screening_results/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the record/section approved (audit trail)
      await secureApiClient.put(`/api/edit/newborn_screening_results/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NEWBORN SCREENING RESULTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Newborn Screening ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="newborn-screening-results-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Newborn Screening Results</h2></div>
        <div className="empty-state">No newborn screening records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="newborn-screening-results-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Newborn Screening Results</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NewbornScreeningResultsDocumentPDFTemplate document={pdfData} />} fileName="Newborn_Screening_Results.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search newborn screening results..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Newborn Screening ${idx + 1}`)}</h3>
            </div>
...
export default NewbornScreeningResultsDocument;
