 ...
 *
 * 11 Sections:
 *   1. visual-acuity: visualAcuityOD, visualAcuityOS
 *   2. intraocular-pressure: intraocularPressureOD, intraocularPressureOS
 *   3. pupil-response: pupilResponseOD, pupilResponseOS
 *   4. cup-disc-ratio: cupDiscRatioOD, cupDiscRatioOS
 *   5. macular-appearance: macularAppearanceOD, macularAppearanceOS
 *   6. retinal-vascular-changes: retinalVascularChangesOD, retinalVascularChangesOS
 *   7. diabetic-retinopathy: diabeticRetinopathyGradeOD, diabeticRetinopathyGradeOS, hypertensiveRetinopathyGradeOD, hypertensiveRetinopathyGradeOS
 *   8. oct: opticalCoherenceTomographyOD, opticalCoherenceTomographyOS
 *   9. imaging: fundusPhotographyPerformed, fluoresceinAngiographyFindings
 *  10. peripheral-vitreous: peripheralRetinalExaminationOD, peripheralRetinalExaminationOS, vitreousExaminationOD, vitreousExaminationOS
 *  11. amsler-grid: amslerGridTestOD, amslerGridTestOS, retinoschisisseverity
 */
...
const RetinalExaminationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.retinal_examinations) return Array.isArray(r.retinal_examinations) ? r.retinal_examinations : [r.retinal_examinations];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.retinal_examinations) return Array.isArray(dd.retinal_examinations) ? dd.retinal_examinations : [dd.retinal_examinations]; return [dd]; }
      return [r];
    });
  ...
  /* ═══════ REHYDRATE PENDING DRAFTS ═══════
     A Save stages a draft locally + in localStorage so it survives refresh (shown in JSX, NOT in
     DB/PDF). Map each record's _id (handling _id.$oid) to its render index and repopulate state. */
  useEffect(() => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Retinal Examination ${idx + 1}`.toLowerCase();
  ...
  const stageSentenceDraft = (id, fn, idx, sid, fullText, markFn) => {
    const editKey = `${fn}-${idx}`;
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageSentenceDraft(id, fn, idx, sid, fullText, n => { n[`${fn}-${idx}-s${sentenceIdx}`] = 'edited'; return n; });
      return;
    }
    ...
    stageSentenceDraft(id, fn, idx, sid, fullText, n => {
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Collect this section's pending edits from localEdits using the file's "-<idx>" suffix convention.
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const dotIdx = fieldPart.lastIndexOf('.');
        // Treat dot-suffix as arrayIndex ONLY when the segment after the LAST dot is purely numeric.
        ...
        const resp = await secureApiClient.put(`/api/edit/retinal_examinations/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/retinal_examinations/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[RetinalExaminations] Approve error:', err); }
  ...
  const copyAllText = useCallback(async () => {
    ...
    pdfData.forEach((r, idx) => {
      text += `Retinal Examination ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="retinal-examinations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Retinal Examinations</h2></div>
        <div className="empty-state">No retinal examination records available</div>
      </div>
    );
  }
    ...
    <div className="retinal-examinations-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<RetinalExaminationsDocumentPDFTemplate document={pdfData} />} fileName={`retinal-examinations-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search retinal examinations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Retinal Examination ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'visual-acuity')}
            {renderSection(record, idx, 'intraocular-pressure')}
            {renderSection(record, idx, 'pupil-response')}
            {renderSection(record, idx, 'cup-disc-ratio')}
            {renderSection(record, idx, 'macular-appearance')}
            {renderSection(record, idx, 'retinal-vascular-changes')}
            {renderSection(record, idx, 'diabetic-retinopathy')}
            {renderSection(record, idx, 'oct')}
            {renderSection(record, idx, 'imaging')}
            {renderSection(record, idx, 'peripheral-vitreous')}
            {renderSection(record, idx, 'amsler-grid')}
          </div>
        ))}
      </div>
    </div>
