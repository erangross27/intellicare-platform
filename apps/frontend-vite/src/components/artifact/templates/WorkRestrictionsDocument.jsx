/**
 * WorkRestrictionsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: work_restrictions
 *
 * 5 Sections:
 *   1. clinical-findings: findings, assessment
 *   2. restrictions-details: restrictions, liftingLimit, returnToWork, duration, clearanceRequired
 *   3. plan-recommendations: plan, recommendations
 *   4. provider-info: provider, facility
 *   5. additional-notes: notes
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import WorkRestrictionsDocumentPDFTemplate from '../pdf-templates/WorkRestrictionsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './WorkRestrictionsDocument.css';
...
const DRAFT_KEY = 'work_restrictionsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['findings', 'assessment', 'liftingLimit', 'returnToWork', 'duration', 'plan', 'provider', 'facility', 'notes', 'type', 'status'];
...
/* ═══════ COMPONENT ═══════ */
const WorkRestrictionsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.work_restrictions) return Array.isArray(r.work_restrictions) ? r.work_restrictions : [r.work_restrictions];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.work_restrictions) return Array.isArray(dd.work_restrictions) ? dd.work_restrictions : [dd.work_restrictions]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Work Restrictions ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* stageDraft: persist a staged DRAFT locally + to localStorage (survives refresh). NO DB write.
     fieldKey = the localEdits key WITHOUT the "-idx" suffix (e.g. "findings"); value = full field value.
     Re-editing an already-approved section drops its approvedSections flag so the button returns to yellow. */
  const stageDraft = useCallback((record, fieldKey, idx, sid, trackKey, value) => {
  ...
  // Save = stage a DRAFT locally (NOT written to MongoDB and NOT shown in the PDF until Approve commits it).
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    const saveVal = valueOverride !== undefined ? valueOverride : editValue;
    stageDraft(record, fn, idx, sid, editTrackingKey, saveVal);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // localEdits keys for this section are "<field>-<idx>"; commit the staged (pending) ones now.
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/work_restrictions/${id}/edit`, { field, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/work_restrictions/${id}/approve`, { sectionId: sid, approved: true });
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        if (val && typeof val === 'object' && !Array.isArray(val)) {
          ...
          if (entries.length > 0) {
            text += `${label}\n`;
            entries.forEach(([subKey, subVal]) => { text += `${humanizeKey(subKey)}: ${fmtVal(subVal)}\n`; });
            text += '\n';
          }
        }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== WORK RESTRICTIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Work Restrictions ${idx + 1}\n${'='.repeat(40)}\n\n`;
      if (hasVal(r.date)) text += `Date: ${formatDate(r.date)}\n`;
      if (hasVal(r.type)) text += `Type: ${r.type}\n`;
      if (hasVal(r.status)) text += `Status: ${r.status}\n`;
      text += '\n';
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; stageDraft(record, fn, idx, sid, editKey, currentArr); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  /* ═══════ RENDER: OBJECT FIELD (modifiedDuty, results) — dynamic-key, per-subfield editable via dot-path ═══════ */
  const renderObjectField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    if (!hasVal(val) || typeof val !== 'object' || Array.isArray(val)) return null;
    const label = FIELD_LABELS[fn] || fn;
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
    const entries = Object.entries(val).filter(([, v]) => hasVal(v) && typeof v !== 'object');
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {entries.map(([subKey, subVal]) => {
          ...
          if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
            const phrase = searchTerm.toLowerCase().trim();
            const ll = label.toLowerCase(), sl = subLabel.toLowerCase();
            if (!ll.includes(phrase) && !phrase.includes(ll) && !sl.includes(phrase) && !displayVal.toLowerCase().includes(phrase)) return null;
          }

          return (
            <div key={subKey}>
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(String(subVal)); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const curObj = { ...(getFieldValue(record, fn, idx) && typeof getFieldValue(record, fn, idx) === 'object' ? getFieldValue(record, fn, idx) : {}) }; curObj[subKey] = editValue; stageDraft(record, fn, idx, sid, editKey, curObj); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <>
                    <div className="row-content">
                      <span className="content-value"><strong>{highlightText(subLabel)}:</strong> {highlightText(displayVal)}</span>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="work-restrictions-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Work Restrictions</h2></div>
        <div className="empty-state">No work restrictions records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="work-restrictions-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Work Restrictions</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<WorkRestrictionsDocumentPDFTemplate document={pdfData} />} fileName={`work-restrictions-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search restrictions, duration, provider..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(record.type ? `Work Restrictions ${idx + 1} - ${record.type}` : `Work Restrictions ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'clinical-findings')}
            {renderSection(record, idx, 'restrictions-details')}
...
export default WorkRestrictionsDocument;
