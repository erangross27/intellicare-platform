/**
 * SocialFunctionalAssessmentDocument.jsx
 * June 2026 — Collection: social_functional_assessment (NEW template)
 *
 * Cloned from PointOfCareUltrasoundHeartRateDocument (identical generic shape):
 *   date header renderDateField; OBJECT recursive render; ARRAY renderArrayField (string arrays) +
 *   date-grouped renderRecommendationsField; STRING per-sentence; 4-level search; per-section approve;
 *   Copy row/Section/All; pdfData memo; blue-glow theme.
 *
 * FIELDS (18 → 100% schema coverage):
 *   DATE:   date
 *   STRING: livingSituation, financialStatus, transportation, drivingStatus, type, provider,
 *           facility, findings, assessment, plan, notes, status
 *   ARRAY:  socialActivities (string[]), communityResources (string[]),
 *           recommendations (array of {recommendation, date})
 *   OBJECT: supportSystem, results
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import SocialFunctionalAssessmentDocumentPDFTemplate from '../pdf-templates/SocialFunctionalAssessmentDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SocialFunctionalAssessmentDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [trackKey]: { localKey, localValue, payload } } }
     trackKey   — unique per editable target (matches editedFields/editedSentences key)
     localKey   — the localEdits key (`${field}-${idx}`); whole-field value lives here for render
     localValue — value to store at localEdits[localKey]
     payload    — exact { field, value } that the original save() would have PUT to the DB */
const DRAFT_KEY = 'social_functional_assessmentPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  'living-support': ['livingSituation', 'supportSystem', 'financialStatus', 'transportation', 'drivingStatus'],
  'social-engagement': ['socialActivities', 'communityResources'],
...
const SECTION_ORDER = ['living-support', 'social-engagement', 'clinical', 'results-section', 'recommendations-section', 'notes-status'];

const DATE_FIELDS = ['date'];
const STRING_FIELDS = ['livingSituation', 'financialStatus', 'transportation', 'drivingStatus', 'type', 'provider', 'facility', 'findings', 'assessment', 'plan', 'notes', 'status'];
/* DISPLAY/EDIT recursive object fields */
const OBJECT_FIELDS = ['supportSystem', 'results'];
/* Plain string arrays (editable tags/lines) */
const STRING_ARRAY_FIELDS = ['socialActivities', 'communityResources'];
/* Array-of-objects {recommendation, date} (date-grouped, editable) */
const OBJECT_ARRAY_FIELDS = ['recommendations'];

const KEY_OVERRIDES = {
  adl: 'ADL', iadl: 'IADL', poc: 'POC', snf: 'SNF',
};
...
/* ═══════ COMPONENT ═══════ */
const SocialFunctionalAssessmentDocument = ({ document: docProp, data, templateData }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [copiedSection, setCopiedSection] = useState(null);
  const [copiedItems, setCopiedItems] = useState({});
  const [showCopied, setShowCopied] = useState(false);
  const [localEdits, setLocalEdits] = useState({});
  // localEdits keys (`${field}-${idx}`) that are staged drafts (saved locally, NOT yet committed
  // to DB/PDF). Cleared on Approve so committed values then flow into pdfData/PDF.
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.social_functional_assessment) return Array.isArray(r.social_functional_assessment) ? r.social_functional_assessment : [r.social_functional_assessment];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.social_functional_assessment) return Array.isArray(dd.social_functional_assessment) ? dd.social_functional_assessment : [dd.social_functional_assessment]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      if (!recDrafts) return;
      Object.entries(recDrafts).forEach(([trackKey, entry]) => {
        if (!entry || typeof entry !== 'object') return;
        const localKey = entry.localKey;
        if (!localKey) return;
        // localKey is always `${field}-${origIdx}` — remap its idx to the current render index.
        const remappedLocal = localKey.replace(/-\d+$/, `-${idx}`);
        nLocal[remappedLocal] = entry.localValue;
        nPending[remappedLocal] = true;
        // trackKey embeds the original idx as its first `-<number>` group — remap to current idx.
        const remapped = trackKey.replace(/-\d+/, `-${idx}`);
        // Sentence/array/leaf-style markers (suffix -s/-a/-r<n>) go to editedSentences; whole-field
        // (and dotted object-leaf) markers go to editedFields — matching how the save handlers set them.
        if (/-(?:s|a|r)\d+$/.test(remapped)) {
          nSentences[remapped] = 'edited';
        } else {
          nFields[remapped] = 'edited';
        }
      });
    });
  ...
  /* Stage one edit as a DRAFT (localStorage). NO DB write here — Approve commits it.
     recordId : record _id (string)
     localKey : localEdits key (`${field}-${idx}`) — whole-field value for render
     localValue : value to store at localEdits[localKey]
     trackKey : unique per editable target (matches editedFields/editedSentences key)
     payload : exact { field, value } the original save would have PUT to the DB */
  const stageDraft = useCallback((recordId, localKey, localValue, trackKey, payload) => {
    const store = readDrafts();
    if (!store[recordId]) store[recordId] = {};
    store[recordId][trackKey] = { localKey, localValue, payload };
    writeDrafts(store);
  }, []);
  ...
  const arrToText = useCallback((arr) => {
    ...
    return arr.map(a => fmtScalar(a)).join(' ');
  }, []);
  ...
  const fieldSearchText = useCallback((f, val) => {
    ...
    if (STRING_ARRAY_FIELDS.includes(f)) return arrToText(val);
    if (OBJECT_FIELDS.includes(f)) return flattenSearchable(val);
    return fmtVal(val);
  }, [recsToText, arrToText, fmtVal]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `social & functional assessment ${idx + 1}`.toLowerCase();
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, valueOverride, editTrackingKey) => {
    ...
    stageDraft(id, localKey, saveVal, trackKey, { field: fn, value: saveVal });
    setEditingField(null); setEditValue(''); setSaveError(null);
  }, [editValue, safeId, stageDraft]);

  /* save a nested OBJECT leaf by dot-path (e.g. results.score) — value stays a STRING */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    // Build the updated whole-object value for local render (Approve PUTs only the dotted leaf).
    const cur = localEdits[localKey] !== undefined ? localEdits[localKey] : record[rootField];
    const clone = JSON.parse(JSON.stringify(cur ?? {}));
    let node = clone;
    for (let i = 0; i < path.length - 1; i++) node = node[path[i]];
    node[path[path.length - 1]] = newVal;
    // Stage a DRAFT (no DB write). Payload carries the dotted leaf path for Approve.
    setLocalEdits(prev => ({ ...prev, [localKey]: clone }));
    setPendingEdits(prev => ({ ...prev, [localKey]: true }));
    setEditedFields(prev => ({ ...prev, [leafKeyTrack]: 'edited' }));
    setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
    stageDraft(id, localKey, clone, leafKeyTrack, { field: dottedField, value: newVal });
    setEditingField(null); setEditValue(''); setSaveError(null);
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      // Stage a DRAFT (no DB write). Approve commits the whole field value.
      setLocalEdits(prev => ({ ...prev, [localKey]: fullText }));
      setPendingEdits(prev => ({ ...prev, [localKey]: true }));
      setEditedSentences(prev => ({ ...prev, [`${fn}-${idx}-s${sentenceIdx}`]: 'edited' }));
      setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
      stageDraft(id, localKey, fullText, `${fn}-${idx}-s${sentenceIdx}`, { field: fn, value: fullText });
      setEditingField(null); setEditValue(''); setSaveError(null);
      return;
    }
  ...
  // then clear pending so the committed values flow into pdfData/PDF.
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Collect this record's drafts whose trackKey belongs to a field in this section, this idx.
      const store = readDrafts();
      const recDrafts = store[id] || {};
      const toCommit = Object.entries(recDrafts).filter(([trackKey]) =>
        fields.some(f => trackKey.startsWith(`${f}-${idx}-`) || trackKey === `${f}-${idx}`)
      );
      // Persist each staged edit now (field, value — dotted field for object leaves).
      for (const [, entry] of toCommit) {
        if (!entry || !entry.payload) continue;
        const resp = await secureApiClient.put(`/api/edit/social_functional_assessment/${id}/edit`, entry.payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the record/section approved (audit trail)
      await secureApiClient.put(`/api/edit/social_functional_assessment/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF.
      const committedLocalKeys = toCommit.map(([, e]) => e && e.localKey).filter(Boolean);
      ...
      if (store[id]) {
        toCommit.forEach(([trackKey]) => { delete store[id][trackKey]; });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_ARRAY_FIELDS.includes(f)) {
        ...
        if (!sameAsTitle) text += `${label}\n`;
        arr.forEach((a, i) => { text += `${i + 1}. ${fmtScalar(a)}\n`; });
        text += '\n';
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== SOCIAL & FUNCTIONAL ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Social & Functional Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: STRING ARRAY FIELD (editable per-item) ═══════ */
                  ...
                  <div className="edit-field-container">
                    ...
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => {
                        ...
                        const newArr = currentArr.map((a, ai) => ai === i ? trimmed : a);
                        const localKey = `${fn}-${idx}`;
                        // Stage a DRAFT (no DB write). Approve commits the whole array.
                        setLocalEdits(prev => ({ ...prev, [localKey]: newArr }));
                        setPendingEdits(prev => ({ ...prev, [localKey]: true }));
                        setEditedSentences(prev => ({ ...prev, [itemKey]: 'edited' }));
                        setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
                        stageDraft(id2, localKey, newArr, itemKey, { field: fn, value: newArr });
                        setEditingField(null); setEditValue(''); setSaveError(null);
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (STRING_ARRAY_FIELDS.includes(f)) return renderArrayField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="social-functional-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Social &amp; Functional Assessment</h2></div>
        <div className="empty-state">No social &amp; functional assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="social-functional-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Social &amp; Functional Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<SocialFunctionalAssessmentDocumentPDFTemplate document={pdfData} />} fileName={`social-functional-assessment-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search social & functional assessment records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Social & Functional Assessment ${idx + 1}`)}</h3>
            </div>
...
export default SocialFunctionalAssessmentDocument;
