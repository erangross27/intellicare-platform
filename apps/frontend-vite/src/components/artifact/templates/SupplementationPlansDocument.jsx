/**
 * SupplementationPlansDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: supplementation_plans
 *
 * 5 Sections:
 *   1. supplement-details: supplement, dosage, condition, status, type
 *   2. clinical-reasoning: reasoning, findings
 *   3. provider-info: provider, facility, date
 *   4. assessment-plan: assessment, plan
 *   5. recommendations-notes: recommendations, notes
 */
...
const SECTION_TITLES = {
  'supplement-details': 'Supplement Details',
...
const SECTION_FIELDS = {
  'supplement-details': ['supplement', 'dosage', 'condition', 'status', 'type'],
  'clinical-reasoning': ['reasoning', 'findings'],
...
/* recText: display text from an array item that may be a {recommendation,...} object */
const recText = (item) => (item && typeof item === 'object' && !Array.isArray(item)) ? String(item.recommendation ?? item.text ?? item.name ?? item.value ?? '') : String(item ?? '');
const NUMBER_FIELDS = [];
const STRING_FIELDS = ['supplement', 'dosage', 'condition', 'status', 'type', 'reasoning', 'findings', 'provider', 'facility', 'assessment', 'plan', 'notes'];
...
/* ═══════ COMPONENT ═══════ */
const SupplementationPlansDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.supplementation_plans) return Array.isArray(r.supplementation_plans) ? r.supplementation_plans : [r.supplementation_plans];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.supplementation_plans) return Array.isArray(dd.supplementation_plans) ? dd.supplementation_plans : [dd.supplementation_plans]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // fieldPart is "field" (value = full field value) or "field.arrayIndex" (legacy)
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Supplementation Plan ${idx + 1}`.toLowerCase();
  ...
  // sid is needed to drop the matching approve flag so the button returns to yellow Pending Approve.
  const stageDraft = useCallback((record, fn, idx, sid, value, trackingKey) => {
    const id = safeId(record); if (!id) return;
    const editKey = `${fn}-${idx}`;
    setLocalEdits(prev => ({ ...prev, [editKey]: value }));
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
    setEditedFields(prev => ({ ...prev, [trackingKey || editKey]: 'edited' }));
    if (sid) setApprovedSections(prev => { const k = `${sid}-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; });
    // Persist the full field value under the field name (drafts survive refresh).
    const store = readDrafts();
  ...
  // Stage one sentence edit as a DRAFT (no DB write). localStorage keeps it across refresh; Approve commits it.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // staged editKeys for THIS record + this section's fields
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/supplementation_plans/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/supplementation_plans/${id}/approve`, { sectionId: sid, approved: true });
      ...
      const store = readDrafts();
      if (store[id]) {
        toCommit.forEach(k => { const fp = k.slice(0, -suffix.length); const di = fp.lastIndexOf('.'); const base = (di !== -1 && /^\d+$/.test(fp.slice(di + 1))) ? fp.slice(0, di) : fp; delete store[id][base]; });
    ...
    } catch (err) { console.error('[SupplementationPlans] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        text += `${label}\n${items.map((item, i) => `${i + 1}. ${recText(item)}`).join('\n')}\n\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== SUPPLEMENTATION PLANS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Supplementation Plan ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; const orig = currentArr[itemIdx]; currentArr[itemIdx] = (orig && typeof orig === 'object' && !Array.isArray(orig)) ? { ...orig, recommendation: editValue } : editValue; stageDraft(record, fn, idx, sid, currentArr, editKey); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="supplementation-plans-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Supplementation Plans</h2></div>
        <div className="empty-state">No supplementation plans records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="supplementation-plans-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Supplementation Plans</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<SupplementationPlansDocumentPDFTemplate document={pdfData} />} fileName={`supplementation-plans-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search supplementation plans..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Supplementation Plans ${idx + 1}`)}</h3>
              {record.status && <span className="status-badge">{highlightText(record.status)}</span>}
            </div>
            {renderSection(record, idx, 'supplement-details')}
            {renderSection(record, idx, 'clinical-reasoning')}
