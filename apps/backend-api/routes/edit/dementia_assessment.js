/**
 * Dementia Assessment Edit Route
 * ServiceId: dementia-assessment-edit-service
...
function buildContext(req, operation = 'read') { return { serviceId: 'dementia-assessment-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }

const ALLOWED_FIELDS = [
  'date', 'provider', 'facility', 'status', 'dementiaType', 'cdrsScore',
  'functionalStatus.adls', 'functionalStatus.iadls',
  'behavioralSymptoms',
  'cognitiveEnhancers',
  'caregiverBurden',
  'safetyAssessment.driving', 'safetyAssessment.homeAlone', 'safetyAssessment.medication', 'safetyAssessment.wandering',
  'advanceDirectives.healthcarePowerOfAttorney', 'advanceDirectives.livingWill', 'advanceDirectives.financialPowerOfAttorney',
  'findings', 'assessment', 'plan',
  'results', 'recommendations', 'notes',
];

// Object fields whose nested leaves are edited via dotted paths (e.g. "results.foo.bar")
const ALLOWED_FIELD_PREFIXES = ['results.'];
function isFieldAllowed(field) {
  if (ALLOWED_FIELDS.includes(field)) return true;
  return ALLOWED_FIELD_PREFIXES.some(p => field.startsWith(p));
}
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('dementia_assessment', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[DementiaAssessment] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('dementia_assessment', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[DementiaAssessment] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
