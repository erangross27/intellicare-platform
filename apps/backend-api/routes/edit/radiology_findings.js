...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'radiology-findings-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'modalityUsed', 'technique', 'date', 'facility', 'provider',
  'contrast.type', 'contrast.amount', 'contrast.reaction',
  'findings', 'comparison', 'impression',
  'biRads', 'tirads', 'pirads',
  'results',
  'recommendations', 'assessment', 'plan', 'notes', 'status',
];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('radiology_findings', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[radiology_findings] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('radiology_findings', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[radiology_findings] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
