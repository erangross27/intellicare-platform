...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'preventive-biomarkers-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['biomarkerType', 'biomarkerName', 'panelName', 'resultValue', 'resultUnit', 'referenceRange', 'riskCategory', 'abnormalFlag', 'calculatedRiskScore', 'testMethodology', 'specimenType', 'fastingStatus', 'qualityControlStatus', 'indicationForTesting', 'diseaseRiskAssessment', 'trendComparison', 'clinicalSignificance', 'riskFactorsPresent', 'interventionsRecommended', 'recommendedFollowUp', 'retestInterval', 'provider', 'facility', 'interpretingPhysician', 'aiProcessed', 'date'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('preventive_biomarkers', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[preventive_biomarkers] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('preventive_biomarkers', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[preventive_biomarkers] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
