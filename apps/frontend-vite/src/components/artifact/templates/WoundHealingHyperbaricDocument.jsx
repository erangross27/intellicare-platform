/**
 * WoundHealingHyperbaricDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: wound_healing_hyperbaric
 *
 * 6 Sections:
 *   1. wound-assessment: woundEtiology, woundLocationAnatomical, woundDimensionsCm, wagnerClassification
 *   2. vascular-status: ankleBrachialIndex, toeBrachialIndex, transcutaneousOxygenPressure
 *   3. hbot-treatment: hbotTreatmentPressureATA, hbotTreatmentDurationMinutes, hbotTotalSessionsCompleted
 *   4. wound-status: woundBedTissuePercentage, periWoundSkinCondition, woundExudateCharacteristics, infectionSignsPresent, osteomyelitisStatus
 *   5. patient-metrics: diabeticFootRiskCategory, hemoglobinA1cPercent, serumAlbuminGdL, prealbumin
 *   6. management-plan: offloadingDeviceType, compressionTherapyRegimen, woundHealingTrajectory, hbotContraindicationsScreened, middleEarBarotraumaRisk, postHbotTcpo2Response
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import WoundHealingHyperbaricDocumentPDFTemplate from '../pdf-templates/WoundHealingHyperbaricDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './WoundHealingHyperbaricDocument.css';
...
const DRAFT_KEY = 'wound_healing_hyperbaricPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['woundEtiology', 'woundLocationAnatomical', 'woundDimensionsCm', 'woundBedTissuePercentage', 'periWoundSkinCondition', 'woundExudateCharacteristics', 'osteomyelitisStatus', 'diabeticFootRiskCategory', 'offloadingDeviceType', 'compressionTherapyRegimen', 'woundHealingTrajectory', 'middleEarBarotraumaRisk'];
const DATE_FIELDS = [];
...
/* ======= COMPONENT ======= */
const WoundHealingHyperbaricDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.wound_healing_hyperbaric) return Array.isArray(r.wound_healing_hyperbaric) ? r.wound_healing_hyperbaric : [r.wound_healing_hyperbaric];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.wound_healing_hyperbaric) return Array.isArray(dd.wound_healing_hyperbaric) ? dd.wound_healing_hyperbaric : [dd.wound_healing_hyperbaric]; return [dd]; }
      return [r];
    });
  ...
  /* ======= UTILS ======= */
  /* Epoch-zero sentinel: some records store a Date (1970-01-01T00:00:00Z) where a
     free-text string is expected (e.g. woundExudateCharacteristics). Treat as no-value. */
  const isEpochSentinel = (v) => {
    if (v == null) return false;
    const s = typeof v === 'string' ? v : (v.$date ? String(v.$date) : (v instanceof Date ? v.toISOString() : ''));
    return /^1970-01-01T00:00:00(\.000)?Z$/.test(s);
  };
  const hasVal = useCallback((v) => { if (v === null || v === undefined || v === '') return false; if (isEpochSentinel(v)) return false; if (typeof v === 'boolean') return true; if (typeof v === 'number') return true; if (typeof v === 'string') return v.trim() !== ''; if (Array.isArray(v)) return v.length > 0; if (typeof v === 'object') return Object.keys(v).length > 0; return true; }, []);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Wound Healing Hyperbaric ${idx + 1}`.toLowerCase();
  ...
  // Stage a full-field DRAFT locally (no DB write). localStorage keeps it across refresh; Approve commits it.
  const stageDraft = useCallback((record, fn, idx, sid, value, sentenceMarks) => {
    const id = safeId(record); if (!id) return;
    const editKey = `${fn}-${idx}`;
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // Pending edits for this record live under `${field}-${idx}` (localEdits holds the full field value).
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k]) return false;
        const m = k.match(/^(.+)-(\d+)$/);
        if (!m || parseInt(m[2], 10) !== idx) return false;
        // arrayIndex ONLY when the trailing dot-segment is purely numeric (none here — full field values)
        const last = m[1].slice(m[1].lastIndexOf('.') + 1);
        const baseField = /^\d+$/.test(last) ? m[1].slice(0, m[1].lastIndexOf('.')) : m[1];
        return fields.includes(baseField);
      });
      for (const editKey of toCommit) {
        const m = editKey.match(/^(.+)-(\d+)$/);
        const fieldPart = m[1];
        const last = fieldPart.slice(fieldPart.lastIndexOf('.') + 1);
        const payload = { value: localEdits[editKey] };
        if (/^\d+$/.test(last) && fieldPart.includes('.')) { payload.field = fieldPart.slice(0, fieldPart.lastIndexOf('.')); payload.arrayIndex = parseInt(last, 10); }
        else { payload.field = fieldPart; }
        const resp = await secureApiClient.put(`/api/edit/wound_healing_hyperbaric/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/wound_healing_hyperbaric/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[WoundHealingHyperbaric] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== WOUND HEALING HYPERBARIC ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Wound Healing Hyperbaric ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="wound-healing-hyperbaric-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Wound Healing Hyperbaric</h2></div>
        <div className="empty-state">No wound healing hyperbaric records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="wound-healing-hyperbaric-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Wound Healing Hyperbaric</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<WoundHealingHyperbaricDocumentPDFTemplate document={pdfData} />} fileName={`wound-healing-hyperbaric-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search wound healing hyperbaric records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Wound Healing Hyperbaric ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'wound-assessment')}
            {renderSection(record, idx, 'vascular-status')}
            {renderSection(record, idx, 'hbot-treatment')}
            {renderSection(record, idx, 'wound-status')}
...
export default WoundHealingHyperbaricDocument;
