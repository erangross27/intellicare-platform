/**
 * OccupationalHealthAssessmentDocument.jsx
 * July 2026 — Canonical one-pass polish: inline editing, blue glow theme
 * Collection: occupational_health_assessment
 * API: /api/edit/occupational_health_assessment
 *
 * 6 Sections:
 *   1. assessment-info: date (record date, BlueDatePicker), employerName, jobTitle, employmentStartDate, assessmentType
 *   2. fitness-determination: fitForDutyDetermination
 *   3. assessment-indicators: previousWorkInjury, ergonomicAssessmentNeeded, hearingConservationProgram,
 *                             lostWorkDays, nextAssessmentDue, respiratoryClearance, audiometryBaseline,
 *                             medicalSurveillanceProgram, clearanceExpirationDate
 *   4. work-environment: workEnvironmentHazards[], physicalDemands[], personalProtectiveEquipment[], chemicalExposures[]
 *   5. restrictions-accommodations: workRestrictions[], accommodationsRecommended[]
 *   6. monitoring-immunization: biologicalMonitoring[], immunizationRequirements[]
 *
 * NOTE: `date` is the record date — rendered as an editable BlueDatePicker field (first field of the
 *       Assessment Information section, NEVER a header pill). Booleans render BOTH true and false
 *       (Yes/No via BlueSelect). lostWorkDays hides when exactly 0.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import OccupationalHealthAssessmentDocumentPDFTemplate from '../pdf-templates/OccupationalHealthAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './OccupationalHealthAssessmentDocument.css';
...
const DRAFT_KEY = 'occupational_health_assessmentPendingEdits';
const readDrafts = () => {
...
/* medicalSurveillanceProgram is a short narrative / comma list -> split into item rows. */
const SENTENCE_FIELDS = ['medicalSurveillanceProgram'];
...
/* NUMBER_FIELDS stored as real numbers -> typed number input saving parseFloat(value). */
...
/* DATE_FIELDS: real record `date` (2026-01-15) -> BlueDatePicker, rendered as the first field of the
...
/* lostWorkDays 0 means "no lost work days recorded" — hidden when exactly 0. */
...
/* splitByComma: parenthesis-aware comma split (whitespace-after-comma guard keeps "$1,000" style intact) */
...
/* formatDate: Date / {$date} / ISO string -> "January 15, 2026" (record date field) */
...
/* ===== COMPONENT ===== */
const OccupationalHealthAssessmentDocument = ({ document: docProp, data, templateData }) => {
  ...
  /* hasFieldVal: presence check with per-field zero hiding (4-AREA RULE: used by JSX, Copy Section, Copy All and mirrored in PDF) */
  ...
  /* numberShows: a NUMBER_FIELDS value is present when it is a finite number (or numeric string),
     applying per-field zero hiding via hasFieldVal. */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      const docTitle = 'occupational health assessment';
      const rt = `occupational health assessment ${oi + 1}`;
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/occupational_health_assessment/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/occupational_health_assessment/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== OCCUPATIONAL HEALTH ASSESSMENT ===\n\n';
    pdfData.forEach((r) => {
      const oi = r._originalIdx ?? 0;
      text += `Occupational Health Assessment ${oi + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ===== RENDER: DATE FIELD (BlueDatePicker — saves ISO 'YYYY-MM-DDT00:00:00.000Z') ===== */
  ...
  /* ===== RENDER: SIMPLE FIELD (no splitting — short text / enum values) ===== */
  ...
  /* ===== RENDER: NUMBER FIELD (num-stepper — saves real Number via parseFloat, not a string) ===== */
  ...
  /* ===== RENDER: BOOLEAN FIELD (Yes/No BlueSelect — saves REAL booleans; false still renders "No") ===== */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          /* saveArrayItem — stages a DRAFT locally (full array under `${fn}-${idx}`); committed on Pending Approve. */
        ...
        <div key={fn}>
          <div className="rec-mini-card">
            ...
            {sentences.map((sentence, sIdx) => {
              ...
              /* Regular sentence row — saveThisSentence shared by Save button AND Ctrl+Enter */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="occupational-health-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Occupational Health Assessment</h2></div>
        <div className="empty-state">No occupational health assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="occupational-health-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Occupational Health Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OccupationalHealthAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Occupational_Health_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search occupational health assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          return (
            <div key={safeId(record) || idx} className="record-card">
              ...
              <div className="record-header">
                <h3 className="record-name">{highlightText(`Occupational Health Assessment ${origIdx + 1}`)}</h3>
              </div>
              {renderSection(record, origIdx, 'assessment-info')}
              {renderSection(record, origIdx, 'fitness-determination')}
              {renderSection(record, origIdx, 'assessment-indicators')}
              {renderSection(record, origIdx, 'work-environment')}
              {renderSection(record, origIdx, 'restrictions-accommodations')}
              {renderSection(record, origIdx, 'monitoring-immunization')}
            </div>
          );
        })}
      </div>
...
export default OccupationalHealthAssessmentDocument;
