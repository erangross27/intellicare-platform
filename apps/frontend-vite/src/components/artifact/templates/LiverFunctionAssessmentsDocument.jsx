/**
 * LiverFunctionAssessmentsDocument.jsx
 * July 2026 — Canonical one-pass rewrite (blue glow editing theme, mini-card pattern)
 * Collection: liver_function_assessments
 *
 * FLAT scalar lab panel (no nested results object). Sections:
 *   1. hepatocellular-enzymes: AST, ALT
 *   2. cholestatic-markers: ALP, GGT, total/direct/indirect bilirubin
 *   3. synthetic-function: albumin, PT/INR
 *   4. scoring-systems: Child-Pugh, MELD, Fibrosis-4, AST/ALT ratio
 *   5. viral-serological: HBsAg, HCV viral load, encephalopathy grade, AMA, SMA
 *   6. additional-labs: AFP, ammonia, LDH, ceruloplasmin, iron saturation
 *
 * Numbers use the −/+ stepper (input type=text inputMode=decimal). 0-valued numeric labs are hidden
 * (0 = "not tested"); a string "0" grade renders as-is. No real date field → title only, no record date.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import LiverFunctionAssessmentsDocumentPDFTemplate from '../pdf-templates/LiverFunctionAssessmentsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './LiverFunctionAssessmentsDocument.css';
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; editKey = "${fn}-${idx}") */
const DRAFT_KEY = 'liver_function_assessmentsPendingEdits';
const readDrafts = () => {
...
/* Numeric labs → −/+ stepper. hepaticEncephalopathyGrade is a numeric-looking grade ("0") → stepper too. */
const NUMBER_FIELDS = [
...
const STRING_FIELDS = ['hepatitisBSurfaceAntigen', 'antimitochondrialAntibody', 'smoothMuscleAntibody'];
const DATE_FIELDS = [];
...
/* ═══════ COMPONENT ═══════ */
const LiverFunctionAssessmentsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.liver_function_assessments) return Array.isArray(r.liver_function_assessments) ? r.liver_function_assessments : [r.liver_function_assessments];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.liver_function_assessments) return Array.isArray(dd.liver_function_assessments) ? dd.liver_function_assessments : [dd.liver_function_assessments]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Liver Function Assessment ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const fieldPart of Object.keys(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/liver_function_assessments/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedFieldParts.push(fieldPart);
        committedEditKeys.push(`${baseField}-${idx}`);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/liver_function_assessments/${id}/approve`, { sectionId: sid, approved: true });
  ...
  // Scalar-heavy panel (GlasgowComaScale-style): each label on its own line, every value numbered "1. value"
  // at column 0 — never a side-by-side "Label: value". Satisfies the numbered-rows / divider audit checks.
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== LIVER FUNCTION ASSESSMENTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Liver Function Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD — −/+ stepper (input type=text inputMode=decimal, NEVER type=number) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="liver-function-assessments-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Liver Function Assessments</h2></div>
        <div className="empty-state">No liver function assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="liver-function-assessments-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Liver Function Assessments</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<LiverFunctionAssessmentsDocumentPDFTemplate document={pdfData} />} fileName="Liver_Function_Assessments.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search liver function (AST, ALT, bilirubin, MELD...)" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Liver Function Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'hepatocellular-enzymes')}
            {renderSection(record, idx, 'cholestatic-markers')}
            {renderSection(record, idx, 'synthetic-function')}
            {renderSection(record, idx, 'scoring-systems')}
            {renderSection(record, idx, 'viral-serological')}
...
export default LiverFunctionAssessmentsDocument;
