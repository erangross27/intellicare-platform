...
const COLLECTION = 'care_coordination';
const COMMA_SPLIT_FIELDS = [];
const ARRAY_FIELDS = new Set(['primaryDiagnoses', 'activeMedications', 'dischargeMedications', 'pendingTests', 'medicalEquipmentNeeds']);
const OBJECT_FIELDS = new Set(['homeHealthServices', 'caregiverInformation', 'socialDeterminants', 'advanceDirectives', 'followUpAppointments', 'patientEducationProvided']);
const NARRATIVE_FIELDS = new Set(['referralReason', 'careCoordinator', 'functionalStatus', 'mobilityLevel', 'fallRiskAssessment', 'cognitiveStatus', 'insuranceAuthorization', 'languageBarriers', 'culturalConsiderations']);
const DATE_FIELDS = new Set(['referralDate']);
const ZERO_SENTINEL_FIELDS = new Set(['readmissionRiskScore']);
const KEY_OVERRIDES = {};
const LABELS = {
  referralDate: 'Referral Date', referralSource: 'Referral Source', referralDestination: 'Referral Destination', referralReason: 'Referral Reason', transitionType: 'Transition Type', careCoordinator: 'Care Coordinator',
  primaryDiagnoses: 'Primary Diagnoses', activeMedications: 'Active Medications', dischargeMedications: 'Discharge Medications',
  followUpAppointments: 'Follow-Up Appointments', pendingTests: 'Pending Tests', medicalEquipmentNeeds: 'Medical Equipment Needs',
  homeHealthServices: 'Home Health Services', caregiverInformation: 'Caregiver Information', socialDeterminants: 'Social Determinants', advanceDirectives: 'Advance Directives',
  functionalStatus: 'Functional Status', mobilityLevel: 'Mobility Level', fallRiskAssessment: 'Fall Risk Assessment', cognitiveStatus: 'Cognitive Status',
  insuranceAuthorization: 'Insurance Authorization', languageBarriers: 'Language Barriers', culturalConsiderations: 'Cultural Considerations',
  patientEducationProvided: 'Patient Education Provided', readmissionRiskScore: 'Readmission Risk Score',
};
const SECTIONS = [
  { title: 'Referral', fields: ['referralDate', 'referralSource', 'referralDestination', 'referralReason', 'transitionType', 'careCoordinator'] },
  { title: 'Primary Diagnoses', fields: ['primaryDiagnoses'] },
  { title: 'Active Medications', fields: ['activeMedications'] },
  { title: 'Discharge Medications', fields: ['dischargeMedications'] },
  { title: 'Follow-Up Appointments', fields: ['followUpAppointments'] },
  { title: 'Pending Tests', fields: ['pendingTests'] },
  { title: 'Medical Equipment Needs', fields: ['medicalEquipmentNeeds'] },
  { title: 'Home Health Services', fields: ['homeHealthServices'] },
  { title: 'Caregiver Information', fields: ['caregiverInformation'] },
  { title: 'Social Determinants', fields: ['socialDeterminants'] },
  { title: 'Advance Directives', fields: ['advanceDirectives'] },
  { title: 'Functional Status', fields: ['functionalStatus'] },
  { title: 'Mobility Level', fields: ['mobilityLevel'] },
  { title: 'Fall Risk Assessment', fields: ['fallRiskAssessment'] },
  { title: 'Cognitive Status', fields: ['cognitiveStatus'] },
  { title: 'Insurance Authorization', fields: ['insuranceAuthorization'] },
  { title: 'Language Barriers', fields: ['languageBarriers'] },
  { title: 'Cultural Considerations', fields: ['culturalConsiderations'] },
  { title: 'Patient Education Provided', fields: ['patientEducationProvided'] },
  { title: 'Readmission Risk', fields: ['readmissionRiskScore'] },
];
...
const CareCoordinationDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Care Coordination</Text><Text style={styles.noData}>No care coordination data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Care Coordination</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Care Coordination {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
export default CareCoordinationDocumentPDFTemplate;
