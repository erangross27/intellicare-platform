/**
 * Addiction Medicine Consultations Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/addiction_medicine_consultations/:id/edit    — Edit a single field
 * PUT /api/edit/addiction_medicine_consultations/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'addiction-medicine-consultations-edit-service',
    userId: req.user?.id,
    operation,
...
// Allowed editable fields — parent field names for dot-notation support
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    // Support arrayIndex for array fields (both top-level and nested)
    ...
    const result = await sda.update('addiction_medicine_consultations', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[AddictionMedicineConsultations] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('addiction_medicine_consultations', { _id: objectId }, {
      $set: {
    ...
    console.error('[AddictionMedicineConsultations] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
