/**
 * SocialDeterminantsOfHealth Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/social_determinants_of_health/:id/edit    — Edit a single field
 * PUT /api/edit/social_determinants_of_health/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'social-determinants-of-health-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('social_determinants_of_health', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[SocialDeterminantsOfHealth] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('social_determinants_of_health', { _id: objectId }, {
      $set: {
    ...
    console.error('[SocialDeterminantsOfHealth] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
