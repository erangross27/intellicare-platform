 ...
 * Collection: sleep_hygiene_education (displayed as "Sleep Hygiene Education")
 *
 * 8 Sections:
 *   1. record-info: date, primarySleepComplaint, epworthSleepinessScore
 *   2. sleep-metrics: averageSleepLatency, totalSleepTime, nighttimeAwakenings, screenTimeBeforeBed
 *   3. sleep-schedule: regularBedtime, regularWakeTime, weekendSleepSchedule, bedtimeRoutine
 *   4. lifestyle-factors: caffeineIntake, alcoholConsumption, nicotineUse, exerciseHabits, dinnertimeHabits, nappingFrequency
 *   5. sleep-environment: bedroomEnvironment
 *   6. education-interventions: educationTopicsCovered, behavioralInterventions, stimulusControlInstructions, sleepRestrictionTherapy
 *   7. techniques-materials: relaxationTechniques, educationalMaterials
 *   8. follow-up: followUpInterval, sleepDiaryRequested
 */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldName]: value } }  (value = string or array, mirroring localEdits) */
...
const FIELD_LABELS = {
  date: 'Date',
  primarySleepComplaint: 'Primary Sleep Complaint',
  epworthSleepinessScore: 'Epworth Sleepiness Score',
  averageSleepLatency: 'Average Sleep Latency',
  totalSleepTime: 'Total Sleep Time',
  nighttimeAwakenings: 'Nighttime Awakenings',
  screenTimeBeforeBed: 'Screen Time Before Bed',
  regularBedtime: 'Regular Bedtime',
  regularWakeTime: 'Regular Wake Time',
  weekendSleepSchedule: 'Weekend Sleep Schedule',
  bedtimeRoutine: 'Bedtime Routine',
  caffeineIntake: 'Caffeine Intake',
  alcoholConsumption: 'Alcohol Consumption',
  nicotineUse: 'Nicotine Use',
  exerciseHabits: 'Exercise Habits',
  dinnertimeHabits: 'Dinnertime Habits',
  ...
  stimulusControlInstructions: 'Stimulus Control Instructions',
  sleepRestrictionTherapy: 'Sleep Restriction Therapy',
  relaxationTechniques: 'Relaxation Techniques',
  educationalMaterials: 'Educational Materials',
  followUpInterval: 'Follow-Up Interval',
  sleepDiaryRequested: 'Sleep Diary Requested',
};

const SECTION_FIELDS = {
  'record-info': ['date', 'primarySleepComplaint', 'epworthSleepinessScore'],
  'sleep-metrics': ['averageSleepLatency', 'totalSleepTime', 'nighttimeAwakenings', 'screenTimeBeforeBed'],
  'sleep-schedule': ['regularBedtime', 'regularWakeTime', 'weekendSleepSchedule', 'bedtimeRoutine'],
  'lifestyle-factors': ['caffeineIntake', 'alcoholConsumption', 'nicotineUse', 'exerciseHabits', 'dinnertimeHabits', 'nappingFrequency'],
  'sleep-environment': ['bedroomEnvironment'],
  'education-interventions': ['educationTopicsCovered', 'behavioralInterventions', 'stimulusControlInstructions', 'sleepRestrictionTherapy'],
  'techniques-materials': ['relaxationTechniques', 'educationalMaterials'],
  'follow-up': ['followUpInterval', 'sleepDiaryRequested'],
};

const NUMBER_FIELDS = ['averageSleepLatency', 'totalSleepTime', 'nighttimeAwakenings', 'screenTimeBeforeBed', 'epworthSleepinessScore'];
const DATE_FIELDS = ['date'];
const BOOLEAN_FIELDS = ['nicotineUse', 'stimulusControlInstructions', 'sleepRestrictionTherapy', 'sleepDiaryRequested'];
const ARRAY_FIELDS = ['educationTopicsCovered', 'behavioralInterventions', 'relaxationTechniques', 'educationalMaterials'];
const STRING_FIELDS = ['primarySleepComplaint', 'bedtimeRoutine', 'regularBedtime', 'regularWakeTime', 'weekendSleepSchedule', 'caffeineIntake', 'alcoholConsumption', 'bedroomEnvironment', 'exerciseHabits', 'dinnertimeHabits', 'nappingFrequency', 'followUpInterval'];
...
const SleepHygieneEducationDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Sleep Hygiene Education ${idx + 1}`.toLowerCase();
  ...
  // editKey is always `${fn}-${idx}` (matches the keys pdfData/getFieldValue read). draftFn = field
  // name stored in the localStorage record bucket; draftVal = the value (string or full array).
  const stageDraft = useCallback((record, fn, idx, editKey, value, draftVal, sid) => {
    ...
    store[id][fn] = draftVal !== undefined ? draftVal : value;
    writeDrafts(store);
  }, [safeId]);
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    setSaveError(null);
    const editKey = `${fn}-${idx}`;
    stageDraft(record, fn, idx, editKey, saveVal, saveVal, sid);
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageDraft(record, fn, idx, `${fn}-${idx}`, fullText, fullText, sid);
  ...
  function saveCommaItem(record, fn, idx, sid, sIdx, ciIdx) {
    ...
    const rebuilt = `${p.label}: ${items.join(delimiter)}${isSemicolon ? '' : '.'}`;
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/sleep_hygiene_education/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/sleep_hygiene_education/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this record's committed drafts from localStorage
      const store = readDrafts();
      if (store[id]) { toCommit.forEach(k => { const fp = k.slice(0, -suffix.length); const base = /\.\d+$/.test(fp) ? fp.slice(0, fp.lastIndexOf('.')) : fp; delete store[id][base]; }); if (Object.keys(store[id]).length === 0) delete store[id]; writeDrafts(store); }
  ...
  const copyAllText = useCallback(async () => {
    ...
    pdfData.forEach((r, idx) => {
      text += `Sleep Hygiene Education ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; /* Stage as a DRAFT (no DB write). localStorage keeps it across refresh; Approve commits it. */ stageDraft(record, fn, idx, `${fn}-${idx}`, currentArr, currentArr, sid); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderStringField = (record, fn, idx, sid) => {
        ...
        <div key={fn}>
          <div className="rec-mini-card">
            ...
            {sentences.map((sentence, sIdx) => {
              ...
              /* Unlabeled sentence with 2+ comma items: split into rows */
              if (!parsed.isLabeled) {
    ...
    /* Single-value string: saveSentence */
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="sleep-hygiene-education-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Sleep Hygiene Education</h2></div>
        <div className="empty-state">No sleep hygiene education records available</div>
      </div>
    );
  }
    ...
    <div className="sleep-hygiene-education-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<SleepHygieneEducationDocumentPDFTemplate document={pdfData} />} fileName={`sleep-hygiene-education-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search sleep hygiene education..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Sleep Hygiene Education ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'record-info')}
            {renderSection(record, idx, 'sleep-metrics')}
            {renderSection(record, idx, 'sleep-schedule')}
            {renderSection(record, idx, 'lifestyle-factors')}
            {renderSection(record, idx, 'sleep-environment')}
            {renderSection(record, idx, 'education-interventions')}
            {renderSection(record, idx, 'techniques-materials')}
