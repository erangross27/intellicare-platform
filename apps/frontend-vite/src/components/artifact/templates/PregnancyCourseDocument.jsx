/**
 * PregnancyCourseDocument.jsx
 * June 2026 — FULL TEMPLATE STANDARD — 100% coverage, all 14 extractable fields typed.
 * Collection: pregnancy_course
 *
 * TYPED FIELDS:
 *   OBJECT (recursive renderObjectField/renderObjectNode/renderObjectLeaf):
 *     firstTrimester, secondTrimester, thirdTrimester, results
 *   DATE (date-picker): date
 *   ARRAY (per-item): recommendations
 *   STRING per-sentence narratives (splitBySentence + SENTENCE_FIELDS): findings, assessment, plan, notes
 *   STRING simple: type, provider, facility, status
 *
 * Sections:
 *   1. record-details: date, type, provider, facility, status
 *   2. clinical: findings, assessment, plan
 *   3. first-trimester: firstTrimester (object)
 *   4. second-trimester: secondTrimester (object)
 *   5. third-trimester: thirdTrimester (object)
 *   6. results: results (object)
 *   7. recommendations: recommendations (array)
 *   8. notes: notes
 */
...
const DRAFT_KEY = 'pregnancy_coursePendingEdits';
const readDrafts = () => {
...
/* simple (non-sentence) strings */
const SIMPLE_STRING_FIELDS = ['type', 'provider', 'facility', 'status'];
const STRING_FIELDS = [...SENTENCE_FIELDS, ...SIMPLE_STRING_FIELDS];
...
/* ═══════ COMPONENT ═══════ */
const PregnancyCourseDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.pregnancy_course) return Array.isArray(r.pregnancy_course) ? r.pregnancy_course : [r.pregnancy_course];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.pregnancy_course) return Array.isArray(dd.pregnancy_course) ? dd.pregnancy_course : [dd.pregnancy_course]; return [dd]; }
      return [r];
    });
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Draft store shape: { [recordId]: { [fieldPart]: value } } where fieldPart is "field" or "field.arrayIndex". */
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        // sentence narratives mark via editedSentences (-s0); everything else via editedFields
        const lastDot = fieldPart.lastIndexOf('.');
        const baseField = lastDot === -1 ? fieldPart : fieldPart.slice(0, lastDot);
        if (lastDot === -1 && SENTENCE_FIELDS.includes(baseField)) {
  ...
  /* Resolve nested dot-path field values from record */
  ...
  /* searchable string for any typed field value */
  const fieldSearchString = useCallback((f, val) => {
    if (OBJECT_FIELDS.includes(f)) return flattenSearchable(val);
    if (ARRAY_FIELDS.includes(f)) return (Array.isArray(val) ? val : []).map(it => (it && typeof it === 'object') ? flattenSearchable(it) : String(it)).join(' ');
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (fieldSearchString(f, val).toLowerCase().includes(phrase)) return true;
      }
    }
    return false;
  }, [searchTerm, getFieldValue, fieldSearchString]);
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) {
      return fieldSearchString(fn, val).toLowerCase().includes(phrase);
    }
    return false;
  }, [searchTerm, getFieldValue, fieldSearchString]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pregnancy Course ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (val !== null && val !== undefined && fieldSearchString(f, val).toLowerCase().includes(phrase)) return true;
        }
      }
      return false;
    });
  }, [records, searchTerm, getFieldValue, fieldSearchString]);

  /* ═══════ PDF DATA ═══════ */
  const pdfData = useMemo(() => {
    return filteredRecords.map((record, idx) => {
      const merged = { ...record };
      OBJECT_FIELDS.forEach(of => { if (record[of] && typeof record[of] === 'object' && !Array.isArray(record[of])) merged[of] = { ...record[of] }; });
      Object.keys(localEdits).forEach(key => {
        ...
        if (m && parseInt(m[2]) === idx) {
          ...
          if (fn.includes('.')) {
            const parts = fn.split('.');
            if (parts.length === 2 && merged[parts[0]] && typeof merged[parts[0]] === 'object') {
  ...
  /* save an OBJECT leaf at dotted path under rootField */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    // Stage a DRAFT (no DB write). The whole edited object clone is held under `${rootField}-${idx}`;
    // localStorage keeps it across refresh; Approve commits the dotted leaf path to the DB.
  ...
  const sectionHasEdits = useCallback((idx, sid) => {
    ...
    return fields.some(f =>
      Object.keys(editedFields).some(k => k.startsWith(`${f}-${idx}`) || k.startsWith(`${f}-${idx}-`)) ||
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Pending edit keys for THIS record that belong to THIS section's fields.
    ...
    try {
      for (const editKey of toCommit) {
        ...
        // For array fields the per-item ".N" key carries the commit (with arrayIndex); skip the
        // redundant bare whole-array key so we don't double-write the same field.
        if (!isArrayIndex && Array.isArray(localEdits[editKey])) continue;
        ...
        const resp = await secureApiClient.put(`/api/edit/pregnancy_course/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/pregnancy_course/${id}/approve`, { sectionId: sid, approved: true });
      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this record's drafts for the committed field parts from localStorage
      const store = readDrafts();
      if (store[id]) {
  ...
  const objectCopyLines = useCallback((label, value, indent) => {
    ...
    if (Array.isArray(value)) {
      ...
      value.filter(x => !isEmptyDeep(x)).forEach((it, i) => {
        if (isScalar(it)) out.push(`${pad}  ${i + 1}. ${fmtScalar(it)}`);
        else out.push(...objectCopyLines('', it, indent + 1));
      });
      return out;
    }
    if (label) out.push(`${pad}${label}:`);
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        text += `${label}\n`;
        items.filter(it => !isEmptyDeep(it)).forEach((item, i) => {
          const s = (item && typeof item === 'object') ? (item.recommendation || flattenSearchable(item)) : String(item);
          text += `${i + 1}. ${s}\n`;
        });
        text += '\n';
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PREGNANCY COURSE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Pregnancy Course ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT NODE (recursive; humanizeKey + nested-mini-card; editable leaves; arrays per-item) ═══════ */
      ...
      return (
        <React.Fragment key={path.join('-') || rootField}>
          ...
          <div className="nested-group">
            {items.map((it, i) => (
              isScalar(it) ? renderObjectLeaf(record, rootField, [...path, String(i)], idx, sid, it)
                : <div className="nested-mini-card" key={i}>{renderObjectNode(record, rootField, idx, sid, '', it, [...path, String(i)], depth + 1)}</div>
            ))}
          </div>
        </React.Fragment>
      );
    ...
    if (Array.isArray(val)) {
      ...
      return (
        <div key={fn} className="rec-mini-card">
          ...
          {items.map((it, i) => (
            isScalar(it) ? renderObjectLeaf(record, fn, [String(i)], idx, sid, it)
              : <div className="nested-mini-card" key={i}>{renderObjectNode(record, fn, idx, sid, '', it, [String(i)], 1)}</div>
          ))}
        </div>
      );
    }
  ...
  /* ═══════ RENDER: ARRAY FIELD (recommendations — per-item editing) ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          const itemStr = isObj ? (item.recommendation != null ? String(item.recommendation) : flattenSearchable(item)) : String(item);
          const itemDate = isObj && item.date ? formatDate(item.date) : '';
          ...
          return (
            <div key={itemIdx}>
              {itemDate && <div className="nested-subtitle sub-label">{highlightText(itemDate)}</div>}
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const cur = Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : []; const newItem = isObj ? { ...cur[itemIdx], recommendation: editValue } : editValue; const currentArr = [...cur]; currentArr[itemIdx] = newItem; const fieldPart = `${fn}.${itemIdx}`; const stagedKey = `${fieldPart}-${idx}`; setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: currentArr, [stagedKey]: newItem })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true, [stagedKey]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const store = readDrafts(); if (!store[id]) store[id] = {}; store[id][fieldPart] = newItem; writeDrafts(store); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  /* ═══════ RENDER: STRING FIELD with splitBySentence (per-sentence for narratives) ═══════ */
  const renderStringField = (record, fn, idx, sid, title) => {
    ...
    const allowSentences = SENTENCE_FIELDS.includes(fn);

    /* Multi-sentence: render with splitBySentence (narrative fields only) */
    if (allowSentences && sentences.length > 1) {
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const id3 = safeId(record); if (!id3) return; const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullText })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const store = readDrafts(); if (!store[id3]) store[id3] = {}; store[id3][fn] = fullText; writeDrafts(store); setEditingField(null); setEditValue(''); setSaveError(null); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
    ...
    /* Single-value / simple string: simple editable */
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
    const isModified = editedFields[editKey];
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pregnancy-course-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pregnancy Course</h2></div>
        <div className="empty-state">No pregnancy course records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pregnancy-course-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pregnancy course..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
...
export default PregnancyCourseDocument;
