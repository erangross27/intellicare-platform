...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'omissions-refusals-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'refusalType',
  'refusalReason',
  'refusalDocumentationMethod',
  'refusedMedications',
  'refusedProcedures',
  'refusedDiagnosticTests',
  'decisionMakingCapacity',
  'miniMentalStateExam',
  'informedConsentProvided',
  'risksBenefitsDiscussed',
  'alternativeTreatmentOffered',
  'followUpPlanAfterRefusal',
  'witnessPresent',
  'familyMemberInvolved',
  'advanceDirectiveReviewed',
  'medicalContraindications',
  'previousAdverseReactions',
  'ethicsConsultationRequested',
  'legalConsultationSought',
  'emergencyOverrideConsidered',
  'psychiatricEvaluationOrdered',
  'riskStratificationScore',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('omissions_refusals', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[omissions_refusals] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('omissions_refusals', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[omissions_refusals] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
