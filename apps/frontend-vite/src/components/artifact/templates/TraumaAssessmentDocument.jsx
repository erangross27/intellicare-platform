...
/* Numeric-typed fields — edited via <input type="number"> (parseFloat + isNaN guard) */
  ...
  const renderSimpleField = (record, fn, idx, sid) => {
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(DATETIME_FIELDS.includes(fn) ? toISODateTime(val) : DATE_FIELDS.includes(fn) ? toISODate(val) : isNumber ? String(val) : displayVal); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              {isNumber ? (
                <input type="number" step="any" className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) { saveSimpleField(); } }} />
              ) : isDateField ? (
                <input type={DATETIME_FIELDS.includes(fn) ? 'datetime-local' : 'date'} className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) { saveSimpleField(); } }} />
              ) : (
    ...
    <div className="trauma-assessment-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<TraumaAssessmentDocumentPDFTemplate document={pdfData} />} fileName={`trauma-assessment-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
