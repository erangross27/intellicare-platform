/**
 * Arterial Blood Gases Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/arterial_blood_gases/:id/edit    — Edit a single field
 * PUT /api/edit/arterial_blood_gases/:id/approve — Approve a section
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'arterial-blood-gases-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('arterial_blood_gases', { _id: objectId }, {
      $set: {
        [updatePath]: value,
        updatedAt: new Date(),
    ...
    console.error('[ArterialBloodGases] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    console.error('[ArterialBloodGases] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
