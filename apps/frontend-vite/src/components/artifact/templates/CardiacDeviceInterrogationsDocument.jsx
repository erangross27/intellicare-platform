/**
 * CardiacDeviceInterrogationsDocument.jsx
 * June 2026 — Collection: cardiac_device_interrogations — FULL TEMPLATE STANDARD (100% / 22 fields).
 *
 * Donor cloned: PointOfCareUltrasoundHeartRateDocument
 *   - OBJECT fields → recursive renderObjectField / renderObjectNode / renderObjectLeaf
 *     (humanizeKey + nested-mini-card; typed leaf editing: number→number input, boolean→Yes/No, else text)
 *   - STRING narrative fields → per-sentence renderStringField (splitBySentence / reconstructFullText /
 *     saveSentence / per-sentence keys / added+edited badges)
 *   - ARRAY fields → renderArrayField (array-of-objects OR array-of-scalars, numbered, editable scalars)
 *   - 4-level search, per-section approve (Pending → Approved), Copy row/Section/All, pdfData memo.
 *
 * FIELDS (22 → 100% schema coverage):
 *   STRING short: deviceType, manufacturer, model, serialNumber, implantDate, interrogationDate
 *     (implantDate/interrogationDate are STRING in schema — plain text, NOT date pickers)
 *   STRING narrative (per-sentence): interrogationReason, clinicalAssessment, followUpPlan, recommendations
 *   OBJECT (recursive): batteryStatus, pacingParameters, pacingPercentages, sensingThresholds,
 *     icdTherapy, atrialFibrillationBurden, remoteMonitoring
 *   ARRAY: leads, arrhythmiaEpisodes, alerts, programmingChanges, deviceAlerts
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import CardiacDeviceInterrogationsDocumentPDFTemplate from '../pdf-templates/CardiacDeviceInterrogationsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './CardiacDeviceInterrogationsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.<dotpath>") */
const DRAFT_KEY = 'cardiac_device_interrogationsPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  interrogationDate: 'Interrogation Date',
  interrogationReason: 'Interrogation Reason',
  batteryStatus: 'Battery Status',
  leads: 'Leads',
  pacingParameters: 'Pacing Parameters',
  pacingPercentages: 'Pacing Percentages',
  arrhythmiaEpisodes: 'Arrhythmia Episodes',
  icdTherapy: 'ICD Therapy',
  atrialFibrillationBurden: 'Atrial Fibrillation Burden',
  alerts: 'Alerts',
  remoteMonitoring: 'Remote Monitoring',
  programmingChanges: 'Programming Changes',
  clinicalAssessment: 'Clinical Assessment',
  followUpPlan: 'Follow-Up Plan',
  sensingThresholds: 'Sensing Thresholds',
...
/* short scalar strings (no sentence split) */
const SHORT_STRING_FIELDS = ['deviceType', 'manufacturer', 'model', 'serialNumber', 'implantDate', 'interrogationDate'];
/* narrative strings (per-sentence) */
...
/* array fields (array-of-objects OR array-of-scalars) */
...
/* ═══════ COMPONENT ═══════ */
const CardiacDeviceInterrogationsDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.cardiac_device_interrogations) return Array.isArray(r.cardiac_device_interrogations) ? r.cardiac_device_interrogations : [r.cardiac_device_interrogations];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.cardiac_device_interrogations) return Array.isArray(dd.cardiac_device_interrogations) ? dd.cardiac_device_interrogations : [dd.cardiac_device_interrogations]; return [dd]; }
      return [r];
    });
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Draft store shape: { [recordId]: { [fieldPart]: value } }
     fieldPart = the editKey with the trailing "-<idx>" removed (e.g. "clinicalAssessment",
     "batteryStatus.voltage", "leads"). We re-derive the render index from the record's position. */
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (dotIdx === -1) {
          // Plain field (short string, sentence string, or whole array) — value applies directly.
          const editKey = `${fieldPart}-${idx}`;
          nLocal[editKey] = value;
          nPending[editKey] = true;
          nFields[editKey] = 'edited';
        } else {
          // Object-leaf dotted path (e.g. "batteryStatus.voltage") — merge into the root object clone
          // stored under "<root>-<idx>", and use the leaf tracking key "<root>-<idx>-<dotpath>".
          ...
          nFields[`${rootField}-${idx}-${dotPath}`] = 'edited';
        }
      });
    });
  ...
  /* array → searchable text (handles array-of-objects or array-of-scalars) */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `cardiac device interrogation ${idx + 1}`.toLowerCase();
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. batteryStatus.voltage) */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    const dottedField = `${rootField}.${path.join('.')}`;
    setSaveError(null);
    // Stage the merged object clone locally (for rendering) under the root field's editKey.
    const localKey = `${rootField}-${idx}`;
    setLocalEdits(prev => {
    ...
    setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
    // Stage the per-leaf dotted field draft (Approve PUTs each dotted leaf individually).
    const store = readDrafts();
    if (!store[id]) store[id] = {};
  ...
  // Sentence save = stage a DRAFT (full reconstructed text) locally + localStorage. No DB write until Approve.
  ...
  /* save an editable ARRAY scalar item */
  // Array-scalar save = stage the whole new array as a DRAFT locally + localStorage. No DB write until Approve.
  const saveArrayItem = useCallback((record, fn, idx, sid, itemIdx, itemKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    const currentArr = Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [];
    const newArr = currentArr.map((it, i) => i === itemIdx ? newVal : it);
    setSaveError(null);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // Pull this record's drafts whose fieldPart belongs to one of this section's fields.
      const store = readDrafts();
      ...
      // Persist each staged field now. arrayIndex ONLY when the segment after the LAST dot is purely numeric.
      for (const fieldPart of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/cardiac_device_interrogations/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/cardiac_device_interrogations/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF. localEdits keys use "<field>-<idx>".
      setPendingEdits(prev => {
        const n = { ...prev };
        toCommit.forEach(fieldPart => {
          const baseField = fieldPart.includes('.') ? fieldPart.slice(0, fieldPart.indexOf('.')) : fieldPart;
          delete n[`${baseField}-${idx}`];
        });
        return n;
      });
      // Drop this record's section drafts from localStorage (now committed)
      if (store[id]) {
        toCommit.forEach(fieldPart => { delete store[id][fieldPart]; });
    ...
    } catch (err) { console.error('[CardiacDeviceInterrogations] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (ARRAY_FIELDS.includes(f)) {
        ...
        arr.forEach((it, i) => {
          if (isScalar(it)) { text += `${i + 1}. ${fmtScalar(it)}\n`; }
          else { text += `${i + 1}.\n`; objectCopyLines('', it, 1).forEach(l => { text += `${l}\n`; }); }
        });
        text += '\n';
        ...
        if (!sameAsTitle) text += `${label}\n`;
        // Mirror the JSX: "Label: value" → label header + value (comma-split ≥3 → numbered items); plain sentence → line.
        sentences.forEach((sent) => {
          const parsed = parseLabel(sent);
          if (parsed) {
            text += `${parsed.label}\n`;
            const parts = splitByComma(parsed.content);
            if (parts.length >= 3) parts.forEach((p, i) => { text += `${i + 1}. ${p}\n`; });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== CARDIAC DEVICE INTERROGATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Cardiac Device Interrogation ${idx + 1}\n${'='.repeat(40)}\n\n`;
      SECTION_ORDER.forEach(sid => {
        const fields = SECTION_FIELDS[sid] || [];
        if (fields.some(f => hasVal(getFieldValue(r, f, idx)))) text += buildSectionCopyText(r, idx, sid);
      });
      text += '\n';
    });
    const ok = await copyToClipboard(text);
    if (ok) { setShowCopied(true); setTimeout(() => setShowCopied(false), 2000); }
  }, [pdfData, copyToClipboard, buildSectionCopyText, getFieldValue, hasVal]);

  /* ═══════ RENDER: SHORT STRING (no sentence split) ═══════ */
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(strVal); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus rows={1} onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter' && e.ctrlKey) { e.preventDefault(); handleSaveField(record, fn, idx, sid); } }} />
  ...
  // Parse "Label: value" (short capitalized label + colon+space) → {label, content}; else null.
  const parseLabel = (text) => {
    const m = String(text || '').match(/^\s*([A-Z][A-Za-z0-9 /&()'’.-]{0,39}?):\s+(.+)$/s);
    if (!m) return null;
    const label = m[1].trim(); const content = m[2].trim();
    return (label && content) ? { label, content } : null;
  };
  // Parenthesis-aware comma split — keeps "(wound check, interrogate device)" intact.
  const splitByComma = (text) => {
    const s = String(text || ''); const out = []; let cur = '', depth = 0;
    for (let i = 0; i < s.length; i++) { const ch = s[i]; if (ch === '(') depth++; else if (ch === ')') depth = Math.max(0, depth - 1); if (ch === ',' && depth === 0) { const t = cur.trim(); if (t) out.push(t); cur = ''; } else cur += ch; }
  ...
  const renderStringField = (record, fn, idx, sid) => {
      ...
      return (
        <div key={fn} className="rec-mini-card">
          ...
          {sentences.map((sentence, sIdx) => {
            ...
            const displayText = parsed ? parsed.content : sentence;
            return (
              <div key={sIdx}>
                {parsed && <div className="nested-subtitle sub-label">{highlightText(parsed.label)}</div>}
    ...
    const isEditing = editingField === editKey;
    const isModified = editedFields[editKey];
    const parsedSingle = parseLabel(strVal);
    const singleContent = parsedSingle ? parsedSingle.content : strVal;
    const commaParts = splitByComma(singleContent);
    const useParts = !!parsedSingle && commaParts.length >= 3;   // labeled value with ≥3 comma items → one row per item
    return (
      <div key={fn} className="rec-mini-card">
        {showSubLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
        {parsedSingle && <div className="nested-subtitle sub-label">{highlightText(parsedSingle.label)}</div>}
        {isEditing ? (
          ...
          commaParts.map((p, pi) => (
            <div key={pi} className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { setEditingField(editKey); setEditValue(strVal); setSaveError(null); }}>
              <div className="row-content"><span className="content-value">{highlightText(p)}</span><span className="edit-indicator">&#9998;</span></div>
              <button className={`copy-btn ${copiedItems[`${editKey}-c${pi}`] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(p, `${editKey}-c${pi}`); }}>{copiedItems[`${editKey}-c${pi}`] ? 'Copied!' : 'Copy'}</button>
            </div>
          ))
        ) : (
          <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { setEditingField(editKey); setEditValue(strVal); setSaveError(null); }}>
            <div className="row-content"><span className="content-value">{highlightText(singleContent)}</span><span className="edit-indicator">&#9998;</span></div>
            <button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${label}\n${strVal}`, editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button>
          </div>
        )}
        {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
  ...
  /* ═══════ RENDER: ARRAY FIELD (array-of-objects OR array-of-scalars, numbered) ═══════ */
  const renderArrayField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    const items = Array.isArray(val) ? val.map((it, i) => ({ it, i })).filter(({ it }) => !isEmptyDeep(it)) : [];
    ...
    return (
      <div key={fn} className="rec-mini-card">
        {showSubLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
        {items.map(({ it, i }, displayIdx) => {
          const itemMatches = phraseMatch || (searchTerm.trim() && (isScalar(it) ? fmtScalar(it) : flattenSearchable(it)).toLowerCase().includes(phrase));
          if (!itemMatches && searchTerm.trim()) return null;
          if (isScalar(it)) {
            /* scalar item — editable */
            const itemKey = `${fn}-${idx}-a${i}`;
            const isEditing = editingField === itemKey;
            const badge = editedFields[itemKey];
            const itemStr = fmtScalar(it);
            return (
              <div key={i}>
                <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(itemStr); setSaveError(null); } }}>
                  {isEditing ? (
                    <div className="edit-field-container">
                      <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter' && e.ctrlKey) { e.preventDefault(); saveArrayItem(record, fn, idx, sid, i, itemKey, editValue.trim()); } }} />
                      {saveError && <div className="save-error">{saveError}</div>}
                      <div className="edit-actions">
                        <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveArrayItem(record, fn, idx, sid, i, itemKey, editValue.trim()); }}>{saving ? 'Saving...' : 'Save'}</button>
                        <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                      </div>
                    </div>
                  ) : (
                    <>
                      <div className="row-content"><span className="content-value">{displayIdx + 1}. {highlightText(itemStr)}</span><span className="edit-indicator">&#9998;</span></div>
          ...
          /* object item — recursive read-only nested rendering of its leaves (editable via saveLeaf path through array not supported; render nested object leaves under item index path) */
          const itemKey = `${fn}-${idx}-a${i}`;
          const objEntries = Object.entries(it).filter(([, v]) => !isEmptyDeep(v));
          return (
            <div key={i} className="nested-mini-card">
              <div className="nested-subtitle sub-label">{`${displayIdx + 1}.`}</div>
              <div className="nested-group">
                {objEntries.map(([k, v]) => (
                  isScalar(v)
                    ? renderObjectLeaf(record, fn, [String(i), k], idx, sid, v)
                    : <div className="nested-mini-card" key={k}>{renderObjectNode(record, fn, idx, sid, humanizeKey(k), v, [String(i), k], 1)}</div>
                ))}
              </div>
              <button className={`copy-btn ${copiedItems[`${itemKey}-copy`] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(objectCopyLines('', it, 0).join('\n'), `${itemKey}-copy`); }}>{copiedItems[`${itemKey}-copy`] ? 'Copied!' : 'Copy'}</button>
            </div>
          );
        })}
      </div>
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (SENTENCE_FIELDS.includes(f)) return renderStringField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="cardiac-device-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Cardiac Device Interrogations</h2></div>
        <div className="empty-state">No cardiac device interrogation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="cardiac-device-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Cardiac Device Interrogations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<CardiacDeviceInterrogationsDocumentPDFTemplate document={pdfData} />} fileName={`cardiac-device-interrogations-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search cardiac device interrogation records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          <div key={idx} className="record-card">
            <div className="record-header">
              <div className="record-meta-row">
                {hasVal(record.deviceType) && <span className="record-date">{fmtVal(record.deviceType)}</span>}
                {hasVal(record.manufacturer) && <span className="record-date">{fmtVal(record.manufacturer)}</span>}
                {hasVal(record.interrogationDate) && <span className="record-date">{fmtVal(record.interrogationDate)}</span>}
              </div>
              <h3 className="record-name">{highlightText(`Cardiac Device Interrogation ${idx + 1}`)}</h3>
            </div>
...
export default CardiacDeviceInterrogationsDocument;
