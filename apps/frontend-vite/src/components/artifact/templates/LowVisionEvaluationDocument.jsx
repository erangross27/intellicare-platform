/**
 * LowVisionEvaluationDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: low_vision_evaluation
 *
 * MongoDB fields (verified):
 *   visualAcuityBestCorrectedOD (string), visualAcuityBestCorrectedOS (string),
 *   contrastSensitivityOD (string), contrastSensitivityOS (string),
 *   centralVisualFieldDefect (string), preferredRetinalLocus (string),
 *   visualFieldExtentDegrees (string),
 *   readingSpeedWordsPerMinute (number), criticalPrintSize (string),
 *   magnificationRequirement (string), nearAdditionPower (number),
 *   workingDistanceCm (number),
 *   colorVisionDeficiency (string), glareSensitivity (string),
 *   vfq25CompositeScore (number), binoclularVisionStatus (string),
 *   lowVisionEtiology (string),
 *   opticalDevicesPrescribed (array), electronicMagnificationDevices (array),
 *   eccentricViewingTrainingStatus (string), orientationMobilityReferral (boolean),
 *   lightingRecommendationsLux (string),
 *   drivingVisionStandards (string), occupationalVisionRequirements (string),
 *   rehabilitationGoals (array)
 *
 * 10 Sections:
 *   1. visual-acuity: visualAcuityBestCorrectedOD, visualAcuityBestCorrectedOS, contrastSensitivityOD, contrastSensitivityOS
 *   2. visual-field: centralVisualFieldDefect, preferredRetinalLocus, visualFieldExtentDegrees
 *   3. reading-function: readingSpeedWordsPerMinute, criticalPrintSize, magnificationRequirement, nearAdditionPower, workingDistanceCm
 *   4. vision-quality: colorVisionDeficiency, glareSensitivity, vfq25CompositeScore, binoclularVisionStatus
 *   5. etiology: lowVisionEtiology
 *   6. optical-devices: opticalDevicesPrescribed
 *   7. electronic-devices: electronicMagnificationDevices
 *   8. rehabilitation: eccentricViewingTrainingStatus, orientationMobilityReferral, lightingRecommendationsLux
 *   9. functional-status: drivingVisionStandards, occupationalVisionRequirements
 *  10. goals: rehabilitationGoals
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import LowVisionEvaluationDocumentPDFTemplate from '../pdf-templates/LowVisionEvaluationDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './LowVisionEvaluationDocument.css';
...
const STRING_FIELDS = [
  ...
  'criticalPrintSize', 'magnificationRequirement',
  'colorVisionDeficiency', 'glareSensitivity',
  'binoclularVisionStatus',
  'lowVisionEtiology',
  'eccentricViewingTrainingStatus', 'lightingRecommendationsLux',
...
const DRAFT_KEY = 'low_vision_evaluationPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const LowVisionEvaluationDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.low_vision_evaluation) return Array.isArray(r.low_vision_evaluation) ? r.low_vision_evaluation : [r.low_vision_evaluation];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.low_vision_evaluation) return Array.isArray(dd.low_vision_evaluation) ? dd.low_vision_evaluation : [dd.low_vision_evaluation]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // fieldPart = "field" (string/number/boolean/sentence) or "field.arrayIndex" (array element)
        ...
        if (isArrayPart) {
          ...
          // local edit for an array field stores the WHOLE array; merge into prior draft array if present
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Low Vision Evaluation ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Persist each staged draft fieldPart for this record that belongs to a field in this section.
      const store = readDrafts();
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/low_vision_evaluation/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedLocalKeys.push(`${baseField}-${idx}`);
        delete store[id][fieldPart];
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/low_vision_evaluation/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== LOW VISION EVALUATION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Low Vision Evaluation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* Single-value string: simple editable — uses saveSentence(record, fn, idx, sid, 0) */
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
    const isModified = editedFields[editKey];
  ...
  if (!records || records.length === 0) {
    return (
      <div className="low-vision-evaluation-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Low Vision Evaluation</h2></div>
        <div className="empty-state">No low vision evaluation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="low-vision-evaluation-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Low Vision Evaluation</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<LowVisionEvaluationDocumentPDFTemplate document={pdfData} />} fileName="Low_Vision_Evaluation.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search low vision evaluation..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Low Vision Evaluation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'visual-acuity')}
            {renderSection(record, idx, 'visual-field')}
            {renderSection(record, idx, 'reading-function')}
            {renderSection(record, idx, 'vision-quality')}
            {renderSection(record, idx, 'etiology')}
            {renderSection(record, idx, 'optical-devices')}
            {renderSection(record, idx, 'electronic-devices')}
...
export default LowVisionEvaluationDocument;
