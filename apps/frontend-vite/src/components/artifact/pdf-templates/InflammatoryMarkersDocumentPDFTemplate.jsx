...
/**
 * InflammatoryMarkersDocumentPDFTemplate - box-free canonical (LETTER)
 * Config-driven from the JSX SECTION_ORDER/SECTION_TITLES/FIELD_LABELS/SECTION_FIELDS.
 * Renders EVERY populated field the JSX renders for JSX/PDF field parity.
 * No boxes: underline rules only (documentTitle 2 / recordTitle+sectionTitle 1 / fieldLabel 0.5).
 * Nested objects (complement/immunoglobulins/results) + arrays render via a generic recursive
 * renderer; inline "Key: value" is allowed in the PDF only. Record date uses record.date
 * (NEVER createdAt/updatedAt). esr/crp/ferritin are unit STRINGS, rendered as-is.
 */
...
const SECTION_ORDER = ['session-info', 'marker-results', 'complement-immunoglobulins', 'clinical-notes', 'additional-notes'];
...
const InflammatoryMarkersDocumentPDFTemplate = ({ document: docProp, data = docProp }) => {
  let records = [];
  if (Array.isArray(data)) records = data;
  else if (data && typeof data === 'object') {
    if (data.inflammatory_markers) records = Array.isArray(data.inflammatory_markers) ? data.inflammatory_markers : [data.inflammatory_markers];
    else records = [data];
  }
    ...
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <Text style={styles.documentTitle}>Inflammatory Markers</Text>
          <Text style={styles.noData}>No inflammatory markers records available.</Text>
        </Page>
      </Document>
    );
  }
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, rIdx) => (
          <View key={rIdx}>
            <Text style={styles.recordTitle} break={rIdx > 0}>{safeString(`Inflammatory Markers ${rIdx + 1}`)}</Text>
...
export default InflammatoryMarkersDocumentPDFTemplate;
