/**
 * MonitoringPlansDocument.jsx
 * July 2026 — Canonical one-pass polish (inline editing, blue glow theme)
 * Collection: monitoring_plans
 *
 * 2 Sections:
 *   1. plan-overview: date (BlueDatePicker), provider, specialty, frequency
 *   2. monitoring-plan: laboratory (narrative, ';'-clause split), imaging, clinicalAssessments,
 *                       parameters (array of strings)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MonitoringPlansDocumentPDFTemplate from '../pdf-templates/MonitoringPlansDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './MonitoringPlansDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'plan-overview': 'Plan Overview',
  'monitoring-plan': 'Monitoring Details',
};
...
const SECTION_FIELDS = {
  'plan-overview': ['date', 'provider', 'specialty', 'frequency'],
  'monitoring-plan': ['laboratory', 'imaging', 'clinicalAssessments', 'parameters'],
};

const DATE_FIELDS = ['date'];
const NUMBER_FIELDS = [];
const STRING_FIELDS = ['provider', 'specialty', 'frequency', 'laboratory', 'imaging', 'clinicalAssessments'];
const ARRAY_STRING_FIELDS = ['parameters'];
const ALL_ARRAY_FIELDS = [...ARRAY_STRING_FIELDS];

/* Sentinel-zero: 0-valued NUMBER_FIELDS are the extractor's unset default → HIDDEN everywhere. */
...
const DRAFT_KEY = 'monitoring_plansPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const MonitoringPlansDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.monitoring_plans) return Array.isArray(r.monitoring_plans) ? r.monitoring_plans : [r.monitoring_plans];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.monitoring_plans) return Array.isArray(dd.monitoring_plans) ? dd.monitoring_plans : [dd.monitoring_plans]; return [dd]; }
      return [r];
    });
  ...
  /* hasFieldVal: hasVal + sentinel-zero hide (a 0-valued NUMBER_FIELD is the extractor's unset default) */
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) {
      ...
      if (DATE_FIELDS.includes(fn)) return formatDate(val).toLowerCase().includes(phrase) || fmtVal(val).toLowerCase().includes(phrase);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Monitoring Plan ${idx + 1}`.toLowerCase();
      ...
      for (const sFields of Object.values(SECTION_FIELDS)) {
        for (const f of sFields) {
          ...
          if (val !== null && val !== undefined) {
            if (Array.isArray(val)) { if (val.some(item => String(item).toLowerCase().includes(phrase))) return true; }
            else if (DATE_FIELDS.includes(f)) { if (formatDate(val).toLowerCase().includes(phrase) || fmtVal(val).toLowerCase().includes(phrase)) return true; }
            else if (fmtVal(val).toLowerCase().includes(phrase)) return true;
          }
        }
      }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const field = m[1];
        await secureApiClient.put(`/api/edit/monitoring_plans/${id}/edit`, { field, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/monitoring_plans/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    if (!body.trim()) return '';
    return `${title}\n${'='.repeat(40)}\n\n${body}`;
  }, [getFieldValue, hasFieldVal, fmtVal, splitBySentence, formatSentenceFieldLines, formatDate]);

  const copyAllText = useCallback(async () => {
    let text = '=== MONITORING PLANS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Monitoring Plan ${idx + 1}\n${'='.repeat(40)}\n\n`;
          ...
          {isEditing ? (
            <div className="edit-field-container">
              ...
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date'); return; } handleSaveField(record, fn, idx, sid, null, new Date(editValue + 'T00:00:00.000Z').toISOString()); }}>{saving ? 'Saving...' : 'Save'}</button>
              ...
              <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(String(item)); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveArrayItem(record, fn, idx, ai, false, sid); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="monitoring-plans-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Monitoring Plans</h2></div>
        <div className="empty-state">No monitoring plans records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="monitoring-plans-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Monitoring Plans</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MonitoringPlansDocumentPDFTemplate document={pdfData} />} fileName="Monitoring_Plans.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search monitoring plans..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Monitoring Plan ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'plan-overview')}
...
export default MonitoringPlansDocument;
