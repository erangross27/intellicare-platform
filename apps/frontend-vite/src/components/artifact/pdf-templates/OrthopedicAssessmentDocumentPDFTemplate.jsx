...
/**
 * OrthopedicAssessmentDocumentPDFTemplate - box-free canonical (LETTER)
 * Config-driven from the JSX SECTION_ORDER/SECTION_TITLES/FIELD_LABELS/SECTION_FIELDS.
 * Renders EVERY populated field the JSX renders for JSX/PDF field parity.
 * No boxes: underline rules only (documentTitle 2 / recordTitle+sectionTitle 1 / fieldLabel 0.5).
 * Nested objects (neurovascularStatus/compartmentSyndrome/results) + arrays render via a generic
 * recursive renderer; inline "Key: value" is allowed in the PDF only. Record date uses record.date
 * (NEVER createdAt/updatedAt). safeString folds special glyphs to ASCII (degree sign -> " deg")
 * using \uXXXX escapes ONLY so Helvetica never hits a missing glyph.
 */
...
const SECTION_ORDER = ['assessment-info', 'neurovascular-status', 'skin-integrity', 'fractures', 'dislocations', 'compartment-syndrome', 'findings', 'assessment', 'plan', 'recommendations', 'results', 'notes'];
...
/* HELPERS (mirror the JSX) - safeString uses \uXXXX escapes ONLY (never literal smart-quotes / degree
   sign / invisible chars). Folds special glyphs to ASCII so Helvetica never hits a missing glyph. */
...
const OrthopedicAssessmentDocumentPDFTemplate = ({ document: docProp, data = docProp }) => {
  let records = [];
  if (Array.isArray(data)) records = data;
  else if (data && typeof data === 'object') {
    if (data.orthopedic_assessment) records = Array.isArray(data.orthopedic_assessment) ? data.orthopedic_assessment : [data.orthopedic_assessment];
    else records = [data];
  }
    ...
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <Text style={styles.documentTitle}>Orthopedic Assessment</Text>
          <Text style={styles.noData}>No orthopedic assessment records available.</Text>
        </Page>
      </Document>
    );
  }
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, rIdx) => (
          <View key={rIdx}>
            <Text style={styles.recordTitle} break={rIdx > 0}>{safeString(`Orthopedic Assessment ${rIdx + 1}`)}</Text>
...
export default OrthopedicAssessmentDocumentPDFTemplate;
