/**
 * Oral Pathology Biopsy Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/oral_pathology_biopsy/:id/edit    — Edit a single field
 * PUT /api/edit/oral_pathology_biopsy/:id/approve — Approve pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'oral-pathology-biopsy-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'specimenSite',
  'biopsyTechnique',
  'specimenDimensionsCm',
  'clinicalLesionDescription',
  'histopathologicDiagnosis',
  'epithelialDysplasiaGrade',
  'binaryDysplasiaClassification',
  ...
  'marginStatus',
  'closestMarginDistanceMm',
  'ajccTnmStage',
  'worstPatternOfInvasion',
  'tumorBuddingScore',
  'hpvAssociationStatus',
  'immunohistochemistryPanel',
  'boneInvolvement',
  'extranodalExtension',
  'salivaryGlandPathology',
  'odontogenicTumorClassification',
  'directImmunofluorescenceFindings',
  'lichenoidInflammationPattern',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('oral_pathology_biopsy', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[oral_pathology_biopsy] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('oral_pathology_biopsy', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[oral_pathology_biopsy] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
