/**
 * MedicationAdministrationRecordsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: medication_administration_records
 *
 * 4 Sections:
 *   1. session-info: medicationName (string), dosageAmount (number), dosageUnit (string),
 *                    routeOfAdministration (string), administrationTime (date), scheduledTime (date)
 *   2. personnel: administeringNurse (string), witnessingNurse (string)
 *   3. patient-response: patientResponse (string), administrationSite (string),
 *                        patientAllergies (array strings), vitalSignsPreAdmin (string),
 *                        vitalSignsPostAdmin (string), painScorePre (number), painScorePost (number),
 *                        adverseReaction (string), glucoseLevel (number), inrValue (number)
 *   4. medication-details: lotNumber (string), expirationDate (date), refusalReason (string),
 *                          priorMedCheck (boolean), infusionRate (number),
 *                          concentrationStrength (string), barcodeVerification (boolean)
 *
 * Widgets: numbers → −/+ num-stepper (type=text inputMode=decimal). booleans → BlueSelect Yes/No.
 * dates → BlueDatePicker. hide-zero: a numeric 0 is treated as empty (infusionRate/glucoseLevel/inrValue).
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MedicationAdministrationRecordsDocumentPDFTemplate from '../pdf-templates/MedicationAdministrationRecordsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './MedicationAdministrationRecordsDocument.css';

/* ======= CONSTANTS ======= */
const SECTION_TITLES = {
  'session-info': 'Session Information',
  'personnel': 'Personnel',
  'patient-response': 'Patient Response & Vitals',
...
const FIELD_LABELS = {
  ...
  administeringNurse: 'Administering Nurse',
  witnessingNurse: 'Witnessing Nurse',
  patientResponse: 'Patient Response',
  administrationSite: 'Administration Site',
  patientAllergies: 'Patient Allergies',
  vitalSignsPreAdmin: 'Vital Signs Pre-Administration',
  vitalSignsPostAdmin: 'Vital Signs Post-Administration',
  painScorePre: 'Pain Score (Pre)',
  painScorePost: 'Pain Score (Post)',
  ...
  priorMedCheck: 'Prior Med Check',
  infusionRate: 'Infusion Rate',
  concentrationStrength: 'Concentration/Strength',
  barcodeVerification: 'Barcode Verification',
};

const SECTION_FIELDS = {
  'session-info': ['medicationName', 'dosageAmount', 'dosageUnit', 'routeOfAdministration', 'administrationTime', 'scheduledTime'],
  'personnel': ['administeringNurse', 'witnessingNurse'],
  'patient-response': ['patientResponse', 'administrationSite', 'patientAllergies', 'vitalSignsPreAdmin', 'vitalSignsPostAdmin', 'painScorePre', 'painScorePost', 'adverseReaction', 'glucoseLevel', 'inrValue'],
  'medication-details': ['lotNumber', 'expirationDate', 'refusalReason', 'priorMedCheck', 'infusionRate', 'concentrationStrength', 'barcodeVerification'],
};

const NUMBER_FIELDS = ['dosageAmount', 'infusionRate', 'painScorePre', 'painScorePost', 'glucoseLevel', 'inrValue'];
const BOOLEAN_FIELDS = ['priorMedCheck', 'barcodeVerification'];
const DATE_FIELDS = ['administrationTime', 'scheduledTime', 'expirationDate'];
const ARRAY_FIELDS = ['patientAllergies'];
const STRING_FIELDS = ['medicationName', 'dosageUnit', 'routeOfAdministration', 'administeringNurse', 'witnessingNurse', 'patientResponse', 'administrationSite', 'vitalSignsPreAdmin', 'vitalSignsPostAdmin', 'adverseReaction', 'lotNumber', 'refusalReason', 'concentrationStrength'];
...
const DRAFT_KEY = 'medication_administration_recordsPendingEdits';
const readDrafts = () => {
...
/* ======= COMPONENT ======= */
const MedicationAdministrationRecordsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.medication_administration_records) return Array.isArray(r.medication_administration_records) ? r.medication_administration_records : [r.medication_administration_records];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.medication_administration_records) return Array.isArray(dd.medication_administration_records) ? dd.medication_administration_records : [dd.medication_administration_records]; return [dd]; }
      return [r];
    });
  ...
  /* ======= UTILS ======= */
  // hide-zero: a numeric 0 is treated as empty (infusionRate / glucoseLevel / inrValue never render as a blank "0")
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Medication Administration Record ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/medication_administration_records/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/medication_administration_records/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MEDICATION ADMINISTRATION RECORDS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Medication Administration Record ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ======= RENDER: NUMBER FIELD — num-stepper (NEVER type=number); hide-zero gated in hasVal ======= */
  ...
  /* ======= RENDER: BOOLEAN FIELD — BlueSelect Yes/No (renders "No" for false) ======= */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="medication-administration-records-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Medication Administration Records</h2></div>
        <div className="empty-state">No medication administration records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="medication-administration-records-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Medication Administration Records</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MedicationAdministrationRecordsDocumentPDFTemplate document={pdfData} />} fileName="Medication_Administration_Records.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search medication administration records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Medication Administration Record ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'personnel')}
            {renderSection(record, idx, 'patient-response')}
            {renderSection(record, idx, 'medication-details')}
          </div>
        ))}
      </div>
    </div>
...
export default MedicationAdministrationRecordsDocument;
