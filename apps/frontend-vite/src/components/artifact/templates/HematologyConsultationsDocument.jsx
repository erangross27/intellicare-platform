/**
 * HematologyConsultationsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: hematology_consultations
 *
 * 4 Sections:
 *   1. session-info: referringPhysician (string), chiefComplaint (string)
 *   2. cbc-morphology: hemoglobinLevel, hematocritValue, whiteBloodCellCount, plateletCount,
 *                      meanCorpuscularVolume, reticulocyteCount, peripheralBloodSmear, bonemarrowBiopsy,
 *                      cellularityPercentage, blastPercentage
 *   3. specialized-studies: flowCytometry, cytogeneticsKaryotype, molecularGenetics,
 *                           coagulationProfile, fibrinogenLevel, ddimerLevel,
 *                           ironStudies, vitaminB12Level, folateLevel,
 *                           haptoglobinLevel, lactateDehydrogenase, directAntiglobulinTest,
 *                           hemoglobinElectrophoresis
 *   4. clinical-history: splenomegaly, lymphadenopathy, bleedingHistory, thrombosisHistory,
 *                        familyHematologyHistory, currentAnticoagulation, ecogPerformanceStatus
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HematologyConsultationsDocumentPDFTemplate from '../pdf-templates/HematologyConsultationsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './HematologyConsultationsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'hematology_consultationsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['referringPhysician', 'chiefComplaint', 'peripheralBloodSmear', 'bonemarrowBiopsy', 'flowCytometry', 'cytogeneticsKaryotype', 'molecularGenetics', 'coagulationProfile', 'ironStudies', 'directAntiglobulinTest', 'hemoglobinElectrophoresis', 'splenomegaly', 'lymphadenopathy', 'bleedingHistory', 'thrombosisHistory', 'familyHematologyHistory', 'currentAnticoagulation'];
const DATE_FIELDS = [];
...
/* ═══════ COMPONENT ═══════ */
const HematologyConsultationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.hematology_consultations) return Array.isArray(r.hematology_consultations) ? r.hematology_consultations : [r.hematology_consultations];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.hematology_consultations) return Array.isArray(dd.hematology_consultations) ? dd.hematology_consultations : [dd.hematology_consultations]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Hematology Consultation ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/hematology_consultations/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/hematology_consultations/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HEMATOLOGY CONSULTATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Hematology Consultation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="hematology-consultations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Hematology Consultations</h2></div>
        <div className="empty-state">No hematology consultation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="hematology-consultations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Hematology Consultations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HematologyConsultationsDocumentPDFTemplate document={pdfData} />} fileName="Hematology_Consultations.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search hematology consultations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Hematology Consultation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'cbc-morphology')}
            {renderSection(record, idx, 'specialized-studies')}
...
export default HematologyConsultationsDocument;
