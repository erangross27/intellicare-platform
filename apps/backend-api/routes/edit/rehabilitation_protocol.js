...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'rehabilitation-protocol-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'phases', 'phases.phaseName', 'phases.duration', 'phases.weightBearing', 'phases.romGoals',
  'phases.exercises', 'phases.restrictions', 'phases.milestones',
  'braceProtocol', 'braceProtocol.type', 'braceProtocol.settings', 'braceProtocol.duration',
  'cpmProtocol', 'cpmProtocol.prescribed', 'cpmProtocol.settings',
  'results',
  'findings', 'assessment', 'plan', 'notes', 'recommendations'
];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('rehabilitation_protocol', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[rehabilitation_protocol] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('rehabilitation_protocol', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[rehabilitation_protocol] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
