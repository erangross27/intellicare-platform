...
const SECTIONS = [
  { id: 'schoolInformation', title: 'School Information', fields: ['date', 'grade', 'school', 'provider', 'facility'] }, { id: 'academicPerformance', title: 'Academic Performance', fields: ['academicPerformance'] }, { id: 'behaviorSocial', title: 'Behavior & Social', fields: ['behaviorInClass', 'peerInteractions'] }, { id: 'strengths', title: 'Strengths', fields: ['strengths'] }, { id: 'concerns', title: 'Concerns', fields: ['concerns'] }, { id: 'specialEducation', title: 'Special Education', fields: ['specialEducation', 'iepOr504Plan'] }, { id: 'schoolForms', title: 'School Forms Completed', fields: ['schoolFormsCompleted'] }, { id: 'findings', title: 'Findings', fields: ['findings'] }, { id: 'results', title: 'Results', fields: ['results'] }, { id: 'clinicalNotes', title: 'Clinical Notes', fields: ['assessment', 'plan', 'recommendations', 'notes', 'status'] },
];
const FIELD_LABELS = {
...
export default function SchoolPerformanceDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>School Performance</Text></View>
    {!records.length && <Text style={styles.noDataText}>No school performance data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>School Performance {recordIndex + 1}</Text></View>
