/**
 * BoneMarrowReportsDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: bone_marrow_reports
 *
 * 6 Sections:
 *   1. cellularity-morphology: cellularity, myeloidToErythroidRatio, megakaryocyteCount, blastPercentage, blastLineage, aspirateQuality
 *   2. dysplasia-fibrosis: dysplasticChanges, reticulinFibrosis, collagenFibrosis, ringSideroblasts
 *   3. cell-populations: plasmaCellPercentage, lymphocytePercentage, cd34PositiveCells
 *   4. ancillary: flowCytometryResults, fishResults, molecularMutations, cytogeneticsKaryotype
 *   5. iron-staining: ironStoresAssessment, tnfAlphaStaining
 *   6. classification-risk: mdsClassification, ipssRiskScore, biopsyLength
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import BoneMarrowReportsDocumentPDFTemplate from '../pdf-templates/BoneMarrowReportsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './BoneMarrowReportsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'bone_marrow_reportsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'cellularity-morphology': 'Cellularity & Morphology',
  'dysplasia-fibrosis': 'Dysplasia & Fibrosis',
...
const FIELD_LABELS = {
  ...
  myeloidToErythroidRatio: 'Myeloid-to-Erythroid Ratio',
  megakaryocyteCount: 'Megakaryocyte Count',
  blastPercentage: 'Blast %',
  blastLineage: 'Blast Lineage',
  aspirateQuality: 'Aspirate Quality',
  dysplasticChanges: 'Dysplastic Changes',
  reticulinFibrosis: 'Reticulin Fibrosis',
  collagenFibrosis: 'Collagen Fibrosis',
  ringSideroblasts: 'Ring Sideroblasts (%)',
  plasmaCellPercentage: 'Plasma Cell %',
  lymphocytePercentage: 'Lymphocyte %',
  cd34PositiveCells: 'CD34+ Cells (%)',
  flowCytometryResults: 'Flow Cytometry Results',
  fishResults: 'FISH Results',
  molecularMutations: 'Molecular Mutations',
  cytogeneticsKaryotype: 'Cytogenetics / Karyotype',
  ironStoresAssessment: 'Iron Stores Assessment',
  tnfAlphaStaining: 'TNF-Alpha Staining',
  mdsClassification: 'MDS Classification',
  ipssRiskScore: 'IPSS Risk Score',
  biopsyLength: 'Biopsy Length (mm)',
};

const SECTION_FIELDS = {
  'cellularity-morphology': ['cellularity', 'myeloidToErythroidRatio', 'megakaryocyteCount', 'blastPercentage', 'blastLineage', 'aspirateQuality'],
  'dysplasia-fibrosis': ['dysplasticChanges', 'reticulinFibrosis', 'collagenFibrosis', 'ringSideroblasts'],
  'cell-populations': ['plasmaCellPercentage', 'lymphocytePercentage', 'cd34PositiveCells'],
  'ancillary': ['flowCytometryResults', 'fishResults', 'molecularMutations', 'cytogeneticsKaryotype'],
  'iron-staining': ['ironStoresAssessment', 'tnfAlphaStaining'],
  'classification-risk': ['mdsClassification', 'ipssRiskScore', 'biopsyLength'],
};

const NUMBER_FIELDS = ['blastPercentage', 'ringSideroblasts', 'plasmaCellPercentage', 'lymphocytePercentage', 'biopsyLength', 'cd34PositiveCells'];
const ARRAY_FIELDS = ['flowCytometryResults', 'fishResults', 'molecularMutations', 'dysplasticChanges'];
const BOOLEAN_FIELDS = [];
const STRING_FIELDS = ['cellularity', 'myeloidToErythroidRatio', 'megakaryocyteCount', 'reticulinFibrosis', 'collagenFibrosis', 'ironStoresAssessment', 'cytogeneticsKaryotype', 'mdsClassification', 'blastLineage', 'ipssRiskScore', 'aspirateQuality', 'tnfAlphaStaining'];
...
/* ═══════ COMPONENT ═══════ */
const BoneMarrowReportsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.bone_marrow_reports) return Array.isArray(r.bone_marrow_reports) ? r.bone_marrow_reports : [r.bone_marrow_reports];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.bone_marrow_reports) return Array.isArray(dd.bone_marrow_reports) ? dd.bone_marrow_reports : [dd.bone_marrow_reports]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // fieldPart is "field" or "field.arrayIndex"; localEdits is always keyed by base "field-idx".
        ...
        if (isArr) nFields[`${fieldPart}-${idx}`] = 'edited';
        else { nFields[localKey] = 'edited'; nSentences[`${baseField}-${idx}-s0`] = 'edited'; }
      });
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Bone Marrow Report ${idx + 1}`.toLowerCase();
  ...
  function saveCommaItem(record, fn, idx, sid, sIdx, ciIdx) {
    ...
    setLocalEdits(prev => ({ ...prev, [localKey]: fullText }));
    setPendingEdits(prev => ({ ...prev, [localKey]: true }));
    const sentenceKey = `${localKey}-s${sIdx}`;
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // localEdits keys are "field-idx" (whole array stored under "field-idx" for ARRAY_FIELDS).
    ...
    try {
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/bone_marrow_reports/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/bone_marrow_reports/${id}/approve`, { sectionId: sid, approved: true });
      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this section's drafts from localStorage (now committed)
      const store = readDrafts();
      if (store[id]) { fields.forEach(f => { delete store[id][f]; Object.keys(store[id]).forEach(fp => { if (fp.startsWith(`${f}.`)) delete store[id][fp]; }); }); if (Object.keys(store[id]).length === 0) delete store[id]; writeDrafts(store); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== BONE MARROW REPORT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Bone Marrow Report ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="bone-marrow-reports-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Bone Marrow Report</h2></div>
        <div className="empty-state">No bone marrow report data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="bone-marrow-reports-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Bone Marrow Report</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<BoneMarrowReportsDocumentPDFTemplate document={pdfData} />} fileName={`bone-marrow-report-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search bone marrow report..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Bone Marrow Report ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'cellularity-morphology')}
            {renderSection(record, idx, 'dysplasia-fibrosis')}
            {renderSection(record, idx, 'cell-populations')}
            {renderSection(record, idx, 'ancillary')}
            {renderSection(record, idx, 'iron-staining')}
            {renderSection(record, idx, 'classification-risk')}
          </div>
        ))}
      </div>
    </div>
...
export default BoneMarrowReportsDocument;
