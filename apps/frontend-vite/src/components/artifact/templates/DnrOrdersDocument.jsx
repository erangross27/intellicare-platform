/**
 * DnrOrdersDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: dnr_orders
 *
 * 10 Sections:
 *   1. code-status: codeStatusLevel (sentence)
 *   2. resuscitation: resuscitationPreferences (sentence)
 *   3. advance-directive: patientAdvanceDirectiveStatus (sentence)
 *   4. healthcare-proxy: healthcareProxyDesignated (sentence)
 *   5. functional-status: functionalStatusBaseline (sentence)
 *   6. nutrition-hydration: artificalNutritionHydration (sentence)
 *   7. emergency-contacts: emergencyContactInformation (sentence)
 *   8. organ-donation: organDonationStatus (sentence)
 *   9. palliative-prognosis: palliativeCarePlanActive (boolean), prognosisDiscussion, decisionMakingCapacity
 *  10. order-details: physianOrderDateTime (date), orderReviewSchedule, hospitalVsOutpatientScope,
 *      comfortMeasuresSpecified (array), witnessInformation (array), familyNotificationPreferences,
 *      religiousConsiderations, reversalConditions
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import DnrOrdersDocumentPDFTemplate from '../pdf-templates/DnrOrdersDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './DnrOrdersDocument.css';
...
const SECTION_TITLES = {
  ...
  'resuscitation': 'Resuscitation Preferences',
  'advance-directive': 'Advance Directive Status',
  'healthcare-proxy': 'Healthcare Proxy',
  'functional-status': 'Functional Status Baseline',
  'nutrition-hydration': 'Nutrition & Hydration',
  'emergency-contacts': 'Emergency Contacts',
  'organ-donation': 'Organ Donation',
  'palliative-prognosis': 'Palliative Care & Prognosis',
  'order-details': 'Order & Additional Details',
};

const FIELD_LABELS = {
  codeStatusLevel: 'Code Status Level',
  resuscitationPreferences: 'Resuscitation Preferences',
  patientAdvanceDirectiveStatus: 'Advance Directive Status',
  healthcareProxyDesignated: 'Healthcare Proxy Designated',
  functionalStatusBaseline: 'Functional Status Baseline',
  artificalNutritionHydration: 'Artificial Nutrition & Hydration',
  emergencyContactInformation: 'Emergency Contact Information',
  organDonationStatus: 'Organ Donation Status',
  palliativeCarePlanActive: 'Palliative Care Plan Active',
  prognosisDiscussion: 'Prognosis Discussion',
  decisionMakingCapacity: 'Decision Making Capacity',
  physianOrderDateTime: 'Physician Order Date',
  orderReviewSchedule: 'Order Review Schedule',
  hospitalVsOutpatientScope: 'Hospital vs Outpatient Scope',
  comfortMeasuresSpecified: 'Comfort Measures Specified',
  witnessInformation: 'Witness Information',
  familyNotificationPreferences: 'Family Notification Preferences',
  religiousConsiderations: 'Religious Considerations',
  reversalConditions: 'Reversal Conditions',
};

const SECTION_FIELDS = {
  'code-status': ['codeStatusLevel'],
  'resuscitation': ['resuscitationPreferences'],
  'advance-directive': ['patientAdvanceDirectiveStatus'],
  'healthcare-proxy': ['healthcareProxyDesignated'],
  'functional-status': ['functionalStatusBaseline'],
  'nutrition-hydration': ['artificalNutritionHydration'],
  'emergency-contacts': ['emergencyContactInformation'],
  'organ-donation': ['organDonationStatus'],
...
/* Every narrative string field is a sentence field so it splits by sentence → semicolon → comma. The
   7 tail fields (prognosisDiscussion … reversalConditions) were previously rendering as one unsplit
   textarea blob. */
const SENTENCE_FIELDS = ['codeStatusLevel', 'resuscitationPreferences', 'patientAdvanceDirectiveStatus', 'healthcareProxyDesignated', 'functionalStatusBaseline', 'artificalNutritionHydration', 'emergencyContactInformation', 'organDonationStatus', 'prognosisDiscussion', 'decisionMakingCapacity', 'orderReviewSchedule', 'hospitalVsOutpatientScope', 'familyNotificationPreferences', 'religiousConsiderations', 'reversalConditions'];
const DATE_FIELDS = ['physianOrderDateTime'];
const BOOLEAN_FIELDS = ['palliativeCarePlanActive'];
const ARRAY_FIELDS = ['comfortMeasuresSpecified', 'witnessInformation'];
...
/* splitBySemicolon + splitGuardedComma (memory 6a4a4cd5): sentence → semicolon → guarded comma.
   Guards on the comma: paren-aware; skip no-space commas; keep Oxford ", and/or X" attached; skip date
   commas; next non-space char must be a letter/'('/'>'. */
...
const DRAFT_KEY = 'dnr_ordersPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const DnrOrdersDocument = ({ document: docProp }) => {
  ...
  const safeIdRaw = useCallback((r) => { if (!r?._id) return null; if (typeof r._id === 'string') return r._id; if (r._id.$oid) return r._id.$oid; return String(r._id); }, []);
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // Treat the dot-suffix as an arrayIndex ONLY when the segment after the LAST dot is purely numeric.
        const lastDot = fieldPart.lastIndexOf('.');
        const tail = lastDot !== -1 ? fieldPart.slice(lastDot + 1) : '';
        const isArrayElem = lastDot !== -1 && /^\d+$/.test(tail);
        if (isArrayElem) {
          const fn = fieldPart.slice(0, lastDot);
          const arrIdx = parseInt(tail, 10);
          const arrKey = `${fn}-${idx}`;
          // Array fields keep the WHOLE array in localEdits; splice the staged element back in.
          const base = Array.isArray(nLocal[arrKey]) ? nLocal[arrKey] : [...(Array.isArray(record[fn]) ? record[fn] : [])];
          base[arrIdx] = value;
          nLocal[arrKey] = base;
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `DNR Order ${idx + 1}`.toLowerCase();
  ...
  // NOT written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve (commit happens there).
  ...
  // Save one sentence = stage a DRAFT locally + persist to localStorage (survives refresh). No DB write;
  // Pending Approve commits it later.
  ...
  /* saveSentenceLeaf: edit ONE leaf of an unlabeled sentence addressed by (sIdx, pi=semicolon-part,
     ...
     reconstructFullText; empty edit removes the leaf (memory 6a4a4cd5). Stages a draft (no DB write). */
  function saveSentenceLeaf(record, fn, idx, sid, sIdx, pi, ci, isComma) {
    const id = safeId(record); if (!id) return;
    const currentVal = String(getFieldValue(record, fn, idx) || '');
    const sentences = splitBySentence(currentVal);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      const toCommit = Object.keys(recDrafts).filter(fieldPart => {
        const lastDot = fieldPart.lastIndexOf('.');
        const tail = lastDot !== -1 ? fieldPart.slice(lastDot + 1) : '';
        const base = (lastDot !== -1 && /^\d+$/.test(tail)) ? fieldPart.slice(0, lastDot) : fieldPart;
      ...
      for (const fieldPart of toCommit) {
        ...
        const tail = lastDot !== -1 ? fieldPart.slice(lastDot + 1) : '';
        const isArrayElem = lastDot !== -1 && /^\d+$/.test(tail);
        const payload = { field: isArrayElem ? fieldPart.slice(0, lastDot) : fieldPart, value: recDrafts[fieldPart] };
        if (isArrayElem) payload.arrayIndex = parseInt(tail, 10);
        const resp = await secureApiClient.put(`/api/edit/dnr_orders/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/dnr_orders/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[DnrOrders] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const renderArrayField = (record, fn, idx, sid, title) => {
    ...
    const label = FIELD_LABELS[fn] || fn;
    const sl = label.toLowerCase() !== (title || '').toLowerCase(); // single-name rule (6a4a6168)
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemText); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const newVal = editValue; setSaveError(null); const arr2 = [...(Array.isArray(localEdits[`${fn}-${idx}`]) ? localEdits[`${fn}-${idx}`] : val)]; arr2[arrIdx] = newVal; setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: arr2 })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { const k = `${sid}-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; }); const store = readDrafts(); if (!store[id]) store[id] = {}; store[id][`${fn}.${arrIdx}`] = newVal; writeDrafts(store); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.value); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const idS = safeId(record); if (!idS) return; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = `${parsed.label}: ${editValue.trim()}`; const fullText = reconstructFullText(sentences2); setSaveError(null); setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullText })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); setEditedSentences(prev => ({ ...prev, [sentenceKey]: 'edited' })); setApprovedSections(prev => { const k = `${sid}-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; }); const store = readDrafts(); if (!store[idS]) store[idS] = {}; store[idS][fn] = fullText; writeDrafts(store); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
                      ...
                      <div className={`numbered-row ${leafBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!leafEditing) { setEditingField(leafKey); setEditValue(leaf.text); setSaveError(null); } }}>
                        {leafEditing ? (
                          <div className="edit-field-container">
                            ...
                            {saveError && <div className="save-error">{saveError}</div>}
                            <div className="edit-actions">
                              <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveSentenceLeaf(record, fn, idx, sid, sIdx, leaf.pi, leaf.ci, leaf.comma); }}>{saving ? 'Saving...' : 'Save'}</button>
                              <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                            </div>
                          </div>
                        ) : (
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      ...
      if (BOOLEAN_FIELDS.includes(f)) return val === true || val === false;
      return hasVal(val);
    });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="dnr-orders-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">DNR Orders</h2></div>
        <div className="empty-state">No DNR order records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="dnr-orders-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">DNR Orders</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<DnrOrdersDocumentPDFTemplate document={pdfData} />} fileName="DNR_Orders.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search DNR orders..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`DNR Order ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'code-status')}
            {renderSection(record, idx, 'resuscitation')}
            {renderSection(record, idx, 'advance-directive')}
            {renderSection(record, idx, 'healthcare-proxy')}
            {renderSection(record, idx, 'functional-status')}
            {renderSection(record, idx, 'nutrition-hydration')}
            {renderSection(record, idx, 'emergency-contacts')}
            {renderSection(record, idx, 'organ-donation')}
            {renderSection(record, idx, 'palliative-prognosis')}
...
export default DnrOrdersDocument;
