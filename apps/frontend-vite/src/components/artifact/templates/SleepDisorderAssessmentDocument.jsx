 ...
 * July 2026 — recursive, dot-path editable sleep disorder assessment
...
const SECTION_TITLES = {
  ...
  screeningScores: 'Screening Scores',
  sleepMetrics: 'Sleep Metrics',
  symptoms: 'Symptoms',
  disorderClassifications: 'Disorder Classifications',
  physicalExam: 'Physical Examination',
  narcolepsySymptoms: 'Narcolepsy Symptoms',
  sleepEnvironment: 'Sleep Environment',
  priorTesting: 'Prior Testing',
  cpapCompliance: 'CPAP Compliance',
};
const FIELD_LABELS = {
...
const SECTION_FIELDS = {
  assessmentInfo: ['date', 'status'],
  screeningScores: ['epworthSleepinessScore', 'pittsburghSleepQualityIndex', 'stopBangScore'],
  sleepMetrics: ['averageSleepDuration', 'sleepLatency', 'nighttimeAwakenings'],
  symptoms: ['snoringFrequency', 'witnessedApneas', 'restlessLegSymptoms', 'periodicLimbMovements'],
  disorderClassifications: ['parasomniaType', 'insomniaSubtype', 'circadianRhythmDisorder'],
  physicalExam: ['neckCircumference', 'mallampatiScore'],
  narcolepsySymptoms: ['cataplexyPresent', 'sleepParalysisFrequency', 'hypnagogicHallucinations'],
  sleepEnvironment: ['bedtimeRoutine', 'caffeineIntakeDaily', 'sleepEnvironmentIssues'],
  priorTesting: ['priorPolysomnographyDate', 'apneaHypopneaIndex', 'oxygenDesaturationIndex'],
  cpapCompliance: ['currentCpapCompliance'],
};
const DATE_FIELDS = ['date', 'priorPolysomnographyDate'];
const DATETIME_FIELDS = [];
const ENUM_FIELDS = ['status', 'snoringFrequency', 'insomniaSubtype', 'circadianRhythmDisorder'];
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'status' ? ['Active', 'Inactive', 'Resolved']
    : field === 'snoringFrequency' ? ['Never', 'Rarely', 'Sometimes', 'Frequently', 'Nightly']
      : field === 'insomniaSubtype' ? ['Sleep Onset Insomnia', 'Sleep Maintenance Insomnia', 'Early Morning Awakening', 'Mixed']
        : field === 'circadianRhythmDisorder' ? ['None', 'Delayed Sleep Phase', 'Advanced Sleep Phase', 'Irregular', 'Shift Work'] : [];
...
const SleepDisorderAssessmentDocument = ({ document: docProp }) => {
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/sleep_disorder_assessment/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/sleep_disorder_assessment/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SleepDisorderAssessment] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Sleep Disorder Assessment Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="sleep-disorder-assessment-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Sleep Disorder Assessment</h2></div><div className="empty-state">No sleep disorder assessment available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Sleep Disorder Assessment Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="sleep-disorder-assessment-document" ref={containerRef}>
    <div className="document-header">
      ...
      <div className="header-actions">
        ...
        <PDFDownloadLink document={<SleepDisorderAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Sleep_Disorder_Assessment.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search sleep disorder assessment..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Sleep Disorder Assessment Record ' + (recordIndex + 1))}</h3></div>
