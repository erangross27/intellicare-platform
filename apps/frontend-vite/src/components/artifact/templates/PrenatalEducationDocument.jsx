/**
 * PrenatalEducationDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: prenatal_education
 *
 * 10 Sections:
 *   1. clinical-info: date, provider, facility
 *   2. education-topics: topicsDiscussed
 *   3. education-status: childbirtClassesEnrolled, breastfeedingEducation, pretermLaborPrecautions, nutritionCounseling, exerciseGuidance
 *   4. classes-attended: classesAttended
 *   5. warning-signs: warningSignsReviewed
 *   6. restrictions-mods: travelRestrictions, workModifications
 *   7. findings-section: findings
 *   8. results-section: results (OBJECT)
 *   9. assessment-section: assessment
 *  10. plan-section: plan
 *  11. recommendations-section: recommendations (OBJECT ARRAY {recommendation, date})
 *  12. notes-section: notes, status
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PrenatalEducationDocumentPDFTemplate from '../pdf-templates/PrenatalEducationDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './PrenatalEducationDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldName]: value } }  (value = the whole field value) */
const DRAFT_KEY = 'prenatal_educationPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'warning-signs': 'Warning Signs Reviewed',
  'restrictions-mods': 'Restrictions & Modifications',
...
const FIELD_LABELS = {
  ...
  childbirtClassesEnrolled: 'Childbirth Classes Enrolled',
  breastfeedingEducation: 'Breastfeeding Education',
  pretermLaborPrecautions: 'Preterm Labor Precautions',
  ...
  workModifications: 'Work Modifications',
  findings: 'Findings',
  results: 'Results',
...
const SECTION_FIELDS = {
  ...
  'education-topics': ['topicsDiscussed'],
  'education-status': ['childbirtClassesEnrolled', 'breastfeedingEducation', 'pretermLaborPrecautions', 'nutritionCounseling', 'exerciseGuidance'],
  'classes-attended': ['classesAttended'],
  'warning-signs': ['warningSignsReviewed'],
  'restrictions-mods': ['travelRestrictions', 'workModifications'],
  'findings-section': ['findings'],
  'results-section': ['results'],
  'assessment-section': ['assessment'],
  'plan-section': ['plan'],
  'recommendations-section': ['recommendations'],
  'notes-section': ['notes', 'status'],
};

const BOOLEAN_FIELDS = ['childbirtClassesEnrolled', 'breastfeedingEducation', 'pretermLaborPrecautions', 'nutritionCounseling', 'exerciseGuidance'];
const DATE_FIELDS = ['date'];
const ARRAY_FIELDS = ['topicsDiscussed', 'classesAttended', 'warningSignsReviewed', 'workModifications'];
const OBJECT_FIELDS = ['results'];
const OBJECT_ARRAY_FIELDS = ['recommendations'];
const STRING_FIELDS = ['provider', 'facility', 'travelRestrictions', 'findings', 'assessment', 'plan', 'notes', 'status'];
...
/* ═══════ COMPONENT ═══════ */
const PrenatalEducationDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.prenatal_education) return Array.isArray(r.prenatal_education) ? r.prenatal_education : [r.prenatal_education];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.prenatal_education) return Array.isArray(dd.prenatal_education) ? dd.prenatal_education : [dd.prenatal_education]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Prenatal Education ${idx + 1}`.toLowerCase();
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.ef) — value stays a STRING.
     Stages the whole cloned root object as a DRAFT (no DB write); Approve commits it. */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
    // Stage draft: store the whole cloned root object so it survives refresh.
    const store = readDrafts();
    const curStore = store[id]?.[rootField] !== undefined ? store[id][rootField] : record[rootField];
    const draftClone = JSON.parse(JSON.stringify(curStore ?? {}));
    let dn = draftClone;
  ...
  // Stage a sentence-field DRAFT locally (no DB write). localStorage keeps it across refresh; Approve commits.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // localEdits keys are `${fieldName}-${idx}` (whole field value). Commit each pending one in this section.
      ...
      for (const editKey of toCommit) {
        ...
        const fn = m[1];
        await secureApiClient.put(`/api/edit/prenatal_education/${id}/edit`, { field: fn, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/prenatal_education/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[PrenatalEducation] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        if (recs.length > 0) {
          text += `${label}\n`;
          recs.forEach((rec, i) => { const r = (rec?.recommendation || '').trim(); const d = (rec?.date || '').trim(); text += `${i + 1}. ${r}${d ? ` (${d})` : ''}\n`; });
          text += '\n';
        }
      } else if (OBJECT_FIELDS.includes(f)) {
        if (!isEmptyDeep(val) && !isScalar(val)) {
          text += `${label}\n`;
          const walk = (obj, indent) => {
            Object.entries(obj).filter(([, v]) => !isEmptyDeep(v)).forEach(([k, v]) => {
              const pad = '  '.repeat(indent);
              if (isScalar(v)) text += `${pad}${humanizeKey(k)}: ${fmtScalar(v)}\n`;
              else { text += `${pad}${humanizeKey(k)}:\n`; walk(v, indent + 1); }
            });
          };
          walk(val, 0);
          text += '\n';
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PRENATAL EDUCATION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Prenatal Education ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable scalar leaf in results) ═══════ */
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              const stageKey = `${fn}-${idx}`;
                              setLocalEdits(prev => ({ ...prev, [stageKey]: newArr }));
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      if (OBJECT_ARRAY_FIELDS.includes(f)) return Array.isArray(val) && val.filter(x => !isEmptyDeep(x)).length > 0;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="prenatal-education-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Prenatal Education</h2></div>
        <div className="empty-state">No prenatal education records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="prenatal-education-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Prenatal Education</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PrenatalEducationDocumentPDFTemplate document={pdfData} />} fileName={`prenatal-education-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search prenatal education records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Prenatal Education ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'clinical-info')}
            {renderSection(record, idx, 'education-topics')}
            {renderSection(record, idx, 'education-status')}
            {renderSection(record, idx, 'classes-attended')}
            {renderSection(record, idx, 'warning-signs')}
            {renderSection(record, idx, 'restrictions-mods')}
...
export default PrenatalEducationDocument;
