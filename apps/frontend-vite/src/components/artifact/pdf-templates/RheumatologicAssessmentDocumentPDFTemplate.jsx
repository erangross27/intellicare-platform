...
const SECTIONS = [
  ...
  { id: 'clinicalPresentation', title: 'Clinical Presentation', fields: ['chiefComplaint', 'symptomDuration'] },
  { id: 'morningStiffness', title: 'Morning Stiffness', fields: ['morningStiffness'] },
  { id: 'jointInvolvement', title: 'Joint Involvement', fields: ['jointInvolvement'] },
  { id: 'systemicSymptoms', title: 'Systemic Symptoms', fields: ['systemicSymptoms'] },
...
export default function RheumatologicAssessmentDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Rheumatologic Assessment</Text></View>
    {!records.length && <Text style={styles.noDataText}>No rheumatologic assessment data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Rheumatologic Assessment {recordIndex + 1}</Text></View>
