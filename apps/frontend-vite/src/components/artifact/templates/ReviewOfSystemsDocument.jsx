/**
 * ReviewOfSystemsDocument.jsx
 * July 2026 — recursive, dot-path editable review of systems
...
const FIELD_LABELS = {
  ...
  constitutional: 'Constitutional', heent: 'HEENT', eyes: 'Eyes', ent: 'ENT', cardiovascular: 'Cardiovascular', respiratory: 'Respiratory', gastrointestinal: 'Gastrointestinal', genitourinary: 'Genitourinary', musculoskeletal: 'Musculoskeletal', neurological: 'Neurological', psychiatric: 'Psychiatric', endocrine: 'Endocrine', hematologic: 'Hematologic', skin: 'Skin', sleepSymptoms: 'Sleep Symptoms',
...
const SECTION_FIELDS = {
  ...
  systems: ['constitutional', 'heent', 'eyes', 'ent', 'cardiovascular', 'respiratory', 'gastrointestinal', 'genitourinary', 'musculoskeletal', 'neurological', 'psychiatric', 'endocrine', 'hematologic', 'skin', 'sleepSymptoms'],
  clinicalFindings: ['findings'],
  clinicalAssessment: ['assessment'],
  treatmentPlan: ['plan'],
  additionalNotes: ['notes'],
...
const OBJECT_FIELDS = ['genitourinary', 'psychiatric', 'results'];
const OBJECT_ARRAY_FIELDS = ['recommendations'];
const COMMA_FIELDS = ['constitutional', 'heent', 'eyes', 'ent', 'cardiovascular', 'respiratory', 'gastrointestinal', 'musculoskeletal', 'neurological', 'endocrine', 'hematologic', 'skin', 'sleepSymptoms'];
const SEMICOLON_FIELDS = ['constitutional', 'heent', 'eyes', 'ent', 'cardiovascular', 'respiratory', 'gastrointestinal', 'musculoskeletal', 'neurological', 'endocrine', 'hematologic', 'skin', 'sleepSymptoms', 'findings', 'assessment', 'plan', 'notes'];
...
const ReviewOfSystemsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.review_of_systems) return Array.isArray(record.review_of_systems) ? record.review_of_systems : [record.review_of_systems];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.review_of_systems) return Array.isArray(nested.review_of_systems) ? nested.review_of_systems : [nested.review_of_systems];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/review_of_systems/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/review_of_systems/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[ReviewOfSystems] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Review Of Systems ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
    ...
    return <div className="section" key={sectionId}><div className="mini-cards-container">
      ...
      {sectionId === 'results' ? renderResults(record, recordIndex, sectionId)
          : sectionId === 'recommendations' ? renderRecommendations(record, recordIndex, sectionId)
            : fields.map((field) => OBJECT_FIELDS.includes(field)
  ...
  if (!records.length) return <div className="review-of-systems-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Review Of Systems</h2></div><div className="empty-state">No review of systems records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Review Of Systems ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="review-of-systems-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Review Of Systems</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<ReviewOfSystemsDocumentPDFTemplate document={pdfData} />} fileName="Review_Of_Systems.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search review of systemss..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Review Of Systems ' + (recordIndex + 1))}</h3></div>
