/**
 * MaternalFetalReportsDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: maternal_fetal_reports
 *
 * 6 Sections:
 *   1. gestational-info: gestationalAgeWeeks, gestationalAgeDays, fetalPresentation, fetalHeartRate
 *   2. biometry: estimatedFetalWeight, fetalWeightPercentile, biparietalDiameter, headCircumference, abdominalCircumference, femurLength
 *   3. doppler-assessment: umbilicalArteryDopplerPI, middleCerebralArteryPI, cerebroplascentalRatio, cerebrospinalFluidIndex
 *   4. fetal-wellbeing: nonstressTestReactivity, biophysicalProfileScore, intrauterineGrowthRestriction, fetalAnomaliesDetected
 *   5. placenta-fluid: placentalLocation, placentalGrade, cervicalLength, oligohydramnios, polyhydramnios
 *   6. maternal-vitals: maternalBloodPressureSystolic, maternalBloodPressureDiastolic
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MaternalFetalReportsDocumentPDFTemplate from '../pdf-templates/MaternalFetalReportsDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './MaternalFetalReportsDocument.css';
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits field, e.g. "fetalHeartRate";
   array fields stage their FULL array under the bare field name). */
const DRAFT_KEY = 'maternal_fetal_reportsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['nonstressTestReactivity', 'placentalLocation', 'fetalPresentation'];
/* MEANINGFUL_ZERO_FIELDS: numerics where 0 is a valid value to render as-is, NOT a "not-measured" sentinel.
 * placentalGrade is a numeric-valued grade (Grannum 0) — "0" is a real reading, render it as-is on the
 * −/+ stepper. Every other numeric that reads 0 (gestationalAgeDays, doppler PIs, cerebroplacental ratio,
 * biophysicalProfileScore) is a not-recorded sentinel → hide-zero. A doctor edit to 0 still shows it. */
...
/* ═══════ COMPONENT ═══════ */
const MaternalFetalReportsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.maternal_fetal_reports) return Array.isArray(r.maternal_fetal_reports) ? r.maternal_fetal_reports : [r.maternal_fetal_reports];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.maternal_fetal_reports) return Array.isArray(dd.maternal_fetal_reports) ? dd.maternal_fetal_reports : [dd.maternal_fetal_reports]; return [dd]; }
      return [r];
    });
  ...
  /* numberShows: numeric "not recorded" (0) hidden unless meaningful-zero field or doctor-edited */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Maternal Fetal Report ${idx + 1}`.toLowerCase();
  ...
  // saveSentence = stage a DRAFT locally + write to the pending-drafts localStorage store (no DB write).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // localEdits keys for this section's pending edits (key shape: "field-idx"; arrays stage the full array)
    const toCommit = Object.keys(localEdits).filter(k => {
      if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
      const fieldPart = k.slice(0, -suffix.length);
      const baseField = (() => { const dot = fieldPart.lastIndexOf('.'); return (dot !== -1 && /^\d+$/.test(fieldPart.slice(dot + 1))) ? fieldPart.slice(0, dot) : fieldPart; })();
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const payload = { field: isArrayIdx ? fieldPart.slice(0, dot) : fieldPart, value: localEdits[editKey] };
        if (isArrayIdx) payload.arrayIndex = parseInt(fieldPart.slice(dot + 1), 10);
        const resp = await secureApiClient.put(`/api/edit/maternal_fetal_reports/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/maternal_fetal_reports/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const formatSentenceFieldLines = useCallback((text) => {
    ...
    splitBySentence(text).forEach(s => {
      ...
      if (parsed.isLabeled) {
        ...
        let m = 1; (parts.length >= 2 ? parts : [parsed.value]).forEach(it => lines.push(`  ${m++}. ${String(it).replace(/[;.]+$/, '').trim()}`));
      } else { lines.push(`${n++}. ${String(s).replace(/[;.]+$/, '').trim()}`); }
    });
    return lines;
  }, [splitBySentence]);

  /* Copy: label on its own line, then value numbered "1. value" at column 0 (GlasgowComaScale style —
     never side-by-side "Label: value"). Section header + ==== divider only emitted when body non-empty. */
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MATERNAL FETAL REPORTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Maternal Fetal Report ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { currentArr.splice(itemIdx, 1, ...subParts); } else { currentArr[itemIdx] = trimmed; } const arrEditKey = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [arrEditKey]: currentArr })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setPendingEdits(prev => ({ ...prev, [arrEditKey]: true })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const storeA = readDrafts(); if (!storeA[id]) storeA[id] = {}; storeA[id][fn] = currentArr; writeDrafts(storeA); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                            ...
                            <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                              {ciEditing ? (
                                <div className="edit-field-container">
                                  ...
                                  {saveError && <div className="save-error">{saveError}</div>}
                                  <div className="edit-actions">
                                    <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const currentVal2 = String(getFieldValue(record, fn, idx) || ''); const sentences2 = splitBySentence(currentVal2); const s2 = sentences2[sIdx] || ''; const p2 = parseLabel(s2); if (!p2.isLabeled) return; const items2 = splitByComma(p2.value); const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { items2.splice(ciIdx, 1, ...subParts); } else { items2[ciIdx] = trimmed.replace(/[;.]+$/, '').trim(); } const rebuilt = `${p2.label}: ${items2.join(', ')}.`; const allS = [...sentences2]; allS[sIdx] = rebuilt; const fullText2 = reconstructFullText(allS); const editKey2 = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [editKey2]: fullText2 })); const marks = { [commaKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ciIdx + ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setPendingEdits(prev => ({ ...prev, [editKey2]: true })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const store2 = readDrafts(); if (!store2[id2]) store2[id2] = {}; store2[id2][fn] = fullText2; writeDrafts(store2); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                                    <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                                  </div>
                                </div>
                              ) : (
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const id3 = safeId(record); if (!id3) return; const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); const editKey3 = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [editKey3]: fullText })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setPendingEdits(prev => ({ ...prev, [editKey3]: true })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const store3 = readDrafts(); if (!store3[id3]) store3[id3] = {}; store3[id3][fn] = fullText; writeDrafts(store3); setEditingField(null); setEditValue(''); setSaveError(null); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="maternal-fetal-reports-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Maternal Fetal Reports</h2></div>
        <div className="empty-state">No maternal fetal report records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="maternal-fetal-reports-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Maternal Fetal Reports</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MaternalFetalReportsDocumentPDFTemplate document={pdfData} />} fileName="Maternal_Fetal_Reports.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search maternal fetal report records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Maternal Fetal Report ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'gestational-info')}
            {renderSection(record, idx, 'biometry')}
            {renderSection(record, idx, 'doppler-assessment')}
            {renderSection(record, idx, 'fetal-wellbeing')}
            {renderSection(record, idx, 'placenta-fluid')}
            {renderSection(record, idx, 'maternal-vitals')}
          </div>
        ))}
      </div>
    </div>
...
export default MaternalFetalReportsDocument;
