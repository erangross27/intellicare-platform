/**
 * WorkersCompEvaluationsDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: workers_comp_evaluations
 *
 * 4 Sections:
 *   1. injury: injuryMechanism, anatomicalSiteInjured, occupationalDisease, painScaleNumerical
 *   2. exam-findings: rangeOfMotionMeasurements, manualMuscleTestGrades, neurologicalDeficits, phalemsTest, tinelsSign, quickDashScore, oswestryDisabilityIndex
 *   3. impairment-status: maxMedicalImprovement, permanentPartialImpairment, workRestrictions, returnToWorkStatus, functionalCapacityEvaluation
 *   4. opinions: causationOpinion, apportionmentAnalysis, independentMedicalExam, treatmentNecessity, vocationalRehabilitation, futureCareCosts
 *
 * NOTE: schema has NO record date/provider/facility field — record header renders TITLE + index ONLY.
 */
...
const FIELD_LABELS = {
  ...
  anatomicalSiteInjured: 'Anatomical Site Injured',
  occupationalDisease: 'Occupational Disease',
  painScaleNumerical: 'Pain Scale (0-10)',
  rangeOfMotionMeasurements: 'Range of Motion Measurements',
  manualMuscleTestGrades: 'Manual Muscle Test Grades',
  neurologicalDeficits: 'Neurological Deficits',
  phalemsTest: "Phalen's Test",
  tinelsSign: "Tinel's Sign",
  quickDashScore: 'QuickDASH Score',
  oswestryDisabilityIndex: 'Oswestry Disability Index',
  maxMedicalImprovement: 'Maximum Medical Improvement',
  permanentPartialImpairment: 'Permanent Partial Impairment (%)',
  ...
  apportionmentAnalysis: 'Apportionment Analysis',
  independentMedicalExam: 'Independent Medical Exam',
  treatmentNecessity: 'Treatment Necessity',
  vocationalRehabilitation: 'Vocational Rehabilitation',
  futureCareCosts: 'Future Care Costs',
};

const SECTION_FIELDS = {
  'injury': ['injuryMechanism', 'anatomicalSiteInjured', 'occupationalDisease', 'painScaleNumerical'],
  'exam-findings': ['rangeOfMotionMeasurements', 'manualMuscleTestGrades', 'neurologicalDeficits', 'phalemsTest', 'tinelsSign', 'quickDashScore', 'oswestryDisabilityIndex'],
  'impairment-status': ['maxMedicalImprovement', 'permanentPartialImpairment', 'workRestrictions', 'returnToWorkStatus', 'functionalCapacityEvaluation'],
  'opinions': ['causationOpinion', 'apportionmentAnalysis', 'independentMedicalExam', 'treatmentNecessity', 'vocationalRehabilitation', 'futureCareCosts'],
};

const NUMBER_FIELDS = ['painScaleNumerical', 'quickDashScore', 'oswestryDisabilityIndex', 'permanentPartialImpairment'];
/* MEANINGFUL_ZERO_FIELDS: clinical scores/ratings where 0 is a valid finding (no pain / no
   disability / no impairment) → always show when present, never hidden as "not recorded". */
const MEANINGFUL_ZERO_FIELDS = ['painScaleNumerical', 'quickDashScore', 'oswestryDisabilityIndex', 'permanentPartialImpairment'];
const ARRAY_FIELDS = ['workRestrictions'];
const BOOLEAN_FIELDS = ['maxMedicalImprovement', 'independentMedicalExam'];
const STRING_FIELDS = ['injuryMechanism', 'anatomicalSiteInjured', 'occupationalDisease', 'rangeOfMotionMeasurements', 'manualMuscleTestGrades', 'neurologicalDeficits', 'phalemsTest', 'tinelsSign', 'returnToWorkStatus', 'functionalCapacityEvaluation', 'causationOpinion', 'apportionmentAnalysis', 'treatmentNecessity', 'vocationalRehabilitation', 'futureCareCosts'];
...
/* ═══════ COMPONENT ═══════ */
const WorkersCompEvaluationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.workers_comp_evaluations) return Array.isArray(r.workers_comp_evaluations) ? r.workers_comp_evaluations : [r.workers_comp_evaluations];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.workers_comp_evaluations) return Array.isArray(dd.workers_comp_evaluations) ? dd.workers_comp_evaluations : [dd.workers_comp_evaluations]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Workers Compensation Evaluation ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const suffix = `-${idx}`;
      // Staged edits for this record live in localEdits keyed "<fieldPart>-<idx>"; commit only this section's.
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/workers_comp_evaluations/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/workers_comp_evaluations/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== WORKERS COMPENSATION EVALUATION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Workers Compensation Evaluation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="workers-comp-evaluations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Workers Compensation Evaluation</h2></div>
        <div className="empty-state">No workers compensation evaluation data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="workers-comp-evaluations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Workers Compensation Evaluation</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<WorkersCompEvaluationsDocumentPDFTemplate document={pdfData} />} fileName={`workers-comp-evaluations-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search workers compensation evaluation..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Workers Compensation Evaluation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'injury')}
            {renderSection(record, idx, 'exam-findings')}
            {renderSection(record, idx, 'impairment-status')}
            {renderSection(record, idx, 'opinions')}
          </div>
        ))}
      </div>
    </div>
