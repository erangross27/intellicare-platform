/**
 * DischargePlanning Edit Route
 * ServiceId: discharge-planning-edit-service
...
function buildContext(req, operation = 'read') { return { serviceId: 'discharge-planning-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }

const ALLOWED_FIELDS = [
  'date', 'provider', 'facility', 'status', 'expectedLOS', 'dischargeDestination', 'returnToWork',
  'followUpInstructions', 'activityRestrictions', 'warningSignsToWatch',
  'comprehensiveDischargeReadiness', 'findings', 'assessment', 'plan', 'results', 'recommendations', 'notes'
];

function isFieldAllowed(field) {
  if (ALLOWED_FIELDS.includes(field)) return true;
  const root = field.split('.')[0];
  return ALLOWED_FIELDS.includes(root);
}
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('discharge_planning', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[DischargePlanning] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('discharge_planning', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[DischargePlanning] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
