/**
 * PsychotropicMedicationsDocument.jsx
 * March 2026 -- Complete rewrite with inline editing, blue glow theme
 * Collection: psychotropic_medications
 *
 * 7 Sections:
 *   1. medication-info: name, genericName, dosage, frequency, route, active
 *   2. prescription-details: startDate, endDate, duration, durationDays, durationUnit, prescriber, refills
 *   3. indication-instructions: indication, instructions
 *   4. current-medications: current (array of med objects)
 *   5. past-medications: past (array of med objects)
 *   6. medication-changes: medicationChanges (array of change objects)
 *   7. safety-allergies: sideEffects, drugInteractions, allergiesAdverse, safetyWarning
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PsychotropicMedicationsDocumentPDFTemplate from '../pdf-templates/PsychotropicMedicationsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PsychotropicMedicationsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits key sans the "-<idx>" suffix) */
const DRAFT_KEY = 'psychotropic_medicationsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['name', 'genericName', 'dosage', 'frequency', 'route', 'duration', 'durationUnit', 'prescriber', 'indication', 'instructions', 'safetyWarning'];
const OBJECT_ARRAY_FIELDS = ['current', 'past', 'medicationChanges', 'allergiesAdverse'];

/* MEANINGFUL_ZERO_FIELDS: numerics where 0 is a real value. For psychotropic meds durationDays:0 and
   refills:0 are extractor not-set sentinels → hide both (none are meaningful-zero). */
const MEANINGFUL_ZERO_FIELDS = [];
/* isHiddenZero: a sentinel-zero numeric (0 meaning "not set") that must be hidden in render/Copy/PDF */
const isHiddenZero = (fn, val) => NUMBER_FIELDS.includes(fn) && Number(val) === 0 && !MEANINGFUL_ZERO_FIELDS.includes(fn);
/* isEpochSentinel: a date defaulted to 1970-01-01 (Unix epoch 0 = extractor garbage) → treat as empty */
...
/* sameAsTitle: field label duplicates its section title (single-name collision) */
...
/* flattenItem: array item → readable string (sideEffects/drugInteractions are plain strings) */
...
/* OBJECT_ARRAY_CONFIG: per object-array field, the sub-fields stacked under each item's primary text.
   The primary (medication name / action+medication) becomes the item header; the rest stack below it. */
...
/* ======= COMPONENT ======= */
const PsychotropicMedicationsDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Psychotropic Medications ${idx + 1}`.toLowerCase();
  ...
  // localEditsKey is the canonical `${fn}-${idx}` slot (holds the FULL merged field value: text / array / object array).
  // trackKey is the per-row marker key (editedFields/editedSentences). sid lets us clear the approved flag.
  const stageDraft = useCallback((record, idx, localEditsKey, value, trackKey, sid, isSentence) => {
    const id = safeId(record); if (!id) return;
    setLocalEdits(prev => ({ ...prev, [localEditsKey]: value }));
    setPendingEdits(prev => ({ ...prev, [localEditsKey]: true }));
    if (isSentence) setEditedSentences(prev => ({ ...prev, [trackKey]: 'edited' }));
    ...
    // fieldPart = the localEdits key sans the trailing "-<idx>" suffix.
    const fieldPart = localEditsKey.replace(new RegExp(`-${idx}$`), '');
    const store = readDrafts();
  ...
  // Find the sid (section) that owns a given field name (for clearing the approved flag on re-edit).
  const sidForField = useCallback((fn) => Object.keys(SECTION_FIELDS).find(s => (SECTION_FIELDS[s] || []).includes(fn)) || null, []);

  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    const id = safeId(record); if (!id) return;
    const saveVal = valueOverride !== undefined ? valueOverride : editValue;
    const trackKey = editTrackingKey || `${fn}-${idx}`;
    stageDraft(record, idx, `${fn}-${idx}`, saveVal, trackKey, sid || sidForField(fn), false);
  }, [editValue, safeId, stageDraft, sidForField]);
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageDraft(record, idx, `${fn}-${idx}`, fullText, `${fn}-${idx}-s${sentenceIdx}`, sid || sidForField(fn), true);
      return;
    }
    ...
    const sidOwner = sid || sidForField(fn);
    if (sidOwner) setApprovedSections(prev => { const k = `${sidOwner}-${idx}`; if (!prev[k]) return prev; const nn = { ...prev }; delete nn[k]; return nn; });
    const store = readDrafts();
    if (!store[id2]) store[id2] = {};
    store[id2][`${fn}`] = fullText;
    writeDrafts(store);
    setEditingField(null); setEditValue('');
  }
  ...
  // localEdits holds the FULL merged value per `${fn}-${idx}` slot, so each commit replaces the whole
  // field (field + value, no arrayIndex). The "<field>.<idx>" dotted-suffix case never occurs here because
  // localEdits is always keyed `${fn}-${idx}` (the trailing dot-segment is never purely numeric).
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    // Collect this record's pending edits whose field belongs to this section, using the "-<idx>" suffix convention.
    const suffix = `-${idx}`;
    ...
    try {
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "<field>" (never numeric-suffixed here)
        ...
        const resp = await secureApiClient.put(`/api/edit/psychotropic_medications/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/psychotropic_medications/${id}/approve`, { sectionId: sid, approved: true });
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        items.forEach((item, i) => {
          text += `${i + 1}. ${objPrimary(f, item)}\n`;
          (OBJECT_ARRAY_CONFIG[f] || []).forEach(([k, subLabel]) => {
            const v = item[k];
            if (!hasVal(v)) return;
        ...
        if (items.length) text += `${labelLine}${items.map((item, i) => `${i + 1}. ${item}`).join('\n')}\n\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PSYCHOTROPIC MEDICATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Psychotropic Medications ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    ...
    const isModified = editedFields[editKey];
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
    const step = stepFor(val);
    const numVal = parseFloat(editValue); const safeNum = Number.isFinite(numVal) ? numVal : 0;
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(String(val)); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              ...
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (editValue === '' || !Number.isFinite(parseFloat(editValue))) { setSaveError('Please enter a valid number'); return; } handleSaveField(record, fn, idx, sid, null, parseFloat(editValue)); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; stageDraft(record, idx, `${fn}-${idx}`, currentArr, editKey, sid || sidForField(fn), false); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  /* ======= RENDER: OBJECT ARRAY FIELD (current, past, medicationChanges, allergiesAdverse) =======
     Canonical STACKED array-of-object: each item = a card whose header (.nested-subtitle) is the item's
     primary text (medication name / action+medication), then each sub-field a .nested-mini-card with a
     .nested-subtitle.sub-label ABOVE a value-only editable row. Single-name gate drops the top-level
     field label when it equals the section title (Current/Past/Medication Changes render directly). */
  const renderObjectArrayField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    const items = Array.isArray(val) ? val.filter(it => !isEmptyDeep(it)) : [];
    if (items.length === 0) return null;
    const label = FIELD_LABELS[fn] || fn;
    const subs = OBJECT_ARRAY_CONFIG[fn] || [];
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
                ...
                <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(item); setSaveError(null); } }}>
                  {isEditing ? (
                    <div className="edit-field-container">
                      ...
                      {saveError && <div className="save-error">{saveError}</div>}
                      <div className="edit-actions">
                        <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const currentArr = [...items]; currentArr[itemIdx] = editValue; stageDraft(record, idx, `${fn}-${idx}`, currentArr, editKey, sid || sidForField(fn), false); }}>{saving ? 'Saving...' : 'Save'}</button>
                        <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                      </div>
                    </div>
                  ) : (
          ...
          const primary = objPrimary(fn, item);
          return (
            <div key={itemIdx} className="nested-field-card">
              <div className="nested-subtitle">{highlightText(primary)}</div>
              {subs.map(([key, subLabel]) => {
                const rawVal = item[key];
                if (!hasVal(rawVal)) return null;
                const displayValue = Array.isArray(rawVal) ? rawVal.filter(Boolean).join(', ') : String(rawVal);
                    ...
                    <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayValue); setSaveError(null); } }}>
                      {isEditing ? (
                        <div className="edit-field-container">
                          ...
                          {saveError && <div className="save-error">{saveError}</div>}
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id3 = safeId(record); if (!id3) return; const newVal = Array.isArray(rawVal) ? editValue.split(',').map(s => s.trim()).filter(Boolean) : editValue; const newItems = items.map((it, i) => i === itemIdx ? { ...it, [key]: newVal } : it); stageDraft(record, idx, `${fn}-${idx}`, newItems, editKey, sid || sidForField(fn), false); }}>{saving ? 'Saving...' : 'Save'}</button>
                            <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                          </div>
                        </div>
                      ) : (
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const id4 = safeId(record); if (!id4) return; const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); stageDraft(record, idx, `${fn}-${idx}`, fullText, sentenceKey, sid || sidForField(fn), true); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => hasVal(getFieldValue(record, f, idx), f));
  ...
  if (!records || records.length === 0) {
    return (
      <div className="psychotropic-medications-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Psychotropic Medications</h2></div>
        <div className="empty-state">No psychotropic medication records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="psychotropic-medications-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Psychotropic Medications</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PsychotropicMedicationsDocumentPDFTemplate document={pdfData} />} fileName="Psychotropic_Medications.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search medications, indications, prescriber..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Psychotropic Medications ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'medication-info')}
            {renderSection(record, idx, 'prescription-details')}
            {renderSection(record, idx, 'indication-instructions')}
            {renderSection(record, idx, 'current-medications')}
            {renderSection(record, idx, 'past-medications')}
            {renderSection(record, idx, 'medication-changes')}
            {renderSection(record, idx, 'safety-allergies')}
          </div>
        ))}
      </div>
    </div>
...
export default PsychotropicMedicationsDocument;
