/**
 * FamilyHistory Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/family_history/:id/edit    — Edit a single field
 * PUT /api/edit/family_history/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'family-history-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'maternalHistory',
  'paternalHistory',
  'grandparentalHistory',
  'adoptionHistory',
  'ethnicityHealthRisks',
  'ageOfOnsetPatterns',
  'reproductiveHistory',
  'longevityPatterns',
  'siblingHistory',
  'familialCancerHistory',
  'cardiovascularFamilyHistory',
  'diabetesFamilyHistory',
  'psychiatricFamilyHistory',
  'neurologicalFamilyHistory',
  'geneticDisorders',
  'autoimmuneFamilyHistory',
  'endocrineFamilyHistory',
  'renalFamilyHistory',
  'pulmonaryFamilyHistory',
  'gastrointestinalFamilyHistory',
  'geneticTestingHistory',
  'consanguinity',
  'pedigreeAvailable',
  'createdAt', // editable record date (date picker)
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('family_history', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[FamilyHistory] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('family_history', { _id: objectId }, {
      $set: {
    ...
    console.error('[FamilyHistory] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
