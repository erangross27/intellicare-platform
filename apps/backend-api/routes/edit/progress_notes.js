...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'progress-notes-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['chiefComplaint', 'historyOfPresentIllness', 'vitalSigns', 'painScale', 'glasgowComaScale', 'physicalExamination', 'medicationChanges', 'currentMedications', 'allergiesAndReactions', 'laboratoryValues', 'imagingFindings', 'functionalStatus', 'respiratoryStatus', 'cardiovascularStatus', 'neurologicalStatus', 'woundAssessment', 'nutritionalStatus', 'fluidBalance', 'infectionStatus', 'dischargePlanning', 'patientEducation', 'interventionsPerformed', 'socialFactors', 'clinicalStability'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('progress_notes', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[progress_notes] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('progress_notes', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[progress_notes] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
