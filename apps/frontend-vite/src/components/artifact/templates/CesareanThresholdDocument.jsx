/**
 * CesareanThresholdDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: cesarean_threshold
 *
 * 6 Sections:
 *   1. maternal-factors: maternalAge, maternalBmi, maternalHemoglobin, maternalVitalSigns, maternalComorbidities
 *   2. obstetric-history: previousCesareans, priorVaginalDelivery, cephalopelvicDisproportion
 *   3. labor-progress: cervicalDilation, stationAtDelivery, laborDuration, bishopScore, failureToProgress
 *   4. fetal-assessment: fetalWeight, gestationalAge, fetalPresentation, fetalHeartRatePattern, neonatalDistress
 *   5. amniotic-cord: oligohydramnios, polyhydramnios, placentalLocation, umbilicalCordComplications
 *   6. anesthesia-induction: epiduralAnesthesia, chorioamnionitis, inductionAugmentation
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import CesareanThresholdDocumentPDFTemplate from '../pdf-templates/CesareanThresholdDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './CesareanThresholdDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'cesarean_thresholdPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'labor-progress': 'Labor Progress',
  'fetal-assessment': 'Fetal Assessment',
  'amniotic-cord': 'Amniotic & Cord',
  'anesthesia-induction': 'Anesthesia & Induction',
};

const FIELD_LABELS = {
  maternalAge: 'Maternal Age (years)',
  maternalBmi: 'Maternal BMI (kg/m²)',
  maternalHemoglobin: 'Maternal Hemoglobin (g/dL)',
  maternalVitalSigns: 'Maternal Vital Signs',
  maternalComorbidities: 'Maternal Comorbidities',
  previousCesareans: 'Previous Cesareans',
  priorVaginalDelivery: 'Prior Vaginal Delivery',
  cephalopelvicDisproportion: 'Cephalopelvic Disproportion',
  cervicalDilation: 'Cervical Dilation (cm)',
  stationAtDelivery: 'Station at Delivery',
  laborDuration: 'Labor Duration (hrs)',
  bishopScore: 'Bishop Score',
  failureToProgress: 'Failure to Progress',
  fetalWeight: 'Fetal Weight (g)',
  gestationalAge: 'Gestational Age (weeks)',
  fetalPresentation: 'Fetal Presentation',
  fetalHeartRatePattern: 'Fetal Heart Rate Pattern',
  neonatalDistress: 'Neonatal Distress',
  oligohydramnios: 'Oligohydramnios',
  polyhydramnios: 'Polyhydramnios',
  placentalLocation: 'Placental Location',
  umbilicalCordComplications: 'Umbilical Cord Complications',
  epiduralAnesthesia: 'Epidural Anesthesia',
  chorioamnionitis: 'Chorioamnionitis',
  inductionAugmentation: 'Induction / Augmentation',
};

const SECTION_FIELDS = {
  'maternal-factors': ['maternalAge', 'maternalBmi', 'maternalHemoglobin', 'maternalVitalSigns', 'maternalComorbidities'],
  'obstetric-history': ['previousCesareans', 'priorVaginalDelivery', 'cephalopelvicDisproportion'],
  'labor-progress': ['cervicalDilation', 'stationAtDelivery', 'laborDuration', 'bishopScore', 'failureToProgress'],
  'fetal-assessment': ['fetalWeight', 'gestationalAge', 'fetalPresentation', 'fetalHeartRatePattern', 'neonatalDistress'],
  'amniotic-cord': ['oligohydramnios', 'polyhydramnios', 'placentalLocation', 'umbilicalCordComplications'],
  'anesthesia-induction': ['epiduralAnesthesia', 'chorioamnionitis', 'inductionAugmentation'],
};

const NUMBER_FIELDS = ['maternalAge', 'gestationalAge', 'maternalBmi', 'laborDuration', 'cervicalDilation', 'fetalWeight', 'previousCesareans', 'stationAtDelivery', 'maternalHemoglobin', 'bishopScore'];
const ARRAY_FIELDS = ['maternalComorbidities', 'umbilicalCordComplications'];
const BOOLEAN_FIELDS = ['priorVaginalDelivery', 'cephalopelvicDisproportion', 'oligohydramnios', 'polyhydramnios', 'chorioamnionitis', 'epiduralAnesthesia', 'failureToProgress', 'neonatalDistress'];
const STRING_FIELDS = ['fetalPresentation', 'placentalLocation', 'fetalHeartRatePattern', 'maternalVitalSigns', 'inductionAugmentation'];
...
/* ═══════ COMPONENT ═══════ */
const CesareanThresholdDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.cesarean_threshold) return Array.isArray(r.cesarean_threshold) ? r.cesarean_threshold : [r.cesarean_threshold];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.cesarean_threshold) return Array.isArray(dd.cesarean_threshold) ? dd.cesarean_threshold : [dd.cesarean_threshold]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Cesarean Threshold Assessment ${idx + 1}`.toLowerCase();
  ...
  // Save = stage a DRAFT locally + localStorage (survives refresh). No DB write until Approve.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const suffix = `-${idx}`;
      // Commit only this section's staged fields (localEdits key = "field-idx", possibly "field.arrayIndex-idx")
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/cesarean_threshold/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/cesarean_threshold/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (toCommit.length > 0) {
        const store = readDrafts();
        if (store[id]) {
          toCommit.forEach(k => { const fp = k.slice(0, -suffix.length); const bf = fp.includes('.') ? fp.slice(0, fp.indexOf('.')) : fp; delete store[id][bf]; });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== CESAREAN THRESHOLD ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Cesarean Threshold Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="cesarean-threshold-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Cesarean Threshold Assessment</h2></div>
        <div className="empty-state">No cesarean threshold assessment data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="cesarean-threshold-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Cesarean Threshold Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<CesareanThresholdDocumentPDFTemplate document={pdfData} />} fileName={`cesarean-threshold-assessment-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search cesarean threshold assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Cesarean Threshold Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'maternal-factors')}
            {renderSection(record, idx, 'obstetric-history')}
            {renderSection(record, idx, 'labor-progress')}
            {renderSection(record, idx, 'fetal-assessment')}
            {renderSection(record, idx, 'amniotic-cord')}
            {renderSection(record, idx, 'anesthesia-induction')}
          </div>
        ))}
      </div>
    </div>
...
export default CesareanThresholdDocument;
