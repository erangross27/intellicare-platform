/**
 * FirstTrimesterScreenResultDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: first_trimester_screen_result
 *
 * 7 Sections:
 *   1. risk-assessment: trisomy21Risk(string), trisomy18Risk(string), trisomy13Risk(string), screenPositive(boolean)
 *   2. ultrasound-measurements: crownRumpLength(number), nuchalTranslucency(number), bipariatalDiameter(number), foetalHeartRate(number), nasalBonePresent(boolean)
 *   3. biochemistry-markers: pappALevel(number), pappAMom(number), freeBetaHcgLevel(number), freeBetaHcgMom(number)
 *   4. doppler-flow: ductusVenosusFlow(string), tricuspidRegurgitation(boolean)
 *   5. maternal-info: maternalAge(number), maternalWeight(number), ethnicity(string)
 *   6. pregnancy-details: gestationalAge(string), multipleFetuses(boolean), chorionicityType(string)
 *   7. recommendations: geneticCounsellingRecommended(boolean), diagnosticTestingOffered(boolean)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import FirstTrimesterScreenResultDocumentPDFTemplate from '../pdf-templates/FirstTrimesterScreenResultDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './FirstTrimesterScreenResultDocument.css';
...
const FIELD_LABELS = {
  ...
  bipariatalDiameter: 'Biparietal Diameter',
  foetalHeartRate: 'Foetal Heart Rate',
  nasalBonePresent: 'Nasal Bone Present',
  pappALevel: 'PAPP-A Level',
  pappAMom: 'PAPP-A MoM',
  freeBetaHcgLevel: 'Free Beta-hCG Level',
  freeBetaHcgMom: 'Free Beta-hCG MoM',
  ...
  multipleFetuses: 'Multiple Fetuses',
  chorionicityType: 'Chorionicity Type',
  geneticCounsellingRecommended: 'Genetic Counselling Recommended',
  diagnosticTestingOffered: 'Diagnostic Testing Offered',
};
...
/* 10 number fields — ALL use input[type=number step=any] + parseFloat+isNaN→block save */
const NUMBER_FIELDS = [
  'maternalAge', 'crownRumpLength', 'nuchalTranslucency', 'pappALevel', 'pappAMom',
  'freeBetaHcgLevel', 'freeBetaHcgMom', 'foetalHeartRate', 'bipariatalDiameter', 'maternalWeight',
];
...
const STRING_FIELDS = ['trisomy21Risk', 'trisomy18Risk', 'trisomy13Risk', 'ductusVenosusFlow', 'ethnicity', 'gestationalAge', 'chorionicityType'];
/* Ductus venosus flow → BlueSelect (Doppler flow categories) */
...
/* Multi-number measurement strings ("12w 0d") → per-number stepper, unit text literal */
const MEASURE_FIELDS = ['gestationalAge', 'trisomy21Risk', 'trisomy18Risk', 'trisomy13Risk'];
/* Numbers that read 0 across every populated record = extractor "not measured" sentinels → hide */
const ZERO_SENTINEL_FIELDS = ['pappALevel', 'freeBetaHcgLevel', 'foetalHeartRate', 'bipariatalDiameter', 'maternalWeight'];
const isZeroSentinel = (fn, v) => ZERO_SENTINEL_FIELDS.includes(fn) && Number(v) === 0;

/* tokenizeMeasure: split into segments, flagging DEPTH-0 numeric runs as editable (numbers inside parens stay literal) */
...
const DRAFT_KEY = 'first_trimester_screen_resultPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const FirstTrimesterScreenResultDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.first_trimester_screen_result) return Array.isArray(r.first_trimester_screen_result) ? r.first_trimester_screen_result : [r.first_trimester_screen_result];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.first_trimester_screen_result) return Array.isArray(dd.first_trimester_screen_result) ? dd.first_trimester_screen_result : [dd.first_trimester_screen_result]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `First Trimester Screen ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Staged edits for THIS record + section: localEdits keys are `${fieldPart}-${idx}`.
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/first_trimester_screen_result/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/first_trimester_screen_result/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      const val = getFieldValue(record, f, idx);
      if (!hasVal(val) || isZeroSentinel(f, val)) return;
      ...
      } else if (MEASURE_FIELDS.includes(f)) {
  ...
  const copyAllText = useCallback(async () => {
    let text = `First Trimester Screen Result\n${COPY_LINE_EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `First Trimester Screen Result ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  /* ═══════ Multi-number measure editor: one −/+ stepper per depth-0 number; unit/label text literal ═══════ */
        ...
        <div className={`num-stepper-row${multi ? ' multi-num' : ''}`}>
          {segs.map((sg, sIdx) => sg.isNum ? (
            <React.Fragment key={sIdx}>
              <button type="button" className="num-step" onClick={e => { e.stopPropagation(); const t = tokenizeMeasure(editValue); const cur = parseFloat(t[sIdx]?.text); if (t[sIdx]) t[sIdx] = { ...t[sIdx], text: String(Math.max(0, (isNaN(cur) ? 0 : cur) - 1)) }; setEditValue(t.map(x => x.text).join('')); }}>&minus;</button>
              <input type="text" inputMode="decimal" className="edit-number" value={sg.text} autoFocus={sIdx === firstNum} onChange={e => { const t = tokenizeMeasure(editValue); if (t[sIdx]) t[sIdx] = { ...t[sIdx], text: e.target.value }; setEditValue(t.map(x => x.text).join('')); }} onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
              <button type="button" className="num-step" onClick={e => { e.stopPropagation(); const t = tokenizeMeasure(editValue); const cur = parseFloat(t[sIdx]?.text); if (t[sIdx]) t[sIdx] = { ...t[sIdx], text: String((isNaN(cur) ? 0 : cur) + 1) }; setEditValue(t.map(x => x.text).join('')); }}>+</button>
            </React.Fragment>
  ...
  /* ═══════ RENDER: MEASURE FIELD (multi-number, e.g. "12w 0d") ═══════ */
  const renderMeasureField = (record, fn, idx, sid) => {
    ...
    // No editable number (e.g. a category value like "Low"/"High") → plain text editor
    if (!tokenizeMeasure(displayVal).some(s => s.isNum)) return renderStringField(record, fn, idx, sid, SECTION_TITLES[sid]);
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayVal); setSaveError(null); } }}>
          {isEditing ? renderMeasureEditor(() => handleSaveField(record, fn, idx, sid, null, editValue)) : (
            <>
  ...
  /* ═══════ RENDER: NUMBER FIELD — −/+ stepper + zero-sentinel hide ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="first-trimester-screen-result-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">First Trimester Screen Result</h2></div>
        <div className="empty-state">No first trimester screen result records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="first-trimester-screen-result-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">First Trimester Screen Result</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FirstTrimesterScreenResultDocumentPDFTemplate document={pdfData} />} fileName="First_Trimester_Screen_Result.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search first trimester screen results..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">First Trimester Screen Result {idx + 1}</h3>
            </div>
            {renderSection(record, idx, 'risk-assessment')}
            {renderSection(record, idx, 'ultrasound-measurements')}
            {renderSection(record, idx, 'biochemistry-markers')}
            {renderSection(record, idx, 'doppler-flow')}
...
export default FirstTrimesterScreenResultDocument;
