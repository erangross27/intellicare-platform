/**
 * EstimatedBloodLossDocumentPDFTemplate.jsx
 * Box-free B&W LETTER PDF — mirrors EstimatedBloodLossDocument.jsx sections/labels exactly
 * (4-AREA RULE: JSX = Copy Section = Copy All = PDF). Collection: estimated_blood_loss.
 * react-pdf: wrap is BOOLEAN only; each section title rides inside its first field's wrap=false View
 * so it never orphans at a page bottom. Title/section/label each get a borderBottom underline (no boxes).
 */
...
/* renderSentenceSection: parseLabel + comma-split for text fields (mirrors JSX formatSentenceFieldLines) */
...
/* renderStringArraySection: simple array of strings (e.g. bloodProductsGiven) */
const renderStringArraySection = (title, arr) => {
  if (!Array.isArray(arr) || arr.filter(Boolean).length === 0) return null;
  const rows = []; let n = 1;
...
/* ═══ COMPONENT ═══ */
const EstimatedBloodLossDocumentPDFTemplate = ({ document: templateData }) => {
    ...
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <View style={styles.documentHeader}><Text style={styles.documentTitle}>Estimated Blood Loss</Text></View>
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => {
          ...
          const hasBloodLoss = hasVal(record.amount) || hasVal(record.transfusionRequired) ||
            (Array.isArray(record.bloodProductsGiven) && record.bloodProductsGiven.filter(Boolean).length > 0);

          return (
            <View key={idx} style={styles.recordContainer} break={idx > 0}>
              {/* Record Header */}
              <View style={styles.recordHeader} wrap={false}>
                <Text style={styles.recordTitle}>{`Estimated Blood Loss ${idx + 1}`}</Text>
              </View>
              ...
              {hasBloodLoss && (
                <View style={styles.section}>
                  <View style={styles.fieldBox} wrap={false}>
                    <Text style={styles.sectionTitle}>Blood Loss Details</Text>
                    {hasVal(record.amount) && (
                      <View style={{ marginBottom: 4 }}>
                        <Text style={styles.fieldLabel}>Estimated Blood Loss</Text>
                        <Text style={styles.fieldValue}>{safeString(fmtVal(record.amount))}</Text>
                      </View>
                    )}
                    {hasVal(record.transfusionRequired) && (
                      <View style={{ marginBottom: 4 }}>
                        <Text style={styles.fieldLabel}>Transfusion Required</Text>
                        <Text style={styles.fieldValue}>{fmtVal(record.transfusionRequired)}</Text>
                      </View>
                    )}
                  </View>
                  {renderStringArraySection('Blood Products Given', record.bloodProductsGiven)}
                </View>
              )}
...
export default EstimatedBloodLossDocumentPDFTemplate;
