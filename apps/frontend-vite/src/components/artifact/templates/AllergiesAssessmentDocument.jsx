...
const SECTION_TITLES = { details: 'Allergy Details', notes: 'Notes & Follow-up' };
const FIELD_LABELS = {
  allergen: 'Allergen', reaction: 'Reaction', severity: 'Severity', type: 'Type',
  status: 'Status', dateIdentified: 'Date Identified', management: 'Management',
  compliance: 'Compliance', notes: 'Notes',
};
const SECTION_FIELDS = {
  details: ['allergen', 'reaction', 'severity', 'type', 'status', 'dateIdentified'],
  notes: ['notes'],
};
const DATE_FIELDS = new Set(['dateIdentified']);
const BOOLEAN_FIELDS = new Set();
const NUMBER_FIELDS = new Set();
const HIDE_ZERO_FIELDS = new Set();
const ARRAY_FIELDS = new Set();
const COMMA_ARRAY_FIELDS = [];
const COMMA_STRING_FIELDS = new Set(['reaction']);
const SENTENCE_STRING_FIELDS = new Set(['allergen', 'reaction', 'severity', 'notes']);
const ENUM_OPTIONS = {
...
const unwrap = source => {
  ...
  rows = rows.flatMap(row => {
    ...
    if (row?.allergies_assessments) return Array.isArray(row.allergies_assessments) ? row.allergies_assessments : [row.allergies_assessments];
    if (row?.documentData) {
      const nested = row.documentData;
      if (Array.isArray(nested)) return nested;
      if (nested?.allergies_assessments) return Array.isArray(nested.allergies_assessments) ? nested.allergies_assessments : [nested.allergies_assessments];
      return [nested];
    }
    return [row];
  });
...
const AllergiesAssessmentDocument = ({ document }) => {
  ...
  const approve = useCallback(async (record, section, index) => {
    ...
    try {
      ...
      for (const [path, value] of Object.entries(drafts)) { if (!SECTION_FIELDS[section].includes(path.split('.')[0])) continue; const response = await secureApiClient.put(`/api/edit/allergies_assessments/${id}/edit`, { field: path, value }); if (response?.success === false) throw new Error(response.error || 'Save failed'); committed.push(path); }
      await secureApiClient.put(`/api/edit/allergies_assessments/${id}/approve`, { sectionId: section, approved: true });
