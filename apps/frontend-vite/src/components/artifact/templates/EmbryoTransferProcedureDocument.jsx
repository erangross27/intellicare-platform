/**
 * EmbryoTransferProcedureDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: embryo_transfer_procedure
 *
 * 8 Sections:
 *   1. transfer-info: transferDate, embryologistName, numberOfEmbryosTransferred
 *   2. embryo-assessment: embryoDevelopmentStage, gardnerGradingScore, embryoQualityGrade, innerCellMassGrade, trophectodermGrade
 *   3. transfer-technique: catheterType, transferTechniqueType, ultrasoundGuidanceUsed, catheterLoadingTechnique, trialTransferPerformed
 *   4. endometrial-assessment: endometrialThicknessMm, endometrialPattern, embryoPlacementDistanceFromFundusCm
 *   5. transfer-quality: transferDifficultyScore, cervicalMucusRemoved, retainedEmbryosOnCatheter, bloodOnCatheterTip, mucusOnCatheterTip
 *   6. genetic-testing: preimplantationGeneticTestingPerformed, pgtResult, assistedHatchingPerformed
 *   7. hormonal-support: embryoCryopreservationMethod, progesteroneSupportProtocol, estrogenLevelOnTransferDayPgMl, progesteroneLevelOnTransferDayNgMl
 *   8. follow-up: betaHcgTestDate, procedureComplications
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import EmbryoTransferProcedureDocumentPDFTemplate from '../pdf-templates/EmbryoTransferProcedureDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './EmbryoTransferProcedureDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'embryo_transfer_procedurePendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'embryo-assessment': 'Embryo Assessment',
  'transfer-technique': 'Transfer Technique',
  'endometrial-assessment': 'Endometrial Assessment',
  'transfer-quality': 'Transfer Quality',
...
const FIELD_LABELS = {
  ...
  embryologistName: 'Embryologist Name',
  numberOfEmbryosTransferred: 'Number of Embryos Transferred',
  embryoDevelopmentStage: 'Embryo Development Stage',
  gardnerGradingScore: 'Gardner Grading Score',
  ...
  transferTechniqueType: 'Transfer Technique Type',
  ultrasoundGuidanceUsed: 'Ultrasound Guidance Used',
  catheterLoadingTechnique: 'Catheter Loading Technique',
  trialTransferPerformed: 'Trial Transfer Performed',
  ...
  transferDifficultyScore: 'Transfer Difficulty Score',
  cervicalMucusRemoved: 'Cervical Mucus Removed',
  retainedEmbryosOnCatheter: 'Retained Embryos on Catheter',
  bloodOnCatheterTip: 'Blood on Catheter Tip',
  mucusOnCatheterTip: 'Mucus on Catheter Tip',
  preimplantationGeneticTestingPerformed: 'Preimplantation Genetic Testing Performed',
  pgtResult: 'PGT Result',
  assistedHatchingPerformed: 'Assisted Hatching Performed',
  embryoCryopreservationMethod: 'Embryo Cryopreservation Method',
  progesteroneSupportProtocol: 'Progesterone Support Protocol',
  estrogenLevelOnTransferDayPgMl: 'Estrogen Level on Transfer Day (pg/mL)',
  progesteroneLevelOnTransferDayNgMl: 'Progesterone Level on Transfer Day (ng/mL)',
  betaHcgTestDate: 'Beta hCG Test Date',
  procedureComplications: 'Procedure Complications',
};

const SECTION_FIELDS = {
  'transfer-info': ['transferDate', 'embryologistName', 'numberOfEmbryosTransferred'],
  'embryo-assessment': ['embryoDevelopmentStage', 'gardnerGradingScore', 'embryoQualityGrade', 'innerCellMassGrade', 'trophectodermGrade'],
  'transfer-technique': ['catheterType', 'transferTechniqueType', 'ultrasoundGuidanceUsed', 'catheterLoadingTechnique', 'trialTransferPerformed'],
  'endometrial-assessment': ['endometrialThicknessMm', 'endometrialPattern', 'embryoPlacementDistanceFromFundusCm'],
  'transfer-quality': ['transferDifficultyScore', 'cervicalMucusRemoved', 'retainedEmbryosOnCatheter', 'bloodOnCatheterTip', 'mucusOnCatheterTip'],
  'genetic-testing': ['preimplantationGeneticTestingPerformed', 'pgtResult', 'assistedHatchingPerformed'],
  'hormonal-support': ['embryoCryopreservationMethod', 'progesteroneSupportProtocol', 'estrogenLevelOnTransferDayPgMl', 'progesteroneLevelOnTransferDayNgMl'],
  'follow-up': ['betaHcgTestDate', 'procedureComplications'],
};
...
/* ENUMS (act like a REI embryologist — ASRM/Gardner catalog). Stored lowercase ('vitrification',
   'frozen embryo transfer (fet)') → enumCanonical Title-Cases the display+save; an off-scale
   descriptive value (e.g. embryoDevelopmentStage 'blastocyst stage (day 5)') is kept via
   enumOptionsWith so nothing is lost while still offering the standard picklist. */
const ENUM_OPTIONS = {
...
/* HIDE-ZERO: the batch extractor writes 0 into numerics it could not read (sentinel). A stored 0 in a
   lab/measurement field (endometrial thickness, placement distance, estrogen/progesterone level) is
   clinically impossible → hide it. MEANINGFUL_ZERO_FIELDS = counts where 0 is a REAL result. */
...
/* ═══════ COMPONENT ═══════ */
const EmbryoTransferProcedureDocument = ({ document: docProp }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // fieldPart is "field" (object/dot-path) or "field.arrayIndex" (numeric trailing segment).
  ...
  /* hide-zero: a numeric 0 is an extractor sentinel unless it's a meaningful-zero field or doctor-edited. */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Embryo Transfer Procedure ${idx + 1}`.toLowerCase();
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageDraft(fullText, (n) => { n[`${fn}-${idx}-s${sentenceIdx}`] = 'edited'; return n; });
      return;
    }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Commit this record's staged drafts whose base field belongs to this section.
      const store = readDrafts();
      const recDrafts = store[id] || {};
      const committedLocalKeys = [];
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        // arrayIndex ONLY when the segment after the LAST dot is purely numeric (reverses save convention).
        const lastDot = fieldPart.lastIndexOf('.');
        const isArrayElem = lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1));
        const baseField = isArrayElem ? fieldPart.slice(0, lastDot) : fieldPart;
        ...
        const resp = await secureApiClient.put(`/api/edit/embryo_transfer_procedure/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedLocalKeys.push(`${baseField}-${idx}`);
        delete recDrafts[fieldPart];
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/embryo_transfer_procedure/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; committedLocalKeys.forEach(k => delete n[k]); return n; });
      // Persist the trimmed draft store (or remove the record entry if now empty)
  ...
  // Mirror the JSX renderStringField: LABELED sentence → sub-label (DASH, colon dropped) + ≥2 comma rows;
  // UNLABELED sentence → ONE whole row (no comma-split, so "Dr. X, MD (…)" stays intact).
  ...
  // Canonical: EQ under section title, DASH under EVERY field label, values numbered ('1.' even singles),
  // enum canonical display, hide-zero mirror, NO side-by-side "Label: value".
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (NUMBER_FIELDS.includes(f) && !numericShows(f, val, idx)) return; // hide-zero
      text += `${label}\n${'-'.repeat(40)}\n`;
      if (DATE_FIELDS.includes(f)) {
        ...
        items.forEach((item, i) => { text += `${i + 1}. ${typeof item === 'object' ? JSON.stringify(item) : item}\n`; });
    ...
    return text;
  }, [getFieldValue, hasVal, fmtVal, numericShows, formatSentenceFieldLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== EMBRYO TRANSFER PROCEDURE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Embryo Transfer Procedure ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD — −/+ stepper, validate on save ═══════ */
  const renderNumberField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    if (!numericShows(fn, val, idx)) return null; // hide-zero: extractor sentinel unless meaningful/edited
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
    const label = FIELD_LABELS[fn] || fn;
    const showLabel = label.toLowerCase() !== (SECTION_TITLES[sid] || '').toLowerCase();
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* Sentence path (parseLabel → nested-subtitle for "Label: value") */
    ...
    /* Single-value string: simple editable with saveSentence */
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      if (NUMBER_FIELDS.includes(f)) return numericShows(f, val, idx); // hide-zero: sentinel 0 doesn't count
      return hasVal(val);
    });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="embryo-transfer-procedure-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Embryo Transfer Procedure</h2></div>
        <div className="empty-state">No embryo transfer procedure records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="embryo-transfer-procedure-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Embryo Transfer Procedure</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<EmbryoTransferProcedureDocumentPDFTemplate document={pdfData} />} fileName="Embryo_Transfer_Procedure.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search embryo transfer procedure..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Embryo Transfer Procedure ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'transfer-info')}
            {renderSection(record, idx, 'embryo-assessment')}
            {renderSection(record, idx, 'transfer-technique')}
            {renderSection(record, idx, 'endometrial-assessment')}
            {renderSection(record, idx, 'transfer-quality')}
            {renderSection(record, idx, 'genetic-testing')}
            {renderSection(record, idx, 'hormonal-support')}
...
export default EmbryoTransferProcedureDocument;
