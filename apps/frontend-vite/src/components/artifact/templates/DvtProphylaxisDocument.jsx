/**
 * DvtProphylaxisDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: dvt_prophylaxis
 *
 * 7 Sections:
 *   1. session-info: date (date picker), provider, facility, status
 *   2. medication-details: medication, dose, duration
 *   3. mechanical-prophylaxis: mechanicalProphylaxis[]
 *   4. clinical-findings: findings (sentence), assessment (sentence)
 *   5. plan-section: plan (sentence)
 *   6. recommendations-section: recommendations[]
 *   7. notes-section: notes (sentence)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import DvtProphylaxisDocumentPDFTemplate from '../pdf-templates/DvtProphylaxisDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './DvtProphylaxisDocument.css';
...
const FIELD_LABELS = {
  ...
  dose: 'Dose',
  duration: 'Duration',
  mechanicalProphylaxis: 'Mechanical Prophylaxis',
  findings: 'Findings',
...
const SECTION_FIELDS = {
  ...
  'medication-details': ['medication', 'dose', 'duration'],
  'mechanical-prophylaxis': ['mechanicalProphylaxis'],
...
const ARRAY_FIELDS = ['mechanicalProphylaxis', 'recommendations'];
const DATE_FIELDS = ['date'];
const RESULTS_FIELD = 'results';

/* Fixed-choice fields → BlueSelect (RTL-safe). status is a known record-state enum; empty in Angela's record
   but 'active' elsewhere in the collection. enumOptionsWith keeps the stored value so nothing is rewritten. */
const ENUM_OPTIONS = {
  status: ['Active', 'Completed', 'Not Active'],
};
const ENUM_FIELDS = Object.keys(ENUM_OPTIONS);
/* enumCanonical: normalize a stored value to its canonical-cased option ('active' → 'Active'). */
...
/* Number+unit measurement fields → −/+ stepper editing the number with the unit kept as a chip.
   dose "40mg SQ" → num 40 + unit "mg SQ". Off-shape values (e.g. "40 mg SC q24h" — digit in the unit,
   ranges, ratios) return null from splitNumberUnit and fall back to plain text editing. */
const MEASURE_FIELDS = ['dose'];
...
/* formatKey: convert a dynamic object key into a readable label */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits field name, e.g. "plan") */
const DRAFT_KEY = 'dvt_prophylaxisPendingEdits';
const readDrafts = () => {
...
/* ======= COMPONENT ======= */
const DvtProphylaxisDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `DVT Prophylaxis ${idx + 1}`.toLowerCase();
  ...
  // Save = stage a DRAFT locally (full array under `${fn}-${idx}`) + persist to localStorage. NO DB write.
  const handleSaveArrayItem = useCallback((record, fn, idx, arrIdx, value) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    const fieldKey = `${fn}-${idx}`;
    const editKey = `${fn}.${arrIdx}-${idx}`;
    const newArr = [...(localEdits[fieldKey] || record[fn] || [])];
    newArr[arrIdx] = value;
    setLocalEdits(prev => ({ ...prev, [fieldKey]: newArr }));
  ...
  // Save one sentence = stage a DRAFT (full text under `${fn}-${idx}`) + persist to localStorage. NO DB write.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // staged drafts for this record live in localEdits under "<fieldPart><suffix>" with pendingEdits[key] set
      ...
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // localEdits stores full field value (string or array) under field name
        ...
        const resp = await secureApiClient.put(`/api/edit/dvt_prophylaxis/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/dvt_prophylaxis/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[DvtProphylaxis] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const formatSentenceFieldLines = useCallback((text) => {
    ...
    const lines = []; let n = 1; let first = true;
    sentences.forEach(s => {
      const parsed = parseLabel(s);
      if (parsed.isLabeled) {
        // labeled clause → sub-label + DASH divider + numbering RESTART
        if (!first) lines.push('');
        ...
        // unlabeled sentence → comma-split into rows when it's a list (>=3), else one row (running count)
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      // single-name rule: field label == section title → hide the label (+ its divider)
      ...
      if (f === RESULTS_FIELD) {
        ...
        // each dynamic key carries its OWN label — always show it (+ DASH + numbered value)
        ...
        const items = val.map(item => (typeof item === 'object' && item.recommendation) ? item.recommendation : String(item)).filter(Boolean);
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== DVT PROPHYLAXIS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `DVT Prophylaxis ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  /* ======= RENDER: NUMBER+UNIT MEASUREMENT FIELD (−/+ stepper on the number, unit chip) ======= */
  const renderMeasureField = (record, fn, idx, sid, title) => {
    ...
    const isModified = editedFields[editKey];
    const step = stepFor(nu.num);
    const bump = (d) => setEditValue(v => Math.max(0, (parseFloat(v || '0') || 0) + d * step).toString());
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
  ...
  /* ======= RENDER: ARRAY FIELD (mechanicalProphylaxis, recommendations) ======= */
  const renderArrayField = (record, fn, idx, sid) => {
    ...
    return arr.map((item, arrIdx) => {
      const itemText = typeof item === 'object' ? (item.recommendation || String(item)) : String(item);
      if (!itemText) return null;

      if (searchTerm.trim() && !phraseMatch) {
        const phrase = searchTerm.toLowerCase().trim();
        if (!itemText.toLowerCase().includes(phrase) && !label.toLowerCase().includes(phrase)) return null;
      }
      ...
      <div key={fn}>
        <div className="rec-mini-card">
          ...
          {sentences.map((sentence, sIdx) => {
            ...
            /* Unlabeled sentence that is really a comma LIST (>=3 items) → split into flat editable rows */
            if (!parsed.isLabeled) {
                          ...
                          <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                            {ciEditing ? (
                              <div className="edit-field-container">
                                ...
                                {saveError && <div className="save-error">{saveError}</div>}
                                <div className="edit-actions">
                                  <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const currentVal2 = String(getFieldValue(record, fn, idx) || ''); const sentences2 = splitBySentence(currentVal2); const items2 = splitByComma(sentences2[sIdx] || ''); const trimmed = editValue.trim(); if (!trimmed) items2.splice(ciIdx, 1); else items2[ciIdx] = trimmed; sentences2[sIdx] = items2.join(', '); const fullText2 = reconstructFullText(sentences2); setSaveError(null); stageSentenceDraft(record, fn, idx, fullText2); setEditedSentences(prev => ({ ...prev, [commaKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                                  <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                                </div>
                              </div>
                            ) : (
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      if (f === RESULTS_FIELD) return val && typeof val === 'object' && !Array.isArray(val) && Object.keys(val).length > 0;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="dvt-prophylaxis-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">DVT Prophylaxis</h2></div>
        <div className="empty-state">No DVT prophylaxis records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="dvt-prophylaxis-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">DVT Prophylaxis</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<DvtProphylaxisDocumentPDFTemplate document={pdfData} />} fileName="DVT_Prophylaxis.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search DVT prophylaxis..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`DVT Prophylaxis ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'medication-details')}
            {renderSection(record, idx, 'mechanical-prophylaxis')}
...
export default DvtProphylaxisDocument;
