/**
 * FertilityTrackingDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: fertility_tracking
 *
 * 5 Sections:
 *   1. cycle-tracking: menstrualCycleLength, basalBodyTemperature, ovulationPredictorKitResult
 *   2. hormone-levels: luteinizingHormoneLevel, follicleStimulatingHormoneLevel, estrogenLevel, progesteroneLevel, anitalMullerianhormoneLevel, thyroidStimulatingHormone, prolactinLevel, testosteroneLevel, insulinResistanceIndex
 *   3. cervical-assessment: cervicalMucusConsistency, cervicalPosition, cervicalFirmness, cervicalOpening
 *   4. ovarian-uterine: antalFollicleCount, endometrialThickness, dominantFollicleSize
 *   5. sperm-parameters: spermConcentration, spermMotility, spermMorphology, intercourseFrequency
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import FertilityTrackingDocumentPDFTemplate from '../pdf-templates/FertilityTrackingDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './FertilityTrackingDocument.css';
...
const SECTION_TITLES = {
  ...
  'ovarian-uterine': 'Ovarian & Uterine',
...
const SECTION_FIELDS = {
  'cycle-tracking': ['menstrualCycleLength', 'basalBodyTemperature', 'ovulationPredictorKitResult'],
  'hormone-levels': ['luteinizingHormoneLevel', 'follicleStimulatingHormoneLevel', 'estrogenLevel', 'progesteroneLevel', 'anitalMullerianhormoneLevel', 'thyroidStimulatingHormone', 'prolactinLevel', 'testosteroneLevel', 'insulinResistanceIndex'],
  'cervical-assessment': ['cervicalMucusConsistency', 'cervicalPosition', 'cervicalFirmness', 'cervicalOpening'],
  'ovarian-uterine': ['antalFollicleCount', 'endometrialThickness', 'dominantFollicleSize'],
  'sperm-parameters': ['spermConcentration', 'spermMotility', 'spermMorphology', 'intercourseFrequency'],
};

const BOOLEAN_FIELDS = ['ovulationPredictorKitResult'];
const NUMBER_FIELDS = ['basalBodyTemperature', 'menstrualCycleLength', 'luteinizingHormoneLevel', 'follicleStimulatingHormoneLevel', 'estrogenLevel', 'progesteroneLevel', 'anitalMullerianhormoneLevel', 'antalFollicleCount', 'endometrialThickness', 'dominantFollicleSize', 'intercourseFrequency', 'spermConcentration', 'spermMotility', 'spermMorphology', 'thyroidStimulatingHormone', 'prolactinLevel', 'insulinResistanceIndex', 'testosteroneLevel'];
const STRING_FIELDS = ['cervicalMucusConsistency', 'cervicalPosition', 'cervicalFirmness', 'cervicalOpening'];

/* Zero = extractor sentinel / not-documented — hidden in JSX + Copy + PDF.
   In-record proof: this record carries 11 real measured values (cycle length 28, LH 6.8, FSH 14.2,
   estrogen 52, progesterone 0.8, AMH 0.3, AFC 3, endometrial 4.2, TSH 2.1, prolactin 12.4, testosterone 28),
   yet basalBodyTemperature/dominantFollicleSize/insulinResistanceIndex read 0 — measured quantities that are
   never truly 0 → extractor sentinels. Sperm metrics 0 = male-partner analysis not part of this female tracking
   record. intercourseFrequency 0 = not documented (all self-tracked fields — cervical*, BBT — are blank here). */
const ZERO_SENTINEL_FIELDS = [
  'basalBodyTemperature', 'dominantFollicleSize', 'insulinResistanceIndex', 'intercourseFrequency',
  'spermConcentration', 'spermMotility', 'spermMorphology',
];
...
const DRAFT_KEY = 'fertilityTrackingPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const FertilityTrackingDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Fertility Tracking ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Stage one field's value as a DRAFT in the pending-drafts localStorage store (survives refresh).
  // sid (when provided) clears the approved flag so a re-edit returns the button to yellow Pending.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Staged edits for THIS record+section: editKey = "field-idx" (no dotted/arrayIndex fields here)
      ...
      // Persist each staged field to the DB now (field, plus arrayIndex only for a numeric dot-suffix)
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/fertility_tracking/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/fertility_tracking/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[FertilityTracking] Approve error:', err); setSaveError('Save failed. Please try again.'); }
  ...
  // UNLABELED sentence → ONE whole numbered row (no comma-split, so a value stays intact).
  ...
  // single-name gate), values numbered ('1.' even singles), zero-sentinel mirror, empty section dropped.
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== FERTILITY TRACKING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Fertility Tracking ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    ...
    if (ZERO_SENTINEL_FIELDS.includes(fn) && Number(val) === 0) return null; // hide extractor-sentinel / not-documented 0
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
    const label = FIELD_LABELS[fn] || fn;
    const showLabel = label.toLowerCase() !== (SECTION_TITLES[sid] || '').toLowerCase();
  ...
  const renderSection = (record, idx, sid) => {
    ...
    // every field is sentinel/empty (Cervical Assessment / Sperm Parameters here) drops entirely.
  ...
  if (!records || records.length === 0) {
    return (
      <div className="fertility-tracking-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Fertility Tracking</h2></div>
        <div className="empty-state">No fertility tracking records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="fertility-tracking-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Fertility Tracking</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FertilityTrackingDocumentPDFTemplate document={pdfData} />} fileName="Fertility_Tracking.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search fertility tracking..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Fertility Tracking ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'cycle-tracking')}
            {renderSection(record, idx, 'hormone-levels')}
            {renderSection(record, idx, 'cervical-assessment')}
            {renderSection(record, idx, 'ovarian-uterine')}
            {renderSection(record, idx, 'sperm-parameters')}
          </div>
        ))}
      </div>
    </div>
...
export default FertilityTrackingDocument;
