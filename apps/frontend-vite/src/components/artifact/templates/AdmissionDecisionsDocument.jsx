/**
 * AdmissionDecisionsDocument.jsx
 * June 2026 — Collection: admission_decisions (NEW template)
 *
 * Donor cloned (generic date+object+array+string shape):
 *  - PointOfCareUltrasoundHeartRateDocument: date header renderDateField; OBJECT recursive render;
 *    ARRAY date-grouped recommendations; STRING per-sentence; 4-level search; per-section approve;
 *    Copy row/Section/All; pdfData memo; blue-glow theme.
 *
 * NOTE: this collection has BOTH a singular STRING field `recommendation` AND a separate ARRAY
 * field `recommendations` (array of {recommendation, date}) — they are kept distinct.
 *
 * FIELDS (14 → 100% schema coverage):
 *   DATE:   date
 *   STRING: recommendation, patientPreference, alternativePlan, type, provider, facility,
...
import AdmissionDecisionsDocumentPDFTemplate from '../pdf-templates/AdmissionDecisionsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './AdmissionDecisionsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'admission_decisionsPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  'decision': ['recommendation', 'patientPreference', 'alternativePlan'],
...
const SECTION_ORDER = ['decision', 'clinical', 'results-section', 'recommendations-section', 'notes-status'];

const DATE_FIELDS = ['date'];
/* singular STRING `recommendation` is distinct from the ARRAY `recommendations` */
const STRING_FIELDS = ['recommendation', 'patientPreference', 'alternativePlan', 'type', 'provider', 'facility', 'findings', 'assessment', 'plan', 'notes', 'status'];
...
const KEY_OVERRIDES = {
  ed: 'ED', icu: 'ICU', los: 'LOS', ews: 'EWS', dnr: 'DNR', poc: 'POC',
};
...
/* ═══════ COMPONENT ═══════ */
const AdmissionDecisionsDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.admission_decisions) return Array.isArray(r.admission_decisions) ? r.admission_decisions : [r.admission_decisions];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.admission_decisions) return Array.isArray(dd.admission_decisions) ? dd.admission_decisions : [dd.admission_decisions]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `admission decision ${idx + 1}`.toLowerCase();
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.ews) — value stays a STRING */
  // Stage a DRAFT for a nested OBJECT leaf locally (NO DB write). The whole root object is staged under
  // `${rootField}-${idx}`; Approve commits it. localStorage keeps it across refresh.
  ...
  // Stage a single edited/deleted sentence as a DRAFT (NO DB write). Whole field text staged under
  // `${fn}-${idx}`; Approve commits it. localStorage keeps it across refresh.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Staged edits for THIS record's THIS section (localEdits key = "field-idx"; field has no dots here)
    ...
    try {
      // Persist each staged field to the DB now (field, or field+arrayIndex when the
      // trailing dot-segment is purely numeric — none are in this template)
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/admission_decisions/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/admission_decisions/${id}/approve`, { sectionId: sid, approved: true });
      ...
      console.error('[AdmissionDecisions] Approve error:', err);
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== ADMISSION DECISIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Admission Decision ${idx + 1}\n${'='.repeat(40)}\n\n`;
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              // Stage as a DRAFT (NO DB write); Approve commits it. localStorage survives refresh.
                              const editKey = `${fn}-${idx}`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="admission-decisions-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Admission Decision</h2></div>
        <div className="empty-state">No admission decision records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="admission-decisions-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Admission Decision</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<AdmissionDecisionsDocumentPDFTemplate document={pdfData} />} fileName={`admission-decisions-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search admission decision records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Admission Decision ${idx + 1}`)}</h3>
            </div>
...
export default AdmissionDecisionsDocument;
