/**
 * HeightMeasurementsDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: height_measurements
 *
 * 5 Sections:
 *   1. primary-measurement: heightCentimeters, heightFeetInches, measurementMethod, patientPosition
 *   2. surrogate-measures: armSpanMeasurement, kneeHeightMeasurement, crownHeelLength
 *   3. pediatric-growth: heightPercentile, heightVelocity, heightZScore, midParentalHeight, growthChartReference, bodyMassIndexPercentile
 *   4. bmi: bodyMassIndex
 *   5. quality-conditions: shoesRemoved, estimatedHeight, kyphoscoliosis, instrumentCalibrated, patientCooperative, spinalDeformity, measurementAccuracy, heightLossFromBaseline
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HeightMeasurementsDocumentPDFTemplate from '../pdf-templates/HeightMeasurementsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './HeightMeasurementsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'height_measurementsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'primary-measurement': 'Primary Measurement',
  'surrogate-measures': 'Surrogate Measures',
  'pediatric-growth': 'Pediatric Growth',
  'bmi': 'Body Mass Index',
...
const FIELD_LABELS = {
  ...
  heightFeetInches: 'Height (ft-in)',
  measurementMethod: 'Measurement Method',
  patientPosition: 'Patient Position',
  armSpanMeasurement: 'Arm Span (cm)',
  kneeHeightMeasurement: 'Knee Height (cm)',
  crownHeelLength: 'Crown-Heel Length (cm)',
  heightPercentile: 'Height Percentile',
  heightVelocity: 'Height Velocity (cm/yr)',
  heightZScore: 'Height Z-Score',
  midParentalHeight: 'Mid-Parental Height (cm)',
  growthChartReference: 'Growth Chart Reference',
  ...
  instrumentCalibrated: 'Instrument Calibrated',
  patientCooperative: 'Patient Cooperative',
  spinalDeformity: 'Spinal Deformity',
  measurementAccuracy: 'Measurement Accuracy',
  heightLossFromBaseline: 'Height Loss from Baseline (cm)',
};

const SECTION_FIELDS = {
  'primary-measurement': ['heightCentimeters', 'heightFeetInches', 'measurementMethod', 'patientPosition'],
  'surrogate-measures': ['armSpanMeasurement', 'kneeHeightMeasurement', 'crownHeelLength'],
  'pediatric-growth': ['heightPercentile', 'heightVelocity', 'heightZScore', 'midParentalHeight', 'growthChartReference', 'bodyMassIndexPercentile'],
  'bmi': ['bodyMassIndex'],
  'quality-conditions': ['measuredBy', 'shoesRemoved', 'estimatedHeight', 'kyphoscoliosis', 'instrumentCalibrated', 'patientCooperative', 'spinalDeformity', 'measurementAccuracy', 'heightLossFromBaseline'],
};

const NUMBER_FIELDS = ['heightCentimeters', 'armSpanMeasurement', 'kneeHeightMeasurement', 'heightPercentile', 'heightVelocity', 'heightZScore', 'crownHeelLength', 'midParentalHeight', 'bodyMassIndex', 'bodyMassIndexPercentile', 'heightLossFromBaseline'];
const ARRAY_FIELDS = [];
const BOOLEAN_FIELDS = ['shoesRemoved', 'estimatedHeight', 'kyphoscoliosis', 'instrumentCalibrated', 'patientCooperative'];
const STRING_FIELDS = ['heightFeetInches', 'measurementMethod', 'patientPosition', 'growthChartReference', 'measuredBy', 'spinalDeformity', 'measurementAccuracy'];
...
/* ═══════ COMPONENT ═══════ */
const HeightMeasurementsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.height_measurements) return Array.isArray(r.height_measurements) ? r.height_measurements : [r.height_measurements];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.height_measurements) return Array.isArray(dd.height_measurements) ? dd.height_measurements : [dd.height_measurements]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Height Measurements ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const suffix = `-${idx}`;
    try {
      // Only this section's pending edits (localEdits keyed "<field>-<idx>" or "<field>.<arrayIndex>-<idx>")
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/height_measurements/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/height_measurements/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HEIGHT MEASUREMENTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Height Measurements ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="height-measurements-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Height Measurements</h2></div>
        <div className="empty-state">No height measurements data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="height-measurements-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Height Measurements</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HeightMeasurementsDocumentPDFTemplate document={pdfData} />} fileName={`height-measurements-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search height measurements..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Height Measurements ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'primary-measurement')}
            {renderSection(record, idx, 'surrogate-measures')}
            {renderSection(record, idx, 'pediatric-growth')}
            {renderSection(record, idx, 'bmi')}
            {renderSection(record, idx, 'quality-conditions')}
          </div>
        ))}
      </div>
    </div>
...
export default HeightMeasurementsDocument;
