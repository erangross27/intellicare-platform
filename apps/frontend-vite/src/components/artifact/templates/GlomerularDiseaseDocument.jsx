/**
 * GlomerularDiseaseDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: glomerular_disease
 *
 * 6 Sections:
 *   1. session-info: date (date picker), provider (string), facility (string), status (string)
 *   2. biopsy-findings: biopsyFindings.date, biopsyFindings.glomeruli, biopsyFindings.interstitium,
 *                       biopsyFindings.vessels, biopsyFindings.immunofluorescence, biopsyFindings.electronMicroscopy
 *   3. serologies: serologies.ana, serologies.antiDsDna, serologies.anca, serologies.antiGbm,
 *                  serologies.complement.C3, serologies.complement.C4
 *   4. clinical: diagnosis (string), findings (long), assessment (long)
 *   5. management: plan (string), immunosuppression (array), notes (long)
 *   6. (none — all fields covered above)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import GlomerularDiseasePDFTemplate from '../pdf-templates/GlomerularDiseasePDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './GlomerularDiseaseDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = localEdits field name, e.g. "findings"
   or "serologies.complement.C3"; value may be a string, array, or nested object) */
const DRAFT_KEY = 'glomerular_diseasePendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  status: 'Status',
  diagnosis: 'Diagnosis',
  'biopsyFindings.date': 'Biopsy Date',
  'biopsyFindings.glomeruli': 'Glomeruli',
  'biopsyFindings.interstitium': 'Interstitium',
  'biopsyFindings.vessels': 'Vessels',
  'biopsyFindings.immunofluorescence': 'Immunofluorescence',
  'biopsyFindings.electronMicroscopy': 'Electron Microscopy',
  'serologies.ana': 'ANA',
  'serologies.antiDsDna': 'Anti-dsDNA',
  'serologies.anca': 'ANCA',
  'serologies.antiGbm': 'Anti-GBM',
  'serologies.complement.C3': 'Complement C3',
  'serologies.complement.C4': 'Complement C4',
  findings: 'Findings',
...
const SECTION_FIELDS = {
  ...
  'biopsy-findings': ['biopsyFindings.date', 'biopsyFindings.glomeruli', 'biopsyFindings.interstitium', 'biopsyFindings.vessels', 'biopsyFindings.immunofluorescence', 'biopsyFindings.electronMicroscopy'],
  'serologies': ['serologies.ana', 'serologies.antiDsDna', 'serologies.anca', 'serologies.antiGbm', 'serologies.complement.C3', 'serologies.complement.C4'],
  'clinical': ['diagnosis', 'findings', 'assessment', 'results'],
  'management': ['plan', 'immunosuppression', 'recommendations', 'notes'],
};

const NUMBER_FIELDS = [];
const STRING_FIELDS = ['provider', 'facility', 'status', 'diagnosis', 'biopsyFindings.date', 'biopsyFindings.glomeruli', 'biopsyFindings.interstitium', 'biopsyFindings.vessels', 'biopsyFindings.immunofluorescence', 'biopsyFindings.electronMicroscopy', 'serologies.ana', 'serologies.antiDsDna', 'serologies.anca', 'serologies.antiGbm', 'serologies.complement.C3', 'serologies.complement.C4', 'findings', 'assessment', 'plan', 'notes'];
const DATE_FIELDS = ['date'];
const ARRAY_FIELDS = ['immunosuppression'];
/* Array-of-objects {recommendation, date} (date-grouped, editable) */
const OBJECT_ARRAY_FIELDS = ['recommendations'];
/* OBJECT field (recursive editable leaves) */
const OBJECT_FIELDS = ['results'];
const LONG_TEXT_FIELDS = ['biopsyFindings.glomeruli', 'biopsyFindings.immunofluorescence', 'biopsyFindings.electronMicroscopy', 'findings', 'assessment', 'notes'];
/* NO_SPLIT_FIELDS: fields to keep as one whole row. glomeruli DOES split — its bare-numeric labeled clause
   ("Total glomeruli: 22") is rendered as a −/+ stepper (numLabeled path) rather than a numeric textarea. */
...
const KEY_OVERRIDES = {
  ef: 'EF', lvef: 'LVEF', gfr: 'GFR', egfr: 'eGFR', acr: 'ACR', pcr: 'PCR', bun: 'BUN', upcr: 'UPCR', uacr: 'UACR',
};
...
/* ═══════ COMPONENT ═══════ */
const GlomerularDiseaseDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.glomerular_disease) return Array.isArray(r.glomerular_disease) ? r.glomerular_disease : [r.glomerular_disease];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.glomerular_disease) return Array.isArray(dd.glomerular_disease) ? dd.glomerular_disease : [dd.glomerular_disease]; return [dd]; }
      return [r];
    });
  ...
  /* dot-path getFieldValue: supports "biopsyFindings.glomeruli", "serologies.complement.C3" etc. */
  ...
  /* searchable string for a field value — flattens objects/arrays of objects */
  const searchableVal = useCallback((fn, val) => {
    if (OBJECT_FIELDS.includes(fn) || OBJECT_ARRAY_FIELDS.includes(fn)) return flattenSearchable(val);
    return fmtVal(val);
  }, [fmtVal]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Glomerular Disease ${idx + 1}`.toLowerCase();
  ...
  /* Stage a DRAFT locally (NO DB write). localStorage keeps it across refresh; Approve commits it.
     localEditKey = `${fn}-${idx}` (the canonical key used everywhere); the draft store keys by record _id
     then by `fn` (the localEdits field part). `value` may be a string, array, or nested object. */
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.gfr) — value stays a STRING. DRAFT only. */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // localEdits keys are `${fn}-${idx}` where fn is the section field (plain, dotted-string,
      // array root, or object root). Commit each staged value whole — no arrayIndex (fn never ends
      // in a purely-numeric dot-segment for this template).
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/glomerular_disease/${id}/edit`, { field: fn, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/glomerular_disease/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[GlomerularDisease] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_ARRAY_FIELDS.includes(f)) {
        ...
        arr.forEach((item) => { const rt = (item?.recommendation || '').trim(); const rd = (item?.date || '').trim(); if (rd !== lastDate) { if (rd) body += `${rd}\n`; lastDate = rd; n = 1; } body += `${n++}. ${rt}\n`; });
        body += '\n';
        return;
      }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== GLOMERULAR DISEASE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Glomerular Disease ${idx + 1}\n${'='.repeat(40)}\n\n`;
        ...
        <div key={fn}>
          <div className="rec-mini-card">
            ...
            {sentences.map((sentence, sIdx) => {
              ...
              /* Regular sentence row. A labeled clause whose value is a bare number (e.g. "Total glomeruli: 22")
                 gets a −/+ stepper so it isn't a numeric textarea (widget harness) nor a side-by-side row. */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="glomerular-disease-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Glomerular Disease</h2></div>
        <div className="empty-state">No glomerular disease records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="glomerular-disease-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Glomerular Disease</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<GlomerularDiseasePDFTemplate document={pdfData} />} fileName="Glomerular_Disease.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search glomerular disease records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Glomerular Disease ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'biopsy-findings')}
            {renderSection(record, idx, 'serologies')}
...
export default GlomerularDiseaseDocument;
