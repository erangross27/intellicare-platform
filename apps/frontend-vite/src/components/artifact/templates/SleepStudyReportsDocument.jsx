 ...
 *
 * 5 Sections:
 *   1. sleep-indices: apneaHypopneaIndex, respiratoryDisturbanceIndex, oxygenDesaturationIndex
 *   2. oxygenation: minimumOxygenSaturation, meanOxygenSaturation
 *   3. sleep-architecture: totalSleepTime, sleepEfficiency, sleepLatency, remLatency, wakeAfterSleepOnset, arousalIndex
 *   4. respiratory-events: centralApneaIndex, obstructiveApneaIndex, mixedApneaIndex, hypopneaIndex
 *   5. other-measurements: periodicLimbMovementIndex, epworthSleepinessScale, pittsburghSleepQualityIndex, snoreIntensity, cpapTitrationPressure, bodyPositionDependency, cheyneStokesBrewing
 */
...
const STRING_FIELDS = ['snoreIntensity'];
...
const SleepStudyReportsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.sleep_study_reports) return Array.isArray(r.sleep_study_reports) ? r.sleep_study_reports : [r.sleep_study_reports];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.sleep_study_reports) return Array.isArray(dd.sleep_study_reports) ? dd.sleep_study_reports : [dd.sleep_study_reports]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Sleep Study Report ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // stageDraft — write a field's pending value to the localStorage draft store (survives refresh).
  // sid is passed so the section's Approve button returns to yellow Pending on re-edit.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Collect this record's staged drafts belonging to this section (editKey = "field-idx").
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/sleep_study_reports/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/sleep_study_reports/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (BOOLEAN_FIELDS.includes(f)) {
        ...
        arr.forEach((item, i) => {
          ...
          if (item && typeof item === 'object' && !Array.isArray(item)) {
            itemStr = Object.entries(item).filter(([, v]) => v !== null && v !== undefined && v !== '').map(([k, v]) => `${k}: ${typeof v === 'boolean' ? (v ? 'Yes' : 'No') : v}`).join(', ');
          } else { itemStr = String(item); }
          if (itemStr) text += `  ${i + 1}. ${itemStr}\n`;
        });
        text += '\n';
  ...
  /* ═══════ RENDER: ARRAY FIELD (string-items or object-items) ═══════ */
              ...
              <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const existingArr = Array.isArray(getFieldValue(record, fn, idx)) ? [...getFieldValue(record, fn, idx)] : [...arr]; existingArr[aIdx] = editValue; setSaveError(null); stageDraft(record, fn, idx, existingArr, sid); setEditedFields(prev => ({ ...prev, [itemKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="sleep-study-reports-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Sleep Study Reports</h2></div>
        <div className="empty-state">No sleep study reports available</div>
      </div>
    );
  }
    ...
    <div className="sleep-study-reports-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<SleepStudyReportsDocumentPDFTemplate document={pdfData} />} fileName={`sleep-study-reports-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search sleep study reports..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Sleep Study Report ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'sleep-indices')}
            {renderSection(record, idx, 'oxygenation')}
            {renderSection(record, idx, 'sleep-architecture')}
            {renderSection(record, idx, 'respiratory-events')}
            {renderSection(record, idx, 'other-measurements')}
          </div>
        ))}
      </div>
    </div>
