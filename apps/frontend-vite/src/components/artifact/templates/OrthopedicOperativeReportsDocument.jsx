/**
 * OrthopedicOperativeReportsDocument.jsx
 * March 2026 — Canonical one-pass polish: inline editing, blue glow theme, BlueDatePicker.
 * Collection: orthopedic_operative_reports (displayed as "Orthopedic Operative Reports")
 *
 * Record `date` → editable record-header date pill (BlueDatePicker).
 * assistants + implants → arrays of strings (labeled array items).
 * All remaining fields are narrative strings.
 *
 * 9 Sections:
 *   1. procedure-details: date (header), procedureName, anatomicalSite, laterality, approach
 *   2. surgical-team: surgeon, assistants
 *   3. clinical-indication: indication
 *   4. implants-hardware: implants, hardwareRemoved
 *   5. operative-findings: findings
 *   6. intraoperative-data: tourniquet, estimatedBloodLoss, complications
 *   7. post-operative-assessment: xrayVerification
 *   8. facility: facility
 *   9. notes: notes
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import OrthopedicOperativeReportsDocumentPDFTemplate from '../pdf-templates/OrthopedicOperativeReportsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './OrthopedicOperativeReportsDocument.css';
...
/* `date` renders as the editable record-header pill (NOT in the section body / Copy All).
   Its edits are committed via the section that lists it (procedure-details). */
const HEADER_DATE_FIELD = 'date';
const HEADER_DATE_SECTION = 'procedure-details';
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } } — fieldPart is the field name, or "field.arrIdx" for
   an array element; value is the full field value (string) or the element string. */
const DRAFT_KEY = 'orthopedic_operative_reportsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const OrthopedicOperativeReportsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.orthopedic_operative_reports) return Array.isArray(r.orthopedic_operative_reports) ? r.orthopedic_operative_reports : [r.orthopedic_operative_reports];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.orthopedic_operative_reports) return Array.isArray(dd.orthopedic_operative_reports) ? dd.orthopedic_operative_reports : [dd.orthopedic_operative_reports]; return [dd]; }
      return [r];
    });
  ...
  /* ═══════ SEARCH — 4-LEVEL ═══════ */
  const matchesFieldValue = useCallback((f, val, phrase) => {
    if (val === null || val === undefined) return false;
    if (DATE_FIELDS.includes(f)) return formatDate(val).toLowerCase().includes(phrase);
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (matchesFieldValue(f, getFieldValue(record, f, 0), phrase)) return true;
    }
    return false;
  }, [searchTerm, getFieldValue, matchesFieldValue]);
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (label.includes(phrase) || phrase.includes(label)) return true;
    return matchesFieldValue(fn, getFieldValue(record, fn, idx), phrase);
  }, [searchTerm, getFieldValue, matchesFieldValue]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Orthopedic Operative Report ${idx + 1}`.toLowerCase();
      if (rt.includes(phrase) || phrase.includes(rt)) { record._showAllSections = true; return true; }
      for (const t of Object.values(SECTION_TITLES)) { if (t.toLowerCase().includes(phrase) || phrase.includes(t.toLowerCase())) return true; }
      for (const l of Object.values(FIELD_LABELS)) { if (l.toLowerCase().includes(phrase) || phrase.includes(l.toLowerCase())) return true; }
      for (const sFields of Object.values(SECTION_FIELDS)) {
        for (const f of sFields) {
          if (matchesFieldValue(f, getFieldValue(record, f, idx), phrase)) return true;
        }
      }
      return false;
    });
  }, [records, searchTerm, getFieldValue, matchesFieldValue]);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // staged editKeys ("field-idx", value = full field value: string OR full array) for this section
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/orthopedic_operative_reports/${id}/edit`, { field: fn, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/orthopedic_operative_reports/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this section's drafts from localStorage (now committed) — whole field + any "field.idx" elems
      const store = readDrafts();
      if (store[id]) {
        fields.forEach(f => {
          delete store[id][f];
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== ORTHOPEDIC OPERATIVE REPORTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Orthopedic Operative Report ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: RECORD-HEADER DATE PILL (BlueDatePicker) ═══════ */
        ...
        {isEditing ? (
          <span className="edit-field-container" onClick={e => e.stopPropagation()}>
            ...
            <span className="edit-actions">
              <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date'); return; } handleSaveField(record, fn, idx, HEADER_DATE_SECTION, null, editValue + 'T00:00:00.000Z'); }}>{saving ? 'Saving...' : 'Save'}</button>
  ...
  /* ═══════ RENDER: DATE FIELD (in-section BlueDatePicker) ═══════ */
  ...
  /* ═══════ RENDER: ARRAY FIELD (array of strings; labeled items) ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {val.map((item, arrIdx) => {
          ...
          return (
            <div key={arrIdx}>
              {itemParsed.isLabeled && <div className="nested-subtitle sub-label">{highlightText(itemParsed.label)}</div>}
  ...
  if (!records || records.length === 0) {
    return (
      <div className="orthopedic-operative-reports-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Orthopedic Operative Reports</h2></div>
        <div className="empty-state">No orthopedic operative reports available</div>
      </div>
    );
  }
  ...
  return (
    <div className="orthopedic-operative-reports-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Orthopedic Operative Reports</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OrthopedicOperativeReportsDocumentPDFTemplate document={pdfData} />} fileName="Orthopedic_Operative_Reports.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search orthopedic operative reports..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Orthopedic Operative Report ${idx + 1}`)}</h3>
            </div>
...
export default OrthopedicOperativeReportsDocument;
