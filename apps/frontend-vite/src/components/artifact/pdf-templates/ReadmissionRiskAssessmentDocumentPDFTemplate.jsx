/**
 * ReadmissionRiskAssessmentDocumentPDFTemplate.jsx
 * June 2026 — Helvetica — LETTER size — readmission risk assessment
 * Collection: readmission_risk_assessment
 * NO BLUE COLORS (#606060/#9a9a9a/#bcbcbc BANNED) — #000000/#333333/#cccccc/#f5f5f5 ONLY
 * BOX-FREE black & white. Rule #74: sectionTitle rendered INSIDE the first present field's View (no orphan siblings).
 */
...
/* MEANINGFUL_ZERO_FIELDS: numerics where 0 is a valid clinical finding (low-risk / "none"), NOT a "not recorded" sentinel.
   Matches the frontend template; excludes lengthOfStayDays & medicationCount where 0 reads as missing data. */
...
/* renderDateRow: date value formatted, sectionTitle inside View (Rule #74) */
...
const SECTION_CONFIGS = [
  {
    ...
    fields: [
      ...
      { key: 'laceIndexScore', label: 'LACE Index Score', isNumber: true },
      { key: 'hospitalScore', label: 'HOSPITAL Score', isNumber: true },
      { key: 'readmissionProbability', label: 'Readmission Probability (%)', isNumber: true },
      { key: 'riskStratificationLevel', label: 'Risk Stratification Level', isSentence: true },
      { key: 'charlsonComorbidityIndex', label: 'Charlson Comorbidity Index', isNumber: true },
      { key: 'functionalStatusScore', label: 'Functional Status Score', isNumber: true },
    ],
  },
  {
    title: 'Utilization & Admission',
    fields: [
      { key: 'indexAdmissionDiagnosis', label: 'Index Admission Diagnosis', isSentence: true },
      { key: 'lengthOfStayDays', label: 'Length of Stay (days)', isNumber: true },
      { key: 'priorAdmissionsCount', label: 'Prior Admissions Count', isNumber: true },
      { key: 'priorEmergencyVisitsCount', label: 'Prior Emergency Visits Count', isNumber: true },
      { key: 'dischargeDisposition', label: 'Discharge Disposition', isSentence: true },
      { key: 'heartFailureExacerbation', label: 'Heart Failure Exacerbation', isBoolean: true },
      { key: 'activeCancerDiagnosis', label: 'Active Cancer Diagnosis', isBoolean: true },
      { key: 'chronicKidneyDiseaseStage', label: 'Chronic Kidney Disease Stage', isSentence: true },
    ],
  },
  {
    title: 'Medication & Clinical Risk',
    fields: [
      { key: 'medicationCount', label: 'Medication Count', isNumber: true },
      { key: 'highRiskMedications', label: 'High-Risk Medications', isArray: true },
      { key: 'polypharmacyIndicator', label: 'Polypharmacy', isBoolean: true },
      { key: 'cognitiveImpairmentPresent', label: 'Cognitive Impairment Present', isBoolean: true },
      { key: 'nutritionalRiskIndicator', label: 'Nutritional Risk Indicator', isSentence: true },
      { key: 'pendingTestResultsAtDischarge', label: 'Pending Test Results at Discharge', isBoolean: true },
    ],
  },
  {
    title: 'Social Support & Mitigation',
    fields: [
      { key: 'adequateSocialSupport', label: 'Adequate Social Support', isBoolean: true },
      { key: 'healthLiteracyScore', label: 'Health Literacy Score', isSentence: true },
      { key: 'followUpAppointmentScheduled', label: 'Follow-Up Appointment Scheduled', isBoolean: true },
      { key: 'mitigationInterventions', label: 'Mitigation Interventions', isArray: true },
      { key: 'assessmentPerformedBy', label: 'Assessment Performed By', isSentence: true },
    ],
  },
];
          ...
          <View key={index} style={styles.recordContainer}>
            ...
            <View style={styles.recordHeader} wrap={false}>
              <Text style={styles.recordTitle}>
                {`Readmission Risk Assessment ${index + 1}`}
              </Text>
            </View>
