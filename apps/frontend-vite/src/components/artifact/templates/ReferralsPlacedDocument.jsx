...
const SECTION_FIELDS = {
  'referral-details': ['date', 'specialty', 'reason', 'urgency', 'status'],
...
const ENUM_FIELDS = {
  urgency: ['routine', 'urgent', 'emergent', 'elective'],
...
const ReferralsPlacedDocument = ({ document: docProp }) => {
  ...
  const buildStringGroups = useCallback((text) => {
    const groups = [];
    splitBySentence(text).forEach((sentence, sentenceIndex) => {
      const parsed = parseLabel(sentence);
      const rows = (parsed.isLabeled ? splitByComma(parsed.value) : [sentence]).map((row, commaIndex) => ({
        text: row.replace(/[;.]+$/, '').trim(),
        sentenceIndex,
        commaIndex: parsed.isLabeled ? commaIndex : null,
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Referrals Placed ${idx + 1}`.toLowerCase();
  ...
  const formatSentenceFieldLines = useCallback((text) => {
    ...
    buildStringGroups(text).forEach(group => {
      if (group.label) {
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        body.push(...formatSentenceFieldLines(fmtVal(val)));
      }
      body.push('');
    });
    ...
    <div className="referrals-placed-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<ReferralsPlacedDocumentPDFTemplate document={pdfData} />} fileName="Referrals_Placed.pdf" className="copy-btn">
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Referrals Placed ${idx + 1}`)}</h3>
            </div>
