/**
 * FunctionalStatusDocumentPDFTemplate.jsx
 * Box-free B&W LETTER (canonical, memory 6a2d6af6) — mirrors the JSX: real record.date (never createdAt),
 * status enum canonical, values numbered ('1.' even singles), single-name label gate, OBJECT leaves
 * (adlItems/iadlItems/results) stacked, mobilityAids string-array numbered, recommendations date-grouped,
...
const SECTION_ORDER = ['session-info', 'adl-assessment', 'iadl-assessment', 'mobility', 'clinical-findings', 'results-section', 'recommendations-section', 'plan-notes'];
const SECTION_TITLES = {
  'session-info': 'Session Information',
  'adl-assessment': 'ADL Assessment',
  'iadl-assessment': 'IADL Assessment',
...
const FIELD_LABELS = {
  ...
  adlScore: 'ADL Score', adlItems: 'ADL Items', iadlScore: 'IADL Score', iadlItems: 'IADL Items',
  mobilityAids: 'Mobility Aids', findings: 'Findings', assessment: 'Assessment',
  results: 'Results', recommendations: 'Recommendations', plan: 'Plan', notes: 'Notes',
};
const SECTION_FIELDS = {
  'session-info': ['date', 'provider', 'facility', 'status'],
  'adl-assessment': ['adlScore', 'adlItems'],
  'iadl-assessment': ['iadlScore', 'iadlItems'],
  'mobility': ['mobilityAids'],
...
const OBJECT_FIELDS = ['adlItems', 'iadlItems', 'results'];
const OBJECT_ARRAY_FIELDS = ['recommendations'];
const ARRAY_FIELDS = ['mobilityAids'];
...
const FunctionalStatusDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].functional_status && Array.isArray(templateData[0].functional_status)) records = templateData[0].functional_status;
    else records = templateData;
  } else if (templateData && templateData.functional_status) {
    records = Array.isArray(templateData.functional_status) ? templateData.functional_status : [templateData.functional_status];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.functional_status ? (Array.isArray(dd.functional_status) ? dd.functional_status : [dd.functional_status]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Functional Status</Text></View>
        <Text style={styles.emptyState}>No functional status records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Functional Status ${idx + 1}`}</Text>
...
export default FunctionalStatusDocumentPDFTemplate;
