...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'plastic-surgery-consultations-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['consultationDate', 'provider', 'facility', 'procedureCategory', 'anesthesiaType', 'smokingStatus', 'chiefConcern', 'requestedProcedure', 'recommendedProcedure', 'anatomicalArea', 'physicalExamFindings', 'surgicalHistory', 'medicalContraindications', 'bmi', 'patientExpectations', 'photographicDocumentation', 'psychologicalClearance', 'risksDiscussed', 'alternativeTreatments', 'estimatedCost', 'surgicalTechnique', 'preoperativeRequirements', 'estimatedRecoveryTime', 'consentStatus', 'followUpPlan'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('plastic_surgery_consultations', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[plastic_surgery_consultations] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('plastic_surgery_consultations', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[plastic_surgery_consultations] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
