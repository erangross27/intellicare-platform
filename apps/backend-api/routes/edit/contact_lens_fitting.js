...
function buildContext(req, op = 'read') { return { serviceId: 'contact-lens-fitting-edit-service', userId: req.user?.id, operation: op, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [op === 'read' ? 'read' : 'write'] }; }
...
const ALLOWED_FIELDS = [
  ...
  'lensCentration', 'lensMovement', 'fluoresceinPattern',
  'overRefractionSphere', 'overRefractionCylinder', 'oxygenTransmissibility',
  'wearingSchedule', 'replacementSchedule',
];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('contact_lens_fitting', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[contact_lens_fitting] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('contact_lens_fitting', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[contact_lens_fitting] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
