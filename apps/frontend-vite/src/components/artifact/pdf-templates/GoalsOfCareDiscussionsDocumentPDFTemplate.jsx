/**
 * GoalsOfCareDiscussionsDocumentPDFTemplate.jsx
 * Box-free B&W LETTER (canonical, memory 6a2d6af6) — mirrors the JSX: real record.assessmentDate (never
 * createdAt), narratives split on [.;] with abbrev/single-initial/genus guard + labeled comma-split (plan
 * "Code Status: ..."), values numbered ('1.' even singles), single-name label gate, recommendations date-
 * grouped. Rule #74: each field is ONE wrap={false} atomic View with the sectionTitle riding INSIDE the first
 * present field's View. Static PHI footer. Collection: goals_of_care_discussions.
 */
...
const GoalsOfCareDiscussionsDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].goals_of_care_discussions && Array.isArray(templateData[0].goals_of_care_discussions)) records = templateData[0].goals_of_care_discussions;
    else records = templateData;
  } else if (templateData && templateData.goals_of_care_discussions) {
    records = Array.isArray(templateData.goals_of_care_discussions) ? templateData.goals_of_care_discussions : [templateData.goals_of_care_discussions];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.goals_of_care_discussions ? (Array.isArray(dd.goals_of_care_discussions) ? dd.goals_of_care_discussions : [dd.goals_of_care_discussions]) : [dd]);
...
export default GoalsOfCareDiscussionsDocumentPDFTemplate;
