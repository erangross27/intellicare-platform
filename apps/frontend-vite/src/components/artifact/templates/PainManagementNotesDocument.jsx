/**
 * PainManagementNotesDocument.jsx
 * June 2026 — Complete rebuild for the REAL pain_management_notes schema.
 * Collection: pain_management_notes (displayed as "Pain Management Notes")
 *
 * 6 Sections:
 *   1. overview: date, painScaleScore
 *   2. pain-profile: painLocation, painQuality, painDurationDays
 *   3. scores: oswestryDisabilityIndex, rolandMorrisScore, briefPainInventoryScore,
 *              painInterferenceScore, catastrophizingScale, centralSensitizationInventory,
 *              neuropathicPainScale, sleepDisturbanceScore
 *   4. medications: currentMedicationList, morphineEquivalentDose, opioidRiskToolScore
 *   5. function-exam: functionalCapacityEvaluation, rangeOfMotionDegrees, muscleStrengthGrade,
 *                     neurodynamicTesting, triggerPointLocations
 *   6. interventions: interventionalProcedures, injectionSite, medicationVolumeMl, fluoroscopyTime
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PainManagementNotesDocumentPDFTemplate from '../pdf-templates/PainManagementNotesDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './PainManagementNotesDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'pain_management_notesPendingEdits';
const readDrafts = () => {
...
/* MEANINGFUL_ZERO_FIELDS: 0 is a valid clinical finding, NOT a "not-recorded" sentinel:
   painScaleScore 0 = no pain; morphineEquivalentDose 0 = opioid-free (key safety datapoint);
   opioidRiskToolScore 0 = no/low ORT risk. These always render when present, even at 0.
   The remaining instrument scores (Oswestry/Roland-Morris/BPI/etc.) use 0 as an
   "instrument-not-administered" sentinel and stay hidden unless doctor-edited. */
...
const DATE_FIELDS = [];
const STRING_FIELDS = ['functionalCapacityEvaluation', 'muscleStrengthGrade', 'neurodynamicTesting', 'injectionSite'];
...
/* formatDate: toLocaleDateString en-US long; raw fallback */
...
/* ═══════ COMPONENT ═══════ */
const PainManagementNotesDocument = ({ document: docProp }) => {
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Draft store shape: { [recordId]: { [fieldPart]: value } } where fieldPart is "field" (scalar/string/
     comma-list/whole-array) or "field.arrayIndex" (single array element). */
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayElem) {
          // Array element draft: localEdits holds the whole array under `${fn}-${idx}`
          const fn = fieldPart.slice(0, dotIdx);
          const arrIndex = parseInt(lastSeg, 10);
          const editKey = `${fn}-${idx}`;
          const base = Array.isArray(nLocal[editKey])
            ? nLocal[editKey]
            : (Array.isArray(record[fn]) ? [...record[fn]] : []);
          base[arrIndex] = value;
          nLocal[editKey] = base;
          nPending[`${fn}.${arrIndex}-${idx}`] = true;
          nFields[`${fn}.${arrIndex}-${idx}`] = 'edited';
        } else {
          // Scalar / string / comma-list / whole-field draft
          const fn = fieldPart;
          const editKey = `${fn}-${idx}`;
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pain Management Notes ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (val && (Array.isArray(val) ? val.some(item => String(item).toLowerCase().includes(phrase)) : (DATE_FIELDS.includes(f) ? formatDate(val).toLowerCase().includes(phrase) : fmtVal(val).toLowerCase().includes(phrase)))) return true;
        }
      }
      return false;
    });
  ...
  const pdfData = useMemo(() => {
    return filteredRecords.map((record, idx) => {
      ...
      Object.keys(localEdits).forEach(key => {
        ...
        // Pending drafts (scalar edit on this key, OR any staged array-element edit on this base field)
        // stay OUT of the PDF/Copy All until approved.
        const fn = m[1];
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Collect this record's pending edits that belong to this section, then PUT each to the DB.
      // Scalar/string/comma-list/whole-array drafts: pending key = `${fn}-${idx}`, value in localEdits[key].
      // Array-element drafts: pending key = `${fn}.${arrayIndex}-${idx}`, value = localEdits[`${fn}-${idx}`][arrayIndex].
      const committedPendingKeys = [];
      for (const pk of Object.keys(pendingEdits)) {
        if (!pendingEdits[pk] || !pk.endsWith(`-${idx}`)) continue;
        const fieldPart = pk.slice(0, pk.length - `-${idx}`.length); // "field" or "field.arrayIndex"
        const dotIdx = fieldPart.lastIndexOf('.');
        const lastSeg = dotIdx !== -1 ? fieldPart.slice(dotIdx + 1) : '';
        const isArrayElem = dotIdx !== -1 && /^\d+$/.test(lastSeg);
        const fn = isArrayElem ? fieldPart.slice(0, dotIdx) : fieldPart;
        if (!fields.includes(fn)) continue;
        const payload = { field: fn };
        if (isArrayElem) {
          const arrayIndex = parseInt(lastSeg, 10);
          const arr = localEdits[`${fn}-${idx}`];
          payload.value = Array.isArray(arr) ? arr[arrayIndex] : undefined;
          payload.arrayIndex = arrayIndex;
        } else {
          payload.value = localEdits[`${fn}-${idx}`];
        }
        const resp = await secureApiClient.put(`/api/edit/pain_management_notes/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedPendingKeys.push(pk);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/pain_management_notes/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; committedPendingKeys.forEach(k => delete n[k]); return n; });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PAIN MANAGEMENT NOTES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Pain Management Notes ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: DATE FIELD (string, formatted) ═══════ */
  ...
  const renderArrayField = (record, fn, idx, sid) => {
    ...
    const label = FIELD_LABELS[fn] || fn;
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
    /* single-name gate: never duplicate the section title as a nested-subtitle */
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(item); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; setSaveError(null); const cur = splitByComma(String(getFieldValue(record, fn, idx) || '')).map(s => s.trim()).filter(Boolean); cur[ciIdx] = editValue.trim(); const fullText = cur.join(', '); stageDraft(record, fn, idx, sid, fullText); setEditedSentences(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pain-management-notes-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pain Management Notes</h2></div>
        <div className="empty-state">No pain management notes data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pain-management-notes-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Pain Management Notes</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PainManagementNotesDocumentPDFTemplate document={pdfData} />} fileName="Pain_Management_Notes.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pain management notes..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Pain Management Notes ${idx + 1}`)}</h3>
            </div>
            ...
            {renderSection(record, idx, 'function-exam')}
...
export default PainManagementNotesDocument;
