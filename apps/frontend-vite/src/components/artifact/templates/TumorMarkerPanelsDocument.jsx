/**
 * TumorMarkerPanelsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: tumor_marker_panels
 *
 * 4 Sections:
 *   1. panel-info: panelIndication, specimenType, collectionDateTime, clinicalContext
 *   2. primary-markers: afpLevel, ceaLevel, ca125Level, ca199Level, ca153Level, psaTotal, psaFree, betaHcgLevel
 *   3. secondary-markers: ldhLevel, calcitoninLevel, thyroglobulinLevel, chromograninALevel, neuronSpecificEnolase, squamousCellCarcinomaAntigen, cyfra211Level, ca242Level, her2NeuLevel
 *   4. clinical-summary: abnormalMarkers, previousMarkerComparison
 */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   full array under their plain field name, matching this template's localEdits convention) */
const DRAFT_KEY = 'tumor_marker_panelsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'panel-info': 'Panel Information',
  'primary-markers': 'Primary Tumor Markers',
  'secondary-markers': 'Secondary Tumor Markers',
  'clinical-summary': 'Clinical Summary',
};

const FIELD_LABELS = {
  panelIndication: 'Panel Indication',
  specimenType: 'Specimen Type',
  collectionDateTime: 'Collection Date/Time',
  clinicalContext: 'Clinical Context',
  afpLevel: 'AFP Level',
  ceaLevel: 'CEA Level',
  ca125Level: 'CA-125 Level',
  ca199Level: 'CA 19-9 Level',
  ca153Level: 'CA 15-3 Level',
  psaTotal: 'PSA Total',
  psaFree: 'PSA Free',
  betaHcgLevel: 'Beta-HCG Level',
  ldhLevel: 'LDH Level',
  calcitoninLevel: 'Calcitonin Level',
  thyroglobulinLevel: 'Thyroglobulin Level',
  chromograninALevel: 'Chromogranin A Level',
  neuronSpecificEnolase: 'Neuron-Specific Enolase',
  squamousCellCarcinomaAntigen: 'Squamous Cell Carcinoma Antigen',
  cyfra211Level: 'CYFRA 21-1 Level',
  ca242Level: 'CA 242 Level',
  her2NeuLevel: 'HER2/neu Level',
  abnormalMarkers: 'Abnormal Markers',
  previousMarkerComparison: 'Previous Marker Comparison',
};

const SECTION_FIELDS = {
  'panel-info': ['panelIndication', 'specimenType', 'collectionDateTime', 'clinicalContext'],
  'primary-markers': ['afpLevel', 'ceaLevel', 'ca125Level', 'ca199Level', 'ca153Level', 'psaTotal', 'psaFree', 'betaHcgLevel'],
  'secondary-markers': ['ldhLevel', 'calcitoninLevel', 'thyroglobulinLevel', 'chromograninALevel', 'neuronSpecificEnolase', 'squamousCellCarcinomaAntigen', 'cyfra211Level', 'ca242Level', 'her2NeuLevel'],
  'clinical-summary': ['abnormalMarkers', 'previousMarkerComparison'],
};

const NUMBER_FIELDS = ['afpLevel', 'ceaLevel', 'ca125Level', 'ca199Level', 'ca153Level', 'psaTotal', 'psaFree', 'betaHcgLevel', 'ldhLevel', 'calcitoninLevel', 'thyroglobulinLevel', 'chromograninALevel', 'neuronSpecificEnolase', 'squamousCellCarcinomaAntigen', 'cyfra211Level', 'ca242Level', 'her2NeuLevel'];
...
const TumorMarkerPanelsDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Tumor Marker Panel ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      await secureApiClient.put(`/api/edit/tumor_marker_panels/${id}/approve`, { sectionId: sid, approved: true });
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); const id = safeId(record); if (!id) return; const lk = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [lk]: fullText })); setPendingEdits(prev => ({ ...prev, [lk]: true })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setApprovedSections(prev => { const k = `${sid}-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; }); const store = readDrafts(); if (!store[id]) store[id] = {}; store[id][fn] = fullText; writeDrafts(store); setEditingField(null); setEditValue(''); setSaveError(null); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="tumor-marker-panels-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Tumor Marker Panels</h2></div>
        <div className="empty-state">No tumor marker panel records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="tumor-marker-panels-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search tumor marker panels..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'primary-markers')}
            {renderSection(record, idx, 'secondary-markers')}
...
export default TumorMarkerPanelsDocument;
