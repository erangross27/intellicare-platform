...
const COLLECTION = 'bone_health';
const COMMA_SPLIT_FIELDS = ['boneProtectionTherapy', 'assessment'];
const ARRAY_FIELDS = new Set(['fractures', 'riskFactors']);
const OBJECT_FIELDS = new Set(['dexaScan', 'recommendations', 'results', 'additionalData']);
const NARRATIVE_FIELDS = new Set(['boneProtectionTherapy', 'findings', 'assessment', 'plan', 'notes']);
const DATE_FIELDS = new Set(['date']);
const ZERO_SENTINEL_FIELDS = new Set([]);
const KEY_OVERRIDES = { tScore: 'T-Score' };
const LABELS = {
  date: 'Date', type: 'Type', status: 'Status', provider: 'Provider', facility: 'Facility',
  dexaScan: 'DEXA Scan', boneProtectionTherapy: 'Bone Protection Therapy', fractures: 'Fractures',
  riskFactors: 'Risk Factors', findings: 'Findings', assessment: 'Assessment', plan: 'Plan',
  recommendations: 'Recommendations', results: 'Results', notes: 'Notes', additionalData: 'Additional Data',
};
const SECTIONS = [
  { title: 'Date', fields: ['date'] },
  { title: 'Record Information', fields: ['provider', 'facility'] },
  { title: 'Record Details', fields: ['type', 'status'] },
  { title: 'DEXA Scan', fields: ['dexaScan'] },
  { title: 'Bone Protection Therapy', fields: ['boneProtectionTherapy'] },
  { title: 'Fractures', fields: ['fractures'] },
...
const BoneHealthDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Bone Health</Text><Text style={styles.noData}>No bone health data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Bone Health</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Bone Health {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
