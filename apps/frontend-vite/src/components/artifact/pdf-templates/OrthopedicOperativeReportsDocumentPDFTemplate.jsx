/**
 * OrthopedicOperativeReportsDocumentPDFTemplate.jsx
 ...
 *   - assistants + implants -> arrays of strings (numbered items under the field label).
...
const SECTION_ORDER = ['procedure-details', 'surgical-team', 'clinical-indication', 'implants-hardware', 'operative-findings', 'intraoperative-data', 'post-operative-assessment', 'facility', 'notes'];
const SECTION_TITLES = {
  'procedure-details': 'Procedure Details',
  'surgical-team': 'Surgical Team',
  'clinical-indication': 'Clinical Indication',
  'implants-hardware': 'Implants and Hardware',
  'operative-findings': 'Operative Findings',
  'intraoperative-data': 'Intraoperative Data',
  'post-operative-assessment': 'Post-Operative Assessment',
  'facility': 'Facility',
  'notes': 'Notes',
};
const FIELD_LABELS = {
  ...
  xrayVerification: 'X-Ray Verification',
  facility: 'Facility',
  notes: 'Notes',
};
const SECTION_FIELDS = {
  'procedure-details': ['date', 'procedureName', 'anatomicalSite', 'laterality', 'approach'],
  'surgical-team': ['surgeon', 'assistants'],
  'clinical-indication': ['indication'],
  'implants-hardware': ['implants', 'hardwareRemoved'],
  'operative-findings': ['findings'],
  'intraoperative-data': ['tourniquet', 'estimatedBloodLoss', 'complications'],
  'post-operative-assessment': ['xrayVerification'],
  'facility': ['facility'],
  'notes': ['notes'],
};
...
const ARRAY_FIELDS = ['assistants', 'implants'];
const STRING_FIELDS = ['procedureName', 'anatomicalSite', 'laterality', 'approach', 'surgeon', 'indication', 'hardwareRemoved', 'findings', 'tourniquet', 'estimatedBloodLoss', 'complications', 'xrayVerification', 'facility', 'notes'];
...
const OrthopedicOperativeReportsDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].orthopedic_operative_reports && Array.isArray(templateData[0].orthopedic_operative_reports)) records = templateData[0].orthopedic_operative_reports;
    else records = templateData;
  } else if (templateData && templateData.orthopedic_operative_reports) {
    records = Array.isArray(templateData.orthopedic_operative_reports) ? templateData.orthopedic_operative_reports : [templateData.orthopedic_operative_reports];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.orthopedic_operative_reports ? (Array.isArray(dd.orthopedic_operative_reports) ? dd.orthopedic_operative_reports : [dd.orthopedic_operative_reports]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Orthopedic Operative Reports</Text></View>
        <Text style={styles.emptyState}>No orthopedic operative reports available.</Text>
      </Page></Document>
    );
  }
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <View wrap={false}>
              ...
              <Text style={styles.recordTitle}>{safeString(`Orthopedic Operative Report ${idx + 1}`)}</Text>
            </View>
...
export default OrthopedicOperativeReportsDocumentPDFTemplate;
