/**
 * WeeklyVirtualCheckInsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: weekly_virtual_check_ins (displayed as "Weekly Virtual Check-Ins")
 *
 * 6 Sections:
 *   1. record-info: provider, facility, type, platform, frequency, status, scheduled
 *   2. purpose: purpose
 *   3. findings-assessment: findings, assessment
 *   4. plan: plan
 *   5. recommendations: recommendations [{recommendation, date}]
 *   6. notes: notes
 */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the "${fn}-${idx}" minus the idx, i.e. the field key) */
const DRAFT_KEY = 'weeklyVirtualCheckInsPendingEdits';
const readDrafts = () => {
...
const splitByComma = (text) => {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (ch === '(') { depth++; current += ch; }
      ...
      /* date-aware: skip comma if next token looks like a 4-digit year */
...
const WeeklyVirtualCheckInsDocument = ({ document: docProp }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        // Mark the field edited so the section shows Pending Approve. recommendations stages the whole
        // array under "recommendations-<idx>"; mark its tracking key so sectionHasEdits picks it up.
        if (fieldPart === 'recommendations') nFields[`recommendations.0.recommendation-${idx}`] = 'edited';
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Weekly Virtual Check-In ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "field" or "field.<n>"
        ...
        const resp = await secureApiClient.put(`/api/edit/weekly_virtual_check_ins/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/weekly_virtual_check_ins/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    ...
    pdfData.forEach((r, idx) => {
      text += `Weekly Virtual Check-In ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="weekly-virtual-check-ins-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Weekly Virtual Check-Ins</h2></div>
        <div className="empty-state">No weekly virtual check-in records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="weekly-virtual-check-ins-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search weekly virtual check-ins..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
...
export default WeeklyVirtualCheckInsDocument;
