/**
 * Biologic Therapy Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance.
 * Collection: biologic_therapy (flat unified-medical-schema fields).
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'biologic-therapy-edit-service',
    userId: req.user?.id,
    operation,
...
// ALL non-system schema fields for biologic_therapy.
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('biologic_therapy', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[biologic_therapy] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('biologic_therapy', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[biologic_therapy] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
