/**
 * SpecimensDocument.jsx
 * July 2026 — recursive, dot-path editable specimens
...
import SpecimensDocumentPDFTemplate from '../pdf-templates/SpecimensDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SpecimensDocument.css';

const DRAFT_KEY = 'specimensPendingEdits';
...
const FIELD_LABELS = {
  type: 'Type', description: 'Description', handling: 'Handling', pathologyNumber: 'Pathology Number', preliminaryResults: 'Preliminary Results', finalResults: 'Final Results', findings: 'Findings', assessment: 'Assessment', plan: 'Plan', recommendations: 'Recommendations', notes: 'Notes', provider: 'Provider', facility: 'Facility', status: 'Status', date: 'Date', results: 'Results', additionalData: 'Additional Data',
};
const SECTION_FIELDS = {
  specimenInformation: ['type', 'description', 'handling', 'pathologyNumber'],
  reportedResults: ['preliminaryResults', 'finalResults', 'findings'],
  assessmentPlan: ['assessment', 'plan'],
  recommendationList: ['recommendations'],
  notes: ['notes'],
  providerInformation: ['provider', 'facility', 'status', 'date'],
  structuredResults: ['results'],
  additionalInformation: ['additionalData'],
};
const DATE_FIELDS = ['date'];
const DATETIME_FIELDS = [];
const ENUM_FIELDS = ['type', 'status'];
const NUMBER_FIELDS = [];
...
const NARRATIVE_PATHS = ['description', 'handling', 'preliminaryResults', 'finalResults', 'findings', 'assessment', 'plan', 'recommendations[]', 'notes', 'results.flowCytometry.interpretation'];
const PARENTHETICAL_LABEL_FIELDS = [];
const PARENTHETICAL_SEMICOLON_FIELDS = [];
const COMMA_FIELDS = ['handling'];
const COMMA_ARRAY_SPLIT_FIELDS = [];
const ARRAY_FIELDS = ['recommendations'];
const SEMICOLON_FIELDS = ['description', 'preliminaryResults', 'assessment', 'plan', 'recommendations', 'results.flowCytometry.interpretation'];
...
const KEY_LABELS = { flowCytometry: 'Flow Cytometry', cellCount: 'Cell Count', specimenAdequacy: 'Specimen Adequacy' };
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'type' ? ['Surgical Pathology', 'Cytology', 'Hematology', 'Microbiology', 'Molecular Pathology', 'Other']
    : field === 'status' ? ['Complete', 'Preliminary', 'Pending', 'In Progress', 'Cancelled']
      : [];
...
const SpecimensDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.specimens) return Array.isArray(record.specimens) ? record.specimens : [record.specimens];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.specimens) return Array.isArray(nested.specimens) ? nested.specimens : [nested.specimens];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/specimens/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/specimens/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[Specimens] Approve error:', error);
    ...
    return <div key={field} className="field-group">
      ...
      {groups.map((group, groupIndex) => <div key={groupIndex} className={'rec-mini-card nested-mini-card' + (group.type === 'unlabeled' ? ' regular-row-group' : '')}>
        ...
        {group.items.map((item) => {
          ...
          const labeledDate = Boolean(parsed && /date$/i.test(parsed.label) && !Number.isNaN(new Date(rowValue).getTime()));
          ...
          if (labeledDate) {
            props.editorType = 'date';
            props.displayValue = formatDate(rowValue);
            props.onStart = () => { setEditingField(key + '-part-' + item.partIndex); setEditValue(toInputDate(rowValue)); setSaveError(null); };
            props.onSave = () => {
              if (Number.isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date'); return; }
              const parts = splitFieldValue(field, getValue(record, field, recordIndex));
              parts[item.partIndex] = parsed.label + ': ' + formatDate(editValue + 'T00:00:00.000Z');
              stagePath(record, field, recordIndex, sectionId, joinFieldParts(field, parts));
            };
          } else {
  ...
  const copyAll = useCallback(async () => {
    let text = '=== SPECIMENS ===\n\n';
    records.forEach((record, recordIndex) => {
      text += 'Specimens Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  const renderSection = (record, recordIndex, sectionId) => {
    ...
    return <div className="section" key={sectionId}><div className="mini-cards-container">
      ...
      {sectionId === 'results' ? renderResults(record, recordIndex, sectionId)
          : sectionId === 'recommendations' && !fieldIn(ARRAY_FIELDS, 'recommendations') ? renderRecommendations(record, recordIndex, sectionId)
            : fields.map((field) => field === 'recommendations'
              && !fieldIn(ARRAY_FIELDS, field) ? renderRecommendations(record, recordIndex, sectionId)
  ...
  if (!records.length) return <div className="specimens-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Specimens</h2></div><div className="empty-state">No specimens available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Specimens Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="specimens-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Specimens</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<SpecimensDocumentPDFTemplate document={pdfData} />} fileName="Specimens.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search specimens..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Specimens Record ' + (recordIndex + 1))}</h3></div>
...
export default SpecimensDocument;
