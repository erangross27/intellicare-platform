...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'supportive-care-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['palliativeConsultDate', 'painScaleScore', 'edmontonSymptomAssessmentScore', 'karnofskyPerformanceStatus', 'ecogPerformanceStatus', 'opioidMedicationName', 'morphineEquivalentDose', 'nauseaVomitingSeverity', 'dyspneaSeverity', 'constrainedBowelMovementDays', 'symptomManagementMedications', 'nutritionalSupportType', 'albuminLevel', 'prealbumin', 'weightLossPercentage', 'sleepDisturbanceScale', 'anxietyLevel', 'depressionScreeningScore', 'socialWorkConsult', 'chaplainServices', 'familyMeetingDate', 'advanceDirectiveStatus', 'codeStatus', 'hospiceReferralDate'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('supportive_care', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[supportive_care] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('supportive_care', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[supportive_care] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
