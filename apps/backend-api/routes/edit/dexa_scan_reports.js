/**
 * DexaScanReports Edit Route
 * ServiceId: dexa-scan-reports-edit-service
...
function buildContext(req, operation = 'read') { return { serviceId: 'dexa-scan-reports-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
...
const ALLOWED_FIELDS = [
  ...
  'whoClassification', 'fracRiskMajor10Year', 'fracRiskHip10Year',
  'vertebralFracturesPresent', 'vertebralFractureGrade',
  'bodyCompositionAnalyzed', 'leanBodyMass', 'bodyFatPercentage',
  'scanQuality', 'artifactsPresent', 'treatmentRecommended', 'followUpInterval',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('dexa_scan_reports', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[DexaScanReports] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('dexa_scan_reports', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[DexaScanReports] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
