 ...
 * July 2026 — recursive, dot-path editable sepsis management
...
const SECTION_TITLES = {
  ...
  organDysfunction: 'Organ Dysfunction',
  bundleCompliance: 'Bundle Compliance',
};
const FIELD_LABELS = {
...
const SECTION_FIELDS = {
  assessment: ['date', 'sepsisSeverity', 'qsofaScore', 'sofaScore', 'sepsisOnsetDateTime'],
  infection: ['suspectedInfectionSource', 'bloodCultureCollectionTime', 'bloodCultureResults'],
  laboratory: ['initialLactateLevel', 'repeatLactateLevel', 'procalcitoninLevel', 'whiteBloodCellCount'],
  antibiotics: ['antibioticAdministrationTime', 'timeToAntibiotics', 'empiricAntibioticRegimen'],
  ...
  organDysfunction: ['organDysfunctionSites', 'mechanicalVentilationRequired', 'renalReplacementTherapy'],
  bundleCompliance: ['sepsisBundle3HourCompliance', 'sepsisBundle6HourCompliance'],
};
...
const ENUM_FIELDS = ['sepsisSeverity'];
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'sepsisSeverity' ? ['Sepsis', 'Severe sepsis', 'Septic shock'] : [];
...
const SepsisManagementDocument = ({ document: docProp }) => {
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/sepsis_management/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/sepsis_management/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SepsisManagement] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Sepsis Management Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="sepsis-management-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Sepsis Management</h2></div><div className="empty-state">No sepsis management available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Sepsis Management Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="sepsis-management-document" ref={containerRef}>
    <div className="document-header">
      ...
      <div className="header-actions">
        ...
        <PDFDownloadLink document={<SepsisManagementDocumentPDFTemplate document={pdfData} />} fileName="Sepsis_Management.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search sepsis management..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Sepsis Management Record ' + (recordIndex + 1))}</h3></div>
