...
function buildContext(req, op = 'read') { return { serviceId: 'neurological-assessment-edit-service', userId: req.user?.id, operation: op, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [op === 'read' ? 'read' : 'write'] }; }
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('neurological_assessment', { _id: oid }, { $set: { [updatePath]: value, [`doctorEdits.${field}`]: { value, editedAt: new Date(), editedBy: req.user?.id || 'doctor' } } }, buildContext(req, 'write'));
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const oid = toObjectId(req.params.id);
    if (!oid) return res.status(400).json({ error: 'Invalid ID' });
    const result = await sda.update('neurological_assessment', { _id: oid }, { $set: { [`doctorEdits.approvedSections.${sectionId}`]: { approved: true, approvedAt: new Date(), approvedBy: req.user?.id || 'doctor' } } }, buildContext(req, 'write'));
