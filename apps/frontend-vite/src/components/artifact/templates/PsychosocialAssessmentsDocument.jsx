...
const PsychosocialAssessmentsDocument = ({ document: docProp }) => {
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        if (leaves.length > 0) {
          text += `${label}\n`;
          leaves.forEach(leaf => { text += `${leaf.label}: ${fmtVal(leaf.value)}\n`; });
          text += '\n';
        }
  ...
  /* ═══════ RENDER: ARRAY FIELD (recommendations — array of objects with .recommendation & .date) ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          const itemDate = typeof item === 'object' && item?.date ? item.date : null;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; if (typeof item === 'object') { currentArr[itemIdx] = { ...currentArr[itemIdx], recommendation: editValue }; } else { currentArr[itemIdx] = editValue; } stageDraft(record, idx, fn, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <>
                    <div className="row-content">
                      ...
                      {itemDate && <div style={{ fontSize: '14px', color: '#60a5fa', marginTop: '4px' }}>Date: {highlightText(itemDate)}</div>}
                      <span className="edit-indicator">&#9998;</span>
                    </div>
                    <button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(itemDate ? `${itemStr} (${itemDate})` : itemStr, editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button>
                  </>
                )}
              </div>
              {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(isBool ? (leaf.value ? 'Yes' : 'No') : leafStr); setSaveError(null); } }}>
                {isEditing ? (
                  ...
                  <>
                    ...
                    <button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${leaf.label}: ${leafStr}`, editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button>
                  </>
                )}
              </div>
              {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
    ...
    <div className="psychosocial-assessments-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<PsychosocialAssessmentsDocumentPDFTemplate document={pdfData} />} fileName={`psychosocial-assessments-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
