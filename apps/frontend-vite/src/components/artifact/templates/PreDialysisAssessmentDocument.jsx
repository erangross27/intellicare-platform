/**
 * PreDialysisAssessmentDocument.jsx
...
const FIELD_LABELS = {
  ...
  vitaminD25OH: 'Vitamin D 25-OH',
  serum25OHD3: 'Serum 25-OH-D3',
  alkalinePhosphatase: 'Alkaline Phosphatase',
  acidosis: 'Acidosis',
  serumBicarbonate: 'Serum Bicarbonate',
  vascularAccessType: 'Vascular Access Type',
  vascularAccessMaturation: 'Vascular Access Maturation',
  fluidOverload: 'Fluid Overload',
  nyhaClass: 'NYHA Class',
  echocardiogramEF: 'Echocardiogram EF',
  hepatitisBsAg: 'Hepatitis B sAg',
  hepatitisCantibody: 'Hepatitis C Antibody',
...
const SECTION_FIELDS = {
  ...
  'acid-base': ['acidosis', 'serumBicarbonate'],
  'vascular-access': ['vascularAccessType', 'vascularAccessMaturation'],
  'cardiovascular': ['fluidOverload', 'nyhaClass', 'echocardiogramEF'],
  'infectious-disease': ['hepatitisBsAg', 'hepatitisCantibody', 'hivStatus'],
};
...
const DRAFT_KEY = 'pre_dialysis_assessmentPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const PreDialysisAssessmentDocument = ({ document: docProp }) => {
  ...
  const formatDisplayValue = useCallback((fn, val) => {
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        const displayStr = formatDisplayValue(f, val).toLowerCase();
        if (displayStr.includes(phrase)) return true;
      }
    }
    return false;
  }, [searchTerm, getFieldValue, formatDisplayValue]);
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) {
      const displayStr = formatDisplayValue(fn, val).toLowerCase();
      return displayStr.includes(phrase);
    }
    return false;
  }, [searchTerm, getFieldValue, formatDisplayValue]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pre-Dialysis Assessment Record ${idx + 1}`.toLowerCase();
  ...
  }, [records, searchTerm, getFieldValue, formatDisplayValue]);
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* Stage a DRAFT into the localStorage pending-drafts store (survives refresh). fieldPart = "field"
     (or "field.arrayIndex" for array elements). Also drops the section's approved flag so the
     button returns to yellow Pending Approve. NO DB write here — Approve commits. */
  const stageDraft = useCallback((id, fieldPart, value, sid, idx) => {
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    stageDraft(id, fn, saveVal, sid, idx);
    setEditingField(null); setEditValue('');
  }, [editValue, safeId, stageDraft]);

  // Save one sentence: stage as a DRAFT (no DB write). localStorage keeps it across refresh; Approve commits.
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageDraft(id, fn, fullText, sid, idx);
      setEditingField(null); setEditValue('');
      return;
    }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/pre_dialysis_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/pre_dialysis_assessment/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[PreDialysisAssessment] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    ...
    const isEditing = editingField === editKey;
    const label = FIELD_LABELS[fn] || fn;
    const displayVal = formatDisplayValue(fn, val);
    const isModified = editedFields[editKey];
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); const id3 = safeId(record); if (!id3) return; setSaveError(null); setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullText })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); stageDraft(id3, fn, fullText, sid, idx); setEditingField(null); setEditValue(''); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pre-dialysis-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pre-Dialysis Assessment</h2></div>
        <div className="empty-state">No pre-dialysis assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pre-dialysis-assessment-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search assessments..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'acid-base')}
            {renderSection(record, idx, 'vascular-access')}
            {renderSection(record, idx, 'cardiovascular')}
...
export default PreDialysisAssessmentDocument;
