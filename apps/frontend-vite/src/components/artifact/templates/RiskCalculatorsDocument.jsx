 ...
 * July 2026 — recursive, dot-path editable risk calculators
...
const SECTION_TITLES = {
  calculatorInformation: 'Calculator Information',
  riskResults: 'Risk Results',
  patientData: 'Patient Data',
  clinicalValues: 'Clinical Values',
  clinicalDecision: 'Clinical Decision',
  validationLimitations: 'Validation & Limitations',
};
const FIELD_LABELS = {
  calculatorName: 'Calculator Name', calculatorVersion: 'Calculator Version', calculationDate: 'Calculation Date', provider: 'Provider', facility: 'Facility', riskCategory: 'Risk Category',
  totalScore: 'Total Score', riskPercentage: 'Risk Percentage', riskClassification: 'Risk Classification', timeframe: 'Timeframe', outcomesPredicted: 'Outcomes Predicted',
  patientAge: 'Patient Age', patientSex: 'Patient Sex', comorbidities: 'Comorbidities', medications: 'Medications',
  labValues: 'Lab Values', vitalSigns: 'Vital Signs', inputParameters: 'Input Parameters',
  clinicalIndication: 'Clinical Indication', clinicalDecisionMade: 'Clinical Decision Made', recommendedInterventions: 'Recommended Interventions',
  calculationMethod: 'Calculation Method', validationStatus: 'Validation Status', limitationsNoted: 'Limitations Noted', comparisonToPopulation: 'Comparison to Population',
};
const SECTION_FIELDS = {
  calculatorInformation: ['calculatorName', 'calculatorVersion', 'calculationDate', 'provider', 'facility', 'riskCategory'],
  riskResults: ['totalScore', 'riskPercentage', 'riskClassification', 'timeframe', 'outcomesPredicted'],
  patientData: ['patientAge', 'patientSex', 'comorbidities', 'medications'],
  clinicalValues: ['labValues', 'vitalSigns', 'inputParameters'],
  clinicalDecision: ['clinicalIndication', 'clinicalDecisionMade', 'recommendedInterventions'],
  validationLimitations: ['calculationMethod', 'validationStatus', 'limitationsNoted', 'comparisonToPopulation'],
};
const DATE_FIELDS = ['calculationDate'];
const ENUM_FIELDS = ['patientSex', 'riskClassification', 'validationStatus'];
const NUMBER_FIELDS = ['totalScore', 'riskPercentage', 'patientAge'];
const NUMBER_STRING_FIELDS = ['labValues', 'vitalSigns', 'inputParameters'];
const OBJECT_FIELDS = [];
const NARRATIVE_PATHS = [];
const PARENTHETICAL_LABEL_FIELDS = [];
const COMMA_FIELDS = ['outcomesPredicted', 'comorbidities', 'recommendedInterventions'];
const COMMA_ARRAY_FIELDS = ['outcomesPredicted', 'comorbidities', 'recommendedInterventions'];
...
const SEMICOLON_FIELDS = ['medications', 'inputParameters', 'clinicalIndication', 'calculationMethod', 'limitationsNoted', 'comparisonToPopulation'];
...
const KEY_LABELS = { labValues: 'Lab Values', vitalSigns: 'Vital Signs', inputParameters: 'Input Parameters' };
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'patientSex' ? ['Female', 'Male', 'Other', 'Unknown']
    : field === 'riskClassification' ? ['Low risk', 'Borderline risk', 'Intermediate risk', 'High risk']
      : field === 'validationStatus' ? ['Pending', 'Reviewed', 'Validated', 'Not validated']
        : [];
...
const RiskCalculatorsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.risk_calculators) return Array.isArray(record.risk_calculators) ? record.risk_calculators : [record.risk_calculators];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.risk_calculators) return Array.isArray(nested.risk_calculators) ? nested.risk_calculators : [nested.risk_calculators];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/risk_calculators/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/risk_calculators/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[RiskCalculators] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Risk Calculators ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="risk-calculators-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Risk Calculators</h2></div><div className="empty-state">No risk calculators records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Risk Calculators ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="risk-calculators-document" ref={containerRef}>
    <div className="document-header">
      ...
      <div className="header-actions">
        ...
        <PDFDownloadLink document={<RiskCalculatorsDocumentPDFTemplate document={pdfData} />} fileName="Risk_Calculators.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search risk calculators..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Risk Calculators ' + (recordIndex + 1))}</h3></div>
