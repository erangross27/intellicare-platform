...
/* decimal-aware step for the −/+ num-stepper */
const stepFor = (val) => { const m = String(val).trim().match(/\.(\d+)/); return m ? Math.pow(10, -m[1].length) : 1; };
/* split "Label: value" array items so the label renders as a sub-label above a value-only row */
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                ...
                <input className="edit-number" type="text" inputMode="decimal" value={editValue} autoFocus onChange={e => setEditValue(e.target.value)} onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          const reconstruct = (v) => (parsed.isLabeled ? `${parsed.label}: ${v}` : v);
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayValue); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const newItem = reconstruct(editValue); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = newItem; const ek = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [ek]: currentArr })); setPendingEdits(prev => ({ ...prev, [ek]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const store = readDrafts(); if (!store[id]) store[id] = {}; store[id][`${fn}.${itemIdx}`] = { fieldPart: fn, kind: 'array', value: currentArr, markerState: 'edited', arrayMarker: itemIdx, db: { field: fn, value: newItem, arrayIndex: itemIdx } }; /* keep sibling array-item entries' whole-array snapshot in sync */ Object.keys(store[id]).forEach(k => { if (store[id][k] && store[id][k].kind === 'array' && store[id][k].fieldPart === fn) store[id][k].value = currentArr; }); writeDrafts(store); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
    ...
    <div className="preventive-medicine-assessments-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<PreventiveMedicineAssessmentsDocumentPDFTemplate document={pdfData} />} fileName="Preventive_Medicine_Assessments.pdf" className="copy-btn">
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Preventive Medicine Assessment ${idx + 1}`)}</h3>
            </div>
