 ...
 * July 2026 — recursive, dot-path editable risk counseling
...
const SECTION_TITLES = {
  cardiovascularMetabolic: 'Cardiovascular & Metabolic Risk',
  organSystems: 'Organ Systems Risk',
  screeningSusceptibility: 'Screening & Susceptibility',
  lifestyleGenetics: 'Lifestyle & Genetic Factors',
};
const FIELD_LABELS = {
...
const SECTION_FIELDS = {
  cardiovascularMetabolic: ['cardiovascularRiskScore', 'bleedingRiskScore', 'osteoporosisFractureRisk', 'hypertensionTargetOrganDamage', 'diabeticRetinopathyRisk', 'diabeticNephropathyRisk'],
  organSystems: ['fallRiskAssessment', 'thromboembolismRisk', 'perioperativeRiskIndex', 'renalDiseaseProgression', 'obesityComplicationRisk'],
  screeningSusceptibility: ['cancerScreeningRisk', 'strokeRiskFactors', 'infectiousDiseaseSusceptibility', 'polypharmacyRisk', 'cognitiveDeclineRisk', 'pregnancyComplicationRisk'],
  lifestyleGenetics: ['lifestyleModificationTargets', 'geneticPredispositionFactors', 'anticoagulationRiskBenefit'],
};
const DATE_FIELDS = [];
const ENUM_FIELDS = [];
...
const RiskCounselingDocument = ({ document: docProp }) => {
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/risk_counseling/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/risk_counseling/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[RiskCounseling] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Risk Counseling ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="risk-counseling-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Risk Counseling</h2></div><div className="empty-state">No risk counseling records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Risk Counseling ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="risk-counseling-document" ref={containerRef}>
    <div className="document-header">
      ...
      <div className="header-actions">
        ...
        <PDFDownloadLink document={<RiskCounselingDocumentPDFTemplate document={pdfData} />} fileName="Risk_Counseling.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search risk counseling..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Risk Counseling ' + (recordIndex + 1))}</h3></div>
