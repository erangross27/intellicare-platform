      ...
      subscribeWithSelector((set, get) => ({
        ...
        startWorkflow: (workflow) => {
          ...
          if (window.socketService) {
            window.socketService.emit('workflow:start', { 
        ...
        advanceStep: () => {
          ...
          if (currentStep < activeWorkflow.steps.length - 1) {
            ...
            // Emit to backend
            if (window.socketService) {
              window.socketService.emit('workflow:advance', {
        ...
        completeWorkflow: () => {
          ...
          // Emit to backend
          if (window.socketService) {
            window.socketService.emit('workflow:complete', {
              workflowId: activeWorkflow.id,
              data: stepData
            });
          }
        ...
        cancelWorkflow: () => {
          ...
          if (activeWorkflow) {
            ...
            if (window.socketService) {
              window.socketService.emit('workflow:cancel', {
                workflowId: activeWorkflow.id
              });
            }
          }
        ...
        restoreSession: () => {
          ...
          if (activeWorkflow && window.socketService) {
            console.log('🔄 Restoring workflow session');
            window.socketService.emit('workflow:restore', {
              workflowId: activeWorkflow.id,
