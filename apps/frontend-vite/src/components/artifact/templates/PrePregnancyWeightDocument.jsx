/**
 * PrePregnancyWeightDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: pre_pregnancy_weight
 *
 * 4 Sections:
 *   1. measurements: prePregnancyWeight, prePregnancyBmi, bmiCategory, heightMeasurement, weightMeasurementMethod, obesityClass
 *   2. weight-history: weightStability, weightLossHistory, previousPregnancyWeightGain, gestationalWeightGainGoal
 *   3. risk-factors: metabolicRiskFactors, insulinResistanceMarkers, prePregnancyA1c, thyroidFunction, cardiovascularRisk, sleepApneaRisk, nutritionalDeficiencies
 *   4. interventions: nutritionalCounseling, bariatricSurgeryHistory, exerciseTolerance, eatingDisorderHistory, contraceptiveWeightEffect
 */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldName]: value } }  (fieldName = the whole field value, field-level) */
const DRAFT_KEY = 'pre_pregnancy_weightPendingEdits';
const readDrafts = () => {
...
/* MEANINGFUL_ZERO_FIELDS: numeric fields where 0 is a real clinical value. For pre-pregnancy
   weight/BMI/height/A1c/weight-gain, 0 is physiologically impossible → it is a sentinel/missing
   marker and must be hidden, not rendered as "0". (Empty allowlist = hide 0 for all numerics.) */
...
const STRING_FIELDS = ['bmiCategory', 'weightMeasurementMethod', 'obesityClass', 'weightLossHistory', 'gestationalWeightGainGoal', 'insulinResistanceMarkers', 'thyroidFunction', 'cardiovascularRisk', 'exerciseTolerance', 'contraceptiveWeightEffect'];
...
const PrePregnancyWeightDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.pre_pregnancy_weight) return Array.isArray(r.pre_pregnancy_weight) ? r.pre_pregnancy_weight : [r.pre_pregnancy_weight];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.pre_pregnancy_weight) return Array.isArray(dd.pre_pregnancy_weight) ? dd.pre_pregnancy_weight : [dd.pre_pregnancy_weight]; return [dd]; }
      return [r];
    });
  ...
  /* numberShows: hide numeric "not recorded" sentinel (0) unless the field is a meaningful-zero
     field or the value was doctor-edited. Prevents A1c=0 / weight=0 sentinels rendering as "0". */
  ...
  /* stageLocalEdit — stage a field-level DRAFT locally + persist to the pending-drafts localStorage
     store (survives refresh). NOT written to MongoDB and NOT shown in the PDF until Pending Approve.
     This is the single place that records a pending edit + clears the section's approved flag. */
  const stageLocalEdit = useCallback((record, fn, idx, sid, fullValue) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pre-Pregnancy Weight ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Save = stage a DRAFT locally + localStorage (survives refresh). Commits to DB only on Approve.
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    const id = safeId(record); if (!id) return;
    const saveVal = valueOverride !== undefined ? valueOverride : editValue;
    setSaveError(null);
    stageLocalEdit(record, fn, idx, sid, saveVal);
    const trackKey = editTrackingKey || `${fn}-${idx}`;
    setEditedFields(prev => ({ ...prev, [trackKey]: 'edited' }));
    setEditingField(null); setEditValue('');
  }, [editValue, safeId, stageLocalEdit]);

  // saveSentence = stage a DRAFT (no DB write). localStorage keeps it across refresh; Approve commits.
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageLocalEdit(record, fn, idx, sid, fullText);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // localEdits keys here are field-level ("field-idx"); commit each pending one for this section.
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/pre_pregnancy_weight/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/pre_pregnancy_weight/${id}/approve`, { sectionId: sid, approved: true });
      ...
      const store = readDrafts();
      if (store[id]) {
        fields.forEach(f => { if (store[id] && f in store[id]) delete store[id][f]; });
    ...
    } catch (err) { console.error('[PrePregnancyWeight] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const copyAllText = useCallback(async () => {
    ...
    pdfData.forEach((r, idx) => {
      text += `Pre-Pregnancy Weight ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { currentArr.splice(itemIdx, 1, ...subParts); } else { currentArr[itemIdx] = trimmed; } stageLocalEdit(record, fn, idx, sid, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pre-pregnancy-weight-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pre-Pregnancy Weight</h2></div>
        <div className="empty-state">No pre-pregnancy weight records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pre-pregnancy-weight-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pre-pregnancy weight records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'weight-history')}
...
export default PrePregnancyWeightDocument;
