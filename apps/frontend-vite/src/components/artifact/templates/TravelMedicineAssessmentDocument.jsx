/**
 * TravelMedicineAssessmentDocument.jsx
 * June 2026 — Collection: travel_medicine_assessment (NEW template)
 *
 * Mirrors MyositisAssessmentDocument (date field, per-sentence string editing on [.;],
 ...
 *   BOOLEAN (Yes/No, render false as "No"): ruralExposure, yellowFeverVaccineGiven,
 *     certificateOfVaccinationIssued, altitudeSicknessRisk, foodWaterSafetyEducation,
 *     immunocompromisedStatus
 *   STRING ARRAY (numbered editable rows): destinationCountries, vaccinesAdministered,
 *     preExistingConditions, exposureRisks
 *   STRING (per-sentence): purposeOfTravel, accommodationType, malariaProphylaxisPrescribed,
 *     malariaRiskLevel, travelersDiarrheaProphylaxis, insectBorneDiseasePrecautions,
 *     pregnancyStatus, previousTravelHistory, travelInsuranceStatus, emergencyContactAbroad
 */
...
const DRAFT_KEY = 'travel_medicine_assessmentPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const TravelMedicineAssessmentDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.travel_medicine_assessment) return Array.isArray(r.travel_medicine_assessment) ? r.travel_medicine_assessment : [r.travel_medicine_assessment];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.travel_medicine_assessment) return Array.isArray(dd.travel_medicine_assessment) ? dd.travel_medicine_assessment : [dd.travel_medicine_assessment]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `travel medicine assessment ${idx + 1}`;
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    } catch (err) { console.error('[TravelMedicineAssessment] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== TRAVEL MEDICINE ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Travel Medicine Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="travel-medicine-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Travel Medicine Assessment</h2></div>
        <div className="empty-state">No travel medicine assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="travel-medicine-assessment-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search travel medicine assessment records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
...
export default TravelMedicineAssessmentDocument;
