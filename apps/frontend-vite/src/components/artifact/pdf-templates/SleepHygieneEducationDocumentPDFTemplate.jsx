...
const SECTIONS = [
  { id: 'recordInfo', title: 'Record Information', fields: ['date', 'primarySleepComplaint', 'epworthSleepinessScore'] },
  { id: 'sleepMetrics', title: 'Sleep Metrics', fields: ['averageSleepLatency', 'totalSleepTime', 'nighttimeAwakenings', 'screenTimeBeforeBed'] },
  { id: 'sleepSchedule', title: 'Sleep Schedule', fields: ['regularBedtime', 'regularWakeTime', 'weekendSleepSchedule', 'bedtimeRoutine'] },
  { id: 'lifestyleFactors', title: 'Lifestyle Factors', fields: ['caffeineIntake', 'alcoholConsumption', 'nicotineUse', 'exerciseHabits', 'dinnertimeHabits', 'nappingFrequency'] },
  { id: 'sleepEnvironment', title: 'Sleep Environment', fields: ['bedroomEnvironment'] },
  { id: 'educationInterventions', title: 'Education & Interventions', fields: ['educationTopicsCovered', 'behavioralInterventions', 'stimulusControlInstructions', 'sleepRestrictionTherapy'] },
  { id: 'techniquesMaterials', title: 'Techniques & Materials', fields: ['relaxationTechniques', 'educationalMaterials'] },
  { id: 'followUp', title: 'Follow-Up', fields: ['followUpInterval', 'sleepDiaryRequested'] },
];
const FIELD_LABELS = {
...
export default function SleepHygieneEducationDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Sleep Hygiene Education</Text></View>
    {!records.length && <Text style={styles.noDataText}>No sleep hygiene education data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Sleep Hygiene Education Record {recordIndex + 1}</Text></View>
