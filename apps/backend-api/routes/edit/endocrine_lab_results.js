...
function buildContext(req, operation = 'read') { return { serviceId: 'endocrine-lab-results-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['date', 'provider', 'facility', 'thyroidStimulatingHormone', 'freeThyroxineT4', 'freeTriiodothyronineT3', 'hemoglobinA1c', 'fastingGlucose', 'cortisol', 'cortisolCollectionTime', 'adrenocorticotropicHormone', 'parathyroidHormone', 'serumCalcium', 'ionizedCalcium', 'vitamin25OHD', 'prolactin', 'luteinizingHormone', 'follicleStimulatingHormone', 'testosterone', 'estradiol', 'insulinLikeGrowthFactor1', 'serumOsmolality', 'thyroidPeroxidaseAntibody', 'thyroglobulinAntibody', 'cPeptide'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('endocrine_lab_results', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[EndocrineLabResults] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('endocrine_lab_results', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[EndocrineLabResults] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
