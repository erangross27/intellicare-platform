/**
 * CpapManagementDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: cpap_management (displayed as "CPAP Management")
 *
 * 6 Sections:
 *   1. record-info: date, cpapMode, epworthSleepinessScore
 *   2. pressure-settings: cpapPressureSetting, inspiratoryPressure, expiratoryPressure, pressureRangeMin, pressureRangeMax, rampTime, rampStartPressure, humidifierSetting
 *   3. compliance-data: averageUsageHoursPerNight, adherencePercentage, residualAhiOnCpap, complianceDownloadDate, therapeuticPressureAchieved
 *   4. apnea-indices: centralApneaIndex, obstructiveApneaIndex, maskLeakRate
 *   5. mask-equipment: maskType, maskSize, maskBrand, machineManufacturer, machineSerialNumber, nextMaskReplacement
 *   6. side-effects: sideEffectsReported
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import CpapManagementDocumentPDFTemplate from '../pdf-templates/CpapManagementDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import './CpapManagementDocument.css';
...
const DRAFT_KEY = 'cpapManagementPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  expiratoryPressure: 'Expiratory Pressure',
  pressureRangeMin: 'Pressure Range Min',
  pressureRangeMax: 'Pressure Range Max',
  rampTime: 'Ramp Time',
  rampStartPressure: 'Ramp Start Pressure',
  humidifierSetting: 'Humidifier Setting',
  averageUsageHoursPerNight: 'Average Usage Hours Per Night',
  adherencePercentage: 'Adherence Percentage',
  residualAhiOnCpap: 'Residual AHI On CPAP',
  complianceDownloadDate: 'Compliance Download Date',
  therapeuticPressureAchieved: 'Therapeutic Pressure Achieved',
  ...
  nextMaskReplacement: 'Next Mask Replacement',
...
const STRING_FIELDS = ['cpapMode', 'maskType', 'maskSize', 'maskBrand', 'machineSerialNumber', 'machineManufacturer', 'nextMaskReplacement'];
/* Numeric 0 = "not set" sentinel for batch-extracted CPAP fields → HIDE, EXCEPT the residual-event/measurement
   fields where 0 is a real therapeutic result (no residual AHI / no central-or-obstructive apneas / a measured leak).
   Mirrors the cpap_bipap_management twin's MEANINGFUL_ZERO_FIELDS. */
const MEANINGFUL_ZERO_FIELDS = new Set(['residualAhiOnCpap', 'maskLeakRate', 'centralApneaIndex', 'obstructiveApneaIndex']);
/* Fixed-choice fields → dropdown. */
const ENUM_FIELDS = {
  cpapMode: ['CPAP', 'APAP', 'BiPAP', 'ASV'],
  maskType: ['nasal mask', 'nasal pillows', 'full face mask', 'oral mask', 'hybrid mask', 'total face mask'],
  maskSize: ['Small', 'Medium', 'Large', 'Wide', 'Small-Wide'],
};
...
/* splitByComma: paren-aware; keep Oxford ", and/or X"; skip no-space commas ("$18,000") and date commas. */
...
/* ═══════ COMPONENT ═══════ */
const CpapManagementDocument = ({ document: docProp }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      if (!recDrafts) return;
      // Group array-element drafts by base field so localEdits holds the full array
      const arrayAccum = {};
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayElem) {
          ...
          if (!arrayAccum[baseField]) {
            const orig = record[baseField];
            arrayAccum[baseField] = Array.isArray(orig) ? [...orig] : [];
          }
          arrayAccum[baseField][arrIndex] = value;
      ...
      Object.entries(arrayAccum).forEach(([baseField, arr]) => {
  ...
  /* fieldShows: for a NUMBER_FIELD not in MEANINGFUL_ZERO_FIELDS, a value of 0 is an extraction-default sentinel → hidden.
     A doctor-committed 0 (record.doctorEdits.editedFields) or a 0 edited this session always shows. Non-number → hasVal. */
  const fieldShows = useCallback((record, fn, idx) => {
    const k = `${fn}-${idx}`;
    const v = localEdits[k] !== undefined ? localEdits[k] : record[fn];
    if (typeof v === 'number' && v === 0 && NUMBER_FIELDS.includes(fn) && !MEANINGFUL_ZERO_FIELDS.has(fn)) {
    ...
    return hasVal(v);
  }, [localEdits, editedFields, hasVal]);

  /* Canonical: splits on '.' AND ';' with the abbreviation+decimal guard ("2.8" and "N20" never break). */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `CPAP Management ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    setApproving(true); setSaveError(null);
    try {
      // Drafts for this record drive the PUTs: fieldPart is "field" or "field.arrayIndex"
      const store = readDrafts();
      const recDrafts = store[id] || {};
      const fieldPartsToCommit = Object.keys(recDrafts).filter(fp => {
        const baseField = fp.includes('.') ? fp.slice(0, fp.indexOf('.')) : fp;
        return fields.includes(baseField);
      });
      for (const fieldPart of fieldPartsToCommit) {
        ...
        await secureApiClient.put(`/api/edit/cpap_management/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/cpap_management/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (store[id]) {
        fieldPartsToCommit.forEach(fp => { delete store[id][fp]; });
    ...
    } catch (err) { console.error(err); setSaveError('Save failed.'); } finally { setApproving(false); }
  }, [safeId]);
  ...
  /* EQ/DASH numbered lines for one field — the exact mirror of the PDF fieldLines.
     `shows` gates numeric-0 hide against the rendered record (draft-aware in JSX; plain-record in Copy All). */
  const buildFieldLines = useCallback((record, f, idx, showsFn) => {
    ...
    if (ARRAY_FIELDS.includes(f)) {
      ...
      if (!hasVal(val)) return lines;
      lines.push(label, COPY_LINE_DASH, `1. ${val ? 'Yes' : 'No'}`, '');
    } else if (NUMBER_FIELDS.includes(f)) {
      if (!showsFn(f)) return lines;
      ...
      if (sentences.length <= 1) {
        lines.push(label, COPY_LINE_DASH, `1. ${strVal}`, '');
      } else {
        lines.push(label, COPY_LINE_DASH);
        let n = 0;
        sentences.forEach(s => {
          const p = parseLabel(s);
          if (p.isLabeled) { const ci = splitByComma(p.value); lines.push(p.label, COPY_LINE_DASH); n = 0; if (ci.length >= 3) ci.forEach(c => lines.push(`${++n}. ${c}`)); else lines.push(`${++n}. ${p.value}`); }
    ...
    return lines;
  }, [getFieldValue, hasVal, fmtVal, formatDate, splitBySentence, splitByComma]);

  const buildSectionCopyText = useCallback((record, idx, sid) => {
    const title = SECTION_TITLES[sid];
    const showsFn = (f) => fieldShows(record, f, idx);
    const lines = (SECTION_FIELDS[sid] || []).flatMap(f => buildFieldLines(record, f, idx, showsFn));
    if (lines.length === 0) return '';
    return `${title}\n${COPY_LINE_EQ}\n\n${lines.join('\n')}\n`;
  }, [buildFieldLines, fieldShows]);

  const copyAllText = useCallback(async () => {
    let text = '=== CPAP MANAGEMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `CPAP Management ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD (−/+ stepper; sentinel-0 hidden) ═══════ */
  const renderNumberField = (record, fn, idx, sid) => {
    if (!fieldShows(record, fn, idx)) return null;   // hide extraction-default 0 for sentinel numeric fields
    const val = getFieldValue(record, fn, idx);
    const editKey = `${fn}-${idx}`;
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                <button type="button" className="num-step" onClick={e => { e.stopPropagation(); bump(-1); }}>&minus;</button>
                <input type="number" step={stepFor(editValue)} className="edit-number" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter') { e.stopPropagation(); const numVal = parseFloat(editValue); if (editValue.trim() === '' || isNaN(numVal)) { setSaveError('Please enter a valid number'); return; } handleSaveField(record, fn, idx, sid, null, numVal); } }} />
  ...
  /* ═══════ RENDER: STRING FIELD (ENUM dropdown, else splitBySentence) ═══════ */
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* Fixed-choice field → dropdown editor */
    ...
    if (enumOpts) {
      ...
      return (
        <div key={fn} className="rec-mini-card">
          {showLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
          <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); const canon = enumOpts.find(o => o.toLowerCase() === strVal.trim().toLowerCase()); setEditValue(canon || strVal); setSaveError(null); } }}>
            {isEditing ? (
              <div className="edit-field-container">
                <select className="edit-select" value={editValue} autoFocus onChange={e => setEditValue(e.target.value)} onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }}>{enumOptionsWith(enumOpts, editValue).map(o => <option key={o} value={o}>{o}</option>)}</select>
                {saveError && <div className="save-error">{saveError}</div>}
                <div className="edit-actions">
    ...
    /* Single-value string: uses saveSentence with sentenceIdx=0 */
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="cpap-management-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">CPAP Management</h2></div>
        <div className="empty-state">No CPAP management records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="cpap-management-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">CPAP Management</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<CpapManagementDocumentPDFTemplate document={pdfData} />} fileName="CPAP_Management.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search CPAP management records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`CPAP Management ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'record-info')}
            {renderSection(record, idx, 'pressure-settings')}
            {renderSection(record, idx, 'compliance-data')}
            {renderSection(record, idx, 'apnea-indices')}
...
export default CpapManagementDocument;
