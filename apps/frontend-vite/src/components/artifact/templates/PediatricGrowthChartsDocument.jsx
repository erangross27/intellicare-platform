/**
 * PediatricGrowthChartsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: pediatric_growth_charts
 *
 * 4 Sections:
 *   1. assessment-info: assessmentDate (date), programType (string)
 *   2. growth-measurements: findings (string)
 *   3. progress-goals: progress (string), goals (string)
 *   4. recommendations-followup: recommendations (string), followUp (string)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PediatricGrowthChartsDocumentPDFTemplate from '../pdf-templates/PediatricGrowthChartsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './PediatricGrowthChartsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } } */
const DRAFT_KEY = 'pediatric_growth_chartsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'progress-goals': 'Progress and Goals',
  'recommendations-followup': 'Recommendations and Follow-Up',
};
...
/* splitByComma: parenthesis-aware comma split (thousands-safe) */
...
/* ═══════ COMPONENT ═══════ */
const PediatricGrowthChartsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.pediatric_growth_charts) return Array.isArray(r.pediatric_growth_charts) ? r.pediatric_growth_charts : [r.pediatric_growth_charts];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.pediatric_growth_charts) return Array.isArray(dd.pediatric_growth_charts) ? dd.pediatric_growth_charts : [dd.pediatric_growth_charts]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pediatric Growth Chart ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/pediatric_growth_charts/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/pediatric_growth_charts/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PEDIATRIC GROWTH CHARTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Pediatric Growth Chart ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pediatric-growth-charts-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pediatric Growth Charts</h2></div>
        <div className="empty-state">No pediatric growth chart records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pediatric-growth-charts-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Pediatric Growth Charts</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PediatricGrowthChartsDocumentPDFTemplate document={pdfData} />} fileName="Pediatric_Growth_Charts.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search growth charts..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Pediatric Growth Chart ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'assessment-info')}
            {renderSection(record, idx, 'growth-measurements')}
...
export default PediatricGrowthChartsDocument;
