/**
 * Medication Action Plan Edit Route
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'medication-action-plan-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'polypharmacyRiskScore',
  'medicationAdherenceScore',
  'beersListMedicationCount',
  'anticholinergicBurdenScore',
  'fallsRiskMedicationCount',
  'has2BledScore',
  'cha2ds2VascScore',
  'opioidMorphineEquivalentDailyDose',
  'renalDoseAdjustmentRequired',
  'hepaticDoseAdjustmentRequired',
  'nextTherapeuticDrugMonitoringDate',
  'medicationReconciliationStatus',
  'qtProlongationRiskCategory',
  'sedationRiskAssessment',
  'targetInrRange',
  'insulinSlidingScaleParameters',
  'medicationTaperSchedule',
...
/* Allow-listed subfields per object-array field (array-of-objects per extraction prompt). */
const ALLOWED_SUBFIELDS = {
  drugDrugInteractionAlerts: ['severity', 'drugs', 'effect', 'recommendation'],
  therapeuticDuplicationFlags: ['class', 'medications', 'recommendation'],
  therapeuticSubstitutionOptions: ['nonFormulary', 'alternative', 'rationale'],
  adverseDrugReactionHistory: ['drug', 'reaction', 'naranjoScore', 'causality', 'year'],
  deprescribingPriorityList: ['medication', 'priority', 'rationale'],
};
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('medication_action_plan', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[medication_action_plan] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('medication_action_plan', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[medication_action_plan] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
