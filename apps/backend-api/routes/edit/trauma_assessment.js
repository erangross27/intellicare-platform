...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'trauma-assessment-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'injurySeverityScore',
  'primarySurveyAirway',
  'primarySurveyBreathing',
  'primarySurveyCirculation',
  'primarySurveyDisability',
  'primarySurveyExposure',
  'injuredBodyRegions',
  'hemorrhageClass',
  'fastExamResult',
  'penetratingInjuryLocation',
  'burnTotalBodySurfaceArea',
  'compartmentSyndromeRisk',
  'cervicalSpineImmobilization',
  'pelvicBinderApplied',
  'massiveTransfusionProtocol',
  'emergentInterventionsPerformed',
  'consultingServicesActivated',
  'traumaBayArrivalTime',
  'timeToCtScan',
  'traumaDisposition',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('trauma_assessment', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[TraumaAssessment] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('trauma_assessment', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[TraumaAssessment] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
