/**
 * InjuryDetailsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: injury_details
 *
 * 5 Sections:
 *   1. session-info: date (date picker), provider (string), facility (string), status (string)
 *   2. incident-info: mechanism (long), timeOfInjury, locationOfIncident, speed, height
 *   3. response-info: lossOfConsciousness (boolean), protectiveEquipment (array), ambulanceArrival, fieldTreatment (array)
 *   4. clinical-findings: findings (long), assessment (long), plan, recommendations (object-array {recommendation,date}), results (object), notes (long)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import InjuryDetailsDocumentPDFTemplate from '../pdf-templates/InjuryDetailsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './InjuryDetailsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits field, e.g. "findings") */
const DRAFT_KEY = 'injury_detailsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['timeOfInjury', 'locationOfIncident', 'speed', 'height', 'ambulanceArrival', 'provider', 'facility', 'plan', 'status'];

/* ═══════ OBJECT HELPERS (results) ═══════ */
const KEY_OVERRIDES = {};
...
/* objectToLines: recursive humanized key/value lines for Copy (indented) */
const objectToLines = (value, depth) => {
  if (isEmptyDeep(value)) return [];
  const pad = '  '.repeat(depth);
  if (isScalar(value)) return [`${pad}${fmtScalar(value)}`];
  const lines = [];
  Object.entries(value).filter(([, v]) => !isEmptyDeep(v)).forEach(([k, v]) => {
    if (isScalar(v)) { lines.push(`${pad}${humanizeKey(k)}: ${fmtScalar(v)}`); }
    else { lines.push(`${pad}${humanizeKey(k)}`); lines.push(...objectToLines(v, depth + 1)); }
  });
  return lines;
};
/* flattenSearchable: deep text of an object/array for search matching */
const flattenSearchable = (value) => {
  if (isEmptyDeep(value)) return '';
  if (isScalar(value)) return fmtScalar(value);
  if (Array.isArray(value)) return value.map(flattenSearchable).join(' ');
  return Object.entries(value).map(([k, v]) => `${humanizeKey(k)} ${flattenSearchable(v)}`).join(' ');
};
...
/* splitByComma: parenthesis-aware comma split (thousands-separator guard: comma only splits when followed by whitespace) */
...
/* ═══════ COMPONENT ═══════ */
const InjuryDetailsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.injury_details) return Array.isArray(r.injury_details) ? r.injury_details : [r.injury_details];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.injury_details) return Array.isArray(dd.injury_details) ? dd.injury_details : [dd.injury_details]; return [dd]; }
      return [r];
    });
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (ARRAY_FIELDS.includes(f)) {
        ...
        for (const item of arr) { if (String(item).toLowerCase().includes(phrase)) return true; }
      } else if (OBJECT_ARRAY_FIELDS.includes(f) || OBJECT_FIELDS.includes(f)) {
        if (flattenSearchable(getFieldValue(record, f, 0)).toLowerCase().includes(phrase)) return true;
      } else {
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (ARRAY_FIELDS.includes(fn)) {
      ...
      for (const item of arr) { if (String(item).toLowerCase().includes(phrase)) return true; }
    } else if (OBJECT_ARRAY_FIELDS.includes(fn) || OBJECT_FIELDS.includes(fn)) {
      return flattenSearchable(getFieldValue(record, fn, idx)).toLowerCase().includes(phrase);
    } else {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Injury Details ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          if (ARRAY_FIELDS.includes(f)) {
            ...
            if (flattenSearchable(getFieldValue(record, f, idx)).toLowerCase().includes(phrase)) return true;
          } else {
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* Resolve which sectionId owns a field (for clearing the per-section approve key on re-edit). */
  const findSid = useCallback((fn) => {
    for (const sid of Object.keys(SECTION_FIELDS)) {
      if (SECTION_FIELDS[sid].includes(fn)) return sid;
    }
    return null;
  }, []);
  ...
  /* Save = stage a DRAFT locally + persist to the pending-drafts localStorage store (survives refresh).
     ...
     commits). `fn` is the localEdits field part; `value` is the full merged field value. */
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    // Build the merged rootField object with this leaf updated, then stage the whole object as a draft.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const suffix = `-${idx}`;
      // This section's pending edits, keyed `${field}-${idx}` (field part has no embedded numeric dot-suffix here).
      ...
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "field" (no numeric arrayIndex suffix in this template)
        ...
        const resp = await secureApiClient.put(`/api/edit/injury_details/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/injury_details/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[InjuryDetails] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_ARRAY_FIELDS.includes(f)) {
        ...
        recs.forEach(r => {
          const rec = (r && typeof r === 'object') ? r : { recommendation: String(r ?? ''), date: '' };
      ...
      if (OBJECT_FIELDS.includes(f)) {
        ...
        const lines = objectToLines(val, 0);
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== INJURY DETAILS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Injury Details ${idx + 1}\n${'='.repeat(40)}\n\n`;
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayVal); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              ...
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveBooleanField(record, fn, idx); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
            ...
            if (parsed.isLabeled) {
              return (
                <div key={ai} className="rec-mini-card" style={{ marginTop: ai > 0 ? 8 : 0 }}>
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(parsed.value); setSaveError(null); } }}>
                    {isEditing ? (<div className="edit-field-container"><textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />{saveError && <div className="save-error">{saveError}</div>}<div className="edit-actions"><button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const fullVal = `${parsed.label}: ${editValue.trim()}`; const id = safeId(record); if (!id) return; setSaveError(null); const a = [...(getEffectiveArray(record, fn, idx))]; a[ai] = fullVal; stageDraft(record, fn, idx, a); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button><button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button></div></div>
                    ) : (<><div className="row-content"><span className="content-value">{highlightText(parsed.value)}</span><span className="edit-indicator">&#9998;</span></div><button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(itemStr, editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button></>)}
                  </div>
  ...
  const renderStringField = (record, fn, idx, sid, title) => {
    ...
    /* Multi-sentence for SENTENCE_FIELDS */
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; number+unit/ratio -> number input, bool -> Yes/No, text -> textarea) ═══════ */
  ...
  const renderRecommendationsField = (record, fn, idx, sid) => {
    ...
    /* Tolerate string-array as well as object-array {recommendation,date} */
    const norm = recs.map(r => (r && typeof r === 'object') ? r : { recommendation: String(r ?? ''), date: '' });

    const groups = [];
    norm.forEach((rec, rIdx) => {
      const d = (rec?.date || '').trim();
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              const newArr = currentArr.map((r, i) => {
                                if (i !== rIdx) return (r && typeof r === 'object') ? { ...r } : r;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="injury-details-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Injury Details</h2></div>
        <div className="empty-state">No injury details records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="injury-details-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Injury Details</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<InjuryDetailsDocumentPDFTemplate document={pdfData} />} fileName="Injury_Details.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search injury details..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Injury Details ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'incident-info')}
            {renderSection(record, idx, 'response-info')}
...
export default InjuryDetailsDocument;
