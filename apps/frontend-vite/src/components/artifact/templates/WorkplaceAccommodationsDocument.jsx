/**
 * WorkplaceAccommodationsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: workplace_accommodations (displayed as "Workplace Accommodations")
 *
 * 7 Sections:
 *   1. record-info: accommodationRequestDate, adaQualifyingDisability, icd10DiagnosisCodes, accommodationDurationMonths
 *   2. functional-assessment: functionalCapacityEvaluation, liftingCapacityKg, prolongedStandingLimitations, ergonomicWorkstationRequired
 *   3. sensory-cognitive: visualAcuityCorrection, hearingAssistiveTechnology, cognitiveAccommodations
 *   4. medical-conditions: neurologicalImpairments, cardiovascularRestrictions, pulmonaryFunctionLimitations, diabeticManagementNeeds, seizureDisorderPrecautions, chemicalSensitivities
 *   5. medication-mobility: medicationSideEffects, mobilityAssistiveDevices
 *   6. psychological: psyFunctionalAssessment
 *   7. return-to-work: returnToWorkProgression, occupationalTherapyRecommendations
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import WorkplaceAccommodationsDocumentPDFTemplate from '../pdf-templates/WorkplaceAccommodationsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './WorkplaceAccommodationsDocument.css';
...
const SECTION_TITLES = {
  ...
  'medication-mobility': 'Medication & Mobility',
  'psychological': 'Psychological',
  'return-to-work': 'Return to Work',
};

const FIELD_LABELS = {
  accommodationRequestDate: 'Accommodation Request Date',
  adaQualifyingDisability: 'ADA Qualifying Disability',
  icd10DiagnosisCodes: 'ICD-10 Diagnosis Codes',
  accommodationDurationMonths: 'Accommodation Duration (Months)',
  functionalCapacityEvaluation: 'Functional Capacity Evaluation',
  liftingCapacityKg: 'Lifting Capacity (kg)',
  prolongedStandingLimitations: 'Prolonged Standing Limitations',
  ergonomicWorkstationRequired: 'Ergonomic Workstation Required',
  visualAcuityCorrection: 'Visual Acuity Correction',
  hearingAssistiveTechnology: 'Hearing Assistive Technology',
  cognitiveAccommodations: 'Cognitive Accommodations',
  neurologicalImpairments: 'Neurological Impairments',
  cardiovascularRestrictions: 'Cardiovascular Restrictions',
  pulmonaryFunctionLimitations: 'Pulmonary Function Limitations',
  diabeticManagementNeeds: 'Diabetic Management Needs',
  seizureDisorderPrecautions: 'Seizure Disorder Precautions',
  chemicalSensitivities: 'Chemical Sensitivities',
  medicationSideEffects: 'Medication Side Effects',
  mobilityAssistiveDevices: 'Mobility Assistive Devices',
  psyFunctionalAssessment: 'Psychological Functional Assessment',
  returnToWorkProgression: 'Return to Work Progression',
  occupationalTherapyRecommendations: 'Occupational Therapy Recommendations',
};

const SECTION_FIELDS = {
  'record-info': ['accommodationRequestDate', 'adaQualifyingDisability', 'icd10DiagnosisCodes', 'accommodationDurationMonths'],
  'functional-assessment': ['functionalCapacityEvaluation', 'liftingCapacityKg', 'prolongedStandingLimitations', 'ergonomicWorkstationRequired'],
  'sensory-cognitive': ['visualAcuityCorrection', 'hearingAssistiveTechnology', 'cognitiveAccommodations'],
  'medical-conditions': ['neurologicalImpairments', 'cardiovascularRestrictions', 'pulmonaryFunctionLimitations', 'diabeticManagementNeeds', 'seizureDisorderPrecautions', 'chemicalSensitivities'],
  'medication-mobility': ['medicationSideEffects', 'mobilityAssistiveDevices'],
  'psychological': ['psyFunctionalAssessment'],
  'return-to-work': ['returnToWorkProgression', 'occupationalTherapyRecommendations'],
};

const NUMBER_FIELDS = ['liftingCapacityKg', 'accommodationDurationMonths'];
const DATE_FIELDS = ['accommodationRequestDate'];
const BOOLEAN_FIELDS = ['adaQualifyingDisability', 'ergonomicWorkstationRequired'];
const ARRAY_FIELDS = ['icd10DiagnosisCodes', 'cognitiveAccommodations', 'medicationSideEffects', 'mobilityAssistiveDevices'];
const STRING_FIELDS = ['functionalCapacityEvaluation', 'prolongedStandingLimitations', 'visualAcuityCorrection', 'hearingAssistiveTechnology', 'neurologicalImpairments', 'cardiovascularRestrictions', 'pulmonaryFunctionLimitations', 'diabeticManagementNeeds', 'chemicalSensitivities', 'seizureDisorderPrecautions', 'psyFunctionalAssessment', 'returnToWorkProgression', 'occupationalTherapyRecommendations'];
...
const DRAFT_KEY = 'workplaceAccommodationsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const WorkplaceAccommodationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.workplace_accommodations) return Array.isArray(dd.workplace_accommodations) ? dd.workplace_accommodations : [dd.workplace_accommodations]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // fieldPart is "field" or "field.arrayIndex"; localEdits is always keyed "field-idx"
        ...
        if (isArrayItem) {
          ...
          nFields[`${fieldName}.${arrayIndex}-${idx}`] = 'edited';
        } else {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Workplace Accommodations ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      // Persist each staged field to the DB now. localEdits keys are "field-idx" (no dotted arrayIndex),
      // so for array fields we send the whole array value; arrayIndex is added only for purely-numeric
      // trailing dot-segments (none here).
      for (const editKey of toCommit) {
        const suffix = `-${idx}`;
        const fieldPart = editKey.slice(0, -suffix.length); // "field" (or "field.N" if a numeric tail existed)
        const lastDot = fieldPart.lastIndexOf('.');
        const isArrayItem = lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1));
        ...
        const resp = await secureApiClient.put(`/api/edit/workplace_accommodations/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/workplace_accommodations/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this record's drafts for the approved fields from localStorage (now committed)
      const store = readDrafts();
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== WORKPLACE ACCOMMODATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Workplace Accommodations ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="workplace-accommodations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Workplace Accommodations</h2></div>
        <div className="empty-state">No workplace accommodations records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="workplace-accommodations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Workplace Accommodations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<WorkplaceAccommodationsDocumentPDFTemplate document={pdfData} />} fileName={`workplace-accommodations-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search workplace accommodations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            ...
            <div className="record-header">
              {hasVal(record.createdAt) && (
                <div className="record-meta-row">
                  <span className="record-date">{highlightText(formatDate(record.createdAt))}</span>
                </div>
              )}
              <h3 className="record-name">{highlightText(`Workplace Accommodations ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'record-info')}
            {renderSection(record, idx, 'functional-assessment')}
            {renderSection(record, idx, 'sensory-cognitive')}
            {renderSection(record, idx, 'medical-conditions')}
            {renderSection(record, idx, 'medication-mobility')}
            {renderSection(record, idx, 'psychological')}
...
export default WorkplaceAccommodationsDocument;
