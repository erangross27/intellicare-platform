/**
 * GlucoseMonitoringFrequencyDocumentPDFTemplate.jsx
 * Box-free B&W LETTER (canonical, memory 6a2d6af6) — mirrors the JSX: real record.date (never createdAt),
 * status enum canonical, ALL narrative string fields sentence-split ([.;] with abbrev/single-initial/genus
 * guard + labeled comma-split), results recursive object, recommendations date-grouped, values numbered
 * ('1.' even singles), single-name label gate. Rule #74: each field is ONE wrap={false} atomic View with the
 * sectionTitle riding INSIDE the first present field's View. Static PHI footer. Collection: glucose_monitoring_frequency.
 */
...
const FIELD_LABELS = {
  ...
  recommended: 'Recommended Frequency', current: 'Current Frequency', status: 'Status',
  findings: 'Findings', assessment: 'Assessment', plan: 'Plan',
  results: 'Results', recommendations: 'Recommendations',
  adjustments: 'Adjustments', concerns: 'Concerns', notes: 'Notes',
};
...
const KEY_OVERRIDES = { hba1c: 'HbA1c', a1c: 'A1c', bg: 'BG', cgm: 'CGM', smbg: 'SMBG', tir: 'TIR', gmi: 'GMI' };
...
const GlucoseMonitoringFrequencyDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].glucose_monitoring_frequency && Array.isArray(templateData[0].glucose_monitoring_frequency)) records = templateData[0].glucose_monitoring_frequency;
    else records = templateData;
  } else if (templateData && templateData.glucose_monitoring_frequency) {
    records = Array.isArray(templateData.glucose_monitoring_frequency) ? templateData.glucose_monitoring_frequency : [templateData.glucose_monitoring_frequency];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.glucose_monitoring_frequency ? (Array.isArray(dd.glucose_monitoring_frequency) ? dd.glucose_monitoring_frequency : [dd.glucose_monitoring_frequency]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Glucose Monitoring Frequency</Text></View>
        <Text style={styles.emptyState}>No glucose monitoring frequency records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Glucose Monitoring Frequency ${idx + 1}`}</Text>
...
export default GlucoseMonitoringFrequencyDocumentPDFTemplate;
