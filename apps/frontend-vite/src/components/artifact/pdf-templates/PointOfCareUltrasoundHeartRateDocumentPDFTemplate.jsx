/**
 * PointOfCareUltrasoundHeartRateDocumentPDFTemplate.jsx
 * June 2026 — Helvetica — A4 — BLACK & WHITE only (#000000 titles/borders/values, NO blue).
 * Collection: point_of_care_ultrasound_heart_rate.
 *
...
const FIELD_LABELS = {
  date: 'Date', heartRate: 'Heart Rate', unit: 'Unit', method: 'Method', type: 'Type',
  provider: 'Provider', facility: 'Facility', findings: 'Findings', assessment: 'Assessment',
  plan: 'Plan', notes: 'Notes', status: 'Status', results: 'Results', recommendations: 'Recommendations',
};
...
const PointOfCareUltrasoundHeartRateDocumentPDFTemplate = ({ document: data }) => {
  let records = [];
  if (Array.isArray(data)) {
    if (data.length === 1 && data[0]?.point_of_care_ultrasound_heart_rate) records = Array.isArray(data[0].point_of_care_ultrasound_heart_rate) ? data[0].point_of_care_ultrasound_heart_rate : [data[0].point_of_care_ultrasound_heart_rate];
    else records = data;
  } else if (data?.point_of_care_ultrasound_heart_rate) records = Array.isArray(data.point_of_care_ultrasound_heart_rate) ? data.point_of_care_ultrasound_heart_rate : [data.point_of_care_ultrasound_heart_rate];
  else if (data?.documentData) { const dd = data.documentData; if (Array.isArray(dd)) records = dd; else if (dd?.point_of_care_ultrasound_heart_rate) records = Array.isArray(dd.point_of_care_ultrasound_heart_rate) ? dd.point_of_care_ultrasound_heart_rate : [dd.point_of_care_ultrasound_heart_rate]; else if (dd && typeof dd === 'object') records = [dd]; }
  else if (data && typeof data === 'object') records = [data];
  records = (records || []).filter(r => r && typeof r === 'object');

  if (records.length === 0) {
    return (<Document><Page size="A4" style={styles.page}><View style={styles.documentHeader}><Text style={styles.title}>Point-of-Care Ultrasound — Heart Rate</Text></View><Text style={styles.emptyState}>No records available</Text></Page></Document>);
  }

  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.title}>Point-of-Care Ultrasound — Heart Rate</Text></View>
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer}>
            <View style={styles.recordHeader} wrap={false}>
              <Text style={styles.recordTitle}>{`Point-of-Care Ultrasound — Heart Rate ${String(record._recordNumber || idx + 1)}`}</Text>
...
export default PointOfCareUltrasoundHeartRateDocumentPDFTemplate;
