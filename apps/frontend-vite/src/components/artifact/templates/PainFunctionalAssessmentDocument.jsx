/**
 * PainFunctionalAssessmentDocument.jsx
 * March 2026 — Canonical one-pass: inline editing, blue glow theme
 * Collection: pain_functional_assessment
 *
 * Record date: top-level `date` → editable BlueDatePicker in the record header.
 * `painOnsetDate` → editable BlueDatePicker FIELD inside Pain Overview (not the header).
 *
 * 8 Sections:
 *   1. pain-overview: painIntensityScore, painLocationPrimary, painDuration, painOnsetDate, painFrequency
 *   2. pain-secondary-quality: painLocationSecondary, painQuality
 *   3. modifying-factors: aggravatingFactors, alleviatingFactors
 *   4. functional-impact: functionalImpactScore, activitiesOfDailyLivingImpact, instrumentalActivitiesImpact, sleepDisturbance, workStatusImpact, psychosocialImpact
 *   5. mobility-rom: mobilityLimitations, rangeOfMotionLimitations
 *   6. current-medications: currentPainMedications, morphineEquivalentDose
 *   7. interventions-history: nonPharmacologicInterventions, priorSurgicalInterventions
 *   8. scores-goals: painInterferenceScore, catastrophizingScore, functionalGoals, redFlagSymptoms
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PainFunctionalAssessmentDocumentPDFTemplate from '../pdf-templates/PainFunctionalAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './PainFunctionalAssessmentDocument.css';
...
const DRAFT_KEY = 'pain_functional_assessmentPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['painLocationPrimary', 'painDuration', 'painFrequency', 'activitiesOfDailyLivingImpact', 'instrumentalActivitiesImpact', 'sleepDisturbance', 'workStatusImpact', 'psychosocialImpact', 'rangeOfMotionLimitations'];
...
/* splitByComma: parenthesis-aware comma split (thousands guard: only split when followed by whitespace) */
...
/* Epoch sentinel: a 1970-01-01 date means "not set" → treat as empty (memory 6a…). */
...
/* ═══════ COMPONENT ═══════ */
const PainFunctionalAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.pain_functional_assessment) return Array.isArray(r.pain_functional_assessment) ? r.pain_functional_assessment : [r.pain_functional_assessment];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.pain_functional_assessment) return Array.isArray(dd.pain_functional_assessment) ? dd.pain_functional_assessment : [dd.pain_functional_assessment]; return [dd]; }
      return [r];
    });
  ...
  // Number fields hide zero / non-finite. Dates hide the 1970 epoch sentinel.
  ...
  const arrayHasVal = useCallback((v) => Array.isArray(v) && v.filter(x => x !== null && x !== undefined && String(x).trim() !== '').length > 0, []);
  const fieldHasVal = useCallback((record, f, idx) => {
    const v = getFieldValue(record, f, idx);
    if (NUMBER_FIELDS.includes(f)) return numberHasVal(v);
    if (DATE_FIELDS.includes(f)) return dateHasVal(v);
    if (ARRAY_FIELDS.includes(f)) return arrayHasVal(v);
    return hasVal(v);
  }, [getFieldValue, hasVal, numberHasVal, dateHasVal, arrayHasVal]);
  ...
  const clearApprovedForField = useCallback((fn, idx) => {
    let sid = null;
    for (const [s, fields] of Object.entries(SECTION_FIELDS)) { if (fields.includes(fn)) { sid = s; break; } }
    if (!sid) return;
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pain Functional Assessment ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/pain_functional_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/pain_functional_assessment/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    return text;
  }, [getFieldValue, hasVal, numberHasVal, dateHasVal, fmtVal, splitBySentence, formatSentenceFieldLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== PAIN FUNCTIONAL ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Pain Functional Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
      if (dateHasVal(r.date)) text += `Date\n${formatDate(r.date)}\n\n`;
  ...
  }, [pdfData, copyToClipboard, buildSectionCopyText, dateHasVal]);
        ...
        {isEditing ? (
          <span className="edit-field-container" onClick={e => e.stopPropagation()}>
            ...
            <span className="edit-actions">
              <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date'); return; } handleSaveField(record, 'date', idx, 'header', editValue + 'T00:00:00.000Z'); }}>{saving ? 'Saving...' : 'Save'}</button>
  ...
  /* ═══════ RENDER: NUMBER FIELD — num-stepper (hide zero / non-finite) ═══════ */
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; stageDraft(record, fn, idx, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); clearApprovedForField(fn, idx); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
            ...
            {sentences.map((sentence, sIdx) => {
              ...
              if (parsed.isLabeled) {
                ...
                if (commaItems.length >= 2) {
                  const showLabelSubtitle = parsed.label !== label;
                  return (
                    <div key={sIdx} className="rec-mini-card" style={{ marginTop: 8 }}>
                      {showLabelSubtitle && <div className="nested-subtitle">{highlightText(parsed.label)}</div>}
              ...
              const showLabelSingle = parsed.isLabeled && parsed.label !== label;
              return (
                <div key={sIdx} className={showLabelSingle ? 'rec-mini-card' : ''} style={showLabelSingle ? { marginTop: 8 } : undefined}>
                  {showLabelSingle && <div className="nested-subtitle">{highlightText(parsed.label)}</div>}
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => fieldHasVal(record, f, idx));
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pain-functional-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pain Functional Assessment</h2></div>
        <div className="empty-state">No pain functional assessment data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pain-functional-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Pain Functional Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PainFunctionalAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Pain_Functional_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pain functional assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Pain Functional Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'pain-overview')}
            {renderSection(record, idx, 'pain-secondary-quality')}
            {renderSection(record, idx, 'modifying-factors')}
            {renderSection(record, idx, 'functional-impact')}
            {renderSection(record, idx, 'mobility-rom')}
            {renderSection(record, idx, 'current-medications')}
            {renderSection(record, idx, 'interventions-history')}
            {renderSection(record, idx, 'scores-goals')}
          </div>
        ))}
      </div>
    </div>
...
export default PainFunctionalAssessmentDocument;
