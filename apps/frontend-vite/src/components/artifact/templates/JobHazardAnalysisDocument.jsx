/**
 * JobHazardAnalysisDocument.jsx
 * June 2026 — Inline editing, blue glow theme
 * Collection: job_hazard_analysis
 *
 * 12 Sections:
 *   1.  job-information: date, jobTitle, workLocation, supervisorName
 *   2.  task-sequence-steps: taskSequenceSteps[]
 *   3.  identified-hazards: identifiedHazards[], hazardCategory[]
 *   4.  injuries-exposure: potentialInjuries[], exposureRoutes[]
 *   5.  risk-assessment: riskSeverityLevel, probabilityOfOccurrence
 *   6.  physical-exposures: noiseExposureLevel, vibrationExposure, thermalStressFactors
 *   7.  chemical-exposures: chemicalExposureSubstances[], permissibleExposureLimits[]
 *   8.  control-measures: existingControlMeasures[], engineeringControls[], administrativeControls[], requiredPersonalProtectiveEquipment[]
 *   9.  ergonomic-risk-factors: ergonomicRiskFactors[]
 *   10. surveillance-emergency: requiredMedicalSurveillance[], emergencyResponseProcedures[]
 *   11. training-requirements: trainingRequirements[]
 *   12. review-status: reviewedByOccupationalHealth, analysisReviewDate
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import JobHazardAnalysisDocumentPDFTemplate from '../pdf-templates/JobHazardAnalysisDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './JobHazardAnalysisDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; arrays stored whole) */
const DRAFT_KEY = 'job_hazard_analysisPendingEdits';
const readDrafts = () => {
...
const SIMPLE_FIELDS = ['jobTitle', 'workLocation', 'supervisorName', 'riskSeverityLevel', 'probabilityOfOccurrence'];
...
/* Date-typed fields rendered with formatDate, edited with a native date-picker (saved as ISO so the DB type stays a Date) */
const DATE_FIELDS = ['date', 'analysisReviewDate'];
/* Numeric fields — edited with type=number input, saved as Number (parseFloat+isNaN guard) so DB type stays numeric */
const NUMBER_FIELDS = ['noiseExposureLevel'];
/* Numeric fields where 0 means "not assessed" — none here.
   noiseExposureLevel 0 dB is implausible but, per spec, stays visible (plain numeric with null/undefined check only). */
const HIDE_ZERO_FIELDS = [];
...
/* splitByComma: parenthesis-aware comma split with date-aware guard
   — items like "Mechanical lifting aids (hoists, dollies)" stay intact */
...
/* ═══════ COMPONENT ═══════ */
const JobHazardAnalysisDocument = ({ document: docProp, data, templateData }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      const docTitle = 'job hazard analysis';
      const rt = `job hazard analysis ${oi + 1}`;
  ...
  /* stageDraft = stage a DRAFT locally + write it to the pending-drafts localStorage store (survives refresh).
     ...
     fieldPart is the field name (arrays are staged whole). */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Collect this record's staged drafts for the fields in this section.
      // localEdits keys are `${fieldPart}-${idx}` (fieldPart = field name; arrays staged whole).
      const suffix = `-${idx}`;
      ...
      // Persist each staged field to the DB now (field, or field+arrayIndex for array-element keys)
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/job_hazard_analysis/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/job_hazard_analysis/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (store[id]) {
        toCommit.forEach(editKey => {
          const fieldPart = editKey.slice(0, -suffix.length);
          const lastDot = fieldPart.lastIndexOf('.');
          const draftKey = lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1)) ? fieldPart.slice(0, lastDot) : fieldPart;
          delete store[id][draftKey];
        });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== JOB HAZARD ANALYSIS ===\n\n';
    pdfData.forEach((r) => {
      const oi = r._originalIdx ?? 0;
      text += `Job Hazard Analysis ${oi + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: SIMPLE FIELD (no splitting — dates, titles, levels, names) ═══════ */
  ...
  /* ═══════ RENDER: NUMBER FIELD (type=number, saved as Number) ═══════ */
  ...
  /* ═══════ RENDER: DATE FIELD (native date-picker, saved as ISO) ═══════ */
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; stageDraft(record, fn, idx, sid, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                    ...
                    <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ciParsed.value); setSaveError(null); } }}>
                      {ciEditing ? (
                        <div className="edit-field-container">
                          ...
                          {saveError && <div className="save-error">{saveError}</div>}
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const allItems = splitByComma(String(getFieldValue(record, fn, idx) || '')); allItems[ciIdx] = `${ciParsed.label}: ${editValue.trim()}`; const fullText = allItems.join(', '); setSaveError(null); stageDraft(record, fn, idx, sid, fullText); setEditedSentences(prev => ({ ...prev, [commaKey]: 'edited' })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                            <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                          </div>
                        </div>
                      ) : (
                  ...
                  <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                    {ciEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const allItems = splitByComma(String(getFieldValue(record, fn, idx) || '')); allItems[ciIdx] = editValue.trim(); const fullText = allItems.join(', '); setSaveError(null); stageDraft(record, fn, idx, sid, fullText); setEditedSentences(prev => ({ ...prev, [commaKey]: 'edited' })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="job-hazard-analysis-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Job Hazard Analysis</h2></div>
        <div className="empty-state">No job hazard analysis records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="job-hazard-analysis-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Job Hazard Analysis</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<JobHazardAnalysisDocumentPDFTemplate document={pdfData} />} fileName="Job_Hazard_Analysis.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search job hazard analysis..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          return (
            <div key={safeId(record) || idx} className="record-card">
              <div className="record-header">
                <h3 className="record-name">{highlightText(`Job Hazard Analysis ${origIdx + 1}`)}</h3>
              </div>
              {renderSection(record, origIdx, 'job-information')}
              {renderSection(record, origIdx, 'task-sequence-steps')}
              {renderSection(record, origIdx, 'identified-hazards')}
              {renderSection(record, origIdx, 'injuries-exposure')}
              {renderSection(record, origIdx, 'risk-assessment')}
              {renderSection(record, origIdx, 'physical-exposures')}
              {renderSection(record, origIdx, 'chemical-exposures')}
              {renderSection(record, origIdx, 'control-measures')}
              {renderSection(record, origIdx, 'ergonomic-risk-factors')}
              {renderSection(record, origIdx, 'surveillance-emergency')}
              {renderSection(record, origIdx, 'training-requirements')}
              {renderSection(record, origIdx, 'review-status')}
            </div>
          );
        })}
      </div>
...
export default JobHazardAnalysisDocument;
