/**
 * BehavioralHealthGoalsDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: behavioral_health_goals
 *
 * 6 Sections:
 *   1. goal-info: date, goalCategory, goalDescription, priority, lifecycleStatus, achievementStatus
 *   2. targets: measurableTarget, targetMeasure, targetValue, baselineValue, currentValue, targetDate, startDate
 *   3. addresses: addresses
 *   4. plan: interventions, barriers, supportSystem
 *   5. engagement: expressedBy, acceptedBy, patientEngagement, safetyConsiderations, reviewFrequency, lastReviewedDate
 *   6. tracking: outcomeReferences, relatedGoals, modificationHistory
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import BehavioralHealthGoalsDocumentPDFTemplate from '../pdf-templates/BehavioralHealthGoalsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './BehavioralHealthGoalsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'behavioral_health_goalsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'goal-info': 'Goal Information',
  'targets': 'Targets & Measures',
  'addresses': 'Addresses',
  'plan': 'Plan',
...
const FIELD_LABELS = {
  ...
  measurableTarget: 'Measurable Target',
  ...
  outcomeReferences: 'Outcome References',
  relatedGoals: 'Related Goals',
  modificationHistory: 'Modification History',
};

const SECTION_FIELDS = {
  'goal-info': ['date', 'goalCategory', 'goalDescription', 'priority', 'lifecycleStatus', 'achievementStatus'],
  'targets': ['measurableTarget', 'targetMeasure', 'targetValue', 'baselineValue', 'currentValue', 'targetDate', 'startDate'],
  'addresses': ['addresses'],
  'plan': ['interventions', 'barriers', 'supportSystem'],
  'engagement': ['expressedBy', 'acceptedBy', 'patientEngagement', 'safetyConsiderations', 'reviewFrequency', 'lastReviewedDate'],
  'tracking': ['outcomeReferences', 'relatedGoals', 'modificationHistory'],
};

const DATE_FIELDS = ['date', 'targetDate', 'startDate', 'lastReviewedDate'];
const NUMBER_FIELDS = ['targetValue', 'baselineValue', 'currentValue'];
const ARRAY_FIELDS = ['addresses', 'interventions', 'barriers', 'supportSystem', 'outcomeReferences', 'relatedGoals', 'modificationHistory'];
const SEMICOLON_FIELDS = ['measurableTarget'];
const STRING_FIELDS = ['goalCategory', 'goalDescription', 'priority', 'lifecycleStatus', 'achievementStatus', 'targetMeasure', 'expressedBy', 'acceptedBy', 'patientEngagement', 'safetyConsiderations', 'reviewFrequency'];
...
/* ═══════ COMPONENT ═══════ */
const BehavioralHealthGoalsDocument = ({ document: docProp }) => {
  ...
  const safeIdStatic = useCallback((r) => { if (!r?._id) return null; if (typeof r._id === 'string') return r._id; if (r._id.$oid) return r._id.$oid; return String(r._id); }, []);
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // fieldPart = "field" or "field.arrayIndex"; localEdits keys are always "field-idx"
  ...
  function reconstructSemicolonText(items) {
    if (!items || items.length === 0) return '';
    return items.map(s => s.replace(/[;]+$/, '').trim()).filter(Boolean).join('; ');
  }
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Behavioral Health Goal ${idx + 1}`.toLowerCase();
  ...
  // fieldPart = "field" or "field.arrayIndex" (the latter ONLY for array elements); editKey is always "field-idx".
  const stageDraft = useCallback((record, fn, idx, fieldPart, value, marks) => {
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    stageDraft(record, fn, idx, fn, saveVal, { fields: { [trackKey]: 'edited' } });
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageDraft(record, fn, idx, fn, fullText, { sentences: { [`${fn}-${idx}-s${sentenceIdx}`]: 'edited' } });
      return;
    }
    ...
    stageDraft(record, fn, idx, fn, fullText, { sentences: sentMarks });
  }

  function saveSemicolonItem(record, fn, idx, sid, itemIdx) {
    const id = safeId(record); if (!id) return;
    const currentVal = String(getFieldValue(record, fn, idx) || '');
    ...
    setSaveError(null);
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      const updated = [...items]; updated.splice(itemIdx, 1);
      const fullText = reconstructSemicolonText(updated);
      stageDraft(record, fn, idx, fn, fullText, { sentences: { [`${fn}-${idx}-sc${itemIdx}`]: 'edited' } });
      return;
    }
    const updated = [...items]; updated[itemIdx] = editedVal;
    const fullText = reconstructSemicolonText(updated);
    stageDraft(record, fn, idx, fn, fullText, { sentences: { [`${fn}-${idx}-sc${itemIdx}`]: 'edited' } });
  }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Persist each staged field for THIS section to the DB now (field, or field+arrayIndex for array elements)
      const store = readDrafts();
      const recDrafts = store[id] || {};
      const committedFieldParts = [];
      const committedEditKeys = [];
      for (const fieldPart of Object.keys(recDrafts)) {
        // fieldPart = "field" or "field.arrayIndex". arrayIndex ONLY when the trailing dot-segment is numeric.
        const lastDot = fieldPart.lastIndexOf('.');
        const isArrayElem = lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1));
        const baseField = isArrayElem ? fieldPart.slice(0, lastDot) : fieldPart;
        if (!fields.includes(baseField)) continue; // not part of this section
        ...
        const resp = await secureApiClient.put(`/api/edit/behavioral_health_goals/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedFieldParts.push(fieldPart);
        committedEditKeys.push(`${baseField}-${idx}`);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/behavioral_health_goals/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    return text;
  }, [getFieldValue, hasVal, fmtVal, numberShows, splitBySentence, splitBySemicolon, formatSentenceFieldLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== BEHAVIORAL HEALTH GOALS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Behavioral Health Goal ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: SEMICOLON STRING FIELD (measurableTarget) ═══════ */
  const renderSemicolonField = (record, fn, idx, sid, title) => {
    const val = getFieldValue(record, fn, idx); if (!hasVal(val)) return null;
    const strVal = fmtVal(val);
    const items = splitBySemicolon(strVal);
    const label = FIELD_LABELS[fn] || fn;
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(item); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveSemicolonItem(record, fn, idx, sid, iIdx); }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
    ...
    /* Single item, same as plain string with saveSentence */
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="behavioral-health-goals-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Behavioral Health Goals</h2></div>
        <div className="empty-state">No behavioral health goals records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="behavioral-health-goals-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Behavioral Health Goals</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<BehavioralHealthGoalsDocumentPDFTemplate document={pdfData} />} fileName={`behavioral-health-goals-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search behavioral health goals..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(record.goalCategory || `Behavioral Health Goal ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'goal-info')}
            {renderSection(record, idx, 'targets')}
            {renderSection(record, idx, 'addresses')}
            {renderSection(record, idx, 'plan')}
            {renderSection(record, idx, 'engagement')}
...
export default BehavioralHealthGoalsDocument;
