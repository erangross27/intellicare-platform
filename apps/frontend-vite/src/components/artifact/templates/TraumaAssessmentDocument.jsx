/**
 * TraumaAssessmentDocument.jsx
 * June 2026 — Inline editing, blue glow theme
 * Collection: trauma_assessment
 *
 * 6 Sections:
 *   1. triage-mechanism: date, traumaMechanism, triageCategory, traumaTeamActivationLevel
 *   2. trauma-scores: glasgowComaScore, revisedTraumaScore, injurySeverityScore
 *   3. primary-survey: primarySurveyAirway, primarySurveyBreathing, primarySurveyCirculation, primarySurveyDisability, primarySurveyExposure
 *   4. injuries: injuredBodyRegions[], hemorrhageClass, fastExamResult, penetratingInjuryLocation[], burnTotalBodySurfaceArea, compartmentSyndromeRisk
 *   5. interventions-protocols: cervicalSpineImmobilization, pelvicBinderApplied, massiveTransfusionProtocol, emergentInterventionsPerformed[], consultingServicesActivated[]
 *   6. timing-disposition: traumaBayArrivalTime, timeToCtScan, traumaDisposition
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import TraumaAssessmentDocumentPDFTemplate from '../pdf-templates/TraumaAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './TraumaAssessmentDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldName]: value } }  (value = full field value, array for array fields) */
const DRAFT_KEY = 'trauma_assessmentPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  traumaTeamActivationLevel: 'Trauma Team Activation Level',
  ...
  primarySurveyAirway: 'Airway',
  primarySurveyBreathing: 'Breathing',
  primarySurveyCirculation: 'Circulation',
  primarySurveyDisability: 'Disability',
  primarySurveyExposure: 'Exposure',
  injuredBodyRegions: 'Injured Body Regions',
  hemorrhageClass: 'Hemorrhage Class',
  fastExamResult: 'FAST Exam Result',
  penetratingInjuryLocation: 'Penetrating Injury Location',
  burnTotalBodySurfaceArea: 'Burn TBSA (%)',
  compartmentSyndromeRisk: 'Compartment Syndrome Risk',
  cervicalSpineImmobilization: 'Cervical Spine Immobilization',
  pelvicBinderApplied: 'Pelvic Binder Applied',
  massiveTransfusionProtocol: 'Massive Transfusion Protocol',
  emergentInterventionsPerformed: 'Emergent Interventions Performed',
  consultingServicesActivated: 'Consulting Services Activated',
  traumaBayArrivalTime: 'Trauma Bay Arrival Time',
  timeToCtScan: 'Time to CT Scan (minutes)',
  traumaDisposition: 'Trauma Disposition',
};
...
const SENTENCE_FIELDS = ['traumaMechanism', 'primarySurveyAirway', 'primarySurveyBreathing', 'primarySurveyCirculation', 'primarySurveyDisability', 'primarySurveyExposure', 'fastExamResult', 'compartmentSyndromeRisk'];
const SIMPLE_FIELDS = ['date', 'triageCategory', 'traumaTeamActivationLevel', 'glasgowComaScore', 'revisedTraumaScore', 'injurySeverityScore', 'hemorrhageClass', 'burnTotalBodySurfaceArea', 'traumaBayArrivalTime', 'timeToCtScan', 'traumaDisposition'];
const BOOLEAN_FIELDS = ['cervicalSpineImmobilization', 'pelvicBinderApplied', 'massiveTransfusionProtocol'];
const ARRAY_FIELDS = ['injuredBodyRegions', 'penetratingInjuryLocation', 'emergentInterventionsPerformed', 'consultingServicesActivated'];
...
/* Datetime fields — display date + time, edit prefilled as YYYY-MM-DDTHH:mm */
const DATETIME_FIELDS = ['traumaBayArrivalTime'];
/* Numeric fields where 0 means "not recorded" — hidden when exactly 0.
   timeToCtScan 0 and burnTotalBodySurfaceArea 0 = not recorded; GCS 0 is an impossible
   score (min 3) and RTS 0 is treated as a not-recorded sentinel here.
   injurySeverityScore is NOT hidden: ISS 0 is clinically MEANINGFUL (no injury). */
const HIDE_ZERO_FIELDS = ['glasgowComaScore', 'revisedTraumaScore', 'burnTotalBodySurfaceArea', 'timeToCtScan'];
/* Numeric-typed fields — edited through the custom text-backed stepper. */
const NUMBER_FIELDS = ['glasgowComaScore', 'revisedTraumaScore', 'injurySeverityScore', 'burnTotalBodySurfaceArea', 'timeToCtScan'];
const COMMA_ARRAY_FIELDS = ['traumaMechanism', 'primarySurveyAirway', 'primarySurveyBreathing', 'fastExamResult'];
...
const formatDateTime = (dateValue) => {
  if (!dateValue) return '';
  try { const d = new Date(dateValue.$date || dateValue); if (isNaN(d.getTime())) return String(dateValue); return `${d.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}, ${d.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}`; } catch { return String(dateValue); }
};
...
/* ═══════ COMPONENT ═══════ */
const TraumaAssessmentDocument = ({ document: docProp, data, templateData }) => {
  ...
  const splitBySentence = useCallback((text) => {
    ...
    for (let index = 0; index < text.length; index += 1) {
      ...
      const safePeriod = character === '.' && depth === 0 && /\s/.test(next)
      ...
      if (safePeriod || safeSemicolon) { push(); while (/\s/.test(text[index + 1] || '')) index += 1; }
  ...
  const splitEditableClauses = useCallback((text) => {
    ...
    for (let index = 0; index < text.length; index += 1) {
      ...
      const safePeriod = character === '.' && depth === 0 && /\s/.test(next)
        ...
        && !/\b[A-Z]$/.test(current) && !/\d$/.test(current);
      const safeSemicolon = character === ';' && depth === 0;
      if (safePeriod || safeSemicolon) { push(character); while (/\s/.test(text[index + 1] || '')) index += 1; }
      else current += character;
    }
    push(''); return out;
  }, []);
  ...
  /* valueMatchesPhrase: matches formatted values AND "Label: Value" entries (formatted dates included via fmtFieldVal) */
  const valueMatchesPhrase = useCallback((fn, val, phrase) => {
    if (val === null || val === undefined) return false;
    const label = (FIELD_LABELS[fn] || fn).toLowerCase();
    if (Array.isArray(val)) {
      return val.some(item => {
        const t = arrItemText(item).toLowerCase();
        return t.includes(phrase) || `${label}: ${t}`.includes(phrase);
      });
    }
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      const docTitle = 'trauma assessment';
      const rt = `trauma assessment ${oi + 1}`;
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* stageDraft: write the staged full field value into the pending-drafts localStorage store (survives refresh).
  ...
  // Save = stage a DRAFT locally + localStorage. NOT written to DB/PDF until Approve.
  ...
  /* saveSentence: reconstructFullText period-restoration — used by BOTH Save button and Ctrl+Enter */
  ...
  /* saveLabeledSentence: rebuild "Label: v1, v2" sentence then reconstructFullText — shared by Save button and Ctrl+Enter */
  ...
  /* saveSingleCommaItem: single-value comma list — labelPrefix set for "Label: value" items */
  function saveSingleCommaItem(record, fn, idx, sid, ciIdx, commaKey, labelPrefix) {
    const id = safeId(record); if (!id) return;
    const allItems = splitByComma(String(getFieldValue(record, fn, idx) || ''));
    allItems[ciIdx] = labelPrefix ? `${labelPrefix}: ${editValue.trim()}` : editValue.trim();
  ...
  /* saveArrayItem: per-item array editing via arrayIndex — shared by Save button and Ctrl+Enter */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // Collect this record's pending edits for this section (localEdits keyed `${fn}-${idx}`)
      const suffix = `-${idx}`;
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length); // "field" (no numeric dot-suffix in this template)
        const lastSeg = fieldPart.split('.').pop();
        const baseField = /^\d+$/.test(lastSeg) ? fieldPart.slice(0, fieldPart.lastIndexOf('.')) : fieldPart;
      ...
      for (const editKey of toCommit) {
        ...
        const lastSeg = fieldPart.split('.').pop();
        const isArrayIdx = /^\d+$/.test(lastSeg);
        const payload = { field: isArrayIdx ? fieldPart.slice(0, fieldPart.lastIndexOf('.')) : fieldPart, value: localEdits[editKey] };
        if (isArrayIdx) payload.arrayIndex = parseInt(lastSeg, 10);
        await secureApiClient.put(`/api/edit/trauma_assessment/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/trauma_assessment/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== TRAUMA ASSESSMENT ===\n\n';
    pdfData.forEach((r) => {
      const oi = r._originalIdx ?? 0;
      text += `Trauma Assessment ${oi + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: SIMPLE FIELD (no splitting — dates, scores, categories, dispositions) ═══════ */
  const renderSimpleField = (record, fn, idx, sid) => {
    ...
    const isDateField = DATE_FIELDS.includes(fn) || DATETIME_FIELDS.includes(fn);
    /* saveSimpleField: numbers parsed to real numbers (parseFloat + isNaN guard so bad input never persists) */
    const saveSimpleField = () => {
      if (isNumber) {
        ...
        if (DATETIME_FIELDS.includes(fn)) {
    ...
    return (
      <div key={fn} className="rec-mini-card nested-mini-card">
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(isDateField ? toISODate(val) : isNumber ? String(val) : displayVal); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              {isNumber ? (
                <div className="number-edit-row">
                  ...
                  <input type="text" inputMode="decimal" className="edit-number" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) saveSimpleField(); }} />
                  <button type="button" className="num-step" onClick={e => { e.stopPropagation(); setEditValue(String((Number(editValue) || 0) + 1)); }}>+</button>
                </div>
              ) : isDateField ? (
                <BlueDatePicker value={editValue} onSelect={setEditValue} />
              ) : (
                <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) { saveSimpleField(); } }} />
              )}
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveSimpleField(); }}>{saving ? 'Saving...' : 'Save'}</button>
  ...
  /* ═══════ RENDER: ARRAY FIELD (per-item editing with arrayIndex — items VERBATIM) ═══════ */
  ...
  const renderSentenceField = (record, fn, idx, sid) => {
    ...
    /* Multi-sentence: render with period-first splitting, parseLabel for subtitles — field label only on the mini-card (first row), not per sentence */
                ...
                return (
                  <div key={ciIdx} className="rec-mini-card" style={{ marginTop: 8 }}>
                    ...
                    <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ciParsed.value); setSaveError(null); } }}>
                      {ciEditing ? (
                        <div className="edit-field-container">
                          <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) { saveSingleCommaItem(record, fn, idx, sid, ciIdx, commaKey, ciParsed.label); } }} />
                          {saveError && <div className="save-error">{saveError}</div>}
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveSingleCommaItem(record, fn, idx, sid, ciIdx, commaKey, ciParsed.label); }}>{saving ? 'Saving...' : 'Save'}</button>
                            <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                          </div>
                        </div>
                      ) : (
              ...
              return (
                <div key={ciIdx}>
                  <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                    {ciEditing ? (
                      <div className="edit-field-container">
                        <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) { saveSingleCommaItem(record, fn, idx, sid, ciIdx, commaKey, null); } }} />
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveSingleCommaItem(record, fn, idx, sid, ciIdx, commaKey, null); }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
            ...
            <div key={clauseIndex} className={parsed.isLabeled ? 'nested-mini-card' : ''}>
              ...
              {items.map((item, itemIndex) => {
                ...
                const saveItem = () => {
                  ...
                  const rebuiltSource = COMMA_ARRAY_FIELD_SET.has(fn) ? currentItems.join(', ') : currentItems[0];
                  currentClauses[clauseIndex].text = currentParsed.isLabeled ? `${currentParsed.label}: ${rebuiltSource}` : rebuiltSource;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="trauma-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Trauma Assessment</h2></div>
        <div className="empty-state">No trauma assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="trauma-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Trauma Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<TraumaAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Trauma_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search trauma assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          return (
            <div key={safeId(record) || idx} className="record-card">
              <div className="record-header">
                ...
                <h3 className="record-name">{highlightText(`Trauma Assessment ${origIdx + 1}`)}</h3>
              </div>
              {renderSection(record, origIdx, 'triage-mechanism')}
              {renderSection(record, origIdx, 'trauma-scores')}
              {renderSection(record, origIdx, 'primary-survey')}
              {renderSection(record, origIdx, 'injuries')}
              {renderSection(record, origIdx, 'interventions-protocols')}
...
export default TraumaAssessmentDocument;
