/**
 * BloodProductsDocument.jsx
 * June 2026 — Collection: blood_products (NEW template)
 *
 * Donor cloned: PointOfCareUltrasoundHeartRateDocument — date header renderDateField;
 ...
 *   blue-glow editing theme.
 *
 * FIELDS (15 → 100% schema coverage):
 *   DATE:   date
 *   STRING: productType, indication, unitsOrdered, unitsTransfused, bloodType, crossmatch,
 *           transfusionReaction, vitalSigns, orderingProvider, administeringNurse, facility, notes
 *   OBJECT: pretransfusionLabs, posttransfusionLabs (recursive key-value)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import BloodProductsDocumentPDFTemplate from '../pdf-templates/BloodProductsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './BloodProductsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  where fieldPart is the exact field to PUT
   (e.g. "notes" for a string, or "pretransfusionLabs.hgb" for a nested object leaf). */
const DRAFT_KEY = 'blood_productsPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  unitsTransfused: 'Units Transfused',
  ...
  facility: 'Facility',
  notes: 'Notes',
  pretransfusionLabs: 'Pre-Transfusion Labs',
  posttransfusionLabs: 'Post-Transfusion Labs',
};
...
/* STRING fields edited with a number stepper (instead of a textarea): the value is a leading number
   + fixed unit ("8 units"). splitNumberUnit edits ONLY the number, keeps the unit as a fixed affix,
   and reassembles on save (lossless). A non-numeric value falls back to the normal textarea. */
const NUMBER_FIELDS = ['unitsOrdered', 'unitsTransfused'];
...
/* ═══════ COMPONENT ═══════ */
const BloodProductsDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.blood_products) return Array.isArray(r.blood_products) ? r.blood_products : [r.blood_products];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.blood_products) return Array.isArray(dd.blood_products) ? dd.blood_products : [dd.blood_products]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (dotIdx === -1) {
          // plain field (string/date) — localEdits key is `${field}-${idx}`
          const editKey = `${fieldPart}-${idx}`;
          nLocal[editKey] = value;
          nPending[editKey] = true;
          nFields[editKey] = 'edited';
        } else {
          // nested object leaf, e.g. "pretransfusionLabs.hgb" → merge value into the root object draft
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `blood products / transfusion ${idx + 1}`.toLowerCase();
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. pretransfusionLabs.hgb) — value stays a STRING.
     Staged as a DRAFT (no DB write); localStorage keeps it across refresh; Approve commits it. */
  ...
  // Save one sentence = stage a DRAFT locally + localStorage (survives refresh). NOT written to
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    setSaving(true);
    try {
      // Pull this record's drafts and commit only the ones belonging to this section's fields.
      const store = readDrafts();
      const recDrafts = store[id] || {};
      const committedFieldParts = [];
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        const rootField = fieldPart.includes('.') ? fieldPart.slice(0, fieldPart.indexOf('.')) : fieldPart;
        if (!fields.includes(rootField)) continue;
        // GOTCHA: dot-suffix → arrayIndex ONLY when the segment after the LAST dot is purely numeric.
        ...
        const resp = await secureApiClient.put(`/api/edit/blood_products/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedFieldParts.push(fieldPart);
      }
      await secureApiClient.put(`/api/edit/blood_products/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (committedFieldParts.length > 0) {
        const s = readDrafts();
        if (s[id]) {
          committedFieldParts.forEach(fp => { delete s[id][fp]; });
    ...
    } catch (err) { console.error('[BloodProducts] Approve error:', err); } finally { setSaving(false); }
  }, [safeId]);
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== BLOOD PRODUCTS / TRANSFUSION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Blood Products / Transfusion ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* Numeric field ("8 units") → number stepper: edit only the number, keep the unit fixed.
       Mirrors renderObjectLeaf's number editor; reassembles losslessly via splitNumberUnit. */
    const nu = NUMBER_FIELDS.includes(fn) ? splitNumberUnit(strVal) : null;
    if (nu) {
      const editKey = `${fn}-${idx}`;
      const isEditing = editingField === editKey;
      const isModified = editedFields[editKey];
      const saveNumber = () => { const n = parseFloat(editValue); if (isNaN(n)) { setSaveError('Please enter a valid number'); return; } handleSaveField(record, fn, idx, sid, nu.unit ? `${n}${nu.sep || ' '}${nu.unit}` : String(n)); };
      return (
        <div key={fn} className="rec-mini-card">
            ...
            {isEditing ? (
              <div className="edit-field-container">
                <div className="number-edit-row">
                  <input type="number" step="any" className="edit-number" value={editValue} autoFocus onChange={e => setEditValue(e.target.value)} onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter') { e.preventDefault(); saveNumber(); } }} />
  ...
  if (!records || records.length === 0) {
    return (
      <div className="blood-products-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Blood Products / Transfusion</h2></div>
        <div className="empty-state">No blood products / transfusion records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="blood-products-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Blood Products / Transfusion</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<BloodProductsDocumentPDFTemplate document={pdfData} />} fileName={`blood-products-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search blood products / transfusion records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          <div key={idx} className="record-card">
            <div className="record-header">
              <div className="record-meta-row">
                ...
                {hasVal(record.orderingProvider) && <span className="record-date">{fmtVal(record.orderingProvider)}</span>}
                {hasVal(record.facility) && <span className="record-date">{fmtVal(record.facility)}</span>}
              </div>
              <h3 className="record-name">{highlightText(`Blood Products / Transfusion ${idx + 1}`)}</h3>
            </div>
...
export default BloodProductsDocument;
