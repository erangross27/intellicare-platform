...
const COLLECTION = 'cancer_related_side_effects';
const COMMA_SPLIT_FIELDS = ['findings', 'assessment', 'plan', 'notes', 'secondaryMalignancyRisk'];
const ARRAY_FIELDS = new Set(['recommendations']);
const OBJECT_FIELDS = new Set(['lymphedema', 'neuropathy', 'fatigue', 'sexualDysfunction', 'fertilityImpact', 'results', 'additionalData']);
const NARRATIVE_FIELDS = new Set(['findings', 'assessment', 'plan', 'notes', 'secondaryMalignancyRisk']);
const DATE_FIELDS = new Set(['date']);
const KEY_OVERRIDES = { qualityOfLifeImpact: 'Quality of Life Impact' };
const LABELS = {
  date: 'Date', type: 'Type', status: 'Status', provider: 'Provider', facility: 'Facility',
  lymphedema: 'Lymphedema', neuropathy: 'Neuropathy', fatigue: 'Fatigue', cognitiveChanges: 'Cognitive Changes',
  sexualDysfunction: 'Sexual Dysfunction', fertilityImpact: 'Fertility Impact', secondaryMalignancyRisk: 'Secondary Malignancy Risk',
...
const SECTIONS = [
  ...
  { title: 'Lymphedema', fields: ['lymphedema'] },
  { title: 'Neuropathy', fields: ['neuropathy'] },
  { title: 'Fatigue', fields: ['fatigue'] },
  { title: 'Cognitive Changes', fields: ['cognitiveChanges'] },
  { title: 'Sexual Dysfunction', fields: ['sexualDysfunction'] },
  { title: 'Fertility Impact', fields: ['fertilityImpact'] },
  { title: 'Secondary Malignancy Risk', fields: ['secondaryMalignancyRisk'] },
...
const CancerRelatedSideEffectsDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Cancer Related Side Effects</Text><Text style={styles.noData}>No cancer related side effects data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Cancer Related Side Effects</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Cancer Related Side Effects {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
