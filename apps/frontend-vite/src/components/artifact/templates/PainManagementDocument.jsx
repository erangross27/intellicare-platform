/**
 * PainManagementDocument.jsx
 * Canonical one-pass polish — blue glow editing theme
 * Collection: pain_management
 *
 * Sections: Record Information · Pain Assessment · Pain Modifiers · Functional Impact ·
 *           Treatment History · Current Treatment · Procedures & Imaging · Monitoring ·
 *           Psychological Assessment · Follow-Up & Referrals
 *
 * Scalar strings inline-editable (renderStringField). Dates edit via BlueDatePicker
 * (renderDateField). Numbers via the canonical num-stepper (renderNumberField). Arrays are
 * editable rows (renderArrayField). Nested objects (urineDrugScreen/prescriptionMonitoringQuery)
 * render read-only, decomposed (label ABOVE value — never side-by-side "Label: value").
 * Numeric 0 and empty ""/[]/{}/null are hidden. Record date uses record.date (never createdAt).
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PainManagementDocumentPDFTemplate from '../pdf-templates/PainManagementDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './PainManagementDocument.css';
...
const DRAFT_KEY = 'pain_managementPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'followup-referrals': 'Follow-Up & Referrals',
};
...
const FIELD_LABELS = {
  ...
  opioidMorphineEquivalent: 'Opioid Morphine Equivalent (mg/day)',
  interventionalProcedures: 'Interventional Procedures',
  imagingStudies: 'Imaging Studies',
  physicalTherapyOrders: 'Physical Therapy Orders',
  urineDrugScreen: 'Urine Drug Screen',
  prescriptionMonitoringQuery: 'Prescription Monitoring Query',
  painManagementAgreement: 'Pain Management Agreement',
  psychologicalScreening: 'Psychological Screening',
...
const SECTION_FIELDS = {
  ...
  'pain-assessment': ['painLocation', 'painOnsetDate', 'painDuration', 'painIntensityScore', 'painCharacteristics'],
  'pain-modifiers': ['aggravatingFactors', 'alleviatingFactors', 'radiationPattern'],
  'functional-impact': ['functionalImpairment'],
  'treatment-history': ['previousTreatments'],
  'current-treatment': ['currentMedications', 'opioidMorphineEquivalent'],
  'procedures-imaging': ['interventionalProcedures', 'imagingStudies', 'physicalTherapyOrders'],
  'monitoring': ['urineDrugScreen', 'prescriptionMonitoringQuery', 'painManagementAgreement'],
  'psychological': ['psychologicalScreening'],
  'followup-referrals': ['followUpInterval', 'referralsOrdered'],
};

const NUMBER_FIELDS = ['painIntensityScore', 'opioidMorphineEquivalent'];
const DATE_FIELDS = ['date', 'painOnsetDate'];
const STRING_FIELDS = ['provider', 'facility', 'painDuration', 'radiationPattern', 'functionalImpairment', 'physicalTherapyOrders', 'painManagementAgreement', 'psychologicalScreening', 'followUpInterval'];
const ARRAY_FIELDS = ['painLocation', 'painCharacteristics', 'aggravatingFactors', 'alleviatingFactors', 'previousTreatments', 'currentMedications', 'interventionalProcedures', 'imagingStudies', 'referralsOrdered'];
const OBJECT_FIELDS = ['urineDrugScreen', 'prescriptionMonitoringQuery'];

/* ═══════ OBJECT-FIELD HELPERS ═══════ */
const KEY_OVERRIDES = {
  uds: 'UDS', pmp: 'PMP', pdmp: 'PDMP', ome: 'OME', mme: 'MME', prn: 'PRN', ptt: 'PTT',
  thc: 'THC', id: 'ID', dob: 'DOB', npi: 'NPI',
};
...
/* splitByComma: parenthesis-aware; only splits on a comma FOLLOWED by whitespace (keeps "5,000" intact) */
...
/* ═══════ COMPONENT ═══════ */
const PainManagementDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.pain_management) return Array.isArray(r.pain_management) ? r.pain_management : [r.pain_management];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.pain_management) return Array.isArray(dd.pain_management) ? dd.pain_management : [dd.pain_management]; return [dd]; }
      return [r];
    });
  ...
  /* ═══════ UTILS ═══════ */
  // numeric 0 and empty ""/[]/{}/null are treated as "not set" and hidden (numeric-0-is-empty rule).
  const hasVal = useCallback((v) => { if (v === null || v === undefined || v === '') return false; if (typeof v === 'boolean') return true; if (typeof v === 'number') return v !== 0; if (typeof v === 'string') return v.trim() !== ''; if (Array.isArray(v)) return v.filter(x => !isEmptyDeep(x)).length > 0; if (typeof v === 'object') return !isEmptyDeep(v); return true; }, []);
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (Array.isArray(val)) { for (const item of val) { if (item && String(item).toLowerCase().includes(phrase)) return true; } }
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pain Management ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      // Persist each staged field to the DB now (whole field value: string or full array)
      for (const editKey of toCommit) {
        const fn = editKey.slice(0, editKey.lastIndexOf('-'));
        const resp = await secureApiClient.put(`/api/edit/pain_management/${id}/edit`, { field: fn, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/pain_management/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_FIELDS.includes(f)) {
        ...
        if (showLabel) text += `${label}\n`;
        items.forEach((it, i) => { text += `${i + 1}. ${fmtScalar(it)}\n`; });
        text += '\n';
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PAIN MANAGEMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Pain Management ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT FIELD (read-only, decomposed) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pain-management-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pain Management</h2></div>
        <div className="empty-state">No pain management records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pain-management-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Pain Management</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PainManagementDocumentPDFTemplate document={pdfData} />} fileName="Pain_Management.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pain management..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Pain Management ${idx + 1}`)}</h3>
            </div>
...
export default PainManagementDocument;
