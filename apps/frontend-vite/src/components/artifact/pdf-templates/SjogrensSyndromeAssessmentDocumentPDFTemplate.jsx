...
const SECTIONS = [
  { id: 'recordInfo', title: 'Assessment Information', fields: ['date', 'type', 'provider', 'facility', 'status'] },
  { id: 'sicca', title: 'Sicca Symptoms & Testing', fields: ['sicca'] },
  { id: 'glandTesting', title: 'Salivary Gland Testing', fields: ['salivarGlandBiopsy', 'sialography'] },
  { id: 'manifestations', title: 'Systemic Manifestations & Scores', fields: ['systemicManifestations', 'essdaiScore', 'esspriScore'] },
  { id: 'findingsAssessment', title: 'Findings & Assessment', fields: ['findings', 'assessment'] },
  { id: 'plan', title: 'Plan', fields: ['plan'] },
  { id: 'recommendations', title: 'Recommendations', fields: ['recommendations'] },
  { id: 'resultsNotes', title: 'Results & Notes', fields: ['results', 'notes'] },
];
const FIELD_LABELS = {
...
export default function SjogrensSyndromeAssessmentDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Sjogrens Syndrome Assessment</Text></View>
    {!records.length && <Text style={styles.noDataText}>No sjogrens syndrome assessment data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Sjogrens Syndrome Assessment Record {recordIndex + 1}</Text></View>
