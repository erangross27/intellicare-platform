...
function buildContext(req, operation = 'read') { return { serviceId: 'fall-prevention-education-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['fallRiskAssessmentScore', 'bergBalanceScale', 'timedUpAndGoTest', 'functionalReachTest', 'fearOfFallingScale', 'fallHistoryPreviousYear', 'vitaminDLevelNgMl', 'orthostasisPresent', 'cognitiveImpairmentSeverity', 'visualAcuityImpairment', 'peripheralNeuropathyPresent', 'polypharmacyPresent', 'medicationsFallRisk', 'medicationReviewCompleted', 'gaitAbnormalities', 'assistiveDeviceType', 'homeHazardsIdentified', 'strengthTrainingRecommended', 'balanceTrainingType', 'footwearAssessment', 'educationTopicsCovered'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('fall_prevention_education', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[FallPreventionEducation] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('fall_prevention_education', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[FallPreventionEducation] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
