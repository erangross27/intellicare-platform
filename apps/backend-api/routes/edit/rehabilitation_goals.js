...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'rehabilitation-goals-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['goalDescription', 'functionalCategory', 'priorityLevel', 'achievementStatus', 'date', 'startDate', 'targetDate', 'duration', 'frequency', 'baselinePerformance', 'currentPerformance', 'targetPerformance', 'measurableOutcome', 'percentComplete', 'therapyDiscipline', 'responsibleTherapist', 'interventionApproach', 'barriers', 'facilitators', 'equipmentRequired', 'patientAgreement', 'caregiverInvolvement', 'dischargeCriteria', 'anticipatedDischargeDisposition', 'outcomeScale', 'modificationReason'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('rehabilitation_goals', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[rehabilitation_goals] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('rehabilitation_goals', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[rehabilitation_goals] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
