/**
 * ImmunizationStatusDocument.jsx
 * Canonical one-pass rewrite — inline editing, blue glow theme, box-free PDF parity.
 * Collection: immunization_status
 *
 * Sections:
 *   1. record-info: date (DATE), provider, facility
 *   2. vaccine-status: influenza, pneumococcal, covid19Booster, tdap, zoster (semicolon-decomposed strings)
 *   3. vaccines-administered: other (array → numbered items)
 *   4. summary: summary (semicolon-decomposed string)
 *   5. clinical: findings, assessment, plan, notes (per-sentence narrative)
 *   6. results: results (nested object, read-only recursive)
 *   7. recommendations: recommendations (array → numbered items)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ImmunizationStatusDocumentPDFTemplate from '../pdf-templates/ImmunizationStatusDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './ImmunizationStatusDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Shape: { [recordId]: { [fieldPart]: value } } */
const DRAFT_KEY = 'immunization_statusPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['provider', 'facility', 'influenza', 'pneumococcal', 'covid19Booster', 'tdap', 'zoster', 'summary', 'findings', 'assessment', 'plan', 'notes'];
...
const arrItemText = (item) => (typeof item === 'object' && item !== null) ? (item.recommendation || item.text || flattenSearchable(item)) : String(item ?? '');
const flattenSearchable = (v) => {
  if (v === null || v === undefined) return '';
  if (isScalar(v)) return typeof v === 'boolean' ? (v ? 'Yes' : 'No') : String(v);
...
/* ═══════ COMPONENT ═══════ */
const ImmunizationStatusDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.immunization_status) return Array.isArray(dd.immunization_status) ? dd.immunization_status : [dd.immunization_status]; return [dd]; }
  ...
  /* ═══════ UTILS ═══════ */
  const hasVal = useCallback((v) => { if (v === null || v === undefined || v === '') return false; if (typeof v === 'boolean') return true; if (typeof v === 'number') return true; if (typeof v === 'string') return v.trim() !== ''; if (Array.isArray(v)) return v.filter(x => x !== null && x !== undefined && x !== '').length > 0; if (typeof v === 'object') return !isEmptyDeep(v); return true; }, []);
  const fmtVal = useCallback((v) => { if (typeof v === 'boolean') return v ? 'Yes' : 'No'; if (typeof v === 'number') return String(v); if (Array.isArray(v)) return v.map(arrItemText).join('; '); if (v && typeof v === 'object') return flattenSearchable(v); return String(v || ''); }, []);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Immunization Status ${idx + 1}`.toLowerCase();
      if (rt.includes(phrase) || phrase.includes(rt) || /^immunization\s+status(\s+\d+)?$/i.test(phrase)) { record._showAllSections = true; return true; }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/immunization_status/${id}/edit`, { field: fieldPart, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/immunization_status/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const objectCopyLines = useCallback((value) => {
    const lines = [];
    const walk = (label, v, indent) => {
      const pad = '  '.repeat(indent);
      if (isEmptyDeep(v)) return;
      if (isScalar(v)) { lines.push(`${pad}${label ? label + ': ' : ''}${fmtVal(v)}`); return; }
      if (label) lines.push(`${pad}${label}:`);
      Object.entries(v).filter(([, x]) => !isEmptyDeep(x)).forEach(([k, x]) => walk(humanizeKey(k), x, indent + (label ? 1 : 0)));
    };
    if (value && typeof value === 'object') Object.entries(value).filter(([, x]) => !isEmptyDeep(x)).forEach(([k, x]) => walk(humanizeKey(k), x, 0));
    return lines;
  }, [fmtVal]);
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        const items = (Array.isArray(val) ? val : []).filter(x => hasVal(x)).map(arrItemText);
        if (items.length) { text += head; items.forEach((it, i) => { text += `${i + 1}. ${it}\n`; }); text += '\n'; }
      } else if (OBJECT_FIELDS.includes(f)) {
        const lines = objectCopyLines(val);
        if (lines.length) { text += `${head}${lines.join('\n')}\n\n`; }
      } else {
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== IMMUNIZATION STATUS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Immunization Status ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: ARRAY FIELD (numbered items in Copy/PDF; unnumbered rows on-screen) ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, iIdx) => {
          ...
          const itemText = arrItemText(item);
          if (searchTerm.trim() && !record._showAllSections && !sectionTitleMatches(sid) && !itemText.toLowerCase().includes(searchTerm.toLowerCase().trim())) return null;
          return (
            <div key={iIdx} className="numbered-row">
              <div className="row-content"><span className="content-value">{highlightText(itemText)}</span></div>
  ...
  /* ═══════ RENDER: OBJECT FIELD (results — recursive, read-only) ═══════ */
  const renderObjectNode = (value, keyPath) => {
    const entries = Object.entries(value).filter(([, v]) => !isEmptyDeep(v));
    return entries.map(([k, v]) => {
      const nodeKey = `${keyPath}.${k}`;
      if (isScalar(v)) {
        const itemKey = `obj-${nodeKey}`;
        return (
          <div key={k} className="rec-mini-card">
            <div className="nested-subtitle">{highlightText(humanizeKey(k))}</div>
            <div className="numbered-row">
              <div className="row-content"><span className="content-value">{highlightText(fmtVal(v))}</span></div>
              <button className={`copy-btn ${copiedItems[itemKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${humanizeKey(k)}\n${fmtVal(v)}`, itemKey); }}>{copiedItems[itemKey] ? 'Copied!' : 'Copy'}</button>
            </div>
          </div>
        );
      }
      return (
        <div key={k} className="nested-mini-card">
          ...
          <div className="nested-group">{renderObjectNode(v, nodeKey)}</div>
        </div>
      );
    });
  };
  ...
  const renderObjectField = (record, fn, idx, sid) => {
    ...
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
    return <React.Fragment key={fn}>{renderObjectNode(val, fn)}</React.Fragment>;
  };
  ...
  if (!records || records.length === 0) {
    return (
      <div className="immunization-status-document" ref={containerRef}>
        <div className="document-header"><h1 className="document-title">Immunization Status</h1></div>
        <div className="no-data">No immunization status data available.</div>
      </div>
    );
  }
  ...
  return (
    <div className="immunization-status-document" ref={containerRef}>
      <div className="document-header">
        <h1 className="document-title">Immunization Status</h1>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ImmunizationStatusDocumentPDFTemplate document={pdfData} />} fileName="Immunization_Status.pdf" className="pdf-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
        <div className="search-container">
          <input type="text" className="search-input" placeholder="Search immunization records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          {searchTerm && <button className="clear-search" onClick={() => setSearchTerm('')}>&times;</button>}
        </div>
      </div>
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="immunization-status-record">
            <div className="record-header">
              <h2 className="record-title">{highlightText(`Immunization Status ${idx + 1}`)}</h2>
            </div>
...
export default ImmunizationStatusDocument;
