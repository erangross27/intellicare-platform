/**
 * OralPathologyBiopsyDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: oral_pathology_biopsy (displayed as "Oral Pathology Biopsy")
 *
 * 6 Sections:
 *   1. specimen-info: specimenSite, biopsyTechnique, specimenDimensionsCm, clinicalLesionDescription
 *   2. histopathology: histopathologicDiagnosis, epithelialDysplasiaGrade, binaryDysplasiaClassification
 *   3. tumor-characteristics: tumorDifferentiation, depthOfInvasionMm, tumorThicknessMm, perineuralInvasion, lymphovascularInvasion
 *   4. margins-staging: marginStatus, closestMarginDistanceMm, ajccTnmStage, worstPatternOfInvasion, tumorBuddingScore
 *   5. ancillary-studies: hpvAssociationStatus, immunohistochemistryPanel, boneInvolvement, extranodalExtension
 *   6. special-pathology: salivaryGlandPathology, odontogenicTumorClassification, directImmunofluorescenceFindings, lichenoidInflammationPattern
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import OralPathologyBiopsyDocumentPDFTemplate from '../pdf-templates/OralPathologyBiopsyDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './OralPathologyBiopsyDocument.css';
...
const DRAFT_KEY = 'oral_pathology_biopsyPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['specimenSite', 'biopsyTechnique', 'specimenDimensionsCm', 'clinicalLesionDescription', 'histopathologicDiagnosis', 'epithelialDysplasiaGrade', 'binaryDysplasiaClassification', 'tumorDifferentiation', 'marginStatus', 'ajccTnmStage', 'worstPatternOfInvasion', 'tumorBuddingScore', 'hpvAssociationStatus', 'salivaryGlandPathology', 'odontogenicTumorClassification', 'directImmunofluorescenceFindings', 'lichenoidInflammationPattern'];
const DATE_FIELDS = [];
...
/* ======= COMPONENT ======= */
const OralPathologyBiopsyDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Oral Pathology Biopsy ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/oral_pathology_biopsy/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/oral_pathology_biopsy/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== ORAL PATHOLOGY BIOPSY ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Oral Pathology Biopsy ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="oral-pathology-biopsy-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Oral Pathology Biopsy</h2></div>
        <div className="empty-state">No oral pathology biopsy records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="oral-pathology-biopsy-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Oral Pathology Biopsy</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OralPathologyBiopsyDocumentPDFTemplate document={pdfData} />} fileName="Oral_Pathology_Biopsy.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search oral pathology biopsy records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Oral Pathology Biopsy ${idx + 1}`)}</h3>
            </div>
            ...
            {renderSection(record, idx, 'margins-staging')}
            {renderSection(record, idx, 'ancillary-studies')}
            {renderSection(record, idx, 'special-pathology')}
          </div>
        ))}
      </div>
    </div>
...
export default OralPathologyBiopsyDocument;
