/**
 * InsomniaAssessmentDocument.jsx
 * June 2026 — Inline editing, blue glow theme
 * Collection: insomnia_assessment
 *
 * 6 Sections:
 *   1. sleep-metrics: sleepLatency, totalSleepTime, wakeAfterSleepOnset, numberOfAwakenings, sleepEfficiency
 *   2. insomnia-characterization: insomniaType, insomniaSeverityIndex, symptomDuration, sleepScheduleRegularity
 *   3. behavioral-environmental: bedtimeRoutine, caffeineIntake, alcoholUsePattern, daytimeNapping, sleepEnvironmentFactors
 *   4. comorbidities-medications: comorbidSleepDisorders[], psychiatricComorbidities[], medicationsCausingInsomnia[], currentSleepMedications[], epworthSleepinessScale
 *   5. arousal-diagnostics: cognitiveArousal, physiologicArousal, sleepDiaryCompleted, actigraphyUsed, polysomnographyIndicated
 *   6. treatment-planning: cbtiEligibility
 *
 * NOTE: this schema HAS a date field — record header shows a date badge above the numbered
 *       record title. `date` is header-only (never in a section).
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import InsomniaAssessmentDocumentPDFTemplate from '../pdf-templates/InsomniaAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './InsomniaAssessmentDocument.css';
...
const DRAFT_KEY = 'insomnia_assessmentPendingEdits';
const readDrafts = () => {
...
const SENTENCE_FIELDS = ['bedtimeRoutine', 'caffeineIntake', 'alcoholUsePattern', 'daytimeNapping', 'sleepEnvironmentFactors', 'cognitiveArousal', 'physiologicArousal', 'cbtiEligibility'];
/* sleepScheduleRegularity is SIMPLE (verbatim) — values like
   "irregular (weekday 10:30 PM-6:00 AM; weekend 11:30 PM-9:00 AM)" carry semicolons
   INSIDE parentheses; splitBySemicolon is not parenthesis-aware and would break them. */
const SIMPLE_FIELDS = ['insomniaType', 'symptomDuration', 'sleepScheduleRegularity'];
/* NUMBER_FIELDS: stored as real numbers in MongoDB (e.g. sleepLatency 6, totalSleepTime 6.7,
   numberOfAwakenings 2.4, insomniaSeverityIndex 15, epworthSleepinessScale 20). They must use a
   TYPED number input that saves parseFloat(value) so a clinician edit persists a Number, not "6". */
...
/* DATE_FIELDS: real record `date` (2026-02-10) → BlueDatePicker, rendered as the first field of the
   Assessment Information section (NOT a header pill). */
const DATE_FIELDS = ['date'];
/* Numeric fields where 0 means "not assessed / questionnaire not administered" — hidden when exactly 0.
   - sleepLatency 0 min, totalSleepTime 0 h, sleepEfficiency 0% are not clinically reportable values.
   - insomniaSeverityIndex 0 / epworthSleepinessScale 0: a patient assessed FOR insomnia scoring 0 on
     every questionnaire item is clinically incoherent — 0 is an extraction sentinel for "not administered".
   NOT hidden: wakeAfterSleepOnset (0 min = no mid-night waking, meaningful in pure sleep-onset insomnia)
   and numberOfAwakenings (0 = no awakenings, equally meaningful).
   Real decimals (e.g. totalSleepTime 6.7, numberOfAwakenings 2.4) still display. */
...
/* splitByComma: parenthesis-aware comma split with date-aware guard
   — items like "variable duration (10-90 min)" stay intact */
...
/* formatDate: Date / {$date} / ISO string → "February 10, 2026" (record date field) */
const formatDate = (dateValue) => {
  if (!dateValue) return '';
  try { const d = new Date(dateValue.$date || dateValue); if (isNaN(d.getTime())) return String(dateValue); return d.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }); } catch { return String(dateValue); }
};
/* toInputDate: any date value → 'YYYY-MM-DD' for BlueDatePicker's controlled value */
...
/* ═══════ COMPONENT ═══════ */
const InsomniaAssessmentDocument = ({ document: docProp, data, templateData }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        // A dotted purely-numeric suffix → array element (track as editedFields["field.N-idx"]); otherwise a field edit.
  ...
  /* numberShows: a NUMBER_FIELDS value is present when it is a finite number (or numeric string),
     applying per-field zero hiding via hasFieldVal (wakeAfterSleepOnset 0 / numberOfAwakenings 0 stay visible). */
  const numberShows = useCallback((record, fn, idx) => { const v = getFieldValue(record, fn, idx); if (v === null || v === undefined || v === '') return false; const n = typeof v === 'number' ? v : parseFloat(v); if (isNaN(n)) return false; return hasFieldVal(fn, n); }, [getFieldValue, hasFieldVal]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      const docTitle = 'insomnia assessment';
      const rt = `insomnia assessment ${oi + 1}`;
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Collect this section's pending edits (keys are `${fieldPart}-${idx}`; fieldPart base must be a section field)
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/insomnia_assessment/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/insomnia_assessment/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== INSOMNIA ASSESSMENT ===\n\n';
    pdfData.forEach((r) => {
      const oi = r._originalIdx ?? 0;
      text += `Insomnia Assessment ${oi + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: DATE FIELD (BlueDatePicker — saves ISO 'YYYY-MMT00:00:00.000Z') ═══════ */
  ...
  /* ═══════ RENDER: SIMPLE FIELD (no splitting — metrics, scores, type/duration classifications, schedule) ═══════ */
  ...
  /* ═══════ RENDER: NUMBER FIELD (typed number input — saves real Number via parseFloat, not a string) ═══════ */
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                ...
                <input type="text" inputMode="decimal" className="edit-number" value={editValue} onChange={e => { setEditValue(e.target.value); setSaveError(null); }} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter') { e.stopPropagation(); const numVal = parseFloat(editValue); if (isNaN(numVal) || editValue.trim() === '') { setSaveError('Please enter a valid number'); return; } handleSaveField(record, fn, idx, sid, null, numVal); } }} />
  ...
  /* ═══════ RENDER: BOOLEAN FIELD (Yes/No select — saves REAL booleans, false still renders "No" — clinically critical: diary NOT completed / PSG NOT indicated) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="insomnia-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Insomnia Assessment</h2></div>
        <div className="empty-state">No insomnia assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="insomnia-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Insomnia Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<InsomniaAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Insomnia_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search insomnia assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => {
          ...
          return (
            <div key={safeId(record) || idx} className="record-card">
              {/* Title-only header. The record `date` is an editable BlueDatePicker field inside the Assessment Information section (never a header pill). */}
              <div className="record-header">
                <h3 className="record-name">{highlightText(`Insomnia Assessment ${origIdx + 1}`)}</h3>
              </div>
              {renderSection(record, origIdx, 'assessment-info')}
              {renderSection(record, origIdx, 'sleep-metrics')}
              {renderSection(record, origIdx, 'insomnia-characterization')}
              {renderSection(record, origIdx, 'behavioral-environmental')}
              {renderSection(record, origIdx, 'comorbidities-medications')}
              {renderSection(record, origIdx, 'arousal-diagnostics')}
              {renderSection(record, origIdx, 'treatment-planning')}
            </div>
          );
        })}
      </div>
...
export default InsomniaAssessmentDocument;
