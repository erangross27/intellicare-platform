 ...
 * July 2026 — recursive, dot-path editable screening compliance
...
const SECTION_TITLES = {
  screeningInfo: 'Screening Information', providerFacility: 'Provider & Facility', scheduleDates: 'Schedule & Dates', resultsCompliance: 'Results & Compliance', guidelinesEligibility: 'Guidelines & Eligibility', followupReminders: 'Follow-Up & Reminders',
};
const FIELD_LABELS = {
  screeningType: 'Screening Type', screeningCategory: 'Screening Category', complianceStatus: 'Compliance Status', riskLevel: 'Risk Level', provider: 'Provider', facility: 'Facility', orderingProvider: 'Ordering Provider', date: 'Date', lastScreeningDate: 'Last Screening Date', dueDate: 'Due Date', nextDueDate: 'Next Due Date', screeningResult: 'Screening Result', screeningCompleted: 'Screening Completed', nonComplianceReason: 'Non-Compliance Reason', patientRefusalDocumented: 'Patient Refusal Documented', guidelineReference: 'Guideline Reference', eligibilityCriteria: 'Eligibility Criteria', ageAtScreening: 'Age at Screening', insuranceCoverage: 'Insurance Coverage', followUpRequired: 'Follow-Up Required', followUpRecommendation: 'Follow-Up Recommendation', remindersSent: 'Reminders Sent', lastReminderDate: 'Last Reminder Date', qualityMeasureCode: 'Quality Measure Code', barrierToCompliance: 'Barriers to Compliance',
};
const SECTION_FIELDS = {
  screeningInfo: ['screeningType', 'screeningCategory', 'complianceStatus', 'riskLevel'], providerFacility: ['provider', 'facility', 'orderingProvider'], scheduleDates: ['date', 'lastScreeningDate', 'dueDate', 'nextDueDate'], resultsCompliance: ['screeningResult', 'screeningCompleted', 'nonComplianceReason', 'patientRefusalDocumented'], guidelinesEligibility: ['guidelineReference', 'eligibilityCriteria', 'ageAtScreening', 'insuranceCoverage'], followupReminders: ['followUpRequired', 'followUpRecommendation', 'remindersSent', 'lastReminderDate', 'qualityMeasureCode', 'barrierToCompliance'],
};
const DATE_FIELDS = ['date', 'lastScreeningDate', 'dueDate', 'nextDueDate', 'lastReminderDate'];
const ENUM_FIELDS = ['complianceStatus', 'screeningCompleted', 'patientRefusalDocumented', 'insuranceCoverage', 'followUpRequired'];
...
const COMMA_FIELDS = [];
const COMMA_ARRAY_FIELDS = ['barrierToCompliance'];
const ARRAY_FIELDS = ['eligibilityCriteria', 'barrierToCompliance'];
const SEMICOLON_FIELDS = ['nonComplianceReason', 'followUpRecommendation'];
...
const enumOptionsFor = (path, value) => {
  ...
  const options = ['screeningCompleted', 'patientRefusalDocumented', 'followUpRequired'].includes(field) ? ['yes', 'no']
    : field === 'complianceStatus' ? ['Up to Date', 'Due Soon', 'Due', 'Overdue', 'Not Applicable']
      : field === 'insuranceCoverage' ? ['covered', 'partial', 'not covered', 'unknown'] : [];
...
const ScreeningComplianceDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.screening_compliance) return Array.isArray(record.screening_compliance) ? record.screening_compliance : [record.screening_compliance];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.screening_compliance) return Array.isArray(nested.screening_compliance) ? nested.screening_compliance : [nested.screening_compliance];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/screening_compliance/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/screening_compliance/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[ScreeningCompliance] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Screening Compliance ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="screening-compliance-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Screening Compliance</h2></div><div className="empty-state">No screening compliance records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Screening Compliance ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="screening-compliance-document" ref={containerRef}>
    <div className="document-header">
      ...
      <div className="header-actions">
        ...
        <PDFDownloadLink document={<ScreeningComplianceDocumentPDFTemplate document={pdfData} />} fileName="Screening_Compliance.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search screening compliance..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Screening Compliance ' + (recordIndex + 1))}</h3></div>
