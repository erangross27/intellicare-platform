/**
 * IvfCycleMonitoringDocument.jsx
 * July 2026 — Canonical one-pass polish: inline editing, blue glow theme
 * Collection: ivf_cycle_monitoring
 *
 * 6 Sections:
 *   1. cycle-overview: cycleDay, stimulationProtocol, totalGonadotropinDays, cumulativeGonadotropinDose
 *   2. follicle-assessment: follicleCount, leadFollicleDiameter, folliclesGreaterThan14mm, follicleDistributionRight, follicleDistributionLeft, ovarianVolume
 *   3. endometrial-status: endometrialThickness, endometrialPattern, freeFluidInPelvis
 *   4. hormone-levels: serumEstradiolLevel, serumLhLevel, serumProgesteroneLevel, antiMullerianHormone
 *   5. medication-protocol: gonadotropinType, gonadotropinDose, gnrhAntagonistStarted, ovarianResponseCategory, ovarianHyperstimulationRisk
 *   6. trigger-retrieval: triggerMedicationType, triggerMedicationDose, scheduledRetrievalDateTime
 *
 * Widgets: numbers → num-stepper; booleans → BlueSelect Yes/No; dates → BlueDatePicker; arrays → per-item.
 * Numeric 0 is a "not measured" sentinel here → skipped (numberShows). No record date (only createdAt).
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import IvfCycleMonitoringDocumentPDFTemplate from '../pdf-templates/IvfCycleMonitoringDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './IvfCycleMonitoringDocument.css';
...
const DRAFT_KEY = 'ivf_cycle_monitoringPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['stimulationProtocol', 'endometrialPattern', 'gonadotropinType', 'ovarianResponseCategory', 'ovarianHyperstimulationRisk', 'triggerMedicationType'];
...
/* ═══════ COMPONENT ═══════ */
const IvfCycleMonitoringDocument = ({ document: docProp }) => {
  ...
  /* number presence: render a numeric value only when it is meaningful (0 is a "not measured" sentinel → skip) */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `IVF Cycle Monitoring ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // localEdits keys are `${field}-${idx}` (value = full field value: string, number, boolean, OR full array).
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/ivf_cycle_monitoring/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/ivf_cycle_monitoring/${id}/approve`, { sectionId: sid, approved: true });
  ...
  // Copy: label on its own line (head), each scalar value numbered "1." — numbering lives ONLY in Copy/PDF,
  // never on-screen, and the label is never side-by-side with its value ("Label: value").
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (NUMBER_FIELDS.includes(f)) {
        ...
        const items = Array.isArray(val) ? val.filter(x => x !== null && x !== undefined && x !== '') : [];
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== IVF CYCLE MONITORING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `IVF Cycle Monitoring ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD (num-stepper; hide-zero) ═══════ */
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      ...
      if (ARRAY_FIELDS.includes(f)) { const v = getFieldValue(record, f, idx); return Array.isArray(v) && v.filter(x => x !== null && x !== undefined && x !== '').length > 0; }
  ...
  if (!records || records.length === 0) {
    return (
      <div className="ivf-cycle-monitoring-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">IVF Cycle Monitoring</h2></div>
        <div className="empty-state">No IVF cycle monitoring records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="ivf-cycle-monitoring-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">IVF Cycle Monitoring</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<IvfCycleMonitoringDocumentPDFTemplate document={pdfData} />} fileName="IVF_Cycle_Monitoring.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search IVF cycle monitoring..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`IVF Cycle Monitoring ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'cycle-overview')}
            {renderSection(record, idx, 'follicle-assessment')}
            {renderSection(record, idx, 'endometrial-status')}
            {renderSection(record, idx, 'hormone-levels')}
            {renderSection(record, idx, 'medication-protocol')}
            {renderSection(record, idx, 'trigger-retrieval')}
          </div>
        ))}
      </div>
    </div>
...
export default IvfCycleMonitoringDocument;
