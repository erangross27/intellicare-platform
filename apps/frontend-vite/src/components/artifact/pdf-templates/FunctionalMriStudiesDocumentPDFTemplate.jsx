/**
 * FunctionalMriStudiesDocumentPDFTemplate.jsx
 * Box-free B&W LETTER (canonical, memory 6a2d6af6) — mirrors the JSX: real record.reportDate (never createdAt),
 * values numbered ('1.' even singles), single-name label gate, narratives split on [.;] (labeled → sub-label +
 * numbered comma items). safeString scrubs non-Latin-1 glyphs. Rule #74: each field is ONE wrap={false} atomic
 * View with the sectionTitle riding INSIDE the first present field's View. Static PHI footer. Collection:
 * functional_mri_studies. reportType/urgency are free-text (not enums).
 */
...
const SECTION_ORDER = ['report-info', 'clinical-indication', 'findings-section', 'follow-up', 'recommendations-section'];
...
const FIELD_LABELS = {
  reportDate: 'Report Date', reportType: 'Report Type', urgency: 'Urgency',
  clinicalIndication: 'Clinical Indication', findings: 'Findings', followUp: 'Follow Up', recommendations: 'Recommendations',
};
...
const FunctionalMriStudiesDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].functional_mri_studies && Array.isArray(templateData[0].functional_mri_studies)) records = templateData[0].functional_mri_studies;
    else records = templateData;
  } else if (templateData && templateData.functional_mri_studies) {
    records = Array.isArray(templateData.functional_mri_studies) ? templateData.functional_mri_studies : [templateData.functional_mri_studies];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.functional_mri_studies ? (Array.isArray(dd.functional_mri_studies) ? dd.functional_mri_studies : [dd.functional_mri_studies]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Functional MRI Studies</Text></View>
        <Text style={styles.emptyState}>No functional MRI study records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Functional MRI Study ${idx + 1}`}</Text>
...
export default FunctionalMriStudiesDocumentPDFTemplate;
