...
function buildContext(req, operation = 'read') { return { serviceId: 'kidney-function-reports-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['date', 'provider', 'facility', 'status', 'notes', 'details', 'type', 'category', 'description', 'summary', 'findings', 'recommendations', 'comments', 'attachments', 'serumCreatinine', 'estimatedGFR', 'bloodUreaNitrogen', 'creatinineClearance', 'urinalysisProtein', 'albuminCreatinineRatio', 'ckdStage', 'sodiumLevel', 'potassiumLevel', 'phosphorusLevel', 'calciumLevel', 'parathyroidHormone', 'hemoglobinLevel', 'hematocritLevel', 'bicarbonateLevel', 'proteinuria24Hour', 'urineSpecificGravity', 'renalUltrasoundFindings', 'dialysisType', 'ktVRatio', 'cystatinC', 'microalbuminuria', 'fractionalExcretionSodium'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('kidney_function_reports', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[KidneyFunctionReports] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('kidney_function_reports', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[KidneyFunctionReports] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
