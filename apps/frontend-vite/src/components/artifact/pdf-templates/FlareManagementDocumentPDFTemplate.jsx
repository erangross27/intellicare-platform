/**
 * FlareManagementDocumentPDFTemplate.jsx
 ...
 * stacked (sub-label + numbered value; keys with spaces kept literal), recommendations date-grouped, the three
 * flare-criteria narratives split as labeled sub-group + numbered comma items. splitBySentence splits on [.;].
 * Rule #74: each field is ONE wrap={false} atomic View with the sectionTitle riding INSIDE the first present
 * field's View (anti-orphan). PHI footer is STATIC only. Collection: flare_management
 */
...
const SECTION_ORDER = ['session-info', 'rescue-therapy', 'escalation-criteria', 'admission-criteria', 'findings-section', 'assessment-section', 'plan-section', 'results-section', 'recommendations-section', 'notes-section'];
...
const FIELD_LABELS = {
  ...
  rescueTherapy: 'Rescue Therapy', escalationCriteria: 'Escalation Criteria', admissionCriteria: 'Admission Criteria',
...
const KEY_OVERRIDES = {
  esr: 'ESR', crp: 'CRP', wbc: 'WBC', hgb: 'Hgb', hct: 'Hct', plt: 'PLT', bun: 'BUN', alt: 'ALT', ast: 'AST',
  cdai: 'CDAI', das28: 'DAS28', hbi: 'HBI', fcp: 'FCP',
};
...
const sentenceRows = (text) => {
  ...
  sentences.forEach(s => {
    ...
    if (parsed.isLabeled) {
      ...
      if (parts.length >= 3) parts.forEach(p => rows.push({ type: 'item', text: safeString(strip(p)), num: n++ }));
      else rows.push({ type: 'item', text: safeString(strip(s)), num: n++ });
    }
  });
  return rows;
};
...
const FlareManagementDocumentPDFTemplate = ({ document: docProp }) => {
  let records = [];
  if (Array.isArray(docProp)) {
    if (docProp.length > 0 && docProp[0].flare_management && Array.isArray(docProp[0].flare_management)) records = docProp[0].flare_management;
    else records = docProp;
  } else if (docProp && docProp.flare_management) {
    records = Array.isArray(docProp.flare_management) ? docProp.flare_management : [docProp.flare_management];
  } else if (docProp) {
    records = [docProp];
  }
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Flare Management</Text></View>
        <Text style={styles.emptyState}>No flare management data available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Flare Management ${idx + 1}`}</Text>
...
export default FlareManagementDocumentPDFTemplate;
