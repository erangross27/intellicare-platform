/**
 * FetalAssessmentDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * June 2026 — 100% schema coverage at FULL TEMPLATE STANDARD.
 *   Added OBJECT recursive editor (renderObjectLeaf/renderObjectNode/renderObjectField — donor
 *   PointOfCareUltrasoundHeartRateDocument) for fetalMovement, leopoldManeuvers, results;
 *   array-of-{recommendation,date} renderRecommendationsField for recommendations.
 * Collection: fetal_assessment
 *
 * Sections:
 *   1. session-info: date (date picker), provider, facility, status
 *   2. fetal-parameters: fetalHeartRate, fundalHeight, fundalHeightPercentile, fetalPosition, fetalPresentation, estimatedFetalWeight (all STRING)
 *   3. fetal-movement: fetalMovement (OBJECT — recursive editor; concernsReported boolean → Yes/No)
 *   4. leopold-maneuvers: leopoldManeuvers (OBJECT — recursive editor)
 *   5. findings-section: findings (sentence with parseLabel)
 *   6. assessment-section: assessment (sentence)
 *   7. plan-section: plan (sentence with parseLabel)
 *   8. results-section: results (OBJECT — recursive editor)
 *   9. recommendations-section: recommendations (ARRAY of {recommendation, date})
 *  10. notes-section: notes (sentence)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import FetalAssessmentDocumentPDFTemplate from '../pdf-templates/FetalAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './FetalAssessmentDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'session-info': 'Session Information',
  'fetal-parameters': 'Fetal Parameters',
...
const FIELD_LABELS = {
  ...
  fundalHeightPercentile: 'Fundal Height Percentile',
  ...
  leopoldManeuvers: 'Leopold Maneuvers',
  findings: 'Findings',
...
const SECTION_FIELDS = {
  ...
  'leopold-maneuvers': ['leopoldManeuvers'],
  'findings-section': ['findings'],
  'assessment-section': ['assessment'],
  'plan-section': ['plan'],
  'results-section': ['results'],
...
const STRING_FIELDS = ['provider', 'facility', 'fetalHeartRate', 'fundalHeight', 'fundalHeightPercentile', 'fetalPosition', 'fetalPresentation', 'estimatedFetalWeight', 'findings', 'assessment', 'plan', 'notes'];
const DATE_FIELDS = ['date'];
/* Known clinical/status scale → BlueSelect full-scale dropdown. status = Active/Completed/Not Active. */
...
/* DISPLAY/EDIT recursive object fields (booleans → Yes/No select, dotted-path saves) */
...
/* humanizeKey for OBJECT nested keys */
const KEY_OVERRIDES = {
  efw: 'EFW', afi: 'AFI', nst: 'NST', bpp: 'BPP', fhr: 'FHR', ga: 'GA', us: 'US',
};
...
/* deep value helpers (for OBJECT/ARRAY fields) */
const isEmptyDeep = (v) => {
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or dotted "root.path" for OBJECT leaves) */
const DRAFT_KEY = 'fetalAssessmentPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const FetalAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.fetal_assessment) return Array.isArray(r.fetal_assessment) ? r.fetal_assessment : [r.fetal_assessment];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.fetal_assessment) return Array.isArray(dd.fetal_assessment) ? dd.fetal_assessment : [dd.fetal_assessment]; return [dd]; }
      return [r];
    });
    return arr.filter(r => r && typeof r === 'object');
  }, [docProp]);

  /* ═══════ SAFE ID (module-local helper used by rehydrate + handlers) ═══════ */
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Draft store shape: { [recordId]: { [field]: fullFieldValue } } where field is a top-level field name.
     We mark the field as edited/pending so the section shows yellow "Pending Approve" until approved. */
  useEffect(() => {
  ...
  const splitBySentence = useCallback((text) => {
    ...
    return text.split(/(?<!\b(?:Mr|Mrs|Ms|Dr|St|Jr|Sr|Prof|Rev|Gen|Col|Sgt|vs|etc|EFW|AFI|NST|BPP))[.;](?:\s+)/).map(s => s.trim()).filter(s => s && !/^[;.,!?]+$/.test(s));
  }, []);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Fetal Assessment ${idx + 1}`.toLowerCase();
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. fetalMovement.concernsReported) */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    // Build the full merged object value locally (no DB write); stage it as a draft.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Collect this record's pending edits for this section (localEdits key = "<fieldPart>-<idx>")
      const suffix = `-${idx}`;
      ...
      for (const localKey of toCommit) {
        ...
        const payload = { field: isArrayIndex ? fieldPart.slice(0, lastDot) : fieldPart, value: localEdits[localKey] };
        if (isArrayIndex) payload.arrayIndex = parseInt(trailing, 10);
        const resp = await secureApiClient.put(`/api/edit/fetal_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/fetal_assessment/${id}/approve`, { sectionId: sid, approved: true });
      ...
      const store = readDrafts();
      if (store[id]) {
        toCommit.forEach(localKey => { const fieldPart = localKey.slice(0, -suffix.length); const baseField = fieldPart.includes('.') ? fieldPart.slice(0, fieldPart.indexOf('.')) : fieldPart; delete store[id][baseField]; });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    if (!body.trim()) return ''; // every field empty → drop the whole section (no orphan title+EQ)
    return `${title}\n${'='.repeat(40)}\n\n${body}`;
  }, [getFieldValue, hasVal, fmtVal, splitBySentence, formatSentenceFieldLines, objectCopyLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== FETAL ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Fetal Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; bool→Yes/No, number+unit→number input, "4/5"→text) ═══════ */
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              setSaveError(null);
                              stageSentenceDraft(record, fn, idx, sid, newArr, { [itemKey]: 'edited' });
                            }}>{saving ? 'Saving...' : 'Save'}</button>
                            <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                          </div>
                        </div>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="fetal-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Fetal Assessment</h2></div>
        <div className="empty-state">No fetal assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="fetal-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Fetal Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FetalAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Fetal_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search fetal assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Fetal Assessment ${idx + 1}`)}</h3>
            </div>
...
export default FetalAssessmentDocument;
