 ...
 *
 * Covers case overview, indexed team roles, clinical narrative, recommendations,
 * generic results leaves, and notes.
 */
...
const DRAFT_KEY = 'symptomProgressionPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  overview: ['date', 'type', 'status', 'provider', 'facility'],
  progression: ['timeline', 'week1', 'week2', 'week3', 'current'],
...
const ENUM_FIELDS = { status: ['stable', 'improving', 'worsening', 'resolved', 'active'] };
const COMMA_ARRAY_FIELDS = new Set(['notes']);
...
const isDateField = path => path === 'date' || /^recommendations\.\d+\.date$/.test(path);
...
const SymptomProgressionDocument = ({ document: docProp }) => {
  ...
  const fmtVal = useCallback((input) => { const v = scalarValue(input); if (typeof v === 'boolean') return v ? 'Yes' : 'No'; if (typeof v === 'number') return String(v); return String(v || '').replace(/\u2192/g, '->'); }, []);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/symptom_progression/${id}/edit`, payload);
    ...
    <div className="symptom-progression-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<SymptomProgressionDocumentPDFTemplate document={pdfData} />} fileName="Symptom_Progression.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search symptom progression..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
