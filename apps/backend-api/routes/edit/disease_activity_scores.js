/**
 * DiseaseActivityScores Edit Route
 * ServiceId: disease-activity-scores-edit-service
...
function buildContext(req, operation = 'read') { return { serviceId: 'disease-activity-scores-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }

const ALLOWED_FIELDS = [
  'date', 'provider', 'facility', 'status', 'findings', 'recommendations', 'results', 'assessment',
  'plan', 'notes',
  'mayoScore.stoolFrequency', 'mayoScore.rectalBleeding', 'mayoScore.endoscopicFindings',
  'mayoScore.physicianAssessment', 'mayoScore.totalScore'
];

// Dotted-prefixes for recursive OBJECT-field leaf edits (results + score indices)
const ALLOWED_PREFIXES = [
  'results.', 'mayoScore.', 'harveyBradshaw.', 'cdai.', 'partialMayo.',
  'simpleClinicalColitisActivity.', 'pucai.'
];
function isFieldAllowed(field) {
  if (typeof field !== 'string') return false;
  if (ALLOWED_FIELDS.includes(field)) return true;
  return ALLOWED_PREFIXES.some(p => field.startsWith(p) && field.length > p.length);
}
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('disease_activity_scores', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[DiseaseActivityScores] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('disease_activity_scores', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[DiseaseActivityScores] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
