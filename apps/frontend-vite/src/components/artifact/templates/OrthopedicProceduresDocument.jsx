/**
 * OrthopedicProceduresDocument.jsx
 * Canonical one-pass polish — inline editing, blue glow theme.
 * Collection: orthopedic_procedures
 *
 * Sections:
 *   1. procedure-info: date (title-header BlueDatePicker), provider, facility, status, time, type
 *   2. procedure: procedure (splitBySentence — semicolon list)
 *   3. technique: technique (splitBySentence + parseLabel — embedded labels)
 *   4. anesthesia: anesthesia
 *   5. findings: findings (splitBySentence — semicolon list)
 *   6. post-procedure-imaging: postProcedureImaging.study, .findings, .adequateReduction (nested bool → BlueSelect)
 *   7. immobilization: immobilization (splitBySentence — semicolon list)
 *   8. complications: complications (array of strings)
 *   9. assessment: assessment
 *  10. plan: plan
 *  11. recommendations: recommendations (array of objects {recommendation, date})
 *  12. notes: notes (splitBySentence + parseLabel — embedded labels)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import OrthopedicProceduresDocumentPDFTemplate from '../pdf-templates/OrthopedicProceduresDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './OrthopedicProceduresDocument.css';

/* ═══════ CONSTANTS ═══════ */
const API_BASE = '/api/edit/orthopedic_procedures';
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = dotted field path, e.g. "postProcedureImaging.study") */
const DRAFT_KEY = 'orthopedic_proceduresPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'findings': 'Findings',
  'post-procedure-imaging': 'Post-Procedure Imaging',
...
const FIELD_LABELS = {
  ...
  anesthesia: 'Anesthesia',
  findings: 'Findings',
  'postProcedureImaging.study': 'Study',
  'postProcedureImaging.findings': 'Imaging Findings',
  'postProcedureImaging.adequateReduction': 'Adequate Reduction',
  immobilization: 'Immobilization',
  complications: 'Complications',
  assessment: 'Assessment',
  plan: 'Plan',
...
const SECTION_FIELDS = {
  'procedure-info': ['date', 'provider', 'facility', 'status', 'time', 'type'],
  'procedure': ['procedure'],
  'technique': ['technique'],
  'anesthesia': ['anesthesia'],
  'findings': ['findings'],
  'post-procedure-imaging': ['postProcedureImaging.study', 'postProcedureImaging.findings', 'postProcedureImaging.adequateReduction'],
  'immobilization': ['immobilization'],
  'complications': ['complications'],
  'assessment': ['assessment'],
  'plan': ['plan'],
...
const BOOLEAN_FIELDS = ['postProcedureImaging.adequateReduction'];
const ARRAY_FIELDS = ['complications'];
const OBJECT_ARRAY_FIELDS = ['recommendations'];
const STRING_FIELDS = [
  'provider', 'facility', 'status', 'time', 'type',
  'procedure', 'technique', 'anesthesia', 'findings',
  'postProcedureImaging.study', 'postProcedureImaging.findings',
  'immobilization', 'assessment', 'plan', 'notes',
];

/* ═══════ SHARED SCALAR/OBJECT HELPERS ═══════ */
...
/* parseLabel: detect "Label: value" patterns (skip subordinate-clause openers so a grammatical colon
   like "If X: Y" never becomes a subtitle). */
...
/* splitByComma: parenthesis-aware comma split with a thousands/adjacent-digit guard */
...
/* Resolve nested dot-path values: "postProcedureImaging.study" -> record.postProcedureImaging.study */
...
/* ═══════ COMPONENT ═══════ */
const OrthopedicProceduresDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r?.orthopedic_procedures) return Array.isArray(r.orthopedic_procedures) ? r.orthopedic_procedures : [r.orthopedic_procedures];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.orthopedic_procedures) return Array.isArray(dd.orthopedic_procedures) ? dd.orthopedic_procedures : [dd.orthopedic_procedures]; return [dd]; }
      if (r?.records && Array.isArray(r.records) && !r.date && !r.procedure) return r.records;
      return [r];
    });
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        const strVal = Array.isArray(val) ? JSON.stringify(val) : typeof val === 'object' ? JSON.stringify(val) : fmtVal(val);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Orthopedic Procedure ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    } catch (err) { console.error('[OrthopedicProcedures] Approve error:', err); setSaveError('Approve failed.'); }
    ...
    sentences.forEach(s => {
      ...
      if (parsed.isLabeled) {
        ...
        if (parts.length >= 2) {
          ...
          parts.forEach(item => { const ip = parseLabel(item); lines.push(`  ${n++}. ${ip.isLabeled ? `${ip.label}: ${ip.value}` : item}`); });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    // Skip sections with no populated fields (so empty section titles never leak into Copy All / JSX-PDF parity).
        ...
        if (arr.length) {
          ...
          arr.forEach((it, i) => {
            if (typeof it === 'object' && it) {
              const primaryKey = it.recommendation !== undefined ? 'recommendation' : Object.keys(it)[0];
            ...
            } else { text += `${i + 1}. ${fmtScalar(it)}\n`; }
          });
          text += '\n';
        }
      } else if (ARRAY_FIELDS.includes(f) || Array.isArray(val)) {
        const arr = Array.isArray(val) ? val.filter(Boolean) : [val];
        text += `${label}\n${arr.map((it, i) => `${i + 1}. ${typeof it === 'object' ? fmtScalar(it.recommendation || JSON.stringify(it)) : it}`).join('\n')}\n\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== ORTHOPEDIC PROCEDURES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Orthopedic Procedure ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: HEADER DATE (title-header BlueDatePicker) ═══════ */
        ...
        {isEditing ? (
          <span className="edit-field-container" onClick={e => e.stopPropagation()}>
            ...
            <span className="edit-actions">
              <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date'); return; } handleSaveField(record, 'date', idx, 'procedure-info', null, editValue + 'T00:00:00.000Z'); }}>{saving ? 'Saving...' : 'Save'}</button>
  ...
  /* ═══════ RENDER: STRING FIELD with splitBySentence + parseLabel + comma decomposition ═══════ */
  ...
  /* ═══════ RENDER: ARRAY OF STRINGS (complications) ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          const itemStr = typeof item === 'object' ? String(item.recommendation || JSON.stringify(item)) : String(item);
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const newArr = [...items]; newArr[itemIdx] = editValue.trim(); stageDraft(record, idx, fn, newArr); setEditedFields(prev => ({ ...prev, [itemKey]: 'edited' })); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  /* ═══════ RENDER: OBJECT-ARRAY FIELD (recommendations: {recommendation, date}) ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          const primaryKey = (item && typeof item === 'object' && item.recommendation !== undefined) ? 'recommendation' : (item && typeof item === 'object' ? Object.keys(item)[0] : null);
          const heading = primaryKey ? (fmtScalar(item[primaryKey]) || `${humanizeKey(fn)} ${itemIdx + 1}`) : fmtScalar(item);
          const subEntries = (item && typeof item === 'object') ? Object.entries(item).filter(([k, v]) => k !== primaryKey && !isEmptyDeep(v)) : [];
          const itemText = (item && typeof item === 'object') ? Object.values(item).map(v => fmtScalar(v)).join(' ').toLowerCase() : fmtScalar(item).toLowerCase();
          ...
          return (
            <div key={itemIdx} className="nested-mini-card" style={{ marginTop: itemIdx > 0 ? 8 : 0 }}>
              <div className={`numbered-row ${headBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!headEditing) { setEditingField(itemKey); setEditValue(heading); setSaveError(null); } }}>
                {headEditing ? (
                  <div className="edit-field-container">
                    <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => {
                        ...
                        const newArr = curArr.map((r, i) => i === itemIdx ? (typeof r === 'object' && primaryKey ? { ...r, [primaryKey]: trimmed } : trimmed) : r);
              ...
              {subEntries.map(([k, v]) => {
                ...
                const subStr = k === 'date' ? formatDate(v) : fmtScalar(v);
                return (
                  <div key={k} className="numbered-row">
                    <div className="row-content"><span className="content-subtitle">{highlightText(humanizeKey(k))}</span><span className="content-value">{highlightText(subStr)}</span></div>
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      if (f === 'date') return false; // date lives in the record header, not the section body
    ...
    return (
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            if (f === 'date') return null; // rendered in the record header
  ...
  if (!records || records.length === 0) {
    return (
      <div className="orthopedic-procedures-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Orthopedic Procedures</h2></div>
        <div className="empty-state">No orthopedic procedure records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="orthopedic-procedures-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Orthopedic Procedures</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OrthopedicProceduresDocumentPDFTemplate document={pdfData} />} fileName="Orthopedic_Procedures.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search orthopedic procedures..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={safeId(record) || idx} className="record-card">
            <div className="record-header">
              <div className="record-meta-row">{renderHeaderDate(record, idx)}</div>
              <div className="record-title-row"><h3 className="record-name">{highlightText(`Orthopedic Procedure ${idx + 1}`)}</h3></div>
            </div>
...
export default OrthopedicProceduresDocument;
