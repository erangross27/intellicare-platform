...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'workers-compensation-evaluation-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['date', 'injuryDate', 'dateReportedToEmployer', 'claimNumber', 'employerName', 'occupationTitle', 'workStatus', 'workRestrictions', 'maxMedicalImprovement', 'mmiDate', 'returnToWorkDate', 'lostWorkDays', 'causalityOpinion', 'apportionmentPercentage', 'permanentImpairmentRating', 'functionalCapacityEvaluation', 'injuryMechanism', 'bodyPartsAffected', 'treatmentAuthorization', 'futureCareMedicalNeeds', 'vocationalRehabilitation', 'jobDemandAnalysis', 'modifiedDutyOptions', 'priorWorkInjuries', 'insuranceCarrier'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('workers_compensation_evaluation', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[workers_compensation_evaluation] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('workers_compensation_evaluation', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[workers_compensation_evaluation] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
