/**
 * DexaScanReportsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: dexa_scan_reports
 *
 * 7 Sections:
 *   1. t-scores: tScoreLumbarSpine, tScoreFemoralNeck, tScoreTotalHip
 *   2. z-scores: zScoreLumbarSpine, zScoreFemoralNeck
 *   3. bone-density: boneDensityLumbarSpine, boneDensityFemoralNeck, boneDensityTotalHip, totalBodyBoneDensity
 *   4. classification-risk: whoClassification, fracRiskMajor10Year, fracRiskHip10Year
 *   5. vertebral-fractures: vertebralFracturesPresent, vertebralFractureGrade
 *   6. body-composition: bodyCompositionAnalyzed, leanBodyMass, bodyFatPercentage
 *   7. scan-followup: scanQuality, artifactsPresent, treatmentRecommended, followUpInterval,
 *                     priorDexaScanDate, boneDensityChangePercent, leastSignificantChange
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import DexaScanReportsDocumentPDFTemplate from '../pdf-templates/DexaScanReportsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './DexaScanReportsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the field name) */
const DRAFT_KEY = 'dexa_scan_reportsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'classification-risk': 'WHO Classification & Fracture Risk',
  'vertebral-fractures': 'Vertebral Fractures',
  'body-composition': 'Body Composition',
  'scan-followup': 'Scan Quality & Follow-up',
};
...
const SECTION_FIELDS = {
  't-scores': ['tScoreLumbarSpine', 'tScoreFemoralNeck', 'tScoreTotalHip'],
  'z-scores': ['zScoreLumbarSpine', 'zScoreFemoralNeck'],
  'bone-density': ['boneDensityLumbarSpine', 'boneDensityFemoralNeck', 'boneDensityTotalHip', 'totalBodyBoneDensity'],
  'classification-risk': ['whoClassification', 'fracRiskMajor10Year', 'fracRiskHip10Year'],
  'vertebral-fractures': ['vertebralFracturesPresent', 'vertebralFractureGrade'],
  'body-composition': ['bodyCompositionAnalyzed', 'leanBodyMass', 'bodyFatPercentage'],
  'scan-followup': ['scanQuality', 'artifactsPresent', 'treatmentRecommended', 'followUpInterval', 'priorDexaScanDate', 'boneDensityChangePercent', 'leastSignificantChange'],
};
...
const BOOLEAN_FIELDS = ['vertebralFracturesPresent', 'bodyCompositionAnalyzed', 'artifactsPresent', 'treatmentRecommended'];
const DATE_FIELDS = ['priorDexaScanDate'];
const STRING_FIELDS = ['whoClassification', 'vertebralFractureGrade', 'scanQuality'];

/* The batch extractor writes 0 into every numeric it could not read (z-scores, bone density,
   FRAX, body composition). On a DEXA report a fabricated "0" reads as a REAL result
   (Z-score 0 = age-matched mean), so sentinel zeros are hidden like empty strings. */
const HIDE_ZERO_FIELDS = NUMBER_FIELDS;

const COPY_LINE_EQ = '='.repeat(40);
const COPY_LINE_DASH = '-'.repeat(40);

/* Known clinical scales → full-scale dropdowns (WHO densitometric classes, Genant vertebral
   fracture grades, scan-quality grades). Off-scale stored values stay selectable. */
const ENUM_FIELDS = {
  whoClassification: ['Normal', 'Osteopenia', 'Osteoporosis', 'Severe Osteoporosis'],
  vertebralFractureGrade: ['Grade 0 (Normal)', 'Grade 1 (Mild)', 'Grade 2 (Moderate)', 'Grade 3 (Severe)'],
...
/* T-/Z-scores and bone-density change are legitimately NEGATIVE — never clamp those. */
const NON_NEGATIVE_FIELDS = ['boneDensityLumbarSpine', 'boneDensityFemoralNeck', 'boneDensityTotalHip', 'totalBodyBoneDensity', 'fracRiskMajor10Year', 'fracRiskHip10Year', 'leanBodyMass', 'bodyFatPercentage', 'followUpInterval', 'leastSignificantChange'];
/* BMD is reported in g/cm² to 3 decimals → fine 0.01 step; months step whole */
const BMD_STEP_FIELDS = ['boneDensityLumbarSpine', 'boneDensityFemoralNeck', 'boneDensityTotalHip', 'totalBodyBoneDensity', 'leastSignificantChange'];
const stepForField = (fn, v) => {
  if (fn === 'followUpInterval') return 1;
  if (BMD_STEP_FIELDS.includes(fn)) return 0.01;
  return /\.\d/.test(String(v)) ? 0.1 : 1;
};
...
/* ═══════ COMPONENT ═══════ */
const DexaScanReportsDocument = ({ document: docProp }) => {
  ...
  /* field-aware visibility: sentinel 0 in a numeric field = "not measured" → hidden everywhere */
  const fieldShows = useCallback((fn, v) => hasVal(v) && !(typeof v === 'number' && v === 0 && HIDE_ZERO_FIELDS.includes(fn)), [hasVal]);

  /* stepper helper: adjust editValue by the field's step, clamped where negatives are impossible */
  const stepEditValue = (fn, dir) => {
    setEditValue(prev => {
      const n = parseFloat(prev);
      const s = stepForField(fn, prev);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `DEXA Scan Report ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Only commit pending edits for fields belonging to THIS section
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/dexa_scan_reports/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/dexa_scan_reports/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    return text;
  }, [getFieldValue, fieldShows, fmtVal, formatSentenceFieldLines, splitBySentence]);

  const copyAllText = useCallback(async () => {
    let text = `DEXA Scan Reports\n${COPY_LINE_EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `DEXA Scan Report ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  /* ═══════ RENDER: ENUM FIELD (known clinical scale dropdown) ═══════ */
  ...
  /* ═══════ RENDER: STRING EDITABLE FIELD (with sentence support) ═══════ */
  const renderStringEditableField = (record, fn, idx, sid, title) => {
    const val = getFieldValue(record, fn, idx); if (!fieldShows(fn, val)) return null;
    const strVal = fmtVal(val);
    const sentences = splitBySentence(strVal);
    const label = FIELD_LABELS[fn] || fn;
    ...
    // If multi-sentence, render sentence-editable
    if (sentences.length > 1) {
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            return renderStringEditableField(record, f, idx, sid, title);
          })}
        </div>
      </div>
    );
  ...
  if (!records || records.length === 0) {
    return (
      <div className="dexa-scan-reports-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">DEXA Scan Reports</h2></div>
        <div className="empty-state">No DEXA scan report records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="dexa-scan-reports-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">DEXA Scan Reports</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<DexaScanReportsDocumentPDFTemplate document={pdfData} />} fileName="DEXA_Scan_Reports.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search DEXA scan reports..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`DEXA Scan Report ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 't-scores')}
            {renderSection(record, idx, 'z-scores')}
            {renderSection(record, idx, 'bone-density')}
            {renderSection(record, idx, 'classification-risk')}
            {renderSection(record, idx, 'vertebral-fractures')}
            {renderSection(record, idx, 'body-composition')}
            {renderSection(record, idx, 'scan-followup')}
          </div>
        ))}
      </div>
    </div>
...
export default DexaScanReportsDocument;
