/**
 * FitnessForDutyEvaluationsDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: fitness_for_duty_evaluations
 *
 * 7 Sections:
 *   1. employee-info: employeeIdentifier, jobClassification, physicalDemandsLevel, functionalCapacityRating, returnToWorkDate
 *   2. pulmonary: vitalCapacity, fev1Measurement, peakFlowRate, respiratoryClearance
 *   3. vision-hearing: snellenVisualAcuity, colorVisionStatus, peripheralVisionFields, audiometryResults
 *   4. cardiovascular-fitness: stressTestResult, maximumOxygenUptake
 *   5. musculoskeletal: gripStrengthMeasurement, rangeOfMotionAssessment, liftingCapacityTest, balanceStabilityAssessment
 *   6. clearances: psychologicalClearance, substanceScreeningResults, immunizationStatus, dotPhysicalClassification, medicalRestrictions
 *   7. risk-assessment: occupationalHealthRisk
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import FitnessForDutyEvaluationsDocumentPDFTemplate from '../pdf-templates/FitnessForDutyEvaluationsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './FitnessForDutyEvaluationsDocument.css';
...
const DRAFT_KEY = 'fitness_for_duty_evaluationsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'musculoskeletal': 'Musculoskeletal Assessment',
  'clearances': 'Clearances & Compliance',
...
const FIELD_LABELS = {
  ...
  physicalDemandsLevel: 'Physical Demands Level',
  functionalCapacityRating: 'Functional Capacity Rating',
  returnToWorkDate: 'Return to Work Date',
  vitalCapacity: 'Vital Capacity (mL)',
  fev1Measurement: 'FEV1 Measurement (mL)',
  peakFlowRate: 'Peak Flow Rate (L/s)',
  respiratoryClearance: 'Respiratory Clearance',
  snellenVisualAcuity: 'Snellen Visual Acuity',
  colorVisionStatus: 'Color Vision Status',
  peripheralVisionFields: 'Peripheral Vision Fields',
  audiometryResults: 'Audiometry Results',
  stressTestResult: 'Stress Test Result',
  maximumOxygenUptake: 'Maximum Oxygen Uptake (VO2max)',
  gripStrengthMeasurement: 'Grip Strength (kg)',
  rangeOfMotionAssessment: 'Range of Motion Assessment',
  liftingCapacityTest: 'Lifting Capacity Test',
  balanceStabilityAssessment: 'Balance and Stability Assessment',
  psychologicalClearance: 'Psychological Clearance',
  substanceScreeningResults: 'Substance Screening Results',
  immunizationStatus: 'Immunization Status',
  dotPhysicalClassification: 'DOT Physical Classification',
...
const STRING_FIELDS = ['employeeIdentifier', 'jobClassification', 'physicalDemandsLevel', 'functionalCapacityRating', 'snellenVisualAcuity', 'colorVisionStatus', 'peripheralVisionFields', 'audiometryResults', 'stressTestResult', 'rangeOfMotionAssessment', 'liftingCapacityTest', 'balanceStabilityAssessment', 'substanceScreeningResults', 'immunizationStatus', 'dotPhysicalClassification', 'occupationalHealthRisk'];
const DATE_FIELDS = ['returnToWorkDate'];
/* Fixed-choice clinical scales → BlueSelect */
const ENUM_FIELDS = ['physicalDemandsLevel', 'functionalCapacityRating'];
const ENUM_OPTIONS = {
  physicalDemandsLevel: ['Sedentary', 'Light', 'Medium', 'Heavy', 'Very Heavy'],
  functionalCapacityRating: ['Full', 'Modified', 'Restricted', 'Unable'],
};
...
/* Comma-list result strings → one editable row per item (genuine lists) */
...
const toInputDate = (dateValue) => {
  if (!dateValue) return '';
  try { const d = new Date(dateValue.$date || dateValue); return isNaN(d.getTime()) ? '' : d.toISOString().split('T')[0]; } catch { return ''; }
};

/* ═══════ COMPONENT ═══════ */
const FitnessForDutyEvaluationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.fitness_for_duty_evaluations) return Array.isArray(r.fitness_for_duty_evaluations) ? r.fitness_for_duty_evaluations : [r.fitness_for_duty_evaluations];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.fitness_for_duty_evaluations) return Array.isArray(dd.fitness_for_duty_evaluations) ? dd.fitness_for_duty_evaluations : [dd.fitness_for_duty_evaluations]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Fitness for Duty Evaluation ${idx + 1}`.toLowerCase();
  ...
  // fieldPart = "field" (scalar/array-whole) — kept generic so the Approve parser can reverse it.
  const stageDraft = useCallback((record, fn, idx, sid, value, fieldPart) => {
    const id = safeId(record); if (!id) return;
    const part = fieldPart || fn;
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    setApproving(true);
    try {
      // Collect this record's pending edits for this section's fields (localEdits keys are "<fieldPart>-<idx>").
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/fitness_for_duty_evaluations/${id}/edit`, payload);
        if (!resp || !resp.success) throw new Error((resp && resp.error) || 'save failed');
      }
      // Flag the record approved (audit trail)
      await secureApiClient.put(`/api/edit/fitness_for_duty_evaluations/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const next = { ...prev }; toCommit.forEach(k => delete next[k]); return next; });
      // Drop this record's committed drafts from localStorage (only the committed fieldParts)
      const store = readDrafts();
      if (store[id]) {
    ...
    } catch (err) { console.error('[FitnessForDutyEvaluations] Approve error:', err); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        if (items.length > 0) { body += head; items.forEach((item, i) => { body += `${i + 1}. ${item}\n`; }); body += '\n'; }
      } else if (COMMA_FIELDS.includes(f)) {
        body += head;
        splitByComma(fmtVal(val)).forEach((it, i) => { body += `${i + 1}. ${it}\n`; });
        body += '\n';
  ...
  const copyAllText = useCallback(async () => {
    let text = `Fitness For Duty Evaluations\n${COPY_LINE_EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Fitness for Duty Evaluation ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { currentArr.splice(itemIdx, 1, ...subParts); } else { currentArr[itemIdx] = trimmed; } /* Stage the WHOLE array as a DRAFT (no DB write); Approve commits the full array. */ stageDraft(record, fn, idx, sid, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderEnumField = (record, fn, idx, sid) => {
    ...
    const isEditing = editingField === editKey;
    const label = FIELD_LABELS[fn] || fn;
    const rawVal = fmtVal(val);
    const displayVal = enumCanonical(ENUM_OPTIONS[fn], rawVal);
    const isModified = editedFields[editKey];
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayVal); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              <BlueSelect value={editValue} options={enumOptionsWith(ENUM_OPTIONS[fn], rawVal)} onChange={v => setEditValue(v)} />
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
  ...
  /* ═══════ RENDER: COMMA FIELD (comma-list string → one editable row per item) ═══════ */
  const renderCommaField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx); if (!hasVal(val)) return null;
    const strVal = fmtVal(val);
    const items = splitByComma(strVal);
    if (items.length < 2) return renderStringField(record, fn, idx, sid, SECTION_TITLES[sid]);
    const label = FIELD_LABELS[fn] || fn;
    const sl = label.toLowerCase() !== (SECTION_TITLES[sid] || '').toLowerCase();
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
    return (
              ...
              <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(item); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const cur = splitByComma(String(getFieldValue(record, fn, idx) || '')); const t = editValue.trim(); if (t) cur[ci] = t; else cur.splice(ci, 1); handleSaveField(record, fn, idx, sid, null, cur.join(', '), itemKey); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
        ...
        <div key={fn}>
          <div className="rec-mini-card">
            ...
            {sentences.map((sentence, sIdx) => {
                            ...
                            <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                              {ciEditing ? (
                                <div className="edit-field-container">
                                  ...
                                  {saveError && <div className="save-error">{saveError}</div>}
                                  <div className="edit-actions">
                                    <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const currentVal2 = String(getFieldValue(record, fn, idx) || ''); const sentences2 = splitBySentence(currentVal2); const s2 = sentences2[sIdx] || ''; const p2 = parseLabel(s2); if (!p2.isLabeled) return; const items2 = splitByComma(p2.value); const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { items2.splice(ciIdx, 1, ...subParts); } else { items2[ciIdx] = trimmed.replace(/[;.]+$/, '').trim(); } const rebuilt = `${p2.label}: ${items2.join(', ')}.`; const allS = [...sentences2]; allS[sIdx] = rebuilt; const fullText2 = reconstructFullText(allS); setSaveError(null); /* Stage as a DRAFT (no DB write); Approve commits it. */ stageDraft(record, fn, idx, sid, fullText2); const marks = { [commaKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ciIdx + ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                                    <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                                  </div>
                                </div>
                              ) : (
              ...
              /* Regular sentence row — ALWAYS in a mini-card (unlabeled = no subtitle) for a consistent boxed look */
              return (
                <div key={sIdx} className="rec-mini-card" style={{ marginTop: 8 }}>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="fitness-for-duty-evaluations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Fitness for Duty Evaluation</h2></div>
        <div className="empty-state">No fitness for duty evaluation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="fitness-for-duty-evaluations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Fitness for Duty Evaluation</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FitnessForDutyEvaluationsDocumentPDFTemplate document={pdfData} />} fileName="Fitness_For_Duty_Evaluations.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search fitness for duty evaluation records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">Fitness for Duty Evaluation {idx + 1}</h3>
            </div>
            {renderSection(record, idx, 'employee-info')}
            {renderSection(record, idx, 'pulmonary')}
            {renderSection(record, idx, 'vision-hearing')}
            {renderSection(record, idx, 'cardiovascular-fitness')}
            {renderSection(record, idx, 'musculoskeletal')}
            {renderSection(record, idx, 'clearances')}
...
export default FitnessForDutyEvaluationsDocument;
