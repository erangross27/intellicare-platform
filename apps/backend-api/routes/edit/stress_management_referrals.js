/**
 * StressManagementReferrals Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/stress_management_referrals/:id/edit    — Edit a single field
 * PUT /api/edit/stress_management_referrals/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'stress-management-referrals-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('stress_management_referrals', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[StressManagementReferrals] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('stress_management_referrals', { _id: objectId }, {
      $set: {
    ...
    console.error('[StressManagementReferrals] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
