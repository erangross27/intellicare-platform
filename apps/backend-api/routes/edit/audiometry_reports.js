/**
 * Audiometry Reports Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/audiometry_reports/:id/edit    — Edit a single field
 * PUT /api/edit/audiometry_reports/:id/approve — Approve a section
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'audiometry-reports-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('audiometry_reports', { _id: objectId }, { $set: { [updatePath]: value, updatedAt: new Date(), updatedBy: req.user?.id } }, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) {
    console.error('[AudiometryReports] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('audiometry_reports', { _id: objectId }, { $set: { [`approvedSections.${sectionId}`]: approved, approvalTimestamp: new Date(), approvedBy: req.user?.id } }, context);
    return res.json({ success: true, recordId: id, sectionId, approved });
  } catch (err) {
    console.error('[AudiometryReports] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
