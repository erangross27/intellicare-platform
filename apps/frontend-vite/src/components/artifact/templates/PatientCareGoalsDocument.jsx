/**
 * PatientCareGoalsDocument.jsx
 * March 2026 — Canonical one-pass rewrite: inline editing, blue glow theme, canonical widgets.
 * Collection: patient_care_goals
 *
 * Record date: top-level `date` → editable record-header date (BlueDatePicker).
 * Sections:
 *   1. goal-info: goalDescription, goalCategory (enum), goalPriority (enum)
 *   2. status: lifecycleStatus (enum), achievementStatus (enum), statusReason
 *   3. dates: startDate, targetDate, achievedDate, reviewDate, modifiedDate (BlueDatePicker)
 *   4. measurements: targetMeasure, targetValue, targetUnit, baselineMeasure, currentMeasure
 *   5. addressed-conditions: addressedConditions (array)
 *   6. stakeholders: expressedBy (enum), acceptedBy
 *   7. interventions: interventions (array)
 *   8. barriers: barriers (array)
 *   9. outcome: outcomeNotes, outcomeReferences (array)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PatientCareGoalsDocumentPDFTemplate from '../pdf-templates/PatientCareGoalsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PatientCareGoalsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = localEdits field name; value may be a string or array) */
const DRAFT_KEY = 'patient_care_goalsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['goalDescription', 'statusReason', 'targetMeasure', 'targetValue', 'targetUnit', 'baselineMeasure', 'currentMeasure', 'acceptedBy', 'outcomeNotes'];
/* Only narrative fields get sentence-split; short phrases stay whole */
...
/* ═══════ COMPONENT ═══════ */
const PatientCareGoalsDocument = ({ document: docProp }) => {
  ...
  /* ═══════ UTILS ═══════ */
  const hasVal = useCallback((v) => { if (v === null || v === undefined || v === '') return false; if (typeof v === 'boolean') return true; if (typeof v === 'number') return true; if (typeof v === 'string') return v.trim() !== ''; if (Array.isArray(v)) return v.filter(x => x !== null && x !== undefined && String(x).trim() !== '').length > 0; if (typeof v === 'object') return Object.keys(v).length > 0; return true; }, []);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `patient care goal ${idx + 1}`.toLowerCase();
  ...
  /* Stage a DRAFT locally (NO DB write). localStorage keeps it across refresh; Approve commits it.
     localEditKey = `${fn}-${idx}`; the draft store keys by record _id then by `fn`. value may be a string or array. */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/patient_care_goals/${id}/edit`, { field: fn, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/patient_care_goals/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[PatientCareGoals] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PATIENT CARE GOALS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Patient Care Goal ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: RECORD-HEADER DATE (BlueDatePicker) ═══════ */
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* Multi-sentence OR a single "Label: value" sentence: decompose for long text fields */
    if (sentences.length > 1 || (sentences.length === 1 && parseLabel(sentences[0]).isLabeled)) {
  ...
  /* ═══════ RENDER: ARRAY FIELD (simple string array) ═══════ */
  const renderArrayField = (record, fn, idx, sid) => {
    const arr = getEffectiveArray(record, fn, idx).filter(x => x !== null && x !== undefined && String(x).trim() !== '');
  ...
  const getPriorityColor = (priority) => {
    ...
    if (p.includes('high')) return { bg: 'rgba(239, 68, 68, 0.2)', border: '#ef4444', text: '#ef4444' };
    if (p.includes('medium')) return { bg: 'rgba(251, 146, 60, 0.2)', border: '#fb923c', text: '#fb923c' };
    if (p.includes('low')) return { bg: 'rgba(34, 197, 94, 0.2)', border: '#22c55e', text: '#22c55e' };
  ...
  if (!records || records.length === 0) {
    return (
      <div className="patient-care-goals-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Patient Care Goals</h2></div>
        <div className="empty-state">No patient care goals data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="patient-care-goals-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Patient Care Goals</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PatientCareGoalsDocumentPDFTemplate document={pdfData} />} fileName="Patient_Care_Goals.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search patient care goals..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        {searchTerm && <span className="search-results">Showing {filteredRecords.length} of {records.length} records</span>}
      </div>
      <div className="records-container">
        {filteredRecords.map((record, idx) => {
          const priorityColor = getPriorityColor(getFieldValue(record, 'goalPriority', idx));
          return (
            <div key={idx} className="record-card">
              <div className="record-header">
                <div className="record-meta-row">
                  ...
                  {priorityColor && (
                    <span className="priority-badge" style={{ backgroundColor: priorityColor.bg, borderColor: priorityColor.border, color: priorityColor.text }}>
                      {highlightText(enumCanonical(ENUM_OPTIONS.goalPriority, fmtVal(getFieldValue(record, 'goalPriority', idx))))}
                    </span>
                  )}
                </div>
                <h3 className="record-name">{highlightText(fmtVal(getFieldValue(record, 'goalDescription', idx)) || `Patient Care Goal ${idx + 1}`)}</h3>
              </div>
...
export default PatientCareGoalsDocument;
