/**
 * AbnormalResultsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: abnormal_results (displayed as "Abnormal Results")
 *
 * 5 Sections:
 *   1. record-info: reportDate, reportType, urgency
 *   2. clinical-indication: clinicalIndication
 *   3. findings: findings
...
import AbnormalResultsDocumentPDFTemplate from '../pdf-templates/AbnormalResultsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './AbnormalResultsDocument.css';
...
const SECTION_FIELDS = {
  'record-info': ['reportDate', 'reportType', 'urgency'],
...
const ARRAY_FIELDS = [];
const OBJECT_FIELDS = [];
const STRING_FIELDS = ['reportType', 'clinicalIndication', 'findings', 'urgency', 'recommendations', 'followUp'];
...
const DRAFT_KEY = 'abnormalResultsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const AbnormalResultsDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Abnormal Results ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Stage a DRAFT for fieldPart=fn into the per-record localStorage store (survives refresh).
  // Returns nothing; sectionId markers handled by callers. NOT written to DB until Approve.
  ...
  // Drop the approved flag for this record's section so a re-edit returns to yellow "Pending Approve".
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // editKeys for this section+record that are staged drafts. Key format: "field-idx".
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/abnormal_results/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/abnormal_results/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[AbnormalResults] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== ABNORMAL RESULTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Abnormal Results ${idx + 1}\n${'='.repeat(40)}\n\n`;
      Object.keys(SECTION_FIELDS).forEach(sid => {
  ...
  if (!records || records.length === 0) {
    return (
      <div className="abnormal-results-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Abnormal Results</h2></div>
        <div className="empty-state">No abnormal results available</div>
      </div>
    );
  }
  ...
  return (
    <div className="abnormal-results-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Abnormal Results</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<AbnormalResultsDocumentPDFTemplate document={pdfData} />} fileName={`abnormal-results-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search abnormal results..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            ...
            <div className="record-header">
              {hasVal(record.reportDate) && (
                <div className="record-meta-row">
                  <span className="record-date">{highlightText(formatDate(record.reportDate))}</span>
                </div>
              )}
              <h3 className="record-name">{highlightText(`Abnormal Results ${idx + 1}`)}</h3>
            </div>
...
export default AbnormalResultsDocument;
