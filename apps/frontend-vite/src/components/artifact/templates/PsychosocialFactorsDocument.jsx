/**
 * PsychosocialFactorsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: psychosocial_factors
 *
 * 12 Sections:
 *   1. clinical-info: date, provider, facility
 *   2. stressors: stressors
 *   3. support: support
 *   4. coping-strategies: copingStrategies
 *   5. mental-health: mentalHealth
 *   6. findings: findings
 *   7. assessment: assessment
 *   8. plan: plan
 *   9. results-section: results (OBJECT, recursive editable)
 *  10. recommendations-section: recommendations (ARRAY of {recommendation, date})
 *  11. notes-status: notes, status
 *
 * Field types: date(Date), provider/facility/stressors/support/copingStrategies/
 *   mentalHealth/findings/assessment/plan/notes/status(string), recommendations(ARRAY),
 *   results(OBJECT).
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PsychosocialFactorsDocumentPDFTemplate from '../pdf-templates/PsychosocialFactorsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PsychosocialFactorsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = base field name; value may be
   a string, an array (recommendations), or an object (results)). */
const DRAFT_KEY = 'psychosocial_factorsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['provider', 'facility', 'stressors', 'support', 'copingStrategies', 'mentalHealth', 'findings', 'assessment', 'plan', 'notes', 'status'];
...
/* value helpers for OBJECT recursion */
const isEmptyDeep = (v) => {
...
/* ═══════ COMPONENT ═══════ */
const PsychosocialFactorsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.psychosocial_factors) return Array.isArray(dd.psychosocial_factors) ? dd.psychosocial_factors : [dd.psychosocial_factors]; return [dd]; }
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Psychosocial Factor ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* Stage a DRAFT for field `fn` of record `idx` into state + the localStorage draft store.
     NO DB write — the value is committed only on Approve (handleApproveSection). */
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    // Build the merged object (the WHOLE rootField value) and stage it as a DRAFT (no DB write).
    setLocalEdits(prev => {
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      // editKeys here are always `${field}-${idx}` (field = plain base name). The dot-suffix → arrayIndex
      // rule only fires when the segment after the LAST dot is purely numeric — which never happens here.
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/psychosocial_factors/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/psychosocial_factors/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[PsychosocialFactors] Approve error:', err); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      // Rule #47: don't print the field label when it duplicates the section title (single-field sections).
      ...
      if (OBJECT_ARRAY_FIELDS.includes(f)) {
        ...
        const useFmt = sentences.length > 1 || (!!sp && sp.isLabeled && splitByComma(sp.value).length >= 2);
        if (useFmt) {
          text += labelLine;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PSYCHOSOCIAL FACTORS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Psychosocial Factor ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    const showSubLabel = label.trim().toLowerCase() !== (SECTION_TITLES[sid] || '').trim().toLowerCase();
    // Route a single "Label: a, b, c" sentence through the multi path so its value comma-splits into rows.
    const parsedSingle = sentences.length === 1 ? parseLabel(sentences[0]) : null;
    const useMulti = sentences.length > 1 || (!!parsedSingle && parsedSingle.isLabeled && splitByComma(parsedSingle.value).length >= 2);
  ...
  const renderObjectLeaf = (record, rootField, path, idx, sid, value) => {
    ...
    const leafStep = stepFor(editStartValue);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="psychosocial-factors-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Psychosocial Factors</h2></div>
        <div className="empty-state">No psychosocial factors data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="psychosocial-factors-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Psychosocial Factors</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PsychosocialFactorsDocumentPDFTemplate document={pdfData} />} fileName="Psychosocial_Factors.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search psychosocial factors..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Psychosocial Factor ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'clinical-info')}
            {renderSection(record, idx, 'stressors')}
            {renderSection(record, idx, 'support')}
...
export default PsychosocialFactorsDocument;
