/**
 * Pharmacy Review Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/pharmacy_review/:id/edit    — Edit a single field
 * PUT /api/edit/pharmacy_review/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') { return { serviceId: 'pharmacy-review-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }

const ALLOWED_FIELDS = [
  'medicationName', 'activeIngredient', 'dosageStrength', 'dosageForm', 'routeOfAdministration',
  'prescribedQuantity', 'daysSupply', 'therapeuticClass', 'indicationForUse', 'contraindications',
  'drugInteractions', 'allergyStatus', 'adverseReactions', 'therapeuticDuplication', 'renalFunction',
  'hepaticFunction', 'pregnancyCategory', 'pharmacokineticFactors', 'adherenceAssessment', 'clinicalResponse',
  'monitoringParameters', 'costEffectivenessAnalysis', 'drugUtilizationReview',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('pharmacy_review', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[PharmacyReview] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('pharmacy_review', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[PharmacyReview] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
