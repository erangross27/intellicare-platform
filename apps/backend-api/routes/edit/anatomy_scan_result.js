/**
 * Anatomy Scan Result Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/anatomy_scan_result/:id/edit    — Edit a single field
 * PUT /api/edit/anatomy_scan_result/:id/approve — Approve a section
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'anatomy-scan-result-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('anatomy_scan_result', { _id: objectId }, {
      $set: {
        [field]: value,
        updatedAt: new Date(),
    ...
    console.error('[AnatomyScanResult] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    console.error('[AnatomyScanResult] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
