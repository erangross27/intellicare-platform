# Task 110: getCancerStagingReports Grid Implementation

## Function Details
- **Function Name**: getCancerStagingReports
- **Category**: Oncology
- **Collection**: cancer_staging_reports
- **Priority**: CRITICAL
...
- cancer_staging_reportsId (ObjectId) - Primary key
...
### Real-Time Updates
- WebSocket event: `cancer_staging_reports:update`
...
db.cancer_staging_reports.createIndex({ patientId: 1, date: -1 })
db.cancer_staging_reports.createIndex({ status: 1, practiceId: 1 })

db.cancer_staging_reports.createIndex({ lastModifiedAt: -1 })
```
...
### API Endpoints
- GET /api/medical/cancer_staging_reports
- POST /api/medical/cancer_staging_reports
- PUT /api/medical/cancer_staging_reports/:id
- DELETE /api/medical/cancer_staging_reports/:id
- GET /api/medical/cancer_staging_reports/export
...
```javascript
{
  ...
  metadata: {
    gridType: "cancer_staging_reports",
...
### Backend Tasks
- [ ] Create MongoDB schema for cancer_staging_reports
...
## Related Functions
- Previous: [109-getTumorMarkerPanels]
- Next: [111-getPalliativeCareNotes]
