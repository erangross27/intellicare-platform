/**
 * FMLA Documentation Note Edit Route
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'fmla-documentation-note-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'patientFunctionalCapacity',
  'functionalImpairmentScale',
  'symptomSeverityScore',
  'cognitiveImpairmentPresent',
  'workRestrictions',
  'liftingCapacityPounds',
  'standingToleranceHours',
  'modifiedDutyRecommendations',
  'stressLimitationRequired',
  'intermittentLeaveRequired',
  'reducedScheduleNeeded',
  'disabilityDurationEstimate',
  'returnToWorkDate',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('fmla_documentation_note', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[fmla_documentation_note] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('fmla_documentation_note', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[fmla_documentation_note] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
