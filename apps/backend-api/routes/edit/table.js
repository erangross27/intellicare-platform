...
/* Generic/fallback template — dynamic fields.  Block only system/internal keys. */
const BLOCKED_FIELDS = new Set(['_id', '__v', 'patientId', 'patient_id', 'documentId', 'practiceId', 'createdAt', 'updatedAt', 'doctorEdits']);
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    if (BLOCKED_FIELDS.has(rootField)) return res.status(400).json({ success: false, error: `Field "${field}" is not editable` });
    ...
    await sda.update('table', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[table] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('table', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[table] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
