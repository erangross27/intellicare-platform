/**
 * MechanismOfInjuryDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: mechanism_of_injury
 *
 * 5 Sections:
 *   1. session-info: date (date), type (string), provider (string), facility (string), status (string)
 *   2. injury-details: dateOfInjury (date), mechanism (string-long), activity (string)
 *   3. symptoms-treatment: immediateSymptoms (array), initialTreatment (string), timeToSurgery (string)
 *   4. clinical-findings: findings (string-long), assessment (string), plan (string), recommendations (object-array), results (object)
 *   5. additional-notes: notes (string)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MechanismOfInjuryDocumentPDFTemplate from '../pdf-templates/MechanismOfInjuryDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './MechanismOfInjuryDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits base key "field") */
const DRAFT_KEY = 'mechanism_of_injuryPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['type', 'provider', 'facility', 'status', 'mechanism', 'activity', 'initialTreatment', 'timeToSurgery', 'findings', 'assessment', 'plan', 'notes'];
...
/* ═══════ COMPONENT ═══════ */
const MechanismOfInjuryDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.mechanism_of_injury) return Array.isArray(r.mechanism_of_injury) ? r.mechanism_of_injury : [r.mechanism_of_injury];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.mechanism_of_injury) return Array.isArray(dd.mechanism_of_injury) ? dd.mechanism_of_injury : [dd.mechanism_of_injury]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Mechanism of Injury ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Stage a DRAFT for the whole field (localEdits key `${fn}-${idx}`) + persist to localStorage so it
  ...
  /* ═══════ OBJECT LEAF SAVE (dotted path) ═══════ */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    // Build the merged whole-object value (drafts store the entire field under `${rootField}-${idx}`).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // Pending draft keys for this record + this section's fields. Keys are `${field}-${idx}`.
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/mechanism_of_injury/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/mechanism_of_injury/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[MechanismOfInjury] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MECHANISM OF INJURY ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Mechanism of Injury ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; number+unit -> number input, ratio "4/5" stays text) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="mechanism-of-injury-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Mechanism of Injury</h2></div>
        <div className="empty-state">No mechanism of injury records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="mechanism-of-injury-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Mechanism of Injury</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MechanismOfInjuryDocumentPDFTemplate document={pdfData} />} fileName="Mechanism_Of_Injury.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search mechanism of injury..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Mechanism of Injury ${idx + 1}`)}</h3>
            </div>
...
export default MechanismOfInjuryDocument;
