/**
 * PriorAuthorizationFormsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: prior_authorization_forms
 *
 * 7 Sections:
 *   1. patient-identification: memberIdNumber, subscriberIdNumber
 *   2. requested-service: requestedServiceCptCode, requestedMedicationNdcNumber, medicationDosageAmount, medicationFrequency, treatmentDurationDays, requestedServiceLocation
 *   3. diagnosis-info: primaryDiagnosisIcd10Code, secondaryDiagnosisIcd10Codes, urgencyLevel
 *   4. clinical-history: priorTriedMedications, contraindicationsPresent, allergiesToMedications, comorbidConditions
 *   5. supporting-evidence: labResultsSupporting, imagingStudiesSupporting, functionalStatusScore, painScaleRating, symptomSeverityScore
 *   6. provider-info: physicianSpecialty, estimatedCostOfTreatment, clinicalTrialEligibility
 *   7. patient-status: pregnancyStatus, renalFunctionGfr
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PriorAuthorizationFormsDocumentPDFTemplate from '../pdf-templates/PriorAuthorizationFormsDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PriorAuthorizationFormsDocument.css';
...
const DRAFT_KEY = 'prior_authorization_formsPendingEdits';
const readDrafts = () => {
...
const MEANINGFUL_ZERO_FIELDS = []; // no numeric field where 0 is a real clinical value (0 days / $0 / GFR 0 / pain-not-assessed are extractor defaults)
...
/* ═══════ COMPONENT ═══════ */
const PriorAuthorizationFormsDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Prior Authorization Form ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const suffix = `-${idx}`;
      // Collect this record's staged edits for fields belonging to THIS section
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length);
        const baseField = fieldPart.includes('.') && /^\d+$/.test(fieldPart.slice(fieldPart.lastIndexOf('.') + 1))
          ? fieldPart.slice(0, fieldPart.lastIndexOf('.')) : fieldPart;
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/prior_authorization_forms/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/prior_authorization_forms/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PRIOR AUTHORIZATION FORMS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Prior Authorization Form ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(parsed.isLabeled ? parsed.value : itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = parsed.isLabeled ? `${parsed.label}: ${editValue}` : editValue; stageFieldDraft(id, fn, idx, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      return hasVal(val) && !isHiddenZero(f, val);
    });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="prior-authorization-forms-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Prior Authorization Forms</h2></div>
        <div className="empty-state">No prior authorization forms available</div>
      </div>
    );
  }
  ...
  return (
    <div className="prior-authorization-forms-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Prior Authorization Forms</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PriorAuthorizationFormsDocumentPDFTemplate document={pdfData} />} fileName="Prior_Authorization_Forms.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search prior authorization forms..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Prior Authorization Form ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'patient-identification')}
            {renderSection(record, idx, 'requested-service')}
            {renderSection(record, idx, 'diagnosis-info')}
            {renderSection(record, idx, 'clinical-history')}
            {renderSection(record, idx, 'supporting-evidence')}
...
export default PriorAuthorizationFormsDocument;
