...
const COLLECTION = 'carbohydrate_counting_education';
const COMMA_SPLIT_FIELDS = [];
const ARRAY_FIELDS = new Set(['mealTimingPatterns', 'learningObjectivesAchieved']);
const OBJECT_FIELDS = new Set(['additionalData']);
const NARRATIVE_FIELDS = new Set(['followUpRecommendations']);
const DATE_FIELDS = new Set([]);
const ZERO_SENTINEL_FIELDS = new Set(['insulinSensitivityFactor', 'timeInRange', 'glycemicVariability', 'severeHypoglycemiaHistory', 'diabeticKetoacidosisHistory']);
const KEY_OVERRIDES = { HbA1c: 'HbA1c' };
const LABELS = {
  patientDiabetesType: 'Diabetes Type', baselineHbA1c: 'Baseline HbA1c', targetGlucoseRange: 'Target Glucose Range',
  currentInsulinRegimen: 'Current Insulin Regimen', insulinToCarbohydrateRatio: 'Insulin-to-Carbohydrate Ratio',
  insulinSensitivityFactor: 'Insulin Sensitivity Factor', rapidActingInsulinType: 'Rapid-Acting Insulin', basalInsulinType: 'Basal Insulin',
  glucometerAccuracy: 'Glucometer Accuracy', continuousGlucoseMonitor: 'Continuous Glucose Monitor', timeInRange: 'Time in Range', glycemicVariability: 'Glycemic Variability',
  carbohydrateCountingCompetency: 'Carbohydrate Counting Competency', nutritionLabelReadingSkills: 'Nutrition Label Reading Skills',
  portionSizeEstimation: 'Portion Size Estimation', glycemicIndexKnowledge: 'Glycemic Index Knowledge',
  mealTimingPatterns: 'Meal Timing Patterns', hypoglycemiaAwareness: 'Hypoglycemia Awareness',
  severeHypoglycemiaHistory: 'Severe Hypoglycemia History', diabeticKetoacidosisHistory: 'Diabetic Ketoacidosis History', gastroparesisPresence: 'Gastroparesis Presence',
  educationSessionDuration: 'Education Session Duration', learningObjectivesAchieved: 'Learning Objectives Achieved',
  followUpRecommendations: 'Follow-Up Recommendations', additionalData: 'Additional Data',
};
const SECTIONS = [
  { title: 'Diabetes Profile', fields: ['patientDiabetesType', 'baselineHbA1c', 'targetGlucoseRange'] },
  { title: 'Insulin Regimen', fields: ['currentInsulinRegimen', 'insulinToCarbohydrateRatio', 'insulinSensitivityFactor', 'rapidActingInsulinType', 'basalInsulinType'] },
  { title: 'Glucose Monitoring', fields: ['glucometerAccuracy', 'continuousGlucoseMonitor', 'timeInRange', 'glycemicVariability'] },
  { title: 'Carbohydrate Counting Skills', fields: ['carbohydrateCountingCompetency', 'nutritionLabelReadingSkills', 'portionSizeEstimation', 'glycemicIndexKnowledge'] },
  { title: 'Meal Timing Patterns', fields: ['mealTimingPatterns'] },
  { title: 'Hypoglycemia and Complications', fields: ['hypoglycemiaAwareness', 'severeHypoglycemiaHistory', 'diabeticKetoacidosisHistory', 'gastroparesisPresence'] },
  { title: 'Education Session', fields: ['educationSessionDuration', 'learningObjectivesAchieved'] },
  { title: 'Follow-Up Recommendations', fields: ['followUpRecommendations'] },
...
const CarbohydrateCountingEducationDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Carbohydrate Counting Education</Text><Text style={styles.noData}>No carbohydrate counting education data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Carbohydrate Counting Education</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Carbohydrate Counting Education {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
