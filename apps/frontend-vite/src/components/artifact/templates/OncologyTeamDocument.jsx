/**
 * OncologyTeamDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: oncology_team
 *
 * 4 Sections:
 *   1. team-member-info: name, role, credentials, specialty
 *   2. contact-info: provider, facility
 *   3. clinical-notes: findings, assessment, plan, notes
 *   4. recommendations: recommendations (array)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import OncologyTeamPDFTemplate from '../pdf-templates/OncologyTeamPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './OncologyTeamDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'team-member-info': 'Team Member Information',
...
const SECTION_FIELDS = {
  'team-member-info': ['name', 'role', 'credentials', 'specialty'],
  'contact-info': ['provider', 'facility'],
...
const STRING_FIELDS = ['name', 'role', 'credentials', 'specialty', 'provider', 'facility', 'findings', 'assessment', 'plan', 'notes'];
...
const DRAFT_KEY = 'oncology_teamPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const OncologyTeamDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.oncology_team) return Array.isArray(r.oncology_team) ? r.oncology_team : [r.oncology_team];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.oncology_team) return Array.isArray(dd.oncology_team) ? dd.oncology_team : [dd.oncology_team]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Oncology Team Member ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Collect staged (pending) edits for this record that belong to this section's fields.
      ...
      // Persist each staged field to the DB now (field, or field+arrayIndex for numeric dot-suffixes).
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/oncology_team/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/oncology_team/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[OncologyTeam] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (ARRAY_FIELDS.includes(f)) {
        ...
        items.forEach((item, i) => { text += `${i + 1}. ${String(item).replace(/[;.]+$/, '').trim()}\n`; });
        text += '\n';
      } else {
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== ONCOLOGY TEAM ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `${r.name || `Oncology Team Member ${idx + 1}`}\n${'='.repeat(40)}\n\n`;
  ...
  const renderArrayField = (record, fn, idx, sid) => {
    ...
    // Suppress the field label when it duplicates the section title (single-name gate).
  ...
  if (!records || records.length === 0) {
    return (
      <div className="oncology-team-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Oncology Team</h2></div>
        <div className="empty-state">No oncology team records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="oncology-team-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Oncology Team</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OncologyTeamPDFTemplate document={pdfData} />} fileName="Oncology_Team.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search oncology team records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          <div key={idx} className="record-card">
            <div className="record-header">
              <div className="record-meta-row">
                ...
                {hasVal(record.status) && <span className={`status-badge ${String(record.status).toLowerCase()}`}>{record.status}</span>}
              </div>
              <h3 className="record-name">{highlightText(record.name || `Oncology Team Member ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'team-member-info')}
            {renderSection(record, idx, 'contact-info')}
...
export default OncologyTeamDocument;
