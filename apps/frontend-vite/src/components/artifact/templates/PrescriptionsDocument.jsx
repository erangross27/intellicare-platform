 ...
 *
 * 4 Sections:
 *   1. prescription-details: medication, dosage, frequency, quantity, refills
 *   2. provider-info: prescriber, pharmacy, date
 *   3. indication: indication
 *   4. instructions: instructions
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PrescriptionsPDFTemplate from '../pdf-templates/PrescriptionsPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './PrescriptionsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'prescriptionsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['medication', 'dosage', 'frequency', 'quantity', 'prescriber', 'pharmacy', 'indication', 'instructions'];
...
/* ═══════ COMPONENT ═══════ */
const PrescriptionsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.prescriptions) return Array.isArray(r.prescriptions) ? r.prescriptions : [r.prescriptions];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.prescriptions) return Array.isArray(dd.prescriptions) ? dd.prescriptions : [dd.prescriptions]; return [dd]; }
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Prescription ${idx + 1}`.toLowerCase();
  ...
  // NOT written to MongoDB / NOT in the PDF until Approve commits it.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Staged drafts for THIS section = pending localEdits whose base field belongs to this section.
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/prescriptions/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/prescriptions/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[Prescriptions] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  if (!records || records.length === 0) {
    return (
      <div className="prescriptions-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Prescriptions</h2></div>
        <div className="empty-state">No prescription records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="prescriptions-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Prescriptions</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PrescriptionsPDFTemplate document={pdfData} />} fileName="Prescriptions.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search prescriptions..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Prescription ${idx + 1}`)}</h3>
            </div>
