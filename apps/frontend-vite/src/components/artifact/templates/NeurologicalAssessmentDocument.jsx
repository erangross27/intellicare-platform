/**
 * NeurologicalAssessmentDocument.jsx
 * March 2026 — Canonical one-pass polish, inline editing, blue glow theme
 * Collection: neurological_assessment
 *
 * 7 Sections:
 *   1. assessment-info: assessmentDate (BlueDatePicker), assessmentTime
 *   2. clinical-status: clinicalStatus (long narrative → splitBySentence)
 *   3. vital-signs: vitalSigns (compact measurement list — kept whole)
 *   4. interventions: interventions (long narrative → splitBySentence)
 *   5. response: response
 *   6. plan: plan (long narrative → splitBySentence)
 *   7. recommendations: recommendations — array of {recommendation, date}; each object decomposes into a
 *                       nested-subtitle (recommendation text) + a formatted date row (BlueDatePicker)
 *
 * Record-date rule: record has ONLY createdAt/updatedAt (ingestion timestamps) and no real clinical
 * `date` field → NO record date pill (title only). assessmentDate is rendered as a FIELD.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NeurologicalAssessmentDocumentPDFTemplate from '../pdf-templates/NeurologicalAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './NeurologicalAssessmentDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'neurological_assessmentPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  'assessment-info': ['assessmentDate', 'assessmentTime'],
...
/* ═══════ COMPONENT ═══════ */
const NeurologicalAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.neurological_assessment) return Array.isArray(r.neurological_assessment) ? r.neurological_assessment : [r.neurological_assessment];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.neurological_assessment) return Array.isArray(dd.neurological_assessment) ? dd.neurological_assessment : [dd.neurological_assessment]; return [dd]; }
      return [r];
    });
  ...
  /* ═══════ SEARCH — 4-LEVEL ═══════ */
  const recArrayMatches = useCallback((val, phrase) => {
    if (!Array.isArray(val)) return false;
    return val.some(item => {
      if (item && typeof item === 'object') {
        const rt = (item.recommendation || '').toLowerCase();
        const rd = formatDate(item.date || '').toLowerCase();
        return rt.includes(phrase) || rd.includes(phrase);
      }
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (Array.isArray(val)) { if (recArrayMatches(val, phrase)) return true; }
        else if (fmtVal(val).toLowerCase().includes(phrase)) return true;
      }
    }
    return false;
  }, [searchTerm, getFieldValue, fmtVal, recArrayMatches]);
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) {
      if (Array.isArray(val)) return recArrayMatches(val, phrase);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Neurological Assessment ${idx + 1}`.toLowerCase();
  ...
  }, [records, searchTerm, getFieldValue, fmtVal, recArrayMatches]);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/neurological_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/neurological_assessment/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NEUROLOGICAL ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Neurological Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="neurological-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Neurological Assessment</h2></div>
        <div className="empty-state">No neurological assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="neurological-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Neurological Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NeurologicalAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Neurological_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search neurological assessments..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Neurological Assessment ${idx + 1}`)}</h3>
            </div>
...
export default NeurologicalAssessmentDocument;
