/**
 * EmergencyAirwayManagementDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: emergency_airway_management
 *
 * 7 Sections:
 *   1. indication-assessment: date, airwayIndicationType, preIntubationGlasgowComaScale, mallampatiScore, difficultAirwayPredictors
 *   2. pre-oxygenation: preOxygenationMethod, preOxygenationDuration, initialOxygenSaturation
 *   3. rapid-sequence: rapidSequenceInductionPerformed, inductionAgent, inductionAgentDose, paralyticsAdministered, paralyticDose
 *   4. intubation-details: intubationMethod, laryngoscopeBladeType, cormackLehaneGrade, numberOfIntubationAttempts, endotrachealTubeSize, tubeDepthAtTeeth, cuffPressure
 *   5. confirmation-methods: confirmationMethods, endTidalCO2Initial
 *   6. complications-rescue: postIntubationComplications, rescueAirwayDeviceUsed, surgicalAirwayPerformed
 *   7. operator-info: operatorExperienceLevel
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import EmergencyAirwayManagementDocumentPDFTemplate from '../pdf-templates/EmergencyAirwayManagementDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './EmergencyAirwayManagementDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'emergency_airway_managementPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'indication-assessment': 'Indication & Assessment',
  'pre-oxygenation': 'Pre-Oxygenation',
  'rapid-sequence': 'Rapid Sequence Induction',
  'intubation-details': 'Intubation Details',
  'confirmation-methods': 'Confirmation Methods',
  'complications-rescue': 'Complications & Rescue',
  'operator-info': 'Operator Information',
};

/* Units ride INSIDE the label (canonical) so the numeric value is displayed/edited raw. */
const FIELD_LABELS = {
  date: 'Date',
...
const SECTION_FIELDS = {
  'indication-assessment': ['date', 'airwayIndicationType', 'preIntubationGlasgowComaScale', 'mallampatiScore', 'difficultAirwayPredictors'],
  'pre-oxygenation': ['preOxygenationMethod', 'preOxygenationDuration', 'initialOxygenSaturation'],
  'rapid-sequence': ['rapidSequenceInductionPerformed', 'inductionAgent', 'inductionAgentDose', 'paralyticsAdministered', 'paralyticDose'],
  'intubation-details': ['intubationMethod', 'laryngoscopeBladeType', 'cormackLehaneGrade', 'numberOfIntubationAttempts', 'endotrachealTubeSize', 'tubeDepthAtTeeth', 'cuffPressure'],
  'confirmation-methods': ['confirmationMethods', 'endTidalCO2Initial'],
  'complications-rescue': ['postIntubationComplications', 'rescueAirwayDeviceUsed', 'surgicalAirwayPerformed'],
  'operator-info': ['operatorExperienceLevel'],
};
...
/* ENUMS (act like an emergency airway operator — Mallampati/Cormack-Lehane scales + device catalog).
   Stored lowercase ('direct laryngoscopy') → enumCanonical Title-Cases the display+save; an off-scale
   descriptive value is kept via enumOptionsWith so nothing is lost while still offering the picklist. */
const ENUM_OPTIONS = {
...
/* HIDE-ZERO: the batch extractor writes 0 into numerics it could not read (sentinel). Every airway
   numeric here (GCS min 3, SpO2, ETT size, tube depth, cuff pressure, ETCO2, duration, attempts) is
   never clinically 0 when actually recorded → a stored 0 is hidden. No meaningful-zero fields exist. */
...
/* ═══════ COMPONENT ═══════ */
const EmergencyAirwayManagementDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Emergency Airway Management ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/emergency_airway_management/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedLocalKeys.push(`${baseField}-${idx}`);
        delete recDrafts[fieldPart];
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/emergency_airway_management/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== EMERGENCY AIRWAY MANAGEMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Emergency Airway Management ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="emergency-airway-management-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Emergency Airway Management</h2></div>
        <div className="empty-state">No emergency airway management records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="emergency-airway-management-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Emergency Airway Management</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<EmergencyAirwayManagementDocumentPDFTemplate document={pdfData} />} fileName="Emergency_Airway_Management.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search emergency airway management..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Emergency Airway Management ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'indication-assessment')}
            {renderSection(record, idx, 'pre-oxygenation')}
            {renderSection(record, idx, 'rapid-sequence')}
            {renderSection(record, idx, 'intubation-details')}
            {renderSection(record, idx, 'confirmation-methods')}
            {renderSection(record, idx, 'complications-rescue')}
            {renderSection(record, idx, 'operator-info')}
          </div>
        ))}
      </div>
    </div>
...
export default EmergencyAirwayManagementDocument;
