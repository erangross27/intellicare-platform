/**
 * SurgicalOncologyDocument.jsx
 * March 2026 -- Complete rewrite with inline editing, blue glow theme
 * Collection: surgical_oncology
 *
 * 10 Sections:
 *   1. procedure-info: procedureType, dateOfSurgery, surgeon, provider, facility
 *   2. pathology-findings: pathologyFindings.tumorSize, pathologyFindings.margins, pathologyFindings.lymphNodesExamined, pathologyFindings.lymphNodesPositive, pathologyFindings.extrandalExtension, pathologyFindings.lymphovascularInvasion, pathologyFindings.perineuralInvasion
 *   3. reconstruction: reconstruction (OBJECT, recursive editor)
 *   4. findings: findings
 *   5. results: results (OBJECT, recursive editor)
...
import SurgicalOncologyDocumentPDFTemplate from '../pdf-templates/SurgicalOncologyDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SurgicalOncologyDocument.css';
...
const DRAFT_KEY = 'surgicalOncologyPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  'procedure-info': ['procedureType', 'dateOfSurgery', 'surgeon', 'provider', 'facility'],
  'pathology-findings': ['pathologyFindings.tumorSize', 'pathologyFindings.margins', 'pathologyFindings.lymphNodesExamined', 'pathologyFindings.lymphNodesPositive', 'pathologyFindings.extrandalExtension', 'pathologyFindings.lymphovascularInvasion', 'pathologyFindings.perineuralInvasion'],
  'reconstruction': ['reconstruction'],
  'findings': ['findings'],
  'results': ['results'],
  'assessment': ['assessment'],
...
const BOOLEAN_FIELDS = ['pathologyFindings.extrandalExtension', 'pathologyFindings.lymphovascularInvasion', 'pathologyFindings.perineuralInvasion'];
const DATE_FIELDS = ['dateOfSurgery'];
const ARRAY_FIELDS = ['complications'];
const OBJECT_FIELDS = ['reconstruction', 'results'];
const OBJECT_ARRAY_FIELDS = ['recommendations'];
const NUMBER_FIELDS = ['pathologyFindings.lymphNodesExamined', 'pathologyFindings.lymphNodesPositive'];
const STRING_FIELDS = ['procedureType', 'surgeon', 'provider', 'facility', 'pathologyFindings.tumorSize', 'pathologyFindings.margins', 'findings', 'assessment', 'plan', 'notes'];

/* ======= OBJECT-FIELD HELPERS (donor: PointOfCareUltrasoundHeartRateDocument) ======= */
...
/* ======= COMPONENT ======= */
const SurgicalOncologyDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.surgical_oncology) return Array.isArray(r.surgical_oncology) ? r.surgical_oncology : [r.surgical_oncology];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.surgical_oncology) return Array.isArray(dd.surgical_oncology) ? dd.surgical_oncology : [dd.surgical_oncology]; return [dd]; }
      return [r];
    });
  ...
  /* searchable text per field type (object -> flattened, recommendations -> rec text + date) */
  const fieldSearchText = useCallback((fn, val) => {
    if (val === null || val === undefined) return '';
    if (OBJECT_ARRAY_FIELDS.includes(fn)) return (Array.isArray(val) ? val : []).map(r => `${r?.recommendation || ''} ${r?.date || ''}`).join(' ');
  ...
  /* Deep get: supports dot-path like pathologyFindings.tumorSize */
  const deepGet = useCallback((obj, path) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Surgical Oncology ${idx + 1}`.toLowerCase();
  ...
  /* ======= EDIT HANDLERS =======
     Save = stage a DRAFT locally (localEdits) + persist it to the pending-drafts localStorage store
     (survives refresh). NOT written to MongoDB and NOT shown in the PDF until the user clicks Pending
     Approve (handleApproveSection commits). localEdits key = `${fieldPart}-${idx}` where fieldPart is the
     field name (may be dotted, e.g. pathologyFindings.tumorSize); object/array fields stage the WHOLE
     value under fieldPart. The draft store mirrors that: store[recordId][fieldPart] = value. */
  const stageDraft = useCallback((record, fieldPart, idx, value, sid, trackKey, sentenceKey) => {
    const editKey = `${fieldPart}-${idx}`;
    setLocalEdits(prev => ({ ...prev, [editKey]: value }));
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
    if (sentenceKey) setEditedSentences(prev => ({ ...prev, [sentenceKey]: 'edited' }));
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    setSaveError(null);
    stageDraft(record, fn, idx, saveVal, sid, editTrackingKey || `${fn}-${idx}`);
  }, [editValue, safeId, stageDraft]);

  /* save a nested OBJECT leaf by dot-path (e.g. reconstruction.type, results.x.y) */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    // Build the whole updated object (staged under `${rootField}-${idx}`), then defer it as a draft.
    const cur = localEdits[`${rootField}-${idx}`] !== undefined ? localEdits[`${rootField}-${idx}`] : record[rootField];
    const clone = JSON.parse(JSON.stringify(cur ?? {}));
    let node = clone;
    for (let i = 0; i < path.length - 1; i++) node = node[path[i]];
    node[path[path.length - 1]] = newVal;
    stageDraft(record, rootField, idx, clone, sid, leafKeyTrack);
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageDraft(record, fn, idx, fullText, sid, null, `${fn}-${idx}-s${sentenceIdx}`);
      return;
    }
    ...
    // Stage the merged full text as a DRAFT + mark per-sentence badges (edited / added). No DB write.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // localEdits keys are `${fieldPart}-${idx}`; a key belongs to this section when its fieldPart ∈ fields.
    ...
    try {
      for (const editKey of toCommit) {
        ...
        // arrayIndex ONLY when the segment after the LAST dot is purely numeric (none here, but mirror the rule).
        ...
        await secureApiClient.put(`/api/edit/surgical_oncology/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/surgical_oncology/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[SurgicalOncology] Approve error:', err); }
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        const walk = (obj, depth) => {
          Object.entries(obj).forEach(([k, v]) => {
            ...
            else { lines.push(`${pad}${humanizeKey(k)}:`); walk(v, depth + 1); }
          });
        };
        walk(val, 0);
        text += `${label}\n${lines.join('\n')}\n\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== SURGICAL ONCOLOGY ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Surgical Oncology ${idx + 1}\n${'='.repeat(40)}\n\n`;
      if (hasVal(r.date)) text += `Date: ${formatDate(r.date)}\n`;
      if (hasVal(r.status)) text += `Status: ${fmtVal(r.status)}\n`;
      text += '\n';
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; stageDraft(record, fn, idx, currentArr, sid, editKey); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                            ...
                            <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                              {ciEditing ? (
                                <div className="edit-field-container">
                                  ...
                                  {saveError && <div className="save-error">{saveError}</div>}
                                  <div className="edit-actions">
                                    <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const currentVal2 = String(getFieldValue(record, fn, idx) || ''); const sentences2 = splitBySentence(currentVal2); const s2 = sentences2[sIdx] || ''; const p2 = parseLabel(s2); if (!p2.isLabeled) return; const items2 = splitByComma(p2.value); const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { items2.splice(ciIdx, 1, ...subParts); } else { items2[ciIdx] = trimmed.replace(/[;.]+$/, '').trim(); } const rebuilt = `${p2.label}: ${items2.join(', ')}.`; const allS = [...sentences2]; allS[sIdx] = rebuilt; const fullText2 = reconstructFullText(allS); setSaveError(null); const extraMarks = {}; for (let ei = 1; ei < subParts.length; ei++) extraMarks[`${fn}-${idx}-s${sIdx}-c${ciIdx + ei}`] = 'added'; if (Object.keys(extraMarks).length) setEditedSentences(prev => ({ ...prev, ...extraMarks })); stageDraft(record, fn, idx, fullText2, sid, null, commaKey); }}>{saving ? 'Saving...' : 'Save'}</button>
                                    <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                                  </div>
                                </div>
                              ) : (
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); setSaveError(null); const extraMarks = {}; for (let ei = 1; ei < subParts.length; ei++) extraMarks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; if (Object.keys(extraMarks).length) setEditedSentences(prev => ({ ...prev, ...extraMarks })); stageDraft(record, fn, idx, fullText, sid, null, sentenceKey); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  /* ======= RENDER: OBJECT LEAF (editable; booleans -> Yes/No select, number+unit -> number input) ======= */
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              setSaveError(null);
                              stageDraft(record, fn, idx, newArr, sid, null, itemKey);
                            }}>{saving ? 'Saving...' : 'Save'}</button>
                            <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                          </div>
                        </div>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="surgical-oncology-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Surgical Oncology</h2></div>
        <div className="empty-state">No surgical oncology records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="surgical-oncology-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Surgical Oncology</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<SurgicalOncologyDocumentPDFTemplate document={pdfData} />} fileName={`surgical-oncology-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search surgical oncology..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          <div key={idx} className="record-card">
            <div className="record-header">
              <div className="record-meta-row">
                ...
                {hasVal(record.status) && <span className="record-status">{highlightText(fmtVal(record.status))}</span>}
              </div>
              <h3 className="record-name">{highlightText(`Surgical Oncology ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'procedure-info')}
            {renderSection(record, idx, 'pathology-findings')}
            {renderSection(record, idx, 'reconstruction')}
...
export default SurgicalOncologyDocument;
