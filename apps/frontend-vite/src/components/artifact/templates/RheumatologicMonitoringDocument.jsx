 ...
 * July 2026 — recursive, dot-path editable rheumatologic monitoring
...
const SECTION_FIELDS = {
  ...
  diseaseActivityMonitoring: ['diseaseActivityMonitoring'],
  medicationMonitoring: ['medicationMonitoring'],
  immunizationStatus: ['immunizationStatus'],
  screeningProtocols: ['screeningProtocols'],
  clinicalFindings: ['findings'],
  clinicalAssessment: ['assessment'],
  treatmentPlan: ['plan'],
  results: ['results'],
...
const RheumatologicMonitoringDocument = ({ document: docProp }) => {
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/rheumatologic_monitoring/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/rheumatologic_monitoring/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[RheumatologicMonitoring] Approve error:', error);
  ...
  const buildSectionCopyText = useCallback((record, recordIndex, sectionId) => {
    ...
    (SECTION_FIELDS[sectionId] || []).forEach((field) => {
      ...
      } else if (fieldIn(COMMA_ARRAY_FIELDS, field)) {
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Rheumatologic Monitoring ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
    ...
    return <div className="section" key={sectionId}><div className="mini-cards-container">
      ...
      {sectionId === 'results' ? renderResults(record, recordIndex, sectionId)
          : sectionId === 'recommendations' ? renderRecommendations(record, recordIndex, sectionId)
            : fields.map((field) => fieldIn(COMMA_ARRAY_FIELDS, field)
  ...
  if (!records.length) return <div className="rheumatologic-monitoring-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Rheumatologic Monitoring</h2></div><div className="empty-state">No rheumatologic monitoring records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Rheumatologic Monitoring ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="rheumatologic-monitoring-document" ref={containerRef}>
    <div className="document-header">
      ...
      <div className="header-actions">
        ...
        <PDFDownloadLink document={<RheumatologicMonitoringDocumentPDFTemplate document={pdfData} />} fileName="Rheumatologic_Monitoring.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search rheumatologic monitorings..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Rheumatologic Monitoring ' + (recordIndex + 1))}</h3></div>
