...
/**
 * LifestyleCounselingDocumentPDFTemplate - box-free canonical (LETTER)
 ...
 * Record date is the record.date field the JSX edits - NEVER createdAt/updatedAt.
 */
...
const SECTION_ORDER = ['session-info', 'lifestyle-factors', 'clinical-notes', 'recommendations', 'additional-notes'];
...
const LifestyleCounselingDocumentPDFTemplate = ({ document: docProp, data = docProp }) => {
  let records = [];
  if (Array.isArray(data)) {
    if (data.length === 1 && data[0]?.lifestyle_counseling) records = Array.isArray(data[0].lifestyle_counseling) ? data[0].lifestyle_counseling : [data[0].lifestyle_counseling];
    else records = data;
  } else if (data?.lifestyle_counseling) records = Array.isArray(data.lifestyle_counseling) ? data.lifestyle_counseling : [data.lifestyle_counseling];
    ...
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <Text style={styles.documentTitle}>Lifestyle Counseling</Text>
          <Text style={styles.noData}>No lifestyle counseling records available.</Text>
        </Page>
      </Document>
    );
  }
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, rIdx) => (
          <View key={rIdx}>
            <Text style={styles.recordTitle} break={rIdx > 0}>{safeString(`Lifestyle Counseling ${rIdx + 1}`)}</Text>
...
export default LifestyleCounselingDocumentPDFTemplate;
