/**
 * EarlyMaternityLeaveDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: early_maternity_leave
 *
 * 4 Sections:
 *   1. leave-status: considering, reason, timing, status, date, provider, facility
 *   2. employment-details: findings
 *   3. medical-clearance: assessment
 *   4. return-plan: plan, notes
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import EarlyMaternityLeaveDocumentPDFTemplate from '../pdf-templates/EarlyMaternityLeaveDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './EarlyMaternityLeaveDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'early_maternity_leavePendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  provider: 'Provider',
  facility: 'Facility',
  findings: 'Employment & FMLA Details',
  assessment: 'Medical Clearance Assessment',
  plan: 'Return Plan',
  notes: 'Notes',
};

const SECTION_FIELDS = {
  'leave-status': ['considering', 'reason', 'timing', 'status', 'date', 'provider', 'facility'],
  'employment-details': ['findings'],
  'medical-clearance': ['assessment'],
  'return-plan': ['plan', 'notes'],
};

const NUMBER_FIELDS = [];
const BOOLEAN_FIELDS = ['considering'];
const ARRAY_FIELDS = ['recommendations'];
const DATE_FIELDS = ['date'];
const STRING_FIELDS = ['reason', 'timing', 'provider', 'facility', 'findings', 'assessment', 'plan', 'notes', 'type'];

/* Fixed-choice fields → BlueSelect (RTL-safe). status is a known record-state enum (stored lowercase 'active'). */
const ENUM_OPTIONS = {
...
/* ═══════ COMPONENT ═══════ */
const EarlyMaternityLeaveDocument = ({ document: docProp }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // fieldPart = "field" or "field.arrayIndex"; localEdits is keyed "field-idx" (full value)
        const dotIdx = fieldPart.lastIndexOf('.');
        const isArrayElt = dotIdx !== -1 && /^\d+$/.test(fieldPart.slice(dotIdx + 1));
        const fieldName = isArrayElt ? fieldPart.slice(0, dotIdx) : fieldPart;
        const localKey = `${fieldName}-${idx}`;
        nLocal[localKey] = value;
        nPending[localKey] = true;
        const trackKey = isArrayElt ? `${fieldName}.${fieldPart.slice(dotIdx + 1)}-${idx}` : localKey;
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Early Maternity Leave ${idx + 1}`.toLowerCase();
  ...
  /* Find which section a field belongs to (to clear its approved flag on re-edit). */
  ...
  /* Stage a DRAFT locally + write it to the pending-drafts localStorage store (survives refresh).
     ...
     localEdits is keyed `${fn}-${idx}` and stores the FULL field value (string, or the full array for
     array fields). The localStorage fieldPart mirrors that: `fn` (or `fn.arrayIndex` for an array element). */
  const stageDraft = useCallback((record, fn, idx, fullValue, trackingMarks, arrayIndex) => {
    const id = safeId(record); if (!id) return;
    const localKey = `${fn}-${idx}`;
    setLocalEdits(prev => ({ ...prev, [localKey]: fullValue }));
    setPendingEdits(prev => ({ ...prev, [localKey]: true }));
    if (trackingMarks && trackingMarks.fields) setEditedFields(prev => ({ ...prev, ...trackingMarks.fields }));
    if (trackingMarks && trackingMarks.sentences) setEditedSentences(prev => ({ ...prev, ...trackingMarks.sentences }));
    ...
    if (sid) {
      const approveKey = `${sid}-${idx}`;
      setApprovedSections(prev => { if (!prev[approveKey]) return prev; const n = { ...prev }; delete n[approveKey]; return n; });
    }
    const fieldPart = typeof arrayIndex === 'number' ? `${fn}.${arrayIndex}` : fn;
    const store = readDrafts();
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    stageDraft(record, fn, idx, saveVal, { fields: { [trackKey]: 'edited' } });
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      setEditedSentences(prev => { const m = { ...prev }; Object.keys(m).forEach(k => { if (k.startsWith(`${fn}-${idx}-s`)) delete m[k]; }); return m; });
      stageDraft(record, fn, idx, fullText, { sentences: { [`${fn}-${idx}-s${sentenceIdx}`]: 'edited' } });
      return;
    }
    ...
    stageDraft(record, fn, idx, fullText, { sentences: marks });
  }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Collect this record's staged drafts (localStorage fieldPart → value) for fields in this section.
      const store = readDrafts();
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const payload = { field: fieldName, value: localEdits[localKey] };
        if (isArrayElt) payload.arrayIndex = parseInt(fieldPart.slice(dotIdx + 1), 10);
        const resp = await secureApiClient.put(`/api/edit/early_maternity_leave/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedLocalKeys.push(localKey);
      }
      await secureApiClient.put(`/api/edit/early_maternity_leave/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (store2[id]) {
        for (const fieldPart of Object.keys(store2[id])) {
          ...
          const isArrayElt = dotIdx !== -1 && /^\d+$/.test(fieldPart.slice(dotIdx + 1));
          const fieldName = isArrayElt ? fieldPart.slice(0, dotIdx) : fieldPart;
          if (fields.includes(fieldName)) delete store2[id][fieldPart];
        }
    ...
    } catch (err) { console.error('[EarlyMaternityLeave] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const formatSentenceFieldLines = useCallback((text) => {
    ...
    sentences.forEach(s => {
      ...
      if (parsed.isLabeled) {
        ...
        // unlabeled sentence → one numbered row (mirror the JSX renderStringField, which never comma-splits unlabeled)
        lines.push(`${n++}. ${s}`);
      }
      first = false;
    });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== EARLY MATERNITY LEAVE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Early Maternity Leave ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { currentArr.splice(itemIdx, 1, ...subParts); } else { currentArr[itemIdx] = trimmed; } stageDraft(record, fn, idx, currentArr, { fields: { [editKey]: 'edited' } }, itemIdx); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  /* ═══════ RENDER: ENUM FIELD (fixed-choice BlueSelect + canonical casing) ═══════ */
  const renderEnumField = (record, fn, idx, sid) => {
    ...
    const displayVal = enumCanonical(fn, fmtVal(val)); // 'active' → 'Active' so the trigger + option match (no duplicate)
    const isModified = editedFields[editKey];
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="early-maternity-leave-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Early Maternity Leave</h2></div>
        <div className="empty-state">No early maternity leave records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="early-maternity-leave-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Early Maternity Leave</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<EarlyMaternityLeaveDocumentPDFTemplate document={pdfData} />} fileName="Early_Maternity_Leave.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search early maternity leave records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Early Maternity Leave ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'leave-status')}
            {renderSection(record, idx, 'employment-details')}
            {renderSection(record, idx, 'medical-clearance')}
            {renderSection(record, idx, 'return-plan')}
          </div>
        ))}
      </div>
    </div>
...
export default EarlyMaternityLeaveDocument;
