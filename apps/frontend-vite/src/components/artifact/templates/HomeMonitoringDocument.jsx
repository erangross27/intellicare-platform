/**
 * HomeMonitoringDocument.jsx
 * Canonical one-pass polish - blue glow editing theme
 * Collection: home_monitoring
 *
 * Sections: Session Information - Blood Pressure Monitoring - Monitoring Data - Symptoms & Assessment
 *
 * Editable scalar strings (provider/findings/assessment/plan/notes) are inline-editable and the
 * date field edits via BlueDatePicker. Read-only display scalars (type/facility/status) render as
 * read rows. Nested objects (bloodPressure/glucose/weight/oxygenSaturation/peakFlow/results) and
 * arrays (symptoms/recommendations) are decomposed for display (label ABOVE value - never a
 * side-by-side "Label: value" span) and rendered read-only. Empty {}/[] are gated out entirely.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HomeMonitoringDocumentPDFTemplate from '../pdf-templates/HomeMonitoringDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './HomeMonitoringDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" -> fn) */
const DRAFT_KEY = 'home_monitoringPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  'session-info': ['date', 'type', 'provider', 'facility'],
  'blood-pressure': ['bloodPressure'],
  'monitoring-data': ['glucose', 'weight', 'oxygenSaturation', 'peakFlow'],
  'symptoms-assessment': ['symptoms', 'findings', 'assessment', 'plan', 'recommendations', 'results', 'notes', 'status'],
};

/* Editable scalar strings (backend-ALLOWED) - inline text editing */
...
const ARRAY_FIELDS = ['symptoms', 'recommendations'];
const OBJECT_FIELDS = ['bloodPressure', 'glucose', 'weight', 'oxygenSaturation', 'peakFlow', 'results'];
const NUMBER_FIELDS = [];
...
/* ======= COMPONENT ======= */
const HomeMonitoringDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.home_monitoring) return Array.isArray(r.home_monitoring) ? r.home_monitoring : [r.home_monitoring];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.home_monitoring) return Array.isArray(dd.home_monitoring) ? dd.home_monitoring : [dd.home_monitoring]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Home Monitoring ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/home_monitoring/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/home_monitoring/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HOME MONITORING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Home Monitoring ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ======= RENDER: OBJECT FIELD (bloodPressure/glucose/weight/oxygenSaturation/peakFlow/results) ======= */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, i) => {
          ...
          if (!isScalar(item)) return renderObjectNode(`Item ${i + 1}`, item, itemKey);
  ...
  /* ======= RENDER: NUMBER FIELD (num-stepper - canonical; NUMBER_FIELDS empty) ======= */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="home-monitoring-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Home Monitoring</h2></div>
        <div className="empty-state">No home monitoring records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="home-monitoring-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Home Monitoring</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HomeMonitoringDocumentPDFTemplate document={pdfData} />} fileName="Home_Monitoring.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search home monitoring..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Home Monitoring ${idx + 1}`)}</h3>
            </div>
...
export default HomeMonitoringDocument;
