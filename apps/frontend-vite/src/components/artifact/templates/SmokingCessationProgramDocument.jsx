/**
 * SmokingCessationProgramDocument.jsx
 * July 2026 — recursive, dot-path editable smoking cessation program
 * Collection: smoking_cessation_program
 */
import React, { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import SmokingCessationProgramDocumentPDFTemplate from '../pdf-templates/SmokingCessationProgramDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SmokingCessationProgramDocument.css';

const DRAFT_KEY = 'smoking_cessation_programPendingEdits';
...
const FIELD_LABELS = {
  date: 'Date', type: 'Program Type', provider: 'Provider', facility: 'Facility', findings: 'Findings', assessment: 'Assessment', plan: 'Plan', recommendations: 'Recommendations', results: 'Results', status: 'Status', notes: 'Notes',
};
const SECTION_FIELDS = {
  programDetails: ['date', 'type', 'provider', 'facility'],
...
const COMMA_FIELDS = ['findings', 'results'];
const COMMA_ARRAY_SPLIT_FIELDS = ['recommendations'];
const ARRAY_FIELDS = ['recommendations'];
const SEMICOLON_FIELDS = ['assessment'];
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'status' ? ['Active', 'Completed', 'Inactive'] : [];
...
const SmokingCessationProgramDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.smoking_cessation_program) return Array.isArray(record.smoking_cessation_program) ? record.smoking_cessation_program : [record.smoking_cessation_program];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.smoking_cessation_program) return Array.isArray(nested.smoking_cessation_program) ? nested.smoking_cessation_program : [nested.smoking_cessation_program];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/smoking_cessation_program/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/smoking_cessation_program/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SmokingCessationProgram] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    let text = '=== SMOKING CESSATION PROGRAM ===\n\n';
    records.forEach((record, recordIndex) => {
      text += 'Smoking Cessation Program Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  const renderSection = (record, recordIndex, sectionId) => {
    ...
    return <div className="section" key={sectionId}><div className="mini-cards-container">
      ...
      {sectionId === 'results' ? renderResults(record, recordIndex, sectionId)
          : fields.map((field) => MIXED_OBJECT_ARRAY_FIELDS.includes(field) && Array.isArray(getValue(record, field, recordIndex)) && getValue(record, field, recordIndex).some((item) => !isScalar(item))
  ...
  if (!records.length) return <div className="smoking-cessation-program-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Smoking Cessation Program</h2></div><div className="empty-state">No smoking cessation program available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Smoking Cessation Program Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="smoking-cessation-program-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Smoking Cessation Program</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<SmokingCessationProgramDocumentPDFTemplate document={pdfData} />} fileName="Smoking_Cessation_Program.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search smoking cessation program..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Smoking Cessation Program Record ' + (recordIndex + 1))}</h3></div>
...
export default SmokingCessationProgramDocument;
