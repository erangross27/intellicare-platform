/**
 * ConsultationDetails Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/consultation_details/:id/edit    — Edit a single field
 * PUT /api/edit/consultation_details/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'consultation-details-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('consultation_details', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[ConsultationDetails] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('consultation_details', { _id: objectId }, {
      $set: {
    ...
    console.error('[ConsultationDetails] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
