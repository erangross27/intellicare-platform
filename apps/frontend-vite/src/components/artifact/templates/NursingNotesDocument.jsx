/**
 * NursingNotesDocument.jsx
 * March 2026 — Canonical one-pass rewrite: inline editing, blue glow theme.
 * Collection: nursing_notes (displayed as "Nursing Notes")
 *
 * Sections (record has NO `date`; assessmentDate is a real FIELD, not a record-header pill):
 *   1. assessment-info : assessmentDate (date), assessmentTime (string)
 *   2. clinical-status : clinicalStatus (string)
 *   3. vital-signs     : vitalSigns (string comma-list)
 *   4. interventions   : interventions (string comma-list)
 *   5. response        : response (string comma-list)
 *   6. plan            : plan (string comma-list)
 * Empty in this record → skipped: recommendations ([]), additionalData ({}).
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NursingNotesDocumentPDFTemplate from '../pdf-templates/NursingNotesDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './NursingNotesDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'nursing_notesPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const NursingNotesDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.nursing_notes) return Array.isArray(r.nursing_notes) ? r.nursing_notes : [r.nursing_notes];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.nursing_notes) return Array.isArray(dd.nursing_notes) ? dd.nursing_notes : [dd.nursing_notes]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Nursing Note ${idx + 1}`.toLowerCase();
  ...
  // Stage an edited comma item of an UNLABELED single-sentence comma-list field as a DRAFT (no DB write).
  function saveUnlabeledCommaItem(record, fn, idx, sid, ciIdx) {
    const id = safeId(record); if (!id) return;
    const currentVal = String(getFieldValue(record, fn, idx) || '');
    ...
    if (!trimmed || /^[;.,!?]+$/.test(trimmed)) {
      ...
      marks[`${fn}-${idx}-c${ciIdx}`] = 'edited';
      for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-c${ciIdx + ei}`] = 'added';
    }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/nursing_notes/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/nursing_notes/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const formatSentenceFieldLines = useCallback((text) => {
    ...
    sentences.forEach(s => {
      ...
      if (parsed.isLabeled) {
        ...
        const parts = single ? splitByComma(s) : [s];
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        const decompose = sentences.length > 1 || (sentences.length === 1 && (parseLabel(sentences[0]).isLabeled || splitByComma(sentences[0]).length >= 2));
        if (decompose) {
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NURSING NOTES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Nursing Note ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: STRING FIELD with splitBySentence + comma decomposition ═══════ */
  const renderStringField = (record, fn, idx, sid) => {
    ...
    const firstParsed = parseLabel(sentences[0] || '');
    const singleUnlabeledComma = sentences.length === 1 && !firstParsed.isLabeled && splitByComma(sentences[0] || '').length >= 2;

    /* Multi-sentence OR a single labeled "Label: v1, v2…" sentence OR a single unlabeled comma list: decompose */
    if (sentences.length > 1 || (sentences.length === 1 && firstParsed.isLabeled) || singleUnlabeledComma) {
        ...
        <div key={fn}>
          <div className="rec-mini-card">
            ...
            {sentences.map((sentence, sIdx) => {
              ...
              /* Unlabeled single-sentence comma list → comma rows (field label already shown above) */
              if (!parsed.isLabeled && sentences.length === 1) {
                            ...
                            <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                              {ciEditing ? (
                                <div className="edit-field-container">
                                  ...
                                  {saveError && <div className="save-error">{saveError}</div>}
                                  <div className="edit-actions">
                                    <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveUnlabeledCommaItem(record, fn, idx, sid, ciIdx); }}>{saving ? 'Saving...' : 'Save'}</button>
                                    <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                                  </div>
                                </div>
                              ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="nursing-notes-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Nursing Notes</h2></div>
        <div className="empty-state">No nursing notes available</div>
      </div>
    );
  }
  ...
  return (
    <div className="nursing-notes-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Nursing Notes</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NursingNotesDocumentPDFTemplate document={pdfData} />} fileName="Nursing_Notes.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search nursing notes..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Nursing Note ${idx + 1}`)}</h3>
            </div>
...
export default NursingNotesDocument;
