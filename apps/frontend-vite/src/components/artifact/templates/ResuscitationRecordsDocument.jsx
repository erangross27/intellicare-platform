 ...
 *
 * 10 Sections:
 *   1. arrest-info: arrestDateTime, arrestLocation, arrestWitnessed, initialRhythm
 *   2. cpr-defib: bystanderCprProvided, cprStartTime, timeToFirstCompression, timeToFirstDefibrillation, totalShocksDelivered, shockEnergiesDelivered
 *   3. medications: epinephrineDoses, amiodaroneDoses
 *   4. airway-mgmt: airwayManagement, intubationAttempts, etTubeSize, etTubeDepth
 *   5. vascular-access: vascularAccess
 *   6. reversible-causes: reversibleCausesAddressed
 *   7. outcome: returnOfSpontaneousCirculation, roscTime, totalResuscitationDuration, resuscitationOutcome
 *   8. termination: terminationReason
 *   9. post-rosc: postRoscCareInitiated
 *   10. team-leader: teamLeaderName
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ResuscitationRecordsPDFTemplate from '../pdf-templates/ResuscitationRecordsPDFTemplate';
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldName]: value } }  (value is the full field value — whole array for array fields) */
...
const SECTION_TITLES = {
  'arrest-info': 'Arrest Information',
  'cpr-defib': 'CPR & Defibrillation',
  'medications': 'Medications Administered',
  'airway-mgmt': 'Airway Management',
  'vascular-access': 'Vascular Access',
  'reversible-causes': 'Reversible Causes',
  'outcome': 'Outcome',
  'termination': 'Termination',
  'post-rosc': 'Post-ROSC Care',
...
const FIELD_LABELS = {
  ...
  reversibleCausesAddressed: 'Reversible Causes Addressed',
  ...
  postRoscCareInitiated: 'Post-ROSC Care Initiated',
  teamLeaderName: 'Team Leader Name',
};

const SECTION_FIELDS = {
  'arrest-info': ['arrestDateTime', 'arrestLocation', 'arrestWitnessed', 'initialRhythm'],
  'cpr-defib': ['bystanderCprProvided', 'cprStartTime', 'timeToFirstCompression', 'timeToFirstDefibrillation', 'totalShocksDelivered', 'shockEnergiesDelivered'],
  'medications': ['epinephrineDoses', 'amiodaroneDoses'],
  'airway-mgmt': ['airwayManagement', 'intubationAttempts', 'etTubeSize', 'etTubeDepth'],
  'vascular-access': ['vascularAccess'],
  'reversible-causes': ['reversibleCausesAddressed'],
  'outcome': ['returnOfSpontaneousCirculation', 'roscTime', 'totalResuscitationDuration', 'resuscitationOutcome'],
  'termination': ['terminationReason'],
  'post-rosc': ['postRoscCareInitiated'],
...
const STRING_FIELDS = ['arrestLocation', 'initialRhythm', 'airwayManagement', 'etTubeSize', 'etTubeDepth', 'resuscitationOutcome', 'terminationReason', 'teamLeaderName'];
...
/* toInputDateTime: produce a 'YYYY-MM-DDTHH:mm' string for <input type="datetime-local">.
   Uses local-clock components (not UTC) so the displayed time matches the picker. */
...
const ResuscitationRecordsDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Resuscitation Record ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Stage a DRAFT for an edited field in localStorage (survives refresh). Keyed by the record's id.
  ...
  // Drop a record-level approved flag for the section containing this field so re-edit returns to yellow Pending Approve.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // localEdits keys are "field-idx"; pending ones for this section's fields are the staged drafts.
      const toCommit = fields
        .map(f => `${f}-${idx}`)
        .filter(k => pendingEdits[k] && localEdits[k] !== undefined);
      for (const editKey of toCommit) {
        const dashIdx = editKey.lastIndexOf('-');
        const fieldPart = editKey.slice(0, dashIdx); // "field" — never a numeric arrayIndex suffix here
        const lastSeg = fieldPart.includes('.') ? fieldPart.slice(fieldPart.lastIndexOf('.') + 1) : '';
        const payload = { field: /^\d+$/.test(lastSeg) ? fieldPart.slice(0, fieldPart.lastIndexOf('.')) : fieldPart, value: localEdits[editKey] };
        if (/^\d+$/.test(lastSeg)) payload.arrayIndex = parseInt(lastSeg, 10);
        await secureApiClient.put(`/api/edit/resuscitation_records/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/resuscitation_records/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATETIME_FIELDS.includes(f)) {
        ...
        const suffix = (f === 'timeToFirstCompression' || f === 'timeToFirstDefibrillation' || f === 'totalResuscitationDuration') ? ' minutes' : '';
        text += `${label}: ${val}${suffix}\n\n`;
  ...
  const copyAllText = useCallback(async () => {
    ...
    pdfData.forEach((r, idx) => {
      text += `Resuscitation Record ${idx + 1}\n${'='.repeat(40)}\n\n`;
          ...
          {isEditing ? (
            <div className="edit-field-container">
              ...
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const d = new Date(editValue); if (isNaN(d.getTime())) { setSaveError('Please enter a valid date/time'); return; } handleSaveField(record, fn, idx, sid, null, d.toISOString()); }}>{saving ? 'Saving...' : 'Save'}</button>
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    ...
    const isEditing = editingField === editKey;
    const label = FIELD_LABELS[fn] || fn;
    const suffix = (fn === 'timeToFirstCompression' || fn === 'timeToFirstDefibrillation' || fn === 'totalResuscitationDuration') ? ' minutes' : '';
    ...
    return (
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            if (DATETIME_FIELDS.includes(f)) return renderDateField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="resuscitation-records-document" ref={containerRef}>
    ...
    <div className="resuscitation-records-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<ResuscitationRecordsPDFTemplate document={pdfData} />} fileName={`resuscitation-records-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search resuscitation records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            ...
            <div className="record-header">
              {hasVal(record.date || record.arrestDateTime || record.createdAt) && (
                <div className="record-meta-row">
                  <span className="record-date">{formatDate(record.date || record.arrestDateTime || record.createdAt)}</span>
                  {hasVal(record.returnOfSpontaneousCirculation) && (
                    <span className={`status-badge ${record.returnOfSpontaneousCirculation ? 'rosc-yes' : 'rosc-no'}`}>
                      {record.returnOfSpontaneousCirculation ? 'ROSC Achieved' : 'No ROSC'}
                    </span>
                  )}
                </div>
              )}
              <h3 className="record-name">{highlightText(`Resuscitation Record ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'arrest-info')}
            {renderSection(record, idx, 'cpr-defib')}
            {renderSection(record, idx, 'medications')}
            {renderSection(record, idx, 'airway-mgmt')}
            {renderSection(record, idx, 'vascular-access')}
            {renderSection(record, idx, 'reversible-causes')}
            {renderSection(record, idx, 'outcome')}
            {renderSection(record, idx, 'termination')}
            {renderSection(record, idx, 'post-rosc')}
            {renderSection(record, idx, 'team-leader')}
          </div>
        ))}
      </div>
    </div>
