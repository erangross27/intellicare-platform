 ...
 * Collection: tube_feeding_order (displayed as "Tube Feeding Orders")
 *
 * 10 Sections:
 *   1. formula-info: formulaName, formulaCaloriesDensity, proteinContentPerLiter
 *   2. feeding-route-method: feedingRouteType, feedingTubeSize, feedingMethodType
 *   3. rate-advancement: initialInfusionRate, goalInfusionRate, rateAdvancementSchedule
 *   4. caloric-protein-goals: dailyCaloricGoal, dailyProteinGoal
 *   5. water-flush-protocol: freeWaterFlushVolume, freeWaterFlushFrequency
 *   6. gastric-monitoring: gastricResidualVolumeThreshold, gastricResidualCheckFrequency
 *   7. safety-positioning: headOfBedElevation, prokineticsOrdered
 *   8. cyclic-bolus-feeding: cyclicFeedingStartTime, cyclicFeedingDurationHours, bolusVolume, bolusFrequency
 *   9. supplements-patency: modularsSupplements, tubePatencyFlushSolution
 *  10. risk-glycemic: refeedingSyndromeRisk, glycemicControlProtocol
 */
...
const SECTION_TITLES = {
  ...
  'feeding-route-method': 'Feeding Route & Method',
  'rate-advancement': 'Rate & Advancement',
  'caloric-protein-goals': 'Caloric & Protein Goals',
  'water-flush-protocol': 'Water Flush Protocol',
  'gastric-monitoring': 'Gastric Monitoring',
  'safety-positioning': 'Safety & Positioning',
  'cyclic-bolus-feeding': 'Cyclic & Bolus Feeding',
  'supplements-patency': 'Supplements & Patency',
  'risk-glycemic': 'Risk & Glycemic',
};

const FIELD_LABELS = {
  formulaName: 'Formula Name',
  formulaCaloriesDensity: 'Formula Calories Density',
  proteinContentPerLiter: 'Protein Content Per Liter',
  feedingRouteType: 'Feeding Route Type',
  feedingTubeSize: 'Feeding Tube Size',
  feedingMethodType: 'Feeding Method Type',
  initialInfusionRate: 'Initial Infusion Rate',
  goalInfusionRate: 'Goal Infusion Rate',
  rateAdvancementSchedule: 'Rate Advancement Schedule',
  dailyCaloricGoal: 'Daily Caloric Goal',
  dailyProteinGoal: 'Daily Protein Goal',
  freeWaterFlushVolume: 'Free Water Flush Volume',
  ...
  headOfBedElevation: 'Head of Bed Elevation',
  ...
  bolusVolume: 'Bolus Volume',
  bolusFrequency: 'Bolus Frequency',
  modularsSupplements: 'Modulars / Supplements',
...
/* For tube-feeding quantities, 0 is a sentinel ("not ordered / not applicable"), never a
   meaningful clinical value (e.g. gastricResidualVolumeThreshold:0 = post-pyloric N/A,
   bolusVolume:0 / cyclicFeedingDurationHours:0 = continuous feeding). Hide zeros. */
const MEANINGFUL_ZERO_FIELDS = [];
const BOOLEAN_FIELDS = ['prokineticsOrdered', 'refeedingSyndromeRisk'];
const ARRAY_FIELDS = ['modularsSupplements'];
const STRING_FIELDS = ['formulaName', 'feedingRouteType', 'feedingMethodType', 'rateAdvancementSchedule', 'freeWaterFlushFrequency', 'gastricResidualCheckFrequency', 'cyclicFeedingStartTime', 'bolusFrequency', 'tubePatencyFlushSolution', 'glycemicControlProtocol'];
const DATE_FIELDS = [];
...
/* ═══════ COMPONENT ═══════ */
const TubeFeedingOrderDocument = ({ document: docProp }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayElem) {
          // Array element draft: reconstruct the full array into localEdits[`${fn}-${idx}`]
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Tube Feeding Order ${idx + 1}`.toLowerCase();
    ...
    return filteredRecords.map((record, idx) => {
      ...
      Object.keys(localEdits).forEach(key => {
        ...
        if (m && parseInt(m[2]) === idx) {
          const fn = m[1];
          // pending drafts stay OUT of the PDF until approved (whole-value or any array-element draft)
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const suffix = `-${idx}`;
      // pendingEdits keys: "field-idx" (whole value) or "field.arrayIndex-idx" (array element)
      ...
      for (const editKey of toCommit) {
        ...
        if (isArrayElem) {
          ...
          await secureApiClient.put(`/api/edit/tube_feeding_order/${id}/edit`, { field: fn, value, arrayIndex });
        } else {
          await secureApiClient.put(`/api/edit/tube_feeding_order/${id}/edit`, { field: fieldPart, value: localEdits[fieldPart + suffix] });
        }
      }
      await secureApiClient.put(`/api/edit/tube_feeding_order/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (store[id]) {
        fields.forEach(f => {
          delete store[id][f];
          Object.keys(store[id]).forEach(fp => { const ld = fp.lastIndexOf('.'); if (ld !== -1 && fp.slice(0, ld) === f && /^\d+$/.test(fp.slice(ld + 1))) delete store[id][fp]; });
        });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      const val = getFieldValue(record, f, idx);
      if (NUMBER_FIELDS.includes(f) && val === 0 && !MEANINGFUL_ZERO_FIELDS.includes(f)) return;
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    if (val === 0 && !MEANINGFUL_ZERO_FIELDS.includes(fn)) return null;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; const localKey = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [localKey]: currentArr })); setPendingEdits(prev => ({ ...prev, [`${fn}.${itemIdx}-${idx}`]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const store = readDrafts(); if (!store[id2]) store[id2] = {}; store[id2][`${fn}.${itemIdx}`] = editValue; writeDrafts(store); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      return hasVal(val) || (NUMBER_FIELDS.includes(f) && val === 0 && MEANINGFUL_ZERO_FIELDS.includes(f));
    });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="tube-feeding-order-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Tube Feeding Orders</h2></div>
        <div className="empty-state">No tube feeding orders available</div>
      </div>
    );
  }
  ...
  return (
    <div className="tube-feeding-order-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Tube Feeding Orders</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<TubeFeedingOrderDocumentPDFTemplate document={pdfData} />} fileName={`tube-feeding-orders-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search tube feeding orders..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'formula-info')}
            {renderSection(record, idx, 'feeding-route-method')}
            {renderSection(record, idx, 'rate-advancement')}
            {renderSection(record, idx, 'caloric-protein-goals')}
            {renderSection(record, idx, 'water-flush-protocol')}
            {renderSection(record, idx, 'gastric-monitoring')}
            {renderSection(record, idx, 'safety-positioning')}
            {renderSection(record, idx, 'cyclic-bolus-feeding')}
            {renderSection(record, idx, 'supplements-patency')}
            {renderSection(record, idx, 'risk-glycemic')}
          </div>
        ))}
      </div>
    </div>
