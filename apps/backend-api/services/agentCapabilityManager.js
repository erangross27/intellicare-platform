...
class AgentCapabilityManager {
  ...
  async initialize() {
    ...
    this.serviceToken = await serviceAccountManager.authenticate('agent-capability-manager');
    return this;
  }
  ...
  getFunctionListing(language = 'en', category = null) {
    ...
    const categories = {
      ...
      medical: {
        ...
        functions: ["addMedicalHistory", "getFullMedicalReport", "updateMedicalHistory", "deleteMedicalHistory", "getLabResults", "addLabResult", "updateLabResult", "deleteLabResult", "getMedications", "addMedication", "updateMedication", "deleteMedication", "checkDrugInteractions", "getVitalSigns", "addVitalSigns", "updateVitalSigns", "deleteVitalSigns", "getAllergies", "addAllergy", "updateAllergy", "deleteAllergy", "getVaccinations", "addVaccination", "updateVaccination", "deleteVaccination"]
      },
      documents: {
        name: isHebrew ? "מסמכים" : "Documents",
        functions: ["uploadDocument", "getDocuments", "analyzeDocument", "deleteDocument", "searchDocuments", "batchUploadDocuments", "getDocumentOCR", "categorizeDocument", "retrievePendingUpload", "previewPendingDocument", "analyzeUploadedDocuments"]
      },
      appointments: {
        name: isHebrew ? "תורים" : "Appointments",
      ...
      imaging: {
        ...
        functions: ["orderImaging", "getImagingResults", "uploadImagingResults", "viewDICOM", "compareImagingStudies", "getImagingHistory", "analyzeMedicalImage", "compareMedicalImages", "getMedicalImageHistory"]
      },
      insurance: {
        name: isHebrew ? "ביטוח" : "Insurance",
        functions: ["verifyInsurance", "checkCoverage", "submitClaim", "getClaimStatus", "updateInsurance", "getPriorAuthorization", "estimateCosts", "startMedicareImport", "checkMedicareImportStatus", "getMedicaidEnrollment", "getMedicaidDrugUtilization", "checkMedicaidEligibility"]
      },
      billing: {
      ...
      visitRecording: {
        name: isHebrew ? "הקלטת ביקורים" : "Visit Recording",
        functions: ["startVisitRecording", "endVisitRecording", "startNewPatientVisit", "getPatientVisits"]
      }
    };
...
• "Cancel Monday's appointment"
  ...
  isCapabilityQuery(message) {
    ...
    // First check: Is this ONLY "help" or "help <command>"?
    // This prevents "can you help me do X" from triggering help menu
    if (/^\s*help\s*$/.test(lowerMessage) || /^\s*help\s+\w+/.test(lowerMessage)) {
      return true;
    }

    // Hebrew standalone help
    if (/^\s*עזרה\s*$/.test(lowerMessage) || /^\s*עזרה\s+\w+/.test(lowerMessage)) {
      return true;
    }

    // Now check other capability keywords (but exclude generic "help")
    const capabilityKeywords = [
      // English - specific questions about capabilities
      'what can you do', 'what do you do', 'your capabilities', 'your functions',
      'how to use', 'what functions', 'list functions', 'show functions',
      'available functions', 'what features', 'what can i do',
      // Hebrew - specific questions about capabilities
      'מה אתה יכול', 'מה אתה עושה', 'היכולות שלך', 'הפונקציות שלך',
      'איך להשתמש', 'אילו פונקציות', 'רשימת פונקציות', 'הצג פונקציות',
...
module.exports = new AgentCapabilityManager();