/**
 * DiabetesManagementDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: diabetes_management
 *
 * 11 Sections:
 *   1. session-info: date (date picker), provider, facility
 *   2. diabetes-overview: diabetesType, diagnosisDate (date)
 *   3. glycemic-control: hba1cValue (number), hba1cDate (date), fastingGlucose (number), randomGlucose (number), targetHba1c (number)
 *   4. medications-section: currentMedications[] (array of strings)
 *   5. insulin-section: insulinRegimen (object), basalInsulinDose (number), bolusInsulinRatio, correctionFactor (number)
 *   6. monitoring-section: glucoseMonitoringMethod, timeInRange (number), glucoseVariability (number)
 *   7. hypoglycemia-section: hypoglycemicEpisodes (number), severeHypoglycemiaHistory (boolean), hyperglycemicEpisodes (number), diabeticKetoacidosisHistory (boolean)
 *   8. complications-section: diabeticRetinopathy, lastEyeExamDate (date), diabeticNephropathy, urineAlbuminCreatinineRatio (number), estimatedGfr (number), diabeticNeuropathy
 *   9. cardiovascular-section: cardiovascularDisease, bloodPressure, statinTherapy (boolean), bodyMassIndex (number), weightChange (number)
 *  10. foot-exam-section: footExamCompleted (boolean), peripheralPulses
 *  11. lifestyle-followup: diabetesEducation, nutritionCounseling (boolean), physicalActivityLevel, smokingStatus, nextFollowUpDate (date)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import DiabetesManagementDocumentPDFTemplate from '../pdf-templates/DiabetesManagementDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './DiabetesManagementDocument.css';
...
const SECTION_TITLES = {
  ...
  'insulin-section': 'Insulin Regimen',
  'monitoring-section': 'Glucose Monitoring',
  'hypoglycemia-section': 'Hypoglycemia & Hyperglycemia',
  'complications-section': 'Complications',
  'cardiovascular-section': 'Cardiovascular & Body Composition',
  'foot-exam-section': 'Foot Examination',
  'lifestyle-followup': 'Lifestyle & Follow-up',
};
...
const FIELD_LABELS = {
  ...
  targetFastingGlucose: 'Target Fasting Glucose',
  targetPostprandialGlucose: 'Target Postprandial Glucose',
  lipidPanel: 'Lipid Panel',
  currentMedications: 'Current Medications',
  insulinRegimen: 'Insulin Regimen',
  basalInsulinDose: 'Basal Insulin Dose',
  bolusInsulinRatio: 'Bolus Insulin Ratio',
  correctionFactor: 'Correction Factor',
  glucoseMonitoringMethod: 'Monitoring Method',
  timeInRange: 'Time in Range (%)',
  glucoseVariability: 'Glucose Variability (%)',
  hypoglycemicEpisodes: 'Hypoglycemic Episodes',
  severeHypoglycemiaHistory: 'Severe Hypoglycemia History',
  hyperglycemicEpisodes: 'Hyperglycemic Episodes',
  diabeticKetoacidosisHistory: 'DKA History',
  diabeticRetinopathy: 'Diabetic Retinopathy',
  lastEyeExamDate: 'Last Eye Exam Date',
  diabeticNephropathy: 'Diabetic Nephropathy',
  urineAlbuminCreatinineRatio: 'Urine Albumin/Creatinine Ratio',
  ...
  footExamCompleted: 'Foot Exam Completed',
...
const SECTION_FIELDS = {
  ...
  'diabetes-overview': ['diabetesType', 'diagnosisDate'],
  'glycemic-control': ['hba1cValue', 'hba1cDate', 'fastingGlucose', 'randomGlucose', 'targetHba1c', 'targetFastingGlucose', 'targetPostprandialGlucose', 'lipidPanel'],
  'medications-section': ['currentMedications'],
  'insulin-section': ['insulinRegimen', 'basalInsulinDose', 'bolusInsulinRatio', 'correctionFactor'],
  'monitoring-section': ['glucoseMonitoringMethod', 'cgmData', 'timeInRange', 'glucoseVariability'],
  'hypoglycemia-section': ['hypoglycemicEpisodes', 'severeHypoglycemiaHistory', 'hyperglycemicEpisodes', 'diabeticKetoacidosisHistory'],
  'complications-section': ['diabeticRetinopathy', 'lastEyeExamDate', 'diabeticNephropathy', 'urineAlbuminCreatinineRatio', 'estimatedGfr', 'diabeticNeuropathy'],
  'cardiovascular-section': ['cardiovascularDisease', 'bloodPressure', 'statinTherapy', 'bodyMassIndex', 'weightChange'],
  'foot-exam-section': ['footExamCompleted', 'peripheralPulses'],
  'lifestyle-followup': ['diabetesEducation', 'nutritionCounseling', 'physicalActivityLevel', 'smokingStatus', 'nextFollowUpDate'],
};

const ARRAY_FIELDS = ['currentMedications'];
const DATE_FIELDS = ['date', 'diagnosisDate', 'hba1cDate', 'lastEyeExamDate', 'nextFollowUpDate'];
const NUMBER_FIELDS = ['hba1cValue', 'fastingGlucose', 'randomGlucose', 'targetHba1c', 'basalInsulinDose', 'correctionFactor', 'timeInRange', 'glucoseVariability', 'hypoglycemicEpisodes', 'hyperglycemicEpisodes', 'urineAlbuminCreatinineRatio', 'estimatedGfr', 'bodyMassIndex', 'weightChange'];
const BOOLEAN_FIELDS = ['severeHypoglycemiaHistory', 'diabeticKetoacidosisHistory', 'statinTherapy', 'footExamCompleted', 'nutritionCounseling'];
const SENTENCE_FIELDS = ['diabetesEducation', 'diabeticRetinopathy', 'cardiovascularDisease', 'peripheralPulses'];

const COPY_LINE_EQ = '='.repeat(40);
const COPY_LINE_DASH = '-'.repeat(40);

/* Sentinel zeros: the extractor writes 0 for numerics it could not read (HbA1c/glucose/BMI/
   correction factor 0 are not real results). Episode COUNTS and weightChange 0 ("no change")
   ARE real values and stay visible. Cross-record evidence: correctionFactor is 0 or 40,
   weightChange spans -12..28 including a real 0. */
...
/* weightChange is legitimately NEGATIVE (weight loss) — never clamp it */
const NON_NEGATIVE_FIELDS = NUMBER_FIELDS.filter(f => f !== 'weightChange');

/* Known fixed-choice scales → full-scale dropdowns; descriptive stored values stay selectable.
   Retinopathy/nephropathy/neuropathy/monitoring/activity are DESCRIPTIVE across records → text. */
const ENUM_FIELDS = {
...
/* humanizeKey: totalCholesterol -> Total Cholesterol, LDL -> LDL (acronyms preserved) */
...
const DRAFT_KEY = 'diabetes_managementPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const DiabetesManagementDocument = ({ document: docProp }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        const baseField = isArr ? fieldPart.slice(0, lastDot) : fieldPart;
  ...
  const fmtVal = useCallback((v) => { if (typeof v === 'boolean') return v ? 'Yes' : 'No'; if (typeof v === 'number') return String(v); if (v && typeof v === 'object' && !Array.isArray(v)) return Object.entries(v).filter(([, sv]) => sv !== null && sv !== undefined && sv !== '').map(([k, sv]) => `${humanizeKey(k)}: ${sv}`).join(' '); return String(v || ''); }, []);
  ...
  /* sentinel-aware visibility: 0 in a not-measured numeric = hidden; counts/weightChange 0 stay */
  const fieldShows = useCallback((fn, v) => hasVal(v) && !(typeof v === 'number' && v === 0 && HIDE_ZERO_FIELDS.includes(fn)), [hasVal]);

  /* stepper helper: adjust editValue by the value's step, clamped where negatives are impossible */
  const stepEditValue = (fn, dir) => {
    setEditValue(prev => {
      ...
      if (NON_NEGATIVE_FIELDS.includes(fn) && next < 0) next = 0;
      return String(next);
    });
  };
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Diabetes Management ${idx + 1}`.toLowerCase();
  ...
  const handleSaveField = useCallback((record, fn, idx, _sid, _sentIdx, valueOverride, editTrackingKey) => {
    ...
    // Persist the staged draft so it survives refresh (separate localStorage key, NOT the PDF/DB source).
    const store = readDrafts();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Persist each staged field for THIS section to the DB now. Draft store holds the precise
      // fieldPart ("field" or "field.arrayIndex") + value for this record.
      const store = readDrafts();
      ...
      for (const fieldPart of Object.keys(recDrafts)) {
        ...
        const isArr = lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1));
        const baseField = isArr ? fieldPart.slice(0, lastDot) : fieldPart;
        if (!fields.includes(baseField)) continue; // only commit fields belonging to this section
        const payload = { field: baseField, value: recDrafts[fieldPart] };
        if (isArr) payload.arrayIndex = parseInt(fieldPart.slice(lastDot + 1), 10);
        const resp = await secureApiClient.put(`/api/edit/diabetes_management/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        delete store[id][fieldPart];
        committedLocalKeys.push(`${baseField}-${idx}`);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/diabetes_management/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; committedLocalKeys.forEach(k => delete n[k]); return n; });
      // Drop this record's committed drafts from localStorage (keep any drafts for other sections)
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        const walk = (obj) => {
          Object.entries(obj).filter(([, v]) => hasVal(v)).forEach(([subKey, subVal]) => {
            text += `${humanizeKey(subKey)}\n${COPY_LINE_DASH}\n`;
            if (subVal && typeof subVal === 'object' && !Array.isArray(subVal)) walk(subVal);
            else text += `1. ${typeof subVal === 'boolean' ? (subVal ? 'Yes' : 'No') : String(subVal)}\n`;
          });
        };
        const entries = Object.entries(val).filter(([, v]) => hasVal(v));
        if (entries.length > 0) { text += head; walk(val); text += '\n'; }
      } else {
        text += `${head}1. ${fmtVal(val)}\n\n`;
      }
    });
    return text;
  }, [getFieldValue, hasVal, fieldShows, fmtVal, formatSentenceFieldLines]);

  const copyAllText = useCallback(async () => {
    let text = `Diabetes Management\n${COPY_LINE_EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Diabetes Management ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  /* ═══════ RENDER: ARRAY FIELD (currentMedications) ═══════ */
      ...
      <div key={fn}>
        <div className="rec-mini-card">
          ...
          {sentences.map((sentence, sIdx) => {
            ...
            /* Unlabeled sentence with >=3 guarded commas -> comma rows (whole-sentence edit) */
            if (!parsed.isLabeled) {
                  ...
                  <div key={sIdx}>
                    {isEditing ? (
                      ...
                      commaItems.map((ci, ciIdx) => {
                        ...
                        if (!ciMatches && searchTerm.trim()) return null;
                        return (
                          <div key={ciIdx} className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { setEditingField(sentenceKey); setEditValue(sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); }}>
  ...
  /* ═══════ RENDER: OBJECT FIELD (insulinRegimen, etc.) ═══════ */
  const renderObjectField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    if (!hasVal(val) || typeof val !== 'object' || Array.isArray(val)) return null;
    const label = FIELD_LABELS[fn] || fn;
    const sl = label.trim().toLowerCase() !== (SECTION_TITLES[sid] || '').trim().toLowerCase();
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
    const entries = Object.entries(val).filter(([, v]) => hasVal(v));
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {entries.map(([subKey, subVal]) => {
          ...
          const copyKey = `${fn}.${subKey}-${idx}`;
          /* nested object value → its own sub-rows (never a "k: v" joined blob) */
          if (subVal && typeof subVal === 'object' && !Array.isArray(subVal)) {
            return (
              <div key={subKey} className="nested-mini-card">
                <div className="nested-subtitle sub-label">{highlightText(subLabel)}</div>
                {Object.entries(subVal).filter(([, v]) => hasVal(v)).map(([k2, v2]) => (
                  <div key={k2} className="nested-mini-card">
                    <div className="nested-subtitle sub-label">{highlightText(humanizeKey(k2))}</div>
                    <div className="numbered-row">
                      <div className="row-content"><span className="content-value">{highlightText(String(v2))}</span></div>
                      <button className={`copy-btn ${copiedItems[`${copyKey}.${k2}`] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(String(v2), `${copyKey}.${k2}`); }}>{copiedItems[`${copyKey}.${k2}`] ? 'Copied!' : 'Copy'}</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          return (
            <div key={subKey} className="nested-mini-card">
              ...
              <div className="numbered-row">
                ...
                <button className={`copy-btn ${copiedItems[copyKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(displayVal, copyKey); }}>{copiedItems[copyKey] ? 'Copied!' : 'Copy'}</button>
              </div>
            </div>
          );
        })}
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            // Check for object values (insulinRegimen, targetFastingGlucose, etc.)
            const val = getFieldValue(record, f, idx);
            if (val && typeof val === 'object' && !Array.isArray(val)) return renderObjectField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="diabetes-management-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Diabetes Management</h2></div>
        <div className="empty-state">No diabetes management records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="diabetes-management-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Diabetes Management</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<DiabetesManagementDocumentPDFTemplate document={pdfData} />} fileName="Diabetes_Management.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search diabetes management..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Diabetes Management ${idx + 1}`)}</h3>
            </div>
            ...
            {renderSection(record, idx, 'insulin-section')}
            {renderSection(record, idx, 'monitoring-section')}
            {renderSection(record, idx, 'hypoglycemia-section')}
            {renderSection(record, idx, 'complications-section')}
            {renderSection(record, idx, 'cardiovascular-section')}
            {renderSection(record, idx, 'foot-exam-section')}
            {renderSection(record, idx, 'lifestyle-followup')}
          </div>
        ))}
      </div>
    </div>
...
export default DiabetesManagementDocument;
