/**
 * MedicalCertificatesDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: medical_certificates
 *
 * 3 Sections:
 *   1. certificate-info: reportDate (date), reportType (string), clinicalIndication (string), urgency (string)
 *   2. clinical-details: findings (string-long), recommendations (string-long)
 *   3. follow-up: followUp (string)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MedicalCertificatesDocumentPDFTemplate from '../pdf-templates/MedicalCertificatesDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './MedicalCertificatesDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name) */
const DRAFT_KEY = 'medical_certificatesPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  'certificate-info': ['reportDate', 'reportType', 'clinicalIndication', 'urgency'],
  'clinical-details': ['findings', 'recommendations'],
  'follow-up': ['followUp'],
};

const STRING_FIELDS = ['reportType', 'clinicalIndication', 'urgency', 'findings', 'recommendations', 'followUp'];
const NUMBER_FIELDS = [];
...
/* ═══════ COMPONENT ═══════ */
const MedicalCertificatesDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.medical_certificates) return Array.isArray(r.medical_certificates) ? r.medical_certificates : [r.medical_certificates];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.medical_certificates) return Array.isArray(dd.medical_certificates) ? dd.medical_certificates : [dd.medical_certificates]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Medical Certificate ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Find the sectionId that owns a given field name (to clear its approve button on re-edit).
  const sectionForField = useCallback((fn) => {
  ...
  // Stage a DRAFT in localEdits + the pending-drafts localStorage store (survives refresh).
  // NOT written to MongoDB and NOT shown in the PDF until the user clicks Approve (handleApproveSection commits).
  const stageDraft = useCallback((record, fn, idx, fullText, marks) => {
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Staged drafts for THIS section: editKey = "field-idx" whose base field is in this section.
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/medical_certificates/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail).
      await secureApiClient.put(`/api/edit/medical_certificates/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      const labelLine = sameAsTitle ? '' : `${label}\n`;
      const val = getFieldValue(record, f, idx);
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MEDICAL CERTIFICATES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Medical Certificate ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="medical-certificates-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Medical Certificates</h2></div>
        <div className="empty-state">No medical certificates records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="medical-certificates-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Medical Certificates</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MedicalCertificatesDocumentPDFTemplate document={pdfData} />} fileName="Medical_Certificates.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search medical certificates..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Medical Certificate ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'certificate-info')}
...
export default MedicalCertificatesDocument;
