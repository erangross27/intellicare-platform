...
const SECTIONS = [
  ...
  { title: 'Supplement', fields: ['supplement', 'dosage', 'condition'] },
  { title: 'Reasoning', fields: ['reasoning'] },
...
const LABELS = {
  date: 'Date', type: 'Plan Type', provider: 'Provider', facility: 'Facility',
  supplement: 'Supplement', dosage: 'Dosage', condition: 'Condition', reasoning: 'Reasoning',
...
const unwrap = (input) => {
  ...
  return (Array.isArray(input) ? input : [input])
    .flatMap((item) => {
      if (item?.supplementation_plans)
        return Array.isArray(item.supplementation_plans)
          ? item.supplementation_plans
          : [item.supplementation_plans];
      if (item?.documentData?.supplementation_plans)
        return Array.isArray(item.documentData.supplementation_plans)
          ? item.documentData.supplementation_plans
          : [item.documentData.supplementation_plans];
...
const SupplementationPlansDocumentPDFTemplate = ({
  document,
  data,
  templateData,
}) => {
        ...
        records.map((record, recordIndex) => (
          <Page size="A4" style={styles.page} key={recordIndex}>
            ...
            <View style={styles.record}>
              ...
              >{`Supplementation Plans ${recordIndex + 1}`}</Text>
