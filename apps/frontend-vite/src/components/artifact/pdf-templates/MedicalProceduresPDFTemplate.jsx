/**
 * MedicalProceduresPDFTemplate.jsx
 * Box-free B&W LETTER (canonical, memory 6a2d6af6) - mirrors the JSX: real record.date (never
 * createdAt/updatedAt), narratives split on [.;] with abbrev/single-initial guard + labeled comma-split,
 * values numbered ('1.' even singles), single-name label gate, status enum canonicalized, specimens/
 * implants as numbered lists. Rule #74: each field is ONE wrap={false} atomic View with the sectionTitle
 * riding INSIDE the first present field's View. Static PHI footer. Collection: medical_procedures.
 * safeString uses ONLY \uXXXX escapes (0 non-ASCII source bytes).
 */
...
const SECTION_ORDER = ['procedure-information', 'indication', 'technique', 'findings', 'complications', 'outcome', 'specimens', 'implants', 'notes'];
...
const SECTION_FIELDS = {
  'procedure-information': ['date', 'procedureName', 'procedureType', 'status', 'provider', 'facility', 'anesthesia'],
  'indication': ['indication'],
  'technique': ['technique'],
  'findings': ['findings'],
  'complications': ['complications'],
...
const ARRAY_FIELDS = ['specimens', 'implants'];
const ENUM_OPTIONS = { status: ['Scheduled', 'In Progress', 'Completed', 'On Hold', 'Cancelled'] };
const ENUM_FIELDS = Object.keys(ENUM_OPTIONS);
const enumCanonical = (fn, val) => { const opts = ENUM_OPTIONS[fn] || []; const hit = opts.find(o => o.toLowerCase() === String(val ?? '').trim().toLowerCase()); return hit || String(val ?? ''); };
...
/* safeString: normalize the handful of non-Helvetica glyphs to ASCII. \uXXXX escapes ONLY - never paste a
   literal smart-quote / dash / BOM into this source (that triggers "Unterminated regular expression"). */
...
const renderField = (record, f, sectionTitle, isFirst) => {
  ...
  if (ARRAY_FIELDS.includes(f)) {
    ...
    return [(
      <View key={f} style={styles.fieldGroup} wrap={false}>
        ...
        {items.map((it, i) => (<Text key={i} style={styles.value}>{i + 1}. {safeString(String(it).trim())}</Text>))}
      </View>
    )];
  }
  ...
  if (DATE_FIELDS.includes(f)) {
    ...
    body = <Text style={styles.value}>1. {safeString(enumCanonical(f, val))}</Text>;
  } else {
...
const MedicalProceduresPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].medical_procedures && Array.isArray(templateData[0].medical_procedures)) records = templateData[0].medical_procedures;
    else records = templateData;
  } else if (templateData && templateData.medical_procedures) {
    records = Array.isArray(templateData.medical_procedures) ? templateData.medical_procedures : [templateData.medical_procedures];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.medical_procedures ? (Array.isArray(dd.medical_procedures) ? dd.medical_procedures : [dd.medical_procedures]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Medical Procedures</Text></View>
        <Text style={styles.emptyState}>No medical procedure records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Medical Procedure ${idx + 1}`}</Text>
...
export default MedicalProceduresPDFTemplate;
