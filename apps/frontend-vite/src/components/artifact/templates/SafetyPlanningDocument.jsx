 ...
 * July 2026 — recursive, dot-path editable safety planning
...
const SECTION_TITLES = {
  riskAssessment: 'Risk Assessment', riskFactors: 'Risk Factors Identified', warningSigns: 'Warning Signs Identified', protectiveFactors: 'Protective Factors Present', copingStrategies: 'Coping Strategies Documented', safetyMeasures: 'Safety Measures', medicationChanges: 'Psychiatric Medication Changes', followUp: 'Follow-Up',
};
const FIELD_LABELS = {
  suicidalIdeationSeverity: 'Suicidal Ideation Severity', columbiaSuicideSeverityScore: 'Columbia Suicide Severity Score', phq9Score: 'PHQ-9 Score', gad7Score: 'GAD-7 Score', suicideAttemptHistory: 'Suicide Attempt History', substanceUseScreening: 'Substance Use Screening', riskFactorsIdentified: 'Risk Factors Identified', warningSignsIdentified: 'Warning Signs Identified', protectiveFactorsPresent: 'Protective Factors Present', copingStrategiesDocumented: 'Coping Strategies Documented', meansRestrictionImplemented: 'Means Restriction Implemented', emergencyContactsVerified: 'Emergency Contacts Verified', crisisHotlineProvided: 'Crisis Hotline Provided', familyMembersNotified: 'Family Members Notified', environmentalHazardsAssessed: 'Environmental Hazards Assessed', involuntaryCommitmentCriteria: 'Involuntary Commitment Criteria', medicationComplianceAssessed: 'Medication Compliance Assessed', psychiatricMedicationChanges: 'Psychiatric Medication Changes', followUpAppointmentScheduled: 'Follow-Up Appointment Scheduled', safetyPlanCompletionDate: 'Safety Plan Completion Date', riskLevelDesignation: 'Risk Level Designation',
};
const SECTION_FIELDS = {
  riskAssessment: ['suicidalIdeationSeverity', 'columbiaSuicideSeverityScore', 'phq9Score', 'gad7Score', 'suicideAttemptHistory', 'substanceUseScreening', 'riskLevelDesignation'], riskFactors: ['riskFactorsIdentified'], warningSigns: ['warningSignsIdentified'], protectiveFactors: ['protectiveFactorsPresent'], copingStrategies: ['copingStrategiesDocumented'], safetyMeasures: ['meansRestrictionImplemented', 'emergencyContactsVerified', 'crisisHotlineProvided', 'familyMembersNotified', 'environmentalHazardsAssessed', 'involuntaryCommitmentCriteria', 'medicationComplianceAssessed'], medicationChanges: ['psychiatricMedicationChanges'], followUp: ['followUpAppointmentScheduled', 'safetyPlanCompletionDate'],
};
const DATE_FIELDS = ['followUpAppointmentScheduled', 'safetyPlanCompletionDate'];
const ENUM_FIELDS = ['suicidalIdeationSeverity', 'riskLevelDesignation'];
...
const COMMA_FIELDS = ['substanceUseScreening'];
...
const SEMICOLON_FIELDS = ['suicideAttemptHistory'];
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'riskLevelDesignation' ? ['Low', 'Moderate', 'High', 'Critical']
    : field === 'suicidalIdeationSeverity' ? ['None', 'Passive', 'Active', 'Severe'] : [];
...
const SafetyPlanningDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.safety_planning) return Array.isArray(record.safety_planning) ? record.safety_planning : [record.safety_planning];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.safety_planning) return Array.isArray(nested.safety_planning) ? nested.safety_planning : [nested.safety_planning];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/safety_planning/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/safety_planning/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SafetyPlanning] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Safety Planning ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="safety-planning-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Safety Planning</h2></div><div className="empty-state">No safety planning records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Safety Planning ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="safety-planning-document" ref={containerRef}>
    <div className="document-header">
      ...
      <div className="header-actions">
        ...
        <PDFDownloadLink document={<SafetyPlanningDocumentPDFTemplate document={pdfData} />} fileName="Safety_Planning.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search safety planning..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Safety Planning ' + (recordIndex + 1))}</h3></div>
