/**
 * MedicalAlertsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: medical_alerts
 *
 * 5 Sections:
 *   1. report-info: reportDate (date), reportType, clinicalIndication, urgency
 *   2. alert-details: alertType, alertTitle, severity, status, triggeredBy, triggeredDate (date)
 *   3. acknowledgment: acknowledgedBy, acknowledgedDate (date), resolution, expirationDate (date)
 *   4. related-info: relatedCondition, relatedMedication, findings, followUp
 *   5. notes-recommendations: notes, recommendations
 *
 * Record-date rule: this collection has NO clinical `date` field (only createdAt/updatedAt ingestion
 * timestamps) → NO record date is rendered in the header (title only). `reportDate` IS a real date field
 * and is rendered as a normal field (BlueDatePicker), not a header pill.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MedicalAlertsDocumentPDFTemplate from '../pdf-templates/MedicalAlertsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './MedicalAlertsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'medical_alertsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'acknowledgment': 'Acknowledgment & Resolution',
...
const SECTION_FIELDS = {
  'report-info': ['reportDate', 'reportType', 'clinicalIndication', 'urgency'],
  'alert-details': ['alertType', 'alertTitle', 'severity', 'status', 'triggeredBy', 'triggeredDate'],
  'acknowledgment': ['acknowledgedBy', 'acknowledgedDate', 'resolution', 'expirationDate'],
  'related-info': ['relatedCondition', 'relatedMedication', 'findings', 'followUp'],
  'notes-recommendations': ['notes', 'recommendations'],
};

/* No numeric fields in this collection — num-stepper renderer kept for parity but unused. */
const NUMBER_FIELDS = [];

const STRING_FIELDS = ['reportType', 'clinicalIndication', 'urgency', 'alertType', 'alertTitle', 'severity', 'status', 'triggeredBy', 'acknowledgedBy', 'resolution', 'relatedCondition', 'relatedMedication', 'findings', 'followUp', 'notes', 'recommendations'];
const DATE_FIELDS = ['reportDate', 'triggeredDate', 'acknowledgedDate', 'expirationDate'];
...
/* ═══════ COMPONENT ═══════ */
const MedicalAlertsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.medical_alerts) return Array.isArray(r.medical_alerts) ? r.medical_alerts : [r.medical_alerts];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.medical_alerts) return Array.isArray(dd.medical_alerts) ? dd.medical_alerts : [dd.medical_alerts]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Medical Alert ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/medical_alerts/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/medical_alerts/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MEDICAL ALERTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Medical Alert ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="medical-alerts-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Medical Alerts</h2></div>
        <div className="empty-state">No medical alerts records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="medical-alerts-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Medical Alerts</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MedicalAlertsDocumentPDFTemplate document={pdfData} />} fileName="Medical_Alerts.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search medical alerts..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Medical Alert ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'report-info')}
            {renderSection(record, idx, 'alert-details')}
            {renderSection(record, idx, 'acknowledgment')}
            {renderSection(record, idx, 'related-info')}
...
export default MedicalAlertsDocument;
