/**
 * IndianDietExchangeListsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: indian_diet_exchange_lists (displayed as "Indian Diet Exchange Lists")
 *
 * 9 Sections:
 *   1. record-info: date, provider, facility, status, provided
 *   2. categories: categories
 *   3. instructions: instructions
 *   4. findings: findings
 *   5. assessment: assessment
 *   6. plan: plan
 *   7. results: results (OBJECT — recursive renderObjectField/Node/Leaf)
 *   8. recommendations: recommendations (ARRAY of {recommendation, date})
 *   9. notes: notes
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import IndianDietExchangeListsDocumentPDFTemplate from '../pdf-templates/IndianDietExchangeListsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './IndianDietExchangeListsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = top-level field name; value = the
   full reconstructed field value the save handler built — string, array, or object). */
const DRAFT_KEY = 'indian_diet_exchange_listsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['instructions', 'provider', 'facility', 'findings', 'assessment', 'plan', 'notes', 'status'];

/* humanizeKey: snake/camel → Title Case for object leaf labels */
...
/* splitByComma: parenthesis-aware comma split (thousands-guard: only split on a comma followed by whitespace) */
...
/* ═══════ COMPONENT ═══════ */
const IndianDietExchangeListsDocument = ({ document: docProp }) => {
  ...
  /* Stage a DRAFT locally + write it to the pending-drafts localStorage store (survives refresh).
     ...
     commits). `fn` is the top-level field; `fullValue` is the complete reconstructed field value
     (string / array / object) — same value the old code PUT to the DB. */
  ...
  /* Stage a nested OBJECT leaf edit by dot-path (e.g. results.calories) — value stays a STRING.
     DRAFT only: rebuild the full object locally, stage it, and persist to localStorage. No DB write. */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    stageFieldDraft(record, rootField, idx, sid, clone);
    setEditedFields(prev => ({ ...prev, [leafKeyTrack]: 'edited' }));
    setEditingField(null); setEditValue('');
  }, [safeId, localEdits, stageFieldDraft]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Indian Diet Exchange Lists ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // localEdits keys are always "<field>-<idx>" carrying the FULL reconstructed field value
      // (string / array / object). Commit each pending field for this section now.
      const toCommit = fields
        .map(f => `${f}-${idx}`)
        .filter(editKey => pendingEdits[editKey] && localEdits[editKey] !== undefined);
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, editKey.length - `-${idx}`.length); // top-level field name
        const dotIdx = fieldPart.lastIndexOf('.');
        // arrayIndex ONLY when the trailing dot-segment is purely numeric (none here — guard anyway)
        const trailingNumeric = dotIdx !== -1 && /^\d+$/.test(fieldPart.slice(dotIdx + 1));
        const payload = { field: trailingNumeric ? fieldPart.slice(0, dotIdx) : fieldPart, value: localEdits[editKey] };
        if (trailingNumeric) payload.arrayIndex = parseInt(fieldPart.slice(dotIdx + 1), 10);
        const resp = await secureApiClient.put(`/api/edit/indian_diet_exchange_lists/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/indian_diet_exchange_lists/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    const fields = SECTION_FIELDS[sid] || [];
    // Skip a section with no populated fields — its title must not pollute Copy All (keeps JSX/PDF parity).
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        if (showLabelCopy) text += `${label}\n`;
        const walk = (obj, indent) => { Object.entries(obj).forEach(([k, v]) => { if (isEmptyDeep(v)) return; if (isScalar(v)) text += `${indent}${humanizeKey(k)}: ${fmtScalar(v)}\n`; else { text += `${indent}${humanizeKey(k)}\n`; walk(v, indent + '  '); } }); };
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== INDIAN DIET EXCHANGE LISTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Indian Diet Exchange Lists ${idx + 1}\n${'='.repeat(40)}\n\n`;
          ...
          {isEditing ? (
            <div className="edit-field-container">
              ...
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const boolVal = editValue === 'Yes'; if (!safeId(record)) return; stageFieldDraft(record, fn, idx, sid, boolVal); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(toInputDate(val)); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              ...
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date'); return; } if (!safeId(record)) return; const dateVal = editValue + 'T00:00:00.000Z'; stageFieldDraft(record, fn, idx, sid, dateVal); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (!safeId(record)) return; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; stageFieldDraft(record, fn, idx, sid, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* Single-value string: saveSentence editable */
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
    const isModified = editedFields[editKey];
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(strVal); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              ...
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (!safeId(record)) return; stageFieldDraft(record, fn, idx, sid, editValue); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              stageFieldDraft(record, fn, idx, sid, newArr);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="indian-diet-exchange-lists-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Indian Diet Exchange Lists</h2></div>
        <div className="empty-state">No indian diet exchange lists available</div>
      </div>
    );
  }
  ...
  return (
    <div className="indian-diet-exchange-lists-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Indian Diet Exchange Lists</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<IndianDietExchangeListsDocumentPDFTemplate document={pdfData} />} fileName="Indian_Diet_Exchange_Lists.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search indian diet exchange lists..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Indian Diet Exchange Lists ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'record-info')}
            {renderSection(record, idx, 'categories')}
            {renderSection(record, idx, 'instructions')}
...
export default IndianDietExchangeListsDocument;
