/**
 * ParentalConcernsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: parental_concerns
 *
 * 8 Sections:
 *   1. record-info: provider, facility, date, status
 *   2. concerns: concerns (array of {topic, description, addressed})
 *   3. family-home: familySupport, homeEnvironment, parentingStress, siblingRelationships
 *   4. findings: findings (narrative string, per-sentence editing)
 *   5. assessment-plan: assessment, plan
 *   6. results: results (OBJECT, recursive key/value, editable leaves)
 *   7. recommendations: recommendations (array of strings or {recommendation, date})
 *   8. notes: notes
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ParentalConcernsDocumentPDFTemplate from '../pdf-templates/ParentalConcernsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './ParentalConcernsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the record field name) */
const DRAFT_KEY = 'parental_concernsPendingEdits';
const readDrafts = () => {
...
const CONCERNS_FIELD = 'concerns';
const STRING_FIELDS = ['provider', 'facility', 'status', 'familySupport', 'homeEnvironment', 'parentingStress', 'siblingRelationships', 'findings', 'assessment', 'plan', 'notes'];
...
/* ═══════ COMPONENT ═══════ */
const ParentalConcernsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.parental_concerns) return Array.isArray(r.parental_concerns) ? r.parental_concerns : [r.parental_concerns];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.parental_concerns) return Array.isArray(dd.parental_concerns) ? dd.parental_concerns : [dd.parental_concerns]; return [dd]; }
      return [r];
    });
  ...
  const shouldShowSection = useCallback((record, sid, idx) => {
    ...
    for (const f of fields) {
      if (f === CONCERNS_FIELD) {
        const concerns = getFieldValue(record, f, idx) || [];
        if (Array.isArray(concerns)) {
          for (const c of concerns) {
            if ([c.topic, c.description, c.addressed, 'Topic', 'Description', 'Addressed'].some(v => v && String(v).toLowerCase().includes(phrase))) return true;
          }
        }
        continue;
      }
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Parental Concerns ${idx + 1}`.toLowerCase();
      ...
      for (const [sid, fields] of Object.entries(SECTION_FIELDS)) {
        for (const f of fields) {
          if (f === CONCERNS_FIELD) {
            ...
            if (Array.isArray(concerns)) {
              for (const c of concerns) {
                if ([c.topic, c.description, c.addressed].some(v => v && String(v).toLowerCase().includes(phrase))) return true;
              }
            }
            continue;
          }
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* Stage a DRAFT locally (localEdits + pendingEdits) and persist to the pending-drafts localStorage
     store (survives refresh). NOT written to MongoDB and NOT shown in the PDF until Approve commits.
     localEdits/pendingEdits/draft are keyed by the field name "<fn>" → editKey "<fn>-<idx>". */
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.score) — value stays a STRING/number/bool */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    // Build the updated root object and STAGE it (no DB write). Approve commits the whole object.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Each staged editKey for this section/record = "<field>-<idx>"; localEdits[key] is the full field value.
      ...
      for (const editKey of toCommit) {
        ...
        // arrayIndex ONLY when the segment after the LAST dot is purely numeric (none occur here, but stay safe)
        ...
        const resp = await secureApiClient.put(`/api/edit/parental_concerns/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/parental_concerns/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[ParentalConcerns] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      if (f === CONCERNS_FIELD) { const c = getFieldValue(record, f, idx); return Array.isArray(c) && c.length > 0; }
    ...
    fields.forEach(f => {
      if (f === CONCERNS_FIELD) {
        ...
        if (Array.isArray(concerns) && concerns.length > 0) {
          concerns.forEach((c, cIdx) => {
            text += `${cIdx + 1}. ${c.topic || 'Concern'}\n`;
            if (c.description) text += `   Description: ${c.description}\n`;
            if (c.addressed) text += `   Addressed: ${c.addressed}\n`;
          });
          text += '\n';
        }
        return;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PARENTAL CONCERNS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Parental Concerns ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: CONCERN LEAF (sub-field: sub-label ABOVE a plain editable value row; never "Label: value") ═══════ */
  const renderConcernLeaf = (record, idx, sid, cIdx, key, value) => {
    const editKey = `concerns.${cIdx}.${key}-${idx}`;
    const isEditing = editingField === editKey;
    const isModified = editedFields[editKey];
    ...
    if (searchTerm.trim() && !record._showAllSections && !sectionTitleMatches(sid)) {
      const phrase = searchTerm.toLowerCase().trim();
      if (!valStr.toLowerCase().includes(phrase) && !label.toLowerCase().includes(phrase)) return null;
    }
    return (
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(valStr); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              ...
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentConcerns = (getFieldValue(record, 'concerns', idx) || []).map(c => (c && typeof c === 'object') ? { ...c } : c); if (currentConcerns[cIdx] && typeof currentConcerns[cIdx] === 'object') currentConcerns[cIdx][key] = editValue; stageDraft(record, 'concerns', idx, sid, currentConcerns); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
  ...
  /* ═══════ RENDER: CONCERNS SECTION (array of objects → topic heading + leaf sub-fields) ═══════ */
  const renderConcernsSection = (record, idx) => {
    const sid = 'concerns';
    const concerns = getFieldValue(record, 'concerns', idx) || [];
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          <div className="section-header">
            <h4 className="section-title">{highlightText('Concerns')}</h4>
          ...
          {concerns.map((concern, cIdx) => {
            ...
            /* Search filter at concern level */
            if (searchTerm.trim() && !record._showAllSections && !sectionTitleMatches(sid)) {
              const phrase = searchTerm.toLowerCase().trim();
              const concernText = [concern.topic, concern.description, concern.addressed, 'Topic', 'Description', 'Addressed'].filter(Boolean).join(' ').toLowerCase();
              if (!concernText.includes(phrase)) return null;
            }

            const topic = (concern.topic || '').trim();
            const leaves = Object.entries(concern).filter(([k, v]) => k !== 'topic' && hasVal(v));
            return (
              <div key={cIdx} className="rec-mini-card">
                <div className="nested-subtitle">{highlightText(topic || `Concern ${cIdx + 1}`)}</div>
                {leaves.map(([k, v]) => renderConcernLeaf(record, idx, sid, cIdx, k, v))}
              </div>
            );
          })}
        </div>
  ...
  const renderSection = (record, idx, sid) => {
    if (sid === 'concerns') return renderConcernsSection(record, idx);
    const title = SECTION_TITLES[sid];
    if (!shouldShowSection(record, sid, idx)) return null;
    const fields = SECTION_FIELDS[sid] || [];
  ...
  if (!records || records.length === 0) {
    return (
      <div className="parental-concerns-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Parental Concerns</h2></div>
        <div className="empty-state">No parental concerns records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="parental-concerns-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Parental Concerns</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ParentalConcernsDocumentPDFTemplate document={pdfData} />} fileName="Parental_Concerns.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search parental concerns..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Parental Concerns ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'record-info')}
            {renderSection(record, idx, 'concerns')}
            {renderSection(record, idx, 'family-home')}
...
export default ParentalConcernsDocument;
