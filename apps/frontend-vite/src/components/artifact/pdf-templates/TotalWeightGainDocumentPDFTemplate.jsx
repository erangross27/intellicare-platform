...
const TotalWeightGainDocumentPDFTemplate = ({ document: data }) => {
  ...
  if (Array.isArray(data)) {
    records = data;
  } else if (data?.total_weight_gain && Array.isArray(data.total_weight_gain)) {
    records = data.total_weight_gain;
  } else if (data?.documentData) {
    const docData = data.documentData;
    if (Array.isArray(docData)) {
      records = docData;
    } else if (docData?.total_weight_gain) {
      records = docData.total_weight_gain;
  ...
  if (!records || records.length === 0) {
    return (<Document><Page size="LETTER" style={styles.page}><View style={styles.documentHeader}><Text style={styles.title}>Total Weight Gain</Text></View><Text style={styles.emptyState}>No records available</Text></Page></Document>);
  }

  return (
    <Document>
      <Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.title}>Total Weight Gain</Text></View>
          ...
          return (
            <View key={idx} style={styles.recordContainer}>
              <View style={styles.recordHeader} wrap={false}>
                <Text style={styles.recordTitle}>{`Total Weight Gain ${idx + 1}`}</Text>
                {record.date && <Text style={styles.recordMeta}>{formatDate(record.date)}</Text>}
              </View>

              {/* 1. Weight Summary */}
              {(hasVal(record.amount) || hasVal(record.unit) || hasVal(record.timeframe) || hasVal(record.date) || hasVal(record.provider) || hasVal(record.facility)) && (
                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Weight Summary</Text>
                  {hasVal(record.amount) && renderFieldRow('Amount', record.amount)}
                  {hasVal(record.unit) && renderFieldRow('Unit', record.unit)}
                  {renderSentenceField('Timeframe', record.timeframe, ctr)}
              ...
              {Array.isArray(record.recommendations) && record.recommendations.length > 0 && (
                <View style={styles.section}>
                  ...
                  {renderArrayField('Items', record.recommendations, ctr)}
                </View>
              )}
            </View>
          );
