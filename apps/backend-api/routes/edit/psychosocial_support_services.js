...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'psychosocial-support-services-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['phq9Score', 'gad7Score', 'columbiaScore', 'auditScore', 'aceScore', 'primaryDiagnosis', 'suicidalIdeation', 'substanceUseDisorder', 'traumaHistory', 'crisisInterventionProvided', 'therapyModalityProvided', 'copingMechanisms', 'referralsMade', 'treatmentAdherence', 'supportSystemStrength', 'functionalAssessment', 'cognitiveFunction', 'socialDeterminantsScreening', 'housingStability', 'culturalConsiderations', 'dischargeDisposition'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('psychosocial_support_services', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[psychosocial_support_services] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('psychosocial_support_services', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[psychosocial_support_services] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
