/**
 * ReadmissionRiskAssessmentDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: readmission_risk_assessment
 *
 * 4 Sections:
 *   1. risk-scores: date, laceIndexScore, hospitalScore, readmissionProbability, riskStratificationLevel, charlsonComorbidityIndex, functionalStatusScore
 *   2. utilization: indexAdmissionDiagnosis, lengthOfStayDays, priorAdmissionsCount, priorEmergencyVisitsCount, dischargeDisposition, heartFailureExacerbation, activeCancerDiagnosis, chronicKidneyDiseaseStage
 *   3. medications-risk: medicationCount, highRiskMedications, polypharmacyIndicator, cognitiveImpairmentPresent, nutritionalRiskIndicator, pendingTestResultsAtDischarge
 *   4. social-mitigation: adequateSocialSupport, healthLiteracyScore, followUpAppointmentScheduled, mitigationInterventions, assessmentPerformedBy
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ReadmissionRiskAssessmentDocumentPDFTemplate from '../pdf-templates/ReadmissionRiskAssessmentDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './ReadmissionRiskAssessmentDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'risk-scores': 'Risk Scores',
  'utilization': 'Utilization & Admission',
  'medications-risk': 'Medication & Clinical Risk',
  'social-mitigation': 'Social Support & Mitigation',
};

const FIELD_LABELS = {
  date: 'Date',
  laceIndexScore: 'LACE Index Score',
  hospitalScore: 'HOSPITAL Score',
  readmissionProbability: 'Readmission Probability (%)',
  riskStratificationLevel: 'Risk Stratification Level',
  charlsonComorbidityIndex: 'Charlson Comorbidity Index',
  functionalStatusScore: 'Functional Status Score',
  indexAdmissionDiagnosis: 'Index Admission Diagnosis',
  lengthOfStayDays: 'Length of Stay (days)',
  priorAdmissionsCount: 'Prior Admissions Count',
  priorEmergencyVisitsCount: 'Prior Emergency Visits Count',
  dischargeDisposition: 'Discharge Disposition',
  heartFailureExacerbation: 'Heart Failure Exacerbation',
  activeCancerDiagnosis: 'Active Cancer Diagnosis',
  chronicKidneyDiseaseStage: 'Chronic Kidney Disease Stage',
  medicationCount: 'Medication Count',
  highRiskMedications: 'High-Risk Medications',
  polypharmacyIndicator: 'Polypharmacy',
  cognitiveImpairmentPresent: 'Cognitive Impairment Present',
  nutritionalRiskIndicator: 'Nutritional Risk Indicator',
  pendingTestResultsAtDischarge: 'Pending Test Results at Discharge',
  adequateSocialSupport: 'Adequate Social Support',
  healthLiteracyScore: 'Health Literacy Score',
  followUpAppointmentScheduled: 'Follow-Up Appointment Scheduled',
  mitigationInterventions: 'Mitigation Interventions',
  assessmentPerformedBy: 'Assessment Performed By',
};

const SECTION_FIELDS = {
  'risk-scores': ['date', 'laceIndexScore', 'hospitalScore', 'readmissionProbability', 'riskStratificationLevel', 'charlsonComorbidityIndex', 'functionalStatusScore'],
  'utilization': ['indexAdmissionDiagnosis', 'lengthOfStayDays', 'priorAdmissionsCount', 'priorEmergencyVisitsCount', 'dischargeDisposition', 'heartFailureExacerbation', 'activeCancerDiagnosis', 'chronicKidneyDiseaseStage'],
  'medications-risk': ['medicationCount', 'highRiskMedications', 'polypharmacyIndicator', 'cognitiveImpairmentPresent', 'nutritionalRiskIndicator', 'pendingTestResultsAtDischarge'],
  'social-mitigation': ['adequateSocialSupport', 'healthLiteracyScore', 'followUpAppointmentScheduled', 'mitigationInterventions', 'assessmentPerformedBy'],
};
...
const STRING_FIELDS = ['riskStratificationLevel', 'indexAdmissionDiagnosis', 'dischargeDisposition', 'healthLiteracyScore', 'nutritionalRiskIndicator', 'chronicKidneyDiseaseStage', 'assessmentPerformedBy'];
const ENUM_FIELDS = {
  riskStratificationLevel: ['Low', 'Moderate', 'High'],
  dischargeDisposition: ['Home', 'Home with Home Health', 'Skilled Nursing Facility', 'Inpatient Rehabilitation', 'Long-Term Acute Care', 'Hospice', 'Other'],
  healthLiteracyScore: ['Adequate', 'Marginal', 'Inadequate'],
  nutritionalRiskIndicator: ['Low Risk', 'At Risk', 'High Risk'],
  chronicKidneyDiseaseStage: ['1', '2', '3a', '3b', '4', '5', 'End Stage'],
};
const COPY_LINE_EQ = '='.repeat(40);
const COPY_LINE_DASH = '-'.repeat(40);
/* MEANINGFUL_ZERO_FIELDS: numerics where 0 is a valid clinical finding (low-risk / "none"), NOT a "not recorded" sentinel.
   LACE/HOSPITAL/Charlson/probability score 0 = lowest risk; prior admissions/ED visits 0 = none; functional status 0 = total dependence.
   Excludes lengthOfStayDays & medicationCount where 0 reads as missing data. */
...
const DRAFT_KEY = 'readmission_risk_assessmentPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const ReadmissionRiskAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.readmission_risk_assessment) return Array.isArray(r.readmission_risk_assessment) ? r.readmission_risk_assessment : [r.readmission_risk_assessment];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.readmission_risk_assessment) return Array.isArray(dd.readmission_risk_assessment) ? dd.readmission_risk_assessment : [dd.readmission_risk_assessment]; return [dd]; }
      return [r];
    });
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Draft store keys are "field" (scalar) or "field.itemIdx" (array element). For array elements we
     reassemble the full array into localEdits[`field-idx`] (the render shape this template uses). */
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayElem) {
          ...
          const itemIdx = parseInt(fieldPart.slice(lastDot + 1), 10);
          const baseKey = `${fn}-${idx}`;
          const existing = Array.isArray(nLocal[baseKey]) ? nLocal[baseKey] : [...(Array.isArray(record[fn]) ? record[fn] : [])];
          existing[itemIdx] = value;
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Readmission Risk Assessment ${idx + 1}`.toLowerCase();
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    // Stage the draft (scalar → keyed by field name)
    const store = readDrafts();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Commit each staged draft for this record's fields-in-this-section.
      // Draft keys are "field" (scalar → localEdits[`field-idx`]) or "field.itemIdx" (array element).
      const store = readDrafts();
      const recDrafts = store[id] || {};
      const committedLocalKeys = []; // localEdits keys (`field-idx`) cleared from pending after commit
      const committedDraftKeys = []; // draft-store keys removed after commit
      for (const fieldPart of Object.keys(recDrafts)) {
        const lastDot = fieldPart.lastIndexOf('.');
        const isArrayElem = lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1));
        const baseField = isArrayElem ? fieldPart.slice(0, lastDot) : fieldPart;
        if (!fields.includes(baseField)) continue; // not part of the section being approved
        ...
        const resp = await secureApiClient.put(`/api/edit/readmission_risk_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedDraftKeys.push(fieldPart);
        committedLocalKeys.push(`${baseField}-${idx}`);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/readmission_risk_assessment/${id}/approve`, { sectionId: sid, approved: true });
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        rows = items.flatMap(item => {
          ...
          if (!parsed.isLabeled) return [`${number++}. ${item}`];
          return [`${parsed.label}:`, `  ${number++}. ${parsed.value}`];
        });
        ...
        if (sentences.length > 1 || hasStructuredSingle) rows = formatSentenceFieldLines(strVal);
        else rows = [strVal];
      } else {
        rows = [fmtVal(val)];
      }
      ...
      rows.forEach((row, rowIndex) => {
        if (/^\s*\d+\.\s/.test(row) || /:$/.test(row)) text += `${row}\n`;
        else text += `${rowIndex + 1}. ${row}\n`;
      });
      text += '\n';
    });
  ...
  const copyAllText = useCallback(async () => {
    let text = `Readmission Risk Assessment\n${COPY_LINE_EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Readmission Risk Assessment ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  const stepFor = (fn) => fn === 'readmissionProbability' ? 1 : 1;
  const adjustedNumber = (raw, delta, step) => {
    const base = Number(raw);
    return String((Number.isFinite(base) ? base : 0) + (delta * step));
  };
    ...
    return (
      <div key={fn} className="rec-mini-card nested-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          return (
            <div key={itemIdx} data-edit-field={fn} className={parsed.isLabeled ? 'rec-mini-card nested-mini-card' : ''} style={parsed.isLabeled ? { marginTop: 4 } : undefined}>
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    if (ENUM_FIELDS[fn]) {
      ...
      const isEditing = editingField === editKey;
      const isModified = editedFields[editKey];
      const options = ENUM_FIELDS[fn].includes(strVal) ? ENUM_FIELDS[fn] : [strVal, ...ENUM_FIELDS[fn]];
      return (
        <div key={fn} className="rec-mini-card nested-mini-card" data-edit-field={fn}>
          <div className="nested-subtitle">{highlightText(label)}</div>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="readmission-risk-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Readmission Risk Assessment</h2></div>
        <div className="empty-state">No readmission risk assessment data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="readmission-risk-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Readmission Risk Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ReadmissionRiskAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Readmission_Risk_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search readmission risk assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Readmission Risk Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'risk-scores')}
            {renderSection(record, idx, 'utilization')}
            {renderSection(record, idx, 'medications-risk')}
            {renderSection(record, idx, 'social-mitigation')}
          </div>
        ))}
      </div>
    </div>
...
export default ReadmissionRiskAssessmentDocument;
