/**
 * IntraoperativeCholangiographyDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: intraoperative_cholangiography
 *
 * 6 Sections:
 *   1. procedure-info: performanceDate (date), indication (string), technique (string)
 *   2. contrast-details: contrastType (string), contrastVolume (string)
 *   3. bile-duct-visualization: bileDuctVisualized (string), commonBileDuct (string),
 *                               hepaticDucts (string), cysticDuct (string)
 *   4. findings: duodenalFilling (string), choledocholithiasis (string),
 *                ductalDilation (string), filllingDefects (string)
 *   5. variants-complications: anatomicalVariants (string), complications (array/empty)
 *   6. assessment: interpretation (string), recommendedActions (string), imageQuality (string)
 *   7. recommendations-section: recommendations (object array {recommendation, date})
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import IntraoperativeCholangiographyDocumentPDFTemplate from '../pdf-templates/IntraoperativeCholangiographyDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './IntraoperativeCholangiographyDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = localEdits field key, e.g. "indication") */
const DRAFT_KEY = 'intraoperative_cholangiographyPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = [
  'indication', 'technique', 'contrastType', 'contrastVolume',
  'commonBileDuct', 'hepaticDucts', 'cysticDuct',
  'ductalDilation', 'filllingDefects',
  'anatomicalVariants', 'interpretation', 'recommendedActions', 'imageQuality',
];
const DATE_FIELDS = ['performanceDate'];
const ARRAY_FIELDS = ['complications'];
const OBJECT_ARRAY_FIELDS = ['recommendations'];
/* Yes/No enum fields → BlueSelect (values stored lowercase 'yes'/'no', displayed canonical) */
...
/* isEmptyDeep: recursively detect empty values (for object-array filtering) */
const isEmptyDeep = (v) => {
...
/* recsToText: flatten recommendations object-array for search */
...
/* ═══════ COMPONENT ═══════ */
const IntraoperativeCholangiographyDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.intraoperative_cholangiography) return Array.isArray(r.intraoperative_cholangiography) ? r.intraoperative_cholangiography : [r.intraoperative_cholangiography];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.intraoperative_cholangiography) return Array.isArray(dd.intraoperative_cholangiography) ? dd.intraoperative_cholangiography : [dd.intraoperative_cholangiography]; return [dd]; }
      return [r];
    });
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (OBJECT_ARRAY_FIELDS.includes(f)) {
          if (recsToText(val).toLowerCase().includes(phrase)) return true;
        } else if (Array.isArray(val)) {
          if (val.some(item => fmtVal(item).toLowerCase().includes(phrase))) return true;
        } else if (DATE_FIELDS.includes(f)) {
          if (formatDate(val).toLowerCase().includes(phrase)) return true;
        } else {
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) {
      if (OBJECT_ARRAY_FIELDS.includes(fn)) {
        return recsToText(val).toLowerCase().includes(phrase);
      }
      if (Array.isArray(val)) {
        return val.some(item => fmtVal(item).toLowerCase().includes(phrase));
      }
      if (DATE_FIELDS.includes(fn)) {
        return formatDate(val).toLowerCase().includes(phrase);
      }
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Intraoperative Cholangiography ${idx + 1}`.toLowerCase();
  ...
  /* saveRecItem: edit one recommendation's text within the object-array */
  ...
  /* stageSentenceDraft: stage full-text as a DRAFT locally + persist to localStorage (no DB write). */
  const stageSentenceDraft = (record, fn, idx, sid, fullText, markMutator) => {
    const id = safeId(record); if (!id) return;
    const editKey = `${fn}-${idx}`;
    setLocalEdits(prev => ({ ...prev, [editKey]: fullText }));
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
    setEditedSentences(prev => markMutator({ ...prev }));
    if (sid) setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
    const store = readDrafts();
    if (!store[id]) store[id] = {};
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    // localEdits keys are `${field}-${idx}`; collect the pending ones belonging to this section.
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const field = m[1];
        const resp = await secureApiClient.put(`/api/edit/intraoperative_cholangiography/${id}/edit`, { field, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/intraoperative_cholangiography/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[IntraoperativeCholangiography] Approve error:', err); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        const arr = (Array.isArray(val) ? val : []).filter(r => !isEmptyDeep(r));
        ...
        arr.forEach((rec) => {
          ...
          if (!rt) return;
          if (rd !== lastDate) { if (rd) body += `${rd}\n`; lastDate = rd; n = 1; }
          body += `${n++}. ${rt}\n`;
        });
        body += '\n';
      } else if (ARRAY_FIELDS.includes(f)) {
        const arr = Array.isArray(val) ? val.filter(x => !isEmptyDeep(x)) : (hasVal(val) ? [val] : []);
        if (arr.length === 0) return;
        body += head;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== INTRAOPERATIVE CHOLANGIOGRAPHY ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Intraoperative Cholangiography ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayVal); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const newArr = [...items]; newArr[aIdx] = editValue; const editKeyA = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [editKeyA]: newArr })); setPendingEdits(prev => ({ ...prev, [editKeyA]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const storeA = readDrafts(); if (!storeA[id]) storeA[id] = {}; storeA[id][fn] = newArr; writeDrafts(storeA); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  /* ═══════ RENDER: ENUM FIELD (BlueSelect, e.g. Yes/No) ═══════ */
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const idL = safeId(record); if (!idL) return; const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); const editKeyL = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [editKeyL]: fullText })); setPendingEdits(prev => ({ ...prev, [editKeyL]: true })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const storeL = readDrafts(); if (!storeL[idL]) storeL[idL] = {}; storeL[idL][fn] = fullText; writeDrafts(storeL); setEditingField(null); setEditValue(''); setSaveError(null); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  /* ═══════ RENDER: RECOMMENDATIONS (object-array {recommendation, date}, date-grouped) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="intraoperative-cholangiography-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Intraoperative Cholangiography</h2></div>
        <div className="empty-state">No intraoperative cholangiography records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="intraoperative-cholangiography-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Intraoperative Cholangiography</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<IntraoperativeCholangiographyDocumentPDFTemplate document={pdfData} />} fileName="Intraoperative_Cholangiography.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search cholangiography records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              {hasVal(record.performanceDate) && (
                <div className="record-meta-row">
                  <span className="record-date">{formatDate(record.performanceDate)}</span>
                </div>
              )}
              <h3 className="record-name">{highlightText(`Intraoperative Cholangiography ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'procedure-info')}
            {renderSection(record, idx, 'contrast-details')}
            {renderSection(record, idx, 'bile-duct-visualization')}
            {renderSection(record, idx, 'findings')}
            {renderSection(record, idx, 'variants-complications')}
...
export default IntraoperativeCholangiographyDocument;
