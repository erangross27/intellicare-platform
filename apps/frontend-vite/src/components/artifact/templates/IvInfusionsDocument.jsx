/**
 * IvInfusionsDocument.jsx
 * June 2026 — Inline editing, blue glow theme
 * Collection: iv_infusions
 *
 * 5 Sections:
 *   1. infusion-details: infusionMedication, dosageAmount, infusionRate, totalVolumeInfused, infusionDuration, infusionPump, infusionProtocol
 *   2. iv-access: ivAccessSite, catheterType, catheterGauge, centralLineBundle
 *   3. safety-reactions: vesicantMedication, compatibilityCheck, premedications[], infusionReactions[], extravasationOccurrence, phlebitisScore
 *   4. monitoring-labs: baselineVitalSigns, postInfusionVitalSigns, fluidBalance, electrolyteLevels, renalFunction, hepaticFunction
 *   5. nursing-education: nursingAssessment, patientEducation
 *
 * NOTE: this schema has NO date field — record header shows ONLY the numbered record title.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import IvInfusionsDocumentPDFTemplate from '../pdf-templates/IvInfusionsDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './IvInfusionsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" — this template stores the
   full field value per edit, including reconstructed arrays/sentences; no arrayIndex suffix). */
const DRAFT_KEY = 'iv_infusionsPendingEdits';
const readDrafts = () => {
...
const SIMPLE_FIELDS = ['infusionMedication', 'infusionPump', 'ivAccessSite', 'catheterType', 'catheterGauge'];
/* NUMERIC_FIELDS: always numbers in the DB — must edit via type=number (parseFloat+isNaN)
   so a saved value never silently degrades the stored Number into a String. */
...
/* Numeric fields where 0 means "not recorded" — hidden when exactly 0.
   ONLY dosageAmount / infusionRate / totalVolumeInfused / infusionDuration:
   a dose, rate, volume or duration of exactly 0 was never recorded.
   CRITICAL: phlebitisScore and fluidBalance are deliberately NOT listed and ALWAYS
   display even when 0 — grade-0 phlebitis (no symptoms on the VIP scale) and a
   fluid balance of 0 mL (intake exactly equals output) are real clinical values.
   Real decimals (e.g. dosageAmount 0.5) still display. */
...
/* splitByComma: parenthesis-aware comma split with date-aware guard
   — items like "Diphenhydramine 25 mg IV (pre-dose, slow push)" stay intact */
...
/* ═══════ COMPONENT ═══════ */
const IvInfusionsDocument = ({ document: docProp, data, templateData }) => {
  ...
  /* ═══════ REHYDRATE PENDING DRAFTS — a Save survives refresh (shown in JSX, NOT in DB/PDF) ═══════ */
  useEffect(() => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      const docTitle = 'iv infusions';
      const rt = `iv infusions ${oi + 1}`;
  ...
  // stageDraft persists one field's draft value under the record id (fieldPart = field name; this template
  // stores the full reconstructed field value per edit, so there is never an arrayIndex suffix).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      // localEdits keys are "<fieldPart>-<idx>" where fieldPart is the field name (full value stored
      // per edit). Per the dotted-name rule, treat a dot-suffix as arrayIndex ONLY when the segment
      // after the LAST dot is purely numeric — this template never produces that, so value is sent whole.
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "field" (no arrayIndex in this template)
        ...
        await secureApiClient.put(`/api/edit/iv_infusions/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/iv_infusions/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== IV INFUSIONS ===\n\n';
    pdfData.forEach((r) => {
      const oi = r._originalIdx ?? 0;
      text += `IV Infusions ${oi + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: SIMPLE FIELD (no splitting — medications, sites, catheters, pumps, measurements) ═══════ */
  ...
  /* ═══════ RENDER: NUMERIC FIELD (type=number — saves REAL numbers via parseFloat+isNaN, never degrades stored Number to String) ═══════ */
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                ...
                <input type="text" inputMode="decimal" className="edit-number" value={editValue} autoFocus onChange={e => setEditValue(e.target.value)} onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter') { e.preventDefault(); saveNum(); } }} />
  ...
  if (!records || records.length === 0) {
    return (
      <div className="iv-infusions-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">IV Infusions</h2></div>
        <div className="empty-state">No IV infusions records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="iv-infusions-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">IV Infusions</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<IvInfusionsDocumentPDFTemplate document={pdfData} />} fileName="IV_Infusions.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search IV infusions..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          return (
            <div key={safeId(record) || idx} className="record-card">
              ...
              <div className="record-header">
                <h3 className="record-name">{highlightText(`IV Infusions ${origIdx + 1}`)}</h3>
              </div>
              {renderSection(record, origIdx, 'infusion-details')}
              {renderSection(record, origIdx, 'iv-access')}
              {renderSection(record, origIdx, 'safety-reactions')}
              {renderSection(record, origIdx, 'monitoring-labs')}
              {renderSection(record, origIdx, 'nursing-education')}
            </div>
          );
        })}
      </div>
...
export default IvInfusionsDocument;
