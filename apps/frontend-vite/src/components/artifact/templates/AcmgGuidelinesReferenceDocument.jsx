/**
 * AcmgGuidelinesReferenceDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: acmg_guidelines_reference
 *
 * 11 Sections (Provider FIRST):
 *   1. provider-info: provider
 *   2. encounter-date: date (Date, date-picker)
 *   3. guideline-topic: guidelineTopic
 *   4. guideline-version: guidelineVersion
 *   5. variant-classification: variantClassification
 *   6. evidence-level: evidenceLevel
 *   7. clinical-recommendations: clinicalRecommendations (per-sentence)
 *   8. screening-guidelines: screeningGuidelines (per-sentence)
 *   9. applicability: applicability (per-sentence)
 *  10. reference-url: referenceUrl
 *  11. additional-notes: notes (per-sentence)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import AcmgGuidelinesReferenceDocumentPDFTemplate from '../pdf-templates/AcmgGuidelinesReferenceDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './AcmgGuidelinesReferenceDocument.css';
...
const FIELD_LABELS = {
  provider: 'Provider',
  date: 'Date',
  guidelineTopic: 'Guideline Topic',
...
const SECTION_FIELDS = {
  ...
  'guideline-version': ['guidelineVersion'],
  'variant-classification': ['variantClassification'],
  'evidence-level': ['evidenceLevel'],
  'clinical-recommendations': ['clinicalRecommendations'],
...
const NUMBER_FIELDS = [];
const STRING_FIELDS = ['provider', 'guidelineTopic', 'guidelineVersion', 'variantClassification', 'evidenceLevel', 'clinicalRecommendations', 'screeningGuidelines', 'applicability', 'referenceUrl', 'notes'];
/* Per-sentence narrative fields */
const SENTENCE_FIELDS = ['clinicalRecommendations', 'screeningGuidelines', 'applicability', 'notes'];
...
const DRAFT_KEY = 'acmg_guidelines_referencePendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const AcmgGuidelinesReferenceDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `ACMG Guideline ${idx + 1}`.toLowerCase();
  ...
  // Save one sentence = stage a DRAFT locally + localStorage (survives refresh). NOT written to DB/PDF until Approve.
  ...
  // Approve = COMMIT all staged drafts for this record's section fields to MongoDB, then clear pending so
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/acmg_guidelines_reference/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/acmg_guidelines_reference/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== ACMG GUIDELINES REFERENCE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `ACMG Guideline ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const buildFullSummary = useCallback(() => {
    ...
    return text;
  }, [pdfData, buildSectionCopyText]);

  /* ═══════ MAIN RENDER ═══════ */
  if (!records || records.length === 0) {
    return (
      <div className="acmg-guidelines-reference-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">ACMG Guidelines Reference</h2></div>
        <div className="empty-state">No ACMG guidelines reference data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="acmg-guidelines-reference-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">ACMG Guidelines Reference</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<AcmgGuidelinesReferenceDocumentPDFTemplate document={pdfData} />} fileName={`acmg-guidelines-reference-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search ACMG guidelines..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`ACMG Guideline ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'provider-info')}
            {renderSection(record, idx, 'encounter-date')}
            {renderSection(record, idx, 'guideline-topic')}
            {renderSection(record, idx, 'guideline-version')}
            {renderSection(record, idx, 'variant-classification')}
            {renderSection(record, idx, 'evidence-level')}
            {renderSection(record, idx, 'clinical-recommendations')}
            {renderSection(record, idx, 'screening-guidelines')}
            {renderSection(record, idx, 'applicability')}
            {renderSection(record, idx, 'reference-url')}
...
export default AcmgGuidelinesReferenceDocument;
