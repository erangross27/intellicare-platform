/**
 * NuclearMedicineAssessmentDocument.jsx
 * Canonical one-pass polish — blue glow editing theme
 * Collection: nuclear_medicine_assessment
 *
 * Sections: Record Information · Thyroid Scan · Parathyroid SPECT · Cardiac Perfusion ·
 *           Radiation Dosimetry · SPECT Technical Parameters · Findings · Assessment ·
 *           Plan · Recommendations · Notes
 *
 * Scalar string fields (provider/findings/assessment/plan/notes) are inline-editable
 * (mirrors the canonical renderStringField). The real record `date` is edited via
 * BlueDatePicker. Deeply-nested objects (thyroidScan/parathyroidSPECT/cardiacPerfusion/
 * radiationDosimetry/spectTechnicalParameters) render via a generic recursive renderer
 * (label ABOVE value — never a side-by-side "Label: value" span) and are read-only.
 * recommendations = array of {recommendation, date} → nested-subtitle (text) + date row.
 * Empty nested objects (petScan/boneScan/ventilationPerfusion/results/additionalData) skip.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NuclearMedicineAssessmentDocumentPDFTemplate from '../pdf-templates/NuclearMedicineAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './NuclearMedicineAssessmentDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'nuclear_medicine_assessmentPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'parathyroid': 'Parathyroid SPECT',
  'cardiac-perfusion': 'Cardiac Perfusion',
  'radiation': 'Radiation Dosimetry',
  'spect-tech': 'SPECT Technical Parameters',
  'findings': 'Findings',
...
const FIELD_LABELS = {
  ...
  facility: 'Facility',
  status: 'Status',
  thyroidScan: 'Thyroid Scan',
  parathyroidSPECT: 'Parathyroid SPECT',
  cardiacPerfusion: 'Cardiac Perfusion',
  radiationDosimetry: 'Radiation Dosimetry',
  spectTechnicalParameters: 'SPECT Technical Parameters',
  findings: 'Findings',
  assessment: 'Assessment',
  plan: 'Plan',
  recommendations: 'Recommendations',
...
const SECTION_FIELDS = {
  ...
  'thyroid-scan': ['thyroidScan'],
  'parathyroid': ['parathyroidSPECT'],
  'cardiac-perfusion': ['cardiacPerfusion'],
  'radiation': ['radiationDosimetry'],
  'spect-tech': ['spectTechnicalParameters'],
  'findings': ['findings'],
  'assessment': ['assessment'],
  'plan': ['plan'],
  'recommendations': ['recommendations'],
...
const STRING_FIELDS = ['provider', 'findings', 'assessment', 'plan', 'notes'];
/* Read-only scalars */
const DISPLAY_FIELDS = ['type', 'facility', 'status'];
/* Editable record date (BlueDatePicker) */
const DATE_FIELDS = ['date'];
/* Read-only, decomposed nested objects */
const OBJECT_FIELDS = ['thyroidScan', 'parathyroidSPECT', 'cardiacPerfusion', 'radiationDosimetry', 'spectTechnicalParameters'];
/* Array of {recommendation, date} objects */
...
const KEY_OVERRIDES = {
  ...
  lvef: 'LVEF', sss: 'SSS', srs: 'SRS', sds: 'SDS',
  spect: 'SPECT', ecg: 'ECG', tid: 'TID', lv: 'LV', suvmax: 'SUV Max', vq: 'V/Q',
};
...
/* ═══════ COMPONENT ═══════ */
const NuclearMedicineAssessmentDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.nuclear_medicine_assessment) return Array.isArray(r.nuclear_medicine_assessment) ? r.nuclear_medicine_assessment : [r.nuclear_medicine_assessment];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.nuclear_medicine_assessment) return Array.isArray(dd.nuclear_medicine_assessment) ? dd.nuclear_medicine_assessment : [dd.nuclear_medicine_assessment]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Nuclear Medicine Assessment ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/nuclear_medicine_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/nuclear_medicine_assessment/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NUCLEAR MEDICINE ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Nuclear Medicine Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT FIELD (thyroidScan / parathyroidSPECT / cardiacPerfusion / … — read-only) ═══════ */
  ...
  /* ═══════ RENDER: ARRAY FIELD (read-only, decomposed) — unused (ARRAY_FIELDS empty) ═══════ */
  ...
  /* ═══════ RENDER: RECOMMENDATIONS — array of {recommendation, date} ═══════
     Each object decomposes into a nested-subtitle (recommendation text) + a formatted date row.
     The date row is editable via BlueDatePicker (date-valued rows must never be a plain textarea). */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {recs.map((rec, rIdx) => {
          ...
          return (
            <div key={rIdx} className="rec-mini-card" style={{ marginTop: 8 }}>
              <div className="nested-subtitle">{highlightText(recText)}</div>
              {displayDate && (
  ...
  /* ═══════ RENDER: NUMBER FIELD (num-stepper — never native type=number) — unused (NUMBER_FIELDS empty) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="nuclear-medicine-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Nuclear Medicine Assessment</h2></div>
        <div className="empty-state">No nuclear medicine assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="nuclear-medicine-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Nuclear Medicine Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NuclearMedicineAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Nuclear_Medicine_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search nuclear medicine assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Nuclear Medicine Assessment ${idx + 1}`)}</h3>
            </div>
...
export default NuclearMedicineAssessmentDocument;
