 ...
 * July 2026 - canonical editable template for tropical_disease_assessment.
 */
...
const SECTION_ORDER = ['travel-history', 'vector-exposure', 'fever-presentation', 'parasitology', 'serology', 'clinical-findings', 'treatment', 'complications', 'public-health'];
...
const STRING_ARRAY_FIELDS = ['travelHistoryCountries', 'vectorExposure', 'neurologicSymptoms', 'complicationsDeveloped'];
const COMMA_SPLIT_FIELDS = new Set(['exposureHistory', 'vaccinationStatus', 'suspectedPathogen', 'malariaSpecies']);
const SEMICOLON_SPLIT_FIELDS = new Set(['prophylaxisCompliance', 'feverPattern', 'parasitologyResults', 'rapidDiagnosticTest', 'serologyResults', 'rashCharacteristics', 'antimicrobialTherapy', 'treatmentResponse']);
...
/* ═══════ COMPONENT ═══════ */
const TropicalDiseaseAssessmentDocument = ({ document: docProp, data, templateData }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `tropical disease assessment ${idx + 1}`;
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      const approveResp = await secureApiClient.put(`/api/edit/tropical_disease_assessment/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[TropicalDiseaseAssessment] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
    ...
    <div className="tropical-disease-assessment-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<TropicalDiseaseAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Tropical_Disease_Assessment.pdf" className="copy-btn">
