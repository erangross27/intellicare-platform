/**
 * IntradialyticMonitoringDocument.jsx
 * March 2026 — Canonical one-pass polish (inline editing, blue glow theme)
 * Collection: intradialytic_monitoring
 *
 * This record has NO `date` field (only createdAt/updatedAt ingestion timestamps) → NO record date is
 * rendered (title only, per the Hematology gold standard).
 *
 * 7 Sections:
 *   1. weight-ultrafiltration: preDialysisWeight, postDialysisWeight, ultrafiltrationGoal, ultrafiltrationRate (num-stepper)
 *   2. treatment-params: bloodFlowRate, dialysateFlowRate, treatmentTime (num-stepper), dialyzerType (string)
 *   3. pressure-readings: transmembranePresssure, venousPressure, arterialPressure (num-stepper; arterialPressure is −200)
 *   4. dialysate-settings: dialysateSodium, dialysatePotassium, dialysateCalcium, dialysateBicarbonate (num-stepper)
 *   5. anticoag-adequacy: heparinDose, accessRecirculation, ktVUrea (num-stepper)
 *   6. vital-signs: systolicBloodPressure, diastolicBloodPressure, heartRate, oxygenSaturation
 *      (ARRAYS of time-series numbers → one compact comma-joined series line each)
 *   7. complications: cramping, hypotensiveEpisodes (BlueSelect Yes/No), fluidOverloadSymptoms (narrative)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import IntradialyticMonitoringDocumentPDFTemplate from '../pdf-templates/IntradialyticMonitoringDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './IntradialyticMonitoringDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name) */
const DRAFT_KEY = 'intradialytic_monitoringPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'weight-ultrafiltration': 'Weight & Ultrafiltration',
  'treatment-params': 'Treatment Parameters',
  'pressure-readings': 'Pressure Readings',
  'dialysate-settings': 'Dialysate Settings',
  'anticoag-adequacy': 'Anticoagulation & Adequacy',
  'vital-signs': 'Vital Signs Trends',
  'complications': 'Complications',
};

const FIELD_LABELS = {
  preDialysisWeight: 'Pre-Dialysis Weight (kg)',
  postDialysisWeight: 'Post-Dialysis Weight (kg)',
  ultrafiltrationGoal: 'Ultrafiltration Goal (mL)',
  ultrafiltrationRate: 'Ultrafiltration Rate (mL/hr)',
  ...
  transmembranePresssure: 'Transmembrane Pressure (mmHg)',
  venousPressure: 'Venous Pressure (mmHg)',
  arterialPressure: 'Arterial Pressure (mmHg)',
  dialysateSodium: 'Sodium (mEq/L)',
  dialysatePotassium: 'Potassium (mEq/L)',
  dialysateCalcium: 'Calcium (mEq/L)',
  dialysateBicarbonate: 'Bicarbonate (mEq/L)',
  ...
  cramping: 'Cramping',
  hypotensiveEpisodes: 'Hypotensive Episodes',
  fluidOverloadSymptoms: 'Fluid Overload Symptoms',
};

const SECTION_FIELDS = {
  'weight-ultrafiltration': ['preDialysisWeight', 'postDialysisWeight', 'ultrafiltrationGoal', 'ultrafiltrationRate'],
  'treatment-params': ['bloodFlowRate', 'dialysateFlowRate', 'treatmentTime', 'dialyzerType'],
  'pressure-readings': ['transmembranePresssure', 'venousPressure', 'arterialPressure'],
  'dialysate-settings': ['dialysateSodium', 'dialysatePotassium', 'dialysateCalcium', 'dialysateBicarbonate'],
  'anticoag-adequacy': ['heparinDose', 'accessRecirculation', 'ktVUrea'],
  'vital-signs': ['systolicBloodPressure', 'diastolicBloodPressure', 'heartRate', 'oxygenSaturation'],
  'complications': ['cramping', 'hypotensiveEpisodes', 'fluidOverloadSymptoms'],
};

/* 17 scalar number fields — ALL use the −/+ num-stepper (input type=text inputMode=decimal) */
const NUMBER_FIELDS = [
  'preDialysisWeight', 'postDialysisWeight', 'ultrafiltrationGoal', 'ultrafiltrationRate',
  'bloodFlowRate', 'dialysateFlowRate', 'treatmentTime',
  'transmembranePresssure', 'venousPressure', 'arterialPressure',
  'dialysateSodium', 'dialysatePotassium', 'dialysateCalcium', 'dialysateBicarbonate',
  'heparinDose', 'accessRecirculation', 'ktVUrea',
];

const BOOLEAN_FIELDS = ['cramping', 'hypotensiveEpisodes'];
const STRING_FIELDS = ['dialyzerType', 'fluidOverloadSymptoms'];
/* 4 time-series arrays-of-numbers — one compact comma-joined series line each (read-only) */
const ARRAY_FIELDS = ['systolicBloodPressure', 'diastolicBloodPressure', 'heartRate', 'oxygenSaturation'];
const DATE_FIELDS = [];
...
/* splitByComma: parenthesis-aware comma split (thousands-safe: needs whitespace after the comma) */
...
/* ═══════ COMPONENT ═══════ */
const IntradialyticMonitoringDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.intradialytic_monitoring) return Array.isArray(r.intradialytic_monitoring) ? r.intradialytic_monitoring : [r.intradialytic_monitoring];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.intradialytic_monitoring) return Array.isArray(dd.intradialytic_monitoring) ? dd.intradialytic_monitoring : [dd.intradialytic_monitoring]; return [dd]; }
      return [r];
    });
    return arr.filter(r => r && typeof r === 'object');
  }, [docProp]);

  /* ═══════ REHYDRATE PENDING DRAFTS (survive refresh; shown in JSX, NOT DB/PDF) ═══════ */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Intradialytic Monitoring ${idx + 1}`.toLowerCase();
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    // Stage as a DRAFT in localStorage (survives refresh). Approve commits it to the DB.
    const store = readDrafts();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // This section's staged (pending) edit keys, e.g. "bloodFlowRate-0"
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/intradialytic_monitoring/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/intradialytic_monitoring/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== INTRADIALYTIC MONITORING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Intradialytic Monitoring ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: ARRAY FIELD — read-only comma-joined time-series line ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="intradialytic-monitoring-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Intradialytic Monitoring</h2></div>
        <div className="empty-state">No intradialytic monitoring records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="intradialytic-monitoring-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Intradialytic Monitoring</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<IntradialyticMonitoringDocumentPDFTemplate document={pdfData} />} fileName="Intradialytic_Monitoring.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search intradialytic monitoring..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Intradialytic Monitoring ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'weight-ultrafiltration')}
            {renderSection(record, idx, 'treatment-params')}
            {renderSection(record, idx, 'pressure-readings')}
            {renderSection(record, idx, 'dialysate-settings')}
            {renderSection(record, idx, 'anticoag-adequacy')}
...
export default IntradialyticMonitoringDocument;
