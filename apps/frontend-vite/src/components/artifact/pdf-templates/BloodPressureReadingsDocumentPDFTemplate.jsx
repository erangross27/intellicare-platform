...
const COLLECTION = 'blood_pressure_readings';
const COMMA_SPLIT_FIELDS = [];
const ARRAY_FIELDS = new Set(['antihypertensiveMedications']);
const OBJECT_FIELDS = new Set(['additionalData']);
const NARRATIVE_FIELDS = new Set([]);
const DATE_FIELDS = new Set([]);
const ZERO_SENTINEL_FIELDS = new Set(['meanArterialPressure', 'pulseWidth', 'heartRate', 'orthostaticSystolic', 'orthostaticDiastolic', 'ankleBrachialIndex', 'toeBrachialIndex', 'centralAorticPressure', 'augmentationIndex', 'pulseWaveVelocity', 'cardiovascularRiskScore']);
const KEY_OVERRIDES = {};
const LABELS = {
  systolicPressure: 'Systolic Pressure', diastolicPressure: 'Diastolic Pressure',
  meanArterialPressure: 'Mean Arterial Pressure', pulseWidth: 'Pulse Width', heartRate: 'Heart Rate',
  cuffSize: 'Cuff Size', measurementPosition: 'Measurement Position', measurementArm: 'Measurement Arm',
  ahaStageClassification: 'AHA Stage Classification', hypertensiveCrisis: 'Hypertensive Crisis',
  orthostaticMeasurement: 'Orthostatic Measurement', orthostaticSystolic: 'Orthostatic Systolic',
  orthostaticDiastolic: 'Orthostatic Diastolic', ankleBrachialIndex: 'Ankle-Brachial Index',
  toeBrachialIndex: 'Toe-Brachial Index', centralAorticPressure: 'Central Aortic Pressure',
  augmentationIndex: 'Augmentation Index', pulseWaveVelocity: 'Pulse Wave Velocity',
  ambulatoryMonitoring: 'Ambulatory Monitoring', homeMonitoring: 'Home Monitoring',
  whiteCoatEffect: 'White Coat Effect', maskedHypertension: 'Masked Hypertension',
  antihypertensiveMedications: 'Antihypertensive Medications', cardiovascularRiskScore: 'Cardiovascular Risk Score',
  additionalData: 'Additional Data',
};
const SECTIONS = [
  { title: 'Blood Pressure Reading', fields: ['systolicPressure', 'diastolicPressure', 'meanArterialPressure', 'pulseWidth', 'heartRate'] },
  { title: 'Measurement Technique', fields: ['cuffSize', 'measurementPosition', 'measurementArm'] },
  { title: 'Classification', fields: ['ahaStageClassification', 'hypertensiveCrisis'] },
  { title: 'Orthostatic Measurements', fields: ['orthostaticMeasurement', 'orthostaticSystolic', 'orthostaticDiastolic'] },
  { title: 'Vascular Indices', fields: ['ankleBrachialIndex', 'toeBrachialIndex', 'centralAorticPressure', 'augmentationIndex', 'pulseWaveVelocity'] },
  { title: 'Monitoring Context', fields: ['ambulatoryMonitoring', 'homeMonitoring', 'whiteCoatEffect', 'maskedHypertension'] },
  { title: 'Antihypertensive Medications', fields: ['antihypertensiveMedications'] },
  { title: 'Cardiovascular Risk Score', fields: ['cardiovascularRiskScore'] },
...
const BloodPressureReadingsDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Blood Pressure Readings</Text><Text style={styles.noData}>No blood pressure reading data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Blood Pressure Readings</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Blood Pressure Reading {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
