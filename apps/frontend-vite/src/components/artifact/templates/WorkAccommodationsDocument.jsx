/**
 * WorkAccommodationsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: work_accommodations
 *
 * 6 Sections:
 *   1. session-info: provider, facility, needed, date
 *   2. stressors: currentStressors
 *   3. accommodations: recommendedAccommodations
 *   4. clinical: findings, assessment, plan, leaveStatus
 *   5. results-section: recommendations (array of {recommendation, date}), results (OBJECT)
 *   6. notes-section: notes, status
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import WorkAccommodationsDocumentPDFTemplate from '../pdf-templates/WorkAccommodationsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './WorkAccommodationsDocument.css';
...
const DRAFT_KEY = 'work_accommodationsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'results-section': 'Results & Recommendations',
  'notes-section': 'Notes',
};
...
const SECTION_FIELDS = {
  'session-info': ['provider', 'facility', 'needed', 'date'],
  'stressors': ['currentStressors'],
  'accommodations': ['recommendedAccommodations'],
  'clinical': ['findings', 'assessment', 'plan', 'leaveStatus'],
  'results-section': ['recommendations', 'results'],
  'notes-section': ['notes', 'status'],
};

const BOOLEAN_FIELDS = ['needed'];
const DATE_FIELDS = ['date'];
const ARRAY_FIELDS = ['currentStressors', 'recommendedAccommodations'];
/* Array-of-objects {recommendation, date} (date-grouped, editable) */
const OBJECT_ARRAY_FIELDS = ['recommendations'];
/* DISPLAY/EDIT recursive object field */
const OBJECT_FIELDS = ['results'];
const STRING_FIELDS = ['provider', 'facility', 'findings', 'assessment', 'plan', 'leaveStatus', 'notes', 'status'];

/* humanizeKey for OBJECT field rendering */
...
/* ═══════ COMPONENT ═══════ */
const WorkAccommodationsDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Work Accommodation ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Persist a staged DRAFT (full field value) to the pending-drafts localStorage store (survives refresh)
  // + clear this section's approve flag so re-edit returns to yellow Pending Approve. No DB write.
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    // Build the full updated object value (clone + set the leaf) and stage it as a DRAFT (no DB write).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // localEdits keys are always "<field>-<idx>" holding the FULL field value; commit each pending one.
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/work_accommodations/${id}/edit`, { field: fn, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/work_accommodations/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[WorkAccommodations] Approve error:', err); } finally { setSaving(false); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_ARRAY_FIELDS.includes(f)) {
        const recsArr = Array.isArray(val) ? val : [];
        text += `${label}\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== WORK ACCOMMODATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Work Accommodation ${idx + 1}\n${'='.repeat(40)}\n\n`;
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullText })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); stageDraft(record, fn, idx, sid, fullText); setEditingField(null); setEditValue(''); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="work-accommodations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Work Accommodations</h2></div>
        <div className="empty-state">No work accommodation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="work-accommodations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Work Accommodations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<WorkAccommodationsDocumentPDFTemplate document={pdfData} />} fileName={`work-accommodations-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search stressors, accommodations, FMLA, provider..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Work Accommodation ${idx + 1}`)}</h3>
            </div>
...
export default WorkAccommodationsDocument;
