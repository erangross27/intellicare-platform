/**
 * BehavioralAssessmentDocument.jsx
 * June 2026 — Collection: behavioral_assessment — FULL TEMPLATE STANDARD (100% schema coverage).
 *
 * Donors cloned:
 *  - PointOfCareUltrasoundHeartRateDocument: 4-level search, per-section approve, Copy row/Section/All,
 *    pdfData memo, DATE field (date-picker), per-sentence renderStringField, recursive OBJECT renderer
 *    (renderObjectField / renderObjectNode / renderObjectLeaf, humanizeKey + nested-mini-card),
 *    array-of-{recommendation,date} renderRecommendationsField.
 *  - PastMedicalHistoryDocument: per-sentence editing semantics (splitBySentence + reconstructFullText).
 *
 * FIELDS (21 → 100% schema coverage):
 *   DATE:   date (record-header date-picker)
 *   STRING (simple): temperament, attentionSpan, activityLevel, panicAttackFrequency, weightChange,
 *                    provider, facility, status
 *   STRING (per-sentence narrative): socialSkills, emotionalRegulation, findings, assessment, plan, notes
 *   ARRAY (string list): anxietySymptoms, adhdSymptoms, autismRedFlags
 *   ARRAY (recommendation list / {recommendation,date} or plain string): recommendations
 *   OBJECT (recursive): tantrums (frequency + triggers[]), results
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import SearchBar from '../components/SearchBar';
import BehavioralAssessmentPDFTemplate from '../pdf-templates/BehavioralAssessmentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './BehavioralAssessmentDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'record-info': 'Record Information',
  'behavioral-profile': 'Behavioral Profile',
  'tantrums-section': 'Tantrums',
  'symptoms': 'Symptoms',
  'clinical': 'Clinical',
...
const FIELD_LABELS = {
  ...
  tantrums: 'Tantrums',
  anxietySymptoms: 'Anxiety Symptoms',
  panicAttackFrequency: 'Panic Attack Frequency',
  weightChange: 'Weight Change',
  adhdSymptoms: 'ADHD Symptoms',
  autismRedFlags: 'Autism Red Flags',
  findings: 'Findings',
...
const SECTION_FIELDS = {
  ...
  'behavioral-profile': ['temperament', 'attentionSpan', 'activityLevel', 'socialSkills', 'emotionalRegulation'],
  'tantrums-section': ['tantrums'],
  'symptoms': ['anxietySymptoms', 'panicAttackFrequency', 'weightChange', 'adhdSymptoms', 'autismRedFlags'],
...
const SECTION_ORDER = ['record-info', 'behavioral-profile', 'tantrums-section', 'symptoms', 'clinical', 'results-section', 'recommendations-section', 'notes-status'];

const DATE_FIELDS = ['date'];
/* simple single-line strings (no per-sentence splitting) */
const SIMPLE_STRING_FIELDS = ['provider', 'facility', 'status', 'temperament', 'attentionSpan', 'activityLevel', 'panicAttackFrequency', 'weightChange'];
/* per-sentence narrative strings */
const SENTENCE_FIELDS = ['socialSkills', 'emotionalRegulation', 'findings', 'assessment', 'plan', 'notes'];
/* plain string arrays */
const STRING_ARRAY_FIELDS = ['anxietySymptoms', 'adhdSymptoms', 'autismRedFlags'];
/* array of {recommendation, date} OR plain strings */
const OBJECT_ARRAY_FIELDS = ['recommendations'];
/* recursive object fields */
const OBJECT_FIELDS = ['tantrums', 'results'];

const KEY_OVERRIDES = { adhd: 'ADHD', asd: 'ASD', iq: 'IQ', cbcl: 'CBCL', gad: 'GAD' };
...
const DRAFT_KEY = 'behavioralAssessmentPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const BehavioralAssessmentDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.behavioral_assessment) return Array.isArray(r.behavioral_assessment) ? r.behavioral_assessment : [r.behavioral_assessment];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.behavioral_assessment) return Array.isArray(dd.behavioral_assessment) ? dd.behavioral_assessment : [dd.behavioral_assessment]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        // Per-row badge for sentence/array/recommendation/object fields (first item) so it shows as modified.
        if (SENTENCE_FIELDS.includes(fieldPart)) nSentences[`${fieldPart}-${idx}-s0`] = 'edited';
        else if (STRING_ARRAY_FIELDS.includes(fieldPart)) nSentences[`${fieldPart}-${idx}-a0`] = 'edited';
        else if (OBJECT_ARRAY_FIELDS.includes(fieldPart)) nSentences[`${fieldPart}-${idx}-r0`] = 'edited';
      });
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `behavioral assessment ${idx + 1}`.toLowerCase();
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. tantrums.frequency) — value stays a STRING */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    const editKey = `${rootField}-${idx}`;
    // Build the merged root object (apply the leaf at its dot-path) from the latest known value.
    ...
    setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
    // Stage the merged root object as a DRAFT (survives refresh; committed on Approve).
    const store = readDrafts();
    if (!store[id]) store[id] = {};
  ...
  /* save a string-array item by index */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Commit each staged field in this section (editKey = "<field>-<idx>"; value is the full field value).
      ...
      for (const editKey of toCommit) {
        ...
        const fieldPart = m[1]; // "field" — values here are never dotted (whole-field objects/arrays/strings)
        ...
        const resp = await secureApiClient.put(`/api/edit/behavioral_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/behavioral_assessment/${id}/approve`, { sectionId: sid, approved: true });
      ...
      console.error('[BehavioralAssessment] Approve error:', err);
  ...
  const objectCopyLines = useCallback((label, value, indent) => {
    ...
    if (Array.isArray(value)) {
      ...
      value.filter(v => !isEmptyDeep(v)).forEach(v => { isScalar(v) ? out.push(`${pad}  - ${fmtScalar(v)}`) : out.push(...objectCopyLines('', v, indent + 1)); });
      return out;
    }
    if (label) out.push(`${pad}${label}:`);
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        recs.forEach((r) => {
          const rec = typeof r === 'string' ? r.trim() : (r?.recommendation || '').trim();
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== BEHAVIORAL ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Behavioral Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: SIMPLE STRING FIELD (single-line, no per-sentence) ═══════ */
  ...
  /* ═══════ RENDER: STRING ARRAY FIELD (anxietySymptoms, adhdSymptoms, autismRedFlags) ═══════ */
          ...
          return (
            <div key={aIdx}>
              <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(itemText); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter' && e.ctrlKey) { e.preventDefault(); saveArrayItem(record, fn, idx, sid, aIdx, itemKey, editValue); } }} />
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
  ...
  /* ═══════ RENDER: OBJECT (recursive; humanizeKey + nested-mini-card; editable leaves; arrays as leaf list) ═══════ */
  const renderObjectNode = (record, rootField, idx, sid, label, value, path, depth) => {
    if (isEmptyDeep(value)) return null;
    const labelClass = depth > 0 ? 'nested-subtitle sub-label' : 'nested-subtitle';
    if (isScalar(value)) return renderObjectLeaf(record, rootField, path, idx, sid, value);
    if (Array.isArray(value)) {
      const arrItems = value.filter(v => !isEmptyDeep(v));
  ...
  /* ═══════ RENDER: RECOMMENDATIONS — array of {recommendation, date} or plain strings ═══════ */
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              setLocalEdits(prev => ({ ...prev, [editKey2]: newArr }));
  ...
  if (!records || records.length === 0) {
    return (
      <article className="behavioral-assessment-document" ref={containerRef}>
        <header className="document-header"><h1 className="document-title">Behavioral Assessment</h1></header>
        <div className="empty-state">No behavioral assessment records available.</div>
      </article>
    );
  }
  ...
  return (
    <article className="behavioral-assessment-document" ref={containerRef}>
      <header className="document-header">
        <h1 className="document-title">Behavioral Assessment</h1>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<BehavioralAssessmentPDFTemplate document={pdfData} />} fileName="Behavioral_Assessment.pdf" className="copy-btn">
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <div className="record-title-row"><h3 className="record-name">{highlightText(`Behavioral Assessment ${idx + 1}`)}</h3></div>
            </div>
...
export default BehavioralAssessmentDocument;
