/**
 * ProteinuriaAssessmentDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: proteinuria_assessment
 *
 * 9 Sections:
 *   1. general-info: date, provider, facility, status
 *   2. measurements: uacr, uacrCategory, upcr, twentyFourHourProtein
 *   3. protein-trend: proteinTrend (array of {date,value,unit,type})
 *   4. urinalysis: hematuria, hematuriaType, rbcCasts
 *   5. electrophoresis: urineElectrophoresis (object keys)
 *   6. findings-assessment: findings, assessment
 *   7. plan-notes: plan, notes
 *   8. results-section: results (recursive object)
 *   9. recommendations-section: recommendations (array of {recommendation, date})
 */
...
const DRAFT_KEY = 'proteinuria_assessmentPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  status: 'Status',
  uacr: 'UACR',
  uacrCategory: 'UACR Category',
  upcr: 'UPCR',
  twentyFourHourProtein: '24-Hour Protein',
  hematuria: 'Hematuria',
  hematuriaType: 'Hematuria Type',
  rbcCasts: 'RBC Casts',
  findings: 'Findings',
...
const STRING_FIELDS = ['provider', 'facility', 'status', 'uacr', 'uacrCategory', 'upcr', 'twentyFourHourProtein', 'hematuriaType', 'findings', 'assessment', 'plan', 'notes'];
...
/* ======= COMPONENT ======= */
const ProteinuriaAssessmentDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Proteinuria Assessment ${idx + 1}`.toLowerCase();
  ...
  /* ======= EDIT HANDLERS ======= */
  /* Stage a DRAFT locally + persist to localStorage. NO DB write. The full field value is stored
     under localEdits[`${fieldKey}-${idx}`] (matching the existing convention) and flagged pending so
     it shows in the JSX but stays OUT of pdfData/PDF/Copy All until Approve commits it. */
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.proteinLevel) — value stays a STRING/boolean */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    // Stage the full root-object clone (with the leaf updated) as a DRAFT. NO DB write — Approve commits.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Pending localEdits keys belonging to this section (key form: "<field>-<idx>", field has no numeric dot-suffix here)
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/proteinuria_assessment/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/proteinuria_assessment/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (f === 'proteinTrend') {
        ...
        text += '\n';
      } else if (f === 'urineElectrophoresis') {
        text += `${label}\n`;
        Object.entries(val).forEach(([key, value]) => { text += `${key}: ${value}\n`; });
        text += '\n';
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PROTEINURIA ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Proteinuria Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ======= RENDER: PROTEIN TREND (array of objects) ======= */
  const renderProteinTrend = (record, idx, sid) => {
    const val = getFieldValue(record, 'proteinTrend', idx);
    const items = Array.isArray(val) ? val.filter(Boolean) : [];
    if (items.length === 0) return null;
    if (searchTerm.trim() && !fieldMatches(record, 'proteinTrend', idx) && !sectionTitleMatches(sid)) return null;
    ...
    return (
      <div key="proteinTrend" className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          const isEditing = editingField === editKey;
          const isModified = editedFields[editKey];
          const displayText = `${formatDate(item.date)}: ${item.value}${item.unit ? ` ${item.unit}` : ''}${item.type ? ` (${item.type})` : ''}`;

          if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
            const phrase = searchTerm.toLowerCase().trim();
            if (!displayText.toLowerCase().includes(phrase) && !'protein trend'.includes(phrase)) return null;
          }

          return (
            <div key={itemIdx}>
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(`${item.value}${item.unit ? ` ${item.unit}` : ''}`); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const updatedItem = { ...item, value: editValue.trim() }; const currentArr = [...(Array.isArray(getFieldValue(record, 'proteinTrend', idx)) ? getFieldValue(record, 'proteinTrend', idx) : [])]; currentArr[itemIdx] = updatedItem; stageDraft(record, idx, sid, 'proteinTrend', currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  /* ======= RENDER: URINE ELECTROPHORESIS (object) ======= */
  const renderUrineElectrophoresis = (record, idx, sid) => {
    const val = getFieldValue(record, 'urineElectrophoresis', idx);
    if (!val || typeof val !== 'object' || Object.keys(val).length === 0) return null;
    if (searchTerm.trim() && !fieldMatches(record, 'urineElectrophoresis', idx) && !sectionTitleMatches(sid)) return null;

    return (
      <div key="urineElectrophoresis" className="rec-mini-card">
        <div className="nested-subtitle">{highlightText('Urine Electrophoresis')}</div>
        {Object.entries(val).map(([key, value], entryIdx) => {
          const editKey = `urineElectrophoresis.${key}-${idx}`;
          const isEditing = editingField === editKey;
          const isModified = editedFields[editKey];
          ...
          if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
            const phrase = searchTerm.toLowerCase().trim();
            if (!key.toLowerCase().includes(phrase) && !displayStr.toLowerCase().includes(phrase) && !'urine electrophoresis'.includes(phrase)) return null;
          }
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentObj = { ...(getFieldValue(record, 'urineElectrophoresis', idx) || {}) }; currentObj[key] = editValue; stageDraft(record, idx, sid, 'urineElectrophoresis', currentObj); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <>
                    <div className="row-content">
                      <div style={{ flex: 1 }}>
                        <span style={{ fontWeight: 700, color: '#93c5fd', textTransform: 'uppercase', fontSize: '14px' }}>{highlightText(key)}</span>
                        <div className="content-value">{highlightText(displayStr)}</div>
                      </div>
                      <span className="edit-indicator">&#9998;</span>
                    </div>
                    <button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${key}: ${displayStr}`, editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button>
                  </>
                )}
              </div>
              {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
  ...
  /* ======= RENDER: OBJECT LEAF (editable; number+unit -> number input, text -> textarea, "4/5" stays text, boolean -> Yes/No select) ======= */
    ...
    return (
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            if (f === 'proteinTrend') return renderProteinTrend(record, idx, sid);
            if (f === 'urineElectrophoresis') return renderUrineElectrophoresis(record, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="proteinuria-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Proteinuria Assessment</h2></div>
        <div className="empty-state">No proteinuria assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="proteinuria-assessment-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search proteinuria assessment records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'protein-trend')}
            {renderSection(record, idx, 'urinalysis')}
            {renderSection(record, idx, 'electrophoresis')}
...
export default ProteinuriaAssessmentDocument;
