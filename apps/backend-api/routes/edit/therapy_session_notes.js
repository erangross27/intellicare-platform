/**
 * TherapySessionNotes Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/therapy_session_notes/:id/edit    — Edit a single field
 * PUT /api/edit/therapy_session_notes/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'therapy-session-notes-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('therapy_session_notes', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[TherapySessionNotes] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('therapy_session_notes', { _id: objectId }, {
      $set: {
    ...
    console.error('[TherapySessionNotes] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
