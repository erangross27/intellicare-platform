...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'vital-signs-logs-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['provider', 'facility', 'date', 'systolicBloodPressure', 'diastolicBloodPressure', 'heartRate', 'respiratoryRate', 'temperatureCelsius', 'temperatureFahrenheit', 'temperatureSite', 'oxygenSaturation', 'supplementalOxygenFlow', 'oxygenDeliveryMethod', 'painScore', 'bloodGlucose', 'patientPosition', 'bloodPressureCuffSize', 'meanArterialPressure', 'pulseCharacter', 'respiratoryEffort', 'measurementMethod', 'bodyMassIndex', 'weightKilograms', 'heightCentimeters'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('vital_signs_logs', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[vital_signs_logs] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('vital_signs_logs', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[vital_signs_logs] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
