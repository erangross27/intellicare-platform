...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'prognosis-discussion-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['date', 'provider', 'facility', 'status', 'patientUnderstandingButAnxious', 'patientTearful', 'familySupport', 'brotherVerySupportive', 'providedEmotionalSupportAndResources', 'emphasizedProgressionCanBeSlowed', 'findings', 'assessment', 'plan', 'recommendations', 'results', 'financialConcernsSignificant', 'willNeedCloseMonitoring', 'notes'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('prognosis_discussion', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[prognosis_discussion] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('prognosis_discussion', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[prognosis_discussion] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
