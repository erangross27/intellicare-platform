...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'speech-therapy-assessments-edit-service',
    userId: req.user?.id,
    operation,
...
// Object sections (communicationAssessment, swallowingAssessment, cognitiveLanguage,
// voiceAssessment, treatmentPlan) have DYNAMIC keys that vary per record, so the
// object ROOTS are allow-listed and the route accepts any `root.<key>` dotted path
// via the rootField check below.
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('speech_therapy_assessments', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[speech_therapy_assessments] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('speech_therapy_assessments', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[speech_therapy_assessments] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
