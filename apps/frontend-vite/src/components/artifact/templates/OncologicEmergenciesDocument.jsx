/**
 * OncologicEmergenciesDocument.jsx
 * Canonical one-pass polish — inline editing, blue glow theme.
 * Collection: oncologic_emergencies
 *
 * 8 Sections:
 *   1. visit-info: date, provider, facility, status
 *   2. neutropenic-fever: neutropenicFever.definition, neutropenicFever.instructions
 *   3. tumor-lysis: tumorLysisSyndrome.warningSigns, tumorLysisSyndrome.action
 *   4. hypercalcemia: hypercalcemia.warningSigns, hypercalcemia.action
 *   5. spinal-cord: spinalCordCompression.warningSigns, spinalCordCompression.action
 *   6. svc-syndrome: superiorVenaCavaSyndrome.warningSigns, superiorVenaCavaSyndrome.action
 *   7. brain-metastases: brainMetastases.warningSigns, brainMetastases.action
 *   8. clinical-summary: findings, assessment, plan, notes
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import OncologicEmergenciesDocumentPDFTemplate from '../pdf-templates/OncologicEmergenciesDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './OncologicEmergenciesDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the dotted field, e.g.
   "neutropenicFever.definition" or "status") */
const DRAFT_KEY = 'oncologic_emergenciesPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = [
  ...
  'neutropenicFever.definition', 'neutropenicFever.instructions',
  'tumorLysisSyndrome.warningSigns', 'tumorLysisSyndrome.action',
  'hypercalcemia.warningSigns', 'hypercalcemia.action',
  'spinalCordCompression.warningSigns', 'spinalCordCompression.action',
  'superiorVenaCavaSyndrome.warningSigns', 'superiorVenaCavaSyndrome.action',
  'brainMetastases.warningSigns', 'brainMetastases.action',
  'findings', 'assessment', 'plan', 'notes',
];
const DATE_FIELDS = ['date'];

/* getNestedValue: resolve dotted-path fields like 'neutropenicFever.definition' */
...
/* ═══════ COMPONENT ═══════ */
const OncologicEmergenciesDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.oncologic_emergencies) return Array.isArray(r.oncologic_emergencies) ? r.oncologic_emergencies : [r.oncologic_emergencies];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.oncologic_emergencies) return Array.isArray(dd.oncologic_emergencies) ? dd.oncologic_emergencies : [dd.oncologic_emergencies]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Oncologic Emergencies ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/oncologic_emergencies/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/oncologic_emergencies/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== ONCOLOGIC EMERGENCIES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Oncologic Emergencies ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="oncologic-emergencies-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Oncologic Emergencies</h2></div>
        <div className="empty-state">No oncologic emergencies records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="oncologic-emergencies-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Oncologic Emergencies</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OncologicEmergenciesDocumentPDFTemplate document={pdfData} />} fileName="Oncologic_Emergencies.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search oncologic emergencies records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Oncologic Emergencies ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'visit-info')}
            {renderSection(record, idx, 'neutropenic-fever')}
            {renderSection(record, idx, 'tumor-lysis')}
            {renderSection(record, idx, 'hypercalcemia')}
            {renderSection(record, idx, 'spinal-cord')}
            {renderSection(record, idx, 'svc-syndrome')}
            {renderSection(record, idx, 'brain-metastases')}
...
export default OncologicEmergenciesDocument;
