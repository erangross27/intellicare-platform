/**
 * Chemotherapy Regimen Edit Route
 */
...
function buildContext(req, operation = 'read') { return { serviceId: 'chemotherapy-regimen-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }

const ALLOWED_FIELDS = [
  'regimenName', 'intent', 'cycleLength', 'totalCycles', 'growthFactorSupport',
  'provider', 'facility', 'status', 'date',
  'findings', 'assessment', 'plan', 'notes',
  'drugs', 'premedications', 'recommendations', 'results',
];
// Allow editing nested OBJECT sub-paths via dotted notation (e.g. results.ef, results.labs.hgb)
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('chemotherapy_regimen', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[ChemotherapyRegimen] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('chemotherapy_regimen', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[ChemotherapyRegimen] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
