/**
 * PatientPositioningDocument.jsx
 * Canonical one-pass polish — inline editing, blue glow theme.
 * Collection: patient_positioning
 *
 * Sections:
 *   1. procedure: date (title-header BlueDatePicker), procedureName
 *   2. position-info: positionType, positionDetails, headPosition, armPosition, legPosition
 *   3. safety-protection: paddingLocations, safetyStraps, pressurePointProtection,
 *                         positioningDevices, eyeProtection, positionVerification
 *   4. positioning-team: positioningTeam (array of strings)
 *   5. special-considerations: specialConsiderations
 *   6. complications: complications (array of strings)
 *   7. notes-facility: notes, facility
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PatientPositioningDocumentPDFTemplate from '../pdf-templates/PatientPositioningDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './PatientPositioningDocument.css';

/* ═══════ CONSTANTS ═══════ */
const API_BASE = '/api/edit/patient_positioning';
...
const DRAFT_KEY = 'patient_positioningPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'procedure': 'Procedure',
  'position-info': 'Position Information',
  'safety-protection': 'Safety and Protection',
  'positioning-team': 'Positioning Team',
  'special-considerations': 'Special Considerations',
  'complications': 'Complications',
  'notes-facility': 'Notes and Facility',
};
...
const FIELD_LABELS = {
  ...
  paddingLocations: 'Padding Locations',
  safetyStraps: 'Safety Straps',
  pressurePointProtection: 'Pressure Point Protection',
  positioningDevices: 'Positioning Devices',
  eyeProtection: 'Eye Protection',
  positionVerification: 'Position Verification',
  positioningTeam: 'Positioning Team',
...
const SECTION_FIELDS = {
  'procedure': ['date', 'procedureName'],
  'position-info': ['positionType', 'positionDetails', 'headPosition', 'armPosition', 'legPosition'],
  'safety-protection': ['paddingLocations', 'safetyStraps', 'pressurePointProtection', 'positioningDevices', 'eyeProtection', 'positionVerification'],
  'positioning-team': ['positioningTeam'],
  'special-considerations': ['specialConsiderations'],
  'complications': ['complications'],
  'notes-facility': ['notes', 'facility'],
};

const DATE_FIELDS = ['date'];
const ARRAY_FIELDS = ['paddingLocations', 'safetyStraps', 'positioningDevices', 'positioningTeam', 'complications'];
const STRING_FIELDS = [
  'procedureName', 'positionType', 'positionDetails', 'headPosition', 'armPosition', 'legPosition',
  'pressurePointProtection', 'eyeProtection', 'positionVerification', 'specialConsiderations', 'notes', 'facility',
];
...
/* An epoch 1970-01-01 date value is a "not set" sentinel — treat it as empty. */
const isEpochDate = (dateValue) => {
  if (!dateValue) return false;
  try { const d = new Date(dateValue.$date || dateValue); return d.getTime() === 0 || (d.getUTCFullYear() === 1970 && d.getUTCMonth() === 0 && d.getUTCDate() === 1); } catch { return false; }
};
...
/* ═══════ COMPONENT ═══════ */
const PatientPositioningDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r?.patient_positioning) return Array.isArray(r.patient_positioning) ? r.patient_positioning : [r.patient_positioning];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.patient_positioning) return Array.isArray(dd.patient_positioning) ? dd.patient_positioning : [dd.patient_positioning]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Patient Positioning ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    } catch (err) { console.error('[PatientPositioning] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        if (arr.length) {
          text += `${label}\n`;
          arr.forEach((it, i) => { text += `${i + 1}. ${fmtScalar(it)}\n`; });
          text += '\n';
        }
      } else {
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PATIENT POSITIONING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Patient Positioning ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderHeaderDate = (record, idx) => {
    ...
    if (!hasVal(val) || isEpochDate(val)) return null;
        ...
        {isEditing ? (
          <span className="edit-field-container" onClick={e => e.stopPropagation()}>
            ...
            <span className="edit-actions">
              <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date'); return; } handleSaveField(record, 'date', idx, 'procedure', null, editValue + 'T00:00:00.000Z'); }}>{saving ? 'Saving...' : 'Save'}</button>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="patient-positioning-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Patient Positioning</h2></div>
        <div className="empty-state">No patient positioning records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="patient-positioning-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Patient Positioning</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PatientPositioningDocumentPDFTemplate document={pdfData} />} fileName="Patient_Positioning.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search patient positioning..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={safeId(record) || idx} className="record-card">
            <div className="record-header">
              ...
              <div className="record-title-row"><h3 className="record-name">{highlightText(`Patient Positioning ${idx + 1}`)}</h3></div>
            </div>
...
export default PatientPositioningDocument;
