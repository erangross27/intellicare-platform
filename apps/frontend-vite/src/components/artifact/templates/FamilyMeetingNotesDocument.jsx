/**
 * FamilyMeetingNotesDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: family_meeting_notes
 *
 * 9 Sections:
 *   1. session-info: date (date picker), provider, facility, status
 *   2. attendees-section: attendees[] (string array)
 *   3. discussion-section: discussionPoints[] (string array)
 *   4. concerns-section: familyConcerns[] (string array)
 *   5. decisions-section: decisions[] (string array)
 *   6. support-section: supportNeeded (simple string)
 ...
 *   9. notes-section: findings, assessment, plan, notes (sentence fields)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import FamilyMeetingNotesDocumentPDFTemplate from '../pdf-templates/FamilyMeetingNotesDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './FamilyMeetingNotesDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'family_meeting_notesPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'discussion-section': 'Discussion Points',
  'concerns-section': 'Family Concerns',
  'decisions-section': 'Decisions',
  'support-section': 'Support Needed',
  'results-section': 'Results',
  'recommendations-section': 'Recommendations',
  'notes-section': 'Notes & Assessment',
};
...
const SECTION_FIELDS = {
  ...
  'attendees-section': ['attendees'],
  'discussion-section': ['discussionPoints'],
  'concerns-section': ['familyConcerns'],
  'decisions-section': ['decisions'],
  'support-section': ['supportNeeded'],
  'results-section': ['results'],
  'recommendations-section': ['recommendations'],
  'notes-section': ['findings', 'assessment', 'plan', 'notes'],
};

const STRING_ARRAY_FIELDS = ['attendees', 'discussionPoints', 'familyConcerns', 'decisions'];
...
/* ═══════ COMPONENT ═══════ */
const FamilyMeetingNotesDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Family Meeting Notes ${idx + 1}`.toLowerCase();
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.ef) — whole object staged as a DRAFT in localEdits.
     NOT written to MongoDB until Approve commits it. */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    // Persist the whole merged object to the draft store (clone is set synchronously above).
    const store = readDrafts();
    if (!store[id]) store[id] = {};
  ...
  // saveSentence stages a DRAFT (no DB write). localStorage keeps it across refresh; Approve commits it.
  const stageSentenceDraft = (id, fn, fullText) => {
    const store = readDrafts();
    if (!store[id]) store[id] = {};
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageSentenceDraft(id, fn, fullText);
      setEditingField(null); setEditValue('');
      return;
    }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/family_meeting_notes/${id}/edit`, payload);
      }
      // Flag the record/section approved (audit trail)
      await secureApiClient.put(`/api/edit/family_meeting_notes/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[FamilyMeetingNotes] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    return text;
  }, [getFieldValue, getStringArrayItemValue, hasVal, fmtVal, formatSentenceFieldLines, objectCopyLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== FAMILY MEETING NOTES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Family Meeting Notes ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: SIMPLE EDITABLE FIELD (textarea) ═══════ */
  ...
  const renderStringArraySection = (record, idx, fn, sid) => {
    ...
    const showLabel = title.toLowerCase().trim() !== sectionTitle.toLowerCase().trim();
    const phraseMatch = !searchTerm.trim() || sectionTitleMatches(sid) || record._showAllSections;

    return (
      <div key={fn} className="rec-mini-card">
        {showLabel && <div className="nested-subtitle">{highlightText(title)}</div>}
        {arr.map((item, arrIdx) => {
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      if (STRING_ARRAY_FIELDS.includes(f)) return Array.isArray(record[f]) && record[f].length > 0;
      return hasVal(val);
    });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="family-meeting-notes-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Family Meeting Notes</h2></div>
        <div className="empty-state">No family meeting notes records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="family-meeting-notes-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Family Meeting Notes</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FamilyMeetingNotesDocumentPDFTemplate document={pdfData} />} fileName="Family_Meeting_Notes.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search family meeting notes..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Family Meeting Notes ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'attendees-section')}
            {renderSection(record, idx, 'discussion-section')}
            {renderSection(record, idx, 'concerns-section')}
            {renderSection(record, idx, 'decisions-section')}
            {renderSection(record, idx, 'support-section')}
...
export default FamilyMeetingNotesDocument;
