/**
 * FmlaDocumentationNoteDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: fmla_documentation_note
 *
 * 5 Sections:
 *   1. functional-capacity: patientFunctionalCapacity, functionalImpairmentScale, symptomSeverityScore, cognitiveImpairmentPresent
 *   2. work-restrictions: workRestrictions, liftingCapacityPounds, standingToleranceHours, modifiedDutyRecommendations, stressLimitationRequired
 *   3. leave-details: intermittentLeaveRequired, reducedScheduleNeeded, disabilityDurationEstimate, returnToWorkDate
 *   4. diagnosis-treatment: primaryDiagnosisCode, treatmentCompliance, treatmentFrequency, medicationSideEffects, prognosisOutcome
 *   5. other: followUpSchedule, familyMemberCondition
 *
 * Record header: TITLE/index ONLY — schema has NO record date/provider/facility field.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import FmlaDocumentationNoteDocumentPDFTemplate from '../pdf-templates/FmlaDocumentationNoteDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './FmlaDocumentationNoteDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'fmla_documentation_notePendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  patientFunctionalCapacity: 'Patient Functional Capacity',
  functionalImpairmentScale: 'Functional Impairment Scale',
  ...
  liftingCapacityPounds: 'Lifting Capacity (lbs)',
  standingToleranceHours: 'Standing Tolerance (hrs)',
  modifiedDutyRecommendations: 'Modified Duty Recommendations',
  stressLimitationRequired: 'Stress Limitation Required',
  intermittentLeaveRequired: 'Intermittent Leave Required',
  reducedScheduleNeeded: 'Reduced Schedule Needed',
  disabilityDurationEstimate: 'Disability Duration Estimate',
  ...
  prognosisOutcome: 'Prognosis Outcome',
  followUpSchedule: 'Follow-Up Schedule',
  familyMemberCondition: 'Family Member Condition',
};

const SECTION_FIELDS = {
  'functional-capacity': ['patientFunctionalCapacity', 'functionalImpairmentScale', 'symptomSeverityScore', 'cognitiveImpairmentPresent'],
  'work-restrictions': ['workRestrictions', 'liftingCapacityPounds', 'standingToleranceHours', 'modifiedDutyRecommendations', 'stressLimitationRequired'],
  'leave-details': ['intermittentLeaveRequired', 'reducedScheduleNeeded', 'disabilityDurationEstimate', 'returnToWorkDate'],
  'diagnosis-treatment': ['primaryDiagnosisCode', 'treatmentCompliance', 'treatmentFrequency', 'medicationSideEffects', 'prognosisOutcome'],
  'other': ['followUpSchedule', 'familyMemberCondition'],
};

const NUMBER_FIELDS = ['symptomSeverityScore', 'liftingCapacityPounds', 'standingToleranceHours'];
const DATE_FIELDS = ['returnToWorkDate'];
const ARRAY_FIELDS = ['workRestrictions'];
const BOOLEAN_FIELDS = ['intermittentLeaveRequired', 'reducedScheduleNeeded', 'cognitiveImpairmentPresent', 'stressLimitationRequired'];
const STRING_FIELDS = ['patientFunctionalCapacity', 'disabilityDurationEstimate', 'primaryDiagnosisCode', 'functionalImpairmentScale', 'treatmentCompliance', 'treatmentFrequency', 'prognosisOutcome', 'medicationSideEffects', 'modifiedDutyRecommendations', 'followUpSchedule', 'familyMemberCondition'];
...
/* ═══════ COMPONENT ═══════ */
const FmlaDocumentationNoteDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.fmla_documentation_note) return Array.isArray(r.fmla_documentation_note) ? r.fmla_documentation_note : [r.fmla_documentation_note];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.fmla_documentation_note) return Array.isArray(dd.fmla_documentation_note) ? dd.fmla_documentation_note : [dd.fmla_documentation_note]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // fieldPart = "field" or "field.arrayIndex"; localEdits is keyed by "field-idx"
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `FMLA Documentation Note ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // localEdits keys are "field-idx" (array edits store the whole array under the bare field key).
      const suffix = `-${idx}`;
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length); // "field" (no dot — bare field name)
        const lastDot = fieldPart.lastIndexOf('.');
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/fmla_documentation_note/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/fmla_documentation_note/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== FMLA DOCUMENTATION NOTE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `FMLA Documentation Note ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="fmla-documentation-note-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">FMLA Documentation Note</h2></div>
        <div className="empty-state">No FMLA documentation note data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="fmla-documentation-note-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">FMLA Documentation Note</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FmlaDocumentationNoteDocumentPDFTemplate document={pdfData} />} fileName={`fmla-documentation-note-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search FMLA documentation note..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`FMLA Documentation Note ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'functional-capacity')}
            {renderSection(record, idx, 'work-restrictions')}
            {renderSection(record, idx, 'leave-details')}
            {renderSection(record, idx, 'diagnosis-treatment')}
            {renderSection(record, idx, 'other')}
          </div>
        ))}
      </div>
    </div>
...
export default FmlaDocumentationNoteDocument;
