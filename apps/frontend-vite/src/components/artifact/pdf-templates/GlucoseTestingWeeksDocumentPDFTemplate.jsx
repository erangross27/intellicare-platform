 ...
 * OBJECT fields oneHourTest/threeHourTest/results rendered recursively as humanized key/value lines.
...
const FIELD_LABELS = {
  ...
  oneHourTest: '1-Hour Test', threeHourTest: '3-Hour Test', results: 'Results', recommendations: 'Recommendations',
};
...
const GlucoseTestingWeeksDocumentPDFTemplate = ({ document: data }) => {
  let records = [];
  if (Array.isArray(data)) {
    if (data.length === 1 && data[0]?.glucose_testing_weeks) records = Array.isArray(data[0].glucose_testing_weeks) ? data[0].glucose_testing_weeks : [data[0].glucose_testing_weeks];
    else records = data;
  } else if (data?.glucose_testing_weeks) records = Array.isArray(data.glucose_testing_weeks) ? data.glucose_testing_weeks : [data.glucose_testing_weeks];
  else if (data?.documentData) { const dd = data.documentData; if (Array.isArray(dd)) records = dd; else if (dd?.glucose_testing_weeks) records = Array.isArray(dd.glucose_testing_weeks) ? dd.glucose_testing_weeks : [dd.glucose_testing_weeks]; else if (dd && typeof dd === 'object') records = [dd]; }
  else if (data && typeof data === 'object') records = [data];
  records = (records || []).filter(r => r && typeof r === 'object');

  if (records.length === 0) {
    return (<Document><Page size="A4" style={styles.page}><View style={styles.documentHeader}><Text style={styles.title}>Glucose Tolerance Test</Text></View><Text style={styles.emptyState}>No records available</Text></Page></Document>);
  }

  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.title}>Glucose Tolerance Test</Text></View>
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer}>
            <View style={styles.recordHeader} wrap={false}>
              <Text style={styles.recordTitle}>{`Glucose Tolerance Test ${String(record._recordNumber || idx + 1)}`}</Text>
...
export default GlucoseTestingWeeksDocumentPDFTemplate;
