/**
 * PumpAdvancedSettingsDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: pump_advanced_settings
 *
 * 4 Sections:
 *   1. device: date, provider, facility, pumpManufacturer, pumpModelNumber, pumpSerialNumber, softwareFirmwareVersion
 *   2. basal-bolus: basalRateProfile(array), bolusDeliveryMode, maxBolusLimit, insulinCarbRatio, correctionFactor, activeInsulinTime
 *   3. targets: targetGlucoseRange(object)
 *   4. safety: occlusioPressureLimit, lockoutInterval, continuousRateLimit, alarmVolumeLevel, batteryBackupDuration,
 *              autoSuspendThreshold, tempBasalDuration, airInLineDetection, doseErrorReductionSystem,
 *              wirelessCommunicationEnabled, nightModeSchedule(object)
 */
...
const DRAFT_KEY = 'pump_advanced_settingsPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  pumpModelNumber: 'Pump Model',
  pumpSerialNumber: 'Pump Serial Number',
  softwareFirmwareVersion: 'Software/Firmware Version',
  basalRateProfile: 'Basal Rate Profile',
  bolusDeliveryMode: 'Bolus Delivery Mode',
  maxBolusLimit: 'Max Bolus Limit (U)',
  insulinCarbRatio: 'Insulin:Carb Ratio (1:x g)',
  correctionFactor: 'Correction Factor (1:x mg/dL)',
  activeInsulinTime: 'Active Insulin Time (hours)',
  targetGlucoseRange: 'Target Glucose Range',
  occlusioPressureLimit: 'Occlusion Pressure Limit',
  lockoutInterval: 'Lockout Interval (minutes)',
  continuousRateLimit: 'Continuous Rate Limit (U/hr)',
  alarmVolumeLevel: 'Alarm Volume Level',
  batteryBackupDuration: 'Battery Backup Duration (hours)',
  autoSuspendThreshold: 'Auto-Suspend Threshold (mg/dL)',
  tempBasalDuration: 'Temp Basal Duration (minutes)',
  airInLineDetection: 'Air-In-Line Detection',
  doseErrorReductionSystem: 'Dose Error Reduction System',
  wirelessCommunicationEnabled: 'Wireless Communication Enabled',
  nightModeSchedule: 'Night Mode Schedule',
};

/* Humanized labels for OBJECT field sub-keys */
...
const SECTION_FIELDS = {
  'device': ['date', 'provider', 'facility', 'pumpManufacturer', 'pumpModelNumber', 'pumpSerialNumber', 'softwareFirmwareVersion'],
  'basal-bolus': ['basalRateProfile', 'bolusDeliveryMode', 'maxBolusLimit', 'insulinCarbRatio', 'correctionFactor', 'activeInsulinTime'],
  'targets': ['targetGlucoseRange'],
  'safety': ['occlusioPressureLimit', 'lockoutInterval', 'continuousRateLimit', 'alarmVolumeLevel', 'batteryBackupDuration', 'autoSuspendThreshold', 'tempBasalDuration', 'airInLineDetection', 'doseErrorReductionSystem', 'wirelessCommunicationEnabled', 'nightModeSchedule'],
};

const NUMBER_FIELDS = ['maxBolusLimit', 'insulinCarbRatio', 'correctionFactor', 'activeInsulinTime', 'occlusioPressureLimit', 'lockoutInterval', 'continuousRateLimit', 'alarmVolumeLevel', 'batteryBackupDuration', 'autoSuspendThreshold', 'tempBasalDuration'];
const ARRAY_FIELDS = ['basalRateProfile'];
...
/* parseColonSpace: split on the FIRST ": " (colon+space). Safe when the label itself
   contains colons that are NOT followed by a space, e.g. "12:00AM-3:00AM: 0.55 U/hr". */
...
/* splitNumberUnit: "0.55 U/hr" -> { num: "0.55", unit: "U/hr" }; null if it doesn't start with a number.
   Lets a numeric value with a unit be edited via a number spinner while preserving the unit. */
...
/* objectEntries: populated key/value pairs of an object field (hide-empty) */
...
/* ═══════ COMPONENT ═══════ */
const PumpAdvancedSettingsDocument = ({ document: docProp }) => {
  ...
  /* ═══════ UTILS ═══════ */
  const hasVal = useCallback((v) => { if (v === null || v === undefined || v === '') return false; if (typeof v === 'boolean') return true; if (typeof v === 'number') return true; if (typeof v === 'string') return v.trim() !== ''; if (Array.isArray(v)) return v.length > 0; if (typeof v === 'object') return objectEntries(v).length > 0; return true; }, []);
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        ...
        else if (typeof val === 'object') { if (objectEntries(val).some(([k, v]) => humanizeKey(k).toLowerCase().includes(phrase) || String(k).toLowerCase().includes(phrase) || fmtObjectValue(v).toLowerCase().includes(phrase))) return true; }
        else if (DATE_FIELDS.includes(f)) { if (formatDate(val).toLowerCase().includes(phrase)) return true; }
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) {
      ...
      if (typeof val === 'object') return objectEntries(val).some(([k, v]) => humanizeKey(k).toLowerCase().includes(phrase) || String(k).toLowerCase().includes(phrase) || fmtObjectValue(v).toLowerCase().includes(phrase));
      if (DATE_FIELDS.includes(fn)) return formatDate(val).toLowerCase().includes(phrase);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pump Advanced Settings ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (Array.isArray(val)) { if (val.some(item => String(item).toLowerCase().includes(phrase))) return true; }
          else if (typeof val === 'object') { if (objectEntries(val).some(([k, v]) => humanizeKey(k).toLowerCase().includes(phrase) || fmtObjectValue(v).toLowerCase().includes(phrase))) return true; }
          else if (DATE_FIELDS.includes(f)) { if (formatDate(val).toLowerCase().includes(phrase)) return true; }
          else if (fmtVal(val).toLowerCase().includes(phrase)) return true;
        }
      }
  ...
  // Stage a sentence-level DRAFT (no DB write). localStorage keeps it across refresh; Approve commits it.
  function stageFieldDraft(record, fn, idx, sid, fullText, sentenceMarks) {
    const id = safeId(record); if (!id) return;
    const editKey = `${fn}-${idx}`;
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageFieldDraft(record, fn, idx, sid, fullText, { [`${fn}-${idx}-s${sentenceIdx}`]: 'edited' });
      return;
    }
    ...
    stageFieldDraft(record, fn, idx, sid, fullText, marks);
  }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // localEdits keys are "field-idx"; commit the staged (pending) ones for THIS section.
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/pump_advanced_settings/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/pump_advanced_settings/${id}/approve`, { sectionId: sid, approved: true });
      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this record's drafts for the committed fields from localStorage (now committed)
      const store = readDrafts();
      if (store[id]) { toCommit.forEach(k => { const fp = k.slice(0, -suffix.length); const di = fp.lastIndexOf('.'); const base = di !== -1 && /^\d+$/.test(fp.slice(di + 1)) ? fp.slice(0, di) : fp; delete store[id][base]; }); if (Object.keys(store[id]).length === 0) delete store[id]; writeDrafts(store); }
  ...
  /* ═══════ RENDER: DATE FIELD (string via formatDate) ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          const isEditing = editingField === editKey;
          const isModified = editedFields[editKey];
          const itemStr = typeof item === 'object' && item !== null ? fmtObjectValue(item) : String(item);
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(nu ? nu.num : (parsed.isLabeled ? parsed.value : itemStr)); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; let saveVal; if (nu) { const n = parseFloat(editValue); if (Number.isNaN(n)) { setSaveError('Please enter a valid number'); return; } saveVal = `${parsed.label}: ${n}${nu.unit ? ' ' + nu.unit : ''}`; } else { saveVal = parsed.isLabeled ? `${parsed.label}: ${editValue}` : editValue; } const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = saveVal; const lk = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [lk]: currentArr })); setPendingEdits(prev => ({ ...prev, [lk]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n2 = { ...prev }; delete n2[`${sid}-${idx}`]; return n2; }); const store = readDrafts(); if (!store[id2]) store[id2] = {}; store[id2][fn] = currentArr; writeDrafts(store); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; stageFieldDraft(record, fn, idx, sid, fullText, marks); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      ...
      if (DATE_FIELDS.includes(f)) return Boolean(formatDate(getFieldValue(record, f, idx)));
      if (OBJECT_FIELDS.includes(f)) return objectEntries(getFieldValue(record, f, idx)).length > 0;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pump-advanced-settings-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pump Advanced Settings</h2></div>
        <div className="empty-state">No pump advanced settings data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pump-advanced-settings-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pump advanced settings..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Pump Advanced Settings ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'device')}
            {renderSection(record, idx, 'basal-bolus')}
...
export default PumpAdvancedSettingsDocument;
