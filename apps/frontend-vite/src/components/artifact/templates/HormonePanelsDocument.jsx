/**
 * HormonePanelsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: hormone_panels
 *
 * 5 Sections:
 *   1. thyroid-panel: thyroidStimulatingHormone, freeThyroxine, freeTriiodothyronine
 *   2. reproductive: totalTestosterone, freeTestosterone, bioavailableTestosterone,
 *                    sexHormoneBindingGlobulin, estradiol, progesterone,
 *                    luteinizingHormone, follicleStimulatingHormone, prolactin
 *   3. adrenal-metabolic: cortisol, cortisolCollectionTime (string), growthHormone,
 *                         insulinLikeGrowthFactor1, insulin, cPeptide,
 *                         homaInsulinResistanceIndex, adrenocorticotropicHormone,
 *                         dehydroepiandrosteroneSulfate
 *   4. bone-mineral: parathyroidHormone, vitamin25OHD
 *   5. autoimmune: thyroidPeroxidaseAntibody, thyroglobulinAntibody
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HormonePanelsDocumentPDFTemplate from '../pdf-templates/HormonePanelsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './HormonePanelsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'hormone_panelsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'reproductive': 'Reproductive Hormones',
  'adrenal-metabolic': 'Adrenal & Metabolic Hormones',
  'bone-mineral': 'Bone & Mineral Metabolism',
  'autoimmune': 'Autoimmune Markers',
};

const FIELD_LABELS = {
  thyroidStimulatingHormone: 'Thyroid Stimulating Hormone (TSH) (mIU/L)',
  freeThyroxine: 'Free Thyroxine (T4) (ng/dL)',
  freeTriiodothyronine: 'Free Triiodothyronine (T3) (pg/mL)',
  totalTestosterone: 'Total Testosterone (ng/dL)',
  freeTestosterone: 'Free Testosterone (pg/mL)',
  bioavailableTestosterone: 'Bioavailable Testosterone (ng/dL)',
  sexHormoneBindingGlobulin: 'Sex Hormone Binding Globulin (nmol/L)',
  estradiol: 'Estradiol (pg/mL)',
  progesterone: 'Progesterone (ng/mL)',
  luteinizingHormone: 'Luteinizing Hormone (mIU/mL)',
  follicleStimulatingHormone: 'FSH (mIU/mL)',
  prolactin: 'Prolactin (ng/mL)',
  cortisol: 'Cortisol (mcg/dL)',
  cortisolCollectionTime: 'Cortisol Collection Time',
  growthHormone: 'Growth Hormone (ng/mL)',
  insulinLikeGrowthFactor1: 'IGF-1 (ng/mL)',
  insulin: 'Insulin (mcIU/mL)',
  cPeptide: 'C-Peptide (ng/mL)',
  homaInsulinResistanceIndex: 'HOMA-IR Index',
  adrenocorticotropicHormone: 'ACTH (pg/mL)',
  dehydroepiandrosteroneSulfate: 'DHEA-S (mcg/dL)',
  parathyroidHormone: 'Parathyroid Hormone (pg/mL)',
  vitamin25OHD: 'Vitamin D 25-OH (ng/mL)',
  thyroidPeroxidaseAntibody: 'Thyroid Peroxidase Antibody (TPO) (IU/mL)',
  thyroglobulinAntibody: 'Thyroglobulin Antibody (IU/mL)',
};

const SECTION_FIELDS = {
  'thyroid-panel': ['thyroidStimulatingHormone', 'freeThyroxine', 'freeTriiodothyronine'],
  'reproductive': ['totalTestosterone', 'freeTestosterone', 'bioavailableTestosterone', 'sexHormoneBindingGlobulin', 'estradiol', 'progesterone', 'luteinizingHormone', 'follicleStimulatingHormone', 'prolactin'],
  'adrenal-metabolic': ['cortisol', 'cortisolCollectionTime', 'growthHormone', 'insulinLikeGrowthFactor1', 'insulin', 'cPeptide', 'homaInsulinResistanceIndex', 'adrenocorticotropicHormone', 'dehydroepiandrosteroneSulfate'],
  'bone-mineral': ['parathyroidHormone', 'vitamin25OHD'],
  'autoimmune': ['thyroidPeroxidaseAntibody', 'thyroglobulinAntibody'],
};

/* 24 number fields — ALL use the −/+ num-stepper + parseFloat+isNaN→block save (incl. the many 0s) */
const NUMBER_FIELDS = [
  'thyroidStimulatingHormone', 'freeThyroxine', 'freeTriiodothyronine',
  'totalTestosterone', 'freeTestosterone', 'bioavailableTestosterone',
  'sexHormoneBindingGlobulin', 'estradiol', 'progesterone',
  'luteinizingHormone', 'follicleStimulatingHormone', 'prolactin',
  'cortisol', 'growthHormone', 'insulinLikeGrowthFactor1',
  'insulin', 'cPeptide', 'homaInsulinResistanceIndex',
  'adrenocorticotropicHormone', 'dehydroepiandrosteroneSulfate',
  'parathyroidHormone', 'vitamin25OHD',
  'thyroidPeroxidaseAntibody', 'thyroglobulinAntibody',
];

/* cortisolCollectionTime ("8 AM") is a STRING, not a number → stays on the text/sentence widget */
const STRING_FIELDS = ['cortisolCollectionTime'];
const DATE_FIELDS = [];
...
/* ═══════ COMPONENT ═══════ */
const HormonePanelsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.hormone_panels) return Array.isArray(r.hormone_panels) ? r.hormone_panels : [r.hormone_panels];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.hormone_panels) return Array.isArray(dd.hormone_panels) ? dd.hormone_panels : [dd.hormone_panels]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Hormone Panel ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/hormone_panels/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/hormone_panels/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HORMONE PANELS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Hormone Panel ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD — −/+ num-stepper + parseFloat+isNaN→block save (renders 0s) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="hormone-panels-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Hormone Panels</h2></div>
        <div className="empty-state">No hormone panel records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="hormone-panels-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Hormone Panels</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HormonePanelsDocumentPDFTemplate document={pdfData} />} fileName="Hormone_Panels.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search hormone panels..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Hormone Panel ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'thyroid-panel')}
            {renderSection(record, idx, 'reproductive')}
            {renderSection(record, idx, 'adrenal-metabolic')}
            {renderSection(record, idx, 'bone-mineral')}
            {renderSection(record, idx, 'autoimmune')}
          </div>
        ))}
      </div>
    </div>
...
export default HormonePanelsDocument;
