/**
 * IntegrativeOncologyDocument.jsx
 * March 2026 — Canonical one-pass polish, inline editing, blue glow theme
 * Collection: integrative_oncology
 *
 * 6 Sections:
 *   1. session-info: date (BlueDatePicker), provider, facility
 *   2. complementary-therapies: complementaryTherapies (string array → list)
 *   3. mind-body-practices: mindBodyPractices (string array → list)
 *   4. acupuncture-section: acupuncture (boolean → BlueSelect Yes/No)
 *   5. supplements: supplements — array of {name, dose}; each object decomposes into a nested-subtitle
 *                   (supplement name) + an editable dose row (mirrors HospitalAdmissionNotes array-of-objects)
 *   6. clinical-notes: findings, notes, assessment, plan, recommendations (list), status
 *
 * Empty nested objects (nutritionalSupport {} / exerciseProgram {} / results {}) are skipped.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import IntegrativeOncologyDocumentPDFTemplate from '../pdf-templates/IntegrativeOncologyDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './IntegrativeOncologyDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'integrative_oncologyPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['provider', 'facility', 'findings', 'notes', 'assessment', 'plan', 'status'];
...
/* ═══════ COMPONENT ═══════ */
const IntegrativeOncologyDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.integrative_oncology) return Array.isArray(r.integrative_oncology) ? r.integrative_oncology : [r.integrative_oncology];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.integrative_oncology) return Array.isArray(dd.integrative_oncology) ? dd.integrative_oncology : [dd.integrative_oncology]; return [dd]; }
      return [r];
    });
  ...
  const fmtVal = useCallback((v) => { if (typeof v === 'boolean') return v ? 'Yes' : 'No'; if (typeof v === 'number') return String(v); if (Array.isArray(v)) return v.map(x => (x && typeof x === 'object') ? Object.values(x).join(' ') : String(x)).join(', '); return String(v || ''); }, []);
  ...
  /* searchable string for any value (arrays / objects flattened) */
  const searchStr = useCallback((v) => {
    if (v === null || v === undefined) return '';
    if (typeof v === 'boolean') return v ? 'yes' : 'no';
    if (Array.isArray(v)) return v.map(searchStr).join(' ');
    if (typeof v === 'object') return Object.values(v).map(searchStr).join(' ');
    return String(v);
  }, []);
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined && searchStr(val).toLowerCase().includes(phrase)) return true;
    }
    return false;
  }, [searchTerm, getFieldValue, searchStr]);
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) return searchStr(val).toLowerCase().includes(phrase);
    return false;
  }, [searchTerm, getFieldValue, searchStr]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Integrative Oncology ${idx + 1}`.toLowerCase();
  ...
  }, [records, searchTerm, getFieldValue, searchStr]);
  ...
  // Stage a single string-array item edit as a DRAFT (whole array staged under `${fn}-${idx}`; no DB write).
  const handleSaveArrayItem = useCallback((record, fn, idx, arrIdx, value, sid) => {
    ...
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
    const marks = { [`${fn}.${arrIdx}-${idx}`]: 'edited' };
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/integrative_oncology/${id}/edit`, { field: fn, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/integrative_oncology/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        const supps = Array.isArray(val) ? val.filter(s => s && typeof s === 'object' && ((s.name || '').trim() || (s.dose || '').trim())) : [];
        if (supps.length > 0) {
          if (showLabel) text += `${label}\n`;
          supps.forEach((s, i) => { const nm = (s.name || '').trim(); const ds = (s.dose || '').trim(); text += `  ${i + 1}. ${nm}${ds ? ` - ${ds}` : ''}\n`; });
          text += '\n';
        }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== INTEGRATIVE ONCOLOGY ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Integrative Oncology ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: ARRAY FIELD (string list) ═══════ */
  ...
  /* ═══════ RENDER: SUPPLEMENTS — array of {name, dose} ═══════
     Each object decomposes into a nested-subtitle (supplement name) + an editable dose row. */
  const renderSupplementsField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    const supps = Array.isArray(val) ? val.filter(s => s && typeof s === 'object' && ((s.name || '').trim() || (s.dose || '').trim())) : [];
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {supps.map((supp, sIdx) => {
          ...
          const dose = (supp.dose || '').trim();
          const itemKey = `${fn}-${idx}-s${sIdx}`;
          const isEditing = editingField === itemKey;
          const badge = editedSentences[itemKey];
          const itemMatches = phraseMatch || labelMatch || !searchTerm.trim() || name.toLowerCase().includes(phrase) || dose.toLowerCase().includes(phrase);
          if (!itemMatches && searchTerm.trim()) return null;
          return (
            <div key={sIdx} className="rec-mini-card" style={{ marginTop: 8 }}>
              {name && <div className="nested-subtitle">{highlightText(name)}</div>}
              {dose && (
                <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(dose); setSaveError(null); } }}>
                  {isEditing ? (
                    <div className="edit-field-container">
                      <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                      {saveError && <div className="save-error">{saveError}</div>}
                      <div className="edit-actions">
                        <button className="save-btn" disabled={saving} onClick={e => {
                          ...
                          const cur = currentArr[sIdx];
                          if (cur && typeof cur === 'object') currentArr[sIdx] = { ...cur, dose: editValue.trim() };
                          setSaveError(null);
                    ...
                    <>
                      <div className="row-content"><span className="content-value">{highlightText(dose)}</span><span className="edit-indicator">&#9998;</span></div>
                      <button className={`copy-btn ${copiedItems[itemKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${name}${dose ? ` - ${dose}` : ''}`, itemKey); }}>{copiedItems[itemKey] ? 'Copied!' : 'Copy'}</button>
                    </>
                  )}
                </div>
              )}
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    const showLabelSingle = label.trim().toLowerCase() !== sectionTitle.trim().toLowerCase();
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (OBJECT_ARRAY_FIELDS.includes(f)) return renderSupplementsField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="integrative-oncology-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Integrative Oncology</h2></div>
        <div className="empty-state">No integrative oncology records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="integrative-oncology-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Integrative Oncology</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<IntegrativeOncologyDocumentPDFTemplate document={pdfData} />} fileName="Integrative_Oncology.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search integrative oncology records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Integrative Oncology ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'complementary-therapies')}
            {renderSection(record, idx, 'mind-body-practices')}
            {renderSection(record, idx, 'acupuncture-section')}
...
export default IntegrativeOncologyDocument;
