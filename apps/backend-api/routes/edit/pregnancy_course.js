...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'pregnancy-course-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'firstTrimester', 'firstTrimester.earlyBleeding', 'firstTrimester.screening', 'firstTrimester.complications',
  'secondTrimester', 'secondTrimester.anatomyScanFindings', 'secondTrimester.glucoseTesting', 'secondTrimester.complications',
  'thirdTrimester', 'thirdTrimester.growthAssessment', 'thirdTrimester.surveillance', 'thirdTrimester.complications',
];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('pregnancy_course', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[pregnancy_course] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('pregnancy_course', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[pregnancy_course] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
