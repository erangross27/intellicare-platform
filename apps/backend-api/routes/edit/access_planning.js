/**
 * Access Planning Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/access_planning/:id/edit    — Edit a single field
 * PUT /api/edit/access_planning/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'access-planning-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'date', 'plannedDate',
  'accessType', 'indication', 'plannedSite', 'vascularMapping',
  'surgeon', 'preoperativeConsiderations',
  'anticoagulation', 'temporaryAccess', 'maturationTime',
  'followUp', 'provider', 'facility', 'notes',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    if (!objectId) {
      return res.status(400).json({ success: false, error: 'Invalid access planning ID' });
    }

    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');

    await sda.update('access_planning', { _id: objectId }, {
      $set: {
        [field]: value,
        updatedAt: new Date(),
    ...
    console.error('[AccessPlanning] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    console.error('[AccessPlanning] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
