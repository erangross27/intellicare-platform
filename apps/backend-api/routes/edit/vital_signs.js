...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'vital-signs-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['bloodPressure', 'heartRate', 'temperature', 'respiratoryRate', 'oxygenSaturation', 'position', 'weight', 'height', 'bmi', 'headCircumference', 'painScore', 'bloodGlucose', 'peakFlow', 'normalRanges', 'previousVisit', 'date'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('vital_signs', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[vital_signs] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('vital_signs', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[vital_signs] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
