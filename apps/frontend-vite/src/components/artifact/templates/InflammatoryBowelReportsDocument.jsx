/**
 * InflammatoryBowelReportsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: inflammatory_bowel_reports
 *
 * 5 Sections:
 *   1. session-info: date (date picker), montrealClassification (string), rectalBleedingSeverity (string), perirectalDisease (string)
 *   2. disease-activity: crohnsDiseasesActivityIndex, mayoEndoscopicScore, simpleEndoscopicScore,
 *                        wellBeingGeneralScore, bowelMovementsPerDay
 *   3. lab-markers: fecalCalprotectinLevel, fecalLactoferrinLevel, cReactiveProteinLevel,
 *                   erythrocyteSedimentationRate, albuminLevel, hemoglobinLevel, hematocritValue
 *   4. body-metrics: bodyWeightKg, idealBodyWeightRatio, abdominalMassPresence,
 *                    opiateUseForAbdominalPain, endoscopicRemissionAchieved
 *   5. manifestations: arthropathicManifestations (array)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import InflammatoryBowelReportsDocumentPDFTemplate from '../pdf-templates/InflammatoryBowelReportsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './InflammatoryBowelReportsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; this template stores the
   whole field value under "field-idx", so fieldPart is always a plain field name, never dotted) */
const DRAFT_KEY = 'inflammatory_bowel_reportsPendingEdits';
const readDrafts = () => {
...
const NUMBER_FIELDS = [
  ...
  'fecalCalprotectinLevel', 'fecalLactoferrinLevel',
  'bowelMovementsPerDay', 'cReactiveProteinLevel', 'erythrocyteSedimentationRate',
  'albuminLevel', 'hemoglobinLevel', 'wellBeingGeneralScore',
  'hematocritValue', 'bodyWeightKg', 'idealBodyWeightRatio',
];

const BOOLEAN_FIELDS = ['abdominalMassPresence', 'opiateUseForAbdominalPain', 'endoscopicRemissionAchieved'];
const STRING_FIELDS = ['montrealClassification', 'parisClassification', 'ileocolonicAnatomy', 'rectalBleedingSeverity', 'perirectalDisease'];
...
/* ═══════ COMPONENT ═══════ */
const InflammatoryBowelReportsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.inflammatory_bowel_reports) return Array.isArray(r.inflammatory_bowel_reports) ? r.inflammatory_bowel_reports : [r.inflammatory_bowel_reports];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.inflammatory_bowel_reports) return Array.isArray(dd.inflammatory_bowel_reports) ? dd.inflammatory_bowel_reports : [dd.inflammatory_bowel_reports]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Inflammatory Bowel Report ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // localEdits keys are "field-idx" (whole-field value); commit only this section's pending fields.
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k]) return false;
        const m = k.match(/^(.+)-(\d+)$/);
        if (!m || parseInt(m[2], 10) !== idx) return false;
        // fieldPart = m[1] (plain field name in this template — no dotted arrayIndex keys exist).
        // GOTCHA: arrayIndex only when the trailing dot-segment is purely numeric; never the case here.
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/inflammatory_bowel_reports/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/inflammatory_bowel_reports/${id}/approve`, { sectionId: sid, approved: true });
      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this section's committed fields from the localStorage draft store
      if (toCommit.length > 0) {
        const store = readDrafts();
        if (store[id]) { toCommit.forEach(k => { const f = k.match(/^(.+)-(\d+)$/)[1]; delete store[id][f]; }); if (Object.keys(store[id]).length === 0) delete store[id]; writeDrafts(store); }
      }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== INFLAMMATORY BOWEL REPORTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Inflammatory Bowel Report ${idx + 1}\n${'='.repeat(40)}\n\n`;
                ...
                <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(itemStr); setSaveError(null); } }}>
                  {isEditing ? (
                    <div className="edit-field-container">
                      ...
                      {saveError && <div className="save-error">{saveError}</div>}
                      <div className="edit-actions">
                        <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveField(record, fn, idx, sid, null, editValue.trim(), itemKey); }}>{saving ? 'Saving...' : 'Save'}</button>
                        <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                      </div>
                    </div>
                  ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="inflammatory-bowel-reports-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Inflammatory Bowel Reports</h2></div>
        <div className="empty-state">No inflammatory bowel reports records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="inflammatory-bowel-reports-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Inflammatory Bowel Reports</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<InflammatoryBowelReportsDocumentPDFTemplate document={pdfData} />} fileName="Inflammatory_Bowel_Reports.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search inflammatory bowel reports..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Inflammatory Bowel Report ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'disease-activity')}
            {renderSection(record, idx, 'lab-markers')}
            {renderSection(record, idx, 'body-metrics')}
            {renderSection(record, idx, 'manifestations')}
          </div>
        ))}
      </div>
    </div>
...
export default InflammatoryBowelReportsDocument;
