/**
 * CytologyReportsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: cytology_reports
 *
 * 4 Sections:
 *   1. report-info: date, pathologist, specimenType, bethesdaCategory
 *   2. specimen-analysis: adequacy (sentence/semicolon split), cellularity
 *   3. findings: cytologicFindings (sentence/semicolon split + comma-split)
 *   4. diagnosis: diagnosis (sentence with parseLabel)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import CytologyReportsDocumentPDFTemplate from '../pdf-templates/CytologyReportsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import './CytologyReportsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; whole-field values) */
const DRAFT_KEY = 'cytologyReportsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'specimen-analysis': 'Specimen Analysis',
  findings: 'Findings',
  diagnosis: 'Diagnosis',
};
...
const SECTION_FIELDS = {
  'report-info': ['date', 'pathologist', 'specimenType', 'bethesdaCategory'],
  'specimen-analysis': ['adequacy', 'cellularity'],
  findings: ['cytologicFindings'],
  diagnosis: ['diagnosis'],
};

const DATE_FIELDS = ['date'];
const SENTENCE_FIELDS = ['adequacy', 'cytologicFindings', 'diagnosis'];

/* Fixed-choice → <select>. The Bethesda System categories; enumOptionsWith keeps any unlisted stored value. */
const ENUM_OPTIONS = {
  bethesdaCategory: ['NILM', 'ASCUS', 'ASC-US', 'ASC-H', 'LSIL', 'HSIL', 'AGC', 'AIS', 'SCC', 'Adenocarcinoma', 'Unsatisfactory'],
};
...
/* ═══════ COMPONENT ═══════ */
const CytologyReportsDocument = ({ document: docProp }) => {
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        const displayVal = f === 'date' ? formatDate(val) : fmtVal(val);
        if (displayVal.toLowerCase().includes(phrase)) return true;
      }
    }
    return false;
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) {
      ...
      return displayVal.toLowerCase().includes(phrase);
    }
    return false;
  }, [searchTerm, getFieldValue, fmtVal, formatDate]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Cytology Report ${idx + 1}`.toLowerCase();
  ...
  function stageFieldDraft(record, fn, idx, sid, fullText, applyMarkers) {
    const id = safeId(record); if (!id) return;
    const editKey = `${fn}-${idx}`;
    setSaveError(null);
    setLocalEdits(prev => ({ ...prev, [editKey]: fullText }));
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
    if (applyMarkers) applyMarkers();
    if (sid) setApprovedSections(prev => {
      const k = `${sid}-${idx}`;
      if (!prev[k]) return prev;
      const next = { ...prev }; delete next[k]; return next;
  ...
  function saveCommaItemInSentence(record, fn, idx, sIdx, commaIdx, newItemText) {
    ...
    stageFieldDraft(record, fn, idx, undefined, fullText, () => {
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // localEdits keys are whole-field "field-idx"; commit each pending one for this section's fields.
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/cytology_reports/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/cytology_reports/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[CytologyReports] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    return text;
  }, [getFieldValue, hasVal, fmtVal, formatDate, formatSentenceFieldLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== CYTOLOGY REPORTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Cytology Report ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  const renderDateField = (record, fn, idx, sid, title) => {
    ...
    return (
      <div key={fn} className={sl ? 'rec-mini-card' : ''}>
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(dateInputVal); setSaveError(null); } }}>
          {isEditing ? (
      ...
      <div key={fn}>
        <div className="rec-mini-card">
          ...
          {sentences.map((sentence, sIdx) => {
            ...
            /* Labeled (but <3 comma items) → sub-label + single value row; unlabeled → plain row (never inline "label: value") */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="cytology-reports-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Cytology Reports</h2></div>
        <div className="empty-state">No cytology report records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="cytology-reports-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Cytology Reports</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<CytologyReportsDocumentPDFTemplate document={pdfData} />} fileName="Cytology_Reports.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search cytology reports..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          <div key={idx} className="record-card">
            <div className="record-header">
              <div className="record-title-row">
                <h3 className="record-name">{highlightText(`Cytology Report ${idx + 1}`)}</h3>
              </div>
            </div>
            {renderSection(record, idx, 'report-info')}
            {renderSection(record, idx, 'specimen-analysis')}
...
export default CytologyReportsDocument;
