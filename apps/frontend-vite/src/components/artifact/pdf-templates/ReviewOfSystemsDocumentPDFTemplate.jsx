...
const SECTIONS = [
  ...
  { id: 'systems', title: 'Review Of Systems', fields: ['constitutional', 'heent', 'eyes', 'ent', 'cardiovascular', 'respiratory', 'gastrointestinal', 'genitourinary', 'musculoskeletal', 'neurological', 'psychiatric', 'endocrine', 'hematologic', 'skin', 'sleepSymptoms'] },
...
export default function ReviewOfSystemsDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Review Of Systems</Text></View>
    {!records.length && <Text style={styles.noDataText}>No review of systems data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Review Of Systems {recordIndex + 1}</Text></View>
