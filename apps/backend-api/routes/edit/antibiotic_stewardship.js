...
function buildContext(req, operation = 'read') { return { serviceId: 'antibiotic-stewardship-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['antibioticName', 'antibioticClass', 'indicationForUse', 'dosage', 'routeOfAdministration', 'durationOfTherapy', 'cultureSource', 'cultureSensitivity', 'microbiologyResult', 'prescribingPhysician', 'stewardshipReviewer', 'interventionType', 'interventionAccepted', 'deEscalationPerformed', 'biomarkerValues', 'clinicalImprovement', 'adverseDrugReaction', 'therapeuticDrugMonitoring', 'renalAdjustmentRequired', 'creatinineClearance', 'restrictedAntibiotic', 'empiricTherapy', 'prophylacticUse', 'timeToAppropriateTherapy', 'costSavingsEstimate'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('antibiotic_stewardship', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[AntibioticStewardship] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('antibiotic_stewardship', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[AntibioticStewardship] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
