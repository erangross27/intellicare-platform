/**
 * ExcessiveGlucoseMonitoringDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: excessive_glucose_monitoring
 *
 * 7 Sections:
 *   1. session-info: date (date picker), provider, facility
 *   2. monitoring-details: monitoringFrequency (number), prescribedFrequency (number), excessCheckCount (number), diabetesType, insulinRegimen
 *   3. clinical-indicators: hemoglobinA1c (number), glucoseVariability, hypoglycemiaEpisodes (number), anxietyScreeningScore (number)
 *   4. monitoring-resources: testStripUtilization (number), continuousGlucoseMonitor, monitoringTriggers (array), costImpact (number)
 *   5. education-evaluation: diabetesEducationDate (date), psychologicalEvaluation (sentence)
 *   6. intervention-plan: interventionRecommended (sentence), alternativeMonitoringOffered, fingerInjuryPresent
 *   7. support-coverage: caregiverInfluence, insuranceCoverage
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ExcessiveGlucoseMonitoringDocumentPDFTemplate from '../pdf-templates/ExcessiveGlucoseMonitoringDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import './ExcessiveGlucoseMonitoringDocument.css';
...
const FIELD_LABELS = {
  ...
  prescribedFrequency: 'Prescribed Frequency',
  excessCheckCount: 'Excess Check Count',
  ...
  anxietyScreeningScore: 'Anxiety Screening Score',
  testStripUtilization: 'Test Strip Utilization',
  continuousGlucoseMonitor: 'Continuous Glucose Monitor',
  monitoringTriggers: 'Monitoring Triggers',
  costImpact: 'Cost Impact',
  diabetesEducationDate: 'Diabetes Education Date',
  psychologicalEvaluation: 'Psychological Evaluation',
  interventionRecommended: 'Intervention Recommended',
  alternativeMonitoringOffered: 'Alternative Monitoring Offered',
  fingerInjuryPresent: 'Finger Injury Present',
  caregiverInfluence: 'Caregiver Influence',
  insuranceCoverage: 'Insurance Coverage',
};

const SECTION_FIELDS = {
  'session-info': ['date', 'provider', 'facility'],
  'monitoring-details': ['monitoringFrequency', 'prescribedFrequency', 'excessCheckCount', 'diabetesType', 'insulinRegimen'],
  'clinical-indicators': ['hemoglobinA1c', 'glucoseVariability', 'hypoglycemiaEpisodes', 'anxietyScreeningScore'],
  'monitoring-resources': ['testStripUtilization', 'continuousGlucoseMonitor', 'monitoringTriggers', 'costImpact'],
  'education-evaluation': ['diabetesEducationDate', 'psychologicalEvaluation'],
  'intervention-plan': ['interventionRecommended', 'alternativeMonitoringOffered', 'fingerInjuryPresent'],
  'support-coverage': ['caregiverInfluence', 'insuranceCoverage'],
};

const NUMBER_FIELDS = ['monitoringFrequency', 'prescribedFrequency', 'excessCheckCount', 'hemoglobinA1c', 'hypoglycemiaEpisodes', 'anxietyScreeningScore', 'testStripUtilization', 'costImpact'];
const DATE_FIELDS = ['date', 'diabetesEducationDate'];
const SENTENCE_FIELDS = ['psychologicalEvaluation', 'interventionRecommended'];
const ARRAY_FIELDS = ['monitoringTriggers'];
/* Fields where a stored 0 means "not assessed" (implausible as a true zero for an over-tester with
   documented anxiety): test-strip utilization, cost impact, anxiety screening score → hidden when 0. */
const ZERO_SENTINEL_FIELDS = ['anxietyScreeningScore', 'testStripUtilization', 'costImpact'];

/* Copy dividers: EQ (====) under section/record titles, DASH (----) under EVERY field label + sub-category. */
const COPY_LINE_EQ = '='.repeat(40);
const COPY_LINE_DASH = '-'.repeat(40);

/* stepFor: decimal-aware step for the −/+ number stepper (9.1 → 0.1, 8 → 1). */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits field key, e.g. "monitoringFrequency") */
const DRAFT_KEY = 'excessive_glucose_monitoringPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const ExcessiveGlucoseMonitoringDocument = ({ document: docProp }) => {
  ...
  // Drafts are keyed by record id (handling _id.$oid) → { [fieldPart]: value }, where fieldPart is the
  // localEdits field key ("monitoringFrequency", or the whole-array value under "monitoringTriggers").
  useEffect(() => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Excessive Glucose Monitoring ${idx + 1}`.toLowerCase();
  ...
  // Save array item = stage a DRAFT (full updated array) locally + localStorage. No DB write until Approve.
  const handleSaveArrayItem = useCallback((record, fn, idx, arrIdx, value) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    const localKey = `${fn}-${idx}`;
    const badgeKey = `${fn}.${arrIdx}-${idx}`;
    ...
    setEditedFields(prev => ({ ...prev, [badgeKey]: 'edited' }));
  ...
  const stageSentenceDraft = (record, fn, idx, fullText) => {
    const id = safeId(record); if (!id) return;
    const localKey = `${fn}-${idx}`;
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Pending localEdits keys for THIS record's fields in THIS section ("field-idx").
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/excessive_glucose_monitoring/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/excessive_glucose_monitoring/${id}/approve`, { sectionId: sid, approved: true });
      ...
      const store = readDrafts();
      if (store[id]) {
        toCommit.forEach(k => { const fp = k.slice(0, -suffix.length); const d = fp.lastIndexOf('.'); const base = (d !== -1 && /^\d+$/.test(fp.slice(d + 1))) ? fp.slice(0, d) : fp; delete store[id][base]; });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== EXCESSIVE GLUCOSE MONITORING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Excessive Glucose Monitoring ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                ...
                <input type="text" inputMode="decimal" className="edit-number" value={editValue} autoFocus onClick={e => e.stopPropagation()} onChange={e => setEditValue(e.target.value)} onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter') { e.preventDefault(); handleSaveField(record, fn, idx, sid); } }} />
                <button type="button" className="num-step" disabled={saving} onClick={e => { e.stopPropagation(); const st = parseFloat(stepFor(editValue)) || 1; const dec = (String(st).split('.')[1] || '').length; const cur = parseFloat(editValue); setEditValue(Math.max(0, (isNaN(cur) ? 0 : cur) + st).toFixed(dec)); }}>+</button>
              </div>
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                          ...
                          <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                            {ciEditing ? (
                              <div className="edit-field-container">
                                ...
                                {saveError && <div className="save-error">{saveError}</div>}
                                <div className="edit-actions">
                                  <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const currentVal2 = String(getFieldValue(record, fn, idx) || ''); const sentences2 = splitBySentence(currentVal2); const items2 = splitByComma(sentences2[sIdx] || ''); const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { items2.splice(ciIdx, 1, ...subParts); } else { items2[ciIdx] = trimmed.replace(/[;.]+$/, '').trim(); } const rebuilt = items2.join(', '); const allS = [...sentences2]; allS[sIdx] = rebuilt; const fullText2 = reconstructFullText(allS); setSaveError(null); stageSentenceDraft(record, fn, idx, fullText2); const marks = { [commaKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ciIdx + ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                                  <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                                </div>
                              </div>
                            ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="excessive-glucose-monitoring-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Excessive Glucose Monitoring</h2></div>
        <div className="empty-state">No excessive glucose monitoring records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="excessive-glucose-monitoring-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Excessive Glucose Monitoring</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ExcessiveGlucoseMonitoringDocumentPDFTemplate document={pdfData} />} fileName="Excessive_Glucose_Monitoring.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search excessive glucose monitoring..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Excessive Glucose Monitoring ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'monitoring-details')}
            {renderSection(record, idx, 'clinical-indicators')}
            {renderSection(record, idx, 'monitoring-resources')}
            {renderSection(record, idx, 'education-evaluation')}
            {renderSection(record, idx, 'intervention-plan')}
            {renderSection(record, idx, 'support-coverage')}
          </div>
        ))}
      </div>
    </div>
...
export default ExcessiveGlucoseMonitoringDocument;
