/**
 * SportsNutritionPlanDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: sports_nutrition_plan (displayed as "Sports Nutrition Plan")
 *
 * 8 Sections:
 *   1. record-info: date, prescribingDietitian, athleteSportDiscipline, competitionLevel
 *   2. training-goals: trainingPhase, bodyCompositionGoal
 *   3. body-composition: currentBodyWeight, targetBodyWeight, bodyFatPercentage, leanBodyMass, basalMetabolicRate, totalDailyEnergyExpenditure
 *   4. macronutrients: dailyCaloricTarget, macronutrientProteinGrams, macronutrientCarbohydrateGrams, macronutrientFatGrams
 *   5. hydration: hydrationTargetLiters, sweatRateMillilitersPerHour
 *   6. nutrition-timing: preWorkoutNutritionProtocol, intraWorkoutNutritionProtocol, postWorkoutNutritionProtocol
 *   7. supplements: supplementationRegimen
 *   8. restrictions: dietaryRestrictions, foodAllergies
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import SportsNutritionPlanDocumentPDFTemplate from '../pdf-templates/SportsNutritionPlanDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './SportsNutritionPlanDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field"; value may be array for array fields) */
const DRAFT_KEY = 'sports_nutrition_planPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'hydration': 'Hydration',
  'nutrition-timing': 'Nutrition Timing',
...
const FIELD_LABELS = {
  date: 'Date',
  prescribingDietitian: 'Prescribing Dietitian',
  athleteSportDiscipline: 'Athlete Sport / Discipline',
  competitionLevel: 'Competition Level',
  trainingPhase: 'Training Phase',
  bodyCompositionGoal: 'Body Composition Goal',
  currentBodyWeight: 'Current Body Weight',
  targetBodyWeight: 'Target Body Weight',
  bodyFatPercentage: 'Body Fat Percentage',
  leanBodyMass: 'Lean Body Mass',
  basalMetabolicRate: 'Basal Metabolic Rate',
  totalDailyEnergyExpenditure: 'Total Daily Energy Expenditure',
  dailyCaloricTarget: 'Daily Caloric Target',
  macronutrientProteinGrams: 'Protein (g)',
  macronutrientCarbohydrateGrams: 'Carbohydrate (g)',
  macronutrientFatGrams: 'Fat (g)',
  hydrationTargetLiters: 'Hydration Target (L)',
  sweatRateMillilitersPerHour: 'Sweat Rate (mL/hr)',
  mealFrequencyPerDay: 'Meal Frequency Per Day',
  nutritionPlanDurationWeeks: 'Nutrition Plan Duration (weeks)',
  preWorkoutNutritionProtocol: 'Pre-Workout Nutrition Protocol',
  intraWorkoutNutritionProtocol: 'Intra-Workout Nutrition Protocol',
  postWorkoutNutritionProtocol: 'Post-Workout Nutrition Protocol',
  supplementationRegimen: 'Supplementation Regimen',
  dietaryRestrictions: 'Dietary Restrictions',
  foodAllergies: 'Food Allergies',
};

const SECTION_FIELDS = {
  'record-info': ['date', 'prescribingDietitian', 'athleteSportDiscipline', 'competitionLevel'],
  'training-goals': ['trainingPhase', 'bodyCompositionGoal'],
  'body-composition': ['currentBodyWeight', 'targetBodyWeight', 'bodyFatPercentage', 'leanBodyMass', 'basalMetabolicRate', 'totalDailyEnergyExpenditure'],
  'macronutrients': ['dailyCaloricTarget', 'macronutrientProteinGrams', 'macronutrientCarbohydrateGrams', 'macronutrientFatGrams'],
  'hydration': ['hydrationTargetLiters', 'sweatRateMillilitersPerHour', 'mealFrequencyPerDay', 'nutritionPlanDurationWeeks'],
  'nutrition-timing': ['preWorkoutNutritionProtocol', 'intraWorkoutNutritionProtocol', 'postWorkoutNutritionProtocol'],
  'supplements': ['supplementationRegimen'],
  'restrictions': ['dietaryRestrictions', 'foodAllergies'],
};

const NUMBER_FIELDS = ['currentBodyWeight', 'targetBodyWeight', 'bodyFatPercentage', 'leanBodyMass', 'basalMetabolicRate', 'totalDailyEnergyExpenditure', 'dailyCaloricTarget', 'macronutrientProteinGrams', 'macronutrientCarbohydrateGrams', 'macronutrientFatGrams', 'hydrationTargetLiters', 'sweatRateMillilitersPerHour', 'mealFrequencyPerDay', 'nutritionPlanDurationWeeks'];
const DATE_FIELDS = ['date'];
const ARRAY_FIELDS = ['supplementationRegimen', 'dietaryRestrictions', 'foodAllergies'];
const STRING_FIELDS = ['athleteSportDiscipline', 'competitionLevel', 'trainingPhase', 'bodyCompositionGoal', 'preWorkoutNutritionProtocol', 'intraWorkoutNutritionProtocol', 'postWorkoutNutritionProtocol', 'prescribingDietitian'];
...
/* ═══════ COMPONENT ═══════ */
const SportsNutritionPlanDocument = ({ document: docProp }) => {
  ...
  const safeIdEarly = (r) => { if (!r?._id) return null; if (typeof r._id === 'string') return r._id; if (r._id.$oid) return r._id.$oid; return String(r._id); };
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Sports Nutrition Plan ${idx + 1}`.toLowerCase();
  ...
  // Re-editing after approval drops the section's approved flag so the button returns to yellow Pending.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Pending drafts for THIS section's fields, keyed `${fn}-${idx}`
      const toCommit = Object.keys(localEdits).filter(k =>
        pendingEdits[k] && k.endsWith(suffix) && fields.includes(k.slice(0, -suffix.length))
      );
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "field" (no dotted-index keys in localEdits)
        const value = localEdits[editKey];
        if (Array.isArray(value)) {
          // Array field: persist each element with its arrayIndex
          for (let ai = 0; ai < value.length; ai++) {
            const resp = await secureApiClient.put(`/api/edit/sports_nutrition_plan/${id}/edit`, { field: fieldPart, value: value[ai], arrayIndex: ai });
            if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
          }
        } else {
          const resp = await secureApiClient.put(`/api/edit/sports_nutrition_plan/${id}/edit`, { field: fieldPart, value });
          if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        }
      }
      // Flag the record/section approved (audit trail)
      await secureApiClient.put(`/api/edit/sports_nutrition_plan/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      const val = getFieldValue(record, f, idx);
      if (!hasVal(val) && !(NUMBER_FIELDS.includes(f) && val === 0)) return;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== SPORTS NUTRITION PLAN ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Sports Nutrition Plan ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: ARRAY FIELD (pre-split, per-item editing with dot-path keys) ═══════ */
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* Multi-sentence: render with splitBySentence / splitBySemicolon */
    if (sentences.length > 1) {
        ...
        <div key={fn}>
          <div className="rec-mini-card nested-mini-card">
            ...
            {sentences.map((sentence, sIdx) => {
              ...
              return (
                <div key={sIdx} className="rec-mini-card nested-mini-card" data-edit-field={fn} style={{ marginTop: 8 }}>
          ...
          <div className="rec-mini-card">
            ...
            {singleCommaItems.map((ci, ciIdx) => {
              ...
              if (ciParsed.isLabeled) {
                ...
                return (
                  <div key={ciIdx} className="rec-mini-card nested-mini-card" data-edit-field={fn} style={{ marginTop: 8 }}>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="sports-nutrition-plan-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Sports Nutrition Plan</h2></div>
        <div className="empty-state">No sports nutrition plan data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="sports-nutrition-plan-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Sports Nutrition Plan</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<SportsNutritionPlanDocumentPDFTemplate document={pdfData} />} fileName="Sports_Nutrition_Plan.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search sports nutrition plan..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Sports Nutrition Plan ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'record-info')}
            {renderSection(record, idx, 'training-goals')}
            {renderSection(record, idx, 'body-composition')}
            {renderSection(record, idx, 'macronutrients')}
            {renderSection(record, idx, 'hydration')}
            {renderSection(record, idx, 'nutrition-timing')}
...
export default SportsNutritionPlanDocument;
