...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'spinal-manipulation-record-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'spinalSegmentsTreated',
  'manipulationTechnique',
  'thrustDirection',
  'patientPositioning',
  'forceAmplitude',
  'vertebralSubluxationComplex',
  'preManipulationRomCervical',
  'postManipulationRomCervical',
  'preManipulationRomLumbar',
  'postManipulationRomLumbar',
  'painVasPreTreatment',
  'painVasPostTreatment',
  'oswestryDisabilityIndex',
  'neckDisabilityIndex',
  'palpationFindings',
  'sacroiliacJointDysfunction',
  'straightLegRaiseTest',
  'neurologicalClearance',
  'dermatomeInvolvement',
  'vertebrobasilarScreening',
  'contraindicationsScreened',
  'cavitationAchieved',
  'muscleEnergyTechniqueApplied',
  'adverseReactionDocumented',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('spinal_manipulation_record', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[spinal_manipulation_record] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('spinal_manipulation_record', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[spinal_manipulation_record] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
