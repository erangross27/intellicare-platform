/**
 * HydrationManagementDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: hydration_management (displayed as "Hydration Management")
 *
 * 6 Sections:
 *   1. fluid-balance: fluidIntakeVolume24h, fluidOutputVolume24h, netFluidBalance, urineOutput
 *   2. lab-values: serumSodium, serumOsmolality, bloodUreaNitrogen, serumCreatinine, bunCreatinineRatio, specificGravity, estimatedGlomerularFiltrationRate
 *   3. physical-assessment: skinTurgorAssessment, mucousMembraneAssessment, peripheralEdemaGrade, centralVenousPressure, jugularVenousDistension, pulmonaryEdemaPresent
 *   4. hydration-status: hydrationStatus
 *   5. fluid-therapy: intravenousFluidType, intravenousFluidRate, diureticTherapy
 *   6. restrictions-electrolytes: fluidRestrictionOrdered, fluidRestrictionAmount, hyponatremiaPresent, hypernatremiaPresent
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HydrationManagementDocumentPDFTemplate from '../pdf-templates/HydrationManagementDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './HydrationManagementDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'hydration_managementPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'restrictions-electrolytes': 'Restrictions & Electrolytes',
};

const FIELD_LABELS = {
  fluidIntakeVolume24h: 'Fluid Intake Volume (24h)',
  fluidOutputVolume24h: 'Fluid Output Volume (24h)',
  netFluidBalance: 'Net Fluid Balance',
  urineOutput: 'Urine Output',
  serumSodium: 'Serum Sodium',
  ...
  skinTurgorAssessment: 'Skin Turgor Assessment',
  mucousMembraneAssessment: 'Mucous Membrane Assessment',
  peripheralEdemaGrade: 'Peripheral Edema Grade',
  centralVenousPressure: 'Central Venous Pressure',
  jugularVenousDistension: 'Jugular Venous Distension',
  pulmonaryEdemaPresent: 'Pulmonary Edema Present',
  hydrationStatus: 'Hydration Status',
  intravenousFluidType: 'Intravenous Fluid Type',
  intravenousFluidRate: 'Intravenous Fluid Rate',
  diureticTherapy: 'Diuretic Therapy',
  fluidRestrictionOrdered: 'Fluid Restriction Ordered',
  fluidRestrictionAmount: 'Fluid Restriction Amount',
  hyponatremiaPresent: 'Hyponatremia Present',
  hypernatremiaPresent: 'Hypernatremia Present',
};

const SECTION_FIELDS = {
  'fluid-balance': ['fluidIntakeVolume24h', 'fluidOutputVolume24h', 'netFluidBalance', 'urineOutput'],
  'lab-values': ['serumSodium', 'serumOsmolality', 'bloodUreaNitrogen', 'serumCreatinine', 'bunCreatinineRatio', 'specificGravity', 'estimatedGlomerularFiltrationRate'],
  'physical-assessment': ['skinTurgorAssessment', 'mucousMembraneAssessment', 'peripheralEdemaGrade', 'centralVenousPressure', 'jugularVenousDistension', 'pulmonaryEdemaPresent'],
  'hydration-status': ['hydrationStatus'],
  'fluid-therapy': ['intravenousFluidType', 'intravenousFluidRate', 'diureticTherapy'],
  'restrictions-electrolytes': ['fluidRestrictionOrdered', 'fluidRestrictionAmount', 'hyponatremiaPresent', 'hypernatremiaPresent'],
};

const NUMBER_FIELDS = ['fluidIntakeVolume24h', 'fluidOutputVolume24h', 'netFluidBalance', 'urineOutput', 'serumSodium', 'serumOsmolality', 'bloodUreaNitrogen', 'serumCreatinine', 'bunCreatinineRatio', 'specificGravity', 'centralVenousPressure', 'fluidRestrictionAmount', 'intravenousFluidRate', 'estimatedGlomerularFiltrationRate'];
const DATE_FIELDS = [];
const ARRAY_FIELDS = ['intravenousFluidType', 'diureticTherapy'];
const BOOLEAN_FIELDS = ['jugularVenousDistension', 'pulmonaryEdemaPresent', 'fluidRestrictionOrdered', 'hyponatremiaPresent', 'hypernatremiaPresent'];
const STRING_FIELDS = ['skinTurgorAssessment', 'mucousMembraneAssessment', 'peripheralEdemaGrade', 'hydrationStatus'];
...
/* ═══════ COMPONENT ═══════ */
const HydrationManagementDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.hydration_management) return Array.isArray(r.hydration_management) ? r.hydration_management : [r.hydration_management];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.hydration_management) return Array.isArray(dd.hydration_management) ? dd.hydration_management : [dd.hydration_management]; return [dd]; }
      return [r];
    });
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayElem) {
          ...
          // merge the staged element into the (possibly already-merged) full array under "field-idx"
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Hydration Management ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // pending editKeys for THIS record+section: "<field>-<idx>" or "<field>.<arrayIndex>-<idx>"
      ...
      for (const editKey of toCommit) {
        ...
        if (isArrayElem) {
          ...
          const resp = await secureApiClient.put(`/api/edit/hydration_management/${id}/edit`, { field: baseField, value, arrayIndex: arrIndex });
          if (!resp || resp.success === false) throw new Error((resp && resp.error) || 'save failed');
        } else {
          const resp = await secureApiClient.put(`/api/edit/hydration_management/${id}/edit`, { field: fieldPart, value: localEdits[`${fieldPart}-${idx}`] });
          if (!resp || resp.success === false) throw new Error((resp && resp.error) || 'save failed');
        }
      }
      await secureApiClient.put(`/api/edit/hydration_management/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (store[id]) {
        Object.keys(store[id]).forEach(fieldPart => {
          const lastDot = fieldPart.lastIndexOf('.');
          const baseField = (lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1))) ? fieldPart.slice(0, lastDot) : fieldPart;
          if (fields.includes(baseField)) delete store[id][fieldPart];
        });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HYDRATION MANAGEMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Hydration Management ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD — num-stepper (text input + −/+); renders every number incl 0 ═══════ */
  ...
  /* ═══════ RENDER: DATE FIELD — BlueDatePicker (canonical; DATE_FIELDS empty here) ═══════ */
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* Single-value string: simple editable via saveSentence */
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
    const isModified = editedFields[editKey];
  ...
  if (!records || records.length === 0) {
    return (
      <div className="hydration-management-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Hydration Management</h2></div>
        <div className="empty-state">No hydration management records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="hydration-management-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Hydration Management</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HydrationManagementDocumentPDFTemplate document={pdfData} />} fileName="Hydration_Management.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search hydration management..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Hydration Management ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'fluid-balance')}
            {renderSection(record, idx, 'lab-values')}
            {renderSection(record, idx, 'physical-assessment')}
            {renderSection(record, idx, 'hydration-status')}
            {renderSection(record, idx, 'fluid-therapy')}
            {renderSection(record, idx, 'restrictions-electrolytes')}
          </div>
        ))}
      </div>
    </div>
...
export default HydrationManagementDocument;
