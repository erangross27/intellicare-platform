/**
 * FluidIntakeDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: fluid_intake
 *
 * 6 Sections:
 *   1. session-info: date (date picker), provider (string), facility (string), status (string)
 *   2. fluid-details: iv (sentence), enteral (string), oral (string), bloodProducts (string), other (string), total (string)
...
import FluidIntakeDocumentPDFTemplate from '../pdf-templates/FluidIntakeDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './FluidIntakeDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the field name, e.g. "iv" or "results") */
const DRAFT_KEY = 'fluid_intakePendingEdits';
const readDrafts = () => {
...
const NUMBER_LIKE_FIELDS = ['iv', 'enteral', 'oral', 'bloodProducts', 'other', 'total'];

const STRING_FIELDS = ['provider', 'facility', 'status', 'iv', 'enteral', 'oral', 'bloodProducts', 'other', 'total', 'assessment', 'plan', 'findings', 'notes'];
...
/* splitNumberUnit: "1800" -> {num:'1800',sep:'',unit:''}; "0.55 U/hr" -> {num:'0.55',sep:' ',unit:'U/hr'};
   "7,902 mL LR" -> {num:'7902',sep:' ',unit:'mL LR'} (thousands stripped); null if not leading-numeric / a ratio */
...
/* ═══════ COMPONENT ═══════ */
const FluidIntakeDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.fluid_intake) return Array.isArray(r.fluid_intake) ? r.fluid_intake : [r.fluid_intake];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.fluid_intake) return Array.isArray(dd.fluid_intake) ? dd.fluid_intake : [dd.fluid_intake]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Fluid Intake ${idx + 1}`.toLowerCase();
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    // Stage a DRAFT (no DB write). Mutate a clone of the whole object; Approve commits it.
    setLocalEdits(prev => {
  ...
  const stageFieldDraft = (id, fn, idx, sid, fullText, sentenceMarks) => {
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageFieldDraft(id, fn, idx, sid, fullText, { [`${fn}-${idx}-s${sentenceIdx}`]: 'edited' });
      return;
    }
    ...
    stageFieldDraft(id, fn, idx, sid, fullText, marks);
  }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // localEdits keys are `${field}-${idx}` (whole field, incl. object/array fields stored whole).
    ...
    try {
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "field" (no numeric dotted arrayIndex in this template)
        ...
        await secureApiClient.put(`/api/edit/fluid_intake/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/fluid_intake/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[FluidIntake] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const copyAllText = useCallback(async () => {
    let text = `Fluid Intake\n${COPY_LINE_EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Fluid Intake ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="fluid-intake-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Fluid Intake</h2></div>
        <div className="empty-state">No fluid intake records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="fluid-intake-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Fluid Intake</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FluidIntakeDocumentPDFTemplate document={pdfData} />} fileName="Fluid_Intake.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search fluid intake..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">Fluid Intake {idx + 1}</h3>
            </div>
...
export default FluidIntakeDocument;
