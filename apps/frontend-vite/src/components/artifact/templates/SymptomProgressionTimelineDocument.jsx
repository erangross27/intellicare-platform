 ...
 *
 * 5 Sections:
 *   1. symptom-overview: symptomOnsetDate, symptomCharacteristics, anatomicalLocation, symptomFrequency, symptomDurationMinutes, symptomProgressionPattern, timeToMaximumSymptom, symptomResolutionDate
 *   2. severity-scores: symptomSeverityScore, painIntensityNrs, functionalCapacityMets, functionalImpactScore, qualityOfLifeScore, nyhaClassification, glasgowComaScale, edssScore
 *   3. factors-symptoms: triggeringFactors, alleviatingFactors, associatedSymptoms
 *   4. functional-measures: walkingDistanceMeters, sixMinuteWalkDistance
 *   5. intervention-biomarkers: interventionResponse, biomarkerLevels
 */
...
const DRAFT_KEY = 'symptom_progression_timelinePendingEdits';
const readDrafts = () => {
...
const SymptomProgressionTimelineDocument = ({ document: docProp }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        const fieldName = isArrayElem ? fieldPart.slice(0, dotIdx) : fieldPart;
        const localKey = `${fieldName}-${idx}`;
        nPending[localKey] = true;
        if (isArrayElem) {
          // Merge the staged element into the record's array under the whole-field localEdits key.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Each staged field for this record+section lives in localEdits under "field-idx" (whole value/array).
      ...
      for (const localKey of toCommit) {
        const fn = localKey.slice(0, localKey.lastIndexOf('-'));
        // GOTCHA: arrayIndex applies ONLY when the trailing dot-segment is purely numeric. localEdits keys
        // here are never dotted (the whole array is stored under "field-idx"), so we always send field+value.
        const dotIdx = fn.lastIndexOf('.');
        const payload = (dotIdx !== -1 && /^\d+$/.test(fn.slice(dotIdx + 1)))
          ? { field: fn.slice(0, dotIdx), value: localEdits[localKey], arrayIndex: parseInt(fn.slice(dotIdx + 1), 10) }
          : { field: fn, value: localEdits[localKey] };
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: currentArr })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); const store = readDrafts(); if (!store[id]) store[id] = {}; store[id][`${fn}.${itemIdx}`] = editValue; writeDrafts(store); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
    ...
    <div className="symptom-progression-timeline-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<SymptomProgressionTimelineDocumentPDFTemplate document={pdfData} />} fileName={`symptom-progression-timeline-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              {hasVal(record.symptomOnsetDate) && (
                <div className="record-meta-row">
                  <span className="record-date">{formatDate(record.symptomOnsetDate)}</span>
                </div>
              )}
              <h3 className="record-name">{highlightText(record.symptomCharacteristics || `Symptom Progression Timeline ${idx + 1}`)}</h3>
            </div>
