/**
 * ImmunizationScheduleDocument.jsx
 * June 2026 — Inline editing, blue glow theme
 * Collection: immunization_schedule
 *
 * 6 Sections:
 *   1. vaccine-details: vaccineName, vaccineManufacturer, lotNumber, expirationDate
 *   2. administration: date, administrationDate, scheduledDate, doseNumber, dosageAmount, routeOfAdministration, anatomicalSite
 *   3. provider-facility: administeringProvider, facilityName
 *   4. codes-documentation: cptCode, cvxCode, visDate, visVersion
 *   5. consent-safety: consentObtained, adverseReactionReported, adverseReactionDetails, contraindications[], vaccineRefused, refusalReason
 *   6. series-status: immunizationSeriesComplete, nextDueDate, immunityVerified
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ImmunizationScheduleDocumentPDFTemplate from '../pdf-templates/ImmunizationScheduleDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './ImmunizationScheduleDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" — never dotted here) */
const DRAFT_KEY = 'immunization_schedulePendingEdits';
const readDrafts = () => {
...
const SIMPLE_FIELDS = ['vaccineName', 'vaccineManufacturer', 'lotNumber', 'dosageAmount', 'routeOfAdministration', 'anatomicalSite', 'administeringProvider', 'facilityName', 'cptCode', 'cvxCode', 'visVersion'];
...
/* Date-typed fields rendered via renderDateField (native date-picker; save appends T00:00:00.000Z
   to match the ISO-string shape in real data). formatDate skips null/empty gracefully —
   scheduledDate/visDate/nextDueDate are null in real data. */
const DATE_FIELDS = ['date', 'expirationDate', 'administrationDate', 'scheduledDate', 'visDate', 'nextDueDate'];
/* doseNumber is numeric — renderNumberField saves a REAL number (Number + isNaN guard) so the
   {…}-shaped DB type is never corrupted by a free-text string. */
const NUMBER_FIELDS = ['doseNumber'];
/* doseNumber 0 is invalid (doses are 1-based) — hide zeros (4-AREA RULE) */
const HIDE_ZERO_FIELDS = ['doseNumber'];

/* parseLabel: detect "Label: value" patterns (skip subordinate-clause openers so a grammatical
   colon like "If chest pain recurs: ..." is NOT mis-split into a Label:Value subtitle) */
...
/* splitByComma: parenthesis-aware comma split with thousands guard
   (only split on a comma FOLLOWED BY whitespace → "$18,000" / "1,000 mg" stay intact) */
...
/* ═══════ COMPONENT ═══════ */
const ImmunizationScheduleDocument = ({ document: docProp, data, templateData }) => {
  ...
  /* ═══════ SEARCH — doc/record titles, section titles, labels, 'Label: Value' entries ═══════ */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      const docTitle = 'immunization schedule';
      const rt = `immunization ${oi + 1}`;
      ...
      /* Level 3: field labels + 'Label: Value' values */
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* stageDraft: stage a Save as a DRAFT locally + persist to the pending-drafts localStorage store
     (survives refresh). NOT written to MongoDB and NOT shown in the PDF until the user clicks
     Approve (handleApproveSection commits). fieldPart is the field name ("field"); localEdits keys
     are always "field-idx" here — never dotted — so Approve never adds an arrayIndex. */
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    setSaveError(null);
    // Stage as a DRAFT (no DB write). Approve commits it to MongoDB + flows it into the PDF.
  ...
  /* saveSentence: per-sentence save with reconstructFullText period-restoration
     (used by BOTH the Save button and Ctrl+Enter) */
  ...
  /* saveLabeledSentence: rebuild "Label: value" sentence then reconstructFullText
     (shared by Save button and Ctrl+Enter) */
  function saveLabeledSentence(record, fn, idx, sid, sIdx, parsed, isSemicolon, sentenceKey) {
    const id = safeId(record); if (!id) return;
    const trimmed = editValue.trim();
    const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp);
  ...
  /* saveSingleLabeledComma / saveSingleComma: single-value comma-item saves
     (shared by Save button and Ctrl+Enter) */
  function saveSingleLabeledComma(record, fn, idx, sid, ciIdx, ciLabel, commaKey) {
    const id = safeId(record); if (!id) return;
    const allItems = splitByComma(String(getFieldValue(record, fn, idx) || ''));
    allItems[ciIdx] = `${ciLabel}: ${editValue.trim()}`;
  ...
  function saveSingleComma(record, fn, idx, sid, ciIdx, commaKey) {
    const id = safeId(record); if (!id) return;
    const allItems = splitByComma(String(getFieldValue(record, fn, idx) || ''));
  ...
  /* saveArrayItem: per-item array save via arrayIndex (shared by Save button and Ctrl+Enter) */
  function saveArrayItem(record, fn, idx, sid, itemIdx) {
    const id = safeId(record); if (!id) return;
    const editKey = `${fn}.${itemIdx}-${idx}`;
    setSaveError(null);
    // Stage the FULL updated array as a DRAFT (no DB write). Approve PUTs the whole array
    // (no arrayIndex needed — the edit endpoint accepts a full-array value).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        const fieldPart = editKey.replace(/-(\d+)$/, ''); // "field" (never dotted in this template)
        const dotIdx = fieldPart.lastIndexOf('.');
        // arrayIndex ONLY when the trailing dot-segment is purely numeric (GOTCHA)
        ...
        const resp = await secureApiClient.put(`/api/edit/immunization_schedule/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/immunization_schedule/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[ImmunizationSchedule] Approve error:', err); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (ARRAY_FIELDS.includes(f)) {
        ...
        /* scalars — string / date / number / boolean — label on its OWN line, value numbered
           (never "Label: value" side-by-side; fmtFieldVal renders dates long + booleans Yes/No) */
        body += `${head}1. ${fmtFieldVal(f, val)}\n\n`;
      }
    });
    if (!body.trim()) return '';
    return `${title}\n${'='.repeat(40)}\n\n${body}`;
  }, [getFieldValue, hasFieldVal, fmtVal, fmtFieldVal, splitBySentence, formatSentenceFieldLines, arrItemText]);

  const copyAllText = useCallback(async () => {
    let text = '=== IMMUNIZATION SCHEDULE ===\n\n';
    pdfData.forEach((r) => {
      const oi = r._originalIdx ?? 0;
      text += `Immunization ${oi + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: SIMPLE FIELD (no splitting — dates, codes, names; vaccineName parentheses stay intact) ═══════ */
  ...
  /* ═══════ RENDER: DATE FIELD (native date-picker — saves ISO string) ═══════ */
  ...
  /* ═══════ RENDER: NUMBER FIELD (type=number — saves REAL number via Number+isNaN) ═══════ */
          ...
          return (
            <div key={itemIdx}>
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) { saveArrayItem(record, fn, idx, sid, itemIdx); } }} />
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveArrayItem(record, fn, idx, sid, itemIdx); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                ...
                return (
                  <div key={ciIdx} className="rec-mini-card" style={{ marginTop: 8 }}>
                    ...
                    <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ciParsed.value); setSaveError(null); } }}>
                      {ciEditing ? (
                        <div className="edit-field-container">
                          <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) { saveSingleLabeledComma(record, fn, idx, sid, ciIdx, ciParsed.label, commaKey); } }} />
                          {saveError && <div className="save-error">{saveError}</div>}
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveSingleLabeledComma(record, fn, idx, sid, ciIdx, ciParsed.label, commaKey); }}>{saving ? 'Saving...' : 'Save'}</button>
                            <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                          </div>
                        </div>
                      ) : (
              ...
              return (
                <div key={ciIdx}>
                  <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                    {ciEditing ? (
                      <div className="edit-field-container">
                        <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) { saveSingleComma(record, fn, idx, sid, ciIdx, commaKey); } }} />
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveSingleComma(record, fn, idx, sid, ciIdx, commaKey); }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="immunization-schedule-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Immunization Schedule</h2></div>
        <div className="empty-state">No immunization schedule records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="immunization-schedule-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Immunization Schedule</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ImmunizationScheduleDocumentPDFTemplate document={pdfData} />} fileName="Immunization_Schedule.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search immunization schedule..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          return (
            <div key={safeId(record) || idx} className="record-card">
              <div className="record-header">
                <h3 className="record-name">{highlightText(`Immunization ${origIdx + 1}`)}</h3>
              </div>
              {renderSection(record, origIdx, 'vaccine-details')}
              {renderSection(record, origIdx, 'administration')}
              {renderSection(record, origIdx, 'provider-facility')}
              {renderSection(record, origIdx, 'codes-documentation')}
              {renderSection(record, origIdx, 'consent-safety')}
              {renderSection(record, origIdx, 'series-status')}
            </div>
          );
        })}
      </div>
...
export default ImmunizationScheduleDocument;
