/**
 * EstimatedDeliveryDateDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * June 2026 — FULL standard: per-sentence string editing, recommendations array (date-grouped),
 *             results recursive OBJECT renderer (renderObjectField / renderObjectNode / renderObjectLeaf)
 * Collection: estimated_delivery_date
 *
 * 5 Sections:
 *   1. session-info: date (date picker), provider, facility, status
 *   2. edd-info: gestationalAge, method, reliability
 *   3. clinical-details: findings, assessment, plan
 *   4. results-section: results (OBJECT)
 *   5. recommendations-section: recommendations (ARRAY of {recommendation, date})
 *   6. notes-section: notes
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import EstimatedDeliveryDateDocumentPDFTemplate from '../pdf-templates/EstimatedDeliveryDateDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './EstimatedDeliveryDateDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [editKey]: { localValue, dbField, dbValue, arrayIndex? } } }
     - editKey is the file's own localEdits key ("field-idx") so it round-trips on refresh.
     - localValue is what goes back into localEdits (string, object clone, or array).
     - dbField/dbValue/arrayIndex are exactly what Approve PUTs to /api/edit (dotted leaf paths,
       array payloads, scalar fields) so the commit replays the original save's DB write. */
const DRAFT_KEY = 'estimated_delivery_datePendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  ...
  'edd-info': ['gestationalAge', 'method', 'reliability'],
...
const SECTION_ORDER = ['session-info', 'edd-info', 'clinical-details', 'results-section', 'recommendations-section', 'notes-section'];
...
/* Narrative fields → Copy/PDF sentence-split + aggressive comma-split + numbered. Everything else
   (provider/facility/status/gestationalAge/reliability) stays PLAIN so commas in names like
   "Dr. Rachel Thompson, MD, FACOG" are never mangled. */
...
const KEY_OVERRIDES = {
  edd: 'EDD', lmp: 'LMP', ga: 'GA', crl: 'CRL', bpd: 'BPD', us: 'US', hc: 'HC', afi: 'AFI', efw: 'EFW',
};
...
/* ======= COMPONENT ======= */
const EstimatedDeliveryDateDocument = ({ document: docProp }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([draftKey, entry]) => {
        ...
        // draftKey is the file's localEdits key, but the stored idx may differ from this render's idx.
        const baseKey = draftKey.replace(/-\d+$/, '');
        const localKey = `${baseKey}-${idx}`;
        nLocal[localKey] = entry.localValue;
        nPending[localKey] = true;
        // Mark the field/sentence as edited so the badge + Pending Approve button show.
        if (entry.marker && entry.marker.kind === 'sentence') nSentences[`${baseKey}-${idx}-s${entry.marker.sentenceIdx}`] = 'edited';
        else if (entry.marker && entry.marker.kind === 'rec') nSentences[`${baseKey}-${idx}-r${entry.marker.recIdx}`] = 'edited';
        else if (entry.marker && entry.marker.kind === 'leaf') nFields[`${baseKey}-${idx}-${entry.marker.path}`] = 'edited';
  ...
  /* formatSentenceFieldLines: narrative field → numbered Copy lines. parseLabel sub-headings, aggressive
     comma-split (guarded), plan-rejoin for the numbered plan. Mirrors the PDF renderSentenceSection. */
  ...
  const highlightText = useCallback((text) => { if (!searchTerm.trim() || text === null || text === undefined) return text; const phrase = searchTerm.trim(); const regex = new RegExp(`(${phrase.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, 'gi'); return String(text).split(regex).map((part, i) => regex.test(part) ? <mark key={i}>{part}</mark> : part); }, [searchTerm]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `estimated delivery date ${idx + 1}`.toLowerCase();
  ...
  // NOT written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve (Approve commits).
  const handleSaveField = useCallback((record, fn, idx, sid, valueOverride) => {
    ...
    store[id][editKey] = { localValue: saveVal, dbField: fn, dbValue: saveVal };
    writeDrafts(store);
    setEditingField(null); setEditValue(''); setSaveError(null);
  }, [editValue, safeId]);

  /* save a nested OBJECT leaf by dot-path (e.g. results.efw) — value stays a STRING.
     Stages a DRAFT only (no DB write); Approve replays the dotted-field PUT. */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
    // Stage one draft entry per leaf so multiple leaves of the same object each replay their dotted PUT.
    const store = readDrafts();
    if (!store[id]) store[id] = {};
    store[id][`${editKey}::${path.join('.')}`] = { localValue: clone, localKey: editKey, dbField: dottedField, dbValue: newVal, marker: { kind: 'leaf', path: path.join('.') } };
    writeDrafts(store);
    setEditingField(null); setEditValue(''); setSaveError(null);
  }, [safeId]);

  // Stages a DRAFT only (no DB write); Approve replays the { field: fn, value: fullText } PUT.
  function stageSentenceDraft(id, fn, idx, fullText) {
    ...
    const store = readDrafts();
    if (!store[id]) store[id] = {};
    store[id][editKey] = { localValue: fullText, dbField: fn, dbValue: fullText };
    writeDrafts(store);
  }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      // Collect this record's drafts whose field belongs to THIS section + this render idx.
      const committedLocalKeys = new Set();
      const draftKeysCommitted = [];
      for (const [draftKey, entry] of Object.entries(recDrafts)) {
        if (!entry || typeof entry !== 'object') continue;
        // localKey = the file's localEdits key ("field-idx"); for leaves it's stored explicitly.
        const localKey = entry.localKey || draftKey;
        const m = localKey.match(/^(.+)-(\d+)$/);
        if (!m || parseInt(m[2], 10) !== idx) continue;
        if (!fields.includes(m[1])) continue;
        // Build the PUT payload. arrayIndex ONLY when the field's trailing dot-segment is purely numeric.
        const dbField = entry.dbField;
        const payload = { value: entry.dbValue };
        const dotIdx = dbField.lastIndexOf('.');
        if (dotIdx !== -1 && /^\d+$/.test(dbField.slice(dotIdx + 1))) {
          payload.field = dbField.slice(0, dotIdx);
          payload.arrayIndex = parseInt(dbField.slice(dotIdx + 1), 10);
        } else {
          payload.field = dbField;
        }
        const resp = await secureApiClient.put(`/api/edit/estimated_delivery_date/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedLocalKeys.add(localKey);
        draftKeysCommitted.push(draftKey);
      }
      // Flag the section approved (audit trail) — preserve the original endpoint shape.
      await secureApiClient.put(`/api/edit/estimated_delivery_date/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (store[id]) {
        draftKeysCommitted.forEach(k => delete store[id][k]);
      ...
      console.error('[EstimatedDeliveryDate] Approve error:', err);
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        if (!sameAsTitle) text += `${label}\n${COPY_LINE_DASH}\n`;
        formatSentenceFieldLines(fmtVal(val, f), f === 'plan').forEach(l => { text += `${l}\n`; });
        text += '\n';
      } else {
        const strVal = fmtVal(val, f);
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== ESTIMATED DELIVERY DATE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Estimated Delivery Date ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              // Stage a DRAFT only (no DB write); Approve replays the { field: fn, value: newArr } PUT.
                              const editKey = `${fn}-${idx}`;
                              ...
                              store[id2][editKey] = { localValue: newArr, dbField: fn, dbValue: newArr, marker: { kind: 'rec', recIdx: rIdx } };
                              writeDrafts(store);
                              setEditingField(null); setEditValue(''); setSaveError(null);
                            }}>{saving ? 'Saving...' : 'Save'}</button>
                            <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
  ...
  if (!records || records.length === 0) return (<div className="estimated-delivery-date-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Estimated Delivery Date</h2></div><div className="empty-state">No estimated delivery date records available</div></div>);

  return (
    <div className="estimated-delivery-date-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Estimated Delivery Date</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<EstimatedDeliveryDateDocumentPDFTemplate document={pdfData} />} fileName="Estimated_Delivery_Date.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container"><input type="text" className="search-input" placeholder="Search estimated delivery date..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />{searchTerm && <span className="search-results">Showing {filteredRecords.length} of {records.length} records</span>}</div>
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header"><div className="record-meta-row">{record.date && <span className="record-date">{highlightText(formatDate(record.date))}</span>}{hasVal(record.status) && <span className="record-date">{highlightText(fmtVal(record.status, 'status'))}</span>}</div><h3 className="record-name">{highlightText(`Estimated Delivery Date ${idx + 1}`)}</h3></div>
...
export default EstimatedDeliveryDateDocument;
