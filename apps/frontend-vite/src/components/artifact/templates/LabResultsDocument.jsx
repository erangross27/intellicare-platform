 ...
 * July 2026 — Canonical rewrite with inline editing, blue glow theme
 * Collection: lab_results
 *
 * 3 Sections:
 *   1. test-info: date (date picker), category (string), panel (string), testType (string),
 *                 testName (string), value (string)
 *   2. results: results (array)
 *   3. interpretation: interpretation (string narrative)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import LabResultsDocumentPDFTemplate from '../pdf-templates/LabResultsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './LabResultsDocument.css';
...
const STRING_FIELDS = ['category', 'panel', 'testType', 'testName', 'interpretation'];
...
const DRAFT_KEY = 'lab_resultsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const LabResultsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.lab_results) return Array.isArray(r.lab_results) ? r.lab_results : [r.lab_results];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.lab_results) return Array.isArray(dd.lab_results) ? dd.lab_results : [dd.lab_results]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Lab Result ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/lab_results/${id}/edit`, { field: fn, value: localEdits[editKey] });
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/lab_results/${id}/approve`, { sectionId: sid, approved: true });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="lab-results-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Lab Results</h2></div>
        <div className="empty-state">No lab results records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="lab-results-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Lab Results</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<LabResultsDocumentPDFTemplate document={pdfData} />} fileName="Lab_Results.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search lab results..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Lab Result ${idx + 1}`)}</h3>
            </div>
...
export default LabResultsDocument;
