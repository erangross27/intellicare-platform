 ...
 *   2. dietary-restrictions: protein, sodium, potassium, phosphorus, fluid
 *   3. lab-nutritional: albumin, prealbumin, nutritional status, dietitian consult
 *   4. lifestyle-modifications: recursive object
 *   5. results-section: recursive object
 *   6. clinical-notes: findings, assessment, plan, notes
 *   7. supplementation: array
 *   8. recommendations-section: date-grouped array of objects
 */
...
const SECTION_TITLES = {
  ...
  'lab-nutritional': 'Lab Values and Nutritional Status',
...
const FIELD_LABELS = {
  ...
  proteinRestriction: 'Protein Restriction', sodiumRestriction: 'Sodium Restriction',
  potassiumRestriction: 'Potassium Restriction', phosphorusRestriction: 'Phosphorus Restriction',
  fluidRestriction: 'Fluid Restriction', albumin: 'Albumin', prealbumin: 'Prealbumin',
  nutritionalStatus: 'Nutritional Status', dietitianConsult: 'Dietitian Consult',
  lifestyleModifications: 'Lifestyle Modifications', results: 'Results', findings: 'Findings',
  assessment: 'Assessment', plan: 'Plan', notes: 'Notes', supplementation: 'Supplementation',
...
const SECTION_FIELDS = {
  ...
  'lifestyle-modifications': ['lifestyleModifications'],
  'results-section': ['results'],
...
const ARRAY_FIELDS = ['supplementation'];
const STRING_FIELDS = ['type', 'provider', 'facility', 'status', 'proteinRestriction', 'sodiumRestriction', 'potassiumRestriction', 'phosphorusRestriction', 'fluidRestriction', 'albumin', 'prealbumin', 'nutritionalStatus', 'findings', 'assessment', 'plan', 'notes'];
/* DISPLAY/EDIT recursive object fields */
const OBJECT_FIELDS = ['lifestyleModifications', 'results'];
...
const RenalNutritionDocument = ({ document: docProp }) => {
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of editKeysForSection) {
        ...
        for (const [dbField, dbValue] of Object.entries(payloads)) {
          const resp = await secureApiClient.put(`/api/edit/renal_nutrition/${id}/edit`, { field: dbField, value: dbValue });
    ...
    } catch (err) { console.error('[RenalNutrition] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const objectCopyLines = useCallback(function buildObjectCopyLines(label, value, indent) {
    ...
    if (isScalar(value)) {
      const rows = typeof value === 'string' ? splitNarrative(value).parts : [fmtScalar(value)];
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_ARRAY_FIELDS.includes(f)) {
        ...
        items.forEach((item, itemIndex) => { body += `${itemIndex + 1}. ${fmtScalar(item)}\n`; });
        body += '\n';
  ...
  const renderArrayField = (record, fn, idx, sid) => {
    ...
    return (<div key={fn} className="rec-mini-card">{!sameAsTitle && <div className="nested-subtitle">{highlightText(label)}</div>}<div className="nested-mini-card">
      {val.map((item, itemIndex) => { if (isEmptyDeep(item)) return null; const path = `${fn}.${itemIndex}`; const editKey = `${path}-${idx}`; const isEditing = editingField === editKey; const isModified = editedFields[editKey];
        return <div key={path} data-edit-field={path}><div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(String(item)); } }}>
          {isEditing ? <div className="edit-field-container"><textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus/><div className="edit-actions"><button className="save-btn" onClick={e => { e.stopPropagation(); const nextValue = editValue.trim(); const updated = [...val]; updated[itemIndex] = nextValue; setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); stageDraft(record, idx, sid, fn, updated, path, nextValue); }}>Save</button><button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); }}>Cancel</button></div></div>
    ...
    <div className="renal-nutrition-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<RenalNutritionDocumentPDFTemplate document={pdfData} />} fileName="Renal_Nutrition.pdf" className="copy-btn">
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'lifestyle-modifications')}
