/**
 * InsuranceFormsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: insurance_forms
 *
 * 4 Sections:
 *   1. patient-diagnosis: patientMedicalRecordNumber, primaryDiagnosisCode,
 *                         secondaryDiagnosisCodes, cptProcedureCodes, preAuthorizationNumber
 *   2. clinical-assessment: medicalNecessityJustification, symptomOnsetDate,
 *                           chronicConditionIndicator, treatmentFailureIndicator, urgencyLevel,
 *                           nyhaClassification, rutherfordClassification
 *   3. vital-signs-scores: functionalStatusScore, glasgowComaScale, ankleBrachialIndex,
 *                          estimatedGlomerularFiltrationRate, bodyMassIndex,
 *                          painScaleRating, hemoglobinA1c, systolicBloodPressure,
 *                          diastolicBloodPressure, comorbidityIndex
 *   4. treatment-history: priorTreatmentHistory, medicationAllergies, expectedLengthOfStay
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import InsuranceFormsDocumentPDFTemplate from '../pdf-templates/InsuranceFormsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './InsuranceFormsDocument.css';
...
const FIELD_LABELS = {
  ...
  cptProcedureCodes: 'CPT Procedure Codes',
  ...
  chronicConditionIndicator: 'Chronic Condition Indicator',
  treatmentFailureIndicator: 'Treatment Failure Indicator',
  ...
  estimatedGlomerularFiltrationRate: 'Estimated Glomerular Filtration Rate (eGFR)',
  ...
  expectedLengthOfStay: 'Expected Length of Stay (days)',
};

const SECTION_FIELDS = {
  'patient-diagnosis': ['patientMedicalRecordNumber', 'primaryDiagnosisCode', 'secondaryDiagnosisCodes', 'cptProcedureCodes', 'preAuthorizationNumber'],
  'clinical-assessment': ['medicalNecessityJustification', 'symptomOnsetDate', 'chronicConditionIndicator', 'treatmentFailureIndicator', 'urgencyLevel', 'nyhaClassification', 'rutherfordClassification'],
  'vital-signs-scores': ['functionalStatusScore', 'glasgowComaScale', 'ankleBrachialIndex', 'estimatedGlomerularFiltrationRate', 'bodyMassIndex', 'painScaleRating', 'hemoglobinA1c', 'systolicBloodPressure', 'diastolicBloodPressure', 'comorbidityIndex'],
  'treatment-history': ['priorTreatmentHistory', 'medicationAllergies', 'expectedLengthOfStay'],
};

/* Number fields — input[type=number step=any] + parseFloat+isNaN→block save */
const NUMBER_FIELDS = [
  'functionalStatusScore', 'glasgowComaScale', 'ankleBrachialIndex',
  'estimatedGlomerularFiltrationRate', 'bodyMassIndex',
  'painScaleRating', 'hemoglobinA1c', 'systolicBloodPressure',
  'diastolicBloodPressure', 'expectedLengthOfStay', 'comorbidityIndex',
];

const BOOLEAN_FIELDS = ['chronicConditionIndicator', 'treatmentFailureIndicator'];
const DATE_FIELDS = ['symptomOnsetDate'];
const ARRAY_FIELDS = ['secondaryDiagnosisCodes', 'cptProcedureCodes', 'priorTreatmentHistory', 'medicationAllergies'];
const STRING_FIELDS = ['patientMedicalRecordNumber', 'primaryDiagnosisCode', 'preAuthorizationNumber', 'medicalNecessityJustification', 'nyhaClassification', 'rutherfordClassification', 'urgencyLevel'];
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field"; arrays store the whole array) */
const DRAFT_KEY = 'insurance_formsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const InsuranceFormsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.insurance_forms) return Array.isArray(r.insurance_forms) ? r.insurance_forms : [r.insurance_forms];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.insurance_forms) return Array.isArray(dd.insurance_forms) ? dd.insurance_forms : [dd.insurance_forms]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Insurance Form ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Stage a DRAFT into the pending-drafts localStorage store (survives refresh). NOT a DB write.
  // store shape: { [recordId]: { [fieldPart]: value } }; here fieldPart is always the field name.
  ...
  // Save = stage a DRAFT locally + localStorage. NOT written to MongoDB / NOT in PDF until Approve.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // localEdits keys for this record/section that are still pending drafts (suffix "-<idx>").
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/insurance_forms/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail).
      await secureApiClient.put(`/api/edit/insurance_forms/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF.
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this record's committed drafts from localStorage (only the just-committed field parts).
      const store = readDrafts();
      if (store[id]) {
    ...
    } catch (err) { console.error('[InsuranceForms] Approve error:', err); setSaveError('Save failed. Please try again.'); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== INSURANCE FORMS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Insurance Form ${idx + 1}\n${'='.repeat(40)}\n\n`;
                  ...
                  <div className="edit-field-container">
                    ...
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => {
                        ...
                        const newArr = [...arr]; newArr[itemIdx] = editValue.trim();
                        setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: newArr }));
                        setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true }));
                        setEditedFields(prev => ({ ...prev, [itemKey]: 'edited' }));
                        stageDraft(record, fn, newArr, sid, idx);
                        setEditingField(null); setEditValue(''); setSaveError(null);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="insurance-forms-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Insurance Forms</h2></div>
        <div className="empty-state">No insurance forms records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="insurance-forms-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Insurance Forms</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<InsuranceFormsDocumentPDFTemplate document={pdfData} />} fileName="Insurance_Forms.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search insurance forms..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Insurance Form ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'patient-diagnosis')}
...
export default InsuranceFormsDocument;
