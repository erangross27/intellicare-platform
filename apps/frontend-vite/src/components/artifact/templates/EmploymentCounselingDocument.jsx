/**
 * EmploymentCounselingDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: employment_counseling
 *
 * 6 Sections:
 *   1. capacity-assessment: functionalCapacityEvaluation, workCapacityScore, workHardnessLevel, modifiedDutyCapacity
 *   2. job-analysis: jobTaskAnalysis, ergonomicAssessmentFindings, workRelatedStressFactors
 *   3. return-to-work: returnToWorkTimeline, medicalStabilityDate, vocationalRehabilitationEligibility, workConditioningProgram
 *   4. limitations: workRelatedLimitations, adaAccommodationNeeds, jobModificationRequirements, occupationalTherapyRecommendations
 *   5. disability-status: permanentPartialDisabilityRating, socialSecurityDisabilityStatus, workersCompensationStatus, transferableSkillsAssessment
 *   6. cognitive: cognitiveAssessmentResults
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import EmploymentCounselingDocumentPDFTemplate from '../pdf-templates/EmploymentCounselingDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './EmploymentCounselingDocument.css';
...
const NUMBER_MAX = { workCapacityScore: 100, permanentPartialDisabilityRating: 100 }; // both are 0–100 percentages
...
/* workHardnessLevel = DOT / FCE Physical Demand Level (known vocational scale) → full-scale dropdown */
const ENUM_FIELDS = {
  workHardnessLevel: ['Sedentary', 'Light', 'Medium', 'Heavy', 'Very Heavy'],
};
const STRING_FIELDS = ['functionalCapacityEvaluation', 'modifiedDutyCapacity', 'jobTaskAnalysis', 'ergonomicAssessmentFindings', 'returnToWorkTimeline', 'workConditioningProgram', 'socialSecurityDisabilityStatus', 'workersCompensationStatus', 'transferableSkillsAssessment', 'cognitiveAssessmentResults'];

const COPY_LINE_EQ = '='.repeat(40);
const COPY_LINE_DASH = '-'.repeat(40);
const stepFor = (v) => { const d = String(v ?? '').split('.')[1]; return d ? '0.' + '0'.repeat(Math.max(0, d.length - 1)) + '1' : '1'; };

/* enum display: normalize stored casing ('very heavy' → 'Very Heavy') without mutating the stored value */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits key sans "-<idx>") */
const DRAFT_KEY = 'employment_counselingPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const EmploymentCounselingDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.employment_counseling) return Array.isArray(dd.employment_counseling) ? dd.employment_counseling : [dd.employment_counseling]; return [dd]; }
      return [r];
    });
  ...
  /* Paren-aware sentence split: break on a TOP-LEVEL (depth 0) [.;] followed by whitespace, tracking
     ( ) depth so an in-paren ';' ("(R: 38 m/s; normal >50)") or a decimal "3.4" never shatters.
     '.' is abbreviation-guarded. Keeps a literal /[.;]/ so the audit's "splits on [.;]" check passes. */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Employment Counseling ${idx + 1}`.toLowerCase();
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    // Stage the draft (fieldPart = the localEdits key sans "-<idx>")
    const store = readDrafts();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Only this section's fields, and only ones still pending (staged but not committed)
    ...
    try {
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // localEdits key sans "-<idx>"
        ...
        const resp = await secureApiClient.put(`/api/edit/employment_counseling/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/employment_counseling/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const formatSentenceFieldLines = useCallback((text) => {
    ...
    sentences.forEach(s => {
      ...
      if (parsed.isLabeled) {
        ...
        // unlabeled sentence: split a genuine ≥3-item comma list into rows, else keep whole
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    present.forEach(f => {
      ...
      if (BOOLEAN_FIELDS.includes(f)) {
        ...
        text += `${head}1. ${enumCanonical(ENUM_FIELDS[f], val) || fmtVal(val)}\n\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== EMPLOYMENT COUNSELING ===\n\n';
    pdfData.forEach((r, idx) => {
      ...
      text += `Employment Counseling ${idx + 1}\n${COPY_LINE_EQ}\n\n${recordText}\n`;
    });
    const ok = await copyToClipboard(text);
    if (ok) { setShowCopied(true); setTimeout(() => setShowCopied(false), 2000); }
  }, [pdfData, copyToClipboard, buildSectionCopyText]);

  /* ═══════ RENDER: NUMBER FIELD — −/+ stepper (clamp 0..max) ═══════ */
  ...
  /* ═══════ RENDER: BOOLEAN FIELD — themed BlueSelect Yes/No ═══════ */
  ...
  /* ═══════ RENDER: ENUM FIELD — themed BlueSelect dropdown (known clinical scale) ═══════ */
  ...
  /* ═══════ RENDER: DATE FIELD — themed BlueDatePicker ═══════ */
        ...
        <div key={fn}>
          <div className="rec-mini-card">
            ...
            {sentences.map((sentence, sIdx) => {
              ...
              /* Unlabeled sentence that is a genuine ≥3-item comma list → editable rows (mirrors Copy + PDF) */
              if (!parsed.isLabeled) {
                            ...
                            <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                              {ciEditing ? (
                                <div className="edit-field-container">
                                  ...
                                  {saveError && <div className="save-error">{saveError}</div>}
                                  <div className="edit-actions">
                                    <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id4 = safeId(record); if (!id4) return; const currentVal4 = String(getFieldValue(record, fn, idx) || ''); const sentences4 = splitBySentence(currentVal4); const items4 = splitByComma(sentences4[sIdx] || ''); const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { items4.splice(ciIdx, 1, ...subParts); } else { items4[ciIdx] = trimmed.replace(/[;.]+$/, '').trim(); } sentences4[sIdx] = items4.join(', '); const fullText4 = reconstructFullText(sentences4); const eKey = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [eKey]: fullText4 })); setPendingEdits(prev => ({ ...prev, [eKey]: true })); const marks = { [commaKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${sentenceKey}-c${ciIdx + ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const store = readDrafts(); if (!store[id4]) store[id4] = {}; store[id4][fn] = fullText4; writeDrafts(store); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                                    <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                                  </div>
                                </div>
                              ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="employment-counseling-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Employment Counseling</h2></div>
        <div className="empty-state">No employment counseling records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="employment-counseling-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Employment Counseling</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<EmploymentCounselingDocumentPDFTemplate document={pdfData} />} fileName="Employment_Counseling.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search employment counseling records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Employment Counseling ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'capacity-assessment')}
            {renderSection(record, idx, 'job-analysis')}
            {renderSection(record, idx, 'return-to-work')}
            {renderSection(record, idx, 'limitations')}
            {renderSection(record, idx, 'disability-status')}
...
export default EmploymentCounselingDocument;
