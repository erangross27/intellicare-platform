/**
 * GeriatricMedicationsDocumentPDFTemplate.jsx
 * Box-free B&W LETTER (canonical, memory 6a2d6af6) — mirrors the JSX exactly: config-driven from
 * SECTION_ORDER / SECTION_TITLES / FIELD_LABELS / SECTION_FIELDS, real record dates (startDate/endDate,
 * never createdAt), route enum canonical, active boolean Yes/No, sideEffects/drugInteractions string
 * arrays, narratives (dosage/safetyWarning/instructions) split on [.;] with a single-letter genus guard
 * (C. difficile) + CLAUSE_OPENER guard. SENTINEL-ZERO on durationDays (0 = indefinite/not specified);
 * refills 0 is a real prescription state and kept. Values numbered ('1.' even singles), single-name
 * label gate. Rule #74: each field is ONE wrap={false} atomic View with the sectionTitle riding INSIDE
 * the first present field's View. Static PHI footer. Collection: geriatric_medications.
 */
...
const SECTION_ORDER = ['medication-info', 'prescriber-info', 'treatment-duration', 'safety-info'];
const SECTION_TITLES = {
  'medication-info': 'Medication Information',
  'prescriber-info': 'Prescriber and Instructions',
  'treatment-duration': 'Treatment Duration',
  'safety-info': 'Safety and Interactions',
};
const FIELD_LABELS = {
  ...
  startDate: 'Start Date', endDate: 'End Date', duration: 'Duration', durationDays: 'Duration (Days)',
  durationUnit: 'Duration Unit', active: 'Active', refills: 'Refills',
...
const GeriatricMedicationsDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].geriatric_medications && Array.isArray(templateData[0].geriatric_medications)) records = templateData[0].geriatric_medications;
    else records = templateData;
  } else if (templateData && templateData.geriatric_medications) {
    records = Array.isArray(templateData.geriatric_medications) ? templateData.geriatric_medications : [templateData.geriatric_medications];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.geriatric_medications ? (Array.isArray(dd.geriatric_medications) ? dd.geriatric_medications : [dd.geriatric_medications]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Geriatric Medications</Text></View>
        <Text style={styles.emptyState}>No geriatric medications records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Geriatric Medications ${idx + 1}`}</Text>
...
export default GeriatricMedicationsDocumentPDFTemplate;
