...
const SECTIONS = [
  { title: 'Assessment Details', fields: [['date', 'Date', 'date'], ['provider', 'Provider']] },
  { title: 'Environmental Allergens', fields: [['environmentalAllergens', 'Environmental Allergens', 'arrayKeep']] },
  { title: 'Skin Test Results', fields: [['skinTestResults', 'Skin Test Results', 'arrayComma']] },
  { title: 'Laboratory Results', fields: [['totalIge', 'Total IgE'], ['eosinophilCount', 'Eosinophil Count']] },
...
const rowsFor = ([, , type], value) => {
  ...
  if (type === 'object') return Object.entries(value || {}).filter(([, item]) => hasVal(item)).map(([key, item]) => ({ subtitle: humanize(key), value: String(item) }));
...
const unwrap = source => {
  ...
  rows = rows.flatMap(row => {
    ...
    if (row?.documentData) { const nested = row.documentData; if (Array.isArray(nested)) return nested; if (nested?.allergy_assessments) return Array.isArray(nested.allergy_assessments) ? nested.allergy_assessments : [nested.allergy_assessments]; return [nested]; }
    return [row];
  });
  return rows.filter(row => row && typeof row === 'object');
};

const AllergyAssessmentsDocumentPDFTemplate = ({ document: documentProp, data: dataProp, templateData }) => {
  const records = React.useMemo(() => unwrap(documentProp ?? dataProp ?? templateData), [documentProp, dataProp, templateData]);
  if (!records.length) return <Document><Page size="LETTER" style={styles.page}><View style={styles.documentHeader}><Text style={styles.documentTitle}>Allergy Assessments</Text></View><Text style={styles.noData}>No allergy assessment records available</Text></Page></Document>;
