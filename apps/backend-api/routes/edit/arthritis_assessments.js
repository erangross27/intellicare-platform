/**
 * Arthritis Assessments Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/arthritis_assessments/:id/edit    — Edit a single field
 * PUT /api/edit/arthritis_assessments/:id/approve — Approve a section
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'arthritis-assessments-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('arthritis_assessments', { _id: objectId }, {
      $set: {
        [updatePath]: value,
        updatedAt: new Date(),
    ...
    console.error('[ArthritisAssessments] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    console.error('[ArthritisAssessments] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
