...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'return-to-work-plan-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'date', 'diagnosisCode', 'treatingPhysician', 'workersCompClaim',
  'jobTitle', 'employerName',
  'injuryDate', 'injuryDescription',
  'workAbilityStatus', 'maxWorkCategory', 'estimatedReturnDate',
  'hoursPerDayAllowed', 'daysPerWeekAllowed', 'modifiedDutyStartDate', 'modifiedDutyEndDate',
  'liftingRestriction', 'postureRestrictions', 'durationRestrictions', 'repetitiveMotionLimits', 'environmentalRestrictions',
  'accommodationsRequired',
  'physicalTherapyRequired', 'functionalCapacityEvaluation', 'maximumMedicalImprovement', 'permanentRestrictions', 'restrictionReviewDate',
];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('return_to_work_plan', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[return_to_work_plan] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('return_to_work_plan', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[return_to_work_plan] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
