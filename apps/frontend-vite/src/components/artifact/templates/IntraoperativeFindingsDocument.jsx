/**
 * IntraoperativeFindingsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: intraoperative_findings
 *
 * 10 Sections:
 *   1. anatomy-status: normalAnatomy (boolean)
 *   2. anatomical-variants: anatomicalVariants (array, empty)
 *   3. pathological-findings: pathologicalFindings (array strings)
 *   4. tissue-assessment: remnantTissue (string)
 *   5. adhesions: adhesions.severity (string), adhesions.location (array strings)
 *   6. contamination-status: contamination (string)
 *   7. additional-procedures: additionalProcedures (array, empty)
 *   8. organ-assessment: perforationStatus (string), liverAppearance (string)
 *   9. general-info: date (date), provider (string), facility (string), status (string)
 *  10. results-section: results (OBJECT, recursive key-value)
 *  11. recommendations-section: recommendations (ARRAY of {recommendation, date}, date-grouped)
 *  12. clinical-notes: findings (sentence), assessment (sentence), plan (sentence), notes (sentence)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import IntraoperativeFindingsDocumentPDFTemplate from '../pdf-templates/IntraoperativeFindingsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './IntraoperativeFindingsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the editKey segment before "-<idx>") */
const DRAFT_KEY = 'intraoperative_findingsPendingEdits';
const readDrafts = () => {
...
/* humanizeKey: convert object keys to display labels */
const KEY_OVERRIDES = {};
...
const FIELD_LABELS = {
  normalAnatomy: 'Normal Anatomy',
  anatomicalVariants: 'Anatomical Variants',
  pathologicalFindings: 'Pathological Findings',
  remnantTissue: 'Remnant Tissue',
  'adhesions.severity': 'Severity',
  'adhesions.location': 'Location',
  contamination: 'Contamination',
  additionalProcedures: 'Additional Procedures',
  perforationStatus: 'Perforation Status',
  liverAppearance: 'Liver Appearance',
  date: 'Date',
  provider: 'Provider',
  facility: 'Facility',
  status: 'Status',
...
const SECTION_FIELDS = {
  'anatomy-status': ['normalAnatomy'],
  'anatomical-variants': ['anatomicalVariants'],
  'pathological-findings': ['pathologicalFindings'],
  'tissue-assessment': ['remnantTissue'],
  'adhesions': ['adhesions.severity', 'adhesions.location'],
  'contamination-status': ['contamination'],
  'additional-procedures': ['additionalProcedures'],
  'organ-assessment': ['perforationStatus', 'liverAppearance'],
...
const BOOLEAN_FIELDS = ['normalAnatomy'];
const DATE_FIELDS = ['date'];
const ARRAY_FIELDS = ['anatomicalVariants', 'pathologicalFindings', 'additionalProcedures', 'adhesions.location'];
const OBJECT_FIELDS = ['results'];
const OBJECT_ARRAY_FIELDS = ['recommendations'];
const SENTENCE_FIELDS = ['findings', 'assessment', 'plan', 'notes'];
const STRING_FIELDS = ['remnantTissue', 'contamination', 'perforationStatus', 'liverAppearance', 'provider', 'facility', 'status', 'adhesions.severity'];
...
/* ═══════ COMPONENT ═══════ */
const IntraoperativeFindingsDocument = ({ document: docProp }) => {
  ...
  // editKeys (`${fieldPart}-${idx}`) that are staged DRAFTS (saved locally, NOT yet committed to DB/PDF). Cleared on Approve.
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.intraoperative_findings) return Array.isArray(r.intraoperative_findings) ? r.intraoperative_findings : [r.intraoperative_findings];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.intraoperative_findings) return Array.isArray(dd.intraoperative_findings) ? dd.intraoperative_findings : [dd.intraoperative_findings]; return [dd]; }
      return [r];
    });
  ...
  /* Resolve nested field like "adhesions.severity" from a record */
  const resolveNestedField = useCallback((record, fn) => {
  ...
  const getFieldValue = useCallback((record, fn, idx) => {
    ...
    return resolveNestedField(record, fn);
  }, [localEdits, resolveNestedField]);
  ...
  const getEffectiveArray = useCallback((record, fn, idx) => {
    ...
    const val = resolveNestedField(record, fn);
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (OBJECT_FIELDS.includes(f) || OBJECT_ARRAY_FIELDS.includes(f)) { if (flattenSearchable(val).toLowerCase().includes(phrase)) return true; }
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (OBJECT_FIELDS.includes(fn) || OBJECT_ARRAY_FIELDS.includes(fn)) return flattenSearchable(val).toLowerCase().includes(phrase);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Intraoperative Findings ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* Stage a DRAFT for one field of one record: localEdits + pendingEdits + localStorage (survives refresh).
     fieldPart is the editKey segment BEFORE "-<idx>" (e.g. "provider", "results", "adhesions.severity").
     NO DB write here — handleApproveSection is the ONLY committer. */
  const stageDraft = useCallback((record, idx, sid, fieldPart, fullValue) => {
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    const currentEditKey = editTrackingKey || editingField;

    if (isArray) {
      const arrMatch = currentEditKey?.match(/-ai(\d+)$/);
  ...
  }, [editValue, editingField, safeId, getEffectiveArray, stageDraft]);

  /* save a nested OBJECT leaf by dot-path (e.g. results.ef) — stages the WHOLE root object as a draft */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Commit only this section's pending edits (editKey = `${fieldPart}-${idx}`, fieldPart in this section's fields)
      const suffix = `-${idx}`;
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length);
        // editKey fieldPart equals the field name (incl. dotted like "adhesions.severity"); match this section's fields.
      ...
      for (const editKey of toCommit) {
        ...
        // arrayIndex ONLY when the segment after the LAST dot is purely numeric (reverse of fieldName.arrayIndex)
        if (lastDot !== -1 && /^\d+$/.test(trailing)) {
          payload.field = fieldPart.slice(0, lastDot);
          payload.arrayIndex = parseInt(trailing, 10);
        }
        const resp = await secureApiClient.put(`/api/edit/intraoperative_findings/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/intraoperative_findings/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[IntraoperativeFindings] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const formatSentenceFieldLines = useCallback((text) => {
    ...
    sentences.forEach(s => {
      ...
      if (parsed.isLabeled) {
        ...
        let commaParts = splitByComma(textToSplit);
        if (commaParts.length < 2) {
          const parenListMatch = textToSplit.match(/^\d+\s*\((.+)\)$/);
          if (parenListMatch) commaParts = parenListMatch[1].split(',').map(p => p.trim()).filter(Boolean);
        }
        const displayParts = commaParts.length >= 2 ? commaParts : [textToSplit];
        if (displayParts.length >= 2) {
          lines.push(parsed.label + ':');
          displayParts.forEach(item => { lines.push(`  ${n++}. ${item}`); });
        } else { lines.push(`${n++}. ${s}`); }
      } else { lines.push(`${n++}. ${s}`); }
    });
    return lines;
  ...
  const objectCopyLines = useCallback((label, value, indent) => {
    ...
    const out = [];
    if (isScalar(value)) { out.push(`${pad}${label}: ${fmtScalar(value)}`); return out; }
    if (Array.isArray(value)) {
      out.push(`${pad}${label}`);
      value.filter(x => !isEmptyDeep(x)).forEach((v, i) => { isScalar(v) ? out.push(`${pad}  ${i + 1}. ${fmtScalar(v)}`) : objectCopyLines(humanizeKey(String(i + 1)), v, indent + 1).forEach(l => out.push(l)); });
      return out;
    }
  ...
  const recommendationsCopyLines = useCallback((recs) => {
    const out = [];
    (Array.isArray(recs) ? recs : []).forEach((rec, i) => {
      const t = typeof rec === 'object' ? (rec?.recommendation || '') : String(rec);
      const d = typeof rec === 'object' ? (rec?.date || '') : '';
      if (!t.trim()) return;
      out.push(`${i + 1}. ${t}${d ? ` (${d})` : ''}`);
    });
    return out;
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_FIELDS.includes(f)) {
        ...
        text += `${label}\n`;
        recommendationsCopyLines(val).forEach(l => { text += `${l}\n`; });
        text += '\n';
      } else if (ARRAY_FIELDS.includes(f)) {
        text += `${label}\n`;
        const arr = Array.isArray(val) ? val : [];
        arr.forEach(item => { text += `${n++}. ${item}\n`; });
        text += '\n';
    ...
    return text;
  }, [getFieldValue, hasVal, fmtVal, splitBySentence, formatSentenceFieldLines, objectCopyLines, recommendationsCopyLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== INTRAOPERATIVE FINDINGS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Intraoperative Findings ${idx + 1}\n${'='.repeat(40)}\n\n`;
      if (r.date) text += `Date\n${formatDate(r.date)}\n\n`;
      Object.keys(SECTION_FIELDS).forEach(sid => {
        const fields = SECTION_FIELDS[sid];
        const hasAny = fields.some(f => {
          const val = ARRAY_FIELDS.includes(f) ? (Array.isArray(resolveNestedField(r, f)) ? resolveNestedField(r, f) : []) : resolveNestedField(r, f);
          return hasVal(val);
        });
        if (!hasAny) return;
        ...
        fields.forEach(f => {
          const label = FIELD_LABELS[f] || f;
          const val = resolveNestedField(r, f);
          if (!hasVal(val)) return;
          if (OBJECT_FIELDS.includes(f)) {
            text += `${label}\n`;
            Object.entries(val).filter(([, v]) => !isEmptyDeep(v)).forEach(([k, v]) => objectCopyLines(humanizeKey(k), v, 0).forEach(l => { text += `${l}\n`; }));
  ...
  }, [pdfData, copyToClipboard, hasVal, fmtVal, splitBySentence, formatSentenceFieldLines, resolveNestedField, objectCopyLines, recommendationsCopyLines]);

  /* ═══════ RENDER: EDITABLE FIELD (string, boolean, date) ═══════ */
  const renderEditableField = (record, fn, idx, sid, title) => {
    ...
    return (
      <div key={fn} className="rec-mini-card">
        <div className="nested-subtitle">{highlightText(label)}</div>
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(isBool ? (val ? 'Yes' : 'No') : fmtVal(val)); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              {isBool ? (
                <BlueSelect value={editValue} options={['Yes', 'No']} onChange={v => { setEditValue(v); setSaveError(null); }} />
              ) : (
                <textarea className="edit-textarea" value={editValue} onChange={e => { setEditValue(e.target.value); setSaveError(null); }} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
              )}
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => {
                  e.stopPropagation();
                  if (isBool) {
                    handleSaveField(record, fn, idx, sid, null, editValue === 'Yes');
                  } else {
                    handleSaveField(record, fn, idx, sid);
                  }
        ...
        <div key={fn}>
          <div className="rec-mini-card">
            ...
            {sentences.map((sentence, sIdx) => {
              ...
              /* Only comma-split labeled sentences */
              if (parsed.isLabeled) {
                const textToSplit = parsed.value;
                let commaParts = splitByComma(textToSplit);
                if (commaParts.length < 2) {
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; boolean -> select, number+unit -> number input, "4/5" stays text) ═══════ */
  ...
  const renderRecommendationsField = (record, fn, idx, sid) => {
    ...
    const norm = (rec) => (typeof rec === 'object' && rec !== null)
      ? { text: (rec.recommendation || '').trim(), date: (rec.date || '').trim() }
      : { text: String(rec || '').trim(), date: '' };

    const groups = [];
    recs.forEach((rec, rIdx) => {
      const { date: d } = norm(rec);
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {groups.map((group, gIdx) => {
          const anyVisible = !searchTerm.trim() || phraseMatch || labelMatch || group.date.toLowerCase().includes(phrase) || group.items.some(({ rec }) => norm(rec).text.toLowerCase().includes(phrase));
          ...
          return (
            <div key={gIdx} className="rec-mini-card" style={{ marginTop: 8 }}>
              ...
              {group.items.map(({ rec, rIdx }) => {
                const { text: recText, date: recDate } = norm(rec);
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              const newArr = currentArr.map((r, i) => {
                                if (i !== rIdx) return r;
                                return (typeof r === 'object' && r !== null) ? { ...r, recommendation: trimmed } : trimmed;
                              });
                              setSaveError(null);
                              stageDraft(record, idx, sid, fn, newArr);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="intraoperative-findings-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Intraoperative Findings</h2></div>
        <div className="empty-state">No intraoperative findings records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="intraoperative-findings-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Intraoperative Findings</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<IntraoperativeFindingsDocumentPDFTemplate document={pdfData} />} fileName="Intraoperative_Findings.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search intraoperative findings..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Intraoperative Findings ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'anatomy-status')}
            {renderSection(record, idx, 'anatomical-variants')}
            {renderSection(record, idx, 'pathological-findings')}
            {renderSection(record, idx, 'tissue-assessment')}
            {renderSection(record, idx, 'adhesions')}
            {renderSection(record, idx, 'contamination-status')}
            {renderSection(record, idx, 'additional-procedures')}
            {renderSection(record, idx, 'organ-assessment')}
...
export default IntraoperativeFindingsDocument;
