...
class WorkflowEngine extends EventEmitter {
  ...
  async initialize() {
    try {
      ...
      const authResult = await serviceAccountManager.authenticate(this.serviceId);

      // Check if authentication failed
      if (!authResult) {
        throw new Error('Failed to authenticate WorkflowEngine - no token received');
      }
  ...
  async handleStartWorkflow(socket, data) {
    try {
      ...
      const dbSession = await SecureDataAccess.insert('workflow_sessions', session, context);
      ...
      // Log audit
      await immutableAuditService.logAction({
  ...
  async handleCompleteWorkflow(socket, data) {
    try {
      ...
      await SecureDataAccess.insert('workflow_history', {
        ...session,
        userId: socket.userId,
        practiceId: socket.practiceId
      }, context);
  ...
  async loadWorkflowDefinitions() {
    try {
      const context = {
        serviceId: this.serviceId,
        apiKey: this.serviceToken?.apiKey || this.serviceToken,
        operation: 'loadWorkflowDefinitions',
        practiceId: 'global'
      };
      
      const workflows = await SecureDataAccess.query('workflows', {}, {}, context);
  ...
  loadDefaultWorkflows() {
    ...
    const defaultWorkflows = require('./workflowDefinitions');
  ...
  async loadUserSkills() {
    try {
      const context = {
        serviceId: this.serviceId,
        apiKey: this.serviceToken?.apiKey || this.serviceToken,
        operation: 'loadUserSkills',
        practiceId: 'global'
      };
      
      const skills = await SecureDataAccess.query('user_skills', {}, {}, context);
