/**
 * Infusion Therapy Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/infusion_therapy/:id/edit    — Edit a single field
 * PUT /api/edit/infusion_therapy/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') { return { serviceId: 'infusion-therapy-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }

const ALLOWED_FIELDS = [
  // Infusion
  'infusionMedication', 'infusionDose', 'infusionRate', 'infusionDuration',
  'totalVolumeAdministered', 'diluentSolution', 'infusionPumpType', 'infusionProtocol',
  // Vascular Access
  'vascularAccessType', 'vascularAccessSite', 'vascularAccessGauge',
  // Vitals & Labs
  'preInfusionVitalSigns', 'postInfusionVitalSigns', 'patientWeight', 'bodyMassIndex',
  'estimatedGlomerularFiltrationRate', 'laboratoryMonitoring',
  // Safety
  'premedications', 'allergyScreening', 'concurrentMedications',
  'infusionComplications', 'infusionTolerability',
  // Clinical & Timing
  'therapeuticIndication', 'infusionStartTime', 'infusionEndTime',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('infusion_therapy', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[InfusionTherapy] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('infusion_therapy', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[InfusionTherapy] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
