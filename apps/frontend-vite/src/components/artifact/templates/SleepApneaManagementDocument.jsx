 ...
 *
 * 7 Sections:
 *   1. diagnostic-metrics: apneaHypopneaIndex, oxygenDesaturationIndex, lowestOxygenSaturation, sleepApneaSeverity, epworthSleepinessScore
 *   2. cpap-therapy: cpapPrescribed, cpapPressureSetting, cpapComplianceRate, averageCpapUsageHours, residualAhiOnTherapy
 *   3. mask-equipment: maskType, maskLeakRate
 *   4. sleep-study-results: remSleepPercentage, arousalIndex, centralApneaIndex, obstructiveApneaIndex
 *   5. physical-exam: neckCircumference, mallampatiScore, tonsilSize, positionalApnea
 *   6. symptoms: witnessedApneas, comorbidHypertension
 *   7. treatment: surgicalInterventions, alternativeTherapyType
 */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; arrays stored as full array) */
...
const FIELD_LABELS = {
  ...
  sleepApneaSeverity: 'Sleep Apnea Severity',
  epworthSleepinessScore: 'Epworth Sleepiness Score',
  cpapPrescribed: 'CPAP Prescribed',
  cpapPressureSetting: 'CPAP Pressure Setting',
  cpapComplianceRate: 'CPAP Compliance Rate',
  averageCpapUsageHours: 'Average CPAP Usage',
  residualAhiOnTherapy: 'Residual AHI on Therapy',
  maskType: 'Mask Type',
  maskLeakRate: 'Mask Leak Rate',
  ...
  tonsilSize: 'Tonsil Size',
  positionalApnea: 'Positional Apnea',
  witnessedApneas: 'Witnessed Apneas',
  comorbidHypertension: 'Comorbid Hypertension',
  surgicalInterventions: 'Surgical Interventions',
  alternativeTherapyType: 'Alternative Therapy Type',
};

const SECTION_FIELDS = {
  'diagnostic-metrics': ['apneaHypopneaIndex', 'oxygenDesaturationIndex', 'lowestOxygenSaturation', 'sleepApneaSeverity', 'epworthSleepinessScore'],
  'cpap-therapy': ['cpapPrescribed', 'cpapPressureSetting', 'cpapComplianceRate', 'averageCpapUsageHours', 'residualAhiOnTherapy'],
  'mask-equipment': ['maskType', 'maskLeakRate'],
  'sleep-study-results': ['remSleepPercentage', 'arousalIndex', 'centralApneaIndex', 'obstructiveApneaIndex'],
  'physical-exam': ['neckCircumference', 'mallampatiScore', 'tonsilSize', 'positionalApnea'],
  'symptoms': ['witnessedApneas', 'comorbidHypertension'],
  'treatment': ['surgicalInterventions', 'alternativeTherapyType'],
};

const BOOLEAN_FIELDS = ['cpapPrescribed', 'positionalApnea', 'witnessedApneas', 'comorbidHypertension'];
...
const STRING_FIELDS = ['sleepApneaSeverity', 'cpapPressureSetting', 'maskType', 'mallampatiScore', 'tonsilSize', 'alternativeTherapyType'];

/* ======= Clinical interpretation helpers ======= */
const getAHIInterpretation = (score) => { if (score < 5) return { color: '#22c55e', text: 'Normal' }; if (score < 15) return { color: '#3b82f6', text: 'Mild OSA' }; if (score < 30) return { color: '#f59e0b', text: 'Moderate OSA' }; return { color: '#ef4444', text: 'Severe OSA' }; };
const getODIInterpretation = (score) => { if (score < 5) return { color: '#22c55e', text: 'Normal' }; if (score < 15) return { color: '#3b82f6', text: 'Mild' }; if (score < 30) return { color: '#f59e0b', text: 'Moderate' }; return { color: '#ef4444', text: 'Severe' }; };
const getLowestO2Interpretation = (score) => { if (score >= 90) return { color: '#22c55e', text: 'Normal' }; if (score >= 85) return { color: '#3b82f6', text: 'Mild Desaturation' }; if (score >= 80) return { color: '#f59e0b', text: 'Moderate Desaturation' }; return { color: '#ef4444', text: 'Severe Desaturation' }; };
const getEpworthInterpretation = (score) => { if (score <= 5) return { color: '#22c55e', text: 'Normal' }; if (score <= 10) return { color: '#3b82f6', text: 'Mild Sleepiness' }; if (score <= 12) return { color: '#f59e0b', text: 'Moderate Sleepiness' }; return { color: '#ef4444', text: 'Severe Sleepiness' }; };
const getCPAPComplianceInterpretation = (score) => { if (score >= 70) return { color: '#22c55e', text: 'Good Compliance' }; if (score >= 50) return { color: '#3b82f6', text: 'Moderate Compliance' }; if (score >= 30) return { color: '#f59e0b', text: 'Poor Compliance' }; return { color: '#ef4444', text: 'Very Poor Compliance' }; };
const getCPAPUsageInterpretation = (hours) => { if (hours >= 6) return { color: '#22c55e', text: 'Excellent Usage' }; if (hours >= 4) return { color: '#3b82f6', text: 'Good Usage' }; if (hours >= 2) return { color: '#f59e0b', text: 'Inadequate Usage' }; return { color: '#ef4444', text: 'Poor Usage' }; };
const getResidualAHIInterpretation = (score) => { if (score < 5) return { color: '#22c55e', text: 'Well Controlled' }; if (score < 10) return { color: '#3b82f6', text: 'Acceptable' }; if (score < 15) return { color: '#f59e0b', text: 'Suboptimal' }; return { color: '#ef4444', text: 'Poor Control' }; };
...
const Legend = () => (
  <div className="chart-legend">
    <div className="legend-item"><div className="legend-color" style={{ backgroundColor: '#22c55e' }} /><span>Normal/Good</span></div>
    ...
    <div className="legend-item"><div className="legend-color" style={{ backgroundColor: '#ef4444' }} /><span>Severe/Poor</span></div>
  </div>
);
...
const BarChart = ({ label, percentage, rawValue, color, interpretation, highlightFn }) => (
  <div className="bar-chart-row">
    ...
    <div className="bar-container">
      <div className="bar-background"><div className="bar-fill" style={{ width: `${Math.min(100, Math.max(0, percentage))}%`, backgroundColor: color }} /></div>
...
const SleepApneaManagementDocument = ({ document: docProp }) => {
  ...
  /* ======= REHYDRATE PENDING DRAFTS =======
     Restore staged drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF). */
  useEffect(() => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Sleep Apnea Management ${idx + 1}`.toLowerCase();
  ...
  /* ======= EDIT HANDLERS ======= */
  // Stage a single field as a DRAFT (no DB write). localStorage keeps it across refresh; Approve commits it.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // localEdits keys are `${field}-${idx}`; commit those that belong to this section's fields and are pending.
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/sleep_apnea_management/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/sleep_apnea_management/${id}/approve`, { sectionId: sid, approved: true });
      ...
      console.error('[SleepApneaManagement] Approve error:', err);
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== SLEEP APNEA MANAGEMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Sleep Apnea Management ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="sleep-apnea-management-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Sleep Apnea Management</h2></div>
        <div className="empty-state">No sleep apnea management records available</div>
      </div>
    );
  }
    ...
    <div className="sleep-apnea-management-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<SleepApneaManagementDocumentPDFTemplate document={pdfData} />} fileName={`sleep-apnea-management-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search sleep apnea management..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          return (
            <div key={idx} className="record-card">
              <div className="record-header">
                ...
                <h3 className="record-name">{highlightText(record.sleepApneaSeverity || `Sleep Apnea Management ${idx + 1}`)}</h3>
              </div>
              ...
              {renderSection(record, idx, 'diagnostic-metrics')}
              {renderSection(record, idx, 'cpap-therapy')}
              {renderSection(record, idx, 'mask-equipment')}
              {renderSection(record, idx, 'sleep-study-results')}
