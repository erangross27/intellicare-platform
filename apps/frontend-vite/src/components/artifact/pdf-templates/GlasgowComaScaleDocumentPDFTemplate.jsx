/**
 * GlasgowComaScaleDocumentPDFTemplate.jsx
 * Box-free B&W LETTER (canonical, memory 6a2d6af6) — mirrors the JSX: real record.assessmentDate (never
 * createdAt), narratives split on [.;] with abbrev/single-initial/genus guard, labeled sentences comma-split
 * into a subtitle group, values numbered ('1.' even singles), single-name label gate (clinicalStatus label ==
 * 'Clinical Status' section title), recommendations date-grouped. Rule #74: each field is ONE wrap={false}
...
const SECTION_ORDER = ['assessment-info', 'clinical-status', 'vitals-interventions', 'response-plan', 'recommendations'];
...
const GlasgowComaScaleDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].glasgow_coma_scale && Array.isArray(templateData[0].glasgow_coma_scale)) records = templateData[0].glasgow_coma_scale;
    else records = templateData;
  } else if (templateData && templateData.glasgow_coma_scale) {
    records = Array.isArray(templateData.glasgow_coma_scale) ? templateData.glasgow_coma_scale : [templateData.glasgow_coma_scale];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.glasgow_coma_scale ? (Array.isArray(dd.glasgow_coma_scale) ? dd.glasgow_coma_scale : [dd.glasgow_coma_scale]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Glasgow Coma Scale</Text></View>
        <Text style={styles.emptyState}>No Glasgow Coma Scale records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Glasgow Coma Scale ${idx + 1}`}</Text>
...
export default GlasgowComaScaleDocumentPDFTemplate;
