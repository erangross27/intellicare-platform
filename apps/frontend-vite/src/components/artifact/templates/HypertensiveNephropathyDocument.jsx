/**
 * HypertensiveNephropathyDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: hypertensive_nephropathy
 *
 * 7 Sections:
 *   1. session-info: date (date picker), provider (string), facility (string), status (string)
 *   2. clinical-findings: findings (string), assessment (string)
 *   3. blood-pressure-control: bloodPressureControl (OBJECT — recursive editor)
 *   4. target-organ-damage: targetOrganDamage (string array)
 *   5. medications: medications (object array {class, agent, dose})
 *   6. results-section: results (OBJECT — recursive editor)
 *   7. management-plan: plan (string), recommendations (object array {recommendation, date}), notes (string)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HypertensiveNephropathyDocumentPDFTemplate from '../pdf-templates/HypertensiveNephropathyDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './HypertensiveNephropathyDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = root field name) */
const DRAFT_KEY = 'hypertensive_nephropathyPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  bloodPressureControl: 'Blood Pressure Control',
  targetOrganDamage: 'Target Organ Damage',
  medications: 'Medications',
  results: 'Results',
  plan: 'Plan',
...
const SECTION_FIELDS = {
  ...
  'blood-pressure-control': ['bloodPressureControl'],
  'target-organ-damage': ['targetOrganDamage'],
  'medications': ['medications'],
  'results-section': ['results'],
  'management-plan': ['plan', 'recommendations', 'notes'],
};

const NUMBER_FIELDS = [];

const STRING_FIELDS = ['provider', 'facility', 'status', 'findings', 'assessment', 'plan', 'notes'];
const DATE_FIELDS = ['date'];
/* OBJECT fields — recursive editor (renderObjectField/Node/Leaf) */
const OBJECT_FIELDS = ['bloodPressureControl', 'results'];
/* Array-of-strings (renderArrayField) */
const STRING_ARRAY_FIELDS = ['targetOrganDamage'];
/* Array-of-objects {recommendation, date} — date-grouped editor */
const OBJECT_ARRAY_FIELDS = ['recommendations'];
/* Array-of-objects {class, agent, dose} — per-item recursive editor */
const OBJECT_LIST_FIELDS = ['medications'];

const KEY_OVERRIDES = {
  bp: 'BP', abpm: 'ABPM', sbp: 'SBP', dbp: 'DBP', map: 'MAP',
};
...
/* ratio splitter — "120/80" stays text (BP), edits numerator only */
...
/* ═══════ COMPONENT ═══════ */
const HypertensiveNephropathyDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.hypertensive_nephropathy) return Array.isArray(r.hypertensive_nephropathy) ? r.hypertensive_nephropathy : [r.hypertensive_nephropathy];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.hypertensive_nephropathy) return Array.isArray(dd.hypertensive_nephropathy) ? dd.hypertensive_nephropathy : [dd.hypertensive_nephropathy]; return [dd]; }
      return [r];
    });
    return arr.filter(r => r && typeof r === 'object');
  }, [docProp]);

  const safeIdFor = useCallback((r) => { if (!r?._id) return null; if (typeof r._id === 'string') return r._id; if (r._id.$oid) return r._id.$oid; return String(r._id); }, []);
  ...
  const fieldSearchText = useCallback((f, val) => {
    ...
    if (OBJECT_LIST_FIELDS.includes(f)) return flattenSearchable(val);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Hypertensive Nephropathy ${idx + 1}`.toLowerCase();
  ...
  /* Stage a DRAFT locally + write it to the pending-drafts localStorage store (survives refresh).
     ...
     `fn` is the ROOT field name; `value` is the full (rebuilt) field value to store. */
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    // Rebuild the full root object with the edited leaf, then stage the whole object as a draft.
  ...
  /* ═══════ SAVE: STRING-ARRAY ITEM ═══════ */
  const saveArrayItem = useCallback((record, fn, idx, sid, arrIdx, itemKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    const currentArr = Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [];
    const trimmed = newVal.trim();
    let newArr;
    if (!trimmed) { newArr = currentArr.filter((_, i) => i !== arrIdx); }
    else { newArr = currentArr.map((v, i) => i === arrIdx ? trimmed : v); }
    setSaveError(null);
  ...
  /* ═══════ SAVE: OBJECT-LIST (array of objects) SUBFIELD ═══════ */
  const saveObjectListField = useCallback((record, fn, idx, sid, arrIdx, subKey, itemKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    const currentArr = Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [];
    const trimmed = newVal.trim();
    const newArr = currentArr.map((r, i) => i === arrIdx ? { ...r, [subKey]: trimmed } : { ...r });
    setSaveError(null);
    stageDraft(record, fn, idx, sid, newArr);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Only the pending drafts that belong to THIS section's fields.
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/hypertensive_nephropathy/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/hypertensive_nephropathy/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        text += `${label}\n`;
        (Array.isArray(val) ? val : []).filter(x => !isEmptyDeep(x)).forEach((item, i) => { text += `  ${i + 1}. ${fmtScalar(item)}\n`; });
        text += '\n';
      } else if (OBJECT_LIST_FIELDS.includes(f)) {
        text += `${label}\n`;
        (Array.isArray(val) ? val : []).filter(it => !isEmptyDeep(it)).forEach((item, i) => {
          const parts = Object.entries(item).filter(([, v]) => !isEmptyDeep(v)).map(([k, v]) => `${humanizeKey(k)}: ${fmtScalar(v)}`);
          text += `  ${i + 1}. ${parts.join('; ')}\n`;
        });
        text += '\n';
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HYPERTENSIVE NEPHROPATHY ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Hypertensive Nephropathy ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; number+unit -> number input, bool -> Yes/No select, "120/80" stays text) ═══════ */
  ...
  /* ═══════ RENDER: OBJECT FIELD (top-level recursive editor) ═══════ */
  ...
  /* ═══════ RENDER: STRING-ARRAY FIELD (editable items) ═══════ */
              ...
              <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(itemText); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveArrayItem(record, fn, idx, sid, aIdx, itemKey, editValue); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  /* ═══════ RENDER: OBJECT-LIST FIELD (array of objects {class, agent, dose}) ═══════ */
  const renderObjectListField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          const primaryKey = item.agent !== undefined ? 'agent' : Object.keys(item)[0];
          ...
          const headKey = `${fn}-${idx}-o${itemIdx}`;
          const headEditing = editingField === headKey;
          const headBadge = editedFields[headKey];
          return (
            <div key={itemIdx} className="nested-mini-card" style={{ marginTop: itemIdx > 0 ? 8 : 0 }}>
              <div className={`numbered-row ${headBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!headEditing) { setEditingField(headKey); setEditValue(fmtScalar(item[primaryKey])); setSaveError(null); } }}>
                {headEditing ? (
                  <div className="edit-field-container">
                    <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveObjectListField(record, fn, idx, sid, itemIdx, primaryKey, headKey, editValue); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <>
                    ...
                    <button className={`copy-btn ${copiedItems[headKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(heading, headKey); }}>{copiedItems[headKey] ? 'Copied!' : 'Copy'}</button>
                  </>
                )}
              </div>
                    ...
                    <div className={`numbered-row ${subBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!subEditing) { setEditingField(subKey); setEditValue(subStr); setSaveError(null); } }}>
                      {subEditing ? (
                        <div className="edit-field-container">
                          ...
                          {saveError && <div className="save-error">{saveError}</div>}
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveObjectListField(record, fn, idx, sid, itemIdx, k, subKey, editValue); }}>{saving ? 'Saving...' : 'Save'}</button>
                            <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                          </div>
                        </div>
                      ) : (
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (OBJECT_LIST_FIELDS.includes(f)) return renderObjectListField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="hypertensive-nephropathy-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Hypertensive Nephropathy</h2></div>
        <div className="empty-state">No hypertensive nephropathy records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="hypertensive-nephropathy-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Hypertensive Nephropathy</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HypertensiveNephropathyDocumentPDFTemplate document={pdfData} />} fileName={`hypertensive-nephropathy-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search hypertensive nephropathy..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Hypertensive Nephropathy ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'clinical-findings')}
            {renderSection(record, idx, 'blood-pressure-control')}
            {renderSection(record, idx, 'target-organ-damage')}
            {renderSection(record, idx, 'medications')}
            {renderSection(record, idx, 'results-section')}
            {renderSection(record, idx, 'management-plan')}
          </div>
        ))}
      </div>
    </div>
...
export default HypertensiveNephropathyDocument;