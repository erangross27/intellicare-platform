/**
 * GaitAnalysisDocumentPDFTemplate.jsx
 * Box-free B&W LETTER (canonical, memory 6a2d6af6) — mirrors the JSX: real record.date (never createdAt),
 * status enum canonical, festination / freezingOfGait.present booleans Yes/No, DOTTED-PATH fields
 * (freezingOfGait.frequency/triggers/duration resolved via getVal), values numbered ('1.' even singles),
 * single-name label gate, results object stacked, recommendations date-grouped, narratives split on [.;].
 * Rule #74: each field is ONE wrap={false} atomic View with the sectionTitle riding INSIDE the first present
 * field's View. Static PHI footer. Collection: gait_analysis.
 */
...
const SECTION_ORDER = ['session-info', 'gait-parameters', 'freezing-festination', 'balance-posture', 'clinical-notes', 'results-section', 'recommendations-section'];
...
const FIELD_LABELS = {
  ...
  gaitPattern: 'Gait Pattern', strideLength: 'Stride Length', armSwing: 'Arm Swing', turningSteps: 'Turning Steps',
  walkingSpeed: 'Walking Speed', dualTasking: 'Dual Tasking',
  'freezingOfGait.present': 'Freezing of Gait Present', 'freezingOfGait.frequency': 'Freezing Frequency',
  'freezingOfGait.triggers': 'Freezing Triggers', 'freezingOfGait.duration': 'Freezing Duration',
  festination: 'Festination', pullTest: 'Pull Test', posture: 'Posture', assistiveDevice: 'Assistive Device',
...
const GaitAnalysisDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].gait_analysis && Array.isArray(templateData[0].gait_analysis)) records = templateData[0].gait_analysis;
    else records = templateData;
  } else if (templateData && templateData.gait_analysis) {
    records = Array.isArray(templateData.gait_analysis) ? templateData.gait_analysis : [templateData.gait_analysis];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.gait_analysis ? (Array.isArray(dd.gait_analysis) ? dd.gait_analysis : [dd.gait_analysis]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Gait Analysis</Text></View>
        <Text style={styles.emptyState}>No gait analysis records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Gait Analysis ${idx + 1}`}</Text>
...
export default GaitAnalysisDocumentPDFTemplate;
