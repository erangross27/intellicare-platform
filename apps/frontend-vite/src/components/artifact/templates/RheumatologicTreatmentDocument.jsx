 ...
 * July 2026 — recursive, dot-path editable rheumatologic treatment
...
const SECTION_TITLES = {
  ...
  nsaids: 'NSAIDs',
  adjunctTherapies: 'Adjunct Therapies',
...
const FIELD_LABELS = {
  ...
  dmards: 'DMARDs', biologics: 'Biologics', corticosteroids: 'Corticosteroids', nsaids: 'NSAIDs', adjunctTherapies: 'Adjunct Therapies',
  findings: 'Findings',
...
const SECTION_FIELDS = {
  ...
  corticosteroids: ['corticosteroids'],
  nsaids: ['nsaids'],
  adjunctTherapies: ['adjunctTherapies'],
  clinicalFindings: ['findings'],
  clinicalAssessment: ['assessment'],
  treatmentPlan: ['plan'],
  results: ['results'],
...
const OBJECT_FIELDS = ['dmards', 'biologics', 'corticosteroids', 'results'];
const NARRATIVE_PATHS = ['dmards[].response', 'biologics[].response', 'corticosteroids.current'];
const PARENTHETICAL_LABEL_FIELDS = ['corticosteroids.current'];
const COMMA_FIELDS = ['dmards[].sideEffects', 'corticosteroids.complications', 'adjunctTherapies'];
const COMMA_ARRAY_FIELDS = ['dmards[].sideEffects', 'corticosteroids.complications', 'adjunctTherapies'];
const ARRAY_FIELDS = ['nsaids', 'adjunctTherapies'];
const SEMICOLON_FIELDS = ['dmards[].response', 'biologics[].response', 'findings', 'assessment', 'plan', 'notes'];
...
const KEY_LABELS = {
  startDate: 'Start Date', sideEffects: 'Side Effects', nsaids: 'NSAIDs', dmards: 'DMARDs',
...
const RheumatologicTreatmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.rheumatologic_treatment) return Array.isArray(record.rheumatologic_treatment) ? record.rheumatologic_treatment : [record.rheumatologic_treatment];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.rheumatologic_treatment) return Array.isArray(nested.rheumatologic_treatment) ? nested.rheumatologic_treatment : [nested.rheumatologic_treatment];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/rheumatologic_treatment/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/rheumatologic_treatment/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[RheumatologicTreatment] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Rheumatologic Treatment ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="rheumatologic-treatment-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Rheumatologic Treatment</h2></div><div className="empty-state">No rheumatologic treatment records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Rheumatologic Treatment ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="rheumatologic-treatment-document" ref={containerRef}>
    <div className="document-header">
      ...
      <div className="header-actions">
        ...
        <PDFDownloadLink document={<RheumatologicTreatmentDocumentPDFTemplate document={pdfData} />} fileName="Rheumatologic_Treatment.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search rheumatologic treatments..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Rheumatologic Treatment ' + (recordIndex + 1))}</h3></div>
