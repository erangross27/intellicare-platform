/**
 * SoapNotesDocument.jsx
 * July 2026 — recursive, dot-path editable SOAP notes
...
import SoapNotesDocumentPDFTemplate from '../pdf-templates/SoapNotesDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SoapNotesDocument.css';

const DRAFT_KEY = 'soap_notesPendingEdits';
const SECTION_TITLES = {
  subjectiveChiefComplaint: 'Subjective — Chief Complaint',
  subjectiveHpi: 'Subjective — History of Present Illness',
  subjectiveHistory: 'Subjective — History',
  objectiveVitals: 'Objective — Vital Signs',
...
const FIELD_LABELS = {
  chiefComplaint: 'Chief Complaint', historyOfPresentIllness: 'History of Present Illness', pastMedicalHistory: 'Past Medical History', currentMedications: 'Current Medications', allergies: 'Allergies', socialHistory: 'Social History', familyHistory: 'Family History', vitalSigns: 'Vital Signs', painScale: 'Pain Scale', bodyMassIndex: 'Body Mass Index', glasgowComaScale: 'Glasgow Coma Scale', physicalExamination: 'Physical Examination', reviewOfSystems: 'Review of Systems', laboratoryResults: 'Laboratory Results', imagingStudies: 'Imaging Studies', diagnosticTests: 'Diagnostic Tests', clinicalImpression: 'Clinical Impression', functionalStatus: 'Functional Status', riskFactors: 'Risk Factors', followUpInstructions: 'Follow-Up Instructions', patientEducation: 'Patient Education', icdTenCodes: 'ICD-10 Codes', cptCodes: 'CPT Codes', additionalData: 'Additional Data',
};
const SECTION_FIELDS = {
  subjectiveChiefComplaint: ['chiefComplaint'],
  subjectiveHpi: ['historyOfPresentIllness'],
  subjectiveHistory: ['pastMedicalHistory', 'currentMedications', 'allergies', 'socialHistory', 'familyHistory'],
  objectiveVitals: ['vitalSigns', 'painScale', 'bodyMassIndex', 'glasgowComaScale'],
  objectiveExam: ['physicalExamination', 'reviewOfSystems'],
  objectiveResults: ['laboratoryResults', 'imagingStudies', 'diagnosticTests'],
  assessment: ['clinicalImpression', 'functionalStatus', 'riskFactors'],
  plan: ['followUpInstructions', 'patientEducation'],
  coding: ['icdTenCodes', 'cptCodes'],
  additionalData: ['additionalData'],
};
const DATE_FIELDS = [];
const DATETIME_FIELDS = [];
const ENUM_FIELDS = [];
const NUMBER_FIELDS = ['painScale', 'bodyMassIndex', 'glasgowComaScale'];
const NUMBER_STRING_FIELDS = [];
const NUMBER_UNITS = { painScale: '/10', bodyMassIndex: 'kg/m²', glasgowComaScale: '/15' };
...
const COMMA_FIELDS = ['chiefComplaint', 'historyOfPresentIllness', 'vitalSigns', 'physicalExamination', 'reviewOfSystems', 'socialHistory', 'functionalStatus', 'clinicalImpression'];
const COMMA_ARRAY_SPLIT_FIELDS = [];
const ARRAY_FIELDS = ['pastMedicalHistory', 'currentMedications', 'allergies', 'familyHistory', 'laboratoryResults', 'imagingStudies', 'diagnosticTests', 'icdTenCodes', 'cptCodes', 'riskFactors'];
const SEMICOLON_FIELDS = ['patientEducation', 'clinicalImpression'];
...
const splitGuardedComma = (text) => {
  ...
  for (let index = 0; index < source.length; index += 1) {
    ...
    const dateComma = /\b(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}$/i.test(before) && /^\d{4}\b/.test(trimmed);
    const protectedComma = dateComma
      || (/\d$/.test(before) && /^\d{3}\b/.test(trimmed))
...
const SoapNotesDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.soap_notes) return Array.isArray(record.soap_notes) ? record.soap_notes : [record.soap_notes];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.soap_notes) return Array.isArray(nested.soap_notes) ? nested.soap_notes : [nested.soap_notes];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/soap_notes/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/soap_notes/' + id + '/approve', { sectionId, approved: true });
  ...
  const copyAll = useCallback(async () => {
    let text = '=== SOAP NOTES ===\n\n';
    records.forEach((record, recordIndex) => {
      text += 'SOAP Notes Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="soap-notes-document" ref={containerRef}><div className="document-header"><h2 className="document-title">SOAP Notes</h2></div><div className="empty-state">No SOAP notes available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['SOAP Notes Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="soap-notes-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">SOAP Notes</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<SoapNotesDocumentPDFTemplate document={pdfData} />} fileName="SOAP_Notes.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search SOAP notes..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('SOAP Notes Record ' + (recordIndex + 1))}</h3></div>
...
export default SoapNotesDocument;
