...
function buildContext(req, operation = 'read') { return { serviceId: 'endocrinology-consultations-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['hemoglobinA1c', 'fastingPlasmaGlucose', 'oralGlucoseToleranceTest', 'thyroidStimulatingHormone', 'freeThyroxineT4', 'freeTriiodothyronineT3', 'thyroglobulinAntibody', 'thyroidPeroxidaseAntibody', 'serumCortisol', 'dexamethasoneSuppressionTest', 'acthStimulationTest', 'parathyroidHormone', 'vitamin25Hydroxy', 'ionizedCalcium', 'insulinLevel', 'cPeptide', 'homaIrIndex', 'growthHormoneLevelBaseline', 'igf1Level', 'prolactinLevel', 'totalTestosterone', 'freeTestosterone', 'dheasLevel', 'microalbuminuria', 'diabeticRetinopathyGrade'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('endocrinology_consultations', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[EndocrinologyConsultations] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('endocrinology_consultations', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[EndocrinologyConsultations] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
