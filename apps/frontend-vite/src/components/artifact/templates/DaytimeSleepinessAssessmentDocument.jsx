/**
 * DaytimeSleepinessAssessmentDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: daytime_sleepiness_assessment
 *
 * 7 Sections:
 *   1. record-info: date, epworthSleepinessScore, averageSleepDuration, sleepLatency
 *   2. wakefulness-assessment: maintenanceOfWakefulnessScore, frequencyOfNodding, refreshedUponWaking, caffeineIntakeDaily
 *   3. sleep-symptoms: snoreFrequency, witnessedApneas, restlessnessAtNight, cataplexyPresent, sleepParalysisOccurrence, hypnagogicHallucinations
 *   4. functional-impact: drivingImpairment, occupationalImpact, nappingFrequency
 *   5. sleep-profile: chronotypeMorningEvening, shiftWorkSchedule, sleepHygieneScore
 *   6. medications: stimulantMedicationUse, sedativeMedicationUse
 *   7. scores: stopBangScore, functionalOutcomesOfSleepScore
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import DaytimeSleepinessAssessmentDocumentPDFTemplate from '../pdf-templates/DaytimeSleepinessAssessmentDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import './DaytimeSleepinessAssessmentDocument.css';
...
const DRAFT_KEY = 'daytime_sleepiness_assessmentPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const DaytimeSleepinessAssessmentDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Daytime Sleepiness Assessment ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    // Collect this record's pending edits that belong to this section (localEdits keys are "<fieldPart>-<idx>")
    const suffix = `-${idx}`;
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const isArrayIdx = dotIdx !== -1 && /^\d+$/.test(trailing); // arrayIndex ONLY when trailing segment is purely numeric
        ...
        const resp = await secureApiClient.put(`/api/edit/daytime_sleepiness_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/daytime_sleepiness_assessment/${id}/approve`, { sectionId: sid, approved: true });
      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this record's committed fields from the localStorage draft store
      const store = readDrafts();
      if (store[id]) { toCommit.forEach(k => { const fp = k.slice(0, -suffix.length); const base = fp.includes('.') ? fp.slice(0, fp.indexOf('.')) : fp; delete store[id][base]; }); if (Object.keys(store[id]).length === 0) delete store[id]; writeDrafts(store); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        text += `${formatDate(val)}\n\n`;
      } else if (NUMBER_FIELDS.includes(f) || BOOLEAN_FIELDS.includes(f)) {
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== DAYTIME SLEEPINESS ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Daytime Sleepiness Assessment ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD (stepper per checklist - NO native arrows) ═══════ */
  const renderNumberField = (record, fn, idx, sid) => {
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {isModified && <span className="modified-badge" style={{ display: 'block', marginTop: '2px' }}>edited - click Pending Approve to save</span>}
      </div>
    );
  };

  /* ═══════ RENDER: DATE FIELD (BlueDatePicker per checklist - no native date) ═══════ */
  ...
  const renderStringField = (record, fn, idx, sid) => {
            ...
            {sentences.map((sentence, sIdx) => {
                      ...
                      {commaItems.map((ci, ciIdx) => {
                        ...
                        return (
                          <div key={ciIdx}>
                            ...
                            {ciBadge && <span className={`modified-badge ${ciBadge === 'added' ? 'added' : ''}`} style={{ display: 'block', marginTop: '2px' }}>{ciBadge === 'added' ? 'added - click Pending Approve to save' : 'edited - click Pending Approve to save'}</span>}
                          </div>
                        );
                      })}
                    </div>
              ...
              return (
                <div key={sIdx} className="rec-mini-card" style={{ marginTop: 8 }}>
                  ...
                  {badge && <span className={`modified-badge ${badge === 'added' ? 'added' : ''}`} style={{ display: 'block', marginTop: '2px' }}>{badge === 'added' ? 'added - click Pending Approve to save' : 'edited - click Pending Approve to save'}</span>}
                </div>
              );
            })}
          </div>
    ...
    /* Single-value string: saveSentence(record, fn, idx, sid, 0) */
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="daytime-sleepiness-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Daytime Sleepiness Assessment</h2></div>
        <div className="empty-state">No daytime sleepiness assessment data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="daytime-sleepiness-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Daytime Sleepiness Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<DaytimeSleepinessAssessmentDocumentPDFTemplate document={pdfData} />} fileName={`daytime-sleepiness-assessment-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search daytime sleepiness assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Daytime Sleepiness Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'record-info')}
            {renderSection(record, idx, 'wakefulness-assessment')}
            {renderSection(record, idx, 'sleep-symptoms')}
            {renderSection(record, idx, 'functional-impact')}
            {renderSection(record, idx, 'sleep-profile')}
...
export default DaytimeSleepinessAssessmentDocument;
