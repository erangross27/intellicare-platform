...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'radiation-therapy-edit-service',
    userId: req.user?.id,
    operation,
...
// `planning` is a dynamic-key object — front-end edits scalar leaves via planning.<key> dot-path.
// Allow-listing the `planning` ROOT covers all subkeys (route validates rootField against this list).
const ALLOWED_FIELDS = ['site', 'intent', 'technique', 'provider', 'facility', 'status', 'dose', 'totalDose', 'fractions', 'startDate', 'endDate', 'concurrentChemo', 'planning', 'sideEffects', 'response', 'assessment', 'findings', 'plan', 'recommendations', 'notes'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('radiation_therapy', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[radiation_therapy] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('radiation_therapy', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[radiation_therapy] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
