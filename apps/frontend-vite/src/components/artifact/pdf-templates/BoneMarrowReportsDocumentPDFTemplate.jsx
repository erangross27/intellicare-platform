...
const COLLECTION = 'bone_marrow_reports';
const COMMA_SPLIT_FIELDS = [];
const ARRAY_FIELDS = new Set(['flowCytometryResults', 'fishResults', 'molecularMutations', 'dysplasticChanges']);
const OBJECT_FIELDS = new Set([]);
const NARRATIVE_FIELDS = new Set([]);
const DATE_FIELDS = new Set([]);
const ZERO_SENTINEL_FIELDS = new Set(['blastPercentage', 'ringSideroblasts', 'plasmaCellPercentage', 'lymphocytePercentage', 'biopsyLength', 'cd34PositiveCells']);
const KEY_OVERRIDES = {};
const LABELS = {
  blastPercentage: 'Blast Percentage', cellularity: 'Cellularity',
  myeloidToErythroidRatio: 'Myeloid to Erythroid Ratio', megakaryocyteCount: 'Megakaryocyte Count',
  reticulinFibrosis: 'Reticulin Fibrosis', collagenFibrosis: 'Collagen Fibrosis',
  ironStoresAssessment: 'Iron Stores Assessment', cytogeneticsKaryotype: 'Cytogenetics Karyotype',
  flowCytometryResults: 'Flow Cytometry Results', fishResults: 'FISH Results',
  molecularMutations: 'Molecular Mutations', mdsClassification: 'MDS Classification',
  blastLineage: 'Blast Lineage', dysplasticChanges: 'Dysplastic Changes', ipssRiskScore: 'IPSS Risk Score',
  ringSideroblasts: 'Ring Sideroblasts', plasmaCellPercentage: 'Plasma Cell Percentage',
  lymphocytePercentage: 'Lymphocyte Percentage', aspirateQuality: 'Aspirate Quality',
  biopsyLength: 'Biopsy Length', cd34PositiveCells: 'CD34 Positive Cells', tnfAlphaStaining: 'TNF-Alpha Staining',
};
const SECTIONS = [
  { title: 'Cell Counts', fields: ['blastPercentage', 'ringSideroblasts', 'plasmaCellPercentage', 'lymphocytePercentage', 'cd34PositiveCells'] },
  { title: 'Morphology', fields: ['cellularity', 'myeloidToErythroidRatio', 'megakaryocyteCount', 'aspirateQuality', 'biopsyLength'] },
  { title: 'Fibrosis and Iron', fields: ['reticulinFibrosis', 'collagenFibrosis', 'ironStoresAssessment'] },
  { title: 'Cytogenetics and Karyotype', fields: ['cytogeneticsKaryotype'] },
  { title: 'Flow Cytometry Results', fields: ['flowCytometryResults'] },
  { title: 'FISH Results', fields: ['fishResults'] },
  { title: 'Molecular Mutations', fields: ['molecularMutations'] },
  { title: 'Dysplastic Changes', fields: ['dysplasticChanges'] },
  { title: 'MDS Assessment', fields: ['mdsClassification', 'blastLineage', 'ipssRiskScore'] },
  { title: 'TNF-Alpha Staining', fields: ['tnfAlphaStaining'] },
];
...
const BoneMarrowReportsDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Bone Marrow Reports</Text><Text style={styles.noData}>No bone marrow report data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Bone Marrow Reports</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Bone Marrow Report {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
export default BoneMarrowReportsDocumentPDFTemplate;
