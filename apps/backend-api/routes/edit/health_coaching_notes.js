...
function buildContext(req, operation = 'read') { return { serviceId: 'health-coaching-notes-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['date', 'nextSessionScheduledDate', 'coachingSessionNumber', 'sessionDurationMinutes', 'healthRiskAssessmentScore', 'weeklyExerciseMinutes', 'sleepQualityRating', 'averageNightlySleepHours', 'selfEfficacyScore', 'stressLevel', 'goalAttainmentPercentage', 'coachingModality', 'behaviorChangeStage', 'programCompletionStatus', 'substanceUseScreening', 'primaryHealthGoals', 'nutritionGoalsSet', 'physicalActivityGoals', 'motivationalInterviewingTechniques', 'barriersIdentified', 'actionPlanSteps', 'biometricProgressTracked', 'screeningsDue', 'immunizationsDiscussed', 'socialDeterminantsAddressed', 'referralsMade'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('health_coaching_notes', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[HealthCoachingNotes] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('health_coaching_notes', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[HealthCoachingNotes] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
