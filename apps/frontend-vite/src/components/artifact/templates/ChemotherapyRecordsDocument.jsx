/**
 * ChemotherapyRecordsDocument.jsx
 * June 2026 — Collection: chemotherapy_records — FULL TEMPLATE STANDARD
 *
 * 12 non-system fields (100% schema coverage):
 *   DATE:    date (record-header date-picker)
 *   NUMBER:  cycle, day, bsa (number input, parseFloat, hide-zero)
 *   ARRAY:   medications, premedications, dosing, toxicities (string-array rows)
 *   OBJECT:  labValues, vitals (recursive renderObjectNode / renderObjectLeaf)
 *   STRING:  regimen (simple), response (per-sentence editable)
 *
 * Donor: PointOfCareUltrasoundHeartRateDocument (typed renderers, 4-level search,
 *   recursive objects, per-sentence) + PastMedicalHistoryDocument (per-sentence).
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import SearchBar from '../components/SearchBar';
import ChemotherapyRecordsPDFTemplate from '../pdf-templates/ChemotherapyRecordsPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import './ChemotherapyRecordsDocument.css';
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = a localEdits key minus its "-<idx>") */
const DRAFT_KEY = 'chemotherapy_recordsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'premedications-section': 'Premedications',
  'dosing-section': 'Dosing',
  'toxicities-section': 'Toxicities',
  'labs': 'Lab Values',
  'vitals-section': 'Vitals',
  'response-section': 'Response Assessment',
};
...
const SECTION_FIELDS = {
  'treatment': ['regimen', 'cycle', 'day', 'bsa'],
  'medications-section': ['medications'],
  'premedications-section': ['premedications'],
  'dosing-section': ['dosing'],
  'toxicities-section': ['toxicities'],
...
const SECTION_ORDER = ['treatment', 'medications-section', 'premedications-section', 'dosing-section', 'toxicities-section', 'labs', 'vitals-section', 'response-section'];

const DATE_FIELDS = ['date'];
const NUMBER_FIELDS = ['cycle', 'day', 'bsa'];
const ARRAY_FIELDS = ['medications', 'premedications', 'dosing', 'toxicities'];
const OBJECT_FIELDS = ['labValues', 'vitals'];
const SENTENCE_FIELDS = ['response'];
const SIMPLE_STRING_FIELDS = ['regimen'];
...
/* ═══════ COMPONENT ═══════ */
const ChemotherapyRecordsDocument = ({ document: docProp, data, templateData }) => {
  ...
  const fieldSearchText = useCallback((f, val) => {
    if (ARRAY_FIELDS.includes(f)) return (Array.isArray(val) ? val : []).map(fmtVal).join(' ');
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `chemotherapy record ${idx + 1}`.toLowerCase();
      if (rt.includes(phrase) || phrase.includes(rt)) { record._showAllSections = true; return true; }
      for (const t of Object.values(SECTION_TITLES)) { if (t.toLowerCase().includes(phrase) || phrase.includes(t.toLowerCase())) return true; }
      for (const l of Object.values(FIELD_LABELS)) { if (l.toLowerCase().includes(phrase) || phrase.includes(l.toLowerCase())) return true; }
      if (formatDate(getFieldValue(record, 'date', idx)).toLowerCase().includes(phrase)) return true;
  ...
  /* save an array item (string-array row) — stage a DRAFT only (no DB write until Approve) */
  const saveArrayItem = useCallback((record, fn, idx, sid, arrayIndex, trackKey) => {
    const id = safeId(record); if (!id) return;
    const currentArr = Array.isArray(getFieldValue(record, fn, idx)) ? [...getFieldValue(record, fn, idx)] : [];
    const trimmed = editValue.trim();
    setSaveError(null);
    const editKey = `${fn}-${idx}`;
    const newArr = [...currentArr]; newArr[arrayIndex] = trimmed;
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. labValues.wbc) — stage a DRAFT only (no DB write until Approve) */
  ...
  // saveSentence = stage a DRAFT only (no DB write until Approve). localStorage keeps it across refresh.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    setApproving(true);
    const fields = SECTION_FIELDS[sid] || [];
    // localEdits keys are "<field>-<idx>"; commit those that are pending AND belong to this section.
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const field = m[1];
        // arrayIndex applies only when the trailing dot-segment of the field name is purely numeric.
        const dotIdx = field.lastIndexOf('.');
        const payload = { field, value: localEdits[editKey] };
        if (dotIdx !== -1 && /^\d+$/.test(field.slice(dotIdx + 1))) {
          payload.field = field.slice(0, dotIdx);
          payload.arrayIndex = parseInt(field.slice(dotIdx + 1), 10);
        }
        const resp = await secureApiClient.put(`/api/edit/chemotherapy_records/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/chemotherapy_records/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[ChemotherapyRecords] Approve error:', err); }
  ...
  /* object → copy lines (recursive): scalar leaf = label + DASH + "1. value"; nested key = heading */
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (ARRAY_FIELDS.includes(f)) {
        emitLabel();
        (Array.isArray(val) ? val : []).filter(it => hasVal(it)).forEach((it, i) => { text += `${i + 1}. ${fmtVal(it)}\n`; });
        text += '\n';
        ...
        emitLabel();
        if (sentences.length > 1) { sentences.forEach((s, i) => { text += `${i + 1}. ${s}\n`; }); text += '\n'; }
        else { text += `1. ${strVal}\n\n`; }
      } else {
  ...
  const copyAllText = useCallback(async () => {
    let text = `CHEMOTHERAPY RECORDS\n${COPY_LINE_EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Chemotherapy Record ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
        ...
        {isEditing ? (
          <span className="edit-field-container" onClick={e => e.stopPropagation()}>
            ...
            <span className="edit-actions">
              <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date'); return; } handleSaveField(record, fn, idx, 'header', editValue + 'T00:00:00.000Z'); }}>{saving ? 'Saving...' : 'Save'}</button>
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx); if (!hasVal(val)) return null;
    const numVal = Number(val); if (!Number.isFinite(numVal) || numVal === 0) return null;
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
    const label = FIELD_LABELS[fn] || fn;
    const displayVal = String(numVal);
    const isModified = editedFields[editKey];
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                <button type="button" className="num-step" disabled={saving} onClick={e => { e.stopPropagation(); const sv = parseFloat(stepFor(numVal)) || 1; const dec = (String(sv).split('.')[1] || '').length; const cur = parseFloat(editValue); setEditValue(((isNaN(cur) ? 0 : cur) - sv).toFixed(dec)); }}>−</button>
                <input type="number" step={stepFor(numVal)} className="edit-number" value={editValue} autoFocus onChange={e => setEditValue(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); const n = parseFloat(editValue); if (!isNaN(n)) handleSaveField(record, fn, idx, sid, n); } if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                <button type="button" className="num-step" disabled={saving} onClick={e => { e.stopPropagation(); const sv = parseFloat(stepFor(numVal)) || 1; const dec = (String(sv).split('.')[1] || '').length; const cur = parseFloat(editValue); setEditValue(((isNaN(cur) ? 0 : cur) + sv).toFixed(dec)); }}>+</button>
              </div>
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const n = parseFloat(editValue); if (isNaN(n)) { setSaveError('Please enter a valid number'); return; } handleSaveField(record, fn, idx, sid, n); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
  ...
  /* ═══════ RENDER: SIMPLE STRING FIELD (regimen — single value, no sentence split) ═══════ */
  ...
  /* ═══════ RENDER: SENTENCE FIELD (response — per-sentence editable) ═══════ */
  ...
  /* ═══════ RENDER: ARRAY FIELD (string-array rows) ═══════ */
  const renderArrayField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    const items = Array.isArray(val) ? val : [];
    const visibleItems = items.map((it, ai) => ({ it, ai })).filter(({ it }) => hasVal(it));
    ...
    return (
      <div key={fn} className="rec-mini-card">
        {showSubLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
        {visibleItems.map(({ it, ai }) => {
          const itemText = fmtVal(it);
          ...
          return (
            <div key={ai}>
              <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(itemText); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter' && e.ctrlKey) { e.preventDefault(); saveArrayItem(record, fn, idx, sid, ai, itemKey); } }} />
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveArrayItem(record, fn, idx, sid, ai, itemKey); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
            ...
            <div className="edit-field-container">
              {isBool ? (
                ...
                <div className="number-edit-row">
                  <button type="button" className="num-step" disabled={saving} onClick={e => { e.stopPropagation(); const sv = parseFloat(stepFor(editStartValue)) || 1; const dec = (String(sv).split('.')[1] || '').length; const cur = parseFloat(editValue); setEditValue(((isNaN(cur) ? 0 : cur) - sv).toFixed(dec)); }}>−</button>
                  <input type="number" step={stepFor(editStartValue)} className="edit-number" value={editValue} autoFocus onChange={e => setEditValue(e.target.value)} onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                  <button type="button" className="num-step" disabled={saving} onClick={e => { e.stopPropagation(); const sv = parseFloat(stepFor(editStartValue)) || 1; const dec = (String(sv).split('.')[1] || '').length; const cur = parseFloat(editValue); setEditValue(((isNaN(cur) ? 0 : cur) + sv).toFixed(dec)); }}>+</button>
  ...
  if (!records || records.length === 0) {
    return (
      <article className="chemotherapy-records-document" ref={containerRef}>
        <header className="document-header"><h1 className="document-title">Chemotherapy Records</h1></header>
  ...
  return (
    <article className="chemotherapy-records-document" ref={containerRef}>
      <header className="document-header">
        <h1 className="document-title">Chemotherapy Records</h1>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ChemotherapyRecordsPDFTemplate document={pdfData} />} fileName="Chemotherapy_Records.pdf">
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <div className="record-title-row"><h3 className="record-name">{highlightText(`Chemotherapy Record ${idx + 1}`)}</h3></div>
            </div>
...
export default ChemotherapyRecordsDocument;
