/**
 * MacrosomiaThresholdDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: macrosomia_threshold
 *
 * 5 Sections:
 *   1. threshold-data: macrosomiaThresholdUsed, estimatedFetalWeight, birthWeight, birthWeightPercentile, gestationalAge, largeForGestationalAge
 *   2. biometry: abdominalCircumference, biparietalDiameter
 *   3. maternal-factors: maternalAge, maternalBmi, maternalDiabetesStatus, maternalGlucoseControl, previousMacrosomicInfant
 *   4. delivery-outcomes: shoulderDystocia, cesareanSection, birthTrauma
 *   5. neonatal: neonatalHypoglycemia, neonatalPolycythemia, nicuAdmission, apgarScore1Min, apgarScore5Min, umbilicalCordPh
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MacrosomiaThresholdDocumentPDFTemplate from '../pdf-templates/MacrosomiaThresholdDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './MacrosomiaThresholdDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'macrosomia_thresholdPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  macrosomiaThresholdUsed: 'Macrosomia Threshold Used',
  estimatedFetalWeight: 'Estimated Fetal Weight',
  birthWeight: 'Birth Weight',
  birthWeightPercentile: 'Birth Weight Percentile',
  gestationalAge: 'Gestational Age',
  largeForGestationalAge: 'Large for Gestational Age',
  abdominalCircumference: 'Abdominal Circumference',
  biparietalDiameter: 'Biparietal Diameter',
  maternalAge: 'Maternal Age',
  maternalBmi: 'Maternal BMI',
  maternalDiabetesStatus: 'Maternal Diabetes Status',
  maternalGlucoseControl: 'Maternal Glucose Control',
  previousMacrosomicInfant: 'Previous Macrosomic Infant',
  shoulderDystocia: 'Shoulder Dystocia',
  cesareanSection: 'Cesarean Section',
  birthTrauma: 'Birth Trauma',
  neonatalHypoglycemia: 'Neonatal Hypoglycemia',
  neonatalPolycythemia: 'Neonatal Polycythemia',
  nicuAdmission: 'NICU Admission',
  apgarScore1Min: 'Apgar Score (1 min)',
  apgarScore5Min: 'Apgar Score (5 min)',
  umbilicalCordPh: 'Umbilical Cord pH',
};

const SECTION_FIELDS = {
  'threshold-data': ['macrosomiaThresholdUsed', 'estimatedFetalWeight', 'birthWeight', 'birthWeightPercentile', 'gestationalAge', 'largeForGestationalAge'],
  'biometry': ['abdominalCircumference', 'biparietalDiameter'],
  'maternal-factors': ['maternalAge', 'maternalBmi', 'maternalDiabetesStatus', 'maternalGlucoseControl', 'previousMacrosomicInfant'],
  'delivery-outcomes': ['shoulderDystocia', 'cesareanSection', 'birthTrauma'],
  'neonatal': ['neonatalHypoglycemia', 'neonatalPolycythemia', 'nicuAdmission', 'apgarScore1Min', 'apgarScore5Min', 'umbilicalCordPh'],
};

const NUMBER_FIELDS = ['birthWeight', 'gestationalAge', 'macrosomiaThresholdUsed', 'birthWeightPercentile', 'maternalBmi', 'estimatedFetalWeight', 'abdominalCircumference', 'biparietalDiameter', 'maternalAge', 'apgarScore1Min', 'apgarScore5Min', 'umbilicalCordPh'];
/* Numeric fields where 0 is a "not measured" sentinel (physiologically impossible / not a real value) → hide.
   EXCEPTION: apgarScore1Min/apgarScore5Min — 0 is a genuine, critical clinical Apgar value, never hide. */
const ZERO_SENTINEL_FIELDS = ['birthWeight', 'gestationalAge', 'macrosomiaThresholdUsed', 'birthWeightPercentile', 'maternalBmi', 'estimatedFetalWeight', 'abdominalCircumference', 'biparietalDiameter', 'maternalAge', 'umbilicalCordPh'];
const BOOLEAN_FIELDS = ['largeForGestationalAge', 'shoulderDystocia', 'cesareanSection', 'neonatalHypoglycemia', 'previousMacrosomicInfant', 'neonatalPolycythemia', 'nicuAdmission'];
const ARRAY_FIELDS = ['birthTrauma'];
const STRING_FIELDS = ['maternalDiabetesStatus', 'maternalGlucoseControl'];
...
/* ═══════ COMPONENT ═══════ */
const MacrosomiaThresholdDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.macrosomia_threshold) return Array.isArray(r.macrosomia_threshold) ? r.macrosomia_threshold : [r.macrosomia_threshold];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.macrosomia_threshold) return Array.isArray(dd.macrosomia_threshold) ? dd.macrosomia_threshold : [dd.macrosomia_threshold]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // This template stages every edit under the plain field name ("fn"); editKey = "fn-idx".
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Macrosomia Threshold ${idx + 1}`.toLowerCase();
  ...
  // Stage a full-text DRAFT for a string field (no DB write). localStorage keeps it across refresh; Approve commits.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const suffix = `-${idx}`;
      // Staged edits for THIS record (localEdits keys are "field-idx"); keep only this section's fields.
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length); // "field" (this template never uses "field.arrayIndex" here)
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/macrosomia_threshold/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/macrosomia_threshold/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (ARRAY_FIELDS.includes(f)) {
        ...
        body += head;
        items.forEach((item, i) => { body += `${i + 1}. ${String(item).replace(/[;.]+$/, '').trim()}\n`; });
        body += '\n';
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MACROSOMIA THRESHOLD ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Macrosomia Threshold ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="macrosomia-threshold-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Macrosomia Threshold</h2></div>
        <div className="empty-state">No macrosomia threshold records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="macrosomia-threshold-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Macrosomia Threshold</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MacrosomiaThresholdDocumentPDFTemplate document={pdfData} />} fileName="Macrosomia_Threshold.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search macrosomia threshold records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Macrosomia Threshold ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'threshold-data')}
            {renderSection(record, idx, 'biometry')}
            {renderSection(record, idx, 'maternal-factors')}
            {renderSection(record, idx, 'delivery-outcomes')}
            {renderSection(record, idx, 'neonatal')}
          </div>
        ))}
      </div>
    </div>
...
export default MacrosomiaThresholdDocument;
