/**
 * SymptomProgressionTimelineDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: symptom_progression_timeline
 *
 * Covers symptom overview, severity scores, factors, functional measures,
 * intervention response, and biomarker values.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import SymptomProgressionTimelineDocumentPDFTemplate from '../pdf-templates/SymptomProgressionTimelineDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SymptomProgressionTimelineDocument.css';
...
const DRAFT_KEY = 'symptomProgressionTimelinePendingEdits';
const readDrafts = () => {
...
const NUMBER_FIELDS = new Set(['symptomSeverityScore', 'painIntensityNrs', 'functionalCapacityMets', 'functionalImpactScore', 'qualityOfLifeScore', 'glasgowComaScale', 'edssScore', 'symptomDurationMinutes', 'walkingDistanceMeters', 'sixMinuteWalkDistance', 'timeToMaximumSymptom']);
const ENUM_FIELDS = { nyhaClassification: ['I', 'II', 'III', 'IV'] };
...
/* ═══════ COMPONENT ═══════ */
const SymptomProgressionTimelineDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.symptom_progression_timeline) return Array.isArray(r.symptom_progression_timeline) ? r.symptom_progression_timeline : [r.symptom_progression_timeline];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.symptom_progression_timeline) return Array.isArray(dd.symptom_progression_timeline) ? dd.symptom_progression_timeline : [dd.symptom_progression_timeline]; return [dd]; }
      return [r];
    });
  ...
  const fmtVal = useCallback((input) => { const v = scalarValue(input); if (typeof v === 'boolean') return v ? 'Yes' : 'No'; if (typeof v === 'number') return String(v); return String(v || '').replace(/\u03bcg/g, 'mcg').replace(/\u03bc/g, 'u').replace(/\u2192/g, '->'); }, []);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Symptom Progression Timeline ${idx + 1}`.toLowerCase();
  ...
  const sectionHasEdits = useCallback((record, idx, sid) => {
    const fields = sectionFields(record, sid);
    const inSection = path => fields.some(field => path === field || path.startsWith(`${field}.`));
    return fields.some(f =>
      Object.keys(pendingEdits).some(k => { const match = k.match(/^(.+)-(\d+)$/); return match && Number(match[2]) === idx && inSection(match[1]); }) ||
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      const toCommit = Object.keys(localEdits).filter(k => {
        ...
        return fields.some(field => m[1] === field || m[1].startsWith(`${field}.`));
      });
      for (const editKey of toCommit) {
        const m = editKey.match(/^(.+)-(\d+)$/);
        const fieldPart = m[1];
        const payload = { field: fieldPart, value: localEdits[editKey] };
        const resp = await secureApiClient.put(`/api/edit/symptom_progression_timeline/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/symptom_progression_timeline/${id}/approve`, { sectionId: sid, approved: true });
      ...
      const store = readDrafts();
      if (store[id]) {
        Object.keys(store[id] || {}).forEach(path => {
          if (fields.some(field => path === field || path.startsWith(`${field}.`))) delete store[id][path];
        });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (ARRAY_FIELDS.has(f)) {
        ...
        let unlabeledNumber = 1;
        val.forEach(item => {
          buildStringGroups(fmtVal(item), f).forEach(group => {
            if (group.label) {
              body.push(group.label, COPY_LINE_DASH);
              group.rows.forEach((row, itemIndex) => body.push(`${itemIndex + 1}. ${row.text}`));
            } else group.rows.forEach(row => body.push(`${unlabeledNumber++}. ${row.text}`));
          });
        });
        body.push('');
        return;
      }
      const formattedDate = isDateField(f) ? formatDate(val) : '';
      if (isDateField(f) && !formattedDate) return;
      if (!sameAsTitle(label, title)) body.push(label, COPY_LINE_DASH);
      if (isDateField(f)) {
        body.push(`1. ${formattedDate}`);
      } else {
  ...
  const copyAllText = useCallback(async () => {
    ...
    pdfData.forEach((r, idx) => {
      text += `Symptom Progression Timeline ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    ...
    const isEditing = editingField === editKey;
    const label = fieldLabel(fn);
    const displayVal = String(scalarValue(val));
    const isModified = editedFields[editKey];
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayVal); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              ...
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={event => { event.stopPropagation(); handleSaveField(record, fn, idx, sid, null, editValue); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={event => { event.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
  ...
  const renderStringField = (record, fn, idx, sid, title, embedded = false) => {
    const val = getFieldValue(record, fn, idx); if (!hasVal(val)) return null;
    ...
    if (sentences.length > 1 || (sentences[0] && parseLabel(sentences[0]).isLabeled) || COMMA_ARRAY_FIELDS.has(fn)) {
      ...
      return (
        <div key={fn}>
          <div className={embedded ? '' : 'rec-mini-card nested-mini-card'}>
            {!embedded && !sameAsTitle(label, title) && <div className="nested-subtitle">{highlightText(label)}</div>}
    ...
    return (
      <div key={fn} className={embedded ? '' : 'rec-mini-card nested-mini-card'} data-edit-field={fn}>
        {!embedded && !sameAsTitle(label, title) && <div className="nested-subtitle">{highlightText(label)}</div>}
  ...
  /* ═══════ RENDER: ARRAY FIELD as one semantic mini-card with indexed editable leaves ═══════ */
  const renderArrayField = (record, fn, idx, sid, title) => {
    const values = getFieldValue(record, fn, idx);
    if (!Array.isArray(values) || !values.some(hasVal)) return null;
    const label = fieldLabel(fn);
    return (
      <div key={fn} className="rec-mini-card nested-mini-card">
        ...
        {values.map((value, arrayIndex) => hasVal(value)
          ? renderStringField(record, `${fn}.${arrayIndex}`, idx, sid, title, true)
          : null)}
      </div>
    );
  };
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (ARRAY_FIELDS.has(f)) return renderArrayField(record, f, idx, sid, title);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="symptom-progression-timeline-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Symptom Progression Timeline</h2></div>
        <div className="empty-state">No symptom progression timeline records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="symptom-progression-timeline-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Symptom Progression Timeline</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<SymptomProgressionTimelineDocumentPDFTemplate document={pdfData} />} fileName="Symptom_Progression_Timeline.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search symptom progression timeline..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Symptom Progression Timeline ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'symptom-overview')}
            {renderSection(record, idx, 'severity-scores')}
            {renderSection(record, idx, 'factors-symptoms')}
            {renderSection(record, idx, 'functional-measures')}
            {renderSection(record, idx, 'intervention-biomarkers')}
          </div>
        ))}
      </div>
    </div>
...
export default SymptomProgressionTimelineDocument;
