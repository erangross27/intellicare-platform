/**
 * PsychiatricProgressNotesDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: psychiatric_progress_notes
 *
 * 5 Sections:
 *   1. visit-info: consultDate
 *   2. referral: referralReason, chiefComplaint
 *   3. examination: examFindings
 *   4. diagnosis-plan: diagnosis, treatmentPlan
 *   5. follow-up: followUpPlan, recommendations
 */
...
const SEMICOLON_FIELDS = ['treatmentPlan', 'followUpPlan'];
...
const DRAFT_KEY = 'psychiatric_progress_notesPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const PsychiatricProgressNotesDocument = ({ document: docProp }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayElem) {
          // array element draft (e.g. "recommendations.2" = single element). localEdits holds the
          // WHOLE array under "field-idx"; rebuild it from the record + any element drafts.
          const baseField = fieldPart.slice(0, lastDot);
          const arrIdx = parseInt(fieldPart.slice(lastDot + 1), 10);
          const localKey = `${baseField}-${idx}`;
          const arr = Array.isArray(nLocal[localKey]) ? [...nLocal[localKey]]
  ...
  const splitBySemicolon = useCallback((text) => {
    if (!text || typeof text !== 'string') return [];
    const result = []; let current = ''; let parenDepth = 0;
    for (let i = 0; i < text.length; i++) {
      const ch = text[i];
      if (ch === '(') parenDepth++;
      else if (ch === ')') parenDepth = Math.max(0, parenDepth - 1);
      if (ch === ';' && parenDepth === 0) { const trimmed = current.trim(); if (trimmed) result.push(trimmed); current = ''; }
      else { current += ch; }
    }
    const trimmed = current.trim(); if (trimmed) result.push(trimmed);
    return result;
  }, []);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Psychiatric Progress Notes ${idx + 1}`.toLowerCase();
  ...
  // Stage a sentence/semicolon-item edit as a DRAFT (no DB write). localStorage keeps it across
  // refresh; Approve commits it.
  function stageSentenceDraft(record, fn, idx, sid, fullText, marks) {
    const id = safeId(record); if (!id) return;
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    /* Determine if this field uses semicolons */
    const useSemicolon = SEMICOLON_FIELDS.includes(fn);
    const items = useSemicolon ? splitBySemicolon(currentVal) : splitBySentence(currentVal);
    const editedVal = editValue.trim();
    setSaveError(null);

    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      const updated = [...items]; updated.splice(sentenceIdx, 1);
      const fullText = useSemicolon ? updated.join('; ') : reconstructFullText(updated);
      stageSentenceDraft(record, fn, idx, sid, fullText, { [`${fn}-${idx}-s${sentenceIdx}`]: 'edited' });
      return;
    }

    if (useSemicolon) {
      /* Semicolon field: split editValue by semicolons, splice into items */
      const newItems = splitBySemicolon(editedVal);
      const updated = [...items]; updated.splice(sentenceIdx, 1, ...newItems);
      const fullText = updated.join('; ');
      const orig = items[sentenceIdx] || '';
      const changed = newItems[0]?.trim() !== orig.trim();
      const marks = {};
      if (changed) marks[`${fn}-${idx}-s${sentenceIdx}`] = 'edited';
      const extra = newItems.length - 1;
      for (let ei = 0; ei < extra; ei++) marks[`${fn}-${idx}-s${sentenceIdx + 1 + ei}`] = 'added';
      stageSentenceDraft(record, fn, idx, sid, fullText, marks);
    } else {
      /* Sentence field: handle '. Test' splitting */
      const newSentences = splitBySentence(editedVal);
      const updated = [...items]; updated.splice(sentenceIdx, 1, ...newSentences);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Collect this record's staged drafts for this section's fields.
      const store = readDrafts();
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const baseField = hasArrayIndex ? fieldPart.slice(0, lastDot) : fieldPart;
        ...
        const resp = await secureApiClient.put(`/api/edit/psychiatric_progress_notes/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedLocalKeys.push(`${baseField}-${idx}`);
        delete store[id][fieldPart];
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/psychiatric_progress_notes/${id}/approve`, { sectionId: sid, approved: true });

      // Drop committed drafts from localStorage
  ...
  const formatSentenceFieldLines = useCallback((text, useSemicolon) => {
    const items = useSemicolon ? splitBySemicolon(text) : splitBySentence(text);
    const lines = []; let n = 1;
    items.forEach(s => {
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PSYCHIATRIC PROGRESS NOTES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Psychiatric Progress Notes ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: ARRAY FIELD (recommendations — objects with {recommendation, date}) ═══════ */
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(recText); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { const newItems = subParts.map(sp => ({ recommendation: sp, date: item?.date || '' })); currentArr.splice(itemIdx, 1, ...newItems); } else { currentArr[itemIdx] = { ...item, recommendation: trimmed }; } /* Stage as a DRAFT (no DB write); Approve commits per-element with arrayIndex. */ setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: currentArr })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { const k = `${sid}-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; }); const store = readDrafts(); if (!store[id]) store[id] = {}; store[id][`${fn}.${itemIdx}`] = currentArr[itemIdx]; writeDrafts(store); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderStringField = (record, fn, idx, sid, title) => {
    ...
    /* Multi-item: render each item as editable row */
    if (items.length > 1) {
                            ...
                            <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                              {ciEditing ? (
                                <div className="edit-field-container">
                                  ...
                                  {saveError && <div className="save-error">{saveError}</div>}
                                  <div className="edit-actions">
                                    <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const currentVal2 = String(getFieldValue(record, fn, idx) || ''); const items2 = useSemicolon ? splitBySemicolon(currentVal2) : splitBySentence(currentVal2); const s2 = items2[sIdx] || ''; const p2 = parseLabel(s2); if (!p2.isLabeled) return; const cItems2 = splitByComma(p2.value); const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { cItems2.splice(ciIdx, 1, ...subParts); } else { cItems2[ciIdx] = trimmed.replace(/[;.]+$/, '').trim(); } const rebuilt = useSemicolon ? `${p2.label}: ${cItems2.join(', ')}` : `${p2.label}: ${cItems2.join(', ')}.`; const allS = [...items2]; allS[sIdx] = rebuilt; const fullText2 = useSemicolon ? allS.join('; ') : reconstructFullText(allS); setSaveError(null); /* Stage as a DRAFT (no DB write); Approve commits it. */ setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullText2 })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); const marks = { [commaKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ciIdx + ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setApprovedSections(prev => { const k = `${sid}-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; }); const dstore = readDrafts(); if (!dstore[id2]) dstore[id2] = {}; dstore[id2][fn] = fullText2; writeDrafts(dstore); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                                    <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                                  </div>
                                </div>
                              ) : (
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = useSemicolon ? `${parsed.label}: ${newValue}` : `${parsed.label}: ${newValue}`; const items2 = useSemicolon ? splitBySemicolon(String(getFieldValue(record, fn, idx) || '')) : splitBySentence(String(getFieldValue(record, fn, idx) || '')); items2[sIdx] = reconstructed; const fullText = useSemicolon ? items2.join('; ') : reconstructFullText(items2); const idL = safeId(record); if (!idL) return; setSaveError(null); /* Stage as a DRAFT (no DB write); Approve commits it. */ setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullText })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setApprovedSections(prev => { const k = `${sid}-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; }); const dstore = readDrafts(); if (!dstore[idL]) dstore[idL] = {}; dstore[idL][fn] = fullText; writeDrafts(dstore); setEditingField(null); setEditValue(''); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="psychiatric-progress-notes-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Psychiatric Progress Notes</h2></div>
        <div className="empty-state">No psychiatric progress notes records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="psychiatric-progress-notes-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search psychiatric progress notes records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'examination')}
            {renderSection(record, idx, 'diagnosis-plan')}
...
export default PsychiatricProgressNotesDocument;
