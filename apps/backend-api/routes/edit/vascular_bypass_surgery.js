...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'vascular-bypass-surgery-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'bypassGraftType',
  'proximalAnastomosisLocation',
  'distalAnastomosisLocation',
  'graftDiameterMillimeters',
  'graftLengthCentimeters',
  'saphenousVeinDiameter',
  'rutherfordClassification',
  'wifiScore',
  'tasciilClassification',
  'runoffScore',
  'glasgowAneurysmScore',
  'preoperativeAnkleBrachialIndex',
  'postoperativeAnkleBrachialIndex',
  'toeBrachialIndex',
  'transcutaneousOxygenPressure',
  'intraoperativeGraftFlowRate',
  'peakSystolicVelocityGraft',
  'velocityRatioVr',
  'clampTimeMinutes',
  'estimatedBloodLossMilliliters',
  'completionAngiogramResult',
  'graftPatencyStatus',
  'limbSalvageStatus',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('vascular_bypass_surgery', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[VascularBypassSurgery] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('vascular_bypass_surgery', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[VascularBypassSurgery] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
