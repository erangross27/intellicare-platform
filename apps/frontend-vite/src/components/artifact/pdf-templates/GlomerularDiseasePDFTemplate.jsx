/**
 * GlomerularDiseasePDFTemplate.jsx
 * Box-free B&W LETTER (canonical, memory 6a2d6af6) — mirrors the JSX: real record.date (never createdAt),
 * DOTTED-PATH biopsyFindings.* / serologies.* fields resolved via getVal, status enum canonical, only LONG_TEXT
 * fields sentence-split ([.;] with abbrev/single-initial/genus guard + labeled comma-split), glomeruli kept whole
 * (NO_SPLIT — bare-numeric "Total glomeruli: 22" clause), immunosuppression array + recommendations date-grouped,
 * results recursive object, values numbered ('1.' even singles), single-name label gate. Rule #74: each field is
 * ONE wrap={false} atomic View with the sectionTitle riding INSIDE the first present field's View. Static PHI
 * footer. Collection: glomerular_disease.
 */
...
const SECTION_ORDER = ['session-info', 'biopsy-findings', 'serologies', 'clinical', 'management'];
...
const FIELD_LABELS = {
  date: 'Date', provider: 'Provider', facility: 'Facility', status: 'Status', diagnosis: 'Diagnosis',
  'biopsyFindings.date': 'Biopsy Date', 'biopsyFindings.glomeruli': 'Glomeruli', 'biopsyFindings.interstitium': 'Interstitium',
  'biopsyFindings.vessels': 'Vessels', 'biopsyFindings.immunofluorescence': 'Immunofluorescence', 'biopsyFindings.electronMicroscopy': 'Electron Microscopy',
  'serologies.ana': 'ANA', 'serologies.antiDsDna': 'Anti-dsDNA', 'serologies.anca': 'ANCA', 'serologies.antiGbm': 'Anti-GBM',
  'serologies.complement.C3': 'Complement C3', 'serologies.complement.C4': 'Complement C4',
  findings: 'Findings', assessment: 'Assessment', plan: 'Plan', immunosuppression: 'Immunosuppression',
...
const GlomerularDiseasePDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].glomerular_disease && Array.isArray(templateData[0].glomerular_disease)) records = templateData[0].glomerular_disease;
    else records = templateData;
  } else if (templateData && templateData.glomerular_disease) {
    records = Array.isArray(templateData.glomerular_disease) ? templateData.glomerular_disease : [templateData.glomerular_disease];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.glomerular_disease ? (Array.isArray(dd.glomerular_disease) ? dd.glomerular_disease : [dd.glomerular_disease]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Glomerular Disease</Text></View>
        <Text style={styles.emptyState}>No glomerular disease records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Glomerular Disease ${idx + 1}`}</Text>
...
export default GlomerularDiseasePDFTemplate;
