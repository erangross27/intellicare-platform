/**
 * ConsultationTimelineDocument.jsx
 * June 2026 — FULL TEMPLATE STANDARD rewrite (100% coverage, 15 fields)
 * Collection: consultation_timeline
 *
 * Sections:
 *   1. timeline-info: date(date-picker), specialty, consultant, provider, facility, type, status, requestedTime, responseTime (simple)
 *   2. clinical: findings, assessment, plan (per-sentence narratives)
 *   3. results-section: results (recursive OBJECT)
 *   4. follow-up: recommendations, notes (per-sentence narratives)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ConsultationTimelineDocumentPDFTemplate from '../pdf-templates/ConsultationTimelineDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './ConsultationTimelineDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits field name, e.g. "findings" or "results") */
const DRAFT_KEY = 'consultation_timelinePendingEdits';
const readDrafts = () => {
...
const SIMPLE_FIELDS = ['specialty', 'consultant', 'provider', 'facility', 'type', 'status', 'requestedTime', 'responseTime'];
...
/* parseLabel: detect "Label: value" patterns in sentences */
...
/* ═══════ COMPONENT ═══════ */
const ConsultationTimelineDocument = ({ document: docProp }) => {
  ...
  const formatDate = useCallback((d) => { if (!d) return ''; try { const dt = new Date(d.$date || d); if (isNaN(dt.getTime())) return String(d); return dt.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }); } catch { return String(d); } }, []);
  ...
  /* getFieldValue: supports flat fields + object fields via localEdits */
  ...
  /* flatten any value (incl. objects) into a searchable string */
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (DATE_FIELDS.includes(f)) { if (val && formatDate(val).toLowerCase().includes(phrase)) return true; }
      else if (flattenSearchable(val).toLowerCase().includes(phrase)) return true;
    }
    return false;
  }, [searchTerm, getFieldValue, formatDate, flattenSearchable]);
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (DATE_FIELDS.includes(fn)) return val && formatDate(val).toLowerCase().includes(phrase);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Consultation Timeline ${idx + 1}`.toLowerCase();
  ...
  }, [records, searchTerm, getFieldValue, formatDate, flattenSearchable]);
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    // Stage as a DRAFT (no DB write). localStorage keeps it across refresh; Approve commits the whole object.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // staged edits for THIS record + section: localEdits key = `${field}-${idx}` and pending
    const toCommit = Object.keys(localEdits).filter(k => {
      if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
      const fieldPart = k.slice(0, -suffix.length);
      const baseField = /\.\d+$/.test(fieldPart) ? fieldPart.replace(/\.\d+$/, '') : fieldPart;
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const arrMatch = fieldPart.match(/\.(\d+)$/);
        if (arrMatch) { payload.field = fieldPart.slice(0, fieldPart.lastIndexOf('.')); payload.arrayIndex = parseInt(arrMatch[1], 10); }
        else { payload.field = fieldPart; }
        const resp = await secureApiClient.put(`/api/edit/consultation_timeline/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/consultation_timeline/${id}/approve`, { sectionId: sid, approved: true });
      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this record's committed drafts from localStorage
      const store = readDrafts();
      if (store[id]) {
        toCommit.forEach(k => { const fieldPart = k.slice(0, -suffix.length); const baseField = fieldPart.replace(/\.\d+$/, ''); delete store[id][baseField]; });
    ...
    } catch (err) { console.error('[ConsultationTimeline] Approve error:', err); }
  ...
  /* sentence field → numbered lines (parseLabel + comma-split) */
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    return text;
  }, [getFieldValue, hasVal, fmtVal, formatDate, objectCopyLines, formatSentenceFieldLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== CONSULTATION TIMELINE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Consultation Timeline ${idx + 1}\n${'='.repeat(40)}\n\n`;
          ...
          {isEditing ? (
            <div className="edit-field-container">
              ...
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date'); return; } handleSaveField(record, fn, idx, sid, editValue + 'T00:00:00.000Z', editKey); }}>{saving ? 'Saving...' : 'Save'}</button>
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayVal); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              ...
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const out = isBool ? (editValue === 'Yes') : editValue; handleSaveField(record, fn, idx, sid, out, editKey); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="consultation-timeline-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Consultation Timeline</h2></div>
        <div className="empty-state">No consultation timeline records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="consultation-timeline-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Consultation Timeline</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ConsultationTimelineDocumentPDFTemplate document={pdfData} />} fileName={`consultation-timeline-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search consultation timeline..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Consultation Timeline ${idx + 1}`)}</h3>
            </div>
...
export default ConsultationTimelineDocument;
