 ...
 * July 2026 — recursive, dot-path editable skin grafting evaluation
...
const SECTION_TITLES = {
  graftSpecifications: 'Graft Specifications',
  recipientBed: 'Recipient Bed Assessment',
  graftOutcomes: 'Graft Outcomes',
  donorSite: 'Donor Site',
  graftComplications: 'Graft Complications',
  perfusionLabs: 'Perfusion & Labs',
  scarTherapy: 'Scar & Therapy',
};
const FIELD_LABELS = {
...
const SECTION_FIELDS = {
  graftSpecifications: ['graftDonorSiteLocation', 'graftThicknessType', 'splitThicknessGraftDepth', 'graftExpansionRatio'],
  recipientBed: ['recipientBedVascularityScore', 'woundBedPreparationMethod', 'woundBedBacterialBioburden', 'recipientSiteArea'],
  graftOutcomes: ['graftTakePercentage', 'graftFixationTechnique', 'plasmaticImbibitionStatus', 'inosculationOnsetDay'],
  donorSite: ['donorSiteHealingDays', 'seromaPrevention', 'totalBurnSurfaceArea'],
  graftComplications: ['graftFailureMechanism', 'graftContracturePercentage', 'dermalSubstitutePriorUse'],
  perfusionLabs: ['recipientSiteTranscutaneousOxygenTension', 'laserDopplerPerfusionIndex', 'preoperativeAlbuminLevel', 'hemoglobinA1cLevel'],
  scarTherapy: ['vancouverScarScaleScore', 'cutometerElasticityMeasurement', 'negativePresureWoundTherapySettings'],
};
const DATE_FIELDS = [];
const DATETIME_FIELDS = [];
const ENUM_FIELDS = [];
const NUMBER_FIELDS = ['splitThicknessGraftDepth', 'recipientBedVascularityScore', 'recipientSiteArea', 'graftTakePercentage', 'inosculationOnsetDay', 'donorSiteHealingDays', 'totalBurnSurfaceArea', 'graftContracturePercentage', 'recipientSiteTranscutaneousOxygenTension', 'laserDopplerPerfusionIndex', 'preoperativeAlbuminLevel', 'hemoglobinA1cLevel', 'vancouverScarScaleScore', 'cutometerElasticityMeasurement'];
...
const LABELED_LIST_FIELDS = ['woundBedBacterialBioburden', 'negativePresureWoundTherapySettings'];
...
const SkinGraftingEvaluationDocument = ({ document: docProp }) => {
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/skin_grafting_evaluation/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/skin_grafting_evaluation/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SkinGraftingEvaluation] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Skin Grafting Evaluation Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="skin-grafting-evaluation-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Skin Grafting Evaluation</h2></div><div className="empty-state">No skin grafting evaluation available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Skin Grafting Evaluation Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="skin-grafting-evaluation-document" ref={containerRef}>
    <div className="document-header">
      ...
      <div className="header-actions">
        ...
        <PDFDownloadLink document={<SkinGraftingEvaluationDocumentPDFTemplate document={pdfData} />} fileName="Skin_Grafting_Evaluation.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search skin grafting evaluation..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Skin Grafting Evaluation Record ' + (recordIndex + 1))}</h3></div>
