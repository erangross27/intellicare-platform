/**
 * EggRetrievalProcedureDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: egg_retrieval_procedure
 *
 * 6 Sections:
 *   1. procedure-info: procedureDate, retrievalPhysician, anesthesiaType, procedureDuration, needleGauge, aspirationPressure
 *   2. oocyte-results: totalOocytesRetrieved, matureOocytesMII, immatureOocytesMI, germinaalVesicleOocytes, degeneratedOocytes
 *   3. ovarian-details: leftOvaryFolliclesAspirated, rightOvaryFolliclesAspirated, antralFollicleCount, antiMullerianHormoneLevel
 *   4. hormonal-levels: preRetrievalEstradiolLevel, preRetrievalProgesteroneLevel, endometrialThickness
 *   5. trigger-protocol: triggerMedicationType, triggerToRetrievalInterval, flushingMediaUsed
 *   6. post-procedure: ovarianHyperstimulationRisk, proceduralComplications, estimatedBloodLoss, postProcedurePainScore
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import EggRetrievalProcedureDocumentPDFTemplate from '../pdf-templates/EggRetrievalProcedureDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './EggRetrievalProcedureDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'egg_retrieval_procedurePendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'oocyte-results': 'Oocyte Results',
  'ovarian-details': 'Ovarian Details',
  'hormonal-levels': 'Hormonal Levels',
...
const FIELD_LABELS = {
  ...
  retrievalPhysician: 'Retrieval Physician',
  anesthesiaType: 'Anesthesia Type',
  procedureDuration: 'Procedure Duration (min)',
  needleGauge: 'Needle Gauge',
  aspirationPressure: 'Aspiration Pressure (mmHg)',
  totalOocytesRetrieved: 'Total Oocytes Retrieved',
  matureOocytesMII: 'Mature Oocytes (MII)',
  immatureOocytesMI: 'Immature Oocytes (MI)',
  germinaalVesicleOocytes: 'Germinal Vesicle Oocytes',
  degeneratedOocytes: 'Degenerated Oocytes',
  leftOvaryFolliclesAspirated: 'Left Ovary Follicles Aspirated',
  rightOvaryFolliclesAspirated: 'Right Ovary Follicles Aspirated',
  antralFollicleCount: 'Antral Follicle Count',
  antiMullerianHormoneLevel: 'Anti-Mullerian Hormone Level (ng/mL)',
  preRetrievalEstradiolLevel: 'Pre-Retrieval Estradiol Level (pg/mL)',
  preRetrievalProgesteroneLevel: 'Pre-Retrieval Progesterone Level (ng/mL)',
  endometrialThickness: 'Endometrial Thickness (mm)',
  triggerMedicationType: 'Trigger Medication Type',
  triggerToRetrievalInterval: 'Trigger to Retrieval Interval (hrs)',
  flushingMediaUsed: 'Flushing Media Used',
  ovarianHyperstimulationRisk: 'Ovarian Hyperstimulation Risk',
  proceduralComplications: 'Procedural Complications',
  estimatedBloodLoss: 'Estimated Blood Loss (mL)',
  postProcedurePainScore: 'Post-Procedure Pain Score',
};

const SECTION_FIELDS = {
  'procedure-info': ['procedureDate', 'retrievalPhysician', 'anesthesiaType', 'procedureDuration', 'needleGauge', 'aspirationPressure'],
  'oocyte-results': ['totalOocytesRetrieved', 'matureOocytesMII', 'immatureOocytesMI', 'germinaalVesicleOocytes', 'degeneratedOocytes'],
  'ovarian-details': ['leftOvaryFolliclesAspirated', 'rightOvaryFolliclesAspirated', 'antralFollicleCount', 'antiMullerianHormoneLevel'],
  'hormonal-levels': ['preRetrievalEstradiolLevel', 'preRetrievalProgesteroneLevel', 'endometrialThickness'],
  'trigger-protocol': ['triggerMedicationType', 'triggerToRetrievalInterval', 'flushingMediaUsed'],
  'post-procedure': ['ovarianHyperstimulationRisk', 'proceduralComplications', 'estimatedBloodLoss', 'postProcedurePainScore'],
};
...
/* ENUMS (act like a medical professional — IVF/REI catalog, memory 6a4abb66 + 6a4908b2). Stored lowercase
   ('low') → enumCanonical Title-Cases the display+save; an off-scale descriptive value (the other record's
   anesthesiaType 'Monitored Anesthesia Care (MAC) with IV propofol…') is kept via enumOptionsWith. */
const ENUM_OPTIONS = {
...
/* HIDE-ZERO: the batch extractor writes 0 into numerics it could not read (sentinel). Cross-record evidence
   (2 records) + clinical impossibility (folliclesAspirated 0 while oocytes were retrieved) → hide those 0s.
   MEANINGFUL_ZERO_FIELDS = counts + pain score where 0 is a REAL result (0 degenerated / 0 pain). */
...
/* ═══════ COMPONENT ═══════ */
const EggRetrievalProcedureDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Egg Retrieval Procedure ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/egg_retrieval_procedure/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedLocalKeys.push(`${baseField}-${idx}`);
        delete recDrafts[fieldPart];
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/egg_retrieval_procedure/${id}/approve`, { sectionId: sid, approved: true });
  ...
  // UNLABELED sentence → ONE whole row (no comma-split, so "Dr. X, MD, FACOG (…)" stays intact).
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== EGG RETRIEVAL PROCEDURE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Egg Retrieval Procedure ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: ENUM FIELD — BlueSelect + case-insensitive canonical (memory 6a4b38d2) ═══════ */
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* Sentence path (parseLabel → nested-subtitle for "Label: value" like triggerMedicationType "Dual trigger: …") */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="egg-retrieval-procedure-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Egg Retrieval Procedure</h2></div>
        <div className="empty-state">No egg retrieval procedure records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="egg-retrieval-procedure-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Egg Retrieval Procedure</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<EggRetrievalProcedureDocumentPDFTemplate document={pdfData} />} fileName="Egg_Retrieval_Procedure.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search egg retrieval procedure..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Egg Retrieval Procedure ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'procedure-info')}
            {renderSection(record, idx, 'oocyte-results')}
            {renderSection(record, idx, 'ovarian-details')}
            {renderSection(record, idx, 'hormonal-levels')}
            {renderSection(record, idx, 'trigger-protocol')}
...
export default EggRetrievalProcedureDocument;
