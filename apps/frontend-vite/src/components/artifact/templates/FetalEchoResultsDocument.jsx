/**
 * FetalEchoResultsDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: fetal_echo_results
 *
 * 6 Sections:
 *   1. cardiac-biometry: fetalHeartRate, cardiothoracicRatio, cardiacAxis, gestationalAge
 *   2. views-outflow: fourChamberView, leftVentricularOutflowTract, rightVentricularOutflowTract, ductalArchView, aorticArchAssessment
 *   3. valves-function: tricuspidValveFunction, mitralValveFunction, ventricularFunction, tricuspidRegurgitationVelocity
 *   4. septum-shunts: septalIntegrity, ductusArteriosusFlow, foramenOvaleFlow
 *   5. venous-return: pulmonaryVeinReturn, systemicVenousReturn
 *   6. other: rythmAnalysis, pericardialSpace, coronaryArteryOrigins, fetalHydrocardiaMeasurement, cardiacMalformationSeverity
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import FetalEchoResultsDocumentPDFTemplate from '../pdf-templates/FetalEchoResultsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './FetalEchoResultsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'fetal_echo_resultsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'cardiac-biometry': 'Cardiac Biometry',
  'views-outflow': 'Views & Outflow',
  'valves-function': 'Valves & Function',
  'septum-shunts': 'Septum & Shunts',
...
const FIELD_LABELS = {
  ...
  fourChamberView: 'Four-Chamber View',
  leftVentricularOutflowTract: 'Left Ventricular Outflow Tract',
  rightVentricularOutflowTract: 'Right Ventricular Outflow Tract',
  ductalArchView: 'Ductal Arch View',
  aorticArchAssessment: 'Aortic Arch Assessment',
  tricuspidValveFunction: 'Tricuspid Valve Function',
  mitralValveFunction: 'Mitral Valve Function',
  ventricularFunction: 'Ventricular Function',
  tricuspidRegurgitationVelocity: 'TR Velocity (m/s)',
  septalIntegrity: 'Septal Integrity',
  ductusArteriosusFlow: 'Ductus Arteriosus Flow',
  foramenOvaleFlow: 'Foramen Ovale Flow',
  pulmonaryVeinReturn: 'Pulmonary Vein Return',
  systemicVenousReturn: 'Systemic Venous Return',
  rythmAnalysis: 'Rhythm Analysis',
  pericardialSpace: 'Pericardial Space',
  coronaryArteryOrigins: 'Coronary Artery Origins',
  fetalHydrocardiaMeasurement: 'Fetal Hydrocardia Measurement',
  cardiacMalformationSeverity: 'Cardiac Malformation Severity',
};

const SECTION_FIELDS = {
  'cardiac-biometry': ['fetalHeartRate', 'cardiothoracicRatio', 'cardiacAxis', 'gestationalAge'],
  'views-outflow': ['fourChamberView', 'leftVentricularOutflowTract', 'rightVentricularOutflowTract', 'ductalArchView', 'aorticArchAssessment'],
  'valves-function': ['tricuspidValveFunction', 'mitralValveFunction', 'ventricularFunction', 'tricuspidRegurgitationVelocity'],
  'septum-shunts': ['septalIntegrity', 'ductusArteriosusFlow', 'foramenOvaleFlow'],
  'venous-return': ['pulmonaryVeinReturn', 'systemicVenousReturn'],
  'other': ['rythmAnalysis', 'pericardialSpace', 'coronaryArteryOrigins', 'fetalHydrocardiaMeasurement', 'cardiacMalformationSeverity'],
};

const NUMBER_FIELDS = ['fetalHeartRate', 'cardiothoracicRatio', 'tricuspidRegurgitationVelocity', 'fetalHydrocardiaMeasurement'];
const STRING_FIELDS = ['gestationalAge', 'cardiacAxis', 'fourChamberView', 'leftVentricularOutflowTract', 'rightVentricularOutflowTract', 'tricuspidValveFunction', 'mitralValveFunction', 'aorticArchAssessment', 'ductalArchView', 'septalIntegrity', 'ventricularFunction', 'ductusArteriosusFlow', 'foramenOvaleFlow', 'pulmonaryVeinReturn', 'systemicVenousReturn', 'rythmAnalysis', 'pericardialSpace', 'coronaryArteryOrigins', 'cardiacMalformationSeverity'];
...
/* ═══════ COMPONENT ═══════ */
const FetalEchoResultsDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Fetal Echocardiography Results ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const suffix = `-${idx}`;
      // Pending edits for THIS section: editKey = "field-idx" (field may contain dots; arrayIndex only when last segment is numeric)
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/fetal_echo_results/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/fetal_echo_results/${id}/approve`, { sectionId: sid, approved: true });
  ...
  // ('1.' even singles), hide-zero mirror, unlabeled single sentence with ≥3 guarded comma items split, empty drop.
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== FETAL ECHOCARDIOGRAPHY RESULTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Fetal Echocardiography Results ${idx + 1}\n${'='.repeat(40)}\n\n`;
      Object.keys(SECTION_FIELDS).forEach(sid => {
  ...
  if (!records || records.length === 0) {
    return (
      <div className="fetal-echo-results-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Fetal Echocardiography Results</h2></div>
        <div className="empty-state">No fetal echocardiography results data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="fetal-echo-results-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Fetal Echocardiography Results</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FetalEchoResultsDocumentPDFTemplate document={pdfData} />} fileName="Fetal_Echo_Results.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search fetal echocardiography results..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Fetal Echocardiography Results ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'cardiac-biometry')}
            {renderSection(record, idx, 'views-outflow')}
            {renderSection(record, idx, 'valves-function')}
            {renderSection(record, idx, 'septum-shunts')}
            {renderSection(record, idx, 'venous-return')}
...
export default FetalEchoResultsDocument;
