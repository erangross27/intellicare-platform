/**
 * Deep Brain Stimulation Edit Route
 * ServiceId: deep-brain-stimulation-edit-service
...
function buildContext(req, operation = 'read') { return { serviceId: 'deep-brain-stimulation-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }

const ALLOWED_FIELDS = [
  'date', 'provider', 'facility', 'status',
  'target', 'laterality', 'implantDate', 'programmingSettings',
  'response', 'complications',
  'findings', 'results',
  'assessment', 'plan', 'recommendations', 'notes',
];

/* Allow dotted sub-paths for OBJECT fields (e.g. results.foo, programmingSettings.rightSide.amplitude) */
const ALLOWED_DOTTED_PREFIXES = ['results.', 'programmingSettings.'];
function isFieldAllowed(field) {
  if (ALLOWED_FIELDS.includes(field)) return true;
  return ALLOWED_DOTTED_PREFIXES.some(p => field.startsWith(p));
}
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('deep_brain_stimulation', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[DeepBrainStimulation] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('deep_brain_stimulation', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[DeepBrainStimulation] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
