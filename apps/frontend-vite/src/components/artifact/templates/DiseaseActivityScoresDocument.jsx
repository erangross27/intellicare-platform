/**
 * DiseaseActivityScoresDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: disease_activity_scores
 *
 * 11 Sections:
 *   1. session-info: date (date picker), provider, facility
 *   2. mayo-score: mayoScore.stoolFrequency, .rectalBleeding, .endoscopicFindings, .physicianAssessment, .totalScore
 *   3. harvey-bradshaw: harveyBradshaw (hide if empty)
 *   4. cdai-section: cdai (hide if empty)
 *   5. partial-mayo: partialMayo (hide if empty)
 *   6. sccai-section: simpleClinicalColitisActivity (hide if empty)
 *   7. pucai-section: pucai (hide if empty)
 *   8. findings-section: findings (hide if empty)
 *   9. assessment-section: assessment (sentence)
 *  10. plan-section: plan (hide if empty)
 *  11. notes-section: notes (sentence)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import DiseaseActivityScoresDocumentPDFTemplate from '../pdf-templates/DiseaseActivityScoresDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './DiseaseActivityScoresDocument.css';
...
/* −/+ stepper step = the value's own smallest digit (decimal-aware): integers by 1, one-decimal by 0.1,
   etc. The customer TYPES the exact value; we NEVER impose a fixed increment (memory 6a4a3fae). */
...
const DRAFT_KEY = 'disease_activity_scoresPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'harvey-bradshaw': 'Harvey-Bradshaw Index',
  'cdai-section': 'CDAI',
  'partial-mayo': 'Partial Mayo Score',
  'sccai-section': 'Simple Clinical Colitis Activity Index',
  'pucai-section': 'PUCAI',
...
const FIELD_LABELS = {
  ...
  'mayoScore.physicianAssessment': 'Physician Assessment',
  'mayoScore.totalScore': 'Total Score',
  harveyBradshaw: 'Harvey-Bradshaw Index',
  cdai: 'CDAI',
  partialMayo: 'Partial Mayo Score',
  simpleClinicalColitisActivity: 'Simple Clinical Colitis Activity Index',
  pucai: 'PUCAI',
  findings: 'Findings',
  results: 'Results',
...
const SECTION_FIELDS = {
  ...
  'mayo-score': ['mayoScore.stoolFrequency', 'mayoScore.rectalBleeding', 'mayoScore.endoscopicFindings', 'mayoScore.physicianAssessment', 'mayoScore.totalScore'],
  'harvey-bradshaw': ['harveyBradshaw'],
  'cdai-section': ['cdai'],
  'partial-mayo': ['partialMayo'],
  'sccai-section': ['simpleClinicalColitisActivity'],
  'pucai-section': ['pucai'],
  'findings-section': ['findings'],
  'results-section': ['results'],
  'assessment-section': ['assessment'],
...
const MAYO_SUB_FIELDS = ['mayoScore.stoolFrequency', 'mayoScore.rectalBleeding', 'mayoScore.endoscopicFindings', 'mayoScore.physicianAssessment', 'mayoScore.totalScore'];
const SENTENCE_FIELDS = ['findings', 'assessment', 'plan', 'notes'];
const DATE_FIELDS = ['date'];
const NUMBER_FIELDS = [];
const BOOLEAN_FIELDS = [];
/* Recursive OBJECT fields (recursive editor) — score indices + results */
const OBJECT_FIELDS = ['harveyBradshaw', 'cdai', 'partialMayo', 'simpleClinicalColitisActivity', 'pucai', 'results'];
/* Array-of-objects {recommendation, date} (date-grouped, editable) */
const OBJECT_ARRAY_FIELDS = ['recommendations'];
const EMPTY_OBJECT_SECTIONS = ['harvey-bradshaw', 'cdai-section', 'partial-mayo', 'sccai-section', 'pucai-section', 'results-section'];

/* ═══════ OBJECT/ARRAY HELPERS (recursive editor support) ═══════ */
...
/* ═══════ COMPONENT ═══════ */
const DiseaseActivityScoresDocument = ({ document: docProp }) => {
  ...
  /* getFieldValue: supports dot-notation for mayoScore sub-fields */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Disease Activity Scores ${idx + 1}`.toLowerCase();
  ...
  const pdfData = useMemo(() => {
    return filteredRecords.map((record, idx) => {
      const merged = { ...record };
      // Deep copy mayoScore if present
      if (record.mayoScore && typeof record.mayoScore === 'object') {
        merged.mayoScore = { ...record.mayoScore };
      }
      Object.keys(localEdits).forEach(key => {
        ...
        if (m && parseInt(m[2]) === idx) {
          ...
          if (dotParts.length === 2) {
            // mayoScore.stoolFrequency → merged.mayoScore.stoolFrequency
            if (!merged[dotParts[0]]) merged[dotParts[0]] = {};
            merged[dotParts[0]] = { ...merged[dotParts[0]], [dotParts[1]]: localEdits[key] };
          } else {
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    // Stage the whole edited object as a DRAFT (no DB write). Approve commits the full object.
    setLocalEdits(prev => {
  ...
  const stageFieldDraft = useCallback((record, fn, idx, sid, fullText) => {
    const id = safeId(record); if (!id) return;
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // localEdits keys for THIS record + THIS section that are still pending drafts
      ...
      for (const editKey of toCommit) {
        ...
        if (lastDot !== -1 && /^\d+$/.test(trailing)) {
          // arrayIndex form: field="...", arrayIndex=<num>
          payload.field = fieldPart.slice(0, lastDot);
          payload.arrayIndex = parseInt(trailing, 10);
        } else {
          // dotted-but-not-numeric (e.g. "mayoScore.stoolFrequency") stays a whole field name
          payload.field = fieldPart;
        }
        const resp = await secureApiClient.put(`/api/edit/disease_activity_scores/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/disease_activity_scores/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[DiseaseActivityScores] Approve error:', err); }
  ...
  /* object → canonical copy lines (recursive): each scalar leaf = sub-label + '-' + "1. value" (NEVER
  ...
  /* Canonical section copy: title + '='; every field = sub-label + '-' + numbered value row(s). NEVER
     ...
     title → hide the sub-label (assessment/notes/status/object sections). */
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    const title = SECTION_TITLES[sid];
    let text = `${title}\n${COPY_LINE_EQ}\n\n`;
    const fields = SECTION_FIELDS[sid] || [];

    if (EMPTY_OBJECT_SECTIONS.includes(sid)) {
      const fn = fields[0];
      const rawField = fn.includes('.') ? fn.split('.')[0] : fn;
      const objVal = record[rawField];
      if (!objVal || (typeof objVal === 'object' && !Array.isArray(objVal) && Object.keys(objVal).length === 0)) return '';
    }
  ...
  const copyAllText = useCallback(async () => {
    let text = `Disease Activity Scores\n${COPY_LINE_EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Disease Activity Scores ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD (parseFloat + isNaN warning) ═══════ */
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; number+unit -> number input, "4/5" ratio, text -> textarea) ═══════ */
                  ...
                  <div key={rIdx}>
                    <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(recText); setSaveError(null); } }}>
                      {isEditing ? (
                        <div className="edit-field-container">
                          <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                          {saveError && editingField === itemKey && <div className="save-error">{saveError}</div>}
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              e.stopPropagation();
  ...
  const renderSection = (record, idx, sid) => {
    ...
    // For empty object sections (harveyBradshaw, cdai, partialMayo, simpleClinicalColitisActivity, pucai)
    // hide if the underlying object is empty {}
    if (EMPTY_OBJECT_SECTIONS.includes(sid)) {
      const fn = fields[0];
      const rawField = fn.includes('.') ? fn.split('.')[0] : fn;
      const objVal = record[rawField];
      if (!objVal || (typeof objVal === 'object' && !Array.isArray(objVal) && Object.keys(objVal).length === 0)) return null;
      // If string, check for empty
      if (typeof objVal === 'string' && !objVal.trim()) return null;
    }

    // For mayo-score section, check if mayoScore object has any sub-field values
    if (sid === 'mayo-score') {
      const ms = record.mayoScore;
      if (!ms || typeof ms !== 'object' || Object.keys(ms).length === 0) return null;
      const anyVal = MAYO_SUB_FIELDS.some(f => hasVal(getFieldValue(record, f, idx)));
  ...
  if (!records || records.length === 0) {
    return (
      <div className="disease-activity-scores-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Disease Activity Scores</h2></div>
        <div className="empty-state">No disease activity scores records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="disease-activity-scores-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Disease Activity Scores</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<DiseaseActivityScoresDocumentPDFTemplate document={pdfData} />} fileName="Disease_Activity_Scores.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search disease activity scores..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Disease Activity Scores ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'mayo-score')}
            {renderSection(record, idx, 'harvey-bradshaw')}
            {renderSection(record, idx, 'cdai-section')}
            {renderSection(record, idx, 'partial-mayo')}
            {renderSection(record, idx, 'sccai-section')}
            {renderSection(record, idx, 'pucai-section')}
...
export default DiseaseActivityScoresDocument;
