/**
 * OphthalmologyExaminationsDocument.jsx
 * March 2026 — Canonical one-pass polish (blue glow theme, inline editing)
 * Collection: ophthalmology_examinations  (FLAT record — string/number fields only)
 *
 * 7 Sections:
 *   1. visual-acuity: visualAcuityOD, visualAcuityOS, refractionOD, refractionOS, colorVisionTesting
 *   2. intraocular-pressure: intraocularpressureOD (number), intraocularpressureOS (number)
 *   3. pupillary-exam: pupillaryReactionOD, pupillaryReactionOS, extraocularMovements, confrontationVisualFields
 *   4. anterior-segment: slitLampBiomicroscopy (long), gonioscopy, anteriorChamberDepth (number), lensStatus, cornealTopography
 *   5. posterior-segment: fundoscopyFindings (long), cupToDiscRatioOD (number), cupToDiscRatioOS (number), maculaAssessment (long)
 *   6. diagnostic-testing: opticalCoherenceTomography (long), fluoresceinAngiography (long), perimetryResults (long)
 *   7. retinopathy: diabeticRetinopathySeverity
 *
 * No top-level `date` field → title-only header (no record date), like the Hematology gold standard.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import OphthalmologyExaminationsDocumentPDFTemplate from '../pdf-templates/OphthalmologyExaminationsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './OphthalmologyExaminationsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'ophthalmology_examinationsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'visual-acuity': 'Visual Acuity and Refraction',
  'intraocular-pressure': 'Intraocular Pressure',
  'pupillary-exam': 'Pupillary and Motility Exam',
  'anterior-segment': 'Anterior Segment',
  'posterior-segment': 'Posterior Segment',
  'diagnostic-testing': 'Diagnostic Testing',
  'retinopathy': 'Retinopathy Assessment',
};

const FIELD_LABELS = {
  visualAcuityOD: 'Visual Acuity OD (Right Eye)',
  visualAcuityOS: 'Visual Acuity OS (Left Eye)',
  refractionOD: 'Refraction OD',
  refractionOS: 'Refraction OS',
  colorVisionTesting: 'Color Vision Testing',
  intraocularpressureOD: 'IOP OD (Right Eye, mmHg)',
  intraocularpressureOS: 'IOP OS (Left Eye, mmHg)',
  pupillaryReactionOD: 'Pupillary Reaction OD',
  pupillaryReactionOS: 'Pupillary Reaction OS',
  extraocularMovements: 'Extraocular Movements',
  confrontationVisualFields: 'Confrontation Visual Fields',
  slitLampBiomicroscopy: 'Slit Lamp Biomicroscopy',
  gonioscopy: 'Gonioscopy',
  anteriorChamberDepth: 'Anterior Chamber Depth',
  lensStatus: 'Lens Status',
  cornealTopography: 'Corneal Topography',
  fundoscopyFindings: 'Fundoscopy Findings',
  cupToDiscRatioOD: 'Cup-to-Disc Ratio OD',
  cupToDiscRatioOS: 'Cup-to-Disc Ratio OS',
  maculaAssessment: 'Macula Assessment',
  opticalCoherenceTomography: 'Optical Coherence Tomography',
  fluoresceinAngiography: 'Fluorescein Angiography',
  perimetryResults: 'Perimetry Results',
  diabeticRetinopathySeverity: 'Diabetic Retinopathy Severity',
};

const SECTION_FIELDS = {
  'visual-acuity': ['visualAcuityOD', 'visualAcuityOS', 'refractionOD', 'refractionOS', 'colorVisionTesting'],
  'intraocular-pressure': ['intraocularpressureOD', 'intraocularpressureOS'],
  'pupillary-exam': ['pupillaryReactionOD', 'pupillaryReactionOS', 'extraocularMovements', 'confrontationVisualFields'],
  'anterior-segment': ['slitLampBiomicroscopy', 'gonioscopy', 'anteriorChamberDepth', 'lensStatus', 'cornealTopography'],
  'posterior-segment': ['fundoscopyFindings', 'cupToDiscRatioOD', 'cupToDiscRatioOS', 'maculaAssessment'],
  'diagnostic-testing': ['opticalCoherenceTomography', 'fluoresceinAngiography', 'perimetryResults'],
  'retinopathy': ['diabeticRetinopathySeverity'],
};

/* 5 number fields — num-stepper (text input, never type=number) + parseFloat+isNaN→block save */
const NUMBER_FIELDS = ['intraocularpressureOD', 'intraocularpressureOS', 'cupToDiscRatioOD', 'cupToDiscRatioOS', 'anteriorChamberDepth'];

const STRING_FIELDS = [
  'visualAcuityOD', 'visualAcuityOS', 'refractionOD', 'refractionOS', 'colorVisionTesting',
  'pupillaryReactionOD', 'pupillaryReactionOS', 'extraocularMovements', 'confrontationVisualFields',
  'slitLampBiomicroscopy', 'gonioscopy', 'lensStatus', 'cornealTopography',
  'fundoscopyFindings', 'maculaAssessment',
  'opticalCoherenceTomography', 'fluoresceinAngiography', 'perimetryResults',
  'diabeticRetinopathySeverity',
];
const DATE_FIELDS = [];
...
/* splitByComma: parenthesis-aware comma split (thousands-separator guard: comma must be followed by ws) */
...
/* ═══════ COMPONENT ═══════ */
const OphthalmologyExaminationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.ophthalmology_examinations) return Array.isArray(r.ophthalmology_examinations) ? r.ophthalmology_examinations : [r.ophthalmology_examinations];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.ophthalmology_examinations) return Array.isArray(dd.ophthalmology_examinations) ? dd.ophthalmology_examinations : [dd.ophthalmology_examinations]; return [dd]; }
      return [r];
    });
  ...
  /* ═══════ UTILS ═══════ */
  // hide-zero: a top-level numeric 0 is treated as "not set" (empty) — never render a bogus 0 row.
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Ophthalmology Examination ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/ophthalmology_examinations/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/ophthalmology_examinations/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== OPHTHALMOLOGY EXAMINATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Ophthalmology Examination ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="ophthalmology-examinations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Ophthalmology Examinations</h2></div>
        <div className="empty-state">No ophthalmology examination records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="ophthalmology-examinations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Ophthalmology Examinations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OphthalmologyExaminationsDocumentPDFTemplate document={pdfData} />} fileName="Ophthalmology_Examinations.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search ophthalmology examinations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Ophthalmology Examination ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'visual-acuity')}
            {renderSection(record, idx, 'intraocular-pressure')}
            {renderSection(record, idx, 'pupillary-exam')}
            {renderSection(record, idx, 'anterior-segment')}
            {renderSection(record, idx, 'posterior-segment')}
            {renderSection(record, idx, 'diagnostic-testing')}
            {renderSection(record, idx, 'retinopathy')}
          </div>
        ))}
      </div>
    </div>
...
export default OphthalmologyExaminationsDocument;
