/**
 * FertilityPreservationDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: fertility_preservation
 *
 * 8 Sections:
 *   1. diagnosis-section: primaryDiagnosis (string)
 *   2. ovarian-reserve: antiMullerianHormone (number), antralFollicleCount (number), follicleStimulatingHormone (number), estradiolBaseline (number)
 *   3. ovarian-stimulation: ovarianStimulationProtocol (string), gonadotropinTotalDose (number), stimulationDuration (number), triggerMedicationType (string)
 *   4. oocyte-embryo: oocytesRetrieved (number), matureOocytesMII (number), oocyteCryopreservationMethod (string), oocytesSurvivingThaw (number), embryosCryopreserved (number), blastocystGradingGardner (string)
 *   5. sperm-preservation: spermCryopreservationSamples (number), spermConcentrationPreFreeze (number), totalMotileSpermCount (number)
 *   6. tissue-preservation: ovarianTissueCryopreservation (boolean), testicularTissueExtraction (string), gnrhAgonistOvarianSuppression (boolean)
 *   7. risk-assessment: oncofertilityUrgencyWindow (number), gonadotoxicityRiskScore (string), ovarianHyperstimulationSyndromeRisk (string)
 *   8. storage-section: cryostorageLocation (string)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import FertilityPreservationDocumentPDFTemplate from '../pdf-templates/FertilityPreservationDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './FertilityPreservationDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'diagnosis-section': 'Primary Diagnosis',
  'ovarian-reserve': 'Ovarian Reserve Assessment',
  'ovarian-stimulation': 'Ovarian Stimulation',
  'oocyte-embryo': 'Oocyte & Embryo Preservation',
  'sperm-preservation': 'Sperm Preservation',
...
const SECTION_FIELDS = {
  'diagnosis-section': ['primaryDiagnosis'],
  'ovarian-reserve': ['antiMullerianHormone', 'antralFollicleCount', 'follicleStimulatingHormone', 'estradiolBaseline'],
  'ovarian-stimulation': ['ovarianStimulationProtocol', 'gonadotropinTotalDose', 'stimulationDuration', 'triggerMedicationType'],
  'oocyte-embryo': ['oocytesRetrieved', 'matureOocytesMII', 'oocyteCryopreservationMethod', 'oocytesSurvivingThaw', 'embryosCryopreserved', 'blastocystGradingGardner'],
  'sperm-preservation': ['spermCryopreservationSamples', 'spermConcentrationPreFreeze', 'totalMotileSpermCount'],
  'tissue-preservation': ['ovarianTissueCryopreservation', 'testicularTissueExtraction', 'gnrhAgonistOvarianSuppression'],
  'risk-assessment': ['oncofertilityUrgencyWindow', 'gonadotoxicityRiskScore', 'ovarianHyperstimulationSyndromeRisk'],
  'storage-section': ['cryostorageLocation'],
};

/* 14 number fields — ALL use input[type=number step=any] + parseFloat+isNaN→block save */
const NUMBER_FIELDS = [
...
const STRING_FIELDS = [
  'primaryDiagnosis', 'ovarianStimulationProtocol', 'triggerMedicationType',
  'oocyteCryopreservationMethod', 'blastocystGradingGardner',
  'testicularTissueExtraction', 'gonadotoxicityRiskScore',
  'cryostorageLocation',
];

/* Zero = extractor sentinel / not-applicable — hidden in JSX + Copy + PDF.
   In-record proof: AMH 2.8 + AFC 16 are real ovarian-reserve values, yet baseline FSH/estradiol read 0
   (a measured hormone panel is never truly 0) → extractor sentinels. Sperm metrics are 0 for a female
   oocyte-freeze patient = N/A. oocytesSurvivingThaw is 0 in a FRESH vitrification cycle (eggs just frozen,
   never thawed) = N/A. oncofertilityUrgencyWindow 0 for elective "unexplained infertility" (non-oncologic) = N/A.
   Real clinical COUNTS (oocytes/embryos retrieved, follicle count) are NOT in this list — a real 0 there is meaningful. */
const ZERO_SENTINEL_FIELDS = [
  'follicleStimulatingHormone', 'estradiolBaseline', 'oocytesSurvivingThaw',
  'spermCryopreservationSamples', 'spermConcentrationPreFreeze', 'totalMotileSpermCount',
  'oncofertilityUrgencyWindow',
];

/* Known clinical scale → BlueSelect full-scale dropdown. OHSS risk = Low/Moderate/High/Very High. */
const ENUM_FIELDS = ['ovarianHyperstimulationSyndromeRisk'];
...
const DRAFT_KEY = 'fertility_preservationPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const FertilityPreservationDocument = ({ document: docProp }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        // mark edited markers using the file's own conventions (field-level + sentence s0)
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Fertility Preservation ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/fertility_preservation/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/fertility_preservation/${id}/approve`, { sectionId: sid, approved: true });
  ...
  // UNLABELED sentence → ONE whole numbered row (no comma-split, so an address/facility line stays intact).
  ...
  // Canonical: EQ under section title, DASH under EVERY field label (suppressed when label == section title —
  // single-name gate), values numbered ('1.' even singles), enum canonical display, zero-sentinel mirror.
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    if (!body.trim()) return ''; // every field hidden/sentinel → drop the whole section (no orphan title+EQ)
    return `${SECTION_TITLES[sid]}\n${'='.repeat(40)}\n\n${body}`;
  }, [getFieldValue, hasVal, fmtVal, splitBySentence, formatSentenceFieldLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== FERTILITY PRESERVATION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Fertility Preservation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    ...
    if (ZERO_SENTINEL_FIELDS.includes(fn) && Number(val) === 0) return null; // hide extractor-sentinel / N-A 0
    const editKey = `${fn}-${idx}`;
  ...
  /* ═══════ RENDER: BOOLEAN FIELD — select Yes/No dropdown, convert to boolean on save ═══════ */
  const renderBooleanField = (record, fn, idx, sid) => {
    ...
    return (
      <div key={fn} className="rec-mini-card">
        {showLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(typeof val === 'boolean' ? (val ? 'true' : 'false') : (String(val).toLowerCase() === 'yes' ? 'true' : 'false')); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
  ...
  /* ═══════ RENDER: ENUM FIELD — BlueSelect full-scale dropdown (known clinical scale) ═══════ */
                            ...
                            <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                              {ciEditing ? (
                                <div className="edit-field-container">
                                  ...
                                  {saveError && <div className="save-error">{saveError}</div>}
                                  <div className="edit-actions">
                                    <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const currentVal2 = String(getFieldValue(record, fn, idx) || ''); const sentences2 = splitBySentence(currentVal2); const s2 = sentences2[sIdx] || ''; const p2 = parseLabel(s2); if (!p2.isLabeled) return; const items2 = splitByComma(p2.value); const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { items2.splice(ciIdx, 1, ...subParts); } else { items2[ciIdx] = trimmed.replace(/[;.]+$/, '').trim(); } const rebuilt = `${p2.label}: ${items2.join(', ')}.`; const allS = [...sentences2]; allS[sIdx] = rebuilt; const fullText2 = reconstructFullText(allS); setSaveError(null); const dKey = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [dKey]: fullText2 })); setPendingEdits(prev => ({ ...prev, [dKey]: true })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); { const store = readDrafts(); if (!store[id2]) store[id2] = {}; store[id2][fn] = fullText2; writeDrafts(store); } const marks = { [commaKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ciIdx + ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                                    <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                                  </div>
                                </div>
                              ) : (
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); setSaveError(null); const id3 = safeId(record); if (!id3) return; const dKey = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [dKey]: fullText })); setPendingEdits(prev => ({ ...prev, [dKey]: true })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); { const store = readDrafts(); if (!store[id3]) store[id3] = {}; store[id3][fn] = fullText; writeDrafts(store); } const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setEditingField(null); setEditValue(''); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  const renderSection = (record, idx, sid) => {
    ...
    // A field actually renders only if it has a value AND is not a hidden zero-sentinel — so a section whose
    // every field is sentinel/empty (e.g. Sperm Preservation for a female oocyte-freeze patient) drops entirely.
  ...
  if (!records || records.length === 0) {
    return (
      <div className="fertility-preservation-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Fertility Preservation</h2></div>
        <div className="empty-state">No fertility preservation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="fertility-preservation-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Fertility Preservation</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FertilityPreservationDocumentPDFTemplate document={pdfData} />} fileName="Fertility_Preservation.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search fertility preservation..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Fertility Preservation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'diagnosis-section')}
            {renderSection(record, idx, 'ovarian-reserve')}
            {renderSection(record, idx, 'ovarian-stimulation')}
            {renderSection(record, idx, 'oocyte-embryo')}
            {renderSection(record, idx, 'sperm-preservation')}
            {renderSection(record, idx, 'tissue-preservation')}
            {renderSection(record, idx, 'risk-assessment')}
            {renderSection(record, idx, 'storage-section')}
          </div>
        ))}
      </div>
    </div>
...
export default FertilityPreservationDocument;
