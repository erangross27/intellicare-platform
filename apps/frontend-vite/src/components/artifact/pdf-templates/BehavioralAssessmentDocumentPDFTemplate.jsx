...
const COLLECTION = 'behavioral_assessment';
const COMMA_SPLIT_FIELDS = ['temperament', 'attentionSpan', 'activityLevel', 'socialSkills', 'emotionalRegulation', 'assessment', 'notes'];
const ARRAY_FIELDS = new Set(['anxietySymptoms', 'adhdSymptoms', 'autismRedFlags', 'recommendations']);
const OBJECT_FIELDS = new Set(['tantrums', 'results', 'additionalData']);
const NARRATIVE_FIELDS = new Set(['temperament', 'attentionSpan', 'activityLevel', 'socialSkills', 'emotionalRegulation', 'findings', 'assessment', 'plan', 'notes']);
...
const LABELS = {
  ...
  anxietySymptoms: 'Anxiety Symptoms', panicAttackFrequency: 'Panic Attack Frequency',
  weightChange: 'Weight Change', adhdSymptoms: 'ADHD Symptoms', autismRedFlags: 'Autism Red Flags',
  findings: 'Findings', assessment: 'Assessment', plan: 'Plan', recommendations: 'Recommendations',
  results: 'Results', notes: 'Notes', additionalData: 'Additional Data',
};
const SECTIONS = [
  { title: 'Date', fields: ['date'] },
  { title: 'Record Information', fields: ['provider', 'facility'] },
  { title: 'Record Details', fields: ['type', 'status'] },
  { title: 'Behavioral Profile', fields: ['temperament', 'attentionSpan', 'activityLevel', 'socialSkills', 'emotionalRegulation', 'tantrums'] },
  { title: 'Symptoms', fields: ['anxietySymptoms', 'panicAttackFrequency', 'weightChange', 'adhdSymptoms', 'autismRedFlags'] },
...
const humanizeKey = key => { const s = String(key ?? '').replace(/_/g, ' ').replace(/([a-z0-9])([A-Z])/g, '$1 $2').replace(/([A-Z]+)([A-Z][a-z])/g, '$1 $2'); return s.charAt(0).toUpperCase() + s.slice(1); };
const objectLeaves = (value, labelText = '') => {
  ...
  if (typeof value === 'object') return Object.entries(value).flatMap(([key, child]) => objectLeaves(child, humanizeKey(key)));
...
const rowsFor = (record, field) => { const value = record[field]; if (!hasValue(value)) return []; if (DATE_FIELDS.has(field)) return isEpochDate(value) ? [] : [{ subtitle: '', value: formatDate(value) }]; if (ARRAY_FIELDS.has(field)) return value.filter(hasValue).map(item => ({ subtitle: '', value: displayValue(item) })); if (OBJECT_FIELDS.has(field)) return objectLeaves(value).map(leaf => ({ subtitle: leaf.label, value: displayValue(leaf.value) })); if (NARRATIVE_FIELDS.has(field)) return splitClauses(value, COMMA_SPLIT_FIELDS.includes(field)).map(text => parseLabel(text)); return [{ subtitle: '', value: displayValue(value) }]; };
