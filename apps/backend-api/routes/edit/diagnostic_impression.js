/**
 * DiagnosticImpression Edit Route
 * ServiceId: diagnostic-impression-edit-service
...
function buildContext(req, operation = 'read') { return { serviceId: 'diagnostic-impression-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
...
const ALLOWED_FIELDS = [
  ...
  'primaryDiagnosis.diagnosis', 'primaryDiagnosis.icd10Code', 'primaryDiagnosis.severity',
  'differentialDiagnoses', 'comorbidities', 'provisionalDiagnoses', 'ruleOutDiagnoses',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('diagnostic_impression', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[DiagnosticImpression] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('diagnostic_impression', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[DiagnosticImpression] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
