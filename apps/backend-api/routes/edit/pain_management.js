/**
 * PainManagement Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/pain_management/:id/edit    — Edit a single field
 * PUT /api/edit/pain_management/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'pain-management-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'physicalTherapyOrders',
  'painManagementAgreement',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('pain_management', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[PainManagement] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('pain_management', { _id: objectId }, {
      $set: {
    ...
    console.error('[PainManagement] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
