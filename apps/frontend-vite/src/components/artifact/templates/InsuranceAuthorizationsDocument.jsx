/**
 * InsuranceAuthorizationsDocument.jsx
 * February 2026 — Canonical one-pass rewrite with inline editing, blue glow theme
 * Collection: insurance_authorizations
 *
 * 5 Sections:
 *   1. authorization-info: authorizationNumber, memberIdNumber, requestingPhysicianNpi, servicingProviderNpi
 *   2. coverage: authorizationValidFromDate (date), authorizationExpirationDate (date),
 *                authorizedUnits (number), estimatedCostAmount (number)
 *   3. procedures-diagnoses: requestedProcedureCodes (array), primaryDiagnosisCode, secondaryDiagnosisCodes (array)
 *   4. clinical-justification: medicalNecessityJustification, priorTherapyAttempts (array), clinicalCriteriaMet (array),
 *                              comorbidityFactors (array), functionalCapacityEvaluation, pharmacotherapyTrialDocumentation
 *   5. decision-review: authorizationDecisionRationale, surgicalUrgencyLevel, peerReviewOutcome,
 *                       appealSubmissionDate (date), emergencyOverride (bool), radiologicalEvidenceSubmitted (bool)
 *
 * No record `date` field (only createdAt/updatedAt ingestion timestamps) → title only, NO record date.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import InsuranceAuthorizationsPDFTemplate from '../pdf-templates/InsuranceAuthorizationsPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './InsuranceAuthorizationsDocument.css';
...
const STRING_FIELDS = [
  'authorizationNumber', 'memberIdNumber', 'requestingPhysicianNpi', 'servicingProviderNpi',
  'primaryDiagnosisCode', 'medicalNecessityJustification', 'functionalCapacityEvaluation',
  'pharmacotherapyTrialDocumentation', 'authorizationDecisionRationale', 'surgicalUrgencyLevel',
  'peerReviewOutcome',
];
...
const ARRAY_FIELDS = [
  'requestedProcedureCodes', 'secondaryDiagnosisCodes', 'priorTherapyAttempts',
  'clinicalCriteriaMet', 'comorbidityFactors',
];
...
const DRAFT_KEY = 'insurance_authorizationsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const InsuranceAuthorizationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.insurance_authorizations) return Array.isArray(r.insurance_authorizations) ? r.insurance_authorizations : [r.insurance_authorizations];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.insurance_authorizations) return Array.isArray(dd.insurance_authorizations) ? dd.insurance_authorizations : [dd.insurance_authorizations]; return [dd]; }
      return [r];
    });
  ...
  /* fieldHasVal: presence check that hides 0-valued numbers (per "skip 0/empty/null" rule) */
  const fieldHasVal = useCallback((record, fn, idx) => {
    const v = getFieldValue(record, fn, idx);
    if (!hasVal(v)) return false;
    if (NUMBER_FIELDS.includes(fn)) { const n = parseFloat(v); return Number.isFinite(n) && n !== 0; }
    return true;
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Authorization ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/insurance_authorizations/${id}/edit`, { field: fn, value: localEdits[editKey] });
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/insurance_authorizations/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        const n = parseFloat(val); if (!Number.isFinite(n) || n === 0) return;
        text += `${label}\n${n}\n\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== INSURANCE AUTHORIZATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Authorization ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD — num-stepper (text input) + parseFloat+isNaN→block save, hide-zero ═══════ */
  const renderNumberField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    const num = parseFloat(val);
    if (!hasVal(val) || !Number.isFinite(num) || num === 0) return null; // hide-zero
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
    const label = FIELD_LABELS[fn] || fn;
    const showSubLabel = label.trim().toLowerCase() !== (SECTION_TITLES[sid] || '').trim().toLowerCase();
  ...
  if (!records || records.length === 0) {
    return (
      <div className="insurance-authorizations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Insurance Authorizations</h2></div>
        <div className="empty-state">No insurance authorization records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="insurance-authorizations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Insurance Authorizations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<InsuranceAuthorizationsPDFTemplate document={pdfData} />} fileName="Insurance_Authorizations.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search insurance authorizations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Authorization ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'authorization-info')}
            {renderSection(record, idx, 'coverage')}
            {renderSection(record, idx, 'procedures-diagnoses')}
            {renderSection(record, idx, 'clinical-justification')}
            {renderSection(record, idx, 'decision-review')}
          </div>
        ))}
      </div>
    </div>
...
export default InsuranceAuthorizationsDocument;
