/**
 * GoalsOfCareDiscussionsDocument.jsx
 * June 2026 — REBUILT to REAL 8-field schema (schema-only, 100% coverage)
 * Collection: goals_of_care_discussions
 *
 * Donor cloned: PointOfCareUltrasoundHeartRateDocument
 *  - DATE_FIELDS + renderDateField (record-header date)
 *  - STRING fields per-sentence renderStringField
 *  - ARRAY recommendations (array of {recommendation, date}) — date-grouped numbered list
 *
 * FIELDS (8 → 100% schema coverage):
 *   DATE:   assessmentDate (record-header date)
 *   STRING: assessmentTime, clinicalStatus, vitalSigns, interventions, response, plan
 *   ARRAY:  recommendations (array of {recommendation, date})
 *
 * Sections:
 *   1. assessment:               assessmentTime, clinicalStatus, vitalSigns
 *   2. interventions-response:   interventions, response, plan
 *   3. recommendations-section:  recommendations
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import GoalsOfCareDiscussionsDocumentPDFTemplate from '../pdf-templates/GoalsOfCareDiscussionsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './GoalsOfCareDiscussionsDocument.css';
...
const DRAFT_KEY = 'goals_of_care_discussionsPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  'assessment': ['assessmentDate', 'assessmentTime', 'clinicalStatus', 'vitalSigns'],
  'interventions-response': ['interventions', 'response', 'plan'],
  'recommendations-section': ['recommendations'],
};

const SECTION_ORDER = ['assessment', 'interventions-response', 'recommendations-section'];
...
/* parseLabel: detect "Label: value" (CLAUSE_OPENER guard so 'If X: do Y' isn't mislabeled) */
...
/* splitByComma: parenthesis-aware; only split on comma FOLLOWED BY whitespace (thousands guard) */
...
/* ═══════ COMPONENT ═══════ */
const GoalsOfCareDiscussionsDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.goals_of_care_discussions) return Array.isArray(r.goals_of_care_discussions) ? r.goals_of_care_discussions : [r.goals_of_care_discussions];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.goals_of_care_discussions) return Array.isArray(dd.goals_of_care_discussions) ? dd.goals_of_care_discussions : [dd.goals_of_care_discussions]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (OBJECT_ARRAY_FIELDS.includes(fieldPart)) {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `goals of care discussion ${idx + 1}`.toLowerCase();
      ...
      // header date
      if (hasVal(record.assessmentDate) && formatDate(record.assessmentDate).toLowerCase().includes(phrase)) return true;
      return false;
    });
  }, [records, searchTerm, getFieldValue, fieldSearchText, hasVal]);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Collect this record's pending edits for fields belonging to this section.
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/goals_of_care_discussions/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/goals_of_care_discussions/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const next = { ...prev }; toCommit.forEach(k => delete next[k]); return next; });
      // Drop this section's committed fields from this record's localStorage drafts
      const store = readDrafts();
      ...
      console.error('[GoalsOfCareDiscussions] Approve error:', err);
          ...
          sentences.forEach(s => {
            ...
            if (p.isLabeled) {
              ...
              let m = 1; (parts.length >= 2 ? parts : [p.value]).forEach(it => {
                ...
                if (ip.isLabeled) text += `  ${ip.label}:\n`;
                text += `  ${m++}. ${String(ip.isLabeled ? ip.value : it).replace(/[;.]+$/, '').trim()}\n`;
              });
            } else text += `${n++}. ${String(s).replace(/[;.]+$/, '').trim()}\n`;
          });
          text += '\n';
        } else {
          text += `${sameAsTitle ? '' : `${label}\n`}1. ${strVal}\n\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== GOALS OF CARE DISCUSSIONS ===\n\n';
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    if (sentences.length > 1 || (sentences.length === 1 && parseLabel(sentences[0]).isLabeled)) {
      ...
      const saveLabeledSentence = (sIdx, rebuilt, trackKey) => { const cur = splitBySentence(String(getFieldValue(record, fn, idx) || '')); cur[sIdx] = rebuilt; const fullText = reconstructFullText(cur); handleSaveField(record, fn, idx, sid, fullText, trackKey); setEditedSentences(prev => ({ ...prev, [trackKey]: 'edited' })); setEditingField(null); setEditValue(''); setSaveError(null); };
      return (
        <div key={fn} className="rec-mini-card">
          {showSubLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
          {sentences.map((sentence, sIdx) => {
                          ...
                          <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                            {ciEditing ? (
                              <div className="edit-field-container">
                                ...
                                {saveError && <div className="save-error">{saveError}</div>}
                                <div className="edit-actions">
                                  <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const cur = splitBySentence(String(getFieldValue(record, fn, idx) || '')); const p2 = parseLabel(cur[sIdx] || ''); if (!p2.isLabeled) return; const items2 = splitByComma(p2.value); items2[ciIdx] = editValue.trim().replace(/[;.]+$/, '').trim(); saveLabeledSentence(sIdx, `${p2.label}: ${items2.join(', ')}`, commaKey); }}>{saving ? 'Saving...' : 'Save'}</button>
                                  <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                                </div>
                              </div>
                            ) : (
            ...
            const displayContent = parsed.isLabeled ? parsed.value : sentence;
            return (
                ...
                <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                  {isEditing ? (
                    <div className="edit-field-container">
                      ...
                      {saveError && <div className="save-error">{saveError}</div>}
                      <div className="edit-actions">
                        <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { saveLabeledSentence(sIdx, `${parsed.label}: ${editValue.trim().replace(/[;.]+$/, '').trim()}`, sentenceKey); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                        <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                      </div>
                    </div>
                  ) : (
                    <>
                      <div className="row-content"><span className="content-value">{highlightText(displayContent)}</span><span className="edit-indicator">&#9998;</span></div>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="goals-of-care-discussions-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Goals of Care Discussion</h2></div>
        <div className="empty-state">No goals of care discussion records available</div>
      </div>
    );
  }
    ...
    <div className="goals-of-care-discussions-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<GoalsOfCareDiscussionsDocumentPDFTemplate document={pdfData} />} fileName="Goals_Of_Care_Discussions.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search goals of care discussion records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
...
export default GoalsOfCareDiscussionsDocument;
