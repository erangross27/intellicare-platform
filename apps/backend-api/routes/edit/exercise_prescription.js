...
function buildContext(req, operation = 'read') { return { serviceId: 'exercise-prescription-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['date', 'prescribingPhysician', 'primaryDiagnosis', 'secondaryDiagnoses', 'functionalLimitations', 'baselineFitnessLevel', 'exerciseGoals', 'targetHeartRateRange', 'exerciseFrequency', 'sessionDuration', 'exerciseIntensity', 'specificExercises', 'warmUpProtocol', 'coolDownProtocol', 'painThresholdGuidance', 'progressionCriteria', 'equipmentRequired', 'supervisionLevel', 'returnToPlayCriteria', 'cardiacClearanceObtained', 'homeExerciseProgramIncluded', 'functionalMovementScore', 'reevaluationDate', 'contraindications', 'modificationOptions', 'educationalMaterialsProvided'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('exercise_prescription', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[ExercisePrescription] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('exercise_prescription', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[ExercisePrescription] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
