/**
 * LiverTransplantFollowUpDocument.jsx
 * March 2026 — canonical one-pass rewrite (inline editing, blue-glow theme)
 * Collection: liver_transplant_follow_up
 *
 * Scalar record: numbers (hide-zero, −/+ stepper), booleans (BlueSelect Yes/No),
 * one date (BlueDatePicker — null here → skipped), short strings (splitBySentence),
 * one array (donorSpecificAntibodies). Record has ONLY createdAt/updatedAt → NO record
 * date rendered (title only, like the Hematology gold standard).
 *
 * 8 Sections:
 *   1. transplant-info: transplantDate (date), donorType, meldScorePreTransplant, currentMeldScore
 *   2. immunosuppression: tacrolimus/cyclosporine/everolimus trough levels
 *   3. liver-function: AST, ALT, GGT, ALP, total/direct bilirubin, INR, serum albumin
 *   4. rejection: banffRejectionGrade, rejectionActivityIndex, acuteRejectionEpisodes, chronicDuctopenia (bool)
 *   5. vascular-biliary: hepaticArteryPatency (bool), portalVeinPatency (bool), hepaticVeinOutflowStatus, biliaryAnastomosisStatus
 *   6. viral-surveillance: cmvViralLoad, ebvViralLoad, hbvSurfaceAntigenStatus (bool), hcvRnaQuantitative
 *   7. complications: eGFR, postTransplantDiabetesMellitus (bool), postTransplantLymphoproliferativeDisorder (bool)
 *   8. graft-performance: karnofskyPerformanceStatus, liverStiffnessMeasurement, donorSpecificAntibodies (array)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import LiverTransplantFollowUpDocumentPDFTemplate from '../pdf-templates/LiverTransplantFollowUpDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './LiverTransplantFollowUpDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'liver_transplant_follow_upPendingEdits';
const readDrafts = () => {
...
/* number fields — −/+ stepper, input[type=text inputMode=decimal], hide-zero */
const NUMBER_FIELDS = [
...
/* boolean fields — BlueSelect Yes/No (save editValue === 'Yes') */
...
const STRING_FIELDS = ['donorType', 'banffRejectionGrade', 'hepaticVeinOutflowStatus', 'biliaryAnastomosisStatus'];
...
/* ═══════ COMPONENT ═══════ */
const LiverTransplantFollowUpDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.liver_transplant_follow_up) return Array.isArray(r.liver_transplant_follow_up) ? r.liver_transplant_follow_up : [r.liver_transplant_follow_up];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.liver_transplant_follow_up) return Array.isArray(dd.liver_transplant_follow_up) ? dd.liver_transplant_follow_up : [dd.liver_transplant_follow_up]; return [dd]; }
      return [r];
    });
  ...
  /* fieldPresent: hide-zero for numbers, always-show booleans (Yes/No), non-empty strings/arrays */
  const fieldPresent = useCallback((val, fn) => {
    if (NUMBER_FIELDS.includes(fn)) { const n = typeof val === 'number' ? val : parseFloat(val); return Number.isFinite(n) && n !== 0; }
    if (BOOLEAN_FIELDS.includes(fn)) { if (typeof val === 'boolean') return true; return val === 'true' || val === 'false' || (val !== null && val !== undefined && val !== ''); }
    if (ARRAY_FIELDS.includes(fn)) return Array.isArray(val) && val.filter(v => v !== null && v !== undefined && v !== '').length > 0;
    return hasVal(val);
  }, [hasVal]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Liver Transplant Follow-Up ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/liver_transplant_follow_up/${id}/edit`, { field: fn, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/liver_transplant_follow_up/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[LiverTransplantFollowUp] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (ARRAY_FIELDS.includes(f)) {
        const arr = getEffectiveArray(record, f, idx).filter(v => v !== null && v !== undefined && v !== '');
    ...
    if (!body.trim()) return '';
    return `${title}\n${'='.repeat(40)}\n\n${body}`;
  }, [getFieldValue, getEffectiveArray, fieldPresent, fmtVal, splitBySentence, formatSentenceFieldLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== LIVER TRANSPLANT FOLLOW-UP ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Liver Transplant Follow-Up ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderDateField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx); if (!fieldPresent(val, fn)) return null;
    const editKey = `${fn}-${idx}`;
  ...
  /* ═══════ RENDER: NUMBER FIELD — −/+ stepper, input[type=text inputMode=decimal] ═══════ */
  ...
  const renderBooleanField = (record, fn, idx, sid) => {
    ...
    const isEditing = editingField === editKey;
    const label = FIELD_LABELS[fn] || fn;
    const displayVal = fmtVal(val); // 'Yes' | 'No'
    const isModified = editedFields[editKey];
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
  ...
  /* ═══════ RENDER: ARRAY FIELD (donorSpecificAntibodies) ═══════ */
  const renderArrayField = (record, fn, idx, sid) => {
    const arr = getEffectiveArray(record, fn, idx).filter(v => v !== null && v !== undefined && v !== '');
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const newArr = [...arr]; newArr[aIdx] = editValue; setSaveError(null); const editKey2 = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [editKey2]: newArr })); setPendingEdits(prev => ({ ...prev, [editKey2]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const store = readDrafts(); if (!store[id2]) store[id2] = {}; store[id2][fn] = newArr; writeDrafts(store); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      if (ARRAY_FIELDS.includes(f)) return getEffectiveArray(record, f, idx).filter(v => v !== null && v !== undefined && v !== '').length > 0;
      return fieldPresent(getFieldValue(record, f, idx), f);
    });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="liver-transplant-follow-up-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Liver Transplant Follow-Up</h2></div>
        <div className="empty-state">No liver transplant follow-up records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="liver-transplant-follow-up-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Liver Transplant Follow-Up</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<LiverTransplantFollowUpDocumentPDFTemplate document={pdfData} />} fileName="Liver_Transplant_Follow_Up.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search liver transplant follow-up..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Liver Transplant Follow-Up ${idx + 1}`)}</h3>
            </div>
...
export default LiverTransplantFollowUpDocument;
