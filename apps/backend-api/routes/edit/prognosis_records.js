...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'prognosis-records-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'currentDiseaseStage', 'diseaseStagingClassification', 'diagnosisCode',
  'expectedClinicalCourse', 'survivalEstimateMonths', 'fiveYearSurvivalRate', 'treatmentResponseProbability',
  'functionalRecoveryLikelihood', 'qualityOfLifeProjection', 'confidenceLevel', 'recurrenceRiskPercentage', 'timeToProgressionEstimate',
  'prognosticIndicatorsUsed', 'performanceStatusScore', 'comorbidityImpactScore',
  'biomarkerValues', 'predictedComplicationRisks', 'comparativeReferenceData',
  'modifiableRiskFactors', 'anticipatedMilestones', 'palliativeCareIndicators',
];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('prognosis_records', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[prognosis_records] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('prognosis_records', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[prognosis_records] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
