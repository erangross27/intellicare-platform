...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'thoracic-surgery-assessment-edit-service',
    userId: req.user?.id,
    operation,
...
// 31 extractable schema fields (root-level). Nested OBJECT leaves and ARRAY
// elements are authorized via the rootField check below (field.split('.')[0]),
// so dotted sub-paths like 'pulmonaryFunction.predictedPostop.fev1' are allowed.
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('thoracic_surgery_assessment', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[thoracic_surgery_assessment] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('thoracic_surgery_assessment', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[thoracic_surgery_assessment] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
