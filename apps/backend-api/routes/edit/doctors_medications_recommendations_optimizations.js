/**
 * Doctors Medications Recommendations Optimizations Edit Route
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'doctors-medications-recommendations-optimizations-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'currentDosage',
  'currentRouteOfAdministration',
  'indicationForTherapy',
  'therapeuticClass',
  'durationOfCurrentTherapy',
  'optimizationType',
  'recommendedMedicationName',
  'recommendedDosage',
  'recommendedRouteOfAdministration',
  'rationaleForOptimization',
  'targetTherapeuticGoal',
  'adverseDrugReactionHistory',
  'drugInteractionsConcerns',
  'contraindications',
  'renalFunctionCreatinineClearance',
  'hepaticFunction',
  'monitoringParametersRequired',
  'patientAdherenceAssessment',
  'titrationSchedule',
  'followUpTimeframe',
  'costEffectivenessConsideration',
  'evidenceGradeLevel',
  'acceptanceStatus',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('doctors_medications_recommendations_optimizations', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[doctors_medications_recommendations_optimizations] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('doctors_medications_recommendations_optimizations', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[doctors_medications_recommendations_optimizations] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
