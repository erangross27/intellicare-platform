/**
 * RheumatoidArthritisAssessmentDocument.jsx
 * July 2026 — recursive, dot-path editable rheumatoid arthritis assessment
...
import RheumatoidArthritisAssessmentDocumentPDFTemplate from '../pdf-templates/RheumatoidArthritisAssessmentDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './RheumatoidArthritisAssessmentDocument.css';

const DRAFT_KEY = 'rheumatoid_arthritis_assessmentPendingEdits';
const SECTION_TITLES = {
  recordInformation: 'Record Information',
  diseaseActivity: 'Disease Activity Scores',
...
const FIELD_LABELS = {
  ...
  das28Score: 'DAS28 Score', cdaiScore: 'CDAI Score', sdaiScore: 'SDAI Score', acr20Response: 'ACR20 Response', jointCounts: 'Joint Counts',
  findings: 'Findings', radiographicProgression: 'Radiographic Progression', extraarticularManifestations: 'Extra-articular Manifestations', functionalStatus: 'Functional Status',
...
const SECTION_FIELDS = {
  ...
  diseaseActivity: ['das28Score', 'cdaiScore', 'sdaiScore', 'acr20Response', 'jointCounts'],
  clinicalFindings: ['findings', 'radiographicProgression', 'extraarticularManifestations'],
...
const NUMBER_STRING_FIELDS = ['das28Score', 'cdaiScore', 'sdaiScore'];
const OBJECT_FIELDS = ['jointCounts', 'results'];
const COMMA_FIELDS = ['functionalStatus', 'extraarticularManifestations'];
const COMMA_ARRAY_FIELDS = ['extraarticularManifestations'];
const SEMICOLON_FIELDS = ['findings', 'radiographicProgression', 'functionalStatus', 'assessment', 'plan', 'notes'];
...
const KEY_LABELS = {
  tender28: 'Tender (28 joints)', swollen28: 'Swollen (28 joints)', tender68: 'Tender (68 joints)', swollen66: 'Swollen (66 joints)',
...
const RheumatoidArthritisAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.rheumatoid_arthritis_assessment) return Array.isArray(record.rheumatoid_arthritis_assessment) ? record.rheumatoid_arthritis_assessment : [record.rheumatoid_arthritis_assessment];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.rheumatoid_arthritis_assessment) return Array.isArray(nested.rheumatoid_arthritis_assessment) ? nested.rheumatoid_arthritis_assessment : [nested.rheumatoid_arthritis_assessment];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/rheumatoid_arthritis_assessment/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/rheumatoid_arthritis_assessment/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[RheumatoidArthritisAssessment] Approve error:', error);
  ...
  const renderNarrativeArray = (record, field, recordIndex, sectionId) => {
    ...
    return <div key={field} className="field-group">
      <div className="nested-subtitle field-group-title">{highlightText(label)}</div>
  ...
  const copyAll = useCallback(async () => {
    let text = '=== RHEUMATOID ARTHRITIS ASSESSMENT ===\n\n';
    records.forEach((record, recordIndex) => {
      text += 'Rheumatoid Arthritis Assessment ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
      ...
      {sectionId === 'results' ? renderResults(record, recordIndex, sectionId)
          : sectionId === 'recommendations' ? renderRecommendations(record, recordIndex, sectionId)
            : fields.map((field) => COMMA_ARRAY_FIELDS.includes(field)
              ? renderNarrativeArray(record, field, recordIndex, sectionId)
              : OBJECT_FIELDS.includes(field)
              ? renderRecursiveNode(record, getValue(record, field, recordIndex), field, recordIndex, sectionId, FIELD_LABELS[field])
  ...
  if (!records.length) return <div className="rheumatoid-arthritis-assessment-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Rheumatoid Arthritis Assessment</h2></div><div className="empty-state">No rheumatoid arthritis assessment records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Rheumatoid Arthritis Assessment ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="rheumatoid-arthritis-assessment-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Rheumatoid Arthritis Assessment</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<RheumatoidArthritisAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Rheumatoid_Arthritis_Assessment.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search rheumatoid arthritis assessments..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Rheumatoid Arthritis Assessment ' + (recordIndex + 1))}</h3></div>
...
export default RheumatoidArthritisAssessmentDocument;
