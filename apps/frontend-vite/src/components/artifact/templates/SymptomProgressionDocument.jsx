/**
 * SymptomProgressionDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: symptom_progression
 *
 * 7 Sections:
 *   1. symptom-info: type, provider, facility
 *   2. timeline-section: timeline
 *   3. weekly-status: week1, week2, week3
 *   4. current-status: current
 *   5. findings-section: findings
 *   6. assessment-plan: assessment, plan
 *   7. notes-status: notes, status
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import SymptomProgressionDocumentPDFTemplate from '../pdf-templates/SymptomProgressionDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SymptomProgressionDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [editKey]: { localValue, payload } } }
     - editKey  = the localEdits key ("field-idx" or "rootField-idx")
     - localValue = the value placed into localEdits (string | array | object) for rehydration
     - payload  = the exact PUT body to send at Approve ({ field, value[, arrayIndex] }) */
const DRAFT_KEY = 'symptom_progressionPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['type', 'provider', 'facility', 'timeline', 'week1', 'week2', 'week3', 'current', 'findings', 'assessment', 'plan', 'notes', 'status'];
...
const fieldSearchText = (f, val) => {
  if (OBJECT_ARRAY_FIELDS.includes(f)) return recsToSearchText(val);
...
/* ======= COMPONENT ======= */
const SymptomProgressionDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.symptom_progression) return Array.isArray(r.symptom_progression) ? r.symptom_progression : [r.symptom_progression];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.symptom_progression) return Array.isArray(dd.symptom_progression) ? dd.symptom_progression : [dd.symptom_progression]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([editKey, draft]) => {
        // editKey is "<base>-<oldIdx>"; remap to the CURRENT render index.
        const dash = editKey.lastIndexOf('-');
        const base = dash === -1 ? editKey : editKey.slice(0, dash);
        const liveKey = `${base}-${idx}`;
        nLocal[liveKey] = draft && draft.localValue !== undefined ? draft.localValue : draft;
        nPending[liveKey] = true;
        nFields[liveKey] = 'edited';
        nSentences[`${base}-${idx}-s0`] = 'edited';
      });
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Symptom Progression ${idx + 1}`.toLowerCase();
  ...
  /* ======= EDIT HANDLERS =======
     ...
     (handleApproveSection commits). stageDraft is the single place every save site funnels through:
       editKey   = the localEdits key ("field-idx" or "rootField-idx")
       localValue = value placed into localEdits (string | array | object)
       payload   = exact PUT body to send at Approve ({ field, value[, arrayIndex] }) */
  const stageDraft = useCallback((record, editKey, localValue, payload) => {
    const id = safeId(record); if (!id) return;
    setLocalEdits(prev => ({ ...prev, [editKey]: localValue }));
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
    const store = readDrafts();
    if (!store[id]) store[id] = {};
    store[id][editKey] = { localValue, payload };
    writeDrafts(store);
  }, [safeId]);
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    stageDraft(record, editKey, saveVal, { field: fn, value: saveVal });
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    const dottedField = `${rootField}.${path.join('.')}`; // non-numeric suffix -> DB field, NO arrayIndex
    ...
    // Stage a DRAFT (no DB write). PUT body sends the dotted leaf path; localEdits holds the whole object.
    stageDraft(record, editKey, clone, { field: dottedField, value: newVal });
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      // Stage a DRAFT (no DB write). Approve commits { field: fn, value: fullText }.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    // This section's staged edits: editKey is "<base>-<idx>" where base is the field name (or rootField).
    ...
    try {
      ...
      // Persist each staged field to the DB now using the payload captured at save time.
      for (const editKey of toCommit) {
        const draft = recDrafts[editKey];
        const payload = draft && draft.payload ? draft.payload : { field: editKey.replace(/-\d+$/, ''), value: localEdits[editKey] };
        await secureApiClient.put(`/api/edit/symptom_progression/${id}/edit`, payload);
      }
      // Flag the record/section approved (audit trail)
      await secureApiClient.put(`/api/edit/symptom_progression/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[SymptomProgression] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== SYMPTOM PROGRESSION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Symptom Progression ${idx + 1}\n${'='.repeat(40)}\n\n`;
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              // Stage a DRAFT (no DB write). Approve commits { field: fn, value: newArr }.
                              stageDraft(record, `${fn}-${idx}`, newArr, { field: fn, value: newArr });
  ...
  /* ======= RENDER: OBJECT LEAF (editable; number+unit/ratio -> number input, text -> textarea) ======= */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="symptom-progression-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Symptom Progression</h2></div>
        <div className="empty-state">No symptom progression records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="symptom-progression-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Symptom Progression</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<SymptomProgressionDocumentPDFTemplate document={pdfData} />} fileName={`symptom-progression-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search symptom progression records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Symptom Progression ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'symptom-info')}
            {renderSection(record, idx, 'timeline-section')}
            {renderSection(record, idx, 'weekly-status')}
...
export default SymptomProgressionDocument;
