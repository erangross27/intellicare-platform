/**
 * KidneyTransplantFollowUpDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: kidney_transplant_follow_up
 *
 * 6 Sections:
 *   1. graft-function-labs: estimatedGFR, creatinineTrend, tacrolimusTroughLevel, cyclosporineLevel, proteinuriaQuantification
 *   2. immunosuppression: mycophenolateDose, cniFreeProtocol, belatacetpBased
 *   3. rejection-histology: acuteRejectionEpisodes, lastBiopsyDate, banffClassification, iftaGrade, dsaStatus, panelReactiveAntibody
 *   4. infection-surveillance: bkVirusLoad, cmvViralLoad, ebvViralLoad
 *   5. allograft-imaging: resistiveIndex, transplantRenalArteryPSV, allograftUltrasoundFindings
 *   6. risk-surveillance: kidneyDonorRiskIndex, coldIschemiaTime, delayedGraftFunction, newOnsetDiabetesAfterTransplant, posttransplantMalignancySurveillance
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import KidneyTransplantFollowUpDocumentPDFTemplate from '../pdf-templates/KidneyTransplantFollowUpDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './KidneyTransplantFollowUpDocument.css';
...
const DRAFT_KEY = 'kidney_transplant_follow_upPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'graft-function-labs': 'Graft Function & Labs',
  'immunosuppression': 'Immunosuppression',
  'rejection-histology': 'Rejection & Histology (Banff/IFTA)',
  'infection-surveillance': 'Infection Surveillance',
  'allograft-imaging': 'Allograft Imaging',
  'risk-surveillance': 'Risk & Surveillance',
};
...
const FIELD_LABELS = {
  ...
  cyclosporineLevel: 'Cyclosporine Level (ng/mL)',
  proteinuriaQuantification: 'Proteinuria (g/day)',
  mycophenolateDose: 'Mycophenolate Dose',
  cniFreeProtocol: 'CNI-Free Protocol',
  belatacetpBased: 'Belatacept-Based Regimen',
  acuteRejectionEpisodes: 'Acute Rejection Episodes',
  lastBiopsyDate: 'Last Biopsy Date',
  banffClassification: 'Banff Classification',
  iftaGrade: 'IFTA Grade',
  dsaStatus: 'DSA Status',
  panelReactiveAntibody: 'Panel Reactive Antibody (%)',
  bkVirusLoad: 'BK Virus Load (copies/mL)',
  cmvViralLoad: 'CMV Viral Load (copies/mL)',
  ebvViralLoad: 'EBV Viral Load (copies/mL)',
  resistiveIndex: 'Resistive Index',
  transplantRenalArteryPSV: 'Transplant Renal Artery PSV (cm/s)',
  allograftUltrasoundFindings: 'Allograft Ultrasound Findings',
  kidneyDonorRiskIndex: 'Kidney Donor Risk Index',
  coldIschemiaTime: 'Cold Ischemia Time (hrs)',
  delayedGraftFunction: 'Delayed Graft Function',
  newOnsetDiabetesAfterTransplant: 'New-Onset Diabetes After Transplant',
  posttransplantMalignancySurveillance: 'Post-Transplant Malignancy Surveillance',
};

const SECTION_FIELDS = {
  'graft-function-labs': ['estimatedGFR', 'creatinineTrend', 'tacrolimusTroughLevel', 'cyclosporineLevel', 'proteinuriaQuantification'],
  'immunosuppression': ['mycophenolateDose', 'cniFreeProtocol', 'belatacetpBased'],
  'rejection-histology': ['acuteRejectionEpisodes', 'lastBiopsyDate', 'banffClassification', 'iftaGrade', 'dsaStatus', 'panelReactiveAntibody'],
  'infection-surveillance': ['bkVirusLoad', 'cmvViralLoad', 'ebvViralLoad'],
  'allograft-imaging': ['resistiveIndex', 'transplantRenalArteryPSV', 'allograftUltrasoundFindings'],
  'risk-surveillance': ['kidneyDonorRiskIndex', 'coldIschemiaTime', 'delayedGraftFunction', 'newOnsetDiabetesAfterTransplant', 'posttransplantMalignancySurveillance'],
};

const NUMBER_FIELDS = ['estimatedGFR', 'tacrolimusTroughLevel', 'cyclosporineLevel', 'proteinuriaQuantification', 'bkVirusLoad', 'cmvViralLoad', 'ebvViralLoad', 'resistiveIndex', 'transplantRenalArteryPSV', 'acuteRejectionEpisodes', 'panelReactiveAntibody', 'kidneyDonorRiskIndex', 'coldIschemiaTime'];
/* MEANINGFUL_ZERO_FIELDS: numerics where 0 is a real GOOD-NEWS clinical result, NOT a "not recorded" sentinel —
   ...
   Excluded (true sentinels — 0 is physiologically impossible / "not measured"): estimatedGFR, tacrolimus/
   cyclosporine trough, resistiveIndex, transplantRenalArteryPSV, kidneyDonorRiskIndex, coldIschemiaTime. */
...
const BOOLEAN_FIELDS = ['cniFreeProtocol', 'belatacetpBased', 'delayedGraftFunction', 'newOnsetDiabetesAfterTransplant'];
const DATE_FIELDS = ['lastBiopsyDate'];
const ARRAY_FIELDS = ['creatinineTrend'];
const STRING_FIELDS = ['mycophenolateDose', 'dsaStatus', 'banffClassification', 'allograftUltrasoundFindings', 'iftaGrade', 'posttransplantMalignancySurveillance'];
...
/* ═══════ COMPONENT ═══════ */
const KidneyTransplantFollowUpDocument = ({ document: docProp }) => {
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     fieldPart "field" → restored as localEdits[`field-idx`]; fieldPart "field.N" (N numeric) → array
     element N spliced into the record's array under localEdits[`field-idx`]. */
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (dotIdx !== -1 && /^\d+$/.test(tail)) {
          // array element draft → merge into the full array stored under `${fn}-${idx}`
          const fn = fieldPart.slice(0, dotIdx);
          const arrIndex = parseInt(tail, 10);
          const localKey = `${fn}-${idx}`;
          const base = nLocal[localKey] !== undefined ? nLocal[localKey] : record[fn];
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Kidney Transplant Follow-Up ${idx + 1}`.toLowerCase();
  ...
  // Stage one narrative sentence locally as a DRAFT (no DB write). localStorage keeps it across refresh; Approve commits it.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/kidney_transplant_follow_up/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/kidney_transplant_follow_up/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== KIDNEY TRANSPLANT FOLLOW-UP ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Kidney Transplant Follow-Up ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="kidney-transplant-follow-up-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Kidney Transplant Follow-Up</h2></div>
        <div className="empty-state">No kidney transplant follow-up data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="kidney-transplant-follow-up-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Kidney Transplant Follow-Up</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<KidneyTransplantFollowUpDocumentPDFTemplate document={pdfData} />} fileName={`kidney-transplant-follow-up-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search kidney transplant follow-up..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Kidney Transplant Follow-Up ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'graft-function-labs')}
            {renderSection(record, idx, 'immunosuppression')}
            {renderSection(record, idx, 'rejection-histology')}
            {renderSection(record, idx, 'infection-surveillance')}
            {renderSection(record, idx, 'allograft-imaging')}
            {renderSection(record, idx, 'risk-surveillance')}
          </div>
        ))}
      </div>
    </div>
...
export default KidneyTransplantFollowUpDocument;
