...
const SECTIONS = [
  ...
  { id: 'diseaseActivity', title: 'Disease Activity Scores', fields: ['das28Score', 'cdaiScore', 'sdaiScore', 'acr20Response', 'jointCounts'] },
  { id: 'clinicalFindings', title: 'Clinical Findings', fields: ['findings', 'radiographicProgression', 'extraarticularManifestations'] },
  { id: 'functionalStatus', title: 'Functional Status', fields: ['functionalStatus'] },
...
export default function RheumatoidArthritisAssessmentDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Rheumatoid Arthritis Assessment</Text></View>
    {!records.length && <Text style={styles.noDataText}>No rheumatoid arthritis assessment data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Rheumatoid Arthritis Assessment {recordIndex + 1}</Text></View>
