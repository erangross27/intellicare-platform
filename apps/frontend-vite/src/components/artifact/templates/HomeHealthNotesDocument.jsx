/**
 * HomeHealthNotesDocument.jsx
 * June 2026 — Rebuilt to match REAL data shape (blue glow theme, inline editing)
 * Collection: home_health_notes
 *
 * 5 Sections:
 *   1. assessment: patientVitalSigns, painLevel (#), glucoseReading (#), respiratoryStatus,
 *                  cognitiveStatus, functionalMobility, activitiesOfDailyLiving
 *   2. skin-wound: woundAssessment, skinIntegrity
 *   3. nutrition-meds: nutritionalStatus, medicationAdherence, medicationReconciliation [array], symptomManagement
 *   4. care-environment: caregiverSupport, homeEnvironmentSafety, infectionControlMeasures, equipmentFunction
 *   5. education-planning: patientEducationProvided, emergencyContactInformation, dischargePlanning
 *
 * Field types: NUMBER (spinner) painLevel/glucoseReading; ARRAY medicationReconciliation;
 *              remaining 17 STRING (per-sentence narrative). NO date field (title-only header).
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HomeHealthNotesDocumentPDFTemplate from '../pdf-templates/HomeHealthNotesDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './HomeHealthNotesDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'home_health_notesPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'nutrition-meds': 'Nutrition & Medications',
  'care-environment': 'Care & Environment',
  'education-planning': 'Education & Planning',
};

const FIELD_LABELS = {
  patientVitalSigns: 'Patient Vital Signs',
  ...
  homeEnvironmentSafety: 'Home Environment Safety',
...
const SECTION_FIELDS = {
  'assessment': ['patientVitalSigns', 'painLevel', 'glucoseReading', 'respiratoryStatus', 'cognitiveStatus', 'functionalMobility', 'activitiesOfDailyLiving'],
  'skin-wound': ['woundAssessment', 'skinIntegrity'],
  'nutrition-meds': ['nutritionalStatus', 'medicationAdherence', 'medicationReconciliation', 'symptomManagement'],
  'care-environment': ['caregiverSupport', 'homeEnvironmentSafety', 'infectionControlMeasures', 'equipmentFunction'],
  'education-planning': ['patientEducationProvided', 'emergencyContactInformation', 'dischargePlanning'],
};

const NUMBER_FIELDS = ['painLevel', 'glucoseReading'];
const ARRAY_FIELDS = ['medicationReconciliation'];
const STRING_FIELDS = [
  'patientVitalSigns', 'medicationAdherence', 'activitiesOfDailyLiving', 'woundAssessment',
  'functionalMobility', 'cognitiveStatus', 'respiratoryStatus', 'skinIntegrity', 'nutritionalStatus',
  'caregiverSupport', 'homeEnvironmentSafety', 'symptomManagement', 'infectionControlMeasures',
  'equipmentFunction', 'patientEducationProvided', 'emergencyContactInformation', 'dischargePlanning',
];
const DATE_FIELDS = [];

/* Comma-delimited list fields → one item per row (paren-aware, skips thousands commas) */
const COMMA_LIST_FIELDS = ['patientVitalSigns', 'woundAssessment', 'symptomManagement', 'patientEducationProvided'];
...
/* splitCommaList: split on top-level commas — NOT inside parentheses, and NOT thousands commas between digits */
...
/* ═══════ COMPONENT ═══════ */
const HomeHealthNotesDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.home_health_notes) return Array.isArray(r.home_health_notes) ? r.home_health_notes : [r.home_health_notes];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.home_health_notes) return Array.isArray(dd.home_health_notes) ? dd.home_health_notes : [dd.home_health_notes]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Home Health Note ${idx + 1}`.toLowerCase();
  ...
  // Stage a full-field DRAFT for `fn` on this record (no DB write). localStorage survives refresh; Approve commits.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/home_health_notes/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/home_health_notes/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (ARRAY_FIELDS.includes(f)) {
        ...
        items.forEach((it, i) => { text += `${i + 1}. ${String(it)}\n`; });
        text += '\n';
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HOME HEALTH NOTES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Home Health Note ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD — num-stepper (NEVER type=number) + parseFloat+isNaN→block save ═══════ */
  ...
  /* ═══════ RENDER: ARRAY FIELD (numbered editable rows, arrayIndex save) ═══════ */
  ...
  const renderCommaListField = (record, fn, idx, sid, title) => {
    ...
    if (items.length < 2) return renderStringField(record, fn, idx, sid, title);
    const label = FIELD_LABELS[fn] || fn;
    const showSubLabel = label.trim().toLowerCase() !== (SECTION_TITLES[sid] || '').trim().toLowerCase();
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
    const phraseMatch = !searchTerm.trim() || sectionTitleMatches(sid) || record._showAllSections;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="home-health-notes-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Home Health Notes</h2></div>
        <div className="empty-state">No home health notes records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="home-health-notes-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Home Health Notes</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HomeHealthNotesDocumentPDFTemplate document={pdfData} />} fileName="Home_Health_Notes.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search home health notes..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Home Health Note ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'assessment')}
            {renderSection(record, idx, 'skin-wound')}
            {renderSection(record, idx, 'nutrition-meds')}
            {renderSection(record, idx, 'care-environment')}
            {renderSection(record, idx, 'education-planning')}
          </div>
        ))}
      </div>
    </div>
...
export default HomeHealthNotesDocument;
