...
/**
 * InsulinRegimenDocumentPDFTemplate - box-free canonical (LETTER)
 * Config-driven from the JSX SECTION_ORDER/SECTION_TITLES/FIELD_LABELS/SECTION_FIELDS.
 * Renders EVERY populated field the JSX renders for JSX/PDF field parity.
 * No boxes: underline rules only (documentTitle 2 / recordTitle+sectionTitle 1 / fieldLabel 0.5).
 * Nested objects (basalInsulin/bolusInsulin/results) render via dotted field paths (resolvePath)
 * + a generic recursive renderer (objectRows); mealDoses = array of strings; recommendations =
 * array of {recommendation, date}. Record date uses record.date (NEVER createdAt/updatedAt).
 */
...
const SECTION_ORDER = ['session-info', 'regimen-overview', 'basal-insulin', 'bolus-insulin', 'results-section', 'clinical-notes', 'recommendations-section'];
...
const KEY_OVERRIDES = { isf: 'ISF', icr: 'ICR', a1c: 'A1C', mdi: 'MDI', tdd: 'TDD' };
...
/* resolvePath: read a dotted field path (basalInsulin.type) off the record */
const resolvePath = (record, f) => {
  if (!f || !f.includes('.')) return record[f];
...
const arrayRows = (arr, kp) => {
  const rows = [];
  (Array.isArray(arr) ? arr : []).filter(x => !isEmptyDeep(x)).forEach((item, i) => {
...
const recommendationRows = (val) => {
  const items = (Array.isArray(val) ? val : []).filter(r => (typeof r === 'string' ? r.trim() : (r?.recommendation || '').trim()));
...
const fieldPresent = (record, f) => {
  ...
  if (OBJECT_ARRAY_FIELDS.includes(f)) return Array.isArray(v) && v.filter(r => (typeof r === 'string' ? r.trim() : (r?.recommendation || '').trim())).length > 0;
  return hasVal(v);
};
...
const fieldBody = (record, f) => {
  ...
  if (ARRAY_FIELDS.includes(f) || Array.isArray(v)) return arrayRows(v, f);
...
const InsulinRegimenDocumentPDFTemplate = ({ document: docProp, data = docProp }) => {
  let records = [];
  if (Array.isArray(data)) records = data;
  else if (data && typeof data === 'object') {
    if (data.insulin_regimen) records = Array.isArray(data.insulin_regimen) ? data.insulin_regimen : [data.insulin_regimen];
    else if (data.documentData) { const dd = data.documentData; records = Array.isArray(dd) ? dd : (dd?.insulin_regimen ? (Array.isArray(dd.insulin_regimen) ? dd.insulin_regimen : [dd.insulin_regimen]) : [dd]); }
    else records = [data];
  }
    ...
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <Text style={styles.documentTitle}>Insulin Regimen</Text>
          <Text style={styles.noData}>No insulin regimen records available.</Text>
        </Page>
      </Document>
    );
  }
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, rIdx) => (
          <View key={rIdx}>
            <Text style={styles.recordTitle} break={rIdx > 0}>{safeString(`Insulin Regimen ${rIdx + 1}`)}</Text>
...
export default InsulinRegimenDocumentPDFTemplate;
