/**
 * Canonical box-free PDF for syphilis_treatment_follow_up.
 * Mirrors SyphilisTreatmentFollowUpDocument JSX field order, grouping, and numbering.
 */
...
const SECTION_CONFIGS = [
  { id: 'staging-serology', title: 'Staging & Serology', fields: ['syphilisStage', 'initialRprTiter', 'followUpRprTiter', 'vdrlTiter', 'fourFoldTiterDecline', 'titerDeclinePercentage', 'serofastState'] },
  { id: 'treatment', title: 'Treatment', fields: ['treatmentRegimen', 'penicillinDosesCompleted', 'jarischHerxheimerReaction'] },
  { id: 'neurosyphilis-csf', title: 'Neurosyphilis / CSF', fields: ['lumbarPunctureDate', 'csfVdrlResult', 'csfProteinLevel', 'csfWhiteCellCount'] },
  { id: 'hiv-special', title: 'HIV & Special Populations', fields: ['hivCoinfectionStatus', 'cd4CountAtTreatment', 'ophthalmicExamFindings', 'otologicSymptomResolution', 'partnerNotificationStatus', 'pregnancyStatus'] },
  { id: 'response-followup', title: 'Response & Follow-Up', fields: ['clinicalSymptomResolution', 'treatmentFailureCriteria', 'retreatmentIndicated', 'followUpIntervalMonths', 'nextScheduledFollowUp'] },
];
...
const unwrapRecords = (data) => {
  ...
  return input.flatMap(record => {
    if (record?.syphilis_treatment_follow_up) return Array.isArray(record.syphilis_treatment_follow_up) ? record.syphilis_treatment_follow_up : [record.syphilis_treatment_follow_up];
    if (record?.documentData) {
      const nested = record.documentData;
      if (Array.isArray(nested)) return nested;
      if (nested?.syphilis_treatment_follow_up) return Array.isArray(nested.syphilis_treatment_follow_up) ? nested.syphilis_treatment_follow_up : [nested.syphilis_treatment_follow_up];
      return [nested];
    }
    return [record];
...
const SyphilisTreatmentFollowUpDocumentPDFTemplate = ({ document: data }) => {
      ...
      {records.length ? records.map((record, index) => (
          ...
          <View style={styles.pageBody}>
            {index === 0 ? (
              <View style={styles.documentHeader} wrap={false}>
                <Text style={styles.documentTitle}>Syphilis Treatment Follow-Up</Text>
              </View>
            ) : null}
            <View style={styles.recordContainer}>
              <View style={styles.recordHeader} wrap={false}>
                <Text style={styles.recordTitle}>Syphilis Treatment Follow-Up {index + 1}</Text>
              </View>
        ...
        <Page size="A4" style={styles.page}>
          <View style={styles.pageBody}>
            ...
            <Text style={styles.noDataText}>No syphilis treatment follow-up records available.</Text>
          </View>
        </Page>
      )}
    </Document>
...
export default SyphilisTreatmentFollowUpDocumentPDFTemplate;
