/**
 * ChiefComplaints Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/chief_complaints/:id/edit    — Edit a single field
 * PUT /api/edit/chief_complaints/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'chief-complaints-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('chief_complaints', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[ChiefComplaints] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('chief_complaints', { _id: objectId }, {
      $set: {
    ...
    console.error('[ChiefComplaints] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
