...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 16, fontFamily: 'Helvetica-Bold', color: '#000000', marginBottom: 8, borderBottom: '1pt solid #000000', paddingBottom: 4 },
  ...
  subLabel: { fontSize: 14, fontFamily: 'Helvetica-Bold', color: '#000000', marginBottom: 2 },
...
const sentenceUnit = (label, text) => {
  ...
  sentences.forEach(s => {
    ...
    const rawVal = p.label ? p.value : String(s).trim();
...
const AntibiogramReportsDocumentPDFTemplate = ({ document: data }) => {
  let records = data ? (Array.isArray(data) ? data : [data]) : [];
  records = records.flatMap(record => {
    if (record?.antibiogram_reports) return Array.isArray(record.antibiogram_reports) ? record.antibiogram_reports : [record.antibiogram_reports];
    ...
    if (record?.documentData) {
      ...
      if (docData?.antibiogram_reports) return Array.isArray(docData.antibiogram_reports) ? docData.antibiogram_reports : [docData.antibiogram_reports];
      return [docData];
    }
    return [record];
  });
  ...
  if (records.length === 0) {
    return (<Document><Page size="LETTER" style={styles.page}><View style={styles.documentHeader}><Text style={styles.documentTitle}>Antibiogram Reports</Text></View><Text style={styles.emptyState}>No records available</Text></Page></Document>);
  }

  return (
    <Document>
      <Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Antibiogram Reports</Text></View>
