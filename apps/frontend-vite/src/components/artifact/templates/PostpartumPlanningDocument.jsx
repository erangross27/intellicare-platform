/**
 * PostpartumPlanningDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: postpartum_planning
 *
 * 8 Sections:
 *   1. clinical-info: date, provider, facility, status
 *   2. immediate-postpartum: insulinDiscontinuation, pediatricianSelected, contraceptionPlan, maternityLeave
 *   3. glucose-testing: glucoseTestingSchedule
 *   4. breastfeeding: breastfeedingRecommendations, lactationSupport
 *   5. support-preparations: postpartumSupport, homePreparations, mentalHealthScreening
 *   6. risk-reduction: weightManagementPlan, exerciseProgram, metforminConsideration
 *   7. future-pregnancy: futurePregnancyCounseling
 *   8. clinical-notes: findings, assessment, plan, recommendations, results, notes
 */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldName]: value } }  (fieldName = the localEdits key without the "-idx" suffix) */
const DRAFT_KEY = 'postpartumPlanningPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'breastfeeding': 'Breastfeeding Recommendations',
  'support-preparations': 'Support & Preparations',
  'risk-reduction': 'Risk Reduction',
  'future-pregnancy': 'Future Pregnancy',
...
const FIELD_LABELS = {
  ...
  facility: 'Facility',
  status: 'Status',
  insulinDiscontinuation: 'Insulin Discontinuation',
  pediatricianSelected: 'Pediatrician Selected',
  contraceptionPlan: 'Contraception Plan',
  maternityLeave: 'Maternity Leave',
  glucoseTestingSchedule: 'Glucose Testing Schedule',
  breastfeedingRecommendations: 'Breastfeeding Recommendations',
  lactationSupport: 'Lactation Support',
  postpartumSupport: 'Postpartum Support',
  homePreparations: 'Home Preparations',
  mentalHealthScreening: 'Mental Health Screening',
  weightManagementPlan: 'Weight Management Plan',
  exerciseProgram: 'Exercise Program',
  metforminConsideration: 'Metformin Consideration',
  futurePregnancyCounseling: 'Future Pregnancy Counseling',
  findings: 'Findings',
...
const SECTION_FIELDS = {
  ...
  'immediate-postpartum': ['insulinDiscontinuation', 'pediatricianSelected', 'contraceptionPlan', 'maternityLeave'],
  'glucose-testing': ['glucoseTestingSchedule'],
  'breastfeeding': ['breastfeedingRecommendations', 'lactationSupport'],
  'support-preparations': ['postpartumSupport', 'homePreparations', 'mentalHealthScreening'],
  'risk-reduction': ['weightManagementPlan', 'exerciseProgram', 'metforminConsideration'],
  'future-pregnancy': ['futurePregnancyCounseling'],
  'clinical-notes': ['findings', 'assessment', 'plan', 'recommendations', 'results', 'notes'],
};

const BOOLEAN_FIELDS = ['lactationSupport', 'mentalHealthScreening'];
const DATE_FIELDS = ['date'];
const ARRAY_FIELDS = ['breastfeedingRecommendations', 'postpartumSupport', 'homePreparations'];
const OBJECT_FIELDS = ['maternityLeave', 'results'];
const OBJECT_ARRAY_FIELDS = ['recommendations'];
const STRING_FIELDS = ['provider', 'facility', 'status', 'insulinDiscontinuation', 'pediatricianSelected', 'contraceptionPlan', 'glucoseTestingSchedule', 'weightManagementPlan', 'exerciseProgram', 'metforminConsideration', 'futurePregnancyCounseling', 'findings', 'assessment', 'plan', 'notes'];

/* ═══════ OBJECT-FIELD HELPERS (recursive leaf rendering for maternityLeave / results) ═══════ */
const KEY_OVERRIDES = {};
...
const PostpartumPlanningDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.postpartum_planning) return Array.isArray(r.postpartum_planning) ? r.postpartum_planning : [r.postpartum_planning];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.postpartum_planning) return Array.isArray(dd.postpartum_planning) ? dd.postpartum_planning : [dd.postpartum_planning]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      const id = !record?._id ? null : (typeof record._id === 'string' ? record._id : (record._id.$oid || String(record._id)));
  ...
  /* searchable text for any field type (objects / object-arrays included) */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Postpartum Planning ${idx + 1}`.toLowerCase();
  ...
  }, [records, searchTerm, getFieldValue, fmtVal, fieldSearchText, hasVal]);
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* Stage a DRAFT for a full-field value: localEdits + pendingEdits + edited marker + draft localStorage.
     fieldName is the DB field; localEdits/draft are keyed so Approve can replay the DB write. */
  const stageDraft = useCallback((record, fieldName, idx, sid, value, markers) => {
  ...
  // shown in the PDF until the user clicks Approve (handleApproveSection commits). Now synchronous.
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    // Build the full updated root-object value (the localEdits/draft store always holds the FULL field value).
    const cur = localEdits[`${rootField}-${idx}`] !== undefined ? localEdits[`${rootField}-${idx}`] : record[rootField];
    const clone = JSON.parse(JSON.stringify(cur ?? {}));
    let node = clone;
    for (let i = 0; i < path.length - 1; i++) node = node[path[i]];
    node[path[path.length - 1]] = newVal;
    // Stage as a DRAFT (no DB write); Approve commits the whole object via { field: rootField, value }.
    stageDraft(record, rootField, idx, sid, clone, { fields: { [leafKeyTrack]: 'edited' } });
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    // Stage as a DRAFT (no DB write); Approve commits it.
    stageDraft(record, fn, idx, sid, fullText, { sentences: sentenceMarks });
  }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Pending drafts for THIS section: localEdits key = `${fn}-${idx}` where fn is one of this section's fields.
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldName = k.slice(0, -suffix.length);
        return fields.includes(fieldName);
      });
      // Persist each staged field to the DB now. localEdits holds the FULL field value (string / array /
      // object) so a single { field, value } PUT per field is correct (no arrayIndex needed).
      for (const editKey of toCommit) {
        const fieldName = editKey.slice(0, -suffix.length);
        const resp = await secureApiClient.put(`/api/edit/postpartum_planning/${id}/edit`, { field: fieldName, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the record/section approved (audit trail)
      await secureApiClient.put(`/api/edit/postpartum_planning/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const next = { ...prev }; toCommit.forEach(k => delete next[k]); return next; });
      // Drop this record's committed drafts from localStorage
      const store = readDrafts();
      if (store[id]) { toCommit.forEach(k => { const fieldName = k.slice(0, -suffix.length); delete store[id][fieldName]; }); if (Object.keys(store[id]).length === 0) delete store[id]; writeDrafts(store); }

      setApprovedSections(prev => ({ ...prev, [`${sid}-${idx}`]: true }));
      setEditedFields(prev => { const n = { ...prev }; Object.keys(n).forEach(k => { fields.forEach(f => { if (k.startsWith(`${f}-${idx}`) || (k.startsWith(`${f}.`) && k.endsWith(`-${idx}`))) delete n[k]; }); }); return n; });
      setEditedSentences(prev => { const n = { ...prev }; Object.keys(n).forEach(k => { fields.forEach(f => { if (k.startsWith(`${f}-${idx}`)) delete n[k]; }); }); return n; });
    } catch (err) { console.error('[PostpartumPlanning] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== POSTPARTUM PLANNING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Postpartum Planning ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; bool -> Yes/No select, number+unit -> number input, ratio stays text) ═══════ */
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              // Stage as a DRAFT (no DB write); Approve commits the full array via { field: fn, value }.
                              stageDraft(record, fn, idx, sid, newArr, { sentences: { [itemKey]: 'edited' } });
                            }}>{saving ? 'Saving...' : 'Save'}</button>
                            <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                          </div>
                        </div>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="postpartum-planning-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Postpartum Planning</h2></div>
        <div className="empty-state">No postpartum planning records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="postpartum-planning-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search postpartum planning..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'immediate-postpartum')}
            {renderSection(record, idx, 'glucose-testing')}
            {renderSection(record, idx, 'breastfeeding')}
            {renderSection(record, idx, 'support-preparations')}
            {renderSection(record, idx, 'risk-reduction')}
            {renderSection(record, idx, 'future-pregnancy')}
...
export default PostpartumPlanningDocument;
