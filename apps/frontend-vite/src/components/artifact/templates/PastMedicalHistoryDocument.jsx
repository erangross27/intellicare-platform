/**
 * PastMedicalHistoryDocument.jsx
 * Canonical rewrite — inline editing, blue glow theme (mirrors HematologyConsultationsDocument)
 * Collection: past_medical_history
 *
 * Sections:
 *   1. condition-info: condition (string), diagnosisDate (date), status (string)
 *   2. treatment: treatment (string), currentManagement (string)
 *   3. complications: complications (string), hospitalizations (string)
 *   4. provider-notes: provider (string), notes (string)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PastMedicalHistoryDocumentPDFTemplate from '../pdf-templates/PastMedicalHistoryDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './PastMedicalHistoryDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'past_medical_historyPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'condition-info': 'Condition Information',
  'treatment': 'Treatment & Management',
  'complications': 'Complications & Hospitalizations',
...
const SECTION_FIELDS = {
  'condition-info': ['condition', 'diagnosisDate', 'status'],
  'treatment': ['treatment', 'currentManagement'],
  'complications': ['complications', 'hospitalizations'],
  'provider-notes': ['provider', 'notes'],
};
...
const STRING_FIELDS = ['condition', 'status', 'treatment', 'currentManagement', 'complications', 'hospitalizations', 'provider', 'notes'];
const DATE_FIELDS = ['diagnosisDate'];
...
/* ═══════ COMPONENT ═══════ */
const PastMedicalHistoryDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.past_medical_history) return Array.isArray(r.past_medical_history) ? r.past_medical_history : [r.past_medical_history];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.past_medical_history) return Array.isArray(dd.past_medical_history) ? dd.past_medical_history : [dd.past_medical_history]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Condition ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/past_medical_history/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/past_medical_history/${id}/approve`, { sectionId: sid, approved: true });
  ...
  /* ═══════ RENDER: NUMBER FIELD — input[type=text inputMode=decimal] + parseFloat+isNaN→block save ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="past-medical-history-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Past Medical History</h2></div>
        <div className="empty-state">No past medical history records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="past-medical-history-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Past Medical History</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PastMedicalHistoryDocumentPDFTemplate document={pdfData} />} fileName="Past_Medical_History.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search past medical history..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Condition ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'condition-info')}
            {renderSection(record, idx, 'treatment')}
            {renderSection(record, idx, 'complications')}
            {renderSection(record, idx, 'provider-notes')}
          </div>
        ))}
      </div>
    </div>
...
export default PastMedicalHistoryDocument;
