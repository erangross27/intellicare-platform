...
const DRAFT_KEY = 'therapy_requestsPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  requesterSpecialty: 'Specialty',
  performerType: 'Performer Type',
  performerId: 'Performer ID',
  occurrenceTiming: 'Occurrence Timing',
  occurrenceStartDate: 'Start Date',
  occurrenceEndDate: 'End Date',
  ...
  priorAuthorizationNumber: 'Authorization Number',
...
const OBJECT_FIELDS = [];
const STRING_FIELDS = ['requestIntent', 'requestPriority', 'therapyType', 'therapyCode', 'therapyCodeSystem', 'bodyStructure', 'laterality', 'reasonCode', 'reasonReference', 'requesterId', 'requesterName', 'requesterSpecialty', 'performerType', 'performerId', 'occurrenceTiming', 'priorAuthorizationNumber', 'locationId', 'settingType'];
...
const SELECT_OPTIONS = {
  ...
  settingType: ['outpatient', 'inpatient', 'home', 'telehealth'],
};
...
/* ======= COMPONENT ======= */
const TherapyRequestsDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Therapy Request ${idx + 1}`.toLowerCase();
      if (rt.includes(phrase) || phrase.includes(rt)) { record._showAllSections = true; return true; }
      if ('therapy requests'.includes(phrase) || phrase.includes('therapy requests')) { record._showAllSections = true; return true; }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/therapy_requests/${id}/edit`, { field: fieldPart, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/therapy_requests/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== THERAPY REQUESTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Therapy Request ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="therapy-requests-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Therapy Requests</h2></div>
        <div className="empty-state"><p className="empty-text">No therapy requests records available</p></div>
      </div>
    );
  }
  ...
  return (
    <div className="therapy-requests-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Therapy Requests</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<TherapyRequestsDocumentPDFTemplate document={pdfData} />} fileName="Therapy_Requests.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search therapy requests..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-title">{highlightText(`Therapy Request ${idx + 1}`)}</h3>
            </div>
