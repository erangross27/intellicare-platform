/**
 * FunctionalMriStudiesDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: functional_mri_studies
 *
 * 5 Sections:
 *   1. report-info: reportDate (date picker), reportType (string), urgency (string)
 *   2. clinical-indication: clinicalIndication (sentence — very long with "Label: value" patterns)
 *   3. findings-section: findings (sentence — very long with brain mapping "Label: value" patterns)
 *   4. follow-up: followUp (sentence)
 *   5. recommendations-section: recommendations (sentence)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import FunctionalMriStudiesDocumentPDFTemplate from '../pdf-templates/FunctionalMriStudiesDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './FunctionalMriStudiesDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the field name, e.g. "findings") */
const DRAFT_KEY = 'functional_mri_studiesPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const FunctionalMriStudiesDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.functional_mri_studies) return Array.isArray(r.functional_mri_studies) ? r.functional_mri_studies : [r.functional_mri_studies];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.functional_mri_studies) return Array.isArray(dd.functional_mri_studies) ? dd.functional_mri_studies : [dd.functional_mri_studies]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Functional MRI Study ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Staged drafts for this section's fields. editKey = "field-idx" (field never dotted here).
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length);
        const lastDot = fieldPart.lastIndexOf('.');
        // Treat the trailing dot-segment as arrayIndex ONLY when it is purely numeric.
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/functional_mri_studies/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/functional_mri_studies/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = `Functional MRI Studies\n${'='.repeat(40)}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Functional MRI Study ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="functional-mri-studies-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Functional MRI Studies</h2></div>
        <div className="empty-state">No functional MRI studies records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="functional-mri-studies-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Functional MRI Studies</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FunctionalMriStudiesDocumentPDFTemplate document={pdfData} />} fileName="Functional_Mri_Studies.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search functional MRI studies..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Functional MRI Study ${idx + 1}`)}</h3>
            </div>
...
export default FunctionalMriStudiesDocument;
