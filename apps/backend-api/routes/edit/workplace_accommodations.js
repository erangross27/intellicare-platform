...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'workplace-accommodations-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'icd10DiagnosisCodes',
  'adaQualifyingDisability',
  'functionalCapacityEvaluation',
  'liftingCapacityKg',
  'prolongedStandingLimitations',
  'ergonomicWorkstationRequired',
  'visualAcuityCorrection',
  'hearingAssistiveTechnology',
  'cognitiveAccommodations',
  'neurologicalImpairments',
  'cardiovascularRestrictions',
  'pulmonaryFunctionLimitations',
  'diabeticManagementNeeds',
  'medicationSideEffects',
  'mobilityAssistiveDevices',
  'chemicalSensitivities',
  'seizureDisorderPrecautions',
  'psyFunctionalAssessment',
  'accommodationDurationMonths',
  'returnToWorkProgression',
  'occupationalTherapyRecommendations',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('workplace_accommodations', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[workplace_accommodations] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('workplace_accommodations', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[workplace_accommodations] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
