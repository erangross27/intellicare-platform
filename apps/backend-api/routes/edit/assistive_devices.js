/**
 * Assistive Devices Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/assistive_devices/:id/edit    — Edit a single field
 * PUT /api/edit/assistive_devices/:id/approve — Approve a section
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'assistive-devices-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('assistive_devices', { _id: objectId }, {
      $set: {
        [updatePath]: value,
        updatedAt: new Date(),
    ...
    console.error('[AssistiveDevices] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    console.error('[AssistiveDevices] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
