/**
 * MedicationDeprescribingDocument.jsx
 * March 2026 — Canonical one-pass polish: inline editing, blue glow theme
 * Collection: medication_deprescribing
 *
 * Sections:
 *   1. provider-details: deprescribingPharmacist, patientMedicalRecordNumber
 *   2. screening-scores: stoppCriteriaScore, startCriteriaScore, polypharmacyMedicationCount,
 *                        anticholinergicBurdenScale, medicationAdherenceMorisky, qualityOfLifeEq5d, cognitiveStatusMmse
 *   3. renal-hepatic: creatinineClearanceMlMin, estimatedGfrMlMinPerBsa, hepaticImpairmentChildPugh
 *   4. beers-list: beersListMedications
 *   5. falls-risk: fallsRiskMedications
 *   6. drug-interactions: drugInteractionChecked, duplicateTherapyIdentified
 *   7. deprescribing-plan: deprescribingTaperSchedule, withdrawalSyndromeRisk, patientDeprescribingConsent
 *   8. outcomes: medicationsDiscontinued, dosageReductionAchieved
 *   9. adverse-reactions: adverseDrugReactionHistory
 *  10. follow-up: followUpMonitoringPlan
 *
 * No real clinical `date` field (only createdAt/updatedAt ingestion timestamps) → title only, no record date.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MedicationDeprescribingDocumentPDFTemplate from '../pdf-templates/MedicationDeprescribingDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './MedicationDeprescribingDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'medication_deprescribingPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['deprescribingPharmacist', 'patientMedicalRecordNumber', 'hepaticImpairmentChildPugh', 'deprescribingTaperSchedule', 'withdrawalSyndromeRisk', 'followUpMonitoringPlan'];
...
/* ═══════ COMPONENT ═══════ */
const MedicationDeprescribingDocument = ({ document: docProp }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        nFields[isArrayIdx ? `${fieldPart}-${idx}` : editKey] = 'edited';
      });
    });
  ...
  /* ═══════ UTILS ═══════ */
  // hide-zero: a numeric 0 is treated as empty (unrecorded) and skipped, mirrored in PDF + Copy All.
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Medication Deprescribing Record ${idx + 1}`.toLowerCase();
  ...
  // Stage an edited array item as a DRAFT (whole-array value under "field-idx").
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Staged drafts for this record live in localEdits as "<fieldPart>-<idx>" and are flagged pending.
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/medication_deprescribing/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/medication_deprescribing/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MEDICATION DEPRESCRIBING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Medication Deprescribing Record ${idx + 1}\n${'='.repeat(40)}\n\n`;
        ...
        {items.map((item, arrIdx) => {
          ...
          return (
            <div key={arrIdx}>
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(arrKey); setEditValue(itemParsed.isLabeled ? itemParsed.value : itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
  ...
  if (!records || records.length === 0) {
    return (
      <div className="medication-deprescribing-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Medication Deprescribing</h2></div>
        <div className="empty-state">No medication deprescribing data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="medication-deprescribing-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Medication Deprescribing</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MedicationDeprescribingDocumentPDFTemplate document={pdfData} />} fileName="Medication_Deprescribing.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search medication deprescribing..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Medication Deprescribing Record ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'provider-details')}
            {renderSection(record, idx, 'screening-scores')}
            {renderSection(record, idx, 'renal-hepatic')}
            {renderSection(record, idx, 'beers-list')}
            {renderSection(record, idx, 'falls-risk')}
            {renderSection(record, idx, 'drug-interactions')}
            {renderSection(record, idx, 'deprescribing-plan')}
...
export default MedicationDeprescribingDocument;
