...
function buildContext(req, operation = 'read') { return { serviceId: 'geriatric-medications-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['name', 'genericName', 'dosage', 'frequency', 'route', 'startDate', 'endDate', 'duration', 'durationDays', 'durationUnit', 'prescriber', 'indication', 'instructions', 'refills', 'active', 'sideEffects', 'drugInteractions', 'safetyWarning'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('geriatric_medications', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[GeriatricMedications] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('geriatric_medications', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[GeriatricMedications] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
