...
const SECTIONS = [
  { id: 'calculatorInformation', title: 'Calculator Information', fields: ['calculatorName', 'calculatorVersion', 'calculationDate', 'provider', 'facility', 'riskCategory'] },
  { id: 'riskResults', title: 'Risk Results', fields: ['totalScore', 'riskPercentage', 'riskClassification', 'timeframe', 'outcomesPredicted'] },
  { id: 'patientData', title: 'Patient Data', fields: ['patientAge', 'patientSex', 'comorbidities', 'medications'] },
  { id: 'clinicalValues', title: 'Clinical Values', fields: ['labValues', 'vitalSigns', 'inputParameters'] },
  { id: 'clinicalDecision', title: 'Clinical Decision', fields: ['clinicalIndication', 'clinicalDecisionMade', 'recommendedInterventions'] },
  { id: 'validationLimitations', title: 'Validation & Limitations', fields: ['calculationMethod', 'validationStatus', 'limitationsNoted', 'comparisonToPopulation'] },
];
const FIELD_LABELS = {
...
export default function RiskCalculatorsDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Risk Calculators</Text></View>
    {!records.length && <Text style={styles.noDataText}>No risk calculators data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Risk Calculators {recordIndex + 1}</Text></View>
