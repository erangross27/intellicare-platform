/**
 * FootExamDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: foot_exam
 *
 * 5 Sections:
 *   1. session-info: date (date picker), provider, facility, status
 *   2. exam-findings: ulcers, calluses, deformities, nailCondition, skinCondition, circulation, footwear
 *   3. sensation-findings: sensation (long with details), findings (long multi-sentence)
 *   4. assessment-plan: assessment, plan
 *   5. notes-section: notes
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import FootExamDocumentPDFTemplate from '../pdf-templates/FootExamDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './FootExamDocument.css';
...
const DRAFT_KEY = 'foot_examPendingEdits';
const readDrafts = () => {
...
/* No number fields — all fields are strings */
const NUMBER_FIELDS = [];

const STRING_FIELDS = ['provider', 'facility', 'ulcers', 'calluses', 'deformities', 'nailCondition', 'skinCondition', 'circulation', 'sensation', 'footwear', 'findings', 'assessment', 'plan', 'notes'];
...
/* humanizeKey: turn object keys into readable labels */
...
/* ═══════ COMPONENT ═══════ */
const FootExamDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.foot_exam) return Array.isArray(r.foot_exam) ? r.foot_exam : [r.foot_exam];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.foot_exam) return Array.isArray(dd.foot_exam) ? dd.foot_exam : [dd.foot_exam]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      const recDrafts = store[idOf(record)];
      if (!recDrafts) return;
  ...
  /* flattenSearchable: recursively collect all scalar values + humanized keys for search */
  const flattenSearchable = useCallback((v) => {
    if (v === null || v === undefined || v === '') return '';
    if (typeof v !== 'object') { if (typeof v === 'boolean') return v ? 'Yes' : 'No'; return String(v); }
  ...
  const searchVal = useCallback((f, v) => ((ARRAY_FIELDS.includes(f) || OBJECT_FIELDS.includes(f)) ? flattenSearchable(v) : fmtVal(v)), [flattenSearchable, fmtVal]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Foot Exam ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/foot_exam/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/foot_exam/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const objectCopyLines = useCallback((label, value) => {
    ...
    if (value === null || typeof value !== 'object') {
      ...
      out.push(`1. ${fmtVal(value)}`);
      return out;
    }
    if (Array.isArray(value)) {
      if (label) { out.push(label); out.push(COPY_LINE_DASH); }
      let n = 1;
      value.filter(hasVal).forEach(v => { if (v === null || typeof v !== 'object') out.push(`${n++}. ${fmtVal(v)}`); else objectCopyLines('', v).forEach(l => out.push(l)); });
      return out;
    }
    if (label) { out.push(label); out.push(COPY_LINE_DASH); }
    Object.entries(value).filter(([, v]) => hasVal(v)).forEach(([k, v]) => out.push(...objectCopyLines(humanizeKey(k), v)));
    return out;
  }, [hasVal, fmtVal]);
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (ARRAY_FIELDS.includes(f) && Array.isArray(val)) {
        const items = val.map(r => {
          if (typeof r === 'string') return { text: r.trim(), date: '' };
          if (r && typeof r === 'object') return { text: String(r.recommendation || r.text || '').trim(), date: String(r.date || '').trim() };
          return { text: String(r ?? '').trim(), date: '' };
        }).filter(it => it.text);
        if (!items.length) return;
        body += head;
        const groups = [];
        items.forEach(it => { const last = groups[groups.length - 1]; if (last && last.date === it.date) last.items.push(it.text); else groups.push({ date: it.date, items: [it.text] }); });
        groups.forEach(g => { if (g.date) body += `${g.date}\n`; g.items.forEach((t, i) => { body += `${i + 1}. ${t}\n`; }); });
        body += '\n';
        ...
        body += head;
        entries.forEach(([k, v]) => { objectCopyLines(humanizeKey(k), v).forEach(l => { body += `${l}\n`; }); body += '\n'; });
  ...
  /* ═══════ RENDER: RECOMMENDATIONS (ARRAY, date-grouped, read-only) ═══════ */
  const renderRecommendationsField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    if (!hasVal(val) || !Array.isArray(val)) return null;
    const label = FIELD_LABELS[fn] || fn;
    const showSubLabel = label.trim().toLowerCase() !== (SECTION_TITLES[sid] || '').trim().toLowerCase();
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
    const phrase = searchTerm.toLowerCase().trim();

    /* normalize each item to { text, date } */
    const items = val.map(r => {
      if (r === null || r === undefined) return null;
      if (typeof r === 'string') return { text: r.trim(), date: '' };
      if (typeof r === 'object') return { text: String(r.recommendation || r.text || '').trim(), date: String(r.date || '').trim() };
      return { text: String(r).trim(), date: '' };
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {groups.map((group, gIdx) => {
          const groupMatches = !searchTerm.trim() || sectionTitleMatches(sid) || record._showAllSections ||
            label.toLowerCase().includes(phrase) || (group.date && group.date.toLowerCase().includes(phrase)) ||
            group.items.some(t => t.toLowerCase().includes(phrase));
          if (!groupMatches && searchTerm.trim()) return null;
          return (
            <div key={gIdx} className="rec-group">
              {group.date && <div className="rec-date">{highlightText(group.date)}</div>}
              {group.items.map((text, tIdx) => {
                const itemKey = `${fn}-${idx}-g${gIdx}-i${tIdx}`;
                if (searchTerm.trim() && !groupMatches) return null;
                return (
                  <div key={tIdx} className="numbered-row">
                    <div className="row-content"><span className="content-value">{highlightText(`${tIdx + 1}. ${text}`)}</span></div>
  ...
  const renderObjectNode = (label, value, keyPath, depth) => {
    ...
    if (isScalar) {
      ...
      return (
        <div key={keyPath} className="object-leaf">
          {label && <div className="nested-subtitle object-sublabel">{highlightText(label)}</div>}
          <div className="numbered-row">
            <div className="row-content"><span className="content-value">{highlightText(display)}</span></div>
            <button className={`copy-btn ${copiedItems[keyPath] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(label ? `${label}: ${display}` : display, keyPath); }}>{copiedItems[keyPath] ? 'Copied!' : 'Copy'}</button>
          </div>
        </div>
      );
    }
    if (Array.isArray(value)) {
      const entries = value.filter(v => hasVal(v));
      if (entries.length === 0) return null;
      return (
        <div key={keyPath} className="object-node">
          {label && <div className="nested-subtitle object-sublabel">{highlightText(label)}</div>}
          <div className="object-children">{entries.map((v, i) => renderObjectNode('', v, `${keyPath}-${i}`, depth + 1))}</div>
        </div>
      );
    }
    ...
    return (
      <div key={keyPath} className="object-node">
        {label && <div className="nested-subtitle object-sublabel">{highlightText(label)}</div>}
        <div className="object-children">{entries.map(([k, v]) => renderObjectNode(humanizeKey(k), v, `${keyPath}-${k}`, depth + 1))}</div>
      </div>
    );
  };

  /* ═══════ RENDER: OBJECT FIELD (results, read-only) ═══════ */
  const renderObjectField = (record, fn, idx, sid) => {
    ...
    return (
      <div key={fn} className="rec-mini-card">
        {showSubLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
        <div className="object-children">{entries.map(([k, v]) => renderObjectNode(humanizeKey(k), v, `${fn}-${idx}-${k}`, 1))}</div>
      </div>
    );
  };
  ...
  if (!records || records.length === 0) {
    return (
      <div className="foot-exam-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Foot Exam</h2></div>
        <div className="empty-state">No foot exam records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="foot-exam-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Foot Exam</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FootExamDocumentPDFTemplate document={pdfData} />} fileName="Foot_Exam.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search foot exam records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Foot Exam ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'exam-findings')}
            {renderSection(record, idx, 'sensation-findings')}
...
export default FootExamDocument;
