/**
 * GlaucomaAssessmentsDocumentPDFTemplate.jsx
 * Box-free B&W LETTER (canonical, memory 6a2d6af6) — mirrors the JSX: real record.date (never createdAt),
 * narratives split on [.;] with abbrev/single-initial/genus guard, labeled OD:/OS: sentences comma-split into
 * a subtitle group, currentMedications array numbered, values numbered ('1.' even singles), single-name label
 * gate. Rule #74: each field is ONE wrap={false} atomic View with the sectionTitle riding INSIDE the first
 * present field's View. Static PHI footer. Collection: glaucoma_assessments.
 */
...
const SECTION_ORDER = ['session-info', 'iop-measurements', 'examination-findings', 'diagnosis-treatment'];
...
const GlaucomaAssessmentsDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].glaucoma_assessments && Array.isArray(templateData[0].glaucoma_assessments)) records = templateData[0].glaucoma_assessments;
    else records = templateData;
  } else if (templateData && templateData.glaucoma_assessments) {
    records = Array.isArray(templateData.glaucoma_assessments) ? templateData.glaucoma_assessments : [templateData.glaucoma_assessments];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.glaucoma_assessments ? (Array.isArray(dd.glaucoma_assessments) ? dd.glaucoma_assessments : [dd.glaucoma_assessments]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Glaucoma Assessments</Text></View>
        <Text style={styles.emptyState}>No glaucoma assessment records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Glaucoma Assessment ${idx + 1}`}</Text>
...
export default GlaucomaAssessmentsDocumentPDFTemplate;
