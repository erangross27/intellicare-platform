/**
 * WeightMeasurementsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: weight_measurements
 *
 * 6 Sections:
 *   1. weight-basics: bodyWeightKg, bodyWeightLbs, heightCm, bodyMassIndex, bmiClassification
 *   2. body-composition: waistCircumferenceCm, hipCircumferenceCm, waistHipRatio, bodyFatPercentage, leanBodyMassKg, skeletalMuscleMassKg, visceralFatLevel, bodyCompositionMethod
 *   3. weight-targets: idealBodyWeightKg, adjustedBodyWeightKg, weightChangeKg, percentWeightChange
 *   4. nutrition-status: nutritionalStatus, fluidRetentionStatus, fastingStatus, clothingWeight
 *   5. malnutrition-screening: malnutritionScreeningTool, malnutritionScore, sarcopeniaRisk
 *   6. metadata: createdAt, updatedAt
 */
...
const DRAFT_KEY = 'weight_measurementsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'weight-basics': 'Weight Basics',
  'body-composition': 'Body Composition',
  'weight-targets': 'Weight Targets & Changes',
...
const FIELD_LABELS = {
  ...
  bodyWeightLbs: 'Body Weight (lbs)',
  heightCm: 'Height (cm)',
  bodyMassIndex: 'Body Mass Index',
  bmiClassification: 'BMI Classification',
  waistCircumferenceCm: 'Waist Circumference (cm)',
  hipCircumferenceCm: 'Hip Circumference (cm)',
  ...
  bodyCompositionMethod: 'Body Composition Method',
  idealBodyWeightKg: 'Ideal Body Weight (kg)',
  adjustedBodyWeightKg: 'Adjusted Body Weight (kg)',
  weightChangeKg: 'Weight Change (kg)',
  percentWeightChange: 'Percent Weight Change',
  nutritionalStatus: 'Nutritional Status',
  fluidRetentionStatus: 'Fluid Retention Status',
  fastingStatus: 'Fasting Status',
  clothingWeight: 'Clothing Weight',
  malnutritionScreeningTool: 'Malnutrition Screening Tool',
  malnutritionScore: 'Malnutrition Score',
  sarcopeniaRisk: 'Sarcopenia Risk',
  createdAt: 'Created At',
  updatedAt: 'Updated At',
};

const SECTION_FIELDS = {
  'weight-basics': ['bodyWeightKg', 'bodyWeightLbs', 'heightCm', 'bodyMassIndex', 'bmiClassification'],
  'body-composition': ['waistCircumferenceCm', 'hipCircumferenceCm', 'waistHipRatio', 'bodyFatPercentage', 'leanBodyMassKg', 'skeletalMuscleMassKg', 'visceralFatLevel', 'bodyCompositionMethod'],
  'weight-targets': ['idealBodyWeightKg', 'adjustedBodyWeightKg', 'weightChangeKg', 'percentWeightChange'],
  'nutrition-status': ['nutritionalStatus', 'fluidRetentionStatus', 'fastingStatus', 'clothingWeight'],
  'malnutrition-screening': ['malnutritionScreeningTool', 'malnutritionScore', 'sarcopeniaRisk'],
...
const WeightMeasurementsDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Weight Measurement ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // staged drafts whose key belongs to a field in this section
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/weight_measurements/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/weight_measurements/${id}/approve`, { sectionId: sid, approved: true });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="weight-measurements-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Weight Measurements</h2></div>
        <div className="empty-state">No weight measurement records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="weight-measurements-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search weight measurements..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'weight-targets')}
            {renderSection(record, idx, 'nutrition-status')}
            {renderSection(record, idx, 'malnutrition-screening')}
            {renderSection(record, idx, 'metadata')}
          </div>
        ))}
      </div>
    </div>
...
export default WeightMeasurementsDocument;
