...
const SECTIONS = [
  ...
  { id: 'dmards', title: 'DMARDs', fields: ['dmards'] },
  { id: 'biologics', title: 'Biologics', fields: ['biologics'] },
  { id: 'corticosteroids', title: 'Corticosteroids', fields: ['corticosteroids'] },
  { id: 'nsaids', title: 'NSAIDs', fields: ['nsaids'] },
  { id: 'adjunctTherapies', title: 'Adjunct Therapies', fields: ['adjunctTherapies'] },
...
export default function RheumatologicTreatmentDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Rheumatologic Treatment</Text></View>
    {!records.length && <Text style={styles.noDataText}>No rheumatologic treatment data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Rheumatologic Treatment {recordIndex + 1}</Text></View>
