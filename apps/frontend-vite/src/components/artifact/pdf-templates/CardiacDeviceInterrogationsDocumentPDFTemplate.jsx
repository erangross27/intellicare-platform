...
const COLLECTION = 'cardiac_device_interrogations';
const COMMA_SPLIT_FIELDS = ['followUpPlan'];
const ARRAY_FIELDS = new Set(['alerts', 'deviceAlerts', 'programmingChanges']);
const OBJECT_FIELDS = new Set(['batteryStatus', 'pacingParameters', 'pacingPercentages', 'sensingThresholds', 'icdTherapy', 'atrialFibrillationBurden', 'remoteMonitoring', 'leads', 'arrhythmiaEpisodes']);
const NARRATIVE_FIELDS = new Set(['clinicalAssessment', 'followUpPlan', 'recommendations']);
const DATE_FIELDS = new Set(['implantDate', 'interrogationDate']);
const KEY_OVERRIDES = { icdTherapy: 'ICD Therapy', atpAttempts: 'ATP Attempts', mri: 'MRI' };
const LABELS = {
  implantDate: 'Implant Date', interrogationDate: 'Interrogation Date', deviceType: 'Device Type', manufacturer: 'Manufacturer',
  model: 'Model', serialNumber: 'Serial Number', interrogationReason: 'Interrogation Reason',
  batteryStatus: 'Battery Status', leads: 'Leads', pacingParameters: 'Pacing Parameters', pacingPercentages: 'Pacing Percentages',
  sensingThresholds: 'Sensing Thresholds', arrhythmiaEpisodes: 'Arrhythmia Episodes', icdTherapy: 'ICD Therapy',
  atrialFibrillationBurden: 'Atrial Fibrillation Burden', alerts: 'Alerts', deviceAlerts: 'Device Alerts',
  remoteMonitoring: 'Remote Monitoring', programmingChanges: 'Programming Changes',
  clinicalAssessment: 'Clinical Assessment', followUpPlan: 'Follow-Up Plan', recommendations: 'Recommendations',
};
const SECTIONS = [
  { title: 'Interrogation Dates', fields: ['implantDate', 'interrogationDate'] },
  { title: 'Device Information', fields: ['deviceType', 'manufacturer', 'model', 'serialNumber', 'interrogationReason'] },
  { title: 'Battery Status', fields: ['batteryStatus'] },
  { title: 'Leads', fields: ['leads'] },
  { title: 'Pacing Parameters', fields: ['pacingParameters'] },
  { title: 'Pacing Percentages', fields: ['pacingPercentages'] },
  { title: 'Sensing Thresholds', fields: ['sensingThresholds'] },
  { title: 'Arrhythmia Episodes', fields: ['arrhythmiaEpisodes'] },
  { title: 'ICD Therapy', fields: ['icdTherapy'] },
  { title: 'Atrial Fibrillation Burden', fields: ['atrialFibrillationBurden'] },
  { title: 'Alerts', fields: ['alerts', 'deviceAlerts'] },
  { title: 'Remote Monitoring', fields: ['remoteMonitoring'] },
  { title: 'Programming Changes', fields: ['programmingChanges'] },
  { title: 'Clinical Assessment', fields: ['clinicalAssessment'] },
...
const CardiacDeviceInterrogationsDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Cardiac Device Interrogations</Text><Text style={styles.noData}>No cardiac device interrogations data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Cardiac Device Interrogations</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Cardiac Device Interrogations {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
