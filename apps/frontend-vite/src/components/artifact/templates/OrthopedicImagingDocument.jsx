/**
 * OrthopedicImagingDocument.jsx
 * Canonical one-pass polish — blue glow editing theme, BlueDatePicker.
 * Collection: orthopedic_imaging
 *
 * Record `date` (real study date, NEVER createdAt/updatedAt) → editable record-header date pill (BlueDatePicker).
 * Nested objects (mri / xray / ct / results) render read-only via a recursive object renderer (label ABOVE
 * value — never a side-by-side "Label: value"; labeled array items decompose into their own sub-label).
 * Scalar strings (type/provider/facility/status/findings/effusion/assessment/plan/notes) are inline-editable.
 * Arrays (boneContusions/recommendations) render read-only, decomposed.
 *
 * Sections: Study Information · MRI · X-Ray · CT Scan · Clinical Findings · Imaging Measurements · Assessment and Plan
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import OrthopedicImagingDocumentPDFTemplate from '../pdf-templates/OrthopedicImagingDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './OrthopedicImagingDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldName]: value } } — value is the full field value (string). */
const DRAFT_KEY = 'orthopedic_imagingPendingEdits';
const readDrafts = () => {
...
/* `date` renders as the editable record-header pill (NOT in the section body / Copy All). */
...
/* No hide-zero fields (no numeric fields in this collection). */
const HIDE_ZERO_FIELDS = NUMBER_FIELDS;

/* ═══════ OBJECT-FIELD HELPERS ═══════ */
const KEY_OVERRIDES = {
  mri: 'MRI', ct: 'CT', xray: 'X-Ray', orif: 'ORIF', rom: 'ROM', ap: 'AP',
  id: 'ID', mm: 'mm', cm: 'cm',
};
...
/* a nested scalar leaf whose key names a date + whose value is an ISO day → format nicely (read-only display) */
...
const objectCopyBlock = (value, lines, indent) => {
  ...
  Object.entries(value).filter(([, v]) => !isEmptyDeep(v)).forEach(([k, v]) => {
    if (isScalar(v)) { lines.push(`${pad}${humanizeKey(k)}`); lines.push(`${pad}${fmtLeaf(k, v)}`); }
    else if (Array.isArray(v)) { lines.push(`${pad}${humanizeKey(k)}`); v.filter(x => !isEmptyDeep(x)).forEach((it, i) => { if (isScalar(it)) lines.push(`${pad}  ${i + 1}. ${fmtScalar(it)}`); else objectCopyBlock(it, lines, indent + 1); }); }
    else { lines.push(`${pad}${humanizeKey(k)}`); objectCopyBlock(v, lines, indent + 1); }
  });
};

/* parseLabel: detect "Label: value" patterns (CLAUSE_OPENER guard so 'If X: do Y' isn't mislabeled) */
...
/* ═══════ COMPONENT ═══════ */
const OrthopedicImagingDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.orthopedic_imaging) return Array.isArray(r.orthopedic_imaging) ? r.orthopedic_imaging : [r.orthopedic_imaging];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.orthopedic_imaging) return Array.isArray(dd.orthopedic_imaging) ? dd.orthopedic_imaging : [dd.orthopedic_imaging]; return [dd]; }
      return [r];
    });
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (DATE_FIELDS.includes(f)) { if (formatDate(val).toLowerCase().includes(phrase)) return true; }
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Imaging Study ${idx + 1}`.toLowerCase();
      if (rt.includes(phrase) || phrase.includes(rt)) { record._showAllSections = true; return true; }
      if ('orthopedic imaging'.includes(phrase) || phrase.includes('orthopedic imaging')) { record._showAllSections = true; return true; }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const field = m[1];
        await secureApiClient.put(`/api/edit/orthopedic_imaging/${id}/edit`, { field, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/orthopedic_imaging/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    let body = '';
    fields.forEach(f => {
      if (f === HEADER_DATE_FIELD) return; // rendered in the record-header pill, not the section body
      ...
      if (OBJECT_FIELDS.includes(f)) {
        body += head;
        const lines = []; objectCopyBlock(val, lines, 0); lines.forEach(l => { body += `${l}\n`; });
        body += '\n';
      } else if (ARRAY_FIELDS.includes(f)) {
        const items = (Array.isArray(val) ? val : []).filter(x => !isEmptyDeep(x));
        body += head;
        items.forEach((it, i) => { body += `  ${i + 1}. ${fmtScalar(it)}\n`; });
        body += '\n';
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== ORTHOPEDIC IMAGING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Imaging Study ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: RECORD-HEADER DATE PILL (real record date, editable via BlueDatePicker — never createdAt) ═══════ */
  const renderHeaderDate = (record, idx) => {
    ...
    return (
      <span className={`record-date ${isModified ? 'modified' : ''}`} title="Record date" onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(toInputDate(val)); setSaveError(null); } }} style={{ cursor: 'pointer' }}>
        {isEditing ? (
          <span className="edit-field-container" onClick={e => e.stopPropagation()}>
            <BlueDatePicker value={editValue} onSelect={iso => setEditValue(iso)} />
            {saveError && <span className="save-error">{saveError}</span>}
            <span className="edit-actions">
              <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date'); return; } handleSaveField(record, fn, idx, 'study-info', null, editValue + 'T00:00:00.000Z'); }}>{saving ? 'Saving...' : 'Save'}</button>
  ...
  /* ═══════ RENDER: DATE FIELD (in-section BlueDatePicker — canonical; unused: date is header-only) ═══════ */
  ...
  const renderObjectNode = (label, value, key, keyName) => {
    if (isEmptyDeep(value)) return null;
    if (isScalar(value)) {
      const disp = fmtLeaf(keyName, value);
      return (
        <div key={key} className="nested-mini-card">
          <div className="nested-subtitle sub-label">{highlightText(label)}</div>
          <div className="numbered-row">
            <div className="row-content"><span className="content-value">{highlightText(disp)}</span></div>
            <button className={`copy-btn ${copiedItems[key] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(disp, key); }}>{copiedItems[key] ? 'Copied!' : 'Copy'}</button>
          </div>
        </div>
      );
    }
    if (Array.isArray(value)) {
      ...
      return (
        <div key={key} className="nested-mini-card">
          <div className="nested-subtitle sub-label">{highlightText(label)}</div>
          {items.map((it, i) => {
            if (!isScalar(it)) return renderObjectNode(`Item ${i + 1}`, it, `${key}-${i}`, '');
            const str = fmtScalar(it);
    ...
    return (
      <div key={key} className="nested-mini-card">
        <div className="nested-subtitle sub-label">{highlightText(label)}</div>
        {entries.map(([k, v]) => renderObjectNode(humanizeKey(k), v, `${key}-${k}`, k))}
      </div>
    );
  };

  /* ═══════ RENDER: OBJECT FIELD (mri / xray / ct / results — read-only, recursive) ═══════ */
  const renderObjectField = (record, fn, idx, sid, title) => {
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {entries.map(([k, v]) => renderObjectNode(humanizeKey(k), v, `${fn}-${idx}-${k}`, k))}
      </div>
    );
  };
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); setSaveError(null); const editKey3 = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [editKey3]: fullText })); setPendingEdits(prev => ({ ...prev, [editKey3]: true })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); if (sid) setApprovedSections(prev => { const k = `${sid}-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; }); stageDraft(record, fn, fullText); setEditingField(null); setEditValue(''); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      if (f === HEADER_DATE_FIELD) return false; // header pill, not a body row
      return hasFieldVal(f, getFieldValue(record, f, idx));
    });
    ...
    return (
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            if (f === HEADER_DATE_FIELD) return null; // rendered in the record-header pill
  ...
  if (!records || records.length === 0) {
    return (
      <div className="orthopedic-imaging-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Orthopedic Imaging</h2></div>
        <div className="empty-state">No orthopedic imaging records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="orthopedic-imaging-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Orthopedic Imaging</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OrthopedicImagingDocumentPDFTemplate document={pdfData} />} fileName="Orthopedic_Imaging.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search orthopedic imaging..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Imaging Study ${idx + 1}`)}</h3>
            </div>
...
export default OrthopedicImagingDocument;
