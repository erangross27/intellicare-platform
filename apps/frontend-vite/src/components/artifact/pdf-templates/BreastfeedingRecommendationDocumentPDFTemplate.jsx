...
const COLLECTION = 'breastfeeding_recommendation';
const COMMA_SPLIT_FIELDS = ['immunizationStatus'];
const ARRAY_FIELDS = new Set(['maternalMedications', 'contraindications', 'maternalComorbidities', 'infantMedicalConditions', 'maternalSubstanceUse', 'pumpingIndications']);
const OBJECT_FIELDS = new Set([]);
const NARRATIVE_FIELDS = new Set(['immunizationStatus']);
const DATE_FIELDS = new Set([]);
const ZERO_SENTINEL_FIELDS = new Set(['maternalAge', 'bilirubinLevel']);
const KEY_OVERRIDES = {};
const LABELS = {
  maternalAge: 'Maternal Age', gestationalAgeAtDelivery: 'Gestational Age at Delivery', deliveryMethod: 'Delivery Method',
  infantBirthWeight: 'Infant Birth Weight', apgarScoreFiveMinute: 'Apgar Score (5 Minute)',
  maternalMedications: 'Maternal Medications', contraindications: 'Contraindications',
  maternalComorbidities: 'Maternal Comorbidities', infantMedicalConditions: 'Infant Medical Conditions',
  latchAssessmentScore: 'Latch Assessment Score', breastfeedingInitiation: 'Breastfeeding Initiation',
  milkProduction: 'Milk Production', nippleCondition: 'Nipple Condition', feedingFrequency: 'Feeding Frequency',
  supplementationRequired: 'Supplementation Required', jaundicePresent: 'Jaundice Present', bilirubinLevel: 'Bilirubin Level',
  weightLossPercentage: 'Weight Loss Percentage', maternalSubstanceUse: 'Maternal Substance Use',
  lactationConsultationIndicated: 'Lactation Consultation Indicated', exclusiveBreastfeedingGoal: 'Exclusive Breastfeeding Goal',
  pumpingIndications: 'Pumping Indications', immunizationStatus: 'Immunization Status',
};
const SECTIONS = [
  { title: 'Maternal Information', fields: ['maternalAge', 'gestationalAgeAtDelivery', 'deliveryMethod', 'maternalComorbidities', 'maternalMedications', 'maternalSubstanceUse', 'immunizationStatus'] },
  { title: 'Infant Information', fields: ['infantBirthWeight', 'apgarScoreFiveMinute', 'infantMedicalConditions', 'jaundicePresent', 'bilirubinLevel', 'weightLossPercentage'] },
  { title: 'Feeding Assessment', fields: ['breastfeedingInitiation', 'latchAssessmentScore', 'milkProduction', 'nippleCondition', 'feedingFrequency', 'supplementationRequired'] },
  { title: 'Contraindications', fields: ['contraindications'] },
  { title: 'Recommendations', fields: ['lactationConsultationIndicated', 'exclusiveBreastfeedingGoal', 'pumpingIndications'] },
];
...
const BreastfeedingRecommendationDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Breastfeeding Recommendation</Text><Text style={styles.noData}>No breastfeeding recommendation data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Breastfeeding Recommendation</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Breastfeeding Recommendation {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
