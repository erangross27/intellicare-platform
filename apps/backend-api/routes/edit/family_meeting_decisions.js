/**
 * Family Meeting Decisions Edit Route
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'family-meeting-decisions-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'facility',
  'meetingAttendees',
  'patientDecisionMakingCapacity',
  'healthcareProxyDesignated',
  'interpreterUsed',
  'familyConsensusDegree',
  'advanceDirectiveStatus',
  'codeStatus',
  'lifeSustainingTreatmentDecisions',
  'treatmentLimitationsAgreed',
  'artificialNutritionDecision',
  'goalsOfCare',
  'prognosisDiscussed',
  'trialOfTherapyTimeframe',
  'symptomsAddressedPriority',
  'hospicePalliativeReferral',
  'dischargePlanning',
  'ethicsConsultRequested',
  'culturalReligiousConsiderations',
  'documentationSharedWith',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('family_meeting_decisions', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[family_meeting_decisions] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('family_meeting_decisions', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[family_meeting_decisions] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
