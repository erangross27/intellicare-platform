/**
 * PostpartumDiabetesRiskDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: postpartum_diabetes_risk
 *
 * 4 Sections:
 *   1. glucose-metrics: oralGlucoseToleranceTest, fastingPlasmaGlucose, hemoglobinA1c, postpartumScreeningDate
 *   2. risk-factors: gestationalDiabetesHistory, familyHistoryDiabetes, polycysticOvarySyndrome, insulinTherapyDuringPregnancy, macrosomiaHistory, previousStillbirth, hypertensiveDisorders
 *   3. metabolic-profile: prepregnancyBmi, currentBmi, gestationalWeightGain, triglycerideLevels, hdlCholesterol, metabolicSyndrome
 *   4. current-status: breastfeedingStatus, ethnicRiskGroup, maternalAge, thyroidDisorders, corticosteroidUse, contraceptiveMethod
 */
...
const SECTION_TITLES = {
  'glucose-metrics': 'Glucose Metrics',
...
const FIELD_LABELS = {
  oralGlucoseToleranceTest: 'Oral Glucose Tolerance Test',
  fastingPlasmaGlucose: 'Fasting Plasma Glucose',
  hemoglobinA1c: 'Hemoglobin A1c',
  postpartumScreeningDate: 'Postpartum Screening Date',
  ...
  insulinTherapyDuringPregnancy: 'Insulin Therapy During Pregnancy',
  macrosomiaHistory: 'Macrosomia History',
  ...
  gestationalWeightGain: 'Gestational Weight Gain',
  triglycerideLevels: 'Triglyceride Levels',
  hdlCholesterol: 'HDL Cholesterol',
  metabolicSyndrome: 'Metabolic Syndrome',
  breastfeedingStatus: 'Breastfeeding Status',
  ethnicRiskGroup: 'Ethnic Risk Group',
  maternalAge: 'Maternal Age',
  thyroidDisorders: 'Thyroid Disorders',
  corticosteroidUse: 'Corticosteroid Use',
  contraceptiveMethod: 'Contraceptive Method',
};

const SECTION_FIELDS = {
  'glucose-metrics': ['oralGlucoseToleranceTest', 'fastingPlasmaGlucose', 'hemoglobinA1c', 'postpartumScreeningDate'],
  'risk-factors': ['gestationalDiabetesHistory', 'familyHistoryDiabetes', 'polycysticOvarySyndrome', 'insulinTherapyDuringPregnancy', 'macrosomiaHistory', 'previousStillbirth', 'hypertensiveDisorders'],
  'metabolic-profile': ['prepregnancyBmi', 'currentBmi', 'gestationalWeightGain', 'triglycerideLevels', 'hdlCholesterol', 'metabolicSyndrome'],
  'current-status': ['breastfeedingStatus', 'ethnicRiskGroup', 'maternalAge', 'thyroidDisorders', 'corticosteroidUse', 'contraceptiveMethod'],
};

const NUMBER_FIELDS = ['oralGlucoseToleranceTest', 'fastingPlasmaGlucose', 'hemoglobinA1c', 'prepregnancyBmi', 'gestationalWeightGain', 'maternalAge', 'currentBmi', 'triglycerideLevels', 'hdlCholesterol'];
...
const DRAFT_KEY = 'postpartum_diabetes_riskPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const PostpartumDiabetesRiskDocument = ({ document: docProp }) => {
  ...
  /* ═══════ REHYDRATE PENDING DRAFTS (localStorage) — survives refresh; shown in JSX, NOT DB/PDF ═══════ */
  useEffect(() => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Postpartum Diabetes Risk ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // stageDraft: persist a field edit to the localStorage DRAFT store only (survives refresh).
  ...
  // Save = stage a DRAFT locally (+ localStorage). No DB write here.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // staged drafts for THIS section's fields: editKey = "field-idx", value = full field text
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const dotIdx = fieldPart.lastIndexOf('.');
        const hasArrayIdx = dotIdx !== -1 && /^\d+$/.test(fieldPart.slice(dotIdx + 1));
        const payload = { field: hasArrayIdx ? fieldPart.slice(0, dotIdx) : fieldPart, value: localEdits[editKey] };
        if (hasArrayIdx) payload.arrayIndex = parseInt(fieldPart.slice(dotIdx + 1), 10);
        const resp = await secureApiClient.put(`/api/edit/postpartum_diabetes_risk/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/postpartum_diabetes_risk/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[PostpartumDiabetesRisk] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== POSTPARTUM DIABETES RISK ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Postpartum Diabetes Risk ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: DATE FIELD — read-only formatted display ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="postpartum-diabetes-risk-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Postpartum Diabetes Risk</h2></div>
        <div className="empty-state">No postpartum diabetes risk records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="postpartum-diabetes-risk-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search postpartum diabetes risk records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'metabolic-profile')}
...
export default PostpartumDiabetesRiskDocument;
