/**
 * PreoperativePreparationDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: preoperative_preparation
 *
 * 7 Sections:
 *   1. procedure-info: procedureName, date
 *   2. preparation-details: bowelPrep, skinPrep, npoStatus
 *   3. prophylaxis: antibioticProphylaxis, dvtProphylaxis
 *   4. preoperative-testing: preoperativeTesting
 *   5. safety-checklist: consentObtained, siteMarking, timeOut
 *   6. general-info: facility
 *   7. notes: notes
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PreoperativePreparationDocumentPDFTemplate from '../pdf-templates/PreoperativePreparationDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import './PreoperativePreparationDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'preoperative_preparationPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['procedureName', 'bowelPrep', 'skinPrep', 'npoStatus', 'antibioticProphylaxis', 'dvtProphylaxis', 'consentObtained', 'siteMarking', 'timeOut', 'facility', 'notes'];
...
/* ═══════ COMPONENT ═══════ */
const PreoperativePreparationDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.preoperative_preparation) return Array.isArray(r.preoperative_preparation) ? r.preoperative_preparation : [r.preoperative_preparation];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.preoperative_preparation) return Array.isArray(dd.preoperative_preparation) ? dd.preoperative_preparation : [dd.preoperative_preparation]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // fieldPart = "field" (string/array whole-value) or "field.arrayIndex" (single array element)
        const editKey = `${fieldPart}-${idx}`;
        nLocal[editKey] = value;
        nPending[editKey] = true;
        const dotIdx = fieldPart.lastIndexOf('.');
        const isArrayElem = dotIdx !== -1 && /^\d+$/.test(fieldPart.slice(dotIdx + 1));
        if (isArrayElem || Array.isArray(value)) {
          // Array-element draft → mark editedFields with the dot-path key convention (item 0 if whole-array)
          nFields[isArrayElem ? `${fieldPart}-${idx}` : `${fieldPart}.0-${idx}`] = 'edited';
        } else {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Preoperative Preparation ${idx + 1}`.toLowerCase();
  ...
  // stageDraft: shared local-staging helper for sentence saves (no DB write — Approve commits).
  const stageDraft = (id, fn, idx, sid, fullText, sentenceMarks) => {
    const editKey = `${fn}-${idx}`;
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageDraft(id, fn, idx, sid, fullText, { [`${fn}-${idx}-s${sentenceIdx}`]: 'edited' });
      return;
    }
    ...
    stageDraft(id, fn, idx, sid, fullText, marks);
  }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/preoperative_preparation/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/preoperative_preparation/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PREOPERATIVE PREPARATION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Preoperative Preparation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="preoperative-preparation-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Preoperative Preparation</h2></div>
        <div className="empty-state">No preoperative preparation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="preoperative-preparation-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Preoperative Preparation</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PreoperativePreparationDocumentPDFTemplate document={pdfData} />} fileName="Preoperative_Preparation.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search preoperative preparation..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Preoperative Preparation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'procedure-info')}
            {renderSection(record, idx, 'preparation-details')}
            {renderSection(record, idx, 'prophylaxis')}
            {renderSection(record, idx, 'preoperative-testing')}
            {renderSection(record, idx, 'safety-checklist')}
...
export default PreoperativePreparationDocument;
