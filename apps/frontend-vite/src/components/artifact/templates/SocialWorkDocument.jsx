/**
 * SocialWorkDocument.jsx
 * July 2026 — recursive, dot-path editable social work
 * Collection: social_work
 */
import React, { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import SocialWorkDocumentPDFTemplate from '../pdf-templates/SocialWorkDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SocialWorkDocument.css';

const DRAFT_KEY = 'social_workPendingEdits';
const SECTION_TITLES = {
  record: 'Record Details', referral: 'Referral and Primary Concern', housing: 'Housing and Daily Support', financial: 'Insurance and Financial Concerns',
  psychosocial: 'Psychosocial Assessment', resources: 'Community Resources', discharge: 'Discharge and Case Management', safety: 'Safety and Functional Status', interventions: 'Interventions Provided', goals: 'Patient Goals',
};
const FIELD_LABELS = {
  date: 'Date', provider: 'Provider', facility: 'Facility', referralSource: 'Referral Source', primaryConcern: 'Primary Concern', livingArrangement: 'Living Arrangement', primaryCaregiver: 'Primary Caregiver', homeSafetyAssessment: 'Home Safety Assessment', transportationNeeds: 'Transportation Needs', insuranceStatus: 'Insurance Status', financialConcerns: 'Financial Concerns', mentalHealthHistory: 'Mental Health History', substanceUseHistory: 'Substance Use History', supportSystem: 'Support System', culturalConsiderations: 'Cultural Considerations', communityResourcesProvided: 'Community Resources Provided', dischargeDisposition: 'Discharge Disposition', barriersToDischarge: 'Barriers to Discharge', caseManagementLevel: 'Case Management Level', guardianshipStatus: 'Guardianship Status', advanceDirectives: 'Advance Directives', safetyRiskAssessment: 'Safety Risk Assessment', functionalStatus: 'Functional Status', interventionsProvided: 'Interventions Provided', followUpRequired: 'Follow-Up Required', patientGoals: 'Patient Goals',
};
const SECTION_FIELDS = {
  record: ['date', 'provider', 'facility'], referral: ['referralSource', 'primaryConcern'], housing: ['livingArrangement', 'primaryCaregiver', 'homeSafetyAssessment', 'transportationNeeds'],
  financial: ['insuranceStatus', 'financialConcerns'], psychosocial: ['mentalHealthHistory', 'substanceUseHistory', 'supportSystem', 'culturalConsiderations'], resources: ['communityResourcesProvided'],
  discharge: ['dischargeDisposition', 'barriersToDischarge', 'caseManagementLevel', 'guardianshipStatus', 'advanceDirectives', 'followUpRequired'], safety: ['safetyRiskAssessment', 'functionalStatus'], interventions: ['interventionsProvided'], goals: ['patientGoals'],
};
const DATE_FIELDS = ['date'];
const DATETIME_FIELDS = [];
const ENUM_FIELDS = ['caseManagementLevel', 'guardianshipStatus', 'followUpRequired'];
const NUMBER_FIELDS = [];
...
const COMMA_FIELDS = ['culturalConsiderations'];
const COMMA_ARRAY_SPLIT_FIELDS = [];
const ARRAY_FIELDS = ['financialConcerns', 'mentalHealthHistory', 'communityResourcesProvided', 'barriersToDischarge', 'interventionsProvided', 'patientGoals'];
const SEMICOLON_FIELDS = ['dischargeDisposition'];
...
const enumOptionsFor = (path, value) => {
  ...
  const options = { caseManagementLevel: ['Low', 'Moderate', 'High'], guardianshipStatus: ['Self', 'Guardian', 'Pending'], followUpRequired: ['Yes', 'No'] }[field] || [];
...
const SocialWorkDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.social_work) return Array.isArray(record.social_work) ? record.social_work : [record.social_work];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.social_work) return Array.isArray(nested.social_work) ? nested.social_work : [nested.social_work];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/social_work/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/social_work/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SocialWork] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    let text = '=== SOCIAL WORK ===\n\n';
    records.forEach((record, recordIndex) => {
      text += 'Social Work Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="social-work-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Social Work</h2></div><div className="empty-state">No social work available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Social Work Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="social-work-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Social Work</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<SocialWorkDocumentPDFTemplate document={pdfData} />} fileName="Social_Work.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search social work..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Social Work Record ' + (recordIndex + 1))}</h3></div>
...
export default SocialWorkDocument;
