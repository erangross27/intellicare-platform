/**
 * PatientEmotionalResponseDocumentPDFTemplate.jsx
 * June 2026 — Helvetica — A4 — BLACK & WHITE only (#000000 titles/borders/values, NO blue).
 * Collection: patient_emotional_response.
 *
 ...
 * concerns (string array) numbered list; recommendations (array of {recommendation, date}) date-grouped numbered list.
 */
...
const FIELD_LABELS = {
  date: 'Date', emotionalState: 'Emotional State', supportNeeded: 'Support Needed', response: 'Response',
  type: 'Type', provider: 'Provider', facility: 'Facility', findings: 'Findings', assessment: 'Assessment',
  plan: 'Plan', notes: 'Notes', status: 'Status', concerns: 'Concerns', results: 'Results', recommendations: 'Recommendations',
};
  ...
  if (STRING_ARRAY_FIELDS.includes(field)) {
    ...
    return [(
      <View key={field} style={styles.fieldGroup} wrap={items.length > 8 ? undefined : false}>
        {titleNode}
        {showLabel && <Text style={styles.fieldLabel}>{label}</Text>}
        {items.map((it, i) => (<Text key={i} style={styles.value}>{i + 1}. {fmtScalar(it)}</Text>))}
      </View>
    )];
  }
...
const PatientEmotionalResponseDocumentPDFTemplate = ({ document: data }) => {
  let records = [];
  if (Array.isArray(data)) {
    if (data.length === 1 && data[0]?.patient_emotional_response) records = Array.isArray(data[0].patient_emotional_response) ? data[0].patient_emotional_response : [data[0].patient_emotional_response];
    else records = data;
  } else if (data?.patient_emotional_response) records = Array.isArray(data.patient_emotional_response) ? data.patient_emotional_response : [data.patient_emotional_response];
  else if (data?.documentData) { const dd = data.documentData; if (Array.isArray(dd)) records = dd; else if (dd?.patient_emotional_response) records = Array.isArray(dd.patient_emotional_response) ? dd.patient_emotional_response : [dd.patient_emotional_response]; else if (dd && typeof dd === 'object') records = [dd]; }
  else if (data && typeof data === 'object') records = [data];
  records = (records || []).filter(r => r && typeof r === 'object');

  if (records.length === 0) {
    return (<Document><Page size="A4" style={styles.page}><View style={styles.documentHeader}><Text style={styles.title}>Patient Emotional Response</Text></View><Text style={styles.emptyState}>No records available</Text></Page></Document>);
  }

  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.title}>Patient Emotional Response</Text></View>
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer}>
            <View style={styles.recordHeader} wrap={false}>
              <Text style={styles.recordTitle}>{`Patient Emotional Response ${String(record._recordNumber || idx + 1)}`}</Text>
...
export default PatientEmotionalResponseDocumentPDFTemplate;
