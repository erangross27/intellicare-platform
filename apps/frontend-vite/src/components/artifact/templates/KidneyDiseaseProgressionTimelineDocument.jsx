/**
 * KidneyDiseaseProgressionTimelineDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: kidney_disease_progression_timeline
 *
 * 4 Sections:
 *   1. kidney-function: estimatedGlomerularFiltrationRate, serumCreatinine, bloodUreaNitrogen,
 *                       chronicKidneyDiseaseStage, albuminCreatinineRatio, proteinCreatinineRatio,
 *                       albuminuriaCategory, acidBaseStatus
 *   2. mineral-bone: hemoglobinLevel, serumPhosphorus, serumCalcium, parathyroidHormone, vitamin25OHD
 *   3. size-imaging: renalUltrasoundFindings, kidneySizeLeftCm, kidneySizeRightCm,
 *                    systemicBloodPressure, renalBiopsyResults
 *   4. dialysis: dialysisModalityType, dialysisAdequacyKtV, vascularAccessType,
 *                transplantEvaluationStatus, comorbidConditions
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import KidneyDiseaseProgressionTimelineDocumentPDFTemplate from '../pdf-templates/KidneyDiseaseProgressionTimelineDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './KidneyDiseaseProgressionTimelineDocument.css';
...
const NUMBER_FIELDS = [
  ...
  'dialysisAdequacyKtV',
  /* proteinCreatinineRatio is already above — total 13 from schema nums + dialysisAdequacyKtV = 14 */
];

const STRING_FIELDS = [
  'chronicKidneyDiseaseStage', 'albuminuriaCategory', 'acidBaseStatus',
  'renalUltrasoundFindings', 'systemicBloodPressure', 'renalBiopsyResults',
  'dialysisModalityType', 'vascularAccessType', 'transplantEvaluationStatus',
];

const ARRAY_FIELDS = ['comorbidConditions'];

/* Proteinuria markers: 0 is a MEANINGFUL result (no albuminuria/proteinuria).
   All other numbers: 0 is a sentinel (not measured / not on dialysis) → hide. */
const MEANINGFUL_ZERO_FIELDS = ['albuminCreatinineRatio', 'proteinCreatinineRatio'];
const numberHasVal = (fn, v) => {
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldName]: value } }  (value = full field text or array). */
const DRAFT_KEY = 'kidney_disease_progression_timelinePendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const KidneyDiseaseProgressionTimelineDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.kidney_disease_progression_timeline) return Array.isArray(r.kidney_disease_progression_timeline) ? r.kidney_disease_progression_timeline : [r.kidney_disease_progression_timeline];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.kidney_disease_progression_timeline) return Array.isArray(dd.kidney_disease_progression_timeline) ? dd.kidney_disease_progression_timeline : [dd.kidney_disease_progression_timeline]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Kidney Disease Progression Timeline ${idx + 1}`.toLowerCase();
  ...
  const stageDraft = (id, fn, idx, fullText, sid) => {
    const editKey = `${fn}-${idx}`;
  ...
  const handleSaveArrayItem = useCallback((record, fn, idx, itemIdx, sid) => {
    ...
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
    setEditedFields(prev => ({ ...prev, [`${fn}-${idx}-i${itemIdx}`]: 'edited' }));
    if (sid) setApprovedSections(prev => { const k = `${sid}-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; });
    const store = readDrafts();
    if (!store[id]) store[id] = {};
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Persist each staged field in this section to the DB now. localEdits[`${fn}-${idx}`] holds the
      // FULL field value (string or array — array elements were spliced into the array on save), so
      // each field is one PUT with no arrayIndex.
      const toCommit = fields
        .map(f => `${f}-${idx}`)
        .filter(k => pendingEdits[k] && localEdits[k] !== undefined);
      for (const editKey of toCommit) {
        const fn = editKey.slice(0, -`-${idx}`.length);
        const resp = await secureApiClient.put(`/api/edit/kidney_disease_progression_timeline/${id}/edit`, { field: fn, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/kidney_disease_progression_timeline/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      const val = getFieldValue(record, f, idx);
      if (NUMBER_FIELDS.includes(f) ? !numberHasVal(f, val) : !hasVal(val)) return;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== KIDNEY DISEASE PROGRESSION TIMELINE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Kidney Disease Progression Timeline ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx); if (!numberHasVal(fn, val)) return null;
    const editKey = `${fn}-${idx}`;
  ...
  /* ═══════ RENDER: ARRAY FIELD — comorbidConditions ═══════ */
                ...
                <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(itemStr); setSaveError(null); } }}>
                  {isEditing ? (
                    <div className="edit-field-container">
                      ...
                      {saveError && <div className="save-error">{saveError}</div>}
                      <div className="edit-actions">
                        <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveArrayItem(record, fn, idx, iIdx, sid); }}>{saving ? 'Saving...' : 'Save'}</button>
                        <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                      </div>
                    </div>
                  ) : (
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      return NUMBER_FIELDS.includes(f) ? numberHasVal(f, val) : hasVal(val);
    });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="kidney-disease-progression-timeline-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Kidney Disease Progression Timeline</h2></div>
        <div className="empty-state">No kidney disease progression timeline records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="kidney-disease-progression-timeline-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Kidney Disease Progression Timeline</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<KidneyDiseaseProgressionTimelineDocumentPDFTemplate document={pdfData} />} fileName="Kidney_Disease_Progression_Timeline.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search kidney disease progression timeline..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Kidney Disease Progression Timeline ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'kidney-function')}
            {renderSection(record, idx, 'mineral-bone')}
            {renderSection(record, idx, 'size-imaging')}
            {renderSection(record, idx, 'dialysis')}
          </div>
        ))}
      </div>
    </div>
...
export default KidneyDiseaseProgressionTimelineDocument;
