/**
 * DialysisRunSheets Edit Route
 * ServiceId: dialysis-run-sheets-edit-service
...
function buildContext(req, operation = 'read') { return { serviceId: 'dialysis-run-sheets-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
...
const ALLOWED_FIELDS = [
  ...
  'preDialysisBP', 'postDialysisBP',
  'ktVRatio', 'ureaReductionRatio', 'clearanceGoal',
  'vascularAccessType', 'accessThrill', 'accessBruit', 'accessRecirculation',
  'dialyzerType', 'dialysateBath', 'conductivity',
  'transmembranePressure', 'venousPressure', 'arterialPressure',
  'machineNumber',
  'complications',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('dialysis_run_sheets', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[DialysisRunSheets] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('dialysis_run_sheets', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[DialysisRunSheets] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
