/**
 * IcuFlowSheetsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: icu_flow_sheets
 *
 * 6 Sections:
 *   1. session-info: flowSheetDate (date), flowSheetTime (string)
 *   2. vitals-neuro: hourlyVitalSigns (string), neurologicAssessment (string)
 *   3. respiratory-hemodynamic: respiratorySupport (string), hemodynamicSupport (string)
 *   4. fluids-labs: fluidBalance (string), labValues (string)
 *   5. sedation-pain-interventions: sedationLevel, painScore, interventionsPerformed,
 *                                   drainsOutputs, medicationsAdministered (strings)
 *   6. recommendations: recommendations (string / array — gated out when empty)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import IcuFlowSheetsDocumentPDFTemplate from '../pdf-templates/IcuFlowSheetsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './IcuFlowSheetsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'icu_flow_sheetsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'vitals-neuro': 'Vitals & Neurologic',
  'respiratory-hemodynamic': 'Respiratory & Hemodynamic Support',
  'fluids-labs': 'Fluids & Lab Values',
  'sedation-pain-interventions': 'Sedation, Pain & Interventions',
  'recommendations': 'Recommendations',
};

const FIELD_LABELS = {
  flowSheetDate: 'Flow Sheet Date',
  flowSheetTime: 'Flow Sheet Time',
  hourlyVitalSigns: 'Hourly Vital Signs',
  neurologicAssessment: 'Neurologic Assessment',
  respiratorySupport: 'Respiratory Support',
  hemodynamicSupport: 'Hemodynamic Support',
  ...
  drainsOutputs: 'Drains/Outputs',
  medicationsAdministered: 'Medications Administered',
  recommendations: 'Recommendations',
};

const SECTION_FIELDS = {
  'session-info': ['flowSheetDate', 'flowSheetTime'],
  'vitals-neuro': ['hourlyVitalSigns', 'neurologicAssessment'],
  'respiratory-hemodynamic': ['respiratorySupport', 'hemodynamicSupport'],
  'fluids-labs': ['fluidBalance', 'labValues'],
  'sedation-pain-interventions': ['sedationLevel', 'painScore', 'interventionsPerformed', 'drainsOutputs', 'medicationsAdministered'],
  'recommendations': ['recommendations'],
};

const NUMBER_FIELDS = [];
const STRING_FIELDS = ['flowSheetTime', 'hourlyVitalSigns', 'neurologicAssessment', 'respiratorySupport', 'hemodynamicSupport', 'fluidBalance', 'labValues', 'sedationLevel', 'painScore', 'interventionsPerformed', 'drainsOutputs', 'medicationsAdministered', 'recommendations'];
const DATE_FIELDS = ['flowSheetDate'];
...
/* ═══════ COMPONENT ═══════ */
const IcuFlowSheetsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.icu_flow_sheets) return Array.isArray(r.icu_flow_sheets) ? r.icu_flow_sheets : [r.icu_flow_sheets];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.icu_flow_sheets) return Array.isArray(dd.icu_flow_sheets) ? dd.icu_flow_sheets : [dd.icu_flow_sheets]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `ICU Flow Sheet ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/icu_flow_sheets/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/icu_flow_sheets/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    // Skip the section header entirely when nothing populated (never leak an empty "Recommendations" header)
    if (!body) return '';
    return `${title}\n${'='.repeat(40)}\n\n${body}`;
  }, [getFieldValue, hasVal, fmtVal, splitBySentence, formatSentenceFieldLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== ICU FLOW SHEETS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `ICU Flow Sheet ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD — text input + parseFloat+isNaN→block save (no number fields in this template) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="icu-flow-sheets-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">ICU Flow Sheets</h2></div>
        <div className="empty-state">No ICU flow sheet records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="icu-flow-sheets-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">ICU Flow Sheets</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<IcuFlowSheetsDocumentPDFTemplate document={pdfData} />} fileName="Icu_Flow_Sheets.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search ICU flow sheets..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`ICU Flow Sheet ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'vitals-neuro')}
            {renderSection(record, idx, 'respiratory-hemodynamic')}
            {renderSection(record, idx, 'fluids-labs')}
            {renderSection(record, idx, 'sedation-pain-interventions')}
...
export default IcuFlowSheetsDocument;
