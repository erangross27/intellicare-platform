/**
 * WeightMonitoringDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: weight_monitoring (displayed as "Weight Monitoring")
 *
 * 7 Sections:
 *   1. record-info: createdAt (display only)
 *   2. weight-metrics: currentWeight, previousWeight, weightChangeKg, bodyMassIndex, idealBodyWeight, weightLossGoal
 *   3. fluid-status: fluidRetention, dailyFluidIntake, urineOutput24hr, peripheralEdema, abdominalGirth, jugularVenousDistention
 *   4. dietary-restrictions: sodiumRestriction, fluidRestriction, nutritionalStatus
 *   5. medications-compliance: diureticMedications, medicationCompliance
 *   6. cardiac-markers: nyhaClassification, serumSodium, serumCreatinine, bntProBnp
 *   7. monitoring: weightMonitoringFrequency, weightVariabilityPattern
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import WeightMonitoringDocumentPDFTemplate from '../pdf-templates/WeightMonitoringDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './WeightMonitoringDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'weight_monitoringPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'medications-compliance': 'Medications & Compliance',
...
const FIELD_LABELS = {
  ...
  previousWeight: 'Previous Weight (kg)',
  ...
  dailyFluidIntake: 'Daily Fluid Intake (mL)',
  urineOutput24hr: 'Urine Output 24hr (mL)',
  peripheralEdema: 'Peripheral Edema',
  abdominalGirth: 'Abdominal Girth (cm)',
  jugularVenousDistention: 'Jugular Venous Distention',
  sodiumRestriction: 'Sodium Restriction (mg)',
  fluidRestriction: 'Fluid Restriction (mL)',
  nutritionalStatus: 'Nutritional Status',
  diureticMedications: 'Diuretic Medications',
  medicationCompliance: 'Medication Compliance',
  nyhaClassification: 'NYHA Classification',
  serumSodium: 'Serum Sodium (mEq/L)',
  serumCreatinine: 'Serum Creatinine (mg/dL)',
  bntProBnp: 'BNT Pro-BNP (pg/mL)',
  weightMonitoringFrequency: 'Weight Monitoring Frequency',
  weightVariabilityPattern: 'Weight Variability Pattern',
};

const SECTION_FIELDS = {
  'record-info': ['createdAt'],
  'weight-metrics': ['currentWeight', 'previousWeight', 'weightChangeKg', 'bodyMassIndex', 'idealBodyWeight', 'weightLossGoal'],
  'fluid-status': ['fluidRetention', 'dailyFluidIntake', 'urineOutput24hr', 'peripheralEdema', 'abdominalGirth', 'jugularVenousDistention'],
  'dietary-restrictions': ['sodiumRestriction', 'fluidRestriction', 'nutritionalStatus'],
  'medications-compliance': ['diureticMedications', 'medicationCompliance'],
  'cardiac-markers': ['nyhaClassification', 'serumSodium', 'serumCreatinine', 'bntProBnp'],
  'monitoring': ['weightMonitoringFrequency', 'weightVariabilityPattern'],
};

const NUMBER_FIELDS = ['currentWeight', 'previousWeight', 'weightChangeKg', 'bodyMassIndex', 'idealBodyWeight', 'dailyFluidIntake', 'urineOutput24hr', 'sodiumRestriction', 'fluidRestriction', 'abdominalGirth', 'weightLossGoal', 'serumSodium', 'serumCreatinine', 'bntProBnp'];
/* MEANINGFUL_ZERO_FIELDS: numerics where 0 is a valid clinical finding (weightChangeKg 0 = weight stable). All other weight/measurement numerics use 0 as a "not recorded" sentinel → hidden. */
const MEANINGFUL_ZERO_FIELDS = ['weightChangeKg'];
const DATE_FIELDS = ['createdAt'];
const BOOLEAN_FIELDS = ['fluidRetention', 'jugularVenousDistention'];
const ARRAY_FIELDS = ['diureticMedications'];
const STRING_FIELDS = ['nyhaClassification', 'weightMonitoringFrequency', 'peripheralEdema', 'nutritionalStatus', 'weightVariabilityPattern', 'medicationCompliance'];
...
const toInputDate = (dateValue) => {
  if (!dateValue) return '';
  try { return new Date(dateValue.$date || dateValue).toISOString().slice(0, 10); } catch { return ''; }
};

/* ═══════ COMPONENT ═══════ */
const WeightMonitoringDocument = ({ document: docProp, data: dataProp, templateData }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [copiedSection, setCopiedSection] = useState(null);
  const [copiedItems, setCopiedItems] = useState({});
  const [showCopied, setShowCopied] = useState(false);
  const [localEdits, setLocalEdits] = useState({});
  // editKeys (`${field}-${idx}`) that are staged drafts (saved locally, NOT yet in DB/PDF). Cleared on Approve.
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      if (!recDrafts) return;
      // Group dotted array fieldParts so we can rebuild the full array stored under `${field}-${idx}`.
      const arrayBuckets = {};
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (dotIdx !== -1 && /^\d+$/.test(tail)) {
          ...
          if (!arrayBuckets[baseField]) {
            const orig = Array.isArray(record[baseField]) ? [...record[baseField]] : [];
            arrayBuckets[baseField] = orig;
          }
          arrayBuckets[baseField][arrIndex] = value;
      ...
      Object.entries(arrayBuckets).forEach(([baseField, arr]) => {
  ...
  /* numberShows: numeric "not recorded" (0) hidden as sentinel unless meaningful-zero field (weightChangeKg) or doctor-edited. Negative values (weight loss) always show. */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Weight Monitoring ${idx + 1}`.toLowerCase();
  ...
  // Save = stage DRAFT locally (+ localStorage). NO DB write until Approve commits it.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Persist each staged draft for this section's fields. Draft store fieldParts are "field" or
      // "field.arrayIndex" — add arrayIndex ONLY when the segment after the last dot is purely numeric.
      const store = readDrafts();
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const payload = { field: isArrayElem ? fieldPart : baseField, value };
        const resp = await secureApiClient.put(`/api/edit/weight_monitoring/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedFields.push(fieldPart);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/weight_monitoring/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (committedFields.length > 0) {
        committedFields.forEach(fp => { delete recDrafts[fp]; });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      const rows = DATE_FIELDS.includes(f)
        ? [formatDate(val)]
        : BOOLEAN_FIELDS.includes(f)
          ? [val ? 'Yes' : 'No']
          : ARRAY_FIELDS.includes(f)
            ? (Array.isArray(val) ? val : [val]).map(String)
    ...
    return text;
  }, [getFieldValue, hasVal, fmtVal, splitFieldValue, numberShows]);
            ...
            {isEditing ? (
              <div className="edit-field-container">
                ...
                <div className="edit-actions">
                  <button className="save-btn" onClick={event => { event.stopPropagation(); if (!editValue || Number.isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date'); return; } handleSaveField(record, fn, idx, sid, null, `${editValue}T00:00:00.000Z`); }}>Save</button>
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== WEIGHT MONITORING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Weight Monitoring ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    ...
    return (
      <div key={fn} className="rec-mini-card nested-mini-card">
        ...
        <div data-edit-field={fn}><div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayVal); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
  ...
  const renderBooleanField = (record, fn, idx, sid) => {
    ...
    return (
      <div key={fn} className="rec-mini-card nested-mini-card">
        {showLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
        <div data-edit-field={fn}><div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(val ? 'Yes' : 'No'); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
    ...
    return (
      <div key={fn} className="rec-mini-card nested-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          return (
            <div key={itemIdx}>
              <div data-edit-field={`${fn}.${itemIdx}`}><div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: currentArr })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { const k = `${sid}-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; }); const store = readDrafts(); if (!store[id2]) store[id2] = {}; store[id2][`${fn}.${itemIdx}`] = editValue; writeDrafts(store); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                ...
                <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(rowKey); setEditValue(clause); setSaveError(null); } }}>
                  {isEditing ? (
                    <div className="edit-field-container">
                      ...
                      <div className="edit-actions">
                        <button className="save-btn" onClick={event => { event.stopPropagation(); const updated = [...clauses]; updated[clauseIndex] = editValue.trim(); const fullText = updated.filter(Boolean).join('. '); stageSentenceDraft(record, fn, idx, sid, fullText, { [rowKey]: 'edited' }); }}>Save</button>
                        <button className="cancel-btn" onClick={event => { event.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                      </div>
                    </div>
                  ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="weight-monitoring-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Weight Monitoring</h2></div>
        <div className="empty-state">No weight monitoring data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="weight-monitoring-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Weight Monitoring</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<WeightMonitoringDocumentPDFTemplate document={pdfData} />} fileName="Weight_Monitoring.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search weight monitoring..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Weight Monitoring ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'record-info')}
            {renderSection(record, idx, 'weight-metrics')}
            {renderSection(record, idx, 'fluid-status')}
            {renderSection(record, idx, 'dietary-restrictions')}
            {renderSection(record, idx, 'medications-compliance')}
            {renderSection(record, idx, 'cardiac-markers')}
...
export default WeightMonitoringDocument;
