/**
 * SjogrensSyndromeAssessmentDocument.jsx
 * July 2026 — recursive, dot-path editable sjogrens syndrome assessment
...
import SjogrensSyndromeAssessmentDocumentPDFTemplate from '../pdf-templates/SjogrensSyndromeAssessmentDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SjogrensSyndromeAssessmentDocument.css';

const DRAFT_KEY = 'sjogrens_syndrome_assessmentPendingEdits';
const SECTION_TITLES = {
  recordInfo: 'Assessment Information',
  sicca: 'Sicca Symptoms & Testing',
  glandTesting: 'Salivary Gland Testing',
  manifestations: 'Systemic Manifestations & Scores',
...
const FIELD_LABELS = {
  date: 'Date', type: 'Type', provider: 'Provider', facility: 'Facility', status: 'Status', sicca: 'Sicca Symptoms & Testing', salivarGlandBiopsy: 'Salivary Gland Biopsy', sialography: 'Sialography', systemicManifestations: 'Systemic Manifestations', essdaiScore: 'ESSDAI Score', esspriScore: 'ESSPRI Score', findings: 'Findings', assessment: 'Assessment', plan: 'Plan', recommendations: 'Recommendations', results: 'Results', notes: 'Notes',
};
const SECTION_FIELDS = {
  recordInfo: ['date', 'type', 'provider', 'facility', 'status'],
  sicca: ['sicca'],
  glandTesting: ['salivarGlandBiopsy', 'sialography'],
  manifestations: ['systemicManifestations', 'essdaiScore', 'esspriScore'],
  findingsAssessment: ['findings', 'assessment'],
  plan: ['plan'],
  recommendations: ['recommendations'],
  resultsNotes: ['results', 'notes'],
...
const NUMBER_FIELDS = ['essdaiScore', 'esspriScore'];
const NUMBER_STRING_FIELDS = [];
const NUMBER_UNITS = {};
const OBJECT_FIELDS = ['sicca', 'results'];
const MIXED_OBJECT_ARRAY_FIELDS = [];
const OBJECT_ITEM_LABELS = {};
const NARRATIVE_PATHS = ['sicca.dryEyes', 'sicca.dryMouth', 'sicca.schirmerTest', 'sicca.saxonTest'];
const PARENTHETICAL_LABEL_FIELDS = [];
const PARENTHETICAL_SEMICOLON_FIELDS = [];
const COMMA_FIELDS = ['sicca.dryEyes', 'sicca.dryMouth', 'findings', 'assessment', 'plan'];
const COMMA_ARRAY_SPLIT_FIELDS = [];
const ARRAY_FIELDS = ['systemicManifestations'];
const SEMICOLON_FIELDS = ['sicca.schirmerTest', 'salivarGlandBiopsy'];
const LABELED_LIST_FIELDS = ['findings', 'assessment'];
...
const SjogrensSyndromeAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.sjogrens_syndrome_assessment) return Array.isArray(record.sjogrens_syndrome_assessment) ? record.sjogrens_syndrome_assessment : [record.sjogrens_syndrome_assessment];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.sjogrens_syndrome_assessment) return Array.isArray(nested.sjogrens_syndrome_assessment) ? nested.sjogrens_syndrome_assessment : [nested.sjogrens_syndrome_assessment];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/sjogrens_syndrome_assessment/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/sjogrens_syndrome_assessment/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SjogrensSyndromeAssessment] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Sjogrens Syndrome Assessment Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="sjogrens-syndrome-assessment-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Sjogrens Syndrome Assessment</h2></div><div className="empty-state">No sjogrens syndrome assessment available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Sjogrens Syndrome Assessment Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="sjogrens-syndrome-assessment-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Sjogrens Syndrome Assessment</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<SjogrensSyndromeAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Sjogrens_Syndrome_Assessment.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search sjogrens syndrome assessment..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Sjogrens Syndrome Assessment Record ' + (recordIndex + 1))}</h3></div>
...
export default SjogrensSyndromeAssessmentDocument;
