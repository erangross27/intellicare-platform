/**
 * KetoneMonitoringInstructionsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: ketone_monitoring_instructions
 *
 * 4 Sections:
 *   1. session-info: date (date picker), provider (string), facility (string), status (string)
 *   2. monitoring-instructions: whenToCheck (sentence), interpretation (sentence), actionRequired (sentence)
 *   3. clinical-details: findings (sentence), assessment (sentence), plan (sentence)
 *   4. additional-notes: notes (sentence)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import KetoneMonitoringInstructionsDocumentPDFTemplate from '../pdf-templates/KetoneMonitoringInstructionsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './KetoneMonitoringInstructionsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [editKey]: { lv, field, value } } }
     editKey = "<field>-<idx>" (the localEdits key)
     lv      = the localEdits value used for rendering (string | object | array)
     field   = the backend field path the original save would have PUT (may be dotted for object leaves)
     value   = the backend value the original save would have PUT */
const DRAFT_KEY = 'ketone_monitoring_instructionsPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  ...
  'monitoring-instructions': ['whenToCheck', 'interpretation', 'actionRequired'],
...
const SENTENCE_FIELDS = ['whenToCheck', 'interpretation', 'actionRequired', 'findings', 'assessment', 'plan', 'notes'];
...
const KEY_OVERRIDES = {
  bg: 'BG', ketone: 'Ketone', dka: 'DKA', bgl: 'BGL', hr: 'HR', bmi: 'BMI',
};
...
/* ═══════ COMPONENT ═══════ */
const KetoneMonitoringInstructionsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.ketone_monitoring_instructions) return Array.isArray(r.ketone_monitoring_instructions) ? r.ketone_monitoring_instructions : [r.ketone_monitoring_instructions];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.ketone_monitoring_instructions) return Array.isArray(dd.ketone_monitoring_instructions) ? dd.ketone_monitoring_instructions : [dd.ketone_monitoring_instructions]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([editKey, entry]) => {
        ...
        const fieldPart = dashIdx === -1 ? editKey : editKey.substring(0, dashIdx);
        const renderKey = `${fieldPart}-${idx}`;
        nLocal[renderKey] = entry.lv;
        nPending[renderKey] = true;
        nFields[renderKey] = 'edited';
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Ketone Monitoring Instructions ${idx + 1}`.toLowerCase();
  ...
  /* Stage a DRAFT locally + write it to the pending-drafts localStorage store (survives refresh).
     NOT written to MongoDB and NOT shown in the PDF until the user clicks Approve (handleApproveSection commits).
       editKey  = the localEdits render key, "<field>-<idx>"
       localVal = the value used for rendering (string | object | array)
       field    = the backend field path the original save would have PUT (may be dotted for object leaves)
       value    = the backend value the original save would have PUT */
  const stageDraft = useCallback((record, idx, editKey, localVal, field, value) => {
    ...
    store[id][editKey] = { lv: localVal, field, value };
    writeDrafts(store);
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    stageDraft(record, idx, editKey, saveVal, fn, saveVal);
  }, [editValue, safeId, stageDraft]);

  /* save a nested OBJECT leaf by dot-path (e.g. results.ketone) — value stays a STRING */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    // Stage as a DRAFT (no DB write). Backend payload mirrors the original: dotted field + leaf value.
    stageDraft(record, idx, `${rootField}-${idx}`, clone, dottedField, newVal);
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageDraft(record, idx, `${fn}-${idx}`, fullText, fn, fullText);
      return;
    }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // editKeys staged for this record + belonging to a field in this section
    ...
    try {
      // Persist each staged field to the DB now, using the backend payload captured at stage time.
      const store = readDrafts();
      const recDrafts = store[id] || {};
      for (const editKey of toCommit) {
        const entry = recDrafts[editKey];
        // field/value as the original save would have PUT (entry.field may be a dotted object path)
        const field = entry ? entry.field : (editKey.slice(0, -suffix.length));
        ...
        if (lastDot !== -1 && /^\d+$/.test(field.slice(lastDot + 1))) {
          payload.field = field.slice(0, lastDot);
          payload.arrayIndex = parseInt(field.slice(lastDot + 1), 10);
        }
        const resp = await secureApiClient.put(`/api/edit/ketone_monitoring_instructions/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/ketone_monitoring_instructions/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== KETONE MONITORING INSTRUCTIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Ketone Monitoring Instructions ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: SENTENCE FIELD with splitBySentence+parseLabel ═══════ */
            ...
            {sentences.map((sentence, sIdx) => {
              ...
              if (parsed.isLabeled) {
                ...
                if (commaParts.length < 2) {
                  ...
                  if (parenListMatch) { commaParts = parenListMatch[1].split(',').map(s => s.trim()).filter(Boolean); }
                }
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              stageDraft(record, idx, `${fn}-${idx}`, newArr, fn, newArr);
                            }}>{saving ? 'Saving...' : 'Save'}</button>
                            <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                          </div>
                        </div>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="ketone-monitoring-instructions-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Ketone Monitoring Instructions</h2></div>
        <div className="empty-state">No ketone monitoring instructions records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="ketone-monitoring-instructions-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Ketone Monitoring Instructions</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<KetoneMonitoringInstructionsDocumentPDFTemplate document={pdfData} />} fileName="Ketone_Monitoring_Instructions.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search ketone monitoring instructions..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Ketone Monitoring Instructions ${idx + 1}`)}</h3>
            </div>
...
export default KetoneMonitoringInstructionsDocument;
