/**
 * SportsPhysicalExaminationDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: sports_physical_examination
 *
 * 14 Sections:
 *   1. sport-info: sportType, competitionLevel
 *   2. vital-signs: vitalSignsRestingHeartRate, vitalSignsBloodPressure, bmiPercentile
 *   3. cardiac-auscultation: cardiacAuscultationFindings
 *   4. cardiac-screening: marfanScreeningResult
 *   5. cardiac-risk-factors: cardiacRiskFactors
 *   6. neurological: previousConcussionHistory, concussionCount
 *   7. musculoskeletal: musculoskeletalLimitations
 *   8. functional-movement: functionalMovementScreen
 *   9. vision-screening: visionScreeningResult
 *  10. medical-conditions: asthmaExerciseInduced, seizureDisorderControlled, heatIllnessHistory, immunizationCompliance
 *  11. additional-info: singleOrganStatus, orthodonticAppliances, menstrualHistoryFemaleAthlete
 *  12. restrictions: restrictionsImposed
 *  13. clearance: clearanceStatus, followUpRequired
 *  14. provider-info: examinerPhysicianName
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import SportsPhysicalExaminationDocumentPDFTemplate from '../pdf-templates/SportsPhysicalExaminationDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SportsPhysicalExaminationDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'sports_physical_examinationPendingEdits';
const readDrafts = () => {
...
/* Numeric fields where 0 is a sentinel (missing/not-measured), NOT a meaningful value.
...
const SportsPhysicalExaminationDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.sports_physical_examination) return Array.isArray(r.sports_physical_examination) ? r.sports_physical_examination : [r.sports_physical_examination];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.sports_physical_examination) return Array.isArray(dd.sports_physical_examination) ? dd.sports_physical_examination : [dd.sports_physical_examination]; return [dd]; }
      return [r];
    });
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayItem) {
          ...
          const lk = `${fn}-${idx}`;
          const base = nLocal[lk] !== undefined ? nLocal[lk] : (Array.isArray(record[fn]) ? [...record[fn]] : []);
  ...
  /* hasDisplayVal: like hasVal but treats sentinel-0 numeric fields as empty (used for section gating). */
  const hasDisplayVal = useCallback((v, fn) => { if (ZERO_SENTINEL_FIELDS.includes(fn) && Number(v) === 0) return false; return hasVal(v); }, [hasVal]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Sports Physical Examination ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Stage one edit as a DRAFT (no DB write). localStorage keeps it across refresh; Approve commits it.
  // fieldPart = "field" (scalar) or "field.arrayIndex" (one array element). localKey is the
  // localEdits/pendingEdits key (always "field-idx" so a full-array edit stays grouped under one key).
  const stageDraft = useCallback((record, fieldPart, localKey, localValue) => {
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    stageDraft(record, fn, `${fn}-${idx}`, saveVal);
    const trackKey = editTrackingKey || `${fn}-${idx}`;
    setEditedFields(prev => ({ ...prev, [trackKey]: 'edited' }));
    setEditingField(null); setEditValue(''); setSaveError(null);
  }, [editValue, safeId, stageDraft]);

  // Save one sentence = stage the rebuilt full field text as a DRAFT (no DB write). Approve commits.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Commit each staged draft fieldPart belonging to this section (fieldPart = "field" or "field.arrayIndex").
      const store = readDrafts();
      ...
      for (const fieldPart of Object.keys(recDrafts)) {
        const dotIdx = fieldPart.indexOf('.');
        const isArrayItem = dotIdx !== -1 && /^\d+$/.test(fieldPart.slice(dotIdx + 1));
        const fn = isArrayItem ? fieldPart.slice(0, dotIdx) : fieldPart;
        if (!fields.includes(fn)) continue;
        const payload = { field: fn, value: recDrafts[fieldPart] };
        if (isArrayItem) payload.arrayIndex = parseInt(fieldPart.slice(dotIdx + 1), 10);
        const resp = await secureApiClient.put(`/api/edit/sports_physical_examination/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedFns.add(fn);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/sports_physical_examination/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; committedFns.forEach(fn => delete n[`${fn}-${idx}`]); return n; });
      // Drop this section's committed drafts from localStorage (keep any other section's drafts)
      ...
      if (store2[id]) {
        Object.keys(store2[id]).forEach(fieldPart => {
          ...
          const isArrayItem = dotIdx !== -1 && /^\d+$/.test(fieldPart.slice(dotIdx + 1));
          const fn = isArrayItem ? fieldPart.slice(0, dotIdx) : fieldPart;
          if (fields.includes(fn)) delete store2[id][fieldPart];
        });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== SPORTS PHYSICAL EXAMINATION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Sports Physical Examination ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; stageDraft(record, `${fn}.${itemIdx}`, `${fn}-${idx}`, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="sports-physical-examination-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Sports Physical Examination</h2></div>
        <div className="empty-state">No sports physical examination data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="sports-physical-examination-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Sports Physical Examination</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<SportsPhysicalExaminationDocumentPDFTemplate document={pdfData} />} fileName={`sports-physical-examination-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search sports physical examination..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <div className="record-meta-row">
                ...
                {record.clearanceStatus && (
                  <span className={`status-badge ${record.clearanceStatus.toLowerCase().includes('cleared') ? 'status-cleared' : 'status-other'}`}>
                    {highlightText(record.clearanceStatus)}
                  </span>
                )}
              </div>
              <h3 className="record-name">{highlightText(`Sports Physical Examination ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'sport-info')}
            {renderSection(record, idx, 'vital-signs')}
            {renderSection(record, idx, 'cardiac-auscultation')}
            {renderSection(record, idx, 'cardiac-screening')}
            {renderSection(record, idx, 'cardiac-risk-factors')}
            {renderSection(record, idx, 'neurological')}
            {renderSection(record, idx, 'musculoskeletal')}
            {renderSection(record, idx, 'functional-movement')}
            ...
            {renderSection(record, idx, 'clearance')}
...
export default SportsPhysicalExaminationDocument;
