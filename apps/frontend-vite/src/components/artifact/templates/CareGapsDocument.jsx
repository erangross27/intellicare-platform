/**
 * CareGapsDocument.jsx
 * June 2026 — Full rewrite to FULL TEMPLATE STANDARD (100% schema coverage, typed fields).
 * Collection: care_gaps
 *
 * Donor cloned: PointOfCareUltrasoundHeartRateDocument (per-sentence renderStringField,
 *   recursive OBJECT renderer for `results`, date-grouped array renderer for `recommendations`,
 *   4-level search, per-section approve, Copy row/Section/All, pdfData memo, blue-glow theme).
 *
 * FIELDS (12 extractable → 100% schema coverage):
 *   DATE:   date
 *   STRING: type, provider, facility, findings, assessment, plan, notes, status
 *   ARRAY:  screenings (array of {screeningType, category, status, dueDate, actionRequired, priority}),
 *           recommendations (array of {recommendation, date})
 *   OBJECT: results (recursive key-value)
 *
 * Per-sentence narratives: findings, assessment, plan, notes.
 * Short simple fields: type, provider, facility, status, date.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import CareGapsPDFTemplate from '../pdf-templates/CareGapsPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './CareGapsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF/Copy until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the field name, e.g. "findings",
   "results", "screenings" — value is the FULL merged field value the editor produced). */
const DRAFT_KEY = 'care_gapsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'header-info': 'Care Gap Information',
  'screenings-section': 'Screenings',
  'clinical': 'Clinical',
...
const SECTION_FIELDS = {
  'header-info': ['date', 'type', 'provider', 'facility'],
  'screenings-section': ['screenings'],
...
const SECTION_ORDER = ['header-info', 'screenings-section', 'clinical', 'results-section', 'recommendations-section', 'notes-status'];
...
/* Array-of-objects screening items (editable sub-fields) */
const SCREENING_ARRAY_FIELDS = ['screenings'];

/* sub-fields rendered inside each screening item, in display order */
const SCREENING_SUBFIELDS = ['screeningType', 'category', 'status', 'priority', 'dueDate', 'actionRequired'];
const SCREENING_SUBLABELS = {
...
const SCREENING_DATE_SUBFIELDS = ['dueDate'];

const KEY_OVERRIDES = {
  hba1c: 'HbA1c', bp: 'BP', bmi: 'BMI', ldl: 'LDL', hdl: 'HDL', egfr: 'eGFR',
  pcp: 'PCP', poc: 'POC', hcc: 'HCC', icd: 'ICD',
};
...
const getStatusStyle = (status) => {
  ...
  if (s.includes('overdue') || s.includes('missing'))   return { background: 'rgba(239,68,68,0.15)', color: '#ef4444', border: '1px solid rgba(239,68,68,0.4)' };
  if (s.includes('due soon'))                            return { background: 'rgba(234,179,8,0.15)', color: '#eab308', border: '1px solid rgba(234,179,8,0.4)' };
  if (s.includes('completed') || s.includes('current')) return { background: 'rgba(34,197,94,0.15)', color: '#22c55e', border: '1px solid rgba(34,197,94,0.4)' };
  return { background: 'rgba(96,165,250,0.15)', color: '#60a5fa', border: '1px solid rgba(96,165,250,0.4)' };
};

/* ═══════ COMPONENT ═══════ */
const CareGapsDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.care_gaps) return Array.isArray(r.care_gaps) ? r.care_gaps : [r.care_gaps];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.care_gaps) return Array.isArray(dd.care_gaps) ? dd.care_gaps : [dd.care_gaps]; return [dd]; }
      return [r];
    });
  ...
  const screeningsToText = useCallback((scr) => {
    if (!Array.isArray(scr)) return flattenSearchable(scr);
    return scr.map(s => flattenSearchable(s)).join(' ');
  }, []);
  ...
  const fieldSearchText = useCallback((f, val) => {
    ...
    if (SCREENING_ARRAY_FIELDS.includes(f)) return screeningsToText(val);
    if (OBJECT_FIELDS.includes(f)) return flattenSearchable(val);
    return fmtVal(val);
  }, [recsToText, screeningsToText, fmtVal]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `care gap ${idx + 1} ${record.type || ''}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* Stage a DRAFT for one field locally + persist to the pending-drafts localStorage store
     (survives refresh). NOT written to MongoDB / NOT shown in the PDF until Approve commits it. */
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, valueOverride, editTrackingKey) => {
    ...
    setSaveError(null);
    stageDraft(record, fn, idx, sid, saveVal, editTrackingKey || `${fn}-${idx}`);
  }, [editValue, safeId, stageDraft]);

  /* save a nested OBJECT leaf by dot-path (e.g. results.ldl) — value stays a STRING.
     Staged locally as the FULL merged `results` object (NOT written to DB until Approve). */
  ...
  /* save a single sub-field of a screening item in the screenings[] array */
  const saveScreeningSub = useCallback((record, fn, idx, sid, sIdx, subKey, newVal, trackKey) => {
    const id = safeId(record); if (!id) return;
    const currentArr = Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [];
    const newArr = currentArr.map((it, i) => i === sIdx ? { ...it, [subKey]: newVal } : { ...it });
    setSaveError(null);
    stageDraft(record, fn, idx, sid, newArr, trackKey);
  }, [safeId, getFieldValue, stageDraft]);

  /* Stage a narrative-field draft (full reconstructed text) locally + to localStorage. No DB write. */
  const stageSentenceDraft = useCallback((record, fn, idx, sid, fullText, applySentenceMarkers) => {
    const id = safeId(record); if (!id) return;
    const editKey = `${fn}-${idx}`;
    setLocalEdits(prev => ({ ...prev, [editKey]: fullText }));
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
    applySentenceMarkers();
    setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
    const store = readDrafts();
    if (!store[id]) store[id] = {};
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Staged drafts for THIS record that belong to THIS section's fields.
      const toCommit = Object.keys(localEdits).filter(k => {
        ...
        const effectiveField = isArrayIdx ? baseField : fieldPart;
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/care_gaps/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/care_gaps/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[CareGaps] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (SCREENING_ARRAY_FIELDS.includes(f)) {
        const items = Array.isArray(val) ? val.filter(it => !isEmptyDeep(it)) : [];
        if (!sameAsTitle) text += `${label}\n`;
        items.forEach((item, i) => {
          const name = (item?.screeningType || `Screening ${i + 1}`);
          text += `${i + 1}. ${name}\n`;
          SCREENING_SUBFIELDS.forEach(sk => {
            if (sk === 'screeningType') return;
            const sv = item?.[sk];
            if (isEmptyDeep(sv)) return;
            const disp = SCREENING_DATE_SUBFIELDS.includes(sk) ? formatDate(sv) : fmtScalar(sv);
            text += `   ${SCREENING_SUBLABELS[sk] || humanizeKey(sk)}: ${disp}\n`;
          });
        });
        text += '\n';
  ...
  const copyAllText = useCallback(async () => {
    ...
    pdfData.forEach((r, idx) => {
      text += `Care Gap ${idx + 1}${r.type ? ` — ${r.type}` : ''}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: STRING FIELD (per-sentence; status with color badge) ═══════ */
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* Status field: single value with color badge */
    if (fn === 'status') {
      const editKey = `${fn}-${idx}`;
      const isEditing = editingField === editKey;
      const isModified = editedFields[editKey];
        ...
        <div key={fn} className="rec-mini-card">
          ...
          <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(strVal); setSaveError(null); } }}>
            {isEditing ? (
              ...
              <>
                <div className="row-content"><span className="status-badge" style={getStatusStyle(strVal)}>{highlightText(strVal)}</span><span className="edit-indicator">&#9998;</span></div>
  ...
  /* ═══════ RENDER: SCREENINGS — array of {screeningType, category, status, dueDate, actionRequired, priority} ═══════ */
  const renderScreeningSub = (record, fn, idx, sid, sIdx, subKey, subVal) => {
    if (isEmptyDeep(subVal)) return null;
    const isDate = SCREENING_DATE_SUBFIELDS.includes(subKey);
    const displayVal = isDate ? formatDate(subVal) : fmtScalar(subVal);
    const trackKey = `${fn}-${idx}-i${sIdx}-${subKey}`;
    const isEditing = editingField === trackKey;
    const isModified = editedFields[trackKey];
    const subLabel = SCREENING_SUBLABELS[subKey] || humanizeKey(subKey);
    const isStatus = subKey === 'status';
    return (
      <div key={subKey} className="nested-mini-card">
        <div className="nested-subtitle sub-label">{highlightText(subLabel)}</div>
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(trackKey); setEditValue(isDate ? toInputDate(subVal) : fmtScalar(subVal)); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              {isDate ? (
                <input type="date" className="edit-date" value={editValue} onChange={e => setEditValue(e.target.value)} ref={el => { if (el) { el.focus(); try { el.showPicker(); } catch {} } }} onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
              ...
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => {
                  e.stopPropagation();
                  if (isDate) { if (isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date'); return; } saveScreeningSub(record, fn, idx, sid, sIdx, subKey, editValue, trackKey); }
                  else saveScreeningSub(record, fn, idx, sid, sIdx, subKey, editValue.trim(), trackKey);
                }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
            ...
            <>
              <div className="row-content">
                {isStatus
                  ? <span className="status-badge" style={getStatusStyle(displayVal)}>{highlightText(displayVal)}</span>
                  : <span className="content-value">{highlightText(displayVal)}</span>}
                <span className="edit-indicator">&#9998;</span>
              </div>
              <button className={`copy-btn ${copiedItems[trackKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${subLabel}: ${displayVal}`, trackKey); }}>{copiedItems[trackKey] ? 'Copied!' : 'Copy'}</button>
            </>
          )}
        </div>
        {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
  ...
  const renderScreeningsField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    const items = Array.isArray(val) ? val.filter(it => !isEmptyDeep(it)) : [];
    ...
    return (
      <div key={fn} className="rec-mini-card">
        {showSubLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
        {items.map((item, sIdx) => {
          const name = (item?.screeningType || `Screening ${sIdx + 1}`);
          const itemSearchText = flattenSearchable(item).toLowerCase();
          const itemVisible = phraseMatch || labelMatch || !searchTerm.trim() || itemSearchText.includes(phrase);
          if (!itemVisible && searchTerm.trim()) return null;
          /* sub-keys present (object-style scalars) plus the documented order */
          const presentKeys = SCREENING_SUBFIELDS.filter(k => !isEmptyDeep(item?.[k]));
          const extraKeys = (item && typeof item === 'object')
            ? Object.keys(item).filter(k => !SCREENING_SUBFIELDS.includes(k) && isScalar(item[k]) && !isEmptyDeep(item[k]))
            : [];
          return (
            <div key={sIdx} className="rec-mini-card" style={{ marginTop: 8 }}>
              <div className="nested-subtitle">{highlightText(name)}</div>
              {presentKeys.filter(k => k !== 'screeningType').map(k => renderScreeningSub(record, fn, idx, sid, sIdx, k, item[k]))}
              {extraKeys.map(k => renderScreeningSub(record, fn, idx, sid, sIdx, k, item[k]))}
            </div>
          );
        })}
      </div>
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (SCREENING_ARRAY_FIELDS.includes(f)) return renderScreeningsField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="care-gaps-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Care Gaps</h2></div>
        <div className="empty-state">No care gaps data available.</div>
      </div>
    );
  }
  ...
  return (
    <div className="care-gaps-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Care Gaps</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<CareGapsPDFTemplate document={pdfData} />} fileName={`care-gaps-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search care gaps..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(record.type ? `Care Gap — ${record.type}` : `Care Gap ${idx + 1}`)}</h3>
            </div>
...
export default CareGapsDocument;
