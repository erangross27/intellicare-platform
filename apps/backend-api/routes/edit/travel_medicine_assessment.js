...
function buildContext(req, operation = 'read') { return { serviceId: 'travel-medicine-assessment-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['date', 'destinationCountries', 'departureDate', 'returnDate', 'tripDurationDays', 'purposeOfTravel', 'accommodationType', 'ruralExposure', 'vaccinesAdministered', 'yellowFeverVaccineGiven', 'certificateOfVaccinationIssued', 'malariaProphylaxisPrescribed', 'malariaRiskLevel', 'travelersDiarrheaProphylaxis', 'altitudeSicknessRisk', 'insectBorneDiseasePrecautions', 'foodWaterSafetyEducation', 'preExistingConditions', 'immunocompromisedStatus', 'pregnancyStatus', 'previousTravelHistory', 'exposureRisks', 'travelInsuranceStatus', 'emergencyContactAbroad'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('travel_medicine_assessment', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[TravelMedicineAssessment] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('travel_medicine_assessment', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[TravelMedicineAssessment] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
