...
const SECTIONS = [
  { id: 'screeningInfo', title: 'Screening Information', fields: ['screeningType', 'screeningCategory', 'complianceStatus', 'riskLevel'] }, { id: 'providerFacility', title: 'Provider & Facility', fields: ['provider', 'facility', 'orderingProvider'] }, { id: 'scheduleDates', title: 'Schedule & Dates', fields: ['date', 'lastScreeningDate', 'dueDate', 'nextDueDate'] }, { id: 'resultsCompliance', title: 'Results & Compliance', fields: ['screeningResult', 'screeningCompleted', 'nonComplianceReason', 'patientRefusalDocumented'] }, { id: 'guidelinesEligibility', title: 'Guidelines & Eligibility', fields: ['guidelineReference', 'eligibilityCriteria', 'ageAtScreening', 'insuranceCoverage'] }, { id: 'followupReminders', title: 'Follow-Up & Reminders', fields: ['followUpRequired', 'followUpRecommendation', 'remindersSent', 'lastReminderDate', 'qualityMeasureCode', 'barrierToCompliance'] },
];
const FIELD_LABELS = {
...
export default function ScreeningComplianceDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Screening Compliance</Text></View>
    {!records.length && <Text style={styles.noDataText}>No screening compliance data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Screening Compliance {recordIndex + 1}</Text></View>
