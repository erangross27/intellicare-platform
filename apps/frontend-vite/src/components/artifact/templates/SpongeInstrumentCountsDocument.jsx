/**
 * SpongeInstrumentCountsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: sponge_instrument_counts (displayed as "Sponge/Instrument Counts")
 *
 * 6 Sections:
 *   1. record-info: facility, countingNurse, scrubTech
 *   2. procedure: procedureName
 *   3. initial-count: initialCount
 *   4. additional-counts: additionalCounts
 *   5. final-count: finalCount, countCorrect, discrepancy, xrayObtained
 *   6. notes: notes
 */
...
const DRAFT_KEY = 'sponge_instrument_countsPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  additionalCounts: 'Additional Counts',
  finalCount: 'Final Count',
  countCorrect: 'Count Correct',
  discrepancy: 'Discrepancy',
  xrayObtained: 'X-Ray Obtained',
  notes: 'Notes',
};
...
const SECTION_FIELDS = {
  ...
  'final-count': ['finalCount', 'countCorrect', 'discrepancy', 'xrayObtained'],
  'notes': ['notes'],
};
...
/* ═══════ COMPONENT ═══════ */
const SpongeInstrumentCountsDocument = ({ document: docProp }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        const fieldName = isArrayIdx ? fieldPart.slice(0, dotIdx) : fieldPart;
        const editKey = `${fieldName}-${idx}`;
        nLocal[editKey] = value;
        nPending[editKey] = true;
        if (isArrayIdx) {
          nFields[`${fieldName}.${fieldPart.slice(dotIdx + 1)}-${idx}`] = 'edited';
        } else {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Sponge/Instrument Count ${idx + 1}`.toLowerCase();
  ...
  /* Stage a DRAFT locally + write it to the pending-drafts localStorage store (survives refresh).
     ...
     fieldPart = "field" or "field.arrayIndex". The localEdits store always holds the
     full effective value under `${fn}-${idx}` (string, or full array for array fields). */
  const stageEdit = useCallback((record, fn, idx, sid, value, fieldPart) => {
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    setSaveError(null);
    stageEdit(record, fn, idx, sid, saveVal, fn);
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      setSaveError(null);
      stageEdit(record, fn, idx, sid, fullText, fn);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      // localEdits holds the full effective value under `${fn}-${idx}` (string, or full array for array fields).
      const committedKeys = [];
      for (const f of fields) {
        const editKey = `${f}-${idx}`;
        if (!pendingEdits[editKey] || localEdits[editKey] === undefined) continue;
        const resp = await secureApiClient.put(`/api/edit/sponge_instrument_counts/${id}/edit`, { field: f, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedKeys.push(editKey);
      }
      // Flag the record/section approved (audit trail)
      await secureApiClient.put(`/api/edit/sponge_instrument_counts/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (store[id]) {
        fields.forEach(f => {
          Object.keys(store[id]).forEach(fp => { const base = fp.indexOf('.') !== -1 && /^\d+$/.test(fp.slice(fp.indexOf('.') + 1)) ? fp.slice(0, fp.indexOf('.')) : fp; if (base === f) delete store[id][fp]; });
        });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== SPONGE/INSTRUMENT COUNTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Sponge/Instrument Count ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; stageEdit(record, fn, idx, sid, currentArr, `${fn}.${itemIdx}`); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                    ...
                    <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ciParsed.value); setSaveError(null); } }}>
                      {ciEditing ? (
                        <div className="edit-field-container">
                          ...
                          {saveError && <div className="save-error">{saveError}</div>}
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const allItems = splitByComma(String(getFieldValue(record, fn, idx) || '')); allItems[ciIdx] = `${ciParsed.label}: ${editValue.trim()}`; const fullText = allItems.join(', '); setSaveError(null); stageEdit(record, fn, idx, sid, fullText, fn); setEditedSentences(prev => ({ ...prev, [commaKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                            <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                          </div>
                        </div>
                      ) : (
                  ...
                  <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                    {ciEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const allItems = splitByComma(String(getFieldValue(record, fn, idx) || '')); allItems[ciIdx] = editValue.trim(); const fullText = allItems.join(', '); setSaveError(null); stageEdit(record, fn, idx, sid, fullText, fn); setEditedSentences(prev => ({ ...prev, [commaKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="sponge-instrument-counts-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Sponge/Instrument Counts</h2></div>
        <div className="empty-state">No sponge/instrument count records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="sponge-instrument-counts-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search sponge/instrument count records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'initial-count')}
            {renderSection(record, idx, 'additional-counts')}
            {renderSection(record, idx, 'final-count')}
...
export default SpongeInstrumentCountsDocument;
