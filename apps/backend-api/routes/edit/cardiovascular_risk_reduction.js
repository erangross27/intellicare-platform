/**
 * Cardiovascular Risk Reduction Edit Route
 */
...
function buildContext(req, operation = 'read') { return { serviceId: 'cardiovascular-risk-reduction-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
...
const ALLOWED_FIELDS = [
  ...
  'smokingStatus', 'packYearsHistory',
  'framinghamRiskScore', 'reynoldsRiskScore', 'pcr10Score',
...
router.put('/:id/edit', async (req, res) => {
  ...
  } catch (err) { console.error('[CVRisk] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('cardiovascular_risk_reduction', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[CVRisk] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
