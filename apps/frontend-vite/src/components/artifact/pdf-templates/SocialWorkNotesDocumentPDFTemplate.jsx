...
const SECTIONS = [
  { id: 'assessment', title: 'Assessment and Functional Capacity', fields: ['psychosocialAssessmentScore', 'adlFunctionalCapacity', 'iadlAssessmentResults', 'cognitiveCapacityAssessment'] },
  { id: 'support', title: 'Social Support and Caregiver Burden', fields: ['socialSupportNetwork', 'caregiverBurdenAssessment'] },
  { id: 'resources', title: 'Financial, Housing, and Community Resources', fields: ['financialResourceAssessment', 'insuranceCoverageStatus', 'housingStabilityStatus', 'transportationBarriers', 'communityResourceReferrals'] },
  { id: 'health', title: 'Mental Health, Substance Use, and Medication Support', fields: ['mentalHealthDiagnoses', 'substanceUseHistory', 'medicationAdherence'] },
  { id: 'planning', title: 'Advance Care and Discharge Planning', fields: ['advanceDirectiveStatus', 'elderAbuseScreening', 'culturalConsiderations', 'dischargeDisposition'] },
  { id: 'risk', title: 'Safety and Readmission Risks', fields: ['fallRiskFactors', 'readmissionRiskFactors'] },
  { id: 'additional', title: 'Additional Data', fields: ['additionalData'] },
];
const FIELD_LABELS = {
  psychosocialAssessmentScore: 'Psychosocial Assessment Score', adlFunctionalCapacity: 'ADL Functional Capacity', iadlAssessmentResults: 'IADL Assessment Results', cognitiveCapacityAssessment: 'Cognitive Capacity Assessment', socialSupportNetwork: 'Social Support Network', caregiverBurdenAssessment: 'Caregiver Burden Assessment', financialResourceAssessment: 'Financial Resource Assessment', insuranceCoverageStatus: 'Insurance Coverage Status', housingStabilityStatus: 'Housing Stability Status', transportationBarriers: 'Transportation Barriers', communityResourceReferrals: 'Community Resource Referrals', mentalHealthDiagnoses: 'Mental Health Diagnoses', substanceUseHistory: 'Substance Use History', medicationAdherence: 'Medication Adherence', advanceDirectiveStatus: 'Advance Directive Status', elderAbuseScreening: 'Elder Abuse Screening', culturalConsiderations: 'Cultural Considerations', dischargeDisposition: 'Discharge Disposition', fallRiskFactors: 'Fall Risk Factors', readmissionRiskFactors: 'Readmission Risk Factors', additionalData: 'Additional Data',
};
const DATE_FIELDS = [];
...
export default function SocialWorkNotesDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Social Work Notes</Text></View>
    {!records.length && <Text style={styles.noDataText}>No social work notes data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Social Work Notes Record {recordIndex + 1}</Text></View>
