...
function buildContext(req, operation = 'read') { return { serviceId: 'doctors-medications-recommendations-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = [
  'date', 'provider', 'facility', 'prescribedMedication', 'medicationClass',
  'dosageAmount', 'dosageFrequency', 'routeOfAdministration', 'durationOfTreatment',
  'indicationForUse', 'prescriptionStartDate', 'prescriptionEndDate', 'refillsAuthorized',
  'specialInstructions', 'contraindicationsNoted', 'knownAllergies', 'potentialInteractions',
  'therapeuticGoal', 'monitoringParameters', 'adverseEffectsDiscussed',
  'alternativeMedicationsConsidered', 'priorMedicationTrials', 'requiresPriorAuthorization',
  'prescriptionMethod', 'pharmacyDetails', 'followUpRecommendation',
  'patientCounselingCompleted', 'reasonForChange'
];
router.put('/:id/edit', async (req, res) => {
  ...
  } catch (err) { console.error('[DoctorsMedicationsRecommendations] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  ...
  } catch (err) { console.error('[DoctorsMedicationsRecommendations] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
