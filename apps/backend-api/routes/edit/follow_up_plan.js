/**
 * FollowUpPlan Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/follow_up_plan/:id/edit    — Edit a single field
 * PUT /api/edit/follow_up_plan/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'follow-up-plan-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('follow_up_plan', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[FollowUpPlan] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('follow_up_plan', { _id: objectId }, {
      $set: {
    ...
    console.error('[FollowUpPlan] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
