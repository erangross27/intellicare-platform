/**
 * Surrogacy Evaluation Edit Route
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'surrogacy-evaluation-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'gestationalCarrierAge',
  'gravidityParityHistory',
  'previousUncomplictedLiveBirths',
  'bodyMassIndexKgM2',
  'cesareanSectionCount',
  'uterineAnatomyAssessment',
  'endometrialThicknessMm',
  'cervicalCompetenceHistory',
  'bloodTypeAndRhFactor',
  'irregularAntibodyScreen',
  'infectiousDiseasePanel',
  'cytomegalovirusSerostatus',
  'rubellaTiterImmunity',
  'varicellaTiterImmunity',
  'thrombophiliaScreening',
  'antiphospholipidAntibodyPanel',
  'thyroidStimulatingHormoneMuL',
  'hemoglobinA1cPercent',
  'psychologicalClearanceStatus',
  'minnesotaMultiphasicPersonalityInventoryResults',
  'nicotineCotinineScreenResult',
  'urineDrugScreenPanel',
  'preexistingHypertensionHistory',
  'gestationalDiabetesHistory',
  'preeclampsiaEclampsiaHistory',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('surrogacy_evaluation', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[surrogacy_evaluation] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('surrogacy_evaluation', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[surrogacy_evaluation] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
