/**
 * PostpartumGlucoseMonitoringDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: postpartum_glucose_monitoring
 *
 * 4 Sections:
 *   1. monitoring-timeline: immediatePostpartum, sixWeekTest, longTermScreening
 *   2. visit-info: date, provider, facility
 *   3. clinical-findings: findings, assessment
 *   4. management: plan, recommendations, notes
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PostpartumGlucoseMonitoringDocumentPDFTemplate from '../pdf-templates/PostpartumGlucoseMonitoringDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './PostpartumGlucoseMonitoringDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'postpartum_glucose_monitoringPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  sixWeekTest: 'Six-Week Test',
  longTermScreening: 'Long-Term Screening',
  date: 'Date',
  provider: 'Provider',
  facility: 'Facility',
  findings: 'Findings',
...
const SECTION_FIELDS = {
  'monitoring-timeline': ['immediatePostpartum', 'sixWeekTest', 'longTermScreening'],
  'visit-info': ['date', 'provider', 'facility'],
...
const STRING_FIELDS = ['immediatePostpartum', 'sixWeekTest', 'longTermScreening', 'findings', 'assessment', 'plan', 'notes', 'provider', 'facility', 'type', 'status'];
...
/* toInputDate: normalize any stored date to 'YYYY-MM-DD' for BlueDatePicker */
...
/* ═══════ COMPONENT ═══════ */
const PostpartumGlucoseMonitoringDocument = ({ document: docProp }) => {
  ...
  const recIdOf = useCallback((r) => { if (!r?._id) return null; if (typeof r._id === 'string') return r._id; if (r._id.$oid) return r._id.$oid; return String(r._id); }, []);
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        // Mark edited so the Pending Approve button lights up (mirror save markers)
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Postpartum Glucose Monitoring ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Commit each staged field for this section. localEdits keys are "<field>-<idx>".
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // here always a plain field name (no dot)
        const lastDot = fieldPart.lastIndexOf('.');
        const payload = { field: fieldPart, value: localEdits[editKey] };
        // arrayIndex ONLY when the trailing dot-segment is purely numeric (mirrors handleSaveField fieldPart build)
        if (lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1))) {
          payload.field = fieldPart.slice(0, lastDot);
          payload.arrayIndex = parseInt(fieldPart.slice(lastDot + 1), 10);
        }
        const resp = await secureApiClient.put(`/api/edit/postpartum_glucose_monitoring/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/postpartum_glucose_monitoring/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== POSTPARTUM GLUCOSE MONITORING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Postpartum Glucose Monitoring ${idx + 1}\n${'='.repeat(40)}\n\n`;
      ...
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          const itemStr = (typeof item === 'object' && item !== null)
            ? `${item.recommendation || ''}${item.date ? ` (${item.date})` : ''}`
            : String(item);
                            ...
                            <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                              {ciEditing ? (
                                <div className="edit-field-container">
                                  ...
                                  {saveError && <div className="save-error">{saveError}</div>}
                                  <div className="edit-actions">
                                    <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const currentVal2 = String(getFieldValue(record, fn, idx) || ''); const sentences2 = splitBySentence(currentVal2); const s2 = sentences2[sIdx] || ''; const p2 = parseLabel(s2); if (!p2.isLabeled) return; const items2 = splitByComma(p2.value); const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { items2.splice(ciIdx, 1, ...subParts); } else { items2[ciIdx] = trimmed.replace(/[;.]+$/, '').trim(); } const rebuilt = `${p2.label}: ${items2.join(', ')}.`; const allS = [...sentences2]; allS[sIdx] = rebuilt; const fullText2 = reconstructFullText(allS); const lk2 = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [lk2]: fullText2 })); setPendingEdits(prev => ({ ...prev, [lk2]: true })); setApprovedSections(prev => { const k = `${sid}-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; }); const store2 = readDrafts(); if (!store2[id2]) store2[id2] = {}; store2[id2][fn] = fullText2; writeDrafts(store2); const marks = { [commaKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ciIdx + ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                                    <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                                  </div>
                                </div>
                              ) : (
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(toInputDate(val)); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              ...
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveField(record, fn, idx, sid, null, editValue ? `${editValue}T00:00:00.000Z` : ''); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
    ...
    return (
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            if (f === 'date') return renderDateField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="postpartum-glucose-monitoring-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Postpartum Glucose Monitoring</h2></div>
        <div className="empty-state">No postpartum glucose monitoring records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="postpartum-glucose-monitoring-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Postpartum Glucose Monitoring</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PostpartumGlucoseMonitoringDocumentPDFTemplate document={pdfData} />} fileName="Postpartum_Glucose_Monitoring.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search postpartum glucose monitoring records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Postpartum Glucose Monitoring ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'monitoring-timeline')}
...
export default PostpartumGlucoseMonitoringDocument;
