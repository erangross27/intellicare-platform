 ...
 * July 2026 — recursive, dot-path editable shift handoff notes
...
const SECTION_TITLES = {
  acuityScores: 'Acuity & Scoring',
  vitalSigns: 'Vital Signs Trends',
  fluidMedications: 'Fluid Balance & Medications',
  neuroPain: 'Neuro / Pain / Ventilation',
  accessDevices: 'Access & Devices',
  labsSafety: 'Labs & Safety',
  carePlanning: 'Care Planning & Disposition',
};
const FIELD_LABELS = {
...
const SECTION_FIELDS = {
  acuityScores: ['patientAcuityLevel', 'apacheIIScore', 'sofaScore', 'glasgowComaScale'],
  vitalSigns: ['vitalSignsTrends'],
  fluidMedications: ['fluidBalanceTotal', 'medicationChanges', 'vasopressorSupport'],
  neuroPain: ['mechanicalVentilation', 'sedationScore', 'painAssessment'],
  accessDevices: ['centralVenousAccess', 'drainageDevices'],
  labsSafety: ['laboratoryPending', 'isolationPrecautions', 'fallRiskAssessment', 'skinIntegrityStatus'],
  carePlanning: ['nutritionalSupport', 'familyCommunication', 'codeStatus', 'consultationsPending', 'proceduresScheduled', 'dischargePreparation'],
};
const DATE_FIELDS = [];
const DATETIME_FIELDS = [];
const ENUM_FIELDS = ['patientAcuityLevel', 'codeStatus'];
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'patientAcuityLevel' ? ['Low', 'Moderate', 'High', 'Critical']
    : field === 'codeStatus' ? ['Full code', 'DNR', 'DNI', 'DNR/DNI', 'Comfort measures only'] : [];
...
const ShiftHandoffNotesDocument = ({ document: docProp }) => {
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/shift_handoff_notes/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/shift_handoff_notes/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[ShiftHandoffNotes] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Shift Handoff Notes Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="shift-handoff-notes-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Shift Handoff Notes</h2></div><div className="empty-state">No shift handoff notes available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Shift Handoff Notes Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="shift-handoff-notes-document" ref={containerRef}>
    <div className="document-header">
      ...
      <div className="header-actions">
        ...
        <PDFDownloadLink document={<ShiftHandoffNotesDocumentPDFTemplate document={pdfData} />} fileName="Shift_Handoff_Notes.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search shift handoff notes..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Shift Handoff Notes Record ' + (recordIndex + 1))}</h3></div>
