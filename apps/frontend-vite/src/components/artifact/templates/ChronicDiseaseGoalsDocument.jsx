/**
 * ChronicDiseaseGoalsDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: chronic_disease_goals
 *
 * 5 Sections:
 *   1. goal-definition: goalCategory, targetCondition, goalDescription, priority, expressedBy, outcomeCode
 *   2. targets-progress: targetValue, targetMetric, baselineValue, currentValue, achievementPercentage (number), achievementStatus, lifecycleStatus
 *   3. timeline: startDate (date), targetDate (date), reviewDate (date), nextReviewDate (date), measurementFrequency
 *   4. interventions-barriers: interventions (array), barriers (array), addresses (array)
 *   5. care-team-engagement: careTeamMembers (array), patientEngagement, relatedGoals (array), modificationReason
 *
 * Header date: date
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ChronicDiseaseGoalsDocumentPDFTemplate from '../pdf-templates/ChronicDiseaseGoalsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './ChronicDiseaseGoalsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF/Copy until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldName]: value } } — fieldName matches the localEdits "${fn}-${idx}" base. */
const DRAFT_KEY = 'chronic_disease_goalsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'timeline': 'Timeline',
  'interventions-barriers': 'Interventions & Barriers',
  'care-team-engagement': 'Care Team & Engagement',
};
...
const SECTION_FIELDS = {
  'goal-definition': ['goalCategory', 'targetCondition', 'goalDescription', 'priority', 'expressedBy', 'outcomeCode'],
  'targets-progress': ['targetValue', 'targetMetric', 'baselineValue', 'currentValue', 'achievementPercentage', 'achievementStatus', 'lifecycleStatus'],
  'timeline': ['startDate', 'targetDate', 'reviewDate', 'nextReviewDate', 'measurementFrequency'],
  'interventions-barriers': ['interventions', 'barriers', 'addresses'],
  'care-team-engagement': ['careTeamMembers', 'patientEngagement', 'relatedGoals', 'modificationReason'],
};

const DATE_FIELDS = ['date', 'startDate', 'targetDate', 'reviewDate', 'nextReviewDate'];
const NUMBER_FIELDS = ['achievementPercentage'];
const ARRAY_FIELDS = ['interventions', 'barriers', 'addresses', 'careTeamMembers', 'relatedGoals'];
const STRING_FIELDS = ['goalCategory', 'targetCondition', 'goalDescription', 'targetValue', 'targetMetric', 'baselineValue', 'currentValue', 'achievementStatus', 'priority', 'lifecycleStatus', 'expressedBy', 'outcomeCode', 'patientEngagement', 'modificationReason', 'measurementFrequency'];
...
/* ═══════ COMPONENT ═══════ */
const ChronicDiseaseGoalsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.chronic_disease_goals) return Array.isArray(r.chronic_disease_goals) ? r.chronic_disease_goals : [r.chronic_disease_goals];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.chronic_disease_goals) return Array.isArray(dd.chronic_disease_goals) ? dd.chronic_disease_goals : [dd.chronic_disease_goals]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Chronic Disease Goal ${idx + 1}`.toLowerCase();
  ...
  // stageDraft = common helper: stage a field-level value locally + persist to the localStorage draft
  // store (survives refresh). NO DB write — the value is committed only when the user clicks Approve.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const f of toCommit) {
        const editKey = `${f}-${idx}`;
        // arrayIndex applies ONLY when the trailing dot-segment is numeric — never the case here
        // (this template stores whole-field values, incl. full arrays, under "${fn}-${idx}").
        await secureApiClient.put(`/api/edit/chronic_disease_goals/${id}/edit`, { field: f, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/chronic_disease_goals/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== CHRONIC DISEASE GOALS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Chronic Disease Goal ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(parsed.isLabeled ? parsed.value : itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; setSaveError(null); const saveVal = parsed.isLabeled ? `${parsed.label}: ${editValue}` : editValue; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = saveVal; stageDraft(record, fn, idx, sid, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="chronic-disease-goals-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Chronic Disease Goals</h2></div>
        <div className="empty-state">No chronic disease goals data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="chronic-disease-goals-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Chronic Disease Goals</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ChronicDiseaseGoalsDocumentPDFTemplate document={pdfData} />} fileName={`chronic-disease-goals-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search chronic disease goals..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Chronic Disease Goal ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'goal-definition')}
            {renderSection(record, idx, 'targets-progress')}
            {renderSection(record, idx, 'timeline')}
            {renderSection(record, idx, 'interventions-barriers')}
            {renderSection(record, idx, 'care-team-engagement')}
          </div>
        ))}
      </div>
    </div>
...
export default ChronicDiseaseGoalsDocument;
