/**
 * PolycysticKidneyDiseaseDocument.jsx
 * June 2026 — Collection: polycystic_kidney_disease (NEW template)
 *
 * Mirrors MyositisAssessmentDocument:
 *  - PodiatryExaminations recursive generic OBJECT renderer (humanizeKey + nested-mini-card,
 *    hide-empty at every level) for the object fields:
 *      extrarenalManifestations (booleans → Yes/No, string leaf via textarea),
 *      geneticTesting (string leaves), results (OBJECT of value+unit strings → number input).
 *  - AdmissionAssessments per-sentence string editing, date field, splitBySentence on [.;],
 *    single-name-skip showSubLabel, AND the date-grouped renderRecommendationsField for `recommendations`
 *    (array of {recommendation,date}, consecutive same dates show the date once).
 *
 * Editable: narrative strings (type, provider, facility, status, mayoClass, totalKidneyVolume,
 *   findings, assessment, plan, notes), date, tolvaptanCandidate (date), cystComplications (array of
 *   strings), recommendations.
 * Object (recursive renderer, editable leaves): extrarenalManifestations, geneticTesting, results.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PolycysticKidneyDiseaseDocumentPDFTemplate from '../pdf-templates/PolycysticKidneyDiseaseDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './PolycysticKidneyDiseaseDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field"; full field value) */
const DRAFT_KEY = 'polycystic_kidney_diseasePendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  facility: 'Facility',
  status: 'Status',
  mayoClass: 'Mayo Class',
  totalKidneyVolume: 'Total Kidney Volume',
  cystComplications: 'Cyst Complications',
  extrarenalManifestations: 'Extrarenal Manifestations',
  geneticTesting: 'Genetic Testing',
  tolvaptanCandidate: 'Tolvaptan Candidate',
...
/* boolean field (schema type 'boolean'): tolvaptanCandidate — rendered as Yes/No, NOT a date.
   Some legacy records hold a corrupted Date sentinel here; coerce to boolean for display/edit. */
...
/* OBJECT fields (recursive renderer, editable leaves) */
const OBJECT_FIELDS = ['extrarenalManifestations', 'geneticTesting', 'results'];
/* Array-of-strings (editable, numbered rows) */
const STRING_ARRAY_FIELDS = ['cystComplications'];
/* Array-of-objects (date-grouped, editable) */
...
/* coerce a schema-boolean field (may arrive as a real boolean, a "yes/true/1" string, or a legacy
   Date/garbage sentinel) into a true/false. Explicit falsey strings -> false; everything else
   non-empty -> true (legacy Date sentinels represent an affirmative flag). */
const coerceBool = (v) => {
...
/* ═══════ COMPONENT ═══════ */
const PolycysticKidneyDiseaseDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.polycystic_kidney_disease) return Array.isArray(r.polycystic_kidney_disease) ? r.polycystic_kidney_disease : [r.polycystic_kidney_disease];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.polycystic_kidney_disease) return Array.isArray(dd.polycystic_kidney_disease) ? dd.polycystic_kidney_disease : [dd.polycystic_kidney_disease]; return [dd]; }
      return [r];
    });
  ...
  const fieldSearchText = useCallback((f, val) => {
    ...
    if (OBJECT_FIELDS.includes(f) || STRING_ARRAY_FIELDS.includes(f)) return flattenSearchable(val);
    if (BOOLEAN_FIELDS.includes(f)) return coerceBool(val) ? 'yes' : 'no';
    return fmtVal(val);
  }, [recsToText, fmtVal]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `polycystic kidney disease ${idx + 1}`;
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.eGFR) — stage a DRAFT only (no DB write).
     The full merged object is held in localEdits[`${rootField}-${idx}`] and written to the draft store
     under `rootField`; Approve commits the whole field to MongoDB. */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    const store = readDrafts();
    if (!store[id]) store[id] = {};
    store[id][rootField] = committedClone;
    writeDrafts(store);
    setEditingField(null); setEditValue('');
  }, [safeId]);

  // Save one sentence = stage a DRAFT only (no DB write). localStorage keeps it across refresh; Approve commits.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // staged pending edits for this record whose field belongs to this section
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const payload = { field: /^\d+$/.test(lastSeg) ? fieldPart.slice(0, lastDot) : fieldPart, value: localEdits[editKey] };
        if (/^\d+$/.test(lastSeg)) payload.arrayIndex = parseInt(lastSeg, 10);
        const resp = await secureApiClient.put(`/api/edit/polycystic_kidney_disease/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/polycystic_kidney_disease/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[PolycysticKidneyDisease] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const objectCopyLines = useCallback((label, value, indent) => {
    ...
    if (isScalar(value)) {
      if (label) { out.push(`${pad}${label}`); out.push(`${pad}1. ${fmtScalar(value)}`); }
      else out.push(`${pad}${fmtScalar(value)}`);
      return out;
    }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_ARRAY_FIELDS.includes(f)) {
        ...
        const bv = coerceBool(val) ? 'Yes' : 'No';
        text += sameAsTitle ? `${bv}\n\n` : `${label}\n${bv}\n\n`;
      } else {
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== POLYCYSTIC KIDNEY DISEASE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Polycystic Kidney Disease ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: STRING ARRAY (cystComplications) — numbered editable rows ═══════ */
  const renderStringArrayField = (record, fn, idx, sid) => {
    ...
    const phraseMatch = !searchTerm.trim() || sectionTitleMatches(sid) || record._showAllSections;
    const labelMatch = searchTerm.trim() && label.toLowerCase().includes(searchTerm.toLowerCase().trim());
    /* keep original indices stable for dot-path save after hide-empty */
                  ...
                  <div className="edit-field-container">
                    ...
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => {
                        ...
                        // stage a DRAFT only (no DB write) — Approve commits the whole field
                        setSaveError(null);
                        const editKey = `${fn}-${idx}`;
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; number+unit -> number input, text -> textarea, boolean -> Yes/No select) ═══════ */
          ...
          {isEditing ? (
            <div className="edit-field-container">
              {isBool ? (
                ...
                </div>{ratio ? <span className="number-edit-unit">{`/ ${ratio.denom}`}</span> : (nu.unit && <span className="number-edit-unit">{nu.unit}</span>)}</div>
              ) : (
                <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
              )}
              {saveError && <div className="save-error">{saveError}</div>}
  ...
  if (!records || records.length === 0) {
    return (
      <div className="polycystic-kidney-disease-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Polycystic Kidney Disease</h2></div>
        <div className="empty-state">No polycystic kidney disease records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="polycystic-kidney-disease-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Polycystic Kidney Disease</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PolycysticKidneyDiseaseDocumentPDFTemplate document={pdfData} />} fileName="Polycystic_Kidney_Disease.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search polycystic kidney disease records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Polycystic Kidney Disease ${idx + 1}`)}</h3>
            </div>
...
export default PolycysticKidneyDiseaseDocument;
