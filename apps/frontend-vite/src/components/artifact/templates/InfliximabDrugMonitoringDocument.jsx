/**
 * InfliximabDrugMonitoringDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: infliximab_drug_monitoring
 *
 * 7 Sections:
 *   1. drug-levels: infliximabDoseAdministered, infliximabSerumLevel, antiDrugAntibodies
 *   2. disease-activity: crohnsDiseaseActivityIndex, harveyBradshawIndex, mayoClinicScore
 *   3. inflammatory-markers: cReactiveProtein, erythrocyteSedimentationRate, fecalCalprotectin
 *   4. safety-labs: hepaticTransaminases, completeBloodCount, tuberculosisScreening,
 *                   hepatitisBSurfaceAntigen, hepatitisCViralLoad
 *   5. clinical-assessment: infusionReactionSeverity, concomitantImmunosuppression,
 *                           dosageOptimizationRequired, treatmentResponseAssessment, adverseEventProfile
 *   6. planning: nextInfusionDueDate
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import InfliximabDrugMonitoringDocumentPDFTemplate from '../pdf-templates/InfliximabDrugMonitoringDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './InfliximabDrugMonitoringDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; whole-field value) */
const DRAFT_KEY = 'infliximab_drug_monitoringPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['hepaticTransaminases', 'completeBloodCount', 'tuberculosisScreening', 'hepatitisBSurfaceAntigen', 'infusionReactionSeverity', 'treatmentResponseAssessment'];
...
/* ═══════ COMPONENT ═══════ */
const InfliximabDrugMonitoringDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.infliximab_drug_monitoring) return Array.isArray(r.infliximab_drug_monitoring) ? r.infliximab_drug_monitoring : [r.infliximab_drug_monitoring];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.infliximab_drug_monitoring) return Array.isArray(dd.infliximab_drug_monitoring) ? dd.infliximab_drug_monitoring : [dd.infliximab_drug_monitoring]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Infliximab Drug Monitoring ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Stage a DRAFT locally (NO DB write). Writes to the pending-drafts localStorage store so the edit
  // survives refresh, shows in the JSX, but stays OUT of the DB/PDF until the user clicks Approve.
  // Drafts are keyed by record id → field name (whole-field value), mirroring localEdits' `${fn}-${idx}`.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    // Collect this section's staged drafts (localEdits keyed `${fn}-${idx}`, whole-field value).
    const toCommit = fields
      .map(f => `${f}-${idx}`)
      .filter(editKey => pendingEdits[editKey] && localEdits[editKey] !== undefined);
    try {
      for (const editKey of toCommit) {
        const fn = editKey.slice(0, -`-${idx}`.length); // whole field name (no arrayIndex — values are whole-field)
        await secureApiClient.put(`/api/edit/infliximab_drug_monitoring/${id}/edit`, { field: fn, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/infliximab_drug_monitoring/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    ...
    pdfData.forEach((r, idx) => {
      text += `Infliximab Drug Monitoring ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: ARRAY FIELD — comma items with saveCommaItem ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {arr.map((item, itemIdx) => {
          ...
          const commaKey = `${fn}-${idx}-c${itemIdx}`;
          const ciEditing = editingField === commaKey;
          const ciBadge = editedSentences[commaKey];

          return (
            <div key={itemIdx}>
              <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(itemStr); setSaveError(null); } }}>
                {ciEditing ? (
                  <div className="edit-field-container">
                    ...
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => {
                        ...
                        /* ". Test" handling: split on ". " to create new items */
                        ...
                        for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-c${itemIdx + ei}`] = 'added';
                  ...
                  <>
                    ...
                    <button className={`copy-btn ${copiedItems[commaKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(itemStr, commaKey); }}>{copiedItems[commaKey] ? 'Copied!' : 'Copy'}</button>
                  </>
                )}
              </div>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="infliximab-drug-monitoring-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Infliximab Drug Monitoring</h2></div>
        <div className="empty-state">No infliximab drug monitoring records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="infliximab-drug-monitoring-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Infliximab Drug Monitoring</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<InfliximabDrugMonitoringDocumentPDFTemplate document={pdfData} />} fileName="Infliximab_Drug_Monitoring.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search infliximab drug monitoring..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Infliximab Drug Monitoring ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'drug-levels')}
            {renderSection(record, idx, 'disease-activity')}
            {renderSection(record, idx, 'inflammatory-markers')}
            {renderSection(record, idx, 'safety-labs')}
...
export default InfliximabDrugMonitoringDocument;
