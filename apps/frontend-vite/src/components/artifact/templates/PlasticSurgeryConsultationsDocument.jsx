/**
 * PlasticSurgeryConsultationsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: plastic_surgery_consultations
 *
 * 6 Sections:
 *   1. consultation-info: consultationDate, provider, facility, procedureCategory, anesthesiaType, smokingStatus
 *   2. procedures: chiefConcern, requestedProcedure, recommendedProcedure, anatomicalArea
 *   3. clinical-findings: physicalExamFindings, surgicalHistory, medicalContraindications, bmi
 *   4. patient-info: patientExpectations, photographicDocumentation, psychologicalClearance
 *   5. risks-alternatives: risksDiscussed, alternativeTreatments, estimatedCost
 *   6. surgical-plan: surgicalTechnique, preoperativeRequirements, estimatedRecoveryTime, consentStatus, followUpPlan
 */
...
const FIELD_LABELS = {
  ...
  preoperativeRequirements: 'Preoperative Requirements',
...
const SECTION_FIELDS = {
  'consultation-info': ['consultationDate', 'provider', 'facility', 'procedureCategory', 'anesthesiaType', 'smokingStatus'],
  'procedures': ['chiefConcern', 'requestedProcedure', 'recommendedProcedure', 'anatomicalArea'],
  'clinical-findings': ['physicalExamFindings', 'surgicalHistory', 'medicalContraindications', 'bmi'],
  'patient-info': ['patientExpectations', 'photographicDocumentation', 'psychologicalClearance'],
  'risks-alternatives': ['risksDiscussed', 'alternativeTreatments', 'estimatedCost'],
  'surgical-plan': ['surgicalTechnique', 'preoperativeRequirements', 'estimatedRecoveryTime', 'consentStatus', 'followUpPlan'],
};

const DATE_FIELDS = ['consultationDate'];
const NUMBER_FIELDS = ['bmi', 'estimatedCost'];
/* Number fields where a value of 0 is a sentinel (not-estimated / not-applicable),
   NOT a meaningful zero. BMI 0 is physiologically impossible; estimatedCost 0 means
   cost was not discussed/estimated. Suppress these so "0" never leaks to UI/Copy/PDF. */
const SENTINEL_ZERO_FIELDS = ['bmi', 'estimatedCost'];
...
const DRAFT_KEY = 'plastic_surgery_consultationsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const PlasticSurgeryConsultationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.plastic_surgery_consultations) return Array.isArray(r.plastic_surgery_consultations) ? r.plastic_surgery_consultations : [r.plastic_surgery_consultations];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.plastic_surgery_consultations) return Array.isArray(dd.plastic_surgery_consultations) ? dd.plastic_surgery_consultations : [dd.plastic_surgery_consultations]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // fieldPart is "field" or "field.arrayIndex". For array drafts, localEdits holds the full
        // merged array under "field-idx"; here we restore the array element into that slot.
        const lastDot = fieldPart.lastIndexOf('.');
        const isArrayPart = lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1));
        ...
        if (isArrayPart) {
          ...
          const existing = nLocal[`${baseField}-${idx}`];
          const arr = Array.isArray(existing) ? [...existing] : base;
          arr[arrIndex] = value;
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Plastic Surgery Consultation ${idx + 1}`.toLowerCase();
  ...
  // stageDraft — common helper: stage a full-text DRAFT for a field locally + localStorage (no DB write).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // pendingEdits keys for this section+record: "field-idx" (plain/sentence) or "field.itemIdx-idx" (array element)
      const suffix = `-${idx}`;
      const toCommit = Object.keys(pendingEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length); // "field" or "field.itemIdx"
        const lastDot = fieldPart.lastIndexOf('.');
      ...
      for (const editKey of toCommit) {
        ...
        if (isArrayPart) {
          const baseField = fieldPart.slice(0, lastDot);
          const arrIndex = parseInt(fieldPart.slice(lastDot + 1), 10);
          const merged = localEdits[`${baseField}-${idx}`];
          const elemVal = Array.isArray(merged) ? merged[arrIndex] : undefined;
          const resp = await secureApiClient.put(`/api/edit/plastic_surgery_consultations/${id}/edit`, { field: baseField, value: elemVal, arrayIndex: arrIndex });
          if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        } else {
          const resp = await secureApiClient.put(`/api/edit/plastic_surgery_consultations/${id}/edit`, { field: fieldPart, value: localEdits[editKey] });
          if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        }
      }
      await secureApiClient.put(`/api/edit/plastic_surgery_consultations/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (store[id]) {
        toCommit.forEach(editKey => {
          ...
          const lastDot = fieldPart.lastIndexOf('.');
          const isArrayPart = lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1));
          delete store[id][isArrayPart ? fieldPart : fieldPart];
        });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="plastic-surgery-consultations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Plastic Surgery Consultations</h2></div>
        <div className="empty-state">No plastic surgery consultation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="plastic-surgery-consultations-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search plastic surgery consultations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
...
export default PlasticSurgeryConsultationsDocument;
