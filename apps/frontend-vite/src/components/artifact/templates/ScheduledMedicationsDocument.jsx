/**
 * ScheduledMedicationsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: scheduled_medications (displayed as "Scheduled Medications")
 *
 * 4 Sections:
...
import ScheduledMedicationsDocumentPDFTemplate from '../pdf-templates/ScheduledMedicationsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './ScheduledMedicationsDocument.css';
...
const DRAFT_KEY = 'scheduled_medicationsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const ScheduledMedicationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.scheduled_medications) return Array.isArray(r.scheduled_medications) ? r.scheduled_medications : [r.scheduled_medications];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.scheduled_medications) return Array.isArray(dd.scheduled_medications) ? dd.scheduled_medications : [dd.scheduled_medications]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Scheduled Medication ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Persist each staged field in THIS section to the DB now. localEdits keys are `${fn}-${idx}`;
      // array fields store the full array as value (backend $sets the whole field), scalars set directly.
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/scheduled_medications/${id}/edit`, { field: fn, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the record approved (audit trail)
      await secureApiClient.put(`/api/edit/scheduled_medications/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== SCHEDULED MEDICATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Scheduled Medication ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="scheduled-medications-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Scheduled Medications</h2></div>
        <div className="empty-state">No scheduled medications available</div>
      </div>
    );
  }
  ...
  return (
    <div className="scheduled-medications-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Scheduled Medications</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ScheduledMedicationsDocumentPDFTemplate document={pdfData} />} fileName={`scheduled-medications-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search scheduled medications..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Scheduled Medication ${idx + 1}`)}</h3>
            </div>
...
export default ScheduledMedicationsDocument;
