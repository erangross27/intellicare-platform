/**
 * PostOperativeReportsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: post_operative_reports
 *
 * 9 Sections:
 *   1. surgery-info: surgeryDate, procedurePerformed, surgicalFindings
 *   2. complications: complications
 *   3. pacu-info: pacuArrival, pacuDischarge
 *   4. vital-signs: vitalSignsTrend
 *   5. pain-management: painLevel, painManagement
 *   6. recovery-status: nausea, urineOutput, drainOutput, oxygenRequirement
 *   7. mobility-diet: mobilityStatus, diet, disposition
 *   8. discharge-info: dischargeInstructions, followUpPlan
 *   9. prescriptions-restrictions: prescriptions, activityRestrictions, returnPrecautions
...
const DRAFT_KEY = 'post_operative_reportsPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  'surgery-info': ['surgeryDate', 'procedurePerformed', 'surgicalFindings'],
  'complications': ['complications'],
  'pacu-info': ['pacuArrival', 'pacuDischarge'],
  'vital-signs': ['vitalSignsTrend'],
  'pain-management': ['painLevel', 'painManagement'],
  'recovery-status': ['nausea', 'urineOutput', 'drainOutput', 'oxygenRequirement'],
  'mobility-diet': ['mobilityStatus', 'diet', 'disposition'],
  'discharge-info': ['dischargeInstructions', 'followUpPlan'],
  'prescriptions-restrictions': ['prescriptions', 'activityRestrictions', 'returnPrecautions'],
  'recommendations': ['recommendations'],
};
...
/* ═══════ COMPONENT ═══════ */
const PostOperativeReportsDocument = ({ document: docProp }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        // Mark as edited so the Pending Approve button + modified badge show after refresh.
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Post-Operative Report ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ DRAFT STAGING ═══════ */
  // Stage an edit as a DRAFT locally + write it to the pending-drafts localStorage store (survives refresh).
  // NOT written to MongoDB and NOT shown in the PDF until the user clicks Approve (handleApproveSection commits).
  // fieldPart = "field" for scalar/sentence fields, or "field.arrayIndex" for array elements.
  const stageDraft = useCallback((record, idx, sid, fieldPart, value, trackKeys) => {
    const id = safeId(record); if (!id) return;
    const editKey = `${fieldPart}-${idx}`;
    setLocalEdits(prev => ({ ...prev, [editKey]: value }));
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
    if (trackKeys && trackKeys.fields) setEditedFields(prev => ({ ...prev, ...trackKeys.fields }));
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Save = stage a DRAFT locally (NO DB write). Approve commits it. Synchronous now (no setSaving).
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    const id = safeId(record); if (!id) return;
    const saveVal = valueOverride !== undefined ? valueOverride : editValue;
    const trackKey = editTrackingKey || `${fn}-${idx}`;
    stageDraft(record, idx, sid, fn, saveVal, { fields: { [trackKey]: 'edited' } });
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageDraft(record, idx, sid, fn, fullText, { sentences: { [`${fn}-${idx}-s${sentenceIdx}`]: 'edited' } });
      return;
    }
    ...
    // Stage as a DRAFT (no DB write). Approve commits it.
    stageDraft(record, idx, sid, fn, fullText, { sentences: sentMarks });
  }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Pending edit keys for THIS record belonging to THIS section: "field-idx" or "field.arrayIndex-idx".
    ...
    try {
      for (const editKey of toCommit) {
        ...
        // arrayIndex ONLY when the segment after the LAST dot is purely numeric (mirrors stageDraft's fieldPart).
        ...
        await secureApiClient.put(`/api/edit/post_operative_reports/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/post_operative_reports/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== POST-OPERATIVE REPORTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Post-Operative Report ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="post-operative-reports-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Post-Operative Reports</h2></div>
        <div className="empty-state">No post-operative reports available</div>
      </div>
    );
  }
  ...
  return (
    <div className="post-operative-reports-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search procedures, complications, prescriptions..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'pacu-info')}
            {renderSection(record, idx, 'vital-signs')}
            {renderSection(record, idx, 'pain-management')}
            {renderSection(record, idx, 'recovery-status')}
            {renderSection(record, idx, 'mobility-diet')}
            {renderSection(record, idx, 'discharge-info')}
            {renderSection(record, idx, 'prescriptions-restrictions')}
...
export default PostOperativeReportsDocument;
