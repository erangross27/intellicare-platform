/**
 * Edit Route — pharmacogenomic_testing
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'pharmacogenomic-testing-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'pharmacogenomicTestingId',
  'cytochromeP450Genotype',
  'cyp2d6MetabolizerStatus',
  'cyp2c19MetabolizerStatus',
  'cyp2c9Genotype',
  'vkorc1Genotype',
  'calculatedWarfarinSensitivity',
  'recommendedWarfarinDoseRange',
  'factorVLeidenStatus',
  'prothrombinG20210AStatus',
  'tpmtActivityLevel',
  'tpmtGenotype',
  'nudt15Genotype',
  'hlab5701Status',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('pharmacogenomic_testing', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[pharmacogenomic_testing] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('pharmacogenomic_testing', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[pharmacogenomic_testing] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
