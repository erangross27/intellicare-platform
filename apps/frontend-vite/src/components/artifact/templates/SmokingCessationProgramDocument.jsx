 ...
 *
 * 6 Sections:
 *   1. program-details: date, type, provider, facility
 *   2. clinical-findings: findings
 *   3. assessment-plan: assessment, plan
 *   4. recommendations: recommendations
 *   5. results-status: results, status
 *   6. notes: notes
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import SmokingCessationProgramPDFTemplate from '../pdf-templates/SmokingCessationProgramPDFTemplate';
...
const SECTION_FIELDS = {
  'program-details': ['date', 'type', 'provider', 'facility'],
...
const STRING_FIELDS = ['type', 'provider', 'facility', 'findings', 'assessment', 'plan', 'results', 'status', 'notes'];
...
const SmokingCessationProgramDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.smoking_cessation_program) return Array.isArray(r.smoking_cessation_program) ? r.smoking_cessation_program : [r.smoking_cessation_program];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.smoking_cessation_program) return Array.isArray(dd.smoking_cessation_program) ? dd.smoking_cessation_program : [dd.smoking_cessation_program]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayElem) {
          const fieldName = fieldPart.slice(0, lastDot);
          ...
          nFields[`${fieldName}.${arrIndex}-${idx}`] = 'edited';
        } else {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Smoking Cessation Program ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Persist each staged draft fieldPart for this record that belongs to this section.
      const store = readDrafts();
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/smoking_cessation_program/${id}/edit`, payload);
        if (!resp || resp.success === false) throw new Error((resp && resp.error) || 'save failed');
        committedLocalKeys.push(`${fieldName}-${idx}`);
        delete recDrafts[fieldPart];
      }
      // Flag the record/section approved (audit trail)
      await secureApiClient.put(`/api/edit/smoking_cessation_program/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        text += `${label}\n${items.map((item, i) => `${i + 1}. ${typeof item === 'string' ? item : (item.recommendation || JSON.stringify(item))}`).join('\n')}\n\n`;
  ...
  const copyAllText = useCallback(async () => {
    ...
    pdfData.forEach((r, idx) => {
      text += `Smoking Cessation Program ${idx + 1}\n${'='.repeat(40)}\n\n`;
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          const isEditing = editingField === editKey;
          const isModified = editedFields[editKey];
          const itemStr = typeof item === 'string' ? item : (item.recommendation || JSON.stringify(item));
  ...
  if (!records || records.length === 0) {
    return (
      <div className="smoking-cessation-program-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Smoking Cessation Program</h2></div>
        <div className="empty-state">No smoking cessation program records available</div>
      </div>
    );
  }
    ...
    <div className="smoking-cessation-program-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<SmokingCessationProgramPDFTemplate document={pdfData} />} fileName={`smoking-cessation-program-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search smoking cessation programs..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(record.type || `Smoking Cessation Program ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'program-details')}
            ...
            {renderSection(record, idx, 'results-status')}
