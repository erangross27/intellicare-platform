/**
 * Additional Data Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/additional_data/:id/edit    — Edit a single field
 * PUT /api/edit/additional_data/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'additional-data-edit-service',
    userId: req.user?.id,
    operation,
...
// Dynamic field validation — block only system fields
const SYSTEM_FIELDS = ['_id', 'createdAt', 'updatedAt', 'patientId', 'practiceId', 'approvedSections', 'approvalTimestamp', 'approvedBy', 'updatedBy'];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    if (SYSTEM_FIELDS.includes(field) || field.startsWith('_')) {
      return res.status(400).json({ success: false, error: `Field "${field}" is not editable` });
    }

    const objectId = toObjectId(id);
    if (!objectId) {
      return res.status(400).json({ success: false, error: 'Invalid additional data ID' });
    }

    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');

    await sda.update('additional_data', { _id: objectId }, {
      $set: {
        [field]: value,
        updatedAt: new Date(),
    ...
    console.error('[AdditionalData] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    console.error('[AdditionalData] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
