...
const SECTIONS = [
  { id: "record-info", title: "Record Information", fields: ["date", "type", "provider", "facility", "riskLevel"] },
  { id: "ideation", title: "Ideation", fields: ["ideation"] },
  { id: "plan-details", title: "Plan Details", fields: ["plan"] },
  { id: "intent", title: "Intent", fields: ["intent"] },
  { id: "history", title: "History", fields: ["previousAttempts", "psychiatricHospitalizations"] },
  { id: "risk-factors", title: "Risk Factors", fields: ["riskFactors"] },
  { id: "protective-factors", title: "Protective Factors", fields: ["protectiveFactors"] },
  { id: "interventions", title: "Interventions", fields: ["interventions"] },
  { id: "columbia-scale", title: "Columbia Scale (C-SSRS)", fields: ["columbiaScale"] },
...
const LABELS = {
  ...
  columbiaScale: "Columbia Scale (C-SSRS)",
  findings: "Findings",
...
const SuicideRiskAssessmentDocument = ({
  document: docProp,
  data,
  templateData,
}) => {
  const records = useMemo(() => {
    ...
    return (Array.isArray(source) ? source : [source])
      .flatMap((record) => {
        if (record?.suicide_risk_assessment)
          return Array.isArray(record.suicide_risk_assessment)
            ? record.suicide_risk_assessment
            : [record.suicide_risk_assessment];
        if (record?.documentData)
          return Array.isArray(record.documentData)
            ? record.documentData
            : record.documentData?.suicide_risk_assessment
              ? Array.isArray(record.documentData.suicide_risk_assessment)
                ? record.documentData.suicide_risk_assessment
                : [record.documentData.suicide_risk_assessment]
              : [record.documentData];
        if (record?.document)
      ...
      try {
        for (const key of keys) {
          ...
          const response = await secureApiClient.put(
            `/api/edit/suicide_risk_assessment/${id}/edit`,
        ...
        await secureApiClient.put(
          `/api/edit/suicide_risk_assessment/${id}/approve`,
  ...
  const renderLeaf = (
    ...
    const numeric =
      !bool && !date && !enumOptions ? numericShape(value) : null;
    const display = date
      ? formatDate(value)
      : bool
        ? value
  ...
  return (
    <div className="suicide-risk-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h1 className="document-title">Suicide Risk Assessment</h1>
        <div className="header-actions">
          ...
          <PDFDownloadLink
            document={
              <SuicideRiskAssessmentDocumentPDFTemplate document={pdfData} />
            }
            fileName="Suicide_Risk_Assessment.pdf"
            className="copy-btn"
          >
      ...
      <div className="search-container">
        <input
          ...
          placeholder="Search suicide risk assessments..."
          value={searchTerm}
          ...
          {filtered.map(({ record, idx }) => (
            <div className="record-card" key={idx}>
              <div className="record-header">
                <h3 className="record-name">{`Suicide Risk Assessment ${idx + 1}`}</h3>
              </div>
