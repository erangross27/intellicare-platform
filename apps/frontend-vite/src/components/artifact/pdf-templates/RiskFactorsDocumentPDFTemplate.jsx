...
const SECTIONS = [
  { id: 'riskFactorDetails', title: 'Risk Factor Details', fields: ['factor', 'category', 'severity', 'status', 'date', 'type', 'provider', 'facility'] },
...
export default function RiskFactorsDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Risk Factors</Text></View>
    {!records.length && <Text style={styles.noDataText}>No risk factors data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Risk Factors {recordIndex + 1}</Text></View>
