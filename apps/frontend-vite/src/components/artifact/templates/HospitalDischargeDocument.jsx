 ...
 *   3. discharge-plan: plan (string), recommendations (string), notes (string)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HospitalDischargeDocumentPDFTemplate from '../pdf-templates/HospitalDischargeDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './HospitalDischargeDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'hospital_dischargePendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const HospitalDischargeDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.hospital_discharge) return Array.isArray(dd.hospital_discharge) ? dd.hospital_discharge : [dd.hospital_discharge]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // fieldPart is "field" or "field.arrayIndex"; localEdits key is "<fieldPart>-<idx>"
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Hospital Discharge ${idx + 1}`.toLowerCase();
  ...
  // Stage a full-field DRAFT (no DB write). localStorage keeps it across refresh; Approve commits it.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // staged localEdits keys for this record whose base field belongs to this section
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/hospital_discharge/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/hospital_discharge/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HOSPITAL DISCHARGE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Hospital Discharge ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="hospital-discharge-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Hospital Discharge</h2></div>
        <div className="empty-state">No hospital discharge records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="hospital-discharge-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Hospital Discharge</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HospitalDischargeDocumentPDFTemplate document={pdfData} />} fileName={`hospital-discharge-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search hospital discharge..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Hospital Discharge ${idx + 1}`)}</h3>
            </div>
...
export default HospitalDischargeDocument;
