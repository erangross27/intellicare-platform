...
/**
 * IbdBiomarkersDocumentPDFTemplate - box-free canonical (LETTER)
 * Config-driven from the JSX SECTION_ORDER/SECTION_TITLES/FIELD_LABELS/SECTION_FIELDS.
 * Renders EVERY populated field the JSX renders for JSX/PDF field parity.
 * No boxes: underline rules only (documentTitle 2 / recordTitle+sectionTitle 1 / fieldLabel 0.5).
 * Nested `results` object + `recommendations` array-of-objects render via a generic recursive
 * renderer; inline "Key: value" is allowed in the PDF only. Record date uses record.date only
 * (NEVER createdAt/updatedAt). Unit-bearing lab values stay strings; safeString normalizes the
 * Greek mu (U+03BC) to the micro sign (U+00B5) since Helvetica lacks the Greek glyph.
 * safeString uses ONLY \uXXXX escapes (literal smart-quotes/invisible chars break the regex).
 */
...
const SECTION_ORDER = ['overview', 'biomarker-panel', 'findings-plan', 'results-section', 'recommendations-section'];
...
/* HELPERS (mirror the JSX) — safeString uses ONLY \uXXXX escapes for special chars.
   μ (Greek small mu) -> µ (micro sign) because built-in Helvetica lacks the Greek glyph. */
...
/* arrayRows: array of {recommendation, date} (or scalars) — decompose each object recursively */
...
const IbdBiomarkersDocumentPDFTemplate = ({ document: docProp, data = docProp }) => {
  let records = [];
  if (Array.isArray(data)) records = data;
  else if (data && typeof data === 'object') {
    if (data.ibd_biomarkers) records = Array.isArray(data.ibd_biomarkers) ? data.ibd_biomarkers : [data.ibd_biomarkers];
    else if (data.documentData) { const dd = data.documentData; if (Array.isArray(dd)) records = dd; else if (dd?.ibd_biomarkers) records = Array.isArray(dd.ibd_biomarkers) ? dd.ibd_biomarkers : [dd.ibd_biomarkers]; else if (dd && typeof dd === 'object') records = [dd]; }
    else records = [data];
  }
    ...
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <Text style={styles.documentTitle}>IBD Biomarkers</Text>
          <Text style={styles.noData}>No IBD biomarker records available.</Text>
        </Page>
      </Document>
    );
  }
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, rIdx) => (
          <View key={rIdx}>
            <Text style={styles.recordTitle} break={rIdx > 0}>{safeString(`IBD Biomarker Panel ${rIdx + 1}`)}</Text>
...
export default IbdBiomarkersDocumentPDFTemplate;
