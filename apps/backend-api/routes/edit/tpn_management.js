...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'tpn-management-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'tpnIndicationType',
  'estimatedTpnDuration',
  'totalDailyCalories',
  'proteinProvision',
  'dextroseConcentration',
  'lipidEmulsionType',
  'lipidDose',
  'centralVenousAccessType',
  'catheterTipPosition',
  'infusionRate',
  'cyclicTpnSchedule',
  'sodiumProvision',
  'potassiumProvision',
  'phosphorusProvision',
  'calciumGluconateProvision',
  'magnesiumProvision',
  'traceElementsFormulation',
  'multiVitaminAdditive',
  'refeedingSyndromeRisk',
  'baselinePrealbuminLevel',
  'hepaticFunctionPanel',
  'triglycerideMonitoring',
  'bloodGlucoseManagement',
  'tpnAssociatedCholestasis',
  'catheterRelatedBloodstreamInfection',
  'enteralFeedingTolerance',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('tpn_management', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[tpn_management] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('tpn_management', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[tpn_management] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
