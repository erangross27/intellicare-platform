/**
 * ExerciseProgramDocument.jsx
 * March 2026 — inline editing, blue glow theme. One-pass polish July 2026 (Ananya Krishnamurthy, T2DM).
 * Collection: exercise_program
 *
 * 8 Sections:
 *   1. record-info: date (BlueDatePicker), provider (plain), facility (plain), status (ENUM BlueSelect)
 *   2. exercise-details: type, frequency, duration, intensity, purpose (sentence — [.;] + comma split)
 *   3. findings: findings (sentence, single-name)
 *   4. assessment: assessment (sentence, single-name)
 *   5. plan: plan (sentence, single-name)
 *   6. recommendations: recommendations (array of objects [{recommendation, date}]) — grouped by date
 *   7. results: results (OBJECT — recursive key/value, editable leaves: BlueSelect / num-stepper / textarea)
 *   8. notes: notes (sentence, single-name)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ExerciseProgramDocumentPDFTemplate from '../pdf-templates/ExerciseProgramDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './ExerciseProgramDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [field]: value } }  (field = plain field name; localEdits stores the
   FULL field value — whole string / whole array / whole object — keyed by `${field}-${idx}`). */
const DRAFT_KEY = 'exercise_programPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  ...
  'exercise-details': ['type', 'frequency', 'duration', 'intensity', 'purpose'],
  'findings': ['findings'],
  'assessment': ['assessment'],
  'plan': ['plan'],
  'recommendations': ['recommendations'],
...
const ARRAY_FIELDS = ['recommendations'];      // array of objects, grouped by date
const OBJECT_FIELDS = ['results'];
/* Narrative / list fields → sentence-split ([.;]) + aggressive comma-split (labeled AND unlabeled) +
   numbered per-item rows in JSX. Identity/prose fields (provider, facility) stay PLAIN (renderEditableField). */
const SENTENCE_FIELDS = ['type', 'frequency', 'duration', 'intensity', 'purpose', 'findings', 'assessment', 'plan', 'notes'];

/* Copy dividers: EQ (====) under section/record titles, DASH (----) under EVERY field label + sub-category. */
const COPY_LINE_EQ = '='.repeat(40);
const COPY_LINE_DASH = '-'.repeat(40);

/* status → record-status catalog (memory 6a4908b2). Stored 'active' → enumCanonical 'Active'. */
...
const enumOptionsWith = (cur) => {
  const c = enumCanonical(cur);
  return (!c || STATUS_OPTIONS.some(o => o.toLowerCase() === c.toLowerCase())) ? STATUS_OPTIONS : [c, ...STATUS_OPTIONS];
};

/* stepFor: decimal-aware step for the −/+ number stepper (14 → 1, 0.5 → 0.1). */
const stepFor = (v) => { const d = String(v ?? '').split('.')[1]; return d ? '0.' + '0'.repeat(Math.max(0, d.length - 1)) + '1' : '1'; };

/* ═══════ OBJECT-FIELD HELPERS (results) ═══════ */
const KEY_OVERRIDES = {};
...
/* parseLabel: detect "Label: value" patterns (e.g. "Phase 3 target: 8,000-10,000 steps/day, 150+ minutes") */
...
/* splitByComma: parenthesis-aware comma split. Keeps a comma JOINED when it is a decimal grouping
   ("8,000"), a continuation conjunction ("…, and/or/then …"), or follows a trailing and/or. */
...
/* ═══════ COMPONENT ═══════ */
const ExerciseProgramDocument = ({ document: docProp }) => {
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Drafts are keyed by record _id (handling _id.$oid); we map each to its current render index. */
  useEffect(() => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Exercise Program ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Stage a DRAFT: store the FULL field value locally (localEdits keyed `${fn}-${idx}`) + mark it
  // pending + persist to the localStorage draft store (survives refresh). NO DB write — Approve commits.
  ...
  /* ═══════ SAVE ARRAY ITEM FIELD (recommendations) ═══════ */
  // Stage a DRAFT only: rebuild the whole array locally + persist as a draft. NO DB write (Approve commits).
  const handleSaveArrayItemField = useCallback((record, fn, idx, arrayIndex, subField, newValue) => {
    ...
    stageDraft(record, fn, idx, fn, updatedArr); // sid === field === 'recommendations'
    const trackKey = `${fn}.${arrayIndex}.${subField}-${idx}`;
    setEditedFields(prev => ({ ...prev, [trackKey]: 'edited' }));
    setEditingField(null); setEditValue('');
  }, [getFieldValue, safeId, stageDraft]);

  /* ═══════ SAVE OBJECT LEAF (results.x.y dot-path) — value stays STRING/scalar ═══════ */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    // Build the FULL updated object, then stage as a DRAFT (no DB write — Approve commits the whole object).
  ...
  // then flag the record approved, clear pending so committed values now flow into pdfData/PDF, and
  // drop the section's drafts from localStorage.
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // localEdits keys are `${field}-${idx}` and the stored value is the FULL field value
      // (whole string / array / object), so we PUT field + value with no arrayIndex/dotted parse needed.
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/exercise_program/${id}/edit`, { field, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/exercise_program/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[ExerciseProgram] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        // recommendations: array of objects grouped by date; numbering restarts per date group.
        const arr = Array.isArray(val) ? val : [];
        if (sl) text += `${label}\n${COPY_LINE_DASH}\n`;
        const grouped = {};
        arr.forEach(item => { if (!item || typeof item !== 'object') return; const dk = item.date || 'No Date'; (grouped[dk] = grouped[dk] || []).push(item); });
        Object.entries(grouped).forEach(([dk, recs]) => {
          const dd = dk !== 'No Date' ? formatDate(dk) : 'No Date';
          text += `${dd}\n${COPY_LINE_DASH}\n`;
          let na = 1;
          recs.forEach(rec => { text += `${na++}. ${rec.recommendation || ''}\n`; });
        });
        text += '\n';
      } else if (OBJECT_FIELDS.includes(f)) {
        if (sl) text += `${label}\n${COPY_LINE_DASH}\n`;
        // Stacked key / value (NEVER "key: value" side-by-side) with a DASH under each sub-label.
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== EXERCISE PROGRAM ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Exercise Program ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
          ...
          {isEditing ? (
            <div className="edit-field-container">
              ...
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date'); return; } stageDraft(record, fn, idx, sid, editValue + 'T00:00:00.000Z'); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
  ...
  /* ═══════ RENDER: ENUM FIELD (status → BlueSelect) ═══════ */
    ...
    return (
      <div key={fn} className={sl ? 'rec-mini-card' : ''}>
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayVal); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              <BlueSelect value={editValue} options={enumOptionsWith(val)} onChange={(v) => setEditValue(v)} />
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); stageDraft(record, fn, idx, sid, editValue); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
  ...
  /* ═══════ RENDER: PLAIN EDITABLE FIELD (provider, facility — kept whole) ═══════ */
  ...
  const renderRecommendationsField = (record, idx, sid) => {
    ...
    val.forEach((item, aIdx) => {
      ...
      grouped[dateKey].push({ ...item, _origIdx: aIdx });
    });

    return Object.entries(grouped).map(([dateKey, items], gIdx) => {
      const dateDisplay = dateKey !== 'No Date' ? formatDate(dateKey) : 'No Date';
      const groupMatches = phraseMatch || (searchTerm.trim() && (
        dateDisplay.toLowerCase().includes(searchTerm.toLowerCase().trim()) ||
        items.some(item => (item.recommendation || '').toLowerCase().includes(searchTerm.toLowerCase().trim()))
      ));
      if (!groupMatches && searchTerm.trim()) return null;

      return (
        <div key={gIdx} className="rec-mini-card">
          <div className="nested-subtitle">{highlightText(dateDisplay)}</div>
          {items.map((item) => {
            const aIdx = item._origIdx;
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; boolean → BlueSelect, number/ratio → −/+ stepper, text → textarea) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="exercise-program-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Exercise Program</h2></div>
        <div className="empty-state">No exercise program records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="exercise-program-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Exercise Program</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ExerciseProgramDocumentPDFTemplate document={pdfData} />} fileName="Exercise_Program.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search exercise program records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Exercise Program ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'record-info')}
            {renderSection(record, idx, 'exercise-details')}
...
export default ExerciseProgramDocument;
