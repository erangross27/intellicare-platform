/**
 * MayoScoreDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: mayo_score
 *
 * 5 Sections:
 *   1. session-info: endoscopyDate (date picker), endoscopist (string), endoscopicProcedureType (string), diseaseExtent (string)
 *   2. score-components: stoolFrequencyScore, rectalBleedingScore, endoscopicFindingsScore,
 *                        physicianGlobalAssessmentScore, totalMayoScore
 *   3. clinical-status: diseaseActivity (string), endoscopicRemission (bool), clinicalRemission (bool),
 *                       mucosalHealing (bool), treatmentResponse (string), histologicActivity (string)
 *   4. symptom-details: bowelMovementsPerDay, baselineBowelFrequency, urgency (string), abdominalPain (string)
 *   5. labs-medications: cReactiveProtein, fecalCalprotectin, previousMayoScore, currentMedications (array)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MayoScoreDocumentPDFTemplate from '../pdf-templates/MayoScoreDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './MayoScoreDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'mayo_scorePendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['endoscopist', 'endoscopicProcedureType', 'diseaseExtent', 'diseaseActivity', 'treatmentResponse', 'histologicActivity', 'urgency', 'abdominalPain'];
...
/* ======= COMPONENT ======= */
const MayoScoreDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Mayo Score ${idx + 1}`.toLowerCase();
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    const editKey = `${fn}-${idx}`;
    const stageDraft = (fullText, applyMarks) => {
      setSaveError(null);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Pending localEdits whose field belongs to this section (key shape: "field-idx")
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/mayo_score/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/mayo_score/${id}/approve`, { sectionId: sid, approved: true });
      ...
      <div key={fn}>
        <div className="rec-mini-card">
          ...
          {arr.map((item, aIdx) => {
            ...
            const commaItems = parsed.isLabeled ? splitByComma(parsed.value) : [itemStr];

            if (parsed.isLabeled && commaItems.length >= 2) {
              return (
                        ...
                        <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                          {ciEditing ? (
                            <div className="edit-field-container">
                              ...
                              {saveError && <div className="save-error">{saveError}</div>}
                              <div className="edit-actions">
                                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const currentArr = Array.isArray(getFieldValue(record, fn, idx)) ? [...getFieldValue(record, fn, idx)] : []; const currentItem = currentArr[aIdx] || ''; const p2 = parseLabel(typeof currentItem === 'string' ? currentItem : ''); if (!p2.isLabeled) return; const items2 = splitByComma(p2.value); items2[ciIdx] = editValue.trim(); const rebuilt = `${p2.label}: ${items2.join(', ')}`; currentArr[aIdx] = rebuilt; setSaveError(null); const eKey = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [eKey]: currentArr })); setPendingEdits(prev => ({ ...prev, [eKey]: true })); setEditedFields(prev => ({ ...prev, [commaKey]: 'edited' })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const store = readDrafts(); if (!store[id2]) store[id2] = {}; store[id2][fn] = currentArr; writeDrafts(store); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                              </div>
                            </div>
                          ) : (
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const id2 = safeId(record); if (!id2) return; const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); setSaveError(null); const eKey = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [eKey]: fullText })); setPendingEdits(prev => ({ ...prev, [eKey]: true })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const store = readDrafts(); if (!store[id2]) store[id2] = {}; store[id2][fn] = fullText; writeDrafts(store); setEditingField(null); setEditValue(''); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="mayo-score-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Mayo Score (Ulcerative Colitis)</h2></div>
        <div className="empty-state">No mayo score records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="mayo-score-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Mayo Score (Ulcerative Colitis)</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MayoScoreDocumentPDFTemplate document={pdfData} />} fileName="Mayo_Score.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search mayo score records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Mayo Score ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'score-components')}
            {renderSection(record, idx, 'clinical-status')}
            {renderSection(record, idx, 'symptom-details')}
            {renderSection(record, idx, 'labs-medications')}
          </div>
        ))}
      </div>
    </div>
...
export default MayoScoreDocument;
