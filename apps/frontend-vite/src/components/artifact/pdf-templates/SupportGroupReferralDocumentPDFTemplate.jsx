/**
 * SupportGroupReferralDocumentPDFTemplate.jsx
 * June 2026 — Helvetica — A4 — BLACK & WHITE only (#000000 titles/borders/values, NO blue).
 * Collection: support_group_referral.
 *
 ...
 * BOOLEAN field `referred` -> Yes/No line.
...
const FIELD_LABELS = {
  date: 'Date', referred: 'Referred', groupType: 'Group Type', location: 'Location', frequency: 'Frequency',
  contact: 'Contact', type: 'Type', provider: 'Provider', facility: 'Facility', findings: 'Findings',
  assessment: 'Assessment', plan: 'Plan', notes: 'Notes', status: 'Status', results: 'Results', recommendations: 'Recommendations',
};
const SECTION_FIELDS = {
  'referral': ['referred', 'groupType', 'location', 'frequency', 'contact'],
  'clinical': ['findings', 'assessment', 'plan'],
  'results-section': ['results'],
  'recommendations-section': ['recommendations'],
  'notes-status': ['type', 'notes', 'status'],
};
const SECTION_ORDER = ['referral', 'clinical', 'results-section', 'recommendations-section', 'notes-status'];
const DATE_FIELDS = ['date'];
const BOOLEAN_FIELDS = ['referred'];
const STRING_FIELDS = ['groupType', 'location', 'frequency', 'contact', 'type', 'provider', 'facility', 'findings', 'assessment', 'plan', 'notes', 'status'];
...
const SupportGroupReferralDocumentPDFTemplate = ({ document: data }) => {
  let records = [];
  if (Array.isArray(data)) {
    if (data.length === 1 && data[0]?.support_group_referral) records = Array.isArray(data[0].support_group_referral) ? data[0].support_group_referral : [data[0].support_group_referral];
    else records = data;
  } else if (data?.support_group_referral) records = Array.isArray(data.support_group_referral) ? data.support_group_referral : [data.support_group_referral];
  else if (data?.documentData) { const dd = data.documentData; if (Array.isArray(dd)) records = dd; else if (dd?.support_group_referral) records = Array.isArray(dd.support_group_referral) ? dd.support_group_referral : [dd.support_group_referral]; else if (dd && typeof dd === 'object') records = [dd]; }
  else if (data && typeof data === 'object') records = [data];
  records = (records || []).filter(r => r && typeof r === 'object');

  if (records.length === 0) {
    return (<Document><Page size="A4" style={styles.page}><View style={styles.documentHeader}><Text style={styles.title}>Support Group Referral</Text></View><Text style={styles.emptyState}>No records available</Text></Page></Document>);
  }

  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.title}>Support Group Referral</Text></View>
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer}>
            <View style={styles.recordHeader} wrap={false}>
              <Text style={styles.recordTitle}>{`Support Group Referral ${String(record._recordNumber || idx + 1)}`}</Text>
...
export default SupportGroupReferralDocumentPDFTemplate;
