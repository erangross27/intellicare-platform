/**
 * FluidOutputDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: fluid_output
 *
 * 8 Sections:
 *   1. session-info: date (date picker), provider (string), facility (string), status (string)
 *   2. fluid-details: urine (string), drains (string), emesis (string), stool (string), other (string), insensibleLoss (string), total (string)
 *   3. assessment-section: assessment (sentence)
 *   4. plan-section: plan (sentence)
 *   5. findings-section: findings (string)
 *   6. results-section: results (OBJECT, recursive)
 *   7. recommendations-section: recommendations (ARRAY of {recommendation, date}, date-grouped)
 *   8. notes-section: notes (string)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import FluidOutputDocumentPDFTemplate from '../pdf-templates/FluidOutputDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './FluidOutputDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldName]: value } }  (value is the full effective field value) */
const DRAFT_KEY = 'fluid_outputPendingEdits';
const readDrafts = () => {
...
/* No legacy numeric fields */
const NUMBER_FIELDS = [];

/* Numeric-amount fields (stored as strings) — edited via number spinner, preserving any unit */
const NUMBER_LIKE_FIELDS = ['urine', 'drains', 'emesis', 'stool', 'other', 'insensibleLoss', 'total'];

const STRING_FIELDS = ['provider', 'facility', 'status', 'urine', 'drains', 'emesis', 'stool', 'other', 'insensibleLoss', 'total', 'assessment', 'plan', 'findings', 'notes'];
const DATE_FIELDS = ['date'];
/* DISPLAY/EDIT recursive OBJECT field */
const OBJECT_FIELDS = ['results'];
/* Array-of-objects {recommendation, date} (date-grouped, editable) */
const OBJECT_ARRAY_FIELDS = ['recommendations'];
/* Fixed-choice record status → BlueSelect */
...
const KEY_OVERRIDES = {
  iv: 'IV', ivc: 'IVC', ef: 'EF', bp: 'BP', hr: 'HR', uo: 'UO', io: 'I/O',
};
...
/* ═══════ DEEP VALUE HELPERS (object/array fields) ═══════ */
const isEmptyDeep = (v) => {
...
/* splitNumberUnit: "924" -> {num:'924',sep:'',unit:''}; "0.55 U/hr" -> {num:'0.55',sep:' ',unit:'U/hr'};
   "1,566 mL" -> {num:'1566',sep:' ',unit:'mL'} (thousands stripped); null if not leading-numeric / a ratio */
...
/* ═══════ COMPONENT ═══════ */
const FluidOutputDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.fluid_output) return Array.isArray(r.fluid_output) ? r.fluid_output : [r.fluid_output];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.fluid_output) return Array.isArray(dd.fluid_output) ? dd.fluid_output : [dd.fluid_output]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Fluid Output ${idx + 1}`.toLowerCase();
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    // Stage a DRAFT (no DB write). Build the merged object, store under localEdits + the draft store.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // localEdits keys are `${fn}-${idx}`; value is the full effective field value (string/object/array).
      // Persist each staged field for THIS section + record now.
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/fluid_output/${id}/edit`, { field: fn, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/fluid_output/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this record's committed drafts (only the fields in this section) from localStorage
      const store = readDrafts();
    ...
    } catch (err) { console.error('[FluidOutput] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const objectCopyLines = useCallback((label, value) => {
    ...
    if (Array.isArray(value)) {
      if (label) { out.push(label); out.push(COPY_LINE_DASH); }
      let n = 1;
      value.filter(v => !isEmptyDeep(v)).forEach(v => { if (isScalar(v)) out.push(`${n++}. ${fmtScalar(v)}`); else objectCopyLines('', v).forEach(l => out.push(l)); });
      return out;
    }
    if (label) { out.push(label); out.push(COPY_LINE_DASH); }
  ...
  const copyAllText = useCallback(async () => {
    let text = `Fluid Output\n${COPY_LINE_EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Fluid Output ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD (numeric amount stored as string, edited via spinner) ═══════ */
  const renderNumberField = (record, fn, idx, sid, title) => {
    const val = getFieldValue(record, fn, idx); if (!hasVal(val)) return null;
    const strVal = fmtVal(val);
    const nu = splitNumberUnit(strVal);
    if (!nu) return renderStringField(record, fn, idx, sid, title); /* non-numeric → fall back to string editor */
    const label = FIELD_LABELS[fn] || fn;
    const showSubLabel = label.trim().toLowerCase() !== (SECTION_TITLES[sid] || '').trim().toLowerCase();
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
    const editKey = `${fn}-${idx}`;
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(nu.num); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              ...
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const n = parseFloat(editValue); if (isNaN(n)) { setSaveError('Please enter a valid number'); return; } const newVal = `${n}${nu.unit ? nu.sep || ' ' : ''}${nu.unit || ''}`; handleSaveField(record, fn, idx, sid, null, newVal); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable scalar; number/ratio/bool aware) ═══════ */
              ...
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => {
                  ...
                  if (isBool) {
                    ...
                    const n = parseFloat(editValue);
                    if (isNaN(n)) { setSaveError('Please enter a valid number'); return; }
                    newVal = nu.unit ? `${n} ${nu.unit}` : String(n);
                  } else {
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (NUMBER_LIKE_FIELDS.includes(f)) return renderNumberField(record, f, idx, sid, title);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="fluid-output-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Fluid Output</h2></div>
        <div className="empty-state">No fluid output records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="fluid-output-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Fluid Output</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FluidOutputDocumentPDFTemplate document={pdfData} />} fileName="Fluid_Output.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search fluid output..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">Fluid Output {idx + 1}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'fluid-details')}
...
export default FluidOutputDocument;
