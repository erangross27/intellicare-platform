/**
 * EmergencyDispositionDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: emergency_disposition
 *
 * 5 Sections:
 *   1. disposition: dispositionType, acuityLevelAtDisposition, admissionDiagnosis, codeStatus, dispositionDelayReason
 *   2. admission-transfer: admittingService, admittingPhysician, inpatientUnitAssigned, transferFacility, transferReason, transportMode
 *   3. timing: dispositionTimestamp, bedRequestTime, bedAssignedTime, departureTime, lengthOfStay
 *   4. discharge-followup: dischargeInstructions, followUpProvider, followUpTimeframe, prescriptionsMedications, returnPrecautions, workRestrictionsProvided
 *   5. special-flags: leftWithoutBeingSeen, leftAgainstMedicalAdvice, socialWorkConsultCompleted
 *
 * Header: date (record-header date)
 * 26 fields total → 100% coverage
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import EmergencyDispositionDocumentPDFTemplate from '../pdf-templates/EmergencyDispositionDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueTimePicker from '../components/BlueTimePicker';
import BlueSelect from '../components/BlueSelect';
import './EmergencyDispositionDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'emergency_dispositionPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'timing': 'Timing',
  'discharge-followup': 'Discharge & Follow-Up',
...
const FIELD_LABELS = {
  ...
  acuityLevelAtDisposition: 'Acuity Level at Disposition',
  admissionDiagnosis: 'Admission Diagnosis',
  codeStatus: 'Code Status',
  dispositionDelayReason: 'Disposition Delay Reason',
  admittingService: 'Admitting Service',
  admittingPhysician: 'Admitting Physician',
  inpatientUnitAssigned: 'Inpatient Unit Assigned',
  transferFacility: 'Transfer Facility',
  transferReason: 'Transfer Reason',
  transportMode: 'Transport Mode',
  dispositionTimestamp: 'Disposition Timestamp',
  bedRequestTime: 'Bed Request Time',
  bedAssignedTime: 'Bed Assigned Time',
  departureTime: 'Departure Time',
  lengthOfStay: 'Length of Stay (minutes)',
  dischargeInstructions: 'Discharge Instructions',
  followUpProvider: 'Follow-Up Provider',
  followUpTimeframe: 'Follow-Up Timeframe',
  prescriptionsMedications: 'Prescriptions / Medications',
  returnPrecautions: 'Return Precautions',
  workRestrictionsProvided: 'Work Restrictions Provided',
  leftWithoutBeingSeen: 'Left Without Being Seen',
  leftAgainstMedicalAdvice: 'Left Against Medical Advice',
  socialWorkConsultCompleted: 'Social Work Consult Completed',
};

const SECTION_FIELDS = {
  'disposition': ['date', 'dispositionType', 'acuityLevelAtDisposition', 'admissionDiagnosis', 'codeStatus', 'dispositionDelayReason'],
  'admission-transfer': ['admittingService', 'admittingPhysician', 'inpatientUnitAssigned', 'transferFacility', 'transferReason', 'transportMode'],
  'timing': ['dispositionTimestamp', 'bedRequestTime', 'bedAssignedTime', 'departureTime', 'lengthOfStay'],
  'discharge-followup': ['dischargeInstructions', 'followUpProvider', 'followUpTimeframe', 'prescriptionsMedications', 'returnPrecautions', 'workRestrictionsProvided'],
  'special-flags': ['leftWithoutBeingSeen', 'leftAgainstMedicalAdvice', 'socialWorkConsultCompleted'],
};

const DATE_FIELDS = ['date'];
const DATETIME_FIELDS = ['dispositionTimestamp', 'bedRequestTime', 'bedAssignedTime', 'departureTime'];
const NUMBER_FIELDS = ['lengthOfStay'];
const ARRAY_FIELDS = ['prescriptionsMedications', 'returnPrecautions'];
const BOOLEAN_FIELDS = ['workRestrictionsProvided', 'leftWithoutBeingSeen', 'leftAgainstMedicalAdvice', 'socialWorkConsultCompleted'];
const STRING_FIELDS = ['dispositionType', 'admittingService', 'admittingPhysician', 'inpatientUnitAssigned', 'dischargeInstructions', 'followUpProvider', 'followUpTimeframe', 'transferFacility', 'transferReason', 'transportMode', 'acuityLevelAtDisposition', 'admissionDiagnosis', 'codeStatus', 'dispositionDelayReason'];

/* asUTCDate: parse a stored date/datetime string as WALL-CLOCK (tz-independent) — reads only the
   Y-M-D H:mm components so a 'Z' midnight date and a no-Z clinical time both render as authored. */
...
/* formatDate: friendly date display (wall-clock, tz-independent) */
...
/* toInputDate: stored date → BlueDatePicker value "YYYY-MM-DD" (string-parsed, no tz drift) */
const toInputDate = (dateValue) => {
  if (!dateValue) return '';
  return String(dateValue.$date || dateValue).split('T')[0] || '';
};

/* formatDateTime: friendly date + time-of-day display (timing fields carry HH:mm; wall-clock) */
...
/* toInputDateTime: stored datetime → "YYYY-MM-DDTHH:mm" for the BlueDatePicker + BlueTimePicker pair */
...
/* splitNumberUnit: "18.5 ng/mL" → {num, sep, unit} for the number+unit stepper (unit chip untouched) */
...
/* ═══════ COMPONENT ═══════ */
const EmergencyDispositionDocument = ({ document: docProp }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        // mirror the markers the save handlers set so badges + the approve button reappear
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Emergency Department Disposition ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // staged drafts for THIS record whose field belongs to THIS section
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/emergency_disposition/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/emergency_disposition/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const sameAsTitle = (lbl) => lbl.toLowerCase() === (title || '').toLowerCase();
    let body = '';
    fields.forEach(f => {
      const label = FIELD_LABELS[f] || f;
      const head = sameAsTitle(label) ? '' : `${label}\n${DASH}\n`;  // single-name: drop dup label
      const val = getFieldValue(record, f, idx);
      if (DATE_FIELDS.includes(f)) {
        ...
        const items = (Array.isArray(val) ? val : [val]).filter(it => it !== null && it !== undefined && String(it).trim() !== '');
    ...
    if (!body) return '';  // empty-section guard → Copy All drops empty sections
    return `${title}\n${'='.repeat(40)}\n\n${body}`;
  }, [getFieldValue, hasVal, fmtVal, numberShows, splitBySentence, formatSentenceFieldLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== EMERGENCY DEPARTMENT DISPOSITION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Emergency Department Disposition ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: DATE FIELD (input type=date, YYYY-MM-DD) ═══════ */
  ...
  /* ═══════ RENDER: DATETIME FIELD (input type=datetime-local, preserves time-of-day) ═══════ */
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="datetime-edit-row">
                <BlueDatePicker value={editValue.split('T')[0] || ''} onSelect={iso => setEditValue(`${iso || ''}T${editValue.split('T')[1] || '00:00'}`)} />
                <BlueTimePicker value={editValue.split('T')[1] || ''} onChange={hm => setEditValue(`${editValue.split('T')[0] || ''}T${hm}`)} />
              </div>
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const d = editValue.split('T')[0]; const t = editValue.split('T')[1] || '00:00'; if (!d) { setSaveError('Please choose a date'); return; } handleSaveField(record, fn, idx, sid, null, `${d}T${t}:00`); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
  ...
  /* ═══════ RENDER: ARRAY FIELD (labeled items → nested-subtitle + mini-card; number+unit value → stepper) ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          const editableVal = parsed.isLabeled ? parsed.value : itemStr;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="emergency-disposition-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Emergency Department Disposition</h2></div>
        <div className="empty-state">No emergency department disposition data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="emergency-disposition-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Emergency Department Disposition</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<EmergencyDispositionDocumentPDFTemplate document={pdfData} />} fileName="Emergency_Disposition.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search emergency department disposition..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Emergency Department Disposition ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'disposition')}
            {renderSection(record, idx, 'admission-transfer')}
            {renderSection(record, idx, 'timing')}
            {renderSection(record, idx, 'discharge-followup')}
            {renderSection(record, idx, 'special-flags')}
          </div>
        ))}
      </div>
    </div>
...
export default EmergencyDispositionDocument;
