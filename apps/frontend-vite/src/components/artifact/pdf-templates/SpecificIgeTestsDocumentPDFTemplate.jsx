...
const SECTIONS = [
  { id: 'testInformation', title: 'Test Information', fields: ['allergenTested', 'allergenCategory', 'allergenSource', 'allergenCode', 'testMethodology'] },
  { id: 'testResults', title: 'Results', fields: ['igeLevel', 'allergenClass', 'totalIgeLevel', 'referenceRange', 'componentTesting'] },
  { id: 'clinicalAssessment', title: 'Clinical Assessment', fields: ['clinicalRelevance', 'anaphylaxisRisk', 'crossReactivity'] },
  { id: 'treatmentPlan', title: 'Treatment Plan', fields: ['immunotherapyCandidate', 'avoidanceRecommendations', 'medicationInteractions'] },
  { id: 'patientHistory', title: 'Patient History', fields: ['ageAtFirstExposure', 'seasonalVariation', 'geographicRelevance'] },
  { id: 'followUp', title: 'Follow-Up', fields: ['retestRecommendation'] },
  { id: 'additionalInformation', title: 'Additional Information', fields: ['additionalData'] },
];
const FIELD_LABELS = {
...
export default function SpecificIgeTestsDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Specific IgE Tests</Text></View>
    {!records.length && <Text style={styles.noDataText}>No specific IgE tests data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Specific IgE Tests Record {recordIndex + 1}</Text></View>
