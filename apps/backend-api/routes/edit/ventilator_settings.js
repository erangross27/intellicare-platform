...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'ventilator-settings-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'mode', 'tidalVolume', 'tidalVolume.value', 'tidalVolume.unit',
  'respiratoryRate', 'respiratoryRate.set', 'respiratoryRate.total',
  'peep', 'peep.value', 'peep.unit',
  'fio2', 'pressureSupport', 'pressureSupport.value', 'pressureSupport.unit',
  'peakPressure', 'peakPressure.value', 'peakPressure.unit',
  'plateauPressure', 'plateauPressure.value', 'plateauPressure.unit',
  'minuteVentilation', 'minuteVentilation.value', 'minuteVentilation.unit',
  'date', 'provider', 'facility', 'findings', 'assessment', 'plan',
  'recommendations', 'notes', 'status',
  'results'
];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('ventilator_settings', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[ventilator_settings] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('ventilator_settings', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[ventilator_settings] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
