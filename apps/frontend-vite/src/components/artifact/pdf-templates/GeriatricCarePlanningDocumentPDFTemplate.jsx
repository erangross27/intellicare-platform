/**
 * GeriatricCarePlanningDocumentPDFTemplate.jsx
 ...
 * codeStatus enum canonical, DOTTED-PATH fields (advancedDirectives.*) resolved via getVal, booleans
 * (advancedDirectives.livingWill / palliativeCareInvolvement / hospiceDiscussion) Yes/No, goalsOfCare
 * string array, results object stacked, recommendations date-grouped, narratives split on [.;] with a
 ...
 * Static PHI footer. Collection: geriatric_care_planning.
 */
...
const SECTION_ORDER = ['session-info', 'directives', 'goals-prognosis', 'care-planning', 'clinical'];
...
const FIELD_LABELS = {
  ...
  goalsOfCare: 'Goals of Care', prognosisDiscussion: 'Prognosis Discussion',
  palliativeCareInvolvement: 'Palliative Care Involvement', hospiceDiscussion: 'Hospice Discussion',
...
const GeriatricCarePlanningDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].geriatric_care_planning && Array.isArray(templateData[0].geriatric_care_planning)) records = templateData[0].geriatric_care_planning;
    else records = templateData;
  } else if (templateData && templateData.geriatric_care_planning) {
    records = Array.isArray(templateData.geriatric_care_planning) ? templateData.geriatric_care_planning : [templateData.geriatric_care_planning];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.geriatric_care_planning ? (Array.isArray(dd.geriatric_care_planning) ? dd.geriatric_care_planning : [dd.geriatric_care_planning]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Geriatric Care Planning</Text></View>
        <Text style={styles.emptyState}>No geriatric care planning records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Geriatric Care Planning ${idx + 1}`}</Text>
...
export default GeriatricCarePlanningDocumentPDFTemplate;
