/**
 * CriticalViewOfSafetyDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: critical_view_of_safety
 *
 * 4 Sections:
 *   1. cvs-status: achievedStatus (yes/no), calotTriangleDissected (yes/no), twoStructuresRule (yes/no)
 *   2. structures: structuresIdentified (comma-split), cysticArteryIsolated (yes/no), cysticDuctIsolated (yes/no), commonBileDuctCleared, liverBedCleared
 *   3. safety-complications: safetySteps, difficultiesEncountered, bailoutProcedure, conversionNeeded (yes/no), timeToAchieve
 *   4. documentation: documentation
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import CriticalViewOfSafetyDocumentPDFTemplate from '../pdf-templates/CriticalViewOfSafetyDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './CriticalViewOfSafetyDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'critical_view_of_safetyPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  achievedStatus: 'Achieved Status',
  calotTriangleDissected: 'Calot Triangle Dissected',
  twoStructuresRule: 'Two Structures Rule',
  structuresIdentified: 'Structures Identified',
  cysticArteryIsolated: 'Cystic Artery Isolated',
  cysticDuctIsolated: 'Cystic Duct Isolated',
  commonBileDuctCleared: 'Common Bile Duct Cleared',
  liverBedCleared: 'Liver Bed Cleared',
  safetySteps: 'Safety Steps',
  difficultiesEncountered: 'Difficulties Encountered',
  bailoutProcedure: 'Bailout Procedure',
  conversionNeeded: 'Conversion Needed',
...
/* ═══════ COMPONENT ═══════ */
const CriticalViewOfSafetyDocument = ({ document: docProp }) => {
  ...
  // yes/no stored lowercase -> title-case display (matches the dropdown labels); mirrored in Copy + PDF.
  const fmtYesNo = useCallback((v) => { const l = String(v ?? '').toLowerCase().trim(); if (l === 'yes') return 'Yes'; if (l === 'no') return 'No'; return fmtVal(v); }, [fmtVal]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Critical View of Safety ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const suffix = `-${idx}`;
    setApproving(true);
    // Pending edits whose editKey (fieldPart-idx) belongs to a field in this section.
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/critical_view_of_safety/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/critical_view_of_safety/${id}/approve`, { sectionId: sid, approved: true });
  ...
  /* Emit label + DASH + numbered rows for ONE field (draft-aware via getFieldValue). */
  const buildFieldLines = useCallback((record, f, idx) => {
    ...
    if (OBJECT_ARRAY_FIELDS.includes(f)) {
      ...
      // Group by CONSECUTIVE same date; numbering restarts per dated group, continues within undated run.
      let lastDate = null; let n = 0;
      recs.forEach(r => {
        const rec = (r?.recommendation || '').trim();
        const date = (r?.date || '').trim();
        if (date !== lastDate) { if (date) { lines.push(date, COPY_LINE_DASH); n = 0; } lastDate = date; }
        lines.push(`${++n}. ${rec}`);
      });
      lines.push('');
      return lines;
    ...
    if (COMMA_SPLIT_FIELDS.includes(f)) {
      ...
      lines.push(label, COPY_LINE_DASH);
      if (items.length >= 3) items.forEach((item, i) => lines.push(`${i + 1}. ${item}`));
      else lines.push(`1. ${displayVal}`);
      lines.push('');
      ...
      lines.push(label, COPY_LINE_DASH, `1. ${displayVal}`, '');
    }
    return lines;
  }, [getFieldValue, hasVal, fmtVal, splitBySentence, splitByComma]);

  const buildSectionCopyText = useCallback((record, idx, sid) => {
    const title = SECTION_TITLES[sid];
    const lines = (SECTION_FIELDS[sid] || []).flatMap(f => buildFieldLines(record, f, idx));
    if (lines.length === 0) return '';
    return `${title}\n${COPY_LINE_EQ}\n\n${lines.join('\n')}\n`;
  }, [buildFieldLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== CRITICAL VIEW OF SAFETY ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Critical View of Safety ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  const renderYesNoField = (record, fn, idx, sid, title) => {
    const val = getFieldValue(record, fn, idx);
  ...
  /* ═══════ RENDER: COMMA-SPLIT FIELD (structuresIdentified) ═══════ */
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              setSaveError(null);
                              stageDraft(record, idx, fn, newArr, sid);
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (YES_NO_FIELDS.includes(f)) return renderYesNoField(record, f, idx, sid, title);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="critical-view-of-safety-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Critical View of Safety</h2></div>
        <div className="empty-state">No critical view of safety records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="critical-view-of-safety-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Critical View of Safety</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<CriticalViewOfSafetyDocumentPDFTemplate document={pdfData} />} fileName="Critical_View_of_Safety.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search critical view of safety..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Critical View of Safety ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'cvs-status')}
            {renderSection(record, idx, 'structures')}
            {renderSection(record, idx, 'safety-complications')}
...
export default CriticalViewOfSafetyDocument;
