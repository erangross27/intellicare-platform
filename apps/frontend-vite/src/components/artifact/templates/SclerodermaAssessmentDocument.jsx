 ...
 * July 2026 — recursive, dot-path editable scleroderma assessment
...
const SECTION_TITLES = {
  overview: 'Assessment Overview', skin: 'Skin Involvement', raynauds: "Raynaud's Phenomenon", organInvolvement: 'Internal Organ Involvement', findings: 'Findings', assessment: 'Assessment', plan: 'Plan', recommendations: 'Recommendations', results: 'Results', notes: 'Notes and Status',
};
const FIELD_LABELS = {
  type: 'Scleroderma Type', date: 'Date', provider: 'Provider', facility: 'Facility', skinThickness: 'Skin Thickness', raynaudsPhenomenon: "Raynaud's Phenomenon", internalOrganInvolvement: 'Internal Organ Involvement', findings: 'Findings', assessment: 'Assessment', plan: 'Plan', recommendations: 'Recommendations', results: 'Results', notes: 'Notes', status: 'Status',
};
const SECTION_FIELDS = {
  overview: ['type', 'date', 'provider', 'facility'], skin: ['skinThickness'], raynauds: ['raynaudsPhenomenon'], organInvolvement: ['internalOrganInvolvement'], findings: ['findings'], assessment: ['assessment'], plan: ['plan'], recommendations: ['recommendations'], results: ['results'], notes: ['notes', 'status'],
};
const DATE_FIELDS = ['date'];
const ENUM_FIELDS = ['type', 'status'];
const NUMBER_FIELDS = [];
const NUMBER_STRING_FIELDS = [];
const OBJECT_FIELDS = ['skinThickness', 'raynaudsPhenomenon', 'internalOrganInvolvement', 'results'];
const OBJECT_ITEM_LABELS = {};
const NARRATIVE_PATHS = ['skinThickness.distribution', 'raynaudsPhenomenon.severity', 'raynaudsPhenomenon.nailfoldCapillaroscopy', 'internalOrganInvolvement.pulmonary', 'internalOrganInvolvement.cardiac', 'internalOrganInvolvement.renal'];
const PARENTHETICAL_LABEL_FIELDS = [];
const COMMA_FIELDS = ['assessment', ...NARRATIVE_PATHS];
const COMMA_ARRAY_FIELDS = [];
const ARRAY_FIELDS = [];
const SEMICOLON_FIELDS = ['plan', 'notes'];
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'status' ? ['active', 'monitoring', 'stable', 'progressive', 'inactive']
    : field === 'type' ? ['Limited Cutaneous Systemic Sclerosis (lcSSc)', 'Diffuse Cutaneous Systemic Sclerosis (dcSSc)', 'Systemic Sclerosis Sine Scleroderma', 'Overlap Syndrome'] : [];
...
const SclerodermaAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.scleroderma_assessment) return Array.isArray(record.scleroderma_assessment) ? record.scleroderma_assessment : [record.scleroderma_assessment];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.scleroderma_assessment) return Array.isArray(nested.scleroderma_assessment) ? nested.scleroderma_assessment : [nested.scleroderma_assessment];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/scleroderma_assessment/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/scleroderma_assessment/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SclerodermaAssessment] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Scleroderma Assessment ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="scleroderma-assessment-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Scleroderma Assessment</h2></div><div className="empty-state">No scleroderma assessment records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Scleroderma Assessment ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="scleroderma-assessment-document" ref={containerRef}>
    <div className="document-header">
      ...
      <div className="header-actions">
        ...
        <PDFDownloadLink document={<SclerodermaAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Scleroderma_Assessment.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search scleroderma assessment..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Scleroderma Assessment ' + (recordIndex + 1))}</h3></div>
