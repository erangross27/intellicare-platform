/**
 * GynecologyConsultationsDocumentPDFTemplate.jsx
 * Box-free B&W LETTER (canonical, memory 6a2d6af6) — mirrors the JSX: real record.consultDate (never
 * createdAt), recommendations date-grouped numbered list ({recommendation, date}), all string fields
 * [.;] sentence-split with abbrev/single-initial guard + labeled comma-split (thousands guard), values
 * numbered ('1.' even singles), single-name label gate (Recommendations field label hidden under its
 * matching section title). Rule #74: each field is ONE wrap={false} atomic View with the sectionTitle
 * riding INSIDE the first present field's View. Static PHI footer. Collection: gynecology_consultations.
 */
...
const SECTION_ORDER = ['consult-info', 'clinical-findings', 'treatment', 'recommendations'];
...
const FIELD_LABELS = {
  consultDate: 'Consult Date', referralReason: 'Referral Reason', chiefComplaint: 'Chief Complaint',
  examFindings: 'Exam Findings', diagnosis: 'Diagnosis',
  treatmentPlan: 'Treatment Plan', followUpPlan: 'Follow-Up Plan',
  recommendations: 'Recommendations',
};
  ...
  if (OBJECT_ARRAY_FIELDS.includes(f)) {
    ...
    return [(
      <View key={f} style={styles.fieldGroup} wrap={false}>
        ...
        {groups.map((g, gi) => (
          <View key={gi}>
            {g.date ? <Text style={styles.subLabel}>{formatDate(g.date)}</Text> : null}
...
const GynecologyConsultationsDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].gynecology_consultations && Array.isArray(templateData[0].gynecology_consultations)) records = templateData[0].gynecology_consultations;
  ...
  } else if (templateData && templateData.gynecology_consultations) {
    records = Array.isArray(templateData.gynecology_consultations) ? templateData.gynecology_consultations : [templateData.gynecology_consultations];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.gynecology_consultations ? (Array.isArray(dd.gynecology_consultations) ? dd.gynecology_consultations : [dd.gynecology_consultations]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Gynecology Consultations</Text></View>
        <Text style={styles.emptyState}>No gynecology consultation records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Gynecology Consultation ${idx + 1}`}</Text>
...
export default GynecologyConsultationsDocumentPDFTemplate;
