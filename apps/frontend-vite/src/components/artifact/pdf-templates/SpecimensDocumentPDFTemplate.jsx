...
const SECTIONS = [
  { id: 'specimenInformation', title: 'Specimen Information', fields: ['type', 'description', 'handling', 'pathologyNumber'] },
  { id: 'reportedResults', title: 'Results', fields: ['preliminaryResults', 'finalResults', 'findings'] },
  { id: 'assessmentPlan', title: 'Assessment & Plan', fields: ['assessment', 'plan'] },
  { id: 'recommendationList', title: 'Recommendations', fields: ['recommendations'] },
  { id: 'notes', title: 'Notes', fields: ['notes'] },
  { id: 'providerInformation', title: 'Provider Information', fields: ['provider', 'facility', 'status', 'date'] },
  { id: 'structuredResults', title: 'Structured Results', fields: ['results'] },
...
export default function SpecimensDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Specimens</Text></View>
    {!records.length && <Text style={styles.noDataText}>No specimens data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Specimens Record {recordIndex + 1}</Text></View>
