/**
 * IntrauterineInseminationDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: intrauterine_insemination
 *
 * 6 Sections:
 *   1. sperm-prep: spermSourceType, prewashSpermConcentration, postwashSpermConcentration, totalMotileSpermCount, spermMotilityPercentage, spermMorphologyPercentage
 *   2. stimulation: ovulationInductionProtocol, triggerMedicationType, triggerToIuiInterval, leadFollicleCount, leadFollicleDiameter, peakEstradiolLevel, antralFollicleCount, antiMullerianHormoneLevel
 *   3. endometrium: endometrialThickness, endometrialPattern
 *   4. procedure: catheterType, inseminationVolume, cervicalMucusScore, procedureDifficulty, iuiCycleNumber, lhSurgeDetected, doubleInseminationPerformed
 *   5. luteal: lutealPhaseSupport
 *   6. workup: hysterosalpingogramResult
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import IntrauterineInseminationDocumentPDFTemplate from '../pdf-templates/IntrauterineInseminationDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './IntrauterineInseminationDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'intrauterine_inseminationPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  antiMullerianHormoneLevel: 'Anti-Müllerian Hormone (ng/mL)',
...
const STRING_FIELDS = ['spermSourceType', 'ovulationInductionProtocol', 'triggerMedicationType', 'endometrialPattern', 'catheterType', 'procedureDifficulty', 'lutealPhaseSupport', 'hysterosalpingogramResult'];
...
/* ═══════ COMPONENT ═══════ */
const IntrauterineInseminationDocument = ({ document: docProp }) => {
  ...
  /* Resolve a record's _id (handles _id.$oid) — defined inline here so the rehydrate effect can use it. */
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // fieldPart is "field" or "field.arrayIndex"; localEdits always keys by base field "-idx".
        ...
        if (isArrayPart) nFields[`${baseField}.${fieldPart.slice(lastDot + 1)}-${idx}`] = 'edited';
        else { nFields[`${baseField}-${idx}`] = 'edited'; nSentences[`${baseField}-${idx}-s0`] = 'edited'; }
      });
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Intrauterine Insemination ${idx + 1}`.toLowerCase();
  ...
  // stageDraft = stage a sentence/field DRAFT locally + persist to localStorage (survives refresh).
  // Clears the section's approved flag so re-edits return to Pending. NO DB write here (Approve commits).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // localEdits keys are "field-idx" (or "field.arrayIndex-idx"); only commit this section's pending fields.
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/intrauterine_insemination/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/intrauterine_insemination/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== INTRAUTERINE INSEMINATION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Intrauterine Insemination ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="intrauterine-insemination-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Intrauterine Insemination</h2></div>
        <div className="empty-state">No intrauterine insemination data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="intrauterine-insemination-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Intrauterine Insemination</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<IntrauterineInseminationDocumentPDFTemplate document={pdfData} />} fileName="Intrauterine_Insemination.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search intrauterine insemination..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Intrauterine Insemination ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'sperm-prep')}
            {renderSection(record, idx, 'stimulation')}
            {renderSection(record, idx, 'endometrium')}
            {renderSection(record, idx, 'procedure')}
            {renderSection(record, idx, 'luteal')}
            {renderSection(record, idx, 'workup')}
          </div>
        ))}
      </div>
    </div>
...
export default IntrauterineInseminationDocument;
