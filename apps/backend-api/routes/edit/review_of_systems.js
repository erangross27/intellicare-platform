/**
 * ReviewOfSystems Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/review_of_systems/:id/edit    — Edit a single field
 * PUT /api/edit/review_of_systems/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'review-of-systems-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('review_of_systems', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[ReviewOfSystems] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('review_of_systems', { _id: objectId }, {
      $set: {
    ...
    console.error('[ReviewOfSystems] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
