/**
 * PancreasTransplantEvaluationDocument.jsx
 * March 2026 — Canonical rewrite with inline editing, blue glow theme
 * Collection: pancreas_transplant_evaluation
 *
 * Title-only header (record has ONLY createdAt/updatedAt ingestion timestamps, no real `date`).
 *
 * 7 Sections:
 *   1. diabetes-overview: primaryIndicationDiabetesType (sentence), cPeptideLevel, hemoglobinA1c,
 *                         diabetesDurationYears, insulinRequirementTotalDailyDose
 *   2. hypoglycemia: hypoglycemicUnawarenessScore, ryanHypoglycemiaSeverityScore
 *   3. complications: diabetesComplications (array), estimatedGFR,
 *                     autonomicNeuropathyAssessment (sentence), gastroparesisGastricEmptyingTime
 *   4. transplant-category: transplantCategory (sentence), previousKidneyTransplant (boolean)
 *   5. cardiac-vascular: cardiacStressTestResult, coronaryAngiographyFindings, leftVentricularEjectionFraction,
 *                        iliacArteryDuplexFindings, aortoiliacCalcificationScore, bmi
 *   6. immunology: panelReactiveAntibodyPercentage, hlaTyping (sentence), donorSpecificAntibodies (array)
 *   7. infection-screening: activeInfectionScreening (sentence)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PancreasTransplantEvaluationDocumentPDFTemplate from '../pdf-templates/PancreasTransplantEvaluationDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueSelect from '../components/BlueSelect';
import './PancreasTransplantEvaluationDocument.css';
...
/* number fields — num-stepper (input[type=text inputMode=decimal]) + parseFloat/isNaN→block save */
const NUMBER_FIELDS = [
...
/* narrative strings → splitBySentence + parseLabel + comma-split */
const STRING_FIELDS = [
...
/* boolean → BlueSelect (Yes/No), stored as true/false */
const BOOLEAN_FIELDS = ['previousKidneyTransplant'];

/* string arrays */
const ARRAY_FIELDS = ['diabetesComplications', 'donorSpecificAntibodies'];

/* Sentinel-zero: a 0-valued score/measure is the extractor's unset default, never a real value → 0-valued
   NUMBER_FIELDS are HIDDEN everywhere (JSX render + section-visibility + copy + PDF). Memory 6a4fa368.
   cPeptideLevel=0.1 is a real decimal (NOT 0) and renders; only exactly-0 hides. */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldName]: value } } — value is the full field value (string/number/bool/array). */
const DRAFT_KEY = 'pancreas_transplant_evaluationPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const PancreasTransplantEvaluationDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.pancreas_transplant_evaluation) return Array.isArray(r.pancreas_transplant_evaluation) ? r.pancreas_transplant_evaluation : [r.pancreas_transplant_evaluation];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.pancreas_transplant_evaluation) return Array.isArray(dd.pancreas_transplant_evaluation) ? dd.pancreas_transplant_evaluation : [dd.pancreas_transplant_evaluation]; return [dd]; }
      return [r];
    });
  ...
  /* hasFieldVal: hasVal + sentinel-zero hide (a 0-valued score/measure is the extractor's unset default) */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pancreas Transplant Evaluation ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const field = m[1];
        await secureApiClient.put(`/api/edit/pancreas_transplant_evaluation/${id}/edit`, { field, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/pancreas_transplant_evaluation/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PANCREAS TRANSPLANT EVALUATION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Pancreas Transplant Evaluation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD — num-stepper (type=text inputMode=decimal) + parseFloat/isNaN→block ═══════ */
  ...
  /* ═══════ RENDER: BOOLEAN FIELD — BlueSelect (Yes/No), stored true/false ═══════ */
  ...
  /* ═══════ RENDER: ARRAY FIELD (string arrays) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pancreas-transplant-evaluation-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pancreas Transplant Evaluation</h2></div>
        <div className="empty-state">No pancreas transplant evaluation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pancreas-transplant-evaluation-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Pancreas Transplant Evaluation</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PancreasTransplantEvaluationDocumentPDFTemplate document={pdfData} />} fileName="Pancreas_Transplant_Evaluation.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pancreas transplant evaluation..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Pancreas Transplant Evaluation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'diabetes-overview')}
            {renderSection(record, idx, 'hypoglycemia')}
            {renderSection(record, idx, 'complications')}
            {renderSection(record, idx, 'transplant-category')}
            {renderSection(record, idx, 'cardiac-vascular')}
            {renderSection(record, idx, 'immunology')}
            {renderSection(record, idx, 'infection-screening')}
          </div>
        ))}
      </div>
    </div>
...
export default PancreasTransplantEvaluationDocument;
