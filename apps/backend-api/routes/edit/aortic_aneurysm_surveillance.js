...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'aortic-aneurysm-surveillance-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'aneurysmMaxDiameter',
  'aneurysmLocation',
  'previousDiameterMeasurement',
  'annualGrowthRate',
  'saccularMorphology',
  'crawfordClassification',
  'aorticNeckLength',
  'aorticNeckAngulation',
  'iliacArteryDiameter',
  'aorticRootDiameter',
  'sinotubularJunctionDiameter',
  'aorticValvePathology',
  'intraluminalThrombus',
  'maxThrombusBurden',
  'penetratingAorticUlcer',
  'intramualHematoma',
  'peakWallStress',
  'aorticDissectionHistory',
  'falseLumenStatus',
  'primaryEntryTearLocation',
  'connectiveTissueDisorder',
  'geneticMutationStatus',
  'surveillanceImagingModality',
  'recommendedSurveillanceInterval',
  'ruptureRiskScore',
  'endoleakSurveillance',
  'graftMigration',
  'sacRegressionStatus',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('aortic_aneurysm_surveillance', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[AorticAneurysmSurveillance] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('aortic_aneurysm_surveillance', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[AorticAneurysmSurveillance] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
