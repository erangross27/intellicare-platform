/**
 * TransplantEvaluationsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: transplant_evaluations
 *
 * 5 Sections:
 *   1. transplant-info: organType, transplantEligibilityStatus, dialysisHistory, bodyMassIndex
 *   2. cardiac-pulmonary: leftVentricularEjectionFraction, nyhaClassification, coronaryAngiographyResults, pulmonaryFunctionTests
 *   3. immunology-labs: aboBloodType, hlaTyping, crossmatchResults, panelReactiveAntibodies, viralSerologyPanel, cancerScreeningResults
 *   4. functional-psychosocial: functionalStatusAssessment, psychosocialEvaluation, liverBiopsy
 *   5. risk-factors: contraindications, vaccineStatus, calculatedMeldScore, estimatedGlomerularFiltrationRate
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import TransplantEvaluationsDocumentPDFTemplate from '../pdf-templates/TransplantEvaluationsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './TransplantEvaluationsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'transplant_evaluationsPendingEdits';
const readDrafts = () => {
...
const NUMBER_FIELDS = ['bodyMassIndex', 'leftVentricularEjectionFraction', 'panelReactiveAntibodies', 'calculatedMeldScore', 'estimatedGlomerularFiltrationRate'];
const BOOLEAN_FIELDS = [];
const DATE_FIELDS = [];
const ARRAY_FIELDS = ['contraindications', 'vaccineStatus'];
const STRING_FIELDS = ['organType', 'transplantEligibilityStatus', 'dialysisHistory', 'nyhaClassification', 'coronaryAngiographyResults', 'pulmonaryFunctionTests', 'aboBloodType', 'hlaTyping', 'crossmatchResults', 'viralSerologyPanel', 'cancerScreeningResults', 'functionalStatusAssessment', 'psychosocialEvaluation', 'liverBiopsy'];
const PERIOD_SPLIT_FIELDS = new Set(['hlaTyping', 'psychosocialEvaluation', 'cancerScreeningResults', 'functionalStatusAssessment']);
const COMMA_ARRAY_FIELDS = ['pulmonaryFunctionTests', 'hlaTyping', 'viralSerologyPanel'];
...
/* ═══════ COMPONENT ═══════ */
const TransplantEvaluationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.transplant_evaluations) return Array.isArray(dd.transplant_evaluations) ? dd.transplant_evaluations : [dd.transplant_evaluations]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      const id = safeRecId(record);
      const recDrafts = id ? store[id] : null;
      if (!recDrafts) return;
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        const editKey = `${fieldPart}-${idx}`;
        // localEdits is keyed by base field ("fn-idx"); array drafts store the full array value
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Transplant Evaluation ${idx + 1}`.toLowerCase();
  ...
  const pdfData = useMemo(() => {
    // Base-field editKeys ("fn-idx") whose draft is still pending → keep OUT of the PDF until approved.
    // pendingEdits keys are "fieldPart-idx" where fieldPart may be "fn" or "fn.arrayIndex"; localEdits
    // stores the whole field under "fn-idx", so map any pending part back to its base "fn-idx".
    const pendingBaseKeys = {};
    Object.keys(pendingEdits).forEach(pk => {
      if (!pendingEdits[pk]) return;
      const m = pk.match(/^(.+)-(\d+)$/);
      if (!m) return;
      const fieldPart = m[1];
      const dotIdx = fieldPart.lastIndexOf('.');
      const baseField = (dotIdx !== -1 && /^\d+$/.test(fieldPart.slice(dotIdx + 1))) ? fieldPart.slice(0, dotIdx) : fieldPart;
      pendingBaseKeys[`${baseField}-${m[2]}`] = true;
    });
    return filteredRecords.map((record, idx) => {
      const merged = { ...record };
      Object.keys(localEdits).forEach(key => {
        if (pendingBaseKeys[key]) return; // pending drafts stay OUT of the PDF until approved
        const m = key.match(/^(.+)-(\d+)$/);
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Stage a DRAFT in the pending-drafts localStorage store (survives refresh). fieldPart is "fn" or
  // "fn.arrayIndex". NOT written to MongoDB / NOT shown in PDF until Approve commits it.
  ...
  // Drop the matching approve key so a re-edit after approval returns the button to yellow Pending Approve.
  ...
  // Save = stage a DRAFT locally + localStorage. NOT written to MongoDB and NOT shown in the PDF until
  // the user clicks Approve (handleApproveSection commits). Synchronous — no backend call here.
  ...
  // Stage a sentence edit locally (DRAFT). No DB write; Approve commits. Whole field stored under "fn".
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const suffix = `-${idx}`;
      // pendingEdits for THIS section's fields. Key = "fieldPart-idx"; fieldPart = "fn" or "fn.arrayIndex".
      ...
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "fn" or "fn.arrayIndex"
        const dotIdx = fieldPart.lastIndexOf('.');
        const isArrayPart = dotIdx !== -1 && /^\d+$/.test(fieldPart.slice(dotIdx + 1));
        if (isArrayPart) {
          ...
          const elemVal = Array.isArray(arr) ? arr[arrayIndex] : undefined;
          const resp = await secureApiClient.put(`/api/edit/transplant_evaluations/${id}/edit`, { field: baseField, value: elemVal, arrayIndex });
          if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        } else {
          const resp = await secureApiClient.put(`/api/edit/transplant_evaluations/${id}/edit`, { field: fieldPart, value: localEdits[`${fieldPart}-${idx}`] });
          if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        }
      }
      await secureApiClient.put(`/api/edit/transplant_evaluations/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[TransplantEvaluations] Approve error:', err); } finally { setSaving(false); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== TRANSPLANT EVALUATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Transplant Evaluation ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: currentArr })); setPendingEdits(prev => ({ ...prev, [`${fn}.${itemIdx}-${idx}`]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); clearApproved(sid, idx); stageDraft(record, `${fn}.${itemIdx}`, editValue); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
    ...
    return (
      <div key={fn} className="rec-mini-card nested-mini-card">
        ...
        {clauses.map((clause, clauseIndex) => {
          ...
          const groupClass = parsed.isLabeled ? 'nested-mini-card' : '';
          return (
            <div key={clauseIndex} className={groupClass}>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="transplant-evaluations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Transplant Evaluations</h2></div>
        <div className="empty-state">No transplant evaluation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="transplant-evaluations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Transplant Evaluations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<TransplantEvaluationsDocumentPDFTemplate document={pdfData} />} fileName="Transplant_Evaluations.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search transplant evaluations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(record.organType ? `${record.organType.charAt(0).toUpperCase() + record.organType.slice(1)} Transplant Evaluation` : `Transplant Evaluation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'transplant-info')}
            {renderSection(record, idx, 'cardiac-pulmonary')}
            {renderSection(record, idx, 'immunology-labs')}
            {renderSection(record, idx, 'functional-psychosocial')}
...
export default TransplantEvaluationsDocument;
