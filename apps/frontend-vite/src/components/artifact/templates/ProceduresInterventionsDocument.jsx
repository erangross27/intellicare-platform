/**
 * ProceduresInterventionsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: procedures_interventions
 *
 * 6 Sections:
 *   1. timing: assessmentDate, assessmentTime
 *   2. clinical-status: clinicalStatus
 *   3. vital-signs: vitalSigns
 *   4. interventions: interventions
 *   5. response: response
 *   6. plan-recommendations: plan, recommendations
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ProceduresInterventionsDocumentPDFTemplate from '../pdf-templates/ProceduresInterventionsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './ProceduresInterventionsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" — localEdits is keyed by whole field) */
const DRAFT_KEY = 'procedures_interventionsPendingEdits';
const readDrafts = () => {
...
/* scoped comma-split: unlabeled single-sentence clinical comma-lists → one value-only row per part (user aggressive-split pref) */
...
/* ═══════ COMPONENT ═══════ */
const ProceduresInterventionsDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Procedure/Intervention ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* stageDraft — stage a field edit locally (localEdits + pendingEdits) + write to the pending-drafts
     localStorage store (survives refresh). NOT written to MongoDB and NOT shown in the PDF until Approve.
     fieldPart is the DB field name; localEdits is keyed "field-idx" and holds the whole field value. */
  ...
  // Save = stage a DRAFT locally + localStorage (survives refresh). Approve commits it to MongoDB.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // localEdits is keyed "field-idx" and holds the whole field value (string or array).
      const toCommit = fields
        .map(f => `${f}-${idx}`)
        .filter(editKey => pendingEdits[editKey] && localEdits[editKey] !== undefined);
      for (const editKey of toCommit) {
        const fieldName = editKey.slice(0, editKey.length - `-${idx}`.length); // DB field name
        const resp = await secureApiClient.put(`/api/edit/procedures_interventions/${id}/edit`, { field: fieldName, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/procedures_interventions/${id}/approve`, { sectionId: sid, approved: true });
      ...
      console.error('[ProceduresInterventions] Approve error:', err);
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        items.forEach((item, i) => {
          if (typeof item === 'object' && item !== null) {
            ...
            text += recDate ? `${i + 1}. [${recDate}] ${recText}\n` : `${i + 1}. ${recText}\n`;
          } else {
            text += `${i + 1}. ${item}\n`;
          }
        });
        ...
        if (sentences.length > 1) {
          ...
          text += labelLine;
          commaParts.forEach((p, i) => { text += `${i + 1}. ${p}\n`; });
          text += '\n';
        } else {
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PROCEDURES & INTERVENTIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Procedure/Intervention ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: RECOMMENDATIONS ARRAY FIELD (objects with recommendation + date) ═══════ */
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(recText); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; if (isObj) { currentArr[itemIdx] = { ...currentArr[itemIdx], recommendation: editValue }; } else { currentArr[itemIdx] = editValue; } stageDraft(record, fn, idx, sid, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <>
                    <div className="row-content">
                      {recDate && <span className="rec-date-badge">{highlightText(formatDate(recDate))}</span>}
                      <span className="content-value">{highlightText(recText)}</span>
                      <span className="edit-indicator">&#9998;</span>
                    </div>
                    <button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(recDate ? `[${formatDate(recDate)}] ${recText}` : recText, editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button>
                  </>
                )}
              </div>
              {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
  ...
  /* ═══════ RENDER: ARRAY FIELD (simple strings) ═══════ */
  const renderArrayField = (record, fn, idx, sid) => {
    if (fn === 'recommendations') return renderRecommendationsField(record, fn, idx, sid);
    const val = getFieldValue(record, fn, idx);
    const items = Array.isArray(val) ? val.filter(Boolean) : [];
    if (items.length === 0) return null;
    const label = FIELD_LABELS[fn] || fn;
  ...
  /* ═══════ RENDER: COMMA-SPLIT FIELD (scoped clinical comma-lists → value-only rows) ═══════ */
  ...
  const saveCommaPart = (record, fn, idx, sid, pIdx) => {
    const id = safeId(record); if (!id) return;
    const parts = splitByComma(String(getFieldValue(record, fn, idx) || ''));
    const subParts = splitByComma(editValue.trim()).map(p => p.trim()).filter(p => p);
    parts.splice(pIdx, 1, ...subParts);
    const rejoined = parts.map(p => p.trim()).filter(p => p).join(', ');
    setSaveError(null);
    stageDraft(record, fn, idx, sid, rejoined);
    const marks = { [`${fn}-${idx}-c${pIdx}`]: 'edited' };
    for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-c${pIdx + ei}`] = 'added';
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {parts.map((part, pIdx) => {
          ...
          if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
            const phrase = searchTerm.toLowerCase().trim();
            if (!label.toLowerCase().includes(phrase) && !phrase.includes(label.toLowerCase()) && !part.toLowerCase().includes(phrase)) return null;
          }
          return (
            <div key={pIdx}>
              <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(partKey); setEditValue(part); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveCommaPart(record, fn, idx, sid, pIdx); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (COMMA_SPLIT_FIELDS.includes(f)) { const sv = fmtVal(getFieldValue(record, f, idx)); if (hasVal(sv) && shouldCommaSplit(f, sv)) return renderCommaField(record, f, idx, sid); }
  ...
  if (!records || records.length === 0) {
    return (
      <div className="procedures-interventions-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Procedures & Interventions</h2></div>
        <div className="empty-state">No procedures/interventions data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="procedures-interventions-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Procedures & Interventions</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ProceduresInterventionsDocumentPDFTemplate document={pdfData} />} fileName="Procedures_Interventions.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search procedures & interventions..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Procedure/Intervention ${idx + 1}`)}</h3>
            </div>
...
export default ProceduresInterventionsDocument;
