...
const COLLECTION = 'cardiology_admission_notes';
const COMMA_SPLIT_FIELDS = [];
const ARRAY_FIELDS = new Set(['cardiacRiskFactors', 'currentCardiacMedications', 'valvularAbnormalities', 'inotropicSupport']);
const OBJECT_FIELDS = new Set(['chestPainCharacteristics', 'echocardiogramResults', 'hemodynamicParameters', 'anticoagulationStatus', 'cardiacBiomarkerTrend']);
const NARRATIVE_FIELDS = new Set(['chiefCardiacComplaint', 'nyhaClassification', 'killipClassification', 'acuteCoronarySyndromeType', 'ekgFindings', 'coronaryArteryDiseaseHistory', 'arrhythmiaType', 'cardiacCatheterizationPlanned', 'thrombolyticEligibility', 'pulmonaryEdemaPresence', 'telemetryMonitoring', 'functionalCapacity']);
const DATE_FIELDS = new Set(['admissionDate']);
const ZERO_SENTINEL_FIELDS = new Set(['troponinLevel', 'bnpLevel', 'leftVentricularEjectionFraction']);
const KEY_OVERRIDES = { lvedd: 'LVEDD', lvesd: 'LVESD', lvThrombus: 'LV Thrombus', rvFunction: 'RV Function', ptt: 'PTT' };
const LABELS = {
  admissionDate: 'Admission Date', chiefCardiacComplaint: 'Chief Cardiac Complaint', acuteCoronarySyndromeType: 'Acute Coronary Syndrome Type',
  nyhaClassification: 'NYHA Classification', killipClassification: 'Killip Classification',
  chestPainCharacteristics: 'Chest Pain Characteristics', troponinLevel: 'Troponin Level', bnpLevel: 'BNP Level', cardiacBiomarkerTrend: 'Cardiac Biomarker Trend',
  ekgFindings: 'EKG Findings', echocardiogramResults: 'Echocardiogram Results', leftVentricularEjectionFraction: 'Left Ventricular Ejection Fraction',
  hemodynamicParameters: 'Hemodynamic Parameters', coronaryArteryDiseaseHistory: 'Coronary Artery Disease History', arrhythmiaType: 'Arrhythmia Type',
  cardiacRiskFactors: 'Cardiac Risk Factors', currentCardiacMedications: 'Current Cardiac Medications',
  cardiacCatheterizationPlanned: 'Cardiac Catheterization Plan', thrombolyticEligibility: 'Thrombolytic Eligibility',
  valvularAbnormalities: 'Valvular Abnormalities', pulmonaryEdemaPresence: 'Pulmonary Edema', inotropicSupport: 'Inotropic Support',
  telemetryMonitoring: 'Telemetry Monitoring', anticoagulationStatus: 'Anticoagulation Status', functionalCapacity: 'Functional Capacity',
};
const SECTIONS = [
  { title: 'Admission Details', fields: ['admissionDate', 'chiefCardiacComplaint', 'acuteCoronarySyndromeType', 'nyhaClassification', 'killipClassification'] },
  { title: 'Chest Pain Characteristics', fields: ['chestPainCharacteristics'] },
  { title: 'Cardiac Biomarkers', fields: ['troponinLevel', 'bnpLevel'] },
  { title: 'Cardiac Biomarker Trend', fields: ['cardiacBiomarkerTrend'] },
  { title: 'EKG Findings', fields: ['ekgFindings'] },
  { title: 'Echocardiogram', fields: ['echocardiogramResults', 'leftVentricularEjectionFraction'] },
  { title: 'Hemodynamic Parameters', fields: ['hemodynamicParameters'] },
  { title: 'Coronary Artery Disease History', fields: ['coronaryArteryDiseaseHistory'] },
  { title: 'Arrhythmia Type', fields: ['arrhythmiaType'] },
  { title: 'Cardiac Risk Factors', fields: ['cardiacRiskFactors'] },
  { title: 'Current Cardiac Medications', fields: ['currentCardiacMedications'] },
  { title: 'Catheterization and Thrombolytic', fields: ['cardiacCatheterizationPlanned', 'thrombolyticEligibility'] },
  { title: 'Valvular Abnormalities', fields: ['valvularAbnormalities'] },
  { title: 'Pulmonary Edema', fields: ['pulmonaryEdemaPresence'] },
  { title: 'Inotropic Support', fields: ['inotropicSupport'] },
  { title: 'Telemetry Monitoring', fields: ['telemetryMonitoring'] },
  { title: 'Anticoagulation Status', fields: ['anticoagulationStatus'] },
  { title: 'Functional Capacity', fields: ['functionalCapacity'] },
];
...
const CardiologyAdmissionNotesDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Cardiology Admission Notes</Text><Text style={styles.noData}>No cardiology admission notes data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Cardiology Admission Notes</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Cardiology Admission Notes {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
export default CardiologyAdmissionNotesDocumentPDFTemplate;
