/**
 * PastOcularHistoryDocument.jsx
 * July 2026 — Canonical one-pass: inline editing, blue glow theme, box-free PDF
 * Collection: past_ocular_history
 *
 * 4 Sections:
 *   1. ocular-history: lastDilatedExam, glassesContactLensWear, refractiveError, priorEyeSurgery, eyeTrauma
 *   2. provider-info: date (BlueDatePicker), type, provider, facility, status
 *   3. clinical-details: findings, assessment, plan, recommendations (array)
 *   4. additional-notes: notes
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PastOcularHistoryDocumentPDFTemplate from '../pdf-templates/PastOcularHistoryDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './PastOcularHistoryDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the field name, e.g. "provider") */
const DRAFT_KEY = 'past_ocular_historyPendingEdits';
const readDrafts = () => {
...
/* No number fields — all strings + one date + one array */
const NUMBER_FIELDS = [];
...
/* ═══════ COMPONENT ═══════ */
const PastOcularHistoryDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.past_ocular_history) return Array.isArray(r.past_ocular_history) ? r.past_ocular_history : [r.past_ocular_history];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.past_ocular_history) return Array.isArray(dd.past_ocular_history) ? dd.past_ocular_history : [dd.past_ocular_history]; return [dd]; }
      return [r];
    });
  ...
  const fieldSearchText = useCallback((f, val) => {
    if (ARRAY_FIELDS.includes(f)) return (Array.isArray(val) ? val : []).map(x => fmtVal(x)).join(' ');
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Past Ocular History ${idx + 1}`.toLowerCase();
  ...
  /* Stage a DRAFT for one whole field locally + write it to the pending-drafts localStorage store
     ...
     Pending Approve (handleApproveSection commits). */
  ...
  /* Stage the full reconstructed text/array for field `fn` as a DRAFT (no DB write). Survives refresh;
     Approve commits it. Mirrors stageDraft but lets callers set their own sentence markers. */
  const stageFieldDraft = (record, fn, idx, fullValue) => {
    const id = safeId(record); if (!id) return;
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      // the WHOLE field value (string or array), matching the original per-field PUT semantics.
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/past_ocular_history/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/past_ocular_history/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PAST OCULAR HISTORY ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Past Ocular History ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: ARRAY FIELD (per-item editing; unnumbered on screen) ═══════ */
                  ...
                  <div className="edit-field-container">
                    ...
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => {
                        ...
                        setSaveError(null);
                        stageFieldDraft(record, fn, idx, currentArr);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="past-ocular-history-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Past Ocular History</h2></div>
        <div className="empty-state">No past ocular history records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="past-ocular-history-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Past Ocular History</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PastOcularHistoryDocumentPDFTemplate document={pdfData} />} fileName="Past_Ocular_History.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search past ocular history..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Past Ocular History ${idx + 1}`)}</h3>
            </div>
...
export default PastOcularHistoryDocument;
