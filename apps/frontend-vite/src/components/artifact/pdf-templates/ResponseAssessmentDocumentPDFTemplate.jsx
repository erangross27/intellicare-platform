...
const SECTIONS = [
  { id: 'assessmentInformation', title: 'Assessment Information', fields: ['date', 'dateOfResponse', 'status'] },
  { id: 'responseMetrics', title: 'Response Metrics', fields: ['criteria', 'bestResponse', 'depthOfResponse', 'durabilityOfResponse', 'progressionFreesSurvival'] },
  { id: 'measurableDisease', title: 'Measurable Disease', fields: ['measurableDisease'] },
  { id: 'results', title: 'Results', fields: ['results'] },
  { id: 'providerInformation', title: 'Provider Information', fields: ['provider', 'facility'] },
  { id: 'clinicalFindings', title: 'Clinical Findings', fields: ['findings'] },
  { id: 'clinicalAssessment', title: 'Clinical Assessment', fields: ['assessment'] },
  { id: 'treatmentPlan', title: 'Treatment Plan', fields: ['plan'] },
  { id: 'additionalNotes', title: 'Additional Notes', fields: ['notes'] },
...
const sectionBlocks = (record, section) => section.fields.flatMap((field) => {
  ...
  if (field === 'measurableDisease') return measurableBlocks(value);
...
export default function ResponseAssessmentDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Response Assessment</Text></View>
    {!records.length && <Text style={styles.noDataText}>No response assessment data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Response Assessment {recordIndex + 1}</Text></View>
