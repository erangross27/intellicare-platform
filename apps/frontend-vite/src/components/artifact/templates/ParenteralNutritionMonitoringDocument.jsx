/**
 * ParenteralNutritionMonitoringDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: parenteral_nutrition_monitoring (displayed as "Parenteral Nutrition Monitoring")
 *
 * 8 Sections:
 *   1. patient-metrics: patientWeight, prealbumin, serumAlbumin, bloodGlucose
 *   2. lipid-liver-panel: serumTriglycerides, liverFunctionPanel
 *   3. electrolyte-panel: serumSodium, serumPotassium, serumPhosphorus, serumMagnesium, serumCalcium
 *   4. renal-panel: bloodUreaNitrogen, serumCreatinine
 *   5. refeeding-risk: refeedingSyndromeRiskScore
 *   6. central-line-access: centralLineType, catheterRelatedBloodstreamInfection
 *   7. nutrition-provision: totalCaloriesProvided, proteinProvision, dextroseInfusionRate, lipidEmulsionType, fluidBalance
 *   8. metabolic-assessment: indirectCalorimetryREE, respiratoryQuotient, nitrogenBalance, traceElementPanel
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ParenteralNutritionMonitoringDocumentPDFTemplate from '../pdf-templates/ParenteralNutritionMonitoringDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './ParenteralNutritionMonitoringDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'patient-metrics': 'Patient Metrics',
  'lipid-liver-panel': 'Lipid & Liver Panel',
  'electrolyte-panel': 'Electrolyte Panel',
  'renal-panel': 'Renal Panel',
  'refeeding-risk': 'Refeeding Risk',
  'central-line-access': 'Central Line & Access',
  'nutrition-provision': 'Nutrition Provision',
  'metabolic-assessment': 'Metabolic Assessment',
};
...
const FIELD_LABELS = {
  ...
  traceElementPanel: 'Trace Element Panel',
};

const SECTION_FIELDS = {
  'patient-metrics': ['patientWeight', 'prealbumin', 'serumAlbumin', 'bloodGlucose'],
  'lipid-liver-panel': ['serumTriglycerides', 'liverFunctionPanel'],
  'electrolyte-panel': ['serumSodium', 'serumPotassium', 'serumPhosphorus', 'serumMagnesium', 'serumCalcium'],
  'renal-panel': ['bloodUreaNitrogen', 'serumCreatinine'],
  'refeeding-risk': ['refeedingSyndromeRiskScore'],
  'central-line-access': ['centralLineType', 'catheterRelatedBloodstreamInfection'],
  'nutrition-provision': ['totalCaloriesProvided', 'proteinProvision', 'dextroseInfusionRate', 'lipidEmulsionType', 'fluidBalance'],
  'metabolic-assessment': ['indirectCalorimetryREE', 'respiratoryQuotient', 'nitrogenBalance', 'traceElementPanel'],
};

const NUMBER_FIELDS = ['patientWeight', 'prealbumin', 'serumAlbumin', 'bloodGlucose', 'serumTriglycerides', 'serumSodium', 'serumPotassium', 'serumPhosphorus', 'serumMagnesium', 'serumCalcium', 'bloodUreaNitrogen', 'serumCreatinine', 'totalCaloriesProvided', 'proteinProvision', 'dextroseInfusionRate', 'fluidBalance', 'indirectCalorimetryREE', 'respiratoryQuotient', 'nitrogenBalance'];
const BOOLEAN_FIELDS = ['catheterRelatedBloodstreamInfection'];
const STRING_FIELDS = ['liverFunctionPanel', 'refeedingSyndromeRiskScore', 'centralLineType', 'lipidEmulsionType', 'traceElementPanel'];
...
const DRAFT_KEY = 'parenteral_nutrition_monitoringPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const ParenteralNutritionMonitoringDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.parenteral_nutrition_monitoring) return Array.isArray(r.parenteral_nutrition_monitoring) ? r.parenteral_nutrition_monitoring : [r.parenteral_nutrition_monitoring];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.parenteral_nutrition_monitoring) return Array.isArray(dd.parenteral_nutrition_monitoring) ? dd.parenteral_nutrition_monitoring : [dd.parenteral_nutrition_monitoring]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `PN Monitoring ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Pending edits for this record belonging to fields in this section
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/parenteral_nutrition_monitoring/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/parenteral_nutrition_monitoring/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PARENTERAL NUTRITION MONITORING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `PN Monitoring ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="parenteral-nutrition-monitoring-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Parenteral Nutrition Monitoring</h2></div>
        <div className="empty-state">No parenteral nutrition monitoring data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="parenteral-nutrition-monitoring-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Parenteral Nutrition Monitoring</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ParenteralNutritionMonitoringDocumentPDFTemplate document={pdfData} />} fileName="Parenteral_Nutrition_Monitoring.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search parenteral nutrition monitoring..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`PN Monitoring ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'patient-metrics')}
            {renderSection(record, idx, 'lipid-liver-panel')}
            {renderSection(record, idx, 'electrolyte-panel')}
            {renderSection(record, idx, 'renal-panel')}
            {renderSection(record, idx, 'refeeding-risk')}
            {renderSection(record, idx, 'central-line-access')}
            {renderSection(record, idx, 'nutrition-provision')}
            {renderSection(record, idx, 'metabolic-assessment')}
          </div>
        ))}
      </div>
    </div>
...
export default ParenteralNutritionMonitoringDocument;
