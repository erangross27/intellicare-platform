/**
 * BoneMarrowTransplantEvaluationDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: bone_marrow_transplant_evaluation
 *
 * 6 Sections:
 *   1. hla-donor-matching: donorHlaTyping, recipientHlaTyping, hlaMatchGrade, dsaMeanFluorescenceIntensity, donorSpecificAntibodies
 *   2. risk-scores: hctCiComorbidityScore, karnofskyPerformanceStatus, diseaseRiskIndex
 *   3. disease-status: bonemarrowBlastPercentage, pretransplantChimerismBaseline, minimalResidualDisease, mrdSensitivityLevel
 *   4. organ-function: cardiacEjectionFraction, pulmonaryDlcoPercentPredicted, fev1PercentPredicted, hepaticFunctionAssessment, renalGfrPreTransplant, serumFerritinLevel
 *   5. conditioning-graft: conditioningRegimenIntensity, stemCellSource, targetCd34DosePerKg, donorKirHaplotype, gvhdProphylaxisRegimen
 *   6. serostatus: cmvSerostatus, ebvSerostatus
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import BoneMarrowTransplantEvaluationDocumentPDFTemplate from '../pdf-templates/BoneMarrowTransplantEvaluationDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './BoneMarrowTransplantEvaluationDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'hla-donor-matching': 'HLA & Donor Matching',
  'risk-scores': 'Risk Scores (HCT-CI/KPS/DRI)',
  'disease-status': 'Disease Status (Blast%/MRD)',
  'organ-function': 'Organ Function',
  'conditioning-graft': 'Conditioning & Graft',
  'serostatus': 'Serostatus (CMV/EBV)',
};

const FIELD_LABELS = {
  donorHlaTyping: 'Donor HLA Typing',
  recipientHlaTyping: 'Recipient HLA Typing',
  hlaMatchGrade: 'HLA Match Grade',
  dsaMeanFluorescenceIntensity: 'DSA MFI',
  donorSpecificAntibodies: 'Donor-Specific Antibodies',
  hctCiComorbidityScore: 'HCT-CI Score',
  karnofskyPerformanceStatus: 'Karnofsky Performance Status',
  diseaseRiskIndex: 'Disease Risk Index',
  bonemarrowBlastPercentage: 'Blast %',
  pretransplantChimerismBaseline: 'Pre-Transplant Chimerism Baseline',
  minimalResidualDisease: 'Minimal Residual Disease',
  mrdSensitivityLevel: 'MRD Sensitivity Level',
  cardiacEjectionFraction: 'Cardiac EF (%)',
  pulmonaryDlcoPercentPredicted: 'DLCO (% predicted)',
  fev1PercentPredicted: 'FEV1 (% predicted)',
  hepaticFunctionAssessment: 'Hepatic Function Assessment',
  renalGfrPreTransplant: 'Pre-Transplant GFR (mL/min/1.73m²)',
  serumFerritinLevel: 'Serum Ferritin (ng/mL)',
  conditioningRegimenIntensity: 'Conditioning Regimen Intensity',
  stemCellSource: 'Stem Cell Source',
  targetCd34DosePerKg: 'Target CD34 Dose (×10⁶/kg)',
  donorKirHaplotype: 'Donor KIR Haplotype',
...
const SECTION_FIELDS = {
  'hla-donor-matching': ['donorHlaTyping', 'recipientHlaTyping', 'hlaMatchGrade', 'dsaMeanFluorescenceIntensity', 'donorSpecificAntibodies'],
  'risk-scores': ['hctCiComorbidityScore', 'karnofskyPerformanceStatus', 'diseaseRiskIndex'],
  'disease-status': ['bonemarrowBlastPercentage', 'pretransplantChimerismBaseline', 'minimalResidualDisease', 'mrdSensitivityLevel'],
  'organ-function': ['cardiacEjectionFraction', 'pulmonaryDlcoPercentPredicted', 'fev1PercentPredicted', 'hepaticFunctionAssessment', 'renalGfrPreTransplant', 'serumFerritinLevel'],
  'conditioning-graft': ['conditioningRegimenIntensity', 'stemCellSource', 'targetCd34DosePerKg', 'donorKirHaplotype', 'gvhdProphylaxisRegimen'],
  'serostatus': ['cmvSerostatus', 'ebvSerostatus'],
};

const NUMBER_FIELDS = ['dsaMeanFluorescenceIntensity', 'hctCiComorbidityScore', 'karnofskyPerformanceStatus', 'bonemarrowBlastPercentage', 'cardiacEjectionFraction', 'pulmonaryDlcoPercentPredicted', 'fev1PercentPredicted', 'serumFerritinLevel', 'renalGfrPreTransplant', 'targetCd34DosePerKg'];
const ARRAY_FIELDS = [];
const BOOLEAN_FIELDS = ['donorSpecificAntibodies'];
const STRING_FIELDS = ['donorHlaTyping', 'recipientHlaTyping', 'hlaMatchGrade', 'diseaseRiskIndex', 'pretransplantChimerismBaseline', 'minimalResidualDisease', 'mrdSensitivityLevel', 'conditioningRegimenIntensity', 'cmvSerostatus', 'ebvSerostatus', 'hepaticFunctionAssessment', 'gvhdProphylaxisRegimen', 'stemCellSource', 'donorKirHaplotype'];
...
const DRAFT_KEY = 'bone_marrow_transplant_evaluationPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const BoneMarrowTransplantEvaluationDocument = ({ document: docProp }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // fieldPart = "field" or "field.arrayIndex"; localEdits key always uses "field-idx"
        const dotMatch = fieldPart.match(/^(.+)\.(\d+)$/);
        if (dotMatch) {
          const localKey = `${dotMatch[1]}-${idx}`;
          nLocal[localKey] = value;
          nPending[localKey] = true;
          nFields[`${dotMatch[1]}.${dotMatch[2]}-${idx}`] = 'edited';
        } else {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Bone Marrow Transplant Evaluation ${idx + 1}`.toLowerCase();
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    // Stage the draft (fieldPart = field name; this handler never passes an arrayIndex)
    const store = readDrafts();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Persist each staged field for THIS section to the DB now (from the localStorage draft store,
      // whose fieldPart carries the dot-suffix → exact reverse of how saves were staged).
      const store = readDrafts();
      const recDrafts = store[id] || {};
      const committedLocalKeys = [];
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        // fieldPart = "field" or "field.arrayIndex"; arrayIndex ONLY when the last dot-segment is purely numeric
        const dotIdx = fieldPart.lastIndexOf('.');
        let field = fieldPart, arrayIndex;
        if (dotIdx !== -1 && /^\d+$/.test(fieldPart.slice(dotIdx + 1))) {
          field = fieldPart.slice(0, dotIdx);
          arrayIndex = parseInt(fieldPart.slice(dotIdx + 1), 10);
        }
        if (!fields.includes(field)) continue; // only this section's staged edits
        const payload = { field, value };
        if (arrayIndex !== undefined) payload.arrayIndex = arrayIndex;
        const resp = await secureApiClient.put(`/api/edit/bone_marrow_transplant_evaluation/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedLocalKeys.push(`${field}-${idx}`);
        delete recDrafts[fieldPart];
      }
      // Flag the section approved (audit trail) — existing endpoint
      await secureApiClient.put(`/api/edit/bone_marrow_transplant_evaluation/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== BONE MARROW TRANSPLANT EVALUATION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Bone Marrow Transplant Evaluation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="bone-marrow-transplant-evaluation-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Bone Marrow Transplant Evaluation</h2></div>
        <div className="empty-state">No bone marrow transplant evaluation data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="bone-marrow-transplant-evaluation-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Bone Marrow Transplant Evaluation</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<BoneMarrowTransplantEvaluationDocumentPDFTemplate document={pdfData} />} fileName={`bone-marrow-transplant-evaluation-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search bone marrow transplant evaluation..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Bone Marrow Transplant Evaluation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'hla-donor-matching')}
            {renderSection(record, idx, 'risk-scores')}
            {renderSection(record, idx, 'disease-status')}
            {renderSection(record, idx, 'organ-function')}
            {renderSection(record, idx, 'conditioning-graft')}
            {renderSection(record, idx, 'serostatus')}
          </div>
        ))}
      </div>
    </div>
...
export default BoneMarrowTransplantEvaluationDocument;
