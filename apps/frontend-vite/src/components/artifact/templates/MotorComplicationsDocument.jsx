/**
 * MotorComplicationsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: motor_complications
 *
 * 7 Sections:
 *   1. assessment-info: date (date), provider (string), facility (string)
 *   2. motor-fluctuations: motorFluctuations.wearingOff (string), motorFluctuations.onOffPhenomena (string),
 *                          motorFluctuations.delayedOn (string), motorFluctuations.noOn (string),
 *                          motorFluctuations.unpredictableOff (string), motorFluctuations.morningAkinesia (string)
 *   3. dyskinesias: dyskinesias.peakDose (string), dyskinesias.biphasic (string),
 *                   dyskinesias.offPeriod (string), dyskinesias.severity (string),
 *                   dyskinesias.duration (string), dyskinesias.impact (string)
 *   4. timing: offTime (string), onTimeWithDyskinesia (string), onTimeWithoutDyskinesia (string)
 *   5. findings-assessment: findings (string), assessment (string)
 *   6. results: results (object, recursive key-value)
 *   7. plan-recs: plan (string), recommendations (array)
 *   8. notes: notes (string), status (string)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MotorComplicationsDocumentPDFTemplate from '../pdf-templates/MotorComplicationsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './MotorComplicationsDocument.css';
...
const DRAFT_KEY = 'motor_complicationsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = [
  'provider', 'facility',
  'motorFluctuations.wearingOff', 'motorFluctuations.onOffPhenomena', 'motorFluctuations.delayedOn',
  'motorFluctuations.noOn', 'motorFluctuations.unpredictableOff', 'motorFluctuations.morningAkinesia',
  'dyskinesias.peakDose', 'dyskinesias.biphasic', 'dyskinesias.offPeriod',
  'dyskinesias.severity', 'dyskinesias.duration', 'dyskinesias.impact',
  'offTime', 'onTimeWithDyskinesia', 'onTimeWithoutDyskinesia',
...
/* ======= COMPONENT ======= */
const MotorComplicationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.motor_complications) return Array.isArray(r.motor_complications) ? r.motor_complications : [r.motor_complications];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.motor_complications) return Array.isArray(dd.motor_complications) ? dd.motor_complications : [dd.motor_complications]; return [dd]; }
      return [r];
    });
  ...
  /* get nested field value supporting dot-path like "motorFluctuations.wearingOff" */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Motor Complications ${idx + 1}`.toLowerCase();
  ...
  const pdfData = useMemo(() => {
    return filteredRecords.map((record, idx) => {
      ...
      ARRAY_FIELDS.forEach(f => {
        const k = `${f}-${idx}`;
        if (pendingEdits[k]) { merged[f] = record[f]; return; } // pending array drafts stay OUT of the PDF
  ...
  /* ======= EDIT HANDLERS =======
     ...
     The DRAFT fieldPart is the localEdits key WITHOUT the trailing "-idx" suffix (i.e. the field-name `fn`,
     which may be a dot-path like "motorFluctuations.wearingOff"; arrays/objects are stored whole). */
  const stageDraft = useCallback((record, idx, fieldPart, value, trackKey) => {
    ...
    setApprovedSections(prev => {
      ...
      Object.keys(prev).forEach(sk => {
        if (sk.endsWith(`-${idx}`)) {
          const sid = sk.slice(0, -`-${idx}`.length);
          if ((SECTION_FIELDS[sid] || []).includes(fieldPart)) {
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    // Build the full updated object (local edit overrides original), then stage the WHOLE object as a draft.
    const cur = localEdits[`${rootField}-${idx}`] !== undefined ? localEdits[`${rootField}-${idx}`] : record[rootField];
    const clone = JSON.parse(JSON.stringify(cur ?? {}));
    let node = clone;
    for (let i = 0; i < path.length - 1; i++) node = node[path[i]];
    node[path[path.length - 1]] = newVal;
    stageDraft(record, idx, rootField, clone, leafKeyTrack);
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageDraft(record, idx, fn, fullText, null);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Pending drafts for THIS record whose field belongs to THIS section.
    const toCommit = Object.keys(localEdits).filter(k => {
      if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
      return fields.includes(k.slice(0, -suffix.length));
    });
    setSaving(true); setSaveError(null);
    try {
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "field" or "field.subfield" (object/array stored whole)
        ...
        const resp = await secureApiClient.put(`/api/edit/motor_complications/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error((resp && resp.error) || 'save failed');
      }
      await secureApiClient.put(`/api/edit/motor_complications/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MOTOR COMPLICATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Motor Complications ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ======= RENDER: ARRAY FIELD (recommendations) ======= */
                            ...
                            <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                              {ciEditing ? (
                                <div className="edit-field-container">
                                  ...
                                  {saveError && <div className="save-error">{saveError}</div>}
                                  <div className="edit-actions">
                                    <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const currentVal2 = String(getFieldValue(record, fn, idx) || ''); const sentences2 = splitBySentence(currentVal2); const s2 = sentences2[sIdx] || ''; const p2 = parseLabel(s2); if (!p2.isLabeled) return; const items2 = splitByComma(p2.value); const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { items2.splice(ciIdx, 1, ...subParts); } else { items2[ciIdx] = trimmed.replace(/[;.]+$/, '').trim(); } const rebuilt = `${p2.label}: ${items2.join(', ')}.`; const allS = [...sentences2]; allS[sIdx] = rebuilt; const fullText2 = reconstructFullText(allS); setSaveError(null); stageDraft(record, idx, fn, fullText2, null); const marks = { [commaKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ciIdx + ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); }}>{saving ? 'Saving...' : 'Save'}</button>
                                    <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                                  </div>
                                </div>
                              ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="motor-complications-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Motor Complications</h2></div>
        <div className="empty-state">No motor complications records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="motor-complications-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Motor Complications</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MotorComplicationsDocumentPDFTemplate document={pdfData} />} fileName="Motor_Complications.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search motor complications..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Motor Complications ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'assessment-info')}
            {renderSection(record, idx, 'motor-fluctuations')}
            {renderSection(record, idx, 'dyskinesias')}
            {renderSection(record, idx, 'timing')}
            {renderSection(record, idx, 'findings-assessment')}
            {renderSection(record, idx, 'results-section')}
            {renderSection(record, idx, 'plan-recs')}
...
export default MotorComplicationsDocument;
