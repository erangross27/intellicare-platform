...
function buildContext(req, operation = 'read') { return { serviceId: 'neurological-examination-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['glasgowComaScale', 'glasgowComaScaleEye', 'glasgowComaScaleVerbal', 'glasgowComaScaleMotor', 'nihStrokeScale', 'modifiedRankinScale', 'miniMentalStateExam', 'montrealCognitiveAssessment', 'cranialNerveExamination', 'pupillaryResponse', 'extraocularMovements', 'motorStrengthRightUpper', 'motorStrengthLeftUpper', 'motorStrengthRightLower', 'motorStrengthLeftLower', 'deepTendonReflexes', 'pathologicalReflexes', 'sensoryExaminationFindings', 'coordinationTesting', 'gaitAnalysis', 'speechLanguageAssessment', 'meningealSigns', 'cerebellarDysfunction', 'seizureActivity'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('neurological_examination', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[NeurologicalExamination] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('neurological_examination', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[NeurologicalExamination] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
