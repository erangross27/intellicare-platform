...
const { LRUCache } = require(path.resolve(__dirname, '../../../backend/node_modules/lru-cache'));
...
class DBOptimizationService {
  ...
  async initialize() {
    try {
      ...
      this.serviceToken = await serviceAccountManager.authenticate('db-optimization-service');
      ...
      console.log('✅ DB Optimization Service initialized');
      return true;
    } catch (error) {
      console.error('❌ DB Optimization Service initialization failed:', error);
      throw error;
    }
  }
  ...
  getServiceContext(practiceId = 'global') {
    return {
      serviceId: 'db-optimization-service',
      operation: 'db-optimization-operation',
...
const dbOptimizationService = new DBOptimizationService();

// Register with service proxy manager
if (typeof module !== 'undefined' && module.exports) {
  const proxy = getServiceProxy();
  proxy.registerService('dbOptimizationService', () => {
    return dbOptimizationService;
  });
}
