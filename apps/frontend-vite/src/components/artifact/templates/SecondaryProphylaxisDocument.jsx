/**
 * SecondaryProphylaxisDocument.jsx
 * June 2026 — FULL TEMPLATE STANDARD rewrite (100% coverage, all field types).
 * Collection: secondary_prophylaxis
 *
 * Field types:
 *   DATE      date (date-picker, also shown in record header)
 *   STRING    infection, medication, discontinuationCriteria, threshold, type,
 *             provider, facility, status (short) + findings, assessment, plan,
 *             notes, discontinuationCriteria (per-sentence narratives)
 *   OBJECT    results (recursive renderObjectNode/renderObjectLeaf)
 *   ARRAY     recommendations (array of {recommendation, date}, date-grouped)
 *
 * Sections:
 *   1. prophylaxis-info : infection, medication, discontinuationCriteria, threshold, type
 *   2. provider-info    : date, provider, facility
 *   3. clinical         : findings, assessment, plan
 *   4. results-section  : results
 *   5. recommendations-section : recommendations
 *   6. notes-status     : notes, status
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import SecondaryProphylaxisDocumentPDFTemplate from '../pdf-templates/SecondaryProphylaxisDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SecondaryProphylaxisDocument.css';
...
const SECTION_FIELDS = {
  'prophylaxis-info': ['infection', 'medication', 'discontinuationCriteria', 'threshold', 'type'],
...
const SECTION_ORDER = ['prophylaxis-info', 'provider-info', 'clinical', 'results-section', 'recommendations-section', 'notes-status'];

const DATE_FIELDS = ['date'];
/* short, single-value strings */
const STRING_FIELDS = ['infection', 'medication', 'threshold', 'type', 'provider', 'facility', 'status'];
/* per-sentence narrative strings */
const SENTENCE_FIELDS = ['discontinuationCriteria', 'findings', 'assessment', 'plan', 'notes'];
/* recursive display/edit object field */
const OBJECT_FIELDS = ['results'];
/* array-of-objects {recommendation, date} (date-grouped, editable) */
const OBJECT_ARRAY_FIELDS = ['recommendations'];

const KEY_OVERRIDES = { cd4: 'CD4', pcp: 'PCP', mac: 'MAC', hiv: 'HIV', id: 'ID' };
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits field, e.g. "findings" or "results") */
const DRAFT_KEY = 'secondaryProphylaxisPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const SecondaryProphylaxisDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.secondary_prophylaxis) return Array.isArray(r.secondary_prophylaxis) ? r.secondary_prophylaxis : [r.secondary_prophylaxis];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.secondary_prophylaxis) return Array.isArray(dd.secondary_prophylaxis) ? dd.secondary_prophylaxis : [dd.secondary_prophylaxis]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Secondary Prophylaxis ${idx + 1}`.toLowerCase();
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.cd4) — value stays a STRING */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    const editKey = `${rootField}-${idx}`;
    // Build the updated object (whole rootField) as a DRAFT — committed to DB only on Approve.
    setLocalEdits(prev => {
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Only this section's pending edits (key = "<field>-<idx>"; this file never appends ".<arrayIndex>")
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/secondary_prophylaxis/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/secondary_prophylaxis/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[SecondaryProphylaxis] Approve error:', err); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== SECONDARY PROPHYLAXIS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Secondary Prophylaxis ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: SIMPLE STRING FIELD (single-value, no per-sentence) ═══════ */
  ...
  /* ═══════ RENDER: NARRATIVE STRING FIELD (per-sentence) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="secondary-prophylaxis-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Secondary Prophylaxis</h2></div>
        <div className="empty-state">No secondary prophylaxis records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="secondary-prophylaxis-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Secondary Prophylaxis</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<SecondaryProphylaxisDocumentPDFTemplate document={pdfData} />} fileName={`secondary-prophylaxis-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search secondary prophylaxis..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(record.infection || `Secondary Prophylaxis ${idx + 1}`)}</h3>
            </div>
...
export default SecondaryProphylaxisDocument;
