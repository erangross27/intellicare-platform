/**
 * HepatitisCHistoryDocument.jsx
 * Canonical one-pass polish — blue glow editing theme
 * Collection: hepatitis_c_history
 *
 * Sections: Record Information · HCV Details · Coinfections · Liver Assessment ·
 *           Treatment History · Findings · Assessment · Plan · Laboratory Results · Notes
 *
 * Scalar string fields are inline-editable (mirrors the canonical renderStringField).
 * Arrays (previousTreatments/treatmentOutcomes/sideEffects/recommendations) and nested
 * objects (coinfections/results) are decomposed for display (label ABOVE value — never a
 * side-by-side "Label: value" span) and rendered read-only.
 * Display-only scalars (date/type/facility/status/genotype) are NOT in the backend
 * ALLOWED_FIELDS so they render as read rows (no edit widget).
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HepatitisCHistoryDocumentPDFTemplate from '../pdf-templates/HepatitisCHistoryDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './HepatitisCHistoryDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'hepatitis_c_historyPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  cirrhosisHistory: 'Cirrhosis History',
...
const SECTION_FIELDS = {
  ...
  'hcv-details': ['genotype', 'viralLoad', 'treatmentHistory'],
  'coinfections': ['coinfections'],
  'liver-assessment': ['fibrosisStage', 'cirrhosisHistory', 'liverBiopsy', 'hepatocellularCarcinoma'],
  'treatment-history': ['previousTreatments', 'treatmentOutcomes', 'sideEffects', 'complianceIssues', 'relapseHistory'],
  'findings': ['findings'],
  'assessment': ['assessment'],
  'plan': ['plan'],
  'results': ['results'],
...
/* Editable scalar strings (all backend-ALLOWED) — inline text editing */
const STRING_FIELDS = ['provider', 'viralLoad', 'treatmentHistory', 'fibrosisStage', 'cirrhosisHistory', 'liverBiopsy', 'hepatocellularCarcinoma', 'complianceIssues', 'relapseHistory', 'findings', 'assessment', 'plan', 'notes'];
/* Read-only scalars (not in backend ALLOWED_FIELDS) */
const DISPLAY_FIELDS = ['date', 'type', 'facility', 'status', 'genotype'];
/* Read-only, decomposed arrays + objects */
const ARRAY_FIELDS = ['previousTreatments', 'treatmentOutcomes', 'sideEffects', 'recommendations'];
const OBJECT_FIELDS = ['coinfections', 'results'];
const NUMBER_FIELDS = [];
const DATE_FIELDS = [];

/* ═══════ OBJECT-FIELD HELPERS ═══════ */
const KEY_OVERRIDES = {
  hiv: 'HIV', hbv: 'HBV', hcv: 'HCV', hav: 'HAV', hdv: 'HDV', ras: 'RAS', rna: 'RNA',
  il28b: 'IL28B', ns5a: 'NS5A', ns5b: 'NS5B', svr12: 'SVR12', svr24: 'SVR24',
  hepatitisb: 'Hepatitis B', hepatitisa: 'Hepatitis A', hepatitisd: 'Hepatitis D',
  std: 'STD', sti: 'STI', tb: 'TB',
};
...
/* ═══════ COMPONENT ═══════ */
const HepatitisCHistoryDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.hepatitis_c_history) return Array.isArray(r.hepatitis_c_history) ? r.hepatitis_c_history : [r.hepatitis_c_history];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.hepatitis_c_history) return Array.isArray(dd.hepatitis_c_history) ? dd.hepatitis_c_history : [dd.hepatitis_c_history]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Hepatitis C History ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/hepatitis_c_history/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/hepatitis_c_history/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_FIELDS.includes(f)) {
        ...
        const dv = f === 'date' ? formatDate(val) : fmtVal(val);
        if (showLabel) text += `${label}\n`;
        text += `${dv}\n\n`;
      } else {
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HEPATITIS C HISTORY ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Hepatitis C History ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderDisplayField = (record, fn, idx, sid, title) => {
    ...
    const showLabel = label.trim().toLowerCase() !== (title || '').trim().toLowerCase();
    const displayVal = fn === 'date' ? formatDate(val) : fmtVal(val);
    const editKey = `${fn}-${idx}`;
    return (
  ...
  /* ═══════ RENDER: OBJECT FIELD (coinfections / results — read-only) ═══════ */
  ...
  /* ═══════ RENDER: DATE FIELD (canonical — unused: DATE_FIELDS empty) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="hepatitis-c-history-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Hepatitis C History</h2></div>
        <div className="empty-state">No hepatitis C history records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="hepatitis-c-history-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Hepatitis C History</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HepatitisCHistoryDocumentPDFTemplate document={pdfData} />} fileName="Hepatitis_C_History.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search hepatitis C history..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Hepatitis C History ${idx + 1}`)}</h3>
            </div>
...
export default HepatitisCHistoryDocument;
