/**
 * GlucoseTestingWeeksDocument.jsx
 * June 2026 — Collection: glucose_testing_weeks (NEW template)
 *
 * Donor cloned (identical generic shape):
 *  - PointOfCareUltrasoundHeartRateDocument: DATE_FIELDS + renderDateField (record-header date),
 *    date+provider+facility header; OBJECT recursive render (renderObjectField / renderObjectNode /
 *    renderObjectLeaf, humanizeKey + nested-mini-card, hide-empty at every level);
 *    ARRAY-of-{recommendation,date} renderRecommendationsField (date-grouped, numbered list);
 *    STRING per-sentence renderStringField; 4-level search; per-section approve;
 ...
 *   OBJECT: oneHourTest, threeHourTest, results (recursive key-value)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import GlucoseTestingWeeksDocumentPDFTemplate from '../pdf-templates/GlucoseTestingWeeksDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './GlucoseTestingWeeksDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = root field name, e.g. "results") */
const DRAFT_KEY = 'glucose_testing_weeksPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'one-hour-test': '1-Hour Test',
  'three-hour-test': '3-Hour Test',
  'clinical': 'Clinical',
...
const FIELD_LABELS = {
  ...
  notes: 'Notes',
  status: 'Status',
  oneHourTest: '1-Hour Test',
  threeHourTest: '3-Hour Test',
  results: 'Results',
  recommendations: 'Recommendations',
};

const SECTION_FIELDS = {
  'one-hour-test': ['oneHourTest'],
  'three-hour-test': ['threeHourTest'],
...
const SECTION_ORDER = ['one-hour-test', 'three-hour-test', 'clinical', 'results-section', 'recommendations-section', 'notes-status'];

const DATE_FIELDS = ['date'];
const STRING_FIELDS = ['type', 'provider', 'facility', 'findings', 'assessment', 'plan', 'notes', 'status'];
/* header-metadata fields shown in the record header (not in any section) — included in record-level search */
const HEADER_FIELDS = ['date', 'provider', 'facility'];
/* DISPLAY/EDIT recursive object fields */
const OBJECT_FIELDS = ['oneHourTest', 'threeHourTest', 'results'];
/* Array-of-objects {recommendation, date} (date-grouped, editable) */
const OBJECT_ARRAY_FIELDS = ['recommendations'];

const KEY_OVERRIDES = {
  ogtt: 'OGTT', gtt: 'GTT', mg: 'mg', dl: 'dL', mgdl: 'mg/dL', bg: 'BG', fbs: 'FBS', gdm: 'GDM',
};
...
/* ═══════ COMPONENT ═══════ */
const GlucoseTestingWeeksDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.glucose_testing_weeks) return Array.isArray(r.glucose_testing_weeks) ? r.glucose_testing_weeks : [r.glucose_testing_weeks];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.glucose_testing_weeks) return Array.isArray(dd.glucose_testing_weeks) ? dd.glucose_testing_weeks : [dd.glucose_testing_weeks]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `glucose tolerance test ${idx + 1}`.toLowerCase();
      if (rt.includes(phrase) || phrase.includes(rt)) { record._showAllSections = true; return true; }
      for (const t of Object.values(SECTION_TITLES)) { if (t.toLowerCase().includes(phrase) || phrase.includes(t.toLowerCase())) return true; }
      for (const l of Object.values(FIELD_LABELS)) { if (l.toLowerCase().includes(phrase) || phrase.includes(l.toLowerCase())) return true; }
      /* header-metadata fields (date/provider/facility) are displayed but not in any section — scan their content too */
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.fasting) — value stays a STRING */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    // localEdits keys for this section's fields are exactly `${field}-${idx}` (root field; whole value).
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/glucose_testing_weeks/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/glucose_testing_weeks/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[GlucoseTestingWeeks] Approve error:', err); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== GLUCOSE TOLERANCE TEST ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Glucose Tolerance Test ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="glucose-testing-weeks-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Glucose Tolerance Test</h2></div>
        <div className="empty-state">No glucose tolerance test records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="glucose-testing-weeks-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Glucose Tolerance Test</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<GlucoseTestingWeeksDocumentPDFTemplate document={pdfData} />} fileName={`glucose-tolerance-test-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search glucose tolerance test records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Glucose Tolerance Test ${idx + 1}`)}</h3>
            </div>
...
export default GlucoseTestingWeeksDocument;
