/**
 * NephrologyConsultationDetailsDocument.jsx
 * Canonical one-pass polish — blue glow editing theme
 * Collection: nephrology_consultation_details
 *
 * Sections: Record Information · Kidney Disease Progression Timeline · Dialysis Planning ·
 *           Prognosis Discussion · Assessment · Plan
 *
 * Scalar string fields (provider/facility/status + the long narratives) are inline-editable
 * (canonical renderStringField). The record `date` is editable via BlueDatePicker.
 * The deepest structures — kidneyDiseaseProgressionTimeline (array of objects) and
 * dialysisPlanning (3-level nested object) — are decomposed by the generic recursive
 * renderer (label ABOVE value at EVERY depth; boolean leaves show Yes/No) and rendered
 * read-only. Empty strings / 0 / empty arrays / empty objects are skipped, not rendered blank.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NephrologyConsultationDetailsDocumentPDFTemplate from '../pdf-templates/NephrologyConsultationDetailsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './NephrologyConsultationDetailsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'nephrology_consultation_detailsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['provider', 'facility', 'status', 'estimatedTimeToDialysis', 'prognosisDiscussion', 'assessment', 'plan'];
/* Editable date (record date) */
const DATE_FIELDS = ['date'];
/* Read-only, decomposed nested structures */
const OBJECT_ARRAY_FIELDS = ['kidneyDiseaseProgressionTimeline']; // array of {year, eGFR, uacr, stage}
const OBJECT_FIELDS = ['dialysisPlanning'];                        // 3-level nested object
const ARRAY_FIELDS = [];                                           // (no top-level scalar arrays)
const NUMBER_FIELDS = [];
...
/* ═══════ COMPONENT ═══════ */
const NephrologyConsultationDetailsDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.nephrology_consultation_details) return Array.isArray(r.nephrology_consultation_details) ? r.nephrology_consultation_details : [r.nephrology_consultation_details];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.nephrology_consultation_details) return Array.isArray(dd.nephrology_consultation_details) ? dd.nephrology_consultation_details : [dd.nephrology_consultation_details]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Nephrology Consultation Details ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/nephrology_consultation_details/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/nephrology_consultation_details/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_ARRAY_FIELDS.includes(f)) {
        ...
        items.forEach((item) => { const lines = []; objectCopyBlock(item, lines, 0); lines.forEach(l => { text += `${l}\n`; }); text += '\n'; });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NEPHROLOGY CONSULTATION DETAILS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Nephrology Consultation Details ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT FIELD (dialysisPlanning — read-only, recursive) ═══════ */
  ...
  /* ═══════ RENDER: OBJECT-ARRAY FIELD (kidneyDiseaseProgressionTimeline — read-only) ═══════
     Array of {year, eGFR, uacr, stage}. Each object → a nested card whose sub-fields render
     label ABOVE value (eGFR shows as a number leaf; never a side-by-side "Key: value" span). */
      ...
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, i) => (
          <div key={i} className="nested-mini-card">
            {Object.entries(item).filter(([, v]) => !isEmptyDeep(v)).map(([k, v]) => renderObjectNode(humanizeKey(k), v, `${fn}-${idx}-${i}-${k}`))}
          </div>
        ))}
      </div>
    );
  ...
  /* ═══════ RENDER: ARRAY FIELD (read-only, decomposed scalar list) ═══════ */
  ...
  /* ═══════ RENDER: DATE FIELD (canonical BlueDatePicker) ═══════ */
  ...
  /* ═══════ RENDER: NUMBER FIELD (canonical num-stepper — unused: NUMBER_FIELDS empty) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="nephrology-consultation-details-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Nephrology Consultation Details</h2></div>
        <div className="empty-state">No nephrology consultation details records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="nephrology-consultation-details-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Nephrology Consultation Details</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NephrologyConsultationDetailsDocumentPDFTemplate document={pdfData} />} fileName="Nephrology_Consultation_Details.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search nephrology consultation details..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Nephrology Consultation Details ${idx + 1}`)}</h3>
            </div>
...
export default NephrologyConsultationDetailsDocument;
