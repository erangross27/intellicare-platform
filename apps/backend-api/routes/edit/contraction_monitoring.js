...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'contraction-monitoring-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'provider', 'facility', 'date', 'type', 'status',
  'findings', 'assessment', 'plan', 'notes', 'pretermLaborRisk',
  'tocolytics', 'recommendations',
  'trueLabor.contractionFrequency', 'trueLabor.duration', 'trueLabor.intensity', 'trueLabor.cervicalChange',
  // OBJECT roots — dotted sub-paths (e.g. 'braxtonHicks.present', 'results.<key>') allowed via rootField check
  'braxtonHicks', 'results',
];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('contraction_monitoring', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[contraction_monitoring] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('contraction_monitoring', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[contraction_monitoring] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
