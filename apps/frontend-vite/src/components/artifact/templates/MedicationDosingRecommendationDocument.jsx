/**
 * MedicationDosingRecommendationDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: medication_dosing_recommendation
 *
 * 6 Sections (26 non-system fields + date — 100% schema coverage):
 *   1. medication-identity: medicationGenericName, medicationBrandNames, therapeuticClass, indicationForUse
 *   2. standard-dosing: standardAdultDose, pediatricDosingWeight, maximumDailyDose, routeOfAdministration, infusionRateRecommendation, loadingDoseRequired, loadingDoseAmount
 *   3. dose-adjustments: renalDoseAdjustment, hepaticDoseAdjustment, creatinineClearanceThreshold, geriatricDoseConsiderations, dialysisDosingGuidance
 *   4. weight-surface-dosing: bodyWeightBasedDosing, bodySurfaceAreaDosing
 *   5. monitoring-interactions: therapeuticDrugMonitoring, targetTroughLevel, targetPeakLevel, aucTargetRange, halfLifeHours, dosingIntervalHours, cytochromeP450Interactions
 *   6. safety: blackBoxWarningDoseRelated
 *
 * Typed editing: BOOLEAN(4) Yes/No select, NUMBER(3) numeric input, ARRAY(2) per-item, STRING(17) per-sentence/simple.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MedicationDosingRecommendationDocumentPDFTemplate from '../pdf-templates/MedicationDosingRecommendationDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './MedicationDosingRecommendationDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" — whole-field values) */
const DRAFT_KEY = 'medication_dosing_recommendationPendingEdits';
const readDrafts = () => {
...
const OBJECT_FIELDS = [];
/* Per-sentence narrative strings (splitBySentence editing) */
const SENTENCE_FIELDS = ['indicationForUse', 'renalDoseAdjustment', 'hepaticDoseAdjustment', 'geriatricDoseConsiderations', 'dialysisDosingGuidance', 'blackBoxWarningDoseRelated'];
/* Short/simple strings (single-line editing) */
const SIMPLE_STRING_FIELDS = ['medicationGenericName', 'therapeuticClass', 'standardAdultDose', 'pediatricDosingWeight', 'maximumDailyDose', 'routeOfAdministration', 'infusionRateRecommendation', 'loadingDoseAmount', 'targetTroughLevel', 'targetPeakLevel', 'aucTargetRange'];
...
/* ═══════ COMPONENT ═══════ */
const MedicationDosingRecommendationDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.medication_dosing_recommendation) return Array.isArray(r.medication_dosing_recommendation) ? r.medication_dosing_recommendation : [r.medication_dosing_recommendation];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.medication_dosing_recommendation) return Array.isArray(dd.medication_dosing_recommendation) ? dd.medication_dosing_recommendation : [dd.medication_dosing_recommendation]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Dosing Recommendation ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Pending edits for this section: localEdits key = "<fieldPart>-<idx>", fieldPart's base field ∈ section fields.
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/medication_dosing_recommendation/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/medication_dosing_recommendation/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      /* hide-zero: numeric 0 is empty per medical convention (consistent with JSX render + PDF) */
      if (NUMBER_FIELDS.includes(f)) { const n = Number(val); if (isNaN(n) || n === 0) return; }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MEDICATION DOSING RECOMMENDATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Dosing Recommendation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const buildReturnSummary = useCallback(() => {
    return filteredRecords.map((record, idx) => {
      const name = record.medicationGenericName || `Dosing Recommendation ${idx + 1}`;
      const brand = Array.isArray(record.medicationBrandNames) ? record.medicationBrandNames.join(', ') : '';
      const dose = record.standardAdultDose || '';
      const route = record.routeOfAdministration || '';
      const cls = record.therapeuticClass || '';
      const parts = [name];
  ...
  if (!records || records.length === 0) {
    return (
      <div className="medication-dosing-recommendation-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Medication Dosing Recommendations</h2></div>
        <div className="empty-state">No medication dosing recommendations available</div>
      </div>
    );
  }

  return (
    <div className="medication-dosing-recommendation-document" ref={containerRef} data-summary={buildReturnSummary()}>
      <div className="document-header">
        <h2 className="document-title">Medication Dosing Recommendations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MedicationDosingRecommendationDocumentPDFTemplate document={pdfData} />} fileName="Medication_Dosing_Recommendations.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search medication dosing recommendations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(record.medicationGenericName || `Dosing Recommendation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'medication-identity')}
            {renderSection(record, idx, 'standard-dosing')}
            {renderSection(record, idx, 'dose-adjustments')}
            {renderSection(record, idx, 'weight-surface-dosing')}
            {renderSection(record, idx, 'monitoring-interactions')}
...
export default MedicationDosingRecommendationDocument;
