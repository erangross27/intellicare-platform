/**
 * BleedingRiskAssessmentDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: bleeding_risk_assessment
 *
 * 5 Sections:
 *   1. overview: date, bleedingRiskScore, riskCategory, assessmentTool, reassessmentDate
 *   2. anticoagulation: anticoagulantTherapy, indicationForAnticoagulation, concomitantMedications
 *   3. risk-factors: hypertensionUncontrolled, renalDysfunction, liverDysfunction, priorMajorBleeding,
 *      priorBleedingDetails, strokeHistory, labileInr, ageRiskFactor, alcoholAbuse, fallRisk
 *   4. labs: creatinineClearance, currentInr, plateletCount, thrombocytopenia, anemiaPresent, hemoglobinLevel
 *   5. mitigation: mitigationStrategies, geneticFactors
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import BleedingRiskAssessmentDocumentPDFTemplate from '../pdf-templates/BleedingRiskAssessmentDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './BleedingRiskAssessmentDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldName]: value } }  (value is a string, or an array for array fields) */
const DRAFT_KEY = 'bleeding_risk_assessmentPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'risk-factors': 'Bleeding Risk Factors',
  'labs': 'Laboratory Values',
  'mitigation': 'Mitigation & Notes',
};
...
const FIELD_LABELS = {
  ...
  anticoagulantTherapy: 'Anticoagulant Therapy',
  indicationForAnticoagulation: 'Indication for Anticoagulation',
  concomitantMedications: 'Concomitant Medications',
  hypertensionUncontrolled: 'Uncontrolled Hypertension',
  renalDysfunction: 'Renal Dysfunction',
  liverDysfunction: 'Liver Dysfunction',
  priorMajorBleeding: 'Prior Major Bleeding',
  priorBleedingDetails: 'Prior Bleeding Details',
  strokeHistory: 'Stroke History',
  labileInr: 'Labile INR',
  ageRiskFactor: 'Age Risk Factor (>65)',
  alcoholAbuse: 'Alcohol Abuse',
  fallRisk: 'Fall Risk',
  creatinineClearance: 'Creatinine Clearance (mL/min)',
  currentInr: 'Current INR',
  plateletCount: 'Platelet Count (x10⁹/L)',
  thrombocytopenia: 'Thrombocytopenia',
  anemiaPresent: 'Anemia Present',
...
const SECTION_FIELDS = {
  'overview': ['date', 'bleedingRiskScore', 'riskCategory', 'assessmentTool', 'reassessmentDate'],
  'anticoagulation': ['anticoagulantTherapy', 'indicationForAnticoagulation', 'concomitantMedications'],
  'risk-factors': ['hypertensionUncontrolled', 'renalDysfunction', 'liverDysfunction', 'priorMajorBleeding', 'priorBleedingDetails', 'strokeHistory', 'labileInr', 'ageRiskFactor', 'alcoholAbuse', 'fallRisk'],
  'labs': ['creatinineClearance', 'currentInr', 'plateletCount', 'thrombocytopenia', 'anemiaPresent', 'hemoglobinLevel'],
  'mitigation': ['mitigationStrategies', 'geneticFactors'],
};

const NUMBER_FIELDS = ['bleedingRiskScore', 'creatinineClearance', 'currentInr', 'plateletCount', 'hemoglobinLevel'];
...
const ARRAY_FIELDS = ['anticoagulantTherapy', 'concomitantMedications', 'mitigationStrategies'];
const BOOLEAN_FIELDS = ['hypertensionUncontrolled', 'renalDysfunction', 'liverDysfunction', 'priorMajorBleeding', 'strokeHistory', 'labileInr', 'ageRiskFactor', 'alcoholAbuse', 'fallRisk', 'thrombocytopenia', 'anemiaPresent'];
const STRING_FIELDS = ['date', 'riskCategory', 'assessmentTool', 'reassessmentDate', 'indicationForAnticoagulation', 'priorBleedingDetails', 'geneticFactors'];
const DATE_FIELDS = ['date', 'reassessmentDate'];
...
/* ═══════ COMPONENT ═══════ */
const BleedingRiskAssessmentDocument = ({ document: docProp }) => {
  ...
  // Maps each draft record by its _id (handling _id.$oid) to its render index.
  useEffect(() => {
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        ...
        else if (DATE_FIELDS.includes(f) && formatDate(val).toLowerCase().includes(phrase)) return true;
      }
    }
    return false;
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Bleeding Risk Assessment ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (val && (Array.isArray(val) ? val.some(item => String(item).toLowerCase().includes(phrase)) : (fmtVal(val).toLowerCase().includes(phrase) || (DATE_FIELDS.includes(f) && formatDate(val).toLowerCase().includes(phrase))))) return true;
        }
      }
      return false;
    });
  ...
  // Save one sentence = stage the rebuilt full field text as a DRAFT (no DB write). Approve commits it.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    // Collect this record's pending edits for fields in this section (localEdits key = `${fn}-${idx}`).
    const toCommit = fields.filter(f => pendingEdits[`${f}-${idx}`]);
    setSaving(true); setSaveError(null);
    try {
      for (const fn of toCommit) {
        const editKey = `${fn}-${idx}`;
        const value = localEdits[editKey];
        // Array fields stage the whole array → PUT each element with arrayIndex; scalars PUT field+value.
        if (Array.isArray(value)) {
          for (let ai = 0; ai < value.length; ai++) {
            await secureApiClient.put(`/api/edit/bleeding_risk_assessment/${id}/edit`, { field: fn, value: value[ai], arrayIndex: ai });
          }
        } else {
          await secureApiClient.put(`/api/edit/bleeding_risk_assessment/${id}/edit`, { field: fn, value });
        }
      }
      await secureApiClient.put(`/api/edit/bleeding_risk_assessment/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== BLEEDING RISK ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Bleeding Risk Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: DATE FIELD (string field, locale-formatted display) ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(rawVal); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              <input type="text" className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
  ...
  if (!records || records.length === 0) {
    return (
      <div className="bleeding-risk-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Bleeding Risk Assessment</h2></div>
        <div className="empty-state">No bleeding risk assessment data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="bleeding-risk-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Bleeding Risk Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<BleedingRiskAssessmentDocumentPDFTemplate document={pdfData} />} fileName={`bleeding-risk-assessment-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search bleeding risk assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Bleeding Risk Assessment ${idx + 1}`)}</h3>
            </div>
            ...
            {renderSection(record, idx, 'mitigation')}
          </div>
        ))}
      </div>
    </div>
...
export default BleedingRiskAssessmentDocument;
