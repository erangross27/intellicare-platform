...
/**
 * NonMotorSymptomsDocumentPDFTemplate - box-free canonical (LETTER)
 * Config-driven from the JSX SECTION_ORDER/SECTION_TITLES/FIELD_LABELS/SECTION_FIELDS.
 * Renders EVERY populated field the JSX renders for JSX/PDF field parity.
 * No boxes: underline rules only (documentTitle 2 / recordTitle+sectionTitle 1 / fieldLabel 0.5).
 * Deeply nested symptom objects (cognitive/neuropsychiatric{hallucinations{...}}/sleep/autonomic/
 * sensory/results) render via a generic recursive renderer; inline "Key: value" is allowed in the
 * PDF only. Record date uses record.date (NEVER createdAt/updatedAt).
 */
...
const SECTION_ORDER = ['record-info', 'cognitive', 'neuropsychiatric', 'sleep', 'autonomic', 'sensory', 'findings', 'assessment', 'plan', 'results', 'notes'];
...
/* COMMA_LIST_FIELDS: unlabeled comma-separated lists decomposed into numbered items */
...
const sentenceRows = (text, fn) => {
  ...
  splitBySentence(text).forEach(sentence => {
    ...
    if (p.isLabeled) {
      ...
      if (items.length >= 2) items.forEach(it => rows.push({ type: 'item', text: it }));
...
const fieldBody = (record, f) => {
  ...
  const rows = sentenceRows(String(v), f);
...
const NonMotorSymptomsDocumentPDFTemplate = ({ document: docProp, data = docProp }) => {
  let records = [];
  if (Array.isArray(data)) records = data;
  else if (data && typeof data === 'object') {
    if (data.non_motor_symptoms) records = Array.isArray(data.non_motor_symptoms) ? data.non_motor_symptoms : [data.non_motor_symptoms];
    else records = [data];
  }
    ...
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <Text style={styles.documentTitle}>Non-Motor Symptoms</Text>
          <Text style={styles.noData}>No non-motor symptoms records available.</Text>
        </Page>
      </Document>
    );
  }
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, rIdx) => (
          <View key={rIdx}>
            <Text style={styles.recordTitle} break={rIdx > 0}>{safeString(`Non-Motor Symptoms ${rIdx + 1}`)}</Text>
...
export default NonMotorSymptomsDocumentPDFTemplate;
