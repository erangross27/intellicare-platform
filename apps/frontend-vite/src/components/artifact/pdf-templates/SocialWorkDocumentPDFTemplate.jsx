...
const SECTIONS = [
  { id: 'record', title: 'Record Details', fields: ['date', 'provider', 'facility'] }, { id: 'referral', title: 'Referral and Primary Concern', fields: ['referralSource', 'primaryConcern'] },
  { id: 'housing', title: 'Housing and Daily Support', fields: ['livingArrangement', 'primaryCaregiver', 'homeSafetyAssessment', 'transportationNeeds'] }, { id: 'financial', title: 'Insurance and Financial Concerns', fields: ['insuranceStatus', 'financialConcerns'] },
  { id: 'psychosocial', title: 'Psychosocial Assessment', fields: ['mentalHealthHistory', 'substanceUseHistory', 'supportSystem', 'culturalConsiderations'] }, { id: 'resources', title: 'Community Resources', fields: ['communityResourcesProvided'] },
  { id: 'discharge', title: 'Discharge and Case Management', fields: ['dischargeDisposition', 'barriersToDischarge', 'caseManagementLevel', 'guardianshipStatus', 'advanceDirectives', 'followUpRequired'] }, { id: 'safety', title: 'Safety and Functional Status', fields: ['safetyRiskAssessment', 'functionalStatus'] },
  { id: 'interventions', title: 'Interventions Provided', fields: ['interventionsProvided'] }, { id: 'goals', title: 'Patient Goals', fields: ['patientGoals'] },
];
const FIELD_LABELS = {
...
export default function SocialWorkDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Social Work</Text></View>
    {!records.length && <Text style={styles.noDataText}>No social work data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Social Work Record {recordIndex + 1}</Text></View>
