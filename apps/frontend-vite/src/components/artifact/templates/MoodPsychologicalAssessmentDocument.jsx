/**
 * MoodPsychologicalAssessmentDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: mood_psychological_assessment
 *
 * 8 Sections:
 *   1. assessment-info: date, type, provider, facility
 *   2. scores: gdsScore, gadScore, phq9Score
 *   3. sleep-pattern: sleepPattern.hoursPerNight, sleepPattern.nighttimeAwakenings, sleepPattern.sleepQuality
 *   4. psychosocial: socialIsolation, griefLoss
 *   5. findings: findings
 *   6. assessment: assessment
 *   7. plan: plan
 *   8. recommendations-notes: recommendations, notes, status
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MoodPsychologicalAssessmentDocumentPDFTemplate from '../pdf-templates/MoodPsychologicalAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './MoodPsychologicalAssessmentDocument.css';
...
/* LIST_FIELDS: comma-delimited enumerations decomposed into one editable row per item
   (numbered only in Copy/PDF). This user prefers aggressive splitting for narrative lists. */
const LIST_FIELDS = ['socialIsolation', 'findings'];
const STRING_FIELDS = ['type', 'provider', 'facility', 'gdsScore', 'gadScore', 'phq9Score', 'sleepPattern.nighttimeAwakenings', 'sleepPattern.sleepQuality', 'assessment', 'plan', 'notes', 'status'];
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [editKey]: { value, payload } } }
     - editKey = the localEdits key ("field-idx"); value = the localEdits value (string OR full array/object)
     - payload = the exact PUT body to replay on Approve ({ field, value, [arrayIndex] }) */
const DRAFT_KEY = 'mood_psychological_assessmentPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const MoodPsychologicalAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.mood_psychological_assessment) return Array.isArray(r.mood_psychological_assessment) ? r.mood_psychological_assessment : [r.mood_psychological_assessment];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.mood_psychological_assessment) return Array.isArray(dd.mood_psychological_assessment) ? dd.mood_psychological_assessment : [dd.mood_psychological_assessment]; return [dd]; }
  ...
  useEffect(() => {
    ...
    const recId = (r) => { if (!r || !r._id) return null; if (typeof r._id === 'string') return r._id; if (r._id.$oid) return r._id.$oid; return String(r._id); };
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([editKey, draft]) => {
        // editKey is "field-idx" captured at save time; remap its idx to the current render index.
        const dashIdx = editKey.lastIndexOf('-');
        const fieldPart = dashIdx === -1 ? editKey : editKey.slice(0, dashIdx);
        const newKey = `${fieldPart}-${idx}`;
        nLocal[newKey] = draft && Object.prototype.hasOwnProperty.call(draft, 'value') ? draft.value : draft;
        nPending[newKey] = true;
        nFields[newKey] = 'edited';
      });
    });
  ...
  /* getFieldValue: supports dot-path fields like sleepPattern.hoursPerNight */
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (Array.isArray(val)) {
          if (val.some(item => {
            const itemText = typeof item === 'string' ? item : (item?.recommendation || item?.text || JSON.stringify(item));
            return String(itemText).toLowerCase().includes(phrase);
          })) return true;
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Mood & Psychological Assessment ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (val && (Array.isArray(val) ? val.some(item => {
            const itemText = typeof item === 'string' ? item : (item?.recommendation || item?.text || JSON.stringify(item));
            return String(itemText).toLowerCase().includes(phrase);
          }) : (typeof val === 'object' ? flattenSearchable(val).toLowerCase().includes(phrase) : fmtVal(val).toLowerCase().includes(phrase)))) return true;
        }
      }
      return false;
    });
  ...
  const pdfData = useMemo(() => {
    return filteredRecords.map((record, idx) => {
      const merged = { ...record };
      if (merged.sleepPattern) merged.sleepPattern = { ...merged.sleepPattern };
      Object.keys(localEdits).forEach(key => {
        ...
        if (m && parseInt(m[2]) === idx) {
          ...
          if (fieldName.includes('.')) {
            ...
            if (parts[0] === 'sleepPattern' && merged.sleepPattern) {
              merged.sleepPattern[parts[1]] = localEdits[key];
            }
          } else {
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* stageDraft = the single defer-save primitive. Writes the edit to localEdits + the pending-drafts
     localStorage store ONLY (survives refresh, shown in JSX). NO DB write — Approve commits later.
     - editKey   = the localEdits key ("field-idx"); localValue = the value stored in localEdits
     - payload   = the exact PUT body to replay on Approve ({ field, value, [arrayIndex] })
     - trackKeys = editedFields keys to mark 'edited' (drives the yellow Pending Approve button) */
  const stageDraft = useCallback((record, editKey, localValue, payload, trackKeys) => {
    const id = safeId(record); if (!id) return false;
    setLocalEdits(prev => ({ ...prev, [editKey]: localValue }));
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
    if (trackKeys && trackKeys.length) setEditedFields(prev => { const n = { ...prev }; trackKeys.forEach(k => { n[k] = 'edited'; }); return n; });
    const store = readDrafts();
    if (!store[id]) store[id] = {};
    store[id][editKey] = { value: localValue, payload };
    writeDrafts(store);
    return true;
  }, [safeId]);
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    stageDraft(record, editKey, saveVal, { field: fn, value: saveVal }, [trackKey]);
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageDraft(record, editKey, fullText, { field: fn, value: fullText }, []);
  ...
  /* saveCommaItem: save individual comma-delimited item within a sentence */
  function saveCommaItem(record, fn, idx, sid, sIdx, ciIdx) {
    ...
    const editKey = `${fn}-${idx}`;
    // Stage a DRAFT (no DB write). Approve commits the full field text.
    stageDraft(record, editKey, fullText, { field: fn, value: fullText }, []);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      // localEdits keys for this record's section fields that are still pending ("field-idx")
      const toCommit = fields
        .map(f => `${f}-${idx}`)
        .filter(editKey => pendingEdits[editKey] && localEdits[editKey] !== undefined);
      // Persist each staged field to the DB now by replaying its stored PUT payload.
      for (const editKey of toCommit) {
        const draft = recDrafts[editKey];
        const payload = draft && draft.payload ? draft.payload : { field: editKey.slice(0, editKey.lastIndexOf('-')), value: localEdits[editKey] };
        const resp = await secureApiClient.put(`/api/edit/mood_psychological_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/mood_psychological_assessment/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        const items = splitByComma(fmtVal(val)).filter(s => s && s.trim());
        ...
        const items = Array.isArray(val) ? val : [val];
        body += `${label}\n${items.map((item, i) => {
          const recText = typeof item === 'string' ? item : item?.recommendation || item?.text || JSON.stringify(item);
          return `${i + 1}. ${recText}`;
        }).join('\n')}\n\n`;
      } else if (OBJECT_FIELDS.includes(f)) {
        if (isEmptyDeep(val)) return;
        const lines = (lbl, value, indent) => {
          const pad = '  '.repeat(indent); let out = '';
          if (isEmptyDeep(value)) return out;
          if (isScalar(value)) return `${pad}${lbl ? lbl + ': ' : ''}${fmtScalar(value)}\n`;
          if (lbl) out += `${pad}${lbl}:\n`;
          Object.entries(value).filter(([, v]) => !isEmptyDeep(v)).forEach(([k, v]) => { out += lines(humanizeKey(k), v, indent + (lbl ? 1 : 0)); });
          return out;
        };
        let objBody = '';
        Object.entries(val).filter(([, v]) => !isEmptyDeep(v)).forEach(([k, v]) => { objBody += lines(humanizeKey(k), v, 0); });
        if (objBody) body += `${label}\n${objBody}\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MOOD & PSYCHOLOGICAL ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Mood & Psychological Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; stageDraft(record, `${fn}-${idx}`, currentArr, { field: fn, value: editValue, arrayIndex: itemIdx }, [editKey]); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(recText); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const newItem = typeof item === 'string' ? editValue : { ...item, recommendation: editValue }; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = newItem; stageDraft(record, `${fn}-${idx}`, currentArr, { field: fn, value: newItem, arrayIndex: itemIdx }, [editKey]); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  /* ═══════ RENDER: DYNAMIC-KEY OBJECT FIELD (results) — recursive, editable leaves ═══════ */
  const saveObjectLeaf = useCallback((record, rootField, path, idx, leafKey, newVal) => {
    const id = safeId(record); if (!id) return;
    ...
    // Stage a DRAFT (no DB write). On Approve, replay the dotted-field PUT for this leaf.
    stageDraft(record, `${rootField}-${idx}`, clone, { field: dottedField, value: newVal }, [leafKey]);
    setEditingField(null); setEditValue('');
  ...
  const renderObjectLeaf = (record, rootField, path, idx, value) => {
    ...
    return (
      <div key={path.join('.')} className="nested-mini-card">
        ...
        <div className={`numbered-row editable-row${isModified ? ' modified' : ''}`} onClick={() => { if (!isEditing) { setEditingField(leafKey); setEditValue(leafValueString); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus disabled={saving} onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveObjectLeaf(record, rootField, path, idx, leafKey, editValue.trim()); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
            <>
              ...
              <button className={`copy-btn ${copiedItems[leafKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${humanizeKey(path[path.length - 1])}: ${leafValueString}`, leafKey); }}>{copiedItems[leafKey] ? 'Copied!' : 'Copy'}</button>
            </>
          )}
        </div>
        {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
  ...
  /* ═══════ RENDER: LIST FIELD — comma-delimited string → one editable row per item ═══════ */
  const renderListField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx); if (!hasVal(val)) return null;
    const items = splitByComma(fmtVal(val)).filter(s => s && s.trim());
    if (items.length === 0) return null;
    const label = FIELD_LABELS[fn] || fn;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(item); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentItems = splitByComma(String(getFieldValue(record, fn, idx) || '')).filter(s => s && s.trim()); const trimmed = editValue.trim(); if (!trimmed) { currentItems.splice(itemIdx, 1); } else { currentItems[itemIdx] = trimmed; } const joined = currentItems.join(', '); stageDraft(record, `${fn}-${idx}`, joined, { field: fn, value: joined }, [editKey]); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (LIST_FIELDS.includes(f)) return renderListField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="mood-psychological-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Mood & Psychological Assessment</h2></div>
        <div className="empty-state">No mood & psychological assessment data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="mood-psychological-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Mood & Psychological Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MoodPsychologicalAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Mood_Psychological_Assessment.pdf" className="copy-btn">
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-title">{highlightText(`Mood & Psychological Assessment ${idx + 1}`)}</h3>
            </div>
...
export default MoodPsychologicalAssessmentDocument;
