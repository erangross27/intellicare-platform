...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'school-health-forms-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'studentMedicalRecordNumber',
  'primaryCarePhysician',
  'bodyMassIndexPercentile',
  'visionScreeningResults',
  'hearingScreeningDecibels',
  'scoliosisScreeningFindings',
  'tuberculosisScreeningStatus',
  'immunizationRecords',
  'allergicReactionsHistory',
  'currentMedications',
  'emergencyMedicationOrders',
  'chronicMedicalConditions',
  'physicalEducationRestrictions',
  'individualized504Plan',
  'emergencyActionPlan',
  'parentalMedicalConsent',
  'specialistReferrals',
  'mentalHealthServices',
  'communicableDiseaseHistory',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('school_health_forms', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[school_health_forms] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('school_health_forms', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[school_health_forms] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
