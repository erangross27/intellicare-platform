/**
 * PsychosocialSupportServicesDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: psychosocial_support_services
 *
 * 5 Sections:
 *   1. screening-scores: phq9Score, gad7Score, columbiaScore, auditScore, aceScore
 *   2. clinical-assessment: primaryDiagnosis, suicidalIdeation, substanceUseDisorder, traumaHistory, crisisInterventionProvided
 *   3. treatment-support: therapyModalityProvided, copingMechanisms, referralsMade, treatmentAdherence, supportSystemStrength
 *   4. functional-cognitive: functionalAssessment, cognitiveFunction
 *   5. social-factors: socialDeterminantsScreening, housingStability, culturalConsiderations, dischargeDisposition
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PsychosocialSupportServicesDocumentPDFTemplate from '../pdf-templates/PsychosocialSupportServicesDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PsychosocialSupportServicesDocument.css';
...
const STRING_FIELDS = ['primaryDiagnosis', 'treatmentAdherence', 'supportSystemStrength', 'functionalAssessment', 'cognitiveFunction', 'socialDeterminantsScreening', 'housingStability', 'culturalConsiderations', 'dischargeDisposition'];
...
/* isHiddenZero: a NUMBER field whose value is 0 is an extractor "not-assessed" sentinel (hidden everywhere)
   UNLESS the field is listed in MEANINGFUL_ZERO_FIELDS (a real, reportable 0). */
...
const DRAFT_KEY = 'psychosocial_support_servicesPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const PsychosocialSupportServicesDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Psychosocial Support Services ${idx + 1}`.toLowerCase();
  ...
  // Save one sentence = stage a DRAFT locally (full text), write to localStorage. No DB write until Approve.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/psychosocial_support_services/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/psychosocial_support_services/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PSYCHOSOCIAL SUPPORT SERVICES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Psychosocial Support Services ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD (−/+ num-stepper; sentinel-zero hidden) ═══════ */
  const renderNumberField = (record, fn, idx, sid) => {
    ...
    const numStep = stepFor(editValue);
    const safeNum = Number.isFinite(parseFloat(editValue)) ? parseFloat(editValue) : 0;
    const roundStep = (n) => { const p = String(numStep).includes('.') ? String(numStep).split('.')[1].length : 0; return Number(n.toFixed(p)); };
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                <button className="num-step" onClick={e => { e.stopPropagation(); setEditValue(String(roundStep(safeNum - numStep))); }}>&minus;</button>
                ...
                <button className="num-step" onClick={e => { e.stopPropagation(); setEditValue(String(roundStep(safeNum + numStep))); }}>+</button>
              </div>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="psychosocial-support-services-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Psychosocial Support Services</h2></div>
        <div className="empty-state">No psychosocial support services data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="psychosocial-support-services-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Psychosocial Support Services</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PsychosocialSupportServicesDocumentPDFTemplate document={pdfData} />} fileName="Psychosocial_Support_Services.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search psychosocial support services..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Psychosocial Support Services ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'screening-scores')}
            {renderSection(record, idx, 'clinical-assessment')}
            {renderSection(record, idx, 'treatment-support')}
            {renderSection(record, idx, 'functional-cognitive')}
            {renderSection(record, idx, 'social-factors')}
          </div>
        ))}
      </div>
    </div>
...
export default PsychosocialSupportServicesDocument;
