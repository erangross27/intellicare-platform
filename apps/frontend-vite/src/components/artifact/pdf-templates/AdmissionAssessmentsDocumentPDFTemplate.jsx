...
const SECTIONS = [
  { title: 'Admission Information', fields: [['assessmentDate', 'Assessment Date', 'date'], ['assessmentTime', 'Assessment Time']] },
  { title: 'Clinical Status & Vital Signs', fields: [['clinicalStatus', 'Clinical Status', 'sentence'], ['vitalSigns', 'Vital Signs', 'sentence']] },
  { title: 'Interventions & Response', fields: [['interventions', 'Interventions', 'sentenceComma'], ['response', 'Response', 'sentence']] },
  { title: 'Plan', fields: [['plan', 'Plan', 'sentence']] },
...
const AdmissionAssessmentsDocumentPDFTemplate = ({ document: documentProp, data: dataProp, templateData }) => {
  const records = React.useMemo(() => { const source = documentProp ?? dataProp ?? templateData; if (!source) return []; let rows = Array.isArray(source) ? source : [source]; rows = rows.flatMap(row => { if (Array.isArray(row?.records)) return row.records; if (Array.isArray(row?._records)) return row._records; if (row?.admission_assessments) return Array.isArray(row.admission_assessments) ? row.admission_assessments : [row.admission_assessments]; if (row?.documentData) { const nested = row.documentData; if (Array.isArray(nested)) return nested; if (nested?.admission_assessments) return Array.isArray(nested.admission_assessments) ? nested.admission_assessments : [nested.admission_assessments]; return [nested]; } return [row]; }); return rows.filter(row => row && typeof row === 'object'); }, [documentProp, dataProp, templateData]);
  if (!records.length) return <Document><Page size="LETTER" style={styles.page}><View style={styles.documentHeader}><Text style={styles.documentTitle}>Admission Assessments</Text></View><Text style={styles.noData}>No admission assessments available</Text></Page></Document>;
  return <Document>{records.flatMap((record, recordIndex) => PAGE_GROUPS.map((indexes, pageIndex) => { const visible = indexes.map(index => ({ section: SECTIONS[index], index, fields: SECTIONS[index].fields.filter(([field]) => hasVal(record[field])) })).filter(item => item.fields.length); if (!visible.length) return null; return <Page key={`${recordIndex}-${pageIndex}`} size="LETTER" style={styles.page}>{pageIndex === 0 && <View style={styles.documentHeader}><Text style={styles.documentTitle}>Admission Assessments</Text></View>}{pageIndex === 0 && <View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>{`Admission Assessment ${recordIndex + 1}`}</Text></View>}{visible.map(({ section, index, fields }) => <View key={`${index}-${section.title}`} style={styles.section}><View wrap={false}><Text style={styles.sectionTitle}>{section.title}</Text></View>{fields.map((config, fieldIndex) => renderField(config, record[config[0]], `${index}-${fieldIndex}`, section.title))}</View>)}</Page>; }))}</Document>;
};
