/**
 * LigamentReconstructionDocument.jsx
 ...
 *   2. procedure-details: ligament (string), graftType (string), graftSource (string)
 *   3. graft-size: graftSize.length (string), graftSize.diameter (string)
 *   4. tunnel-placement: tunnelPlacement.femoral (string), tunnelPlacement.tibial (string), tunnelPlacement.technique (string)
 *   5. fixation: fixation.femoral (string), fixation.tibial (string), fixation.supplemental (string)
 *   6. clinical-narrative: findings (long string), assessment (string), plan (string), notes (long string)
 *   7. results-section: results (OBJECT, recursive editable leaves)
 *   8. recommendations-section: recommendations (ARRAY of {recommendation, date}, date-grouped)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import LigamentReconstructionDocumentPDFTemplate from '../pdf-templates/LigamentReconstructionDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './LigamentReconstructionDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the API "field"; value = the API value) */
const DRAFT_KEY = 'ligament_reconstructionPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = [
  'provider', 'facility', 'status', 'ligament', 'graftType', 'graftSource',
  'graftSize.length', 'graftSize.diameter',
  'tunnelPlacement.femoral', 'tunnelPlacement.tibial', 'tunnelPlacement.technique',
  'fixation.femoral', 'fixation.tibial', 'fixation.supplemental',
  'findings', 'assessment', 'plan', 'notes',
];
const LONG_TEXT_FIELDS = ['findings', 'notes'];

/* ═══════ KEY HUMANIZATION + VALUE HELPERS (for recursive OBJECT rendering) ═══════ */
const KEY_OVERRIDES = {
  acl: 'ACL', pcl: 'PCL', mcl: 'MCL', lcl: 'LCL', btb: 'BTB', rom: 'ROM',
};
...
const recsToText = (recs) => {
  ...
  return recs.map(r => `${(r?.recommendation || '').trim()} ${(r?.date || '').trim()}`).join(' ');
};
...
/* parseLabelChain: fully decompose a nested "A: B: value" chain into [labels] + the leaf value,
   so a labeled value is never rendered side-by-side (label(s) ABOVE the value, never inline). */
const parseLabelChain = (text) => {
  const labels = [];
  let cur = String(text == null ? '' : text).replace(/[;.]+$/, '').trim();
  let guard = 0;
  let p = parseLabel(cur);
  while (p.isLabeled && guard++ < 6) {
    labels.push(p.label);
    cur = String(p.value).replace(/[;.]+$/, '').trim();
    p = parseLabel(cur);
  }
  return { labels, value: cur };
};
...
/* ═══════ COMPONENT ═══════ */
const LigamentReconstructionDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.ligament_reconstruction) return Array.isArray(r.ligament_reconstruction) ? r.ligament_reconstruction : [r.ligament_reconstruction];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.ligament_reconstruction) return Array.isArray(dd.ligament_reconstruction) ? dd.ligament_reconstruction : [dd.ligament_reconstruction]; return [dd]; }
      return [r];
    });
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT DB/PDF).
     The draft store is keyed by record _id; each entry maps API field -> staged value. */
  useEffect(() => {
  ...
  /* searchable text for any field (objects/arrays flattened) */
  const searchTextFor = useCallback((fn, v) => {
    if (OBJECT_ARRAY_FIELDS.includes(fn)) return recsToText(Array.isArray(v) ? v : []);
  ...
  /* resolve nested dot-path fields like graftSize.length */
  const resolveField = useCallback((record, fn) => {
    if (fn.includes('.')) {
      const parts = fn.split('.');
      let cur = record;
      for (const p of parts) { cur = cur?.[p]; if (cur === undefined || cur === null) return cur; }
      return cur;
    }
    return record[fn];
  }, []);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Ligament Reconstruction ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* stageDraft = persist a single field's staged value to the localStorage draft store + mark it pending.
     fieldPart is EXACTLY the API "field" name; value is EXACTLY the API value the Approve PUT will send.
     This is the ONLY persistence on Save — NO DB write happens until Approve commits. */
  const stageDraft = useCallback((record, fieldPart, value, idx, sid) => {
  ...
  // Save = stage a DRAFT locally (+ localStorage so it survives refresh). NOT written to MongoDB and
  // NOT shown in the PDF until the user clicks Approve (handleApproveSection commits).
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.graftIntegrity) — value stays a STRING.
     Staged as a DRAFT only: localEdits holds the whole merged root object (keyed by rootField), which is
     also the exact value Approve PUTs back as { field: rootField, value: <object> }. NO DB write here. */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    stageDraft(record, rootField, clone, idx, sid);
  ...
  const sectionHasEdits = useCallback((idx, sid) => {
    ...
    return fields.some(f => {
      const dashKey = `${f}-${idx}`;
      return Object.keys(editedFields).some(k => k === dashKey || k.startsWith(`${dashKey}-`) || k.startsWith(`${dashKey}.`)) ||
        Object.keys(editedSentences).some(k => k === dashKey || k.startsWith(`${dashKey}-`));
    });
  }, [editedFields, editedSentences]);

  // Approve = COMMIT this section's staged drafts to MongoDB (the ONLY path that writes to the DB),
  // then clear their pending flags so the committed values now flow into pdfData/PDF/Copy All.
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    try {
      const sectionFields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Collect this record's pending drafts for THIS section's fields (editKey = `${fieldPart}-${idx}`)
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length);
        return sectionFields.includes(fieldPart);
      });
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // API "field" — may be dotted (nested obj path)
        const payload = { field: fieldPart, value: localEdits[editKey] };
        // arrayIndex ONLY when the segment after the LAST dot is purely numeric (none in this template)
        ...
        const resp = await secureApiClient.put(`/api/edit/ligament_reconstruction/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/ligament_reconstruction/${id}/approve`, { sectionId: sid, approved: true });
      ...
      setEditedFields(prev => {
        ...
        Object.keys(n).forEach(k => {
          fields.forEach(f => {
            const dashKey = `${f}-${idx}`;
            if (k === dashKey || k.startsWith(`${dashKey}-`) || k.startsWith(`${dashKey}.`)) delete n[k];
          });
        });
        return n;
      });
      setEditedSentences(prev => {
        ...
        Object.keys(n).forEach(k => {
          fields.forEach(f => {
            const dashKey = `${f}-${idx}`;
            if (k === dashKey || k.startsWith(`${dashKey}-`)) delete n[k];
          });
        });
        return n;
      });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== LIGAMENT RECONSTRUCTION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Ligament Reconstruction ${idx + 1}\n${'='.repeat(40)}\n\n`;
        ...
        <div key={fn}>
          <div className="rec-mini-card">
            ...
            {sentences.map((sentence, sIdx) => {
              ...
              /* Regular sentence row — decompose any nested "A: B: value" chain into stacked
                 subtitles + ONE editable leaf value (never a side-by-side "Label: value" row). */
              const chain = parseLabelChain(sentence);
              const isChainLabeled = chain.labels.length > 0;
              return (
                <div key={sIdx} className={isChainLabeled ? 'rec-mini-card' : ''} style={isChainLabeled ? { marginTop: 8 } : undefined}>
                  {chain.labels.map((lab, lIdx) => (
                    <div key={lIdx} className={`nested-subtitle${lIdx > 0 ? ' sub-label' : ''}`}>{highlightText(lab)}</div>
                  ))}
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(isChainLabeled ? chain.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (isChainLabeled) { const trimmed = editValue.trim().replace(/[;.]+$/, '').trim(); const reconstructed = `${chain.labels.join(': ')}: ${trimmed}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); stageDraft(record, fn, fullText, idx, sid); setEditedSentences(prev => ({ ...prev, [sentenceKey]: 'edited' })); setEditingField(null); setEditValue(''); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
                      <>
                        <div className="row-content"><span className="content-value">{highlightText(isChainLabeled ? chain.value : sentence)}</span><span className="edit-indicator">&#9998;</span></div>
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              stageDraft(record, fn, newArr, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="ligament-reconstruction-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Ligament Reconstruction</h2></div>
        <div className="empty-state">No ligament reconstruction records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="ligament-reconstruction-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Ligament Reconstruction</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<LigamentReconstructionDocumentPDFTemplate document={pdfData} />} fileName="Ligament_Reconstruction.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search ligament reconstruction records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Ligament Reconstruction ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'procedure-details')}
            {renderSection(record, idx, 'graft-size')}
            {renderSection(record, idx, 'tunnel-placement')}
            {renderSection(record, idx, 'fixation')}
...
export default LigamentReconstructionDocument;
