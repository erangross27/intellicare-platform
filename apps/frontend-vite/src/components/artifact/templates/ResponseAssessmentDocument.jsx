/**
 * ResponseAssessmentDocument.jsx
 * July 2026 — recursive, dot-path editable response assessment
 * Collection: response_assessment
 */
import React, { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import ResponseAssessmentDocumentPDFTemplate from '../pdf-templates/ResponseAssessmentDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './ResponseAssessmentDocument.css';

const DRAFT_KEY = 'response_assessmentPendingEdits';
const SECTION_TITLES = {
  assessmentInformation: 'Assessment Information',
  responseMetrics: 'Response Metrics',
  measurableDisease: 'Measurable Disease',
  results: 'Results',
...
const FIELD_LABELS = {
  ...
  depthOfResponse: 'Depth of Response',
  durabilityOfResponse: 'Durability of Response',
  progressionFreesSurvival: 'Progression-Free Survival',
  measurableDisease: 'Measurable Disease',
  results: 'Results',
  provider: 'Provider',
  facility: 'Facility',
...
const SECTION_FIELDS = {
  assessmentInformation: ['date', 'dateOfResponse', 'status'],
  responseMetrics: ['criteria', 'bestResponse', 'depthOfResponse', 'durabilityOfResponse', 'progressionFreesSurvival'],
  measurableDisease: ['measurableDisease'],
  results: ['results'],
...
const DATE_FIELDS = ['date', 'dateOfResponse'];
const ENUM_FIELDS = ['status'];
const OBJECT_FIELDS = ['results'];
const OBJECT_ARRAY_FIELDS = ['measurableDisease', 'recommendations'];
const COMMA_FIELDS = ['findings'];
...
const ResponseAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.response_assessment) return Array.isArray(record.response_assessment) ? record.response_assessment : [record.response_assessment];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.response_assessment) return Array.isArray(nested.response_assessment) ? nested.response_assessment : [nested.response_assessment];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/response_assessment/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/response_assessment/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[ResponseAssessment] Approve error:', error);
  ...
  const leafProps = (record, path, recordIndex, sectionId, value, copyText) => {
    ...
    return {
      ...
      options: ENUM_FIELDS.includes(rootOf(path)) ? ['active', 'inactive', 'completed', 'pending'] : [],
  ...
  const copyAll = useCallback(async () => {
    let text = '=== RESPONSE ASSESSMENT ===\n\n';
    records.forEach((record, recordIndex) => {
      text += 'Response Assessment ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  const renderSection = (record, recordIndex, sectionId) => {
    ...
    return <div className="section" key={sectionId}><div className="mini-cards-container">
      ...
      {sectionId === 'measurableDisease' ? renderMeasurableDisease(record, recordIndex, sectionId)
  ...
  if (!records.length) return <div className="response-assessment-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Response Assessment</h2></div><div className="empty-state">No response assessment records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Response Assessment ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="response-assessment-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Response Assessment</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<ResponseAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Response_Assessment.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search response assessments..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Response Assessment ' + (recordIndex + 1))}</h3></div>
...
export default ResponseAssessmentDocument;
