/**
 * ImmediateInterventionsDocument.jsx
 * June 2026 — Inline editing, blue glow theme
 * Collection: immediate_interventions
 *
 * 5 Sections:
 *   1. intervention-overview: interventionType, timeToIntervention
 *   2. severity-scores: glasgowComaScale, apacheIIScore, sofaScore
 *   3. vital-signs: systolicBloodPressure, diastolicBloodPressure, meanArterialPressure, heartRate, respiratoryRate, oxygenSaturation, oxygenationStatus
 *   4. circulatory-support: inotropicSupport, vasopressorDose, fluidResuscitationVolume, bloodProductsAdministered[], centralVenousPressure
 *   5. laboratory-values: lactateLevel, hemoglobinLevel, plateletCount, internationalNormalizedRatio, activatedPartialThromboplastinTime, creatinineLevel, estimatedGlomerularFiltrationRate, urineOutput
 *
 * NOTE: this schema has NO date field — record header shows ONLY the numbered record title.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ImmediateInterventionsDocumentPDFTemplate from '../pdf-templates/ImmediateInterventionsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './ImmediateInterventionsDocument.css';
...
const FIELD_LABELS = {
  ...
  timeToIntervention: 'Time to Intervention (min)',
  ...
  oxygenationStatus: 'Oxygenation Status',
  inotropicSupport: 'Inotropic Support',
  vasopressorDose: 'Vasopressor Dose',
  fluidResuscitationVolume: 'Fluid Resuscitation Volume (mL)',
  bloodProductsAdministered: 'Blood Products Administered',
  centralVenousPressure: 'Central Venous Pressure (mmHg)',
  lactateLevel: 'Lactate Level (mmol/L)',
  hemoglobinLevel: 'Hemoglobin (g/dL)',
  plateletCount: 'Platelet Count',
  internationalNormalizedRatio: 'INR',
  activatedPartialThromboplastinTime: 'aPTT (sec)',
  creatinineLevel: 'Creatinine (mg/dL)',
  estimatedGlomerularFiltrationRate: 'eGFR',
  urineOutput: 'Urine Output (mL/hr)',
};

const SECTION_FIELDS = {
  'intervention-overview': ['interventionType', 'timeToIntervention'],
  'severity-scores': ['glasgowComaScale', 'apacheIIScore', 'sofaScore'],
  'vital-signs': ['systolicBloodPressure', 'diastolicBloodPressure', 'meanArterialPressure', 'heartRate', 'respiratoryRate', 'oxygenSaturation', 'oxygenationStatus'],
  'circulatory-support': ['inotropicSupport', 'vasopressorDose', 'fluidResuscitationVolume', 'bloodProductsAdministered', 'centralVenousPressure'],
  'laboratory-values': ['lactateLevel', 'hemoglobinLevel', 'plateletCount', 'internationalNormalizedRatio', 'activatedPartialThromboplastinTime', 'creatinineLevel', 'estimatedGlomerularFiltrationRate', 'urineOutput'],
};

const SENTENCE_FIELDS = ['oxygenationStatus', 'vasopressorDose'];
/* String simple field — intervention type is a free-text label, edited as text */
const SIMPLE_FIELDS = ['interventionType'];
/* Numeric fields — edited via type=number input (parseFloat, saves real numbers) */
const NUMBER_FIELDS = ['timeToIntervention', 'glasgowComaScale', 'apacheIIScore', 'sofaScore', 'systolicBloodPressure', 'diastolicBloodPressure', 'meanArterialPressure', 'heartRate', 'respiratoryRate', 'oxygenSaturation', 'lactateLevel', 'centralVenousPressure', 'fluidResuscitationVolume', 'hemoglobinLevel', 'plateletCount', 'internationalNormalizedRatio', 'activatedPartialThromboplastinTime', 'creatinineLevel', 'estimatedGlomerularFiltrationRate', 'urineOutput'];
const BOOLEAN_FIELDS = ['inotropicSupport'];
const ARRAY_FIELDS = ['bloodProductsAdministered'];
/* Numeric fields hidden when EXACTLY 0 — 0 is the extraction sentinel for
   "not part of this intervention" for these fields. EXCLUDED (0 is clinically
   MEANINGFUL, so always shown): sofaScore (0 = no organ dysfunction),
   apacheIIScore (0 = lowest valid acuity). GCS minimum is 3, so GCS 0 = sentinel;
   vitals/labs of 0 are not compatible with a living patient = sentinel.
   Strict === 0 so real decimals (e.g. INR 0.9, creatinine 0.78) still display. */
const HIDE_ZERO_FIELDS = ['timeToIntervention', 'glasgowComaScale', 'systolicBloodPressure', 'diastolicBloodPressure', 'meanArterialPressure', 'heartRate', 'respiratoryRate', 'oxygenSaturation', 'lactateLevel', 'centralVenousPressure', 'fluidResuscitationVolume', 'hemoglobinLevel', 'plateletCount', 'internationalNormalizedRatio', 'activatedPartialThromboplastinTime', 'creatinineLevel', 'estimatedGlomerularFiltrationRate', 'urineOutput'];
...
/* splitByComma: parenthesis-aware comma split with a thousands guard (split only when the
   comma is followed by whitespace) — "1,000" stays intact, "a, b" splits */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the field name; arrays store the full array) */
const DRAFT_KEY = 'immediateInterventionsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const ImmediateInterventionsDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.immediate_interventions) return Array.isArray(r.immediate_interventions) ? r.immediate_interventions : [r.immediate_interventions];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.immediate_interventions) return Array.isArray(dd.immediate_interventions) ? dd.immediate_interventions : [dd.immediate_interventions]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, i) => {
      ...
      if (!recDrafts) return;
      const oi = record._originalIdx ?? i;
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      const docTitle = 'immediate interventions';
      const rt = `immediate interventions ${oi + 1}`;
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* stageDraft — write a staged DRAFT for one field into the pending-drafts localStorage store
     (keyed by record _id → field name). NO DB write here; Approve (handleApproveSection) commits. */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Collect this record's staged drafts for fields in THIS section (localEdits key = "<field>-<idx>").
      const suffix = `-${idx}`;
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length);
        // arrayIndex ONLY when the segment after the LAST dot is purely numeric (this file never builds those)
      ...
      // Persist each staged field to the DB now (field, or field+arrayIndex for "<field>.<n>" keys).
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/immediate_interventions/${id}/edit`, payload);
      }
      // Flag the record/section approved (audit trail)
      await secureApiClient.put(`/api/edit/immediate_interventions/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this record's drafts for committed fields from localStorage (now committed)
      const store = readDrafts();
      if (store[id]) { toCommit.forEach(k => { const fp = k.slice(0, -suffix.length); const lastDot = fp.lastIndexOf('.'); const base = (lastDot !== -1 && /^\d+$/.test(fp.slice(lastDot + 1))) ? fp.slice(0, lastDot) : fp; delete store[id][base]; }); if (Object.keys(store[id]).length === 0) delete store[id]; writeDrafts(store); }

      setApprovedSections(prev => ({ ...prev, [`${sid}-${idx}`]: true }));
      setEditedFields(prev => { const n = { ...prev }; Object.keys(n).forEach(k => { fields.forEach(f => { if (k.startsWith(`${f}-${idx}`) || (k.startsWith(`${f}.`) && k.endsWith(`-${idx}`))) delete n[k]; }); }); return n; });
      setEditedSentences(prev => { const n = { ...prev }; Object.keys(n).forEach(k => { fields.forEach(f => { if (k.startsWith(`${f}-${idx}`)) delete n[k]; }); }); return n; });
    } catch (err) { console.error('[ImmediateInterventions] Approve error:', err); setSaveError('Approve failed. Please try again.'); } finally { setSaving(false); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== IMMEDIATE INTERVENTIONS ===\n\n';
    pdfData.forEach((r) => {
      const oi = r._originalIdx ?? 0;
      text += `Immediate Interventions ${oi + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD (type=number input — parseFloat, saves REAL numbers; 0 saved when meaningful) ═══════ */
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                ...
                <input type="text" inputMode="decimal" className="edit-number" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter') { e.stopPropagation(); saveNumber(); } }} />
              ...
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveNumber(); }}>{saving ? 'Saving...' : 'Save'}</button>
  ...
  /* ═══════ RENDER: SIMPLE FIELD (no splitting — intervention type free-text label) ═══════ */
        ...
        <div key={fn}>
          <div className="rec-mini-card">
            ...
            {sentences.map((sentence, sIdx) => {
              ...
              const saveThisSentence = () => { if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const currentSentences = isSemicolon ? splitBySemicolon(String(getFieldValue(record, fn, idx) || '')) : splitBySentence(String(getFieldValue(record, fn, idx) || '')); currentSentences[sIdx] = reconstructed; const fullText = reconstructFullText(currentSentences, isSemicolon); /* Stage a DRAFT (no DB write); Approve commits. */ setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullText })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); stageDraft(record, fn, fullText); setEditingField(null); setEditValue(''); setSaveError(null); } else { saveSentence(record, fn, idx, sid, sIdx); } };
              return (
          ...
          <div className="rec-mini-card">
            ...
            {singleCommaItems.map((ci, ciIdx) => {
              ...
              if (ciParsed.isLabeled) {
                const saveLabeledCommaItem = () => { const id = safeId(record); if (!id) return; const allItems = splitByComma(String(getFieldValue(record, fn, idx) || '')); allItems[ciIdx] = `${ciParsed.label}: ${editValue.trim()}`; const fullText = allItems.join(', '); /* Stage a DRAFT (no DB write); Approve commits. */ setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullText })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); setEditedSentences(prev => ({ ...prev, [commaKey]: 'edited' })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); stageDraft(record, fn, fullText); setEditingField(null); setEditValue(''); setSaveError(null); };
                return (
              ...
              const savePlainCommaItem = () => { const id = safeId(record); if (!id) return; const allItems = splitByComma(String(getFieldValue(record, fn, idx) || '')); allItems[ciIdx] = editValue.trim(); const fullText = allItems.join(', '); /* Stage a DRAFT (no DB write); Approve commits. */ setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullText })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); setEditedSentences(prev => ({ ...prev, [commaKey]: 'edited' })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); stageDraft(record, fn, fullText); setEditingField(null); setEditValue(''); setSaveError(null); };
              return (
                <div key={ciIdx}>
                  <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                    {ciEditing ? (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="immediate-interventions-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Immediate Interventions</h2></div>
        <div className="empty-state">No immediate interventions records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="immediate-interventions-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Immediate Interventions</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ImmediateInterventionsDocumentPDFTemplate document={pdfData} />} fileName="Immediate_Interventions.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search immediate interventions..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          ...
          return (
            <div key={safeId(record) || idx} className="record-card">
              ...
              <div className="record-header">
                <h3 className="record-name">{highlightText(`Immediate Interventions ${origIdx + 1}`)}</h3>
              </div>
              {renderSection(record, origIdx, 'intervention-overview')}
              {renderSection(record, origIdx, 'severity-scores')}
              {renderSection(record, origIdx, 'vital-signs')}
              {renderSection(record, origIdx, 'circulatory-support')}
              {renderSection(record, origIdx, 'laboratory-values')}
            </div>
          );
        })}
      </div>
...
export default ImmediateInterventionsDocument;
