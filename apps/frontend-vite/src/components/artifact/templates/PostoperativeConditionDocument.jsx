...
const PostoperativeConditionDocument = ({ document: docProp }) => {
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    if (sid === 'recommendations-notes') {
      if (Array.isArray(record.recommendations) && record.recommendations.length > 0) {
        ...
        record.recommendations.forEach((r, i) => { text += `${i + 1}. ${r}\n`; });
        text += '\n';
      }
      const notes = getFieldValue(record, 'notes', idx);
      if (hasVal(notes)) {
        const strVal = fmtVal(notes);
        const sentences = splitBySentence(strVal);
        if (sentences.length > 1) {
          text += 'Notes\n';
          formatSentenceFieldLines(strVal).forEach(l => { text += `${l}\n`; });
  ...
  const renderObjectSection = (record, idx, sid, objField, sectionTitle) => {
    ...
    const copyText = entries.map(([k, v]) => `${k}: ${String(v)}`).join('\n');
  ...
  const renderRecommendationsNotesSection = (record, idx) => {
    ...
    const copyId = `${sid}-${idx}`;
    let copyText = `Recommendations & Notes\n${'='.repeat(40)}\n\n`;
    if (recs.length > 0) {
      copyText += 'Recommendations\n';
      recs.forEach((r, i) => { copyText += `${i + 1}. ${r}\n`; });
      copyText += '\n';
    }
    if (hasVal(notesVal)) { copyText += `Notes\n${fmtVal(notesVal)}\n`; }
    ...
    <div className="postoperative-condition-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<PostoperativeConditionDocumentPDFTemplate document={pdfData} />} fileName={`postoperative-condition-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
