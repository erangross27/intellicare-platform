/**
 * Developmental Milestones Edit Route
 * ServiceId: developmental-milestones-edit-service
...
function buildContext(req, operation = 'read') { return { serviceId: 'developmental-milestones-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
...
const ALLOWED_FIELDS = [
  ...
  'grossMotor', 'fineMotor', 'language', 'socialEmotional', 'cognitive',
  'concerns', 'referrals',
  'assessment', 'notes', 'findings', 'plan',
  'recommendations', 'results',
];

// Dotted-path prefixes allowed for nested OBJECT leaf edits (e.g. results.score)
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('developmental_milestones', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[DevelopmentalMilestones] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('developmental_milestones', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[DevelopmentalMilestones] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
