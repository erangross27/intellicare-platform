/**
 * PrnMedicationsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: prn_medications (displayed as "PRN Medications")
 *
 * 4 Sections:
 *   1. medication-info: name, genericName, dosage, frequency, route
 *   2. prescriber-details: prescriber, indication, instructions
 *   3. duration: startDate, endDate, duration, durationDays, durationUnit, active, refills
 *   4. safety: safetyWarning, sideEffects, drugInteractions
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PrnMedicationsDocumentPDFTemplate from '../pdf-templates/PrnMedicationsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './PrnMedicationsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'prn_medicationsPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  ...
  'prescriber-details': ['prescriber', 'indication', 'instructions'],
  'duration': ['startDate', 'endDate', 'duration', 'durationDays', 'durationUnit', 'active', 'refills'],
  'safety': ['safetyWarning', 'sideEffects', 'drugInteractions'],
};
...
const MEANINGFUL_ZERO_FIELDS = ['refills']; // 0 refills is a real prescribing decision (PRN / Schedule-II controlled substances cannot be refilled) — always shown; other 0s (durationDays) are extractor defaults
...
/* isHiddenZero: a NUMBER field whose value is 0 is an extractor default (not a real clinical 0) → hide */
...
/* ═══════ COMPONENT ═══════ */
const PrnMedicationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.prn_medications) return Array.isArray(r.prn_medications) ? r.prn_medications : [r.prn_medications];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.prn_medications) return Array.isArray(dd.prn_medications) ? dd.prn_medications : [dd.prn_medications]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        // pending key mirrors the save handlers / reference convention: "fieldPart-idx"
        ...
        if (isArrayElem) {
          ...
          nFields[`${field}.${arrayIndex}-${idx}`] = 'edited';
        } else {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `PRN Medication ${idx + 1}`.toLowerCase();
    ...
    return filteredRecords.map((record, idx) => {
      ...
      Object.keys(localEdits).forEach(key => {
        ...
        if (Array.isArray(editVal)) {
          // Array field: pending elements are keyed "field.arrayIndex-idx"; keep them OUT of PDF.
          const orig = Array.isArray(record[field]) ? record[field] : [];
          const out = editVal.map((el, ai) => (pendingEdits[`${field}.${ai}-${idx}`] ? orig[ai] : el));
          merged[field] = out;
        } else {
          if (pendingEdits[`${field}-${idx}`]) return; // pending draft stays OUT of the PDF until approved
          merged[field] = editVal;
        }
      });
      return merged;
    });
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Pending keys for THIS record that belong to a field in THIS section. Key = "fieldPart-idx".
      ...
      for (const k of toCommit) {
        ...
        if (isArrayElem) {
          ...
          const arrayIndex = parseInt(tail, 10);
          const arr = localEdits[`${field}-${idx}`];
          const value = Array.isArray(arr) ? arr[arrayIndex] : undefined;
          const resp = await secureApiClient.put(`/api/edit/prn_medications/${id}/edit`, { field, value, arrayIndex });
          if (!resp || !resp.success) throw new Error((resp && resp.error) || 'save failed');
        } else {
          const value = localEdits[`${fieldPart}-${idx}`];
          const resp = await secureApiClient.put(`/api/edit/prn_medications/${id}/edit`, { field: fieldPart, value });
          if (!resp || !resp.success) throw new Error((resp && resp.error) || 'save failed');
        }
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/prn_medications/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this record's committed drafts (only the fields in this section)
      const store = readDrafts();
      if (store[id]) {
    ...
    } catch (err) { console.error('[PrnMedications] Approve error:', err); setSaveError('Approve failed. Please try again.'); } finally { setSaving(false); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PRN MEDICATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `PRN Medication ${idx + 1}\n${'='.repeat(40)}\n\n`;
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          const editStart = parsed.isLabeled ? parsed.value : itemStr;
          const saveItem = () => { const id2 = safeId(record); if (!id2) return; const newVal = parsed.isLabeled ? `${parsed.label}: ${editValue}` : editValue; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = newVal; setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: currentArr })); setPendingEdits(prev => ({ ...prev, [`${fn}.${itemIdx}-${idx}`]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const store = readDrafts(); if (!store[id2]) store[id2] = {}; store[id2][`${fn}.${itemIdx}`] = newVal; writeDrafts(store); setEditingField(null); setEditValue(''); setSaveError(null); };
          const editBlock = (
            <div className="edit-field-container">
                ...
                <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(editStart); setSaveError(null); } }}>
                  {isEditing ? editBlock : (
                    <>
                      ...
                      <button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${parsed.label}\n${parsed.value}`, editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button>
                    </>
                  )}
                </div>
                {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* Multi-sentence or labeled: render with splitBySentence */
    if (sentences.length > 1 || parseLabel(strVal).isLabeled) {
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      return (hasVal(val) || (NUMBER_FIELDS.includes(f) && val === 0)) && !isHiddenZero(f, val);
    });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="prn-medications-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">PRN Medications</h2></div>
        <div className="empty-state">No PRN medications available</div>
      </div>
    );
  }
  ...
  return (
    <div className="prn-medications-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">PRN Medications</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PrnMedicationsDocumentPDFTemplate document={pdfData} />} fileName="PRN_Medications.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search PRN medications..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`PRN Medication ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'medication-info')}
            {renderSection(record, idx, 'prescriber-details')}
            {renderSection(record, idx, 'duration')}
...
export default PrnMedicationsDocument;
