...
const SECTIONS = [
  ...
  { id: "current-use", title: "Current Substance Use", fields: ["currentUse"] },
  { id: "past-use", title: "Past Substance Use", fields: ["pastUse"] },
  {
    id: "withdrawal",
    title: "Withdrawal Symptoms",
    fields: ["withdrawalSymptoms"],
  },
  { id: "treatment", title: "Treatment History", fields: ["treatmentHistory"] },
  { id: "screening", title: "Screening", fields: ["duidHistory", "cageScore"] },
...
const numericShape = (value) => {
  ...
  let match = source.match(/^(-?\d+(?:\.\d+)?)\s*\/\s*(\d+(?:\.\d+)?)$/);
  if (match) return { type: "ratio", number: match[1], denominator: match[2] };
...
const SubstanceUseAssessmentDocument = ({
  document: docProp,
  data,
  templateData,
}) => {
  const records = useMemo(() => {
    ...
    return (Array.isArray(source) ? source : [source])
      .flatMap((record) => {
        if (record?.substance_use_assessment)
          return Array.isArray(record.substance_use_assessment)
            ? record.substance_use_assessment
            : [record.substance_use_assessment];
        if (record?.documentData)
          return Array.isArray(record.documentData)
            ? record.documentData
            : record.documentData?.substance_use_assessment
              ? Array.isArray(record.documentData.substance_use_assessment)
                ? record.documentData.substance_use_assessment
                : [record.documentData.substance_use_assessment]
              : [record.documentData];
        if (record?.document)
      ...
      try {
        for (const key of keys) {
          ...
          const response = await secureApiClient.put(
            `/api/edit/substance_use_assessment/${id}/edit`,
        ...
        await secureApiClient.put(
          `/api/edit/substance_use_assessment/${id}/approve`,
  ...
  const renderLeaf = (
    ...
    const numeric = !bool && !date ? numericShape(value) : null;
    const display = date
      ? formatDate(value)
      : bool
        ? value
        ...
        {rows.map((row, rowIndex) => {
          ...
          const change = (part, direction) => {
            ...
            if (shape.type === "range")
              ...
              setEditValue(`${next}/${shape.denominator}`);
    ...
    if (Array.isArray(value)) {
      ...
      value.forEach((item, index) => {
        ...
        if (typeof item !== "object" || isDate(item))
          lines.push(...buildLines("", item, `${path}.${index}`, indent));
        else
          Object.entries(item)
            .filter(([, child]) => !empty(child))
            .forEach(([key, child]) =>
  ...
  return (
    <div className="substance-use-assessment-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink
            document={
              <SubstanceUseAssessmentDocumentPDFTemplate document={pdfData} />
            }
            fileName="Substance_Use_Assessment.pdf"
            className="copy-btn"
          >
          ...
          {filtered.map(({ record, idx }) => (
            <div className="record-card" key={idx}>
              <div className="record-header">
                <h3 className="record-name">{`Substance Use Assessment ${idx + 1}`}</h3>
              </div>
