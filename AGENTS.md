...
3. **Inventory delimiter decisions from the real record.** For every populated
   string and string-array item, record whether periods, semicolons, and safe
   top-level commas are split or intentionally kept. A genuine two-clause/list
   value may be split even when it has only one comma; designate such array
   fields explicitly (for example `COMMA_ARRAY_FIELDS`) instead of relying on a
   global item-count threshold. Preserve credentials, appositives, addresses,
   numeric thousands, and parenthetical commas. Reproduce every exact
   user-reported value and require each intended clause to appear as its own JSX,
   Copy, and PDF row before completion.
4. **Honor the editable-leaf DOM contract.** `data-edit-field` belongs on a
...
5. **Use semantic mini-card grouping.** One labeled mini-card contains its
