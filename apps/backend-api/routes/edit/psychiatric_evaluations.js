...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'psychiatric-evaluations-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'psychiatricHistory.hospitalizations',
  'psychiatricHistory.previousMedications',
  'psychiatricHistory.previousTherapy',
  'substanceUseHistory',
  'substanceUseHistory.alcohol',
  'substanceUseHistory.tobacco',
  'substanceUseHistory.cannabis',
  'substanceUseHistory.otherSubstances',
  'mentalStatusExam',
  ...
  'diagnosis',
  'treatmentPlan',
  'treatmentPlan.immediateInterventions',
  'treatmentPlan.medications',
  'treatmentPlan.psychotherapy',
  'treatmentPlan.psychosocialInterventions',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('psychiatric_evaluations', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[psychiatric_evaluations] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('psychiatric_evaluations', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[psychiatric_evaluations] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
