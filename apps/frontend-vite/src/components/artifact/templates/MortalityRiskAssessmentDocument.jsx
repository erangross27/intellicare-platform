/**
 * MortalityRiskAssessmentDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: mortality_risk_assessment
 *
 * 4 Sections:
 *   1. assessment-info: date, assessmentMethod, timeframeOfPrediction, riskCategory, acuityLevel, primaryDiagnosisCode, ageAtAssessment, assessorCredentials, admissionType, reassessmentInterval
 *   2. risk-scores: totalRiskScore, predictedMortalityPercentage, comorbidityCount, functionalStatusScore, organFailureCount, glasgowComaScore
 *   3. organ-support: mechanicalVentilationRequired, vasopressorSupport, renalReplacementTherapy, sepsisPresent, cardiacArrestHistory, malignancyStatus, surgicalProcedureRisk, palliativeCareConsulted
 *   4. risk-factors: vitalSignsAbnormalities, laboratoryRiskFactors
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MortalityRiskAssessmentDocumentPDFTemplate from '../pdf-templates/MortalityRiskAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './MortalityRiskAssessmentDocument.css';
...
const DRAFT_KEY = 'mortality_risk_assessmentPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['assessmentMethod', 'timeframeOfPrediction', 'riskCategory', 'acuityLevel', 'primaryDiagnosisCode', 'malignancyStatus', 'assessorCredentials', 'admissionType', 'surgicalProcedureRisk', 'reassessmentInterval'];
...
/* splitByComma: parenthesis-aware comma split (thousands guard: ignore comma inside 1,000) */
...
/* ═══════ COMPONENT ═══════ */
const MortalityRiskAssessmentDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Mortality Risk Assessment ${idx + 1}`.toLowerCase();
  ...
  // Stage a sentence-edit DRAFT locally (no DB write). localStorage keeps it across refresh; Approve commits it.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Staged drafts for THIS section's fields (localEdits key = "field-idx", value string OR array)
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/mortality_risk_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/mortality_risk_assessment/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MORTALITY RISK ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Mortality Risk Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: BOOLEAN FIELD (Yes/No via BlueSelect) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="mortality-risk-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Mortality Risk Assessment</h2></div>
        <div className="empty-state">No mortality risk assessment data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="mortality-risk-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Mortality Risk Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MortalityRiskAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Mortality_Risk_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search mortality risk assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Mortality Risk Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'assessment-info')}
            {renderSection(record, idx, 'risk-scores')}
            {renderSection(record, idx, 'organ-support')}
...
export default MortalityRiskAssessmentDocument;
