...
class PatientPortalMessagingService {
  ...
  async sendPatientPortalMessage(params, practiceContext) {
    try {
      ...
      if (providerId) {
        // Convert providerId string to ObjectId
        const providerObjectId = new ObjectId(providerId);
        const targetProviderResults = await SecureDataAccess.query(
          'users',
          { _id: providerObjectId,  role: { $in: ['doctor', 'nurse'] } },
          {},
          {
            serviceId: this.serviceId,
            apiKey: this.serviceToken?.apiKey || this.serviceToken,
        ...
        const targetProvider = targetProviderResults[0];
      } else {
  ...
  async requestPrescriptionRefill(params, practiceContext) {
    try {
      ...
      if (prescriptionId) {
        // Convert prescriptionId string to ObjectId
        const prescriptionObjectId = new ObjectId(prescriptionId);
        const prescriptionResults = await SecureDataAccess.query(
          'prescriptions',
          { _id: prescriptionObjectId,  patientId: patientObjectId , limit: 1 },
          {},
          {
            serviceId: this.serviceId,
            apiKey: this.serviceToken?.apiKey || this.serviceToken,
  ...
  async getOrCreateMessageThread(patientId, providerId, practiceContext) {
    try {
      ...
      const existingThreadResults = await SecureDataAccess.query(
        'message_threads',
        { 
          patientId: patientId, 
          providerId: providerId,
      ...
      const existingThread = existingThreadResults[0];
  ...
  async findAvailableProvider(patientId, practiceContext) {
    try {
      ...
      if (patient?.primaryCareProviderId) {
        // primaryCareProviderId might be a string
        const primaryProviderObjectId = typeof patient.primaryCareProviderId === 'string' ? 
          new ObjectId(patient.primaryCareProviderId) : patient.primaryCareProviderId;
        const primaryProviderResults = await SecureDataAccess.query(
          'users',
          { _id: primaryProviderObjectId,  role: { $in: ['doctor', 'nurse'] } },
          {},
          {
            serviceId: this.serviceId,
            apiKey: this.serviceToken?.apiKey || this.serviceToken,
        ...
        const primaryProvider = primaryProviderResults[0];
      ...
      const availableDoctorResults = await SecureDataAccess.query(
        'users',
        { 
          role: 'doctor', 
          status: 'active',
      ...
      const availableDoctor = availableDoctorResults[0];
