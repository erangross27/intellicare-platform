/**
 * Cardiology Admission Edit Route
 * Delegates to cardiology_admission_notes collection
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'cardiology-admission-notes-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'admissionDate',
  'chiefCardiacComplaint',
  'acuteCoronarySyndromeType',
  'nyhaClassification',
  'killipClassification',
  'chestPainCharacteristics.quality',
  'chestPainCharacteristics.severity',
  'chestPainCharacteristics.location',
  'chestPainCharacteristics.radiation',
  'chestPainCharacteristics.duration',
  'chestPainCharacteristics.onset',
  'troponinLevel',
  'bnpLevel',
  'leftVentricularEjectionFraction',
  'ekgFindings',
  'echocardiogramResults.ejectionFraction',
  'echocardiogramResults.wallMotion',
  'echocardiogramResults.valves',
  'echocardiogramResults.complications',
  'hemodynamicParameters.bloodPressure',
  'hemodynamicParameters.heartRate',
  'hemodynamicParameters.oxygenSaturation',
  'cardiacRiskFactors',
  'currentCardiacMedications',
  'cardiacBiomarkerTrend',
  'valvularAbnormalities',
  'anticoagulationStatus.therapy',
  'anticoagulationStatus.indication',
  'anticoagulationStatus.duration',
  'coronaryArteryDiseaseHistory',
  'cardiacCatheterizationPlanned',
  'thrombolyticEligibility',
  'arrhythmiaType',
  'pulmonaryEdemaPresence',
  'telemetryMonitoring',
  'functionalCapacity',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    console.error('[CardiologyAdmission] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('cardiology_admission_notes', { _id: objectId }, {
      $set: {
    ...
    console.error('[CardiologyAdmission] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
