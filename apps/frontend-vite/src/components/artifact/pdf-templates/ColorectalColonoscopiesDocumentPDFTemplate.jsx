/**
 * ColorectalColonoscopiesDocumentPDFTemplate.jsx
 * June 2026 — Helvetica — LETTER size — colorectal colonoscopies (24/24 fields)
 ...
 * Rule #74: section = ONE wrap-gated View; sectionTitle FIRST child; wrap={present>8?undefined:false}.
 */
...
const styles = StyleSheet.create({
  ...
  recordMeta: { fontSize: 11, color: '#333333', marginTop: 3 },
    ...
    <View style={styles.fieldBox} wrap={safeItems.length > 8 ? undefined : false}>
      ...
      {safeItems.map((item, i) => {
        const str = typeof item === 'object' && item !== null
          ...
          : safeString(item);
        return <Text key={i} style={styles.listItem}>{i + 1}. {str}</Text>;
      })}
    </View>
  );
};
...
const SECTION_CONFIGS = [
  {
    ...
    fields: [
      ...
      { key: 'bowelPrepQuality', label: 'Bowel Prep Quality', isSentence: true },
      { key: 'cecumReached', label: 'Cecum Reached', isSentence: true },
      { key: 'cecumIntubationTime', label: 'Cecum Intubation Time (min)', isNumber: true },
      { key: 'withdrawalTime', label: 'Withdrawal Time (min)', isNumber: true },
      { key: 'sedationType', label: 'Sedation Type', isSentence: true },
      { key: 'procedureCompleteness', label: 'Procedure Completeness', isSentence: true },
    ],
  },
  {
    title: 'Findings',
    fields: [
      { key: 'polypsFound', label: 'Polyps Found', isSentence: true },
      { key: 'polypDetails', label: 'Polyp Details', isArray: true },
      { key: 'polypectomyPerformed', label: 'Polypectomy Performed', isSentence: true },
      { key: 'resectionMethod', label: 'Resection Method', isArray: true },
      { key: 'adenomaDetectionRate', label: 'Adenoma Detection Rate', isSentence: true },
    ],
  },
  {
    title: 'Biopsy & Pathology',
    fields: [
      { key: 'biopsiesTaken', label: 'Biopsies Taken', isSentence: true },
      { key: 'biopsyLocations', label: 'Biopsy Locations', isArray: true },
      { key: 'pathologyFindings', label: 'Pathology Findings', isSentence: true },
      { key: 'significantFindings', label: 'Significant Findings', isArray: true },
    ],
  },
  {
    ...
    fields: [
      ...
      { key: 'hemostasisRequired', label: 'Hemostasis Required', isSentence: true },
      { key: 'recommendedSurveillance', label: 'Recommended Surveillance', isSentence: true },
      { key: 'photographicDocumentation', label: 'Photographic Documentation', isSentence: true },
      { key: 'previousColonoscopyDate', label: 'Previous Colonoscopy Date', isDate: true },
    ],
  },
];
...
/* ======= COMPONENT ======= */
const ColorectalColonoscopiesDocumentPDFTemplate = ({ document: data }) => {
      ...
      <Document>
        <Page size="LETTER" style={styles.page}>
          <View style={styles.documentHeader}>
            <Text style={styles.documentTitle}>Colonoscopy Report</Text>
          </View>
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, index) => {
          const metaParts = [];
          if (hasVal(record.procedureDate)) metaParts.push(formatDate(record.procedureDate));
          ...
          return (
            <View key={index} style={styles.recordContainer}>
              ...
              <View style={styles.recordHeader} wrap={false}>
                <Text style={styles.recordTitle}>{`Colonoscopy Report ${index + 1}`}</Text>
                {metaParts.length > 0 && <Text style={styles.recordMeta}>{metaParts.join('  •  ')}</Text>}
              </View>

              {/* Sections — sectionTitle FIRST child of ONE wrap-gated View (Rule #74) */}
...
export default ColorectalColonoscopiesDocumentPDFTemplate;
