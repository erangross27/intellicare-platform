/**
 * PsychosocialAssessmentsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: psychosocial_assessments
 *
 * 8 Sections:
 *   1. assessment-info: provider, facility, date, type
 *   2. screening-scores: edinburghScore, anxietyScreening, domesticViolenceScreen
 *   3. social-support: socialSupport
 *   4. substance-use: substanceUseScreen (object: alcohol, tobacco, drugs)
 *   5. psychosocial-factors: housingStability, financialConcerns, relationshipStress, previousPostpartumDepression
 *   6. findings-assessment: findings, assessment
 *   7. plan-recommendations: plan, recommendations
 *   8. notes-status: notes, status
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PsychosocialAssessmentsDocumentPDFTemplate from '../pdf-templates/PsychosocialAssessmentsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PsychosocialAssessmentsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or dot-path; value may be
   a string, full array, or full object — exactly what localEdits stores under `${field}-${idx}`). */
const DRAFT_KEY = 'psychosocial_assessmentsPendingEdits';
const readDrafts = () => {
...
/* humanizeKey: turn a dynamic object key into a readable label */
...
/* flattenObject: recurse a dynamic-key object into [{path, label, value}] typed leaves (no [object Object]) */
const flattenObject = (obj, prefix = '', labelPrefix = '') => {
  ...
  Object.keys(obj).forEach(k => {
    ...
    if (val !== null && typeof val === 'object' && !Array.isArray(val)) {
      leaves.push(...flattenObject(val, path, label));
    } else if (Array.isArray(val)) {
      const flat = val.map(item => (item !== null && typeof item === 'object') ? (item.name || item.value || JSON.stringify(item)) : String(item)).join(', ');
      if (flat.trim() !== '') leaves.push({ path, label, value: flat });
    } else if (val !== null && val !== undefined && String(val).trim() !== '') {
      leaves.push({ path, label, value: val });
    }
  });
  return leaves;
};
const STRING_FIELDS = ['provider', 'facility', 'type', 'edinburghScore', 'anxietyScreening', 'domesticViolenceScreen', 'socialSupport', 'substanceUseScreen.alcohol', 'substanceUseScreen.tobacco', 'substanceUseScreen.drugs', 'housingStability', 'financialConcerns', 'relationshipStress', 'findings', 'assessment', 'plan', 'notes', 'status'];
...
/* splitByComma: parenthesis-aware comma split with Oxford ("and"/"or") + numeric-thousands ("$12,500") guards */
...
const getRiskScoreColor = (pct) => { if (pct <= 25) return '#22c55e'; if (pct <= 50) return '#3b82f6'; if (pct <= 75) return '#f59e0b'; return '#ef4444'; };
...
const Legend = () => (
  <div className="chart-legend">
    <div className="legend-item"><div className="legend-color" style={{ backgroundColor: '#22c55e' }} /><span>Minimal (0-25%)</span></div>
    <div className="legend-item"><div className="legend-color" style={{ backgroundColor: '#3b82f6' }} /><span>Mild (26-50%)</span></div>
    <div className="legend-item"><div className="legend-color" style={{ backgroundColor: '#f59e0b' }} /><span>Moderate (51-75%)</span></div>
    <div className="legend-item"><div className="legend-color" style={{ backgroundColor: '#ef4444' }} /><span>Severe (76-100%)</span></div>
  </div>
);
...
const BarChart = ({ label, percentage, rawValue, color, interpretation, highlightFn }) => (
  <div className="bar-chart-row">
    ...
    <div className="bar-container">
      <div className="bar-background"><div className="bar-fill" style={{ width: `${percentage}%`, backgroundColor: color }} /></div>
...
/* ═══════ COMPONENT ═══════ */
const PsychosocialAssessmentsDocument = ({ document: docProp }) => {
  ...
  /* getFieldValue: supports dot-path for substanceUseScreen.alcohol etc */
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (OBJECT_FIELDS.includes(f) && typeof val === 'object' && !Array.isArray(val)) { if (flattenObject(val).some(leaf => leaf.label.toLowerCase().includes(phrase) || String(leaf.value).toLowerCase().includes(phrase))) return true; }
        else if (Array.isArray(val)) { if (val.some(item => String(typeof item === 'object' ? (item?.recommendation || JSON.stringify(item)) : item).toLowerCase().includes(phrase))) return true; }
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) {
      if (OBJECT_FIELDS.includes(fn) && typeof val === 'object' && !Array.isArray(val)) return flattenObject(val).some(leaf => leaf.label.toLowerCase().includes(phrase) || String(leaf.value).toLowerCase().includes(phrase));
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = (record.type || `Psychosocial Assessment ${idx + 1}`).toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* stageDraft: persist an edit LOCALLY only (localEdits + pendingEdits + localStorage draft store).
     NOT written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve.
     fieldPart = the localEdits key minus the trailing `-${idx}` (a field name or dot-path). */
  const stageDraft = useCallback((record, idx, fieldPart, value) => {
    ...
    setApprovedSections(prev => {
      ...
      Object.keys(next).forEach(k => {
        if (!k.endsWith(`-${idx}`)) return;
        const sid = k.slice(0, -`-${idx}`.length);
        const baseField = fieldPart.includes('.') ? fieldPart.split('.')[0] : fieldPart;
        if ((SECTION_FIELDS[sid] || []).some(f => f === fieldPart || f === baseField || f.split('.')[0] === baseField)) { delete next[k]; changed = true; }
      });
  ...
  // Save = stage a DRAFT locally (survives refresh). Approve (handleApproveSection) commits to the DB.
  ...
  // pending so the committed values flow into pdfData/PDF. Drafts are dropped from localStorage.
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "field" or dot-path
        // arrayIndex ONLY when the segment after the LAST dot is purely numeric (reverse of save).
        ...
        const resp = await secureApiClient.put(`/api/edit/psychosocial_assessments/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/psychosocial_assessments/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF.
      setPendingEdits(prev => { const next = { ...prev }; toCommit.forEach(k => delete next[k]); return next; });
      // Drop this record's committed drafts from localStorage (only the keys we committed).
      const store = readDrafts();
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        if (items.length > 0) {
          ...
          items.forEach((item, i) => {
            const rt = typeof item === 'object' ? (item?.recommendation || `Recommendation ${i + 1}`) : String(item);
        ...
        const leaves = flattenObject(val);
        if (leaves.length > 0) {
          text += labelLine;
          leaves.forEach(leaf => { text += `${leaf.label}\n1. ${fmtVal(leaf.value)}\n`; });
          text += '\n';
        }
      } else {
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PSYCHOSOCIAL ASSESSMENTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Psychosocial Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: RECOMMENDATIONS (array of objects {recommendation, date}) — STACKED sub-fields ═══════ */
  const renderArrayField = (record, fn, idx, sid) => {
    ...
    const saveRecSubField = (recIdx, subField, newValue) => {
      const id = safeId(record); if (!id) return;
      const currentRecs = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])];
      if (!currentRecs[recIdx]) return;
      currentRecs[recIdx] = { ...currentRecs[recIdx], [subField]: newValue };
      setSaveError(null);
      stageDraft(record, idx, fn, currentRecs);
      setEditedFields(prev => ({ ...prev, [`${fn}-${idx}-r${recIdx}-${subField}`]: 'edited' }));
      setEditingField(null); setEditValue('');
    };
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          const recStr = typeof item === 'object' ? (item?.recommendation || `Recommendation ${itemIdx + 1}`) : String(item);
          const recContent = typeof item === 'object' ? [item.recommendation, item.date].filter(Boolean).join(' ') : recStr;
          ...
          return (
            <div key={itemIdx} className="rec-mini-card" style={{ marginTop: 4 }}>
              <div className="nested-subtitle">{highlightText(recStr)}</div>
              {[{ key: 'recommendation', label: 'Recommendation' }, { key: 'date', label: 'Date' }].map(sf => {
                const sfVal = typeof item === 'object' ? item[sf.key] : (sf.key === 'recommendation' ? item : null);
                if (!hasVal(sfVal)) return null;
                const trackKey = `${fn}-${idx}-r${itemIdx}-${sf.key}`;
                const isEditing = editingField === trackKey;
                const isModified = editedFields[trackKey];
                const sfStr = fmtVal(sfVal);
                return (
                  <div key={sf.key} className="nested-mini-card">
                    <div className="nested-subtitle sub-label">{highlightText(sf.label)}</div>
                    <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(trackKey); setEditValue(sfStr); setSaveError(null); } }}>
                      {isEditing ? (
                        <div className="edit-field-container">
                          <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                          {saveError && <div className="save-error">{saveError}</div>}
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveRecSubField(itemIdx, sf.key, editValue.trim()); }}>{saving ? 'Saving...' : 'Save'}</button>
                            <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                          </div>
                        </div>
                      ) : (
                        <>
                          <div className="row-content"><span className="content-value">{highlightText(sfStr)}</span><span className="edit-indicator">&#9998;</span></div>
                          <button className={`copy-btn ${copiedItems[trackKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${sf.label}\n${sfStr}`, trackKey); }}>{copiedItems[trackKey] ? 'Copied!' : 'Copy'}</button>
                        </>
                      )}
                    </div>
                    {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
  ...
  const renderStringField = (record, fn, idx, sid, title) => {
    ...
    /* Multi-sentence (or a single "Label: a, b, c" list): render structured with splitBySentence */
  ...
  const renderObjectField = (record, fn, idx, sid) => {
    ...
    return (
      <div key={fn} className="rec-mini-card">
        {showLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
        {leaves.map((leaf) => {
          // edit-key uses full dot-path so save targets exact leaf; consistent with editedFields tracking
          const fullPath = `${fn}.${leaf.path}`;
          const editKey = `${fullPath}-${idx}`;
          const isEditing = editingField === editKey;
          const isModified = editedFields[editKey];
          const leafStr = fmtVal(leaf.value);
          const isNum = typeof leaf.value === 'number';
          const isBool = typeof leaf.value === 'boolean';
          const leafStep = stepFor(isNum ? leaf.value : editValue);
          const leafSafeNum = Number.isFinite(parseFloat(editValue)) ? parseFloat(editValue) : 0;
          ...
          if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
            const ll = leaf.label.toLowerCase();
            if (!ll.includes(phrase) && !phrase.includes(ll) && !leafStr.toLowerCase().includes(phrase)) return null;
          }

          return (
            <div key={leaf.path}>
              <div className="nested-subtitle" style={{ fontSize: '14px' }}>{highlightText(leaf.label)}</div>
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(isBool ? (leaf.value ? 'Yes' : 'No') : leafStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    {isBool ? (
                      <BlueSelect value={editValue} options={['Yes', 'No']} onChange={v => { setEditValue(v); setSaveError(null); }} />
                    ...
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => {
                        ...
                        const id = safeId(record); if (!id) return;
                        setSaveError(null);
                        // reconstruct the object preserving shape via dot-path, then stage the whole object as a DRAFT
                        ...
                        node[parts[parts.length - 1]] = saveVal;
                        stageDraft(record, idx, fn, cloned);
                  ...
                  <>
                    <div className="row-content"><span className="content-value">{highlightText(leafStr)}</span><span className="edit-indicator">&#9998;</span></div>
                    <button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${leaf.label}\n${leafStr}`, editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button>
                  </>
                )}
              </div>
              {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
  ...
  /* ═══════ RENDER: SCORE OVERVIEW (bar chart — non-editable display) ═══════ */
  const renderScoreOverview = (record, idx) => {
    ...
    if (searchTerm.trim() && !record._showAllSections) {
      const phrase = searchTerm.toLowerCase().trim();
      const sectionMatch = 'score overview'.includes(phrase) || phrase.includes('score overview');
      const chartMatch = chartData.some(c => [c.label, c.rawValue, c.interpretation].some(t => t.toLowerCase().includes(phrase)));
      if (!sectionMatch && !chartMatch) return null;
    }
    return (
        ...
        <div className="mini-cards-container">
          ...
          <div className="chart-container">
            ...
            {chartData.map((chart, cIdx) => (
              <BarChart key={cIdx} label={chart.label} percentage={chart.percentage} rawValue={chart.rawValue} color={chart.color} interpretation={chart.interpretation} highlightFn={highlightText} />
            ))}
          </div>
        </div>
      </div>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="psychosocial-assessments-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Psychosocial Assessments</h2></div>
        <div className="empty-state">No psychosocial assessments records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="psychosocial-assessments-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Psychosocial Assessments</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PsychosocialAssessmentsDocumentPDFTemplate document={pdfData} />} fileName="Psychosocial_Assessments.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search assessments, PHQ-9, GAD-7, social support..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(record.type || `Psychosocial Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'assessment-info')}
            {renderScoreOverview(record, idx)}
            {renderSection(record, idx, 'screening-scores')}
            {renderSection(record, idx, 'social-support')}
            {renderSection(record, idx, 'substance-use')}
            {renderSection(record, idx, 'psychosocial-factors')}
            ...
            {renderSection(record, idx, 'notes-status')}
          </div>
        ))}
      </div>
    </div>
...
export default PsychosocialAssessmentsDocument;
