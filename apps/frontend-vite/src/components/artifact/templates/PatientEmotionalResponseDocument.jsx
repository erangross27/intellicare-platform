/**
 * PatientEmotionalResponseDocument.jsx
 * June 2026 — Collection: patient_emotional_response (NEW template)
 *
 * Donor cloned (identical generic shape): PointOfCareUltrasoundHeartRateDocument
 *   - date header renderDateField (record-header date + provider + facility)
 *   - OBJECT recursive render (renderObjectField / renderObjectNode / renderObjectLeaf)
 *   - ARRAY: renderConcernsField (string array) + renderRecommendationsField (array of {recommendation, date})
 *   - STRING per-sentence; 4-level search; per-section approve; Copy row/Section/All; pdfData memo.
 *
 * FIELDS (15 → 100% schema coverage):
 *   DATE:   date
 *   STRING: emotionalState, supportNeeded, response, type, provider, facility,
 *           findings, assessment, plan, notes, status
 *   ARRAY:  concerns (string array), recommendations (array of {recommendation, date})
 *   OBJECT: results (recursive key-value)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PatientEmotionalResponseDocumentPDFTemplate from '../pdf-templates/PatientEmotionalResponseDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './PatientEmotionalResponseDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = localEdits-key prefix, e.g. "notes" or "results") */
const DRAFT_KEY = 'patient_emotional_responsePendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  'emotional-state': ['emotionalState', 'response', 'supportNeeded', 'type'],
  'concerns-section': ['concerns'],
...
const SECTION_ORDER = ['emotional-state', 'concerns-section', 'clinical', 'results-section', 'recommendations-section', 'notes-status'];

const DATE_FIELDS = ['date'];
const STRING_FIELDS = ['emotionalState', 'supportNeeded', 'response', 'type', 'provider', 'facility', 'findings', 'assessment', 'plan', 'notes', 'status'];
/* DISPLAY/EDIT recursive object field */
const OBJECT_FIELDS = ['results'];
/* Plain string array (editable tags) */
const STRING_ARRAY_FIELDS = ['concerns'];
/* Array-of-objects {recommendation, date} (date-grouped, editable) */
const OBJECT_ARRAY_FIELDS = ['recommendations'];

const KEY_OVERRIDES = {
  phq: 'PHQ', gad: 'GAD', phq9: 'PHQ-9', gad7: 'GAD-7', cssrs: 'C-SSRS',
};
...
/* ═══════ COMPONENT ═══════ */
const PatientEmotionalResponseDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.patient_emotional_response) return Array.isArray(r.patient_emotional_response) ? r.patient_emotional_response : [r.patient_emotional_response];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.patient_emotional_response) return Array.isArray(dd.patient_emotional_response) ? dd.patient_emotional_response : [dd.patient_emotional_response]; return [dd]; }
      return [r];
    });
  ...
  const arrToText = useCallback((arr) => {
    ...
    return arr.map(x => fmtScalar(x)).join(' ');
  }, []);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `patient emotional response ${idx + 1}`.toLowerCase();
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.phq9) — value stays a STRING */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Staged drafts for THIS section's fields (localEdits key = `${fieldPart}${suffix}`)
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/patient_emotional_response/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/patient_emotional_response/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[PatientEmotionalResponse] Approve error:', err); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PATIENT EMOTIONAL RESPONSE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Patient Emotional Response ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: STRING ARRAY (concerns) — editable per-item rows ═══════ */
  const renderStringArrayField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    const items = Array.isArray(val) ? val : [];
    const present = items.map((it, i) => ({ it, i })).filter(({ it }) => !isEmptyDeep(it));
                  ...
                  <div className="edit-field-container">
                    ...
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => {
                        ...
                        const newArr = currentArr.map((x, xi) => xi === i ? trimmed : x);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="patient-emotional-response-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Patient Emotional Response</h2></div>
        <div className="empty-state">No patient emotional response records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="patient-emotional-response-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Patient Emotional Response</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PatientEmotionalResponseDocumentPDFTemplate document={pdfData} />} fileName={`patient-emotional-response-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search patient emotional response records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Patient Emotional Response ${idx + 1}`)}</h3>
            </div>
...
export default PatientEmotionalResponseDocument;
