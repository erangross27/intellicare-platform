/**
 * HomeSafetyDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: home_safety
 *
 * 8 Sections:
 *   1. fall-risk: fallRiskAssessment, timedUpAndGoTest, functionalReachTest, orthostaticVitalSigns
 *   2. home-environment: homeModificationNeeds, homeOxygenTherapy
 *   3. functional-status: adlIndependenceLevel, mobilityAidsPrescribed
 *   4. sensory-assessment: visionAcuityMeasurement, hearingAssessmentResults
 *   5. medication-safety: medicationManagementSystem, polypharmacyRiskFactors, anticoagulationManagement
 *   6. cognitive-mental: cognitiveImpairmentScreening, depressionScreeningScore
 *   7. nutritional-physical: nutritionalStatusScreening, diabeticFootExamination, skinIntegrityAssessment, tinelSignTesting
 *   8. social-support: caregriverSupport, socialIsolationRisk, emergencyResponsePlan
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HomeSafetyDocumentPDFTemplate from '../pdf-templates/HomeSafetyDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './HomeSafetyDocument.css';
...
/* 3 number fields — num-stepper (−/+ + text inputMode=decimal) + parseFloat+isNaN→block save.
   0 is a MEANINGFUL value here (functionalReachTest 0 in, depressionScreeningScore 0) — rendered for JSX/PDF parity. */
const NUMBER_FIELDS = ['functionalReachTest', 'timedUpAndGoTest', 'depressionScreeningScore'];

/* 1 boolean field — BlueSelect Yes/No */
...
const STRING_FIELDS = [
  'fallRiskAssessment', 'orthostaticVitalSigns', 'homeOxygenTherapy',
  'adlIndependenceLevel', 'visionAcuityMeasurement', 'hearingAssessmentResults',
  'medicationManagementSystem', 'anticoagulationManagement',
  'cognitiveImpairmentScreening', 'nutritionalStatusScreening',
  'diabeticFootExamination', 'skinIntegrityAssessment',
  'caregriverSupport', 'socialIsolationRisk', 'emergencyResponsePlan',
];
...
const DRAFT_KEY = 'home_safetyPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const HomeSafetyDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.home_safety) return Array.isArray(r.home_safety) ? r.home_safety : [r.home_safety];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.home_safety) return Array.isArray(dd.home_safety) ? dd.home_safety : [dd.home_safety]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Home Safety Assessment ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Stage a DRAFT for a whole-field value + persist it to the pending-drafts localStorage store.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Collect this record's staged drafts for fields in THIS section (editKey = `${fieldPart}-${idx}`)
      const suffix = `-${idx}`;
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length);
        // field = everything before a trailing ".<number>" (arrayIndex); otherwise the whole fieldPart
        const baseField = /\.\d+$/.test(fieldPart) ? fieldPart.replace(/\.\d+$/, '') : fieldPart;
        return fields.includes(baseField);
      });
      // Persist each staged field to the DB now (field, or field+arrayIndex for ".<number>" parts)
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/home_safety/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/home_safety/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HOME SAFETY ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Home Safety Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD — num-stepper (−/+ + text inputMode=decimal) + parseFloat+isNaN→block save ═══════ */
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(String(item)); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const newArr = [...arr]; newArr[aIdx] = editValue; stageDraft(record, fn, idx, newArr, sid); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="home-safety-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Home Safety Assessment</h2></div>
        <div className="empty-state">No home safety assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="home-safety-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Home Safety Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HomeSafetyDocumentPDFTemplate document={pdfData} />} fileName="Home_Safety.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search home safety assessments..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Home Safety Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'fall-risk')}
            {renderSection(record, idx, 'home-environment')}
            ...
            {renderSection(record, idx, 'cognitive-mental')}
            {renderSection(record, idx, 'nutritional-physical')}
...
export default HomeSafetyDocument;
