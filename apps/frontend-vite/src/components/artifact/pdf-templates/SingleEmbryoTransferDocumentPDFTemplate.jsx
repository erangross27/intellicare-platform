...
const SECTIONS = [
  { id: 'providerInfo', title: 'Provider Information', fields: ['date', 'provider', 'facility'] },
  { id: 'embryoDetails', title: 'Embryo Details', fields: ['embryoIdentificationNumber', 'embryoDevelopmentalStage', 'embryoQualityGrade', 'innerCellMassGrade', 'trophectodermGrade'] },
  { id: 'cryopreservation', title: 'Cryopreservation', fields: ['embryoCryopreservationDate', 'embryoThawDate', 'embryoSurvivalPostThaw'] },
  { id: 'transferProcedure', title: 'Transfer Procedure', fields: ['transferCycleType', 'endometrialThickness', 'endometrialPattern', 'transferCatheterType', 'transferDifficulty', 'ultrasoundGuidance', 'embryoPlacementLocation'] },
  { id: 'medications', title: 'Medications', fields: ['proceduralMedications'] },
  { id: 'hormonalSupport', title: 'Hormonal Support', fields: ['progesteroneSupplementationType', 'progesteroneDosage', 'estrogenSupplementation'] },
  { id: 'geneticTesting', title: 'Genetic Testing', fields: ['preimplantationGeneticTesting', 'geneticTestingResults'] },
  { id: 'followUp', title: 'Follow-Up', fields: ['scheduledPregnancyTestDate'] },
];
const FIELD_LABELS = {
...
export default function SingleEmbryoTransferDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Single Embryo Transfer</Text></View>
    {!records.length && <Text style={styles.noDataText}>No single embryo transfer data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Single Embryo Transfer Record {recordIndex + 1}</Text></View>
