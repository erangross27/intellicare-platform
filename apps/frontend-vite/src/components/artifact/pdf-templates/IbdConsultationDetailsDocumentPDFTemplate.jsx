/**
 * IbdConsultationDetailsDocumentPDFTemplate.jsx
 * Box-free B&W LETTER (canonical) — mirrors IbdConsultationDetailsDocument.jsx:
 * real record.date (never createdAt), config-driven SECTION_ORDER/SECTION_TITLES/FIELD_LABELS/
 * SECTION_FIELDS, dotted-path fields (resolvePath), deep recursive objects
 * (infliximabDrugMonitoring / barriersAndPsychosocialIssues / results) and object-arrays
 * (symptomProgressionTimeline / rescueTherapyOptions / ibdCareTeam / recommendations) flattened
 * box-free via generic objectRows/fieldBody — scalars inline "Key: value", arrays numbered, nested
 * objects under a subLabel. Narrative strings use [.;] sentence-split (abbrev/single-initial guard,
 * labeled -> subLabel + value, thousands-guarded comma-split). Values numbered ('1.' even singles).
 * Single-name label gate (fecalCalprotectin / infliximabDrugMonitoring / ... === section title -> label hidden).
 * Rule #74: each field is ONE wrap={false} atomic View with the sectionTitle riding INSIDE the first
 * present field's View. Static PHI footer. safeString uses ONLY \uXXXX escapes (mu-sign normalized).
...
/* ═══════ CONSTANTS ═══════ */
const SECTION_ORDER = ['provider-details', 'mayo-score', 'fecal-calprotectin', 'drug-monitoring', 'symptom-timeline', 'findings', 'assessment', 'plan', 'results', 'recommendations', 'rescue-therapy', 'care-team', 'barriers', 'notes'];
...
const FIELD_LABELS = {
  ...
  infliximabDrugMonitoring: 'Infliximab Drug Monitoring',
  symptomProgressionTimeline: 'Symptom Progression Timeline',
  findings: 'Findings',
  assessment: 'Assessment',
  plan: 'Plan',
  results: 'Results',
  recommendations: 'Recommendations',
  rescueTherapyOptions: 'Rescue Therapy Options',
  ibdCareTeam: 'IBD Care Team',
  barriersAndPsychosocialIssues: 'Barriers & Psychosocial Issues',
  notes: 'Notes',
};
const SECTION_FIELDS = {
  ...
  'drug-monitoring': ['infliximabDrugMonitoring'],
  'symptom-timeline': ['symptomProgressionTimeline'],
  'findings': ['findings'],
  'assessment': ['assessment'],
  'plan': ['plan'],
  'results': ['results'],
  'recommendations': ['recommendations'],
  'rescue-therapy': ['rescueTherapyOptions'],
  'care-team': ['ibdCareTeam'],
  'barriers': ['barriersAndPsychosocialIssues'],
  'notes': ['notes'],
};
...
/* Helvetica lacks the Greek mu (U+03BC); micro U+00B5 too — normalize measurement units to ASCII.
   NB: char classes use ONLY \uXXXX escapes — never paste literal smart-quotes/invisible chars here. */
...
const IbdConsultationDetailsDocumentPDFTemplate = ({ document: data }) => {
  let records = [];
  if (Array.isArray(data)) {
    if (data.length === 1 && data[0]?.ibd_consultation_details) records = Array.isArray(data[0].ibd_consultation_details) ? data[0].ibd_consultation_details : [data[0].ibd_consultation_details];
    else records = data;
  } else if (data?.ibd_consultation_details) records = Array.isArray(data.ibd_consultation_details) ? data.ibd_consultation_details : [data.ibd_consultation_details];
  else if (data?.documentData) { const dd = data.documentData; if (Array.isArray(dd)) records = dd; else if (dd?.ibd_consultation_details) records = Array.isArray(dd.ibd_consultation_details) ? dd.ibd_consultation_details : [dd.ibd_consultation_details]; else if (dd && typeof dd === 'object') records = [dd]; }
  ...
  if (records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>IBD Consultation Details</Text></View>
        <Text style={styles.emptyState}>No IBD consultation details records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`IBD Consultation Details ${idx + 1}`}</Text>
...
export default IbdConsultationDetailsDocumentPDFTemplate;
