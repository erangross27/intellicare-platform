/**
 * MedicalReconciliationFormsDocument.jsx
 * March 2026 — Canonical one-pass polish: inline editing, blue glow theme, box-free PDF.
 * Collection: medical_reconciliation_forms
 *
 * 10 Sections:
 *   1. recon-details: patientMedicalRecordNumber, reconciliationProvider, reconciliationMethod,
 *                     primaryCarePhysician, pharmacyContact, medicationAdherenceScore (NUM, hide-zero),
 *                     medicationChangesExplained (BOOL), clinicalPharmacistConsult (BOOL),
 *                     followupMedicationReview
 *   2. home-meds: homeOrCurrentMedications (ARRAY strings)
 *   3. hospital-discharge: hospitalAdmissionMedications (ARRAY), dischargeMedications (ARRAY)
 *   4. med-changes: medicationDiscrepancies (ARRAY), discontinuedMedications (ARRAY),
 *                   newMedicationsPrescribed (ARRAY), medicationDoseAdjustments (ARRAY)
 *   5. allergies: medicationAllergies (ARRAY strings)
 *   6. chronic-meds: chronicConditionsMedications (ARRAY strings)
 *   7. otc-meds: overthecounterMedications (ARRAY)
 *   8. safety-checks: beerscriteriaViolations (ARRAY), drugDrugInteractions (ARRAY strings, labeled)
 *   9. med-history: medicationHistorySource (ARRAY strings)
 *  10. understanding: patientMedicationUnderstanding (STRING long → splitBySentence)
 *
 * Record has only createdAt/updatedAt (no real clinical `date`) → NO record date, title only.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MedicalReconciliationFormsDocumentPDFTemplate from '../pdf-templates/MedicalReconciliationFormsDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './MedicalReconciliationFormsDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'recon-details': 'Reconciliation Details',
  'home-meds': 'Home / Current Medications',
  'hospital-discharge': 'Hospital & Discharge Medications',
  'med-changes': 'Medication Changes',
  'allergies': 'Medication Allergies',
  'chronic-meds': 'Chronic Conditions Medications',
  'otc-meds': 'Over-the-Counter Medications',
  'safety-checks': 'Safety Checks',
  'med-history': 'Medication History Sources',
  'understanding': 'Patient Medication Understanding',
};

const FIELD_LABELS = {
  patientMedicalRecordNumber: 'Medical Record Number',
  reconciliationProvider: 'Reconciliation Provider',
  ...
  medicationChangesExplained: 'Medication Changes Explained',
  clinicalPharmacistConsult: 'Clinical Pharmacist Consult',
  followupMedicationReview: 'Follow-up Medication Review',
  patientMedicationUnderstanding: 'Patient Medication Understanding',
  homeOrCurrentMedications: 'Home / Current Medications',
  hospitalAdmissionMedications: 'Hospital Admission Medications',
  dischargeMedications: 'Discharge Medications',
  medicationDiscrepancies: 'Medication Discrepancies',
  discontinuedMedications: 'Discontinued Medications',
  newMedicationsPrescribed: 'New Medications Prescribed',
  medicationDoseAdjustments: 'Medication Dose Adjustments',
  medicationAllergies: 'Medication Allergies',
  chronicConditionsMedications: 'Chronic Conditions Medications',
  overthecounterMedications: 'Over-the-Counter Medications',
  beerscriteriaViolations: 'Beers Criteria Violations',
  drugDrugInteractions: 'Drug-Drug Interactions',
  medicationHistorySource: 'Medication History Sources',
};

const SECTION_FIELDS = {
  'recon-details': ['patientMedicalRecordNumber', 'reconciliationProvider', 'reconciliationMethod', 'primaryCarePhysician', 'pharmacyContact', 'medicationAdherenceScore', 'medicationChangesExplained', 'clinicalPharmacistConsult', 'followupMedicationReview'],
  'home-meds': ['homeOrCurrentMedications'],
  'hospital-discharge': ['hospitalAdmissionMedications', 'dischargeMedications'],
  'med-changes': ['medicationDiscrepancies', 'discontinuedMedications', 'newMedicationsPrescribed', 'medicationDoseAdjustments'],
  'allergies': ['medicationAllergies'],
  'chronic-meds': ['chronicConditionsMedications'],
  'otc-meds': ['overthecounterMedications'],
  'safety-checks': ['beerscriteriaViolations', 'drugDrugInteractions'],
  'med-history': ['medicationHistorySource'],
  'understanding': ['patientMedicationUnderstanding'],
};

const NUMBER_FIELDS = ['medicationAdherenceScore'];
/* Sentinel-zero: a medication-adherence score of 0 is the extractor's unset default, never a real score,
   so 0-valued NUMBER_FIELDS are HIDDEN everywhere (JSX render + section-visibility + copy + PDF). */
const HIDE_ZERO_FIELDS = NUMBER_FIELDS;
const BOOLEAN_FIELDS = ['medicationChangesExplained', 'clinicalPharmacistConsult'];
const STRING_FIELDS = ['patientMedicalRecordNumber', 'reconciliationProvider', 'reconciliationMethod', 'primaryCarePhysician', 'pharmacyContact', 'followupMedicationReview', 'patientMedicationUnderstanding'];
const ARRAY_FIELDS = ['homeOrCurrentMedications', 'hospitalAdmissionMedications', 'dischargeMedications', 'medicationDiscrepancies', 'discontinuedMedications', 'newMedicationsPrescribed', 'medicationDoseAdjustments', 'medicationAllergies', 'chronicConditionsMedications', 'overthecounterMedications', 'beerscriteriaViolations', 'drugDrugInteractions', 'medicationHistorySource'];
...
/* extractItemLabel: label an array item ("Label: value") for display as a nested-subtitle above the value.
   Uses canonical parseLabel first, then a drug-combo fallback (parseLabel's class omits '+', so a
   "Drug A + Drug B: interaction" label is otherwise missed and would render as a side-by-side row). */
const extractItemLabel = (itemStr) => {
  const p = parseLabel(itemStr);
  if (p.isLabeled) return { label: p.label, value: p.value };
  const m = itemStr.match(/^([A-Z][A-Za-z0-9\s/&()+'"-]{1,58}?):\s+([\s\S]+)/);
  if (m && !CLAUSE_OPENER.test(m[1].trim())) return { label: m[1].trim(), value: m[2].trim() };
  return { label: '', value: itemStr };
};
...
const DRAFT_KEY = 'medical_reconciliation_formsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const MedicalReconciliationFormsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.medical_reconciliation_forms) return Array.isArray(r.medical_reconciliation_forms) ? r.medical_reconciliation_forms : [r.medical_reconciliation_forms];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.medical_reconciliation_forms) return Array.isArray(dd.medical_reconciliation_forms) ? dd.medical_reconciliation_forms : [dd.medical_reconciliation_forms]; return [dd]; }
      return [r];
    });
  ...
  /* hasFieldVal: hasVal + sentinel-zero hide (a 0-valued adherence score is the extractor's unset default) */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Medical Reconciliation Forms ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const field = m[1];
        await secureApiClient.put(`/api/edit/medical_reconciliation_forms/${id}/edit`, { field, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/medical_reconciliation_forms/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (ARRAY_FIELDS.includes(f)) {
        const items = getEffectiveArray(record, f, idx).filter(v => v !== null && v !== undefined && String(v).trim() !== '');
        if (items.length === 0) return;
        body += head; items.forEach((item, i) => { body += `${i + 1}. ${item}\n`; }); body += '\n';
      } else {
        const val = getFieldValue(record, f, idx);
    ...
    if (!body.trim()) return '';
    return `${title}\n${'='.repeat(40)}\n\n${body}`;
  }, [getFieldValue, getEffectiveArray, hasFieldVal, fmtVal, splitBySentence, formatSentenceFieldLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== MEDICAL RECONCILIATION FORMS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Medical Reconciliation Forms ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD — num-stepper (−/+) + input[type=text inputMode=decimal] ═══════ */
  ...
  /* ═══════ RENDER: BOOLEAN FIELD — BlueSelect Yes/No (saves editValue === 'Yes') ═══════ */
  ...
  /* ═══════ RENDER: ARRAY FIELD (string arrays; labeled items → sub-label above the value) ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {arr.map((item, ai) => {
          ...
          const parsed = extractItemLabel(itemStr);
          ...
          return (
            <div key={ai}>
              {parsed.label && <div className="nested-subtitle sub-label">{highlightText(parsed.label)}</div>}
              <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveArrayItem(record, fn, idx, ai, sid); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <>
                    <div className="row-content"><span className="content-value">{highlightText(parsed.label ? parsed.value : itemStr)}</span><span className="edit-indicator">&#9998;</span></div>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="medical-reconciliation-forms-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Medical Reconciliation Forms</h2></div>
        <div className="empty-state">No medical reconciliation forms records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="medical-reconciliation-forms-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Medical Reconciliation Forms</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MedicalReconciliationFormsDocumentPDFTemplate document={pdfData} />} fileName="Medical_Reconciliation_Forms.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search medical reconciliation forms..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Medical Reconciliation Forms ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'recon-details')}
            {renderSection(record, idx, 'home-meds')}
            {renderSection(record, idx, 'hospital-discharge')}
            {renderSection(record, idx, 'med-changes')}
            {renderSection(record, idx, 'allergies')}
            {renderSection(record, idx, 'chronic-meds')}
            {renderSection(record, idx, 'otc-meds')}
            {renderSection(record, idx, 'safety-checks')}
            {renderSection(record, idx, 'med-history')}
            {renderSection(record, idx, 'understanding')}
          </div>
        ))}
      </div>
    </div>
...
export default MedicalReconciliationFormsDocument;
