/**
 * DeliveryPlanningDocument.jsx
 * Updated per checklist (new theme, BlueDatePicker, 4-area copies, stripDelims, numbering)
 * Collection: delivery_planning
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import DeliveryPlanningDocumentPDFTemplate from '../pdf-templates/DeliveryPlanningDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import './DeliveryPlanningDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'provider-info': 'Provider Information',
  'delivery-plan': 'Delivery Plan',
  indications: 'Indications for Delivery',
  anesthesia: 'Anesthesia & Neonatology',
  preparations: 'Special Preparations',
...
const FIELD_LABELS = {
  ...
  facility: 'Facility',
  status: 'Status',
  targetGestationalAge: 'Target Gestational Age',
  plannedDeliveryMode: 'Planned Delivery Mode',
  indicationsForDelivery: 'Indications for Delivery',
  anesthesiaConsiderations: 'Anesthesia Considerations',
  neonatologyConsult: 'Neonatology Consult',
  specialPreparations: 'Special Preparations',
  postpartumContraception: 'Postpartum Contraception',
  earlyDeliveryConditions: 'Early Delivery Conditions',
  trialOfLaborCriteria: 'Trial of Labor Criteria',
  findings: 'Findings',
...
const SECTION_FIELDS = {
  ...
  'delivery-plan': ['targetGestationalAge', 'plannedDeliveryMode'],
  indications: ['indicationsForDelivery'],
  anesthesia: ['anesthesiaConsiderations', 'neonatologyConsult'],
  preparations: ['specialPreparations'],
  conditions: ['earlyDeliveryConditions', 'trialOfLaborCriteria', 'postpartumContraception'],
  'findings-assessment': ['findings', 'assessment'],
  plan: ['plan'],
  'recommendations-section': ['recommendations'],
  'results-section': ['results'],
...
const SECTION_ORDER = ['provider-info', 'delivery-plan', 'indications', 'anesthesia', 'preparations', 'conditions', 'findings-assessment', 'plan', 'recommendations-section', 'results-section', 'notes'];

const SENTENCE_FIELDS = ['earlyDeliveryConditions', 'plan', 'findings', 'assessment', 'notes'];
const ARRAY_FIELDS = ['indicationsForDelivery', 'anesthesiaConsiderations', 'specialPreparations'];
...
/* humanizeKey: object-key -> Title Case label */
const KEY_OVERRIDES = {};
...
const DRAFT_KEY = 'delivery_planningPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const DeliveryPlanningDocument = ({ document: docProp }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [copiedSection, setCopiedSection] = useState(null);
  const [copiedItems, setCopiedItems] = useState({});
  const [showCopied, setShowCopied] = useState(false);
  const [localEdits, setLocalEdits] = useState({});
  // editKeys (`${fn}-${idx}`) staged as drafts (saved locally, NOT yet committed to DB/PDF). Cleared on Approve.
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Delivery Planning ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (val && Array.isArray(val)) { for (const item of val) { if (String(item || '').toLowerCase().includes(phrase)) return true; } }
  ...
  /* ═══════ EDIT HANDLERS ═══════
     ...
     (handleApproveSection commits). All staged values are merged into localEdits under `${fn}-${idx}`,
     and the SAME key is flagged in pendingEdits + persisted in the per-record draft store. */
  ...
  const handleSaveField = useCallback((record, fn, idx, _sid, _sentIdx, valueOverride, editTrackingKey) => {
    ...
    const finalVal = typeof origVal === 'boolean' ? ['yes', 'true'].includes(String(saveVal).toLowerCase().trim()) : saveVal;
    setSaveError(null);
    stageDraft(record, fn, idx, finalVal);
  ...
  /* stage a nested OBJECT leaf edit by dot-path (e.g. results.ef) — value stays a STRING.
     The merged object is staged under `${rootField}-${idx}`; Approve commits the dotted path. */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    // Collect this record's pending edits whose field belongs to this section (key = `${fn}-${idx}`).
    const suffix = `-${idx}`;
    ...
    try {
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/delivery_planning/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/delivery_planning/${id}/approve`, { sectionId: sid, approved: true });
      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const next = { ...prev }; toCommit.forEach(k => delete next[k]); return next; });
      // Drop this record's committed drafts from localStorage
      const store = readDrafts();
      if (store[id]) { toCommit.forEach(k => { const fp = k.slice(0, -suffix.length); const ld = fp.lastIndexOf('.'); const base = (ld !== -1 && /^\d+$/.test(fp.slice(ld + 1))) ? fp.slice(0, ld) : fp; delete store[id][base]; }); if (Object.keys(store[id]).length === 0) delete store[id]; writeDrafts(store); }
    ...
    } catch (err) { console.error('[DeliveryPlanning] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  /* ═══════ FORMAT HELPERS FOR COPY (modern 4-area) ═══════ */
  const formatSentenceFieldLines = useCallback((text) => {
    ...
    sentences.forEach(s => {
      ...
      if (parsed.isLabeled) {
        ...
        const rawParts = splitByComma(clean);
        const parts = rawParts.length >= 3 ? rawParts : [clean];
  ...
  const objectCopyLines = (prefix, obj, depth) => {
    const lines = [];
    Object.entries(obj).forEach(([k, v]) => {
      if (isEmptyDeep(v)) return;
      const lab = prefix ? `${prefix} ${humanizeKey(k)}` : humanizeKey(k);
      if (isScalar(v)) {
        lines.push(`${lab}\n${COPY_LINE_DASH}\n1. ${stripDelims(fmtScalar(v))}`);
      } else {
        lines.push(`${lab}\n${COPY_LINE_DASH}`);
        objectCopyLines('', v, depth + 1).forEach(l => lines.push(l));
      }
    });
    return lines;
  };

  const recommendationsCopyLines = (recs) => {
    const lines = [];
    let n = 1;
    (Array.isArray(recs) ? recs : []).forEach(rec => {
      ...
      if (d) lines.push(d);
      lines.push(`${n++}. ${stripDelims(recText)}`);
    });
    return lines;
  };
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        if (!singleName) { text += `${label}\n${COPY_LINE_DASH}\n`; }
        arr.forEach((item, i) => { text += `${i + 1}. ${stripDelims(item)}\n`; });
        text += '\n';
        ...
        if (typeof val === 'object' && !isEmptyDeep(val)) {
          if (!singleName) { text += `${label}\n${COPY_LINE_DASH}\n`; }
    ...
    return text;
  }, [getFieldValue, getEffectiveArray, hasVal, fmtVal, formatDate, formatSentenceFieldLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== DELIVERY PLANNING ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Delivery Planning ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  const renderEditableField = (record, fn, idx, sid, title) => {
    ...
    return (
      <div key={fn} className={sl ? 'rec-mini-card' : ''}>
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(isDateField ? toDateInputVal(val) : isBool ? (val ? 'Yes' : 'No') : displayVal); setSaveError(null); } }}>
          {isEditing ? (
      ...
      <div key={fn}>
        <div className="rec-mini-card">
          ...
          {sentences.map((sentence, sIdx) => {
            ...
            const sentenceMatches = phraseMatch || labelMatch || (searchTerm.trim() && sentenceText.includes(searchPhrase));
        ...
        <div className="mini-cards-container">
          <div className="section-header">
            ...
            <div className="header-right-actions">
              <button className={`copy-btn ${copiedSection === copyId ? 'copied' : ''}`} onClick={() => {
                const text = `${title}\n${COPY_LINE_EQ}\n\n${arr.map((item, i) => `${i + 1}. ${stripDelims(item)}`).join('\n')}`;
          ...
          <div className="rec-mini-card">
            {arr.map((item, aIdx) => {
              ...
              const editKey = `${fieldName}-${idx}-a${aIdx}`;
                  ...
                  <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveArrayItem(record, fieldName, idx, aIdx); }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  /* ═══════ RENDER: ANESTHESIA & NEONATOLOGY SECTION ═══════ */
  const renderAnesthesiaSection = (record, idx) => {
    const sid = 'anesthesia';
    const title = SECTION_TITLES[sid];
    if (!shouldShowSection(record, sid)) return null;
    const arr = getEffectiveArray(record, 'anesthesiaConsiderations', idx);
    const neo = getFieldValue(record, 'neonatologyConsult', idx);
    if (arr.length === 0 && !hasVal(neo)) return null;
    const copyId = `${sid}-${idx}`;
    const phraseMatch = !searchTerm.trim() || sectionTitleMatches(sid) || record._showAllSections;
    return (
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {arr.length > 0 && (
            <div className="rec-mini-card">
              <div className="nested-subtitle">{highlightText('Anesthesia Considerations')}</div>
              {arr.map((item, aIdx) => {
                const itemStr = String(item || '');
                const editKey = `anesthesiaConsiderations-${idx}-a${aIdx}`;
                const isEditing = editingField === editKey;
                const isModified = editedFields[editKey];
                const itemMatches = phraseMatch || (searchTerm.trim() && itemStr.toLowerCase().includes(searchTerm.toLowerCase().trim()));
                if (!itemMatches && searchTerm.trim()) return null;
                    ...
                    <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                      {isEditing ? (
                        <div className="edit-field-container">
                          ...
                          {saveError && <div className="save-error">{saveError}</div>}
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveArrayItem(record, 'anesthesiaConsiderations', idx, aIdx); }}>{saving ? 'Saving...' : 'Save'}</button>
                            <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                          </div>
                        </div>
                      ) : (
                        <>
                          <div className="row-content"><span className="content-value">{highlightText(itemStr)}</span><span className="edit-indicator">✎</span></div>
                          <button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(stripDelims(itemStr), editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button>
                        </>
                      )}
                    </div>
                    {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
          ...
          {/* Neonatology Consult — boolean select */}
          {hasVal(neo) && renderEditableField(record, 'neonatologyConsult', idx, sid, title)}
        </div>
      </div>
    );
  };
                    ...
                    <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(recText); setSaveError(null); } }}>
                      {isEditing ? (
                        ...
                        <>
                          ...
                          <button className={`copy-btn ${copiedItems[itemKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${stripDelims(recText)}${recDate ? ` (${stripDelims(recDate)})` : ''}`, itemKey); }}>{copiedItems[itemKey] ? 'Copied!' : 'Copy'}</button>
                        </>
                      )}
                    </div>
  ...
  const renderSection = (record, idx, sid) => {
    if (sid === 'indications') return renderArraySection(record, idx, sid, 'indicationsForDelivery');
    if (sid === 'anesthesia') return renderAnesthesiaSection(record, idx);
    if (sid === 'preparations') return renderArraySection(record, idx, sid, 'specialPreparations');
    const title = SECTION_TITLES[sid];
    if (!shouldShowSection(record, sid)) return null;
    const fields = SECTION_FIELDS[sid] || [];
  ...
  if (!records || records.length === 0) {
    return (
      <div className="delivery-planning-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Delivery Planning</h2></div>
        <div className="empty-state">No delivery planning records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="delivery-planning-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Delivery Planning</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<DeliveryPlanningDocumentPDFTemplate document={pdfData} />} fileName={`delivery-planning-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search delivery planning..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Delivery Planning ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'provider-info')}
            {renderSection(record, idx, 'delivery-plan')}
            {renderSection(record, idx, 'indications')}
            {renderSection(record, idx, 'anesthesia')}
            {renderSection(record, idx, 'preparations')}
            {renderSection(record, idx, 'conditions')}
...
export default DeliveryPlanningDocument;
