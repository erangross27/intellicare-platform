/**
 * SleepHygieneEducationDocument.jsx
 * July 2026 — recursive, dot-path editable sleep hygiene education
...
import SleepHygieneEducationDocumentPDFTemplate from '../pdf-templates/SleepHygieneEducationDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SleepHygieneEducationDocument.css';

const DRAFT_KEY = 'sleep_hygiene_educationPendingEdits';
const SECTION_TITLES = {
  ...
  educationInterventions: 'Education & Interventions',
  techniquesMaterials: 'Techniques & Materials',
  followUp: 'Follow-Up',
};
const FIELD_LABELS = {
  date: 'Date', primarySleepComplaint: 'Primary Sleep Complaint', epworthSleepinessScore: 'Epworth Sleepiness Score', averageSleepLatency: 'Average Sleep Latency', totalSleepTime: 'Total Sleep Time', nighttimeAwakenings: 'Nighttime Awakenings', screenTimeBeforeBed: 'Screen Time Before Bed', regularBedtime: 'Regular Bedtime', regularWakeTime: 'Regular Wake Time', weekendSleepSchedule: 'Weekend Sleep Schedule', bedtimeRoutine: 'Bedtime Routine', caffeineIntake: 'Caffeine Intake', alcoholConsumption: 'Alcohol Consumption', nicotineUse: 'Nicotine Use', exerciseHabits: 'Exercise Habits', dinnertimeHabits: 'Dinnertime Habits', nappingFrequency: 'Napping Frequency', bedroomEnvironment: 'Bedroom Environment', educationTopicsCovered: 'Education Topics Covered', behavioralInterventions: 'Behavioral Interventions', stimulusControlInstructions: 'Stimulus Control Instructions', sleepRestrictionTherapy: 'Sleep Restriction Therapy', relaxationTechniques: 'Relaxation Techniques', educationalMaterials: 'Educational Materials', followUpInterval: 'Follow-Up Interval', sleepDiaryRequested: 'Sleep Diary Requested',
};
const SECTION_FIELDS = {
  recordInfo: ['date', 'primarySleepComplaint', 'epworthSleepinessScore'],
  sleepMetrics: ['averageSleepLatency', 'totalSleepTime', 'nighttimeAwakenings', 'screenTimeBeforeBed'],
  sleepSchedule: ['regularBedtime', 'regularWakeTime', 'weekendSleepSchedule', 'bedtimeRoutine'],
  lifestyleFactors: ['caffeineIntake', 'alcoholConsumption', 'nicotineUse', 'exerciseHabits', 'dinnertimeHabits', 'nappingFrequency'],
  sleepEnvironment: ['bedroomEnvironment'],
  educationInterventions: ['educationTopicsCovered', 'behavioralInterventions', 'stimulusControlInstructions', 'sleepRestrictionTherapy'],
  techniquesMaterials: ['relaxationTechniques', 'educationalMaterials'],
  followUp: ['followUpInterval', 'sleepDiaryRequested'],
};
const DATE_FIELDS = ['date'];
const DATETIME_FIELDS = [];
const ENUM_FIELDS = [];
const NUMBER_FIELDS = ['epworthSleepinessScore', 'averageSleepLatency', 'totalSleepTime', 'nighttimeAwakenings', 'screenTimeBeforeBed'];
const NUMBER_STRING_FIELDS = [];
const NUMBER_UNITS = { averageSleepLatency: 'min', totalSleepTime: 'hrs/night', nighttimeAwakenings: 'per night', screenTimeBeforeBed: 'hrs' };
const OBJECT_FIELDS = [];
...
const COMMA_FIELDS = ['caffeineIntake', 'alcoholConsumption', 'exerciseHabits', 'dinnertimeHabits', 'nappingFrequency'];
const COMMA_ARRAY_SPLIT_FIELDS = [];
const ARRAY_FIELDS = ['educationTopicsCovered', 'behavioralInterventions', 'relaxationTechniques', 'educationalMaterials'];
const SEMICOLON_FIELDS = ['bedtimeRoutine', 'bedroomEnvironment', 'exerciseHabits'];
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'status' ? ['Active', 'Inactive', 'Resolved'] : [];
...
const SleepHygieneEducationDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.sleep_hygiene_education) return Array.isArray(record.sleep_hygiene_education) ? record.sleep_hygiene_education : [record.sleep_hygiene_education];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.sleep_hygiene_education) return Array.isArray(nested.sleep_hygiene_education) ? nested.sleep_hygiene_education : [nested.sleep_hygiene_education];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/sleep_hygiene_education/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/sleep_hygiene_education/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SleepHygieneEducation] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    let text = '=== SLEEP HYGIENE EDUCATION ===\n\n';
    records.forEach((record, recordIndex) => {
      text += 'Sleep Hygiene Education Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="sleep-hygiene-education-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Sleep Hygiene Education</h2></div><div className="empty-state">No sleep hygiene education available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Sleep Hygiene Education Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="sleep-hygiene-education-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Sleep Hygiene Education</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<SleepHygieneEducationDocumentPDFTemplate document={pdfData} />} fileName="Sleep_Hygiene_Education.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search sleep hygiene education..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Sleep Hygiene Education Record ' + (recordIndex + 1))}</h3></div>
...
export default SleepHygieneEducationDocument;
