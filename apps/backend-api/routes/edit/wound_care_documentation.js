...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'wound-care-documentation-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['date', 'provider', 'facility', 'woundLocation', 'woundType', 'woundStage', 'woundLength', 'woundWidth', 'woundDepth', 'undermining', 'tunneling', 'woundBed', 'exudateAmount', 'exudateType', 'woundEdges', 'odorPresent', 'painScore', 'debridementMethod', 'primaryDressing', 'secondaryDressing', 'dressingChangeFrequency', 'offloadingDevice', 'periwoundSkin', 'cleansingAgent', 'photographsTaken'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('wound_care_documentation', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[wound_care_documentation] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('wound_care_documentation', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[wound_care_documentation] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
