/**
 * OvarianStimulationProtocolDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: ovarian_stimulation_protocol
 *
 * 8 Sections:
 *   1. protocol-overview: protocolType, gonadotropinType, gnrhAnalogType
 *   2. stimulation-dosing: startingGonadotropinDose, totalGonadotropinDose, stimulationDuration, gnrhAntagonistStartDay
 *   3. trigger-day: triggerMedication, triggerDayEstradiolLevel, triggerDayProgesteroneLevel, triggerDayLhLevel
 *   4. baseline-assessment: baselineAntralFollicleCount, baselineAmhLevel, bolognaCriteriaClassification
 *   5. follicle-monitoring: folliclesGreaterThan14mm, leadFollicleDiameter, endometrialThicknessAtTrigger
 *   6. oocyte-outcome: oocytesRetrieved, matureOocytesMii, oocyteYieldRatio, follicleOutputRate, ovarianSensitivityIndex
 *   7. ohss-management: ohssRiskCategory, ohssProphylaxisMeasures (ARRAY)
 *   8. luteal-support: lutealPhaseSupportRegimen
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import OvarianStimulationProtocolDocumentPDFTemplate from '../pdf-templates/OvarianStimulationProtocolDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './OvarianStimulationProtocolDocument.css';
...
/* number fields — ALL use the −/+ num-stepper (text input, never type=number) + parseFloat+isNaN→block save */
const NUMBER_FIELDS = [
...
/* Arrays (possibly empty): ohssProphylaxisMeasures (strings) */
const ARRAY_NUMBER_FIELDS = [];
const ARRAY_STRING_FIELDS = ['ohssProphylaxisMeasures'];
const ALL_ARRAY_FIELDS = [...ARRAY_NUMBER_FIELDS, ...ARRAY_STRING_FIELDS];

/* Sentinel-zero: a 0-valued protocol metric (dose, level, count, ratio, index) is never a real datum — it's the
   extractor's unset default, so 0-valued NUMBER_FIELDS are HIDDEN everywhere (JSX render + section-visibility
   + copy + PDF) so JSX / PDF / Copy-All agree. Memory 6a4fa368. */
...
const DRAFT_KEY = 'ovarian_stimulation_protocolPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const OvarianStimulationProtocolDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.ovarian_stimulation_protocol) return Array.isArray(r.ovarian_stimulation_protocol) ? r.ovarian_stimulation_protocol : [r.ovarian_stimulation_protocol];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.ovarian_stimulation_protocol) return Array.isArray(dd.ovarian_stimulation_protocol) ? dd.ovarian_stimulation_protocol : [dd.ovarian_stimulation_protocol]; return [dd]; }
      return [r];
    });
  ...
  /* hasFieldVal: hasVal + sentinel-zero hide (a 0-valued protocol metric is the extractor's unset default) */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Ovarian Stimulation Protocol ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const field = m[1];
        await secureApiClient.put(`/api/edit/ovarian_stimulation_protocol/${id}/edit`, { field, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/ovarian_stimulation_protocol/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== OVARIAN STIMULATION PROTOCOL ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Ovarian Stimulation Protocol ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: ARRAY FIELD (numbers or strings) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="ovarian-stimulation-protocol-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Ovarian Stimulation Protocol</h2></div>
        <div className="empty-state">No ovarian stimulation protocol records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="ovarian-stimulation-protocol-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Ovarian Stimulation Protocol</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OvarianStimulationProtocolDocumentPDFTemplate document={pdfData} />} fileName="Ovarian_Stimulation_Protocol.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search ovarian stimulation protocol..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Ovarian Stimulation Protocol ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'protocol-overview')}
            {renderSection(record, idx, 'stimulation-dosing')}
            {renderSection(record, idx, 'trigger-day')}
            {renderSection(record, idx, 'baseline-assessment')}
            {renderSection(record, idx, 'follicle-monitoring')}
            {renderSection(record, idx, 'oocyte-outcome')}
            {renderSection(record, idx, 'ohss-management')}
            {renderSection(record, idx, 'luteal-support')}
          </div>
        ))}
      </div>
    </div>
...
export default OvarianStimulationProtocolDocument;
