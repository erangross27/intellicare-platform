/**
 * SupportGroupReferralDocument.jsx
 * June 2026 — Collection: support_group_referral (NEW template)
 *
 * Donors cloned:
 ...
 *  - StemCellTransplantAssessmentDocument: renderBooleanField (Yes/No <select> saving real boolean),
 *    plus Yes/No branch in Copy Section/All.
 *
 * FIELDS (16 → 100% schema coverage):
 *   DATE:    date
 *   BOOLEAN: referred
 *   STRING:  groupType, location, frequency, contact, type, provider, facility,
...
import SupportGroupReferralDocumentPDFTemplate from '../pdf-templates/SupportGroupReferralDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SupportGroupReferralDocument.css';
...
const DRAFT_KEY = 'support_group_referralPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const SupportGroupReferralDocument = ({ document: docProp, data, templateData }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r.support_group_referral) return Array.isArray(r.support_group_referral) ? r.support_group_referral : [r.support_group_referral];
      if (r.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.support_group_referral) return Array.isArray(dd.support_group_referral) ? dd.support_group_referral : [dd.support_group_referral]; return [dd]; }
      return [r];
    });
    return arr.filter(r => r && typeof r === 'object');
  }, [docProp, data, templateData]);

  /* safeId helper (record _id, handling _id.$oid) — needed by rehydration below */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `support group referral ${idx + 1}`.toLowerCase();
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    // Stage as a DRAFT (no DB write). localStorage keeps it across refresh; Approve commits the full object.
    setLocalEdits(prev => {
    ...
    try {
      ...
      const toCommit = Object.keys(localEdits).filter(k => {
        ...
        const baseField = (() => {
          ...
          if (dotIdx !== -1 && /^\d+$/.test(fieldPart.slice(dotIdx + 1))) return fieldPart.slice(0, dotIdx);
          return fieldPart;
        })();
        return fields.includes(baseField);
      });
      // Persist each staged field to the DB now (field, or field+arrayIndex for numeric-dot keys).
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/support_group_referral/${id}/edit`, payload);
      }
      // Flag the record approved (audit trail)
      await secureApiClient.put(`/api/edit/support_group_referral/${id}/approve`, { sectionId: sid, approved: true });
      ...
      console.error('[SupportGroupReferral] Approve error:', err);
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== SUPPORT GROUP REFERRAL ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Support Group Referral ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="support-group-referral-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Support Group Referral</h2></div>
        <div className="empty-state">No support group referral records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="support-group-referral-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Support Group Referral</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<SupportGroupReferralDocumentPDFTemplate document={pdfData} />} fileName={`support-group-referral-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search support group referral records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Support Group Referral ${idx + 1}`)}</h3>
            </div>
...
export default SupportGroupReferralDocument;
