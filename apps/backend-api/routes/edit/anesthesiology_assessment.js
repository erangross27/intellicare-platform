/**
 * Anesthesiology Assessment Edit Route
 ...
 *
 * PUT /api/edit/anesthesiology_assessment/:id/edit    — Edit a single field
 * PUT /api/edit/anesthesiology_assessment/:id/approve — Approve a section
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'anesthesiology-assessment-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('anesthesiology_assessment', { _id: objectId }, {
      $set: {
        [updatePath]: value,
        updatedAt: new Date(),
    ...
    console.error('[AnesthesiologyAssessment] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    console.error('[AnesthesiologyAssessment] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
