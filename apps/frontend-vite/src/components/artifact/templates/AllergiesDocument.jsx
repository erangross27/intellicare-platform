...
import AllergiesDocumentPDFTemplate from '../pdf-templates/AllergiesDocumentPDFTemplate';
...
const SECTION_TITLES = { details: 'Allergy Details', care: 'Management & Notes' };
const FIELD_LABELS = {
...
const SECTION_FIELDS = {
  ...
  care: ['management', 'compliance', 'notes'],
};
...
const COMMA_STRING_FIELDS = new Set(['management']);
const SENTENCE_STRING_FIELDS = new Set(['allergen', 'reaction', 'severity', 'management', 'compliance', 'notes']);
const ENUM_OPTIONS = {
...
const unwrap = source => {
  ...
  rows = rows.flatMap(row => {
    ...
    if (row?.allergies) return Array.isArray(row.allergies) ? row.allergies : [row.allergies];
    if (row?.documentData) {
      const nested = row.documentData;
      if (Array.isArray(nested)) return nested;
      if (nested?.allergies) return Array.isArray(nested.allergies) ? nested.allergies : [nested.allergies];
      return [nested];
    }
    return [row];
  });
...
const AllergiesDocument = ({ document }) => {
  ...
  const approve = useCallback(async (record, section, index) => {
    ...
    try {
      ...
      for (const [path, value] of Object.entries(drafts)) { if (!SECTION_FIELDS[section].includes(path.split('.')[0])) continue; const response = await secureApiClient.put(`/api/edit/allergies/${id}/edit`, { field: path, value }); if (response?.success === false) throw new Error(response.error || 'Save failed'); committed.push(path); }
      await secureApiClient.put(`/api/edit/allergies/${id}/approve`, { sectionId: section, approved: true });
