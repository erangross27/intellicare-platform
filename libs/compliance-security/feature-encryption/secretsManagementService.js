...
class SecretsManagementService extends EventEmitter {
  constructor() {
    ...
    this.config = {
      ...
      vaultEnabled: false, // Will be loaded during initialization
  ...
  async initialize() {
    ...
    try {
      ...
      // Load configuration through proxy
      const secureConfigService = proxy.getService('secureConfigService');
      this.config.vaultEnabled = secureConfigService.get('VAULT_ENABLED', 'false') === 'true';
      this.config.vaultUrl = secureConfigService.get('VAULT_URL', 'http://localhost:8200');
      this.config.vaultToken = secureConfigService.get('VAULT_TOKEN', '');
      this.config.awsSecretsManager = secureConfigService.get('AWS_SECRETS_MANAGER', 'false') === 'true';
      this.config.awsRegion = secureConfigService.get('AWS_REGION', 'us-east-1');
      ...
      console.log('✅ Secrets Management Service initialized with ServiceProxy');
   ...
   */
  deriveMasterKey() {
    // Get through proxy if available, otherwise use defaults
    let masterPassword = 'default-development-key-not-secure';
    let masterSalt = 'default-salt';
    
    try {
      const proxy = getServiceProxy();
      const secureConfigService = proxy.getService('secureConfigService');
      masterPassword = secureConfigService.get('MASTER_SECRET_KEY') || masterPassword;
      masterSalt = secureConfigService.get('MASTER_SECRET_SALT') || masterSalt;
    } catch (error) {
      // Proxy not available yet, use defaults
      console.warn('SecureConfigService not available for master key derivation, using defaults');
    }
    
    const salt = Buffer.from(masterSalt, 'hex');
  ...
  checkAccess(secretName, operation, userId) {
    ...
    try {
      ...
      // Proxy not available, assume development
      return true;
    }
...
const secretsManagementServiceInstance = new SecretsManagementService();

// Register with service proxy
if (typeof module !== 'undefined' && module.exports) {
    const proxy = getServiceProxy();
    proxy.registerService('secretsManagementService', () => secretsManagementServiceInstance);
}

module.exports = secretsManagementServiceInstance;