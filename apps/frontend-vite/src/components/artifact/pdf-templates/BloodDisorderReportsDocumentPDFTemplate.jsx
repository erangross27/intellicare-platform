...
const COLLECTION = 'blood_disorder_reports';
const COMMA_SPLIT_FIELDS = ['peripheralSmear', 'severity'];
const ARRAY_FIELDS = new Set(['treatment']);
const OBJECT_FIELDS = new Set(['cbc', 'coagulationStudies', 'ironStudies', 'vitaminLevels', 'hemolysisWorkup']);
const NARRATIVE_FIELDS = new Set(['peripheralSmear', 'severity', 'boneMorrow', 'monitoring', 'notes']);
const DATE_FIELDS = new Set(['date']);
const ZERO_SENTINEL_FIELDS = new Set([]);
const KEY_OVERRIDES = {};
const LABELS = {
  date: 'Date', disorderType: 'Disorder Type', diagnosis: 'Diagnosis', etiology: 'Etiology',
  severity: 'Severity', cbc: 'CBC', coagulationStudies: 'Coagulation Studies',
  peripheralSmear: 'Peripheral Smear', boneMorrow: 'Bone Marrow', ironStudies: 'Iron Studies',
  vitaminLevels: 'Vitamin Levels', hemolysisWorkup: 'Hemolysis Workup', treatment: 'Treatment',
  monitoring: 'Monitoring', hematologist: 'Hematologist', facility: 'Facility', notes: 'Notes',
};
const SECTIONS = [
  { title: 'Date', fields: ['date'] },
  { title: 'Disorder', fields: ['disorderType', 'diagnosis', 'etiology', 'severity'] },
  { title: 'CBC', fields: ['cbc'] },
  { title: 'Coagulation Studies', fields: ['coagulationStudies'] },
  { title: 'Peripheral Smear', fields: ['peripheralSmear'] },
  { title: 'Bone Marrow', fields: ['boneMorrow'] },
  { title: 'Iron Studies', fields: ['ironStudies'] },
  { title: 'Vitamin Levels', fields: ['vitaminLevels'] },
  { title: 'Hemolysis Workup', fields: ['hemolysisWorkup'] },
  { title: 'Treatment', fields: ['treatment'] },
  { title: 'Monitoring', fields: ['monitoring'] },
  { title: 'Hematologist and Facility', fields: ['hematologist', 'facility'] },
...
const BloodDisorderReportsDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Blood Disorder Reports</Text><Text style={styles.noData}>No blood disorder report data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Blood Disorder Reports</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Blood Disorder Report {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
export default BloodDisorderReportsDocumentPDFTemplate;
