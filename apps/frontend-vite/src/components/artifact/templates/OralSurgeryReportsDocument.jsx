/**
 * OralSurgeryReportsDocument.jsx
 * June 2026 — Oral Surgery Reports (unified flat schema)
 * Data collection: oral_surgery_reports
 * Edit route: oral_surgery_reports
 *
 * 5 Sections (covering all 25 extractable fields, none added):
 *   1. procedure:           procedureType, toothNumber, procedureDuration, extractionDifficulty,
 *                           impactionClass, impactionPosition
 *   2. anesthesia:          anesthesiaType, anestheticAgent, anestheticVolume
 *   3. surgical-technique:  boneRemoval, toothSectioning, sutureType, sutureCount, hemostasisMethod
 *   4. graft-implant:       graftMaterial, graftVolume, membraneType, implantDiameter, implantLength,
 *                           primaryStability, insertionTorque
 *   5. postoperative:       complications, antibioticProphylaxis, painManagement, followUpInterval
 *
 * Field handling:
 ...
 *   - NUMBER  → numeric presence check (0/absent hidden, NEVER truthiness)
...
import OralSurgeryReportsDocumentPDFTemplate from '../pdf-templates/OralSurgeryReportsDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './OralSurgeryReportsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [field]: value } }  (value may be string/number/boolean/array) */
const DRAFT_KEY = 'oral_surgery_reportsPendingEdits';
const readDrafts = () => {
...
// Simple strings → click-to-edit textarea (renderEditableField)
const SIMPLE_STRING_FIELDS = ['procedureType', 'toothNumber', 'anesthesiaType', 'anestheticAgent', 'anestheticVolume', 'extractionDifficulty', 'impactionClass', 'impactionPosition', 'sutureType', 'hemostasisMethod', 'graftMaterial', 'graftVolume', 'membraneType', 'primaryStability', 'antibioticProphylaxis', 'followUpInterval'];
// Narrative strings → per-sentence editing (renderSentenceEditableField)
...
/* ═══════ COMPONENT ═══════ */
const OralSurgeryReportsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    if (!docProp) return [];
    const pick = (r) => r?.oral_surgery_reports;
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([field, value]) => {
        ...
        nSentences[`${field}-${idx}-s0`] = 'edited';
      });
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Oral Surgery Report ${idx + 1}`.toLowerCase();
  ...
  /* array item edit — uses arrayIndex. Save = stage the full merged array as a DRAFT (no DB write). */
  const handleSaveArrayItem = useCallback((record, field, idx, sectionId, arrayIndex, valueOverride) => {
    ...
    const store = readDrafts();
    if (!store[id]) store[id] = {};
    store[id][field] = currentArr;
    writeDrafts(store);
    setEditingField(null); setEditValue('');
  }, [editValue, safeId, getFieldValue]);

  // Save one sentence: splice it back into the full text, then stage the result as a DRAFT (no DB write).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // Collect this section's pending edits (localEdits keyed "<field>-<idx>").
      const toCommit = fields
        .map(f => `${f}-${idx}`)
        .filter(k => pendingEdits[k] && localEdits[k] !== undefined);
      // Persist each staged field to the DB now. Arrays are stored whole → no arrayIndex needed.
      for (const editKey of toCommit) {
        const field = editKey.slice(0, -`-${idx}`.length);
        const resp = await secureApiClient.put(`/api/edit/oral_surgery_reports/${id}/edit`, { field, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the record approved (audit trail)
      await secureApiClient.put(`/api/edit/oral_surgery_reports/${id}/approve`, { sectionId: sid, approved: true });
      ...
      console.error('[OralSurgeryReports] Approve error:', err);
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (NUMBER_FIELDS.includes(f) || BOOLEAN_FIELDS.includes(f)) {
        ...
        text += `${label}\n`;
        items.forEach((item, i) => { const p = parseLabel(String(item)); text += `${i + 1}. ${p.value || String(item)}\n`; });
        text += '\n';
      } else {
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== ORAL SURGERY REPORTS ===\n\n';
    pdfData.forEach((r, idx) => {
      const rt = `Oral Surgery Report ${idx + 1}`;
      text += `${rt}\n${'='.repeat(40)}\n\n`;
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                ...
                <input type="text" inputMode="decimal" className="edit-number" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter') { e.stopPropagation(); if (String(editValue).trim() === '') { setSaveError('This field cannot be empty.'); return; } const numVal = Number(editValue); if (isNaN(numVal)) { setSaveError('Please enter a valid number'); return; } handleSaveField(record, fn, idx, sid, 0, numVal); } }} />
  ...
  /* ═══════ RENDER: BOOLEAN FIELD — Yes/No select (false is DATA) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="oral-surgery-reports-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Oral Surgery Reports</h2></div>
        <div className="empty-state">No oral surgery report records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="oral-surgery-reports-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Oral Surgery Reports</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OralSurgeryReportsDocumentPDFTemplate document={pdfData} />} fileName="Oral_Surgery_Reports.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search oral surgery reports..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        {searchTerm && <span className="search-results">Showing {filteredRecords.length} of {records.length} records</span>}
      </div>
      <div className="records-container">
        {filteredRecords.map((record, idx) => {
          const title = `Oral Surgery Report ${idx + 1}`;
          return (
            <div key={idx} className="record-card">
              ...
              {renderSection(record, idx, 'anesthesia')}
              {renderSection(record, idx, 'surgical-technique')}
              {renderSection(record, idx, 'graft-implant')}
              {renderSection(record, idx, 'postoperative')}
            </div>
          );
        })}
      </div>
...
export default OralSurgeryReportsDocument;
