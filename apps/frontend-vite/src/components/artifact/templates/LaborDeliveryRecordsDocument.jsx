/**
 * LaborDeliveryRecordsDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: labor_delivery_records
 *
 * 5 Sections:
 *   1. labor-details: admissionDateTime, deliveryDateTime, gestationalAge, laborType, deliveryType, presentations
 *   2. delivery-details: anesthesia, episiotomy, lacerations, bloodLoss
 *   3. complications: complications
 *   4. placenta: placentaDelivery (mechanism, deliveredAt, durationThirdStage, examination, cordClamping)
 *   5. newborn-info: newborn (sex, birthWeight, length, headCircumference, apgar1Minute, apgar5Minutes, cordBloodGases)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import LaborDeliveryRecordsDocumentPDFTemplate from '../pdf-templates/LaborDeliveryRecordsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './LaborDeliveryRecordsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'labor_delivery_recordsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['gestationalAge', 'laborType', 'deliveryType', 'presentations', 'anesthesia', 'lacerations', 'bloodLoss', 'placentaDelivery.mechanism', 'placentaDelivery.deliveredAt', 'placentaDelivery.durationThirdStage', 'placentaDelivery.examination', 'placentaDelivery.cordClamping', 'newborn.sex', 'newborn.birthWeight', 'newborn.length', 'newborn.headCircumference', 'newborn.cordBloodGases'];
...
const formatDate = (dateValue) => {
  if (!dateValue) return '';
  try { const d = new Date(dateValue.$date || dateValue); return d.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' }); } catch { return String(dateValue); }
};
...
/* ═══════ COMPONENT ═══════ */
const LaborDeliveryRecordsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.labor_delivery_records) return Array.isArray(r.labor_delivery_records) ? r.labor_delivery_records : [r.labor_delivery_records];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.labor_delivery_records) return Array.isArray(dd.labor_delivery_records) ? dd.labor_delivery_records : [dd.labor_delivery_records]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      if (!recDrafts) return;
      // Group array-element drafts (field.N) so we can rebuild the whole array under "field-idx".
      const arrayDrafts = {};
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        const dotIdx = fieldPart.lastIndexOf('.');
        const trailing = dotIdx === -1 ? '' : fieldPart.slice(dotIdx + 1);
        if (dotIdx !== -1 && /^\d+$/.test(trailing)) {
          // array element: field + arrayIndex
          const baseField = fieldPart.slice(0, dotIdx);
          const arrIdx = parseInt(trailing, 10);
          if (!arrayDrafts[baseField]) arrayDrafts[baseField] = [];
          arrayDrafts[baseField][arrIdx] = value;
          const editKey = `${baseField}-${idx}`;
          nLocal[editKey] = editKey in nLocal ? nLocal[editKey] : null; // placeholder; filled below
          nPending[editKey] = true;
          nFields[`${baseField}.${arrIdx}-${idx}`] = 'edited';
        } else {
          // scalar / string field (dot-path field name with NON-numeric tail stays whole)
      ...
      // Merge array-element drafts on top of the record's original arrays.
      Object.entries(arrayDrafts).forEach(([baseField, elems]) => {
        const base = Array.isArray(record[baseField]) ? [...record[baseField]] : [];
        elems.forEach((v, i) => { if (v !== undefined) base[i] = v; });
  ...
  const getFieldValue = useCallback((record, fn, idx) => {
    ...
    /* Handle dot-path for nested fields */
    if (fn.includes('.')) {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Labor & Delivery Record ${idx + 1}`.toLowerCase();
  ...
  /* Find the section id that owns a (possibly dotted) field name. */
  ...
  /* Stage a DRAFT to the pending-drafts localStorage store (survives refresh). fieldPart =
     "field" for scalars/strings, or "field.arrayIndex" for a single array element. */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Persist each staged field in THIS section to the DB now (field, or field+arrayIndex for array elements).
      const store = readDrafts();
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        // Only commit drafts that belong to this section.
        ...
        await secureApiClient.put(`/api/edit/labor_delivery_records/${id}/edit`, payload);
        clearedLocalKeys.push(`${baseField}-${idx}`);
        delete recDrafts[fieldPart];
      }
      // Flag the section approved (audit trail).
      await secureApiClient.put(`/api/edit/labor_delivery_records/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== LABOR & DELIVERY RECORDS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Labor & Delivery Record ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: currentArr })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); clearApprovedForField(fn, idx); stageDraft(record, `${fn}.${itemIdx}`, editValue); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="labor-delivery-records-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Labor & Delivery Records</h2></div>
        <div className="empty-state">No labor and delivery records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="labor-delivery-records-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Labor & Delivery Records</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<LaborDeliveryRecordsDocumentPDFTemplate document={pdfData} />} fileName="Labor_Delivery_Records.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search labor & delivery records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Labor & Delivery Record ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'labor-details')}
            {renderSection(record, idx, 'delivery-details')}
            {renderSection(record, idx, 'complications')}
            {renderSection(record, idx, 'placenta')}
            {renderSection(record, idx, 'newborn-info')}
          </div>
        ))}
      </div>
    </div>
...
export default LaborDeliveryRecordsDocument;
