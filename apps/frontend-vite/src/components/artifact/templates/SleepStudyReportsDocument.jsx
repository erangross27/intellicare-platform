/**
 * SleepStudyReportsDocument.jsx
 * July 2026 — recursive, dot-path editable sleep study reports
...
import SleepStudyReportsDocumentPDFTemplate from '../pdf-templates/SleepStudyReportsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SleepStudyReportsDocument.css';

const DRAFT_KEY = 'sleep_study_reportsPendingEdits';
const SECTION_TITLES = {
  studyDetails: 'Study Details',
  sleepIndices: 'Sleep Indices',
  oxygenation: 'Oxygenation',
  sleepArchitecture: 'Sleep Architecture',
  respiratoryEvents: 'Respiratory Events',
...
const SECTION_FIELDS = {
  studyDetails: ['additionalData'],
  sleepIndices: ['apneaHypopneaIndex', 'respiratoryDisturbanceIndex', 'oxygenDesaturationIndex'],
  oxygenation: ['minimumOxygenSaturation', 'meanOxygenSaturation'],
  sleepArchitecture: ['totalSleepTime', 'sleepEfficiency', 'sleepLatency', 'remLatency', 'wakeAfterSleepOnset', 'arousalIndex'],
  respiratoryEvents: ['centralApneaIndex', 'obstructiveApneaIndex', 'mixedApneaIndex', 'hypopneaIndex'],
  otherMeasurements: ['periodicLimbMovementIndex', 'epworthSleepinessScale', 'pittsburghSleepQualityIndex', 'snoreIntensity', 'sleepStageDistribution', 'cpapTitrationPressure', 'bodyPositionDependency', 'cheyneStokesBrewing'],
};
const DATE_FIELDS = [];
const DATETIME_FIELDS = [];
const ENUM_FIELDS = ['snoreIntensity'];
...
const unitForPath = (path) => NUMBER_UNITS[normalizeRulePath(path)] || NUMBER_UNITS[rootOf(path)];
const hasVal = (value) => {
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'snoreIntensity' ? ['Mild', 'Moderate', 'Severe'] : [];
...
const SleepStudyReportsDocument = ({ document: docProp }) => {
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/sleep_study_reports/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/sleep_study_reports/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SleepStudyReports] Approve error:', error);
  ...
  const leafProps = (record, path, recordIndex, sectionId, value, copyText) => {
    ...
    const configuredUnit = unitForPath(path);
  ...
  const renderSleepStageDistribution = (record, field, recordIndex, sectionId) => {
    const items = getValue(record, field, recordIndex);
    if (!Array.isArray(items) || !items.some(hasVal)) return null;
    const stageLabels = ['N1', 'N2', 'N3', 'REM'];
    return <div key={field} className="field-group">
      <div className="nested-subtitle field-group-title">{highlightText(FIELD_LABELS[field])}</div>
      {items.map((item, itemIndex) => {
        const path = field + '.' + itemIndex;
        const label = stageLabels[itemIndex] || 'Stage ' + (itemIndex + 1);
        const shown = displayScalar(item) + ' %';
        return <div className="rec-mini-card nested-mini-card" key={path}>
          <div className="nested-subtitle">{highlightText(label)}</div>
          <EditableLeaf {...leafProps(record, path, recordIndex, sectionId, item, label + '\n' + shown)} />
        </div>;
      })}
    </div>;
  };
  ...
  const renderScalarField = (record, field, recordIndex, sectionId) => {
    ...
    return <div key={field} className="rec-mini-card nested-mini-card">
      ...
      <EditableLeaf {...leafProps(record, field, recordIndex, sectionId, value, label + '\n' + (isDatePathValue(field, value) ? formatDate(value) : DATETIME_FIELDS.includes(field) ? formatDateTime(value) : unitForPath(field) && typeof value === 'number' ? displayScalar(value) + ' ' + unitForPath(field) : displayScalar(value)))} />
    </div>;
  };
  ...
  const renderRecursiveNode = (record, value, basePath, recordIndex, sectionId, label) => {
    ...
    return <div className="nested-group" key={basePath}>
      {label && <div className="nested-subtitle field-group-title">{highlightText(label)}</div>}
      {Object.entries(value).filter(([key, child]) => hasVal(child) && !pathHidden(basePath + '.' + key)).map(([key, child]) =>
  ...
  const copyRecursiveLines = (value, basePath, indent = '') => {
    ...
    return Object.entries(value).filter(([key]) => !pathHidden(basePath + '.' + key)).flatMap(([key, child]) => copyRecursiveLines(child, basePath + '.' + key, indent));
  };
  const buildSectionCopyText = useCallback((record, recordIndex, sectionId) => {
    ...
    (SECTION_FIELDS[sectionId] || []).forEach((field) => {
      ...
      const label = FIELD_LABELS[field] || humanizeKey(field);
      const showLabel = label.toLowerCase() !== SECTION_TITLES[sectionId].toLowerCase();
      if (field === 'sleepStageDistribution' && Array.isArray(value) && value.every(isScalar)) {
        if (showLabel) text += label + '\n';
        const stageLabels = ['N1', 'N2', 'N3', 'REM'];
        value.forEach((item, index) => { text += (stageLabels[index] || 'Stage ' + (index + 1)) + '\n1. ' + displayScalar(item) + ' %\n'; });
      } else if (field === 'measurableDisease') {
      ...
      } else if (unitForPath(field) && typeof value === 'number') {
        if (showLabel) text += label + '\n';
        text += '1. ' + value + ' ' + unitForPath(field) + '\n';
      } else {
        if (showLabel) text += label + '\n';
  ...
  const copyAll = useCallback(async () => {
    let text = '=== SLEEP STUDY REPORTS ===\n\n';
    records.forEach((record, recordIndex) => {
      text += 'Sleep Study Reports Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
      ...
      {sectionId === 'results' ? renderResults(record, recordIndex, sectionId)
          : sectionId === 'recommendations' ? renderRecommendations(record, recordIndex, sectionId)
            : fields.map((field) => field === 'recommendations'
              ...
              : field === 'sleepStageDistribution' && Array.isArray(getValue(record, field, recordIndex)) && getValue(record, field, recordIndex).every(isScalar)
              ? renderSleepStageDistribution(record, field, recordIndex, sectionId)
              : MIXED_SCALAR_OBJECT_FIELDS.includes(field) && isScalar(getValue(record, field, recordIndex))
  ...
  if (!records.length) return <div className="sleep-study-reports-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Sleep Study Reports</h2></div><div className="empty-state">No sleep study reports available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Sleep Study Reports Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="sleep-study-reports-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Sleep Study Reports</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<SleepStudyReportsDocumentPDFTemplate document={pdfData} />} fileName="Sleep_Study_Reports.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search sleep study reports..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Sleep Study Reports Record ' + (recordIndex + 1))}</h3></div>
...
export default SleepStudyReportsDocument;
