/**
 * NonMotorSymptomsDocument.jsx
 * Canonical one-pass polish — blue glow editing theme
 * Collection: non_motor_symptoms
 *
 * Sections: Record Information · Cognitive Symptoms · Neuropsychiatric Symptoms ·
 *           Sleep Symptoms · Autonomic Symptoms · Sensory Symptoms · Findings ·
 *           Assessment · Plan · Results · Additional Information
 *
 * Top-level scalar strings (provider/facility/findings/assessment/plan) are inline-editable
 * (mirrors the canonical renderStringField). `date` is inline-editable via BlueDatePicker.
 * The deeply nested symptom objects (cognitive/neuropsychiatric{hallucinations{…}}/sleep/
 * autonomic/sensory/results) are decomposed by a GENERIC RECURSIVE renderer (label ABOVE
 * value — never a side-by-side "Label: value" span) and rendered read-only. `plan` is a
 * comma-separated list → decomposed into numbered items in Copy (splitByComma).
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NonMotorSymptomsDocumentPDFTemplate from '../pdf-templates/NonMotorSymptomsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './NonMotorSymptomsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'non_motor_symptomsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['provider', 'facility', 'findings', 'assessment', 'plan'];
/* Read-only scalars (display rows, no edit widget) */
const DISPLAY_FIELDS = ['type', 'status'];
/* Comma-separated list fields — decomposed into numbered items in Copy */
const COMMA_LIST_FIELDS = ['plan'];
/* Read-only, decomposed arrays + objects */
const ARRAY_FIELDS = ['recommendations'];
const OBJECT_FIELDS = ['cognitive', 'neuropsychiatric', 'sleep', 'autonomic', 'sensory', 'results'];
const NUMBER_FIELDS = [];
const DATE_FIELDS = ['date'];

/* ═══════ OBJECT-FIELD HELPERS ═══════ */
const KEY_OVERRIDES = {
  moca: 'MoCA', mmse: 'MMSE', rem: 'REM', bp: 'BP', icd: 'ICD',
  rbd: 'RBD', hr: 'HR', ecg: 'ECG', mri: 'MRI',
};
...
/* ═══════ COMPONENT ═══════ */
const NonMotorSymptomsDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.non_motor_symptoms) return Array.isArray(r.non_motor_symptoms) ? r.non_motor_symptoms : [r.non_motor_symptoms];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.non_motor_symptoms) return Array.isArray(dd.non_motor_symptoms) ? dd.non_motor_symptoms : [dd.non_motor_symptoms]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Non-Motor Symptoms ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/non_motor_symptoms/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/non_motor_symptoms/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const formatSentenceFieldLines = useCallback((fn, text) => {
    const sentences = splitBySentence(text);
    const isCommaList = COMMA_LIST_FIELDS.includes(fn);
    const lines = []; let n = 1;
    sentences.forEach(s => {
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_FIELDS.includes(f)) {
        ...
        if (showLabel) text += `${label}\n`;
        if (sentences.length > 1 || (sentences.length === 1 && parseLabel(sentences[0]).isLabeled) || COMMA_LIST_FIELDS.includes(f)) {
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NON-MOTOR SYMPTOMS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Non-Motor Symptoms ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT FIELD (cognitive / neuropsychiatric / … — read-only, recursive) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="non-motor-symptoms-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Non-Motor Symptoms</h2></div>
        <div className="empty-state">No non-motor symptoms records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="non-motor-symptoms-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Non-Motor Symptoms</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NonMotorSymptomsDocumentPDFTemplate document={pdfData} />} fileName="Non_Motor_Symptoms.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search non-motor symptoms..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Non-Motor Symptoms ${idx + 1}`)}</h3>
            </div>
...
export default NonMotorSymptomsDocument;
