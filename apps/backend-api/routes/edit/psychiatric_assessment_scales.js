/**
 * PsychiatricAssessmentScales Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/psychiatric_assessment_scales/:id/edit    — Edit a single field
 * PUT /api/edit/psychiatric_assessment_scales/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'psychiatric-assessment-scales-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('psychiatric_assessment_scales', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[PsychiatricAssessmentScales] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('psychiatric_assessment_scales', { _id: objectId }, {
      $set: {
    ...
    console.error('[PsychiatricAssessmentScales] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
