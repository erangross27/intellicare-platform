...
const COLLECTION = 'cardiology_assessment';
const COMMA_SPLIT_FIELDS = ['findings', 'notes'];
const ARRAY_FIELDS = new Set([]);
const OBJECT_FIELDS = new Set(['echocardiogram', 'electrocardiogram', 'cardiacCatheterization', 'stressTest', 'coronaryArteryDiseaseRiskFactors', 'scheduledProcedures', 'additionalTestingOrdered', 'recommendations', 'results', 'additionalData']);
...
const KEY_OVERRIDES = { lvedd: 'LVEDD', lvesd: 'LVESD', rvsp: 'RVSP', qtcInterval: 'QTc Interval', qrsDuration: 'QRS Duration', stChanges: 'ST Changes', ecgChanges: 'ECG Changes', timiFlow: 'TIMI Flow' };
const LABELS = {
  date: 'Date', provider: 'Provider', facility: 'Facility', type: 'Type', status: 'Status',
  echocardiogram: 'Echocardiogram', electrocardiogram: 'Electrocardiogram', cardiacCatheterization: 'Cardiac Catheterization', stressTest: 'Stress Test',
  coronaryArteryDiseaseRiskFactors: 'Coronary Artery Disease Risk Factors', scheduledProcedures: 'Scheduled Procedures', additionalTestingOrdered: 'Additional Testing Ordered',
  findings: 'Findings', assessment: 'Assessment', plan: 'Plan', recommendations: 'Recommendations', results: 'Results', notes: 'Notes', additionalData: 'Additional Data',
};
const SECTIONS = [
  { title: 'Record Information', fields: ['date', 'provider', 'facility', 'type', 'status'] },
  { title: 'Echocardiogram', fields: ['echocardiogram'] },
  { title: 'Electrocardiogram', fields: ['electrocardiogram'] },
  { title: 'Cardiac Catheterization', fields: ['cardiacCatheterization'] },
  { title: 'Stress Test', fields: ['stressTest'] },
  { title: 'Coronary Artery Disease Risk Factors', fields: ['coronaryArteryDiseaseRiskFactors'] },
  { title: 'Scheduled Procedures', fields: ['scheduledProcedures'] },
  { title: 'Additional Testing Ordered', fields: ['additionalTestingOrdered'] },
...
const CardiologyAssessmentDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Cardiology Assessment</Text><Text style={styles.noData}>No cardiology assessment data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Cardiology Assessment</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Cardiology Assessment {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
