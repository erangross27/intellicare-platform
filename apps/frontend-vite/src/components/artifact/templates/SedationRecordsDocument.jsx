/**
 * SedationRecordsDocument.jsx
 * July 2026 — recursive, dot-path editable sedation records
...
import SedationRecordsDocumentPDFTemplate from '../pdf-templates/SedationRecordsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SedationRecordsDocument.css';

const DRAFT_KEY = 'sedation_recordsPendingEdits';
const SECTION_TITLES = {
  providerInfo: 'Provider Information',
  procedureInfo: 'Procedure Information',
  preSedation: 'Pre-Sedation Assessment',
  medications: 'Medications',
  timingMonitoring: 'Timing & Monitoring',
  vitalsAirway: 'Vitals & Airway',
  eventsInterventions: 'Events & Interventions',
  recovery: 'Recovery',
};
const FIELD_LABELS = {
  date: 'Date', procedureName: 'Procedure Name', sedationLevel: 'Sedation Level', asaClassification: 'ASA Classification', preoperativeFastingHours: 'Preoperative Fasting Hours', baselineVitalSigns: 'Baseline Vital Signs', consentObtained: 'Consent Obtained', allergyVerification: 'Allergy Verification', sedativeMedicationsAdministered: 'Sedative Medications Administered', reversalAgentsGiven: 'Reversal Agents Given', sedationStartTime: 'Sedation Start Time', sedationEndTime: 'Sedation End Time', totalProcedureDuration: 'Total Procedure Duration', ramsaySedationScore: 'Ramsay Sedation Score', intraoperativeVitals: 'Intraoperative Vitals', oxygenSupplementation: 'Oxygen Supplementation', airwayManagement: 'Airway Management', adverseEvents: 'Adverse Events', interventionsRequired: 'Interventions Required', dischargeAldreteScore: 'Discharge Aldrete Score', recoveryRoomArrivalTime: 'Recovery Room Arrival Time', recoveryRoomDischargeTime: 'Recovery Room Discharge Time', postSedationInstructions: 'Post-Sedation Instructions', anesthesiaProviderName: 'Anesthesia Provider Name', monitoringNurseName: 'Monitoring Nurse Name',
};
const SECTION_FIELDS = {
  providerInfo: ['anesthesiaProviderName', 'monitoringNurseName'],
  procedureInfo: ['date', 'procedureName', 'sedationLevel', 'asaClassification'],
  preSedation: ['preoperativeFastingHours', 'baselineVitalSigns', 'consentObtained', 'allergyVerification'],
  medications: ['sedativeMedicationsAdministered', 'reversalAgentsGiven'],
  timingMonitoring: ['sedationStartTime', 'sedationEndTime', 'totalProcedureDuration', 'ramsaySedationScore'],
  vitalsAirway: ['intraoperativeVitals', 'oxygenSupplementation', 'airwayManagement'],
  eventsInterventions: ['adverseEvents', 'interventionsRequired'],
  recovery: ['dischargeAldreteScore', 'recoveryRoomArrivalTime', 'recoveryRoomDischargeTime', 'postSedationInstructions'],
};
const DATE_FIELDS = ['date'];
const ENUM_FIELDS = ['sedationLevel', 'asaClassification'];
const NUMBER_FIELDS = ['preoperativeFastingHours', 'dischargeAldreteScore', 'totalProcedureDuration'];
const NUMBER_STRING_FIELDS = [];
const OBJECT_FIELDS = ['baselineVitalSigns', 'intraoperativeVitals', 'oxygenSupplementation'];
const OBJECT_ITEM_LABELS = { intraoperativeVitals: 'Vital Set' };
const NARRATIVE_PATHS = [];
const PARENTHETICAL_LABEL_FIELDS = [];
const COMMA_FIELDS = [];
const COMMA_ARRAY_FIELDS = [];
const ARRAY_FIELDS = ['sedativeMedicationsAdministered', 'reversalAgentsGiven', 'ramsaySedationScore', 'adverseEvents', 'interventionsRequired'];
const SEMICOLON_FIELDS = ['allergyVerification', 'airwayManagement', 'postSedationInstructions'];
...
const KEY_LABELS = { heartRate: 'Heart Rate', bloodPressure: 'Blood Pressure', oxygenSaturation: 'Oxygen Saturation', respiratoryRate: 'Respiratory Rate', flowRate: 'Flow Rate' };
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'sedationLevel' ? ['Minimal sedation', 'Light sedation', 'Moderate sedation', 'Deep sedation', 'General anesthesia']
    : field === 'asaClassification' ? ['ASA I', 'ASA II', 'ASA III', 'ASA IV', 'ASA V', 'ASA VI', 'ASA E'] : [];
...
const SedationRecordsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.sedation_records) return Array.isArray(record.sedation_records) ? record.sedation_records : [record.sedation_records];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.sedation_records) return Array.isArray(nested.sedation_records) ? nested.sedation_records : [nested.sedation_records];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/sedation_records/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/sedation_records/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SedationRecords] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    let text = '=== SEDATION RECORDS ===\n\n';
    records.forEach((record, recordIndex) => {
      text += 'Sedation Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="sedation-records-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Sedation Records</h2></div><div className="empty-state">No sedation records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Sedation Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="sedation-records-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Sedation Records</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<SedationRecordsDocumentPDFTemplate document={pdfData} />} fileName="Sedation_Records.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search sedation records..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Sedation Record ' + (recordIndex + 1))}</h3></div>
...
export default SedationRecordsDocument;
