...
function buildContext(req, operation = 'read') { return { serviceId: 'foot-orthotics-assessment-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['patientWeightBearing', 'rearfootAlignment', 'forefootToRearfootRelationship', 'subtalarJointROM', 'firstMTPJointROM', 'ankleJointDorsiflexion', 'archHeightIndex', 'footPostureIndex', 'naviularDropTest', 'tooManyToesSign', 'jackTest', 'singleHeelRiseTest', 'colemanBlockTest', 'gaitCycleAbnormalities', 'plantarPressureDistribution', 'legLengthDiscrepancy', 'tibialVarumAngle', 'metatarsalPhalangealAlignment', 'orthoticDeviceType', 'orthoticShellMaterial', 'medialArchSupport', 'metatarsalModifications', 'rearfootPosting', 'forefootPosting', 'diabeticFootRiskCategory'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('foot_orthotics_assessment', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[FootOrthoticsAssessment] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('foot_orthotics_assessment', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[FootOrthoticsAssessment] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
