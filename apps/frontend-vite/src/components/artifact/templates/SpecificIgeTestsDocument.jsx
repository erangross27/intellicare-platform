/**
 * SpecificIgeTestsDocument.jsx
 * July 2026 — recursive, dot-path editable specific IgE tests
 * Collection: specific_ige_tests
 */
import React, { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import SpecificIgeTestsDocumentPDFTemplate from '../pdf-templates/SpecificIgeTestsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SpecificIgeTestsDocument.css';

const DRAFT_KEY = 'specific_ige_testsPendingEdits';
...
const FIELD_LABELS = {
  allergenTested: 'Allergen Tested', allergenCategory: 'Allergen Category', allergenSource: 'Allergen Source', allergenCode: 'Allergen Code', testMethodology: 'Test Methodology', igeLevel: 'IgE Level', allergenClass: 'Allergen Class', totalIgeLevel: 'Total IgE Level', referenceRange: 'Reference Range', componentTesting: 'Component Testing', clinicalRelevance: 'Clinical Relevance', anaphylaxisRisk: 'Anaphylaxis Risk', crossReactivity: 'Cross Reactivity', immunotherapyCandidate: 'Immunotherapy Candidate', avoidanceRecommendations: 'Avoidance Recommendations', medicationInteractions: 'Medication Interactions', ageAtFirstExposure: 'Age at First Exposure', seasonalVariation: 'Seasonal Variation', geographicRelevance: 'Geographic Relevance', retestRecommendation: 'Retest Recommendation', additionalData: 'Additional Data',
};
const SECTION_FIELDS = {
  testInformation: ['allergenTested', 'allergenCategory', 'allergenSource', 'allergenCode', 'testMethodology'],
  testResults: ['igeLevel', 'allergenClass', 'totalIgeLevel', 'referenceRange', 'componentTesting'],
  clinicalAssessment: ['clinicalRelevance', 'anaphylaxisRisk', 'crossReactivity'],
  treatmentPlan: ['immunotherapyCandidate', 'avoidanceRecommendations', 'medicationInteractions'],
  patientHistory: ['ageAtFirstExposure', 'seasonalVariation', 'geographicRelevance'],
  followUp: ['retestRecommendation'],
  additionalInformation: ['additionalData'],
};
const DATE_FIELDS = [];
const DATETIME_FIELDS = [];
const ENUM_FIELDS = ['allergenCategory', 'testMethodology'];
const NUMBER_FIELDS = ['igeLevel', 'allergenClass', 'totalIgeLevel', 'ageAtFirstExposure'];
const NUMBER_STRING_FIELDS = [];
const NUMBER_UNITS = { igeLevel: 'kU/L', totalIgeLevel: 'kU/L', ageAtFirstExposure: 'years' };
const OBJECT_FIELDS = ['additionalData'];
const MIXED_OBJECT_ARRAY_FIELDS = [];
const OBJECT_ITEM_LABELS = {};
const NARRATIVE_PATHS = ['clinicalRelevance', 'componentTesting[]', 'crossReactivity[]', 'avoidanceRecommendations[]', 'medicationInteractions[]', 'geographicRelevance', 'retestRecommendation', 'additionalData.lab.notes[]'];
const PARENTHETICAL_LABEL_FIELDS = [];
const PARENTHETICAL_SEMICOLON_FIELDS = [];
const COMMA_FIELDS = [];
const COMMA_ARRAY_SPLIT_FIELDS = ['additionalData.lab.notes'];
const ARRAY_FIELDS = ['componentTesting', 'crossReactivity', 'avoidanceRecommendations', 'medicationInteractions'];
const SEMICOLON_FIELDS = ['componentTesting', 'medicationInteractions', 'additionalData.lab.notes'];
...
const KEY_LABELS = { igeLevel: 'IgE Level', totalIgeLevel: 'Total IgE Level', allergenCode: 'Allergen Code', lab: 'Lab' };
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'allergenCategory' ? ['Environmental', 'Food', 'Medication', 'Animal', 'Insect', 'Other']
    : field === 'testMethodology' ? ['ImmunoCAP', 'ELISA', 'RAST', 'Component-resolved diagnostics']
      : [];
...
const SpecificIgeTestsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.specific_ige_tests) return Array.isArray(record.specific_ige_tests) ? record.specific_ige_tests : [record.specific_ige_tests];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.specific_ige_tests) return Array.isArray(nested.specific_ige_tests) ? nested.specific_ige_tests : [nested.specific_ige_tests];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/specific_ige_tests/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/specific_ige_tests/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SpecificIgeTests] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    let text = '=== SPECIFIC IGE TESTS ===\n\n';
    records.forEach((record, recordIndex) => {
      text += 'Specific IgE Tests Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="specific-ige-tests-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Specific IgE Tests</h2></div><div className="empty-state">No specific IgE tests available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Specific IgE Tests Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="specific-ige-tests-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Specific IgE Tests</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<SpecificIgeTestsDocumentPDFTemplate document={pdfData} />} fileName="Specific_IgE_Tests.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search specific IgE tests..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Specific IgE Tests Record ' + (recordIndex + 1))}</h3></div>
...
export default SpecificIgeTestsDocument;
