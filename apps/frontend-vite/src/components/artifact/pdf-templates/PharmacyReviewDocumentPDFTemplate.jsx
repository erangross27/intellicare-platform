/**
 * PharmacyReviewDocumentPDFTemplate.jsx
 ...
 *   - BOOLEAN → Yes/No (false is DATA, only null/undefined hidden)
...
/* ======= MAIN COMPONENT ======= */
const PharmacyReviewDocumentPDFTemplate = ({ document: docProp }) => {
  const pick = (r) => r && r.pharmacy_review;
  let records = [];
      ...
      <Document>
        <Page size="A4" style={styles.page}>
          <View style={styles.documentHeader}>
            <Text style={styles.documentTitle}>Pharmacy Review</Text>
          </View>
          <Text style={styles.noDataText}>No pharmacy review data available.</Text>
        </Page>
      </Document>
    );
  }
        ...
        {records.map((record, idx) => {
          ...
          return (
            <View key={idx} style={styles.recordContainer}>
              ...
              {renderSection(record, 'medication')}
              {renderSection(record, 'safety')}
              {renderSection(record, 'clinical-factors')}
              {renderSection(record, 'review-monitoring')}
...
export default PharmacyReviewDocumentPDFTemplate;
