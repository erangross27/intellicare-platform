 ...
 *
 * PUT /api/edit/medication_dosing_recommendation/:id/edit    — Edit a single field
 * PUT /api/edit/medication_dosing_recommendation/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'medication-dosing-recommendation-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'indicationForUse',
  'standardAdultDose',
  'pediatricDosingWeight',
  'maximumDailyDose',
  'renalDoseAdjustment',
  'hepaticDoseAdjustment',
  'creatinineClearanceThreshold',
  'routeOfAdministration',
  'infusionRateRecommendation',
  'loadingDoseRequired',
  'loadingDoseAmount',
  ...
  'halfLifeHours',
  'dosingIntervalHours',
  'bodyWeightBasedDosing',
  'bodySurfaceAreaDosing',
  'geriatricDoseConsiderations',
  'dialysisDosingGuidance',
  'cytochromeP450Interactions',
  'blackBoxWarningDoseRelated',
  'date',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('medication_dosing_recommendation', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[medication_dosing_recommendation] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('medication_dosing_recommendation', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[medication_dosing_recommendation] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
