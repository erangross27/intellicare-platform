/**
 * EndocrineLabResultsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: endocrine_lab_results
 *
 * 4 Sections:
 ...
 *   3. metabolic-hormones: hemoglobinA1c, fastingGlucose, cPeptide, cortisol,
 *                          cortisolCollectionTime, adrenocorticotropicHormone, insulinLikeGrowthFactor1
 *   4. minerals-reproductive: parathyroidHormone, serumCalcium, ionizedCalcium, vitamin25OHD,
 *                             prolactin, luteinizingHormone, follicleStimulatingHormone,
 *                             testosterone, estradiol, serumOsmolality
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import EndocrineLabResultsDocumentPDFTemplate from '../pdf-templates/EndocrineLabResultsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './EndocrineLabResultsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'endocrine_lab_resultsPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  freeThyroxineT4: 'Free Thyroxine (T4)',
  freeTriiodothyronineT3: 'Free Triiodothyronine (T3)',
  thyroidPeroxidaseAntibody: 'Thyroid Peroxidase Antibody (TPO)',
  thyroglobulinAntibody: 'Thyroglobulin Antibody',
  ...
  ionizedCalcium: 'Ionized Calcium (mmol/L)',
  ...
  serumOsmolality: 'Serum Osmolality (mOsm/kg)',
};
...
/* 22 number fields — ALL use input[type=number step=any] + parseFloat+isNaN→block save */
const NUMBER_FIELDS = [
  'thyroidStimulatingHormone', 'freeThyroxineT4', 'freeTriiodothyronineT3',
  'thyroidPeroxidaseAntibody', 'thyroglobulinAntibody',
  'hemoglobinA1c', 'fastingGlucose', 'cPeptide',
  'cortisol', 'adrenocorticotropicHormone', 'insulinLikeGrowthFactor1',
  'parathyroidHormone', 'serumCalcium', 'ionizedCalcium', 'vitamin25OHD',
  'prolactin', 'luteinizingHormone', 'follicleStimulatingHormone',
  'testosterone', 'estradiol', 'serumOsmolality',
];
...
/* Every endocrine analyte is stored 0 when the test was NOT on the panel (no analyte reads exactly 0).
   Treat 0 on any NUMBER field as a not-measured sentinel → hide it in all 4 areas. */
const isZeroSentinel = (fn, v) => NUMBER_FIELDS.includes(fn) && Number(v) === 0;
const COPY_LINE_EQ = '='.repeat(40);
const COPY_LINE_DASH = '-'.repeat(40);
/* stepFor: match the decimal granularity of the current value (e.g. 2.84 → 0.01, 142 → 1) */
...
/* ═══════ COMPONENT ═══════ */
const EndocrineLabResultsDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Endocrine Lab Results ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // Staged drafts for this record + section: editKey = "field-idx" (or "field.arrayIndex-idx")
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/endocrine_lab_results/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/endocrine_lab_results/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    const present = fields.filter(f => {
      ...
      return hasVal(v) && !isZeroSentinel(f, v);
    });
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    ...
    const clamp = (n) => Math.max(0, n); // lab concentrations/counts are never negative
    const isModified = editedFields[editKey];
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                ...
                <input type="text" inputMode="decimal" className="edit-number" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onClick={e => e.stopPropagation()} onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter') { e.preventDefault(); const numVal = parseFloat(editValue); if (isNaN(numVal) || editValue.trim() === '') { setSaveError('Please enter a valid number'); return; } handleSaveField(record, fn, idx, sid, null, clamp(numVal)); } }} />
                <button type="button" className="num-step" disabled={saving} onClick={e => { e.stopPropagation(); const st = parseFloat(stepFor(editValue)) || 1; const dec = (String(st).split('.')[1] || '').length; const cur = parseFloat(editValue); setEditValue(clamp((isNaN(cur) ? 0 : cur) + st).toFixed(dec)); }}>+</button>
              </div>
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const numVal = parseFloat(editValue); if (isNaN(numVal) || editValue.trim() === '') { setSaveError('Please enter a valid number'); return; } handleSaveField(record, fn, idx, sid, null, clamp(numVal)); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="endocrine-lab-results-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Endocrine Lab Results</h2></div>
        <div className="empty-state">No endocrine lab results records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="endocrine-lab-results-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Endocrine Lab Results</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<EndocrineLabResultsDocumentPDFTemplate document={pdfData} />} fileName="Endocrine_Lab_Results.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search endocrine lab results..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Endocrine Lab Results ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'thyroid-panel')}
            {renderSection(record, idx, 'metabolic-hormones')}
            {renderSection(record, idx, 'minerals-reproductive')}
          </div>
        ))}
      </div>
    </div>
...
export default EndocrineLabResultsDocument;
