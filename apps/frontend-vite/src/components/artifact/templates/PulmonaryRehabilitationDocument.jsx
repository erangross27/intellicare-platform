/**
 * PulmonaryRehabilitationDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: pulmonary_rehabilitation
 *
 * Typed editing: DATE (date picker), NUMBER (input type=number, parseFloat + hide-zero),
 *   STRING (per-sentence renderStringField), ARRAY (per-item edit), OBJECT (recursive leaves).
 *
 * 6 Sections:
 *   1. program-overview: date, provider, facility, primaryDiagnosis, programPhase, sessionNumber, totalProgramSessions, secondaryDiagnoses
 *   2. pulmonary-function: baselineFev1, baselineFvc, currentFev1
 *   3. exercise-capacity: sixMinuteWalkDistance, baselineOxygenSaturation, exerciseOxygenSaturation, supplementalOxygenRequired, targetHeartRate, peakHeartRateDuringSession
 *   4. exercise-prescription: exercisePrescription (object)
 *   5. education-support: breathingTechniques, educationTopicsCovered, dyspneaScale, nutritionalCounseling, psychosocialSupport
 *   6. outcomes-adverse: functionalCapacityImprovement, adverseEvents
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PulmonaryRehabilitationDocumentPDFTemplate from '../pdf-templates/PulmonaryRehabilitationDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './PulmonaryRehabilitationDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; whole-field value) */
const DRAFT_KEY = 'pulmonaryRehabilitationPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'exercise-capacity': 'Exercise Capacity & Oximetry',
  'exercise-prescription': 'Exercise Prescription',
  'education-support': 'Education & Support',
  'outcomes-adverse': 'Outcomes & Adverse Events',
};
...
const FIELD_LABELS = {
  ...
  totalProgramSessions: 'Total Program Sessions',
  secondaryDiagnoses: 'Secondary Diagnoses',
  baselineFev1: 'Baseline FEV1 (L)',
  baselineFvc: 'Baseline FVC (L)',
  currentFev1: 'Current FEV1 (L)',
  sixMinuteWalkDistance: '6-Minute Walk Distance (m)',
  baselineOxygenSaturation: 'Baseline SpO2 (%)',
  exerciseOxygenSaturation: 'Exercise SpO2 (%)',
  supplementalOxygenRequired: 'Supplemental Oxygen Required',
  targetHeartRate: 'Target Heart Rate (bpm)',
  peakHeartRateDuringSession: 'Peak Heart Rate During Session (bpm)',
  exercisePrescription: 'Exercise Prescription',
  breathingTechniques: 'Breathing Techniques',
  educationTopicsCovered: 'Education Topics Covered',
  dyspneaScale: 'Dyspnea Scale',
  nutritionalCounseling: 'Nutritional Counseling',
  psychosocialSupport: 'Psychosocial Support',
  functionalCapacityImprovement: 'Functional Capacity Improvement',
  adverseEvents: 'Adverse Events',
};

const SECTION_FIELDS = {
  'program-overview': ['date', 'provider', 'facility', 'primaryDiagnosis', 'programPhase', 'sessionNumber', 'totalProgramSessions', 'secondaryDiagnoses'],
  'pulmonary-function': ['baselineFev1', 'baselineFvc', 'currentFev1'],
  'exercise-capacity': ['sixMinuteWalkDistance', 'baselineOxygenSaturation', 'exerciseOxygenSaturation', 'supplementalOxygenRequired', 'targetHeartRate', 'peakHeartRateDuringSession'],
  'exercise-prescription': ['exercisePrescription'],
  'education-support': ['breathingTechniques', 'educationTopicsCovered', 'dyspneaScale', 'nutritionalCounseling', 'psychosocialSupport'],
  'outcomes-adverse': ['functionalCapacityImprovement', 'adverseEvents'],
};

const DATE_FIELDS = ['date'];
const NUMBER_FIELDS = ['sessionNumber', 'totalProgramSessions', 'baselineFev1', 'baselineFvc', 'currentFev1', 'sixMinuteWalkDistance', 'baselineOxygenSaturation', 'exerciseOxygenSaturation', 'targetHeartRate', 'peakHeartRateDuringSession'];
const ARRAY_FIELDS = ['secondaryDiagnoses', 'breathingTechniques', 'educationTopicsCovered', 'adverseEvents'];
const OBJECT_FIELDS = ['exercisePrescription'];
const STRING_FIELDS = ['provider', 'facility', 'primaryDiagnosis', 'programPhase', 'supplementalOxygenRequired', 'dyspneaScale', 'nutritionalCounseling', 'psychosocialSupport', 'functionalCapacityImprovement'];
...
/* number+unit leaf splitter — returns null for plain text and "4/5" ratios (those stay text) */
const splitNumberUnit = (text) => {
  if (text === null || text === undefined) return null;
  const s = String(text).trim();
  if (s === '') return null;
  if (/^-?\d+(?:\.\d+)?\s*\/\s*\d/.test(s)) return null; // ratio (4/5) -> stays text
  const m = s.match(/^(-?[\d,]*\.?\d+)(\s*)(.*)$/);
  if (!m || !/\d/.test(m[1])) return null;
  return { num: m[1].replace(/,/g, ''), sep: m[2] || '', unit: (m[3] || '').trim() };
};

/* MRC-style ratio "4/5" — edit the numerator as a number, keep the "/denom" suffix */
...
/* ═══════ COMPONENT ═══════ */
const PulmonaryRehabilitationDocument = ({ document: docProp }) => {
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (Array.isArray(val)) { if (val.some(item => flattenSearchable(item).toLowerCase().includes(phrase))) return true; }
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    const val = getFieldValue(record, fn, idx);
    if (val !== null && val !== undefined) {
      if (Array.isArray(val)) return val.some(item => flattenSearchable(item).toLowerCase().includes(phrase));
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pulmonary Rehabilitation ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (val && (Array.isArray(val) ? val.some(item => flattenSearchable(item).toLowerCase().includes(phrase)) : (typeof val === 'object' ? flattenSearchable(val).toLowerCase().includes(phrase) : fmtVal(val).toLowerCase().includes(phrase)))) return true;
        }
      }
      return false;
    });
  ...
  /* OBJECT leaf save — stage the whole merged object as a DRAFT (no DB write). Approve commits it. */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // localEdits keys are always `${fieldName}-${idx}` holding the WHOLE new field value.
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/pulmonary_rehabilitation/${id}/edit`, { field: fieldPart, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/pulmonary_rehabilitation/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    return text;
  }, [getFieldValue, hasVal, fmtVal, numberShows, splitBySentence, formatSentenceFieldLines, objectCopyLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== PULMONARY REHABILITATION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Pulmonary Rehabilitation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT FIELD — exercisePrescription ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pulmonary-rehabilitation-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pulmonary Rehabilitation</h2></div>
        <div className="empty-state">No pulmonary rehabilitation data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pulmonary-rehabilitation-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Pulmonary Rehabilitation</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PulmonaryRehabilitationDocumentPDFTemplate document={pdfData} />} fileName={`pulmonary-rehabilitation-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pulmonary rehabilitation..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            ...
            <div className="record-header">
              {(hasVal(record.date) || hasVal(record.provider) || hasVal(record.facility)) && (
                <div className="record-meta-row">
                  ...
                  {hasVal(record.provider) && <span className="record-date">{fmtVal(getFieldValue(record, 'provider', idx))}</span>}
                  {hasVal(record.facility) && <span className="record-date">{fmtVal(getFieldValue(record, 'facility', idx))}</span>}
                </div>
              )}
              <h3 className="record-name">{highlightText(`Pulmonary Rehabilitation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'program-overview')}
            {renderSection(record, idx, 'pulmonary-function')}
            {renderSection(record, idx, 'exercise-capacity')}
            {renderSection(record, idx, 'exercise-prescription')}
            {renderSection(record, idx, 'education-support')}
            {renderSection(record, idx, 'outcomes-adverse')}
          </div>
        ))}
      </div>
    </div>
...
export default PulmonaryRehabilitationDocument;
