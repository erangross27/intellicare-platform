/**
 * StiScreeningPanelDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: sti_screening_panel
 *
 * 6 Sections:
 *   1. screening-info:    screeningDate
 *   2. hiv-status:        hivAntigenAntibodyResult, hivViralLoadCopies, hivConfirmatoryMethod, cd4CountCellsPerMicroliter, cd4Cd8Ratio, prepEligibilityStatus
 *   3. syphilis-results:  rprTiter, treponemaPallidumAntibody, syphilisStage
 *   4. bacterial-stis:    chlamydiaNaatResult, gonorrheaNaatResult, specimenSourceSite, mycoplasmaGenitaliumNaat, antimicrobialResistanceMarkers
 *   5. viral-hepatitis:   hepatitisBSurfaceAntigen, hepatitisBSurfaceAntibody, hepatitisBCoreAntibody, hepatitisCantibodyResult, hcvRnaQuantitative
 *   6. additional-tests:  hsvTypeSpecificIgG, trichomonasVaginalisResult, humanPapillomavirusHighRisk, hpvGenotyping, expeditedPartnerTherapyProvided
 */
...
const DRAFT_KEY = 'sti_screening_panelPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS   = [
  ...
  'chlamydiaNaatResult', 'gonorrheaNaatResult', 'mycoplasmaGenitaliumNaat',
...
const StiScreeningPanelDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `STI Screening Panel ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Staged drafts for THIS record's fields in THIS section (localEdits keyed `${field}-${idx}`)
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length); // field name (may contain dots, but trailing seg is not numeric)
        const lastDot = fieldPart.lastIndexOf('.');
        const baseField = (lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1))) ? fieldPart.slice(0, lastDot) : fieldPart;
        return fields.includes(baseField);
      });
      // Persist each staged field to the DB now (field name + full value; add arrayIndex only when the
      // trailing dot-segment is purely numeric — here localEdits holds the whole value so it never is).
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/sti_screening_panel/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/sti_screening_panel/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    ...
    pdfData.forEach((r, idx) => {
      text += `STI Screening Panel ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: showLabelSingle ═══════ */
  const showLabelSingle = (label) => (
      ...
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
            ...
            if (!labelL.includes(phrase) && !phrase.includes(labelL) && !itemStr.toLowerCase().includes(phrase)) return null;
          }
                      ...
                      {commaItems.map((ci, ciIdx) => {
                        ...
                        return (
                          <div key={ciIdx}>
                            ...
                            {ciBadge === 'added' && <span className="modified-badge added">added - click Pending Approve to save</span>}
                            {ciBadge && ciBadge !== 'added' && <span className="modified-badge">edited - click Pending Approve to save</span>}
                          </div>
                        );
                      })}
                    </div>
  ...
  if (!records || records.length === 0) {
    return (
      <div className="sti-screening-panel-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">STI Screening Panel</h2></div>
        <div className="empty-state">No STI screening panel records available</div>
      </div>
    );
  }

  const presentFields = filteredRecords.reduce((acc, r) => {
    return acc + Object.values(SECTION_FIELDS).flat().filter(f => hasVal(r[f])).length;
  }, 0);

  return (
    <div className="sti-screening-panel-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">STI Screening Panel</h2>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search STI screening panel..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'syphilis-results')}
            {renderSection(record, idx, 'bacterial-stis')}
            {renderSection(record, idx, 'viral-hepatitis')}
            {renderSection(record, idx, 'additional-tests')}
          </div>
        ))}
      </div>
    </div>
...
export default StiScreeningPanelDocument;
