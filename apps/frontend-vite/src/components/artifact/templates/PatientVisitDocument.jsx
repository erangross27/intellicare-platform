/**
 * PatientVisitDocument.jsx
 * Canonical one-pass rebuild (July 2026) — standard config-driven shape: rec-mini-card rows with
 * .numbered-row.editable-row, BlueDatePicker/BlueSelect widgets, single-name gate, canonical EQ/DASH
 * numbered copy, standard Pending-Approve wiring, box-free PDF. This record is a sparse visit-recording
 * stub: visitDate (date), visitType/status/consentMethod (enums), consentObtained (boolean).
 * Header is TITLE-ONLY (record has visitDate as a FIELD, and only createdAt/updatedAt timestamps — no `date`).
 * Collection: patient_visits
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PatientVisitDocumentPDFTemplate from '../pdf-templates/PatientVisitDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PatientVisitDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve. */
const DRAFT_KEY = 'patient_visitsPendingEdits';
...
/* Free-text narrative (multi-sentence split) — none in the sparse record, kept as canonical fallback */
const STRING_FIELDS = [];
...
const PatientVisitDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.patient_visits) return Array.isArray(r.patient_visits) ? r.patient_visits : [r.patient_visits];
      if (r?.records) return Array.isArray(r.records) ? r.records : [r.records];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.patient_visits) return Array.isArray(dd.patient_visits) ? dd.patient_visits : [dd.patient_visits]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Patient Visit Record ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const [fn, value] of Object.entries(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/patient_visits/${id}/edit`, { field: fn, value });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/patient_visits/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[PatientVisit] Approve error:', err); }
  ...
  const copyAllText = useCallback(async () => {
    let text = `Patient Visit\n${COPY_LINE_EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Patient Visit Record ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="patient-visit-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Patient Visit</h2></div>
        <div className="empty-state">No patient visit records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="patient-visit-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Patient Visit</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PatientVisitDocumentPDFTemplate document={pdfData} />} fileName="Patient_Visit.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search patient visits..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Patient Visit Record ${idx + 1}`)}</h3>
            </div>
...
export default PatientVisitDocument;
