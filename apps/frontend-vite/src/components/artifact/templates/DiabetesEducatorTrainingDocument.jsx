/**
 * DiabetesEducatorTrainingDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: diabetes_educator_training
 *
 * 6 Sections:
 *   1. glycemic-control: hba1cLevel, fastingBloodGlucose, postprandialGlucose, timeInRange
 *   2. insulin-dosing: insulinRegimen, carbohydrateRatio, correctionFactor, bloodGlucoseTarget
 *   3. hypoglycemia-dka: hypoglycemiaFrequency, severeHypoglycemiaHistory, diabeticKetoacidosisHistory
 *   4. complications: diabeticNeuropathy, diabeticRetinopathy, diabeticNephropathy, albuminCreatinineRatio, estimatedGlomerularFiltrationRate
 *   5. foot-exam: footExamFindings, monofilamentTest
 *   6. education-adherence: diabetesType, continuousGlucoseMonitor, selfMonitoringFrequency, diabetesEducationTopics, medicationAdherence, nutritionalAssessment
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import DiabetesEducatorTrainingDocumentPDFTemplate from '../pdf-templates/DiabetesEducatorTrainingDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './DiabetesEducatorTrainingDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }
     fieldPart = "field"            → scalar / string / whole-array field   (localEdits key `${field}-${idx}`)
     fieldPart = "field.itemIdx"    → array element                          (editedFields key `${field}.${itemIdx}-${idx}`) */
const DRAFT_KEY = 'diabetes_educator_trainingPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'hypoglycemia-dka': 'Hypoglycemia & DKA',
  'complications': 'Complications',
  'foot-exam': 'Foot Exam',
  'education-adherence': 'Education & Adherence',
};
...
const FIELD_LABELS = {
  ...
  carbohydrateRatio: 'Carbohydrate Ratio (g/unit)',
  correctionFactor: 'Correction Factor (mg/dL/unit)',
  bloodGlucoseTarget: 'Blood Glucose Target',
  hypoglycemiaFrequency: 'Hypoglycemia Frequency (/week)',
  severeHypoglycemiaHistory: 'Severe Hypoglycemia History',
  diabeticKetoacidosisHistory: 'Diabetic Ketoacidosis History',
  ...
  footExamFindings: 'Foot Exam Findings',
  monofilamentTest: 'Monofilament Test',
  diabetesType: 'Diabetes Type',
  continuousGlucoseMonitor: 'Continuous Glucose Monitor',
  selfMonitoringFrequency: 'Self-Monitoring Frequency (/day)',
  diabetesEducationTopics: 'Diabetes Education Topics',
  medicationAdherence: 'Medication Adherence',
  nutritionalAssessment: 'Nutritional Assessment',
};

const SECTION_FIELDS = {
  'glycemic-control': ['hba1cLevel', 'fastingBloodGlucose', 'postprandialGlucose', 'timeInRange'],
  'insulin-dosing': ['insulinRegimen', 'carbohydrateRatio', 'correctionFactor', 'bloodGlucoseTarget'],
  'hypoglycemia-dka': ['hypoglycemiaFrequency', 'severeHypoglycemiaHistory', 'diabeticKetoacidosisHistory'],
  'complications': ['diabeticNeuropathy', 'diabeticRetinopathy', 'diabeticNephropathy', 'albuminCreatinineRatio', 'estimatedGlomerularFiltrationRate'],
  'foot-exam': ['footExamFindings', 'monofilamentTest'],
  'education-adherence': ['diabetesType', 'continuousGlucoseMonitor', 'selfMonitoringFrequency', 'diabetesEducationTopics', 'medicationAdherence', 'nutritionalAssessment'],
};

const NUMBER_FIELDS = ['hba1cLevel', 'fastingBloodGlucose', 'postprandialGlucose', 'carbohydrateRatio', 'correctionFactor', 'hypoglycemiaFrequency', 'timeInRange', 'albuminCreatinineRatio', 'estimatedGlomerularFiltrationRate', 'selfMonitoringFrequency'];
const ARRAY_FIELDS = ['diabetesEducationTopics'];
const BOOLEAN_FIELDS = ['severeHypoglycemiaHistory', 'diabeticKetoacidosisHistory', 'continuousGlucoseMonitor'];
const STRING_FIELDS = ['diabetesType', 'insulinRegimen', 'bloodGlucoseTarget', 'diabeticNeuropathy', 'diabeticRetinopathy', 'diabeticNephropathy', 'footExamFindings', 'monofilamentTest', 'medicationAdherence', 'nutritionalAssessment'];
...
/* ═══════ COMPONENT ═══════ */
const DiabetesEducatorTrainingDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.diabetes_educator_training) return Array.isArray(r.diabetes_educator_training) ? r.diabetes_educator_training : [r.diabetes_educator_training];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.diabetes_educator_training) return Array.isArray(dd.diabetes_educator_training) ? dd.diabetes_educator_training : [dd.diabetes_educator_training]; return [dd]; }
      return [r];
    });
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Maps each draft's record _id (handles _id.$oid) to its render index. */
  useEffect(() => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Diabetes Education & Self-Management ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Commit each staged draft fieldPart for THIS record that belongs to THIS section.
      const store = readDrafts();
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/diabetes_educator_training/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedFns.add(baseField);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/diabetes_educator_training/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; committedFns.forEach(f => delete n[`${f}-${idx}`]); return n; });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== DIABETES EDUCATION & SELF-MANAGEMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Diabetes Education & Self-Management ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="diabetes-educator-training-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Diabetes Education & Self-Management</h2></div>
        <div className="empty-state">No diabetes education & self-management data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="diabetes-educator-training-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Diabetes Education & Self-Management</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<DiabetesEducatorTrainingDocumentPDFTemplate document={pdfData} />} fileName={`diabetes-educator-training-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search diabetes education & self-management..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Diabetes Education & Self-Management ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'glycemic-control')}
            {renderSection(record, idx, 'insulin-dosing')}
            {renderSection(record, idx, 'hypoglycemia-dka')}
            {renderSection(record, idx, 'complications')}
            {renderSection(record, idx, 'foot-exam')}
            {renderSection(record, idx, 'education-adherence')}
          </div>
        ))}
      </div>
    </div>
...
export default DiabetesEducatorTrainingDocument;
