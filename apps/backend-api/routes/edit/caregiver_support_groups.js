/**
 * Caregiver Support Groups Edit Route
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'caregiver-support-groups-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'caregiverBurdenInventoryScore',
  'dailyLivingActivitiesScore',
  'caregiverDepressionScale',
  'hoursOfCarePerWeek',
  'caregiverStressScale',
  'medicationManagementComplexity',
  'groupSessionAttendance',
  'patientDiagnosisPrimary',
  'caregiverRelationshipToPatient',
  'cognitiveImpairmentSeverity',
  'caregiverPhysicalHealthStatus',
  'socialSupportNetwork',
  'financialStrainLevel',
  'caregiverSleepQuality',
  'functionalDeclineRate',
  'behavioralDisturbancesPresent',
  'respiteCareUtilization',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('caregiver_support_groups', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[caregiver_support_groups] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('caregiver_support_groups', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[caregiver_support_groups] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
