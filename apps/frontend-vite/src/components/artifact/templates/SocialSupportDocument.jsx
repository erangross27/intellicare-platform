/**
 * SocialSupportDocument.jsx
 * July 2026 — recursive, dot-path editable social support
...
import SocialSupportDocumentPDFTemplate from '../pdf-templates/SocialSupportDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SocialSupportDocument.css';

const DRAFT_KEY = 'social_supportPendingEdits';
const SECTION_TITLES = {
  caregiver: 'Caregiver Information', living: 'Living Situation and Family Support', practical: 'Practical Support',
  coordination: 'Care Coordination', readiness: 'Readiness and Psychosocial Support', provider: 'Provider Information', additional: 'Additional Data',
};
const FIELD_LABELS = {
  primaryCaregiverType: 'Primary Caregiver Type', caregiverAvailabilityHours: 'Caregiver Availability Hours', caregiverBurdenScale: 'Caregiver Burden Scale', homeHealthAideHours: 'Home Health Aide Hours',
  livingArrangementStatus: 'Living Arrangement Status', socialIsolationRiskScore: 'Social Isolation Risk Score', familySupportSystemStrength: 'Family Support System Strength', emergencyContactAvailability: 'Emergency Contact Availability',
  medicationManagementSupport: 'Medication Management Support', transportationAccessibility: 'Transportation Accessibility', financialSupportAdequacy: 'Financial Support Adequacy', communityResourceUtilization: 'Community Resource Utilization',
  socialWorkerAssignment: 'Social Worker Assignment', careCoordinationComplexity: 'Care Coordination Complexity', palliativeCareReferralStatus: 'Palliative Care Referral Status', advanceDirectiveDiscussion: 'Advance Directive Discussion',
  dischargeReadinessScore: 'Discharge Readiness Score', psychosocialRiskFactors: 'Psychosocial Risk Factors', supportGroupParticipation: 'Support Group Participation',
  provider: 'Provider', facility: 'Facility', additionalData: 'Additional Data',
};
const SECTION_FIELDS = {
  caregiver: ['primaryCaregiverType', 'caregiverAvailabilityHours', 'caregiverBurdenScale', 'homeHealthAideHours'],
  living: ['livingArrangementStatus', 'socialIsolationRiskScore', 'familySupportSystemStrength', 'emergencyContactAvailability'],
  practical: ['medicationManagementSupport', 'transportationAccessibility', 'financialSupportAdequacy', 'communityResourceUtilization'],
  coordination: ['socialWorkerAssignment', 'careCoordinationComplexity', 'palliativeCareReferralStatus', 'advanceDirectiveDiscussion'],
  readiness: ['dischargeReadinessScore', 'psychosocialRiskFactors', 'supportGroupParticipation'],
  provider: ['provider', 'facility'],
  additional: ['additionalData'],
};
const DATE_FIELDS = [];
const DATETIME_FIELDS = [];
const ENUM_FIELDS = ['primaryCaregiverType', 'livingArrangementStatus', 'familySupportSystemStrength', 'transportationAccessibility', 'financialSupportAdequacy', 'palliativeCareReferralStatus'];
const NUMBER_FIELDS = ['caregiverAvailabilityHours', 'caregiverBurdenScale', 'homeHealthAideHours', 'socialIsolationRiskScore', 'dischargeReadinessScore'];
...
const COMMA_FIELDS = [];
const COMMA_ARRAY_SPLIT_FIELDS = ['communityResourceUtilization'];
const ARRAY_FIELDS = ['communityResourceUtilization', 'psychosocialRiskFactors'];
const SEMICOLON_FIELDS = ['careCoordinationComplexity', 'psychosocialRiskFactors'];
const LABELED_LIST_FIELDS = [];
...
const KEY_LABELS = { preferredContact: 'Preferred Contact', needsInterpreter: 'Needs Interpreter', followUpScore: 'Follow-Up Score' };
...
const joinFieldParts = (field, parts) => {
  if (fieldIn(PARENTHETICAL_SEMICOLON_FIELDS, field) && parts.length >= 3) return parts[0] + ' (' + parts.slice(1).join('; ') + ')';
  ...
  if (fieldIn(COMMA_FIELDS, field) || fieldIn(COMMA_ARRAY_SPLIT_FIELDS, field)) return parts.join(', ');
...
const groupNarrativeParts = (parts, field) => {
  const groups = [];
  let unlabeled = [];
  const flush = () => {
  ...
  parts.forEach((part, partIndex) => {
    ...
    } else if (fieldIn(LABELED_LIST_FIELDS, field) && groups[groups.length - 1]?.type === 'labeled') {
      groups[groups.length - 1].items.push({ value: part, partIndex });
...
const enumOptionsFor = (path, value) => {
  ...
  const options = {
    primaryCaregiverType: ['Spouse', 'Partner', 'Parent', 'Child', 'Sibling', 'Friend', 'Professional caregiver', 'None'],
    livingArrangementStatus: ['Lives alone', 'Family home', 'Assisted living', 'Skilled nursing facility', 'Other'],
    familySupportSystemStrength: ['Strong', 'Moderate', 'Limited', 'None'],
    transportationAccessibility: ['Independent', 'Family assistance', 'Public transportation', 'Medical transport', 'Limited', 'None'],
    financialSupportAdequacy: ['Adequate', 'Requires assistance', 'Inadequate'],
    palliativeCareReferralStatus: ['Not indicated', 'Discussed', 'Referred', 'Active'],
  }[field] || [];
...
const SocialSupportDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.social_support) return Array.isArray(record.social_support) ? record.social_support : [record.social_support];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.social_support) return Array.isArray(nested.social_support) ? nested.social_support : [nested.social_support];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/social_support/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/social_support/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SocialSupport] Approve error:', error);
  ...
  const leafProps = (record, path, recordIndex, sectionId, value, copyText) => {
    ...
    const dateTime = DATETIME_FIELDS.includes(rootOf(path));
    const configuredUnit = NUMBER_UNITS[rootOf(path)];
        ...
        : typeof value === 'number' ? 'number'
          ...
          : numberUnit ? 'number-unit'
            : ENUM_FIELDS.includes(rootOf(path)) ? 'enum'
              : 'text';
    const displayValue = date ? formatDate(value)
      : dateTime ? formatDateTime(value)
        : configuredUnit && typeof value === 'number' ? displayScalar(value) + ' ' + configuredUnit
          : displayScalar(value);
    ...
    return {
      path, value, displayValue, editorType, unit: configuredUnit || numberUnit?.unit,
  ...
  const renderNarrative = (record, field, recordIndex, sectionId) => {
    ...
    const groups = groupNarrativeParts(splitFieldValue(field, value), field);
    return <div key={field} className="field-group">
      ...
      {groups.map((group, groupIndex) => <div key={groupIndex} className={'rec-mini-card nested-mini-card' + (group.type === 'unlabeled' ? ' regular-row-group' : '')}>
        ...
        {group.items.map((item) => {
          const parsed = group.type === 'labeled' && item.label ? { label: item.label, value: item.value } : null;
  ...
  const renderPathNarrative = (record, path, recordIndex, sectionId, label) => {
    ...
    return <div key={path} className="field-group">
      {label && <div className="nested-subtitle field-group-title">{highlightText(label)}</div>}
      {groupNarrativeParts(parts, path).map((group, groupIndex) => <div key={groupIndex} className={'rec-mini-card nested-mini-card' + (group.type === 'unlabeled' ? ' regular-row-group' : '')}>
  ...
  const renderScalarField = (record, field, recordIndex, sectionId) => {
    ...
    return <div key={field} className="rec-mini-card nested-mini-card">
      ...
      <EditableLeaf {...leafProps(record, field, recordIndex, sectionId, value, label + '\n' + (isDatePathValue(field, value) ? formatDate(value) : DATETIME_FIELDS.includes(field) ? formatDateTime(value) : NUMBER_UNITS[field] && typeof value === 'number' ? displayScalar(value) + ' ' + NUMBER_UNITS[field] : displayScalar(value)))} />
    </div>;
  };
  ...
  const renderRecursiveNode = (record, value, basePath, recordIndex, sectionId, label) => {
    ...
    if (Array.isArray(value) && value.every(isScalar) && fieldIn(COMMA_ARRAY_SPLIT_FIELDS, basePath)) return renderNarrativeArray(record, basePath, recordIndex, sectionId, label);
  ...
  const renderRecommendations = (record, recordIndex, sectionId) => {
    ...
    return <div className="recommendation-groups" key="recommendations">{groupRecommendations(recommendations).map((group) => {
  ...
  const copyRecursiveLines = (value, basePath, indent = '') => {
    ...
    if (isScalar(value)) {
      const shown = isDatePathValue(basePath, value) ? formatDate(value)
        : DATETIME_FIELDS.includes(rootOf(basePath)) ? formatDateTime(value)
          : NUMBER_UNITS[rootOf(basePath)] && typeof value === 'number' ? displayScalar(value) + ' ' + NUMBER_UNITS[rootOf(basePath)]
            : displayScalar(value);
    ...
    if (Array.isArray(value) && value.every(isScalar)) {
      return [indent + humanizeKey(String(basePath).split('.').pop()) + '\n' + value.map((item, index) =>
        indent + (index + 1) + '. ' + displayScalar(item)).join('\n')];
    }
  ...
  const buildSectionCopyText = useCallback((record, recordIndex, sectionId) => {
    ...
    (SECTION_FIELDS[sectionId] || []).forEach((field) => {
      ...
      if (field === 'measurableDisease') {
        ...
        if (OBJECT_ITEM_LABELS[field]) (Array.isArray(value) ? value : [value]).forEach((item, itemIndex) => {
          text += OBJECT_ITEM_LABELS[field] + ' ' + (itemIndex + 1) + '\n';
          copyRecursiveLines(item, field + '.' + itemIndex).forEach((line) => { text += line + '\n'; });
        });
        else copyRecursiveLines(value, field).forEach((line) => { text += line + '\n'; });
      } else if (MIXED_OBJECT_ARRAY_FIELDS.includes(field) && Array.isArray(value) && value.some((item) => !isScalar(item))) {
        value.forEach((item, itemIndex) => {
        ...
        if (showLabel) text += label + '\n';
        text += '1. ' + formatDateTime(value) + '\n';
      } else if (NUMBER_UNITS[field] && typeof value === 'number') {
        if (showLabel) text += label + '\n';
        text += '1. ' + value + ' ' + NUMBER_UNITS[field] + '\n';
      } else {
        if (showLabel) text += label + '\n';
  ...
  const copyAll = useCallback(async () => {
    let text = '=== SOCIAL SUPPORT ===\n\n';
    records.forEach((record, recordIndex) => {
      text += 'Social Support Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
    ...
    return <div className="section" key={sectionId}><div className="mini-cards-container">
      ...
      {sectionId === 'results' ? renderResults(record, recordIndex, sectionId)
          : sectionId === 'recommendations' ? renderRecommendations(record, recordIndex, sectionId)
            : fields.map((field) => field === 'recommendations'
              ? renderRecommendations(record, recordIndex, sectionId)
              : MIXED_OBJECT_ARRAY_FIELDS.includes(field) && Array.isArray(getValue(record, field, recordIndex)) && getValue(record, field, recordIndex).some((item) => !isScalar(item))
              ? renderRecursiveNode(record, getValue(record, field, recordIndex), field, recordIndex, sectionId, OBJECT_ITEM_LABELS[field] || (FIELD_LABELS[field]?.toLowerCase() === SECTION_TITLES[sectionId].toLowerCase() ? '' : FIELD_LABELS[field]))
              : fieldIn(ARRAY_FIELDS, field)
  ...
  if (!records.length) return <div className="social-support-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Social Support</h2></div><div className="empty-state">No social support available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Social Support Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="social-support-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Social Support</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<SocialSupportDocumentPDFTemplate document={pdfData} />} fileName="Social_Support.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search social support..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Social Support Record ' + (recordIndex + 1))}</h3></div>
...
export default SocialSupportDocument;
