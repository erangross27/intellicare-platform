/**
 * NarcolepsyAssessmentDocument.jsx
 * June 2026 — Inline editing, blue glow theme
 * Collection: narcolepsy_assessment
 *
 * 7 Sections:
 *   1. assessment-info: date (record date, BlueDatePicker)
 *   2. core-symptoms: excessiveDaytimeSleepiness, epworthSleepinessScore, inadvertentNaps, refreshingNaps, automaticBehaviors
 *   3. cataplexy: cataplexyPresent, cataplexyFrequency, cataplexyTriggers[]
 *   4. rem-phenomena: sleepParalysisEpisodes, hypnagogicHallucinations, hypnopompicHallucinations
 *   5. diagnostic-studies: polySomnographyDate, sleepLatencyMinutes, sleepOnsetREMPeriods, totalSleepTimeHours, remLatencyMinutes, hlaTyping, hypocretinLevel, narcolepsyType, symptomOnsetAge
 *   6. treatment: stimulantMedications[], sodiumOxybateDose
 *   7. impact-comorbidities: functionalImpairment, comorbidSleepDisorders[]
 *
 * NOTE: `date` is the record date — rendered as an editable section field (BlueDatePicker) in the
 *       Assessment Information section, never as a header pill. polySomnographyDate is a separate
 *       section field, also via BlueDatePicker; its display value goes through fmtDate.
 *
 * HIDE-ZERO: hypocretinLevel of 0 (undetectable CSF hypocretin-1) is hidden per the canonical
 *    hide-zero rule, like any numeric 0 - a value of 0 renders no row.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NarcolepsyAssessmentDocumentPDFTemplate from '../pdf-templates/NarcolepsyAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './NarcolepsyAssessmentDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF/Copy until the user clicks Pending Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = base field name; whole field value) */
const DRAFT_KEY = 'narcolepsy_assessmentPendingEdits';
const readDrafts = () => {
...
/* SIMPLE_FIELDS = free-text string fields only. Numerics → NUMBER_FIELDS (typed input,
   saved as real numbers so int/double schema types are preserved). polySomnographyDate →
   DATE_FIELDS (date-picker, saved as ISO datetime). */
const SIMPLE_FIELDS = ['cataplexyFrequency', 'hlaTyping', 'narcolepsyType'];
/* NUMBER_FIELDS: schema int/double fields — edited with <input type=number>, parseFloat+isNaN
   guard, saved as real numbers (NOT strings, which would corrupt the int/double DB type). */
...
/* NO nested-object fields in this schema — OBJECT_FIELDS pattern not needed. */
/* polySomnographyDate is a SIMPLE section field whose display value goes through fmtDate
   and whose edit prefill is ISO "YYYY-MM-DD" (toISODate). Top-level `date` is header-only. */
const DATE_FIELDS = ['date', 'polySomnographyDate'];
/* Numeric fields hidden when exactly 0 (canonical hide-zero: a numeric 0 renders no row).
   totalSleepTimeHours 0 = study not recorded, symptomOnsetAge 0 = not recorded, and
   hypocretinLevel 0 = hidden per the canonical hide-zero rule (consistent JSX/PDF/Copy All).
   Real nonzero decimals (e.g. sleepLatencyMinutes 2.4, totalSleepTimeHours 6.7) still display. */
...
/* splitByComma: parenthesis-aware comma split with date-aware guard
   — items like "Near-miss MVA on Highway 101 on January 8, 2026" and
   "(total 4.5 grams nightly starting dose, max 9 grams)" stay intact */
...
/* ═══════ COMPONENT ═══════ */
const NarcolepsyAssessmentDocument = ({ document: docProp, data, templateData }) => {
  ...
  useEffect(() => {
    ...
    records.forEach((record) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        // restore an edited marker so the Pending Approve button + modified badge reappear
        if (ARRAY_FIELDS.includes(fieldPart)) nFields[`${fieldPart}.0-${idx}`] = 'edited';
        else if (SENTENCE_FIELDS.includes(fieldPart)) nSentences[`${fieldPart}-${idx}-s0`] = 'edited';
  ...
  /* hasFieldVal: null/undefined-safe presence check with per-field zero hiding (4-AREA RULE: used by JSX, Copy Section, Copy All and mirrored in PDF).
     hypocretinLevel 0 is hidden here too (it IS in HIDE_ZERO_FIELDS) per canonical hide-zero. */
  ...
  /* fmtDate: Date or {$date} or string → "February 10, 2026" (header date badge + polySomnographyDate display) */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      const docTitle = 'narcolepsy assessment';
      const rt = `narcolepsy assessment ${oi + 1}`;
  ...
  /* ═══════ EDIT HANDLERS ═══════
     ...
     stageDraft is the single localStorage writer: keys the draft store by record id + base field name,
     storing the whole field value (string/number/bool/array). */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Collect this record's staged drafts for this section's fields (key = "field-idx", value = whole field value)
      const suffix = `-${idx}`;
      ...
      // dot-segment is purely numeric — see GOTCHA; here drafts are whole-field so arrayIndex is unused)
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/narcolepsy_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/narcolepsy_assessment/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (ARRAY_FIELDS.includes(f)) {
        ...
        /* simple/number/date fields incl. date + polySomnographyDate via fieldDisplayVal.
           Label on its OWN line, value beneath — never side-by-side "Label: value". */
        if (showLabel) text += `${label}\n`;
        text += `${fieldDisplayVal(f, val)}\n\n`;
      }
    });
    return text;
  }, [getFieldValue, hasFieldVal, fmtVal, fieldDisplayVal, splitBySentence, splitBySemicolon, formatSentenceFieldLines, arrItemText]);

  const copyAllText = useCallback(async () => {
    let text = '=== NARCOLEPSY ASSESSMENT ===\n\n';
    pdfData.forEach((r) => {
      const oi = r._originalIdx ?? 0;
      text += `Narcolepsy Assessment ${oi + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD (num-stepper: -/+ buttons flanking <input type="text" inputMode="decimal">;
     parseFloat+isNaN guard, saves REAL numbers so the int/double schema type is preserved)
     Hide-zero via hasFieldVal: totalSleepTimeHours/symptomOnsetAge/hypocretinLevel hide at 0. */
  const renderNumberField = (record, fn, idx, sid) => {
    ...
    const saveNumber = () => { const numVal = parseFloat(editValue); if (isNaN(numVal)) { setSaveError('Please enter a valid number'); return; } handleSaveField(record, fn, idx, sid, null, numVal); };
  ...
  /* ═══════ RENDER: DATE FIELD (TYPED — <input type=date> picker; saves ISO datetime "YYYY-MM-DDT00:00:00.000Z" so the date-typed schema field stays a real date; display via fmtDate) ═══════ */
  const renderDateField = (record, fn, idx, sid) => {
    ...
    const saveDate = () => { if (isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date'); return; } handleSaveField(record, fn, idx, sid, null, editValue + 'T00:00:00.000Z'); };
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(toISODate(val)); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              ...
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveDate(); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
  ...
  /* ═══════ RENDER: SIMPLE FIELD (free-text string fields only — cataplexyFrequency, hlaTyping, narcolepsyType) ═══════ */
  ...
  /* ═══════ RENDER: BOOLEAN FIELD (Yes/No select — saves REAL booleans, false still renders "No" — e.g. hypnopompicHallucinations: false) ═══════ */
  ...
  if (!records || records.length === 0) {
    return (
      <div className="narcolepsy-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Narcolepsy Assessment</h2></div>
        <div className="empty-state">No narcolepsy assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="narcolepsy-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Narcolepsy Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NarcolepsyAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Narcolepsy_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search narcolepsy assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => {
          ...
          return (
            <div key={safeId(record) || idx} className="record-card">
              {/* Title only — the record `date` renders as an editable BlueDatePicker field in the
                  Assessment Information section, never as a header pill. */}
              <div className="record-header">
                <h3 className="record-name">{highlightText(`Narcolepsy Assessment ${origIdx + 1}`)}</h3>
              </div>
              {renderSection(record, origIdx, 'assessment-info')}
              {renderSection(record, origIdx, 'core-symptoms')}
              {renderSection(record, origIdx, 'cataplexy')}
              {renderSection(record, origIdx, 'rem-phenomena')}
              {renderSection(record, origIdx, 'diagnostic-studies')}
              {renderSection(record, origIdx, 'treatment')}
              {renderSection(record, origIdx, 'impact-comorbidities')}
            </div>
          );
        })}
      </div>
...
export default NarcolepsyAssessmentDocument;
