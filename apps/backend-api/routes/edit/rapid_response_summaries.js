...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'rapid-response-summaries-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  'triggerCriteria', 'responseTimeMinutes', 'glasgowComaScore',
  'systolicBloodPressure', 'diastolicBloodPressure', 'meanArterialPressure',
  'heartRate', 'respiratoryRate', 'oxygenSaturation', 'coreTemperature', 'fractionalInspiredOxygen',
  'cardiacRhythm', 'ventilationSupport',
  'lactateLevel', 'bloodGlucose',
  'mewsScore', 'newsScore', 'qsofaScore',
  'primaryInterventions', 'vasoactiveAgents', 'fluidResuscitationVolume',
  'dispositionDecision', 'icuTransferRequired', 'codeBlueProgression',
];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('rapid_response_summaries', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[rapid_response_summaries] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('rapid_response_summaries', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[rapid_response_summaries] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
