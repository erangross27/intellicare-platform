/**
 * OphthalmologyExamDocument.jsx
 * Canonical one-pass polish — blue glow editing theme
 * Collection: ophthalmology_exam
 *
 * Sections: Examination Information · Visual Acuity · Refraction · Pupils ·
 *           Motility and Alignment · Slit Lamp Examination · Gonioscopy ·
 *           Intraocular Pressure · Fundoscopy · Optical Coherence Tomography (OCT) ·
 *           Findings · Assessment · Plan · Results · Notes
 *
 * The real record `date` (≠ createdAt) is an editable BlueDatePicker field at the top of the
 * Examination Information section (never a header pill). Editable scalar strings (provider/
 * technician/dilationMedications/assessment/plan) use inline text editing. The deeply-nested
 * exam objects (visualAcuity/refraction/pupils/motilityAlignment/slitLampExamination/gonioscopy/
 * intraocularPressure/fundoscopy/oct) are decomposed read-only (label ABOVE value — never a
 * side-by-side "Label: value" span), including "OD: …; OS: …" summary leaves which are split into
 * OD/OS sub-labels. Nested read-only leaves (incl. the visualAcuity.corrected boolean) are preserved.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import OphthalmologyExamDocumentPDFTemplate from '../pdf-templates/OphthalmologyExamDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './OphthalmologyExamDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'ophthalmology_examPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['provider', 'technician', 'dilationMedications', 'findings', 'assessment', 'plan', 'notes'];
/* Read-only display scalars */
const DISPLAY_FIELDS = ['status', 'facility', 'type'];
/* Real record date → editable BlueDatePicker (rendered as the first field of Examination Information) */
const DATE_FIELDS = ['date'];
/* Read-only, decomposed nested objects (deeply nested: fundoscopy / oct) */
const OBJECT_FIELDS = ['visualAcuity', 'refraction', 'pupils', 'motilityAlignment', 'slitLampExamination', 'gonioscopy', 'intraocularPressure', 'fundoscopy', 'oct', 'results'];
/* Read-only, decomposed arrays */
...
/* parseLabel: detect "Label: value" patterns (skip subordinate-clause openers); strip a leading "N." off the value */
...
/* ═══════ COMPONENT ═══════ */
const OphthalmologyExamDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.ophthalmology_exam) return Array.isArray(r.ophthalmology_exam) ? r.ophthalmology_exam : [r.ophthalmology_exam];
      if (r?._records && Array.isArray(r._records)) return r._records;
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.ophthalmology_exam) return Array.isArray(dd.ophthalmology_exam) ? dd.ophthalmology_exam : [dd.ophthalmology_exam]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Ophthalmology Exam ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/ophthalmology_exam/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/ophthalmology_exam/${id}/approve`, { sectionId: sid, approved: true });
  ...
  /* leafCopyLines: decompose a scalar leaf into label + numbered/plain lines (mirrors renderLeafText).
     Every value/sub-label line is INDENTED so it can never read as a top-level "Label: value" line. */
  const leafCopyLines = useCallback((label, text, lines, indent) => {
    const pad = '  '.repeat(indent);
    if (label) lines.push(`${pad}${label}`);
    const strVal = fmtScalar(text);
    const sentences = splitBySentence(strVal);
    const multi = sentences.length > 1 || (sentences.length === 1 && parseLabel(sentences[0]).isLabeled);
    let n = 1;
    if (multi) {
      sentences.forEach(s => {
        const p = parseLabel(s);
        if (p.isLabeled) {
          lines.push(`${pad}  ${p.label}`);
          const items = splitByComma(p.value);
          if (items.length >= 2) items.forEach(it => lines.push(`${pad}    ${n++}. ${it}`));
          else lines.push(`${pad}    ${n++}. ${p.value}`);
        } else {
          const items = splitByComma(s);
          if (items.length >= 2) items.forEach(it => lines.push(`${pad}  ${n++}. ${it}`));
          else lines.push(`${pad}  ${n++}. ${s}`);
        }
      });
    } else {
      const items = splitByComma(strVal);
      if (items.length >= 2) items.forEach((it, i) => lines.push(`${pad}  ${i + 1}. ${it}`));
      else lines.push(`${pad}  ${strVal}`);
    }
  }, [splitBySentence]);

  const objectCopyBlock = useCallback((value, lines, indent) => {
    const pad = '  '.repeat(indent);
    Object.entries(value).filter(([, v]) => !isEmptyDeep(v)).forEach(([k, v]) => {
      if (isScalar(v)) { leafCopyLines(humanizeKey(k), v, lines, indent); }
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_FIELDS.includes(f)) {
        ...
        if (showLabel) text += `${label}\n`;
        text += `${formatDate(val)}\n\n`;
      } else if (DISPLAY_FIELDS.includes(f)) {
        if (showLabel) text += `${label}\n`;
        text += `${fmtVal(val)}\n\n`;
      } else {
    ...
    return text;
  }, [getFieldValue, hasVal, fmtVal, splitBySentence, formatSentenceFieldLines, objectCopyBlock]);

  const copyAllText = useCallback(async () => {
    let text = '=== OPHTHALMOLOGY EXAM ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Ophthalmology Exam ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: LEAF TEXT (read-only scalar; decomposes "OD: …; OS: …" so no side-by-side row) ═══════ */
  const renderLeafText = (label, text, key) => {
    const strVal = fmtScalar(text);
    const sentences = splitBySentence(strVal);
    const multi = sentences.length > 1 || (sentences.length === 1 && parseLabel(sentences[0]).isLabeled);
    const valueRow = (val, k) => (
      <div key={k} className="numbered-row">
        <div className="row-content"><span className="content-value">{highlightText(val)}</span></div>
        <button className={`copy-btn ${copiedItems[k] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(val, k); }}>{copiedItems[k] ? 'Copied!' : 'Copy'}</button>
      </div>
    );
    if (multi) {
      return (
            ...
            if (p.isLabeled) {
              ...
              return (
                <div key={sIdx} className="nested-mini-card">
                  <div className="nested-subtitle sub-label">{highlightText(p.label)}</div>
                  {items.length >= 2 ? items.map((it, i) => valueRow(it, `${key}-s${sIdx}-c${i}`)) : valueRow(p.value, `${key}-s${sIdx}`)}
                </div>
              );
            }
            const items = splitByComma(s);
            return items.length >= 2
              ? <React.Fragment key={sIdx}>{items.map((it, i) => valueRow(it, `${key}-s${sIdx}-c${i}`))}</React.Fragment>
              : valueRow(s, `${key}-s${sIdx}`);
          })}
        </div>
      );
    }
    ...
    return (
      <div key={key} className="nested-mini-card">
        {label && <div className="nested-subtitle sub-label">{highlightText(label)}</div>}
        {items.length >= 2 ? items.map((it, i) => valueRow(it, `${key}-c${i}`)) : valueRow(strVal, `${key}-v`)}
      </div>
    );
  };
  ...
  const renderObjectNode = (label, value, key) => {
    ...
    if (isScalar(value)) return renderLeafText(label, value, key);
    if (Array.isArray(value)) {
      const items = value.filter(x => !isEmptyDeep(x));
      return (
        <div key={key} className="nested-mini-card">
          {label && <div className="nested-subtitle sub-label">{highlightText(label)}</div>}
          {items.map((it, i) => isScalar(it) ? renderLeafText(null, it, `${key}-${i}`) : renderObjectNode(`Item ${i + 1}`, it, `${key}-${i}`))}
        </div>
      );
    }
  ...
  /* ═══════ RENDER: OBJECT FIELD (read-only, recursive) ═══════ */
  ...
  /* ═══════ RENDER: DATE FIELD (BlueDatePicker — saves ISO) ═══════ */
  ...
  /* ═══════ RENDER: NUMBER FIELD (canonical — unused: NUMBER_FIELDS empty) ═══════ */
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(strVal); setSaveError(null); } }}>
          {isEditing ? (
            ...
            <>
              ...
              <button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(strVal, editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button>
            </>
          )}
        </div>
        {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
  ...
  if (!records || records.length === 0) {
    return (
      <div className="ophthalmology-exam-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Ophthalmology Exam</h2></div>
        <div className="empty-state">No ophthalmology exam records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="ophthalmology-exam-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Ophthalmology Exam</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OphthalmologyExamDocumentPDFTemplate document={pdfData} />} fileName="Ophthalmology_Exam.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search ophthalmology exam..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Ophthalmology Exam ${idx + 1}`)}</h3>
            </div>
...
export default OphthalmologyExamDocument;
