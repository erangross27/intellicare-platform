 ...
 * July 2026 — recursive, dot-path editable return to sport
...
const SECTION_FIELDS = {
  sportInformation: ['sport', 'level', 'type'],
  returnTimeline: ['timelineToRunning', 'timelineToPractice', 'timelineToCompetition'],
  criteria: ['criteria'],
  functionalTests: ['functionalTests'],
  results: ['results'],
  providerInformation: ['provider', 'facility', 'date', 'status'],
  findings: ['findings'],
  assessment: ['assessment'],
  plan: ['plan'],
  notes: ['notes'],
...
const OBJECT_ARRAY_FIELDS = ['criteria', 'recommendations'];
...
const EditableLeaf = ({
  ...
  const numeric = editorType === 'number' || editorType === 'number-unit';
...
const ReturnToSportDocument = ({ document: docProp }) => {
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/return_to_sport/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/return_to_sport/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[ReturnToSport] Approve error:', error);
  ...
  const leafProps = (record, path, recordIndex, sectionId, value, copyText) => {
    ...
    const save = () => {
      ...
      if (editorType === 'number' || editorType === 'number-unit') {
        const number = Number(nextValue);
        if (Number.isNaN(number)) { setSaveError('Please enter a valid number'); return; }
        nextValue = editorType === 'number-unit' ? String(number) + ' ' + numberUnit.unit : number;
    ...
    return {
      ...
      options: ENUM_FIELDS.includes(rootOf(path)) ? [...new Set([String(value), 'Planned', 'In Progress', 'Cleared', 'Deferred'])] : [],
  ...
  const renderCriteria = (record, recordIndex, sectionId) => {
    const items = getValue(record, 'criteria', recordIndex);
    ...
    return <div className="object-array-group">{items.map((item, itemIndex) => <div className="rec-mini-card" key={itemIndex}>
      <div className="nested-subtitle">{highlightText('Criterion ' + (itemIndex + 1))}</div>
      {Object.entries(item || {}).filter(([, value]) => hasVal(value)).map(([key, value]) =>
        renderRecursiveNode(record, value, 'criteria.' + itemIndex + '.' + key, recordIndex, sectionId, humanizeKey(key)))}
    </div>)}</div>;
  };
  const renderFunctionalTests = (record, recordIndex, sectionId) => {
    const items = getValue(record, 'functionalTests', recordIndex);
    if (!Array.isArray(items) || !items.some(hasVal)) return null;
    return <div className="rec-mini-card nested-mini-card regular-row-group">
      {items.map((item, itemIndex) => hasVal(item)
        ? <EditableLeaf key={itemIndex} {...leafProps(record, 'functionalTests.' + itemIndex, recordIndex, sectionId, item, String(item))} />
        : null)}
    </div>;
  };
    ...
    (SECTION_FIELDS[sectionId] || []).forEach((field) => {
      ...
      if (field === 'criteria') {
        value.forEach((item, itemIndex) => {
          text += 'Criterion ' + (itemIndex + 1) + '\n';
          copyRecursiveLines(item, field + '.' + itemIndex).forEach((line) => { text += line + '\n'; });
        });
      } else if (field === 'functionalTests') {
        value.forEach((item, itemIndex) => { if (hasVal(item)) text += (itemIndex + 1) + '. ' + item + '\n'; });
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Return To Sport ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  const renderSection = (record, recordIndex, sectionId) => {
    ...
    return <div className="section" key={sectionId}><div className="mini-cards-container">
      ...
      {sectionId === 'criteria' ? renderCriteria(record, recordIndex, sectionId)
        : sectionId === 'functionalTests' ? renderFunctionalTests(record, recordIndex, sectionId)
          ...
          : sectionId === 'recommendations' ? renderRecommendations(record, recordIndex, sectionId)
            : fields.map((field) => DATE_FIELDS.includes(field) || ENUM_FIELDS.includes(field)
              ? renderScalarField(record, field, recordIndex, sectionId)
              : renderNarrative(record, field, recordIndex, sectionId))}
    </div></div>;
  };

  if (!records.length) return <div className="return-to-sport-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Return To Sport</h2></div><div className="empty-state">No return to sport records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Return To Sport ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="return-to-sport-document" ref={containerRef}>
    <div className="document-header">
      ...
      <div className="header-actions">
        ...
        <PDFDownloadLink document={<ReturnToSportDocumentPDFTemplate document={pdfData} />} fileName="Return_To_Sport.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search return to sports..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Return To Sport ' + (recordIndex + 1))}</h3></div>
