/**
 * TraumaFlowSheetsDocumentPDFTemplate.jsx
 * June 2026 — Helvetica — LETTER size — trauma flow sheet
...
/* formatDateTime: datetime display (date + time, UTC so the stored clock time is preserved) — mirrors JSX (4-AREA RULE) */
const formatDateTime = (dateStr) => {
  if (!dateStr) return '';
  try {
    const date = new Date(dateStr.$date || dateStr);
    if (isNaN(date.getTime())) return String(dateStr);
    const datePart = date.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' });
    const timePart = date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false, timeZone: 'UTC' });
...
/* parseLabel: detect "Label: value" patterns — {0,80}? allows single-letter
   ABCDE primary-survey labels, mirrors JSX (4-AREA RULE) */
...
/* SECTION CONFIGS — mirror JSX sections exactly (4-AREA RULE)
   hideZero on ALL numeric fields: 0 = not recorded in a trauma sheet context */
const SECTION_CONFIGS = [
  {
    title: 'Trauma Scores',
    fields: [
      { key: 'glasgowComaScale', label: 'Glasgow Coma Scale', hideZero: true },
      ...
      { key: 'traumaAndInjurySeverityScore', label: 'TRISS Probability of Survival', hideZero: true },
    ],
  },
  {
    title: 'Vital Signs',
    fields: [
      { key: 'systolicBloodPressure', label: 'Systolic BP (mmHg)', hideZero: true },
      { key: 'diastolicBloodPressure', label: 'Diastolic BP (mmHg)', hideZero: true },
      { key: 'heartRate', label: 'Heart Rate (bpm)', hideZero: true },
      { key: 'respiratoryRate', label: 'Respiratory Rate', hideZero: true },
      { key: 'oxygenSaturation', label: 'Oxygen Saturation (%)', hideZero: true },
    ],
  },
  {
    title: 'Injury & Primary Survey',
    fields: [
      { key: 'mechanismOfInjury', label: 'Mechanism of Injury', isSentence: true },
      { key: 'primarySurveyFindings', label: 'Primary Survey Findings', isSentence: true },
      { key: 'pupillaryResponse', label: 'Pupillary Response', isSentence: true },
      { key: 'fractureSites', label: 'Fracture Sites', isArray: true },
      { key: 'pneumothoraxPresent', label: 'Pneumothorax Present' },
    ],
  },
  {
    title: 'Imaging & Protocols',
    fields: [
      { key: 'focusedAbdominalSonographyScore', label: 'FAST Exam Findings', isSentence: true },
      { key: 'cervicalSpineProtocol', label: 'Cervical Spine Protocol', isSentence: true },
      { key: 'abdominopelvicCTFindings', label: 'Abdominopelvic CT Findings', isSentence: true },
      { key: 'organInjuryScale', label: 'Organ Injury Scale', isSentence: true },
    ],
  },
  {
    title: 'Resuscitation',
    fields: [
      { key: 'fluidResuscitationVolume', label: 'Fluid Resuscitation Volume (mL)', hideZero: true },
      { key: 'bloodProductsAdministered', label: 'Blood Products Administered', isArray: true },
      { key: 'massiveTransfusionProtocol', label: 'Massive Transfusion Protocol' },
      { key: 'intracranialPressure', label: 'Intracranial Pressure (mmHg)', hideZero: true },
    ],
  },
  {
    title: 'Labs & Timing',
    fields: [
      { key: 'hemoglobinLevel', label: 'Hemoglobin (g/dL)', hideZero: true },
      { key: 'lactateLevel', label: 'Lactate (mmol/L)', hideZero: true },
      ...
      <Document>
        <Page size="LETTER" style={styles.page}>
          <View style={styles.documentHeader}>
            <Text style={styles.documentTitle}>Trauma Flow Sheet</Text>
          </View>
        ...
        {records.map((record, index) => {
          ...
          return (
            <View key={index} style={styles.recordContainer}>
              ...
              {/* Record Header — arrival datetime left, status right */}
              <View style={styles.recordHeader} wrap={false}>
                <View style={styles.recordDateRow}>
                  <Text style={styles.recordDate}>{record.traumaBayArrivalTime ? formatDateTime(record.traumaBayArrivalTime) : ''}</Text>
                  <Text style={styles.recordStatus}>{hasVal(record.status) ? fmtVal(record.status) : ''}</Text>
                </View>
                <Text style={styles.recordTitle}>
                  {`Trauma Flow Sheet ${recordNum}`}
                </Text>
              </View>
