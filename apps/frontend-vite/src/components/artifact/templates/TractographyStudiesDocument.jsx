/**
 * TractographyStudiesDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: tractography_studies
 *
 * 4 Sections:
 *   1. header-info: reportDate, reportType, urgency
 *   2. clinical-indication: clinicalIndication
 *   3. findings-followup: findings, followUp
...
const DRAFT_KEY = 'tractographyStudiesPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const TractographyStudiesDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.tractography_studies) return Array.isArray(r.tractography_studies) ? r.tractography_studies : [r.tractography_studies];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.tractography_studies) return Array.isArray(dd.tractography_studies) ? dd.tractography_studies : [dd.tractography_studies]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Tractography Study ${idx + 1}`.toLowerCase();
  ...
  // Save one sentence → splice into full text, then stage as a DRAFT (no DB write). localStorage keeps
  // it across refresh; Approve commits it. Synchronous now (no setSaving / no .then chain).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Only this section's fields that are staged drafts for this record
      ...
      // Persist each staged field to the DB now (field, + arrayIndex only for a numeric trailing segment)
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/tractography_studies/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/tractography_studies/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[TractographyStudies] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  if (!records || records.length === 0) {
    return (
      <div className="tractography-studies-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Tractography Studies</h2></div>
        <div className="empty-state">No tractography studies records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="tractography-studies-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search tractography studies..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'findings-followup')}
...
export default TractographyStudiesDocument;
