/**
 * FootExamDocumentPDFTemplate.jsx
 ...
 * stacked, recommendations date-grouped (string OR {recommendation,date} items), narratives split on [.;]
 ...
 * field's View. Static PHI footer. Collection: foot_exam.
 */
...
const SECTION_ORDER = ['session-info', 'exam-findings', 'sensation-findings', 'results-section', 'assessment-plan', 'recommendations-section', 'notes-section'];
const SECTION_TITLES = {
  'session-info': 'Session Information',
  'exam-findings': 'Examination Findings',
  'sensation-findings': 'Sensation & Detailed Findings',
...
const FIELD_LABELS = {
  ...
  ulcers: 'Ulcers', calluses: 'Calluses', deformities: 'Deformities', nailCondition: 'Nail Condition',
  skinCondition: 'Skin Condition', circulation: 'Circulation', footwear: 'Footwear',
  sensation: 'Sensation', findings: 'Detailed Findings', assessment: 'Assessment', plan: 'Plan',
  results: 'Results', recommendations: 'Recommendations', notes: 'Notes',
};
const SECTION_FIELDS = {
  'session-info': ['date', 'provider', 'facility', 'status'],
  'exam-findings': ['ulcers', 'calluses', 'deformities', 'nailCondition', 'skinCondition', 'circulation', 'footwear'],
  'sensation-findings': ['sensation', 'findings'],
  'results-section': ['results'],
  'assessment-plan': ['assessment', 'plan'],
  'recommendations-section': ['recommendations'],
  'notes-section': ['notes'],
...
const KEY_OVERRIDES = { id: 'ID', url: 'URL', bmi: 'BMI', abi: 'ABI' };
...
const renderField = (record, f, sectionTitle, isFirst) => {
  ...
  if (OBJECT_ARRAY_FIELDS.includes(f)) {
    const recs = (Array.isArray(val) ? val : []).map(r => {
      if (typeof r === 'string') return { recommendation: r.trim(), date: '' };
      if (r && typeof r === 'object') return { recommendation: String(r.recommendation || r.text || '').trim(), date: String(r.date || '').trim() };
      return { recommendation: String(r ?? '').trim(), date: '' };
    }).filter(r => r.recommendation);
    if (recs.length === 0) return [];
    const groups = [];
    recs.forEach((r) => { const d = r.date; const last = groups[groups.length - 1]; if (last && last.date === d) last.items.push(r); else groups.push({ date: d, items: [r] }); });
    return [(
      <View key={f} style={styles.fieldGroup} wrap={false}>
        ...
        {groups.map((group, gIdx) => (
          <View key={gIdx}>
            ...
            {group.items.map((r, i) => (<Text key={i} style={styles.value}>{i + 1}. {safeString(r.recommendation)}</Text>))}
          </View>
        ))}
      </View>
    )];
...
const FootExamDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].foot_exam && Array.isArray(templateData[0].foot_exam)) records = templateData[0].foot_exam;
    else records = templateData;
  } else if (templateData && templateData.foot_exam) {
    records = Array.isArray(templateData.foot_exam) ? templateData.foot_exam : [templateData.foot_exam];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.foot_exam ? (Array.isArray(dd.foot_exam) ? dd.foot_exam : [dd.foot_exam]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Foot Exam</Text></View>
        <Text style={styles.emptyState}>No foot exam records available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Foot Exam ${idx + 1}`}</Text>
...
export default FootExamDocumentPDFTemplate;
