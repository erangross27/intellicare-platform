/**
 * PsychiatricDischargeSummariesDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: psychiatric_discharge_summaries
 *
 * 8 Sections:
 *   1. admission-info: admissionDate, dischargeDate, legalStatus, dischargeDisposition
 *   2. presentation: presentingComplaint, precipitatingFactors, psychoticSymptoms
 *   3. diagnoses: primaryDiagnosis, secondaryDiagnoses
 *   4. clinical-assessment: suicidalIdeation, homicidalIdeation, mentalStatusExamAtDischarge, riskAssessmentAtDischarge
 *   5. treatment: therapeuticInterventions, dischargeMedications, medicationChanges, treatmentCompliance, restraintSeclusionEvents
 *   6. substance-use: substanceUseHistory
 *   7. safety-plan: dischargeSafetyPlan
 *   8. discharge-plan: aftercareArrangements, familyInvolvement, functionalStatus, insightAndJudgment
 */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits key without "-idx") */
const DRAFT_KEY = 'psychiatric_discharge_summariesPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['legalStatus', 'dischargeDisposition', 'presentingComplaint', 'primaryDiagnosis', 'suicidalIdeation', 'homicidalIdeation', 'treatmentCompliance', 'familyInvolvement', 'functionalStatus', 'insightAndJudgment'];

const MSE_SUB_LABELS = {
...
const RISK_SUB_LABELS = {
  suicideRisk: 'Suicide Risk',
  homicideRisk: 'Homicide Risk',
  relapseRisk: 'Relapse Risk',
};

const SUBSTANCE_SUB_LABELS = {
  current: 'Current Use',
  past: 'Past Use',
  lastUse: 'Last Use',
...
const splitByComma = (text) => {
  ...
  const result = []; let current = ''; let depth = 0; let inQ = false;
  for (let i = 0; i < text.length; i++) {
    const ch = text[i];
    if (ch === '"' || ch === '\u201C' || ch === '\u201D') { inQ = !inQ; current += ch; }
    else if (ch === '(' && !inQ) { depth++; current += ch; }
    else if (ch === ')' && !inQ) { depth = Math.max(0, depth - 1); current += ch; }
    else if (ch === ',' && depth === 0 && !inQ) { const t = current.trim(); if (t) result.push(t); current = ''; }
    else { current += ch; }
  }
...
/* ═══════ COMPONENT ═══════ */
const PsychiatricDischargeSummariesDocument = ({ document: docProp }) => {
  ...
  const getFieldValue = useCallback((record, fn, idx) => {
    ...
    /* dot-path support: "substanceUseHistory.current" */
    if (fn.includes('.')) {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Psychiatric Discharge Summary ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (val !== null && val !== undefined) {
            if (typeof val === 'object' && !Array.isArray(val)) {
              for (const sv of Object.values(val)) { if (sv && String(sv).toLowerCase().includes(phrase)) return true; }
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    setSaveError(null);
    let draftFieldPart, draftValue;
    if (fn.includes('.')) {
      /* Nested object sub-field: merge into the parent object under its localEdits key so UI + PDF reflect the edit while preserving object shape. */
      const [parentKey, ...rest] = fn.split('.');
      setLocalEdits(prev => {
        const baseRaw = prev[`${parentKey}-${idx}`] !== undefined ? prev[`${parentKey}-${idx}`] : record[parentKey];
        const base = (baseRaw && typeof baseRaw === 'object' && !Array.isArray(baseRaw)) ? { ...baseRaw } : {};
        let cursor = base;
        for (let i = 0; i < rest.length - 1; i++) {
          const seg = rest[i];
          cursor[seg] = (cursor[seg] && typeof cursor[seg] === 'object' && !Array.isArray(cursor[seg])) ? { ...cursor[seg] } : {};
          cursor = cursor[seg];
        }
        cursor[rest[rest.length - 1]] = saveVal;
        // Stage the merged parent object as the draft (mirrors the localEdits key).
        const store = readDrafts();
        if (!store[id]) store[id] = {};
        store[id][parentKey] = base;
        writeDrafts(store);
        return { ...prev, [`${parentKey}-${idx}`]: base };
      });
      draftFieldPart = parentKey;
    } else {
      ...
      const store = readDrafts();
      if (!store[id]) store[id] = {};
      store[id][draftFieldPart] = draftValue;
      writeDrafts(store);
    }
    setPendingEdits(prev => ({ ...prev, [`${draftFieldPart}-${idx}`]: true }));
  ...
  function saveSemicolonItem(record, fn, idx, itemIdx, newText) {
    ...
    setSaveError(null);
    stageFieldDraft(record, fn, idx, null, fullText);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    // Pending edits belonging to this record + section. localEdits keys = "<fieldPart>-<idx>";
    // fieldPart is the plain mongo field name (full value stored), so it matches SECTION_FIELDS directly.
    const suffix = `-${idx}`;
    ...
    try {
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "field" or "field.numericIndex"
        ...
        const resp = await secureApiClient.put(`/api/edit/psychiatric_discharge_summaries/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/psychiatric_discharge_summaries/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(k => delete n[k]); return n; });
      // Drop this section's committed fieldParts from the record's draft
      const store = readDrafts();
      if (store[id]) {
  ...
  const copyAllText = useCallback(async () => {
    ...
    pdfData.forEach((r, idx) => {
      text += `Psychiatric Discharge Summary ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderDateField = (record, fn, idx, sid) => {
    ...
    const isoVal = val ? (() => { try { return new Date(val.$date || val).toISOString().split('T')[0]; } catch { return ''; } })() : '';
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { currentArr.splice(itemIdx, 1, ...subParts); } else { currentArr[itemIdx] = trimmed; } stageFieldDraft(record, fn, idx, sid, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderStringField = (record, fn, idx, sid, title) => {
        ...
        <div key={fn}>
          <div className="rec-mini-card">
            ...
            {sentences.map((sentence, sIdx) => {
              ...
              /* Labeled sentence with multi-comma items */
              if (parsed.isLabeled) {
              ...
              /* Also split unlabeled sentences by semicolons */
              if (!parsed.isLabeled) {
                const scItems = splitBySemicolon(sentence);
                if (scItems.length >= 2) {
                  return (
                    <div key={sIdx}>
                      {scItems.map((scItem, scIdx) => {
                        const scKey = `${sentenceKey}-sc${scIdx}`;
                        const scEditing = editingField === scKey;
                        const scBadge = editedSentences[scKey];
                        const scMatches = phraseMatch || labelMatch || !searchTerm.trim() || scItem.toLowerCase().includes(searchTerm.toLowerCase().trim());
                        if (!scMatches && searchTerm.trim()) return null;
                        return (
                          <div key={scIdx}>
                            <div className={`numbered-row ${scBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!scEditing) { setEditingField(scKey); setEditValue(scItem); setSaveError(null); } }}>
                              {scEditing ? (
                                <div className="edit-field-container">
                                  <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                                  {saveError && <div className="save-error">{saveError}</div>}
                                  <div className="edit-actions">
                                    <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const currentVal3 = String(getFieldValue(record, fn, idx) || ''); const sentences3 = splitBySentence(currentVal3); const origSc = splitBySemicolon(sentences3[sIdx] || ''); origSc[scIdx] = editValue.trim(); sentences3[sIdx] = origSc.join('; '); const fullText3 = reconstructFullText(sentences3); const id3 = safeId(record); if (!id3) return; setSaveError(null); stageFieldDraft(record, fn, idx, sid, fullText3); setEditedSentences(prev => ({ ...prev, [scKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                                    <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                                  </div>
                                </div>
                              ) : (
                                <>
                                  <div className="row-content"><span className="content-value">{highlightText(scItem)}</span><span className="edit-indicator">&#9998;</span></div>
                                  <button className={`copy-btn ${copiedItems[scKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(scItem, scKey); }}>{copiedItems[scKey] ? 'Copied!' : 'Copy'}</button>
                                </>
                              )}
                            </div>
                            {scBadge && <span className={`modified-badge ${scBadge === 'added' ? 'added' : ''}`}>{scBadge === 'added' ? 'added - click Pending Approve to save' : 'edited - click Pending Approve to save'}</span>}
                          </div>
                        );
                      })}
                    </div>
    ...
    /* Single-value: check for semicolons (2+ semicolons => split into rows) */
  ...
  /* ═══════ RENDER: OBJECT FIELD (nested key/value sub-fields) ═══════ */
  const renderObjectField = (record, fn, idx, sid, subLabels) => {
    const val = getFieldValue(record, fn, idx);
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {entries.map(([subKey, subVal]) => {
          const subLabel = (subLabels && subLabels[subKey]) || prettifyKey(subKey);
          ...
          if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
            const phrase = searchTerm.toLowerCase().trim();
            const lbl = (FIELD_LABELS[fn] || fn).toLowerCase();
            const slbl = subLabel.toLowerCase();
            const svStr = Array.isArray(subVal) ? subVal.join(', ') : String(subVal);
            if (!lbl.includes(phrase) && !phrase.includes(lbl) && !slbl.includes(phrase) && !phrase.includes(slbl) && !svStr.toLowerCase().includes(phrase)) return null;
          }

          /* Sub-value is an array: render as numbered items */
          if (Array.isArray(subVal)) {
            const arrItems = subVal.filter(Boolean);
            if (arrItems.length === 0) return null;
            return (
              <div key={subKey} className="rec-mini-card" style={{ marginTop: 4 }}>
                <div className="nested-subtitle">{highlightText(subLabel)}</div>
                {arrItems.map((arrItem, arrIdx) => {
                  const arrEditKey = `${fn}.${subKey}.${arrIdx}-${idx}`;
                  const arrEditing = editingField === arrEditKey;
                  const arrModified = editedFields[arrEditKey];
                  const arrStr = String(arrItem);
                  return (
                    <div key={arrIdx}>
                      <div className={`numbered-row ${arrModified ? 'modified' : ''} editable-row`} onClick={() => { if (!arrEditing) { setEditingField(arrEditKey); setEditValue(arrStr); setSaveError(null); } }}>
                        {arrEditing ? (
                          <div className="edit-field-container">
                            <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                            {saveError && <div className="save-error">{saveError}</div>}
                            <div className="edit-actions">
                              <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveField(record, `${fn}.${subKey}`, idx, sid, null, editValue.trim(), arrEditKey); }}>{saving ? 'Saving...' : 'Save'}</button>
                              <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                            </div>
                          </div>
                        ) : (
                          <>
                            <div className="row-content"><span className="content-value">{highlightText(arrStr)}</span><span className="edit-indicator">&#9998;</span></div>
                            <button className={`copy-btn ${copiedItems[arrEditKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(arrStr, arrEditKey); }}>{copiedItems[arrEditKey] ? 'Copied!' : 'Copy'}</button>
                          </>
                        )}
                      </div>
                      {arrModified && <span className={`modified-badge ${arrModified === 'added' ? 'added' : ''}`}>{arrModified === 'added' ? 'added - click Pending Approve to save' : 'edited - click Pending Approve to save'}</span>}
                    </div>
                  );
                })}
              </div>
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(displayVal); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveField(record, `${fn}.${subKey}`, idx, sid, null, editValue.trim(), editKey); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (f === 'mentalStatusExamAtDischarge') return renderObjectField(record, f, idx, sid, MSE_SUB_LABELS);
            if (f === 'riskAssessmentAtDischarge') return renderObjectField(record, f, idx, sid, RISK_SUB_LABELS);
            if (f === 'substanceUseHistory') return renderObjectField(record, f, idx, sid, SUBSTANCE_SUB_LABELS);
            if (f === 'dischargeSafetyPlan') return renderObjectField(record, f, idx, sid, SAFETY_PLAN_SUB_LABELS);
            if (f === 'aftercareArrangements') return renderObjectField(record, f, idx, sid, AFTERCARE_SUB_LABELS);
            if (OBJECT_FIELDS.includes(f)) return renderObjectField(record, f, idx, sid, null);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="psychiatric-discharge-summaries-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Psychiatric Discharge Summary</h2></div>
        <div className="empty-state">No psychiatric discharge summary records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="psychiatric-discharge-summaries-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search psychiatric discharge summary records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      ...
      <div className="records-container">
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            ...
            {renderSection(record, idx, 'presentation')}
            {renderSection(record, idx, 'diagnoses')}
            ...
            {renderSection(record, idx, 'discharge-plan')}
          </div>
        ))}
      </div>
    </div>
...
export default PsychiatricDischargeSummariesDocument;
