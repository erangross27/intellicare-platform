 ...
 * OBJECT fields (tantrums, results) rendered recursively as humanized key/value lines.
 * String arrays (anxietySymptoms, adhdSymptoms, autismRedFlags) -> numbered list.
 * recommendations (array of {recommendation, date} or plain strings) date-grouped numbered list.
 */
...
const FIELD_LABELS = {
  ...
  temperament: 'Temperament', attentionSpan: 'Attention Span', activityLevel: 'Activity Level',
  socialSkills: 'Social Skills', emotionalRegulation: 'Emotional Regulation', tantrums: 'Tantrums',
  anxietySymptoms: 'Anxiety Symptoms', panicAttackFrequency: 'Panic Attack Frequency', weightChange: 'Weight Change',
  adhdSymptoms: 'ADHD Symptoms', autismRedFlags: 'Autism Red Flags',
...
/* recursive object node: label = bold heading; value = plain line below; arrays rendered as numbered lines */
      ...
      <View key={field} style={styles.fieldGroup} wrap={recs.length > 8 ? undefined : false}>
        ...
        {groups.map((group, gIdx) => (
          <View key={gIdx}>
            ...
            {group.items.map((r, i) => (<Text key={i} style={styles.value}>{i + 1}. {(typeof r === 'string' ? r : (r?.recommendation || '')).trim()}</Text>))}
          </View>
        ))}
      </View>
    )];
...
const BehavioralAssessmentPDFTemplate = ({ document: data }) => {
  let records = [];
  if (Array.isArray(data)) {
    if (data.length === 1 && data[0]?.behavioral_assessment) records = Array.isArray(data[0].behavioral_assessment) ? data[0].behavioral_assessment : [data[0].behavioral_assessment];
    else records = data;
  } else if (data?.behavioral_assessment) records = Array.isArray(data.behavioral_assessment) ? data.behavioral_assessment : [data.behavioral_assessment];
  else if (data?.documentData) { const dd = data.documentData; if (Array.isArray(dd)) records = dd; else if (dd?.behavioral_assessment) records = Array.isArray(dd.behavioral_assessment) ? dd.behavioral_assessment : [dd.behavioral_assessment]; else if (dd && typeof dd === 'object') records = [dd]; }
  else if (data && typeof data === 'object') records = [data];
  records = (records || []).filter(r => r && typeof r === 'object');

  if (records.length === 0) {
    return (<Document><Page size="A4" style={styles.page}><View style={styles.documentHeader}><Text style={styles.title}>Behavioral Assessment</Text></View><Text style={styles.emptyState}>No records available</Text></Page></Document>);
  }

  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.title}>Behavioral Assessment</Text></View>
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer}>
            <View style={styles.recordHeader} wrap={false}>
              <Text style={styles.recordTitle}>{`Behavioral Assessment ${String(record._recordNumber || idx + 1)}`}</Text>
...
export default BehavioralAssessmentPDFTemplate;
