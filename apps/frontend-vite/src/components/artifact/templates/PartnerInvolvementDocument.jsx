/**
 * PartnerInvolvementDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: partner_involvement
 *
 * 5 Sections:
 *   1. relationship-capacity: partnerRelationshipType, partnerMedicalDecisionCapacity, partnerHealthLiteracyLevel
 *   2. caregiver-wellbeing: partnerCaregiverBurdenScore, partnerDepressionScreening, partnerAnxietyScreening, partnerSocialSupportScore, partnerQualityOfLifeScore, partnerSleepQualityImpact
 *   3. care-roles: partnerMedicationManagementRole, partnerSymptomMonitoringCapability, partnerDietaryManagementInvolvement, partnerPhysicalAssistanceLevel, partnerEmergencyResponseTraining, partnerTreatmentAdherenceMonitoring
 *   4. communication-navigation: partnerCommunicationEffectiveness, partnerHealthcareNavigationSupport
 *   5. coping-impact: partnerCopingMechanismsAssessment, partnerFinancialBurdenImpact, partnerWorkLifeImpairment, partnerReligiousSpiritalSupport
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PartnerInvolvementDocumentPDFTemplate from '../pdf-templates/PartnerInvolvementDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './PartnerInvolvementDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'partner_involvementPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'relationship-capacity': 'Relationship & Capacity',
  'caregiver-wellbeing': 'Caregiver Wellbeing Scores',
  'care-roles': 'Care Roles',
  'communication-navigation': 'Communication & Navigation',
  'coping-impact': 'Coping & Impact',
};

const FIELD_LABELS = {
  partnerRelationshipType: 'Relationship Type',
  partnerMedicalDecisionCapacity: 'Medical Decision-Making Capacity',
  partnerHealthLiteracyLevel: 'Health Literacy Level',
  partnerCaregiverBurdenScore: 'Caregiver Burden Score (Zarit)',
  partnerDepressionScreening: 'Depression Screening (PHQ)',
  partnerAnxietyScreening: 'Anxiety Screening (GAD)',
  partnerSocialSupportScore: 'Social Support Score',
  partnerQualityOfLifeScore: 'Quality of Life Score (SF-36)',
  partnerSleepQualityImpact: 'Sleep Quality Impact (PSQI)',
  partnerMedicationManagementRole: 'Medication Management Role',
  partnerSymptomMonitoringCapability: 'Symptom Monitoring Capability',
  partnerDietaryManagementInvolvement: 'Dietary Management Involvement',
  partnerPhysicalAssistanceLevel: 'Physical Assistance Level',
  partnerEmergencyResponseTraining: 'Emergency Response Training',
  partnerTreatmentAdherenceMonitoring: 'Treatment Adherence Monitoring',
  partnerCommunicationEffectiveness: 'Communication Effectiveness',
  partnerHealthcareNavigationSupport: 'Healthcare Navigation Support',
  partnerCopingMechanismsAssessment: 'Coping Mechanisms Assessment',
  partnerFinancialBurdenImpact: 'Financial Burden Impact',
  partnerWorkLifeImpairment: 'Work-Life Impairment',
  partnerReligiousSpiritalSupport: 'Religious & Spiritual Support',
};

const SECTION_FIELDS = {
  'relationship-capacity': ['partnerRelationshipType', 'partnerMedicalDecisionCapacity', 'partnerHealthLiteracyLevel'],
  'caregiver-wellbeing': ['partnerCaregiverBurdenScore', 'partnerDepressionScreening', 'partnerAnxietyScreening', 'partnerSocialSupportScore', 'partnerQualityOfLifeScore', 'partnerSleepQualityImpact'],
  'care-roles': ['partnerMedicationManagementRole', 'partnerSymptomMonitoringCapability', 'partnerDietaryManagementInvolvement', 'partnerPhysicalAssistanceLevel', 'partnerEmergencyResponseTraining', 'partnerTreatmentAdherenceMonitoring'],
  'communication-navigation': ['partnerCommunicationEffectiveness', 'partnerHealthcareNavigationSupport'],
  'coping-impact': ['partnerCopingMechanismsAssessment', 'partnerFinancialBurdenImpact', 'partnerWorkLifeImpairment', 'partnerReligiousSpiritalSupport'],
};

const NUMBER_FIELDS = ['partnerCaregiverBurdenScore', 'partnerDepressionScreening', 'partnerAnxietyScreening', 'partnerSocialSupportScore', 'partnerQualityOfLifeScore', 'partnerSleepQualityImpact'];
const ARRAY_FIELDS = [];
const BOOLEAN_FIELDS = ['partnerMedicalDecisionCapacity', 'partnerEmergencyResponseTraining', 'partnerTreatmentAdherenceMonitoring'];
const STRING_FIELDS = ['partnerRelationshipType', 'partnerHealthLiteracyLevel', 'partnerMedicationManagementRole', 'partnerSymptomMonitoringCapability', 'partnerDietaryManagementInvolvement', 'partnerPhysicalAssistanceLevel', 'partnerCommunicationEffectiveness', 'partnerHealthcareNavigationSupport', 'partnerCopingMechanismsAssessment', 'partnerFinancialBurdenImpact', 'partnerWorkLifeImpairment', 'partnerReligiousSpiritalSupport'];
...
/* ═══════ COMPONENT ═══════ */
const PartnerInvolvementDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Partner Involvement Assessment ${idx + 1}`.toLowerCase();
  ...
  function clearApprovedForField(sid, idx) {
    setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
  }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const fieldPart of toCommit) {
        ...
        const payload = { field: isArrayIdx ? fieldPart.slice(0, lastDot) : fieldPart, value: recDrafts[fieldPart] };
        if (isArrayIdx) payload.arrayIndex = parseInt(fieldPart.slice(lastDot + 1), 10);
        const resp = await secureApiClient.put(`/api/edit/partner_involvement/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/partner_involvement/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; toCommit.forEach(fp => { delete n[`${fp}-${idx}`]; }); return n; });
      // Drop this record's committed drafts from localStorage
      if (store[id]) { toCommit.forEach(fp => delete store[id][fp]); if (Object.keys(store[id]).length === 0) delete store[id]; writeDrafts(store); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PARTNER INVOLVEMENT ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Partner Involvement Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="partner-involvement-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Partner Involvement Assessment</h2></div>
        <div className="empty-state">No partner involvement assessment data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="partner-involvement-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Partner Involvement Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PartnerInvolvementDocumentPDFTemplate document={pdfData} />} fileName={`partner-involvement-assessment-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search partner involvement assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Partner Involvement Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'relationship-capacity')}
            {renderSection(record, idx, 'caregiver-wellbeing')}
            {renderSection(record, idx, 'care-roles')}
            {renderSection(record, idx, 'communication-navigation')}
            {renderSection(record, idx, 'coping-impact')}
          </div>
        ))}
      </div>
    </div>
...
export default PartnerInvolvementDocument;
