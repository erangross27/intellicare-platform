/**
 * GeneticTestingReportsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: genetic_testing_reports
 *
 * 4 Sections:
 *   1. session-info: reportingDate (date), labAccreditation (string), ethnicity (string)
 *   2. genetic-testing: genesTested (array), testingMethodology (string), testCoverage (string),
 *                       clinicalIndication (string), familyHistory (string)
 *   3. variants-classification: pathogenicVariants (array), variantsOfUncertainSignificance (array),
 *                               acmgClassification (string), inheritancePattern (string),
 *                               penetrance (string), chromosomalLocation (string),
 *                               copyNumberVariants (array), mosaicism (boolean), alleleFrequency (string)
 *   4. clinical-implications: geneticCounselingRecommended (boolean), cascadeTesting (string),
 *                             pharmacogenomicImplications (array)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import GeneticTestingReportsDocumentPDFTemplate from '../pdf-templates/GeneticTestingReportsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './GeneticTestingReportsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = base field name) */
const DRAFT_KEY = 'genetic_testing_reportsPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'genetic-testing': 'Genetic Testing Details',
  'variants-classification': 'Variants & Classification',
...
const FIELD_LABELS = {
  ...
  cascadeTesting: 'Cascade Testing',
  pharmacogenomicImplications: 'Pharmacogenomic Implications',
};
...
const STRING_FIELDS = ['labAccreditation', 'ethnicity', 'testingMethodology', 'testCoverage', 'clinicalIndication', 'familyHistory', 'acmgClassification', 'inheritancePattern', 'penetrance', 'chromosomalLocation', 'alleleFrequency', 'cascadeTesting'];
...
/* ═══════ COMPONENT ═══════ */
const GeneticTestingReportsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.genetic_testing_reports) return Array.isArray(r.genetic_testing_reports) ? r.genetic_testing_reports : [r.genetic_testing_reports];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.genetic_testing_reports) return Array.isArray(dd.genetic_testing_reports) ? dd.genetic_testing_reports : [dd.genetic_testing_reports]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Genetic Testing Report ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (val && Array.isArray(val)) { if (val.some(item => String(item).toLowerCase().includes(phrase))) return true; }
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // stageDraft = stage a DRAFT locally + write to localStorage (survives refresh). NO DB write.
  // Always keyed by `${fn}-${idx}` (full field value). Approve commits to DB.
  const stageDraft = useCallback((record, fn, idx, sid, value, trackKey, kind = 'edited') => {
    const id = safeId(record); if (!id) return;
    const editKey = `${fn}-${idx}`;
    setLocalEdits(prev => ({ ...prev, [editKey]: value }));
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
    setEditedFields(prev => ({ ...prev, [trackKey || editKey]: kind }));
    // Re-edit after approval → drop approved flag so the button returns to yellow Pending Approve
  ...
  // stageDraftSentences = stage a DRAFT + set per-sentence markers (mirrors stageDraft, sentence-based).
  const stageDraftSentences = useCallback((record, fn, idx, sid, value, sentenceMarks) => {
    const id = safeId(record); if (!id) return;
    const editKey = `${fn}-${idx}`;
    setLocalEdits(prev => ({ ...prev, [editKey]: value }));
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
  ...
  const handleSaveArrayItem = useCallback((record, fn, idx, arrayIndex) => {
    ...
    // Stage the WHOLE updated array under `${fn}-${idx}` (matches localEdits convention + DB array set on approve).
    stageDraft(record, fn, idx, null, arr, `${fn}-${idx}-ai${arrayIndex}`);
  ...
  const handleSaveBooleanField = useCallback((record, fn, idx) => {
    ...
    stageDraft(record, fn, idx, null, boolVal, `${fn}-${idx}`);
  }, [editValue, safeId, stageDraft]);
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      // Stage DRAFT (no DB write). Approve commits.
      stageDraftSentences(record, fn, idx, sid, fullText, { [`${fn}-${idx}-s${sentenceIdx}`]: 'edited' });
      return;
    }
    ...
    stageDraftSentences(record, fn, idx, sid, fullText, marks);
  }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    // localEdits keys are `${fieldName}-${idx}` (fieldName has NO numeric dot-suffix here → no arrayIndex).
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const fieldName = m[1];
        // GOTCHA: arrayIndex only when the segment after the LAST dot is purely numeric. Never the case here.
        const lastDot = fieldName.lastIndexOf('.');
        const payload = { field: fieldName, value: localEdits[editKey] };
        if (lastDot !== -1 && /^\d+$/.test(fieldName.slice(lastDot + 1))) {
          payload.field = fieldName.slice(0, lastDot);
          payload.arrayIndex = parseInt(fieldName.slice(lastDot + 1), 10);
        }
        const resp = await secureApiClient.put(`/api/edit/genetic_testing_reports/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/genetic_testing_reports/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== GENETIC TESTING REPORTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Genetic Testing Report ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="genetic-testing-reports-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Genetic Testing Reports</h2></div>
        <div className="empty-state">No genetic testing reports records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="genetic-testing-reports-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Genetic Testing Reports</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<GeneticTestingReportsDocumentPDFTemplate document={pdfData} />} fileName="Genetic_Testing_Reports.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search genetic testing reports..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Genetic Testing Report ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'genetic-testing')}
            {renderSection(record, idx, 'variants-classification')}
            {renderSection(record, idx, 'clinical-implications')}
          </div>
        ))}
      </div>
    </div>
...
export default GeneticTestingReportsDocument;
