/**
 * SubstanceUseAssessment Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 *
 * PUT /api/edit/substance_use_assessment/:id/edit    — Edit a single field
 * PUT /api/edit/substance_use_assessment/:id/approve — Approve all pending edits
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'substance-use-assessment-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    const result = await sda.update('substance_use_assessment', { _id: objectId }, {
      $set: setFields,
    ...
    console.error('[SubstanceUseAssessment] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    await sda.update('substance_use_assessment', { _id: objectId }, {
      $set: {
    ...
    console.error('[SubstanceUseAssessment] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
