...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'visual-acuity-reports-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'logmarRightEye', 'logmarLeftEye',
  'etdrsLetterScoreRightEye', 'etdrsLetterScoreLeftEye',
  'refractiveErrorRightEye', 'refractiveErrorLeftEye',
  'pupillaryDefectRightEye', 'pupillaryDefectLeftEye',
  'contrastSensitivityRightEye', 'contrastSensitivityLeftEye',
  'binocularVisionStatus',
  'testingDistance', 'testingConditions',
  'visualFieldDefects',
];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('visual_acuity_reports', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[visual_acuity_reports] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('visual_acuity_reports', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[visual_acuity_reports] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
