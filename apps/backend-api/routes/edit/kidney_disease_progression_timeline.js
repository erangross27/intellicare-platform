...
function buildContext(req, operation = 'read') { return { serviceId: 'kidney-disease-progression-timeline-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['estimatedGlomerularFiltrationRate', 'serumCreatinine', 'bloodUreaNitrogen', 'chronicKidneyDiseaseStage', 'albuminCreatinineRatio', 'proteinCreatinineRatio', 'albuminuriaCategory', 'acidBaseStatus', 'hemoglobinLevel', 'serumPhosphorus', 'serumCalcium', 'parathyroidHormone', 'vitamin25OHD', 'renalUltrasoundFindings', 'kidneySizeLeftCm', 'kidneySizeRightCm', 'systemicBloodPressure', 'renalBiopsyResults', 'dialysisModalityType', 'dialysisAdequacyKtV', 'vascularAccessType', 'transplantEvaluationStatus', 'comorbidConditions'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('kidney_disease_progression_timeline', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[KidneyDiseaseProgressionTimeline] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('kidney_disease_progression_timeline', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[KidneyDiseaseProgressionTimeline] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
