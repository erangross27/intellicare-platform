/**
 * InfectionSurveillanceDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: infection_surveillance
 *
 * 9 Sections:
 *   1. infection-overview: date, infectionType, infectionSite, onsetDate, detectionMethod
 *   2. microbiology: pathogenIdentified, specimenType, cultureDate
 *   3. resistance-profile: antibioticResistance, multidrugResistant
 *   4. isolation: isolationPrecautions, isolationStartDate
 *   5. device-association: deviceAssociated, deviceType, deviceDaysBeforeInfection
 *   6. surgical-association: surgicalProcedureRelated, procedureName, procedureDate
 *   7. reporting: nhsnCategory, reportableDisease, publicHealthNotified, outbreakAssociated
 *   8. infection-control: infectionControlMeasures, transmissionRoute
 *   9. treatment: antimicrobialTherapy, infectionResolutionDate
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import InfectionSurveillanceDocumentPDFTemplate from '../pdf-templates/InfectionSurveillanceDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './InfectionSurveillanceDocument.css';
...
const DRAFT_KEY = 'infection_surveillancePendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['infectionType', 'infectionSite', 'detectionMethod', 'specimenType', 'isolationPrecautions', 'deviceType', 'procedureName', 'nhsnCategory', 'transmissionRoute'];
...
/* ======= COMPONENT ======= */
const InfectionSurveillanceDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.infection_surveillance) return Array.isArray(r.infection_surveillance) ? r.infection_surveillance : [r.infection_surveillance];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.infection_surveillance) return Array.isArray(dd.infection_surveillance) ? dd.infection_surveillance : [dd.infection_surveillance]; return [dd]; }
      return [r];
    });
  ...
  const fmtVal = useCallback((v) => { if (typeof v === 'boolean') return v ? 'Yes' : 'No'; if (typeof v === 'number') return String(v); if (v instanceof Date || (v && typeof v === 'object' && v.$date)) return formatDate(v); return String(v || ''); }, []);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Infection Surveillance ${idx + 1}`.toLowerCase();
  ...
  /* ======= EDIT HANDLERS ======= */
  // Stage a DRAFT for a whole field (string/array) locally + write it to the pending-drafts
  // localStorage store (survives refresh). NOT written to MongoDB and NOT shown in the PDF until
  // the user clicks Approve (handleApproveSection commits). sid is used to drop a re-edited section
  // back to yellow "Pending Approve".
  ...
  // Save = stage a DRAFT locally (no DB write). Approve (handleApproveSection) commits to MongoDB.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // Each staged draft is keyed `${fn}-${idx}` in localEdits; the value is the WHOLE field (string or array).
      const toCommit = fields
        .map(f => `${f}-${idx}`)
        .filter(editKey => pendingEdits[editKey] && localEdits[editKey] !== undefined);
      for (const editKey of toCommit) {
        // editKey = "field-idx"; the trailing dot-segment is never numeric here, so NO arrayIndex.
        const field = editKey.slice(0, editKey.length - `-${idx}`.length);
        const resp = await secureApiClient.put(`/api/edit/infection_surveillance/${id}/edit`, { field, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/infection_surveillance/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (ARRAY_FIELDS.includes(f)) {
        const raw = getFieldValue(record, f, idx);
        const items = Array.isArray(raw) ? raw.filter(v => v && String(v).trim()) : [];
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== INFECTION SURVEILLANCE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Infection Surveillance ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ======= RENDER: NUMBER FIELD — −/+ stepper (NEVER type=number); renders 0 ======= */
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* Multi-sentence OR a single labeled "Label: v…" sentence: decompose (never side-by-side) */
    if (sentences.length > 1 || (sentences.length === 1 && parseLabel(sentences[0]).isLabeled)) {
  ...
  if (!records || records.length === 0) {
    return (
      <div className="infection-surveillance-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Infection Surveillance</h2></div>
        <div className="empty-state">No infection surveillance records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="infection-surveillance-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Infection Surveillance</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<InfectionSurveillanceDocumentPDFTemplate document={pdfData} />} fileName="Infection_Surveillance.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search infection surveillance records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Infection Surveillance ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'infection-overview')}
            {renderSection(record, idx, 'microbiology')}
            {renderSection(record, idx, 'resistance-profile')}
            {renderSection(record, idx, 'isolation')}
            {renderSection(record, idx, 'device-association')}
            {renderSection(record, idx, 'surgical-association')}
...
export default InfectionSurveillanceDocument;
