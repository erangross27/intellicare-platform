...
const SECTIONS = [
  { id: 'assessmentInfo', title: 'Assessment Information', fields: ['date', 'provider', 'facility'] },
  { id: 'overview', title: 'Sleep Disturbance Overview', fields: ['present', 'status', 'type'] },
  { id: 'description', title: 'Description', fields: ['description'] },
  { id: 'causes', title: 'Causes', fields: ['causes'] },
  { id: 'interventions', title: 'Interventions', fields: ['interventions'] },
  { id: 'clinicalFindings', title: 'Clinical Findings & Assessment', fields: ['findings', 'assessment', 'plan'] },
  { id: 'results', title: 'Results', fields: ['results'] },
  { id: 'followUp', title: 'Follow-Up & Recommendations', fields: ['recommendations', 'notes'] },
];
const FIELD_LABELS = {
...
export default function SleepDisturbancesDocumentPDFTemplate({ document: data }) {
  const records = React.useMemo(() => unwrap(data), [data]);
  return <Document><Page size="LETTER" style={styles.page}>
    <View style={styles.documentHeader} wrap={false}><Text style={styles.documentTitle}>Sleep Disturbances</Text></View>
    {!records.length && <Text style={styles.noDataText}>No sleep disturbances data available</Text>}
    {records.map((record, recordIndex) => <View key={recordIndex} style={styles.recordContainer} break={recordIndex > 0}>
      <View wrap={false}><Text style={styles.recordTitle}>Sleep Disturbances Record {recordIndex + 1}</Text></View>
