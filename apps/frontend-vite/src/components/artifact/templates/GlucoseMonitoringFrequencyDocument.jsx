/**
 * GlucoseMonitoringFrequencyDocument.jsx
 * June 2026 — Full template standard: inline editing, blue glow theme
 * Collection: glucose_monitoring_frequency
 *
 * 6 Sections:
 *   1. session-info: date (date picker), provider (string), facility (string)
 *   2. monitoring-details: recommended (string), current (string), status (string)
 *   3. clinical-assessment: findings (string/long), assessment (string/long), plan (string/long)
 *   4. results-section: results (OBJECT, recursive renderObjectNode/renderObjectLeaf)
 *   5. recommendations-section: recommendations (ARRAY of {recommendation, date}, date-grouped)
 *   6. additional-info: adjustments (string/long), concerns (string/long), notes (string/long)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import GlucoseMonitoringFrequencyDocumentPDFTemplate from '../pdf-templates/GlucoseMonitoringFrequencyDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './GlucoseMonitoringFrequencyDocument.css';
...
const DRAFT_KEY = 'glucose_monitoring_frequencyPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  ...
  'monitoring-details': ['recommended', 'current', 'status'],
  'clinical-assessment': ['findings', 'assessment', 'plan'],
  'results-section': ['results'],
  'recommendations-section': ['recommendations'],
  'additional-info': ['adjustments', 'concerns', 'notes'],
};

const SECTION_ORDER = ['session-info', 'monitoring-details', 'clinical-assessment', 'results-section', 'recommendations-section', 'additional-info'];

/* No number fields — all strings */
const NUMBER_FIELDS = [];

const STRING_FIELDS = ['provider', 'facility', 'recommended', 'current', 'findings', 'assessment', 'plan', 'adjustments', 'concerns', 'notes'];
...
const KEY_OVERRIDES = {
  hba1c: 'HbA1c', a1c: 'A1c', bg: 'BG', cgm: 'CGM', smbg: 'SMBG', tir: 'TIR', gmi: 'GMI',
};
...
/* ═══════ COMPONENT ═══════ */
const GlucoseMonitoringFrequencyDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.glucose_monitoring_frequency) return Array.isArray(r.glucose_monitoring_frequency) ? r.glucose_monitoring_frequency : [r.glucose_monitoring_frequency];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.glucose_monitoring_frequency) return Array.isArray(dd.glucose_monitoring_frequency) ? dd.glucose_monitoring_frequency : [dd.glucose_monitoring_frequency]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Glucose Monitoring Frequency ${idx + 1}`.toLowerCase();
  ...
  /* Stage a DRAFT for one whole field locally + write it to the pending-drafts localStorage store
     ...
     Pending Approve (handleApproveSection commits). fieldPart = the field name; localEdits/drafts
     are keyed per field, so the whole field value is staged (matches the original PUT semantics). */
  const stageDraft = useCallback((record, fn, idx, saveVal, trackKey) => {
    const id = safeId(record); if (!id) return;
    const editKey = `${fn}-${idx}`;
    setLocalEdits(prev => ({ ...prev, [editKey]: saveVal }));
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
    setEditedFields(prev => ({ ...prev, [trackKey || editKey]: 'edited' }));
    // Re-edit after approval → drop the approved flag for every section that owns this field so the
    // button returns to yellow "Pending Approve".
    setApprovedSections(prev => {
      const owners = Object.entries(SECTION_FIELDS).filter(([, fields]) => fields.includes(fn)).map(([s]) => `${s}-${idx}`);
      if (!owners.some(k => prev[k])) return prev;
      const next = { ...prev };
      owners.forEach(k => delete next[k]);
      return next;
    });
    const store = readDrafts();
  ...
  /* Stage a nested OBJECT leaf edit by dot-path (e.g. results.tir) — value stays a STRING.
     Stages the whole reconstructed root object as a DRAFT (no DB write); Approve commits it. */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    setApprovedSections(prev => {
      const owners = Object.entries(SECTION_FIELDS).filter(([, fields]) => fields.includes(rootField)).map(([s]) => `${s}-${idx}`);
      if (!owners.some(k => prev[k])) return prev;
      const next = { ...prev };
      owners.forEach(k => delete next[k]);
      return next;
  ...
  /* Stage the full reconstructed text for field `fn` as a DRAFT (no DB write). Survives refresh;
     Approve commits it. Mirrors stageDraft but lets callers set their own sentence markers. */
  const stageFieldDraft = (record, fn, idx, fullText) => {
    const id = safeId(record); if (!id) return;
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Persist each staged field for this section now. localEdits is keyed "<field>-<idx>" and holds
      // the WHOLE field value (string, array, or object), matching the original per-field PUT semantics.
      ...
      for (const editKey of toCommit) {
        const fieldName = editKey.match(/^(.+)-(\d+)$/)[1];
        const payload = { field: fieldName, value: localEdits[editKey] };
        const resp = await secureApiClient.put(`/api/edit/glucose_monitoring_frequency/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/glucose_monitoring_frequency/${id}/approve`, { sectionId: sid, approved: true });
  ...
  /* object → copy lines (recursive, STACKED — sub-label then numbered value, never "label: value") */
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== GLUCOSE MONITORING FREQUENCY ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Glucose Monitoring Frequency ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="glucose-monitoring-frequency-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Glucose Monitoring Frequency</h2></div>
        <div className="empty-state">No glucose monitoring frequency records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="glucose-monitoring-frequency-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Glucose Monitoring Frequency</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<GlucoseMonitoringFrequencyDocumentPDFTemplate document={pdfData} />} fileName="Glucose_Monitoring_Frequency.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search glucose monitoring frequency..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Glucose Monitoring Frequency ${idx + 1}`)}</h3>
            </div>
...
export default GlucoseMonitoringFrequencyDocument;
