/**
 * Blood Smears Edit Route
 * PUT /api/edit/blood_smears/:id/edit    — Edit a single field
 * PUT /api/edit/blood_smears/:id/approve — Approve pending edits
 */
...
function buildContext(req, operation = 'read') { return { serviceId: 'blood-smears-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }

const ALLOWED_FIELDS = [
  'specimenType', 'collectionSite', 'stainingMethod', 'smearQuality',
  'whiteBloodCellCount', 'blastPercentage',
  'clinicalIndication', 'rouleauxFormation',
  'redBloodCellMorphology', 'plateletEstimate', 'plateletMorphology',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('blood_smears', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[BloodSmears] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('blood_smears', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[BloodSmears] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
