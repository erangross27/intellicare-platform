/**
 * IbdAssessmentDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: ibd_assessment
 *
 * 7 Sections:
 *   1. session-info: date (date picker), provider (string), facility (string), status (string)
 *   2. disease-characteristics: diseaseType (string), diseaseExtent (string),
 *                               diseaseLocation (string), diseaseBehavior (string),
 *                               dateOfDiagnosis (date picker)
 *   3. current-flare: currentFlare.duration (string), currentFlare.severity (string),
 *                     currentFlare.trigger (string)
 *   4. medical-history: previousHospitalizations (array), previousSurgeries (array)
 *   5. clinical-findings: findings (long string, sentence-split)
 *   6. assessment-plan: assessment (string, sentence-split), plan (string, sentence-split)
 *   7. results-section: results (OBJECT, recursive key/value)
 *   8. recommendations-section: recommendations (ARRAY of {recommendation, date}, date-grouped)
 *   9. notes-section: notes (string, sentence-split)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import IbdAssessmentDocumentPDFTemplate from '../pdf-templates/IbdAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './IbdAssessmentDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'session-info': 'Assessment Information',
  'disease-characteristics': 'Disease Characteristics',
  'current-flare': 'Current Flare',
...
const FIELD_LABELS = {
  ...
  'currentFlare.duration': 'Duration',
  'currentFlare.severity': 'Severity',
  'currentFlare.trigger': 'Trigger',
...
const SECTION_FIELDS = {
  ...
  'disease-characteristics': ['diseaseType', 'diseaseExtent', 'diseaseLocation', 'diseaseBehavior', 'dateOfDiagnosis'],
  'current-flare': ['currentFlare.duration', 'currentFlare.severity', 'currentFlare.trigger'],
  'medical-history': ['previousHospitalizations', 'previousSurgeries'],
  'clinical-findings': ['findings'],
  'assessment-plan': ['assessment', 'plan'],
  'results-section': ['results'],
  'recommendations-section': ['recommendations'],
...
const STRING_FIELDS = ['provider', 'facility', 'status', 'diseaseType', 'diseaseExtent', 'diseaseLocation', 'diseaseBehavior',
  'currentFlare.duration', 'currentFlare.severity', 'currentFlare.trigger'];
const LONG_STRING_FIELDS = ['findings', 'assessment', 'plan', 'notes'];
const DATE_FIELDS = ['date', 'dateOfDiagnosis'];
const ARRAY_FIELDS = ['previousHospitalizations', 'previousSurgeries'];
...
const KEY_OVERRIDES = { ef: 'EF', lvef: 'LVEF', ivc: 'IVC', bpm: 'BPM', hr: 'HR', poc: 'POC', pocus: 'POCUS', crp: 'CRP', esr: 'ESR', wbc: 'WBC', hgb: 'Hgb', ibd: 'IBD' };
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits "field-idx" field part, e.g. "findings" or "currentFlare.duration") */
const DRAFT_KEY = 'ibd_assessmentPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const IbdAssessmentDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.ibd_assessment) return Array.isArray(r.ibd_assessment) ? r.ibd_assessment : [r.ibd_assessment];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.ibd_assessment) return Array.isArray(dd.ibd_assessment) ? dd.ibd_assessment : [dd.ibd_assessment]; return [dd]; }
      return [r];
    });
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     fieldPart is the localEdits field part (e.g. "findings", "currentFlare.duration"); value is the full field value. */
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        // Mark BOTH trackers so the owning section's Pending Approve button lights up regardless of field type.
  ...
  /* getFieldValue: supports dotted paths like currentFlare.duration */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `IBD Assessment ${idx + 1}`.toLowerCase();
  ...
  /* Stage a DRAFT locally + persist to the pending-drafts localStorage store (survives refresh).
     fieldPart is the localEdits field part (the full field value lives under `${fieldPart}-${idx}`).
     NOT written to MongoDB and NOT shown in the PDF until the user clicks Approve (handleApproveSection commits).
     Also clears the owning section's approvedSections key so a re-edit returns the button to yellow Pending Approve. */
  const stageDraft = useCallback((record, fieldPart, idx, value) => {
    ...
    setApprovedSections(prev => {
      const sid = Object.keys(SECTION_FIELDS).find(s => SECTION_FIELDS[s].includes(fieldPart));
  ...
  /* save a nested OBJECT leaf by dot-path (e.g. results.crp) — value stays a STRING/scalar.
     Stages the whole rebuilt object as a DRAFT under `${rootField}-${idx}` (committed whole on Approve). */
  ...
  /* ═══════ ARRAY ITEM SAVE ═══════ */
  function saveArrayItem(record, fn, idx, arrIdx) {
    const id = safeId(record); if (!id) return;
    const trimmed = editValue.trim();
    setSaveError(null);
    const currentArr = [...(getFieldValue(record, fn, idx) || [])];
    currentArr[arrIdx] = trimmed;
  ...
  // Approve = COMMIT this section's staged drafts to MongoDB, then clear pending so committed values flow
  // into pdfData/PDF. This is the ONLY path that writes to the database.
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // Pending editKeys for THIS record + THIS section. editKey = `${fieldPart}-${idx}` (fieldPart e.g. "findings" or "currentFlare.duration").
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // "field" or "field.with.dots"
        // arrayIndex ONLY when the segment after the LAST dot is purely numeric (no such keys here, but keep parity).
        ...
        const resp = await secureApiClient.put(`/api/edit/ibd_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/ibd_assessment/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[IbdAssessment] Approve error:', err); setSaveError('Approve failed.'); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== IBD ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `IBD Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* Multi-sentence OR a single labeled "Label: v…" long field: decompose (never side-by-side) */
      ...
      <div key={fn} className="rec-mini-card">
        ...
        {val.map((item, arrIdx) => {
          ...
          if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
            const phrase = searchTerm.toLowerCase().trim();
            const labelMatch = label.toLowerCase().includes(phrase) || phrase.includes(label.toLowerCase());
  ...
  if (!records || records.length === 0) {
    return (
      <div className="ibd-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">IBD Assessment</h2></div>
        <div className="empty-state">No IBD assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="ibd-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">IBD Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<IbdAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Ibd_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search IBD assessments..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`IBD Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'disease-characteristics')}
            {renderSection(record, idx, 'current-flare')}
...
export default IbdAssessmentDocument;
