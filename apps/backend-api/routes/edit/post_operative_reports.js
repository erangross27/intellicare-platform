...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'post-operative-reports-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['surgeryDate', 'procedurePerformed', 'surgicalFindings', 'complications', 'pacuArrival', 'pacuDischarge', 'vitalSignsTrend', 'painLevel', 'painManagement', 'nausea', 'urineOutput', 'drainOutput', 'oxygenRequirement', 'mobilityStatus', 'diet', 'disposition', 'dischargeInstructions', 'followUpPlan', 'prescriptions', 'activityRestrictions', 'returnPrecautions', 'recommendations'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('post_operative_reports', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[post_operative_reports] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('post_operative_reports', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[post_operative_reports] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
