...
function buildContext(req, operation = 'read') { return { serviceId: 'neurology-consultations-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['consultDate', 'referralReason', 'neurologicComplaint', 'symptomOnset', 'symptomProgression', 'neurologicHistory', 'mentalStatusExam', 'cranialNerveExam', 'motorExam', 'sensoryExam', 'reflexes', 'coordination', 'gait', 'imagingReviews', 'eegFindings', 'emgFindings', 'lpFindings', 'strokeScale', 'neurologicDiagnosis', 'treatmentPlan', 'medicationsAdjusted', 'therapyRecommendations', 'followUpPlan', 'recommendations'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('neurology_consultations', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[NeurologyConsultations] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('neurology_consultations', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[NeurologyConsultations] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
