/**
 * NtScanResultDocument.jsx
 * March 2026 — Canonical one-pass rewrite: inline editing, blue glow theme.
 * Collection: nt_scan_result
 *
 * 6 Sections:
 *   1. measurements: nuchalTranslucencyMeasurement (num), crownRumpLength (num), gestationalAgeByNt (text),
 *                    fetalHeartRate (num, hide-0), nasalBonePresence (bool), tricuspidRegurgitation (bool),
 *                    ductusVenosusFlow (text)
 *   2. risk-scores: trisomy21RiskScore, trisomy18RiskScore, trisomy13RiskScore, combinedScreenRisk (text)
 *   3. biochemistry: pappALevel (num), freeBetaHcgLevel (num)
 *   4. clinical-info: fetalNumberConfirmed (num), maternalAge (num), chorionicityDetermination,
 *                     amnionicityDetermination, imageQualityAssessment (text)
 *   5. placental: placentalLocation, uterineArteryDoppler (text)
 *   6. follow-up: followUpRecommendations (array)
 *
 * Record date: NO real `date` field (only createdAt/updatedAt) → title only, no record date pill.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NtScanResultDocumentPDFTemplate from '../pdf-templates/NtScanResultDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './NtScanResultDocument.css';
...
const DRAFT_KEY = 'nt_scan_resultPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['gestationalAgeByNt', 'ductusVenosusFlow', 'trisomy21RiskScore', 'trisomy18RiskScore', 'trisomy13RiskScore', 'combinedScreenRisk', 'chorionicityDetermination', 'amnionicityDetermination', 'placentalLocation', 'uterineArteryDoppler', 'imageQualityAssessment'];
const DATE_FIELDS = [];
...
/* ═══════ COMPONENT ═══════ */
const NtScanResultDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.nt_scan_result) return Array.isArray(r.nt_scan_result) ? r.nt_scan_result : [r.nt_scan_result];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.nt_scan_result) return Array.isArray(dd.nt_scan_result) ? dd.nt_scan_result : [dd.nt_scan_result]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `NT Scan Result ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/nt_scan_result/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/nt_scan_result/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NT SCAN RESULT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `NT Scan Result ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: STRING FIELD with splitBySentence + parseLabel decomposition ═══════ */
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* Single-value string: simple editable (suppress subtitle when it equals the section title) */
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
  ...
  /* ═══════ RENDER: ARRAY FIELD (followUpRecommendations) ═══════ */
                  ...
                  <div className="edit-field-container">
                    ...
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => {
                        ...
                        // Stage as a DRAFT (no DB write). The whole array is staged under "field-idx";
                        // Approve commits the field with its full array value. localStorage survives refresh.
                        ...
                        setLocalEdits(prev => ({ ...prev, [editKey2]: currentArr }));
  ...
  if (!records || records.length === 0) {
    return (
      <div className="nt-scan-result-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">NT Scan Result</h2></div>
        <div className="empty-state">No NT scan result records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="nt-scan-result-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">NT Scan Result</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NtScanResultDocumentPDFTemplate document={pdfData} />} fileName="NT_Scan_Result.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search NT scan result..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`NT Scan Result ${idx + 1}`)}</h3>
            </div>
            ...
            {renderSection(record, idx, 'placental')}
...
export default NtScanResultDocument;
