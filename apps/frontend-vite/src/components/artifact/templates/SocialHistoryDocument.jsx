/**
 * SocialHistoryDocument.jsx
 * July 2026 — recursive, dot-path editable social history
 * Collection: social_history
 */
...
const SECTION_TITLES = {
  record: 'Record Details', tobacco: 'Tobacco Use', alcohol: 'Alcohol Use', substanceUse: 'Substance Use',
  home: 'Home and Housing', employment: 'Employment and Education', support: 'Relationships and Support',
  lifestyle: 'Lifestyle', socialDeterminants: 'Social Determinants', sexualHealth: 'Sexual Health',
  culturalContext: 'Cultural and Service Context', clinical: 'Clinical Notes', recommendations: 'Recommendations', results: 'Results',
};
const FIELD_LABELS = {
  date: 'Date', type: 'Type', provider: 'Provider', facility: 'Facility', status: 'Status',
  smokingStatus: 'Smoking Status', smokingHistory: 'Smoking History', packYearsHistory: 'Pack-Years History', tobaccoQuitDate: 'Tobacco Quit Date', tobaccoType: 'Tobacco Type',
  alcoholUse: 'Alcohol Use', alcoholFrequency: 'Alcohol Frequency', alcoholQuantity: 'Alcohol Quantity', illicitDrugUse: 'Illicit Drug Use', substanceUse: 'Substance Use', drugTypes: 'Drug Types',
  housingStability: 'Housing Stability', homeEnvironment: 'Home Environment', livesAlone: 'Lives Alone', householdMembers: 'Household Members', livingSituation: 'Living Situation',
  employmentStatus: 'Employment Status', occupation: 'Occupation', occupationalExposures: 'Occupational Exposures', educationLevel: 'Education Level',
  maritalStatus: 'Marital Status', caregiverStatus: 'Caregiver Status', familySupport: 'Family Support', supportSystem: 'Support System',
  physicalActivityLevel: 'Physical Activity Level', exercise: 'Exercise', diet: 'Diet', sleepPatterns: 'Sleep Patterns', stressLevel: 'Stress Level',
  healthLiteracy: 'Health Literacy', transportation: 'Transportation', foodSecurity: 'Food Security', insurance: 'Insurance', financialConcerns: 'Financial Concerns', domesticViolence: 'Domestic Violence',
  sexualActivity: 'Sexual Activity', sexualOrientation: 'Sexual Orientation', contraceptionMethod: 'Contraception Method',
  religiousBeliefs: 'Religious Beliefs', culturalFactors: 'Cultural Factors', recentTravel: 'Recent Travel', militaryService: 'Military Service',
  findings: 'Findings', assessment: 'Assessment', plan: 'Plan', notes: 'Notes', recommendations: 'Recommendations', results: 'Results',
};
const SECTION_FIELDS = {
  record: ['date', 'type', 'provider', 'facility', 'status'],
  tobacco: ['smokingStatus', 'smokingHistory', 'packYearsHistory', 'tobaccoQuitDate', 'tobaccoType'],
  alcohol: ['alcoholUse', 'alcoholFrequency', 'alcoholQuantity'],
  substanceUse: ['illicitDrugUse', 'substanceUse', 'drugTypes'],
  home: ['housingStability', 'homeEnvironment', 'livesAlone', 'householdMembers', 'livingSituation'],
  employment: ['employmentStatus', 'occupation', 'occupationalExposures', 'educationLevel'],
  support: ['maritalStatus', 'caregiverStatus', 'familySupport', 'supportSystem'],
  lifestyle: ['physicalActivityLevel', 'exercise', 'diet', 'sleepPatterns', 'stressLevel'],
  socialDeterminants: ['healthLiteracy', 'transportation', 'foodSecurity', 'insurance', 'financialConcerns', 'domesticViolence'],
  sexualHealth: ['sexualActivity', 'sexualOrientation', 'contraceptionMethod'],
  culturalContext: ['religiousBeliefs', 'culturalFactors', 'recentTravel', 'militaryService'],
...
const ENUM_FIELDS = ['status', 'smokingStatus', 'alcoholFrequency', 'illicitDrugUse', 'maritalStatus', 'employmentStatus', 'caregiverStatus', 'housingStability', 'foodSecurity', 'sexualActivity', 'physicalActivityLevel', 'stressLevel'];
const NUMBER_FIELDS = [];
...
const LABELED_LIST_FIELDS = ['familySupport'];
...
const enumOptionsFor = (path, value) => {
  ...
  const options = {
    status: ['Draft', 'Pending', 'Complete', 'Approved', 'Amended'],
    smokingStatus: ['Never smoker', 'Former smoker', 'Current smoker', 'Unknown'],
    alcoholFrequency: ['Never', 'Rarely', 'Monthly', 'Weekly', 'Daily'],
    illicitDrugUse: ['Never', 'Former', 'Current', 'Unknown'],
    maritalStatus: ['Single', 'Married', 'Divorced', 'Widowed', 'Separated', 'Partnered'],
    employmentStatus: ['Employed', 'Unemployed', 'Retired', 'Disabled', 'Student'],
    caregiverStatus: ['None', 'Has caregiver', 'Is caregiver', 'Both'],
    housingStability: ['Stable', 'Unstable', 'At risk', 'Homeless'],
    foodSecurity: ['Secure', 'Insecure', 'At risk'],
    sexualActivity: ['Sexually active', 'Not sexually active', 'Unknown'],
...
const SocialHistoryDocument = ({ document: docProp }) => {
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/social_history/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/social_history/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SocialHistory] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Social History Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="social-history-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Social History</h2></div><div className="empty-state">No social history available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Social History Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="social-history-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Social History</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<SocialHistoryDocumentPDFTemplate document={pdfData} />} fileName="Social_History.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search social history..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Social History Record ' + (recordIndex + 1))}</h3></div>
