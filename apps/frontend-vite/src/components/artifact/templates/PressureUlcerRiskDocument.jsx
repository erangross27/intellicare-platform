/**
 * PressureUlcerRiskDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: pressure_ulcer_risk
 *
 * 4 Sections:
 *   1. assessment-info: date, riskCategory, repositioningFrequency, pressureRedistributionSurface, nextAssessmentDue
 *   2. braden-scale: bradenScore, sensoryPerception, skinMoisture, activity, mobilityLevel, nutrition, frictionAndShear
 *   3. ulcer-status: existingPressureUlcers, ulcerLocations, ulcerStages
 *   4. risk-factors: bodyMassIndex, albuminLevel, hemoglobinLevel, incontinenceBladder, incontinenceBowel, skinTemperature, diabetesMellitus, vascularDisease, mechanicalVentilation, vasopressorUse, edemaPresent
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PressureUlcerRiskDocumentPDFTemplate from '../pdf-templates/PressureUlcerRiskDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './PressureUlcerRiskDocument.css';
...
const FIELD_LABELS = {
  ...
  nutrition: 'Nutrition',
  frictionAndShear: 'Friction & Shear',
...
const STRING_FIELDS = ['riskCategory', 'incontinenceBladder', 'incontinenceBowel', 'skinTemperature', 'pressureRedistributionSurface', 'repositioningFrequency', 'nextAssessmentDue'];
...
const DRAFT_KEY = 'pressure_ulcer_riskPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const PressureUlcerRiskDocument = ({ document: docProp }) => {
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Draft fieldPart is "field" or "field.arrayIndex"; localEdits keys are always "field-idx" (array
     edits store the full array under the plain field key), so a dotted fieldPart maps to base field. */
  useEffect(() => {
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        ...
        else if (fmtFieldVal(f, val).toLowerCase().includes(phrase)) return true;
      }
    }
    return false;
  }, [searchTerm, getFieldValue, fmtFieldVal]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pressure Ulcer Risk Assessment ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (val && (Array.isArray(val) ? val.some(item => String(item).toLowerCase().includes(phrase)) : fmtFieldVal(f, val).toLowerCase().includes(phrase))) return true;
        }
      }
      return false;
    });
  }, [records, searchTerm, getFieldValue, fmtFieldVal]);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    // localEdits keys are always "field-idx" (array edits store the full array under the plain key),
    // so this section's staged edits are the pending keys whose base field is in this section.
    const suffix = `-${idx}`;
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/pressure_ulcer_risk/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/pressure_ulcer_risk/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (store[id]) {
        fields.forEach(f => {
          delete store[id][f];
          Object.keys(store[id]).forEach(dp => { const d = dp.lastIndexOf('.'); if (d !== -1 && /^\d+$/.test(dp.slice(d + 1)) && dp.slice(0, d) === f) delete store[id][dp]; });
        });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PRESSURE ULCER RISK ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Pressure Ulcer Risk Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    /* Single-value string: use saveSentence — single-name-skip showSubLabel guard */
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
    const isModified = editedFields[editKey];
    const showSubLabel = label.toLowerCase() !== (SECTION_TITLES[sid] || '').toLowerCase();
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(strVal); setSaveError(null); } }}>
          {isEditing ? (
            ...
            <>
              ...
              <button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(showSubLabel ? `${label}\n${strVal}` : strVal, editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button>
            </>
          )}
        </div>
        {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pressure-ulcer-risk-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pressure Ulcer Risk</h2></div>
        <div className="empty-state">No pressure ulcer risk data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pressure-ulcer-risk-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Pressure Ulcer Risk</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PressureUlcerRiskDocumentPDFTemplate document={pdfData} />} fileName={`pressure-ulcer-risk-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pressure ulcer risk..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Pressure Ulcer Risk Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'assessment-info')}
            {renderSection(record, idx, 'braden-scale')}
            {renderSection(record, idx, 'ulcer-status')}
...
export default PressureUlcerRiskDocument;
