/**
 * DevelopmentalMilestonesDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: developmental_milestones
 *
 * 8 Sections:
 *   1. session-info: date (date picker), provider, facility, status
 *   2. gross-motor: grossMotor[] — {milestone, achieved}
 *   3. fine-motor: fineMotor[] — {milestone, achieved}
 *   4. language-section: language[] — {milestone, achieved}
 *   5. social-emotional: socialEmotional[] — {milestone, achieved}
 *   6. cognitive-section: cognitive[] — {milestone, achieved}
 *   7. concerns-referrals: concerns[], referrals[]
 *   8. assessment-plan: findings (sentence), assessment (sentence), plan (sentence)
 *   9. results-section: results (OBJECT — recursive key/value)
 *  10. recommendations-section: recommendations[] — {recommendation, date}
 *  11. notes-section: notes (sentence)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import DevelopmentalMilestonesDocumentPDFTemplate from '../pdf-templates/DevelopmentalMilestonesDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import './DevelopmentalMilestonesDocument.css';
...
const DRAFT_KEY = 'developmental_milestonesPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'concerns-referrals': 'Concerns & Referrals',
...
/* humanizeKey: convert object keys to readable labels */
const KEY_OVERRIDES = {};
...
const SECTION_FIELDS = {
  'session-info': ['date', 'provider', 'facility', 'type', 'status'],
...
const MILESTONE_ARRAY_FIELDS = ['grossMotor', 'fineMotor', 'language', 'socialEmotional', 'cognitive'];
const SENTENCE_FIELDS = ['assessment', 'notes', 'findings', 'plan'];
const DATE_FIELDS = ['date'];
const OBJECT_FIELDS = ['results'];
const OBJECT_ARRAY_FIELDS = ['recommendations'];
const STRING_ARRAY_FIELDS = ['concerns', 'referrals'];
const ENUM_FIELDS = { status: ['Active', 'Complete', 'Not Active'] };

const COPY_LINE_EQ = '='.repeat(40);
const COPY_LINE_DASH = '-'.repeat(40);

/* enum helpers: seed <select> with the canonical-cased match; keep off-scale values selectable */
...
/* ═══════ COMPONENT ═══════ */
const DevelopmentalMilestonesDocument = ({ document: docProp }) => {
  ...
  /* Reverse map: field name → sectionId (for clearing the approve button on re-edit) */
  const fieldToSection = useMemo(() => {
    const m = {};
    Object.entries(SECTION_FIELDS).forEach(([sid, fields]) => { fields.forEach(f => { m[f] = sid; }); });
    return m;
  }, []);
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Draft keys are the localEdits keys minus the trailing "-<idx>"; we re-key them to the CURRENT
     render index by matching the record _id. */
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        // Mark the right "edited" tracker. Sentence/comma/rec keys are tracked in editedSentences;
        // milestone-item + simple/object/date keys in editedFields.
        const isMilestoneItem = /\.\d+\..+$/.test(fieldPart);
        if (isMilestoneItem) nFields[editKey] = 'edited';
        else { nFields[editKey] = 'edited'; nSentences[`${fieldPart}-s0`] = 'edited'; }
      });
    });
  ...
  const getMilestoneItemValue = useCallback((record, fn, idx, arrIdx, subField) => {
        ...
        if (Array.isArray(val)) {
          for (const item of val) {
            if (typeof item === 'object') {
              if (item.milestone && String(item.milestone).toLowerCase().includes(phrase)) return true;
            } else {
    ...
    if (val !== null && val !== undefined) {
      if (Array.isArray(val)) {
        return val.some(item => {
          if (typeof item === 'object') return item.milestone && String(item.milestone).toLowerCase().includes(phrase);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Developmental Milestones ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (val && Array.isArray(val)) {
            for (const item of val) {
              if (typeof item === 'object' && item.milestone && String(item.milestone).toLowerCase().includes(phrase)) return true;
    ...
    return filteredRecords.map((record, idx) => {
      ...
      Object.keys(localEdits).forEach(key => {
        ...
        if (m && parseInt(m[2]) === idx) {
          const fieldPath = m[1];
          // Handle dot-path: grossMotor.0.milestone → nested set
  ...
  /* Stage a DRAFT locally + write it to the pending-drafts localStorage store (survives refresh).
     ...
     fieldPart = the localEdits key minus the trailing "-<idx>". rootFn drives which approve button
     returns to yellow (cleared from approvedSections). */
  const stageDraft = useCallback((record, fieldPart, idx, value, rootFn) => {
    const id = safeId(record); if (!id) return;
    const editKey = `${fieldPart}-${idx}`;
    setLocalEdits(prev => ({ ...prev, [editKey]: value }));
    setPendingEdits(prev => ({ ...prev, [editKey]: true }));
    const sid = fieldToSection[rootFn];
  ...
  const handleSaveField = useCallback((record, fn, idx, _sid, _sentIdx, valueOverride, editTrackingKey) => {
    ...
    stageDraft(record, fn, idx, saveVal, fn);
    const trackKey = editTrackingKey || `${fn}-${idx}`;
    setEditedFields(prev => ({ ...prev, [trackKey]: 'edited' }));
    setEditingField(null); setEditValue('');
  }, [editValue, safeId, stageDraft]);

  const handleSaveMilestoneItem = useCallback((record, fn, idx, arrIdx, subField, value) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    const fieldPart = `${fn}.${arrIdx}.${subField}`;
    const editKey = `${fieldPart}-${idx}`;
    // Stage as a DRAFT (no DB write). Approve commits field+arrayIndex+subField to MongoDB.
    stageDraft(record, fieldPart, idx, value, fn);
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    // Build the merged object locally, then stage it as a DRAFT under `${rootField}-${idx}`.
    // Approve commits the WHOLE object (field=rootField, value=object) to MongoDB.
    setLocalEdits(prev => {
      ...
      // Persist the full merged object to the draft store so it survives refresh.
      const store = readDrafts();
      if (!store[id]) store[id] = {};
    ...
    const sid = fieldToSection[rootField];
    if (sid) setApprovedSections(prev => { const k = `${sid}-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; });
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      setSaveError(null);
      stageDraft(record, fn, idx, fullText, fn);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        // Milestone item: "field.<arrIdx>.<subField>" → field + arrayIndex + subField.
        const milestoneMatch = fieldPart.match(/^(.+)\.(\d+)\.(.+)$/);
        if (milestoneMatch) {
          await secureApiClient.put(`/api/edit/developmental_milestones/${id}/edit`, {
            field: milestoneMatch[1], value: localEdits[editKey],
            arrayIndex: parseInt(milestoneMatch[2], 10), subField: milestoneMatch[3],
          });
        } else {
          // Trailing dot-segment numeric → arrayIndex; otherwise field is the whole dotted/plain part.
          ...
          await secureApiClient.put(`/api/edit/developmental_milestones/${id}/edit`, payload);
        }
      }
      await secureApiClient.put(`/api/edit/developmental_milestones/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[DevelopmentalMilestones] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  /* milestone items — mirrors the JSX exactly: numbered milestone rows (running count),
     each followed by an Achieved group (sub-label + DASH + restart "1.") and, when present,
     an Age Achieved group. NO section title here — the caller already emitted it. */
  const buildMilestoneSectionCopyText = useCallback((record, idx, fn) => {
    const arr = record[fn];
    if (!Array.isArray(arr) || arr.length === 0) return '';
    let text = '';
    arr.forEach((item, i) => {
      const milestone = getMilestoneItemValue(record, fn, idx, i, 'milestone') ?? item.milestone;
      const achieved = getMilestoneItemValue(record, fn, idx, i, 'achieved') ?? item.achieved;
      const age = getMilestoneItemValue(record, fn, idx, i, 'ageAchieved') ?? item.ageAchieved;
      if (!milestone) return;
      text += `${i + 1}. ${milestone}\n`;
      if (achieved !== undefined && achieved !== null && achieved !== '') {
        text += `Achieved\n${COPY_LINE_DASH}\n1. ${typeof achieved === 'boolean' ? (achieved ? 'Yes' : 'No') : String(achieved)}\n`;
      }
    ...
    return text ? text + '\n' : '';
  }, [getMilestoneItemValue]);

  const buildObjectCopyLines = useCallback((value, label) => {
    const lines = [];
    ...
    if (Array.isArray(value)) {
      ...
      value.filter(v => !isEmptyDeep(v)).forEach(v => {
        if (isScalar(v)) lines.push(`${n++}. ${fmtScalar(v)}`);
        else buildObjectCopyLines(v, '').forEach(l => lines.push(l));
      });
      return lines;
    }
    if (isScalar(value)) {
      if (label) { lines.push(label); lines.push(COPY_LINE_DASH); }
      lines.push(`1. ${fmtScalar(value)}`);
      return lines;
    }
    if (label) { lines.push(label); lines.push(COPY_LINE_DASH); }
    Object.entries(value).filter(([, v]) => !isEmptyDeep(v)).forEach(([k, v]) => {
      buildObjectCopyLines(v, humanizeKey(k)).forEach(l => lines.push(l));
    });
    return lines;
  }, []);
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    fields.forEach(f => {
      if (MILESTONE_ARRAY_FIELDS.includes(f)) {
        // single-name: milestone field labels equal their section titles → no field head
        text += buildMilestoneSectionCopyText(record, idx, f);
        return;
      }
      ...
      if (OBJECT_FIELDS.includes(f)) {
        ...
        Object.entries(val).filter(([, v]) => !isEmptyDeep(v)).forEach(([k, v]) => {
          buildObjectCopyLines(v, humanizeKey(k)).forEach(l => { text += `${l}\n`; });
        });
        text += '\n';
        return;
      }
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        const items = val.filter(v => v !== null && v !== undefined && String(v).trim() !== '');
    ...
    return text;
  }, [getFieldValue, hasVal, fmtVal, formatSentenceFieldLines, buildMilestoneSectionCopyText, buildObjectCopyLines]);

  const copyAllText = useCallback(async () => {
    let text = `Developmental Milestones\n${COPY_LINE_EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Developmental Milestones ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  /* ═══════ RENDER: STRING ARRAY FIELD (concerns, referrals) — per-item editing that
     splices the ARRAY (never saves a stringified array over the field) ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections && !contentMatches(itemStr)) return null;
          return (
            <div key={itemIdx}>
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; setSaveError(null); stageDraft(record, fn, idx, currentArr, fn); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  /* ═══════ RENDER: SIMPLE EDITABLE FIELD (known fixed-choice → enum <select>) ═══════ */
  ...
  /* ═══════ RENDER: MILESTONE ARRAY SECTION ═══════ */
  const renderMilestoneArraySection = (record, idx, fn, sid) => {
    const arr = record[fn];
    ...
    return arr.map((item, arrIdx) => {
      const milestoneVal = getMilestoneItemValue(record, fn, idx, arrIdx, 'milestone') ?? item.milestone;
      const achievedVal = getMilestoneItemValue(record, fn, idx, arrIdx, 'achieved') ?? item.achieved;
      if (!milestoneVal) return null;

      // Search filtering
      if (searchTerm.trim() && !phraseMatch) {
        const phrase = searchTerm.toLowerCase().trim();
        if (!String(milestoneVal).toLowerCase().includes(phrase) && !title.toLowerCase().includes(phrase)) return null;
      }

      const milestoneEditKey = `${fn}.${arrIdx}.milestone-${idx}`;
      const achievedEditKey = `${fn}.${arrIdx}.achieved-${idx}`;
      const milestoneEditing = editingField === milestoneEditKey;
      const achievedEditing = editingField === achievedEditKey;
      const milestoneBadge = editedFields[milestoneEditKey];
      const achievedBadge = editedFields[achievedEditKey];
      const achievedDisplay = typeof achievedVal === 'boolean' ? (achievedVal ? 'Yes' : 'No') : String(achievedVal ?? '');

      return (
        <div key={arrIdx} className="rec-mini-card">
          {/* Milestone text row */}
          <div className={`numbered-row ${milestoneBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!milestoneEditing) { setEditingField(milestoneEditKey); setEditValue(String(milestoneVal)); setSaveError(null); } }}>
            {milestoneEditing ? (
              <div className="edit-field-container">
                <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                {saveError && <div className="save-error">{saveError}</div>}
                <div className="edit-actions">
                  <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveMilestoneItem(record, fn, idx, arrIdx, 'milestone', editValue); }}>{saving ? 'Saving...' : 'Save'}</button>
                  <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                </div>
              </div>
            ) : (
              <>
                <div className="row-content"><span className="content-value">{highlightText(String(milestoneVal))}</span><span className="edit-indicator">✎</span></div>
                <button className={`copy-btn ${copiedItems[milestoneEditKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${milestoneVal} — Achieved: ${achievedDisplay}`, milestoneEditKey); }}>{copiedItems[milestoneEditKey] ? 'Copied!' : 'Copy'}</button>
              </>
            )}
          </div>
          {milestoneBadge && <span className="modified-badge">edited - click Pending Approve to save</span>}

          {/* Achieved boolean select row */}
          <div className="nested-subtitle" style={{ fontSize: 14, marginTop: 4 }}>Achieved</div>
          <div className={`numbered-row ${achievedBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!achievedEditing) { setEditingField(achievedEditKey); setEditValue(typeof achievedVal === 'boolean' ? (achievedVal ? 'yes' : 'no') : String(achievedVal).toLowerCase()); setSaveError(null); } }}>
            {achievedEditing ? (
              <div className="edit-field-container">
                ...
                {saveError && <div className="save-error">{saveError}</div>}
                <div className="edit-actions">
                  <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const boolVal = editValue === 'yes'; handleSaveMilestoneItem(record, fn, idx, arrIdx, 'achieved', boolVal); }}>{saving ? 'Saving...' : 'Save'}</button>
                  <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                </div>
              </div>
            ) : (
              <>
                <div className="row-content"><span className="content-value">{highlightText(achievedDisplay)}</span><span className="edit-indicator">✎</span></div>
              </>
            )}
          </div>
          {achievedBadge && <span className="modified-badge">edited - click Pending Approve to save</span>}

          {/* Age Achieved (optional sub-field, e.g. NICU corrected age) */}
          {(() => {
            const ageVal = getMilestoneItemValue(record, fn, idx, arrIdx, 'ageAchieved') ?? item.ageAchieved;
            if (ageVal === undefined || ageVal === null || String(ageVal).trim() === '') return null;
            const ageEditKey = `${fn}.${arrIdx}.ageAchieved-${idx}`;
            const ageEditing = editingField === ageEditKey;
            const ageBadge = editedFields[ageEditKey];
            return (
              <>
                <div className="nested-subtitle" style={{ fontSize: 14, marginTop: 4 }}>Age Achieved</div>
                <div className={`numbered-row ${ageBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ageEditing) { setEditingField(ageEditKey); setEditValue(String(ageVal)); setSaveError(null); } }}>
                  {ageEditing ? (
                    <div className="edit-field-container">
                      <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                      {saveError && <div className="save-error">{saveError}</div>}
                      <div className="edit-actions">
                        <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveMilestoneItem(record, fn, idx, arrIdx, 'ageAchieved', editValue.trim()); }}>{saving ? 'Saving...' : 'Save'}</button>
                        <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                      </div>
                    </div>
                  ) : (
                    <div className="row-content"><span className="content-value">{highlightText(String(ageVal))}</span><span className="edit-indicator">✎</span></div>
                  )}
                </div>
                {ageBadge && <span className="modified-badge">edited - click Pending Approve to save</span>}
              </>
            );
          })()}
        </div>
      ...
      <div key={fn}>
        <div className="rec-mini-card">
          ...
          {sentences.map((sentence, sIdx) => {
            ...
            if (parsed.isLabeled) {
              const commaItems = splitByComma(parsed.value.replace(/[;.]+$/, '').trim());
  ...
  const renderObjectNode = (record, rootField, idx, label, value, path, depth) => {
    ...
    if (Array.isArray(value)) {
      // arrays render as rows, never Object.entries index labels ("0"/"1")
      return (
        <React.Fragment key={path.join('-') || rootField}>
          {label && <div className={labelClass}>{highlightText(label)}</div>}
          {value.map((it, i) => {
            if (isEmptyDeep(it)) return null;
            if (!isScalar(it)) return <div className="nested-mini-card" key={i}>{renderObjectNode(record, rootField, idx, '', it, [...path, String(i)], depth + 1)}</div>;
            return renderObjectLeaf(record, rootField, [...path, String(i)], idx, it);
          })}
        </React.Fragment>
      );
    }
                        ...
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              setSaveError(null);
                              stageDraft(record, fn, idx, newArr, fn);
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      if (MILESTONE_ARRAY_FIELDS.includes(f)) return Array.isArray(record[f]) && record[f].length > 0;
      return hasVal(val);
    });
    ...
    return (
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            if (MILESTONE_ARRAY_FIELDS.includes(f)) return renderMilestoneArraySection(record, idx, f, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="developmental-milestones-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Developmental Milestones</h2></div>
        <div className="empty-state">No developmental milestones records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="developmental-milestones-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Developmental Milestones</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<DevelopmentalMilestonesDocumentPDFTemplate document={pdfData} />} fileName="Developmental_Milestones.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search developmental milestones..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Developmental Milestones ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'gross-motor')}
            {renderSection(record, idx, 'fine-motor')}
            {renderSection(record, idx, 'language-section')}
            {renderSection(record, idx, 'social-emotional')}
            {renderSection(record, idx, 'cognitive-section')}
            {renderSection(record, idx, 'concerns-referrals')}
...
export default DevelopmentalMilestonesDocument;
