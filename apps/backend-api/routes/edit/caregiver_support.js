...
function buildContext(req, operation = 'read') { return { serviceId: 'caregiver-support-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
...
const ALLOWED_FIELDS = [
  ...
  'caregiverHealthConcerns', 'sleepDisruptionFrequency', 'socialIsolationLevel',
  'supportGroupParticipation', 'respiteCareUtilization', 'caregiverCounselingReferral',
  'caregiverTrainingProvided', 'caregiverCopingStrategies',
  'homeModificationsNeeded', 'assistiveDevicesUsed',
  'medicalDecisionMakingRole', 'emergencyBackupPlan', 'careTransitionConcerns',
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('caregiver_support', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[CaregiverSupport] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('caregiver_support', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[CaregiverSupport] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
