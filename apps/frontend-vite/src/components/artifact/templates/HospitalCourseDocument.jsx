/**
 * HospitalCourseDocument.jsx
 * July 2026 — Canonical one-pass rewrite (blue-glow inline editing, box-free PDF parity).
 * Collection: hospital_course
 *
 * 4 Sections:
 *   1. admission     : admissionDate (date), dischargeDate (date), lengthOfStay (number), admittingDiagnosis (string)
 *   2. course        : hospitalCourse (narrative), dischargeDiagnosis (array), consultations (array), complications (array)
 *   3. discharge     : condition, disposition, dischargeInstructions, dietRestrictions, dietaryInstructions,
 *                      activityRestrictions, woundCare (strings), equipment (array)
 *   4. documentation : provider, facility (strings), findings, assessment, plan (narratives), results (object),
 *                      recommendations (array), notes, status, electronicSignature (strings)
 *
 * NOTE: the record's `date` field is an INGESTION timestamp — it is NEVER surfaced. Record dates key off
 * admissionDate/dischargeDate only. There is no record-date pill.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HospitalCourseDocumentPDFTemplate from '../pdf-templates/HospitalCourseDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './HospitalCourseDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'hospital_coursePendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  admission: 'Admission & Timeline',
  course: 'Hospital Course & Diagnoses',
  discharge: 'Discharge Status & Instructions',
  documentation: 'Documentation & Sign-Off',
};
...
const FIELD_LABELS = {
  ...
  dischargeDiagnosis: 'Discharge Diagnoses',
  ...
  dietaryInstructions: 'Dietary Instructions',
...
const SECTION_FIELDS = {
  admission: ['admissionDate', 'dischargeDate', 'lengthOfStay', 'admittingDiagnosis'],
  course: ['hospitalCourse', 'dischargeDiagnosis', 'consultations', 'complications'],
  discharge: ['condition', 'disposition', 'dischargeInstructions', 'dietRestrictions', 'dietaryInstructions', 'activityRestrictions', 'woundCare', 'equipment'],
  documentation: ['provider', 'facility', 'findings', 'assessment', 'plan', 'results', 'recommendations', 'notes', 'status', 'electronicSignature'],
};

const NUMBER_FIELDS = ['lengthOfStay'];
const DATE_FIELDS = ['admissionDate', 'dischargeDate'];
const ARRAY_FIELDS = ['dischargeDiagnosis', 'consultations', 'complications', 'equipment', 'recommendations'];
const OBJECT_FIELDS = ['results'];
const STRING_FIELDS = ['admittingDiagnosis', 'hospitalCourse', 'condition', 'disposition', 'dischargeInstructions', 'dietRestrictions', 'dietaryInstructions', 'activityRestrictions', 'woundCare', 'provider', 'facility', 'findings', 'assessment', 'plan', 'notes', 'status', 'electronicSignature'];

/* ═══════ OBJECT-FIELD HELPERS (for the recursive `results` renderer) ═══════ */
...
/* ═══════ COMPONENT ═══════ */
const HospitalCourseDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.hospital_course) return Array.isArray(r.hospital_course) ? r.hospital_course : [r.hospital_course];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.hospital_course) return Array.isArray(dd.hospital_course) ? dd.hospital_course : [dd.hospital_course]; return [dd]; }
      if (r?.data) { const dd = r.data; if (Array.isArray(dd)) return dd; if (dd?.hospital_course) return Array.isArray(dd.hospital_course) ? dd.hospital_course : [dd.hospital_course]; return [dd]; }
      return [r];
    });
  ...
  const fieldSearchText = useCallback((val) => {
    if (val === null || val === undefined) return '';
    if (Array.isArray(val)) return val.map(x => (isScalar(x) ? fmtScalar(x) : objectCopyLines('', x, 0).join(' '))).join(' ');
    if (typeof val === 'object') return objectCopyLines('', val, 0).join(' ');
    return fmtVal(val);
  }, [fmtVal]);
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (fieldSearchText(val).toLowerCase().includes(phrase)) return true;
      }
    }
    return false;
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) {
      return fieldSearchText(val).toLowerCase().includes(phrase);
    }
    return false;
  }, [searchTerm, getFieldValue, fieldSearchText]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Hospital Course ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (val && fieldSearchText(val).toLowerCase().includes(phrase)) return true;
        }
      }
      return false;
    });
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/hospital_course/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/hospital_course/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        text += `${label}\n`;
        val.filter(x => !isEmptyDeep(x)).forEach((item, i) => { text += `${i + 1}. ${isScalar(item) ? fmtScalar(item) : objectCopyLines('', item, 0).join('; ')}\n`; });
        text += '\n';
  ...
  /* ═══════ RENDER: NUMBER FIELD — num-stepper (NEVER type=number) ═══════ */
  ...
  /* ═══════ RENDER: ARRAY FIELD (read-only numbered items) ═══════ */
  const renderArrayField = (record, fn, idx, sid) => {
    ...
    return (
      <div key={fn} className="rec-mini-card">
        <div className="nested-subtitle">{highlightText(label)}</div>
        {items.map((item, iIdx) => {
          const display = isScalar(item) ? fmtScalar(item) : objectCopyLines('', item, 0).join('; ');
          const itemKey = `${fn}-${idx}-${iIdx}`;
          return (
  ...
  /* ═══════ RENDER: OBJECT FIELD (read-only recursive, e.g. results) ═══════ */
  const renderObjectValue = (value, keyPrefix) => {
    if (isScalar(value)) {
      const display = fmtScalar(value);
      return (
        <div className="numbered-row" key={keyPrefix}>
          <div className="row-content"><span className="content-value">{highlightText(display)}</span></div>
          <button className={`copy-btn ${copiedItems[keyPrefix] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(display, keyPrefix); }}>{copiedItems[keyPrefix] ? 'Copied!' : 'Copy'}</button>
        </div>
      );
    }
    const entries = Object.entries(value).filter(([, v]) => !isEmptyDeep(v));
    return entries.map(([k, v]) => (
      <div key={`${keyPrefix}.${k}`} className="rec-mini-card">
        <div className="nested-subtitle">{highlightText(humanizeKey(k))}</div>
        {renderObjectValue(v, `${keyPrefix}.${k}`)}
      </div>
    ));
  };
  ...
  if (!records || records.length === 0) {
    return (
      <div className="hospital-course-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Hospital Course</h2></div>
        <div className="empty-state">No hospital course records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="hospital-course-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Hospital Course</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HospitalCourseDocumentPDFTemplate document={pdfData} />} fileName="Hospital_Course.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search hospital course records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Hospital Course ${idx + 1}`)}</h3>
            </div>
...
export default HospitalCourseDocument;
