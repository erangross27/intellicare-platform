...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'proposed-art-switch-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = ['currentAntiretroviralRegimen', 'proposedAntiretroviralRegimen', 'dosingFrequency', 'pillBurdenCurrent', 'pillBurdenProposed', 'reasonForSwitch', 'treatmentHistory', 'adverseEffectsReported', 'comorbiditiesRelevant', 'cd4Count', 'cd4Percentage', 'viralLoad', 'viralSuppressionStatus', 'resistanceTestingResults', 'tropismResult', 'hlaB5701Status', 'creatinineClearance', 'hepaticFunction', 'hepatitisBStatus', 'hepatitisCStatus', 'drugInteractions', 'concomitantMedications', 'adherenceScore', 'pregnancyStatus'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('proposed_art_switch', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[proposed_art_switch] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('proposed_art_switch', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[proposed_art_switch] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
