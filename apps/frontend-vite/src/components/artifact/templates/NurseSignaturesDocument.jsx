/**
 * NurseSignaturesDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: nurse_signatures (displayed as "Nurse Signatures")
 *
 * 7 Sections:
 *   1. nurse-info: nurseName, nursingLicenseNumber, nursingCredentials, nursingUnitLocation
 *   2. shift-details: shiftStartTime, shiftEndTime, chartingCompletionTime, nurseToPatientsRatio
 *   3. verifications: medicationAdministrationVerification, patientSafetyChecksCompleted, vitalSignsDocumented,
 *                     painAssessmentCompleted, handoffCommunicationCompleted, infectionControlMeasures, patientEducationProvided
 *   4. interventions: nursingInterventionsCompleted (string array)
 *   5. scores: brackenScaleScore, morseScaleFallRisk, glasgowComaScale
 *   6. supervisor-witness: nursingSupervisorName, witnessSignatureRequired, witnessNurseName
 *   7. incidents: criticalValueNotification, incidentReportFiled
 *
 * NO record `date` field (only createdAt/updatedAt ingestion timestamps) → record header is title-only.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NurseSignaturesDocumentPDFTemplate from '../pdf-templates/NurseSignaturesDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './NurseSignaturesDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'nurse-info': 'Nurse Information',
  'shift-details': 'Shift Details',
  'verifications': 'Verifications',
  'interventions': 'Nursing Interventions',
  'scores': 'Clinical Scores',
  'supervisor-witness': 'Supervisor & Witness',
  'incidents': 'Incidents & Notifications',
};

const FIELD_LABELS = {
  nurseName: 'Nurse Name',
  nursingLicenseNumber: 'Nursing License Number',
  nursingCredentials: 'Nursing Credentials',
  nursingUnitLocation: 'Nursing Unit Location',
  shiftStartTime: 'Shift Start Time',
  shiftEndTime: 'Shift End Time',
  chartingCompletionTime: 'Charting Completion Time',
  nurseToPatientsRatio: 'Nurse to Patients Ratio',
  medicationAdministrationVerification: 'Medication Administration Verification',
  patientSafetyChecksCompleted: 'Patient Safety Checks Completed',
  vitalSignsDocumented: 'Vital Signs Documented',
  painAssessmentCompleted: 'Pain Assessment Completed',
  handoffCommunicationCompleted: 'Handoff Communication Completed',
  infectionControlMeasures: 'Infection Control Measures',
  patientEducationProvided: 'Patient Education Provided',
  nursingInterventionsCompleted: 'Nursing Interventions Completed',
  brackenScaleScore: 'Braden Scale Score',
  morseScaleFallRisk: 'Morse Scale Fall Risk',
  glasgowComaScale: 'Glasgow Coma Scale',
  nursingSupervisorName: 'Nursing Supervisor Name',
  witnessSignatureRequired: 'Witness Signature Required',
  witnessNurseName: 'Witness Nurse Name',
  criticalValueNotification: 'Critical Value Notification',
  incidentReportFiled: 'Incident Report Filed',
};

const SECTION_FIELDS = {
  'nurse-info': ['nurseName', 'nursingLicenseNumber', 'nursingCredentials', 'nursingUnitLocation'],
  'shift-details': ['shiftStartTime', 'shiftEndTime', 'chartingCompletionTime', 'nurseToPatientsRatio'],
  'verifications': ['medicationAdministrationVerification', 'patientSafetyChecksCompleted', 'vitalSignsDocumented', 'painAssessmentCompleted', 'handoffCommunicationCompleted', 'infectionControlMeasures', 'patientEducationProvided'],
  'interventions': ['nursingInterventionsCompleted'],
  'scores': ['brackenScaleScore', 'morseScaleFallRisk', 'glasgowComaScale'],
  'supervisor-witness': ['nursingSupervisorName', 'witnessSignatureRequired', 'witnessNurseName'],
  'incidents': ['criticalValueNotification', 'incidentReportFiled'],
};

const NUMBER_FIELDS = ['nurseToPatientsRatio', 'brackenScaleScore', 'morseScaleFallRisk', 'glasgowComaScale'];
const BOOLEAN_FIELDS = ['medicationAdministrationVerification', 'patientSafetyChecksCompleted', 'vitalSignsDocumented', 'painAssessmentCompleted', 'handoffCommunicationCompleted', 'infectionControlMeasures', 'patientEducationProvided', 'witnessSignatureRequired', 'criticalValueNotification', 'incidentReportFiled'];
const STRING_FIELDS = ['nurseName', 'nursingLicenseNumber', 'nursingCredentials', 'nursingUnitLocation', 'shiftStartTime', 'shiftEndTime', 'chartingCompletionTime', 'nursingSupervisorName', 'witnessNurseName'];

/* Arrays (string). */
const ARRAY_NUMBER_FIELDS = [];
const ARRAY_STRING_FIELDS = ['nursingInterventionsCompleted'];
const ALL_ARRAY_FIELDS = [...ARRAY_NUMBER_FIELDS, ...ARRAY_STRING_FIELDS];

/* Sentinel-zero: a nurse-to-patients ratio of 0 is never real — it's the extractor's unset default, so it is
   HIDDEN everywhere (JSX render + section-visibility + copy + PDF). Only this field hides its zero; a clinical
   score (Braden/Morse/GCS) of 0 is a legitimate value and is NOT hidden. Memory 6a4fa368. */
const HIDE_ZERO_FIELDS = ['nurseToPatientsRatio'];
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldName]: value } } — value is the full field value (string, number, boolean, array). */
const DRAFT_KEY = 'nurse_signaturesPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const NurseSignaturesDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.nurse_signatures) return Array.isArray(r.nurse_signatures) ? r.nurse_signatures : [r.nurse_signatures];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.nurse_signatures) return Array.isArray(dd.nurse_signatures) ? dd.nurse_signatures : [dd.nurse_signatures]; return [dd]; }
      return [r];
    });
  ...
  /* hasFieldVal: hasVal + sentinel-zero hide (a 0-valued ratio is the extractor's unset default) */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Nurse Signature ${idx + 1}`.toLowerCase();
  ...
  const handleSaveArrayItem = useCallback((record, fn, idx, arrayIndex, isNumber, sid) => {
    const id = safeId(record); if (!id) return;
    const saveVal = isNumber ? parseFloat(editValue) : editValue;
    if (isNumber && (isNaN(saveVal) || editValue.trim() === '')) { setSaveError('Please enter a valid number'); return; }
    setSaveError(null);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const field = m[1];
        await secureApiClient.put(`/api/edit/nurse_signatures/${id}/edit`, { field, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/nurse_signatures/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (ALL_ARRAY_FIELDS.includes(f)) {
        const items = Array.isArray(val) ? val.filter(v => v !== null && v !== undefined && v !== '') : [];
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NURSE SIGNATURES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Nurse Signature ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: BOOLEAN FIELD — BlueSelect Yes/No (RTL-safe custom dropdown; never a native selector) ═══════ */
  ...
  /* ═══════ RENDER: ARRAY FIELD (strings) ═══════ */
  const renderArrayField = (record, fn, idx, sid) => {
    const arr = getEffectiveArray(record, fn, idx);
    if (arr.length === 0) return null;
    const label = FIELD_LABELS[fn] || fn;
    const isNumArr = ARRAY_NUMBER_FIELDS.includes(fn);
    const showSubLabel = label.trim().toLowerCase() !== (SECTION_TITLES[sid] || '').trim().toLowerCase();
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
    const phraseMatch = !searchTerm.trim() || sectionTitleMatches(sid) || record._showAllSections;
                ...
                {isEditing ? (
                  <div className="edit-field-container">
                    {isNumArr ? (
                      <div className="num-stepper-row">
                        <button type="button" className="num-step" disabled={saving} onClick={e => { e.stopPropagation(); const step = stepFor(editValue); const cur = parseFloat(editValue); setEditValue(String(Math.max(0, Math.round(((isNaN(cur) ? 0 : cur) - step) * 1e6) / 1e6))); }}>&minus;</button>
                        <input type="text" inputMode="decimal" className="edit-number" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter') { e.stopPropagation(); handleSaveArrayItem(record, fn, idx, ai, true, sid); } }} />
                        <button type="button" className="num-step" disabled={saving} onClick={e => { e.stopPropagation(); const step = stepFor(editValue); const cur = parseFloat(editValue); setEditValue(String(Math.max(0, Math.round(((isNaN(cur) ? 0 : cur) + step) * 1e6) / 1e6))); }}>+</button>
                      </div>
                    ) : (
                      <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); handleSaveArrayItem(record, fn, idx, ai, isNumArr, sid); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      if (ALL_ARRAY_FIELDS.includes(f)) return getEffectiveArray(record, f, idx).length > 0;
      if (BOOLEAN_FIELDS.includes(f)) return hasVal(val);
    ...
    return (
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            if (ALL_ARRAY_FIELDS.includes(f)) return renderArrayField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="nurse-signatures-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Nurse Signatures</h2></div>
        <div className="empty-state">No nurse signatures records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="nurse-signatures-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Nurse Signatures</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NurseSignaturesDocumentPDFTemplate document={pdfData} />} fileName="Nurse_Signatures.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search nurse signatures..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Nurse Signature ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'nurse-info')}
            {renderSection(record, idx, 'shift-details')}
            {renderSection(record, idx, 'verifications')}
            {renderSection(record, idx, 'interventions')}
            {renderSection(record, idx, 'scores')}
            {renderSection(record, idx, 'supervisor-witness')}
            {renderSection(record, idx, 'incidents')}
          </div>
        ))}
      </div>
    </div>
...
export default NurseSignaturesDocument;
