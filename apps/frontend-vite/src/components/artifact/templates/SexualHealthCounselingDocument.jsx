 ...
 * July 2026 — recursive, dot-path editable sexual health counseling
...
const SECTION_FIELDS = {
  sessionInfo: ['counselingSessionType', 'chiefSexualHealthConcern', 'motivationalInterviewingStage', 'nextCounselingSessionInterval'],
  patientIdentity: ['sexualOrientationIdentity', 'genderIdentity'],
  sexualHistory: ['sexualPartnerCount12Months', 'condomUseConsistency', 'substanceUseWithSexualActivity', 'sexualTraumaHistory', 'intimatePartnerViolenceScreenResult'],
  stiPrep: ['stiScreeningHistory', 'prepCandidacyStatus', 'prepAdherencePercentage'],
  ...
  goalsReferrals: ['sexualHealthGoals', 'referralToSpecialist'],
};
const DATE_FIELDS = [];
const DATETIME_FIELDS = [];
const ENUM_FIELDS = ['motivationalInterviewingStage', 'condomUseConsistency', 'intimatePartnerViolenceScreenResult', 'fertilityPreservationStatus'];
...
const enumOptionsFor = (path, value) => {
  ...
  const options = field === 'motivationalInterviewingStage' ? ['Precontemplation', 'Contemplation', 'Preparation', 'Action', 'Maintenance']
    : field === 'condomUseConsistency' ? ['Never', 'Rarely', 'Sometimes', 'Usually', 'Always']
      : field === 'intimatePartnerViolenceScreenResult' ? ['Negative', 'Positive', 'Not screened']
        : field === 'fertilityPreservationStatus' ? ['Not discussed', 'Discussed', 'Planned', 'Completed', 'Declined'] : [];
...
const SexualHealthCounselingDocument = ({ document: docProp }) => {
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/sexual_health_counseling/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/sexual_health_counseling/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SexualHealthCounseling] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Sexual Health Counseling Record ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="sexual-health-counseling-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Sexual Health Counseling</h2></div><div className="empty-state">No sexual health counseling available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Sexual Health Counseling Record ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="sexual-health-counseling-document" ref={containerRef}>
    <div className="document-header">
      ...
      <div className="header-actions">
        ...
        <PDFDownloadLink document={<SexualHealthCounselingDocumentPDFTemplate document={pdfData} />} fileName="Sexual_Health_Counseling.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search sexual health counseling..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Sexual Health Counseling Record ' + (recordIndex + 1))}</h3></div>
