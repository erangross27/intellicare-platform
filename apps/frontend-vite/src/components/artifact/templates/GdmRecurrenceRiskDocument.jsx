/**
 * GdmRecurrenceRiskDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: gdm_recurrence_risk
 *
 * 4 Sections:
 *   1. recurrence-score: gdmRecurrenceScore, previousGdmGestationalAge, previousGdmTreatment, previousMacrosomia
 *   2. glucose-status: hba1cLevel, fastingGlucoseLevel, postpartumGlucoseTolerance, oralGlucoseToleranceResults, insulinResistanceMarkers
 *   3. risk-factors: familyHistoryDiabetes, polycysticOvarySyndrome, ethnicRiskFactor, maternalAge, previousStillbirth, chronicHypertension, thyroidDysfunction
 *   4. weight-metrics: pregestationalBmi, interpregnancyWeightGain, interpregnancyInterval, metabolicSyndromeComponents
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import GdmRecurrenceRiskDocumentPDFTemplate from '../pdf-templates/GdmRecurrenceRiskDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './GdmRecurrenceRiskDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'recurrence-score': 'Recurrence Score',
  'glucose-status': 'Glucose Status',
  'risk-factors': 'Risk Factors',
  'weight-metrics': 'Weight Metrics',
};

const FIELD_LABELS = {
  gdmRecurrenceScore: 'GDM Recurrence Score',
  previousGdmGestationalAge: 'Previous GDM Gestational Age',
  previousGdmTreatment: 'Previous GDM Treatment',
  previousMacrosomia: 'Previous Macrosomia',
  hba1cLevel: 'HbA1c Level',
  fastingGlucoseLevel: 'Fasting Glucose Level',
  postpartumGlucoseTolerance: 'Postpartum Glucose Tolerance',
  oralGlucoseToleranceResults: 'Oral Glucose Tolerance Results',
  ...
  ethnicRiskFactor: 'Ethnic Risk Factor',
  maternalAge: 'Maternal Age',
  previousStillbirth: 'Previous Stillbirth',
  chronicHypertension: 'Chronic Hypertension',
  thyroidDysfunction: 'Thyroid Dysfunction',
  pregestationalBmi: 'Pregestational BMI',
  interpregnancyWeightGain: 'Interpregnancy Weight Gain',
  interpregnancyInterval: 'Interpregnancy Interval',
  metabolicSyndromeComponents: 'Metabolic Syndrome Components',
};
...
/* Fields where a literal 0 is a missing-data sentinel (clinically impossible), so hide it.
   interpregnancyWeightGain (0 = no weight gained) and metabolicSyndromeComponents (0 = none) keep 0. */
...
const numericPresent = (fn, v) => {
  if (typeof v === 'number' && v === 0 && ZERO_SENTINEL_FIELDS.includes(fn)) return false;
...
const STRING_FIELDS = ['previousGdmTreatment', 'postpartumGlucoseTolerance', 'insulinResistanceMarkers', 'ethnicRiskFactor', 'thyroidDysfunction'];
const COPY_LINE_EQ = '='.repeat(40);
const COPY_LINE_DASH = '-'.repeat(40);
const stepFor = (v) => { const d = String(v ?? '').split('.')[1]; return d ? parseFloat('0.' + '0'.repeat(Math.max(0, d.length - 1)) + '1') : 1; };

/* OGTT timepoint labels — array is positionally meaningful (fasting / 1h / 2h / 3h glucose) */
...
const DRAFT_KEY = 'gdm_recurrence_riskPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const GdmRecurrenceRiskDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.gdm_recurrence_risk) return Array.isArray(r.gdm_recurrence_risk) ? r.gdm_recurrence_risk : [r.gdm_recurrence_risk];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.gdm_recurrence_risk) return Array.isArray(dd.gdm_recurrence_risk) ? dd.gdm_recurrence_risk : [dd.gdm_recurrence_risk]; return [dd]; }
      return [r];
    });
    return arr.filter(r => r && typeof r === 'object');
  }, [docProp]);

  /* Resolve a record's id (string / $oid) without depending on safeId (declared below). */
  const draftIdOf = (r) => { if (!r?._id) return null; if (typeof r._id === 'string') return r._id; if (r._id.$oid) return r._id.$oid; return String(r._id); };

  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Draft fieldPart is "field" (scalar) or "field.arrayIndex" (one array element). */
  useEffect(() => {
    const store = readDrafts();
    if (!store || Object.keys(store).length === 0) return;
    const nLocal = {}, nPending = {}, nFields = {};
    const arrAccum = {}; // `${fn}-${idx}` -> array being rebuilt from element drafts
    records.forEach((record, idx) => {
      const id = draftIdOf(record);
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayElem) {
          ...
          arrAccum[localKey][arrIndex] = value;
    ...
    Object.entries(arrAccum).forEach(([localKey, arr]) => { nLocal[localKey] = arr; });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `GDM Recurrence Risk ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  /* Stage a DRAFT to the pending-drafts localStorage store (survives refresh). fieldPart is
     "field" (scalar) or "field.arrayIndex" (one array element). NO DB write here — Approve commits. */
  const stageDraft = useCallback((record, fieldPart, value) => {
    const id = draftIdOf(record); if (!id) return;
    const store = readDrafts();
  ...
  }, [editValue, stageDraft, clearApprovedForField]);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Commit each staged draft for this record whose base field belongs to this section.
      const store = readDrafts();
      ...
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/gdm_recurrence_risk/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedKeys.push(`${baseField}-${idx}`);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/gdm_recurrence_risk/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (store2[id]) {
        Object.keys(store2[id]).forEach(fp => {
          const di = fp.indexOf('.');
          const base = (di !== -1 && /^\d+$/.test(fp.slice(di + 1))) ? fp.slice(0, di) : fp;
          if (fields.includes(base)) delete store2[id][fp];
        });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (BOOLEAN_FIELDS.includes(f)) {
        ...
        if (!numericPresent(f, val)) return;
        ...
        body += head;
        let n = 1;
        present.forEach(({ item, i }) => { body += `${n++}. ${ogttLabel(i)} - ${item}\n`; });
        body += '\n';
      } else {
        body += head;
  ...
  const copyAllText = useCallback(async () => {
    let text = `GDM Recurrence Risk\n${COPY_LINE_EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `GDM Recurrence Risk ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx); if (!numericPresent(fn, val)) return null;
  ...
  /* ═══════ RENDER: ARRAY FIELD — OGTT numeric, position-significant (original index preserved) ═══════ */
  const renderArrayField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    const arr = Array.isArray(val) ? val : [];
    /* keep ORIGINAL index; only suppress null/undefined/'' timepoints (not meaningful 0) */
    const present = arr.map((item, i) => ({ item, i })).filter(({ item }) => item !== null && item !== undefined && item !== '');
    ...
    return (
      <div key={fn} className="rec-mini-card">
        {showSubLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
        {present.map(({ item, i: itemIdx }) => {
          const editKey = `${fn}.${itemIdx}-${idx}`;
          const isEditing = editingField === editKey;
          const isModified = editedFields[editKey];
          const itemStr = String(item);
          const tpLabel = ogttLabel(itemIdx);

          if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
            const phrase = searchTerm.toLowerCase().trim();
            const labelLower = label.toLowerCase();
            if (!labelLower.includes(phrase) && !phrase.includes(labelLower) && !itemStr.toLowerCase().includes(phrase) && !tpLabel.toLowerCase().includes(phrase)) return null;
          }

          // Stage an array-element DRAFT (no DB write). Draft fieldPart "fn.itemIdx" → Approve PUTs with arrayIndex.
          const commitArrayEdit = () => {
            const id = draftIdOf(record); if (!id) return;
            if (editValue.trim() === '') { setSaveError('Please enter a valid number'); return; }
            ...
            currentArr[itemIdx] = numVal;
            setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: currentArr }));
            setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true }));
            setEditedFields(prev => ({ ...prev, [editKey]: 'edited' }));
            clearApprovedForField(fn, idx);
            stageDraft(record, `${fn}.${itemIdx}`, numVal);
            setEditingField(null); setEditValue('');
          };

          return (
            <div key={itemIdx}>
              <div className="nested-subtitle" style={{ fontWeight: 400, opacity: 0.8 }}>{highlightText(tpLabel)}</div>
                ...
                {isEditing ? (
                  <div className="edit-field-container">
                    <div className="num-stepper-row">
                      ...
                      <input type="text" inputMode="decimal" className="edit-number" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter') { e.stopPropagation(); commitArrayEdit(); } }} />
                      <button type="button" className="num-step" disabled={saving} onClick={e => { e.stopPropagation(); const c = parseFloat(editValue); const b = isNaN(c) ? 0 : c; setEditValue(String(Math.max(0, Math.round((b + stepFor(editValue)) * 1e6) / 1e6))); }}>+</button>
                    </div>
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); commitArrayEdit(); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <>
                    ...
                    <button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${tpLabel}: ${itemStr}`, editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button>
                  </>
                )}
              </div>
              {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); setSaveError(null); setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullText })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); clearApprovedForField(fn, idx); stageDraft(record, fn, fullText); setEditingField(null); setEditValue(''); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="gdm-recurrence-risk-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">GDM Recurrence Risk</h2></div>
        <div className="empty-state">No GDM recurrence risk records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="gdm-recurrence-risk-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">GDM Recurrence Risk</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<GdmRecurrenceRiskDocumentPDFTemplate document={pdfData} />} fileName="Gdm_Recurrence_Risk.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search GDM recurrence risk records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`GDM Recurrence Risk ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'recurrence-score')}
            {renderSection(record, idx, 'glucose-status')}
            {renderSection(record, idx, 'risk-factors')}
            {renderSection(record, idx, 'weight-metrics')}
          </div>
        ))}
      </div>
    </div>
...
export default GdmRecurrenceRiskDocument;
