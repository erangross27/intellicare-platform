 ...
 *
 * NOTE: singular STRING field `recommendation` is distinct from the ARRAY field `recommendations`.
 */
...
const FIELD_LABELS = {
  date: 'Date', recommendation: 'Admission Recommendation', patientPreference: 'Patient Preference',
  alternativePlan: 'Alternative Plan', type: 'Type', provider: 'Provider', facility: 'Facility',
...
const AdmissionDecisionsDocumentPDFTemplate = ({ document: data }) => {
  let records = [];
  if (Array.isArray(data)) {
    if (data.length === 1 && data[0]?.admission_decisions) records = Array.isArray(data[0].admission_decisions) ? data[0].admission_decisions : [data[0].admission_decisions];
    else records = data;
  } else if (data?.admission_decisions) records = Array.isArray(data.admission_decisions) ? data.admission_decisions : [data.admission_decisions];
  else if (data?.documentData) { const dd = data.documentData; if (Array.isArray(dd)) records = dd; else if (dd?.admission_decisions) records = Array.isArray(dd.admission_decisions) ? dd.admission_decisions : [dd.admission_decisions]; else if (dd && typeof dd === 'object') records = [dd]; }
  else if (data && typeof data === 'object') records = [data];
  records = (records || []).filter(r => r && typeof r === 'object');

  if (records.length === 0) {
    return (<Document><Page size="A4" style={styles.page}><View style={styles.documentHeader}><Text style={styles.title}>Admission Decision</Text></View><Text style={styles.emptyState}>No records available</Text></Page></Document>);
  }

  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.title}>Admission Decision</Text></View>
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer}>
            <View style={styles.recordHeader} wrap={false}>
              <Text style={styles.recordTitle}>{`Admission Decision ${String(record._recordNumber || idx + 1)}`}</Text>
...
export default AdmissionDecisionsDocumentPDFTemplate;
