...
const COLLECTION = 'biologic_therapy_records';
const COMMA_SPLIT_FIELDS = [];
const OBJECT_FIELDS = new Set(['priorTherapies', 'baselineDiseaseAssessment', 'biologicAdministrationPlan', 'firstDoseAdministration', 'responseAssessment', 'adverseEvents', 'safetyMonitoring', 'insuranceAuthorization', 'treatmentPlan', 'switchingBiologics']);
const NARRATIVE_FIELDS = new Set(['indication', 'mechanismOfAction']);
const DATE_FIELDS = new Set(['date']);
const KEY_OVERRIDES = { tbTesting: 'TB Testing', cbcBaseline: 'CBC Baseline', lftsBaseline: 'LFTs Baseline' };
const LABELS = {
  date: 'Date', biologicAgent: 'Biologic Agent', indication: 'Indication',
  mechanismOfAction: 'Mechanism of Action', priorTherapies: 'Prior Therapies',
  baselineDiseaseAssessment: 'Baseline Disease Assessment', biologicAdministrationPlan: 'Biologic Administration Plan',
  firstDoseAdministration: 'First Dose Administration', responseAssessment: 'Response Assessment',
  adverseEvents: 'Adverse Events', safetyMonitoring: 'Safety Monitoring',
  insuranceAuthorization: 'Insurance Authorization', treatmentPlan: 'Treatment Plan',
  switchingBiologics: 'Switching Biologics',
};
const SECTIONS = [
  { title: 'Date', fields: ['date'] },
  { title: 'Therapy Overview', fields: ['biologicAgent', 'indication', 'mechanismOfAction'] },
  { title: 'Prior Therapies', fields: ['priorTherapies'] },
  { title: 'Baseline Disease Assessment', fields: ['baselineDiseaseAssessment'] },
  { title: 'Biologic Administration Plan', fields: ['biologicAdministrationPlan'] },
  { title: 'First Dose Administration', fields: ['firstDoseAdministration'] },
  { title: 'Response Assessment', fields: ['responseAssessment'] },
  { title: 'Adverse Events', fields: ['adverseEvents'] },
  { title: 'Safety Monitoring', fields: ['safetyMonitoring'] },
  { title: 'Insurance Authorization', fields: ['insuranceAuthorization'] },
  { title: 'Treatment Plan', fields: ['treatmentPlan'] },
  { title: 'Switching Biologics', fields: ['switchingBiologics'] },
];
...
const BiologicTherapyRecordsDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Biologic Therapy Records</Text><Text style={styles.noData}>No biologic therapy records data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Biologic Therapy Records</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Biologic Therapy Records {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
