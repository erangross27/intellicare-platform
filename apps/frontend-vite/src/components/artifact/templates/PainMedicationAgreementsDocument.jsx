/**
 * PainMedicationAgreementsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: pain_medication_agreements
 *
 * 8 Sections:
 *   1. provider-info: prescribingPhysicianName, prescribingPhysicianNpi
 *   2. agreement-dates: agreementEffectiveDate, agreementExpirationDate, patientSignatureDate, witnessName
 *   3. medication-details: primaryOpioidMedication, opioidDosageAmount, morphineEquivalentDose, primaryPainDiagnosis
 *   4. additional-medications: additionalPrescribedMedications
 *   5. monitoring-compliance: urineScreeningFrequency, lastUrineScreenDate, designatedPharmacy
 *   6. prohibitions-acknowledgments: earlyRefillProhibition, multipleProviderProhibition, pdmpReviewAcknowledged, safeMedicationStorageAcknowledged, drivingImpairmentWarningAcknowledged, pregnancyRiskAcknowledged, naloxonePrescribed
 *   7. emergency-violations: emergencyContactName, emergencyContactPhone, agreementViolationConsequences
 *   8. non-pharmacologic: nonPharmacologicTherapiesDiscussed
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PainMedicationAgreementsDocumentPDFTemplate from '../pdf-templates/PainMedicationAgreementsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PainMedicationAgreementsDocument.css';
...
const DRAFT_KEY = 'pain_medication_agreementsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['prescribingPhysicianName', 'prescribingPhysicianNpi', 'primaryOpioidMedication', 'opioidDosageAmount', 'primaryPainDiagnosis', 'urineScreeningFrequency', 'designatedPharmacy', 'emergencyContactName', 'emergencyContactPhone', 'agreementViolationConsequences', 'witnessName'];
...
/* splitByComma: parenthesis-aware comma split (only split on comma followed by whitespace) */
...
/* ======= COMPONENT ======= */
const PainMedicationAgreementsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.pain_medication_agreements) return Array.isArray(r.pain_medication_agreements) ? r.pain_medication_agreements : [r.pain_medication_agreements];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.pain_medication_agreements) return Array.isArray(dd.pain_medication_agreements) ? dd.pain_medication_agreements : [dd.pain_medication_agreements]; return [dd]; }
      return [r];
    });
  ...
  const isEpochDate = useCallback((v) => { if (!v) return false; const s = typeof v === 'string' ? v : (v && v.$date) ? v.$date : ''; return typeof s === 'string' && s.startsWith('1970-01-01'); }, []);
  const numberHasVal = useCallback((v) => { const n = Number(v); return Number.isFinite(n) && n !== 0; }, []);
  const dateHasVal = useCallback((v) => hasVal(v) && !isEpochDate(v), [hasVal, isEpochDate]);
  // per-field presence: number fields hide 0, date fields hide epoch, else generic hasVal
  const valHasVal = useCallback((f, v) => { if (NUMBER_FIELDS.includes(f)) return numberHasVal(v); if (DATE_FIELDS.includes(f)) return dateHasVal(v); return hasVal(v); }, [numberHasVal, dateHasVal, hasVal]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pain Medication Agreement ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      // localEdits keys are "<field>-<idx>" (whole-field/whole-array value; no dot-numeric arrayIndex suffix here).
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/pain_medication_agreements/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/pain_medication_agreements/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    return text;
  }, [getFieldValue, valHasVal, fmtVal, splitBySentence, formatSentenceFieldLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== PAIN MEDICATION AGREEMENTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Pain Medication Agreement ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ======= RENDER: RECORD DATE (header BlueDatePicker) ======= */
  const renderRecordDate = (record, idx) => {
    const val = getFieldValue(record, 'date', idx); if (!dateHasVal(val)) return null;
        ...
        {isEditing ? (
          <span className="edit-field-container" onClick={e => e.stopPropagation()}>
            ...
            <span className="edit-actions">
              <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (isNaN(new Date(editValue).getTime())) { setSaveError('Please enter a valid date'); return; } handleSaveField(record, 'date', idx, 'header', null, editValue + 'T00:00:00.000Z'); }}>{saving ? 'Saving...' : 'Save'}</button>
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx); if (!numberHasVal(val)) return null;
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
    const label = FIELD_LABELS[fn] || fn;
    const showLabel = label.toLowerCase() !== (SECTION_TITLES[sid] || '').toLowerCase();
  ...
  const renderDateField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx); if (!dateHasVal(val)) return null;
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
    const label = FIELD_LABELS[fn] || fn;
    const showLabel = label.toLowerCase() !== (SECTION_TITLES[sid] || '').toLowerCase();
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); stageFieldDraft(record, fn, idx, sid, currentArr); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => valHasVal(f, getFieldValue(record, f, idx)));
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pain-medication-agreements-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pain Medication Agreements</h2></div>
        <div className="empty-state">No pain medication agreements available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pain-medication-agreements-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Pain Medication Agreements</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PainMedicationAgreementsDocumentPDFTemplate document={pdfData} />} fileName="Pain_Medication_Agreements.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pain medication agreements..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              {dateHasVal(record.date) && (
                <div className="record-meta-row">
                  {renderRecordDate(record, idx)}
                </div>
              )}
              <h3 className="record-name">{highlightText(record.prescribingPhysicianName || `Pain Medication Agreement ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'provider-info')}
            {renderSection(record, idx, 'agreement-dates')}
            {renderSection(record, idx, 'medication-details')}
            {renderSection(record, idx, 'additional-medications')}
            {renderSection(record, idx, 'monitoring-compliance')}
            {renderSection(record, idx, 'prohibitions-acknowledgments')}
            {renderSection(record, idx, 'emergency-violations')}
            {renderSection(record, idx, 'non-pharmacologic')}
          </div>
        ))}
      </div>
    </div>
...
export default PainMedicationAgreementsDocument;
