/**
 * PreeclampsiaMonitoringDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: preeclampsia_monitoring
 *
 * 5 Sections:
 *   1. vitals: systolicBloodPressure, diastolicBloodPressure, gestationalAge
 *   2. lab-results: proteinuria24Hour, proteinCreatinineRatio, dipstickProteinuria, serumCreatinine, plateletCount, altLevel, astLevel, ldhLevel, serumAlbumin, uricAcidLevel
 *   3. fetal-assessment: fetalWeightPercentile, umbilicalArteryDoppler, middleCerebralArteryDoppler, amnioticFluidVolume
 *   4. symptoms: visualDisturbances, severeHeadache, epigastricPain
 *   5. clinical-status: preeclampsiaWithSevereFeatures, hellpSyndrome, magnesiumSulfateAdministration, antihypertensiveMedication
 */
...
const DRAFT_KEY = 'preeclampsia_monitoringPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const PreeclampsiaMonitoringDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.preeclampsia_monitoring) return Array.isArray(r.preeclampsia_monitoring) ? r.preeclampsia_monitoring : [r.preeclampsia_monitoring];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.preeclampsia_monitoring) return Array.isArray(dd.preeclampsia_monitoring) ? dd.preeclampsia_monitoring : [dd.preeclampsia_monitoring]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Preeclampsia Monitoring ${idx + 1}`.toLowerCase();
  ...
  function stageDraft(id, fn, sid, idx, fullText) {
    const editKey = `${fn}-${idx}`;
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      stageDraft(id, fn, sid, idx, fullText);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // staged edits for this section's fields (editKey = "field-idx")
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/preeclampsia_monitoring/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/preeclampsia_monitoring/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    ...
    pdfData.forEach((r, idx) => {
      text += `Preeclampsia Monitoring ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="preeclampsia-monitoring-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Preeclampsia Monitoring</h2></div>
        <div className="empty-state">No preeclampsia monitoring records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="preeclampsia-monitoring-document" ref={containerRef}>
      ...
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search preeclampsia monitoring..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            ...
            <div className="record-header">
              {hasVal(record.gestationalAge) && (
                <div className="record-meta-row">
                  <span className="record-date">GA: {record.gestationalAge} weeks</span>
                </div>
              )}
              <h3 className="record-name">{highlightText(`Preeclampsia Monitoring ${idx + 1}`)}</h3>
            </div>
...
export default PreeclampsiaMonitoringDocument;
