/**
 * Occupational Exposure Records Edit Route
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'occupational-exposure-records-edit-service',
    userId: req.user?.id,
    operation,
...
const ALLOWED_FIELDS = [
  ...
  'sourcePatientInfectionStatus',
  'chemicalAgentName',
  'sharpsDeviceType',
  'employeeJobTitle',
  'departmentOfExposure',
  'personalProtectiveEquipmentUsed',
  'immediateDecontaminationPerformed',
  'baselineSerologyOrdered',
  'postExposureProphylaxisInitiated',
  'pepMedicationRegimen',
  'hepatitisBVaccinationStatus',
  'hbigAdministered',
  'followUpSchedule',
  'oshaRecordableInjury',
  'workersCompensationClaim',
  'supervisorNotified',
  'infectionControlNotified',
  'riskAssessmentScore',
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('occupational_exposure_records', { _id: objectId }, { $set: setFields, $addToSet: { 'doctorEdits.editedFields': field } }, context);
    return res.json({ success: true, id, editedField: field });
  } catch (err) {
    console.error(`[occupational_exposure_records] PUT /:id/edit error:`, err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess();
    const context = buildContext(req, 'write');
    await sda.update('occupational_exposure_records', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, id, status: 'approved' });
  } catch (err) {
    console.error(`[occupational_exposure_records] PUT /:id/approve error:`, err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
