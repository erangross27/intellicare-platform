...
/**
 * MaternalLabsDocumentPDFTemplate - box-free canonical (LETTER)
 * Config-driven from the JSX SECTION_ORDER/SECTION_TITLES/FIELD_LABELS/SECTION_FIELDS.
 * Renders EVERY populated field the JSX renders (incl. nested objects, arrays-of-objects, dates)
 * for JSX/PDF field parity. No boxes: underline rules only
 * (documentTitle 2 / recordTitle+sectionTitle 1 / fieldLabel 0.5).
 *   glucoseScreening / results = nested objects -> recursive objectRows.
...
const SECTION_ORDER = ['session-info', 'blood-type-antibodies', 'infectious-disease', 'additional-screening', 'glucose-screening', 'lab-results', 'recommendations', 'clinical-notes'];
...
const MaternalLabsDocumentPDFTemplate = ({ document: docProp, data = docProp }) => {
  let records = [];
  if (Array.isArray(data)) {
    if (data.length === 1 && data[0]?.maternal_labs) records = Array.isArray(data[0].maternal_labs) ? data[0].maternal_labs : [data[0].maternal_labs];
    else records = data;
  } else if (data?.maternal_labs) records = Array.isArray(data.maternal_labs) ? data.maternal_labs : [data.maternal_labs];
  else if (data?.documentData) { const dd = data.documentData; records = Array.isArray(dd) ? dd : (dd?.maternal_labs ? (Array.isArray(dd.maternal_labs) ? dd.maternal_labs : [dd.maternal_labs]) : [dd]); }
    ...
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <Text style={styles.documentTitle}>Maternal Labs</Text>
          <Text style={styles.noData}>No maternal labs records available.</Text>
        </Page>
      </Document>
    );
  }
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, rIdx) => (
          <View key={rIdx}>
            <Text style={styles.recordTitle} break={rIdx > 0}>{safeString(`Maternal Labs ${rIdx + 1}`)}</Text>
...
export default MaternalLabsDocumentPDFTemplate;
