...
class ZeroKnowledgeAuthService {
  ...
  getServiceContext(practiceId = 'global', operation = 'database-access') {
    return {
      serviceId: 'zero-knowledge-auth-service',
      operation: operation,
  ...
  async generatePasswordResetToken(userId, email) {
    ...
    const context = this.getServiceContext('global', 'generatePasswordResetToken');
    
    const proxy = getServiceProxy();
    const SecureDataAccess = proxy.getService('secureDataAccess');
    await SecureDataAccess.create('password_reset_tokens', resetData, context);
  ...
  async verifyPasswordResetToken(token) {
    ...
    const context = this.getServiceContext('global', 'verifyPasswordResetToken');
    
    const proxy = getServiceProxy();
    const SecureDataAccess = proxy.getService('secureDataAccess');
    const resetData = await SecureDataAccess.query('password_reset_tokens', {
      hashedToken,
      used: false,
