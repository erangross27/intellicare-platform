/**
 * NursingAssessmentsDocument.jsx
 * March 2026 — Canonical one-pass rewrite with inline editing, blue glow theme
 * Collection: nursing_assessments
 *
 * 5 Sections:
 *   1. vital-signs-scores: vitalSigns (array of objects → each leaf label-above-value),
 *                          painScoreNumeric (number), glasgowComaScale (number),
 *                          bradenScale (number, hide-zero), fallRiskScore (number, hide-zero)
 *   2. system-assessments: skinIntegrityAssessment, woundDescription, mentalStatusExam,
 *                          respiratoryAssessment, cardiovascularAssessment,
 *                          gastrointestinalAssessment, genitourinaryAssessment (long text)
 *   3. functional-nutritional: functionalStatusAssessment (long), nutritionalScreening (string),
 *                              medicationCompliance (boolean → BlueSelect Yes/No)
 *   4. access-therapy: intravenousAccess (long), oxygenTherapy (long)
 *   5. infection-education: infectionControlMeasures (array of strings → list),
 *                           patientEducationProvided (long), dischargePlanning (long)
 *
 * Record date: the record has ONLY createdAt/updatedAt (ingestion timestamps) and no real `date`
 * field → NO record date is rendered (title only, like the Hematology gold standard).
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NursingAssessmentsDocumentPDFTemplate from '../pdf-templates/NursingAssessmentsDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './NursingAssessmentsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'nursing_assessmentsPendingEdits';
const readDrafts = () => {
...
/* Number fields — num-stepper (−/+); bradenScale/fallRiskScore are hide-zero via hasVal */
const NUMBER_FIELDS = ['painScoreNumeric', 'glasgowComaScale', 'bradenScale', 'fallRiskScore'];

const STRING_FIELDS = [
  'skinIntegrityAssessment', 'woundDescription', 'mentalStatusExam',
  'respiratoryAssessment', 'cardiovascularAssessment', 'gastrointestinalAssessment',
  'genitourinaryAssessment', 'functionalStatusAssessment', 'nutritionalScreening',
  'intravenousAccess', 'oxygenTherapy', 'patientEducationProvided', 'dischargePlanning',
];

const BOOLEAN_FIELDS = ['medicationCompliance'];
const ARRAY_FIELDS = ['infectionControlMeasures'];
const OBJECT_ARRAY_FIELDS = ['vitalSigns']; // array of vital-sign objects → each leaf label-above-value
...
/* splitByComma: parenthesis-aware comma split (thousands-guard: only split on ", " not "1,000") */
...
/* ═══════ COMPONENT ═══════ */
const NursingAssessmentsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.nursing_assessments) return Array.isArray(r.nursing_assessments) ? r.nursing_assessments : [r.nursing_assessments];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.nursing_assessments) return Array.isArray(dd.nursing_assessments) ? dd.nursing_assessments : [dd.nursing_assessments]; return [dd]; }
      return [r];
    });
  ...
  /* ═══════ UTILS ═══════ */
  // hide-zero: a numeric 0 (bradenScale/fallRiskScore "not set") is treated as EMPTY → skipped.
  ...
  /* value → searchable string (arrays + vital-sign objects flattened) */
  const valueSearchable = useCallback((val) => {
    if (val === null || val === undefined) return '';
    if (Array.isArray(val)) {
      return val.map(item => (item && typeof item === 'object') ? Object.values(item).map(v => String(v)).join(' ') : fmtVal(item)).join(' ');
    }
    return fmtVal(val);
  }, [fmtVal]);
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (valueSearchable(val).toLowerCase().includes(phrase)) return true;
      }
    }
    return false;
  }, [searchTerm, getFieldValue, valueSearchable]);
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) {
      return valueSearchable(val).toLowerCase().includes(phrase);
    }
    return false;
  }, [searchTerm, getFieldValue, valueSearchable]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Nursing Assessment ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (val !== null && val !== undefined && valueSearchable(val).toLowerCase().includes(phrase)) return true;
        }
      }
      return false;
    });
  }, [records, searchTerm, getFieldValue, valueSearchable]);
  ...
  // Save a nested vital-sign leaf: merge into the whole vitalSigns array (staged under the base field).
  const saveVitalSign = useCallback((record, fn, idx, sid, vsIdx, k, itemKey) => {
    const id = safeId(record); if (!id) return;
    const curr = getFieldValue(record, fn, idx);
    const arr = (Array.isArray(curr) ? curr : (curr ? [curr] : [])).map(o => (o && typeof o === 'object') ? { ...o } : o);
    if (arr[vsIdx] && typeof arr[vsIdx] === 'object') arr[vsIdx] = { ...arr[vsIdx], [k]: editValue };
    const editKey = `${fn}-${idx}`;
  ...
  // Stage a single edited/removed sentence as a DRAFT (no DB write). localStorage keeps it across refresh.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/nursing_assessments/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/nursing_assessments/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_ARRAY_FIELDS.includes(f)) {
        ...
        text += `${label}\n`;
        arr.forEach((vs, i) => {
          if (arr.length > 1) text += `  Set ${i + 1}\n`;
          Object.entries(vs).filter(([, v]) => v !== null && v !== undefined && v !== '').forEach(([k, v]) => {
            const vLabel = VITAL_SIGN_LABELS[k] || k;
            text += `${vLabel}\n${String(v)}\n`;
          });
        });
        text += '\n';
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NURSING ASSESSMENTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Nursing Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: VITAL SIGNS — array of objects, each leaf label-above-value ═══════ */
  const renderVitalSignsField = (record, fn, idx, sid) => {
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {arr.map((vs, vsIdx) => {
          const entries = Object.entries(vs).filter(([, v]) => v !== null && v !== undefined && v !== '');
          if (entries.length === 0) return null;
          return (
            <div key={vsIdx} className="rec-mini-card" style={{ marginTop: 8 }}>
              {arr.length > 1 && <div className="vital-set-label">{highlightText(`Set ${vsIdx + 1}`)}</div>}
              {entries.map(([k, v]) => {
                const vLabel = VITAL_SIGN_LABELS[k] || k;
                const vStr = String(v);
                const itemKey = `${fn}.${vsIdx}.${k}-${idx}`;
                const isEditing = editingField === itemKey;
                const badge = editedFields[itemKey];
                const itemMatches = phraseMatch || labelMatch || !searchTerm.trim() || vLabel.toLowerCase().includes(phrase) || vStr.toLowerCase().includes(phrase);
                if (!itemMatches && searchTerm.trim()) return null;
                return (
                  <div key={k} className="rec-mini-card" style={{ marginTop: 6 }}>
                    <div className="nested-subtitle">{highlightText(vLabel)}</div>
                    <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(vStr); setSaveError(null); } }}>
                      {isEditing ? (
                        <div className="edit-field-container">
                          <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                          {saveError && <div className="save-error">{saveError}</div>}
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveVitalSign(record, fn, idx, sid, vsIdx, k, itemKey); }}>{saving ? 'Saving...' : 'Save'}</button>
                            <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                          </div>
                        </div>
                      ) : (
                        <>
                          <div className="row-content"><span className="content-value">{highlightText(vStr)}</span><span className="edit-indicator">&#9998;</span></div>
                          <button className={`copy-btn ${copiedItems[itemKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${vLabel}\n${vStr}`, itemKey); }}>{copiedItems[itemKey] ? 'Copied!' : 'Copy'}</button>
                        </>
                      )}
                    </div>
  ...
  /* ═══════ RENDER: NUMBER FIELD — num-stepper (−/+) + parseFloat+isNaN→block save ═══════ */
  ...
  /* ═══════ RENDER: ARRAY FIELD (list of strings) ═══════ */
              ...
              <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const currentArr = Array.isArray(getFieldValue(record, fn, idx)) ? [...getFieldValue(record, fn, idx)] : []; currentArr[aIdx] = editValue; setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: currentArr })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); setEditedFields(prev => ({ ...prev, [itemKey]: 'edited' })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const store = readDrafts(); if (!store[id2]) store[id2] = {}; store[id2][fn] = currentArr; writeDrafts(store); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      ...
      return BOOLEAN_FIELDS.includes(f) ? (v !== null && v !== undefined && v !== '') : hasVal(v);
    });
    ...
    return (
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            if (OBJECT_ARRAY_FIELDS.includes(f)) return renderVitalSignsField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="nursing-assessments-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Nursing Assessments</h2></div>
        <div className="empty-state">No nursing assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="nursing-assessments-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Nursing Assessments</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NursingAssessmentsDocumentPDFTemplate document={pdfData} />} fileName="Nursing_Assessments.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search nursing assessments..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Nursing Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'vital-signs-scores')}
            {renderSection(record, idx, 'system-assessments')}
            {renderSection(record, idx, 'functional-nutritional')}
            {renderSection(record, idx, 'access-therapy')}
            {renderSection(record, idx, 'infection-education')}
          </div>
        ))}
      </div>
    </div>
...
export default NursingAssessmentsDocument;
