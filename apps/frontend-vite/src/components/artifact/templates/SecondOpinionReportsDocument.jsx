 ...
 * July 2026 — recursive, dot-path editable second opinion reports
...
const SECTION_TITLES = {
  ...
  clinicalMeasures: 'Clinical Measures',
  imagingPathology: 'Imaging & Pathology',
  recommendationsPlan: 'Recommendations & Plan',
};
const FIELD_LABELS = {
...
const SECTION_FIELDS = {
  ...
  clinicalMeasures: ['glasgowComaScale', 'performanceStatusEcog', 'estimatedGlomerularFiltrationRate', 'ankleBrachialIndex', 'ejectionFractionPercent', 'biomarkerLevels'],
  imagingPathology: ['radiologicalModalityUsed', 'contrastAgentAdministered', 'lesionSizeMeasurements', 'immunohistochemistryResults', 'geneticMutationProfile'],
  recommendationsPlan: ['therapeuticRegimenProposed', 'contraindicationsIdentified', 'prognosticIndicators', 'consultingSpecialtyService', 'clinicalTrialEligibility', 'multidisciplinaryTeamInput', 'previousTreatmentResponse'],
};
const DATE_FIELDS = [];
const ENUM_FIELDS = [];
...
const COMMA_FIELDS = ['therapeuticRegimenProposed', 'multidisciplinaryTeamInput'];
...
const SEMICOLON_FIELDS = ['lesionSizeMeasurements', 'therapeuticRegimenProposed', 'multidisciplinaryTeamInput'];
...
const splitFieldValue = (field, value) => {
  ...
  const portableValue = String(value ?? '').replace(/≤/g, '<=').replace(/≥/g, '>=');
  if (fieldIn(PARENTHETICAL_LABEL_FIELDS, field)) {
    const match = portableValue.match(/^(.+?)\s*\(([A-Za-z][A-Za-z ]+):\s*([^)]+)\)\s*(.*)$/);
    if (match) return [match[1].trim(), match[2].trim() + ': ' + match[3].trim(), match[4].trim()].filter(Boolean);
  }
  const firstPass = fieldIn(SEMICOLON_FIELDS, field) || portableValue.includes('. ')
    ? splitBySentence(portableValue)
    : [portableValue.trim()].filter(Boolean);
...
const SecondOpinionReportsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.second_opinion_reports) return Array.isArray(record.second_opinion_reports) ? record.second_opinion_reports : [record.second_opinion_reports];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.second_opinion_reports) return Array.isArray(nested.second_opinion_reports) ? nested.second_opinion_reports : [nested.second_opinion_reports];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/second_opinion_reports/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/second_opinion_reports/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SecondOpinionReports] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    ...
    records.forEach((record, recordIndex) => {
      text += 'Second Opinion Report ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="second-opinion-reports-document" ref={containerRef}><div className="document-header"><h2 className="document-title">Second Opinion Reports</h2></div><div className="empty-state">No second opinion reports records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['Second Opinion Report ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="second-opinion-reports-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">Second Opinion Reports</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<SecondOpinionReportsDocumentPDFTemplate document={pdfData} />} fileName="Second_Opinion_Reports.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search second opinion reports..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('Second Opinion Report ' + (recordIndex + 1))}</h3></div>
