/**
 * CaregiverSupportGroupsDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: caregiver_support_groups
 *
 * 5 Sections:
 *   1. caregiver-burden-scores: caregiverBurdenInventoryScore, caregiverDepressionScale, caregiverStressScale, caregiverSleepQuality, caregiverPhysicalHealthStatus
 *   2. care-demands: hoursOfCarePerWeek, dailyLivingActivitiesScore, medicationManagementComplexity, functionalDeclineRate, cognitiveImpairmentSeverity, behavioralDisturbancesPresent
 *   3. patient-relationship: patientDiagnosisPrimary, caregiverRelationshipToPatient, socialSupportNetwork, financialStrainLevel
 *   4. support-planning: groupSessionAttendance, respiteCareUtilization, emergencyPreparedness, careTransitionPlanning
 *   5. education-coping: caregiverEducationNeeds, copingStrategiesUsed
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import CaregiverSupportGroupsDocumentPDFTemplate from '../pdf-templates/CaregiverSupportGroupsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './CaregiverSupportGroupsDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'caregiver-burden-scores': 'Caregiver Burden Scores',
  'care-demands': 'Care Demands',
  'patient-relationship': 'Patient & Relationship',
  'support-planning': 'Support & Planning',
  'education-coping': 'Education & Coping',
};

const FIELD_LABELS = {
  caregiverBurdenInventoryScore: 'Caregiver Burden Inventory Score',
  caregiverDepressionScale: 'Depression Scale',
  caregiverStressScale: 'Stress Scale',
  caregiverSleepQuality: 'Caregiver Sleep Quality',
  caregiverPhysicalHealthStatus: 'Caregiver Physical Health Status',
  hoursOfCarePerWeek: 'Hours of Care/Week',
  dailyLivingActivitiesScore: 'ADL Score',
  medicationManagementComplexity: 'Medication Management Complexity',
  functionalDeclineRate: 'Functional Decline Rate',
  cognitiveImpairmentSeverity: 'Cognitive Impairment Severity',
  behavioralDisturbancesPresent: 'Behavioral Disturbances Present',
  patientDiagnosisPrimary: 'Patient Primary Diagnosis',
  caregiverRelationshipToPatient: 'Caregiver Relationship to Patient',
  socialSupportNetwork: 'Social Support Network',
  financialStrainLevel: 'Financial Strain Level',
  groupSessionAttendance: 'Group Session Attendance',
  respiteCareUtilization: 'Respite Care Utilization',
  emergencyPreparedness: 'Emergency Preparedness',
  careTransitionPlanning: 'Care Transition Planning',
  caregiverEducationNeeds: 'Caregiver Education Needs',
  copingStrategiesUsed: 'Coping Strategies Used',
};

const SECTION_FIELDS = {
  'caregiver-burden-scores': ['caregiverBurdenInventoryScore', 'caregiverDepressionScale', 'caregiverStressScale', 'caregiverSleepQuality', 'caregiverPhysicalHealthStatus'],
  'care-demands': ['hoursOfCarePerWeek', 'dailyLivingActivitiesScore', 'medicationManagementComplexity', 'functionalDeclineRate', 'cognitiveImpairmentSeverity', 'behavioralDisturbancesPresent'],
  'patient-relationship': ['patientDiagnosisPrimary', 'caregiverRelationshipToPatient', 'socialSupportNetwork', 'financialStrainLevel'],
  'support-planning': ['groupSessionAttendance', 'respiteCareUtilization', 'emergencyPreparedness', 'careTransitionPlanning'],
  'education-coping': ['caregiverEducationNeeds', 'copingStrategiesUsed'],
};

const NUMBER_FIELDS = ['caregiverBurdenInventoryScore', 'dailyLivingActivitiesScore', 'caregiverDepressionScale', 'hoursOfCarePerWeek', 'caregiverStressScale', 'medicationManagementComplexity', 'groupSessionAttendance'];
const ARRAY_FIELDS = ['caregiverEducationNeeds', 'copingStrategiesUsed'];
const BOOLEAN_FIELDS = ['behavioralDisturbancesPresent', 'respiteCareUtilization', 'emergencyPreparedness', 'careTransitionPlanning'];
const STRING_FIELDS = ['patientDiagnosisPrimary', 'caregiverRelationshipToPatient', 'cognitiveImpairmentSeverity', 'caregiverPhysicalHealthStatus', 'socialSupportNetwork', 'financialStrainLevel', 'caregiverSleepQuality', 'functionalDeclineRate'];
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field"; value may be a string or array) */
const DRAFT_KEY = 'caregiver_support_groupsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const CaregiverSupportGroupsDocument = ({ document: docProp }) => {
  ...
  /* safeId at module-usable form for the rehydrate effect (record _id, handling _id.$oid). */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Caregiver Support Groups ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/caregiver_support_groups/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/caregiver_support_groups/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const formatSentenceFieldLines = useCallback((text) => {
    ...
    sentences.forEach(s => {
      ...
      if (parsed.isLabeled) {
        ...
        parts.forEach((item, gi) => { lines.push(`  ${gi + 1}. ${item}`); });
      } else { lines.push(`${++plainN}. ${s}`); }
    });
    return lines;
  }, [splitBySentence]);
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      const labelLine = label.trim().toLowerCase() !== (title || '').trim().toLowerCase() ? `${label}\n${COPY_LINE_DASH}\n` : '';
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== CAREGIVER SUPPORT GROUPS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Caregiver Support Groups ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              {(() => {
                ...
                return (
                  <div className="num-stepper-row">
                    ...
                    <input type="number" step={stepFor(val)} className="edit-number" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} onClick={e => e.stopPropagation()} disabled={saving} />
                    <button type="button" className="num-step" onClick={e => { e.stopPropagation(); stepNum(1); }} disabled={saving}>+</button>
                  </div>
                );
              })()}
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(parsed.isLabeled ? parsed.value : itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const saveVal = parsed.isLabeled ? `${parsed.label}: ${editValue}` : editValue; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = saveVal; stageDraft(id2, fn, idx, sid, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="caregiver-support-groups-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Caregiver Support Groups</h2></div>
        <div className="empty-state">No caregiver support groups data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="caregiver-support-groups-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Caregiver Support Groups</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<CaregiverSupportGroupsDocumentPDFTemplate document={pdfData} />} fileName={`caregiver-support-groups-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search caregiver support groups..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Caregiver Support Groups ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'caregiver-burden-scores')}
            {renderSection(record, idx, 'care-demands')}
            {renderSection(record, idx, 'patient-relationship')}
            {renderSection(record, idx, 'support-planning')}
            {renderSection(record, idx, 'education-coping')}
          </div>
        ))}
      </div>
    </div>
...
export default CaregiverSupportGroupsDocument;
