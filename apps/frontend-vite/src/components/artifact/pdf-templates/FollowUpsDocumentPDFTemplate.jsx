/**
 * FollowUpsDocumentPDFTemplate.jsx
 ...
 * numbered comma items). safeString scrubs non-Latin-1 glyphs Helvetica can't draw. Rule #74: each field is ONE
 * wrap={false} atomic View with the sectionTitle riding INSIDE the first present field's View. Static PHI footer.
 * Collection: follow_ups. urgency/reportType are free-text (high-variance across records), not enums.
 */
...
const SECTION_ORDER = ['report-info', 'clinical-info', 'recommendations-section', 'follow-up-section'];
...
const FIELD_LABELS = {
  ...
  clinicalIndication: 'Clinical Indication', findings: 'Findings',
  recommendations: 'Recommendations', followUp: 'Follow-Up Required',
};
...
const FollowUpsDocumentPDFTemplate = ({ document: docProp, data }) => {
  const templateData = docProp || data;
  let records = [];
  if (Array.isArray(templateData)) {
    if (templateData.length > 0 && templateData[0].follow_ups && Array.isArray(templateData[0].follow_ups)) records = templateData[0].follow_ups;
    else records = templateData;
  } else if (templateData && templateData.follow_ups) {
    records = Array.isArray(templateData.follow_ups) ? templateData.follow_ups : [templateData.follow_ups];
  } else if (templateData && templateData.documentData) {
    const dd = templateData.documentData;
    records = Array.isArray(dd) ? dd : (dd.follow_ups ? (Array.isArray(dd.follow_ups) ? dd.follow_ups : [dd.follow_ups]) : [dd]);
  ...
  if (!records || records.length === 0) {
    return (
      <Document><Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.documentTitle}>Follow-Ups</Text></View>
        <Text style={styles.emptyState}>No follow-ups available.</Text>
      </Page></Document>
    );
  }
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer} break={idx > 0}>
            <Text style={styles.recordTitle}>{`Follow-Up ${idx + 1}`}</Text>
...
export default FollowUpsDocumentPDFTemplate;
