...
const COLLECTION = 'cardiology_followup_reports';
const COMMA_SPLIT_FIELDS = ['ecgInterpretation'];
const ARRAY_FIELDS = new Set(['edemaLocation', 'currentMedications']);
const OBJECT_FIELDS = new Set(['lipidPanel', 'deviceInterrogation']);
const NARRATIVE_FIELDS = new Set(['primaryDiagnosis', 'heartRhythm', 'chestPainCharacteristics', 'dyspneaSeverity', 'medicationAdherence', 'anticoagulationTherapy', 'ecgInterpretation']);
const DATE_FIELDS = new Set(['date']);
const ZERO_SENTINEL_FIELDS = new Set(['heartRate', 'bodyWeight', 'ejectionFraction', 'bnpLevel', 'troponinLevel', 'inrValue']);
const KEY_OVERRIDES = { LDL: 'LDL', HDL: 'HDL' };
const LABELS = {
  date: 'Date', provider: 'Provider', facility: 'Facility', primaryDiagnosis: 'Primary Diagnosis', nyhaClass: 'NYHA Class', ccsFunctionalClass: 'CCS Functional Class',
  bloodPressure: 'Blood Pressure', heartRate: 'Heart Rate', heartRhythm: 'Heart Rhythm', bodyWeight: 'Body Weight',
  chestPainCharacteristics: 'Chest Pain Characteristics', dyspneaSeverity: 'Dyspnea Severity', edemaLocation: 'Edema Location',
  ejectionFraction: 'Ejection Fraction', bnpLevel: 'BNP Level', troponinLevel: 'Troponin Level', lipidPanel: 'Lipid Panel',
  currentMedications: 'Current Medications', medicationAdherence: 'Medication Adherence', anticoagulationTherapy: 'Anticoagulation Therapy', inrValue: 'INR Value',
  deviceType: 'Device Type', deviceInterrogation: 'Device Interrogation', ecgInterpretation: 'ECG Interpretation', nextFollowupInterval: 'Next Follow-Up Interval',
};
const SECTIONS = [
  { title: 'Record Information', fields: ['date', 'provider', 'facility', 'primaryDiagnosis'] },
  { title: 'Functional Class', fields: ['nyhaClass', 'ccsFunctionalClass'] },
  { title: 'Vitals', fields: ['bloodPressure', 'heartRate', 'heartRhythm', 'bodyWeight'] },
  { title: 'Symptoms', fields: ['chestPainCharacteristics', 'dyspneaSeverity', 'edemaLocation'] },
  { title: 'Cardiac Markers', fields: ['ejectionFraction', 'bnpLevel', 'troponinLevel'] },
  { title: 'Lipid Panel', fields: ['lipidPanel'] },
  { title: 'Medications', fields: ['currentMedications', 'medicationAdherence'] },
  { title: 'Anticoagulation', fields: ['anticoagulationTherapy', 'inrValue'] },
  { title: 'Device', fields: ['deviceType', 'deviceInterrogation'] },
  { title: 'ECG Interpretation', fields: ['ecgInterpretation'] },
  { title: 'Follow-Up', fields: ['nextFollowupInterval'] },
];
...
const CardiologyFollowupReportsDocumentPDFTemplate = ({ document: documentProp, data, templateData }) => {
  const records = unwrapRecords(documentProp || data || templateData);
  if (!records.length) return <Document><Page size="A4" style={styles.page}><Text style={styles.documentTitle}>Cardiology Followup Reports</Text><Text style={styles.noData}>No cardiology followup reports data available</Text></Page></Document>;
  return <Document><Page size="A4" style={styles.page} wrap><Text style={styles.documentTitle}>Cardiology Followup Reports</Text>{records.map((record, index) => <React.Fragment key={record._id?.$oid || String(record._id || index)}><View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>Cardiology Followup Reports {index + 1}</Text></View>{SECTIONS.map((section, sectionIndex) => renderSection(record, section, sectionIndex))}</React.Fragment>)}</Page></Document>;
};
export default CardiologyFollowupReportsDocumentPDFTemplate;
