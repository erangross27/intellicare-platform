/**
 * FertilityMedicationManagementDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: fertility_medication_management
 *
 * 7 Sections:
 *   1. protocol-info: medicationProtocolType, gonadotropinType, gonadotropinDoseIU, gnrhAgonistAgent, gnrhAntagonistAgent
 *   2. trigger-luteal: triggerMedicationType, triggerDoseIU, lutealPhaseSupportRegimen, progesteroneDoseMg
 *   3. hormone-levels: estradiolLevelPgMl, serumLhLevelMiuMl, serumProgesteroneLevelNgMl, antiMullerianHormoneNgMl
 *   4. follicle-monitoring: antralFollicleCount, stimulationDayNumber, totalStimulationDays, folliclesGreaterThan14mm, leadFollicleSizeMm, endometrialThicknessMm
 *   5. risk-assessment: ohssRiskCategory, cabergolineOhssProphylaxis
 *   6. oral-medications: letrozoleDoseMg, clomipheneCitrateDoseMg
 *   7. additional: adjunctMedications, medicationAdverseEvents
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import FertilityMedicationManagementDocumentPDFTemplate from '../pdf-templates/FertilityMedicationManagementDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import './FertilityMedicationManagementDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" — localEdits keys are always "field-idx") */
const DRAFT_KEY = 'fertility_medication_managementPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'trigger-luteal': 'Trigger & Luteal Phase Support',
  'hormone-levels': 'Hormone Levels',
  'follicle-monitoring': 'Follicle Monitoring',
  'risk-assessment': 'Risk Assessment',
  'oral-medications': 'Oral Medications',
  'additional': 'Additional Medications & Adverse Events',
};

const FIELD_LABELS = {
  date: 'Date',
  medicationProtocolType: 'Medication Protocol Type',
  gonadotropinType: 'Gonadotropin Type',
  gonadotropinDoseIU: 'Gonadotropin Dose (IU)',
  gnrhAgonistAgent: 'GnRH Agonist Agent',
  gnrhAntagonistAgent: 'GnRH Antagonist Agent',
  triggerMedicationType: 'Trigger Medication Type',
  triggerDoseIU: 'Trigger Dose (IU)',
  lutealPhaseSupportRegimen: 'Luteal Phase Support Regimen',
  progesteroneDoseMg: 'Progesterone Dose (mg)',
  estradiolLevelPgMl: 'Estradiol Level (pg/mL)',
  serumLhLevelMiuMl: 'Serum LH Level (mIU/mL)',
  serumProgesteroneLevelNgMl: 'Serum Progesterone Level (ng/mL)',
  antiMullerianHormoneNgMl: 'Anti-Mullerian Hormone (ng/mL)',
  antralFollicleCount: 'Antral Follicle Count',
  stimulationDayNumber: 'Stimulation Day Number',
  totalStimulationDays: 'Total Stimulation Days',
  folliclesGreaterThan14mm: 'Follicles > 14mm',
  leadFollicleSizeMm: 'Lead Follicle Size (mm)',
  endometrialThicknessMm: 'Endometrial Thickness (mm)',
  ohssRiskCategory: 'OHSS Risk Category',
  cabergolineOhssProphylaxis: 'Cabergoline OHSS Prophylaxis',
  letrozoleDoseMg: 'Letrozole Dose (mg)',
  clomipheneCitrateDoseMg: 'Clomiphene Citrate Dose (mg)',
  adjunctMedications: 'Adjunct Medications',
  medicationAdverseEvents: 'Medication Adverse Events',
};

const SECTION_FIELDS = {
  'protocol-info': ['medicationProtocolType', 'gonadotropinType', 'gonadotropinDoseIU', 'gnrhAgonistAgent', 'gnrhAntagonistAgent'],
  'trigger-luteal': ['triggerMedicationType', 'triggerDoseIU', 'lutealPhaseSupportRegimen', 'progesteroneDoseMg'],
  'hormone-levels': ['estradiolLevelPgMl', 'serumLhLevelMiuMl', 'serumProgesteroneLevelNgMl', 'antiMullerianHormoneNgMl'],
  'follicle-monitoring': ['antralFollicleCount', 'stimulationDayNumber', 'totalStimulationDays', 'folliclesGreaterThan14mm', 'leadFollicleSizeMm', 'endometrialThicknessMm'],
  'risk-assessment': ['ohssRiskCategory', 'cabergolineOhssProphylaxis'],
  'oral-medications': ['letrozoleDoseMg', 'clomipheneCitrateDoseMg'],
  'additional': ['adjunctMedications', 'medicationAdverseEvents'],
};
...
/* A stored 0 = "not administered / not measured / not applicable to this protocol", justified by in-record
   contradiction: the ovarian-stimulation, trigger, hormone-level and oral-medication metrics are N/A for a
   medicated FET recipient endometrial-prep protocol (no stimulation cycle). Hidden in JSX + Copy + PDF.
   The real values (progesteroneDoseMg, antiMullerianHormoneNgMl, antralFollicleCount, endometrialThicknessMm)
   are non-zero and never in this list. */
...
const STRING_FIELDS = ['medicationProtocolType', 'gonadotropinType', 'gnrhAgonistAgent', 'gnrhAntagonistAgent', 'triggerMedicationType', 'lutealPhaseSupportRegimen'];
/* Known clinical scale → BlueSelect full-scale dropdown (OHSS risk = Low/Moderate/High/Very High). */
const ENUM_FIELDS = ['ohssRiskCategory'];
...
/* ═══════ COMPONENT ═══════ */
const FertilityMedicationManagementDocument = ({ document: docProp }) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Fertility Medication Management ${idx + 1}`.toLowerCase();
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    // Stage in localStorage (survives refresh). Approve commits to DB.
    const store = readDrafts();
  ...
  // Stage a sentence edit as a local DRAFT (no DB write). localStorage keeps it across refresh; Approve commits it.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const suffix = `-${idx}`;
      // localEdits keys are `${fieldPart}-${idx}`. Commit only this section's pending fields.
      ...
      for (const editKey of toCommit) {
        ...
        // arrayIndex ONLY when the segment after the LAST dot is purely numeric (mirrors handleSaveField)
        ...
        const resp = await secureApiClient.put(`/api/edit/fertility_medication_management/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/fertility_medication_management/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        text += `${head}${enumCanonical(ENUM_OPTIONS[f] || [], val)}\n\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== FERTILITY MEDICATION MANAGEMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Fertility Medication Management ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    ...
    if (ZERO_SENTINEL_FIELDS.includes(fn) && Number(val) === 0) return null; // hide "not administered / N-A" sentinel
    const editKey = `${fn}-${idx}`;
    const isEditing = editingField === editKey;
    const label = FIELD_LABELS[fn] || fn;
    const showLabel = label.toLowerCase() !== (SECTION_TITLES[sid] || '').toLowerCase();
  ...
  if (!records || records.length === 0) {
    return (
      <div className="fertility-medication-management-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Fertility Medication Management</h2></div>
        <div className="empty-state">No fertility medication management records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="fertility-medication-management-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Fertility Medication Management</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FertilityMedicationManagementDocumentPDFTemplate document={pdfData} />} fileName="Fertility_Medication_Management.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search fertility medication management..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Fertility Medication Management ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'protocol-info')}
            {renderSection(record, idx, 'trigger-luteal')}
            {renderSection(record, idx, 'hormone-levels')}
            {renderSection(record, idx, 'follicle-monitoring')}
            {renderSection(record, idx, 'risk-assessment')}
            {renderSection(record, idx, 'oral-medications')}
...
export default FertilityMedicationManagementDocument;
