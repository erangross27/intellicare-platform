...
# Create correct MongoDB configuration
sudo tee /etc/mongod.conf > /dev/null << 'MONGOCONFIG'
...
if sudo systemctl is-active --quiet mongod; then
    ...
    mongosh --quiet --eval "db.adminCommand('ping')" 2>/dev/null && echo "✅ MongoDB is responding" || echo "⚠️ Auth required"
else
    echo "❌ MongoDB failed to start. Checking logs..."
    sudo journalctl -u mongod -n 10 --no-pager | grep -i error
fi
