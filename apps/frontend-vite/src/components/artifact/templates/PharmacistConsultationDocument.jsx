/**
 * PharmacistConsultationDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: pharmacist_consultation
 *
 * 8 Sections:
 *   1. consultation-overview: medicationTherapyManagementDate, currentMedicationCount, polypharmacyRiskScore, medicationReconciliationCompleted, discrepanciesIdentifiedCount
 *   2. adherence-assessment: medicationAdherenceRate, morisky8ItemScoreMMAS
 *   3. drug-interactions-safety: drugDrugInteractionsIdentified, drugDiseaseContraindicationsNoted, therapeuticDuplicationFlags, beersListMedicationsPresent, stopp_startCriteriaViolations
 *   4. dosing-pharmacokinetics: creatinineClearanceCalculated, renalDoseAdjustmentsRequired, hepaticDoseAdjustmentsRequired, cytochromeP450InteractionRisk, narrowTherapeuticIndexDrugs, internationalNormalizedRatioINR, anticoagulationTimeInTherapeuticRange
 *   5. immunization-review: immunizationStatusReviewed, vaccinesAdministered
 *   6. adverse-reactions: adverseDrugReactionHistory
 *   7. pharmacist-interventions: pharmacistInterventionsMade
 *   8. cost-savings-followup: costSavingsOpportunitiesIdentified, nextMTMFollowUpDate
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PharmacistConsultationDocumentPDFTemplate from '../pdf-templates/PharmacistConsultationDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PharmacistConsultationDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" — array edits store full array) */
const DRAFT_KEY = 'pharmacist_consultationPendingEdits';
const readDrafts = () => {
...
const BOOLEAN_FIELDS = ['immunizationStatusReviewed', 'medicationReconciliationCompleted'];
const DATE_FIELDS = ['medicationTherapyManagementDate', 'nextMTMFollowUpDate'];
const NUMBER_FIELDS = ['currentMedicationCount', 'polypharmacyRiskScore', 'medicationAdherenceRate', 'morisky8ItemScoreMMAS', 'creatinineClearanceCalculated', 'internationalNormalizedRatioINR', 'anticoagulationTimeInTherapeuticRange', 'discrepanciesIdentifiedCount'];
...
const STRING_FIELDS = ['cytochromeP450InteractionRisk'];
...
/* splitByComma: parenthesis-aware comma split (thousands-comma guard: only split on comma+whitespace) */
...
/* ═══════ COMPONENT ═══════ */
const PharmacistConsultationDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.pharmacist_consultation) return Array.isArray(r.pharmacist_consultation) ? r.pharmacist_consultation : [r.pharmacist_consultation];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.pharmacist_consultation) return Array.isArray(dd.pharmacist_consultation) ? dd.pharmacist_consultation : [dd.pharmacist_consultation]; return [dd]; }
      return [r];
    });
  ...
  /* ═══════ UTILS ═══════ */
  // number 0 is a "not recorded" sentinel here (CrCl/INR/TTR/Morisky default to 0 when not assessed) → hide it.
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pharmacist Consultation ${idx + 1}`.toLowerCase();
  ...
  // stageDraft: shared helper — stage a full-text DRAFT locally + persist to localStorage (no DB write).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Commit each staged field for this section to the DB. localEdits keys are "field-idx".
      const toCommit = Object.keys(localEdits).filter(k => pendingEdits[k] && k.endsWith(`-${idx}`) && fields.includes(k.slice(0, -`-${idx}`.length)));
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -`-${idx}`.length); // "field" (no numeric dot-suffix in this template)
        ...
        const resp = await secureApiClient.put(`/api/edit/pharmacist_consultation/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/pharmacist_consultation/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PHARMACIST CONSULTATION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Pharmacist Consultation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const stepFor = (v) => { const s = String(v); const dot = s.indexOf('.'); if (dot === -1) return 1; const dec = s.length - dot - 1; return dec >= 2 ? 0.01 : 0.1; };
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                <button type="button" className="num-step" disabled={saving} onClick={e => { e.stopPropagation(); const step = stepFor(editValue); const n = Math.max(0, (parseFloat(editValue) || 0) - step); setEditValue(String(Number(n.toFixed(2)))); }}>&minus;</button>
                <input type="text" inputMode="decimal" className="num-stepper-input" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                <button type="button" className="num-step" disabled={saving} onClick={e => { e.stopPropagation(); const step = stepFor(editValue); const n = (parseFloat(editValue) || 0) + step; setEditValue(String(Number(n.toFixed(2)))); }}>+</button>
              </div>
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
  ...
  /* ═══════ RENDER: ARRAY FIELD (per-item editing; labeled "Label: value" items decompose to sub-label + value) ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          const rowVal = parsed.isLabeled ? parsed.value : itemStr;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(rowVal); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = parsed.isLabeled ? `${parsed.label}: ${editValue.trim()}` : editValue; stageDraft(id, fn, idx, sid, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pharmacist-consultation-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pharmacist Consultation</h2></div>
        <div className="empty-state">No pharmacist consultation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pharmacist-consultation-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Pharmacist Consultation</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PharmacistConsultationDocumentPDFTemplate document={pdfData} />} fileName="Pharmacist_Consultation.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pharmacist consultation..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Pharmacist Consultation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'consultation-overview')}
            {renderSection(record, idx, 'adherence-assessment')}
            {renderSection(record, idx, 'drug-interactions-safety')}
            {renderSection(record, idx, 'dosing-pharmacokinetics')}
            {renderSection(record, idx, 'immunization-review')}
            {renderSection(record, idx, 'adverse-reactions')}
            {renderSection(record, idx, 'pharmacist-interventions')}
            {renderSection(record, idx, 'cost-savings-followup')}
          </div>
        ))}
      </div>
    </div>
...
export default PharmacistConsultationDocument;
