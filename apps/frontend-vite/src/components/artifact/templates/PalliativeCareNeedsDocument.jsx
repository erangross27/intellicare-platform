/**
 * PalliativeCareNeedsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: palliative_care_needs
 *
 * 10 Sections:
 *   1. general-info: date, type, provider, facility, status
 *   2. symptoms-addressed: symptomsAddressed (array)
 *   3. pain-assessment: painAssessment (object keys)
 *   4. psychosocial-support: psychosocialSupport (array)
 *   5. care-options: spiritualCare, hospiceDiscussion
 *   6. quality-of-life: qualityOfLifeScore
 *   7. findings: findings
...
import PalliativeCareNeedsDocumentPDFTemplate from '../pdf-templates/PalliativeCareNeedsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PalliativeCareNeedsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'palliative_care_needsPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['type', 'provider', 'facility', 'status', 'qualityOfLifeScore', 'findings', 'assessment', 'plan', 'notes'];
...
/* splitByComma: parenthesis-aware comma split (thousands-separator guard: only split on ", " boundaries) */
...
/* flattenObjectLines: recursively flatten a dynamic-key object into {depth,label,value} lines (value=null => label-only header) */
const flattenObjectLines = (value, depth = 0, label = '') => {
  const out = [];
  if (isEmptyDeep(value)) return out;
  if (value === null || typeof value !== 'object') {
    out.push({ depth, label, value: typeof value === 'boolean' ? (value ? 'Yes' : 'No') : String(value) });
    return out;
  }
  if (Array.isArray(value)) {
    if (label) out.push({ depth, label, value: null });
    value.filter(v => !isEmptyDeep(v)).forEach((v) => { out.push(...flattenObjectLines(v, depth + (label ? 1 : 0), '')); });
    return out;
  }
  if (label) out.push({ depth, label, value: null });
  Object.entries(value).filter(([, v]) => !isEmptyDeep(v)).forEach(([k, v]) => {
    out.push(...flattenObjectLines(v, depth + (label ? 1 : 0), humanizeKey(k)));
  });
  return out;
};
...
/* ═══════ COMPONENT ═══════ */
const PalliativeCareNeedsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.palliative_care_needs) return Array.isArray(r.palliative_care_needs) ? r.palliative_care_needs : [r.palliative_care_needs];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.palliative_care_needs) return Array.isArray(dd.palliative_care_needs) ? dd.palliative_care_needs : [dd.palliative_care_needs]; return [dd]; }
      return [r];
    });
  ...
  // Persist one staged field draft to the pending-drafts localStorage store (survives refresh; NOT DB/PDF).
  ...
  /* flattenSearchable: recursively flatten any value (array/object/nested) into a single searchable string */
  const flattenSearchable = useCallback((v) => {
    if (v === null || v === undefined) return '';
    if (typeof v === 'boolean') return v ? 'yes' : 'no';
    if (isScalarVal(v)) return String(v);
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (Array.isArray(val)) { if (flattenSearchable(val).toLowerCase().includes(phrase)) return true; }
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) {
      if (Array.isArray(val)) return flattenSearchable(val).toLowerCase().includes(phrase);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Palliative Care Needs ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (val && (typeof val === 'object' ? flattenSearchable(val).toLowerCase().includes(phrase) : fmtVal(val).toLowerCase().includes(phrase))) return true;
        }
      }
      return false;
    });
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    const editKey = `${fn}-${idx}`;
    const clearApproved = () => { if (sid) setApprovedSections(prev => { const k = `${sid}-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; }); };
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const suffix = `-${idx}`;
    // localEdits keys for this record + section are "<field>-<idx>" (full merged value per field).
    ...
    try {
      for (const editKey of toCommit) {
        ...
        // arrayIndex ONLY when the segment after the LAST dot is purely numeric; else full dotted field.
        ...
        const resp = await secureApiClient.put(`/api/edit/palliative_care_needs/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/palliative_care_needs/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      const val = getFieldValue(record, f, idx);
      const present = OBJECT_FIELDS.includes(f) || RECOMMENDATIONS_FIELDS.includes(f) || DYNAMIC_OBJECT_FIELDS.includes(f) ? !isEmptyDeep(val) : hasVal(val);
      if (!present) return;
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        body += `${head}1. ${val === true || val === 'true' || val === 'Yes' ? 'Yes' : 'No'}\n\n`;
        ...
        body += head;
        entries.forEach(([k, v], i) => { body += `${i + 1}. ${humanizeKey(k)}: ${v}\n`; });
        body += '\n';
        ...
        items.forEach((it, i) => {
          ...
          else if (it && typeof it === 'object') { const rec = it.recommendation || it.text || ''; const dt = it.date ? formatDate(it.date) : ''; line = rec ? (dt ? `${rec} (${dt})` : rec) : flattenSearchable(it); }
          else line = String(it);
          body += `${i + 1}. ${line}\n`;
        });
        body += '\n';
      } else if (DYNAMIC_OBJECT_FIELDS.includes(f)) {
        const lines = flattenObjectLines(val, 0, '');
        body += head;
        lines.forEach(ln => { body += `${'  '.repeat(ln.depth + 1)}${ln.value === null ? ln.label : (ln.label ? `${ln.label}: ${ln.value}` : ln.value)}\n`; });
        body += '\n';
      } else {
        body += head;
    ...
    if (!body.trim()) return '';
    return `${title}\n${'='.repeat(40)}\n\n${body}`;
  }, [getFieldValue, hasVal, fmtVal, splitBySentence, flattenSearchable]);

  const copyAllText = useCallback(async () => {
    let text = `Palliative Care Needs\n${'='.repeat(40)}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Palliative Care Needs ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT FIELD (painAssessment - key/value pairs) ═══════ */
      ...
      <div key={fn} className="rec-mini-card">
        ...
        {entries.map(([key, value], eIdx) => {
          ...
          if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
            const phrase = searchTerm.toLowerCase().trim();
            const labelLower = label.toLowerCase();
            if (!labelLower.includes(phrase) && !phrase.includes(labelLower) && !key.toLowerCase().includes(phrase) && !valStr.toLowerCase().includes(phrase)) return null;
          }

          return (
            <div key={eIdx}>
              <div className="nested-subtitle" style={{ fontSize: '15px', marginTop: '4px' }}>{highlightText(key)}</div>
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(valStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; setSaveError(null); const currentObj = { ...(getFieldValue(record, fn, idx) || {}) }; currentObj[key] = editValue; setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: currentObj })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { const ak = `${sid}-${idx}`; if (!prev[ak]) return prev; const n = { ...prev }; delete n[ak]; return n; }); stageDraft(id, fn, currentObj); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <>
                    ...
                    <button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${key}: ${valStr}`, editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button>
                  </>
                )}
              </div>
              {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
  ...
  /* ═══════ RENDER: RECOMMENDATIONS (array of strings OR {recommendation, date} objects) ═══════ */
  /* Object items edit the `recommendation` subfield via arrayIndex+subField (preserves object shape). String items save whole item via arrayIndex. */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          const recText = isObj ? String(item.recommendation || item.text || '') : String(item);
          const dateText = isObj && item.date ? formatDate(item.date) : '';
          const displayText = dateText ? `${recText} (${dateText})` : recText;
          const editKey = `${fn}.${itemIdx}-${idx}`;
          const isEditing = editingField === editKey;
          const isModified = editedFields[editKey];
          ...
          if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
            const phrase = searchTerm.toLowerCase().trim();
            const labelLower = label.toLowerCase();
            if (!labelLower.includes(phrase) && !phrase.includes(labelLower) && !flattenSearchable(item).toLowerCase().includes(phrase)) return null;
          }

          // Save = stage a DRAFT (no DB write). The full merged array is committed on Approve.
          const saveRec = () => {
            const id = safeId(record); if (!id) return;
            setSaveError(null);
            const subField = isObj ? (item.recommendation !== undefined || item.text === undefined ? 'recommendation' : 'text') : null;
            const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])];
            currentArr[itemIdx] = isObj ? { ...item, [subField]: editValue } : editValue;
            setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: currentArr }));
            setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true }));
            setEditedFields(prev => ({ ...prev, [editKey]: 'edited' }));
            setApprovedSections(prev => { const ak = `${sid}-${idx}`; if (!prev[ak]) return prev; const n = { ...prev }; delete n[ak]; return n; });
            stageDraft(id, fn, currentArr);
            setEditingField(null); setEditValue('');
          };
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(recText); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveRec(); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  /* ═══════ RENDER: DYNAMIC-KEY OBJECT (results) — recursive, editable scalar leaves saving via dot-path ═══════ */
  const renderDynamicObjectField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    if (isEmptyDeep(val) || isScalarVal(val) || Array.isArray(val)) return null;
    const label = FIELD_LABELS[fn] || fn;
    const showSubLabel = label.trim().toLowerCase() !== (SECTION_TITLES[sid] || '').trim().toLowerCase();
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;

    /* recursively rebuild the root object with one leaf updated at a dot/index path */
    const setAtPath = (root, path, newVal) => {
      const clone = Array.isArray(root) ? [...root] : { ...root };
      const [head, ...rest] = path;
      if (rest.length === 0) { clone[head] = newVal; }
      else { clone[head] = setAtPath(root[head], rest, newVal); }
      return clone;
    };

    const renderNode = (node, path) => {
      return Object.entries(node).filter(([, v]) => !isEmptyDeep(v)).map(([k, v]) => {
        const curPath = [...path, k];
        const keyLabel = humanizeKey(k);
        if (isScalarVal(v)) {
          const editKey = `${fn}.${curPath.join('.')}-${idx}`;
          const isEditing = editingField === editKey;
          const isModified = editedFields[editKey];
          ...
          if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
            const phrase = searchTerm.toLowerCase().trim();
            const labelLower = label.toLowerCase();
            if (!labelLower.includes(phrase) && !phrase.includes(labelLower) && !keyLabel.toLowerCase().includes(phrase) && !valStr.toLowerCase().includes(phrase)) return null;
          }
          // Save = stage a DRAFT (no DB write). The full merged object is committed on Approve.
          const saveLeaf = () => {
            const id = safeId(record); if (!id) return;
            setSaveError(null);
            const root = getFieldValue(record, fn, idx) || {};
            const updated = setAtPath(root, curPath, editValue);
            setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: updated }));
            setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true }));
            setEditedFields(prev => ({ ...prev, [editKey]: 'edited' }));
            setApprovedSections(prev => { const ak = `${sid}-${idx}`; if (!prev[ak]) return prev; const n = { ...prev }; delete n[ak]; return n; });
            stageDraft(id, fn, updated);
            setEditingField(null); setEditValue('');
          };
          return (
            <div key={editKey}>
              <div className="nested-subtitle" style={{ fontSize: '15px', marginTop: '4px' }}>{highlightText(keyLabel)}</div>
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(valStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                  ...
                  <>
                    <div className="row-content"><span className="content-value">{highlightText(valStr)}</span><span className="edit-indicator">&#9998;</span></div>
                    <button className={`copy-btn ${copiedItems[editKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${keyLabel}: ${valStr}`, editKey); }}>{copiedItems[editKey] ? 'Copied!' : 'Copy'}</button>
                  </>
                )}
              </div>
              {isModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
        ...
        /* nested object/array — recurse under a sub-header */
        const childNode = Array.isArray(v) ? Object.fromEntries(v.map((iv, i) => [String(i), iv])) : v;
        return (
          <div key={`${fn}.${curPath.join('.')}-${idx}`} className="rec-mini-card" style={{ marginTop: 6 }}>
            <div className="nested-subtitle">{highlightText(keyLabel)}</div>
            {renderNode(childNode, Array.isArray(v) ? curPath : curPath)}
          </div>
        );
      });
    };
  ...
  if (!records || records.length === 0) {
    return (
      <div className="palliative-care-needs-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Palliative Care Needs</h2></div>
        <div className="empty-state">No palliative care needs records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="palliative-care-needs-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Palliative Care Needs</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PalliativeCareNeedsDocumentPDFTemplate document={pdfData} />} fileName="Palliative_Care_Needs.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search palliative care needs..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Palliative Care Needs ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'general-info')}
            {renderSection(record, idx, 'symptoms-addressed')}
            {renderSection(record, idx, 'pain-assessment')}
            {renderSection(record, idx, 'psychosocial-support')}
            {renderSection(record, idx, 'care-options')}
...
export default PalliativeCareNeedsDocument;
