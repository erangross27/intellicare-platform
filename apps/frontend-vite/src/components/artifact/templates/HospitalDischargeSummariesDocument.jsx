/**
 * HospitalDischargeSummariesDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: hospital_discharge_summaries
 *
 * 9 Sections:
 *   1. session-info: date (date picker), provider (string), facility (string)
 *   2. admission-discharge: admissionDate (date), dischargeDate (date), lengthOfStay (number), admittingComplaint (string)
 *   3. clinical-summary: principalDiagnosis (string), secondaryDiagnoses (array), hospitalCourse (string)
 *   4. procedures-services: proceduresPerformed (array), consultingServices (array)
 *   5. medications: dischargeMedications (array), medicationsDiscontinued (array)
 *   6. safety-alerts: allergiesAndAdverseReactions (array), criticalLabValues (array), codeStatus (string)
 *   7. discharge-info: dischargeCondition (string), dischargeDisposition (string), functionalStatusAtDischarge (string)
 *   8. instructions-education: dischargeInstructions (string), patientEducationProvided (string)
 *   9. follow-up: followUpAppointments (array)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HospitalDischargeSummariesDocumentPDFTemplate from '../pdf-templates/HospitalDischargeSummariesDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './HospitalDischargeSummariesDocument.css';
...
const STRING_FIELDS = [
  'provider', 'facility', 'admittingComplaint', 'principalDiagnosis',
  'hospitalCourse', 'codeStatus', 'dischargeCondition', 'dischargeDisposition',
  'functionalStatusAtDischarge', 'dischargeInstructions', 'patientEducationProvided',
];

const DATE_FIELDS = ['date', 'admissionDate', 'dischargeDate'];

const ARRAY_FIELDS = [
  'secondaryDiagnoses', 'proceduresPerformed', 'consultingServices',
  'dischargeMedications', 'medicationsDiscontinued', 'allergiesAndAdverseReactions',
  'criticalLabValues', 'followUpAppointments',
];
...
const DRAFT_KEY = 'hospital_discharge_summariesPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const HospitalDischargeSummariesDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.hospital_discharge_summaries) return Array.isArray(r.hospital_discharge_summaries) ? r.hospital_discharge_summaries : [r.hospital_discharge_summaries];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.hospital_discharge_summaries) return Array.isArray(dd.hospital_discharge_summaries) ? dd.hospital_discharge_summaries : [dd.hospital_discharge_summaries]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Hospital Discharge Summary ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/hospital_discharge_summaries/${id}/edit`, { field: fn, value: localEdits[editKey] });
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/hospital_discharge_summaries/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HOSPITAL DISCHARGE SUMMARIES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Hospital Discharge Summary ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="hospital-discharge-summaries-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Hospital Discharge Summaries</h2></div>
        <div className="empty-state">No hospital discharge summaries records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="hospital-discharge-summaries-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Hospital Discharge Summaries</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HospitalDischargeSummariesDocumentPDFTemplate document={pdfData} />} fileName="Hospital_Discharge_Summaries.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search hospital discharge summaries..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Hospital Discharge Summary ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'admission-discharge')}
            {renderSection(record, idx, 'clinical-summary')}
            {renderSection(record, idx, 'procedures-services')}
            {renderSection(record, idx, 'medications')}
            {renderSection(record, idx, 'safety-alerts')}
            {renderSection(record, idx, 'discharge-info')}
            {renderSection(record, idx, 'instructions-education')}
...
export default HospitalDischargeSummariesDocument;
