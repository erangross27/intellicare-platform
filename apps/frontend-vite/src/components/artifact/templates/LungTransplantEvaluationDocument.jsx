/**
 * LungTransplantEvaluationDocument.jsx
 * March 2026 — Complete template with inline editing, blue glow theme
 * Collection: lung_transplant_evaluation
 *
 * 8 Sections:
 *   1. diagnosis: primaryPulmonaryDiagnosis, transplantType
 *   2. pulmonary-function: lungAllocationScore, forcedVitalCapacityPercent, fev1PercentPredicted, dlcoPercentPredicted, sixMinuteWalkDistance, supplementalOxygenRequirement
 *   3. hemodynamics: pulmonaryArteryPressureSystolic, meanPulmonaryArteryPressure, pulmonaryVascularResistance
 *   4. cardiac: rightVentricularFunction, leftVentricularEjectionFraction, coronaryArteryDiseaseStatus
 *   5. immunology: panelReactiveAntibodyPercent, hlaTypingResults, cytomegalovirusSerostatus, epsteinBarrVirusSerostatus
 *   6. general-health: bodyMassIndex, estimatedGlomerularFiltrationRate, osteoporosisScreeningResult, gastroesophagealRefluxEvaluation
 *   7. surgical-history: previousThoracicSurgeries, colonizingOrganisms
 *   8. psychosocial: psychosocialClearanceStatus
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import LungTransplantEvaluationDocumentPDFTemplate from '../pdf-templates/LungTransplantEvaluationDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './LungTransplantEvaluationDocument.css';
...
const DRAFT_KEY = 'lung_transplant_evaluationPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  'diagnosis': 'Diagnosis & Transplant Type',
  'pulmonary-function': 'Pulmonary Function',
  'hemodynamics': 'Hemodynamics',
  'cardiac': 'Cardiac Evaluation',
  'immunology': 'Immunology',
  'general-health': 'General Health',
  'surgical-history': 'Surgical History & Microbiology',
  'psychosocial': 'Psychosocial',
};

const FIELD_LABELS = {
  primaryPulmonaryDiagnosis: 'Primary Pulmonary Diagnosis',
  transplantType: 'Transplant Type',
  lungAllocationScore: 'Lung Allocation Score',
  forcedVitalCapacityPercent: 'Forced Vital Capacity (%)',
  fev1PercentPredicted: 'FEV1 (% Predicted)',
  dlcoPercentPredicted: 'DLCO (% Predicted)',
  sixMinuteWalkDistance: 'Six-Minute Walk Distance (m)',
  supplementalOxygenRequirement: 'Supplemental Oxygen Requirement',
  pulmonaryArteryPressureSystolic: 'Pulmonary Artery Pressure Systolic (mmHg)',
  meanPulmonaryArteryPressure: 'Mean Pulmonary Artery Pressure (mmHg)',
  ...
  coronaryArteryDiseaseStatus: 'Coronary Artery Disease Status',
  panelReactiveAntibodyPercent: 'Panel Reactive Antibody (%)',
  hlaTypingResults: 'HLA Typing Results',
  cytomegalovirusSerostatus: 'Cytomegalovirus Serostatus',
  epsteinBarrVirusSerostatus: 'Epstein-Barr Virus Serostatus',
  bodyMassIndex: 'Body Mass Index',
  estimatedGlomerularFiltrationRate: 'Estimated GFR (mL/min)',
  osteoporosisScreeningResult: 'Osteoporosis Screening Result',
  gastroesophagealRefluxEvaluation: 'Gastroesophageal Reflux Evaluation',
  previousThoracicSurgeries: 'Previous Thoracic Surgeries',
  colonizingOrganisms: 'Colonizing Organisms',
  psychosocialClearanceStatus: 'Psychosocial Clearance Status',
};

const SECTION_FIELDS = {
  'diagnosis': ['primaryPulmonaryDiagnosis', 'transplantType'],
  'pulmonary-function': ['lungAllocationScore', 'forcedVitalCapacityPercent', 'fev1PercentPredicted', 'dlcoPercentPredicted', 'sixMinuteWalkDistance', 'supplementalOxygenRequirement'],
  'hemodynamics': ['pulmonaryArteryPressureSystolic', 'meanPulmonaryArteryPressure', 'pulmonaryVascularResistance'],
  'cardiac': ['rightVentricularFunction', 'leftVentricularEjectionFraction', 'coronaryArteryDiseaseStatus'],
  'immunology': ['panelReactiveAntibodyPercent', 'hlaTypingResults', 'cytomegalovirusSerostatus', 'epsteinBarrVirusSerostatus'],
  'general-health': ['bodyMassIndex', 'estimatedGlomerularFiltrationRate', 'osteoporosisScreeningResult', 'gastroesophagealRefluxEvaluation'],
  'surgical-history': ['previousThoracicSurgeries', 'colonizingOrganisms'],
  'psychosocial': ['psychosocialClearanceStatus'],
};

const NUMBER_FIELDS = ['lungAllocationScore', 'forcedVitalCapacityPercent', 'fev1PercentPredicted', 'dlcoPercentPredicted', 'sixMinuteWalkDistance', 'pulmonaryArteryPressureSystolic', 'meanPulmonaryArteryPressure', 'pulmonaryVascularResistance', 'leftVentricularEjectionFraction', 'panelReactiveAntibodyPercent', 'bodyMassIndex', 'estimatedGlomerularFiltrationRate'];
/* MEANINGFUL_ZERO_FIELDS: numerics where 0 is a valid clinical finding (PRA 0% = no detected anti-HLA antibodies) → always show. All other numerics (FEV1/FVC/DLCO/6MWD/PAP/PVR/LVEF/BMI/eGFR/LAS) treat 0 as a sentinel "not measured" → hidden. */
const MEANINGFUL_ZERO_FIELDS = ['panelReactiveAntibodyPercent'];
const ARRAY_FIELDS = ['previousThoracicSurgeries', 'colonizingOrganisms'];
const STRING_FIELDS = ['primaryPulmonaryDiagnosis', 'supplementalOxygenRequirement', 'rightVentricularFunction', 'coronaryArteryDiseaseStatus', 'hlaTypingResults', 'cytomegalovirusSerostatus', 'epsteinBarrVirusSerostatus', 'osteoporosisScreeningResult', 'gastroesophagealRefluxEvaluation', 'transplantType', 'psychosocialClearanceStatus'];
...
/* ═══════ COMPONENT ═══════ */
const LungTransplantEvaluationDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.lung_transplant_evaluation) return Array.isArray(r.lung_transplant_evaluation) ? r.lung_transplant_evaluation : [r.lung_transplant_evaluation];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.lung_transplant_evaluation) return Array.isArray(dd.lung_transplant_evaluation) ? dd.lung_transplant_evaluation : [dd.lung_transplant_evaluation]; return [dd]; }
      return [r];
    });
  ...
  /* numberShows: hide sentinel 0 ("not measured") but always show meaningful-0 fields (PRA 0%) or doctor-edited zeros */
  const numberShows = useCallback((record, fn, idx, valOverride) => {
    const val = valOverride !== undefined ? valOverride : record[fn];
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Lung Transplant Evaluation ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/lung_transplant_evaluation/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/lung_transplant_evaluation/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (NUMBER_FIELDS.includes(f)) {
        if (!numberShows(record, f, idx, val)) return;
        text += `${label}\n${val}\n\n`;
        return;
      }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== LUNG TRANSPLANT EVALUATION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Lung Transplant Evaluation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderNumberField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx); if (!numberShows(record, fn, idx, val)) return null;
    const editKey = `${fn}-${idx}`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(parsed.isLabeled ? parsed.value : itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const saveVal = parsed.isLabeled ? `${parsed.label}: ${editValue}` : editValue; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = saveVal; stageDraft(record, fn, idx, sid, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      if (NUMBER_FIELDS.includes(f)) return numberShows(record, f, idx, val);
      return hasVal(val);
    });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="lung-transplant-evaluation-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Lung Transplant Evaluation</h2></div>
        <div className="empty-state">No lung transplant evaluation data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="lung-transplant-evaluation-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Lung Transplant Evaluation</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<LungTransplantEvaluationDocumentPDFTemplate document={pdfData} />} fileName="Lung_Transplant_Evaluation.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search lung transplant evaluation..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(record.primaryPulmonaryDiagnosis || `Lung Transplant Evaluation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'diagnosis')}
            {renderSection(record, idx, 'pulmonary-function')}
            {renderSection(record, idx, 'hemodynamics')}
            {renderSection(record, idx, 'cardiac')}
            {renderSection(record, idx, 'immunology')}
            {renderSection(record, idx, 'general-health')}
...
export default LungTransplantEvaluationDocument;
