/**
 * GlaucomaAssessmentsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: glaucoma_assessments
 *
 * 4 Sections:
 *   1. session-info: date (date picker), provider (string), facility (string)
 *   2. iop-measurements: intraocularpressureOD, intraocularpressureOS, centralCornealThickness, targetIOP
 *   3. examination-findings: cupToDiscRatioOD, cupToDiscRatioOS, visualFieldDefects (long),
 *                            opticNerveAppearance (long), gonioscopyFindings (long)
 *   4. diagnosis-treatment: glaucomaType, glaucomaSeverity, currentMedications (ARRAY strings),
 *                           treatmentChanges (long), surgicalInterventions, progressionRisk, followUpInterval
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import GlaucomaAssessmentsDocumentPDFTemplate from '../pdf-templates/GlaucomaAssessmentsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './GlaucomaAssessmentsDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'session-info': 'Session Information',
  'iop-measurements': 'IOP & Corneal Measurements',
...
const FIELD_LABELS = {
  ...
  provider: 'Provider',
  facility: 'Facility',
  intraocularpressureOD: 'Intraocular Pressure OD (Right)',
  intraocularpressureOS: 'Intraocular Pressure OS (Left)',
  centralCornealThickness: 'Central Corneal Thickness',
  targetIOP: 'Target IOP',
  cupToDiscRatioOD: 'Cup-to-Disc Ratio OD (Right)',
  cupToDiscRatioOS: 'Cup-to-Disc Ratio OS (Left)',
  visualFieldDefects: 'Visual Field Defects',
  opticNerveAppearance: 'Optic Nerve Appearance',
  gonioscopyFindings: 'Gonioscopy Findings',
  glaucomaType: 'Glaucoma Type',
  glaucomaSeverity: 'Glaucoma Severity',
  currentMedications: 'Current Medications',
  treatmentChanges: 'Treatment Changes',
  surgicalInterventions: 'Surgical Interventions',
  progressionRisk: 'Progression Risk',
  followUpInterval: 'Follow-Up Interval',
};

const SECTION_FIELDS = {
  'session-info': ['date', 'provider', 'facility'],
  'iop-measurements': ['intraocularpressureOD', 'intraocularpressureOS', 'centralCornealThickness', 'targetIOP'],
  'examination-findings': ['cupToDiscRatioOD', 'cupToDiscRatioOS', 'visualFieldDefects', 'opticNerveAppearance', 'gonioscopyFindings'],
  'diagnosis-treatment': ['glaucomaType', 'glaucomaSeverity', 'currentMedications', 'treatmentChanges', 'surgicalInterventions', 'progressionRisk', 'followUpInterval'],
};

/* Reverse lookup: field name → its sectionId (for clearing the section's approve flag on re-edit) */
const FIELD_TO_SECTION = Object.entries(SECTION_FIELDS).reduce((acc, [sid, fields]) => {
  fields.forEach(f => { acc[f] = sid; });
  return acc;
}, {});

/* No number fields — all strings */
const NUMBER_FIELDS = [];

const STRING_FIELDS = [
  'provider', 'facility',
  'intraocularpressureOD', 'intraocularpressureOS', 'centralCornealThickness', 'targetIOP',
  'cupToDiscRatioOD', 'cupToDiscRatioOS', 'visualFieldDefects', 'opticNerveAppearance', 'gonioscopyFindings',
  'glaucomaType', 'glaucomaSeverity', 'treatmentChanges', 'surgicalInterventions', 'progressionRisk', 'followUpInterval',
];
const DATE_FIELDS = ['date'];
const ARRAY_FIELDS = ['currentMedications'];

/* Long string fields — use parseLabel+splitBySentence */
const SENTENCE_FIELDS = ['visualFieldDefects', 'opticNerveAppearance', 'gonioscopyFindings', 'treatmentChanges'];
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; arrays stored as whole-array value) */
const DRAFT_KEY = 'glaucoma_assessmentsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const GlaucomaAssessmentsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.glaucoma_assessments) return Array.isArray(r.glaucoma_assessments) ? r.glaucoma_assessments : [r.glaucoma_assessments];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.glaucoma_assessments) return Array.isArray(dd.glaucoma_assessments) ? dd.glaucoma_assessments : [dd.glaucoma_assessments]; return [dd]; }
      return [r];
    });
  ...
  const getEffectiveArray = useCallback((record, fn, idx) => {
    ...
    if (typeof raw === 'string') return raw.split(',').map(s => s.trim()).filter(s => s);
    return [String(raw)];
  }, [localEdits]);

  const safeId = useCallback((r) => { if (!r?._id) return null; if (typeof r._id === 'string') return r._id; if (r._id.$oid) return r._id.$oid; return String(r._id); }, []);

  /* Stage a DRAFT locally (no DB write): localEdits + pendingEdits + write the pending-drafts
     localStorage store (survives refresh), and drop the field's section approve flag so the button
     returns to yellow Pending Approve. Approve (handleApproveSection) is the only path that writes DB. */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Glaucoma Assessment ${idx + 1}`.toLowerCase();
  ...
  const handleSaveArrayItem = useCallback((record, fn, idx, arrayIndex) => {
    ...
    // Stage a DRAFT only — NO DB write. Approve commits it (whole-array value under `${fn}-${idx}`).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // localEdits keys are `${fieldName}-${idx}` (arrays stored as whole-array value → no arrayIndex)
      const suffix = `-${idx}`;
      const toCommit = Object.keys(localEdits).filter(k =>
        pendingEdits[k] && k.endsWith(suffix) && fields.includes(k.slice(0, -suffix.length))
      );
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -suffix.length); // field name
        ...
        await secureApiClient.put(`/api/edit/glaucoma_assessments/${id}/edit`, payload);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/glaucoma_assessments/${id}/approve`, { sectionId: sid, approved: true });
      ...
      console.error('[GlaucomaAssessments] Approve error:', err);
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== GLAUCOMA ASSESSMENTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Glaucoma Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderStringField = (record, fn, idx, sid, title) => {
    const val = getFieldValue(record, fn, idx); if (!hasVal(val)) return null;
    const strVal = fmtVal(val);
    const isSentenceField = SENTENCE_FIELDS.includes(fn);
    const sentences = isSentenceField ? splitBySentence(strVal) : [];
    const label = FIELD_LABELS[fn] || fn;
    const showSubLabel = label.trim().toLowerCase() !== (SECTION_TITLES[sid] || '').trim().toLowerCase();
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;

    /* Multi-sentence: render with splitBySentence */
    if (isSentenceField && sentences.length > 1) {
  ...
  if (!records || records.length === 0) {
    return (
      <div className="glaucoma-assessments-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Glaucoma Assessments</h2></div>
        <div className="empty-state">No glaucoma assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="glaucoma-assessments-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Glaucoma Assessments</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<GlaucomaAssessmentsDocumentPDFTemplate document={pdfData} />} fileName="Glaucoma_Assessments.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search glaucoma assessments..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Glaucoma Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'iop-measurements')}
...
export default GlaucomaAssessmentsDocument;
