 ...
 *
 * 9 Sections:
 *   1. assessment-info: date, type, status
 *   2. disease-activity: das28Score, cdaiScore, sdaiScore, acr20Response, jointCounts (OBJECT)
 *   3. clinical-findings: findings, radiographicProgression, extraarticularManifestations
 *   4. functional-status: functionalStatus
 *   5. assessment-plan: assessment, plan
 *   6. test-results: results (OBJECT)
 *   7. recommendations: recommendations[] ({ recommendation, date })
 *   8. notes: notes
 *   9. provider-details: provider, facility
 */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits base field, e.g. "findings") */
...
/* humanizeKey: snake/camel → Title Case, with overrides for known acronyms */
const KEY_OVERRIDES = {
...
/* number+unit / ratio leaf splitters — null for plain text */
...
const FIELD_LABELS = {
  ...
  sdaiScore: 'SDAI Score',
  acr20Response: 'ACR20 Response',
  jointCounts: 'Joint Counts',
  results: 'Test Results',
  findings: 'Findings',
  radiographicProgression: 'Radiographic Progression',
  extraarticularManifestations: 'Extra-articular Manifestations',
...
const SECTION_FIELDS = {
  ...
  'disease-activity': ['das28Score', 'cdaiScore', 'sdaiScore', 'acr20Response', 'jointCounts'],
  'clinical-findings': ['findings', 'radiographicProgression', 'extraarticularManifestations'],
  'functional-status': ['functionalStatus'],
  'assessment-plan': ['assessment', 'plan'],
  'test-results': ['results'],
  'recommendations': ['recommendations'],
  'notes': ['notes'],
  'provider-details': ['provider', 'facility'],
};

const SENTENCE_FIELDS = ['findings', 'assessment', 'plan', 'notes', 'functionalStatus', 'radiographicProgression'];
const SIMPLE_FIELDS = ['type', 'status', 'das28Score', 'cdaiScore', 'sdaiScore', 'acr20Response', 'provider', 'facility'];
const ARRAY_FIELDS = ['extraarticularManifestations'];
const REC_ARRAY_FIELDS = ['recommendations'];
...
/* ═══════ COMPONENT ═══════ */
const RheumatoidArthritisAssessmentDocument = ({ document: docProp, data, templateData }) => {
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Draft store shape: { [recordId]: { [fieldPart]: fullValue } }; fieldPart === localEdits base field. */
  useEffect(() => {
    ...
    if (Array.isArray(val)) {
      return val.some(item => {
        if (item && typeof item === 'object') {
          return String(item.recommendation || '').toLowerCase().includes(phrase) || formatDate(item.date).toLowerCase().includes(phrase);
        }
  ...
  const shouldShowSection = useCallback((record, sid, idx) => {
    ...
    for (const f of fields) {
      ...
      if (DATE_FIELDS.includes(f)) { if (formatDate(val).toLowerCase().includes(phrase)) return true; continue; }
      if (valueMatchesPhrase(val, phrase)) return true;
    }
    return false;
  }, [searchTerm, getFieldValue, valueMatchesPhrase]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      const docTitle = 'rheumatoid arthritis assessment';
      const rt = `rheumatoid arthritis assessment ${oi + 1}`;
  ...
  }, [records, searchTerm, getFieldValue, valueMatchesPhrase]);
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // Stage a DRAFT for a record's field locally + persist to the pending-drafts localStorage store
  ...
  /* ═══════ SAVE: OBJECT LEAF (dotted-path update) ═══════ */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    // Build the updated full root-object value and stage it as a DRAFT (no DB write).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Persist each staged field for this section now. localEdits keys are `${fieldPart}-${idx}`
      // where fieldPart === a base field name (full value stored), so no arrayIndex is needed here;
      // arrayIndex is added ONLY when the segment after the LAST dot is purely numeric.
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        await secureApiClient.put(`/api/edit/rheumatoid_arthritis_assessment/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/rheumatoid_arthritis_assessment/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (OBJECT_FIELDS.includes(f)) {
        ...
        if (showLabel) text += `${label}\n`;
        text += `${formatDate(val)}\n\n`;
      } else if (REC_ARRAY_FIELDS.includes(f)) {
        const items = (Array.isArray(val) ? val : [val]).filter(item => hasVal(recItemText(item)));
        if (items.length === 0) return;
        if (showLabel) text += `${label}\n`;
        items.forEach((item, i) => {
          const dt = (item && typeof item === 'object' && item.date) ? ` (${formatDate(item.date)})` : '';
          text += `${i + 1}. ${recItemText(item)}${dt}\n`;
        });
        text += '\n';
    ...
    return text;
  }, [getFieldValue, hasVal, fmtVal, splitBySentence, splitBySemicolon, formatSentenceFieldLines, recItemText, objectCopyLines]);

  const copyAllText = useCallback(async () => {
    let text = '=== RHEUMATOID ARTHRITIS ASSESSMENT ===\n\n';
    pdfData.forEach((r) => {
      const oi = r._originalIdx ?? 0;
      text += `Rheumatoid Arthritis Assessment ${oi + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable; number+unit -> number input, "4/5" ratio, text -> textarea, bool -> select) ═══════ */
  ...
  /* ═══════ RENDER: OBJECT FIELD (jointCounts, results) ═══════ */
  ...
  /* ═══════ RENDER: SIMPLE FIELD (no splitting — provider, facility, status, scores, type) ═══════ */
  ...
  /* ═══════ RENDER: RECOMMENDATIONS ARRAY ({ recommendation, date } objects, per-item editing) ═══════ */
  const renderRecommendations = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    const rawItems = Array.isArray(val) ? val : [];
    const items = rawItems.map((item, i) => ({ item, i })).filter(({ item }) => hasVal(recItemText(item)));
    if (items.length === 0) return null;
    const label = FIELD_LABELS[fn] || fn;
    const showLabel = label.toLowerCase() !== (SECTION_TITLES[sid] || '').toLowerCase();
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
    const phrase = searchTerm.toLowerCase().trim();
    const labelLower = label.toLowerCase();
    const labelMatch = !phrase || labelLower.includes(phrase) || phrase.includes(labelLower);

    return (
      <div key={fn} className="rec-mini-card">
        {showLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
        {items.map(({ item, i: itemIdx }, pos) => {
          const editKey = `${fn}.${itemIdx}-${idx}`;
          const isEditing = editingField === editKey;
          const isModified = editedFields[editKey];
          const recText = recItemText(item);
          const recDate = (item && typeof item === 'object' && item.date) ? formatDate(item.date) : '';

          if (phrase && !sectionTitleMatches(sid) && !record._showAllSections && !labelMatch) {
            if (!recText.toLowerCase().includes(phrase) && !recDate.toLowerCase().includes(phrase)) return null;
          }

          return (
            <div key={itemIdx} className={pos > 0 || recDate ? 'rec-mini-card' : ''} style={pos > 0 ? { marginTop: 8 } : undefined}>
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(recText); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; setSaveError(null); const currentArr = (Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : []).map(x => (x && typeof x === 'object') ? { ...x } : x); if (currentArr[itemIdx] && typeof currentArr[itemIdx] === 'object') { currentArr[itemIdx].recommendation = editValue; } else { currentArr[itemIdx] = editValue; } stageDraft(record, fn, idx, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      if (REC_ARRAY_FIELDS.includes(f)) return Array.isArray(val) && val.some(item => hasVal(recItemText(item)));
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (REC_ARRAY_FIELDS.includes(f)) return renderRecommendations(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="rheumatoid-arthritis-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Rheumatoid Arthritis Assessment</h2></div>
        <div className="empty-state">No rheumatoid arthritis assessment records available</div>
      </div>
    );
  }
    ...
    <div className="rheumatoid-arthritis-assessment-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<RheumatoidArthritisAssessmentDocumentPDFTemplate document={pdfData} />} fileName={`rheumatoid-arthritis-assessment-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search rheumatoid arthritis assessment..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            ...
            <div key={safeId(record) || idx} className="record-card">
              <div className="record-header">
                <div className="header-top-row">
                  ...
                  {hasVal(getFieldValue(record, 'status', origIdx)) && (
                    <span className="status-badge">{highlightText(fmtVal(getFieldValue(record, 'status', origIdx)))}</span>
                  )}
                </div>
                <h3 className="record-name">{highlightText(`Rheumatoid Arthritis Assessment ${origIdx + 1}`)}</h3>
              </div>
              {renderSection(record, origIdx, 'assessment-info')}
              {renderSection(record, origIdx, 'disease-activity')}
              {renderSection(record, origIdx, 'clinical-findings')}
              {renderSection(record, origIdx, 'functional-status')}
              {renderSection(record, origIdx, 'assessment-plan')}
              {renderSection(record, origIdx, 'test-results')}
              {renderSection(record, origIdx, 'recommendations')}
              {renderSection(record, origIdx, 'notes')}
