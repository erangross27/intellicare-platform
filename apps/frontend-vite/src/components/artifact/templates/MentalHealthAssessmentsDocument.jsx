/**
 * MentalHealthAssessmentsDocument.jsx
 * March 2026 — Canonical one-pass polish, inline editing, blue glow theme
 * Collection: mental_health_assessments
 *
 * 11 Sections:
 *   1. assessment-info: assessmentDate (BlueDatePicker), assessmentType
 *   2. presenting-problems: presentingProblems (long)
 *   3. mental-status-exam: mentalStatusExam (long, labeled clauses decompose)
 *   4. screening-scores: phq9Score (STRING, not a number), gad7Score, screeningTools
 *   5. risk-assessment: suicideRisk, homicideRisk
 *   6. functioning-level: functioningLevel (long)
 *   7. provisional-diagnosis: diagnosisProvisional (long)
 *   8. treatment-recommendations: treatmentRecommendations (long)
 *   9. safety-plan: safetyPlan (long)
 *  10. disposition: disposition
 *  11. recommendations: recommendations — array of {recommendation, date}; each object decomposes into a
 *                       nested-subtitle (recommendation text) + a formatted date row (BlueDatePicker)
 *
 * Record date rule: this collection has NO real clinical `date` field (only createdAt/updatedAt ingestion
 * timestamps). assessmentDate is a FIELD, not the record date → the record header is TITLE ONLY.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MentalHealthAssessmentsDocumentPDFTemplate from '../pdf-templates/MentalHealthAssessmentsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './MentalHealthAssessmentsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'mental_health_assessmentsPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  diagnosisProvisional: 'Provisional Diagnosis',
...
const SECTION_FIELDS = {
  'assessment-info': ['assessmentDate', 'assessmentType'],
  'presenting-problems': ['presentingProblems'],
  'mental-status-exam': ['mentalStatusExam'],
  'screening-scores': ['phq9Score', 'gad7Score', 'screeningTools'],
  'risk-assessment': ['suicideRisk', 'homicideRisk'],
  'functioning-level': ['functioningLevel'],
  'provisional-diagnosis': ['diagnosisProvisional'],
...
/* phq9Score / gad7Score are STRINGS ("BDI-II score 14 …") — NOT numbers. No numeric fields here. */
const NUMBER_FIELDS = [];
const OBJECT_ARRAY_FIELDS = ['recommendations'];

const STRING_FIELDS = [
  'assessmentType', 'presentingProblems', 'mentalStatusExam',
  'phq9Score', 'gad7Score', 'screeningTools', 'suicideRisk', 'homicideRisk',
  'functioningLevel', 'diagnosisProvisional', 'treatmentRecommendations', 'safetyPlan', 'disposition',
];
...
/* ═══════ COMPONENT ═══════ */
const MentalHealthAssessmentsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.mental_health_assessments) return Array.isArray(r.mental_health_assessments) ? r.mental_health_assessments : [r.mental_health_assessments];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.mental_health_assessments) return Array.isArray(dd.mental_health_assessments) ? dd.mental_health_assessments : [dd.mental_health_assessments]; return [dd]; }
      return [r];
    });
  ...
  const showFieldLabel = useCallback((fn, sid) => {
    const label = (FIELD_LABELS[fn] || fn).trim().toLowerCase();
    const title = (SECTION_TITLES[sid] || '').trim().toLowerCase();
    return label !== title;
  }, []);
      ...
      if (val !== null && val !== undefined) {
        if (Array.isArray(val)) {
          for (const item of val) {
            if (item && typeof item === 'object') { if (Object.values(item).some(v => String(v || '').toLowerCase().includes(phrase))) return true; }
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Mental Health Assessment ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/mental_health_assessments/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/mental_health_assessments/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    const fields = SECTION_FIELDS[sid] || [];
    const anyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      if (OBJECT_ARRAY_FIELDS.includes(f)) return Array.isArray(val) && val.filter(r => (r?.recommendation || '').trim()).length > 0;
      return hasVal(val);
    });
    if (!anyVal) return ''; // skip empty sections entirely (consistent with JSX/PDF hide-empty)
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MENTAL HEALTH ASSESSMENTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Mental Health Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderDateField = (record, fn, idx, sid) => {
    ...
    return (
      <div key={fn} className="rec-mini-card">
        {showFieldLabel(fn, sid) && <div className="nested-subtitle">{highlightText(label)}</div>}
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    const sentences = splitBySentence(strVal);
    const label = FIELD_LABELS[fn] || fn;
    const showLabel = showFieldLabel(fn, sid);
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="mental-health-assessments-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Mental Health Assessments</h2></div>
        <div className="empty-state">No mental health assessment data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="mental-health-assessments-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Mental Health Assessments</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MentalHealthAssessmentsDocumentPDFTemplate document={pdfData} />} fileName="Mental_Health_Assessments.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search mental health assessments..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Mental Health Assessment ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'assessment-info')}
            {renderSection(record, idx, 'presenting-problems')}
            ...
            {renderSection(record, idx, 'functioning-level')}
            {renderSection(record, idx, 'provisional-diagnosis')}
            {renderSection(record, idx, 'treatment-recommendations')}
...
export default MentalHealthAssessmentsDocument;
