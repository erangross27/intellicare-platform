/**
 * SpermAnalysisDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: sperm_analysis
 *
 * 8 Sections:
 *   1. specimen: specimenCollectionMethod, abstinencePeriodDays, semenVolumeMl, liquefactionTimeMinutes, semenViscosity, semenPh, semenAppearance
 *   2. concentration: spermConcentrationMillionPerMl, totalSpermCountMillion, roundCellConcentrationMillionPerMl, leukocyteConcentrationMillionPerMl
 *   3. motility: totalMotilityPercent, progressiveMotilityPercent, nonProgressiveMotilityPercent, immotileSpermPercent, spermMotilityGrade
 *   4. morphology: normalMorphologyPercent, headDefectsPercent, midpieceDefectsPercent, tailDefectsPercent, teratozoospermiaIndex
 *   5. vitality: spermVitalityPercent, dnaFragmentationIndexPercent, spermAgglutinationGrade
 *   6. antibodies: antispermAntibodiesPresent, marTestPercent
 *   7. biochemistry: fructoseLevelMgPerDl, zincConcentrationUmolPerL, semenCultureResult
 *   8. diagnosis: whoSemenAnalysisDiagnosis
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import BlueSelect from '../components/BlueSelect';
import SpermAnalysisDocumentPDFTemplate from '../pdf-templates/SpermAnalysisDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SpermAnalysisDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'specimen': 'Specimen',
  'concentration': 'Concentration & Count',
  'motility': 'Motility',
  'morphology': 'Morphology',
  'vitality': 'Vitality & DNA',
  'antibodies': 'Antisperm Antibodies',
  'biochemistry': 'Biochemistry & Culture',
...
const SECTION_FIELDS = {
  'specimen': ['specimenCollectionMethod', 'abstinencePeriodDays', 'semenVolumeMl', 'liquefactionTimeMinutes', 'semenViscosity', 'semenPh', 'semenAppearance'],
  'concentration': ['spermConcentrationMillionPerMl', 'totalSpermCountMillion', 'roundCellConcentrationMillionPerMl', 'leukocyteConcentrationMillionPerMl'],
  'motility': ['totalMotilityPercent', 'progressiveMotilityPercent', 'nonProgressiveMotilityPercent', 'immotileSpermPercent', 'spermMotilityGrade'],
  'morphology': ['normalMorphologyPercent', 'headDefectsPercent', 'midpieceDefectsPercent', 'tailDefectsPercent', 'teratozoospermiaIndex'],
  'vitality': ['spermVitalityPercent', 'dnaFragmentationIndexPercent', 'spermAgglutinationGrade'],
  'antibodies': ['antispermAntibodiesPresent', 'marTestPercent'],
  'biochemistry': ['fructoseLevelMgPerDl', 'zincConcentrationUmolPerL', 'semenCultureResult'],
  'diagnosis': ['whoSemenAnalysisDiagnosis'],
};

const NUMBER_FIELDS = ['abstinencePeriodDays', 'semenVolumeMl', 'liquefactionTimeMinutes', 'semenPh', 'spermConcentrationMillionPerMl', 'totalSpermCountMillion', 'roundCellConcentrationMillionPerMl', 'leukocyteConcentrationMillionPerMl', 'totalMotilityPercent', 'progressiveMotilityPercent', 'nonProgressiveMotilityPercent', 'immotileSpermPercent', 'normalMorphologyPercent', 'headDefectsPercent', 'midpieceDefectsPercent', 'tailDefectsPercent', 'teratozoospermiaIndex', 'spermVitalityPercent', 'dnaFragmentationIndexPercent', 'marTestPercent', 'fructoseLevelMgPerDl', 'zincConcentrationUmolPerL'];
/* MEANINGFUL_ZERO_FIELDS: WHO measurements where 0 is a critical clinical finding (concentration/count 0 = azoospermia; motility 0 = asthenozoospermia; morphology 0% = teratozoospermia; vitality 0 = necrozoospermia; volume 0 = aspermia; fructose 0 = ejaculatory-duct obstruction) → always show when present. Sentinel-zero (0 = "not recorded"): abstinencePeriodDays, liquefactionTimeMinutes, semenPh (pH 0 impossible), zincConcentrationUmolPerL → stay hidden at 0 unless doctor-edited. */
...
const ARRAY_FIELDS = ['whoSemenAnalysisDiagnosis'];
const BOOLEAN_FIELDS = ['antispermAntibodiesPresent'];
const STRING_FIELDS = ['specimenCollectionMethod', 'semenViscosity', 'semenAppearance', 'spermMotilityGrade', 'spermAgglutinationGrade', 'semenCultureResult'];
const ENUM_FIELDS = ['semenViscosity', 'spermMotilityGrade', 'spermAgglutinationGrade'];
const COMMA_FIELDS = ['semenCultureResult'];

const enumOptionsFor = (field, currentValue) => {
  const configured = field === 'semenViscosity'
    ? ['normal', 'increased', 'decreased']
    : field === 'spermMotilityGrade'
      ? ['A', 'B', 'C', 'D']
      : field === 'spermAgglutinationGrade'
        ? ['None', 'Slight', 'Moderate', 'Severe']
        : field === 'whoSemenAnalysisDiagnosis'
          ? ['normozoospermia', 'oligozoospermia', 'asthenozoospermia', 'teratozoospermia', 'oligoasthenoteratozoospermia', 'azoospermia', 'aspermia', 'necrozoospermia', 'leukocytospermia']
          : [];
  return [...new Set([String(currentValue || ''), ...configured].filter(Boolean))];
};
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" — array edits store the full array) */
const DRAFT_KEY = 'sperm_analysisPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const SpermAnalysisDocument = ({ document: docProp }) => {
  ...
  function reconstructFullText(sentences, originalText = '') {
    ...
    const separators = [...String(originalText).matchAll(/;\s+|(?<!\b(?:Mr|Mrs|Ms|Dr|St|Jr|Sr|Prof|Rev|Gen|Col|Sgt|vs|etc))(?<!\b[A-Z])(?<!\d)\.\s+/g)]
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Sperm Analysis ${idx + 1}`.toLowerCase();
  ...
  function stageDraft(record, fn, idx, fullText) {
    const id = safeId(record); if (!id) return;
    const editKey = `${fn}-${idx}`;
  ...
  function saveCommaItem(record, fn, idx, sid, sIdx, ciIdx) {
    ...
    const fullText = reconstructFullText(allS, currentVal);
    stageDraft(record, fn, idx, fullText);
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Staged drafts for this section's fields, keyed "field-idx" (array edits store the full array here)
      const suffix = `-${idx}`;
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/sperm_analysis/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/sperm_analysis/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (ARRAY_FIELDS.includes(f)) {
        ...
        const structured = sentences.length > 1 || sentences.some(sentence => parseLabel(sentence).isLabeled);
        if (structured) {
          text += `${label}\n${'-'.repeat(40)}\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = 'Sperm Analysis\n\n';
    pdfData.forEach((r, idx) => {
      text += `Sperm Analysis ${idx + 1}\n${'='.repeat(40)}\n\n`;
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                <button type="button" className="num-step" onClick={e => { e.stopPropagation(); const value = Number.parseFloat(editValue); const base = Number.isFinite(value) ? value : 0; const step = String(val).includes('.') ? 0.1 : 1; setEditValue(String(Number((base - step).toFixed(step < 1 ? 1 : 0)))); }}>−</button>
    ...
    return (
      <div key={fn} className="rec-mini-card nested-mini-card">
        ...
        {items.map((item, itemIdx) => {
          ...
          return (
            <div key={itemIdx} className={parsed.isLabeled ? 'field-group' : ''} style={parsed.isLabeled ? { marginTop: 4 } : undefined}>
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(parsed.isLabeled ? parsed.value : itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    <BlueSelect value={editValue} options={enumOptionsFor(fn, itemStr)} onChange={setEditValue} />
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const saveVal = parsed.isLabeled ? `${parsed.label}: ${editValue}` : editValue; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = saveVal; stageDraft(record, fn, idx, currentArr); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); setEditingField(null); setEditValue(''); setSaveError(null); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
        ...
        <div key={fn}>
          <div className="rec-mini-card nested-mini-card">
            ...
            {sentences.map((sentence, sIdx) => {
              ...
              if (parsed.isLabeled) {
                const commaItems = COMMA_FIELDS.includes(fn) ? splitByComma(parsed.value) : [parsed.value];
                const parsedLabelMatch = searchTerm.trim() && parsed.label.toLowerCase().includes(searchTerm.toLowerCase().trim());
                if (commaItems.length >= 2) {
                  return (
                    <div key={sIdx} className="field-group" style={{ marginTop: 8 }}>
              ...
              return (
                <div key={sIdx} className={parsed.isLabeled ? 'field-group' : ''} style={parsed.isLabeled ? { marginTop: 8 } : undefined}>
                  {parsed.isLabeled && <div className="nested-subtitle">{highlightText(parsed.label)}</div>}
                  <div data-edit-field={fn}>
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const original = String(getFieldValue(record, fn, idx) || ''); const sentences2 = splitBySentence(original); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2, original); stageDraft(record, fn, idx, fullText); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setEditingField(null); setEditValue(''); setSaveError(null); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
    ...
    return (
      <div key={fn} className="rec-mini-card nested-mini-card">
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(strVal); setSaveError(null); } }}>
          {isEditing ? (
            <div className="edit-field-container">
              {ENUM_FIELDS.includes(fn)
                ? <BlueSelect value={editValue} options={enumOptionsFor(fn, strVal)} onChange={setEditValue} />
                : <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />}
              {saveError && <div className="save-error">{saveError}</div>}
              <div className="edit-actions">
                <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (ENUM_FIELDS.includes(fn)) handleSaveField(record, fn, idx, sid, null, editValue); else saveSentence(record, fn, idx, sid, 0); }}>{saving ? 'Saving...' : 'Save'}</button>
                <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
              </div>
            </div>
          ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="sperm-analysis-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Sperm Analysis</h2></div>
        <div className="empty-state">No sperm analysis data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="sperm-analysis-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Sperm Analysis</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<SpermAnalysisDocumentPDFTemplate document={pdfData} />} fileName="Sperm_Analysis.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search sperm analysis..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Sperm Analysis ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'specimen')}
            {renderSection(record, idx, 'concentration')}
            {renderSection(record, idx, 'motility')}
            {renderSection(record, idx, 'morphology')}
            {renderSection(record, idx, 'vitality')}
            {renderSection(record, idx, 'antibodies')}
...
export default SpermAnalysisDocument;
