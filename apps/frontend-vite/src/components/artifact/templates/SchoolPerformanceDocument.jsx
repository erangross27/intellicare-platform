/**
 * SchoolPerformanceDocument.jsx
 * July 2026 — recursive, dot-path editable school performance
...
import SchoolPerformanceDocumentPDFTemplate from '../pdf-templates/SchoolPerformanceDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './SchoolPerformanceDocument.css';

const DRAFT_KEY = 'school_performancePendingEdits';
const SECTION_TITLES = {
  schoolInformation: 'School Information', academicPerformance: 'Academic Performance', behaviorSocial: 'Behavior & Social', strengths: 'Strengths', concerns: 'Concerns', specialEducation: 'Special Education', schoolForms: 'School Forms Completed', findings: 'Findings', results: 'Results', clinicalNotes: 'Clinical Notes',
};
const FIELD_LABELS = {
  date: 'Date', grade: 'Grade', school: 'School', provider: 'Provider', facility: 'Facility', academicPerformance: 'Academic Performance', behaviorInClass: 'Behavior in Class', peerInteractions: 'Peer Interactions', strengths: 'Strengths', concerns: 'Concerns', specialEducation: 'Special Education Services', iepOr504Plan: 'IEP/504 Plan', schoolFormsCompleted: 'School Forms Completed', findings: 'Findings', results: 'Results', assessment: 'Assessment', plan: 'Plan', recommendations: 'Recommendations', notes: 'Notes', status: 'Status',
};
const SECTION_FIELDS = {
  schoolInformation: ['date', 'grade', 'school', 'provider', 'facility'], academicPerformance: ['academicPerformance'], behaviorSocial: ['behaviorInClass', 'peerInteractions'], strengths: ['strengths'], concerns: ['concerns'], specialEducation: ['specialEducation', 'iepOr504Plan'], schoolForms: ['schoolFormsCompleted'], findings: ['findings'], results: ['results'], clinicalNotes: ['assessment', 'plan', 'recommendations', 'notes', 'status'],
};
const DATE_FIELDS = ['date'];
const ENUM_FIELDS = ['status'];
const NUMBER_FIELDS = [];
const NUMBER_STRING_FIELDS = [];
const OBJECT_FIELDS = ['schoolFormsCompleted', 'results'];
const OBJECT_ITEM_LABELS = {
  schoolFormsCompleted: 'School Form',
};
const NARRATIVE_PATHS = [];
const PARENTHETICAL_LABEL_FIELDS = [];
const COMMA_FIELDS = ['behaviorInClass', 'assessment', 'notes'];
const COMMA_ARRAY_FIELDS = ['strengths'];
const ARRAY_FIELDS = ['strengths', 'concerns'];
const SEMICOLON_FIELDS = ['academicPerformance', 'peerInteractions', 'findings', 'plan', 'notes'];
...
const enumOptionsFor = (path, value) => {
  const options = rootOf(path) === 'status' ? ['Current', 'Pending', 'Completed', 'Follow-up needed', 'Closed'] : [];
...
const SchoolPerformanceDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    return (Array.isArray(docProp) ? docProp : [docProp]).flatMap((record) => {
      if (record?.school_performance) return Array.isArray(record.school_performance) ? record.school_performance : [record.school_performance];
      if (record?.documentData) {
        const nested = record.documentData;
        if (Array.isArray(nested)) return nested;
        if (nested?.school_performance) return Array.isArray(nested.school_performance) ? nested.school_performance : [nested.school_performance];
        return [nested];
      }
      return [record];
  ...
  const approveSection = useCallback(async (record, sectionId, recordIndex) => {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        const response = await secureApiClient.put('/api/edit/school_performance/' + id + '/edit', { field: path, value: localEdits[key] });
        if (response?.success === false) throw new Error(response.error || 'Save failed');
      }
      await secureApiClient.put('/api/edit/school_performance/' + id + '/approve', { sectionId, approved: true });
      ...
      console.error('[SchoolPerformance] Approve error:', error);
  ...
  const copyAll = useCallback(async () => {
    let text = '=== SCHOOL PERFORMANCE ===\n\n';
    records.forEach((record, recordIndex) => {
      text += 'School Performance ' + (recordIndex + 1) + '\n' + '='.repeat(40) + '\n\n';
  ...
  if (!records.length) return <div className="school-performance-document" ref={containerRef}><div className="document-header"><h2 className="document-title">School Performance</h2></div><div className="empty-state">No school performance records available</div></div>;
  const visibleRecords = records.filter((record, recordIndex) => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return true;
    return ['School Performance ' + (recordIndex + 1), ...Object.keys(FIELD_LABELS).flatMap((field) => [FIELD_LABELS[field], JSON.stringify(getValue(record, field, recordIndex))])].filter(Boolean).join(' ').toLowerCase().includes(query);
  });

  return <div className="school-performance-document" ref={containerRef}>
    <div className="document-header">
      <h2 className="document-title">School Performance</h2>
      <div className="header-actions">
        <button className={'copy-btn' + (showCopied ? ' copied' : '')} onClick={copyAll}>{showCopied ? 'Copied!' : 'Copy All'}</button>
        <PDFDownloadLink document={<SchoolPerformanceDocumentPDFTemplate document={pdfData} />} fileName="School_Performance.pdf" className="copy-btn">
          {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
        </PDFDownloadLink>
      </div>
    </div>
    <div className="search-container">
      <input type="text" className="search-input" placeholder="Search school performance..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
    ...
    <div className="records-container">{visibleRecords.map((record) => {
      ...
      return <div className="record-card" key={safeId(record) || recordIndex}>
        <div className="record-header"><h3 className="record-name">{highlightText('School Performance ' + (recordIndex + 1))}</h3></div>
...
export default SchoolPerformanceDocument;
