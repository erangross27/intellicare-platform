/**
 * NeurologyProgressNotesDocument.jsx
 * March 2026 — Canonical one-pass polish, inline editing, blue glow theme
 * Collection: neurology_progress_notes
 *
 * 6 Sections:
 *   1. clinical-info: consultDate (BlueDatePicker), referralReason, chiefComplaint (first-person quote)
 *   2. exam-findings: examFindings (long compact comma list — kept whole)
 *   3. diagnosis: diagnosis
 *   4. treatment-plan: treatmentPlan
 *   5. follow-up-plan: followUpPlan (multi-sentence → splitBySentence)
 ...
 *
 * Record-date rule: NO real clinical date field (consultDate is a rendered FIELD, not a header date;
 * createdAt/updatedAt are ingestion timestamps) → title only, no record-header date pill.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NeurologyProgressNotesDocumentPDFTemplate from '../pdf-templates/NeurologyProgressNotesDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './NeurologyProgressNotesDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'neurology_progress_notesPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  'clinical-info': ['consultDate', 'referralReason', 'chiefComplaint'],
...
const STRING_FIELDS = [
  'referralReason', 'chiefComplaint', 'examFindings',
  'diagnosis', 'treatmentPlan', 'followUpPlan',
];
...
/* ═══════ COMPONENT ═══════ */
const NeurologyProgressNotesDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.neurology_progress_notes) return Array.isArray(r.neurology_progress_notes) ? r.neurology_progress_notes : [r.neurology_progress_notes];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.neurology_progress_notes) return Array.isArray(dd.neurology_progress_notes) ? dd.neurology_progress_notes : [dd.neurology_progress_notes]; return [dd]; }
      return [r];
    });
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (Array.isArray(val)) { if (val.some(item => (typeof item === 'object' ? JSON.stringify(item) : String(item)).toLowerCase().includes(phrase))) return true; }
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) {
      if (Array.isArray(val)) return val.some(item => (typeof item === 'object' ? JSON.stringify(item) : String(item)).toLowerCase().includes(phrase));
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Neurology Progress Notes ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/neurology_progress_notes/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/neurology_progress_notes/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NEUROLOGY PROGRESS NOTES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Neurology Progress Notes ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  const renderStringField = (record, fn, idx, sid) => {
    ...
    // single-name gate: when the field label equals its section title, suppress the nested-subtitle
  ...
  if (!records || records.length === 0) {
    return (
      <div className="neurology-progress-notes-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Neurology Progress Notes</h2></div>
        <div className="empty-state">No neurology progress notes records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="neurology-progress-notes-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Neurology Progress Notes</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NeurologyProgressNotesDocumentPDFTemplate document={pdfData} />} fileName="Neurology_Progress_Notes.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search neurology progress notes..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Neurology Progress Notes ${idx + 1}`)}</h3>
            </div>
...
export default NeurologyProgressNotesDocument;
