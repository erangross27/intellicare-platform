/**
 * GeriatricMedicationsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: geriatric_medications
 *
 * 4 Sections:
 *   1. medication-info: name, genericName, dosage, frequency, route
 *   2. prescriber-info: prescriber, indication, instructions
 *   3. treatment-duration: startDate, endDate, duration, durationDays, durationUnit, active, refills
 *   4. safety-info: safetyWarning, sideEffects, drugInteractions
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import GeriatricMedicationsDocumentPDFTemplate from '../pdf-templates/GeriatricMedicationsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './GeriatricMedicationsDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'medication-info': 'Medication Information',
  'prescriber-info': 'Prescriber & Instructions',
  'treatment-duration': 'Treatment Duration',
  'safety-info': 'Safety & Interactions',
};
...
const SECTION_FIELDS = {
  'medication-info': ['name', 'genericName', 'dosage', 'frequency', 'route'],
  'prescriber-info': ['prescriber', 'indication', 'instructions'],
  'treatment-duration': ['startDate', 'endDate', 'duration', 'durationDays', 'durationUnit', 'active', 'refills'],
  'safety-info': ['safetyWarning', 'sideEffects', 'drugInteractions'],
};

const NUMBER_FIELDS = ['durationDays', 'refills'];
/* durationDays 0 = indefinite / not specified for a chronic med (misleading as "0 days") → hide.
   refills 0 is a REAL prescription state (0 refills authorized) → kept + shown on the stepper. */
const HIDE_ZERO_FIELDS = ['durationDays'];
const BOOLEAN_FIELDS = ['active'];
const DATE_FIELDS = ['startDate', 'endDate'];
const ARRAY_FIELDS = ['sideEffects', 'drugInteractions'];
const ENUM_FIELDS = ['route'];
const ENUM_OPTIONS = { route: ['Oral', 'IV', 'IM', 'Subcutaneous', 'Topical', 'Transdermal', 'Sublingual', 'Rectal', 'Inhalation', 'Nasal', 'Ophthalmic', 'Otic'] };
...
const STRING_FIELDS = ['name', 'genericName', 'dosage', 'frequency', 'prescriber', 'indication', 'instructions', 'duration', 'durationUnit', 'safetyWarning'];
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; value may be scalar or array) */
const DRAFT_KEY = 'geriatric_medicationsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const GeriatricMedicationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.geriatric_medications) return Array.isArray(r.geriatric_medications) ? r.geriatric_medications : [r.geriatric_medications];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.geriatric_medications) return Array.isArray(dd.geriatric_medications) ? dd.geriatric_medications : [dd.geriatric_medications]; return [dd]; }
      return [r];
    });
  ...
  // Sentinel-zero: hide 0 on HIDE_ZERO_FIELDS (durationDays 0 = indefinite/not specified).
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Geriatric Medications ${idx + 1}`.toLowerCase();
  ...
  const stageDraft = (id, fn, fullText) => {
    const store = readDrafts();
    if (!store[id]) store[id] = {};
    store[id][fn] = fullText;
    writeDrafts(store);
  };
  const dropApproved = (sid, idx) => { if (!sid) return; setApprovedSections(prev => { const k = `${sid}-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; }); };
  ...
  const handleSaveArrayItem = useCallback((record, fn, idx, arrayIndex) => {
    ...
    // Re-edit after approval → drop the safety-info approved flag so the button returns to yellow Pending
    setApprovedSections(prev => { const k = `safety-info-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; });
    const store = readDrafts();
    if (!store[id]) store[id] = {};
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // pending editKeys ("field-idx") for THIS record whose field belongs to THIS section
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length); // "field" or, defensively, "field.arrayIndex"
      ...
      for (const editKey of toCommit) {
        ...
        if (lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1))) {
          ...
          const resp = await secureApiClient.put(`/api/edit/geriatric_medications/${id}/edit`, { field, value, arrayIndex });
          if (!resp || resp.success === false) throw new Error((resp && resp.error) || 'save failed');
        } else if (Array.isArray(value)) {
          // Array field staged as the full array → persist each element with its arrayIndex
          for (let ai = 0; ai < value.length; ai++) {
            const resp = await secureApiClient.put(`/api/edit/geriatric_medications/${id}/edit`, { field: fieldPart, value: value[ai], arrayIndex: ai });
            if (!resp || resp.success === false) throw new Error((resp && resp.error) || 'save failed');
          }
        } else {
          const resp = await secureApiClient.put(`/api/edit/geriatric_medications/${id}/edit`, { field: fieldPart, value });
          if (!resp || resp.success === false) throw new Error((resp && resp.error) || 'save failed');
        }
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/geriatric_medications/${id}/approve`, { sectionId: sid, approved: true });
      ...
      console.error('[GeriatricMedications] Approve error:', err);
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== GERIATRIC MEDICATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Geriatric Medications ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: ARRAY FIELD — editable items ═══════ */
                            ...
                            <div className={`numbered-row ${ciBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!ciEditing) { setEditingField(commaKey); setEditValue(ci); setSaveError(null); } }}>
                              {ciEditing ? (
                                <div className="edit-field-container">
                                  ...
                                  {saveError && <div className="save-error">{saveError}</div>}
                                  <div className="edit-actions">
                                    <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; const currentVal2 = String(getFieldValue(record, fn, idx) || ''); const sentences2 = splitBySentence(currentVal2); const s2 = sentences2[sIdx] || ''; const p2 = parseLabel(s2); if (!p2.isLabeled) return; const items2 = splitByComma(p2.value); const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(s => s.replace(/[;.]+$/, '').trim()).filter(s => s); if (subParts.length > 1) { items2.splice(ciIdx, 1, ...subParts); } else { items2[ciIdx] = trimmed.replace(/[;.]+$/, '').trim(); } const rebuilt = `${p2.label}: ${items2.join(', ')}.`; const allS = [...sentences2]; allS[sIdx] = rebuilt; const fullText2 = reconstructFullText(allS); const ek2 = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [ek2]: fullText2 })); setPendingEdits(prev => ({ ...prev, [ek2]: true })); const marks = { [commaKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ciIdx + ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); dropApproved(sid, idx); stageDraft(id2, fn, fullText2); setSaveError(null); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                                    <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                                  </div>
                                </div>
                              ) : (
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const id3 = safeId(record); if (!id3) return; const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); const ek3 = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [ek3]: fullText })); setPendingEdits(prev => ({ ...prev, [ek3]: true })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); dropApproved(sid, idx); stageDraft(id3, fn, fullText); setSaveError(null); setEditingField(null); setEditValue(''); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="geriatric-medications-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Geriatric Medications</h2></div>
        <div className="empty-state">No geriatric medications records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="geriatric-medications-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Geriatric Medications</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<GeriatricMedicationsDocumentPDFTemplate document={pdfData} />} fileName="Geriatric_Medications.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search geriatric medications..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Geriatric Medications ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'medication-info')}
            {renderSection(record, idx, 'prescriber-info')}
            {renderSection(record, idx, 'treatment-duration')}
...
export default GeriatricMedicationsDocument;
