/**
 * ImmunizationRecordDocument.jsx
 * June 2026 — Inline editing, blue glow theme
 * Collection: immunization_record
 *
 * 6 Sections:
 *   1. visit-overview: type, provider, facility, status
 *   2. administered-today: givenToday[], administeredToday
 *   3. vaccine-history: previousVaccines[] (items are objects {vaccine, date} → "vaccine — date")
 *   4. due-catch-up: upToDate, nextDue[], catchUpNeeded
 *   5. contraindications: contraindications[]
 *   6. clinical-notes: findings, assessment, plan, recommendations[], results{} (read-only), notes
 *
 * NOTE: this schema HAS a date field — record header shows a date badge above the numbered record title.
 *       `date` is header-only (never in a section). `results` is an object rendered as read-only rows.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import ImmunizationRecordDocumentPDFTemplate from '../pdf-templates/ImmunizationRecordDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './ImmunizationRecordDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [editKey]: { field, value, arrayIndex?, subField? } } }
   editKey is the SAME localEdits key the rest of the file already uses (e.g. "findings-0",
   "previousVaccines.1.date-0", "recommendations-0"). The stored payload is what Approve PUTs. */
const DRAFT_KEY = 'immunization_recordPendingEdits';
const readDrafts = () => {
...
const FIELD_LABELS = {
  ...
  facility: 'Facility',
  status: 'Status',
  givenToday: 'Vaccines Given Today',
  administeredToday: 'Administration Summary',
  previousVaccines: 'Previous Vaccines',
  upToDate: 'Up To Date',
  nextDue: 'Next Due',
  catchUpNeeded: 'Catch-Up Needed',
  contraindications: 'Contraindications',
  findings: 'Findings',
...
const SECTION_FIELDS = {
  'visit-overview': ['date', 'type', 'provider', 'facility', 'status'],
  'administered-today': ['givenToday', 'administeredToday'],
  'vaccine-history': ['previousVaccines'],
  'due-catch-up': ['upToDate', 'nextDue', 'catchUpNeeded'],
  'contraindications': ['contraindications'],
  'clinical-notes': ['findings', 'assessment', 'plan', 'recommendations', 'results', 'notes'],
};

const DATE_FIELDS = ['date'];
const SENTENCE_FIELDS = ['administeredToday', 'findings', 'assessment', 'plan', 'notes'];
const SIMPLE_FIELDS = ['type', 'provider', 'facility', 'status'];
const BOOLEAN_FIELDS = ['upToDate', 'catchUpNeeded'];
const ARRAY_FIELDS = ['givenToday', 'previousVaccines', 'nextDue', 'contraindications', 'recommendations'];
/* results is an object — rendered as READ-ONLY "key: value" rows (objects cannot be edited via the field/arrayIndex API) */
const OBJECT_FIELDS = ['results'];
/* NO HIDE_ZERO fields in this schema — no numeric fields (booleans, arrays, strings and one object only). */
const HIDE_ZERO_FIELDS = [];

/* parseLabel: detect "Label: value" patterns (skip subordinate-clause openers; strip a leading "N. " list marker off the value) */
...
/* splitByComma: parenthesis-aware comma split with thousands guard (only split on ", " — a comma
   followed by whitespace — so "1,000" stays intact; "COVID-19 primary series (Pfizer-BioNTech)" stays intact) */
...
/* ═══════ COMPONENT ═══════ */
const ImmunizationRecordDocument = ({ document: docProp, data, templateData }) => {
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Draft store editKeys mirror the file's localEdits / editedFields / editedSentences key conventions:
       - "<field>-<idx>"                       → whole-field value (string fields, sentence/comma full text)
       - "<field>.<arrIdx>.<subField>-<idx>"   → object-array subfield value (localEdits keyed identically)
       - "<field>.<itemIdx>-<idx>"             → plain array item; localEdits holds the WHOLE array under "<field>-<idx>" */
  useEffect(() => {
    ...
    records.forEach((record) => {
      ...
      Object.entries(recDrafts).forEach(([editKey, entry]) => {
        if (!editKey.endsWith(`-${idx}`)) return; // editKey already carries the render index
        const payload = entry && typeof entry === 'object' ? entry : { value: entry };
        const val = 'value' in payload ? payload.value : entry;
        nPending[editKey] = true;
        if (typeof payload.arrayIndex === 'number' && payload.subField === undefined) {
          // plain array item → rebuild the whole array under "<field>-<idx>", mark per-item editedFields
          const localKey = `${payload.field}-${idx}`;
          const base = Array.isArray(nLocal[localKey]) ? nLocal[localKey] : [...(Array.isArray(record[payload.field]) ? record[payload.field] : [])];
          base[payload.arrayIndex] = val;
          nLocal[localKey] = base;
          nPending[localKey] = true;
          nFields[editKey] = 'edited';
        } else {
          // whole-field or object-array subfield → localEdits keyed exactly by editKey
          nLocal[editKey] = val;
          if (editKey.includes('-s')) nSentences[editKey] = 'edited';
  ...
  /* hasFieldVal: null/undefined-safe presence check with per-field zero hiding (4-AREA RULE: used by JSX, Copy Section, Copy All and mirrored in PDF) — HIDE_ZERO_FIELDS is empty in this schema */
  ...
  /* fmtDate: Date or {$date} or string → "February 12, 2026" (header date badge only — date is never in a section) */
  ...
  /* toInputDate: Date or {$date} or string → "yyyy-mm-dd" for the BlueDatePicker */
  const toInputDate = useCallback((dateVal) => { if (!dateVal) return ''; try { const d = new Date(dateVal.$date || dateVal); if (isNaN(d.getTime())) return ''; return d.toISOString().split('T')[0]; } catch { return ''; } }, []);
  ...
  /* stageDraft — record a Save as a DRAFT (no DB write). Stores the exact PUT payload Approve will send.
     payload = { field, value, arrayIndex?, subField? }. localStorage keeps it across refresh. */
  const stageDraft = useCallback((record, editKey, payload) => {
    const id = safeId(record); if (!id) return;
    const store = readDrafts();
    if (!store[id]) store[id] = {};
    store[id][editKey] = payload;
    writeDrafts(store);
  }, [safeId]);

  /* prettySubLabel: vaccine → "Vaccine", lotNumber → "Lot Number" (object-array subfield labels) */
  const prettySubLabel = useCallback((k) => String(k).replace(/([a-z])([A-Z])/g, '$1 $2').replace(/^./, c => c.toUpperCase()), []);
  /* flattenValue: scalar / array / nested object → readable text for Copy + search (no [object Object]) */
  const flattenValue = useCallback((v) => {
    ...
    return String(v);
  }, [fmtVal, prettySubLabel]);
  /* getObjectArrayItemValue: object-array subfield value respecting localEdits (key: fn.arrIdx.subField-idx) */
  const getObjectArrayItemValue = useCallback((record, fn, idx, arrIdx, subField) => {
    ...
    if (!Array.isArray(arr) || !arr[arrIdx] || typeof arr[arrIdx] !== 'object') return undefined;
  ...
  const valueMatchesPhrase = useCallback((fn, val, phrase) => {
    ...
    if (Array.isArray(val)) {
      return val.some(item => (item && typeof item === 'object' && !Array.isArray(item) ? flattenValue(item) : arrItemText(item)).toLowerCase().includes(phrase));
    }
  ...
  }, [fmtVal, arrItemText, objEntryTexts, flattenValue]);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      const docTitle = 'immunization record';
      const rt = `immunization record ${oi + 1}`;
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    stageDraft(record, localKey, { field: fn, value: saveVal });
    setEditingField(null); setEditValue('');
  }, [editValue, safeId, stageDraft]);

  /* handleSaveObjectArrayItem: saves ONE object-array subfield via arrayIndex+subField — preserves {…} item shape (never clobbers the object with a flat string) */
  const handleSaveObjectArrayItem = useCallback((record, fn, idx, arrIdx, subField, sid, value) => {
    const id = safeId(record); if (!id) return;
    setSaveError(null);
    const lk = `${fn}.${arrIdx}.${subField}-${idx}`;
    setLocalEdits(prev => ({ ...prev, [lk]: value }));
    setPendingEdits(prev => ({ ...prev, [lk]: true }));
    setEditedFields(prev => ({ ...prev, [lk]: 'edited' }));
    setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
    stageDraft(record, lk, { field: fn, value, arrayIndex: arrIdx, subField });
    setEditingField(null); setEditValue('');
  }, [safeId, stageDraft]);
  ...
  function saveSentence(record, fn, idx, sid, sentenceIdx) {
    ...
    if (!editedVal || /^[;.,!?]+$/.test(editedVal)) {
      ...
      setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
      stageDraft(record, localKey, { field: fn, value: fullText });
      setEditingField(null); setEditValue('');
      return;
    }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    // Which draft editKeys belong to THIS section + record? (keys are "<field>-<idx>" or "<field>.<sub>-<idx>")
    const belongs = (k) => fields.some(f => k.startsWith(`${f}-${idx}`) || (k.startsWith(`${f}.`) && k.endsWith(`-${idx}`)));
    const store = readDrafts();
    const recDrafts = store[id] || {};
    const toCommit = Object.keys(recDrafts).filter(belongs);
    try {
      // Persist each staged field to the DB now (payload was captured at Save time: field/value [+arrayIndex/subField])
      for (const editKey of toCommit) {
        const payload = recDrafts[editKey];
        if (!payload || typeof payload !== 'object') continue;
        await secureApiClient.put(`/api/edit/immunization_record/${id}/edit`, payload);
      }
      await secureApiClient.put(`/api/edit/immunization_record/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending → committed edits now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; Object.keys(n).forEach(k => { if (belongs(k)) delete n[k]; }); return n; });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        const rawItems = (Array.isArray(val) ? val : [val]).filter(item => item !== null && item !== undefined && item !== '' && (typeof item !== 'object' || Object.keys(item).length > 0));
        if (rawItems.length === 0) return;
        if (showLabel) text += `${label}\n`;
        rawItems.forEach((item, i) => {
          if (item && typeof item === 'object' && !Array.isArray(item)) {
            const keys = Object.keys(item).filter(k => item[k] !== null && item[k] !== undefined && item[k] !== '');
            const titleKey = keys.find(k => typeof item[k] === 'string' || typeof item[k] === 'number');
            const titleVal = titleKey ? fmtVal(item[titleKey]) : `${label} ${i + 1}`;
            text += `${i + 1}. ${titleVal}\n`;
            keys.filter(k => k !== titleKey).forEach(k => { text += `   ${prettySubLabel(k)}: ${flattenValue(item[k])}\n`; });
          } else {
            text += `${i + 1}. ${arrItemText(item)}\n`;
          }
        });
        text += '\n';
    ...
    return text;
  }, [getFieldValue, hasFieldVal, fmtVal, fmtDate, splitBySentence, splitBySemicolon, formatSentenceFieldLines, arrItemText, objEntryTexts, prettySubLabel, flattenValue]);

  const copyAllText = useCallback(async () => {
    let text = '=== IMMUNIZATION RECORD ===\n\n';
    pdfData.forEach((r) => {
      const oi = r._originalIdx ?? 0;
      text += `Immunization Record ${oi + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: DATE FIELD (BlueDatePicker — record.date, never createdAt/updatedAt) ═══════ */
  ...
  /* ═══════ RENDER: SIMPLE FIELD (no splitting — visit type, provider, facility, status) ═══════ */
  ...
  /* ═══════ RENDER: ARRAY FIELD (per-item editing with arrayIndex — items render VERBATIM, never split; object items like {vaccine, date} render via arrItemText as "vaccine — date") ═══════ */
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {items.map((item, itemIdx) => {
          /* ─── OBJECT item (e.g. {vaccine, dose} / {vaccine, date} / {vaccine, dueDate}): per-subfield editable rows.
                 Editing a subfield saves via arrayIndex+subField so the {…} item shape is PRESERVED (no corruption). ─── */
          if (item && typeof item === 'object' && !Array.isArray(item)) {
            const keys = Object.keys(item).filter(k => item[k] !== null && item[k] !== undefined && item[k] !== '' && !(Array.isArray(item[k]) && item[k].length === 0));
            if (keys.length === 0) return null;
            /* first scalar key = item subtitle (vaccine name) */
            const titleKey = keys.find(k => typeof item[k] === 'string' || typeof item[k] === 'number');
            const titleVal = titleKey ? fmtVal(item[titleKey]) : '';

            if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
              const phrase = searchTerm.toLowerCase().trim();
              const labelLower = label.toLowerCase();
              const combined = flattenValue(item).toLowerCase();
              if (!labelLower.includes(phrase) && !phrase.includes(labelLower) && !combined.includes(phrase)) return null;
            }

            /* READ-ONLY decomposition: vaccine name as a subtitle, each remaining key = its own subtitle + a
               read-only value row (never side-by-side, never inline-editable — array-of-objects are display-only). */
            return (
              <div key={itemIdx} className="rec-mini-card" style={{ marginTop: 4 }}>
                <div className="nested-subtitle">{highlightText(titleVal || `${label} ${itemIdx + 1}`)}</div>
                {keys.map(sk => {
                  if (sk === titleKey) return null; /* shown as subtitle */
                  const subKey = `${fn}.${itemIdx}.${sk}-${idx}`;
                  const subDisplay = flattenValue(item[sk]);
                  if (!subDisplay) return null;
                  return (
                    <div key={sk}>
                      <div className="nested-subtitle" style={{ fontSize: 14, marginTop: 4, borderBottom: 'none', paddingBottom: 0, marginBottom: 2 }}>{highlightText(prettySubLabel(sk))}</div>
                      <div className="numbered-row">
                        <div className="row-content"><span className="content-value">{highlightText(subDisplay)}</span></div>
                        <button className={`copy-btn ${copiedItems[subKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(subDisplay, subKey); }}>{copiedItems[subKey] ? 'Copied!' : 'Copy'}</button>
                      </div>
                    </div>
                  );
                })}
          ...
          /* ─── STRING item (contraindications / recommendations) ─── */
          ...
          const saveArrayItem = () => { const id2 = safeId(record); if (!id2) return; setSaveError(null); const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = editValue; setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: currentArr })); setPendingEdits(prev => ({ ...prev, [editKey]: true, [`${fn}-${idx}`]: true })); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); stageDraft(record, editKey, { field: fn, value: editValue, arrayIndex: itemIdx }); setEditingField(null); setEditValue(''); };
        ...
        <div key={fn}>
          <div className="rec-mini-card">
            ...
            {sentences.map((sentence, sIdx) => {
              ...
              const saveThisSentence = () => { if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const currentSentences = isSemicolon ? splitBySemicolon(String(getFieldValue(record, fn, idx) || '')) : splitBySentence(String(getFieldValue(record, fn, idx) || '')); currentSentences[sIdx] = reconstructed; const fullText = reconstructFullText(currentSentences, isSemicolon); const id3 = safeId(record); if (!id3) return; setSaveError(null); const localKey = `${fn}-${idx}`; setLocalEdits(prev => ({ ...prev, [localKey]: fullText })); setPendingEdits(prev => ({ ...prev, [localKey]: true })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); stageDraft(record, localKey, { field: fn, value: fullText }); setEditingField(null); setEditValue(''); } else { saveSentence(record, fn, idx, sid, sIdx); } };
              return (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="immunization-record-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Immunization Record</h2></div>
        <div className="empty-state">No immunization records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="immunization-record-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Immunization Record</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<ImmunizationRecordDocumentPDFTemplate document={pdfData} />} fileName="Immunization_Record.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search immunization record..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => {
          ...
          return (
            <div key={safeId(record) || idx} className="record-card">
              {/* record.date is an editable Visit Overview field (renderDateField) — no header meta pill, never createdAt/updatedAt */}
              <div className="record-header">
                <h3 className="record-name">{highlightText(`Immunization Record ${origIdx + 1}`)}</h3>
              </div>
              {renderSection(record, origIdx, 'visit-overview')}
              {renderSection(record, origIdx, 'administered-today')}
              {renderSection(record, origIdx, 'vaccine-history')}
              {renderSection(record, origIdx, 'due-catch-up')}
              {renderSection(record, origIdx, 'contraindications')}
              {renderSection(record, origIdx, 'clinical-notes')}
            </div>
          );
        })}
      </div>
...
export default ImmunizationRecordDocument;
