/**
 * PneumoperitoneumDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: pneumoperitoneum
 *
 * 8 Sections:
 *   1. procedure-info: procedureName, date, facility
 *   2. access-details: accessMethod, accessLocation
 *   3. pressure-settings: initialPressure, targetPressure, maximumPressure
 *   4. gas-settings: gasType, totalGasVolume, flowRate, insufflationTime
 *   5. equipment: insufflationEquipment, desufflationMethod
 *   6. findings: visualInspection, adhesions, complications
 *   7. verification-tests: verificationTests
 *   8. additional: specialConsiderations, notes
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PneumoperitoneumDocumentPDFTemplate from '../pdf-templates/PneumoperitoneumDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './PneumoperitoneumDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "field.arrayIndex") */
const DRAFT_KEY = 'pneumoperitoneumPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  'procedure-info': ['procedureName', 'date', 'facility'],
  'access-details': ['accessMethod', 'accessLocation'],
  'pressure-settings': ['initialPressure', 'targetPressure', 'maximumPressure'],
  'gas-settings': ['gasType', 'totalGasVolume', 'flowRate', 'insufflationTime'],
  'equipment': ['insufflationEquipment', 'desufflationMethod'],
  'findings': ['visualInspection', 'adhesions', 'complications'],
  'verification-tests': ['verificationTests'],
  'additional': ['specialConsiderations', 'notes'],
};

const BOOLEAN_FIELDS = [];
const DATE_FIELDS = ['date'];
const ARRAY_FIELDS = ['complications', 'verificationTests'];
const STRING_FIELDS = ['procedureName', 'facility', 'accessMethod', 'accessLocation', 'initialPressure', 'targetPressure', 'maximumPressure', 'gasType', 'totalGasVolume', 'flowRate', 'insufflationTime', 'insufflationEquipment', 'desufflationMethod', 'visualInspection', 'adhesions', 'specialConsiderations', 'notes'];
...
/* ═══════ COMPONENT ═══════ */
const PneumoperitoneumDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.pneumoperitoneum) return Array.isArray(r.pneumoperitoneum) ? r.pneumoperitoneum : [r.pneumoperitoneum];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.pneumoperitoneum) return Array.isArray(dd.pneumoperitoneum) ? dd.pneumoperitoneum : [dd.pneumoperitoneum]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pneumoperitoneum ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // Collect this section's staged drafts (store fieldPart = "field" or "field.arrayIndex")
      const store = readDrafts();
      const recDrafts = store[id] || {};
      const committedLocalKeys = [];
      for (const [fieldPart, draftVal] of Object.entries(recDrafts)) {
        const lastDot = fieldPart.lastIndexOf('.');
        const isArrayElem = lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1));
        ...
        const payload = { field: baseField, value: draftVal };
        if (isArrayElem) payload.arrayIndex = parseInt(fieldPart.slice(lastDot + 1), 10);
        const resp = await secureApiClient.put(`/api/edit/pneumoperitoneum/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedLocalKeys.push(`${baseField}-${idx}`);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/pneumoperitoneum/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (store2[id]) { fields.forEach(f => { Object.keys(store2[id]).forEach(fp => { const ld = fp.lastIndexOf('.'); const bf = (ld !== -1 && /^\d+$/.test(fp.slice(ld + 1))) ? fp.slice(0, ld) : fp; if (bf === f) delete store2[id][fp]; }); }); if (Object.keys(store2[id]).length === 0) delete store2[id]; writeDrafts(store2); }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PNEUMOPERITONEUM ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Pneumoperitoneum ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pneumoperitoneum-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pneumoperitoneum</h2></div>
        <div className="empty-state">No pneumoperitoneum records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pneumoperitoneum-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Pneumoperitoneum</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PneumoperitoneumDocumentPDFTemplate document={pdfData} />} fileName={`pneumoperitoneum-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pneumoperitoneum records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(record.procedureName || `Pneumoperitoneum ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'procedure-info')}
            {renderSection(record, idx, 'access-details')}
            {renderSection(record, idx, 'pressure-settings')}
            {renderSection(record, idx, 'gas-settings')}
            {renderSection(record, idx, 'equipment')}
            {renderSection(record, idx, 'findings')}
            {renderSection(record, idx, 'verification-tests')}
...
export default PneumoperitoneumDocument;
