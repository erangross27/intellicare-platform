/**
 * IntelligentRecommendationsDocumentPDFTemplate.jsx
 * June 2026 — Helvetica — A4 — BLACK & WHITE only (#000000 titles/borders/values, NO blue).
 * Collection: intelligent_recommendations.
 *
 ...
 * ARRAY fields (immediate/shortTerm/longTerm/preventive/psychoBehavioralSupport/recommendations) → numbered list.
 */
...
const FIELD_LABELS = {
  ...
  immediate: 'Immediate', shortTerm: 'Short Term', longTerm: 'Long Term',
  preventive: 'Preventive', psychoBehavioralSupport: 'Psycho-Behavioral Support',
  findings: 'Findings', assessment: 'Assessment', plan: 'Plan',
  results: 'Results', recommendations: 'Recommendations',
  notes: 'Notes', status: 'Status',
};
const SECTION_FIELDS = {
  'recommendations-by-timeframe': ['immediate', 'shortTerm', 'longTerm', 'preventive', 'psychoBehavioralSupport'],
  'clinical': ['findings', 'assessment', 'plan', 'results', 'recommendations'],
  'notes-status': ['notes', 'status'],
};
const SECTION_ORDER = ['recommendations-by-timeframe', 'clinical', 'notes-status'];
const DATE_FIELDS = ['date'];
const STRING_FIELDS = ['provider', 'facility', 'findings', 'assessment', 'plan', 'notes', 'status'];
const OBJECT_FIELDS = ['results'];
const ARRAY_FIELDS = ['immediate', 'shortTerm', 'longTerm', 'preventive', 'psychoBehavioralSupport', 'recommendations'];
...
const flattenLine = (v) => {
  ...
  return Object.entries(v).filter(([, val]) => !isEmptyDeep(val)).map(([k, val]) => `${humanizeKey(k)}: ${flattenLine(val)}`).join('; ');
};
  ...
  if (ARRAY_FIELDS.includes(field)) {
    ...
    return [(
      <View key={field} style={styles.fieldGroup} wrap={items.length > 8 ? undefined : false}>
        {titleNode}
        {showLabel && <Text style={styles.fieldLabel}>{label}</Text>}
        {items.map((item, i) => (<Text key={i} style={styles.value}>{i + 1}. {flattenLine(item)}</Text>))}
      </View>
    )];
  }
...
const IntelligentRecommendationsDocumentPDFTemplate = ({ document: data }) => {
  let records = [];
  if (Array.isArray(data)) {
    if (data.length === 1 && data[0]?.intelligent_recommendations) records = Array.isArray(data[0].intelligent_recommendations) ? data[0].intelligent_recommendations : [data[0].intelligent_recommendations];
    else records = data;
  } else if (data?.intelligent_recommendations) records = Array.isArray(data.intelligent_recommendations) ? data.intelligent_recommendations : [data.intelligent_recommendations];
  else if (data?.documentData) { const dd = data.documentData; if (Array.isArray(dd)) records = dd; else if (dd?.intelligent_recommendations) records = Array.isArray(dd.intelligent_recommendations) ? dd.intelligent_recommendations : [dd.intelligent_recommendations]; else if (dd && typeof dd === 'object') records = [dd]; }
  else if (data && typeof data === 'object') records = [data];
  records = (records || []).filter(r => r && typeof r === 'object');

  if (records.length === 0) {
    return (<Document><Page size="A4" style={styles.page}><View style={styles.documentHeader}><Text style={styles.title}>Intelligent Recommendations</Text></View><Text style={styles.emptyState}>No records available</Text></Page></Document>);
  }

  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.title}>Intelligent Recommendations</Text></View>
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer}>
            <View style={styles.recordHeader} wrap={false}>
              <Text style={styles.recordTitle}>{`Intelligent Recommendations ${String(record._recordNumber || idx + 1)}`}</Text>
...
export default IntelligentRecommendationsDocumentPDFTemplate;
