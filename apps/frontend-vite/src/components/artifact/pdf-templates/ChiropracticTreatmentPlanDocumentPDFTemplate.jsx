/**
 * ChiropracticTreatmentPlanDocumentPDFTemplate.jsx
 ...
 * Collection: chiropractic_treatment_plan
 */
...
const FL = {
  ...
  highVelocityLowAmplitudeIndicated: 'HVLA Indicated', treatmentFrequencyPerWeek: 'Frequency/Week', totalVisitsAuthorized: 'Total Visits', treatmentPhase: 'Treatment Phase', reassessmentIntervalWeeks: 'Reassessment (weeks)', maximumMedicalImprovementDate: 'Maximum Medical Improvement Date',
...
const renderFieldUnit = (fn, value, sTitle, isFirst) => {
  ...
  if (TEXT_BLOCK_FIELDS.has(fn)) {
    ...
    inner = (
      <View style={{ marginBottom: 6 }}>
        {showFieldLabel && <Text style={styles.fieldLabel}>{FL[fn] || fn}</Text>}
        <Text style={styles.listItem}>1. {DATE_FIELDS.has(fn) ? formatDate(value) : fmtVal(value)}</Text>
      </View>
    );
  }
  return (
...
const ChiropracticTreatmentPlanDocumentPDFTemplate = ({ document: templateData }) => {
  ...
  if (!records || records.length === 0) {
    return (<Document><Page size="LETTER" style={styles.page}><View style={styles.documentHeader}><Text style={styles.title}>Chiropractic Treatment Plan</Text></View><Text style={styles.emptyState}>No records available</Text></Page></Document>);
  }

  return (
    <Document>
      <Page size="LETTER" style={styles.page}>
        <View style={styles.documentHeader}><Text style={styles.title}>Chiropractic Treatment Plan</Text></View>
        {records.map((record, idx) => (
          <View key={idx} style={styles.recordContainer}>
            <View style={styles.recordHeader} wrap={false}>
              <Text style={styles.recordTitle}>{`Chiropractic Treatment Plan ${idx + 1}`}</Text>
            ...
            {renderFieldSection('Clinical Scores', ['painVisualAnalogScale', 'oswestryDisabilityIndex', 'neckDisabilityIndex', 'cobbAngleMeasurement'], record)}
            {renderArraySection('Subluxation Levels', record.subluxationLevels)}
            {renderArraySection('Spinal Regions', record.spinalRegionsInvolved)}
            {renderArraySection('Adjustment Techniques', record.adjustmentTechniques)}
            {renderFieldSection('Range of Motion', ['rangeOfMotionCervical', 'rangeOfMotionLumbar'], record)}
            {renderFieldSection('Neurological Exam', ['muscleStrengthGrading', 'deepTendonReflexFindings', 'dermatomeDistribution'], record)}
            {renderArraySection('Orthopedic Tests', record.orthopedicTestResults)}
            {renderFieldSection('Treatment Schedule', ['highVelocityLowAmplitudeIndicated', 'treatmentFrequencyPerWeek', 'totalVisitsAuthorized', 'treatmentPhase', 'reassessmentIntervalWeeks', 'maximumMedicalImprovementDate'], record)}
            {renderArraySection('Adjunctive Therapies', record.adjunctiveTherapiesOrdered)}
            {renderFieldSection('Exercise & Ergonomics', ['homeExerciseProgramPrescribed', 'ergonomicRecommendations'], record)}
            {renderArraySection('Functional Outcome Goals', record.functionalOutcomeGoals)}
...
export default ChiropracticTreatmentPlanDocumentPDFTemplate;
