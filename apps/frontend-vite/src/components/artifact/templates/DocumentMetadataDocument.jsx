/**
 * DocumentMetadataDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: document_metadata
 *
 * 8 Sections:
 *   1. document-info: documentType, specialtyService, urgencyLevel
 *   2. patient-info: patientMedicalRecordNumber, encounterNumber
 *   3. diagnoses: primaryDiagnosis, secondaryDiagnoses[]
 *   4. codes-section: icd10Codes[], cptCodes[]
 *   5. clinical-scores: apatiteScore(num), glasgowComaScale(num), nyhaClass, charlsonComorbidityIndex(num), ankleBrachialIndex(num), bodyMassIndex(num), creatinineClearance(num), ejectionFraction(num)
 *   6. procedures-section: procedurePerformed, anesthesiaType, pathologyGrade, radiologyModality
 *   7. lab-allergies: laboratoryPanels[], medicationAllergies[]
 *   8. infection-control: infectiousDisease, isolationPrecautions
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import DocumentMetadataDocumentPDFTemplate from '../pdf-templates/DocumentMetadataDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './DocumentMetadataDocument.css';
...
const DRAFT_KEY = 'document_metadataPendingEdits';
const readDrafts = () => {
...
const SECTION_TITLES = {
  ...
  'lab-allergies': 'Laboratory & Allergies',
  'infection-control': 'Infection Control',
};

const FIELD_LABELS = {
  documentType: 'Document Type',
  specialtyService: 'Specialty Service',
  ...
  pathologyGrade: 'Pathology Grade',
  radiologyModality: 'Radiology Modality',
...
/* Number fields where 0 means "not measured/not recorded" (biologically impossible or a known sentinel) and should be hidden.
   charlsonComorbidityIndex is NOT listed: a Charlson score of 0 is a meaningful clinical value (no comorbidities). */
...
/* −/+ stepper step = the value's OWN precision (decimal-aware): integer→1, 0.1-value→0.1, etc. */
const stepFor = (v) => { const d = String(v ?? '').split('.')[1]; return d ? '0.' + '0'.repeat(Math.max(0, d.length - 1)) + '1' : '1'; };

/* Known clinical enums → full-scale dropdown (BlueSelect). Stored value kept via enumOptionsWith;
   display normalized to canonical casing (edit-widget-only — stored shape unchanged). */
const ENUM_FIELDS = {
...
/* ═══════ COMPONENT ═══════ */
const DocumentMetadataDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.document_metadata) return Array.isArray(r.document_metadata) ? r.document_metadata : [r.document_metadata];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.document_metadata) return Array.isArray(dd.document_metadata) ? dd.document_metadata : [dd.document_metadata]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Document Metadata ${idx + 1}`.toLowerCase();
  ...
  /* ═══════ EDIT HANDLERS ═══════ */
  // The localStorage fieldPart for a field is just the field name (array fields store the whole array).
  // Stage a DRAFT locally + persist it to the pending-drafts store. NOT written to MongoDB and NOT
  ...
  const handleSaveArrayItem = useCallback((record, fn, idx, arrIdx, value) => {
    ...
    setSaveError(null);
    stageDraft(record, fn, idx, currentArr, editKey);
  }, [safeId, getFieldValue, stageDraft]);

  // Stage a sentence edit as a DRAFT (no DB write). Approve commits it.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const suffix = `-${idx}`;
      // localEdits keys are "field-idx" (array fields store the whole array). Commit each pending field
      // belonging to this section.
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k] || !k.endsWith(suffix)) return false;
        const fieldPart = k.slice(0, -suffix.length); // "field" (no numeric array index suffix here)
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/document_metadata/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/document_metadata/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[DocumentMetadata] Approve error:', err); setSaveError('Approve failed. Please try again.'); }
  ...
  const dispVal = useCallback((fn, val) => ENUM_FIELDS[fn] ? (enumCanonical(ENUM_FIELDS[fn], val) || fmtVal(val)) : fmtVal(val), [fmtVal]);
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    const present = fields.filter(f => {
      ...
      if (STRING_ARRAY_FIELDS.includes(f)) return Array.isArray(v) && v.length > 0;
      return hasVal(v) && !isSuppressedZero(f, v);
    });
    ...
    return text;
  }, [getFieldValue, hasVal, fmtVal, dispVal, formatSentenceFieldLines]);

  const copyAllText = useCallback(async () => {
    let text = `Document Metadata\n${'='.repeat(40)}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Document Metadata ${idx + 1}\n${'='.repeat(40)}\n\n`;
          ...
          {isEditing ? (
            <div className="edit-field-container">
              <div className="num-stepper-row">
                ...
                <input type="number" className="edit-number" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus step={stepFor(editValue)} min="0" onClick={e => e.stopPropagation()} onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } if (e.key === 'Enter') { e.preventDefault(); const parsed = parseFloat(editValue); if (isNaN(parsed)) { setSaveError('Please enter a valid number'); return; } handleSaveField(record, fn, idx, sid, null, parsed); } }} />
                <button type="button" className="num-step" disabled={saving} onClick={e => { e.stopPropagation(); const st = parseFloat(stepFor(editValue)) || 1; const dec = (String(st).split('.')[1] || '').length; const cur = parseFloat(editValue); setEditValue(Math.max(0, (isNaN(cur) ? 0 : cur) + st).toFixed(dec)); }}>+</button>
              </div>
  ...
  const renderStringArrayField = (record, idx, fn, sid) => {
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {arr.map((item, arrIdx) => {
          ...
          const isBadge = editedFields[arrEditKey];

          return (
            <div key={arrIdx}>
              <div className={`numbered-row ${isBadge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(arrEditKey); setEditValue(itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                    {saveError && <div className="save-error">{saveError}</div>}
  ...
  /* ═══════ RENDER: ENUM FIELD — themed BlueSelect dropdown ═══════ */
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      ...
      if (isSuppressedZero(f, val)) return false;
      return hasVal(val);
    });
    ...
    return (
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            if (STRING_ARRAY_FIELDS.includes(f)) return renderStringArrayField(record, idx, f, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="document-metadata-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Document Metadata</h2></div>
        <div className="empty-state">No document metadata records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="document-metadata-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Document Metadata</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<DocumentMetadataDocumentPDFTemplate document={pdfData} />} fileName="Document_Metadata.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search document metadata..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Document Metadata ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'document-info')}
            {renderSection(record, idx, 'patient-info')}
            {renderSection(record, idx, 'diagnoses')}
            {renderSection(record, idx, 'codes-section')}
            {renderSection(record, idx, 'clinical-scores')}
            {renderSection(record, idx, 'procedures-section')}
            {renderSection(record, idx, 'lab-allergies')}
...
export default DocumentMetadataDocument;
