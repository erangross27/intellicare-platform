/**
 * FollowUpAppointmentsDocument.jsx
 * Canonical rebuild (July 2026) — was a bespoke Dec-2025 template (side-by-side copy, non-canonical rows,
 * native select dropdowns, 0 harness-probed fields). Now the standard one-pass shape: rec-mini-card rows with
 * .numbered-row.editable-row, BlueDatePicker/BlueSelect widgets, single-name gate, canonical EQ/DASH numbered
 * copy, standard Pending-Approve wiring, box-free PDF. Multi-record: each appointment renders as its own card.
 * Collection: follow_up_appointments
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import FollowUpAppointmentsDocumentPDFTemplate from '../pdf-templates/FollowUpAppointmentsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './FollowUpAppointmentsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Pending Approve. */
const DRAFT_KEY = 'follow_up_appointmentsPendingEdits';
...
const SECTION_TITLES = {
  ...
  'details': 'Details',
  'scheduling-flags': 'Scheduling Flags',
  'notes-section': 'Notes',
};
const FIELD_LABELS = {
  specialty: 'Specialty', provider: 'Provider', date: 'Date', appointmentDate: 'Appointment Date',
  time: 'Time', appointmentTime: 'Appointment Time', timing: 'Timing', status: 'Status', type: 'Type',
  reason: 'Reason', location: 'Location', facility: 'Facility', instructions: 'Instructions', requiresPrep: 'Requires Prep',
  isScheduled: 'Scheduled', requiresReferral: 'Requires Referral', labsBeforeVisits: 'Labs Before Visit',
  notes: 'Notes',
};
const SECTION_FIELDS = {
  'appointment-info': ['specialty', 'provider', 'date', 'appointmentDate', 'time', 'appointmentTime', 'timing', 'status', 'type'],
  'details': ['reason', 'location', 'facility', 'instructions', 'requiresPrep'],
  'scheduling-flags': ['isScheduled', 'requiresReferral', 'labsBeforeVisits'],
  'notes-section': ['notes'],
};
const SECTION_ORDER = ['appointment-info', 'details', 'scheduling-flags', 'notes-section'];

const DATE_FIELDS = ['date', 'appointmentDate'];
const BOOLEAN_FIELDS = ['isScheduled', 'requiresReferral', 'labsBeforeVisits'];
const ENUM_FIELDS = ['status', 'type'];
const ENUM_OPTIONS = {
  status: ['Scheduled', 'Completed', 'Cancelled', 'No Show', 'Rescheduled', 'Pending'],
  type: ['Consultation', 'Follow-Up', 'Procedure', 'Lab', 'Imaging', 'Screening', 'Other'],
};
...
/* Free-text narrative (multi-sentence split) */
const STRING_FIELDS = ['specialty', 'provider', 'time', 'appointmentTime', 'timing', 'reason', 'location', 'facility', 'instructions', 'requiresPrep', 'notes'];
...
const FollowUpAppointmentsDocument = ({ document: docProp, data }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.follow_up_appointments) return Array.isArray(r.follow_up_appointments) ? r.follow_up_appointments : [r.follow_up_appointments];
      if (r?.records) return Array.isArray(r.records) ? r.records : [r.records];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.follow_up_appointments) return Array.isArray(dd.follow_up_appointments) ? dd.follow_up_appointments : [dd.follow_up_appointments]; return [dd]; }
      return [r];
    });
  ...
  const splitBySentence = useCallback((text) => {
    ...
    /* (?<!\b[A-Z]) guards single-letter initials so "Dr. R. Vashisht" never splits after "R." */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Follow Up Appointment ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const [fn, value] of Object.entries(recDrafts)) {
        ...
        const resp = await secureApiClient.put(`/api/edit/follow_up_appointments/${id}/edit`, { field: fn, value });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/follow_up_appointments/${id}/approve`, { sectionId: sid, approved: true });
    ...
    } catch (err) { console.error('[FollowUpAppointments] Approve error:', err); }
  ...
  const copyAllText = useCallback(async () => {
    let text = `Follow Up Appointments\n${COPY_LINE_EQ}\n\n`;
    pdfData.forEach((r, idx) => {
      text += `Follow Up Appointment ${idx + 1}\n${COPY_LINE_EQ}\n\n`;
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    const hasAnyVal = fields.some(f => { const v = getFieldValue(record, f, idx); return BOOLEAN_FIELDS.includes(f) ? (v !== null && v !== undefined && v !== '') : hasVal(v); });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="follow-up-appointments-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Follow Up Appointments</h2></div>
        <div className="empty-state">No follow up appointments records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="follow-up-appointments-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Follow Up Appointments</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<FollowUpAppointmentsDocumentPDFTemplate document={pdfData} />} fileName="Follow_Up_Appointments.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search follow up appointments..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">Follow Up Appointment {idx + 1}</h3>
            </div>
...
export default FollowUpAppointmentsDocument;
