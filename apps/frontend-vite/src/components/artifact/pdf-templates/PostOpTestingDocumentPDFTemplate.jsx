/**
 * PostopTestingDocumentPDFTemplate.jsx
 * June 2026 — Helvetica — LETTER size — post-operative testing
 * Collection: postop_testing
  ...
  if (typeof val === 'object') {
    return Object.entries(val)
      ...
      .map(([k, v]) => {
        ...
        return `${label}: ${(v && typeof v === 'object') ? safeString(v) : String(v)}`;
      })
      .join(', ');
  }
  return String(val);
...
const SECTION_CONFIGS = [
  {
    title: 'Pain & Wound',
    fields: [
      { key: 'painScore', label: 'Pain Score (0-10)', isNumber: true },
      { key: 'woundHealing', label: 'Wound Healing', isSentence: true },
      { key: 'drainOutput', label: 'Drain Output', isSentence: true },
      { key: 'surgicalSiteComplications', label: 'Surgical Site Complications', isArray: true },
    ],
  },
  {
    title: 'Labs & Imaging',
    fields: [
      ...
      { key: 'coagulationStudies', label: 'Coagulation Studies', isSentence: true },
      { key: 'electrolyteLevels', label: 'Electrolyte Levels', isSentence: true },
      { key: 'renalFunction', label: 'Renal Function', isSentence: true },
      { key: 'liverFunction', label: 'Liver Function', isSentence: true },
    ],
  },
  {
    title: 'Systems Review',
    fields: [
      { key: 'respiratoryFunction', label: 'Respiratory Function', isSentence: true },
      { key: 'cardiovascularMonitoring', label: 'Cardiovascular Monitoring', isSentence: true },
      { key: 'neurologicalExam', label: 'Neurological Exam', isSentence: true },
      { key: 'infectionSurveillance', label: 'Infection Surveillance', isSentence: true },
      { key: 'thromboembolicScreening', label: 'Thromboembolic Screening', isSentence: true },
    ],
  },
  {
    title: 'Recovery & Function',
    fields: [
      { key: 'mobilityAssessment', label: 'Mobility Assessment', isSentence: true },
      { key: 'functionalCapacity', label: 'Functional Capacity', isSentence: true },
      { key: 'anesthesiaRecovery', label: 'Anesthesia Recovery', isSentence: true },
...
/* ======= COMPONENT ======= */
const PostopTestingDocumentPDFTemplate = ({ document: data }) => {
  const records = React.useMemo(() => {
    if (!data) return [];
    let arr = Array.isArray(data) ? data : [data];
    arr = arr.flatMap(r => {
      if (r?.postop_testing) return Array.isArray(r.postop_testing) ? r.postop_testing : [r.postop_testing];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.postop_testing) return Array.isArray(dd.postop_testing) ? dd.postop_testing : [dd.postop_testing]; return [dd]; }
      return [r];
    });
      ...
      <Document>
        <Page size="LETTER" style={styles.page}>
          <View style={styles.documentHeader}>
            <Text style={styles.documentTitle}>Post-Operative Testing</Text>
          </View>
          ...
          <View key={index} style={styles.recordContainer}>
            ...
            <View style={styles.recordHeader} wrap={false}>
              <Text style={styles.recordTitle}>
                {`Post-Operative Testing ${index + 1}`}
              </Text>
            </View>
...
export default PostopTestingDocumentPDFTemplate;
