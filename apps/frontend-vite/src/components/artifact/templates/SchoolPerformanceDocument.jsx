 ...
 *
 * Sections:
 *   1. school-info: date, grade, school, provider, facility
 *   2. academic-performance: academicPerformance
 *   3. behavior-social: behaviorInClass, peerInteractions
 *   4. strengths: strengths (array)
 *   5. concerns: concerns (array)
 *   6. special-education: specialEducation, iepOr504Plan
 *   7. school-forms: schoolFormsCompleted (array of objects)
 *   8. findings: findings (string, per-sentence)
 *   9. results: results (object, recursive)
 *  10. clinical-notes: assessment, plan, recommendations (array of objects), notes, status
 *
 * Donor patterns: PointOfCareUltrasoundHeartRateDocument (renderDateField,
 *   renderObjectField/renderObjectNode/renderObjectLeaf, renderRecommendationsField).
 */
...
const SECTION_TITLES = {
  ...
  'school-forms': 'School Forms Completed',
  'findings': 'Findings',
  'results': 'Results',
...
const FIELD_LABELS = {
  ...
  behaviorInClass: 'Behavior in Class',
  peerInteractions: 'Peer Interactions',
  specialEducation: 'Special Education Services',
  iepOr504Plan: 'IEP/504 Plan',
  strengths: 'Strengths',
  concerns: 'Concerns',
  schoolFormsCompleted: 'School Forms Completed',
  findings: 'Findings',
  results: 'Results',
...
const SECTION_FIELDS = {
  'school-info': ['date', 'grade', 'school', 'provider', 'facility'],
  'academic-performance': ['academicPerformance'],
  'behavior-social': ['behaviorInClass', 'peerInteractions'],
  'strengths': ['strengths'],
  'concerns': ['concerns'],
  'special-education': ['specialEducation', 'iepOr504Plan'],
  'school-forms': ['schoolFormsCompleted'],
  'findings': ['findings'],
  'results': ['results'],
  'clinical-notes': ['assessment', 'plan', 'recommendations', 'notes', 'status'],
};

const BOOLEAN_FIELDS = ['specialEducation'];
...
const STRING_FIELDS = ['grade', 'school', 'provider', 'facility', 'academicPerformance', 'behaviorInClass', 'peerInteractions', 'iepOr504Plan', 'findings', 'assessment', 'plan', 'notes', 'status'];

const KEY_OVERRIDES = { iep: 'IEP', gpa: 'GPA', adhd: 'ADHD' };

/* humanizeKey — for recursive object leaf labels */
...
/* ═══════ VALUE HELPERS (recursive object) ═══════ */
const isEmptyDeep = (v) => {
...
const SchoolPerformanceDocument = ({ document: docProp }) => {
  ...
  /* save a single leaf inside the recursive object field (dotted path) */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    // Stage a DRAFT locally only (no DB write). Approve commits the full merged root value.
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (OBJECT_ARRAY_FIELDS.includes(f) || OBJECT_FIELDS.includes(f)) {
          if (fieldSearchText(f, val).toLowerCase().includes(phrase)) return true;
        } else if (Array.isArray(val)) {
          if (f === 'schoolFormsCompleted') {
            if (val.some(item => {
              const combined = [item.formType, item.completedDate, formatDate(item.completedDate), item.status].filter(Boolean).join(' ').toLowerCase();
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) {
      if (OBJECT_ARRAY_FIELDS.includes(fn) || OBJECT_FIELDS.includes(fn)) return fieldSearchText(fn, val).toLowerCase().includes(phrase);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `School Performance ${idx + 1}`.toLowerCase();
      ...
      for (const fields of Object.values(SECTION_FIELDS)) {
        for (const f of fields) {
          ...
          if (val !== null && val !== undefined) {
            if (OBJECT_ARRAY_FIELDS.includes(f) || OBJECT_FIELDS.includes(f)) {
              if (fieldSearchText(f, val).toLowerCase().includes(phrase)) return true;
            } else if (f === 'schoolFormsCompleted' && Array.isArray(val)) {
              if (val.some(item => [item.formType, item.completedDate, formatDate(item.completedDate), item.status].filter(Boolean).join(' ').toLowerCase().includes(phrase))) return true;
            } else if (Array.isArray(val)) {
              if (val.some(item => String(item).toLowerCase().includes(phrase))) return true;
            } else if (fmtVal(val).toLowerCase().includes(phrase)) return true;
          }
      ...
      if (record.date) {
        const dateStr = [formatDate(record.date), toInputDate(record.date), String(record.date)].join(' ').toLowerCase();
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    // Stage a DRAFT locally only (no DB write). Approve commits it.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      // localEdits keys are `${fn}-${idx}` and hold the full merged field value → write the whole field.
      const toCommit = fields
        .map(f => `${f}-${idx}`)
        .filter(k => pendingEdits[k] && localEdits[k] !== undefined);
      for (const editKey of toCommit) {
        const fieldName = editKey.slice(0, -`-${idx}`.length); // strip trailing "-<idx>"
        const resp = await secureApiClient.put(`/api/edit/school_performance/${id}/edit`, { field: fieldName, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/school_performance/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        text += `${label}\n`;
        val.forEach((form, i) => {
          text += `${i + 1}. ${form.formType || 'N/A'} | ${form.completedDate || 'N/A'} | ${form.status || 'N/A'}\n`;
        });
        text += '\n';
      ...
      <div key={fn} className="rec-mini-card">
        ...
        <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(val ? 'Yes' : 'No'); setSaveError(null); } }}>
          {isEditing ? (
            ...
            <>
              <div className="row-content"><span className={`content-value status-badge ${val ? 'status-yes' : 'status-no'}`}>{highlightText(displayVal)}</span><span className="edit-indicator">&#9998;</span></div>
  ...
  /* ═══════ RENDER: SCHOOL FORMS FIELD (array of objects) ═══════ */
  const renderSchoolFormsField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    const forms = Array.isArray(val) ? val : [];
    if (forms.length === 0) return null;
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;

    return forms.map((form, fIdx) => {
      const editKeyType = `${fn}.${fIdx}.formType-${idx}`;
      const editKeyStatus = `${fn}.${fIdx}.status-${idx}`;
      const isEditingType = editingField === editKeyType;
      const isEditingStatus = editingField === editKeyStatus;
      const isModifiedType = editedFields[editKeyType];
      const isModifiedStatus = editedFields[editKeyStatus];

      if (searchTerm.trim() && !sectionTitleMatches(sid) && !record._showAllSections) {
        const phrase = searchTerm.toLowerCase().trim();
        const combined = [form.formType, form.completedDate, formatDate(form.completedDate), form.status].filter(Boolean).join(' ').toLowerCase();
        if (!combined.includes(phrase)) return null;
      }

      return (
        <div key={fIdx} className="rec-mini-card">
          <div className="nested-subtitle">{highlightText(form.completedDate ? formatDate(form.completedDate) : `Form ${fIdx + 1}`)}</div>
          {/* Form Type row */}
          <div className={`numbered-row ${isModifiedType ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditingType) { setEditingField(editKeyType); setEditValue(form.formType || ''); setSaveError(null); } }}>
            {isEditingType ? (
              <div className="edit-field-container">
                <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                {saveError && <div className="save-error">{saveError}</div>}
                <div className="edit-actions">
                  <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id = safeId(record); if (!id) return; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[fIdx] = { ...currentArr[fIdx], formType: editValue }; setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: currentArr })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); setEditedFields(prev => ({ ...prev, [editKeyType]: 'edited' })); setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; }); const store = readDrafts(); if (!store[id]) store[id] = {}; store[id][fn] = currentArr; writeDrafts(store); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                  <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                </div>
              </div>
            ) : (
              <>
                <div className="row-content">
                  <span className="content-value">{highlightText(form.formType || 'N/A')}</span>
                  <span className={`status-badge status-${(form.status || '').toLowerCase()}`}>{highlightText(form.status || 'N/A')}</span>
                  <span className="edit-indicator">&#9998;</span>
                </div>
                <button className={`copy-btn ${copiedItems[editKeyType] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${form.formType} (${form.status || 'N/A'})`, editKeyType); }}>{copiedItems[editKeyType] ? 'Copied!' : 'Copy'}</button>
              </>
            )}
          </div>
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      if (f === 'specialEducation') return val === true || val === false;
      return hasVal(val);
    });
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (f === 'schoolFormsCompleted') return renderSchoolFormsField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="school-performance-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">School Performance</h2></div>
        <div className="empty-state">No school performance records available</div>
      </div>
    );
  }
    ...
    <div className="school-performance-document" ref={containerRef}>
      <div className="document-header">
        ...
        <div className="header-actions">
          ...
          <PDFDownloadLink document={<SchoolPerformanceDocumentPDFTemplate document={pdfData} />} fileName={`school-performance-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search school performance records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`School Performance ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'school-info')}
            {renderSection(record, idx, 'academic-performance')}
            {renderSection(record, idx, 'behavior-social')}
            {renderSection(record, idx, 'strengths')}
            {renderSection(record, idx, 'concerns')}
            {renderSection(record, idx, 'special-education')}
            {renderSection(record, idx, 'school-forms')}
