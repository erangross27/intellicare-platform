...
/**
 * MultipleSclerosisAssessmentDocumentPDFTemplate - box-free canonical (LETTER)
 * Config-driven from the JSX SECTION_ORDER/SECTION_TITLES/FIELD_LABELS/SECTION_FIELDS.
 * Renders EVERY populated field the JSX renders for JSX/PDF field parity.
 * No boxes: underline rules only (documentTitle 2 / recordTitle+sectionTitle 1 / fieldLabel 0.5).
 * Deeply-nested objects (mriFindings -> mcDonaldCriteria -> disseminationInSpace/inTime) + arrays
 * render via a generic recursive renderer (objectRows/arrayRows); inline "Key: value" is allowed
 * in the PDF only. recommendations = array of {recommendation, date} -> each object decomposes into
 * a subLabel (recommendation text) + a formatted date value row. Record date uses record.date
...
const SECTION_ORDER = ['record-info', 'ms-details', 'mri-findings', 'findings', 'assessment', 'plan', 'recommendations', 'results', 'notes'];
...
/* recommendationRows: array-of-objects [{recommendation, date}] -> subLabel (text) + formatted date value */
...
const MultipleSclerosisAssessmentDocumentPDFTemplate = ({ document: docProp, data = docProp }) => {
  let records = [];
  if (Array.isArray(data)) {
    if (data.length === 1 && data[0]?.multiple_sclerosis_assessment) records = Array.isArray(data[0].multiple_sclerosis_assessment) ? data[0].multiple_sclerosis_assessment : [data[0].multiple_sclerosis_assessment];
    else records = data;
  } else if (data?.multiple_sclerosis_assessment) records = Array.isArray(data.multiple_sclerosis_assessment) ? data.multiple_sclerosis_assessment : [data.multiple_sclerosis_assessment];
    ...
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <Text style={styles.documentTitle}>Multiple Sclerosis Assessment</Text>
          <Text style={styles.noData}>No multiple sclerosis assessment records available.</Text>
        </Page>
      </Document>
    );
  }
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, rIdx) => (
          <View key={rIdx}>
            <Text style={styles.recordTitle} break={rIdx > 0}>{safeString(`Multiple Sclerosis Assessment ${rIdx + 1}`)}</Text>
...
export default MultipleSclerosisAssessmentDocumentPDFTemplate;
