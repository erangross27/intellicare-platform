...
function buildContext(req, operation = 'read') { return { serviceId: 'toxicology-reports-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['patientAge', 'specimenType', 'collectionDateTime', 'chainOfCustody', 'substancesDetected', 'concentrationLevels', 'therapeuticRange', 'analyticalMethod', 'limitOfDetection', 'limitOfQuantification', 'poisoningClassification', 'clinicalSeverity', 'postmortemFindings', 'drugInteractions', 'metabolites', 'halfLife', 'timeToDetection', 'antidoteRecommended', 'qualityControlResults', 'interferingSubstances', 'carboxyhemoglobinLevel', 'cholinesteraseActivity', 'ethylGlucuronide', 'forensicImplications'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('toxicology_reports', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[ToxicologyReports] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('toxicology_reports', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[ToxicologyReports] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
