/**
 * DiabetesManagementNotes Edit Route
 * ServiceId: diabetes-management-notes-edit-service
...
function buildContext(req, operation = 'read') { return { serviceId: 'diabetes-management-notes-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
...
const ALLOWED_FIELDS = [
  ...
  'weight', 'bmi', 'bloodPressure',
  'hba1c', 'glucoseLevel', 'glucoseRange', 'timeInRange', 'controlStatus',
  'diabetesType',
  'insulinRegimen', 'oralMedications', 'otherMedications', 'medicationChanges',
  'selfMonitoring', 'medicationAdherence',
  'complications', 'neuropathyStatus', 'retinopathyStatus', 'nephropathyStatus', 'footExam',
  'dietAdherence', 'exercisePattern', 'diabetesEducation', 'hypoglycemicEvents',
  'findings', 'assessment', 'plan',
  'goals', 'recommendations', 'results',
  'followUp', 'notes'
];
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('diabetes_management_notes', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[DiabetesManagementNotes] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('diabetes_management_notes', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[DiabetesManagementNotes] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
