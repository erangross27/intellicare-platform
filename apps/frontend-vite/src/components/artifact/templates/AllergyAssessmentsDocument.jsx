/**
 * AllergyAssessmentsDocument.jsx
 * June 2026 — Full-standard dedicated template (cloned from OutcomesPredictionsDocument)
 * Collection: allergy_assessments (SINGULAR 'Allergy', plural 'Assessments')
 *
 * 1 Section + header (from unified-medical-schemas.json):
 *   header: date (Date), provider (string)
 *   allergy-testing-section: environmentalAllergens (array), skinTestResults (array),
 *                            specificIgE (object), totalIge (string), eosinophilCount (string)
 */
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = "field" or "rootField.path.to.leaf") */
...
const SECTION_TITLES = {
  'header-section': 'Visit Details',
  'allergy-testing-section': 'Allergy Testing',
};

const FIELD_LABELS = {
  date: 'Date',
  provider: 'Provider',
  environmentalAllergens: 'Environmental Allergens',
  skinTestResults: 'Skin Test Results',
...
const SECTION_FIELDS = {
  'header-section': ['date', 'provider'],
  'allergy-testing-section': ['environmentalAllergens', 'skinTestResults', 'specificIgE', 'totalIge', 'eosinophilCount'],
};

const STRING_FIELDS = ['provider', 'totalIge', 'eosinophilCount'];
/* SENTENCE_FIELDS — narrative string fields rendered with per-sentence editing (Rule #68).
   None of allergy_assessments' string fields are narrative (provider/totalIge/eosinophilCount
   are short lab/identifier values), so this stays empty; machinery retained from donor. */
const SENTENCE_FIELDS = [];
const OBJECT_ARRAY_FIELDS = [];
const DATE_FIELDS = ['date'];
const ARRAY_FIELDS = ['environmentalAllergens', 'skinTestResults'];
const OBJECT_FIELDS = ['specificIgE'];
...
/* ═══════ COMPONENT ═══════ */
const AllergyAssessmentsDocument = ({ document: docProp }) => {
  ...
  // editKeys (localEdits keys) staged as drafts (saved locally, NOT yet committed to DB/PDF). Cleared on Approve.
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     Draft store shape: { [recordId]: { [fieldPart]: value } } where fieldPart is either a plain
     field name (string/array/date) or "rootField.path.to.leaf" for object-leaf edits. */
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (dotIdx === -1) {
          // Plain field edit → localEdits key `${field}-${idx}`
          const editKey = `${fieldPart}-${idx}`;
          nLocal[editKey] = value;
          nPending[editKey] = true;
          nFields[editKey] = 'edited';
        } else {
          // Object-leaf edit → fieldPart = "rootField.path.to.leaf"; localEdits stored as whole object under `${rootField}-${idx}`
          const rootField = fieldPart.slice(0, dotIdx);
          const path = fieldPart.slice(dotIdx + 1).split('.');
          const editKey = `${rootField}-${idx}`;
          const base = nLocal[editKey] !== undefined ? nLocal[editKey] : record[rootField];
  ...
  const fieldMatches = useCallback((record, fn, idx) => {
    ...
    if (val !== null && val !== undefined) {
      ...
      if (typeof val === 'object') {
        return Object.values(val).some(v => String(v || '').toLowerCase().includes(phrase));
      }
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Allergy Assessments ${idx + 1}`.toLowerCase();
  ...
  const handleSaveField = useCallback((record, fn, idx, sid, sentIdx, valueOverride, editTrackingKey) => {
    ...
    // Persist the draft (fieldPart = plain field name) so it survives refresh.
    const store = readDrafts();
  ...
  // Save (object-leaf) = stage a DRAFT locally only. The draft fieldPart is the dotted leaf path
  // ("rootField.path.to.leaf"); committed to DB only on Approve.
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    // Persist the single leaf draft (fieldPart = dotted path) so it survives refresh.
    const store = readDrafts();
    if (!store[id]) store[id] = {};
    store[id][`${rootField}.${path.join('.')}`] = newVal;
    writeDrafts(store);
    setEditingField(null); setEditValue('');
  }, [safeId]);

  // saveSentence = stage the rebuilt full-text DRAFT locally only (fieldPart = plain field name).
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Draft store holds fieldPart → value for this record. fieldPart = "field" (plain),
      // or "rootField.path.to.leaf" (object leaf), or "field.<n>" (array element).
      const store = readDrafts();
      const recDrafts = store[id] || {};
      // Only commit drafts whose base field belongs to this section.
      const draftKeysForSection = Object.keys(recDrafts).filter(fieldPart => {
        const base = fieldPart.indexOf('.') === -1 ? fieldPart : fieldPart.slice(0, fieldPart.indexOf('.'));
        return fields.includes(base);
      });
      // localEdits keys this section staged (plain `${field}-${idx}` or root-object `${rootField}-${idx}`)
      const committedEditKeys = new Set();
      for (const fieldPart of draftKeysForSection) {
        // arrayIndex ONLY when the segment after the LAST dot is purely numeric ("recs.2" → field=recs,arrayIndex=2);
        // dotted object paths ("specificIgE.housing") stay as the full dotted field name.
        ...
        const resp = await secureApiClient.put(`/api/edit/allergy_assessments/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        const baseField = fieldPart.indexOf('.') === -1 ? fieldPart : fieldPart.slice(0, fieldPart.indexOf('.'));
        committedEditKeys.add(`${baseField}-${idx}`);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/allergy_assessments/${id}/approve`, { sectionId: sid, approved: true });
      ...
      if (store[id]) {
        draftKeysForSection.forEach(fp => delete store[id][fp]);
    ...
    } catch (err) { console.error('[AllergyAssessments] Approve error:', err); setSaveError('Approve failed.'); } finally { setSaving(false); }
  }, [safeId]);
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== ALLERGY ASSESSMENTS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Allergy Assessments ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: SIMPLE STRING FIELD (short fields — provider, totalIge, eosinophilCount) ═══════ */
  ...
  /* ═══════ RENDER: SENTENCE-EDITABLE NARRATIVE FIELD (Rule #68 — per-sentence editing) ═══════
     ...
     Retained from donor; SENTENCE_FIELDS is empty for allergy_assessments but machinery preserved. */
  ...
  /* ═══════ RENDER: ARRAY FIELD (tags; comma-separated edit) — environmentalAllergens / skinTestResults ═══════ */
  const renderArrayField = (record, fn, idx, sid) => {
    ...
    // One editable row per item; keep the original index so edits map to the right array slot through search.
    const phrase = searchTerm.toLowerCase().trim();
    const showAll = !phrase || record._showAllSections || sectionTitleMatches(sid) || label.toLowerCase().includes(phrase);
    const visible = display.map((item, origIdx) => ({ item, origIdx })).filter(({ item }) => showAll || item.toLowerCase().includes(phrase));
    if (visible.length === 0) return null;

    // Save one row: commas let a user expand one entry into several; empty value deletes the row.
    // Rebuild the full array and stage it as a draft (committed to DB only on Pending Approve).
    const saveItem = (origIdx) => {
      const parts = editValue.split(',').map(s => s.trim()).filter(s => s);
      const newArr = display.slice();
      if (parts.length === 0) newArr.splice(origIdx, 1);
      else newArr.splice(origIdx, 1, ...parts);
      handleSaveField(record, fn, idx, sid, null, newArr, `${fn}-${idx}-i${origIdx}`);
    };

    return (
      <div key={fn} className="rec-mini-card">
        <div className="nested-subtitle">{highlightText(label)}</div>
        {visible.map(({ item, origIdx }) => {
          const itemKey = `${fn}-${idx}-i${origIdx}`;
          const isEditing = editingField === itemKey;
          const isItemModified = editedFields[itemKey];
          return (
            <React.Fragment key={origIdx}>
              <div className={`numbered-row ${isItemModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(item); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    <textarea className="edit-textarea" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Escape') { setEditingField(null); setEditValue(''); setSaveError(null); } }} />
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveItem(origIdx); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
              ...
              {isItemModified && <span className="modified-badge">edited - click Pending Approve to save</span>}
            </React.Fragment>
          );
        })}
      </div>
    ...
    return (
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            if (OBJECT_ARRAY_FIELDS.includes(f)) return null;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="allergy-assessments-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Allergy Assessments</h2></div>
        <div className="empty-state">No allergy assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="allergy-assessments-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Allergy Assessments</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<AllergyAssessmentsDocumentPDFTemplate document={pdfData} />} fileName={`allergy-assessments-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search allergy assessments..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Allergy Assessments ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'header-section')}
            {renderSection(record, idx, 'allergy-testing-section')}
          </div>
        ))}
      </div>
    </div>
