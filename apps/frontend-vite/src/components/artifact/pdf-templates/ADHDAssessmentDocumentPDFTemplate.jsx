...
const SECTIONS = [
  { title: 'Assessment Information', fields: [['date', 'Date', 'date'], ['type', 'Assessment Type'], ['provider', 'Provider'], ['facility', 'Facility'], ['status', 'Status'], ['screeningTool', 'Screening Tool']] },
  { title: 'Parent & Teacher Forms', fields: [['parentForm', 'Parent Form', 'object'], ['teacherForm', 'Teacher Form', 'object']] },
  { title: 'Symptoms & Impairment', fields: [['symptoms', 'Symptoms', 'object']] },
  { title: 'Diagnostic Review', fields: [['dsmCriteriaMet', 'DSM Criteria Met', 'comma'], ['differentialDiagnosis', 'Differential Diagnosis', 'arrayKeep'], ['comorbidities', 'Comorbidities', 'arrayKeep'], ['familyHistory', 'Family History', 'arrayLabeledKeep']] },
  { title: 'Clinical Review', fields: [['findings', 'Findings', 'sentence'], ['assessment', 'Assessment', 'sentence'], ['plan', 'Plan', 'sentence'], ['notes', 'Notes', 'sentence'], ['results', 'Results', 'object']] },
  { title: 'Recommendations', fields: [['recommendations', 'Recommendations', 'arrayKeep']] },
];
...
const SENTENCE_OBJECT_PATHS = new Set(['teacherForm.classroomBehavior']);
...
const ADHDAssessmentDocumentPDFTemplate = ({ document: documentProp, data: dataProp, templateData }) => {
  const records = React.useMemo(() => { const source = documentProp ?? dataProp ?? templateData; if (!source) return []; let rows = Array.isArray(source) ? source : [source]; rows = rows.flatMap(row => { if (Array.isArray(row?.records)) return row.records; if (Array.isArray(row?._records)) return row._records; if (row?.adhd_assessment) return Array.isArray(row.adhd_assessment) ? row.adhd_assessment : [row.adhd_assessment]; if (row?.documentData) { const nested = row.documentData; if (Array.isArray(nested)) return nested; if (nested?.adhd_assessment) return Array.isArray(nested.adhd_assessment) ? nested.adhd_assessment : [nested.adhd_assessment]; return [nested]; } return [row]; }); return rows.filter(row => row && typeof row === 'object'); }, [documentProp, dataProp, templateData]);
  if (!records.length) return <Document><Page size="LETTER" style={styles.page}><View style={styles.documentHeader}><Text style={styles.documentTitle}>ADHD Assessment</Text></View><Text style={styles.noData}>No ADHD assessment data available</Text></Page></Document>;
  return <Document>{records.flatMap((record, recordIndex) => PAGE_GROUPS.map((indexes, pageIndex) => { const visible = indexes.map(index => ({ section: SECTIONS[index], index, fields: SECTIONS[index].fields.filter(([field]) => hasVal(record[field])) })).filter(item => item.fields.length); if (!visible.length) return null; return <Page key={`${recordIndex}-${pageIndex}`} size="LETTER" style={styles.page}>{pageIndex === 0 && <View style={styles.documentHeader}><Text style={styles.documentTitle}>ADHD Assessment</Text></View>}{pageIndex === 0 && <View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>{record.screeningTool || `ADHD Assessment ${recordIndex + 1}`}</Text></View>}{visible.map(({ section, index, fields }) => <View key={`${index}-${section.title}`} style={styles.section}><View wrap={false}><Text style={styles.sectionTitle}>{section.title}</Text></View>{fields.map((config, fieldIndex) => renderField(config, record[config[0]], `${index}-${fieldIndex}`, section.title))}</View>)}</Page>; }))}</Document>;
};
