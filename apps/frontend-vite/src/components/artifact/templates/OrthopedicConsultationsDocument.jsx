/**
 * OrthopedicConsultationsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: orthopedic_consultations
 *
 * 5 Sections:
 *   1. chief-complaint: chiefComplaint, injuryMechanism, painScale
 *   2. physical-exam: rangeOfMotion, muscleStrength, neurologicalExamination, vascularExamination, specialTests
 *   3. imaging-findings: imagingStudies, jointEffusion, arthroscopicFindings, kellgrenLawrenceGrade, outerbridge, fractureClassification, deformityDescription, boneQuality
 *   4. surgical-info: implantType, priorSurgeries, functionalScore
 *   5. rehab-status: weightBearingStatus, immobilization, physicalTherapyStatus, workersCompensation
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import OrthopedicConsultationsDocumentPDFTemplate from '../pdf-templates/OrthopedicConsultationsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './OrthopedicConsultationsDocument.css';

/* ═══════ CONSTANTS ═══════ */
const SECTION_TITLES = {
  'chief-complaint': 'Chief Complaint and History',
  'physical-exam': 'Physical Examination',
  'imaging-findings': 'Imaging and Findings',
  'surgical-info': 'Surgical Information',
  'rehab-status': 'Rehabilitation and Status',
};
...
const FIELD_LABELS = {
  ...
  arthroscopicFindings: 'Arthroscopic Findings',
  kellgrenLawrenceGrade: 'Kellgren-Lawrence Grade',
  outerbridge: 'Outerbridge Classification',
  fractureClassification: 'Fracture Classification',
  deformityDescription: 'Deformity Description',
  ...
  physicalTherapyStatus: 'Physical Therapy Status',
  workersCompensation: 'Workers Compensation',
};

const SECTION_FIELDS = {
  'chief-complaint': ['chiefComplaint', 'injuryMechanism', 'painScale'],
  'physical-exam': ['rangeOfMotion', 'muscleStrength', 'neurologicalExamination', 'vascularExamination', 'specialTests'],
  'imaging-findings': ['imagingStudies', 'jointEffusion', 'arthroscopicFindings', 'kellgrenLawrenceGrade', 'outerbridge', 'fractureClassification', 'deformityDescription', 'boneQuality'],
  'surgical-info': ['implantType', 'priorSurgeries', 'functionalScore'],
  'rehab-status': ['weightBearingStatus', 'immobilization', 'physicalTherapyStatus', 'workersCompensation'],
};

const NUMBER_FIELDS = ['painScale'];
const BOOLEAN_FIELDS = ['jointEffusion', 'workersCompensation'];
const ARRAY_FIELDS = ['specialTests', 'imagingStudies', 'priorSurgeries'];
const STRING_FIELDS = ['chiefComplaint', 'injuryMechanism', 'rangeOfMotion', 'muscleStrength', 'neurologicalExamination', 'vascularExamination', 'arthroscopicFindings', 'kellgrenLawrenceGrade', 'outerbridge', 'fractureClassification', 'deformityDescription', 'boneQuality', 'implantType', 'functionalScore', 'weightBearingStatus', 'immobilization', 'physicalTherapyStatus'];
const DATE_FIELDS = [];
...
const DRAFT_KEY = 'orthopedic_consultationsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const OrthopedicConsultationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.orthopedic_consultations) return Array.isArray(r.orthopedic_consultations) ? r.orthopedic_consultations : [r.orthopedic_consultations];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.orthopedic_consultations) return Array.isArray(dd.orthopedic_consultations) ? dd.orthopedic_consultations : [dd.orthopedic_consultations]; return [dd]; }
      return [r];
    });
  ...
  /* ═══════ UTILS ═══════ */
  // hide-zero: a top-level numeric 0 is treated as empty (like '' / null) so a 0-valued field renders no row.
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Orthopedic Consultation ${idx + 1}`.toLowerCase();
  ...
  // fieldValue = the FULL effective field value (string or array) keyed by `${fn}-${idx}`.
  const stageDraft = useCallback((record, fn, idx, sid, fieldValue) => {
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      const toCommit = Object.keys(localEdits).filter(k => {
        ...
        const m = k.match(/^(.+)-(\d+)$/);
        return m && parseInt(m[2]) === idx && fields.includes(m[1]);
      });
      for (const editKey of toCommit) {
        const m = editKey.match(/^(.+)-(\d+)$/);
        const fieldPart = m[1]; // field name (may contain dots, but not a trailing numeric arrayIndex)
        ...
        const resp = await secureApiClient.put(`/api/edit/orthopedic_consultations/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/orthopedic_consultations/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== ORTHOPEDIC CONSULTATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Orthopedic Consultation ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="orthopedic-consultations-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Orthopedic Consultations</h2></div>
        <div className="empty-state">No orthopedic consultation records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="orthopedic-consultations-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Orthopedic Consultations</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<OrthopedicConsultationsDocumentPDFTemplate document={pdfData} />} fileName="Orthopedic_Consultations.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search orthopedic consultations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Orthopedic Consultation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'chief-complaint')}
            {renderSection(record, idx, 'physical-exam')}
            {renderSection(record, idx, 'imaging-findings')}
            {renderSection(record, idx, 'surgical-info')}
            {renderSection(record, idx, 'rehab-status')}
          </div>
        ))}
      </div>
    </div>
...
export default OrthopedicConsultationsDocument;
