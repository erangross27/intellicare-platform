/**
 * NicuProgressNotesDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: nicu_progress_notes (displayed as "NICU Progress Notes")
 *
 * 7 Sections:
 *   1. growth: gestationalAgeAtBirth, correctedGestationalAge, birthWeight, currentWeight, weightPercentile
 *   2. respiratory: respiratorySupport, fiO2Requirement, peepSettings, respiratoryDistressSyndrome, surfactantAdministration
 *   3. feeding: feedingType, feedingVolume, feedingTolerance
 *   4. scores: apgarScore1Min, apgarScore5Min, snapScore
 *   5. bilirubin: bilirubinLevel, phototherapyStatus
 *   6. access-lines: umbilicalCatheterStatus, ivAccessType
 *   7. complications: intraventricularHemorrhageGrade, periventricularLeukomalacia, patentDuctusArteriosus, retinopathyOfPrematurity, necrotizingEnterocolitis
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import NicuProgressNotesDocumentPDFTemplate from '../pdf-templates/NicuProgressNotesDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './NicuProgressNotesDocument.css';
...
const DRAFT_KEY = 'nicu_progress_notesPendingEdits';
const readDrafts = () => {
...
const STRING_FIELDS = ['gestationalAgeAtBirth', 'correctedGestationalAge', 'respiratorySupport', 'feedingType', 'feedingTolerance', 'umbilicalCatheterStatus', 'phototherapyStatus', 'intraventricularHemorrhageGrade', 'patentDuctusArteriosus', 'retinopathyOfPrematurity', 'necrotizingEnterocolitis'];
...
/* ═══════ COMPONENT ═══════ */
const NicuProgressNotesDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      ...
      if (r?.nicu_progress) return Array.isArray(r.nicu_progress) ? r.nicu_progress : [r.nicu_progress];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.nicu_progress_notes) return Array.isArray(dd.nicu_progress_notes) ? dd.nicu_progress_notes : [dd.nicu_progress_notes]; if (dd?.nicu_progress) return Array.isArray(dd.nicu_progress) ? dd.nicu_progress : [dd.nicu_progress]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `NICU Progress Note ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    // localEdits keys for this section+record: "<field>-<idx>" (value = full field value, incl. whole arrays)
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/nicu_progress_notes/${id}/edit`, { field: fn, value: localEdits[editKey] });
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/nicu_progress_notes/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (NUMBER_FIELDS.includes(f)) {
        if (Number(val) === 0) return; // hide-zero
        text += `${label}\n${fmtVal(val)}\n\n`;
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== NICU PROGRESS NOTES ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `NICU Progress Note ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: NUMBER FIELD — num-stepper (−/+), input type=text inputMode=decimal (hide-zero) ═══════ */
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      const val = getFieldValue(record, f, idx);
      if (NUMBER_FIELDS.includes(f)) return hasVal(val) && Number(val) !== 0;
      return hasVal(val);
    });
  ...
  if (!records || records.length === 0) {
    return (
      <div className="nicu-progress-notes-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">NICU Progress Notes</h2></div>
        <div className="empty-state">No NICU progress notes available</div>
      </div>
    );
  }
  ...
  return (
    <div className="nicu-progress-notes-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">NICU Progress Notes</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<NicuProgressNotesDocumentPDFTemplate document={pdfData} />} fileName="NICU_Progress_Notes.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search NICU progress notes..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`NICU Progress Note ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'growth')}
            {renderSection(record, idx, 'respiratory')}
            {renderSection(record, idx, 'feeding')}
            {renderSection(record, idx, 'scores')}
            {renderSection(record, idx, 'bilirubin')}
            {renderSection(record, idx, 'access-lines')}
...
export default NicuProgressNotesDocument;
