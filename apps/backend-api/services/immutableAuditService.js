...
class ImmutableAuditService {
  constructor() {
    ...
    this.logFile = path.join(__dirname, '../logs/immutable-audit.log');
    this.hashFile = path.join(__dirname, '../logs/audit-hashes.log');
    this.integrityFile = path.join(__dirname, '../logs/integrity-manifest.json');
    this.initialized = false;
    
    // Write-only mode to prevent tampering
    this.writeOnlyMode = true;
    this.logIntegrity = new Map();
    this.backupFile = path.join(__dirname, '../logs/audit-backup.log');
  ...
  // Removed duplicate logServiceDataAccess method - using the enhanced version below
