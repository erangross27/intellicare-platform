/**
 * Anesthesia Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 * Supports dot-path field names for nested objects (e.g., 'chiefComplaint.complaint')
 *
 * PUT /api/edit/anesthesia/:id/edit    — Edit a single field
 * PUT /api/edit/anesthesia/:id/approve — Approve a section
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'anesthesia-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    // Validate top-level field (supports dot-path like 'chiefComplaint.complaint')
    ...
    await sda.update('anesthesia', { _id: objectId }, {
      $set: {
        [updatePath]: value,
        updatedAt: new Date(),
    ...
    console.error('[Anesthesia] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    console.error('[Anesthesia] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
