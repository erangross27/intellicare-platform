...
const SECTIONS = [
  { title: 'Wound Assessment', fields: [['woundEtiology', 'Wound Etiology', 'comma'], ['woundLocationAnatomical', 'Wound Location (Anatomical)', 'comma'], ['woundDimensionsCm', 'Wound Dimensions (cm)', 'sentenceComma'], ['wagnerClassification', 'Wagner Classification', 'number']] },
  { title: 'Vascular Status', fields: [['ankleBrachialIndex', 'Ankle-Brachial Index', 'number'], ['toeBrachialIndex', 'Toe-Brachial Index', 'number'], ['transcutaneousOxygenPressure', 'Transcutaneous Oxygen Pressure', 'number']] },
  { title: 'HBOT Treatment', fields: [['hbotTreatmentPressureATA', 'HBOT Treatment Pressure (ATA)', 'number'], ['hbotTreatmentDurationMinutes', 'HBOT Treatment Duration (min)', 'number'], ['hbotTotalSessionsCompleted', 'HBOT Total Sessions Completed', 'number']] },
  { title: 'Wound Status & Infection', fields: [['woundBedTissuePercentage', 'Wound Bed Tissue Percentage', 'sentenceComma'], ['periWoundSkinCondition', 'Peri-Wound Skin Condition', 'comma'], ['woundExudateCharacteristics', 'Wound Exudate Characteristics'], ['infectionSignsPresent', 'Infection Signs Present', 'arrayKeep'], ['osteomyelitisStatus', 'Osteomyelitis Status']] },
  { title: 'Patient Metrics', fields: [['diabeticFootRiskCategory', 'Diabetic Foot Risk Category'], ['hemoglobinA1cPercent', 'Hemoglobin A1c (%)', 'number'], ['serumAlbuminGdL', 'Serum Albumin (g/dL)', 'number'], ['prealbumin', 'Prealbumin', 'number']] },
  { title: 'Management Plan & Safety', fields: [['offloadingDeviceType', 'Offloading Device Type'], ['compressionTherapyRegimen', 'Compression Therapy Regimen', 'comma'], ['woundHealingTrajectory', 'Wound Healing Trajectory', 'sentenceComma']] },
  { title: 'Management Plan (continued)', fields: [['hbotContraindicationsScreened', 'HBOT Contraindications Screened', 'arrayKeep'], ['middleEarBarotraumaRisk', 'Middle Ear Barotrauma Risk', 'comma'], ['postHbotTcpo2Response', 'Post-HBOT TcPO2 Response', 'number']] },
];
const PAGE_GROUPS = [[0], [1, 2], [3], [4], [5], [6]];
const isEpochSentinel = value => { if (!value || typeof value !== 'object' || Array.isArray(value) || !Object.hasOwn(value, '$date')) return false; const date = new Date(value.$date); return !Number.isNaN(date.getTime()) && date.getTime() === 0; };
const hasVal = value => !isEpochSentinel(value) && value !== null && value !== undefined && value !== '' && (!Array.isArray(value) || value.some(hasVal)) && (typeof value !== 'object' || Array.isArray(value) || Object.values(value).some(hasVal));
...
const WoundHealingHyperbaricDocumentPDFTemplate = ({ document: documentProp, data: dataProp, templateData }) => {
  const records = React.useMemo(() => { const source = documentProp ?? dataProp ?? templateData; if (!source) return []; let rows = Array.isArray(source) ? source : [source]; rows = rows.flatMap(row => { if (Array.isArray(row?.records)) return row.records; if (Array.isArray(row?._records)) return row._records; if (row?.wound_healing_hyperbaric) return Array.isArray(row.wound_healing_hyperbaric) ? row.wound_healing_hyperbaric : [row.wound_healing_hyperbaric]; if (row?.documentData) { const nested = row.documentData; if (Array.isArray(nested)) return nested; if (nested?.wound_healing_hyperbaric) return Array.isArray(nested.wound_healing_hyperbaric) ? nested.wound_healing_hyperbaric : [nested.wound_healing_hyperbaric]; return [nested]; } return [row]; }); return rows.filter(row => row && typeof row === 'object'); }, [documentProp, dataProp, templateData]);
  if (!records.length) return <Document><Page size="LETTER" style={styles.page}><View style={styles.documentHeader}><Text style={styles.documentTitle}>Wound Healing Hyperbaric</Text></View><Text style={styles.noData}>No wound healing hyperbaric data available</Text></Page></Document>;
  return <Document>{records.flatMap((record, recordIndex) => PAGE_GROUPS.map((indexes, pageIndex) => { const visible = indexes.map(index => ({ section: SECTIONS[index], index, fields: SECTIONS[index].fields.filter(([field]) => hasVal(record[field])) })).filter(item => item.fields.length); if (!visible.length) return null; return <Page key={`${recordIndex}-${pageIndex}`} size="LETTER" style={styles.page}>{pageIndex === 0 && <View style={styles.documentHeader}><Text style={styles.documentTitle}>Wound Healing Hyperbaric</Text></View>}{pageIndex === 0 && <View style={styles.recordHeader} wrap={false}><Text style={styles.recordTitle}>{record.woundLocationAnatomical || `Wound Healing Hyperbaric ${recordIndex + 1}`}</Text></View>}{visible.map(({ section, index, fields }) => <View key={`${index}-${section.title}`} style={styles.section}><View wrap={false}><Text style={styles.sectionTitle}>{section.title}</Text></View>{fields.map((config, fieldIndex) => renderField(config, record[config[0]], `${index}-${fieldIndex}`, section.title))}</View>)}</Page>; }))}</Document>;
};
