...
function buildContext(req, operation = 'read') { return { serviceId: 'insulin-regimen-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['date', 'provider', 'facility', 'status', 'notes', 'details', 'findings', 'recommendations', 'results', 'comments', 'summary', 'assessment', 'plan', 'description', 'type', 'category', 'severity', 'result', 'value', 'unit', 'interpretation', 'followUp', 'regimeType', 'totalDailyDose', 'basalBolusRatio', 'bolusesPerDay', 'basalInsulin', 'bolusInsulin'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('insulin_regimen', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[InsulinRegimen] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('insulin_regimen', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[InsulinRegimen] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
