...
/**
 * NeurologicalAssessmentDocumentPDFTemplate - box-free canonical (LETTER)
 ...
 * Record date rule: record has ONLY createdAt/updatedAt -> NO record date (title only); assessmentDate
 * is a field. NEVER key a record date off createdAt/updatedAt.
 */
...
const SECTION_ORDER = ['assessment-info', 'clinical-status', 'vital-signs', 'interventions', 'response', 'plan', 'recommendations'];
...
const NeurologicalAssessmentDocumentPDFTemplate = ({ document: docProp, data = docProp }) => {
  let records = [];
  if (Array.isArray(data)) {
    if (data.length === 1 && data[0]?.neurological_assessment) records = Array.isArray(data[0].neurological_assessment) ? data[0].neurological_assessment : [data[0].neurological_assessment];
    else records = data;
  } else if (data?.neurological_assessment) records = Array.isArray(data.neurological_assessment) ? data.neurological_assessment : [data.neurological_assessment];
    ...
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <Text style={styles.documentTitle}>Neurological Assessment</Text>
          <Text style={styles.noData}>No neurological assessment records available.</Text>
        </Page>
      </Document>
    );
  }
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {records.map((record, rIdx) => (
          <View key={rIdx}>
            <Text style={styles.recordTitle} break={rIdx > 0}>{safeString(`Neurological Assessment ${rIdx + 1}`)}</Text>
...
export default NeurologicalAssessmentDocumentPDFTemplate;
