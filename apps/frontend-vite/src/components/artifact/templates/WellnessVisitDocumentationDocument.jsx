/**
 * WellnessVisitDocumentationDocument.jsx
 * June 2026 — Complete template with inline editing, blue glow theme
 * Collection: wellness_visit_documentation
 *
 * 5 Sections:
 *   1. visit-vitals: visitType, ageAtVisit, heightInCm, weightInKg, bodyMassIndex, bloodPressureSystolic, bloodPressureDiastolic, vitalSignsDateTime
 *   2. immunizations-screenings: immunizationsAdministered, immunizationsDeclined, screeningsPerformed, developmentalMilestones
 *   3. health-maintenance: healthMaintenanceItemsDue, healthMaintenanceItemsCompleted, chronicConditionsReviewed, currentMedicationsList, medicationReconciliationCompleted
 *   4. history-directives: familyHistoryUpdated, socialHistoryUpdated, advanceDirectivesDiscussed, functionalStatusAssessment
 *   5. screening-scores-plan: fallRiskScore, depressionScreeningScore, laboratoriesOrdered, counselingProvided
 *
 * Field types:
 *   DATE(2): date (header), vitalSignsDateTime (in-section) — input type=date YYYY-MM-DD
 *   NUMBER(8): ageAtVisit, heightInCm, weightInKg, bodyMassIndex, bloodPressureSystolic, bloodPressureDiastolic, fallRiskScore, depressionScreeningScore (parseFloat + hide-zero)
 *   BOOLEAN(4): medicationReconciliationCompleted, familyHistoryUpdated, socialHistoryUpdated, advanceDirectivesDiscussed (Yes/No select)
 *   ARRAY(10): per-item editing (renderArrayField with dot-path keys)
 *   STRING(2): visitType, functionalStatusAssessment (per-sentence)
 */
...
const FIELD_LABELS = {
  ...
  vitalSignsDateTime: 'Vital Signs Date/Time',
  immunizationsAdministered: 'Immunizations Administered',
  immunizationsDeclined: 'Immunizations Declined',
  screeningsPerformed: 'Screenings Performed',
  developmentalMilestones: 'Developmental Milestones',
  healthMaintenanceItemsDue: 'Health Maintenance Items Due',
  healthMaintenanceItemsCompleted: 'Health Maintenance Items Completed',
  chronicConditionsReviewed: 'Chronic Conditions Reviewed',
  currentMedicationsList: 'Current Medications List',
  ...
  advanceDirectivesDiscussed: 'Advance Directives Discussed',
  ...
  laboratoriesOrdered: 'Laboratories Ordered',
  counselingProvided: 'Counseling Provided',
  date: 'Date',
};

const SECTION_FIELDS = {
  'visit-vitals': ['visitType', 'ageAtVisit', 'heightInCm', 'weightInKg', 'bodyMassIndex', 'bloodPressureSystolic', 'bloodPressureDiastolic', 'vitalSignsDateTime'],
...
const DATE_FIELDS = ['vitalSignsDateTime'];
const NUMBER_FIELDS = ['ageAtVisit', 'heightInCm', 'weightInKg', 'bodyMassIndex', 'bloodPressureSystolic', 'bloodPressureDiastolic', 'fallRiskScore', 'depressionScreeningScore'];
/* MEANINGFUL_ZERO_FIELDS: screening scores where 0 is a valid clinical finding (negative screen / no risk) → always show when present, never hidden as "not recorded" */
const MEANINGFUL_ZERO_FIELDS = ['fallRiskScore', 'depressionScreeningScore'];
const BOOLEAN_FIELDS = ['medicationReconciliationCompleted', 'familyHistoryUpdated', 'socialHistoryUpdated', 'advanceDirectivesDiscussed'];
const ARRAY_FIELDS = ['immunizationsAdministered', 'immunizationsDeclined', 'screeningsPerformed', 'developmentalMilestones', 'healthMaintenanceItemsDue', 'healthMaintenanceItemsCompleted', 'chronicConditionsReviewed', 'currentMedicationsList', 'laboratoriesOrdered', 'counselingProvided'];
const STRING_FIELDS = ['visitType', 'functionalStatusAssessment'];
...
/* ═══════ COMPONENT ═══════ */
const WellnessVisitDocumentationDocument = ({ document: docProp }) => {
  ...
  /* Rehydrate pending drafts from localStorage so a Save survives refresh (shown in JSX, NOT in DB/PDF).
     fieldPart is "field" (scalar) or "field.arrayIndex" (one array element). Array drafts are merged
     back into the full array stored under `${fn}-${idx}` (the localEdits convention this file uses). */
  useEffect(() => {
    ...
    records.forEach((record, idx) => {
      ...
      Object.entries(recDrafts).forEach(([fieldPart, value]) => {
        ...
        if (isArrayElem) {
          ...
          const baseArr = Array.isArray(nLocal[`${fn}-${idx}`]) ? nLocal[`${fn}-${idx}`]
  ...
  /* coerceBool: schema-boolean fields (familyHistoryUpdated / socialHistoryUpdated) are sometimes stored as a Date/string in the DB.
     A non-empty/truthy value means "yes, reviewed"; map it back to a real boolean so the field renders instead of silently dropping. */
  const coerceBool = useCallback((v) => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Wellness Visit Documentation ${idx + 1}`.toLowerCase();
  ...
  // Stage an array-element DRAFT: localEdits keeps the FULL array under `${fn}-${idx}` (this file's
  // convention) while the localStorage draft records the single element under `${fn}.${arrayIndex}`
  // so Approve can PUT it with arrayIndex. No DB write here.
  function stageArrayDraft(record, fn, idx, sid, arrayIndex, fullArray, elemValue) {
    const id = safeId(record); if (!id) return;
    setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullArray }));
    setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true }));
    setApprovedSections(prev => { const k = `${sid}-${idx}`; if (!prev[k]) return prev; const n = { ...prev }; delete n[k]; return n; });
    const store = readDrafts();
    if (!store[id]) store[id] = {};
    store[id][`${fn}.${arrayIndex}`] = elemValue;
    writeDrafts(store);
  }
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      // Drafts for this record. fieldPart = "field" (scalar) or "field.arrayIndex" (array element).
      const store = readDrafts();
      const recDrafts = store[id] || {};
      const committedFns = new Set();
      for (const [fieldPart, value] of Object.entries(recDrafts)) {
        const lastDot = fieldPart.lastIndexOf('.');
        const isArrayElem = lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1));
        const fn = isArrayElem ? fieldPart.slice(0, lastDot) : fieldPart;
        if (!fields.includes(fn)) continue; // only this section's fields
        const payload = { field: fn, value };
        if (isArrayElem) payload.arrayIndex = parseInt(fieldPart.slice(lastDot + 1), 10);
        const resp = await secureApiClient.put(`/api/edit/wellness_visit_documentation/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
        committedFns.add(fn);
      }
      // Flag the section approved (audit trail)
      await secureApiClient.put(`/api/edit/wellness_visit_documentation/${id}/approve`, { sectionId: sid, approved: true });

      // Clear pending for committed fields → committed values now flow into pdfData/PDF
      setPendingEdits(prev => { const n = { ...prev }; committedFns.forEach(fn => delete n[`${fn}-${idx}`]); return n; });
      // Remove this record's committed drafts from localStorage
      ...
      if (store2[id]) {
        Object.keys(store2[id]).forEach(fieldPart => {
          ...
          const isArrayElem = lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1));
          const fn = isArrayElem ? fieldPart.slice(0, lastDot) : fieldPart;
          if (committedFns.has(fn)) delete store2[id][fieldPart];
        });
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (BOOLEAN_FIELDS.includes(f)) {
        const bv = coerceBool(val);
        if (typeof bv !== 'boolean') return;
        text += `${label}: ${fmtVal(bv)}\n\n`;
        return;
      }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== WELLNESS VISIT DOCUMENTATION ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Wellness Visit Documentation ${idx + 1}\n${'='.repeat(40)}\n\n`;
              ...
              <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(editKey); setEditValue(parsed.isLabeled ? parsed.value : itemStr); setSaveError(null); } }}>
                {isEditing ? (
                  <div className="edit-field-container">
                    ...
                    {saveError && <div className="save-error">{saveError}</div>}
                    <div className="edit-actions">
                      <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); const id2 = safeId(record); if (!id2) return; setSaveError(null); const saveVal = parsed.isLabeled ? `${parsed.label}: ${editValue}` : editValue; const currentArr = [...(Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx) : [])]; currentArr[itemIdx] = saveVal; stageArrayDraft(record, fn, idx, sid, itemIdx, currentArr, saveVal); setEditedFields(prev => ({ ...prev, [editKey]: 'edited' })); setEditingField(null); setEditValue(''); }}>{saving ? 'Saving...' : 'Save'}</button>
                      <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  const renderSection = (record, idx, sid) => {
    ...
    const hasAnyVal = fields.some(f => {
      ...
      if (BOOLEAN_FIELDS.includes(f)) return typeof coerceBool(getFieldValue(record, f, idx)) === 'boolean';
  ...
  if (!records || records.length === 0) {
    return (
      <div className="wellness-visit-documentation-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Wellness Visit Documentation</h2></div>
        <div className="empty-state">No wellness visit documentation data available</div>
      </div>
    );
  }
  ...
  return (
    <div className="wellness-visit-documentation-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Wellness Visit Documentation</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<WellnessVisitDocumentationDocumentPDFTemplate document={pdfData} />} fileName={`wellness-visit-documentation-${new Date().toISOString().split('T')[0]}.pdf`} className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search wellness visit documentation..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Wellness Visit Documentation ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'visit-vitals')}
            {renderSection(record, idx, 'immunizations-screenings')}
            {renderSection(record, idx, 'health-maintenance')}
            {renderSection(record, idx, 'history-directives')}
            {renderSection(record, idx, 'screening-scores-plan')}
          </div>
        ))}
      </div>
    </div>
