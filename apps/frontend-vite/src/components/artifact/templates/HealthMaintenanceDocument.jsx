/**
 * HealthMaintenanceDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: health_maintenance
 *
 * 3 Sections:
 *   1. session-info: date (date picker), provider (string), facility (string)
 *   2. screening-schedules: dentalCleaning, eyeExam, footExam, skinCancerScreening,
 *                           selfExamEducation, hearingTest, boneHealthScreening
 *   3. clinical-notes: findings, assessment, plan, results (object), recommendations (object-array), notes, status
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import HealthMaintenanceDocumentPDFTemplate from '../pdf-templates/HealthMaintenanceDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './HealthMaintenanceDocument.css';
...
const STRING_FIELDS = ['provider', 'facility', 'dentalCleaning', 'eyeExam', 'footExam', 'skinCancerScreening', 'selfExamEducation', 'hearingTest', 'boneHealthScreening', 'findings', 'assessment', 'plan', 'notes', 'status'];
const DATE_FIELDS = ['date'];
const OBJECT_FIELDS = ['results'];
const OBJECT_ARRAY_FIELDS = ['recommendations'];

/* humanizeKey: convert snake/camel keys to readable labels for nested object editor */
const KEY_OVERRIDES = {};
...
/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   ...
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = the localEdits key prefix, e.g. "findings"
   or "results"/"recommendations" whose value is the full edited object/array) */
const DRAFT_KEY = 'health_maintenancePendingEdits';
const readDrafts = () => {
...
/* ======= COMPONENT ======= */
const HealthMaintenanceDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.health_maintenance) return Array.isArray(r.health_maintenance) ? r.health_maintenance : [r.health_maintenance];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.health_maintenance) return Array.isArray(dd.health_maintenance) ? dd.health_maintenance : [dd.health_maintenance]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Health Maintenance ${idx + 1}`.toLowerCase();
  ...
  /* ======= EDIT HANDLERS =======
     ...
     NOT written to MongoDB and NOT shown in the PDF/Copy until the user clicks Approve (the ONLY DB writer).
     stageDraft is the shared primitive: it records localEdits[fn-idx]=value, marks it pending, drops any
     prior 'approved' flag for the section (so the button returns to yellow Pending Approve), and persists. */
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      const fields = SECTION_FIELDS[sid] || [];
      const suffix = `-${idx}`;
      // Staged drafts for this section's fields (localEdits key = "field-idx", pending only)
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/health_maintenance/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      // Flag the record/section approved (audit trail)
      await secureApiClient.put(`/api/edit/health_maintenance/${id}/approve`, { sectionId: sid, approved: true });
      ...
      console.error('[HealthMaintenance] Approve error:', err);
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        if (Array.isArray(val) && val.length > 0) {
          ...
          val.forEach((rec, i) => {
            ...
            if (!rt) return;
            const p = parseLabel(rt);
            if (p.isLabeled) text += `  ${p.label}\n  ${i + 1}. ${p.value}${rd ? ` (${rd})` : ''}\n`;
            else text += `  ${i + 1}. ${rt}${rd ? ` (${rd})` : ''}\n`;
          });
          text += '\n';
        }
        ...
        const walk = (obj, indent) => {
          Object.entries(obj).forEach(([k, v]) => {
            ...
            else { lines.push(`${indent}${humanizeKey(k)}:`); walk(v, indent + '  '); }
          });
        };
        if (val && typeof val === 'object' && !isEmptyDeep(val)) { walk(val, '  '); if (lines.length) { text += `${label}\n${lines.join('\n')}\n\n`; } }
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== HEALTH MAINTENANCE ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Health Maintenance ${idx + 1}\n${'='.repeat(40)}\n\n`;
          ...
          return (
            <div key={gIdx} className="rec-mini-card" style={{ marginTop: 8 }}>
              ...
              {group.items.map(({ rec, rIdx }) => {
                ...
                const recParsed = parseLabel(recText);
                ...
                return (
                  <div key={rIdx}>
                    {recParsed.isLabeled && <div className="nested-subtitle sub-label">{highlightText(recParsed.label)}</div>}
                    <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(recText); setSaveError(null); } }}>
                      {isEditing ? (
                        <div className="edit-field-container">
                          ...
                          <div className="edit-actions">
                            <button className="save-btn" disabled={saving} onClick={e => {
                              ...
                              // Stage the full edited array as a DRAFT (no DB write). Approve commits it.
                        ...
                        <>
                          <div className="row-content"><span className="content-value">{highlightText(recParsed.isLabeled ? recParsed.value : recText)}</span><span className="edit-indicator">&#9998;</span></div>
                          <button className={`copy-btn ${copiedItems[itemKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(recParsed.isLabeled ? `${recParsed.label}\n${recParsed.value}${recDate ? ` (${recDate})` : ''}` : `${recText}${recDate ? ` (${recDate})` : ''}`, itemKey); }}>{copiedItems[itemKey] ? 'Copied!' : 'Copy'}</button>
                        </>
                      )}
                    </div>
  ...
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    const id = safeId(record); if (!id) return;
    // Build the full edited root object, then stage it as a DRAFT (no DB write). Approve commits
    // the whole `rootField` object to MongoDB. We deliberately store the WHOLE object (not a dotted
    // leaf) so the localEdits key stays "rootField-idx" — the file's own editKey convention.
    setLocalEdits(prev => {
  ...
  if (!records || records.length === 0) {
    return (
      <div className="health-maintenance-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Health Maintenance</h2></div>
        <div className="empty-state">No health maintenance records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="health-maintenance-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Health Maintenance</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<HealthMaintenanceDocumentPDFTemplate document={pdfData} />} fileName="Health_Maintenance.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search health maintenance..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Health Maintenance ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'session-info')}
            {renderSection(record, idx, 'screening-schedules')}
...
export default HealthMaintenanceDocument;
