...
function buildContext(req, operation = 'read') { return { serviceId: 'bunion-surgery-evaluation-edit-service', userId: req.user?.id, operation, practiceId: req.practiceContext?.subdomain || req.practiceContext?.practiceId, permissions: [operation === 'read' ? 'read' : 'write'] }; }
function toObjectId(str) { try { return new mongoose.Types.ObjectId(str); } catch { return null; } }
const ALLOWED_FIELDS = ['halluxValgusAngle', 'intermetatarsalAngle', 'distalMetatarsalArticularAngle', 'sesamoidSubluxationGrade', 'firstMtpJointCongruency', 'firstMtpRangeOfMotion', 'halluximeterMeasurement', 'manchesterOxfordFootQuestionnaire', 'aofasHalluxScore', 'firstMtpJointDegenerativeChanges', 'metatarsusAdductusAngle', 'firstMetatarsalLength', 'firstTmtJointHypermobility', 'mearyAngle', 'calcanealpitchAngle', 'medialEminenceSize', 'halluxInterphalangealAngle', 'bursitisOverMedialEminence', 'recommendedSurgicalProcedure', 'peripheralVascularStatus', 'diabeticFootRiskCategory', 'boneMineralDensityTScore', 'nicotineUseStatus'];
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('bunion_surgery_evaluation', { _id: objectId }, updateOp, context);
    return res.json({ success: true, recordId: id, editedField: field });
  } catch (err) { console.error('[BunionSurgeryEvaluation] PUT /:id/edit error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    const sda = getSecureDataAccess(); const context = buildContext(req, 'write');
    await sda.update('bunion_surgery_evaluation', { _id: objectId }, { $set: { 'doctorEdits.approvedAt': new Date(), 'doctorEdits.approvedBy': req.user?.id, 'doctorEdits.status': 'approved' } }, context);
    return res.json({ success: true, recordId: id, status: 'approved' });
  } catch (err) { console.error('[BunionSurgeryEvaluation] PUT /:id/approve error:', err.message); return res.status(500).json({ success: false, error: err.message }); }
});
