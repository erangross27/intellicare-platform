...
const SECTIONS = [
  { id: "consent-info", title: "Consent Information", fields: ["consentDate", "consentType", "decisionMaker", "obtainingProvider"] },
  { id: "procedure-details", title: "Procedure Details", fields: ["plannedProcedure", "alternativeName", "indication"] },
  { id: "risks", title: "Risks Discussed", fields: ["risksDiscussed"] },
  { id: "benefits", title: "Benefits Discussed", fields: ["benefitsDiscussed"] },
  { id: "alternatives", title: "Alternatives Discussed", fields: ["alternativesDiscussed"] },
  { id: "specific-risks", title: "Specific Risks", fields: ["specificRisks", "anesthesiaRisks"] },
  { id: "questions", title: "Transfusion & Patient Questions", fields: ["bloodTransfusion", "patientQuestions", "patientUnderstanding"] },
  { id: "process", title: "Consent Process", fields: ["interpreterUsed", "witnessPresent"] },
  { id: "signatures", title: "Signatures", fields: ["patientSignature", "providerSignature"] },
  { id: "recommendations", title: "Recommendations", fields: ["recommendations"] },
];
const LABELS = {
  consentDate: "Consent Date", consentType: "Consent Type",
  decisionMaker: "Decision Maker", obtainingProvider: "Obtaining Provider",
  plannedProcedure: "Planned Procedure", alternativeName: "Alternative Name",
  indication: "Indication", risksDiscussed: "Risks Discussed",
  benefitsDiscussed: "Benefits Discussed", alternativesDiscussed: "Alternatives Discussed",
  specificRisks: "Specific Risks", anesthesiaRisks: "Anesthesia Risks",
  bloodTransfusion: "Blood Transfusion", patientQuestions: "Patient Questions",
  patientUnderstanding: "Patient Understanding", interpreterUsed: "Interpreter Used",
  witnessPresent: "Witness Present", patientSignature: "Patient Signature",
  providerSignature: "Provider Signature",
  recommendations: "Recommendations",
};
const COMMA_ARRAY_FIELDS = ["patientQuestions"];
const ENUM_OPTIONS = {
  consentType: ["informed", "implied", "written", "verbal"],
  bloodTransfusion: ["consented", "declined", "not discussed"],
  interpreterUsed: ["Yes", "No"],
};
const MULTI_NUMBER_FIELDS = ["risksDiscussed", "benefitsDiscussed"];
...
const textRows = (path, value) =>
  splitTokens(value).flatMap((token, tokenIndex) => {
    ...
    const parts = fieldMatches(COMMA_ARRAY_FIELDS, path)
...
const SurgicalConsentFormsDocument = ({
  document: docProp,
  data,
  templateData,
}) => {
  const records = useMemo(() => {
    ...
    return (Array.isArray(source) ? source : [source])
      .flatMap((record) => {
        if (record?.surgical_consent_forms)
          return Array.isArray(record.surgical_consent_forms)
            ? record.surgical_consent_forms
            : [record.surgical_consent_forms];
        if (record?.documentData)
          return Array.isArray(record.documentData)
            ? record.documentData
            : record.documentData?.surgical_consent_forms
              ? Array.isArray(record.documentData.surgical_consent_forms)
                ? record.documentData.surgical_consent_forms
                : [record.documentData.surgical_consent_forms]
              : [record.documentData];
        if (record?.document)
    ...
    () =>
      records
        ...
        .filter(
          ({ record }) =>
            !searchTerm.trim() ||
            `surgical consent forms ${flatten(SECTIONS.flatMap((section) => section.fields.map((field) => record[field])))}`
              .toLowerCase()
      ...
      try {
        for (const key of keys) {
          ...
          const response = await secureApiClient.put(
            `/api/edit/surgical_consent_forms/${id}/edit`,
        ...
        await secureApiClient.put(
          `/api/edit/surgical_consent_forms/${id}/approve`,
  ...
  const renderLeaf = (
    ...
    const topLevelLabel = parseLabel(String(value)).label;
    const multiNumber =
      !bool &&
      !date &&
      !enumOptions &&
      !topLevelLabel &&
      fieldMatches(MULTI_NUMBER_FIELDS, path) &&
    ...
    const body = (
      <>
        ...
        {rows.map((row, rowIndex) => {
          ...
          const rowMultiNumber =
            !multiNumber &&
            !numeric &&
            fieldMatches(MULTI_NUMBER_FIELDS, path) &&
            /-?\d+(?:\.\d+)?/.test(row.text)
              ? numberSegments(row.text)
              : null;
          const activeMultiNumber = multiNumber || rowMultiNumber;
          const liveMultiSegments =
            editing && activeMultiNumber
              ? numberSegments(editValue)
              : activeMultiNumber;
  ...
  const renderNode = (record, path, idx, sectionId, value, label = "") => {
    ...
    if (Array.isArray(value)) {
      ...
      if (items.every(({ item }) => typeof item !== "object" || isDate(item))) {
        const groups = [];
        items.forEach((entry) => {
          const labeled = parseLabel(String(entry.item)).label;
          if (labeled) groups.push({ labeled: true, items: [entry] });
          else {
            const last = groups[groups.length - 1];
            if (last && !last.labeled) last.items.push(entry);
            else groups.push({ labeled: false, items: [entry] });
          }
        });
        return (
          <React.Fragment key={path}>
            ...
            {groups.map((group, groupIndex) => (
              <div
                className={`nested-mini-card${group.labeled ? "" : " regular-row-group"}`}
                key={`${path}-group-${groupIndex}`}
              >
  ...
  return (
    <div className="surgical-consent-forms-document" ref={containerRef}>
      <div className="document-header">
        <h1 className="document-title">Surgical Consent Forms</h1>
        <div className="header-actions">
          ...
          <PDFDownloadLink
            document={
              <SurgicalConsentFormsDocumentPDFTemplate document={pdfData} />
            }
            fileName="Surgical_Consent_Forms.pdf"
            className="copy-btn"
          >
      ...
      <div className="search-container">
        <input
          ...
          placeholder="Search surgical consent forms..."
          value={searchTerm}
      ...
      {!records.length ? (
        <div className="empty-state">
          No surgical consent form records available
        </div>
      ) : (
          ...
          {filtered.map(({ record, idx }) => (
            <div className="record-card" key={idx}>
              <div className="record-header">
                <h3 className="record-name">{`Surgical Consent Forms ${idx + 1}`}</h3>
              </div>
