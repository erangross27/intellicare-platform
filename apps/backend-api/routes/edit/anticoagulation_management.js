/**
 * Anticoagulation Management Edit Route
 * Isolated per-collection edit/approve endpoints for HIPAA compliance
 * Supports dot-path field names for nested objects
 *
 * PUT /api/edit/anticoagulation_management/:id/edit    — Edit a single field
 * PUT /api/edit/anticoagulation_management/:id/approve — Approve a section
 */
...
function buildContext(req, operation = 'read') {
  return {
    serviceId: 'anticoagulation-management-edit-service',
    userId: req.user?.id,
    operation,
...
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    await sda.update('anticoagulation_management', { _id: objectId }, {
      $set: {
        [updatePath]: value,
        updatedAt: new Date(),
    ...
    console.error('[AnticoagulationManagement] PUT /:id/edit error:', err.message);
...
router.put('/:id/approve', async (req, res) => {
  try {
    ...
    console.error('[AnticoagulationManagement] PUT /:id/approve error:', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
