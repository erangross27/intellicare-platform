/**
 * PreventiveBiomarkersDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: preventive_biomarkers
 *
 * 6 Sections:
 *   1. biomarker-identification: biomarkerType, biomarkerName, panelName
 *   2. results-assessment: resultValue, resultUnit, referenceRange, riskCategory, abnormalFlag, calculatedRiskScore
 *   3. testing-details: testMethodology, specimenType, fastingStatus, qualityControlStatus
 *   4. clinical-context: indicationForTesting, diseaseRiskAssessment, trendComparison, clinicalSignificance
 *   5. risk-interventions: riskFactorsPresent, interventionsRecommended
 *   6. followup-provider: recommendedFollowUp, retestInterval, provider, facility, interpretingPhysician, aiProcessed
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PreventiveBiomarkersDocumentPDFTemplate from '../pdf-templates/PreventiveBiomarkersDocumentPDFTemplate';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PreventiveBiomarkersDocument.css';
...
const STRING_FIELDS = ['biomarkerType', 'biomarkerName', 'panelName', 'resultUnit', 'referenceRange', 'riskCategory', 'abnormalFlag', 'testMethodology', 'specimenType', 'fastingStatus', 'qualityControlStatus', 'indicationForTesting', 'diseaseRiskAssessment', 'trendComparison', 'clinicalSignificance', 'recommendedFollowUp', 'retestInterval', 'provider', 'facility', 'interpretingPhysician'];
...
const DRAFT_KEY = 'preventive_biomarkersPendingEdits';
const readDrafts = () => {
...
/* ======= COMPONENT ======= */
const PreventiveBiomarkersDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.preventive_biomarkers) return Array.isArray(r.preventive_biomarkers) ? r.preventive_biomarkers : [r.preventive_biomarkers];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.preventive_biomarkers) return Array.isArray(dd.preventive_biomarkers) ? dd.preventive_biomarkers : [dd.preventive_biomarkers]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Preventive Biomarkers ${idx + 1}`.toLowerCase();
  ...
  /* ======= EDIT HANDLERS ======= */
  // Persist a single field's draft into the localStorage store keyed by record id + field name.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const fields = SECTION_FIELDS[sid] || [];
    try {
      // localEdits keys are "<field>-<idx>" (value = full field value, incl. full arrays for array fields).
      const toCommit = Object.keys(localEdits).filter(k => {
        if (!pendingEdits[k]) return false;
        const m = k.match(/^(.+)-(\d+)$/);
        if (!m || parseInt(m[2], 10) !== idx) return false;
        // field part: arrayIndex ONLY when the trailing dot-segment is purely numeric
        const fieldPart = m[1];
      ...
      for (const editKey of toCommit) {
        const fieldPart = editKey.match(/^(.+)-(\d+)$/)[1];
        const lastDot = fieldPart.lastIndexOf('.');
        ...
        const resp = await secureApiClient.put(`/api/edit/preventive_biomarkers/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/preventive_biomarkers/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PREVENTIVE BIOMARKERS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Preventive Biomarkers ${idx + 1}\n${'='.repeat(40)}\n\n`;
      Object.keys(SECTION_FIELDS).forEach(sid => {
  ...
  /* date renders as a display-only record-header pill (not a section field) — no editable date renderer */
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        ...
                        {saveError && <div className="save-error">{saveError}</div>}
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); if (parsed.isLabeled) { const trimmed = editValue.trim(); const subParts = trimmed.split(/\.\s+/).map(sp => sp.replace(/[;.]+$/, '').trim()).filter(sp => sp); const newValue = subParts.join(', '); const reconstructed = `${parsed.label}: ${newValue}`; const sentences2 = splitBySentence(String(getFieldValue(record, fn, idx) || '')); sentences2[sIdx] = reconstructed; const fullText = reconstructFullText(sentences2); setSaveError(null); setLocalEdits(prev => ({ ...prev, [`${fn}-${idx}`]: fullText })); setPendingEdits(prev => ({ ...prev, [`${fn}-${idx}`]: true })); const marks = { [sentenceKey]: 'edited' }; for (let ei = 1; ei < subParts.length; ei++) marks[`${fn}-${idx}-s${sIdx}-c${ei}`] = 'added'; setEditedSentences(prev => ({ ...prev, ...marks })); clearApprovedForField(idx, fn); stageDraft(record, fn, fullText); setEditingField(null); setEditValue(''); } else { saveSentence(record, fn, idx, sid, sIdx); } }}>{saving ? 'Saving...' : 'Save'}</button>
                          <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
  ...
  if (!records || records.length === 0) {
    return (
      <div className="preventive-biomarkers-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Preventive Biomarkers</h2></div>
        <div className="empty-state">No preventive biomarkers records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="preventive-biomarkers-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Preventive Biomarkers</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PreventiveBiomarkersDocumentPDFTemplate document={pdfData} />} fileName="Preventive_Biomarkers.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search preventive biomarkers..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              ...
              <h3 className="record-name">{highlightText(`Preventive Biomarkers ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'biomarker-identification')}
            {renderSection(record, idx, 'results-assessment')}
            {renderSection(record, idx, 'testing-details')}
            {renderSection(record, idx, 'clinical-context')}
            {renderSection(record, idx, 'risk-interventions')}
            {renderSection(record, idx, 'followup-provider')}
          </div>
        ))}
      </div>
    </div>
...
export default PreventiveBiomarkersDocument;
