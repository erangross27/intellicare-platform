/**
 * InsulinTimingInstructionsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: insulin_timing_instructions
 *
 * 11 Sections:
 *   1. insulin-overview: insulinType, insulinBrandName, insulinDeliveryMethod
 *   2. dosing: basalInsulinDosage, bolusInsulinDosage, correctionFactor, carbohydrateRatio
 *   3. administration-timing: basalAdministrationTime, premealTimingMinutes, prandialInsulinTiming, dawnPhenomenonAdjustment
 *   4. pharmacokinetics: onsetOfAction, peakActionTime, durationOfAction
 *   5. glucose-targets: targetBloodGlucoseRange, hypoglycemiaThreshold, glucoseMonitoringFrequency
 *   6. sliding-scale: slidingScaleProtocol (array)
 *   7. meal-timing: mealTimingInstructions
 *   8. injection-site: injectionSiteRotation
 *   9. exercise: exerciseAdjustments
 *   10. sick-day: sickDayManagement
 *   11. storage: insulinStorageInstructions
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import InsulinTimingInstructionsDocumentPDFTemplate from '../pdf-templates/InsulinTimingInstructionsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './InsulinTimingInstructionsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; value = string or full array) */
const DRAFT_KEY = 'insulin_timing_instructionsPendingEdits';
const readDrafts = () => {
...
/* 5 number fields — ALL use input[type=number step=any] + parseFloat+isNaN→block save */
const NUMBER_FIELDS = [
  'basalInsulinDosage', 'bolusInsulinDosage', 'correctionFactor',
  'premealTimingMinutes', 'hypoglycemiaThreshold',
];

/* MEANINGFUL_ZERO_FIELDS: numeric fields where 0 is a real clinical value, not a "not set" sentinel.
   None here — insulin dosages, correction factor, pre-meal timing and the hypoglycemia threshold
   are all positive quantities; a stored 0 means "not prescribed / not set" and is hidden. */
const MEANINGFUL_ZERO_FIELDS = new Set([]);

const STRING_FIELDS = [
  'insulinType', 'insulinBrandName', 'insulinDeliveryMethod',
  'carbohydrateRatio', 'basalAdministrationTime', 'prandialInsulinTiming',
  'dawnPhenomenonAdjustment', 'onsetOfAction', 'peakActionTime', 'durationOfAction',
  'targetBloodGlucoseRange', 'glucoseMonitoringFrequency',
  'mealTimingInstructions', 'injectionSiteRotation', 'exerciseAdjustments',
  'sickDayManagement', 'insulinStorageInstructions',
];
...
/* ═══════ COMPONENT ═══════ */
const InsulinTimingInstructionsDocument = ({ document: docProp }) => {
  ...
  /* fieldHasVal: for NUMBER_FIELDS not in MEANINGFUL_ZERO_FIELDS, a stored 0 is a "not set" sentinel and is hidden. */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Insulin Timing Instructions ${idx + 1}`.toLowerCase();
  ...
  // stageDraft: stash the full rebuilt field text as a local DRAFT (no DB write). Survives refresh via
  // localStorage; Approve commits it. Mirrors handleSaveField's local-only staging.
  ...
  /* ═══════ ARRAY ITEM SAVE ═══════ */
  const handleSaveArrayItem = useCallback((record, fn, idx, sid, arrayIdx) => {
    const id = safeId(record); if (!id) return;
    const trimmed = editValue.trim();
    if (!trimmed) return;
    setSaveError(null);
    const arr = [...getEffectiveArray(record, fn, idx)];
    arr[arrayIdx] = trimmed;
    // Stage the WHOLE array as a local DRAFT (no DB write). Approve commits the full array later.
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    const id = safeId(record); if (!id) return;
    const fields = SECTION_FIELDS[sid] || [];
    // editKey format is `${field}-${idx}`; only commit pending edits whose field belongs to this section.
    ...
    try {
      // Persist each staged field to the DB now (whole string OR whole array value)
      for (const editKey of toCommit) {
        const fieldPart = editKey.slice(0, -(`-${idx}`).length);
        await secureApiClient.put(`/api/edit/insulin_timing_instructions/${id}/edit`, { field: fieldPart, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/insulin_timing_instructions/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== INSULIN TIMING INSTRUCTIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Insulin Timing Instructions ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="insulin-timing-instructions-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Insulin Timing Instructions</h2></div>
        <div className="empty-state">No insulin timing instructions records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="insulin-timing-instructions-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Insulin Timing Instructions</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<InsulinTimingInstructionsDocumentPDFTemplate document={pdfData} />} fileName="Insulin_Timing_Instructions.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search insulin timing instructions..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Insulin Timing Instructions ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'insulin-overview')}
            {renderSection(record, idx, 'dosing')}
            {renderSection(record, idx, 'administration-timing')}
            {renderSection(record, idx, 'pharmacokinetics')}
            {renderSection(record, idx, 'glucose-targets')}
            {renderSection(record, idx, 'sliding-scale')}
            {renderSection(record, idx, 'meal-timing')}
            {renderSection(record, idx, 'injection-site')}
            {renderSection(record, idx, 'exercise')}
            {renderSection(record, idx, 'sick-day')}
            {renderSection(record, idx, 'storage')}
          </div>
        ))}
      </div>
    </div>
...
export default InsulinTimingInstructionsDocument;
