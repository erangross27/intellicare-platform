/**
 * MultipleSclerosisAssessmentDocument.jsx
 * Canonical one-pass polish — blue glow editing theme
 * Collection: multiple_sclerosis_assessment
 *
 * Sections: Record Information · MS Details · MRI Findings · Findings · Assessment ·
 *           Plan · Recommendations · Results · Notes
 *
 * Scalar string fields are inline-editable (renderStringField). The record `date` is an
 * editable BlueDatePicker row (NEVER keyed off createdAt/updatedAt). `edssScore` is a
 * num-stepper number field. `mriFindings` (deeply nested: scalars + mcDonaldCriteria →
 * disseminationInSpace/inTime with boolean `met` leaves) and currentDmt/symptomManagement/
 * results are decomposed for display (label ABOVE value — never a side-by-side "Label: value"
 * span) via a generic recursive renderer; boolean leaves render "Yes"/"No". `recommendations`
 * is an array of {recommendation, date} — each object → a nested-subtitle (recommendation)
 * plus an editable BlueDatePicker date row.
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import MultipleSclerosisAssessmentDocumentPDFTemplate from '../pdf-templates/MultipleSclerosisAssessmentDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import secureApiClient from '../../../services/secureApiClient';
import './MultipleSclerosisAssessmentDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = field name; "${fn}-${idx}" → fn) */
const DRAFT_KEY = 'multiple_sclerosis_assessmentPendingEdits';
const readDrafts = () => {
...
const SECTION_FIELDS = {
  'record-info': ['date', 'msType', 'type', 'provider', 'facility', 'status'],
  'ms-details': ['edssScore', 'relapseHistory', 'currentDmt', 'symptomManagement'],
...
/* Editable scalar strings — inline text editing */
const STRING_FIELDS = ['msType', 'type', 'provider', 'facility', 'status', 'findings', 'assessment', 'plan', 'notes'];
/* Editable date (BlueDatePicker) — record date is `date`, NEVER createdAt/updatedAt */
const DATE_FIELDS = ['date'];
/* Editable number (num-stepper) */
const NUMBER_FIELDS = ['edssScore'];
/* Read-only scalars (none for this record) */
const DISPLAY_FIELDS = [];
/* Read-only, decomposed arrays + nested objects */
const ARRAY_FIELDS = ['relapseHistory'];
const OBJECT_FIELDS = ['mriFindings', 'currentDmt', 'symptomManagement', 'results'];
/* Array-of-objects: recommendations = [{recommendation, date}] */
const OBJECT_ARRAY_FIELDS = ['recommendations'];

/* ═══════ OBJECT-FIELD HELPERS ═══════ */
const KEY_OVERRIDES = {
  ms: 'MS', mri: 'MRI', dmt: 'DMT', edss: 'EDSS', cns: 'CNS', csf: 'CSF', igg: 'IgG',
  t1: 'T1', t2: 'T2', dis: 'DIS', dit: 'DIT', ivig: 'IVIG', ai: 'AI',
  mcdonaldcriteria: 'McDonald Criteria', disseminationinspace: 'Dissemination in Space',
  disseminationintime: 'Dissemination in Time',
};
...
/* object → copy lines with the KEY on its own line then the value (never side-by-side "Key: value").
   A scalar string value that is itself "Label: value" is decomposed onto separate lines too. */
const objectCopyBlock = (value, lines, indent) => {
  ...
  Object.entries(value).filter(([, v]) => !isEmptyDeep(v)).forEach(([k, v]) => {
    if (isScalar(v)) {
      ...
      const str = fmtScalar(v);
      const parsed = typeof v === 'string' ? parseLabel(str) : { isLabeled: false };
      if (parsed.isLabeled) { lines.push(`${pad}${parsed.label}`); lines.push(`${pad}${parsed.value}`); }
      else lines.push(`${pad}${str}`);
    } else if (Array.isArray(v)) {
      lines.push(`${pad}${humanizeKey(k)}`);
      v.filter(x => !isEmptyDeep(x)).forEach((it, i) => {
        if (isScalar(it)) lines.push(`${pad}  ${i + 1}. ${fmtScalar(it)}`);
        else objectCopyBlock(it, lines, indent + 1);
      });
...
const MultipleSclerosisAssessmentDocument = ({ document: docProp, data, templateData }) => {
  const rawData = templateData || docProp || data;
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.multiple_sclerosis_assessment) return Array.isArray(r.multiple_sclerosis_assessment) ? r.multiple_sclerosis_assessment : [r.multiple_sclerosis_assessment];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.multiple_sclerosis_assessment) return Array.isArray(dd.multiple_sclerosis_assessment) ? dd.multiple_sclerosis_assessment : [dd.multiple_sclerosis_assessment]; return [dd]; }
      return [r];
    });
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Multiple Sclerosis Assessment ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      ...
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/multiple_sclerosis_assessment/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/multiple_sclerosis_assessment/${id}/approve`, { sectionId: sid, approved: true });
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        items.forEach((it, i) => {
          ...
          else { const lines = []; objectCopyBlock(it, lines, 1); lines.forEach(l => { text += `${l}\n`; }); }
        });
        text += '\n';
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== MULTIPLE SCLEROSIS ASSESSMENT ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Multiple Sclerosis Assessment ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: DISPLAY (read-only scalar) ═══════ */
  const renderDisplayField = (record, fn, idx, sid, title) => {
  ...
  /* ═══════ RENDER: OBJECT NODE (recursive, read-only, label ABOVE value at every depth) ═══════ */
  const renderObjectNode = (label, value, key) => {
    if (isEmptyDeep(value)) return null;
    if (isScalar(value)) {
      const str = fmtScalar(value);
      const parsed = typeof value === 'string' ? parseLabel(str) : { isLabeled: false, label: '', value: str };
      if (parsed.isLabeled) {
        return (
          <div key={key} className="nested-mini-card">
            ...
            <div className="nested-mini-card">
              ...
              <div className="numbered-row">
                <div className="row-content"><span className="content-value">{highlightText(parsed.value)}</span></div>
                <button className={`copy-btn ${copiedItems[key] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(str, key); }}>{copiedItems[key] ? 'Copied!' : 'Copy'}</button>
              </div>
            </div>
          </div>
        );
      ...
      return (
        <div key={key} className="nested-mini-card">
          <div className="nested-subtitle sub-label">{highlightText(label)}</div>
          <div className="numbered-row">
            <div className="row-content"><span className="content-value">{highlightText(str)}</span></div>
            <button className={`copy-btn ${copiedItems[key] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(str, key); }}>{copiedItems[key] ? 'Copied!' : 'Copy'}</button>
          </div>
        </div>
      );
  ...
  /* ═══════ RENDER: OBJECT FIELD (mriFindings / currentDmt / symptomManagement / results — read-only) ═══════ */
  const renderObjectField = (record, fn, idx, sid, title) => {
    const val = getFieldValue(record, fn, idx);
    if (isEmptyDeep(val) || isScalar(val) || Array.isArray(val)) return null;
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
  ...
  /* ═══════ RENDER: ARRAY FIELD (read-only, decomposed) ═══════ */
  const renderArrayField = (record, fn, idx, sid, title) => {
    ...
    return (
      <div key={fn} className="rec-mini-card">
        {showLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
        {items.map((item, i) => {
          if (!isScalar(item)) return renderObjectNode(`Item ${i + 1}`, item, `${fn}-${idx}-${i}`);
          const str = fmtScalar(item);
          const parsed = parseLabel(str);
          const itemKey = `${fn}-${idx}-${i}`;
          if (parsed.isLabeled) {
            return (
              <div key={i} className="nested-mini-card">
                ...
                <div className="numbered-row">
                  <div className="row-content"><span className="content-value">{highlightText(parsed.value)}</span></div>
                  <button className={`copy-btn ${copiedItems[itemKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(str, itemKey); }}>{copiedItems[itemKey] ? 'Copied!' : 'Copy'}</button>
                </div>
              </div>
            );
          }
  ...
  /* ═══════ RENDER: RECOMMENDATIONS — array of {recommendation, date} ═══════
     Each object → a nested-subtitle (recommendation text) + a formatted BlueDatePicker date row. */
  const renderRecommendationsField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    const recs = Array.isArray(val) ? val : [];
    if (recs.filter(r => (r?.recommendation || '').trim()).length === 0) return null;
    const label = FIELD_LABELS[fn] || fn;
    ...
    return (
      <div key={fn} className="rec-mini-card">
        ...
        {recs.map((rec, rIdx) => {
          ...
          const itemMatches = phraseMatch || labelMatch || !searchTerm.trim() || recText.toLowerCase().includes(phrase) || displayDate.toLowerCase().includes(phrase);
          if (!itemMatches && searchTerm.trim()) return null;
          return (
            <div key={rIdx} className="nested-mini-card">
              <div className="nested-subtitle sub-label">{highlightText(recText)}</div>
              {displayDate && (
                <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(itemKey); setEditValue(toInputDate(recDate)); setSaveError(null); } }}>
                  {isEditing ? (
                    <div className="edit-field-container">
                      ...
                      <div className="edit-actions">
                        <button className="save-btn" disabled={saving} onClick={e => {
                          ...
                          const currentArr = Array.isArray(getFieldValue(record, fn, idx)) ? getFieldValue(record, fn, idx).map(x => (x && typeof x === 'object') ? { ...x } : x) : [];
                          const cur = currentArr[rIdx];
                          if (cur && typeof cur === 'object') currentArr[rIdx] = { ...cur, date: editValue };
                          ...
                          setApprovedSections(prev => { if (!prev[`${sid}-${idx}`]) return prev; const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
                          const store = readDrafts(); if (!store[id2]) store[id2] = {}; store[id2][fn] = currentArr; writeDrafts(store);
                          setEditingField(null); setEditValue('');
                    ...
                    <>
                      ...
                      <button className={`copy-btn ${copiedItems[itemKey] ? 'copied' : ''}`} onClick={e => { e.stopPropagation(); copyItem(`${recText}${displayDate ? ` (${displayDate})` : ''}`, itemKey); }}>{copiedItems[itemKey] ? 'Copied!' : 'Copy'}</button>
                    </>
                  )}
                </div>
              )}
  ...
  /* ═══════ RENDER: DATE FIELD (record date — BlueDatePicker) ═══════ */
              ...
              return (
                <div key={sIdx} className={parsed.isLabeled ? 'rec-mini-card' : ''} style={parsed.isLabeled ? { marginTop: 8 } : undefined}>
                  ...
                  <div className={`numbered-row ${badge ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(sentenceKey); setEditValue(parsed.isLabeled ? parsed.value : sentence.replace(/[;.]+$/, '').trim()); setSaveError(null); } }}>
                    {isEditing ? (
                      <div className="edit-field-container">
                        {parsed.isLabeled && /^-?\d+(\.\d+)?$/.test(String(parsed.value).trim()) ? (
                          <div className="num-stepper-row">
                            <button type="button" className="num-step" disabled={saving} onClick={e => { e.stopPropagation(); const cur = parseFloat(editValue); setEditValue(String((isNaN(cur) ? 0 : cur) - 1)); }}>&minus;</button>
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (DISPLAY_FIELDS.includes(f)) return renderDisplayField(record, f, idx, sid, title);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="multiple-sclerosis-assessment-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Multiple Sclerosis Assessment</h2></div>
        <div className="empty-state">No multiple sclerosis assessment records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="multiple-sclerosis-assessment-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Multiple Sclerosis Assessment</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<MultipleSclerosisAssessmentDocumentPDFTemplate document={pdfData} />} fileName="Multiple_Sclerosis_Assessment.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search multiple sclerosis assessments..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Multiple Sclerosis Assessment ${idx + 1}`)}</h3>
            </div>
...
export default MultipleSclerosisAssessmentDocument;
