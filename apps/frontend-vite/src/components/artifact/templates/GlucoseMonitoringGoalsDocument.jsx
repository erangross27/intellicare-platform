/**
 * GlucoseMonitoringGoalsDocument.jsx
 * March 2026 — Complete rewrite with inline editing, blue glow theme
 * Collection: glucose_monitoring_goals
 *
 * 4 Sections:
 *   1. primary-targets: targetFastingGlucose, targetPostprandialGlucose, targetHbA1c, individualizedHbA1cGoal
 *   2. time-in-range: timeInRangeGoal, timeAboveRangeLimit, timeBelowRangeLimit,
 *                     coefficientOfVariation, meanAmplitudeGlycemicExcursions,
 *                     glucoseManagementIndicator, bgmiTarget, sensorGlucoseTargetRange
 *   3. specialized: gestationalGlucoseTargets (ARRAY numbers), mealTimeGlucoseTargets (ARRAY numbers),
 *                   exerciseGlucoseManagement, intensiveCareGlucoseTarget, hypoglycemiaAwarenessGoal
 *   4. special-populations: prepregnancyGlucoseTarget, geriatricHbA1cTarget,
 *                           chronicKidneyDiseaseGlucoseTarget, cardiovascularRiskBasedTarget,
 *                           pediatricGlucoseGoals (ARRAY), perioperativeGlucoseGoals (ARRAY)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import GlucoseMonitoringGoalsDocumentPDFTemplate from '../pdf-templates/GlucoseMonitoringGoalsDocumentPDFTemplate';
import secureApiClient from '../../../services/secureApiClient';
import './GlucoseMonitoringGoalsDocument.css';
...
/* Arrays of numbers: gestationalGlucoseTargets, mealTimeGlucoseTargets */
/* Arrays (possibly empty): pediatricGlucoseGoals, perioperativeGlucoseGoals */
const ARRAY_NUMBER_FIELDS = ['gestationalGlucoseTargets', 'mealTimeGlucoseTargets'];
const ARRAY_STRING_FIELDS = ['pediatricGlucoseGoals', 'perioperativeGlucoseGoals'];
const ALL_ARRAY_FIELDS = [...ARRAY_NUMBER_FIELDS, ...ARRAY_STRING_FIELDS];

/* Sentinel-zero: a glucose target of 0 (mg/dL or %) is never a real goal — it's the extractor's unset default,
   so 0-valued NUMBER_FIELDS are HIDDEN everywhere (JSX render + section-visibility + copy + PDF). Memory 6a4fa368. */
...
const DRAFT_KEY = 'glucose_monitoring_goalsPendingEdits';
const readDrafts = () => {
...
/* ═══════ COMPONENT ═══════ */
const GlucoseMonitoringGoalsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.glucose_monitoring_goals) return Array.isArray(r.glucose_monitoring_goals) ? r.glucose_monitoring_goals : [r.glucose_monitoring_goals];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.glucose_monitoring_goals) return Array.isArray(dd.glucose_monitoring_goals) ? dd.glucose_monitoring_goals : [dd.glucose_monitoring_goals]; return [dd]; }
      return [r];
    });
  ...
  /* hasFieldVal: hasVal + sentinel-zero hide (a 0-valued glucose target is the extractor's unset default) */
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Glucose Monitoring Goals ${idx + 1}`.toLowerCase();
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const field = m[1];
        await secureApiClient.put(`/api/edit/glucose_monitoring_goals/${id}/edit`, { field, value: localEdits[editKey] });
      }
      await secureApiClient.put(`/api/edit/glucose_monitoring_goals/${id}/approve`, { sectionId: sid, approved: true });
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== GLUCOSE MONITORING GOALS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Glucose Monitoring Goals ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  if (!records || records.length === 0) {
    return (
      <div className="glucose-monitoring-goals-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Glucose Monitoring Goals</h2></div>
        <div className="empty-state">No glucose monitoring goals records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="glucose-monitoring-goals-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Glucose Monitoring Goals</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<GlucoseMonitoringGoalsDocumentPDFTemplate document={pdfData} />} fileName="Glucose_Monitoring_Goals.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search glucose monitoring goals..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(`Glucose Monitoring Goals ${idx + 1}`)}</h3>
            </div>
            {renderSection(record, idx, 'primary-targets')}
            {renderSection(record, idx, 'time-in-range')}
            {renderSection(record, idx, 'specialized')}
            {renderSection(record, idx, 'special-populations')}
          </div>
        ))}
      </div>
    </div>
...
export default GlucoseMonitoringGoalsDocument;
