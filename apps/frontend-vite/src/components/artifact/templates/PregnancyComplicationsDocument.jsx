/**
 * PregnancyComplicationsDocument.jsx
 * June 2026 — FULL standard: 100% field coverage, recursive objects, per-sentence narratives
 * Collection: pregnancy_complications
 *
 * 9 Sections:
 *   1. header-info: date, provider, facility, status
 *   2. clinical-info: findings, assessment, plan, notes (per-sentence editable)
 *   3. hypertensive-disorders: hypertensiveDisorders (object: type, severity, proteinuria, management[])
 *   4. placental-complications: placentalComplications (object: previaType, accreta, abruption, insufficiency)
 *   5. preterm-labor: pretermLabor (object: cervicalLength, ffnResult, episodes[], tocolytics[])
 *   6. fluid-status: iugr, polyhydramnios, oligohydramnios (Yes/No)
 *   7. infections: infections (array of {type, status, notes})
 *   8. results: results (object — recursive key/value)
 *   9. recommendations: recommendations (array of {recommendation, date}, date-grouped)
 */
import React, { useState, useRef, useMemo, useCallback, useEffect } from 'react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import PregnancyComplicationsDocumentPDFTemplate from '../pdf-templates/PregnancyComplicationsDocumentPDFTemplate';
import BlueDatePicker from '../components/BlueDatePicker';
import BlueSelect from '../components/BlueSelect';
import secureApiClient from '../../../services/secureApiClient';
import './PregnancyComplicationsDocument.css';

/* Pending-edit DRAFT store (localStorage). Drafts survive refresh + show in the JSX, but are NOT
   written to MongoDB and NOT shown in the PDF until the user clicks Approve.
   Kept in a SEPARATE key (NOT artifactGridData) so drafts never leak into the PDF/DB source.
   Shape: { [recordId]: { [fieldPart]: value } }  (fieldPart = localEdits key sans the "-<idx>" suffix) */
const DRAFT_KEY = 'pregnancy_complicationsPendingEdits';
const readDrafts = () => {
...
/* recursive objects (humanizeKey + nested-mini-card; editable leaves) */
...
/* array-of-objects {type, status, notes} — generic key/value entries */
...
const STRING_FIELDS = ['type', 'provider', 'facility', 'status', 'findings', 'assessment', 'plan', 'notes'];

/* ═══════ OBJECT-LEAF HELPERS (recursive renderers) ═══════ */
...
/* ═══════ COMPONENT ═══════ */
const PregnancyComplicationsDocument = ({ document: docProp }) => {
  ...
  const records = useMemo(() => {
    ...
    arr = arr.flatMap(r => {
      if (r?.pregnancy_complications) return Array.isArray(r.pregnancy_complications) ? r.pregnancy_complications : [r.pregnancy_complications];
      if (r?.documentData) { const dd = r.documentData; if (Array.isArray(dd)) return dd; if (dd?.pregnancy_complications) return Array.isArray(dd.pregnancy_complications) ? dd.pregnancy_complications : [dd.pregnancy_complications]; return [dd]; }
      return [r];
    });
  ...
  useEffect(() => {
    ...
    const ridOf = (r) => { if (!r?._id) return null; if (typeof r._id === 'string') return r._id; if (r._id.$oid) return r._id.$oid; return String(r._id); };
  ...
  const splitBySentence = useCallback((text) => {
    ...
    // paren-protect '.'/';' inside parentheses so "(LOW; declining ...)" / "(e.g. ...)" stay whole
  ...
  const shouldShowSection = useCallback((record, sid) => {
    ...
    for (const f of fields) {
      ...
      if (val !== null && val !== undefined) {
        if (typeof val === 'boolean') { if (fmtVal(val).toLowerCase().includes(phrase)) return true; }
        else if (Array.isArray(val)) { if (val.some(item => { const s = typeof item === 'object' ? JSON.stringify(item) : String(item); return s.toLowerCase().includes(phrase); })) return true; }
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record, idx) => {
      ...
      const rt = `Pregnancy Complications ${idx + 1}`.toLowerCase();
  ...
  /* save a single nested object leaf — stage the rebuilt root object as a DRAFT (no DB write). */
  const saveLeaf = useCallback((record, rootField, path, idx, sid, leafKeyTrack, newVal) => {
    ...
    setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
    // localStorage draft store (survives refresh). Store the rebuilt root object.
    {
      const cur = localEdits[editKey] !== undefined ? localEdits[editKey] : record[rootField];
      const draftClone = JSON.parse(JSON.stringify(cur ?? {}));
      let node = draftClone;
      for (let i = 0; i < path.length - 1; i++) node = node[path[i]];
      node[path[path.length - 1]] = newVal;
      const store = readDrafts();
      if (!store[id]) store[id] = {};
      store[id][rootField] = draftClone;
      writeDrafts(store);
    }
    setEditingField(null); setEditValue(''); setSaveError(null);
  }, [safeId, localEdits]);
  ...
  function stageSentenceDraft(record, fn, idx, sid, fullText, markFn) {
    const id = safeId(record); if (!id) return;
    const editKey = `${fn}-${idx}`;
  ...
  const handleApproveSection = useCallback(async (record, sid, idx) => {
    ...
    const toCommit = Object.keys(localEdits).filter(k => {
      ...
      const baseField = (() => { const lastDot = fieldPart.lastIndexOf('.'); return lastDot !== -1 && /^\d+$/.test(fieldPart.slice(lastDot + 1)) ? fieldPart.slice(0, lastDot) : fieldPart; })();
    ...
    try {
      for (const editKey of toCommit) {
        ...
        const resp = await secureApiClient.put(`/api/edit/pregnancy_complications/${id}/edit`, payload);
        if (resp && resp.success === false) throw new Error(resp.error || 'save failed');
      }
      await secureApiClient.put(`/api/edit/pregnancy_complications/${id}/approve`, { sectionId: sid, approved: true });
      ...
      const store = readDrafts();
      if (store[id]) {
        toCommit.forEach(k => { const fp = k.slice(0, -suffix.length); const base = fp.includes('.') && /^\d+$/.test(fp.slice(fp.lastIndexOf('.') + 1)) ? fp.slice(0, fp.lastIndexOf('.')) : fp; delete store[id][base]; });
    ...
    } catch (err) { console.error('[PregnancyComplications] Approve error:', err); }
  ...
  const objectCopyLines = useCallback((label, value, indent) => {
    ...
    if (isScalar(value)) {
      const strv = fmtScalar(value);
      if (label) out.push(`${pad}${label}`);
      if (typeof value === 'string') {
        const parsed = parseLabel(strv);
        if (splitBySentence(strv).length > 1 || (parsed.isLabeled && splitByComma(parsed.value).length >= 2)) {
          formatSentenceFieldLines(strv).forEach(l => out.push(`${pad}${l}`));
          return out;
        }
      }
      out.push(`${pad}${strv}`);
    ...
    if (Array.isArray(value)) {
      ...
      value.filter(x => !isEmptyDeep(x)).forEach((item, i) => {
        if (isScalar(item)) out.push(`${pad}  ${i + 1}. ${fmtScalar(item)}`);
        else objectCopyLines('', item, indent + 1).forEach(l => out.push(l));
      });
      return out;
    }
    if (label) out.push(`${pad}${label}:`);
  ...
  const buildSectionCopyText = useCallback((record, idx, sid) => {
    ...
    fields.forEach(f => {
      ...
      if (DATE_FIELDS.includes(f)) {
        ...
        text += '\n';
      } else if (INFECTION_ARRAY_FIELDS.includes(f)) {
        const items = Array.isArray(val) ? val.filter(x => !isEmptyDeep(x)) : [];
        if (!sameAsTitle) text += `${label}\n`;
        items.forEach((item, i) => {
          text += `${i + 1}.\n`;
          objectCopyLines('', item, 1).forEach(l => { text += `${l}\n`; });
        });
        text += '\n';
  ...
  const copyAllText = useCallback(async () => {
    let text = '=== PREGNANCY COMPLICATIONS ===\n\n';
    pdfData.forEach((r, idx) => {
      text += `Pregnancy Complications ${idx + 1}\n${'='.repeat(40)}\n\n`;
  ...
  /* ═══════ RENDER: OBJECT LEAF (editable scalar; bool→Yes/No, number+unit→number input, ratio stays text) ═══════ */
  const renderObjectLeaf = (record, rootField, path, idx, sid, value) => {
    ...
    const parsedLeaf = (isBool || ratio || nu) ? { isLabeled: false } : parseLabel(leafValueString);
    const leafSentences = (isBool || ratio || nu) ? [] : splitBySentence(leafValueString);
    const isNarrative = typeof value === 'string' && !isBool && !ratio && !nu && (leafSentences.length > 1 || parsedLeaf.isLabeled);

    /* NARRATIVE string leaf: label ABOVE the value; each sentence its own value row (label stripped into a
       content-subtitle-label) so no single .content-value is "Label: value". Edit the whole leaf in one textarea. */
    if (isNarrative) {
      return (
          ...
          <div className={`numbered-row ${isModified ? 'modified' : ''} editable-row`} onClick={() => { if (!isEditing) { setEditingField(leafKey); setEditValue(leafValueString); setSaveError(null); } }}>
            {isEditing ? (
              <div className="edit-field-container">
                ...
                {saveError && <div className="save-error">{saveError}</div>}
                <div className="edit-actions">
                  <button className="save-btn" disabled={saving} onClick={e => { e.stopPropagation(); saveLeaf(record, rootField, path, idx, sid, leafKey, editValue.trim()); }}>{saving ? 'Saving...' : 'Save'}</button>
                  <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                </div>
              </div>
            ) : (
              <>
                <div className="row-content" style={{ flexDirection: 'column', alignItems: 'stretch', gap: 4 }}>
                  {leafSentences.map((s, si) => {
                    const p = parseLabel(s);
                    if (p.isLabeled) {
                      const parts = splitByComma(p.value);
                      const shown = parts.length >= 2 ? parts : [p.value];
                      return (
                        <div key={si} style={{ width: '100%' }}>
                          <div className="content-subtitle-label">{highlightText(p.label)}</div>
                          {shown.map((pt, pi) => <span key={pi} className="content-value">{highlightText(pt)}</span>)}
                        </div>
                      );
                    }
                    return <span key={si} className="content-value">{highlightText(s)}</span>;
                  })}
              ...
              {isBool ? (
                ...
                <div className="number-edit-row">
                  <div className="num-stepper-row">
                    <button className="num-step" onClick={() => setEditValue(v => { const s = stepFor(v); const n = (parseFloat(v) || 0) - s; return String(Math.max(0, parseFloat(n.toFixed(6)))); })}>&minus;</button>
                    ...
                    <button className="num-step" onClick={() => setEditValue(v => { const s = stepFor(v); const n = (parseFloat(v) || 0) + s; return String(parseFloat(n.toFixed(6))); })}>+</button>
                  </div>
  ...
  /* ═══════ RENDER: OBJECT NODE (recursive; humanizeKey + nested-mini-card; arrays of scalars/objects supported) ═══════ */
  ...
  /* ═══════ RENDER: INFECTIONS — array of {type, status, notes} (recursive entries, read-only) ═══════ */
  const renderInfectionsField = (record, fn, idx, sid) => {
    const val = getFieldValue(record, fn, idx);
    const items = Array.isArray(val) ? val.map((v, i) => [i, v]).filter(([, v]) => !isEmptyDeep(v)) : [];
    if (items.length === 0) return null;
    const label = FIELD_LABELS[fn] || fn;
    const showSubLabel = label.trim().toLowerCase() !== (SECTION_TITLES[sid] || '').trim().toLowerCase();
    if (searchTerm.trim() && !fieldMatches(record, fn, idx) && !sectionTitleMatches(sid)) return null;
    return (
      <div key={fn} className="rec-mini-card">
        {showSubLabel && <div className="nested-subtitle">{highlightText(label)}</div>}
        {items.map(([i, item]) => (
          <div key={i} className="rec-mini-card" style={{ marginTop: 8 }}>
            {isScalar(item)
              ? renderObjectLeaf(record, fn, [i], idx, sid, item)
              : Object.entries(item).filter(([, v]) => !isEmptyDeep(v)).map(([k, v]) => (
                isScalar(v) ? renderObjectLeaf(record, fn, [i, k], idx, sid, v)
                  : <div className="nested-mini-card" key={k}>{renderObjectNode(record, fn, idx, sid, humanizeKey(k), v, [i, k], 1)}</div>
              ))}
          </div>
        ))}
      </div>
  ...
  /* ═══════ RENDER: RECOMMENDATIONS — array of {recommendation, date}, date-grouped, editable ═══════ */
                                ...
                                <div className="edit-field-container">
                                  ...
                                  <div className="edit-actions">
                                    <button className="save-btn" disabled={saving} onClick={e => {
                                      ...
                                      // Stage as DRAFT (no DB write); Approve commits.
                                      const editKey = `${fn}-${idx}`;
                                      setLocalEdits(prev => ({ ...prev, [editKey]: fullText2 }));
                                      setPendingEdits(prev => ({ ...prev, [editKey]: true }));
                                      ...
                                      setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
                                      { const store = readDrafts(); if (!store[id2]) store[id2] = {}; store[id2][fn] = fullText2; writeDrafts(store); }
                                      setEditingField(null); setEditValue(''); setSaveError(null);
                                    }}>{saving ? 'Saving...' : 'Save'}</button>
                                    <button className="cancel-btn" onClick={e => { e.stopPropagation(); setEditingField(null); setEditValue(''); setSaveError(null); }}>Cancel</button>
                                  </div>
                        ...
                        <div className="edit-actions">
                          <button className="save-btn" disabled={saving} onClick={e => {
                            ...
                            if (parsed.isLabeled) {
                              ...
                              setEditedSentences(prev => ({ ...prev, ...marks }));
                              setApprovedSections(prev => { const n = { ...prev }; delete n[`${sid}-${idx}`]; return n; });
                              { const store = readDrafts(); if (!store[id3]) store[id3] = {}; store[id3][fn] = fullText; writeDrafts(store); }
                              setEditingField(null); setEditValue(''); setSaveError(null);
      ...
      <div key={sid} className="section">
        <div className="mini-cards-container">
          ...
          {fields.map(f => {
            ...
            if (INFECTION_ARRAY_FIELDS.includes(f)) return renderInfectionsField(record, f, idx, sid);
  ...
  if (!records || records.length === 0) {
    return (
      <div className="pregnancy-complications-document" ref={containerRef}>
        <div className="document-header"><h2 className="document-title">Pregnancy Complications</h2></div>
        <div className="empty-state">No pregnancy complications records available</div>
      </div>
    );
  }
  ...
  return (
    <div className="pregnancy-complications-document" ref={containerRef}>
      <div className="document-header">
        <h2 className="document-title">Pregnancy Complications</h2>
        <div className="header-actions">
          <button className={`copy-btn ${showCopied ? 'copied' : ''}`} onClick={copyAllText}>{showCopied ? 'Copied!' : 'Copy All'}</button>
          <PDFDownloadLink document={<PregnancyComplicationsDocumentPDFTemplate document={pdfData} />} fileName="Pregnancy_Complications.pdf" className="copy-btn">
            {({ loading }) => loading ? 'Generating...' : 'Export PDF'}
          </PDFDownloadLink>
        </div>
      </div>
      <div className="search-container">
        <input type="text" className="search-input" placeholder="Search pregnancy complications..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredRecords.map((record, idx) => (
          <div key={idx} className="record-card">
            <div className="record-header">
              <h3 className="record-name">{highlightText(record.provider || `Pregnancy Complications ${idx + 1}`)}</h3>
            </div>
...
export default PregnancyComplicationsDocument;
