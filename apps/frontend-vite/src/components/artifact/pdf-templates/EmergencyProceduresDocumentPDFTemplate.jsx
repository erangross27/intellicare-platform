/**
 * EmergencyProceduresDocumentPDFTemplate.jsx
 * June 2026 — Helvetica — LETTER size — emergency procedures
...
/* ======= UTILS ======= */
/* formatDate: returns '' for empty/invalid dates so those fields are skipped gracefully (4-AREA RULE — matches JSX) */
...
/* formatDateTime: datetime fields render date + time; '' for empty/invalid (real data has procedureEndTime: "") */
...
/* renderSentenceField: period-first splitting with semicolon fallback
   — noCommaSplit keeps natural-grammar strings (e.g. anatomicalSite) as one value (4-AREA RULE) */
const renderSentenceField = (label, text, showLabel, noCommaSplit) => {
  ...
  if (sentences.length < 2) {
    /* Single-value: try comma splitting (skipped for noCommaSplit fields) */
    const commaItems = noCommaSplit ? [strVal] : splitByComma(strVal);
  ...
  sentences.forEach(s => {
    ...
    if (parsed.isLabeled) {
      ...
      const commaItems = semiSub.length >= 2 ? semiSub : (noCommaSplit ? [parsed.value] : splitByComma(parsed.value));
      const hasOxfordComma = commaItems.some(ci => ci.trim().toLowerCase().startsWith('and '));
      if (commaItems.length >= 2 && !hasOxfordComma) {
...
/* renderArrayField (array of strings or objects — items render VERBATIM) */
...
/* Enum canonical display — mirror the JSX (stored 'left' → 'Left') for the 4-area rule */
const PDF_ENUM_OPTIONS = {
  laterality: ['Left', 'Right', 'Bilateral', 'Midline', 'N/A'],
  consentType: ['Verbal', 'Written', 'Informed Consent', 'Emergency (Implied)', 'Telephone'],
};
const enumCanonical = (fn, cur) => { const base = PDF_ENUM_OPTIONS[fn] || []; const hit = base.find(o => o.toLowerCase() === String(cur ?? '').toLowerCase()); return hit || cur; };
...
const SECTION_CONFIGS = [
  {
    ...
    fields: [
      ...
      { key: 'anatomicalSite', label: 'Anatomical Site', isSentence: true, noCommaSplit: true },
      { key: 'laterality', label: 'Laterality', isEnum: true },
    ],
  },
  {
    title: 'Team',
    fields: [
      { key: 'performingPhysician', label: 'Performing Physician' },
      { key: 'assistingStaff', label: 'Assisting Staff', isArray: true },
    ],
  },
  {
    title: 'Timing',
    fields: [
      { key: 'procedureStartTime', label: 'Procedure Start Time', isDateTime: true },
      { key: 'procedureEndTime', label: 'Procedure End Time', isDateTime: true },
    ],
  },
  {
    title: 'Technique & Equipment',
    fields: [
      { key: 'techniqueUsed', label: 'Technique Used', isSentence: true },
      { key: 'equipmentUsed', label: 'Equipment Used', isArray: true },
      { key: 'guidanceMethod', label: 'Guidance Method' },
      { key: 'numberOfAttempts', label: 'Number of Attempts', hideZero: true },
    ],
  },
  {
    title: 'Medications & Sedation',
    fields: [
      { key: 'sedationType', label: 'Sedation Type' },
      { key: 'medicationsAdministered', label: 'Medications Administered', isArray: true },
      { key: 'consentType', label: 'Consent Type', isEnum: true },
    ],
  },
  {
    title: 'Outcome',
    fields: [
      { key: 'procedureSuccess', label: 'Procedure Success' },
      { key: 'immediateOutcome', label: 'Immediate Outcome', isSentence: true },
      { key: 'complications', label: 'Complications', isArray: true },
      { key: 'estimatedBloodLoss', label: 'Estimated Blood Loss' },
      { key: 'specimensObtained', label: 'Specimens Obtained', isArray: true },
      { key: 'vitalSignsDuringProcedure', label: 'Vital Signs During Procedure', isArray: true },
    ],
  },
  {
    title: 'Disposition & Follow-up',
    fields: [
      { key: 'dispositionPostProcedure', label: 'Disposition Post-Procedure', isSentence: true },
      { key: 'postProcedureInstructions', label: 'Post-Procedure Instructions', isSentence: true },
      { key: 'followUpRequired', label: 'Follow-up Required' },
    ],
  },
];

/* fieldHasVal: mirrors JSX hasFieldVal — hideZero + date/datetime validity ('' from formatters = skip; procedureEndTime "" hidden) */
const fieldHasVal = (record, f) => {
  const v = record[f.key];
  if (f.hideZero && v === 0) return false;
  if (f.isDateTime) return formatDateTime(v) !== '';
  if (f.isDate) return formatDate(v) !== '';
  if (f.isArray) {
...
/* ======= COMPONENT ======= */
const EmergencyProceduresDocumentPDFTemplate = ({ document: data }) => {
      ...
      <Document>
        <Page size="LETTER" style={styles.page}>
          <View style={styles.documentHeader}>
            <Text style={styles.documentTitle}>Emergency Procedures</Text>
          </View>
        ...
        {records.map((record, index) => {
          ...
          return (
            <View key={index} style={styles.recordContainer}>
              ...
              {/* Record Header — title only (date renders in its section; no meta pills) */}
              <View style={styles.recordHeader} wrap={false}>
                <Text style={styles.recordTitle}>
                  {`Emergency Procedure ${recordNum}`}
                </Text>
              </View>
              ...
              {SECTION_CONFIGS.map((sectionConfig, sIdx) => {
                ...
                const renderField = (field) => {
                  ...
                  if (field.isSentence) return renderSentenceField(field.label, val, showLabel, field.noCommaSplit);
                  ...
                  if (field.isEnum) return renderFieldRow(field.label, enumCanonical(field.key, val), showLabel);
...
export default EmergencyProceduresDocumentPDFTemplate;
